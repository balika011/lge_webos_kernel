...
#define EMMC_PARTINFO_ADDR 			(0x140000)		// 1Mbyte = 800 sector (1sector = 512byte)
...
#ifdef CONFIG_LGEMMC_PARTITION_LA
unsigned int lgemmc_get_part_offsest_and_size(int partnum, unsigned long long *poffset, unsigned long long *psize)
#else
unsigned int lgemmc_get_part_offset_and_size(int partnum, unsigned int *poffset, unsigned int *psize)
#endif
{
	struct partition_info *info;
