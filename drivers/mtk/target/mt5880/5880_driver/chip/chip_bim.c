...
INT32 CmdEfuseSecureID(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    BIM_GetSecureID((BYTE*)au4SID, sizeof(au4SID));
...
INT32 CmdEfuseSecureData(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    u4SDa7 = BIM_READ32(REG_EFUSE_SDATA_1);
    u4SDa9 = BIM_READ32(REG_EFUSE_SDATA_3);
    u4SDa6 = BIM_READ32(REG_EFUSE_SDATA_0);
    u4SDa8 = BIM_READ32(REG_EFUSE_SDATA_2);
...
INT32 CmdEfuseDump(INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    Printf("IS_SupportVP8():%d\n", IS_SupportVP8());
    Printf("IS_SupportCPUSpeed():%d\n", IS_SupportCPUSpeed());
...
INT32 BIM_GetDeviceID(BYTE *pu1DeviceID, BYTE u1Size)
{
    if (u1Size < DEVICE_ID_LEN)
    {
        return -1;
    }
    
    x_memcpy(pu1DeviceID, (void *)(BIM_BASE + REG_EFUSE_DATA_0), DEVICE_ID_LEN);
    return 0;
}

INT32 BIM_GetDeviceData(BYTE *pu1DeviceID, BYTE u1Size)
{
    if (u1Size < DEVICE_DATA_LEN)
    {
        return -1;
    }
    ...
    if (TZ_IsTrustzoneEnable())
    {
        TZ_BIM_GetRomBytes(0, pu1DeviceID, DEVICE_DATA_LEN);
    }
    else
#endif
    {
        x_memcpy(pu1DeviceID, (void *)0xf4000000, DEVICE_DATA_LEN);
    }
    return 0;
}

INT32 BIM_GetDmKey(BYTE *pu1Buf, BYTE u1Size)
{
    if (u1Size < 16)
    {
        return -1;
    ...
    if (TZ_IsTrustzoneEnable())
    {
        TZ_BIM_GetRomBytes(0x2000, pu1Buf, 16);
    }
    else
#endif
    {
        x_memcpy(pu1Buf, (void *)0xf4002000, 16);
    }
    return 0;
}

INT32 BIM_GetULPK(BYTE *pu1Buf, BYTE u1Size)
{
    if (u1Size < 32)
    {
        return -1;
    ...
    if (TZ_IsTrustzoneEnable())
    {
        TZ_BIM_GetRomBytes(0x1000, pu1Buf, 32);
    }
    else
#endif
    {
        x_memcpy(pu1Buf, (void *)0xf4001000, 32);
    }

    return 0;
}

INT32 BIM_GetRomKey(BYTE *pu1Buf, UINT32 u4Offset, BYTE u1Size)
{
    if (u4Offset > (0x4000-32))
    {
        return -1;
    }
    ...
    if (TZ_IsTrustzoneEnable())
    {
        TZ_BIM_GetRomBytes(u4Offset, pu1Buf, 32);
    }
    else
#endif
    {
        x_memcpy(pu1Buf, (void *)(0xf4000000+u4Offset), 32);
    }
...
INT32 RSADecryption(UINT32 *pu1Signature, UINT32 *pu4PublicKey, UINT32 *pu4CheckSum)
{
    ...
    // 0x00001844 A modMultTemp
    PFN_MODMULTTEMP pfn_modMultTemp;

    if(IS_IC_5880())
    {
        if (BSP_GetIcVersion() == 0x58800000)
        {
            pfn_modMultTemp = (PFN_MODMULTTEMP)0xf400160c;
        }
        else
        {
            // fix me when at-speed ROM code done
            pfn_modMultTemp = (PFN_MODMULTTEMP)0xf4001828;
        }
    }
    else
    {
        if (BSP_GetIcVersion() == 0x58600000)
        {
            pfn_modMultTemp = (PFN_MODMULTTEMP)0xf4001848;
        }
        else
        {
            // fix me when at-speed ROM code done
            pfn_modMultTemp = (PFN_MODMULTTEMP)0xf400180c;
        }
    }
#endif
...
BOOL BIM_VerifySignature(UINT32* pu4Signature, UINT32* pu4CheckSum)
{
    ...
    x_memcpy((void*)au4PublicKey, (void *)BIM_MTK_KEY_ADDR, 256);

#ifdef CC_MTK_PRELOADER
    RSADecryption(pu4Signature+64, au4PublicKey, au4VendorKey);
    ...
    RSADecryption(pu4Signature, au4VendorKey, au4CalulatedCheckSum);
