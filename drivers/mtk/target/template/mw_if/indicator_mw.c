...
// tentatively, change to PWR_GPIO0 soon
#define GPIO_DONE                       PWR_GPIO0
    ...
    case IND_GET_TYPE_STATE:
        ...
        if (pt_comp_id->ui2_id == IND_POWER_ID) {
            ...
            *((IND_STATE_T *)pv_get_info) =
                (GPIO_Output(GPIO_DONE, NULL)) ? IND_STATE_ENABLE : IND_STATE_DISABLE;
            i4_ret = RMR_OK;
        }
        break;
    ...
    case IND_SET_TYPE_STATE:
        ...
        if (pt_comp_id->ui2_id == IND_POWER_ID) {
            ...
            case IND_STATE_DISABLE:
                ...
                i4_state = 1;
                GPIO_Output(GPIO_DONE+1, &i4_state);
...
static INT32 _MW_IndctrDevRegister(CHAR *szIndName, DRV_TYPE_T drvType, UINT16 ui2Id)
{
    ...
    if (ui2Id == IND_POWER_ID) {
        LINT_SUPPRESS_NEXT_EXPRESSION(534);
        BSP_PinGpioSel(GPIO_DONE);
#ifdef CC_SBOX_HWCTRL
        BSP_PinGpioSel(GPIO_DONE+1);
#endif /* CC_SBOX_HWCTRL */
    }
	...
	{
        LINT_SUPPRESS_NEXT_EXPRESSION(534);
	    x_sprintf(ps_name, "%s%d", szIndName, ui2Id);
	}
    ...
    t_reg_comp.u.t_ind.pv_tag       = (void *)drvType;
