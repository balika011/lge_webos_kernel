...
void _DDR_SetDdr3BeforeAI(UINT32 u4Clk, UINT32 fgFull, UINT32 fg64En,
    UINT32 fgDQS, UINT32 fgGDDR, UINT32 u4ChipNum)
{
    ...
    //for (i = 0x98; i <= 0xA4; i += 4)
    //{
    //    DRAM_WRITE32(i, DDR3_OUT_CLK_DELAY_CHAIN_0);
    //}
	...
	{
		// before ECO code.
		u4Val = (DDR_III << 30) | 0x0020F600; // B ver
	}
	else
	{
		// after ECO code.
		u4Val = (DDR_III << 30) | 0x0020FD00; // C ver
	}
#else
	u4Val = (DDR_III << 30) | 0x0020F600;
#endif
...
#ifndef CC_UBOOT
    CHIP_DisplayString("GDDR3 x2\n");
#endif
#else
#ifndef CC_UBOOT
    CHIP_DisplayString("GDDR3 x1\n");
#endif
#endif
...
void _DDR_SetDdr3AfterAI(UINT32 u4Clk, UINT32 fgFull, UINT32 fg64En,
    UINT32 fgDQS, UINT32 fgGDDR, UINT32 u4ChipNum)
{
#if CC_USE_ANALOG_DLL
    UINT32 i;

    DRAM_WRITE32(0xDC, 0x43000000);
    DRAM_WRITE32(0xE0, 0x72000000);

    DRAM_WRITE32(0x08, 0x70082220);
    while(((DRAM_READ32(0x3FC) >> 8) & 0x7) != 0x7);
    WAIT_DRAM_STABLE();
...
#else //CC_SUPPORT_DDR3

void _DDR_SetDdr3BeforeAI(UINT32 u4Clk, UINT32 fgFull, UINT32 fg64En,
    UINT32 fgDQS, UINT32 fgGDDR, UINT32 u4ChipNum)
{
    LOG(1, "Compiled with DDR3 not support!");
    ASSERT(0);
}
