...
;;  Use r2 from outside, it should be from CHIP_RS232Init.
;    MOV     r2, #0x60000000
;    LDR     r2, =0x20018000             ; Use DMX SRAM as stack.
;    MOV     r2, #0x2B000000             ; Use LZHS SRAM as stack.

;    MRC     p15, 0, r0, c9, c1, 1       ; Read CP15 ITCM control register
;    BIC     r1, r0, #CR_TCM_MASK
;    AND     r0, r0, #CR_TCM_ENABLE
;    CMP     r0, #0
;    BNE     _SkipITCMSetup              ; if ITCM is enable, let R1 is the ITCM address.
    MOV     r1, r2                      ; move R2 to R1, the new ITCM address.
;    ORR     r0, r2, #CR_TCM_ENABLE:OR:CR_TCM_4KB
;    MCR     p15, 0, r0, c9, c1, 1       ; Write back ITCM control register

    [ :LNOT:CC_5391_PRELOADER

    ; Zero all ITCM
    ADD     r2, r2, #0x20               ; r2 = 0x60000020
    ADD     r3, r2, #0x0f00
    ADD     r3, r2, #0x00dc             ; r3 is end of ITCM
    MOV     r4, #0
    MOV     r5, #0
    MOV     r6, #0
    MOV     r7, #0
...
_SkipITCMSetup
    ...
    BIC     r0, r0, #MODE_MASK
    ORR     r0, r0, #MODE_SVC
    ADD     r3, r1, #0x1000             ; SVC stack is TCM+4KB.
    MOV     sp, r3
    BIC     r0, r0, #MODE_MASK
    ORR     r0, r0, #MODE_FIQ
    MSR     CPSR_cxsf, r0
    ADD     r3, r1, #0x400              ; SVC stack is TCM+1KB.
    MOV     sp, r3
    BIC     r0, r0, #MODE_MASK
    ORR     r0, r0, #MODE_IRQ
    MSR     CPSR_cxsf, r0
    ...
    ORR     r0, r0, #MODE_ABT
    MSR     CPSR_cxsf, r0
    ADD     r3, r1, #0x800              ; SVC stack is TCM+2KB.
    MOV     sp, r3
    BIC     r0, r0, #MODE_MASK
    ORR     r0, r0, #MODE_UDF
    MSR     CPSR_cxsf, r0
