...
{
    return (PDWNC_READ32(REG_RW_WAKE_RSTCNT) == DEFAULT_RESET_COUNTER);
}
...
void PDWNC_SetupPowerDown(void)
{
    ...
    //PDWNC_WRITE32(REG_RW_WAKE_RSTCNT, DEFAULT_RESET_COUNTER);
    ...
    // Setup CPU clock from CPUPLL to crystal
    CKGEN_WRITE32(REG_CPU_CKCFG, 0x00010000);

    /* wait more than 1.5 IR clock cycle for IR busy bit clear */
    for (u4Val=0; u4Val<512; u4Val++) { }
    /* Do BIM clear IR busy bit. */
    PDWNC_WRITE32(IRRX_CLEAR_BUSY_REG, IRRX_CLR_BSY_IR);

    // Enter Power Down mode.
    _PDWNC_SetT8032(PDWNC_MSK_T8032_STANDBY);

    // In case wakeup PC is not reset, store pdwake status and reboot.
    u4Val = PDWNC_READ32(REG_RO_PDWAKE_STAT);
    // TCM_WAKEUP_STATUS = u4Val;

    // Wakeup to reset to fix bug.
    PDWNC_Reboot();
    while (1) { }
}
