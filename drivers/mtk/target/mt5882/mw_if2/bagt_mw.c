...
#include "mtdmx.h"
#define DMX_VUDU_KEY_T              MTDMX_VUDU_KEY_T
#define DMX_DESC_MODE_AES_ECB       MTDMX_DESC_MODE_AES_ECB             // MTDMX_DESC_MODE_AES_ECB must equal to DMX_DESC_MODE_AES_ECB
#define DMX_DESC_MODE_AES_CBC       MTDMX_DESC_MODE_AES_CBC
#define DMX_DESC_RTB_MODE_T         MTDMX_DESC_RTB_MODE_T
#endif
#else
#include "feeder_if.h"
...
//#include "x_debug.h"
#define DEFINE_IS_LOG	BAGT_IsLog
...
typedef struct _BUF_AGENT_INT_DATA_T
{
    ...
    BUF_AGT_AES_CTR_ENCRYPT t_aes_crt_crypt;
    BUF_AGT_CTRL_MTK_ENCRYPT_INFO_T rMtkEncryptInfo;
    BUF_AGT_CTRL_IBC_DASH_SET_INFO_T rIBC_Dash_Info;
    BUF_AGT_CTRL_HDD_DTCP_IP_INFO_T  rIBC_Hdd_Dtcp_Ip_Info;
...
static MTFEEDER_REQ_DATA_T rLastDataBagt[BAGT_COMP_NS];  // megaa 20110824
static UINT32 u4IdCnt[BAGT_COMP_NS] = {0,0,0,0,0};
static MTFEEDER_NOTIFY_COND eLastCond[BAGT_COMP_NS] = 
{
    FEEDER_NOTIFY_DATA_REQ_DONE,
    FEEDER_NOTIFY_DATA_REQ_DONE,
    FEEDER_NOTIFY_DATA_REQ_DONE,
    FEEDER_NOTIFY_DATA_REQ_DONE,
    FEEDER_NOTIFY_DATA_REQ_DONE
...
static VOID _buf_agt_handle_encrypt(BAGT_COMP_ID_T ui2_buf_agt_id, BUF_AGT_CTRL_IBC_ENCRYPT_INFO_T *prParam)
{
    ...
    prDmxVuduKey->eRtbMode = (DMX_DESC_RTB_MODE_T)prAesInfo->e_rtb_mode;
#else
...
static BOOL _buf_agt_req_ack_fct(
    ...
    BUF_AGT_DATA_BLK_T*         pt_data_blk)
{
    MTFEEDER_REQ_DATA_T       rData;
#if 0  // megaa 20110824: use each instance's own instead!!!
    static MTFEEDER_REQ_DATA_T rLastData;
#endif
    MTFEEDER_REQ_DATA_T      rMsgQ;
    BUF_AGT_MW_NFY_COND_T       e_mw_ack_cond;
    SIZE_T                  zMsgSize;
    INT32                   i4Ret;
    UINT16                  u2MsgQIdx;
    BOOL                        b_ret = TRUE;
    MTFEEDER_NOTIFY_COND eCondition = FEEDER_NOTIFY_DATA_REQ_DONE;
    ...
    x_memset((void *)&rData,0,sizeof(MTFEEDER_REQ_DATA_T));
    ...
    else
    {
        if (gat_bagt_data[ui2_buf_agt_id].eFlowCtrlState == BAGT_FLWCTL_PRE_LOAD_STARTED)
        {
            ...
            MTFeederSetPreLoadDone(gat_bagt_data[ui2_buf_agt_id].eDataType, pt_data_blk->z_size);
        ...
        zMsgSize= sizeof(MTFEEDER_REQ_DATA_T);
        ...
        rData.ui8_vid_pts   = pt_data_blk->ui8_vid_pts;
        rData.ui8_push_time = pt_data_blk->ui8_push_time;
		#ifdef TIME_MEASUREMENT
        rData.eFeederPrintTMS = (MTFeederPrintTMS)pt_data_blk->e_print_tms;
		if (rData.eFeederPrintTMS == FEEDER_PRINT_TMS_PRINT)
		{
			TMS_DIFF_EX(TMS_FLAG_WFD_LATENCY,"WFD_DATA:","_buf_agt_req_ack_fct");
		}
		#endif
        if (gat_bagt_data[ui2_buf_agt_id].fgPushMode)
        {
        	//in push mode, pt_data_blk->pui1_buff is only a offset
            //rData.u4WriteAddr = gat_bagt_data[ui2_buf_agt_id].u4BufAddr + (UINT32)pt_data_blk->pui1_buff;
            //modify a offset to a address
        	rData.u4WriteAddr = (UINT32)pt_data_blk->pui1_buff;
        }
        else
        {
        	//in non-push mode, pt_data_blk->pui1_buff is a address       
            rData.u4WriteAddr = (UINT32)pt_data_blk->pui1_buff;
        }
        rData.u4AlignOffset = rMsgQ.u4AlignOffset;
		rData.eFeederReqType = rMsgQ.eFeederReqType;
		...
		{
			rData.u8FilePos = rMsgQ.u8FilePos;
		}
        ...
        if (pt_data_blk->e_data_fmt == DATA_FMT_IBC)
        {
            ...
            case BUF_AGT_CTRL_IBC_TYPE_ENCRYPT:
                if (pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_encrypt_info.e_mm_crypt_type == BUF_AGT_MM_CRYPT_TYPE_PDCF)
                {
                    rData.eFeederIBC = FEEDER_IBC_PDCF;

                    x_memcpy((VOID *)&gat_bagt_data[ui2_buf_agt_id].t_pdcf_crypt, (VOID *)&pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_encrypt_info.u_encrypt_info.t_pdcf_crypt, sizeof(BUF_AGT_MM_PDCF_CRYPT_INFO_T));
                    rData.pt_pdcf_crypt = &gat_bagt_data[ui2_buf_agt_id].t_pdcf_crypt;
                }

                else if (pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_encrypt_info.e_mm_crypt_type == BUF_AGT_MM_CRYPT_TYPE_CENC)
                {
                    rData.eFeederIBC = FEEDER_IBC_CENC;

                    x_memcpy((VOID *)&gat_bagt_data[ui2_buf_agt_id].t_aes_crt_crypt, (VOID *)&pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_encrypt_info.u_encrypt_info.t_aes_ctr_crypt, sizeof(BUF_AGT_AES_CTR_ENCRYPT));
                    rData.pt_aes_crt_crypt = &gat_bagt_data[ui2_buf_agt_id].t_aes_crt_crypt;
                }
                else
                {
		  ...
		  case BUF_AGT_CTRL_IBC_TYPE_SEQHEADER:
		  {
			  rData.eFeederIBC = FEEDER_IBC_SEQHEADER;
			  
			  if (pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.e_strm_type == BUF_AGT_CTRL_STREAM_TYPE_VIDEO)
			  {
				  rData.eFeederStrmType = BUF_AGT_CTRL_STREAM_TYPE_VIDEO;
				  rData.u.t_vid_seqheader_info.b_is_raw = 
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_vid_seqheader_info.b_is_raw;
				  rData.u.t_vid_seqheader_info.b_is_second_raw =
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_vid_seqheader_info.b_is_second_raw;
				  rData.u.t_vid_seqheader_info.u.t_seq_raw.z_buff_size = 
					  pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_vid_seqheader_info.u.t_seq_raw.z_buff_size;
                  rData.u.t_vid_seqheader_info.u.t_seq_raw.e_frame_rate = 
                      pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_vid_seqheader_info.u.t_seq_raw.e_frame_rate;
				  x_memcpy(gat_bagt_data[ui2_buf_agt_id].pc_buf, 
						  pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_vid_seqheader_info.u.t_seq_raw.pc_buff, 
						  pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_vid_seqheader_info.u.t_seq_raw.z_buff_size);
				  rData.u.t_vid_seqheader_info.u.t_seq_raw.pc_buff = gat_bagt_data[ui2_buf_agt_id].pc_buf;
			  }
			  else if (pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.e_strm_type == BUF_AGT_CTRL_STREAM_TYPE_AUDIO)
			  {
				  rData.eFeederStrmType = BUF_AGT_CTRL_STREAM_TYPE_AUDIO;
				  rData.u.t_aud_seqheader_info.b_frame_length =
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_aud_seqheader_info.b_frame_length;
				  rData.u.t_aud_seqheader_info.b_is_audio_change =
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_aud_seqheader_info.b_is_audio_change;
				  rData.u.t_aud_seqheader_info.i2_channle =
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_aud_seqheader_info.i2_channle;
				  rData.u.t_aud_seqheader_info.ui4_profile =
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_aud_seqheader_info.ui4_profile;
				  rData.u.t_aud_seqheader_info.ui4_sample_rate = 
					   pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_seqheader_info_param.u.t_aud_seqheader_info.ui4_sample_rate;
				  LOG(6,"##bagt,the audio sample rate is %d\n",
					   (int)rData.u.t_aud_seqheader_info.ui4_sample_rate);
			  }
			  break;
		  }

            case BUF_AGT_CTRL_IBC_TYPE_STRM_INFO:
            {
                rData.eFeederIBC = FEEDER_IBC_PARAM_STRM_INFO;
                rData.u4WriteAddr = (UINT32)&pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_strm_info_param;
                rData.u4ReadSize = sizeof(MTBUF_AGT_CTRL_IBC_PARAM_STRM_INFO_T);
                LOG(6,"FEEDER_IBC_PARAM_STRM_INFO Send...\n");
                LOG(6,"ui4_aud_strm_id %d ui4_vid_strm_id %d e_vid_enc %d e_aud_enc %d \n",
                      pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_strm_info_param.ui4_aud_strm_id,
                      pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_strm_info_param.ui4_vid_strm_id,
                      pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_strm_info_param.e_vid_enc,
                      pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_strm_info_param.e_aud_enc);
                break;
            }
            case BUF_AGT_CTRL_IBC_TYPE_DASH_SET_INFO:
				rData.eFeederIBC = FEEDER_IBC_DASH_SET_INFO;
				x_memcpy((VOID *)&gat_bagt_data[ui2_buf_agt_id].rIBC_Dash_Info,
					(VOID*)&pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_dash_info,
					sizeof(BUF_AGT_CTRL_IBC_DASH_SET_INFO_T));
				rData.pt_dash_set_info = &gat_bagt_data[ui2_buf_agt_id].rIBC_Dash_Info;
				break;
            case BUF_AGT_CTRL_IBC_TYPE_HDD_CONTENT_KEY:
            case BUF_AGT_CTRL_IBC_TYPE_DTCP_IP_KEY:
                if(pt_data_blk->t_extra.t_ctrl_ibc.e_cmd_type == BUF_AGT_CTRL_IBC_TYPE_HDD_CONTENT_KEY)
                {
                    rData.eFeederIBC = FEEDER_IBC_HDD_CONTENT_KEY;
                }
                else
                {
                    rData.eFeederIBC = FEEDER_IBC_DTCP_IP_KEY;
                }
                x_memcpy((VOID *)&gat_bagt_data[ui2_buf_agt_id].rIBC_Hdd_Dtcp_Ip_Info,
					(VOID*)&pt_data_blk->t_extra.t_ctrl_ibc.t_param.t_set_hdd_dtcp_ip_info,
					sizeof(BUF_AGT_CTRL_HDD_DTCP_IP_INFO_T));
				rData.pt_hdd_dtcp_ip_info = &gat_bagt_data[ui2_buf_agt_id].rIBC_Hdd_Dtcp_Ip_Info;
                break;
            case BUF_AGT_CTRL_IBC_TYPE_PUSH_CHANGE_AUDIO:
				rData.eFeederIBC = FEEDER_IBC_PUSH_CHANGE_AUDIO;
				break;
            default:
                break;
            }
        ...
        if (rMsgQ.fgPartial)
        {
            ...
            x_memcpy((VOID *)&rLastDataBagt[ui2_buf_agt_id], (VOID *)&rData, sizeof(MTFEEDER_REQ_DATA_T));  // megaa 20110824
			if(e_ack_cond == BUF_AGT_DATA_REQ_ACK_COND_CANCELED
				|| e_ack_cond == BUF_AGT_DATA_REQ_ACK_COND_ERROR)
			{
				eLastCond[ui2_buf_agt_id] = FEEDER_NOTIFY_ERROR;
			}
            b_ret = TRUE;
        }
        else
        {
            if (fgUseLastData[ui2_buf_agt_id])
            {
                ...
                x_memcpy((VOID *)&rData, (VOID *)&rLastDataBagt[ui2_buf_agt_id], sizeof(MTFEEDER_REQ_DATA_T));  // megaa 20110824
                rData.u4ReadSize = rLastDataBagt[ui2_buf_agt_id].u4ReadSize + u4CurSize;  // megaa 20110824
                rData.fgEof=fgEof;
				if(eLastCond[ui2_buf_agt_id] == FEEDER_NOTIFY_ERROR)
				{
					eCondition = FEEDER_NOTIFY_ERROR;
				}
				eLastCond[ui2_buf_agt_id] = FEEDER_NOTIFY_DATA_REQ_DONE;
				if(rData.eFeederReqType == FEEDER_TIME_SEEK)
				{
					rData.u8FilePos = rLastDataBagt[ui2_buf_agt_id].u8FilePos;
				}
            }
            rData.u4Id = rMsgQ.u4Id;

            if (gat_bagt_data[ui2_buf_agt_id].fgPushMode 
				&& BUF_AGT_DATA_REQ_ACK_COND_CANCELED != e_ack_cond)
            {
#if 1
			...
			if(!gat_bagt_data[ui2_buf_agt_id].fgPushMode
				&& e_ack_cond == BUF_AGT_DATA_REQ_ACK_COND_CANCELED)
			{//pull mode cancel ---->error for dlna connection-stalling
				e_ack_cond = BUF_AGT_DATA_REQ_ACK_COND_ERROR;
				eCondition = FEEDER_NOTIFY_ERROR;
            }
			if(BUF_AGT_DATA_REQ_ACK_COND_CANCELED != e_ack_cond)
			{
            	b_ret = MTFeederNotifyHandle(gat_bagt_data[ui2_buf_agt_id].eDataType, eCondition, (UINT32)&rData);
			}
        }
#ifdef BAGT_URI_MODE_1
    ...
    BUF_AGT_BUFFER_FULLNESS_T*  pt_fullness)
{
    ...
    b_ret = MTFeederGetBufferFullness(gat_bagt_data[ui2_buf_agt_id].eDataType,
...
{
    INT32             b_ret;
    MTFEEDER_BUF_INFO_T rFeederBufInfo;
    UINT32            u4Size;
#ifdef LINUX_TURNKEY_SOLUTION
    UINT32            u4Addr;
#endif

    //printf("eeeeeeeeeeeee,_buf_agt_get_shared_memory\n");
    ...
    x_memset(&rFeederBufInfo, 0, sizeof(rFeederBufInfo));
    b_ret = MTFeederGetBufferInfo(gat_bagt_data[ui2_buf_agt_id].eDataType,
                                    &rFeederBufInfo);
    if (FEEDER_E_OK != b_ret)
    {
        return BAGTR_FAILED;
    ...
    //for linux, pv_buffer is user VA for mw, pv_private is kernel PA for mw.
    pt_shared_memory->pv_buffer     = (VOID *)(MTFeederGetUserStartVAddr(rFeederBufInfo.FeederDeviceId)+rFeederBufInfo.u4PhysicalAddrOffset);
    pt_shared_memory->pv_private    = (VOID *)rFeederBufInfo.u4PhysicalAddr+rFeederBufInfo.u4PhysicalAddrOffset;    
#endif
...
UINT32 _buf_agt_QueryFeeder(UINT8 u1AttachedSrcId,
    MTFeederSourceType eSrcType,
    MTFEEDER_BUF_INFO_T *pQryBufInfo,
    MTFEEDER_REQ_DATA_T* pt_ReqData)
#else
static UINT32 _buf_agt_QueryFeeder(MTFeederSourceType eSrcType, MTFEEDER_BUF_INFO_T *pQryBufInfo,
                                          MTFEEDER_REQ_DATA_T* pt_ReqData)
...
#endif
{
	...
	if(pt_ReqData->eFeederReqType == FEEDER_TIME_SEEK)
	{
		rDataBlock.t_req_type = BUF_AGT_TIME_SEEK;
	}
	else
	{
		rDataBlock.t_req_type = BUF_AGT_OFFSET_SEEK;
	}
    ...
    i4Ret = x_msg_q_send(_hBagtMsgQ[u2_id], pt_ReqData, sizeof(MTFEEDER_REQ_DATA_T), 255);
...
static VOID _buf_agt_PreLoad(BAGT_COMP_ID_T ui2_id)
{
    ...
    u4Addr = u4Size = 0;
    MTFeederInitPreLoad(gat_bagt_data[ui2_id].eDataType, &u4Addr, &u4Size);
...
static VOID _buf_agt_Push_Clean_Msg(BAGT_COMP_ID_T ui2_id)
{
    ...
    if(ui2_id >= BAGT_COMP_NS || _hBagtOldDataBlkQ[ui2_id] == 0) return;
#if 0
...
static UINT32 _buf_agt_flush(MTFeederSourceType eSrcType)
{
    LOG(4,"_buf_agt_flush\n");
...
// megaa 20100414
UINT32 _buf_agt_Push_Consume(MTFeederSourceType eSrcType, MTFEEDER_BUF_INFO_T *pQryBufInfo, UINT32 u4NewRPtr)
{
    BAGT_COMP_ID_T ui2_id = ui2_bagt_id[eSrcType];
#if 1
    INT32 i4Ret;
...
#ifdef BAGT_URI_MODE_1
UINT32 _buf_agt_Consume(FeederSourceType eSrcType, FEEDER_BUF_INFO_T *pQryBufInfo, UINT32 u4NewRPtr)
{
...
#ifdef BAGT_URI_MODE_2
// megaa 20100414
static VOID _buf_agt_Overflow(MTFeederSourceType eSrcType)
{
    BAGT_COMP_ID_T ui2_id = ui2_bagt_id[eSrcType];
...
// megaa 20100414
static VOID _buf_agt_Underflow(MTFeederSourceType eSrcType)
{
    BAGT_COMP_ID_T ui2_id = ui2_bagt_id[eSrcType];
...
#ifdef FEEDER_MULTI_INSTANCE
INT64 _buf_agt_Seek(UINT8 u1AttachedSrcId, MTFeederSourceType eSrcType, MTMM_FILE_OPS_T *prFileInfo)
#else
static INT64 _buf_agt_Seek(MTFeederSourceType eSrcType, MTMM_FILE_OPS_T *prFileInfo)
#endif
{
    BAGT_COMP_ID_T u2_id;
    u2_id = ui2_bagt_id[eSrcType];
    ...
    BUF_AGT_SOURCE_TYPE_T         type)
{
    ...
    {
        MTFeederSetSkypeSourceFlag(FALSE);
    }
    else
    {    //skype bufffer layout, no double buffer
        MTFeederSetSkypeSourceFlag(TRUE);
    }
...
#ifdef FEEDER_MULTI_INSTANCE
    MTFeederSetBagtInfo(gat_bagt_data[ui2_buf_agt_id].eDataType, pt_align_info->ui4_align, pt_align_info->ui4_min_blk_size);
#else
    MTFeederSetBagtInfo(pt_align_info->ui4_align, pt_align_info->ui4_min_blk_size);
#endif
    ...
    BUF_AGT_FLOW_CTRL_T*        pt_flow_ctrl)
{
    ...
    if (FEEDER_E_OK != MTFeederSetUriMode(gat_bagt_data[ui2_buf_agt_id].eDataType, pt_flow_ctrl->ui4_buffer_len, pt_flow_ctrl->ui4_keep_buffer_threshold, pt_flow_ctrl->ui4_re_buferf_threshold))
    {
        return BAGTR_FAILED;
    }
...
static BOOL _buf_agt_drv_init(VOID)
{
    ...
    for (i4_idx = 0; i4_idx < BAGT_COMP_NS; i4_idx++)
    {
        ...
        gat_bagt_data[i4_idx].t_align_info.ui4_align         = 0/* no longer used!! BLOCK_DEF_ALIGNMENT*/;
        ...
        gat_bagt_data[i4_idx].ui4_swdmx_id           = 0;
...
#ifdef FEEDER_MULTI_INSTANCE
        MTFeederSetBagtInfo(gat_bagt_data[i4_idx].eDataType, gat_bagt_data[i4_idx].t_align_info.ui4_align, gat_bagt_data[i4_idx].t_align_info.ui4_min_blk_size);
#endif
    }
#ifndef FEEDER_MULTI_INSTANCE
    MTFeederSetBagtInfo(0/* no longer used!! BLOCK_DEF_ALIGNMENT*/, gat_bagt_data[0].t_align_info.ui4_min_blk_size);
#endif
    ...
    SIZE_T*         pz_size)
{
    ...
    switch (e_get_type)
    {
        ...
        case BUF_AGT_GET_TYPE_TIME_TO_SIZE:
            ...
            {
                MTSWDMX_GetInfo(gat_bagt_data[(BAGT_COMP_ID_T)pt_comp_id->ui2_id].ui4_swdmx_id,
                MTSWDMX_GET_TIME_TO_BYTES_INFO,&(((BUF_AGT_BUFFER_TIME_TO_SIZE_T*)(pv_data))->ui4_ms),&(((BUF_AGT_BUFFER_TIME_TO_SIZE_T*)(pv_data))->ui4_size),0);  // megaa 20110401
            }
            break;
#endif
#endif
    ...
    SIZE_T          z_size)
{
    ...
    switch (e_set_type)
    {
        ...
        case BUF_AGT_SET_TYPE_ACK:
        {
            ...
            prAck = (BUF_AGT_ACK_NFY_T *)pv_data;
            LOG(9,"ack id = %d, wait id = %d,queue num = %d\n",prAck->t_block.ui4_req_id,pMwAckList->u4WaitAckId,pMwAckList->u4AckNum);
        ...
        case BUF_AGT_SET_OPERATION_MODE:
            {
                ...
                swdmxid = gat_bagt_data[(BAGT_COMP_ID_T)pt_comp_id->ui2_id].ui4_swdmx_id;
                LOG(0,"set BUF_AGT_SET_OPERATION_MODE,swdmxid = %d\n",swdmxid);
                ...
                {
                    MTSWDMX_SetInfo(swdmxid,MTSWDMX_SET_PLAYBACK_THRESHOLD,MTSWDMX_BUF_MODE_TIME,t_mode_ctrl.t_feeder_buf_info.u.ui4_time_ms,0);
                }
                else if(t_mode_ctrl.t_feeder_buf_info.e_mode == BUF_AGT_FEEDER_BUF_MODE_BYTE)
                {
                    MTSWDMX_SetInfo(swdmxid,MTSWDMX_SET_PLAYBACK_THRESHOLD,MTSWDMX_BUF_MODE_BYTE,0,0);
                }
#endif
#endif
...
INT32 buf_agt_dmx_connect(BAGT_COMP_ID_T ui2_buf_agt_id)
{
    ...
    else if (ui2_buf_agt_id == BAGT_COMP_4)
    {
        ...
        ui2_bagt_id[FEEDER_AUDIO_SOURCE_2] = BAGT_COMP_4;
    }
    else if (ui2_buf_agt_id == BAGT_COMP_5)
    {
        gat_bagt_data[ui2_buf_agt_id].eDataType = FEEDER_AUDIO_SOURCE_3;
        ui2_bagt_id[FEEDER_AUDIO_SOURCE_3] = BAGT_COMP_5;
    }
    #endif
#else
    else if (ui2_buf_agt_id == BAGT_COMP_3)
    {
        gat_bagt_data[ui2_buf_agt_id].eDataType = FEEDER_SOURCE_3;
        ui2_bagt_id[FEEDER_SOURCE_3] = BAGT_COMP_3;
    }
    else if (ui2_buf_agt_id == BAGT_COMP_4)
    {
        gat_bagt_data[ui2_buf_agt_id].eDataType = FEEDER_SOURCE_4;
        ui2_bagt_id[FEEDER_SOURCE_4] = BAGT_COMP_4;
    }
    #ifdef CC_SKYPE_FINE_INSTANCE
    else if (ui2_buf_agt_id == BAGT_COMP_5)
    {
    ...
    #endif
#endif
    gat_bagt_data[ui2_buf_agt_id].ui4_swdmx_id = 0;
    gat_bagt_data[ui2_buf_agt_id].ui4_bagt_qid = 0;
    fgUseLastData[ui2_buf_agt_id]=FALSE;

    if ((ui2_buf_agt_id == BAGT_COMP_1) && (_hBagtMsgQ[BAGT_COMP_1] == (HANDLE_T)NULL))
    {
        VERIFY(x_msg_q_create(&_hBagtMsgQ[BAGT_COMP_1], "Bagt_CMDQ1", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
    }
    else if ((ui2_buf_agt_id == BAGT_COMP_2) && (_hBagtMsgQ[BAGT_COMP_2] == (HANDLE_T)NULL))
    {
        VERIFY(x_msg_q_create(&_hBagtMsgQ[BAGT_COMP_2], "Bagt_CMDQ2", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
    }
#ifndef CC_FBM_FOUR_INST_SUPPORT
    #ifdef CC_SKYPE_FINE_INSTANCE
    else if ((ui2_buf_agt_id == BAGT_COMP_3) && (_hBagtMsgQ[BAGT_COMP_3] == (HANDLE_T)NULL))
    {
        VERIFY(x_msg_q_create(&_hBagtMsgQ[BAGT_COMP_3], "Bagt_CMDQ3", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
    }
    else if ((ui2_buf_agt_id == BAGT_COMP_4) && (_hBagtMsgQ[BAGT_COMP_4] == (HANDLE_T)NULL))
    {
        VERIFY(x_msg_q_create(&_hBagtMsgQ[BAGT_COMP_4], "Bagt_CMDQ4", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
    }
    else if ((ui2_buf_agt_id == BAGT_COMP_5) && (_hBagtMsgQ[BAGT_COMP_5] == (HANDLE_T)NULL))
    {
        VERIFY(x_msg_q_create(&_hBagtMsgQ[BAGT_COMP_5], "Bagt_CMDQ5", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
    }
    #endif
#else
    else if ((ui2_buf_agt_id == BAGT_COMP_3) && (_hBagtMsgQ[BAGT_COMP_3] == (HANDLE_T)NULL))
    ...
    MTFeederOpen(gat_bagt_data[ui2_buf_agt_id].eDataType);

#ifdef FEEDER_MULTI_INSTANCE
    MTFeederSetInfo(gat_bagt_data[ui2_buf_agt_id].eDataType, FEEDER_MM_USE, TRUE);
#else
    MTFeederSetInfo(FEEDER_MM_USE, TRUE);
#endif
    if (!gat_bagt_data[ui2_buf_agt_id].bNotUseFeeder)
    {
        rSetQuery.eDataType = (MTFeederSourceType)ui2_buf_agt_id;
        ...
        MTFeederRegisterPullDataFunc(gat_bagt_data[ui2_buf_agt_id].eDataType, &rSetQuery);
    }

#ifdef FEEDER_MULTI_INSTANCE
    MTFeederSetBagtInfo(gat_bagt_data[ui2_buf_agt_id].eDataType, gat_bagt_data[ui2_buf_agt_id].t_align_info.ui4_align, gat_bagt_data[ui2_buf_agt_id].t_align_info.ui4_min_blk_size);
    MTFeederSetFlush(gat_bagt_data[ui2_buf_agt_id].eDataType);
#else
    MTFeederSetBagtInfo(gat_bagt_data[ui2_buf_agt_id].t_align_info.ui4_align, gat_bagt_data[ui2_buf_agt_id].t_align_info.ui4_min_blk_size);
    MTFeederSetFlush();
#endif
    ...
    MTFeederSetAppQueryMode(gat_bagt_data[ui2_buf_agt_id].eDataType, FEEDER_PULL_MODE);
    MTFeederStart(gat_bagt_data[ui2_buf_agt_id].eDataType);
    MTFeederSetRing(gat_bagt_data[ui2_buf_agt_id].eDataType, TRUE);

    if (gat_bagt_data[ui2_buf_agt_id].fgFlowCtrl)
    {
        BUF_AGT_FLOW_CTRL_T *pt_flow_ctrl = &(gat_bagt_data[ui2_buf_agt_id].t_flow_ctrl);

        if (FEEDER_E_OK != MTFeederSetUriMode(gat_bagt_data[ui2_buf_agt_id].eDataType, pt_flow_ctrl->ui4_buffer_len, pt_flow_ctrl->ui4_keep_buffer_threshold, pt_flow_ctrl->ui4_re_buferf_threshold))  // megaa 20100414
        {
            return RMR_DRV_ERROR;
        }
...
{
    MTFeederSourceType eSourceType = (MTFeederSourceType)u4SrcType;
    MTFEEDER_QUERY_DATA rSetQuery;
    ...
    gat_bagt_data[ui2_buf_agt_id].ui4_bagt_qid = 0;
    gat_bagt_data[ui2_buf_agt_id].ui4_swdmx_id = u1SwdmxId;

	LOG(0,"buf_agt_set_feedersrc,swdmxid = %d\n",u1SwdmxId);
    if(_hBagtMsgQ[ui2_buf_agt_id]== (HANDLE_T)NULL)
    {
        if(ui2_buf_agt_id==BAGT_COMP_1)
        {
            VERIFY(x_msg_q_create(&_hBagtMsgQ[ui2_buf_agt_id], "Bagt_CMDQ1", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
        }
        else if (ui2_buf_agt_id == BAGT_COMP_2)
        {
            VERIFY(x_msg_q_create(&_hBagtMsgQ[ui2_buf_agt_id], "Bagt_CMDQ2", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
        }
#ifndef CC_FBM_FOUR_INST_SUPPORT
        #ifdef CC_SKYPE_FINE_INSTANCE
        else if (ui2_buf_agt_id == BAGT_COMP_3)
        {
            VERIFY(x_msg_q_create(&_hBagtMsgQ[ui2_buf_agt_id], "Bagt_CMDQ3", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
        }
        else if (ui2_buf_agt_id == BAGT_COMP_4)
        {
            VERIFY(x_msg_q_create(&_hBagtMsgQ[ui2_buf_agt_id], "Bagt_CMDQ4", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
        }
        else if (ui2_buf_agt_id == BAGT_COMP_5)
        {
            VERIFY(x_msg_q_create(&_hBagtMsgQ[ui2_buf_agt_id], "Bagt_CMDQ5", sizeof(MTFEEDER_REQ_DATA_T), Bagt_Q_SIZE) == OSR_OK);
        }
        #endif
#else
        else if (ui2_buf_agt_id == BAGT_COMP_3)
    ...
    b_old_data_blk[ui2_buf_agt_id] = FALSE;
    MTFeederOpen(gat_bagt_data[ui2_buf_agt_id].eDataType);
    MTFeederSetInfo(eSourceType,FEEDER_MM_USE, TRUE);
    ...
    rSetQuery.u1SwdmxAttachedId  = u1SwdmxId;

    MTFeederRegisterPullDataFunc(gat_bagt_data[ui2_buf_agt_id].eDataType, &rSetQuery);

    MTFeederSetFlush(gat_bagt_data[ui2_buf_agt_id].eDataType);
    LOG(0,"set feeder src,id = %d\n",ui2_buf_agt_id);
...
INT32 buf_agt_dmx_disconnect(BAGT_COMP_ID_T ui2_buf_agt_id)
{
   if(ui2_buf_agt_id>=BAGT_COMP_NS)
   {
    ...
    MTFeederSetFlush();
#endif
    MTFeederStop(gat_bagt_data[ui2_buf_agt_id].eDataType);
    MTFeederClose(gat_bagt_data[ui2_buf_agt_id].eDataType);
...
#if 1//def LINUX_TURNKEY_SOLUTION    // joshua 20110415 tmp solution
