...
static UINT32 _CHARGetEEPOffset(UINT16 ui2_id)
{
    ...
    {
    case EEPROM_PID_AP:
        u4Offset = EEPROM_MW_OFFSET + EEPROM_MW_SIZE;
        break;
    ...
    if (u4DevFlag == STORG_FLAG_EEPROM)
    {
        switch (ui2_id)
        {
            case EEPROM_PID_AP:
                ...
                ASSERT (u4Size > (EEPROM_DRIVER_SIZE + EEPROM_MW_SIZE));
                // driver use EEPROM_DRIVER_SIZE bytes in front of MW.
                u4Size -= (EEPROM_DRIVER_SIZE + EEPROM_MW_SIZE);
            break;
    ...
    SIZE_T         *pz_get_info_len)
{
    ...
    switch (e_get_type)
    {
        ...
        case CHRDEV_GET_TYPE_MW_PARTITION_OFFSET:
            if (*pz_get_info_len < sizeof(UINT32))
            {
                *pz_get_info_len = sizeof(UINT32);
                return RMR_DRV_NOT_ENOUGH_SPACE;
            ...
            *(UINT32 *)pv_get_info = _CHARGetEEPOffset(EEPROM_PID_MW);
            *pz_get_info_len = sizeof(UINT32);
            i4_ret = RMR_OK;
            break;
     ...
     SIZE_T          z_set_info_len)
{
    ...
    CHRDEV_IO_CTRL_T *pt_IoData = NULL;
    ...
    switch (e_set_type)
    {
        ...
        case CHRDEV_SET_TYPE_ERASE:
            ...
            break;
        case CHRDEV_SET_TYPE_IO_CTRL:
           if (z_set_info_len < sizeof(CHRDEV_IO_CTRL_T))
           {
              return RMR_DRV_NOT_ENOUGH_SPACE;
           }
           pt_IoData = (CHRDEV_IO_CTRL_T *)pv_set_info;
           if(pt_IoData)
           {
               if((pt_IoData->u4IoDataSz <= sizeof(UINT64)) && (pt_IoData->pvIoData != NULL))
               {
                   u4IdType = ((UINT32)pt_comp_id->ui2_id)<<16;
                   u4IdType |= ((UINT16)pt_IoData->u4IoctrlType);
                   x_memcpy((VOID *)&u8IoData, pt_IoData->pvIoData, pt_IoData->u4IoDataSz);
                   i4Ret = STORGSendReq(prDev, STORG_OP_IOCTL, NULL, NULL, NULL,
                            u8IoData, u4IdType, NULL, NULL);
                   if(i4Ret == 0)
                   {
                       i4_ret = RMR_OK;
                   }
...
static INT32 _MW_CharDevInit(UINT32 u4DevFlag, const CHAR *szDefName, DRV_TYPE_T drvType, UINT16 ui2_uid)
{
	...
	if (szDefName)
	{
	    ...
	    x_snprintf(ps_name, 32, szDefName, ui2_uid);
	}
...
INT32 MW_CHAR_Init()
{
    ...
    // EEPROM id = 0 is for AP.
    i4Ret = _MW_CharDevInit(STORG_FLAG_EEPROM, EEPROM_DEF_NAME, DRVT_EEPROM, EEPROM_PID_AP);
    if (i4Ret) {
        ASSERT(0);
        LINT_SUPPRESS_NEXT_EXPRESSION(527);
        return RMR_DRV_ERROR;
