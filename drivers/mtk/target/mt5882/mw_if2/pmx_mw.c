...
static MTVDO_ARG_TYPE_T _PmxMwGetType2ArgType(UINT32 u4GetType);

static MTVDO_ARG_TYPE_T _PmxMwSetType2ArgType(UINT32 u4SetType);
...
void _sync_fb()
{
    ...
    #else
    MTPMX_GetPlaneOrderArray(u4HwOrderPlanes);
    #endif
                        ...
                        x_rm_nfy_fct     pf_nfy)
{
    ...
    if (prCompReg->e_type == DRVT_OSD_PLANE)
    {
        ...
        // handle connect
        MTOSD_PLA_Global_Enable(ucOsdId, 1);
                        ...
                        SIZE_T           z_disc_info_len)
{
    ...
    if (rCompId.e_type == DRVT_VID_PLANE)
    {
        ...
        {
            // overmtal -> vdp_mw.c had removed this function
    ...
    if (rCompId.e_type == DRVT_OSD_PLANE)
    {
        ...
        // handle disconnect
        MTOSD_PLA_Global_Enable(ucOsdId, 0);
    }
    else
    {
       return (RMR_DRV_INV_DISC_INFO);
                    ...
                    SIZE_T*         pz_get_info_len)
{
    ...
    switch (e_get_type)
    {
        ...
        case PLA_MXR_GET_TYPE_BACK_LIGHT:
            {
                ...
                *pz_get_info_len = sizeof(UCHAR);
                ...
                if (MTVDO_GetArg(VDP_1, ucArgType, &i2Val) != MTR_OK)
                {
                    return (RMR_DRV_INV_GET_INFO);
                }
        ...
        case PLA_MXR_GET_TYPE_MIN_MAX:
            {
                ...
                *pz_get_info_len = sizeof(PLA_MXR_MIN_MAX_INFO_T);
                ...
                if ((ucArgType == MTVDO_ARG_BACK_LIGHT_LVL) ||
                    (ucArgType == MTVDO_ARG_ADAPTIVE_BACK_LIGHT))
                {
                    if (MTVDO_GetPeUiRangeParam(VDP_1, ucArgType, &rUiRange) != MTR_OK)
                    {
                        return (RMR_DRV_INV_GET_INFO);
                    }

                    prArg->ui4_min_value = (UINT16)rUiRange.i4Min;
                    prArg->ui4_max_value = (UINT16)rUiRange.i4Max;
                }
                else
                {
                    return (RMR_DRV_INV_GET_INFO);
        ...
        case PLA_MXR_GET_TYPE_RESOLUTION:
            {
                MTPMX_PALEL_INFO_T rPalenInfo = {0};  
                ...
                *pz_get_info_len = sizeof(PLA_MXR_RESOLUTION_INFO_T);
                ...
                if(MTPMX_PANEL_GetPanelInfo(&rPalenInfo)!=MTR_OK)
                {
                    return (RMR_DRV_GET_FAILED);
                }
                prArg->b_is_progressive = 1;
                prArg->ui2_width = rPalenInfo.u4PanelWidth;
                prArg->ui2_height = rPalenInfo.u4PanelHeight;
                prArg->ui2_curr_v_clk = rPalenInfo.u4PixelClk60Hz;
                prArg->ui2_v_clk_max = rPalenInfo.u4VClkMax;
                prArg->ui2_v_clk_min = rPalenInfo.u4VClkMin;
                prArg->ui2_panel_width = rPalenInfo.u4PanelPhysicalWidth;
                prArg->ui2_panel_height = rPalenInfo.u4PanelPhysicalHeight;
            }
            break;
        ...
        case PLA_MXR_GET_TYPE_TV_MODE:
            {
                MTSYS_SRM_TV_MODE_T rTvMode = MTSYS_SRM_TV_MODE_TYPE_NORMAL;
                PLA_MXR_TV_MODE_T *prArg = (PLA_MXR_TV_MODE_T*) pv_get_info;

                if(MTSYS_SRM_GetTvMode(&rTvMode)!=MTR_OK)
                {
                    return (RMR_DRV_GET_FAILED);
                }
                ...
                *pz_get_info_len = sizeof(PLA_MXR_TV_MODE_T);
                ...
                if (rTvMode == MTSYS_SRM_TV_MODE_TYPE_NORMAL)
                {
                    *prArg = PLA_MXR_TV_MODE_NORMAL;
                }
                else if (rTvMode == MTSYS_SRM_TV_MODE_TYPE_PIP)
                {
                    *prArg = PLA_MXR_TV_MODE_PIP;
                }
                else if (rTvMode == MTSYS_SRM_TV_MODE_TYPE_POP)
                {
        ...
        case PLA_MXR_SET_TYPE_CTRL:
            {
                ...
                {
                    MTPMX_PANEL_PowerSequence(TRUE);
                    _fgPanelOnOff = TRUE;
                    //return (RMR_DRV_INV_SET_INFO);
                }
                else if (eArg == PLA_MXR_CTRL_DISABLE)
                {
                    MTPMX_PANEL_PowerSequence(FALSE);
                    _fgPanelOnOff = FALSE;
                    //return (RMR_DRV_INV_SET_INFO);
                }
                else
...
// TODO: REMOVE    	
		        ...
		        MTVDO_SetArg(VDP_1, MTVDO_ARG_GAMMA, ucArg);	        
             }
            break;
...
// TODO: REMOVE          	
                ...
                if (MTVDO_SetArg(VDP_1, ucArgType, (INT16)ucArg) != MTR_OK)
                {
                    return (RMR_DRV_INV_SET_INFO);
                }
            }
...
// TODO: REMOVE           	
                ...
                else
                {
                    ...
                    IGNORE_RET(MTGPIO_Output((INT32)u4Gpio, (INT32*)&u4Value));
                }
            }
            break;
        ...
        case PLA_MXR_SET_TYPE_TV_MODE:
            {
                ...
                MTSYS_SRM_MM_MODE_T eMode;

                rTvMode = (PLA_MXR_TV_MODE_T)(UINT32)pv_set_info;
                if (rTvMode == PLA_MXR_TV_MODE_NORMAL)
                {
                    eMode = MTSYS_SRM_TV_MODE_TYPE_NORMAL;
                }
                else if (rTvMode == PLA_MXR_TV_MODE_PIP)
                {
                    eMode = MTSYS_SRM_TV_MODE_TYPE_PIP;
                }
                else if (rTvMode == PLA_MXR_TV_MODE_POP)
                {
                    eMode = MTSYS_SRM_TV_MODE_TYPE_POP;
                }
                else
                {
                    return (RMR_DRV_INV_SET_INFO);
                }
                MTSYS_SRM_SetTvMode(eMode);
            }
            break;
        ...
        case PLA_MXR_SET_TYPE_TEST_PATTERN:
            {
                if (pv_set_info == PLA_MXR_TEST_PATTERN_DISABLE)
                {
                    ...
                    MTPMX_ShowPattern(_fgPatternEnable);
                }
                else
                {
        ...
        case PLA_MXR_SET_TYPE_BACK_LIGHT_CTRL:
            {
                ...
                {
                    MTPMX_PANEL_EnableBacklightPower(FALSE);
                }
                else
                {
                    MTPMX_PANEL_EnableBacklightPower(TRUE);
                }
...
static void _PmxSetPlaneOrder(UCHAR ucPmxId)
{
    ...
    #endif
    #else
    MTPMX_SetPlaneOrderArray(au4HwZorder);
    #endif
    }
}
...
static MTVDO_ARG_TYPE_T _PmxMwGetType2ArgType(UINT32 u4GetType)
{
    switch (u4GetType)
    {
        // VDP Back Light Args
        case PLA_MXR_GET_TYPE_BACK_LIGHT:
            return MTVDO_ARG_BACK_LIGHT_LVL;

        case PLA_MXR_GET_TYPE_ADP_BACK_LIGHT:
            return MTVDO_ARG_ADAPTIVE_BACK_LIGHT;
    ...
    DISP_QA_MIN_MAX_DFT_T*    pt_min_max_dft_tbl )
{
    ...
    while( !DISP_QA_MIN_MAX_DFT_IS_END(pt_iter) )
    {
        ...
        case DISP_QA_TYPE_BACK_LIGHT:
                eUiVqItem = MTVDO_ARG_BACK_LIGHT_LVL;
            break;
        case DISP_QA_TYPE_ADP_BACK_LIGHT:
                eUiVqItem = MTVDO_ARG_ADAPTIVE_BACK_LIGHT;
            break;
        default:
            return FALSE;
        }
        ...
        MTVDO_SetPeUiRangeParam((UINT8)e_drv_src_type, eUiVqItem, &rUiVqMinMaxDft);
...
static BOOL _PmxParseQaTbl(DISP_QA_TBL_T*    pt_qa_tbl )
{
    BOOL              b_success;
...
static INT32 _RegOSD(void)
{
    ...
    rFctTbl.pf_rm_get = NULL;
    rFctTbl.pf_rm_set = NULL;
