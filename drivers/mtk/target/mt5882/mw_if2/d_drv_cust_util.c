    ...
    const DRV_REG_GRP_NAME_T*  pt_reg_grp_name )
{
    ...
    SIZE_T              z_num_entries = 0;
    INT32               i4_ret;
    DRV_COMP_REG_T      at_reg_comp_ids_checked[MAX_NUM_SINK_COMPS];
    SIZE_T              z_num_entries_checked = 0;
    UINT32              i;
    ...
    x_memset( at_reg_comp_ids,          0, sizeof(at_reg_comp_ids) );
    x_memset( at_reg_comp_ids_checked,  0, sizeof(at_reg_comp_ids_checked) );
    ...
    /* add verify comp check, before register RM group */
    /* the action is for one image */
    /* to support image, the group will always configuration the group */
    z_num_entries_checked = 0;
    for (i = 0; i < z_num_entries; i++)
    {
        if (ID_TYPE_IND == at_reg_comp_ids[i].e_id_type)
        {
            INT32                       i4_ret;
            SIZE_T                      z_len_get = 0;
            
            i4_ret = x_rm_get_comp (
                                at_reg_comp_ids[i].e_type,
                                at_reg_comp_ids[i].u.t_ind.ui2_id,
                                FALSE,
                                ANY_PORT_NUM,
                                RM_GET_TYPE_LAST_ENTRY - 1, /* invalid get type */
                                NULL,                       /* no space */
                                & z_len_get);
            if (RMR_NO_COMP_FOUND != i4_ret)                /* check if the comp exsit by return value */
            {
                at_reg_comp_ids_checked[z_num_entries_checked++] = at_reg_comp_ids[i];
            }
        }
    }
    
    if ( z_num_entries_checked > 0 )
    {
        i4_ret = x_rm_reg_group_name(
            pt_reg_grp_name->s_grp_name,
            at_reg_comp_ids_checked,
            z_num_entries_checked );
        if ( i4_ret != RMR_OK )
        {
            return -1;
        }
