...
static void  (*pfnIreqIsrCbk) (UINT16)= NULL;

static void  (*pfnFastBootCbk) (UINT8) = NULL;
//-----------------------------------------------------------------------------
// Private functions
//-----------------------------------------------------------------------------
typedef enum
...
void _PcmciaIreqIsrCbk(UINT16  ui2_ireq_status)
{
    ...
    if ( pfnIreqIsrCbk != NULL )
    {
       ...
       // CI_DEBUG("_PcmciaIreqIsrCbk(): ui2_ireq_status:%d\n",ui2_ireq_status);
        //pfnIreqIsrCbk(ui2_ireq_status);
        pfnIreqIsrCbk(0);
    }    
#endif
...
void _PcmciaFBClk(UINT8 ui1_fb_status)
{
    CI_DEBUG("[CI_FB_DBG]: ui1_fb_status = %d\n",ui1_fb_status);
    if (NULL != pfnFastBootCbk)
    {
        pfnFastBootCbk(ui1_fb_status);
    }
}
//-----------------------------------------------------------------------------
/*
    ...
    switch (e_get_type)
    {
        case CI_GET_TYPE_POD_ACTIVE:
	       //	b_return_value = PCMCIA_DetectPodActive();
			b_return_value = MTCI0_DetectPodActive();
        ...
        case CI_GET_TYPE_CARD_PRESENT:
			...
			i4_return_value = MTCI0_DetectCard();
        ...
        case CI_GET_TYPE_CARD_READY:
			...
			i4_return_value = MTCI0_TestCardReady();
        ...
        case CI_GET_TYPE_READ_COR:
			...
			i4_return_value = MTCI0_ReadCor(((POD_COR_INFO_T*)pv_get_info)->ui2_cor_address);
        ...
        case CI_GET_TYPE_READ_REG:
			...
			MTCI0_SetPodCiAccessMode(POD_DATA_CHANNEL);
		    //PCMCIA_PcmciaToPod(TRUE);
			MTCI0_PcmciaToPod(TRUE);

            if (MTCI0_ReadReg(0,//to do
            				((POD_REG_DATA_INFO_T*)pv_get_info)->ui1_reg_addr,
            				((POD_REG_DATA_INFO_T*)pv_get_info)->t_data_info.pui1_data,
            				(UINT16)(((POD_REG_DATA_INFO_T *)pv_get_info)->t_data_info.z_data_len)) !=0 )
            {
                CI_DEBUG("Pcmcia_Get(CI_GET_TYPE_READ_REG): read reg error\n");
				return RMR_DRV_GET_FAILED;
			}
            break;
        case CI_GET_TYPE_READ_DATA:// not used now
            t_get_info_len = sizeof(POD_DATA_INFO_T);
            ...
            if (MTCI0_Read(0,
                         ...
                         (UINT16 *)(&((POD_DATA_INFO_T *)pv_get_info)->z_data_len)) != 0)
            {
		...
		case CI_GET_TYPE_SECURE_ID:
			return RMR_OK;
			#if 0
        case CI_GET_TYPE_DEVICE_DATA:// ask mw
        ...
        case CI_GET_TYPE_DEVICE_ID:// ask mw
            t_get_info_len = sizeof(POD_EFUSE_DATA_INFO_T);
            if (*pz_get_info_len < t_get_info_len)
            {
        ...
        case CI_GET_TYPE_SECURE_ID://ask mw
            t_get_info_len = sizeof(POD_EFUSE_DATA_INFO_T);
            if (*pz_get_info_len < t_get_info_len)
            {
             ...
             SIZE_T            z_set_info_len)
{
    ...
    BOOL    fgForceTsThroughCard = FALSE;
    ...
    switch (e_set_type)
    {
    	case CI_SET_TYPE_INIT_CI:
	        ...
	        CI_SetPower(POD_POWER_DRIVING_6MA);// to do
        #endif
		#endif
	        if ((MTCI0_Init() == 0) &&// mtal need to do
	            (MTCI0_EnableIsr() == 0))//also need to modify
	        {
	        }
	        else
	        {
	        ...
	        MTCI0_PcmciaToPod(FALSE);

	        /* Disable TS in and out first */
	        MTCI0_PcmciaEnableTS(FALSE);
	        break;
		case CI_SET_TYPE_MODULE_MODE:
            ...
            if (MTCI0_PcmciaToPod(fgPcmciaToPod) != POD_OK)
            {
		...
		case CI_SET_TYPE_ENABLE_TS:
			...
			if (MTCI0_PcmciaEnableTS(((POD_ENABLE_TS_T*)pv_set_info)->b_pod_ts_mode_enable != 0))
			{
		...
		case CI_SET_TYPE_WRITE_DATA:
			if (MTCI0_Write(0,
						  ...
						  (UINT16)(((POD_DATA_INFO_T*)pv_set_info)->z_data_len)) != 0)
			{
			    CI_DEBUG("Pcmcia_Set(CI_SET_TYPE_WRITE_DATA): error\n");
				return RMR_DRV_SET_FAILED;
			}
			break;
		case CI_SET_TYPE_ACCESS_MODE:	/* may not be needed? */// ask mw
            if (MTCI0_SetPodCiAccessMode(*(POD_ACCESS_MODE_T *)pv_set_info) != 0)
            {
		...
		case CI_SET_TYPE_WRITE_REG:
			...
			if (MTCI0_WriteReg(0,//motify as readreg
							 ((POD_REG_DATA_INFO_T*)pv_set_info)->ui1_reg_addr,
							 ((POD_REG_DATA_INFO_T*)pv_set_info)->t_data_info.pui1_data,
							 (UINT16)(((POD_REG_DATA_INFO_T*)pv_set_info)->t_data_info.z_data_len)) != 0)
			{
		...
		case CI_SET_TYPE_WRITE_COR:
            if (MTCI0_WriteCor(((POD_COR_INFO_T*)pv_set_info)->ui2_cor_address,
							 ((POD_COR_INFO_T*)pv_set_info)->ui1_cor_value) != 0)
			{
		...
		case CI_SET_TYPE_RESET:
            ...
            i4_return_value = MTCI0_Reset(*(BOOL*)pv_set_info);
        ...
        case CI_SET_TYPE_INTERRUPT:// need deeply thinking
//			PCMCIA_RegIsr(PCMCIA_ISR_IREQ, (x_os_isr_fct) pv_set_info, NULL);
            pfnIreqIsrCbk = pv_set_info;
            //PCMCIA_RegIsr(PCMCIA_ISR_IREQ, (x_os_isr_fct) _PcmciaIreqIsrCbk, NULL);
			MTCI0_RegIsr(2, (void*) _PcmciaIreqIsrCbk, NULL);
			break;
        case CI_SET_TYPE_FASTBOOT:
            pfnFastBootCbk = pv_set_info;
            MTCI0_RegFBClk((void *) _PcmciaFBClk);
            break;
        case CI_SET_TYPE_WRORKAROUND_DRV:
            MTCI0_CiCamWorkaround((((CI_CAM_WORKAROUND_DRV_INFO_T*)pv_set_info)->t_cam_workaround),
                                   (((CI_CAM_WORKAROUND_DRV_INFO_T*)pv_set_info)->pv_para));
			break;        	
        	
        case CI_SET_TYPE_FORCE_TS_THROUGH_CARD:
            fgForceTsThroughCard = *(BOOL *)pv_set_info;
            if (fgForceTsThroughCard)
            {
            	MTCI0_SetBypassTsRouting(TRUE);
                MTCI0_PcmciaToPod(TRUE);
                MTCI0_SetBypass(FALSE);
            }
            else
            {
                
                MTCI0_SetBypassTsRouting(FALSE);
                MTCI0_PcmciaToPod(FALSE);
            }
            break;
