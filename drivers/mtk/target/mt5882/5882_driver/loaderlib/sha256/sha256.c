 ...
 *
 */
int SHA256_Reset(SHA256_CONTEXT_T *context)
{
    if (!context)
    {
        return shaNull;
 ...
 *
 */
int SHA256_Result( SHA256_CONTEXT_T *context,
 ...
 *
 */
int SHA256_Input(SHA256_CONTEXT_T *context, const uint8_t *message_array, uint32_t length)
{
    if (!length)
    {
        return shaSuccess;
...
{
    int       t;                      /* Loop counter                */
    uint32_t      temp, temp2;            /* Temporary word value        */
...
int SHA256(const uint8_t *message_array, uint32_t length, uint8_t Message_Digest[SHA256_HASH_SIZE])
{
...
int mtk_hmac_sha256(uint8_t * key, uint32_t key_size, uint8_t * msg, uint32_t msg_size,  uint8_t result[SHA256_HASH_SIZE])
{
	uint8_t ipad[SHA_BLOCK_SIZE], opad[SHA_BLOCK_SIZE], kpad[SHA_BLOCK_SIZE];
	uint8_t hash_result[SHA256_HASH_SIZE];
	uint8_t *buf = 0, i;
	...
	buf = (uint8_t *)x_mem_alloc(SHA_BLOCK_SIZE + msg_size);
	if(buf) {
		/* 1st hash */
...
{
    uint8_t result[SHA256_HASH_SIZE], i;
    uint8_t *key = "key";
