...
static BOOL _fgTrackingMode = TRUE;
...
#if 0
{
  ...
  //printf("RV_VLD 0x0084 = 0x%08X\n", RISCRead(RV_VLD_BASE + 0x0084));
    ...
    if (_fgTrackingMode)
    {
        RISCWrite(VLD_TOP_BASE + RW_VLD_TOP_TRACKING_MODE, 
            RISCRead(VLD_TOP_BASE + RW_VLD_TOP_TRACKING_MODE) | ENABLE_TRACKING | BUF_MBL_OFFSET);
    }
...
static INT32 _VdecRvTest(INT32 i4Argc, const CHAR ** szArgv)
{
  ...
  if (i4Argc < 2)
  {
    ...
    printf("vdec.t case {226: Toggle tracking mode ON/OFF}\n");
...
//          printf("VFIFO read pointer warp 0x%08X => 0x%08X\n", _dwPrVRp, _dwPrVRp - VFIFO_SZ);
  ...
  case 226:
	_fgTrackingMode = !_fgTrackingMode;
	if (_fgTrackingMode)
		printf("Tracking Mode is On\n");
	else
		printf("Tracking Mode is Off\n");
	break;
