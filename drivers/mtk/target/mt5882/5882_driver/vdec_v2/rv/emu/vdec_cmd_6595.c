...
#define VDEC_L2_CACHE // define this to use HalFlushInvalidateDCacheMultipleLine instead of HalFlushInvalidateDCache
...
#define DEC_WAIT_CNT          3000000  // time to wait for decode finish
...
#define RV_VLD_BASE                        (AVC_VLD_BASE + 0xC00)
#define AVC_MV_BASE                        (IO_VIRT + 0x31000)
#define CRC_BASE                           (IO_VIRT + 0x03000)
#else
#define VLD_BASE       0x20002000
#define MC_BASE        0x2000A000
#define RV_VLD_BASE    0x20030C00
#define AVC_MV_BASE    0x20031000
#endif

#if 1 // david
#define PP_BASE        0xF002f000
#else
#define PP_BASE        0x20021000
#endif
...
#define RMEMU_BASE_ADDR  0x299d000  //0xa9fc000  //0x397C000
#define RMEMU_SIZE       0x1a74800  //0xC03000

#define RMEMU_BASE_ADDR2 0x4712000  //0xb900000  //(0x397C000 + 0xC10000)
#define RMEMU_SIZE2      0x1100000  //0xBEFCE0

#define PPY_OUT_BUF1        RMEMU_BASE_ADDR
#define PPY_OUT_BUF1_SZ     0x1FE000                                // 1920*1088 = 2088960
#define PPC_OUT_BUF1        (PPY_OUT_BUF1 + PPY_OUT_BUF1_SZ)
#define PPC_OUT_BUF1_SZ     0xFF000                                 // 1920*1088/2 = 1044480

#define PPY_OUT_BUF2        (PPC_OUT_BUF1 + PPC_OUT_BUF1_SZ)
#define PPY_OUT_BUF2_SZ     0x1FE000                                // 1920*1088 = 2088960
#define PPC_OUT_BUF2        (PPY_OUT_BUF2 + PPY_OUT_BUF2_SZ)
#define PPC_OUT_BUF2_SZ     0xFF000                                 // 1920*1088/2 = 1044480

#define PPY_OUT_BUF3        (PPC_OUT_BUF2 + PPC_OUT_BUF2_SZ)
#define PPY_OUT_BUF3_SZ     0x1FE000                                // 1920*1088 = 2088960
#define PPC_OUT_BUF3        (PPY_OUT_BUF3 + PPY_OUT_BUF3_SZ)
#define PPC_OUT_BUF3_SZ     0xFF000                                 // 1920*1088/2 = 1044480

#define PPY_OUT_BUFR        (PPC_OUT_BUF3 + PPC_OUT_BUF3_SZ)
#define PPY_OUT_BUFR_SZ     0x1FE000                                // 1920*1088 = 2088960
#define PPC_OUT_BUFR        (PPY_OUT_BUFR + PPY_OUT_BUFR_SZ)
#define PPC_OUT_BUFR_SZ     0xFF000                                 // 1920*1088/2 = 1044480
...
#define MV_DIRECT_ADDR      (RMEMU_BASE_ADDR2)
...
#define VFIFO_ADDR          (VLD_PRED_OFFSET + VLD_PRED_OFFSET_SZ)
#define VFIFO_SZ            0x200000                                // 2MB

#define FRMINFO_ADDR        (VFIFO_ADDR + VFIFO_SZ)
#define FRMINFO_SZ          0x4F1A00                                // 30 * 7200 * 24 = 5184000

#define GOLD_ADDR           (FRMINFO_ADDR + FRMINFO_SZ)
#define GOLD_SZ             0x2FD000                                // 1920*1088*1.5

#define EXELIST_ADDR        (GOLD_ADDR + GOLD_SZ)
#define EXELIST_SZ          0x100000

#define TMP_ADDR            (EXELIST_ADDR + EXELIST_SZ)
#define TMP_SZ              6144
...
static BOOL _fgCmpOnPC = TRUE;
static BOOL _fgRprRacing = FALSE;
static BOOL _fgDummyAgent = TRUE;
static BOOL _fgDirectMode = FALSE;

static UINT32 _u4DumpRegFrameNo = -1; // david: dump register of frame #_u4DumpRegFrameNo
static UINT32 _u4SetAvcMv134OfFrameNo = -1; // david: set AVC_MV 134 of frame #_u4SetAvcMv134OfFrameNo because it could be set only for first time
...
BOOL _DmxIdeRead(UINT32 u4Addr, UINT32 u4Size){}
void _DmxIdeGetNextCmd(UINT8 *pu1Msg){}
BOOL _DmxIdeGetFile(UINT32 u4Addr, UINT32 u4Type){}
BOOL _DmxIdeSendName(char *pcItem){}
BOOL _DmxIdeSendNum(UINT32 n){}
void _DmxIdeSendDataLen(UINT32 u4Len){}
void _DmxIdeSendData(UINT32 u4Addr, UINT32 u4Len){}
BOOL _DmxIdeGetCmpResult(void){}
void _DmxIdeTestTxRate(UINT32 u4Addr){}
void _DmxIdeTestRxRate(UINT32 u4Addr){}
void _DmxIdeGetPrm(UINT32 u4Addr){}
BOOL _DmxIdeGetPartFile(UINT32 u4Addr, UINT32 u4Ofst, UINT32 u4Len){}
BOOL _DmxIdeSendProp(char *pcItem, UINT32 u4Width, UINT32 u4Height, BOOL fgSendName){}
UINT32 _DmxIdeGetUop(void){}
#else
extern void _DmxIdeReset(void);
extern BOOL _DmxIdeRead(UINT32 u4Addr, UINT32 u4Size);
extern void _DmxIdeGetNextCmd(UINT8 *pu1Msg);
extern BOOL _DmxIdeGetFile(UINT32 u4Addr, UINT32 u4Type);
extern BOOL _DmxIdeSendName(char *pcItem);
extern BOOL _DmxIdeSendNum(UINT32 n);
extern void _DmxIdeSendDataLen(UINT32 u4Len);
extern void _DmxIdeSendData(UINT32 u4Addr, UINT32 u4Len);
extern BOOL _DmxIdeGetCmpResult(void);
extern void _DmxIdeTestTxRate(UINT32 u4Addr);
extern void _DmxIdeTestRxRate(UINT32 u4Addr);
extern void _DmxIdeGetPrm(UINT32 u4Addr);
extern BOOL _DmxIdeGetPartFile(UINT32 u4Addr, UINT32 u4Ofst, UINT32 u4Len);
extern BOOL _DmxIdeSendProp(char *pcItem, UINT32 u4Width, UINT32 u4Height, BOOL fgSendName);
extern UINT32 _DmxIdeGetUop(void);
#endif
...
static void vShowErrDumpHint(int n, char *szDir, UINT32 u4Width, UINT32 u4Height, UINT32 u4BstLength)
{
  ...
  u4YSize = u4Width * u4NewHeight - 1;
  u4CSize = u4Width * u4NewHeight / 2 - 1;
  
  u4GoldSize = u4Width * u4Height;
  u4GoldSize = u4GoldSize + u4GoldSize / 2 - 1;
  
  printf("d.save.b %s/%s_%07d_Y.bin 0x%X--0x%X\n", DUMP_BASE_DIR, szDir, n, _u4OutBufY, _u4OutBufY + u4YSize);
  printf("d.save.b %s/%s_%07d_C.bin 0x%X--0x%X\n", DUMP_BASE_DIR, szDir, n, _u4OutBufC, _u4OutBufC + u4CSize);
  printf("d.save.b %s/%s_%07d_bst.bin 0x%X--0x%X\n", DUMP_BASE_DIR, szDir, n, VFIFO_ADDR, VFIFO_ADDR + u4BstLength - 1);
  printf("d.save.b %s/%s_%07d_G.bin 0x%X--0x%X\n", DUMP_BASE_DIR, szDir, n, GOLD_ADDR, GOLD_ADDR + u4GoldSize);
#if 0 // david: should handle szDir contain dir case
  if (!_fgFileFromIDE)
  {
	  UNUSED(T32_HostExec(E_T32_EXEC, "d.save.b %s/%s_%07d_Y.bin 0x%X--0x%X\n", DUMP_BASE_DIR, szDir, n, _u4OutBufY, _u4OutBufY + u4YSize));
	  UNUSED(T32_HostExec(E_T32_EXEC, "d.save.b %s/%s_%07d_C.bin 0x%X--0x%X\n", DUMP_BASE_DIR, szDir, n, _u4OutBufC, _u4OutBufC + u4CSize));
  }
#endif
}
...
void vDispViaIde(UINT32 u4DecWidth, UINT32 u4DecHeight)
{
    ...
    UINT32 _dwDecBufIdx, _dwDispBufIdx;
...
static BOOL fgCompareGold(UINT32 n, char *szDir, UINT32 u4DecWidth, UINT32 u4DecHeight, UINT32 u4Width, UINT32 u4Height, BOOL fgFirstTime)
{
  ...
  for (i = 0; i < 4; i++)
  {
      ...
      _u4CRCPtr += 4;
      if (u4Val != RISCRead(CRC_BASE + (2+i)*4))
      {
          printf("!!! CRC Y%d mismatch, %08X v.s %08X !!!\n", i, u4Val, RISCRead(CRC_BASE + (2+i)*4));
          return FALSE;
      }
  }
  ...
  for (i = 0; i < 4; i++)
  {
      u4Val = *(UINT32 *)(GOLD_ADDR + _u4CRCPtr);
      _u4CRCPtr += 4;
      if (u4Val != RISCRead(CRC_BASE + (6+i)*4))
      {
          printf("!!! CRC C%d mismatch, %08X v.s %08X !!!\n", i, u4Val, RISCRead(CRC_BASE + (6+i)*4));
          return FALSE;
      }
  }
...
static void INIT_BARREL_SHIFTER(void)
{
  ...
  if (_fg8253Test)                                  // RPAR: read pointer
    *(UINT32 *)0xF00020b0 = _dwPrVRp & 0x0FFFFFFF;
  else
    *(UINT32 *)0xF00020b0 = VFIFO_ADDR;
    
  *(UINT32 *)0xF0002110 = 0xffffffff;               // RCRW: ???
  *(UINT32 *)0xF00020b4 = (VFIFO_ADDR >> 6);               // DSTA: V FIFO start addr
  *(UINT32 *)0xF00020b8 = (VFIFO_ADDR + VFIFO_SZ) >> 6;    // DETA: V FIFO end addr
...
#else
  while (!((*(UINT32 *)0x200020ec) & 0x00000003));  // wait for VBAR bit 0, 1 raise
#endif
}
...
{
  // soft reset
  RISCWrite(VLD_BASE + 66*4, 1);
  RISCWrite(VLD_BASE + 66*4, 0);
}
...
static void vMcInit(void)
{
  ...
  RISCWrite(MC_BASE + 453*4, (RISCRead(MC_BASE + 453*4) & 0xFFFFFFFE));
  RISCWrite(MC_BASE + 488*4, 4);
...
//  qw = 0x04000000 | (u4CurrPicDecWidth / 16);
  qw = 0x10000000 | (u4CurrPicDecWidth / 16); // david
...
//  qw = (u4CurrPicDecWidth) << 12 | u4CurrPicDecHeight;
  ...
  UINT32 i;	
  UINT32 addr, value;
  Printf("dump resizer start \n");
  for (i = 0; i < RZ_ECO; i++)
  {
     addr = IMG_RESZ_BASE + i*4;
     value = RISCRead(IMG_RESZ_BASE + i*4);
	 Printf("addr:%08X, value:%08X\n", addr, value);
	
  }
  Printf("dump resizer end \n");
  ...
  _u4Trap = 0;
  while (RISCRead(IMG_RESZ_BASE + IMG_RESZ_DONE * 4) == 0)
    _u4Trap++;
...
static BOOL fgTestRvDec(RvPicAttr *prPic, BOOL fgFirstTime, UINT32 n)
{
  ...
  RM_LOG(("To trigger decode...MC + 0x81C=0x%08X MC + 0x71C=0x%08X\n", RISCRead(MC_BASE + 0x81C), RISCRead(MC_BASE + 0x71C)));
  //vDumpMcReg();
