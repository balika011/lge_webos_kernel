...
void vVgaAutoInit(void)
{
    ...
    if (fgApiVideoIsEvtFlgSet(VDO_FLG_VGAAUTO_DOING))
	{
		...
		if(fgIsMainVga() || fgIsPipVga())
		{
			...
			if(bApiQuearyVSCConnectStatus(0)==SV_VD_VGA) bPath = 0;
			else if(bApiQuearyVSCConnectStatus(1)==SV_VD_VGA) bPath = 1;
			else {}
			#endif
		}
		else
		{
			...
			#ifdef CC_SUPPORT_PIPELINE
			if(bApiQuearyVSCConnectStatus(0)==SV_VD_YPBPR) bPath = 0;
			else if(bApiQuearyVSCConnectStatus(1)==SV_VD_YPBPR) bPath = 1;
			else {}
			#endif
		}
...
PRIVATE vVgaAutoDynThr(void)
{
	...
	wRGBmaxsum=IO32ReadFldAlign(STA_SYNC0_00, RMAX)+ IO32ReadFldAlign(STA_SYNC0_00, GMAX) + IO32ReadFldAlign(STA_SYNC0_00, BMAX);
