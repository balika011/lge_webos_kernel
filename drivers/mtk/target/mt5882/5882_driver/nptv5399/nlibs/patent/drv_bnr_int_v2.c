...
static void _BNR_FW_GridDet ( UINT8 fgHV )
{
  UINT16 * au2LineSum[2];
    ...
    else
    {
        ...
        iLevelActiveTh = rBnrCpt.u1VBlkLevelActiveTh * 32 * 4;
...
static void _BNR_FW_WriteReg(void)
{
    ...
    // MNR end
#else
    NR_W(NXNR_0D, MIN(16, _iVBlkConfidence + _SWR(NR_FWBNR_0B, NR_FWBNR_CONF_OFST)), NX_MANUAL_VBLK_CONFIDENCE);
    NR_W(NXNR_0C, MIN(16, _iHBlkConfidence + _SWR(NR_FWBNR_0B, NR_FWBNR_CONF_OFST)), NX_MANUAL_HBLK_CONFIDENCE);
    NR_W(NXNR_16, MIN(16, _iVBlkConfidence + _SWR(NR_FWBNR_0B, NR_FWBNR_CONF_OFST)), NX_MANUAL_VBLK_CONFIDENCE_PR_R);
...
static void _BNR_FW_Log(void)
{
    ...
    if (_rBnrLog.u4LogCnt)
    {
        ...
        case _NXNR_BNR_FW_LOG_STA:
            //LOG(5, "V:SBP[%2d]FP[%2d]BL[%4d]TC[%2d] H:SBP[%2d]FP[%2d]BPC[%5d]BL[%4d]TC[%2d]\n",
            //    _iVBlkSavedBestPeriod, _iVBlkFractionalPeriod, _iVBlkLevel, _iVBlkActiveTrialCount,
            //    _iHBlkSavedBestPeriod, _iHBlkFractionalPeriod, _iHBlkLevel, _iHBlkActiveTrialCount);
            break;
