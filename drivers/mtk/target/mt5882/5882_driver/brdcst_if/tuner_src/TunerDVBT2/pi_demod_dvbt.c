...
//static void  vSetChSearchMode(DEMOD_CTX_T2 *psDemodCtx);
 ...
 */
//-----------------------------------------------------------------------------
UCHAR DVBT_GetDemodReg(DEMOD_CTX_T2 *psDemodCtx, UINT16 u2Addr, UCHAR *pucBuf, UCHAR ucLen)
{
 ...
 */
//-----------------------------------------------------------------------------
UCHAR DVBT_SetDemodReg(DEMOD_CTX_T2 *psDemodCtx, UINT16 u2Addr, UCHAR *pucBuf, UCHAR ucLen)
{
    UCHAR   aucData[cMAX_READ_NUM + 1];
    UCHAR   ii = 0;
 ...
 */
//-----------------------------------------------------------------------------
UINT8 DVBT_Loadcode(DEMOD_CTX_T2 *psDemodCtx, UINT8 *pu1Buffer, UINT16 u2CurRomLen)
{
...
static INT32 s4LoadCode_patch(DEMOD_CTX_T2 *psDemodCtx, UINT8 *u1CodeImage, UINT16 u2CurRomLen, UINT16 u2MaxI2cLen, BOOL fgClk50)
{
...
BOOL DVBT_FwSwitch(DEMOD_CTX_T2 *psDemodCtx)
{
...
void vDVBTGetStatus(DEMOD_CTX_T2 *psDemodCtx)
{
    ...
    {   
		...
		mcSHOW_DBG_MSG4(("Update T Status : SYS ==> %d\n!", psDemodCtx->u1ControlStatus)); 
		mcSHOW_DBG_MSG4(("Update T Status : TPS ==> %d\n!", psDemodCtx->fgTpsLock)); 
		mcSHOW_DBG_MSG4(("Update T Status : TS ==> %d\n!", psDemodCtx->fgTsLock));       
    }
}
...
#if defined(CC_MT5135) || defined(CC_MT5135DVBT)||defined(CC_MT5396)|| defined(CC_MT5368) || defined(CC_MT5136)//EC_code_dB3330: For MT5396DVBTC demod
...
//127/10=1.27=(2.8/1023)*(752/1.62)
  ...
  //UINT32 ucTmep2=0x00;
	...
	//UINT8 ucTemp, ucTemp1;
	DVBT_GetReg(psDemodCtx, 0x9A0, &ucTemp, 1);
	mcSET_BIT(ucTemp, 7);
	mcSET_BIT(ucTemp, 6);
	DVBT_SetReg(psDemodCtx, 0x9A0, &ucTemp, 1);
	DVBT_GetReg(psDemodCtx, 0x9A5, &ucTemp, 1);
	DVBT_GetReg(psDemodCtx, 0x9A6, &ucTemp1, 1);
	DVBT_GetReg(psDemodCtx, 0x9A5, &ucTemp, 1);
	psDemodCtx->u2RfAgcRssi = ((UINT16)ucTemp)<<2 | (ucTemp1&0x03);

	mcSHOW_DBG_MSG(("DVBT: SLD_RFAGC=%d, SLD_IFAGC=%d, Legacy_Index=%d\n",psDemodCtx->u2RfAgcRssi,psDemodCtx->ucSldPraGainIdx,psDemodCtx->s2SldPraOut));
#else
...
void vDVBTSetTunerSSICond(DEMOD_CTX_T2 *psDemodCtx)
{
    //INT16 InOutVal[5];
    INT16 InOutVal[6];
    ...
    mcSHOW_DBG_MSG2(("DVBT: Mod=%d, Code Rate=%d, SLD_RFAGC=%d, SLD_IFAGC=%d, Legacy_Index=%d\n",InOutVal[0],InOutVal[1],InOutVal[2],InOutVal[3],InOutVal[4]));
...
void DVBT_UpdateInfo(DEMOD_CTX_T2 *psDemodCtx)
{
    ...
    mcSHOW_USER_MSG(("Update T Information\n!"));
}
...
void DVBT_SetIF(DEMOD_CTX_T2 *psDemodCtx)
{
    ...
    mcSHOW_DBG_MSG(("psDemodCtx->psTunerCtx->u2IF_Freq = %d\n!", psDemodCtx->psTunerCtx->u2IF_Freq));
...
#if 0
static void DVBT_FwROMPatch(DEMOD_CTX_T2 *psDemodCtx)
{
    UCHAR ucData;
 ...
 */
//-----------------------------------------------------------------------------
INT32 DVBT_Initialize(DEMOD_CTX_T2 *psDemodCtx, UINT8 ucI2cAddress, BOOL fgLoadCode)
{
    //UCHAR   ucPara[ccCMD_REG_NUM];
    //UCHAR   ii;
    UCHAR   aucData[cMAX_READ_NUM + 1];
...
UINT8 DVBT_GetSignalLevel(DEMOD_CTX_T2 *psDemodCtx)
{
    ...
    mcSHOW_DBG_MSG2(("s1AgcMax=%d\n", s1AgcMax));
    mcSHOW_DBG_MSG2(("s1AgcMin=%d\n", s1AgcMin));

    if (s1AgcMax != s1AgcMin)
#if defined(CC_MT5135) || defined(CC_MT5396)|| defined(CC_MT5368) || defined(CC_MT5136)//EC_code_dB3330: For MT5396DVBTC demod
 ...
 */
//-----------------------------------------------------------------------------
INT32 DVBT_GetPER(DEMOD_CTX_T2 *psDemodCtx)  // return value = PER*10^5
{
...
void DVBT_SetBW(DEMOD_CTX_T2 *psDemodCtx, UINT8 ucBW)
{
    ...
    DVBT_DemodReset(psDemodCtx);   //T abd T2 use the same hostcommand.
...
void DVBT_SetTsIf(DEMOD_CTX_T2 *psDemodCtx, UCHAR tsif_format, UCHAR tsif_ctrl, UCHAR tsif_rate)
{
    ...
    ucPara[2] = tsif_ctrl;
    ucPara[3] = tsif_rate;
    mcSHOW_USER_CRI_MSG(("[DVBT_SetTsIf]tsif_format=0x%02x\n",ucPara[1] ));
...
{
    UCHAR ucData, ucData1;

    DVBT_GetDemodReg(psDemodCtx, MT5391_REG_TSIF_CTRL, &ucData, 1);
	DVBT2_GetDemodReg(psDemodCtx, 0x15CE, &ucData1, 1);
	...
	DVBT2_SetDemodReg(psDemodCtx, 0x15CE, &ucData1, 1);
}
...
{
#if defined(CC_MT5135) || defined(CC_MT5135DVBT) || defined(CC_MT5136)
    UINT32 u4Temp;
 ...
 */
//-----------------------------------------------------------------------------
INT32 DVBT_GetPreVBer(DEMOD_CTX_T2 *psDemodCtx) // return value = PreVBer * 10^5
{
    INT32 s4PreVBer;
 ...
 */
//-----------------------------------------------------------------------------
INT32 DVBT_GetPostVBer(DEMOD_CTX_T2 *psDemodCtx) // return value = PostVBer * 10^7
{
...
UINT8 DVBT_GetSync(DEMOD_CTX_T2 *psDemodCtx)
{
    ...
    else if (DVBT_GetTpsLock(psDemodCtx) && DVBT_GetTsLock(psDemodCtx))   // to make sure demod is ready for backend
    {
		...
		psDemodCtx->u1DVBSystem = 0; // Set flag as DVBT
    }
...
TS_FMT_T DVBT_GetTsFmt(DEMOD_CTX_T2 *psDemodCtx)
{
   ...
   mcSHOW_USER_CRI_MSG(("[DVBT_GetTsFmt]0x192=0x%02x\n",ucData));
    mcSHOW_DBG_MSG(("Change FSM Setting\n", ucData));
	...
	mcSHOW_USER_MSG(("[DVBT_GetTsFmt]fgExternalDemod = BOOL_TRUE\n"));
#else
...
VOID DVBT_SetTsOutput(DEMOD_CTX_T2 *psDemodCtx, BOOL fgOutputEnable)
{
    ...
    if(fgOutputEnable)
    {
		...
		#elif defined(CC_MT5136) 
		DVBT_GetReg(psDemodCtx, 0x8FB,  &ucData, 1);
	    ucData = (ucData & 0x30) >> 4;
	    if(ucData == 0x01) //48pin
	    { 
	    	DVBT_GetReg(psDemodCtx, 0x8E1, &ucData, 1);
			ucData = ucData | 0xF0;
			DVBT_SetReg(psDemodCtx, 0x8E1, &ucData, 1);
			ucData = 0xff;
			DVBT_SetReg(psDemodCtx, 0x8E2, &ucData, 1);
...
void DVBT_SetIfAgcCurve(DEMOD_CTX_T2 *psDemodCtx)
{
    ...
    for (ii = 0; ii < 4; ii++){
        ucPara[ii+1] = pEuTunerCtx->m_aucPara[ii+2];  // write m_aucPara[2-5]
        mcSHOW_DBG_MSG4(("pEuTunerCtx->m_aucPara[%d] = %d \n", ii+2, pEuTunerCtx->m_aucPara[ii+2]));
    }
