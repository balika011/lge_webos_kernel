...
#define IO_B2R_READ_BITS(RegName,FieldName)        IO32ReadFldAlign((BLK2RS_BASE+(RegName)) ,(FieldName))   
#define IO_B2R_READ_REG(RegName)                   u4IO32Read4B((BLK2RS_BASE+(RegName))) 
#define IO_B2R_WRITE_BITS(RegName,FieldName,value) vIO32WriteFldAlign((BLK2RS_BASE+(RegName)) ,(value) ,(FieldName))
#define IO_B2R_WRITE_REG(RegName, value)           vIO32Write4B((BLK2RS_BASE+(RegName)), (value))
#ifdef VDP_B2R_PSCAN_CRC_FLOW
...
// Static variables
//---------------------------------------------------------------------------
static B2R_HAL_OBJECT_T _b2r_hw_v1_object[HW_NS];
...
{
    UINT32 u4Offset = B2R_REG_OFFSET(u1Id);
    IO_B2R_WRITE_BITS(B2R_MODE_CTRL + u4Offset, ADDR_MODE, u4AddrMode);
...
UINT32 _B2R_HW_Select( UINT8 id)
{
         ...
         if(id==0)
         {
              IO_B2R_WRITE_BITS(B2R_SRC_SEL , B2R_MAIN_PATH_SEL, 0x00);
               Printf("_B2R_HW_Select Old B2R HD",IO_B2R_READ_BITS(B2R_SRC_SEL,B2R_MAIN_PATH_SEL));
         }
         else if(id==1)
         {
              IO_B2R_WRITE_BITS(B2R_SRC_SEL , B2R_SUB_PATH_SEL, 0x00);
              Printf("_B2R_HW_Select Old B2R SD",IO_B2R_READ_BITS(B2R_SRC_SEL,B2R_SUB_PATH_SEL));
         }
  
    
    vIO32WriteFldAlign(OMUX_00,0x9,OMUX_MAIN_SEL);
    return 0;
}
...
inline static void _Hw_Set_OMux(UINT8 u1Id, UINT8 ucPath)
{
#ifdef __NPTV_HW_IF
    UINT8 OMuxMapTbl[HW_NS]=
    {
     ...
     LOG(3,"_Old_Hw_Set_OMux u1Id=%d,ucPath=%d\n",u1Id,ucPath);
     if(u1Id==0)
     {
          IO_B2R_WRITE_BITS(B2R_SRC_SEL , B2R_MAIN_PATH_SEL, 0x00);
    ...
    IO_B2R_WRITE_BITS(B2R_SRC_SEL , DDDS_B2R_SRC_SEL, 0x00);
    // select pin mux
    if (ucPath == 0)
    {
...
inline static void _Hw_Set_OMux_OryxE3(UINT8 u1Id, UINT8 ucPath)
{
#ifdef __NPTV_HW_IF
    UINT8 OMuxMapTbl[HW_NS]=
    {
    ...
    // select pin mux
    LOG(3,"Old B2r OMux u1Id=%d,ucPath=%d!\n",u1Id,ucPath);
    ...
    if(ucPath == 0)
    {
        if(u1Id == 0)
        {
            ...
            vIO32WriteFldAlign(B2R_DATA_MUX , 1, MAIN_PATH_SEL);
        }
        else if(u1Id == 1)
        {
            vIO32WriteFldAlign(B2R_DATA_MUX , 0, MAIN_PATH_SEL);
        }
    }
    else if(ucPath == 1)
    {
        if(u1Id == 0)
        {
            vIO32WriteFldAlign(B2R_DATA_MUX , 1, SUB_PATH_SEL);
        }
        else if(u1Id == 1)
        {
            vIO32WriteFldAlign(B2R_DATA_MUX , 0, SUB_PATH_SEL);
        }
    }
    
    if(u1Id == 0)
    {
        vIO32WriteFldAlign(B2R_DATA_MUX , 0, HD_CLK_SEL);
    }
    else if(u1Id == 1)
    {
        vIO32WriteFldAlign(B2R_DATA_MUX , 1, SD_CLK_SEL);
    }
...
inline static void _Hw_Set_Burst_Read(UINT8 u1Id, BOOL fgEn)
{
    UINT32 u4Offset = B2R_REG_OFFSET(u1Id);
    ...
    {
        IO_B2R_WRITE_BITS(B2R_MODE_CTRL + u4Offset, BURST_READ_EN, 0);
    }
}
...
static void _Hw_Dump_reg(UINT8 u1Id)
{
    ...
    LOG(5, "\nBase reg: 0x%08x", BLK2RS_BASE);

    for (i = B2R_YS + u4Offset; i < (B2R_TRACK_MB_LINE + u4Offset); i += 16)
    {
...
static UINT32 _B2R_HW_Config(B2R_HAL_OBJECT_T *this)
{
    ...
    do {
        ...
        u4Val1 = (pt->eAddrMode == B2R_AM_BLOCK1 ? 0x1 : (pt->eAddrMode == B2R_AM_BLOCK4 ? 0x0 : 0x3)); 
        u4Val2 = (pt->eSwapMode == B2R_SM_NO_SWAP? 0x2 : (pt->eSwapMode == B2R_SM_SWAP1 ? 0x5 : 0x6));
        _Hw_Set_DramFormat(id, u4Val1, u4Val2, pt->u4Pitch);
        }
        ...
        u4Val1 = DIVIDE_WITH_CARRIER((i4BlankLine + 4), 8);
...
static UINT32 _B2R_HW_Set(B2R_HAL_OBJECT_T *this, B2R_HAL_TYPE_T eType, VOID *pvArg)
{
    ...
    do {
        ...
        case B2R_HAL_OMUX:
        {
            ...
            if(IS_IC_5890_ES3())
            {
                _Hw_Set_OMux_OryxE3(id, ptArg->ucPath);
            }
            else
#endif
            {
                _Hw_Set_OMux(id, ptArg->ucPath);
            }
        }
            break;
        ...
        case B2R_HAL_PATTERN:
        {
            ...
            break;
        case B2R_HAL_BURST_READ:
        {
            BOOL fgEn = *(BOOL *)pvArg;
            _Hw_Set_Burst_Read(id, fgEn);
            break;
        }
        case B2R_HAL_PITCH:
        {
...
static void _B2R_HW_Read_Status(UINT8 u1Id, B2R_HAL_STATUS_T * ptStatus)
{
    ...
    ptStatus->fgTop     = ((u4Line1 & 0x40) == 0) ? TRUE :FALSE ;
...
static UINT32 _B2R_HW_Do_Frc(B2R_HAL_OBJECT_T *this)
{
    ...
    if (ptFrc->ucCurrentNs == 0)
    {
...
#endif
        {
            ...
            {
                IO_B2R_WRITE_BITS(B2R_R_MAP + u4Offset, NEW_EVEN_REPEAT, 1);
            }
            IO_B2R_WRITE_BITS(B2R_R_MAP + u4Offset, REPEAT_CNT, ptFrc->u2TargetNs);
        }
        ...
        IO_B2R_WRITE_BITS(B2R_R_MAP + u4Offset, REPEAT_H_SHIFT, 1);
    ...
    {
        B2R_HAL_FRC_FIELD_T eCurrentField;
        eCurrentField = (ptStatus->u4CurrentField != 0) ? B2R_FRC_FIELD_B : B2R_FRC_FIELD_T;
        if (eCurrentField == ptFrc->eRepeatField)
        {
            IO_B2R_WRITE_BITS(B2R_MODE_CTRL + u4Offset, FIELD_INV, 1);
        }
        else
        {
            IO_B2R_WRITE_BITS(B2R_MODE_CTRL + u4Offset, FIELD_INV, 0);
        }
    }
...
static void _B2R_HW_V1_ISR(UINT8 u1Id, UINT16 u2Vector)
{
    UINT32 u4Offset;
    BOOL fgISR = FALSE;
    B2R_HAL_OBJECT_T *this;
...
inline static void _B2R_HW_V1_ISR_HD(UINT16 u2Vector)
{
    _B2R_HW_V1_ISR(0, u2Vector);
}

inline static void _B2R_HW_V1_ISR_SD(UINT16 u2Vector)
{
    _B2R_HW_V1_ISR(1, u2Vector);
}
...
//---------------------------------------------------------------------------
#ifdef B2R_EMU
void _B2REmu_HW_V1_Init(UINT8 u1Id)
{
    _Hw_Disable(u1Id);
}

void _B2REmu_HW_V1_ISR(UINT16 u2Vector)
{
    BOOL fgHDISR = FALSE;
    BOOL fgSDISR = FALSE;
...
VOID B2R_HW_V1_Init(VOID)
{
    ...
    for (i = 0; i < HW_NS; i++)
    {
        ptO =  &_b2r_hw_v1_object[i];
        x_snprintf((char*)(ptO->acName), 12, "B2R-V1-%s", (i == 0? "HD":"SD"));
        //x_strncpy((char*)(ptO->acName), "B2R-V1", 7);
        ptO->u1Id      = i;
        ptO->u1Used    = FALSE;
        ptO->fgEnable  = FALSE;
        ptO->pvHost    = NULL;
        ...
        ptO->Isr       = (i == 0) ? _B2R_HW_V1_ISR_HD : _B2R_HW_V1_ISR_SD;
...
B2R_HAL_OBJECT_T * B2R_HW_V1_Get_Object(UINT8 u1Idx)
{
    return (u1Idx < sizeof(_b2r_hw_v1_object)/sizeof(_b2r_hw_v1_object[0])) ? &_b2r_hw_v1_object[u1Idx] : NULL;
}
