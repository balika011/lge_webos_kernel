         ...
         (VLDJPG_GetDecMode5391() == E_JPG_DEC_MODE_BASELINE_PIC_VIDEO_OUTPUT))
{
	...
	if (prMemBase->fgProgScan)
	{
        ...
        #ifdef __MODEL_slt__
        prFbmPool = (FBM_POOL_T*)VIRTUAL(FBM_GetPoolInfo((UINT8)FBM_POOL_TYPE_JPEG)); 
        #else
	...
	else
	{
	    ...
	    //prFbmPool = FBM_GetPoolInfo((UINT8)FBM_POOL_TYPE_JPEG); //for mt5372, output 64 M bound restriction
        #ifdef __MODEL_slt__  
        prMemBase->pvFreePoint  =(void*)VIRTUAL((void *) DRVCUST_OptGet(eFbmMemAddr));
        #else
...
if((VLDJPG_GetDecMode5391() == E_JPG_DEC_MODE_BASELINE_PIC_RZ)||(VLDJPG_GetDecMode5391() == E_JPG_DEC_MODE_BASELINE_PIC_VIDEO_OUTPUT))
{
...
else
{
    ...
    HalFlushInvalidateDCacheMultipleLine((UINT32)(prMemYuv->pvComp[0]), (UINT32)(prMemYuv->au4AllocSize[0] + prMemYuv->au4AllocSize[1] + prMemYuv->au4AllocSize[2]));
#else
  ...
  if(_JpgVerifyCmdGetParam(18,0))
  	{
	    ...
	    MID_SetRegionEx(2, MID_AGENT_JPEG, 0,                                                                                                                                              
...
#if defined(CC_MT5365) || defined(CC_MT5395) ||defined(JPEG_IS_POST_MT5368)
    HalFlushInvalidateDCacheMultipleLine((UINT32)(prMemYuv->pvCoefBuffer[0]), (UINT32)((prMemYuv->au4AllocSize[0] << 1) + (prMemYuv->au4AllocSize[1] << 1) + (prMemYuv->au4AllocSize[2] << 1)));
    HalFlushInvalidateDCacheMultipleLine((UINT32)(prMemBase->apvNzBuffer[0]), (UINT32)((prMemYuv->au4AllocSize[0] >> 3) + (prMemYuv->au4AllocSize[1] >> 3) + (prMemYuv->au4AllocSize[2] >> 3)));
#else
    HalFlushInvalidateDCache();
#endif
...
static void _jmem_MwInit		(JPEGHANDLE hMem)
{
    ...
    else
    {
        ...
        else if(VLDJPG_GetDecMode5391() == E_JPG_DEC_MODE_BASELINE_PIC_VIDEO_OUTPUT)
        {
            ...
            VLDJPG_SetDecMode5391((INT32)E_JPG_DEC_MODE_BASELINE_PIC_VIDEO_OUTPUT);
        }
        else
        {
