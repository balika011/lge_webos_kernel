...
static void _FVRPacketCountInt(UINT8 u1Pidx, UINT32 u4PacketAddr)
{
    ...
    UINT8 u1Idx;
    UINT8 u1RemuxFlag = 0x0;
    prPid = _FVR_GetPidStruct(u1Pidx);
    u1Idx = prPid->u1TsIndex;
    /*for ci+1.4 ,check this idx is as remux input source ,then use the remux record*/
    u1RemuxFlag = (FVR_GBL_ARY_W(4,9)>>8)&0xF;
	if((u1RemuxFlag&(0x1<<u1Idx)) != 0)
	{
	  u1Idx = 4; //use remux record buffer.
	}
    ...
    if (prGBL->pfnNotifyFunc != NULL)
    {
     ...
     LOG(1,"Pkt Isr:Ts_idx:%d,Addr:0x%x,Size:0x%x,Pkt Count:0x%x.\r\n",u1Idx,u4PktRp,u4DataLen,u4PktCount);
#endif
 ...
 */
//-----------------------------------------------------------------------------
static void _FVRPicSearchInt(UINT8 u1Pidx, UINT16 u2Pattern, UINT32 u4PktCount, UINT64 u8Pts33)
{
    FVR_PID_STRUCT_T* prPid;
    ...
    rPicInfo.u1Pidx = u1Pidx;
    rPicInfo.u2PicType = u2Pattern;
    rPicInfo.u4PktCount = u4PktCount;
    rPicInfo.u8Pts33 = u8Pts33;
    _FVR_SendPictureEvent(&rPicInfo);
}
...
static void _FVRHandleTimer(UINT8 u1Idx, UINT8 u1SerialNum, UINT32 u4EndAddr)
{
    ...
    _FVR_Lock();
    u1DmemSerial = (UINT8)(FVR_GBL_SERIALNUM_ADDR & 0xFF);
    _FVR_Unlock();
    ...
    {
        _FVR_Lock();
        u4PktCount = FVR_GBL_PKTCOUNTER_TS0;
        u4DmemEndAddr = FVR_GBL_DMEMENDADDR_TS0;
        _FVR_Unlock();
    ...
    {
        _FVR_Lock();
        u4PktCount = FVR_GBL_PKTCOUNTER_TS1;
        u4DmemEndAddr =  FVR_GBL_DMEMENDADDR_TS1;
        _FVR_Unlock();
    }
    else if (u1Idx == 2)
    {
        _FVR_Lock();
        u4PktCount = FVR_GBL_PKTCOUNTER_TS2;
        u4DmemEndAddr =  FVR_GBL_DMEMENDADDR_TS2;
        _FVR_Unlock();
    }
    else if (u1Idx == 3)
    {
        _FVR_Lock();
        u4PktCount = FVR_GBL_PKTCOUNTER_TS3;
        u4DmemEndAddr =  FVR_GBL_DMEMENDADDR_TS3;
        _FVR_Unlock();
    }
	 else if (u1Idx == 4)
    {
        _FVR_Lock();
        u4PktCount = FVR_GBL_PKTCOUNTER_CI14;
        u4DmemEndAddr = FVR_GBL_DMEMENDADDR_CI14;
        _FVR_Unlock();
    }
    else
    {
    ...
    _FVR_SendTimerPktEvent(&rTimeInfo);
#else
    if (prGBL->pfnNotifyFunc != NULL)
    {
...
#ifdef CC_DMX_EMULATION
    LOG(1,"Timer Isr:Ts_idx:%d,Addr:0x%x,Size:0x%x,Pkt Count:0x%x.\r\n",u1Idx,u4PktRp,u4DataLen,u4PktCount);
#endif
...
static void _FVRTimerPktInt(UINT8 u1SerialNum, UINT32 u4EndAddr)
{
    ...
    if (_FVR_IsRecording(2))
    {
        _FVRHandleTimer(2, u1SerialNum, u4EndAddr);
    }
	...
	if (_FVR_IsRecording(4))
	{
	    _FVRHandleTimer(4, u1SerialNum, u4EndAddr);
	}
}
...
//-----------------------------------------------------------------------------
/** _FVRRawScrambleChange
 *
 *  @retval void
 */
//-----------------------------------------------------------------------------
static void _FVRRawScrambleChange(UINT8 u1Pidx, BOOL fgScrambled)
{
    FVR_PID_STRUCT_T* prPid;
    FVR_SCRAMBLE_STATE_T eState;
    ...
    LOG(3, "%s, Pidx %u raw scrambling state: %s\n", __FUNCTION__, u1Pidx, fgScrambled ? "SCRAMBLED" : "CLEAR");

    if (prPid->fgDetectScramble && (prPid->pfnDetectRawScrambleNotify != NULL))
    {
        prPid->pfnDetectRawScrambleNotify(u1Pidx, eState);
    }
} 
 ...
 */
//-----------------------------------------------------------------------------
static void _FVRPacketSitInt(UINT8 u1TsIdx, UINT8 u1SitPkt, UINT32 u4SitWp)
{
    FVR_SIT_PKTREPLACE_INFO_T rSitInfo;

    if(u1TsIdx >= FVR_SINGLE_STREAM_GBL_COUNT)
    {
        return;
    }
    ...
    {
        UINT32 i;
        LOG(2, "SWAP Int - Idx=%u, PktCnt=%u, Wp=0x%x\n", u1TsIdx, u1SitPkt, u4SitWp);
        
        for (i = 0; i < u1SitPkt; i++)
        {
            LOG(2, "Addr=0x%08x\n", FVR_SIT_ARY_W(u1TsIdx, i));
        }
    }
#endif

    rSitInfo.u1TsIndex = u1TsIdx;
    rSitInfo.u1SitPktCnt = u1SitPkt;
    rSitInfo.u4SitWp = u4SitWp;
    _FVR_SendSitCompleteEvent(&rSitInfo);
...
BOOL _FVR_DispatchuPInterrupt(UINT32 u4Status, UINT32 u4Status2)
{
    ...
    if (!_FVR_IsRecording(0) && !_FVR_IsRecording(1) && !_FVR_IsRecording(2) &&
		!_FVR_IsRecording(3)&&!_FVR_IsRecording(4))
    {
        // Clear interrupt
        return TRUE;
    }
    ...
    case 65:    // I,P,B notification
...

        {
            ...
            u8Pts33 = FVR_PER_PID_S_W(u1Pidx, 1);
            u8Pts33 = ((UINT64)(FVR_PER_PID_S_W(u1Pidx, 2) & 0xFF) << 32) | u8Pts33;

            _FVRPicSearchInt(u1Pidx, u2Pattern, u4Status2, u8Pts33);
        }
    ...
    case 78:
        ...
        _FVRRawScrambleChange(u1Pidx, fgScrambled);
        break;

    case 70:
        //  Inserting SIT packet completes  [16- 23] tsindex, 8-15 sit packet counter
        _FVRPacketSitInt(u1Pidx, u1Pattern, u4Status2);
        break;
