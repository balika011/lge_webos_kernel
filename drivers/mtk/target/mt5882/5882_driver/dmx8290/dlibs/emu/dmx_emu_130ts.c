...
static BOOL _FVREmuSetPVRPlayEx(BOOL u1TsoutIdx, UINT32 u4SrcAddr,UINT32 u4DataSize)
{
 ...
 */
//-----------------------------------------------------------------------------
BOOL DMX_Emu_Tsout130_TransmitData( BOOL fgExtSync ,UINT8 u1TsIndex, UINT8 u1TsoutIdx)
{
    ...
    //Set pvr play back
    VERIFY(_FVREmuSetPVRPlayEx(u1TsoutIdx, _u4TsSourceBufAddr ,_u4TsSourceBufSize));
    ...
    VERIFY(DMX_PVRPlay_SetBufferEx(u1TsoutIdx, _u4TsSourceBufAddr, u4BufEnd - 1,
    ...
    if (!_DMX_PVRPlay_130byteEnable(u1TsoutIdx, TRUE , 130))
    {
        Printf("Set 130 byte enable returns FALSE!\n");
        goto FailAndFreeMemory;
    }
    ...
    if (fgExtSync)
    {
        _DMX_SetFramerMode(u1TsIndex, DMX_FRAMER_SERIAL, TRUE, TRUE);
    }
    else
    {
        _DMX_SetFramerMode(u1TsIndex, DMX_FRAMER_SERIAL, FALSE, TRUE);
    }
...
BOOL DMX_EMU_Multi_PacketSize_Test( BOOL fgFramer0Reset, BOOL fgFramer1Reset, UINT8 u1TsoutIdx)
{
    BOOL fgRet = FALSE;
    ...
    //Set pvr play back
    VERIFY(_FVREmuSetPVRPlayEx(u1TsoutIdx, u4SrcAddr ,u4SrcBufSize));

    VERIFY(_FVREmuSetPlayPid(u1TsIndex,u2Pid , u4DstAddr , u4DstBufSize));

    VERIFY(_FVREmuSetTimestamp(u1TsoutIdx, FALSE));

    VERIFY(DMX_PVRPlay_SetBufferEx(u1TsoutIdx, u4SrcAddr, u4SrcAddr + u4Size + 0x500 - 1,
    ...
    if (!_DMX_PVRPlay_130byteEnable(u1TsoutIdx, FALSE, 188))
    {
        Printf("Set 130 byte enable returns FALSE!\n");
        goto FailAndFreeMemory;
    }
