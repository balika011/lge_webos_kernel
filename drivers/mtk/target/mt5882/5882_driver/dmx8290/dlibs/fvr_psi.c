...
static BOOL _FVRUpdateReadPointer(UINT8 u1Pidx, UINT32 u4Rp, UINT32 u4FrameAddr)
{
    ...
    // Debug
    DMXPIDLOG(DMX_LOG_RECORD 7, u1Pidx, "Update RP - pidx: %u, RP: 0x%08x, frame: 0x%08x\n", u1Pidx, u4Rp, u4FrameAddr);
    ...
    UINT32 u4FrameAddr)
{
    ...
    DMXPIDLOG(DMX_LOG_RECORD 6, u1Pidx, "Unlock PSI buffer: pidx %u, frame: 0x%08x, size: %u\n", u1Pidx,
        u4FrameAddr, u4Size);
...
BOOL _FVR_PSI_FlushBuffer(UINT8 u1Pidx)
{
    ...
    DMXPIDLOG(DMX_LOG_RECORD 6, u1Pidx, "Flush PSI buffer: pidx %u\n", u1Pidx);
