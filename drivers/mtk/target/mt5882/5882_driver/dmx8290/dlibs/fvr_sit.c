...
// Static variables
//-----------------------------------------------------------------------------
static FVR_SIT_PATTERN_T _arSitPattern[FVR_SINGLE_STREAM_GBL_COUNT];

static HANDLE_T _hSitCompleteSema[FVR_SINGLE_STREAM_GBL_COUNT] = {NULL_HANDLE};

static HANDLE_T _hSitCmdThread[FVR_SINGLE_STREAM_GBL_COUNT] = {NULL_HANDLE};
static HANDLE_T _hSitCmdMsgQ[FVR_SINGLE_STREAM_GBL_COUNT] = {NULL_HANDLE};
static HANDLE_T _hSitCmdDoneSema[FVR_SINGLE_STREAM_GBL_COUNT] = {NULL_HANDLE};
static HANDLE_T _hSitCmdTimer[FVR_SINGLE_STREAM_GBL_COUNT] = {NULL_HANDLE};
...
static void _FvrSitResetInsertedPacketCount(UINT8 u1TsIdx)
{
    ...
    ASSERT(u1TsIdx < FVR_SINGLE_STREAM_GBL_COUNT);
...
static void _FvrSitDisableInsert(UINT8 u1TsIdx)
{
    ...
    _FVR_Lock();
    u4Reg = FVR_GBL_BASE_W(FVR_GBL_COMMON_REGION, 13);
    u4Reg &=  ~(0xffu << 16);
    FVR_GBL_BASE_W(FVR_GBL_COMMON_REGION, 13) = u4Reg;
...
BOOL _FVR_SitSetPattern(UINT8 u1TsIdx, UINT16 u2Pid, const UINT8 *pu1Data, UINT16 u2Len)
{
    ...
    if ((u1TsIdx >= FVR_SINGLE_STREAM_GBL_COUNT) || !pu1Data)
    {
        fgRet = FALSE;
        goto lbExit1;
    }
