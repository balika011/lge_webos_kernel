...
extern int xhci_add_endpoint(struct usb_device *udev, struct usb_endpoint_descriptor* epdesc);
...
again:         
    /* GJ */
    x_memset(usb_stor_buf, 0, sizeof(usb_stor_buf));
    ...
    for(i=0;i<USB_MAX_STOR_DEV;i++) {
        x_memset(&usb_dev_desc[i],0,sizeof(block_dev_desc_t));
...
int usb_stor_BBB_comdat(ccb *srb, struct us_data *us)
{
    ...
    /* DST SRC LEN!!! */
    x_memset(cbw.CBWCDB,0,16);
    x_memcpy(cbw.CBWCDB, srb->cmd, srb->cmdlen);
...
int usb_storage_probe(struct usb_device *dev, unsigned int ifnum,struct us_data *ss)
{
    ...
    x_memset(ss, 0, sizeof(struct us_data));
    ...
    for (i = 0; i < iface->bNumEndpoints; i++) 
    {
                ...
                == USB_ENDPOINT_XFER_BULK) 
        {
            ...
            if(xhci_add_endpoint(dev, &(iface->ep_desc[i])) != 0)
            {
                return -1;
            }
        }
            ...
            == USB_ENDPOINT_XFER_INT) 
        {
            ...
            Printf("Should not use interrupt device\n");
            ASSERT(0);
        }
    }
    USB_STOR_PRINTF("Endpoints In %d Out %d Int %d\n", ss->ep_in, ss->ep_out, ss->ep_int);

    if(xhci_check_bandwidth(dev) != 0)
    {
        Printf("xhci_check_bandwidth fail\n");
        return -1;
    }
...
int usb_stor_get_info(struct usb_device *dev,struct us_data *ss,block_dev_desc_t *dev_desc)
{
    ...
    x_memcpy(&dev_desc->vendor[0], &usb_stor_buf[8], 8);
    x_memcpy(&dev_desc->product[0], &usb_stor_buf[16], 16);
    x_memcpy(&dev_desc->revision[0], &usb_stor_buf[32], 4);
    dev_desc->vendor[8] = 0;
    ...
    if(result < 0) {
        ...
        cap[0] = 0;
        cap[1] = 0;
    x_memset(pccb->pdata,0,8);
    if(usb_read_capacity(pccb,ss) != 0) {
        Printf("READ_CAP ERROR\n");
        cap[0] = 2880;
        cap[1] = 0x200;
    }
    x_memcpy((void *)&cap[0], (const void *)&usb_stor_buf[0], 8);   
...
unsigned long usb_stor_getblksize(int lun)
{
    if(lun >= usb_max_lun)
    {
        return 0;
    ...
    return (unsigned long)usb_dev_desc[lun].blksz;
}

unsigned long usb_stor_getblknum(int lun)
{
    if(lun >= usb_max_lun)
    {
        return 0;
    ...
    return (unsigned long)usb_dev_desc[lun].lba;
}
