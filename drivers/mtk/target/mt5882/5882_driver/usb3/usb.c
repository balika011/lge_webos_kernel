...
extern int xhci_alloc_dev(struct usb_device *pudev);
extern int xhci_address_device(struct usb_device *pudev);
extern int xhci_add_endpoint(struct usb_device *pudev, struct usb_endpoint_descriptor* epdesc);
extern int xhci_check_bandwidth(struct usb_device *udev);
...
//    UNUSED(list_entry(pr_list, test_command, tlist));
...
int usb_electrical_test(char *argv)
{
    ...
    if(result==0) {
        if (x_strncmp(argv, "se0nak", 6) == 0){
            return usb_sendse0nak();
        }

        if (x_strncmp(argv, "testj", 5) == 0){
            return usb_sendtestj();
        }

        if (x_strncmp(argv, "testk", 5) == 0){
            return usb_sendtestk();
        }

        if (x_strncmp(argv, "testpkt", 7) == 0){
...
/*  
    ...
    while(timeout--) {
        if(!((unsigned long)dev->status & USB_ST_NOT_PROC))
            break;
        wait_ms(1);
    }
*/  
...
int usb_set_maxpacket(struct usb_device *dev)
{
    ...
    for(i=0; i<dev->config.bNumInterfaces;i++) {
        for(ii=0; ii<dev->config.if_desc[i].bNumEndpoints; ii++) {
            ...
            x_memcpy((void *)&rep, (const void *)&dev->config.if_desc[i].ep_desc[ii], 
...
int usb_parse_config(struct usb_device *dev, unsigned char *buffer, int cfgno)
{
    ...
    x_memcpy(&dev->config,buffer,buffer[0]);
    ...
    while(index+1 < dev->config.wTotalLength) {
        switch(head->bDescriptorType) {
            case USB_DT_INTERFACE:
                ...
                x_memcpy(&dev->config.if_desc[ifno],&buffer[index],buffer[index]); /* copy new desc */
            ...
            case USB_DT_ENDPOINT:
                ...
                x_memcpy(&dev->config.if_desc[ifno].ep_desc[epno],&buffer[index],buffer[index]);
...
int usb_new_device(struct usb_device *dev)
{
    ...
    err = xhci_alloc_dev(dev); 
    err = xhci_address_device(dev);  /* set address */
    if (err < 0) {
    ...
    x_memset(dev->mf, 0, sizeof(dev->mf));
    x_memset(dev->prod, 0, sizeof(dev->prod));
    x_memset(dev->serial, 0, sizeof(dev->serial));
...
int usb_hub_configure(struct usb_device *dev)
{
    ...
    x_memcpy((unsigned char *)&hub->desc,buffer,descriptor->bLength);
    /* adjust 16bit values */
    hub->desc.wHubCharacteristics=swap_16(descriptor->wHubCharacteristics);
    /* set the bitmap */
    bitmap=(unsigned char *)&hub->desc.DeviceRemovable[0];
    x_memset(bitmap,0xff,(USB_MAXCHILDREN+1+7)/8); /* devices not removable by default */
    bitmap=(unsigned char *)&hub->desc.PortPowerCtrlMask[0];
    x_memset(bitmap,0xff,(USB_MAXCHILDREN+1+7)/8); /* PowerMask = 1B */
