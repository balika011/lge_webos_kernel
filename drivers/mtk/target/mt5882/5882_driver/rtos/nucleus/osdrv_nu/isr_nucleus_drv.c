...
static void DriverCommonLISR(INT i4Vector)
{
    ...
    #ifdef CC_ARM_GIC
    else if((i4Vector == VECTOR_MISC) || (i4Vector == VECTOR_MISC2))
    {
        VERIFY(BIM_DisableIrq((UINT32)i4Vector));
        if(_arIsrList[i4Vector].pfIsr != NULL)
        {
            _arIsrList[i4Vector].pfIsr((UINT16)(i4Vector)); // call MISC/MISC2 LISR
        }
        VERIFY(BIM_EnableIrq((UINT32)i4Vector));
    }
    #endif
    else
    {
...
#ifdef CC_ARM_GIC
/*----------------------------------------------------------------------------
 * Function: DriverMiscLISR
 *
 * Description:
 *      The misc LISR for registered user ISR.
 *      1. Disable interrupt of current vector
 *      2. Activate the common HISR
 *
 * Inputs: -
 ...
 * Returns: -
 *---------------------------------------------------------------------------*/
static void DriverMiscLISR(UINT16 ui2_vector_id)
{
    UINT32 i, u4Val;
    INT32 i4Vector;
...
static void DriverMisc2LISR(UINT16 ui2_vector_id)
{
    UINT32 i, u4Val;
    INT32 i4Vector;
                 ...
                 x_os_drv_isr_fct   *ppf_old_isr)
{
    ...
    if (ui2_vec_id >= VECTOR_MISC2_BASE)
    {
        ...
        {
            #ifdef CC_ARM_GIC
            x_os_drv_reg_isr(VECTOR_MISC2, DriverMisc2LISR, &pf_old_isr);
            UNUSED(DriverMisc2HISR);
            #else
    ...
    if (ui2_vec_id > MAX_LISR_VECTOR)
    {
        ...
        {
            #ifdef CC_ARM_GIC
            x_os_drv_reg_isr(VECTOR_MISC, DriverMiscLISR, &pf_old_isr);
            UNUSED(DriverMiscHISR);
            #else
