...
static INT32 _MSDCCLI_Read_BS(INT32 i4Argc, const CHAR ** szArgv)
{
    INT32 i4Ret = 0;
...
//                           u4fgDev?"(Dev)":"", u4DstPtr, u4Offset, u4Len);
    ...
    else
    {
        	do
        {
        	  i4Ret = MsdcReadCard(u4Offset, (UINT32 *)u4DstPtr, u4BS);
        	  u4templen += u4BS;
        	  u4DstPtr += u4BS;
...
static INT32 _MSDCCLI_Read_BS1M(INT32 i4Argc, const CHAR ** szArgv)
{
    INT32 i4Ret = 0;
    UINT32 u4Offset, u4Len, u4DstPtr;
    UINT32 u4BS = 0;
   ...
   //Printf("_MSDCCLI_Read\n");
    ...
    u4BS = 1024*1024; //StrToInt(szArgv[4]);
    if(u4BS % 512)
    	{
    		Printf("bs size should be 512 byte align\n");
    	}
...
//                           u4fgDev?"(Dev)":"", u4DstPtr, u4Offset, u4Len);
    ...
    else
    {
     ...
     HAL_GetTime(&rMsdTimeEnd);
    	HAL_GetDeltaTime(&rMsdTimeDelta, &rMsdTimeStart, &rMsdTimeEnd);
    U4Speed = (UINT64)(((UINT64)u4Len  * 1000000 / (1024*1024)) / (UINT64)(rMsdTimeDelta.u4Seconds * 1000000 + rMsdTimeDelta.u4Micros));
    LOG(0,"Read Speed is %d MB/S,read size %d\n", U4Speed, u4Len);
...
static INT32 _MSDCCLI_PartitionWriteProtect(INT32 i4Argc, const CHAR ** szArgv)
{
    UINT32 u4WPconfig;

    if ((i4Argc < 2) || (i4Argc > 2))
    {
        LOG(0, "%s [0 disable,1 enable)] \n", szArgv[0]);
        return 0;
    }
    
    u4WPconfig = StrToInt(szArgv[1]);
    MSDCPART_WpConfig(u4WPconfig);
    return 0;
}
#endif
...
static INT32 _MSDCCLI_HS200Tuning(INT32 i4Argc, const CHAR ** szArgv)
{
    MSDCTest_HS200Tuning(0);
    return 0;
}

static INT32 _MSDCCLI_HS200Calibration(INT32 i4Argc, const CHAR ** szArgv)
{
    UINT32 offset, size;

    if (i4Argc != 3)
    {
        LOG(0, "%s [msdc_offset] [size]\n", szArgv[0]);
        return 0;
    }

    offset = StrToInt(szArgv[1]);
    size = StrToInt(szArgv[2]);
	if(size > 0x8000000)
	{
        LOG(0, "Too big size for test!\n");
		return -1;
	}

    MSDCTest_HS200Calibration(offset, size);
    return 0;
}

static INT32 _MSDCCLI_DDR50Calibration(INT32 i4Argc, const CHAR ** szArgv)
{
    UINT32 offset, size;
    ...
    MSDCTest_DDR50Calibration(offset, size);
    return 0;
}
static INT32 _MSDCCLI_SDR50Calibration(INT32 i4Argc, const CHAR ** szArgv)
{
    UINT32 offset, size;
...
{
#ifdef CC_MSDC_ENABLE
    DECLARE_G_CMD(_MSDCCLI_HostSet, hostset, hs, "msdc.hs"),
	  DECLARE_G_CMD(_MSDCCLI_HostGet, hostget, hg, "msdc.hg"),
    DECLARE_G_CMD(_MSDCCLI_Pinmux, pinmux, pnx, "msdc.pinmux"),
    DECLARE_G_CMD(_MSDCCLI_SwitchVoltage, switchvoltage, sv, "msdc.sv"),
    DECLARE_G_CMD(_MSDCCLI_SysInit, init, init, "msdc.init"),
    DECLARE_G_CMD(_MSDCCLI_Identify, identify, id, "msdc.identify"),
    DECLARE_G_CMD(_MSDCCLI_SetDataMode, setdatamode, sdm, "msdc.setdatamode"),
    DECLARE_G_CMD(_MSDCCLI_SetDataEndMode, setdataendmode, sdem, "msdc.setdataendmode"),
    DECLARE_G_CMD(_MSDCCLI_SetDataBusWidth, setbuswidth, sbw, "msdc.setbuswidth"),
    DECLARE_G_CMD(_MSDCCLI_SetSpeedMode, setspeedmode, ssm, "msdc.setspeedmode"),
    DECLARE_G_CMD(_MSDCCLI_SetDDRMode, setddrmode, sdddrm, "msdc.setddrmode"),
    DECLARE_G_CMD(_MSDCCLI_SetBusClock, setbusclock, sbc, "msdc.setbusclock"),
    DECLARE_G_CMD(_MSDCCLI_PrintConfigInfo, configinfo, pci, "msdc.configinfo"),
    DECLARE_G_CMD(_MSDCCLI_SetContinueClock, setcontinueclock, scc, "msdc.setcontinueclock"),
    DECLARE_G_CMD(_MSDCCLI_GetRegisters, getregisters, gr, "msdc.getregisters"),
    DECLARE_G_CMD(_MSDCCLI_GetDevInfo, getdevinfo, gdi, "msdc.getdevinfo"),
    DECLARE_G_CMD(_MSDCCLI_Read, read, r, "msdc.read"),	
    DECLARE_G_CMD(_MSDCCLI_Read_BS1M, read1M, r1M, "msdc.read1M"),	
    DECLARE_G_CMD(_MSDCCLI_Read_BS, readBS, rBS, "msdc.readBS"),
...
#ifdef CC_PARTITION_WP_SUPPORT
    DECLARE_CMD(_MSDCCLI_PartitionWriteProtect, partwriteprotect, pwp, "msdc.pwriteprotect"),
#endif
    DECLARE_G_CMD(_MSDCCLI_Measure, measure, m, "msdc.measure"),
    DECLARE_G_CMD(_MSDCCLI_PartInfo, partinfo, partinfo, "msdc.partinfo"), 
    DECLARE_G_CMD(_MSDCCLI_HS200Tuning, hs200tuning, hs200t, "msdc.hs200tuning"),
