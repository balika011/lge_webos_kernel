...
INT32 GCPU_EMU_SHA1()
{
    ...
    u4GoldenFileSize = x_strlen(pstrsha1);
    u4BufSize = _DMX_Align(u4GoldenFileSize, GCPU_FIFO_ALIGNMENT);
    u4SrcOffset = rand() % u4BufSize;
    pbSrcBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    if (pbSrcBufAddr == 0)
    {
        ASSERT(0);
    }
...
INT32 GCPU_EMU_MD5()
{
    ...
    u4GoldenFileSize = x_strlen(pstrmd5);
    u4BufSize = _DMX_Align(u4GoldenFileSize, GCPU_FIFO_ALIGNMENT);
    u4SrcOffset = rand() % u4BufSize;
    pbSrcBufAddr = BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    if (pbSrcBufAddr == 0)
...
static BOOL GCPU_EMU_AES_SWCrypt(const UINT8 *pu1Key, KEY_BIT_LEN eKeyLen, const UINT8 au1IV[16],
                              UINT8 *pu1Src, UINT8 *pu1Dst, INT32 i4Length,
                              BLOCK_CIPHER_MODE eMode, BOOL fgEncrypt)
{
    aes_context ctx;
   	UINT8 au1InitVector[16];
    INT32 i4KeySize, i4Mode;

    switch (eKeyLen)
    {
    case KEY_BIT_LEN_128:
        i4KeySize = 128;
        break;
    case KEY_BIT_LEN_192:
        i4KeySize = 192;
        break;
    case KEY_BIT_LEN_256:
        i4KeySize = 256;
        break;
    default:
        Printf("Key size is not 128, 192, or 256 bits\n");
        return FALSE;
    }

    //if (((UINT32)pu1Src & 0xF) || ((UINT32)pu1Dst & 0xF) || (i4Length & 0xF))
    if (i4Length & 0xF)
    {
        Printf("AES buffer address/size is not 16-byte alignment\n");
        return FALSE;
    }

    if ((pu1Key == NULL) || (pu1Src == NULL) || (pu1Dst == NULL))
	{
		Printf("Ivalid parameters!\n");
		return FALSE;
	}

    if ((eMode == BLOCK_CIPHER_MODE_CBC) && (au1IV == NULL))
    {
		Printf("Initialization vector is not set!\n");
        return FALSE;
    }
    ...
    {
        i4Mode = AES_ENCRYPT;
        aes_setkey_enc( &ctx, pu1Key, i4KeySize);
    }
    else
    {
        i4Mode = AES_DECRYPT;
        aes_setkey_dec( &ctx, pu1Key, i4KeySize);
    }

    if (eMode == BLOCK_CIPHER_MODE_ECB)
    {
        while (i4Length > 0)
        {
            aes_crypt_ecb(&ctx, i4Mode, pu1Src, pu1Dst);
    ...
    else if (eMode == BLOCK_CIPHER_MODE_CBC)
    {
        x_memcpy(au1InitVector, au1IV, 16);
        aes_crypt_cbc(&ctx, i4Mode, i4Length, au1InitVector, pu1Src, pu1Dst);
    }
    else
    {
    }
...
static BOOL GCPU_EMU_AES_HWCrypt(const UINT8 *pu1Key, KEY_BIT_LEN eKeyLen, UINT8 au1IV[16],
                              UINT8 *pu1Src, UINT8 *pu1Dst, INT32 i4Length,
                              BLOCK_CIPHER_MODE eMode, BOOL fgEncrypt)
{
	AES_PARAM_T rParam;
	...
	for (i = 0; i < (i4KeySize >> 3); i++)
    {
        rParam.au1Key[i] = *(pu1Key + i);
    }

    x_memcpy(rParam.au1Iv, au1IV, 16);

    rParam.eMode = eMode;
    rParam.eKeyBitLen = eKeyLen;
    rParam.u4SrcStartAddr = (UINT32)pu1Src;
    rParam.u4SrcBufStart = GCPU_LINER_BUFFER_START(((UINT32)pu1Src));
    rParam.u4SrcBufEnd = GCPU_LINER_BUFFER_END(((UINT32)pu1Src + i4Length));
    rParam.u4DstStartAddr = (UINT32)pu1Dst;
    rParam.u4DstBufStart = GCPU_LINER_BUFFER_START(((UINT32)pu1Dst));
    rParam.u4DstBufEnd = GCPU_LINER_BUFFER_END(((UINT32)pu1Dst + i4Length));
    rParam.u4DatLen = i4Length;
    rParam.fgEncrypt = fgEncrypt;
...
{
    UINT8 *pu1DataBufAddr = NULL;
    UINT8 *pu1CipherBufAddr = NULL;
    UINT8 *pu1PlainBufAddr = NULL;
    UINT32 u4TestSize, u4BufSize, i;
    HAL_TIME_T rStartTime, rEndTime;
    HAL_TIME_T rEncTimeDiff, rDecTimeDiff1, rDecTimeDiff2;
    UINT8 au1Key[32], au1IV[16];
    BOOL fgResult;

    u4BufSize = 0x800000;    // 8MB
    pu1DataBufAddr = (UINT8 *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    if (pu1DataBufAddr == 0)
    {
        ASSERT(0);
    }

    pu1CipherBufAddr = (UINT8 *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    if (pu1CipherBufAddr == 0)
    {
        ASSERT(0);
    }

    pu1PlainBufAddr = (UINT8 *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    if (pu1PlainBufAddr == 0)
    {
        ASSERT(0);
    }
    ...
    {
        *(UINT8 *)(pu1DataBufAddr + i) = (UINT8)random(0x100);
    }

    x_memset( au1Key, 0, 32 );
    x_memset( au1IV, 0, 16 );

    // AES-ECB-128
    for(i = 4096; i <= u4BufSize; i *= 2)
    {
        u4TestSize = i;
        Printf("AES-ECB-128 Size: 0x%x Bytes\n", u4TestSize);

        //
        //// SW evaluation
        //
        x_memset( pu1CipherBufAddr, 0, u4TestSize);
        x_memset( pu1PlainBufAddr, 0, u4TestSize);

        // get start time of encryption
        HAL_GetTime(&rStartTime);

        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_128, NULL,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
            goto lbExit1;
        }

        // get end time of encryption
        HAL_GetTime(&rEndTime);
        HAL_GetDeltaTime(&rEncTimeDiff, &rStartTime, &rEndTime);

        // get start time of decryption
        HAL_GetTime(&rStartTime);

        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_128, NULL,
                                pu1CipherBufAddr, pu1PlainBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, FALSE /* decrypt */);
        if (fgResult == FALSE)
        {
            goto lbExit1;
        }
        ...
        // get end time
        HAL_GetTime(&rEndTime);
        HAL_GetDeltaTime(&rDecTimeDiff1, &rStartTime, &rEndTime);

        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-ECB-128 failed at %d!\n", __LINE__);
            while(1);
        }

        // evaluate same src and dst
        HAL_GetTime(&rStartTime);

        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_128, NULL,
                                pu1CipherBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, FALSE /* decrypt */);
        if (fgResult == FALSE)
        {
            goto lbExit1;
        ...
        // get end time
        HAL_GetTime(&rEndTime);
        HAL_GetDeltaTime(&rDecTimeDiff2, &rStartTime, &rEndTime);

        if (x_memcmp(pu1DataBufAddr, pu1CipherBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-ECB-128 failed at %d!\n", __LINE__);
            while(1);
        }

        Printf("SW Take time (encrypt, decrypt1, decrypt2): %u.%06u %u.%06u %u.%06u, ",
                rEncTimeDiff.u4Seconds, rEncTimeDiff.u4Micros,
                rDecTimeDiff1.u4Seconds, rDecTimeDiff1.u4Micros, rDecTimeDiff2.u4Seconds, rDecTimeDiff2.u4Micros);
        Printf("Speed: %u MB/s %u MB/s %u MB/s\n",
            u4TestSize / 1024 * 1000 / 1024 * 1000 / (rEncTimeDiff.u4Seconds * 1000000 + rEncTimeDiff.u4Micros),
            u4TestSize / 1024 * 1000 / 1024 * 1000 / (rDecTimeDiff1.u4Seconds * 1000000 + rDecTimeDiff1.u4Micros),
            u4TestSize / 1024 * 1000 / 1024 * 1000 / (rDecTimeDiff2.u4Seconds * 1000000 + rDecTimeDiff2.u4Micros));
        ...
        fgResult = GCPU_EMU_AES_HWCrypt(au1Key, KEY_BIT_LEN_128, NULL,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("HW-AES-ECB-128 failed at %d!\n", __LINE__);
            while(1);
        }
        ...
        Printf("HW Take time (encrypt, decrypt1, decrypt2): %u.%06u %u.%06u %u.%06u, ",
                rEncTimeDiff.u4Seconds, rEncTimeDiff.u4Micros,
                rDecTimeDiff1.u4Seconds, rDecTimeDiff1.u4Micros, rDecTimeDiff2.u4Seconds, rDecTimeDiff2.u4Micros);
        Printf("Speed: %u MB/s %u MB/s %u MB/s\n",
            u4TestSize / 1024 * 1000 / 1024 * 1000 / (rEncTimeDiff.u4Seconds * 1000000 + rEncTimeDiff.u4Micros),
    ...
    {
        u4TestSize = i;
        Printf("AES-ECB-192 Size: 0x%x Bytes\n", u4TestSize);
        ...
        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_192, NULL,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-ECB-192 failed at %d!\n", __LINE__);
            while(1);
        }
        ...
        fgResult = GCPU_EMU_AES_HWCrypt(au1Key, KEY_BIT_LEN_192, NULL,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("HW-AES-ECB-192 failed at %d!\n", __LINE__);
            while(1);
        }
    ...
    {
        u4TestSize = i;
        Printf("AES-ECB-256 Size: 0x%x Bytes\n", u4TestSize);
        ...
        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_256, NULL,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-ECB-256 failed at %d!\n", __LINE__);
            while(1);
        }
        ...
        fgResult = GCPU_EMU_AES_HWCrypt(au1Key, KEY_BIT_LEN_256, NULL,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_ECB, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("HW-AES-ECB-256 failed at %d!\n", __LINE__);
            while(1);
        }
    ...
    {
        u4TestSize = i;
        Printf("AES-CBC-128 Size: 0x%x Bytes\n", u4TestSize);
        ...
        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_128, au1IV,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
            goto lbExit1;
        }
        ...
        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_128, au1IV,
                                pu1CipherBufAddr, pu1PlainBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, FALSE /* decrypt */);
        if (fgResult == FALSE)
        {
            goto lbExit1;
        }
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-CBC-128 failed at %d!\n", __LINE__);
            while(1);
        }
        ...
        fgResult = GCPU_EMU_AES_HWCrypt(au1Key, KEY_BIT_LEN_128, au1IV,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("HW-AES-CBC-128 failed at %d!\n", __LINE__);
            while(1);
        }
    ...
    {
        u4TestSize = i;
        Printf("AES-CBC-192 Size: 0x%x Bytes\n", u4TestSize);
        ...
        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_192, au1IV,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-CBC-192 failed at %d!\n", __LINE__);
            while(1);
        }
        ...
        fgResult = GCPU_EMU_AES_HWCrypt(au1Key, KEY_BIT_LEN_192, au1IV,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("HW-AES-CBC-192 failed at %d!\n", __LINE__);
            while(1);
        }
    ...
    {
        u4TestSize = i;
        Printf("AES-CBC-256 Size: 0x%x Bytes\n", u4TestSize);
        ...
        fgResult = GCPU_EMU_AES_SWCrypt(au1Key, KEY_BIT_LEN_256, au1IV,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("SW-AES-CBC-256 failed at %d!\n", __LINE__);
            while(1);
        }
        ...
        fgResult = GCPU_EMU_AES_HWCrypt(au1Key, KEY_BIT_LEN_256, au1IV,
                                pu1DataBufAddr, pu1CipherBufAddr, u4TestSize,
                                BLOCK_CIPHER_MODE_CBC, TRUE /* encrypt */);
        if (fgResult == FALSE)
        {
        ...
        if (x_memcmp(pu1DataBufAddr, pu1PlainBufAddr, u4TestSize) != 0)
        {
            Printf("HW-AES-CBC-256 failed at %d!\n", __LINE__);
            while(1);
        }
...
lbExit1:
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pu1DataBufAddr));
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pu1CipherBufAddr));
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pu1PlainBufAddr));
...
{
    UINT8 *pu1SrcBufAddr = NULL;
    UINT32 u4CalcuateSize, u4BufSize, i;
    MD_PARAM_T rParam;
    UINT8 Message_Digest[SHA256_HASH_SIZE];
    HAL_TIME_T rStartTime, rEndTime, rTimeDiff;

    u4BufSize = 0x800000;    // 8MB
    pu1SrcBufAddr = (UINT8 *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    if (pu1SrcBufAddr == 0)
    {
        ASSERT(0);
    }
    ...
    for(i = 0; i < u4BufSize; i++)
    {
        *(UINT8 *)(pu1SrcBufAddr + i) = (UINT8)random(0x100);
    }

    // test size from 1KB to 8MB
    //SHA1
    for(i = 1024; i <= u4BufSize; i *= 2)
    {
        u4CalcuateSize = i;

        Printf("SHA-1 Size: 0x%x Bytes\n", u4CalcuateSize);
        // generate digest by SW
        {
            SHA1_CONTEXT_T rCtx;
            // get start time
            HAL_GetTime(&rStartTime);
            SHA1_Reset(&rCtx);
            SHA1_Input(&rCtx, pu1SrcBufAddr, u4CalcuateSize);
            SHA1_Result(&rCtx, Message_Digest);
            // get end time
            HAL_GetTime(&rEndTime);
            // show sw result
            HAL_GetDeltaTime(&rTimeDiff, &rStartTime, &rEndTime);
            Printf("SW Take time: %d.%06d, ", rTimeDiff.u4Seconds, rTimeDiff.u4Micros);
            Printf("Speed: %d MB/s \n",
                u4CalcuateSize / 1024 * 1000 / 1024 * 1000 / (rTimeDiff.u4Seconds * 1000000 + rTimeDiff.u4Micros));
        }

        // generate digest by HW
        rParam.u4SrcStartAddr = (UINT32) pu1SrcBufAddr;
        rParam.u4SrcBufStart = (UINT32) pu1SrcBufAddr;
        rParam.u4SrcBufEnd = (UINT32) pu1SrcBufAddr + u4BufSize;
        rParam.u4DatLen  = u4CalcuateSize;
        rParam.fgFirstPacket = TRUE;
        rParam.fgLastPacket = TRUE;
        rParam.u8BitCnt = 0;
        ...
        // result check
        if(x_memcmp(rParam.au1Hash, Message_Digest, SHA1_HASH_SIZE) != 0)
        {
            Printf("SHA1 Fail \n");
            while(1);
        }
        ...
        // show hw the result
        HAL_GetDeltaTime(&rTimeDiff, &rStartTime, &rEndTime);
        Printf("HW Take time: %d.%06d, ", rTimeDiff.u4Seconds, rTimeDiff.u4Micros);
        Printf("Speed: %d MB/s \n",
            u4CalcuateSize / 1024 * 1000 / 1024 * 1000 / (rTimeDiff.u4Seconds * 1000000 + rTimeDiff.u4Micros));
    }
    ...
    for(i = 1024; i <= u4BufSize; i *= 2)
    {
        ...
        Printf("SHA-224 Size: 0x%x Bytes\n", u4CalcuateSize);
        // generate digest by SW
        {
            SHA256_CONTEXT_T rCtx;
            HAL_GetTime(&rStartTime);
            SHA224_Reset(&rCtx);
            SHA224_Input(&rCtx, pu1SrcBufAddr, u4CalcuateSize);
            SHA224_Result(&rCtx, Message_Digest);
            // get end time
            HAL_GetTime(&rEndTime);
            // show sw result
            HAL_GetDeltaTime(&rTimeDiff, &rStartTime, &rEndTime);
        ...
        // result check
        if(x_memcmp(rParam.au1Hash, Message_Digest, SHA224_HASH_SIZE) != 0)
        {
            Printf("SHA224 Fail \n");
            while(1);
        }
    ...
    for(i = 1024; i <= u4BufSize; i *= 2)
    {
        ...
        Printf("SHA-256 Size: 0x%x Bytes\n", u4CalcuateSize);
        // generate digest by SW
        {
            SHA256_CONTEXT_T rCtx;
            HAL_GetTime(&rStartTime);
            SHA256_Reset(&rCtx);
            SHA256_Input(&rCtx, pu1SrcBufAddr, u4CalcuateSize);
        ...
        // result check
        if(x_memcmp(rParam.au1Hash, Message_Digest, SHA256_HASH_SIZE) != 0)
        {
            Printf("SHA256 Fail \n");
            while(1);
        }
    ...
    for(i = 1024; i <= u4BufSize; i *= 2)
    {
        ...
        Printf("MD-5 Size: 0x%x Bytes\n", u4CalcuateSize);
        // generate digest by SW
        {
            MD5_CONTEXT_T rCtx;
            HAL_GetTime(&rStartTime);
            MD5_Reset(&rCtx);
            MD5_Input(&rCtx, pu1SrcBufAddr, u4CalcuateSize);
            MD5_Result(&rCtx, Message_Digest);
            // get end time
            HAL_GetTime(&rEndTime);
            // show sw result
            HAL_GetDeltaTime(&rTimeDiff, &rStartTime, &rEndTime);
        ...
        // result check
        if(x_memcmp(rParam.au1Hash, Message_Digest, MD5_HASH_SIZE) != 0)
        {
            Printf("MD5 Fail \n");
            while(1);
        }
    ...
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pu1SrcBufAddr));
...
INT32 GCPU_EMU_MEMCPY()
{
    ...
    pbSrcBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    pbDstBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    pbGoldenBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    ...
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pbSrcBufAddr));
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pbDstBufAddr));
    VERIFY(BSP_FreeAlignedDmaMemory((UINT32)pbGoldenBufAddr));
...
INT32 GCPU_EMU_RC4()
{
    ...
    u4SrcOffset = rand() % u4BufSize;
    u4DstOffset = rand() % u4BufSize;
    pbBuf = (BYTE *)BSP_AllocAlignedDmaMemory(RC4_TABLESIZE, GCPU_FIFO_ALIGNMENT);
    pbSrcBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    pbDstBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    pbGoldenBufAddr = (BYTE *)BSP_AllocAlignedDmaMemory(u4BufSize, GCPU_FIFO_ALIGNMENT);
    ...
    BSP_FreeAlignedDmaMemory((UINT32)pbBuf);
    BSP_FreeAlignedDmaMemory((UINT32)pbSrcBufAddr);
    BSP_FreeAlignedDmaMemory((UINT32)pbDstBufAddr);
    BSP_FreeAlignedDmaMemory((UINT32)pbGoldenBufAddr);
...
INT32 GCPU_EMU_TSDesc()
{
    ...
    rParam.eCipher = TSDESC_CIPHER_CBC_SCTE52;
...
{
    BYTE *pbSrcBufAddr = NULL, *pbDstBufAddr = NULL;
    UINT32 i, u4SrcOffset, u4DstOffset, u4GoldenFileSize, u4BufSize;
    TSDESC_PARAM_T rParam;
    INT32 i4Ret;
    UINT32 u4VirtualSrcAddr = 0, u4VirtualDesAddr = 0;
    ...
    rParam.eForm = TSDESC_188_PACKET_FORM;
    rParam.eMode = TSDESC_FIRST_PACKET_MODE;
    rParam.eCipher = TSDESC_CIPHER_CBC;
    rParam.u4TsState[0] = 0;
    rParam.u4TsState[1] = 0;
    rParam.u4TsState[2] = 0;
    rParam.u4TsState[3] = 0;
