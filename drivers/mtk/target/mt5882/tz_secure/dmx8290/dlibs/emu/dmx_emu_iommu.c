...
BOOL _DMX_EMU_DDI_IOMMU2(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    UINT32 pbGoldenBufAddr, i;
    UINT32 u4Size = 0x8000;
    UINT32 u4PageAddr = BSP_GetMemSize()-0x10000 ; //set the first level page address based on dram size.

    ASSERT(u1TsIndex <= MAX_TS_INDEX);

    //change mmu table
    x_memset((void*)u4PageAddr, 0, 0x400);//0x400

    for(i = 0; i < 0x100000; i+=0x2000)
    {
        HalSet4KPageTable(0x8501000 + i, 0x9600000 + i, (UINT32*)u4PageAddr); 
        HalSet4KPageTable(0x8500000 + i, 0x9601000 + i, (UINT32*)u4PageAddr); 
    }
...
BOOL _DMX_EMU_DDI_IOMMU4(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    //change mmu table
    x_memset((void*)u4PageAddr, 0, 0x400);

    HalSet4KPageTable(0x8501000 + i, 0x9600000 + i, (UINT32*)u4PageAddr); //phy=133M, virt=150M
...
BOOL _DMX_EMU_DDI_IOMMU5(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    //change mmu table
    x_memset((void*)u4PageAddr, 0, 0x400);
    /*for this case ,you need allocate 8k page ,but  you just allocate the 4k page to test it . */
    HalSet4KPageTable(0x8501000, 0x9600000, (UINT32*)u4PageAddr); //phy=133M, virt=150M
    //HalSet4KPageTable(0x8500000, 0x9601000, (UINT32*)u4PageAddr); //phy=133M, virt=150M
...
BOOL _DMX_EMU_DMX_IOMMU2(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    {
        HalSet4KPageTable(0x8501000 + i, 0x9600000 + i, (UINT32*)u4PageAddr); //phy=133M, virt=150M
        HalSet4KPageTable(0x8500000 + i, 0x9601000 + i, (UINT32*)u4PageAddr); //phy=133M, virt=150M
    }
...
BOOL _DMX_EMU_DMX_IOMMU3(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    UINT32 u4DstAddr, u4SrcAddr, u4DstOffset, u4DstStartAddr;
    PID_STRUCT_T* prPidStruct;
    UINT32 u4SrcVA = 0x3f100000;//0xc0000000;

    ASSERT(u1TsIndex <= MAX_TS_INDEX);

    //change mmu table
    HalSet1MSectionTable(0x8700000, u4SrcVA); //phy=135M, virt=3584M
    //HalSet1MSectionTable(0x8000000, u4SrcVA); //phy=128M, virt=145M
    ...
    u4Flags = DMX_PID_FLAG_ALL&(~DMX_PID_FLAG_LOCAL_TSINDEX);
    if (!_DMX_SetPid(u1Pidx, u4Flags, &rPid, FALSE))
    {
        return FALSE;
    }
    ...
    if (u1TsIndex == 2)
    {
       /*disable framer first.*/
	   _DMX_SetFramerEnabled(u1TsIndex,FALSE);
#if defined(CC_MT5890)
        if (!fgBypassFramer)
        {
            DMX_SetDbm_InputSource(u1TsIndex, DMX_DBM_INPUT_FRAMER);
...
BOOL _DMX_EMU_DMXDDI_IOMMU2(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    UINT32 u4SrcOffset ,u4SrcStartAddr, u4GoldenBufAddr;
    UINT32 u4PageAddr = BSP_GetMemSize()-0x10000 ; //set the first level page address based on dram size.
    UINT32 u4PageAddr1 = u4PageAddr+0x400;
    ASSERT(u1TsIndex <= MAX_TS_INDEX);

    //change mmu table
    x_memset((void*)u4PageAddr, 0, 0x400);
    x_memset((void*)u4PageAddr1, 0, 0x400);
    ...
    for(i = 0; i < 0x100000; i+=0x2000)
    {
        ...
        HalSet4KPageTable(0x8601000 + i, 0x9700000 + i, (UINT32*)u4PageAddr1); //phy=134M, virt=151M
        HalSet4KPageTable(0x8600000 + i, 0x9701000 + i, (UINT32*)u4PageAddr1); //phy=134M, virt=151M
    }
...
BOOL _DMX_EMU_DMXDDI_IOMMU3(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
    ...
    UINT32 u4SrcOffset ,u4SrcStartAddr, u4GoldenBufAddr;
    UINT32 u4SrcVA = 0xc0000000;
    UINT32 u4DesVA = 0x3f100000;//0xc0100000;
...
BOOL DMX_EMU_IOMMU_PerformanceTest(UINT8 u1TsIndex, BOOL fgBypassFramer, BOOL fgIommu1, BOOL fgIommu2, BOOL fgIommu3)
{
    ...
    PID_STRUCT_T* prPidStruct;
    UINT32 u4PageAddr = BSP_GetMemSize()-0x100000 ; //set the first level page address based on dram size.
    UINT32 u4PageAddr1 = u4PageAddr+0x400;
	UINT32 u4PageAddr2 = u4PageAddr1+0x400;
	UINT32 u4PageAddr3 = u4PageAddr2+0x400;
	UINT32 u4PageAddr4 = u4PageAddr3+0x400;
	UINT32 u4PageAddr5 = u4PageAddr4+0x400;
    ...
    x_memset((void*)u4PageAddr, 0, 0x400);
    x_memset((void*)u4PageAddr1, 0, 0x400);
    x_memset((void*)u4PageAddr2, 0, 0x400);
    x_memset((void*)u4PageAddr3, 0, 0x400);
    x_memset((void*)u4PageAddr4, 0, 0x400);
    x_memset((void*)u4PageAddr5, 0, 0x400);

    for(i = 0; i < 0x100000; i+=0x2000)
    {
        HalSet4KPageTable(0x8201000 + i, 0x9600000 + i, (UINT32*)u4PageAddr); //phy=133M, virt=150M
        HalSet4KPageTable(0x8700000 + i, 0x9601000 + i, (UINT32*)u4PageAddr); //phy=133M, virt=150M

        HalSet4KPageTable(0x8701000 + i, 0x9700000 + i, (UINT32*)u4PageAddr1); //phy=134M, virt=151M
        HalSet4KPageTable(0x8200000 + i, 0x9701000 + i, (UINT32*)u4PageAddr1); //phy=134M, virt=151M

        HalSet4KPageTable(0x8601000 + i, 0x9500000 + i, (UINT32*)u4PageAddr2);
        HalSet4KPageTable(0x8100000 + i, 0x9501000 + i, (UINT32*)u4PageAddr2);

        HalSet4KPageTable(0x8101000 + i, 0x9400000 + i, (UINT32*)u4PageAddr3);
        HalSet4KPageTable(0x8600000 + i, 0x9401000 + i, (UINT32*)u4PageAddr3);

        HalSet4KPageTable(0x8501000 + i, 0x9300000 + i, (UINT32*)u4PageAddr4);
        HalSet4KPageTable(0x8000000 + i, 0x9301000 + i, (UINT32*)u4PageAddr4);

        HalSet4KPageTable(0x8001000 + i, 0x9200000 + i, (UINT32*)u4PageAddr5);
        HalSet4KPageTable(0x8500000 + i, 0x9201000 + i, (UINT32*)u4PageAddr5);
    }
