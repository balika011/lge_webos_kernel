                             ...
                             void (*callback)(void))
{
    ...
    for(i = 0; i < len; i++)
    {
        ...
        *(unsigned char *)(buf + i) = (unsigned char)random(256);
        #else
...
/* Try SymbianOS's Math Random */
#ifdef __SYMBIAN32__
unsigned long ATX_Symbian_GetRandomBytes(unsigned char *buf, unsigned long len);
static unsigned long rng_symbian32(unsigned char *buf, unsigned long len,
                                            void (*callback)(void))
{
	return ATX_Symbian_GetRandomBytes(buf, len);
}
#endif /* __SYMBIAN32__ */
...
#endif
#ifdef __SYMBIAN32__
   x = rng_symbian32(out, outlen, callback); if (x != 0) { return x; }
#endif
#ifdef ANSI_RNG
