...
#define device_GetSecretUid() \
    npi_device_GetSecretUid(NPI_CMN_CONTEXT_NONSECURE)
...
#define mem_ALLOC(out_ptr, in_size) \
    npi_mem_ALLOC(NPI_CMN_CONTEXT_NONSECURE, in_size, ERR_NP_OUTOFMEMORY, out_ptr)
...
#define mem_Calloc(in_num, out_ptr) \
    npi_mem_Calloc(NPI_CMN_CONTEXT_NONSECURE, in_num, sizeof(**out_ptr), (void **)out_ptr)

#define mem_DUP(src, len, out) do { \
        *(out) = mem_Alloc(len); \
        if (NULL == *(out)) \
            RET(ERR_NOMEM); \
        mem_Copy(*(out), src, len); \
        } while (0)
...
#define xml_AllocNodeVec(in_size, out_nodevec) \
    npi_xml_AllocNodeVec(NPI_CMN_CONTEXT_NONSECURE, in_size, out_nodevec)
...
#define box_ParseBuf(io_box, in_fullbox, out_box) \
    npi_box_ParseBuf(NPI_CMN_CONTEXT_NONSECURE, io_box, in_fullbox, out_box)
...
#define strm_S2BS(io_strm, in_len, out_bs) \
    npi_strm_S2BS(NPI_CMN_CONTEXT_NONSECURE, io_strm, in_len, out_bs)

#define strm_S2Str(io_strm, out_len, out_str)                    \
    npi_strm_S2Str(NPI_CMN_CONTEXT_NONSECURE, io_strm, out_len, out_str)
...
#define smi_ReadParamAsTrackHandleList(in_param, out_trackhandlelist) \
        npi_smi_ReadParamAsTrackHandleList(NPI_CMN_CONTEXT_NONSECURE, in_param, out_trackhandlelist)
...
#define esbman_FreeOutputControlTechnology( io_oct ) \
        npi_esbman_FreeOutputControlTechnology( NPI_CMN_CONTEXT_NONSECURE, io_oct );
...
#define uri_ConvertUriScheme(in_orig_scheme, in_new_scheme, in_url, out_url) \
        npi_uri_ConvertUriScheme(NPI_CMN_CONTEXT_NONSECURE, in_orig_scheme, in_new_scheme, in_url, out_url)
