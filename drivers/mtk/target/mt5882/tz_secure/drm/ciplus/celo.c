...
/* File Name:     celo.c                                                     */
/*                                                                           */
/* Description:   CI+  Cryptographic Functions                               */
/*                                                                           */
...
#define ROTL5(x) (((x) << 5) | (((x) >> 27) & 0x1f))
#define ROTL30(x) (((x) << 30) | (((x) >> 2) & 0x3fffffff))
#define CH(x,y,z) ((z) ^ ((x) & ((y) ^ (z))))
#define PARITY(x,y,z) ((x) ^ (y) ^ (z))
#define MAJ(x,y,z) ((y) ^ (((z) ^ (y)) & ((y) ^ (x))))

#define B_SIGMA_0(x) (((((x) >>  2) & 0x3fffffff) | ((x) << 30)) ^   \
		      ((((x) >> 13) & 0x0007ffff) | ((x) << 19)) ^   \
		      ((((x) >> 22) & 0x000003ff) | ((x) << 10)))
#define B_SIGMA_1(x) (((((x) >>  6) & 0x03ffffff) | ((x) << 26)) ^   \
		      ((((x) >> 11) & 0x001fffff) | ((x) << 21)) ^   \
		      ((((x) >> 25) & 0x0000007f) | ((x) <<  7)))
#define S_SIGMA_0(x) (((((x) >>  7) & 0x01ffffff) | ((x) << 25)) ^   \
		      ((((x) >> 18) & 0x00003fff) | ((x) << 14)) ^   \
		      ((((x) >>  3) & 0x1fffffff)              ))
#define S_SIGMA_1(x) (((((x) >> 17) & 0x00007fff) | ((x) << 15)) ^   \
                      ((((x) >> 19) & 0x00001fff) | ((x) << 13)) ^   \
                      ((((x) >> 10) & 0x003fffff)              ))
#define CH(x,y,z)    ((z) ^ ((x) & ((y) ^ (z))))
#define MAJ(x,y,z)   ((y) ^ (((z) ^ (y)) & ((y) ^ (x))))

#define CE_SHA1_BLOCK_SIZE 64
...
static void ExpandKey(U8BIT *key, U32BIT *w);
static void Sha1Apply(U32BIT w[], U32BIT h[]);
static void Sha256Apply(U32BIT w[], U32BIT h[]);
...
{
   U32BIT *w;
   U32BIT w0, w1, w2, w3, w0t, w1t, w2t, w3t;
   S32BIT round, w_idx;

   w = key->w;

   /* state = in */
   w0 = (plaintext[ 0] << 24 | plaintext[ 1] << 16 |
         plaintext[ 2] <<  8 | plaintext[ 3]       );
   w1 = (plaintext[ 4] << 24 | plaintext[ 5] << 16 |
         plaintext[ 6] <<  8 | plaintext[ 7]       );
   w2 = (plaintext[ 8] << 24 | plaintext[ 9] << 16 |
         plaintext[10] <<  8 | plaintext[11]       );
   w3 = (plaintext[12] << 24 | plaintext[13] << 16 |
   ...
   {
      /* 3 functions combined into tables */
      w0t = (s_t0[(w0 >> 24) & 0xff] ^ 
             s_t1[(w1 >> 16) & 0xff] ^
             s_t2[(w2 >>  8) & 0xff] ^
             s_t3[ w3        & 0xff]);
   ...
   /* 2 functions combined into tables */
   w0t = ((s_t2[(w0 >> 24) & 0xff] & 0xff000000) |
          (s_t3[(w1 >> 16) & 0xff] & 0x00ff0000) |
          (s_t0[(w2 >>  8) & 0xff] & 0x0000ff00) |
          (s_t1[ w3        & 0xff] & 0x000000ff));
   w1t = ((s_t2[(w1 >> 24) & 0xff] & 0xff000000) |
          (s_t3[(w2 >> 16) & 0xff] & 0x00ff0000) |
          (s_t0[(w3 >>  8) & 0xff] & 0x0000ff00) |
          (s_t1[ w0        & 0xff] & 0x000000ff));
   w2t = ((s_t2[(w2 >> 24) & 0xff] & 0xff000000) |
          (s_t3[(w3 >> 16) & 0xff] & 0x00ff0000) |
          (s_t0[(w0 >>  8) & 0xff] & 0x0000ff00) |
          (s_t1[ w1        & 0xff] & 0x000000ff));
   w3t = ((s_t2[(w3 >> 24) & 0xff] & 0xff000000) |
          (s_t3[(w0 >> 16) & 0xff] & 0x00ff0000) |
          (s_t0[(w1 >>  8) & 0xff] & 0x0000ff00) |
          (s_t1[ w2        & 0xff] & 0x000000ff));
   ...
   /* out = state */
   ciphertext[ 0] = (U8BIT)(w0 >> 24);
   ciphertext[ 1] = (U8BIT)(w0 >> 16);
   ciphertext[ 2] = (U8BIT)(w0 >>  8);
   ciphertext[ 3] = (U8BIT)(w0      );
   ciphertext[ 4] = (U8BIT)(w1 >> 24);
   ciphertext[ 5] = (U8BIT)(w1 >> 16);
   ciphertext[ 6] = (U8BIT)(w1 >>  8);
   ciphertext[ 7] = (U8BIT)(w1      );
   ciphertext[ 8] = (U8BIT)(w2 >> 24);
   ciphertext[ 9] = (U8BIT)(w2 >> 16);
   ciphertext[10] = (U8BIT)(w2 >>  8);
   ciphertext[11] = (U8BIT)(w2      );
   ciphertext[12] = (U8BIT)(w3 >> 24);
   ciphertext[13] = (U8BIT)(w3 >> 16);
   ciphertext[14] = (U8BIT)(w3 >>  8);
   ciphertext[15] = (U8BIT)(w3      );
}
 ...
 ****************************************************************************/
void CE_Aes128SetDecryptKey(U8BIT *key, S_AES128_KEY *aes_key)
{
   S32BIT round;
   U32BIT *w;
   U8BIT b0, b1, b2, b3;
   ...
   w += 4;
   for (round = 1; round < 10; ++round, w += 4)
   {
      b0 = (U8BIT)(w[0] >> 24);
      b1 = (U8BIT)(w[0] >> 16);
      b2 = (U8BIT)(w[0] >>  8);
      b3 = (U8BIT)(w[0]      );
      w[0] = (s_i_t0[s_s_box[b0]] ^
              s_i_t1[s_s_box[b1]] ^
              s_i_t2[s_s_box[b2]] ^
              s_i_t3[s_s_box[b3]]);
...
void CE_Aes128Decrypt(U8BIT *ciphertext, S_AES128_KEY *key, U8BIT *plaintext)
{
   ...
   /* state = in */
   w0 = (ciphertext[ 0] << 24 | ciphertext[ 1] << 16 |
         ciphertext[ 2] <<  8 | ciphertext[ 3]       );
   w1 = (ciphertext[ 4] << 24 | ciphertext[ 5] << 16 |
         ciphertext[ 6] <<  8 | ciphertext[ 7]       );
   w2 = (ciphertext[ 8] << 24 | ciphertext[ 9] << 16 |
         ciphertext[10] <<  8 | ciphertext[11]       );
   w3 = (ciphertext[12] << 24 | ciphertext[13] << 16 |
   ...
   {
      /* 3 functions combined into tables */
      w0t = (s_i_t0[(w0 >> 24) & 0xff] ^ 
             s_i_t1[(w3 >> 16) & 0xff] ^
             s_i_t2[(w2 >>  8) & 0xff] ^
             s_i_t3[ w1        & 0xff]);
      w1t = (s_i_t0[(w1 >> 24) & 0xff] ^ 
             s_i_t1[(w0 >> 16) & 0xff] ^
             s_i_t2[(w3 >>  8) & 0xff] ^
             s_i_t3[ w2        & 0xff]);
      w2t = (s_i_t0[(w2 >> 24) & 0xff] ^ 
             s_i_t1[(w1 >> 16) & 0xff] ^
             s_i_t2[(w0 >>  8) & 0xff] ^
             s_i_t3[ w3        & 0xff]);
      w3t = (s_i_t0[(w3 >> 24) & 0xff] ^ 
             s_i_t1[(w2 >> 16) & 0xff] ^
             s_i_t2[(w1 >>  8) & 0xff] ^
             s_i_t3[ w0        & 0xff]);
   ...
   w0t = (s_i_s_box[(w0 >> 24) & 0xff] << 24 |
          s_i_s_box[(w3 >> 16) & 0xff] << 16 |
          s_i_s_box[(w2 >>  8) & 0xff] <<  8 |
          s_i_s_box[ w1        & 0xff]);
   w1t = (s_i_s_box[(w1 >> 24) & 0xff] << 24 |
          s_i_s_box[(w0 >> 16) & 0xff] << 16 |
          s_i_s_box[(w3 >>  8) & 0xff] <<  8 |
          s_i_s_box[ w2        & 0xff]);
   w2t = (s_i_s_box[(w2 >> 24) & 0xff] << 24 |
          s_i_s_box[(w1 >> 16) & 0xff] << 16 |
          s_i_s_box[(w0 >>  8) & 0xff] <<  8 |
          s_i_s_box[ w3        & 0xff]);
   w3t = (s_i_s_box[(w3 >> 24) & 0xff] << 24 |
          s_i_s_box[(w2 >> 16) & 0xff] << 16 |
          s_i_s_box[(w1 >>  8) & 0xff] <<  8 |
          s_i_s_box[ w0        & 0xff]);
   ...
   /* out = state */
   plaintext[ 0] = (U8BIT)(w0 >> 24);
   plaintext[ 1] = (U8BIT)(w0 >> 16);
   plaintext[ 2] = (U8BIT)(w0 >>  8);
   plaintext[ 3] = (U8BIT)(w0      );
   plaintext[ 4] = (U8BIT)(w1 >> 24);
   plaintext[ 5] = (U8BIT)(w1 >> 16);
   plaintext[ 6] = (U8BIT)(w1 >>  8);
   plaintext[ 7] = (U8BIT)(w1      );
   plaintext[ 8] = (U8BIT)(w2 >> 24);
   plaintext[ 9] = (U8BIT)(w2 >> 16);
   plaintext[10] = (U8BIT)(w2 >>  8);
   plaintext[11] = (U8BIT)(w2      );
   plaintext[12] = (U8BIT)(w3 >> 24);
   plaintext[13] = (U8BIT)(w3 >> 16);
   plaintext[14] = (U8BIT)(w3 >>  8);
   plaintext[15] = (U8BIT)(w3      );
}
...
{
   U8BIT *block;
   U8BIT *prev_ciphertext;
   S32BIT i;

   if ((length % CE_AES128_BLOCK_SIZE) == 0)
   {
      block = plaintext;
      prev_ciphertext = iv;
      while (block - plaintext < length)
      {
         for (i = 0; i < CE_AES128_BLOCK_SIZE; ++i)
         {
            iv[i] = prev_ciphertext[i] ^ block[i];
         }
         CE_Aes128Encrypt(iv, key, ciphertext);
         prev_ciphertext = ciphertext;
         block += CE_AES128_BLOCK_SIZE;
         ciphertext += CE_AES128_BLOCK_SIZE;
      }
   }
}
                         ...
                         U8BIT *iv, U8BIT *plaintext)
{
   ...
   if ((length % CE_AES128_BLOCK_SIZE) == 0)
   {
      block = ciphertext + length - CE_AES128_BLOCK_SIZE;
      curr_plain = plaintext + length - CE_AES128_BLOCK_SIZE;
      while (block - ciphertext >= CE_AES128_BLOCK_SIZE)
      {
         prev_block = block - CE_AES128_BLOCK_SIZE;
         CE_Aes128Decrypt(block, key, curr_plain);
         for (i = 0; i < CE_AES128_BLOCK_SIZE; ++i)
         {
            curr_plain[i] ^= prev_block[i];
         }
         block -= CE_AES128_BLOCK_SIZE;
         curr_plain -= CE_AES128_BLOCK_SIZE;
      }

      if (length > 0)
      {
         CE_Aes128Decrypt(ciphertext, key, plaintext);
         for (i = 0; i < CE_AES128_BLOCK_SIZE; ++i)
         {
            plaintext[i] ^= iv[i];
         }
...
void CE_Sha1(U8BIT *message, U32BIT length, U8BIT *digest)
{
   ...
   U32BIT remaining;
   U32BIT remaining_whole;
   U32BIT i, j;

   remaining = length;

   /* Apply SHA-1 on complete blocks (no need for padding yet) */
   while (remaining >= 64)
   {
      for (i = 0, j = 0; i < 64; i += 4, ++j)
      {
         w[j] = (message[i  ] << 24 | message[i+1] << 16 | 
                 message[i+2] <<  8 | message[i+3]       );
      }

      /* Apply SHA-1 functions on w */
      Sha1Apply(w, h);
   ...
   /* Copy next block (last or last-but-one) */
   remaining_whole = remaining & 0xfffffffc;
   for (i = 0, j = 0; i < remaining_whole; i += 4, ++j)
   {
      w[j] = (message[i  ] << 24 | message[i+1] << 16 | 
              message[i+2] <<  8 | message[i+3]       );
   }
   ...
   switch (remaining - remaining_whole)
   {
   case 0:
      w[j] = 0x80000000;
      break;
   case 1:
      w[j] = (message[remaining_whole] << 24 | 0x800000);
      break;
   case 2:
      w[j] = (message[remaining_whole  ] << 24 |
              message[remaining_whole+1] << 16 | 0x8000);
      break;
   case 3:
      w[j] = (message[remaining_whole  ] << 24 | 
              message[remaining_whole+1] << 16 |
              message[remaining_whole+2] <<  8 | 0x80);
      break;
   }

   /* Block not big enough to contain padding + length */
   if (remaining >= 56)
   {
      while (++j < 16)
      {
   ...
   /* Last block can always contain padding + length */
   while (++j < 15)
   {
      w[j] = 0;
   }
   ...
   /* Copy digest */
   digest[ 0] = (U8BIT)(h[0] >> 24);
   digest[ 1] = (U8BIT)(h[0] >> 16);
   digest[ 2] = (U8BIT)(h[0] >>  8);
   digest[ 3] = (U8BIT)(h[0]      );
   digest[ 4] = (U8BIT)(h[1] >> 24);
   digest[ 5] = (U8BIT)(h[1] >> 16);
   digest[ 6] = (U8BIT)(h[1] >>  8);
   digest[ 7] = (U8BIT)(h[1]      );
   digest[ 8] = (U8BIT)(h[2] >> 24);
   digest[ 9] = (U8BIT)(h[2] >> 16);
   digest[10] = (U8BIT)(h[2] >>  8);
   digest[11] = (U8BIT)(h[2]      );
   digest[12] = (U8BIT)(h[3] >> 24);
   digest[13] = (U8BIT)(h[3] >> 16);
   digest[14] = (U8BIT)(h[3] >>  8);
   digest[15] = (U8BIT)(h[3]      );
   digest[16] = (U8BIT)(h[4] >> 24);
   digest[17] = (U8BIT)(h[4] >> 16);
   digest[18] = (U8BIT)(h[4] >>  8);
   digest[19] = (U8BIT)(h[4]      );
}
...
void CE_Sha256(U8BIT *message, U32BIT length, U8BIT *digest)
{
   ...
   /* Apply SHA-256 on complete blocks (no need for padding yet) */
   while (remaining >= 64)
   {
      for (i = 0, j = 0; i < 64; i += 4, ++j)
      {
   ...
   /* Copy digest */
   for (i = 0, j = 0; j < 8; i += 4, ++j)
   {
      digest[i  ] = (U8BIT)(h[j] >> 24);
      digest[i+1] = (U8BIT)(h[j] >> 16);
      digest[i+2] = (U8BIT)(h[j] >>  8);
      digest[i+3] = (U8BIT)(h[j]      );
   }
}
...
static void ExpandKey(U8BIT *key, U32BIT *w)
{
   ...
   for (i = 4, j = 1; i < 44; i += 4, ++j)
   {
      rotword = (w[i-1] << 8) | ((w[i-1] >> 24) & 0xff);
      subword = ((s_t2[(rotword >> 24) & 0xff] & 0xff000000) | 
                 (s_t3[(rotword >> 16) & 0xff] & 0x00ff0000) |
                 (s_t0[(rotword >>  8) & 0xff] & 0x0000ff00) |
                 (s_t1[ rotword        & 0xff] & 0x000000ff));
      w[i  ] = w[i-4] ^ subword ^ rcon[j];
      w[i+1] = w[i-3] ^ w[i  ];
      w[i+2] = w[i-2] ^ w[i+1];
      w[i+3] = w[i-1] ^ w[i+2];
   }
...
static void Sha1Apply(U32BIT w[], U32BIT h[])
{
   ...
   /* Apply SHA-1 functions 0-19 */
   for (i = 0; i < 16; ++i)
   {
      p[5] = ROTL5(p[4]) + CH(p[3],p[2],p[1]) + p[0] + 0x5a827999UL + w[i];
      p[3] = ROTL30(p[3]);
      ++p;
   }

   /* Apply SHA-1 functions 16-19 */
   for (i = 16; i < 20; ++i)
   {
      w[i] = w[i-3] ^ w[i-8] ^ w[i-14] ^ w[i-16];
      w[i] = (w[i] << 1) | ((w[i] >> 31) & 0x1);
      p[5] = ROTL5(p[4]) + CH(p[3],p[2],p[1]) + p[0] + 0x5a827999 + w[i];
      p[3] = ROTL30(p[3]);
      ++p;
   }

   /* Apply SHA-1 functions 20-39 */
   for (i = 20; i < 40; ++i)
   {
      w[i] = w[i-3] ^ w[i-8] ^ w[i-14] ^ w[i-16];
      w[i] = (w[i] << 1) | ((w[i] >> 31) & 0x1);
      p[5] = ROTL5(p[4]) + PARITY(p[3],p[2],p[1]) + p[0] + 0x6ed9eba1 + w[i];
      p[3] = ROTL30(p[3]);
      ++p;
   }

   /* Apply SHA-1 functions 40-59 */
   for (i = 40; i < 60; ++i)
   {
      w[i] = w[i-3] ^ w[i-8] ^ w[i-14] ^ w[i-16];
      w[i] = (w[i] << 1) | ((w[i] >> 31) & 0x1);
      p[5] = ROTL5(p[4]) + MAJ(p[3],p[2],p[1]) + p[0] + 0x8f1bbcdc + w[i];
      p[3] = ROTL30(p[3]);
      ++p;
   }

   /* Apply SHA-1 functions 60-79 */
   for (i = 60; i < 80; ++i)
   {
      w[i] = w[i-3] ^ w[i-8] ^ w[i-14] ^ w[i-16];
      w[i] = (w[i] << 1) | ((w[i] >> 31) & 0x1);
      p[5] = ROTL5(p[4]) + PARITY(p[3],p[2],p[1]) + p[0] + 0xca62c1d6 + w[i];
      p[3] = ROTL30(p[3]);
      ++p;
   }
...
static void Sha256Apply(U32BIT w[], U32BIT h[])
{
   ...
   /* Apply SHA-256 functions 0-15 */
   for (i = 0; i < 16; ++i)
   {
      t1 = p[0] + B_SIGMA_1(p[3]) + CH(p[3], p[2], p[1]) + k[i] + w[i];
      t2 = B_SIGMA_0(p[7]) + MAJ(p[7], p[6], p[5]);
      p[4] += t1;
      p[8] = t1 + t2;
      ++p;
   }

   /* Apply SHA-256 functions 16-63 */
   for (i = 16; i < 64; ++i)
   {
      w[i] = S_SIGMA_1(w[i-2]) + w[i-7] + S_SIGMA_0(w[i-15]) + w[i-16];
      t1 = p[0] + B_SIGMA_1(p[3]) + CH(p[3], p[2], p[1]) + k[i] + w[i];
      t2 = B_SIGMA_0(p[7]) + MAJ(p[7], p[6], p[5]);
      p[4] += t1;
      p[8] = t1 + t2;
