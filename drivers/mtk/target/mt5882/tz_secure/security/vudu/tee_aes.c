 ...
 */
static INT32 mtk_private_rijndaelSetupEncrypt(UINT32 *rk, const UINT8 *key, int keybits)
{
  INT32 i = 0;
  UINT32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
static int mtk_private_rijndaelSetupDecrypt(UINT32 *rk, const UINT8 *key, INT32 keybits)
{
  INT32 nrounds, i, j;
  UINT32 temp;

  /* expand the cipher key: */
  nrounds = mtk_private_rijndaelSetupEncrypt(rk, key, keybits);
...
static void mtk_private_rijndaelEncrypt(const UINT32 *rk, INT32 nrounds, const UINT8 plaintext[16],
  UINT8 ciphertext[16])
{
...
static void mtk_private_rijndaelDecrypt(const UINT32 *rk, int nrounds, const UINT8 ciphertext[16],
  UINT8 plaintext[16])
{
  UINT32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
...
{
    ctx->rk = ctx->buf;
    ctx->nr = mtk_private_rijndaelSetupDecrypt(ctx->rk, key, keysize);
}
...
void mtk_private_aes_crypt_ecb( aes_context *ctx,
                    ...
                    UINT8 output[16] )
{
    if( mode == MTK_AES_ENCRYPT )
    {
        mtk_private_rijndaelEncrypt(ctx->rk, ctx->nr, input, output);
    }
    else
    {
        mtk_private_rijndaelDecrypt(ctx->rk, ctx->nr, input, output);
    }
}
                    ...
                    UINT8 *output )
{
    while( length > 0 )
    {
        mtk_private_aes_crypt_ecb(ctx, mode, input, output);
                    ...
                    UINT8 *output )
{
    ...
    if( mode == MTK_AES_ENCRYPT )
    {
        while( length > 0 )
        {
            ...
            mtk_private_aes_crypt_ecb( ctx, mode, output, output );
            memcpy( iv, output, 16 );
...
void mtk_private_aes_crypt_cbc_scte52( aes_context *ctx,
                    INT32 mode,
                    INT32 length,
                    UINT8 iv[16],
                    UINT8 *input,
                    UINT8 *output )
{
    ...
    if( mode == MTK_AES_ENCRYPT )
    {
        mtk_private_aes_crypt_ecb( ctx, mode, iv, output );
        for( i = 0; i < length; i++ )
                    ...
                    UINT8 *output )
{
    ...
    if( mode == MTK_AES_ENCRYPT )
    {
        ...
        {
            if( n == 0 )
                mtk_private_aes_crypt_ecb( ctx, mode, iv, iv );
