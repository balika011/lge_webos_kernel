...
UINT8 u1DrvNXNRGetBlkConf(void)
{
	...
	#else
		return (BNR_FW_IsStop() ? NR_R(NXNR_20, NX_STA_VBLK_CONFIDENCE) : _SWR(NR_STA_BNR_02, NR_STA_BNR_VBLK_CONF));
	#endif
}
...
static void _vDrvTnrGetParam(void)
{
    ...
    _rNrWriteReg.arMACTbl[0] = MIN(R_Fld(u4PQ00, NR_TBL_C_0), 63);
    _rNrWriteReg.arMACTbl[1] = MIN(R_Fld(u4PQ00, NR_TBL_C_1), 63);
    _rNrWriteReg.arMACTbl[2] = MIN(R_Fld(u4PQ00, NR_TBL_C_2), 63);
    _rNrWriteReg.arMACTbl[3] = MIN(R_Fld(u4PQ00, NR_TBL_C_3), 63);
    _rNrWriteReg.arMACTbl[4] = MIN(R_Fld(u4PQ00, NR_TBL_C_4), 63);
    _rNrWriteReg.arMACTbl[5] = MIN(R_Fld(u4PQ01, NR_TBL_C_5), 63);
    _rNrWriteReg.arMACTbl[6] = MIN(R_Fld(u4PQ01, NR_TBL_C_6), 63);
    _rNrWriteReg.arMACTbl[7] = MIN(R_Fld(u4PQ01, NR_TBL_C_7), 63);
    ...
    _rNrWriteReg.u1MASubWeiNLGain = _SWR(NR_PQ_08,NR_MOVINGTXR_SUB_WEI_NL_NAG_GAIN_DFT);
}
...
void vDrvAnrSetTNR(void)
{
    ...
    vDrvMCNRSetWeiTbl(MCNR_IIR_TBL_MC,  _rNrWriteReg.arMATbl);
...
static void _vMoTnrProc(UINT8 u1TnrStr, UINT8 u1NoiseLvl)
{
    ...
    #if defined(CC_MT5399) || defined(CC_MT5882)
    //UINT32 u4Highfreq;
    //UINT32 u4HighFreqGain;
    #endif

#if defined(CC_MT5399) || defined(CC_MT5882)
    //UINT8 u1HighFreqTH1= _SWR(NR_MO_05, MONR_HIGH_FREQ_TH1);
    //UINT8 u1HighFreqTH2= _SWR(NR_MO_05, MONR_HIGH_FREQ_TH2);
#endif
    // 1. get parameter
    _vDrvTnrGetParam();
...
static void _vAnrMoAnrProc(const NR_NM_STA_T* prNmSta, const NR_NM_ANA_T* prNmAna)
{
	...
	// adap SNR manual threshold
    if (_rMoNrWRegState[E_NR_MOANR_SNR].fgAutoOn())
    {
        vDrvAnrSnrProc(u4NoiseLvl);
    }
...
{
    _SWW(NR_CTRL_01, 1, ANR_SEL);
    _SWW(NR_CTRL_00, 1, EN_MOTION_ANR);
    _SWW(NR_CTRL_00, 1, EN_ADAP_SNR_THM);
    _SWW(NR_CTRL_00, 1, EN_RNR_TH_ADAP);
