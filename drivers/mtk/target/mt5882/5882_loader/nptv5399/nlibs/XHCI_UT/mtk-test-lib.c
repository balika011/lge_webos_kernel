...
//		msleep(randomSleep);
		...
		if(randomIndex == 0){
			addr = (int*)SSUSB_U3_MAC_BASE;
		}
		else if(randomIndex == 1){
			addr = (int*)SSUSB_U3_SYS_BASE;
		}
		else if(randomIndex == 2){
			addr = (int*)SSUSB_U2_SYS_BASE;
		}
...
int f_evaluate_context(int max_exit_latency, int maxp0, int preping_mode, int preping, int besl, int besld){
	...
	printk("max_exit_latency:%d, maxp0:%d, preping_mode:%d, preping:%d\n", 
	        max_exit_latency, maxp0, preping_mode, preping);
...
int f_loopback_loop_gpd(int ep_out, int ep_in, int data_length, int start_add, int gpd_length, struct usb_device *usbdev){
	...
	printk("size:0x%x, buffer_tx:0x%x\n", data_length+start_add, (unsigned int)buffer_tx);
	mapping_tx = dma_map_single(dev, buffer_tx,data_length+start_add, DMA_BIDIRECTIONAL);
#ifdef USE_CHANNEL_B
	kernel_virt_start=0xf2100000;
	buffer_tx = (dma_addr_t *)kernel_virt_start;
	kernel_virt_start=0x50000000;
	mapping_tx = kernel_virt_start;
#endif
	printk("buffer_tx 0x%x dma 0x%x\n", (unsigned int)buffer_tx, mapping_tx);
...
#ifdef USE_CHANNEL_B
	kernel_virt_start=0xf2110000;
	buffer_rx = (dma_addr_t *)kernel_virt_start;
	kernel_virt_start=0x50010000;
	mapping_rx = kernel_virt_start;
#endif
	printk("buffer_rx 0x%x dma 0x%x\n", (unsigned int)buffer_rx, mapping_rx);
	total_running = 0;
...
#define USB3_U1_STATE_INFO	(SSUSB_U3_MAC_BASE + 0x50)	
#define USB3_U2_STATE_INFO	(SSUSB_U3_MAC_BASE + 0x54)	
...
//	int ret;	
	...
	addr = (u32 __iomem *)(SSUSB_U3_XHCI_BASE + 0x97c);
	temp = xhci_readl(xhci, addr);
	//pdn
	...
	port_index = get_port_index(g_port_id);
	port = rh_port[port_index];
	addr = (u32 __iomem *)(SSUSB_U3_XHCI_BASE + 0x978);
	if(hle == 1 && rwe == 1){
		temp = 0xff;
		xhci_writel(xhci, temp, addr);
	}
