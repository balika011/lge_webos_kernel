...
int rh_get_port_status(struct xhci_hcd *xhci, int port_id){
	...
	addr = &xhci->op_regs->port_status_base + NUM_PORT_REGS*(port_id & 0xff);
	temp = xhci_readl(xhci, addr);
//	printk("get port status, actual port %d status  = 0x%x addr  = 0x%x\n", port_id, temp, (unsigned int)addr);
...
//	printk("handle_port_status, 0x%x\n", readl(0xf0074420));
	...
	enablePortClockPower((port_id-1), 0x3);
//	printk("handle_port_status, 0x%x\n", readl(0xf0074420));
...
//	printk("test port_status %x\n", port_status);
	...
	if(port_status & (USB_PORT_STAT_C_CONNECTION << 16)){
		if(port_status & USB_PORT_STAT_CONNECTION){
			...
			printk("connect port status event, connected\n");
			g_port_id = port_id;
			g_port_connect = true;
...
//						, port->port_id, port->port_status, port->port_speed);
					printk("port set: port_id %d, port_status %d, port_speed %d\n"
						, port->port_id, port->port_status, port->port_speed);
					g_port_reset = true;
				}
				else{
			...
			printk("connect port status event, disconnected\n");
//			port->port_id = 0;
			port->port_speed = 0;
	...
	if ((temp & PORT_PLC) && (temp & PORT_PLS_MASK) == XDEV_RESUME){
		//printk(KERN_DEBUG "set resume flag\n");
		g_port_resume = 1;
	}
	if ((temp & PORT_PLC)){
		g_port_plc = 1;
