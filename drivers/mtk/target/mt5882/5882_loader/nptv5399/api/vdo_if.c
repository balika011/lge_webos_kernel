	...
	{ 
		case SV_VS_SCART1:
			if(bType)
				...
				if(bApiQuearyVSCConnectStatus(0)==SV_VD_TVD3D)
					u1VSCConnectADC_main=VSS_SCART;
				if(bApiQuearyVSCConnectStatus(1)==SV_VD_TVD3D)
...
UINT8 bApiVSCMainSubSrc(UINT8 bMainSrc, UINT8 bSubSrc, UINT8 u1SrcIdx)
{
	...
	if(bMainSrc == SV_VD_MAX)
	{
		bMainSrc = bOldMainDec;
	}

	if(bSubSrc == SV_VD_MAX)
	{
		bSubSrc = bOldSubDec;
	}
	...
	LOG(2, "Pipeline  bOldMainDec %d bOldSubDec %d bNewMainDec %d bNewSubDec %d\n",bOldMainDec, bOldSubDec,bNewMainDec,bNewSubDec);
