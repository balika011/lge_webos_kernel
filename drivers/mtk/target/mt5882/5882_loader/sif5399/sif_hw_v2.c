...
//---------------------------------------------------------------------------
/// SIF has 9 data port register. Total 36 bytes, so page read/write unit is 36 bytes.
...
#define PINTNO_SIF      PDWNC_INTNO_SIFM			// SIF0 interrupt number inside Pdwnc
...
static UINT16 SIF_V2_X_Sp_Read(UINT8 u1CHannelID, UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddrNum, UINT32 u4WordAddr,
                               UINT8* pu1Buf, UINT16 u2ByteCnt);
...
static VOID _SIF_WRITE32(UINT32 u4Offset, UINT32 u4Value)
{
    ...
    else if (_rSifDevParm.fgPdwncI2C)
    {
        ...
        else if (u4Offset == RW_SM0_CTRL2_REG)
        {
            IO_WRITE32(PDWNC_BASE, (u4Offset - IO_VAL + 0x9B8), (u4Value));
        }
        else if (u4Offset == RW_SM0_CTRL3_REG)
        {
            IO_WRITE32(PDWNC_BASE, (u4Offset - IO_VAL + 0x908), (u4Value));
        }
        else
        {
            IO_WRITE32(PDWNC_BASE, (u4Offset - IO_VAL + 0x928), (u4Value));
        }
    }
    else if (_rSifDevParm.fgPdwncI2C1)
#endif
#ifdef CC_LGE_PROTO_PCBA
    else if (_rSifDevParm.fgPdwncI2C1 || _rSifDevParm.fgPdwncI2C)
#endif        
    {
        if ((u4Offset == RW_SM0_CTRL0_REG) || (u4Offset == RW_SM0_CTRL1_REG))
        {
        ...
        {
            IO_WRITE32(PDWNC_BASE, (u4Offset - IO_VAL + 0x9B8 + 0x28), (u4Value));
        }
        else if (u4Offset == RW_SM0_CTRL3_REG)
        {
            IO_WRITE32(PDWNC_BASE, (u4Offset - IO_VAL + 0x908 + 0x28), (u4Value));
        }
        else
        {
            IO_WRITE32(PDWNC_BASE, (u4Offset - IO_VAL + 0x928 + 0x28), (u4Value));
        }
    }
#ifndef CC_MTK_LOADER
#ifndef  CC_DISCARD_SUPPORT_PCMCIA
...
static UINT32 _SIF_READ32(UINT32 u4Offset)
{
    ...
    else if (_rSifDevParm.fgPdwncI2C)
    {
        ...
        {
            return IO_READ32(PDWNC_BASE, (u4Offset - IO_VAL + 0x9B8));
        }
        else if (u4Offset == RW_SM0_CTRL3_REG)
        {
            return IO_READ32(PDWNC_BASE, (u4Offset - IO_VAL + 0x908));
        }
        else
        {
            return IO_READ32(PDWNC_BASE, (u4Offset - IO_VAL + 0x928));
        }
    }
    else if (_rSifDevParm.fgPdwncI2C1)
#endif
...
#endif  
    {
        ...
        {
            return IO_READ32(PDWNC_BASE, (u4Offset - IO_VAL + 0x9B8 + 0x28));
        }
        else if (u4Offset == RW_SM0_CTRL3_REG)
        {
            return IO_READ32(PDWNC_BASE, (u4Offset - IO_VAL + 0x908 + 0x28));
        }
        else
        {
            return IO_READ32(PDWNC_BASE, (u4Offset - IO_VAL + 0x928 + 0x28));
        }
    }
#ifndef CC_MTK_LOADER
#ifndef  CC_DISCARD_SUPPORT_PCMCIA
...
#if defined(CC_FAST_INIT)|| (defined(CC_SUPPORT_STR))||defined(CC_LG_SNAP_SHOT)
...
#if defined(CC_MT5890)//The same as MT5861
    IO_WRITE32MSK(BIM_BASE, RW_PINTEN_REG, 0, SIF_M2_INTEN);
    IO_WRITE32MSK(BIM_BASE, RW_PINTCLR_REG, SIF_M2_INTCLR, SIF_M2_INTCLR);
#endif
...
#if SIF_USE_OSAI
#ifdef CC_SIF_USE_INTERRUPT_MODE
    IO_WRITE32MSK(BIM_BASE, RW_PINTEN_REG, SIF_M2_INTEN, SIF_M2_INTEN);
#endif
#endif
#endif
    if (_SIF_ISR_IsPdwncMasterEnable())
...
static INT32 _SIF_PutByte(UINT8 u1Data)
{
    ...
    if (i4Ret != SIF_OK)
    {
        LOG(0, "SIF put byte: sema time out ");
        return i4Ret;
    }
...
static INT32 _SIF_PutMultipleBytes(UINT8* pu1Data, INT32 i4Bytes)
{
    ...
    for (i = 0; i < i4TotalLoops; i++)
    {
...
#if SIF_USE_LENGTHER_PAGE
        _SIF_WRITE32(RW_SM0_D2_REG, u4D2);
        _SIF_WRITE32(RW_SM0_D3_REG, u4D3);
        _SIF_WRITE32(RW_SM0_D4_REG, u4D4);
        _SIF_WRITE32(RW_SM0_D5_REG, u4D5);
        _SIF_WRITE32(RW_SM0_D6_REG, u4D6);
        _SIF_WRITE32(RW_SM0_D7_REG, u4D7);
        _SIF_WRITE32(RW_SM0_D8_REG, u4D8);
#endif
        ...
        {
            LOG(0, "SIF put mutiple byte: sema time out ");
            return i4RetTimeout;
        }
#if SIF_USE_LENGTHER_PAGE
        u4AckL = ACK(_SIF_READ32(RW_SM0_CTRL1_REG));		//from 0x0 ~ 0x0ffff
        u4AckH = ACKH(_SIF_READ32(RW_SM0_CTRL3_REG));		//from 0x0 ~ 0x0fffff
        ...
        if (u4Ack == 0)
        {
            return ((i4Ret > i4Bytes) ? i4Bytes : i4Ret);
        }
    }
...
static INT32 _SIF_GetMultipleBytes(UINT8* pu1Data, INT32 i4Bytes)
{
    ...
    for (i = 0; i < i4TotalLoops; i++)
    {
        ...
        if (i4RetTimeout != SIF_OK)
        {
            LOG(0, "SIF get mutiple byte: sema time out ");
            return i4RetTimeout;
        }
...
#if SIF_USE_LENGTHER_PAGE
        u4D2 = _SIF_READ32(RW_SM0_D2_REG);
        u4D3 = _SIF_READ32(RW_SM0_D3_REG);
        u4D4 = _SIF_READ32(RW_SM0_D4_REG);
        u4D5 = _SIF_READ32(RW_SM0_D5_REG);
        u4D6 = _SIF_READ32(RW_SM0_D6_REG);
        u4D7 = _SIF_READ32(RW_SM0_D7_REG);
        u4D8 = _SIF_READ32(RW_SM0_D8_REG);
#endif
        // save to buffer
#if defined (USE_NEW_MODE)
...
static INT32 _SIF_GetByte(UINT8* pu1Data, BOOL fgLast)
{
    ...
    if (i4Ret != SIF_OK)
    {
        LOG(0, "SIF get byte: sema time out ");
        return i4Ret;
    }
...
#ifdef SIF_CHECK_SDA_LOW
SIF_BUS_STATUS_T SIF_CheckBusStatus (VOID)
{
    return _SIF_CheckBus();
}
#endif
                          ...
                          SIF_BUS_CONTROL_T bus_ctrl)
{
    ...
    if (!(bus_ctrl & SIF_CTL_NO_START))
    {
        ...
        {
            LOG(0, "_SIF_OptRead: Start Bit error, %d.\n", i4Ret);
            return i4Ret;
        }
        if (_SIF_PutByte(u1DevAddr) != 1)   // dev addr
        {
            ...
            if (!(bus_ctrl & SIF_CTL_NO_STOP))
            {
                ...
                if ( i4Ret != SIF_OK)
                {
                    LOG(0, "_SIF_OptRead: Stop Bit error, %d.\n", i4Ret);
                    return i4Ret;
                }
            }
    ...
    {
        LOG(0, "_SIF_OptRead: only %d byte were read, expected %d.\n", i4Cnt, u4DataLen);
    }
#else // CC_SIF_MULTIPLE_DATA_BYTE_ACCESS
    while (i4Cnt < u4DataLen)
    {
        ...
        if (i4Ret != SIF_OK)
        {
            LOG(0, "_SIF_OptRead: terminated at %d byte due to no ACK. return %d\n", i4Cnt, i4Ret);
            break;
        }
                           ...
                           SIF_BUS_CONTROL_T bus_ctrl)
{
    ...
    if (!(bus_ctrl & SIF_CTL_NO_START))
    {
        ...
        if (i4Ret != SIF_OK)                 // start
        {
            LOG(0, "_SIF_OptWrite: Start Bit error, %d.\n", i4Ret);
            return i4Ret;
        }
        ...
        if (_SIF_PutByte(u1DevAddr) != 1)   // dev addr
        {
            ...
            if (!(bus_ctrl & SIF_CTL_NO_STOP))
            {
                ...
                if (i4Ret != SIF_OK)
                {
                    LOG(0, "_SIF_OptWrite: Stop Bit error, %d.\n", i4Ret);
                    return i4Ret;
                }
            }
    ...
    if (i4Cnt != (INT32) u4DataLen)
    {
        LOG(0, "_SIF_OptWrite: only %d byte were written, expected %d.\n", i4Cnt, u4DataLen);
    }
#else // CC_SIF_MULTIPLE_DATA_BYTE_ACCESS
    // data
    for (i4Cnt = 0; i4Cnt < u4DataLen; i4Cnt++)
    {
        ...
        {
            LOG(0, "_SIF_OptWrite: terminated at %d byte due to no ACK.\n", i4Cnt);
            break;
        }
    }
#endif // CC_SIF_MULTIPLE_DATA_BYTE_ACCESS
                       ...
                       UINT8 Bus_Mode)
{
    ...
    if (Bus_Mode == 2)
    {
        u4Reg = (0x03 << 16);
        u4Msk = 0x7f0000;
        _SIF_WRITE32MSK(RW_SM0_CTRL3_REG, u4Reg, u4Msk);  //284
        LOG(3, "dd	_SIF_Write RW_SM0_CTRL1_REG = 0x%x \n", _SIF_READ32(RW_SM0_CTRL1_REG));
        LOG(3, "dd	_SIF_Write RW_SM0_CTRL3_REG = 0x%x \n", _SIF_READ32(RW_SM0_CTRL3_REG));
                        ...
                        UINT8 Bus_Mode)
{
...
#ifdef CC_SIF_MULTIPLE_DATA_BYTE_ACCESS
    // data
    u4Reg = _SIF_READ32(RW_SM0_CTRL3_REG);
    ...
    if (i4Cnt != (INT32) u4DataLen)
    {
        LOG(0, "_SIF_Write: only %d byte were written, expected %d.\n", i4Cnt, u4DataLen);
    }
#else // CC_SIF_MULTIPLE_DATA_BYTE_ACCESS
    // data
    u4Reg = _SIF_READ32(RW_SM0_CTRL3_REG);
    ...
    for (i4Cnt = 0; i4Cnt < u4DataLen; i4Cnt++)
    {
        if (_SIF_PutByte(*(pu1DataBuf + i4Cnt)) != 1)
        {
            LOG(0, "_SIF_Write: terminated at %d byte due to no ACK.\n", i4Cnt);
            break;
        }
    }
#endif // CC_SIF_MULTIPLE_DATA_BYTE_ACCESS
                             ...
                             UINT8* pData, UINT32 u4DataLen)
{
    ...
    if ((BusSts.SIF_BUS_SCL == 0) || (BusSts.SIF_BUS_SDA == 0))// bus is busy
    {
        i4Ret = _SIF_PutStopBit();
        if (i4Ret != SIF_OK)
        {
            LOG(0, "_SIF_DrvRawRead: Stop Bit error, %d.\n", i4Ret);
            //can't return,because of after callback.
        }
        BusSts = _SIF_CheckBus();                                   // check bus status, again.
        if ((BusSts.SIF_BUS_SCL == 0) || (BusSts.SIF_BUS_SDA == 0)) // bus is still busy...
        {
            LOG(0, "[SIF]Bus is too busy to read\n");
    ...
    if ((BusSts.SIF_BUS_SCL == 1) && (BusSts.SIF_BUS_SDA == 1)) 	// bus free
    {
        ...
        else
        {
            ...
            u4Dev = (u4Dev) << 24 ;
            _SIF_WRITE32(RW_SM0_DEV_REG, u4Dev);
            if (u4SubAddrLen)											  // sub-addr
            {
                i4Ret = _SIF_OptWrite(u2ClkDiv, u1DevAddr, pSubAddr, u4SubAddrLen, SIF_CTL_NO_STOP);
                if (i4Ret == (INT32) u4SubAddrLen)
 ...
 *  @param  u1DevAddr   Serial interface device address.
 *  @param  pSUbAddr    Pointer to sub addresses
 *  @param  u4SubAddrLen  Sub address length     fe+c0+register_address+30+fe+c1+31 = 7
 *  @param  pData       Pointer to user's buffer.
 *  @param  u4DataLen   Number of bytes to read.
 *  @retval > 0         Actual number of data bytes read.
 *  @retval <= 0        Error
...
static INT32 _SIF_DrvRawSpRead(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8* pSubAddr, UINT32 u4SubAddrLen,
                               UINT8* pData, UINT32 u4DataLen)
{
    INT32 i4Ret = SIF_ERROR_BUS_BUSY;
    SIF_BUS_STATUS_T BusSts;
    ...
    if ((BusSts.SIF_BUS_SCL == 0) || (BusSts.SIF_BUS_SDA == 0))
    {
        ...
        if (i4Ret != SIF_OK)
        {
            LOG(3, "_SIF_DrvRawSpRead: Stop Bit error, %d.\n", i4Ret);
            //can't return,because of after callback.
        }
        BusSts = _SIF_CheckBus();                                   // check bus status, again.
    ...
    if ((BusSts.SIF_BUS_SCL == 1) && (BusSts.SIF_BUS_SDA == 1))     // bus free
    {
        ...
        i4Ret = _SIF_OptWrite(u2ClkDiv, u1DevAddr, pSubAddr, u4SubAddrLen, SIF_CTL_NO_STOP);
        *(pSubAddr + 1) = (*(pSubAddr + 1) | 0x1);
        if (i4Ret == (INT32) u4SubAddrLen)
        {
            i4Ret = _SIF_OptWrite(u2ClkDiv, u1DevAddr, pSubAddr, u4SubAddrLen - 1, SIF_CTL_NO_STOP);
            if (i4Ret == (INT32) u4SubAddrLen - 1)
            {
                i4Ret = _SIF_OptRead(u2ClkDiv, u1DevAddr | 0x1, pData, u4DataLen, SIF_CTL_NORMAL);
                if (u4DataLen != (UINT32) i4Ret)
                {
                              ...
                              UINT8* pData, UINT32 u4DataLen)
{
    ...
    if ((BusSts.SIF_BUS_SCL == 0) || (BusSts.SIF_BUS_SDA == 0))
    {
        ...
        if (i4Ret != SIF_OK)
        {
            LOG(0, "_SIF_DrvRawWrite: Stop Bit error, %d.\n", i4Ret);
            //can't return,because of after callback.
        }
        BusSts = _SIF_CheckBus();                                   // check bus status, again.
        if ((BusSts.SIF_BUS_SCL == 0) || (BusSts.SIF_BUS_SDA == 0)) // still bus busy...
...
static INT32 _SIF_RawSpReadMultipleSubAddr(UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddrNum, UINT32 u4WordAddr,
                                           UINT8* pu1Buf, UINT16 u2ByteCnt)
{
    ...
    return _SIF_DrvRawSpRead(u2ClkDiv, u1DevAddr, (UINT8*)pu1SubAddr, u1WordAddrNum, pu1Buf, u2ByteCnt);
}
...
static INT32 _SIF_X_RawSpRead(UINT8 u1CHannelID, UINT16 u2ClkDiv, UINT8 u1DevAddr, UINT8 u1WordAddrNum,
                              UINT32 u4WordAddr, UINT8* pu1Buf, UINT16 u2ByteCnt)
{
    INT32 i4Ret = SIF_ERROR_PARAM;
    ...
    {
        _rSifDevParm.fgSystemI2C = TRUE;
        i4Ret = _SIF_RawSpReadMultipleSubAddr(u2ClkDiv, u1DevAddr, u1WordAddrNum, u4WordAddr,
                                              pu1Buf, u2ByteCnt);
        _rSifDevParm.fgSystemI2C = FALSE;
    }
    else if (u1CHannelID == SIF_BUS_TUNER_I2C)
...
static VOID SIF_V2_Init(VOID)
{
    ...
    if (!_fgSifInitWait)
    {
        ...
        {
            LOG(0, "SIF_Init: error, SCL stretch and non open-drain functions are both enabled!\n");
            LOG(0, "SIF_Init: disable SCL stretch!\n");
            ASSERT(0);
            _fgEnableSclStretch = FALSE;
        }

        LOG(0, "SIF_Init: SCL stretch enabled %d, open drain mode %d.\n",
                               ...
                               UINT8* pu1Buf, UINT16 u2ByteCnt)
{
    INT32 i4Ret;
    _SIF_UserLock();
    i4Ret = _SIF_X_RawSpRead(u1CHannelID, u2ClkDiv, u1DevAddr, u1WordAddrNum, u4WordAddr,
                             pu1Buf, u2ByteCnt);
    _SIF_UserUnlock();
    if (i4Ret > 0)
    {
