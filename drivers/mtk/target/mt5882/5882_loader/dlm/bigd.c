 ...
 *---------------------------------------------------------------------------*/
/* $Id: //DTV/RELEASE_BR/r_DTV_X_IDTV1401_002306_12_001_37_001/kernel/drivers/mtk/target/mt5882/5882_loader/dlm/bigd.c#1 $ */
...
#define USE_OPENSSL_bn_mod_exp 0

#if USE_OPENSSL_bn_mod_exp
...
int bd_resize(T b, size_t newsize)
{
/*
...
void InFromBuffer(unsigned char* buffer, size_t n_size, T p)
{
	bd_resize(p, (n_size + 3) / 4);
    memcpy(((void*)p->digits), (void*)buffer, n_size);
}

void OutToBuffer(unsigned char* buffer, size_t n_size, T p)
{
	if (n_size < p->ndigits * 4){
		buffer = (unsigned char**)realloc(buffer, p->ndigits * 4);
	}
	memcpy(buffer + (p->maxdigits - p->ndigits)* 4, ((void*)p->digits), p->ndigits * 4);
}
...
int OutToFile(char *filename, T p)
{
    ...
    fwrite(p->digits, 4, p->ndigits, pOutputFile);
...
int InFromFile(char *filename, T p)
{
    ...
    fread(p->digits, 4, p->ndigits, pInputFile);
...
int InFromFile2(char *filename, T p)
{
...
#if USE_OPENSSL_bn_mod_exp
    BIGNUM *pbn_c,*pbn_d,*pbn_n,*pbn_r;
    BN_CTX *pctx;
    unsigned char *ptmp;
    int tmpcnt, bn2bin_len;
#endif
...
#else
#define dbgprint_bn(bn) tmpcnt=BN_bn2bin(bn,ptmp); while(tmpcnt-->0) printf("%02x ", ptmp[tmpcnt]); printf("\n--\n");
// little endian and big endian translation
#define lebe_trans(a,b,len) for(tmpcnt=0;tmpcnt<len;tmpcnt++) ((unsigned char*)a)[tmpcnt]=((unsigned char*)b)[(len-tmpcnt-1)];
    ptmp = (unsigned char*)malloc(n*sizeof(DIGIT_T));
    pbn_c=BN_new();
    lebe_trans(ptmp,x->digits,x->ndigits*sizeof(DIGIT_T));
    BN_bin2bn(ptmp, x->ndigits*sizeof(DIGIT_T), pbn_c);
    pbn_d=BN_new();
    lebe_trans(ptmp,e->digits,e->ndigits*sizeof(DIGIT_T));
    BN_bin2bn(ptmp, e->ndigits*sizeof(DIGIT_T), pbn_d);
    pbn_n=BN_new();
    lebe_trans(ptmp,m->digits,m->ndigits*sizeof(DIGIT_T));
    BN_bin2bn(ptmp, m->ndigits*sizeof(DIGIT_T), pbn_n);

    pbn_r=BN_new();
    pctx=BN_CTX_new();
    status=(BN_mod_exp(pbn_r,pbn_c,pbn_d,pbn_n,pctx)==1)?(0):(-1);

    bn2bin_len = BN_bn2bin(pbn_r,ptmp);
    lebe_trans(y->digits,ptmp,bn2bin_len);
    while(bn2bin_len < (n*sizeof(DIGIT_T)))
    {
        ((unsigned char*)y->digits)[bn2bin_len++]=0;
    }
    BN_free(pbn_c); BN_free(pbn_d); BN_free(pbn_n); BN_free(pbn_r);
    free(ptmp);
#endif
