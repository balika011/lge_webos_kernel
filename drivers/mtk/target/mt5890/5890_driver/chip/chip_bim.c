...
#if defined(CC_SECURE_BOOT_V2) ||  defined(CC_SECURE_BOOT_SCRAMBLE)
#include "gcpu_if.h"
#endif
...
// The efuse secure id can only access in secure world.
INT32 BIM_GetSecureID(BYTE *pu1SecureID, BYTE u1Size)
{
    UINT32 au4SID[4] = {0};

    if (u1Size > SECURE_ID_LEN)
    {
        return -1;
    }

#if !defined(CC_MTK_LOADER) && defined(CC_TRUSTZONE_SUPPORT)
    if (TZ_IsTrustzoneEnable())
    {
        TZ_BIM_GetSecureID((UINT8 *)au4SID, SECURE_ID_LEN);
    }
    else
#endif
    {
    ...
    x_memcpy((void*)pu1SecureID, (void*)au4SID, u1Size);
    return 0;
}
#endif
...
INT32 RSADecryption(UINT32 *pu1Signature, UINT32 *pu4PublicKey, UINT32 *pu4CheckSum, UINT32 e)
{
    ...
    if (BSP_GetIcVersion() == IC_VER_5890_AA) // Oryx ES1
    {
        pfn_modMultTemp = (PFN_MODMULTTEMP)0xf4001143;
    }
    else if(BSP_GetIcVersion() == IC_VER_5861_AA || BSP_GetIcVersion() == IC_VER_5890_AB) // Gazelle ES1 or Oryx MPS1
    {
        pfn_modMultTemp = (PFN_MODMULTTEMP)0xf400138b;
    }
    else
    {
...
INT32 SHA256(const UINT8 *message_array, UINT32 length, UINT8 Message_Digest[32]);
...
BOOL BIM_VerifyLoaderSig(UINT8* pu1LzhsHeader)
{
#ifdef CC_MTK_PRELOADER
    UINT32 au4MiscBlock[64]; // 2048bit key
    UINT32 au4VendorPublicKey[64]; // 2048bit key
    UINT32 au4VendorPublicKey2[64]; // 2048bit key
    LDR_ENV_T* prLdrEnv = (LDR_ENV_T*)CC_LDR_ENV_OFFSET;
    UINT32 u4LoaderSize, u4e = 3;
    UINT8 au1Block[256];
    UINT8 SHA256_Digest[SHA256_HASH_SIZE];
    static UINT8 *pu1VendorPublicKey2;

    x_memcpy((void*)au4VendorPublicKey, (void *)0xfb009f00, sizeof(au4VendorPublicKey));
    ...
    // get the loader length(plaintext or lzhs file) + lzhs header + pack_file_size(see GetPackFileSize() of sign.c)
    u4LoaderSize = ((*(UINT32 *)(pu1LzhsHeader + 4)) + 0x10 + 0x10) & (~0xf);

    // get customer public key 2
    RSADecryption(pu1LzhsHeader + u4LoaderSize + 256, au4VendorPublicKey, au4VendorPublicKey2, u4e);
    // get misc block
    RSADecryption(pu1LzhsHeader + u4LoaderSize, au4VendorPublicKey2, au4MiscBlock, u4e);

    // store customer public key 2
    x_memcpy((void*)prLdrEnv->au4CustKey, (void*)au4VendorPublicKey2, sizeof(prLdrEnv->au4CustKey));

    /*
    Len  1  1        61        1               32                           32                           32                                32                           32                            32
        +--+--+---------------+--+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
        |02|00|PADDING MISC V1|00|      SHA256 Of Loader       |   SHA256 Of Customer Key 2  |      AES key/IV 2           |      Console lock key       |      Console lock IV        |      Console lock key sig   |
        +--+--+---------------+--+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+-----------------------------+
    */

    memset(au1Block, 0, 256);
    DataSwap((UINT32 *)au1Block, au4MiscBlock, 64, 3);
    if (au1Block[0] == 0x02 && au1Block[1] == 0x00)
    {
        GCPU_LoaderInit(0);
        memset(SHA256_Digest, 0, SHA256_HASH_SIZE);
        GCPU_SHA256(pu1LzhsHeader, u4LoaderSize, SHA256_Digest);
        
        if (x_memcmp(SHA256_Digest, (au1Block + 64), SHA256_HASH_SIZE) != 0)
        {
            CHIP_DisplayString("SHA of preloader is wrong\n");
            goto _HALT;

        }

        pu1VendorPublicKey2 = 0xf00140;
        x_memcpy((void*)pu1VendorPublicKey2, (void*)au4VendorPublicKey2, 256);
        GCPU_SHA256(pu1VendorPublicKey2, 256, SHA256_Digest);

        if (x_memcmp(SHA256_Digest, (au1Block + 96), SHA256_HASH_SIZE) != 0)
        {
            CHIP_DisplayString("SHA of customer key 2 is wrong\n");
            goto _HALT;
        }

        CHIP_DisplayString("store RSA & AES keys in DMX SRAM---!!!\n");
        // store RSA & AES keys in DMX SRAM
        x_memcpy(0xfb005000, au4VendorPublicKey2, sizeof(au4VendorPublicKey2));
        x_memcpy(0xfb005000 + sizeof(au4VendorPublicKey2), (au1Block + 128), 32);

#if defined(CC_LOADER_CONSOLE_KEY_SUPPORT)
        x_memcpy((void*)prLdrEnv->console_key, (void*)(au1Block + 160), sizeof(prLdrEnv->console_key));
        x_memcpy((void*)prLdrEnv->console_iv, (void*)(au1Block + 176), sizeof(prLdrEnv->console_iv));
#endif

#if defined(CC_SECURE_BOOT_SCRAMBLE)
        x_memcpy((void*)prLdrEnv->aes_key2, (void*)(au1Block + 128), sizeof(prLdrEnv->aes_key2));
        x_memcpy((void*)prLdrEnv->aes_iv2, (void*)(au1Block + 144), sizeof(prLdrEnv->aes_iv2));
#endif

#ifdef CC_SECURE_BOOT_SCRAMBLE
        GCPU_SetAesKey((au1Block + 128), KEY_BIT_LEN_128, (au1Block + 144));
#endif // CC_SECURE_BOOT_SCRAMBLE
    }
    else
    {
        CHIP_DisplayString("Misc header is wrong\n");
        goto _HALT;
    }
...
_HALT:
    CHIP_DisplayString("System is halted\n");
    while(1);
#endif
    return FALSE;
}

BOOL BIM_VerifyImgSig(UINT8* pu1LzhsHeader)
{
#ifndef CC_MTK_PRELOADER
    UINT32 au4MiscBlock[64]; // 2048bit key
    UINT32 au4VendorPublicKey2[64]; // 2048bit key
    LDR_ENV_T* prLdrEnv = (LDR_ENV_T*)CC_LDR_ENV_OFFSET;
    UINT32 u4ImgSize, u4e = 3;
    UINT8 au1Block[256];
    UINT8 SHA256_Digest[SHA256_HASH_SIZE];

    x_memcpy((void*)au4VendorPublicKey2, (void *)prLdrEnv->au4CustKey, sizeof(prLdrEnv->au4CustKey));
    ...
    // get the loader length(plaintext or lzhs) + lzhs header + pack_file_size(see GetPackFileSize() of sign.c)
    u4ImgSize = ((*(UINT32 *)(pu1LzhsHeader + 4)) + 0x10 + 0x10) & (~0xf);

    // get misc block
    RSADecryption(pu1LzhsHeader + u4ImgSize, au4VendorPublicKey2, au4MiscBlock, u4e);
    ...
    */
    ...
    {
        GCPU_LoaderInit(0);
        GCPU_SHA256(pu1LzhsHeader, u4ImgSize, SHA256_Digest);
        if (x_memcmp(SHA256_Digest, (au1Block + 224), SHA256_HASH_SIZE) != 0)
        {
            Printf("SHA of Img is wrong\n");
            goto _HALT;
        }
    }
    else
    {
        Printf("Img Misc header is wrong\n");
        goto _HALT;
    }
