...
#define DMX_MM_MARLIN_KEY           1//now,marlin TS support noly 1 keyindex
...
typedef struct
{
    BOOL        b_enable;
    UINT16      u2_compid;
}MW_DMX_MARLIN_TS_CRYPT_KEY_SLOT_T;
...
static MW_DMX_MARLIN_TS_CRYPT_KEY_SLOT_T _arMwDmxMarlinTSKey[DMX_MAX_NUM_CA_KEY];
...
BOOL _MW_DMX_CRYPT_AllocateMarlinTSKeySlot(UINT32 u4Flags, 
                                    UINT16 u2compid,
                                    UINT8 *pu1ReturnIdx)
{
    UINT8 u1StartIdx = DMX_MM_CA_KEY_START;
    UINT8 u1TotalNum = DMX_MM_CA_KEY_START;
    UINT8 u1KeyLoop;
    UINT8 u1KeyIdx   = DMX_MM_CA_KEY_START;
    ...
    {
        u1StartIdx = DMX_MM_CA_KEY_START;
        u1TotalNum = DMX_MM_CA_KEY_START + DMX_MM_MARLIN_KEY;
    }
    
    for(u1KeyLoop = u1StartIdx;u1KeyLoop<u1TotalNum;u1KeyLoop++)
    {
        if(_arMwDmxMarlinTSKey[u1KeyLoop].u2_compid == u2compid)
        {
            u1KeyIdx = u1KeyLoop;
            break;
        }
        if(!_arMwDmxMarlinTSKey[u1KeyLoop].b_enable)
        {
            _arMwDmxMarlinTSKey[u1KeyLoop].u2_compid = u2compid;
        }
        else
        {
...
BOOL _MW_DMX_CRYPT_ResetCAKeySlotByKeyIdx(UINT8 u1KeyIdx)
{
    ...
    if (fgRet)
    {
        ...
        if ((u1KeyIdx >= DMX_DTV_CA_KEY_START) && (u1KeyIdx <= DMX_DTV_CA_KEY_END))
        { 
