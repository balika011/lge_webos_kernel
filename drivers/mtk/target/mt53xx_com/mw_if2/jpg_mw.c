...
//static INT32 _IMGGetProg(DRV_GET_TYPE_T type, VOID* pv_get_info);
//static INT32 _IMGGetCap(DRV_GET_TYPE_T type, VOID* pv_get_info);
...
MTIMAGE_HANDLE mw_hHandle = (MTIMAGE_HANDLE)0xFF;
static MTIMAGE_IMG_FORMAT mw_eFormat=MT_IMAGE_MAX_NEW;
static UINT32 mw_u4ImgId=0xff;
...
#if 1 /* MTAL_PORTING: moved from dtv_linux/dtv_process/callback/mw_adpt_cb_table.c */

typedef struct _JPG_IMG_SET_NFY_PARM{
...
IMG_FILL_BUF_T t_attach_buf;     /* Optional, Only use for JPG_HW_NFY_FILL_BUF */
IMG_NFY_FCT_T   pf_nfy;
} JPG_IMG_SET_NFY_PARM;
...
extern VOID _rm_jpg_nfy(VOID * p_param);
...
INT32 MW_JpgInit(void)
{
    ...
    */
    ...
    i4Ret = x_rm_reg_comp(&_rJpgComp,
                          ...
                          1,
                          "JPG",
#if (defined(CC_MT5368) || defined(CC_MT5396) || defined(CC_MT5398) || defined(CC_MT5880)  || defined(CC_MT5399) || defined(CC_MT5890) || defined(CC_MT5882))
    ...
    SIZE_T*             pz_get_info_len)
{
    ...
    case IMG_GET_TYPE_PROGRESS:
        ...
        i4Ret = MTIMAGE_MW_GetInfo(mw_hHandle, MT_IMAGE_TYPE_PROGRESS, pv_get_info);
        break;
    case IMG_GET_TYPE_LIMITATION:
        //i4Ret = _IMGGetCap(pv_get_info);
        i4Ret = MTIMAGE_MW_GetInfo(mw_hHandle, MT_IMAGE_TYPE_LIMITATION, pv_get_info);
        break;
    default:
...
*/
    ...
    MTIMAGE_MW_RegCb(mw_hHandle, _rm_jpg_nfy);

    #if defined(CC_MT5890)
    if(MTIMAGE_MW_SetInit_Ex(mw_u4ImgId, mw_hHandle, mw_eFormat,(void *)prInitInfo)!=0)// oryx jpeg in CHA, and webp use png fbm
    #else
    if(MTIMAGE_MW_SetInit(mw_u4ImgId, mw_hHandle,(void *)prInitInfo)!=0)
    #endif
    {
        //error case
        i4Ret =1;
...
static INT32 _IMGSetDec(IMG_DECODE_T *prDec)
{
    MTIMG_MW_PARAM_T rInfo;

    x_memset((void *)&rInfo, 0, sizeof(MTIMG_MW_PARAM_T));
    //src bitstream
    ...
    rInfo.bIsVitualOutputAddr = prDec->b_is_virtual_addr;
    rInfo.eFormat=MTIMG_GetImageType(mw_u4ImgId);
    switch (rInfo.eFormat)
    {
    case MT_IMAGE_PNG_NEW:
        //src info
    ...
    else if (prDec->e_dst_cm == GFX_COLORMODE_YUV_422_RS)
    {
        ...
        break;
    case MT_IMAGE_JPEG_NEW:
    case MT_IMAGE_WEBP_NEW:
        if (prDec->e_dst_cm == GFX_COLORMODE_AYCbCr_D8888)
        {
            rInfo.eDispColorMode = AYCBCR8888_DIRECT32;
        }
    ...
    MTIMAGE_MW_SetDecode(mw_u4ImgId, mw_hHandle, &rInfo);       
    //IMG_Decode(rInfo);
    return RMR_OK;
}
...
{
    MTIMAGE_Stop(mw_u4ImgId, mw_hHandle);
    //IMG_Stop(0);
    return RMR_OK;
}
...
static INT32 _IMGSetEnd(void)
{
    MTIMAGE_Close(mw_u4ImgId, mw_hHandle);
    //IMG_Release(0);
    return RMR_OK;
}
...
static INT32 _IMGSetBufFilled(IMG_BUF_FILLED_T* prBufInfo)
{
    ...
    MTIMAGE_MW_SetBufFilled(mw_u4ImgId, mw_hHandle, prBufInfo);
/*  
...
static INT32 _IMGSetTypeConfig(ENUM_IMG_TYPE img_type)
{
    MTIMAGE_IMG_FORMAT imgtype;
    UINT32 u4ImgId=0;
    
    if(img_type==IMAGE_TYPE_JPEG)
    {
       imgtype=MT_IMAGE_JPEG_NEW;
    }
    else if(img_type==IMAGE_TYPE_WEBP)
    {
        imgtype=MT_IMAGE_WEBP_NEW;
    }
    else if(img_type==IMAGE_TYPE_PNG)
    {
        imgtype=MT_IMAGE_PNG_NEW;
        u4ImgId = 1;
        
    }
    else
    {
        imgtype=MT_IMAGE_MAX_NEW;
    }
    ...
    #if (defined(CC_MT5890))
    if(MTIMAGE_MW_Connect_Ex(mw_u4ImgId, mw_eFormat)!=MTR_OK)
    #else
    if(MTIMAGE_MW_Connect(mw_u4ImgId)!=MTR_OK)
    #endif
    {
        return RMR_DRV_SET_FAILED;
    }
    
    if(imgtype!=MT_IMAGE_MAX_NEW)
    {
        //IMG_Config(0,imgtype);
        MTIMAGE_MW_ConfigImageDecoder(mw_u4ImgId, mw_hHandle, imgtype);
        return RMR_OK;
    }
    else
    {
