...
#define GPIO_DONE                       	MTDRVCUST_InitGet(ePowerLedGpio)
    ...
    SIZE_T         *pz_get_info_len)
{
    ...
    case IND_GET_TYPE_STATE:
        ...
        if (pt_comp_id->ui2_id == IND_POWER_ID) {
            ASSERT(pv_get_info!=NULL);
            if (MTDRVCUST_OptGet(ePowerLedGpio) == 0)
            {
                *((IND_STATE_T *)pv_get_info) =
                    (MTGPIO_SetOut(GPIO_DONE, 0)==(MTDRVCUST_OptGet(ePowerLedOnVal))) ? IND_STATE_ENABLE : IND_STATE_DISABLE;
            }
            else
            {
    ...
    case IND_SET_TYPE_STATE:
        ...
        if (pt_comp_id->ui2_id == IND_POWER_ID) {
            ...
            case IND_STATE_DISABLE:
                i4_state = !MTDRVCUST_OptGet(ePowerLedOnVal);
                LINT_SUPPRESS_NEXT_EXPRESSION(534);
                if (MTDRVCUST_OptGet(ePowerLedGpio) == 0)
                {
                    MTGPIO_SetOut(GPIO_DONE, i4_state);
                }
                i4_ret = RMR_OK;
                break;
