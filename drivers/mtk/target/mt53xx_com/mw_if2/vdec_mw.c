...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
 COMP_INFO_FOR_CHANNEL_T _rMChannelCompInfo;
 COMP_INFO_FOR_CHANNEL_T _rPChannelCompInfo;
                       ...
                       VOID*            pv_arg)
{
    ...
    TV_DEC_CC_DATA_T* p_tmp = (TV_DEC_CC_DATA_T *)pv_arg;
...
static VOID _TvdResetNfyStatus( UINT32  u4VidPath )
{
	...
	TVD_DBG_MSG( " _TvdResetNfyStatus(u4VidPath=%d)\n", u4VidPath);
...
#endif
{
    ...
    ui4_ttx_kernel_virt_start[ui1_path]  = (BYTE *) _kernal_mem_alloc_pages( ptCtx->u4SzBuf);        
    ptCtx->pbBuf = ui4_ttx_kernel_virt_start[ui1_path];

    set_tv_dec_ttx_nfy(ui1_path,
        NULL,
        TV_DEC_TTX_COND_MEM_MAP,
        ui4_ttx_kernel_virt_start[ui1_path],
        ptCtx->u4SzBuf);
#endif
...
static void _TvdEuWssStartNotify (UINT8 u1Path)
{
    ...
    MTVDECEX_VBI_StartNotifyWSSOnOff(u1Path, SV_ON, NULL);
}
...
{
    MTVDECEX_VBI_StartNotifyWSSOnOff(u1Path, SV_OFF, NULL);
}
...
static void _TvdUsWssStartNotify (UINT8 u1Path)
{
    ...
    MTVDECEX_VBI_StartNotifyUSWSSOnOff(u1Path, SV_ON, NULL);
}
...
{
    MTVDECEX_VBI_StartNotifyUSWSSOnOff(u1Path, SV_OFF, NULL);
}
...
UINT8 u1Mw_HdmiSetVdoTxtureMode(UINT8 enable)
{
	MTVDECEX_HDMI_VIDEO_MODE_T hdmiVdotxture;
	hdmiVdotxture.enable = enable;
	MTVDECEX_HDMI_W2D_SetVideoMode(&hdmiVdotxture);
	return 1;
}
...
INT32 MW_TvdInit( DRV_CUSTOM_DIR_T* pt_drv_cust_dir )
{
    ...
    UINT32 u4Idx, u4Val=0;
    UINT32 u4TargetCountry;
    ...
    MTVDECEX_SCART_RegPin8ChgNfyCb((MTSCART_PFN_PIN8_NFY_CB)vNotifyScart_Sig_Chg);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_ANA_SIG_CHG, (UINT32)_Tvd_NotifyTVSig, 0);    //Tuner analog tvd notify 
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_SIG_CHG,(UINT32) _DrvNotifySigChg, 0);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_VGA_AUTO, (UINT32)_TvdNotifyAutoDone, 0);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_AUTO_COLOR, (UINT32)_TvdNotifyAutoColorDone, 0);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_RESOLUTION_CHG, (UINT32)_TvdNotifyResolutionChg, 0);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_COMP_SIG_CHG, (UINT32)vDrvNotifyComp_Sig_Chg, 0);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_COLOR_SYSTEM,(UINT32) _TvdNotifyColorSystem, 0);
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_MV_CHG, (UINT32)_TvdNotifyMVChg, 0);
    #ifdef CC_SOURCE_AUTO_DETECT
    MTVDECEX_RegCbFunc(MTVDECEX_CB_FUNC_AUTO_DETECT_SRC, (UINT32)_TvdNOtifyAutoDetectSource, 0);
    #endif

    //MTDRVCUST_GetCountry(&u4TargetCountry);
    u4TargetCountry = MTDRVCUST_GetCountry();
    if(u4TargetCountry==COUNTRY_EU)
    {
        MTVDECEX_VBI_SetTtxNfy((MTVDECEX_PFN_TTX_NFY)TvdNotifyTtxData);
        MTVDECEX_VBI_SetVpsNfy((MTVDECEX_PFN_VPS_NFY)TvdNotifyVPSData);
        MTVDECEX_VBI_SetWssNfy((MTVDECEX_PFN_WSS_NFY)TvdNotifyEUWSSData);
        MTVDECEX_VBI_SetUsWssNfy((MTVDECEX_PFN_WSS_NFY)TvdNotifyUSWSSData);
    }
    else if((u4TargetCountry==COUNTRY_US)||(u4TargetCountry==COUNTRY_BZ))
    { 
        MTVDECEX_VBI_SetCcNfy((MTVDECEX_PFN_CC_NFY)TvdNotifyCCData);
        MTVDECEX_VBI_SetUsWssNfy((MTVDECEX_PFN_WSS_NFY)TvdNotifyUSWSSData);
        MTVDECEX_VBI_SetWssNfy((MTVDECEX_PFN_WSS_NFY)TvdNotifyEUWSSData);
        MTVDECEX_VBI_SetTypeBWssNfy((MTVDECEX_PFN_TYPE_B_WSS_NFY)TvdNotifyTypeBWSSData);
    }
    ...
    if(MTDRVCUST_OptQuery(eTVDCCNRLThresholdBlock, &u4Val) < 0)
    {
        _u1CCNRLBlock = 80;
    }
    else
    ...
    if(MTDRVCUST_OptQuery(eTVDCCNRLThresholdUnblock, &u4Val) < 0)
    {
        _u1CCNRLUnblock = 75;
    }
    else
    ...
    if(MTDRVCUST_OptQuery(eTVDCCPERThresholdBlock, &u4Val) < 0)
    {
        _u1CCPERBlock   = 100;
    }
    else
    ...
    if(MTDRVCUST_OptQuery(eTVDCCPERThresholdUnblock, &u4Val) < 0)
    {
        _u1CCPERUnblock = 90;
    }
    else
...
static INT32 _TvdMwConnect(DRV_COMP_ID_T*   pt_comp_id,
              ...
              x_rm_nfy_fct     pf_nfy)
{
	TVD_DBG_MSG( "\r\n*** _TvdMwConnect  e_conn_type=0x%x***\r\n", e_conn_type);
    ...
    UINT8           u1Ret;
    COMP_INFO_FOR_CHANNEL_T*      pt_rv_channel = NULL;
    ...
    {
        pt_rv_channel = &_rMChannelCompInfo;
        pt_nfy_sts = &_tMainNotifyStatus;
        ui1_path = SV_VP_MAIN;
    }
    else if ( pt_comp_id->ui2_id == 1 ) 
    {
        pt_rv_channel = &_rPChannelCompInfo;
    ...
    else
    {
        ...
        //Printf("Main Select Error, %d, %d\n\n", pt_rv_channel->bDecType, _rPChannelCompInfo.bDecType);
        return(RMR_DRV_INV_CONN_INFO);
    }

#else  /* NOT SUPPORT_S_DETECT */
    u1Ret = u1Mw_SetVideoSrc(ui1_path, rGetVSrcMap(prVSrcCompReg->e_type, prVSrcCompReg->ui2_id));
    ...
    SIZE_T           z_disc_info_len )
{
	...
	TVD_DBG_MSG( "\r\n*** _TvdMwDisconnect  e_disc_type=0x%x***\r\n", e_disc_type);
    ...
    u1Mw_SetVideoSrc(ui1_path, SV_VS_MAX);
          ...
          SIZE_T*         pz_get_info_len)
{
	...
	TVD_DBG_MSG( "\r\n*** _TvdMwGet  e_get_type=0x%x***\r\n", e_get_type);

#ifndef CC_DRIVER_PROGRAM

    MTVDECEX_CS_T u1ColSys;
    UINT8 u1Path = SV_VP_MAIN;
    MTVDO_DEC_TYPE_T eDecType = MTVDO_DEC_NA;
    INT32 i4_return;
    ...
    UINT16 u2VgaClock;
    UINT8 u1VgaPhase;
    MTVDO_TIMING_INFO_T rMtvdoTimingInfo;
    MTVDECEX_PORCHTUNE_TYPE_T ePorchTuneType = MTVDECEX_PORCHTUNE_DEC;
    MTVDECEX_SIG_STATUS eSignalStatus = MTVDECEX_VDO_UNKNOWN;
    MTVDECEX_HDMI_MODE_T eHDMImode = MTVDECEX_HDMI_ARATIO_MAX;
    ...
    case TV_DEC_GET_TYPE_CTRL:
        if (*pz_get_info_len >= sizeof(TV_DEC_CTRL_T))
        {
            ...
            {
                *(TV_DEC_CTRL_T*)pv_get_info = _rMChannelCompInfo.t_Tvd_Ctrl;
            ...
            {
                *(TV_DEC_CTRL_T*)pv_get_info = _rPChannelCompInfo.t_Tvd_Ctrl;
                *pz_get_info_len = sizeof(TV_DEC_CTRL_T);
                i4_return = RMR_OK;
    ...
    case TV_DEC_GET_TYPE_RESOLUTION:
        ...
        {
#ifdef CC_SUPPORT_TVE
            MTTVE_GetBypPath((pt_comp_id->ui2_id==3)?        /* hw byp */
                                         ((MTTVE_DAC1==TV_ENC_SCART_1)?(MTTVE_DAC1):(MTTVE_DAC2)):
                                         ((MTTVE_DAC2==TV_ENC_SCART_2)?(MTTVE_DAC2):(MTTVE_DAC1)), &u1BypPath);
            if( pt_comp_id->ui2_id > 1 && /* SCART out id */ 
                TVE_BYP_EXT_PATH ==  u1BypPath)
            {
    ...
    case TV_DEC_GET_TYPE_TV_SIGNAL:
        ...
        else
        {
            ...
            *pz_get_info_len = sizeof(BOOL);
            ...
            {
                MTTVE_GetBypPath((pt_comp_id->ui2_id==3)?
                                         ((MTTVE_DAC1==TV_ENC_SCART_1)?(MTTVE_DAC1):(MTTVE_DAC2)):
                                         ((MTTVE_DAC2==TV_ENC_SCART_2)?(MTTVE_DAC2):(MTTVE_DAC1)), &u1BypPath);  
                switch(u1BypPath)
                {
    ...
    case TV_DEC_GET_TYPE_COLOR_SYS_CAP:
        if (*pz_get_info_len >= sizeof(UINT32))
        {
            ...
            UINT8 u1IsMainTvd=0;
            UINT8 u1IsPipTvd=0;
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_MAIN_TVD, &u1IsMainTvd);
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_TVD, &u1IsPipTvd);
            if( ((pt_comp_id->ui2_id == 0) && (u1IsMainTvd))  || 
                ((pt_comp_id->ui2_id == 1) && (u1IsPipTvd)))
            {
    ...
    case TV_DEC_GET_TYPE_COLOR_SYS:
        ...
        if (*pz_get_info_len >= sizeof(TV_DEC_COLOR_SYS_T))
        {
            ...
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_MAIN_TVD, &u1IsMainTvd);
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_TVD, &u1IsPipTvd);
            if(((pt_comp_id->ui2_id == 0 || pt_comp_id->ui2_id == 2 
            || pt_comp_id->ui2_id == 3) && (u1IsMainTvd == TRUE))  
            || ((pt_comp_id->ui2_id == 1) && (u1IsPipTvd == TRUE)))
            {
                 MTVDECEX_GetColorSystem(&u1ColSys);
                *pz_get_info_len = sizeof(TV_DEC_COLOR_SYS_T);
                ...
                break;
                case MTVDECEX_CS_AUTO:
                case MTVDECEX_CS_NONE:
                case MTVDECEX_CS_MAX:
                default:
                    break;
                }
    ...
    case TV_DEC_GET_TYPE_CLOCK:
        if (*pz_get_info_len >= sizeof(UINT8))
        {
            UINT8 u1IsMainVga=0;
            UINT8 u1IsPipVga=0;
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_MAIN_VGA, &u1IsMainVga);
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_VGA, &u1IsPipVga);
            if(((pt_comp_id->ui2_id == 0) && u1IsMainVga)
            || ((pt_comp_id->ui2_id == 1) && u1IsPipVga))
            {
                MTVDECEX_GetClock(MTVDECEX_VGA_CLOCK_CURRENT, &u2VgaClock);
                *(INT16*)pv_get_info = u2VgaClock -127;//i2GetTypeClock;
    ...
    case TV_DEC_GET_TYPE_CLOCK_MAX:
        if (*pz_get_info_len >= sizeof(UINT8))
        {
            ...
            || ((pt_comp_id->ui2_id == 1) && u1IsPipVga))
            {
                  MTVDO_GetTimingInfo(pt_comp_id->ui2_id, &rMtvdoTimingInfo);
		    if(rMtvdoTimingInfo.u4Interlace)
		    {
			    *(INT16*)pv_get_info=0;
		    }
		    else
		    {
			    //*(INT16*)pv_get_info = VGA_CLOCK_MAX;
                         MTVDECEX_GetClock(MTVDECEX_VGA_CLOCK_MAX, &u2VgaClock);
			    *(INT16*)pv_get_info = u2VgaClock - 127;
		    }  
    ...
    case TV_DEC_GET_TYPE_PHASE:
        if (*pz_get_info_len >= sizeof(UINT8))
        {
            UINT8 u1IsMainVga=0;
            UINT8 u1IsPipVga=0;
            UINT8 u1IsMainYpbpr=0;
            UINT8 u1IsPipYpbpr=0;
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_MAIN_VGA, &u1IsMainVga);
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_VGA, &u1IsPipVga);
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_MAIN_YPBPR, &u1IsMainYpbpr);
            MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_YPBPR, &u1IsPipYpbpr);
            if(((pt_comp_id->ui2_id == 0) && (u1IsMainVga || u1IsMainYpbpr)) ||
                ((pt_comp_id->ui2_id == 1) && (u1IsPipVga || u1IsPipYpbpr)))
            {
                MTVDECEX_GetPhase(MTVDECEX_VGA_PHASE_CURRENT, &u1VgaPhase);
                *(UINT16*)pv_get_info = u1VgaPhase;//u2GetTypePhase;//
    ...
    case TV_DEC_GET_TYPE_H_POSITION:
	{
		 ...
		 MTVDECEX_GetVideoDecType(ui1_path, &eDecType); 

               MTVDECEX_GetPorch(ui1_path, MTVDECEX_HPORCH_DEFAULT, &def);
               MTVDECEX_GetPorch(ui1_path, MTVDECEX_HPORCH_MAX, &max);
               MTVDECEX_GetPorch(ui1_path, MTVDECEX_HPORCH_MIN, &min);
               MTVDECEX_GetPorch(ui1_path, MTVDECEX_HPORCH_CURRENT, &pos);
               MTVDECEX_GetPorchTune(ui1_path, &ePorchTuneType);

		 if (ePorchTuneType == MTVDECEX_PORCHTUNE_SCPOS)//for DTV
		 {
    ...
    case TV_DEC_GET_TYPE_V_POSITION:
            MTVDECEX_GetPorchTune(pt_comp_id->ui2_id, &ePorchTuneType);
	    if (ePorchTuneType == MTVDECEX_PORCHTUNE_SCPOS)//for DTV
	     {
		    //force SCPOS HV
		    UINT16 def=0,pos=0;
                    MTVDECEX_GetPorch(pt_comp_id->ui2_id, MTVDECEX_VPORCH_DEFAULT, &def);
                    MTVDECEX_GetPorch(pt_comp_id->ui2_id, MTVDECEX_VPORCH_CURRENT, &pos);
                    MTVDECEX_SetPorchTune((UINT8)pt_comp_id->ui2_id, SV_PORCHTUNE_SCPOS);
    ...
    case TV_DEC_GET_TYPE_CAPABILITY:
        if (*pz_get_info_len >= sizeof (UINT32))
        {
            if(pt_comp_id->ui2_id == 0) // main
            {
                ...
                *(UINT32*)pv_get_info = _rMChannelCompInfo.u4_Cap | TV_DEC_CAP_TTX_HW;
            }
            else if (pt_comp_id->ui2_id == 1)
            {
                /* for main/sub TTX_HW flag is needed for middleware 0924 */
                *(UINT32*)pv_get_info = _rPChannelCompInfo.u4_Cap | TV_DEC_CAP_TTX_HW;
            }
    ...
    case TV_DEC_GET_TYPE_MIN_MAX:
        if (*pz_get_info_len >= sizeof(TV_DEC_MIN_MAX_INFO_T))
        {
            ...
            MTVDECEX_GetVideoDecType(u1Path, &eDecType);
            MTVDECEX_GetPorchTune(u1Path, &ePorchTuneType);
		...
		switch (pt_tv_dec_min_max_info->e_get_type)
		{
			...
			{
			        UINT16 u2Tmp = 0;
        	                MTVDECEX_GetPorch(u1Path, MTVDECEX_HPORCH_MIN, &u2Tmp);
        	                pt_tv_dec_min_max_info->ui4_min_value = u2Tmp ;
        	                MTVDECEX_GetPorch(u1Path, MTVDECEX_HPORCH_MAX, &u2Tmp);
        	                pt_tv_dec_min_max_info->ui4_max_value = u2Tmp ;
		       }
			else if (ePorchTuneType == MTVDECEX_PORCHTUNE_SCPOS)//for DTV
			{
			...
			{
				UINT16 def=0;
				MTVDECEX_GetPorch(u1Path, MTVDECEX_HPORCH_DEFAULT, &def);
	...
	//			pt_tv_dec_min_max_info->ui4_max_value = wDrvVideoGetPorch(u1Path,SV_HPORCH_MAX);
			...
			{
			        UINT16 u2Tmp ;
        	                MTVDECEX_GetPorch(u1Path, MTVDECEX_VPORCH_MIN, &u2Tmp);
        	                pt_tv_dec_min_max_info->ui4_min_value = u2Tmp ;
        	                MTVDECEX_GetPorch(u1Path, MTVDECEX_VPORCH_MAX, &u2Tmp);
        	                pt_tv_dec_min_max_info->ui4_max_value = u2Tmp ;
			}
			else  if (ePorchTuneType == MTVDECEX_PORCHTUNE_SCPOS)//for DTV
			{
			...
			{
				UINT16 def=0;
				MTVDECEX_GetPorch(u1Path, MTVDECEX_VPORCH_DEFAULT, &def);
    ...
    case TV_DEC_GET_TYPE_AMBIG_VGA_TIMING:
        ...
        {
            MTVDECEX_VGA_GetAmbiguousTimingNum(&(pt_tv_dec_ambig_vga_timing_info->u.ui2_num));
            i4_return = RMR_OK;
        }
        else 
        {
            //VGAAmbTimingInfo_t rAmbTiming;
            MTVDECEX_VGA_AMBIG_TIMING_T rMTAmbTiming;
            if((pt_tv_dec_ambig_vga_timing_info->e_type) == TV_DEC_AMBIG_VGA_TIMING_TYPE_OPTION_IN_GRP)
            {
                MTVDECEX_VGA_QueryAmbiguousTiming(pt_tv_dec_ambig_vga_timing_info->u.t_opt.ui2_opt_index, &rMTAmbTiming, 1);
            }
            else if((pt_tv_dec_ambig_vga_timing_info->e_type) == TV_DEC_AMBIG_VGA_TIMING_TYPE_CURR_OPT_IN_GRP)
            {
                MTVDECEX_VGA_QueryAmbiguousTiming(QUERY_CUR_AMBIGUOUS_TIMING, &rMTAmbTiming, 1);
            }

            pt_tv_dec_ambig_vga_timing_info->u.t_opt.ui2_opt_index = rMTAmbTiming.bCandiIdx;
            pt_tv_dec_ambig_vga_timing_info->u.t_opt.ui2_ver_frequncy = rMTAmbTiming.bVer_freq;
            pt_tv_dec_ambig_vga_timing_info->u.t_opt.ui4_hor_resolution = rMTAmbTiming.wHor_res;
            pt_tv_dec_ambig_vga_timing_info->u.t_opt.ui4_ver_resolution = rMTAmbTiming.wVer_res;
            
            if(rMTAmbTiming.bVer_freq != 0)
            {
                i4_return = RMR_OK;
            }
            else
    ...
    case TV_DEC_GET_TYPE_VGA_STATUS:
        i4_return = RMR_OK;
        MTVDECEX_GetVideoStatus(MTVDO_SUB, &eSignalStatus);
        switch(eSignalStatus)
        {
            case MTVDECEX_VDO_STABLE:
                *((TV_DEC_VGA_STATUS_T*)pv_get_info) = TV_DEC_VGA_SIGNAL_STABLE;
                break;
            
            case MTVDECEX_VDO_NOSIGNAL:
            case MTVDECEX_VDO_UNKNOWN:
                *((TV_DEC_VGA_STATUS_T*)pv_get_info) = TV_DEC_VGA_SIGNAL_NO_SIGNAL;
                break;
            
            case MTVDECEX_VDO_NOSUPPORT:
        ...
        break;
	case TV_DEC_GET_TYPE_HDMI_DVI_MODE:
        i4_return = RMR_OK;
        MTVDECEX_HDMI_GetMode(&eHDMImode);
        switch(eHDMImode)
        {
            case MTVDECEX_HDMI_NO_SIGNAL:
                *((TV_DEC_HDMI_DVI_MODE_T*)pv_get_info) = TV_DEC_HDMI_DVI_MODE_NO_SIGNAL;
                break;
            
            case MTVDECEX_HDMI_HDMI:
                *((TV_DEC_HDMI_DVI_MODE_T*)pv_get_info) = TV_DEC_HDMI_DVI_MODE_HDMI;
                break;
            
            case MTVDECEX_HDMI_DVI:
                *((TV_DEC_HDMI_DVI_MODE_T*)pv_get_info) = TV_DEC_HDMI_DVI_MODE_DVI;
                break;
            default:
                i4_return = RMR_DRV_INV_GET_INFO;
                break;
	...
	case TV_DEC_GET_TYPE_SCANADJ_INFO:
		{
			TV_DEC_SCANADJ_INFO_T tScanadjInfo;
			if (MTVDECEX_TVD_GetScanadjInfo((MTVDECEX_SCANADJ_INFO_T *)&tScanadjInfo) == MTR_OK)
			{
				*((TV_DEC_SCANADJ_INFO_T *)pv_get_info) = tScanadjInfo;
				*pz_get_info_len = sizeof(TV_DEC_SCANADJ_INFO_T);
				i4_return = RMR_OK;
			}
			else
			{
    ...
    case TV_DEC_GET_TYPE_SIGNAL_STRENGTH:
        ...
        else
        {
            ...
            *pz_get_info_len = sizeof(UINT8);
            i4_return = RMR_OK;
            MTVDECEX_TVD_GetSignalStrengthLevel(2, &u1SigLevel);
            *(UINT8*)pv_get_info = u1SigLevel;
        }
        break;   
	case TV_DEC_GET_TYPE_MHL_PORT_ID:
		...
		else
		{
            ...
            *pz_get_info_len = sizeof(UINT8);
            i4_return = RMR_OK;
		    MTVDECEX_HDMI_GetMhlPortId(&u1Port);
		...
		case TV_DEC_GET_TYPE_SRC_DETECT_STATUS:
			//MTVDECEX_GetSrcDetectStatus((MTVDECEX_TV_DEC_SOURCE_STATUS_T*)pv_get_info);
			//i4_return = RMR_OK;
			{
				_TV_DEC_SOURCE_STATUS_T u1Status;
				if (MTVDECEX_GetSrcDetectStatus((MTVDECEX_TV_DEC_SOURCE_STATUS_T*)&u1Status) == MTR_OK)
				{
					*((_TV_DEC_SOURCE_STATUS_T *)pv_get_info) = u1Status;
					*pz_get_info_len = sizeof(_TV_DEC_SOURCE_STATUS_T);
					i4_return = RMR_OK;
...
static INT32 _TvdMwSet(
    ...
    SIZE_T          z_set_info_len)
{
	TVD_DBG_MSG( "\r\n*** _TvdMwSet  e_set_type=0x%x***\r\n", e_set_type);

#ifndef CC_DRIVER_PROGRAM
    
    INT32          i4RetCode = RMR_DRV_INV_SET_INFO;
    MTVDECEX_CS_T          bColSys;
    INT16          i2Tmp;
    MTVDO_DEC_TYPE_T          eDecType = MTVDO_DEC_NA;
#ifdef SUPPORT_VGA_UI_AMBIGUOUS_TIMING_SELECT        
    ...
    _NotifyStatus* pt_nfy_sts = NULL;
    UINT8          ui1_path = 0;
    COMP_INFO_FOR_CHANNEL_T      rTvechannel;
#if SUPPORT_SCART
    UINT8 u1IsMainScart=0;
#endif
#if SUPPORT_POP
    UINT8 u1IsPipScart=0;
#endif
    ...
    case TV_DEC_SET_TYPE_SIG_BOOSTER://singnal booster add by Hua 0609       
      	if (MTVDECEX_SetSignalBooster((BOOL)((UINT32)pv_set_info))==MTR_OK)
      	{
            i4RetCode = RMR_OK;
      	}
 	break;
    ...
    case TV_DEC_SET_TYPE_PLAY_CC:
             ...
             (((TV_DEC_CC_NFY_INFO_T*)pv_set_info)->pf_cc_nfy != NULL) )
        {
            ...
            MTVDECEX_VBI_StartNotifyCCOnOff(ui1_path, SV_ON, NULL);
    ...
    case TV_DEC_SET_TYPE_STOP_CC:
        ...
        MTVDECEX_VBI_StartNotifyCCOnOff(ui1_path, SV_OFF, NULL);
    ...
    case TV_DEC_SET_TYPE_COLOR_SYS:
        ...
        if( (fgIsCap == FALSE) || 
            (MTVDECEX_SetColorSystem(bColSys) == MTR_OK))
        {
               i4RetCode = RMR_OK;
        }
        else
    ...
    case TV_DEC_SET_TYPE_CLOCK:
        ...
        if(MTR_OK==MTVDECEX_SetClock((UINT16)(i2Tmp+127)))
        {
    ...
    case TV_DEC_SET_TYPE_PHASE:
        if(MTR_OK==MTVDECEX_SetPhase((UINT16)(UINT32)pv_set_info))
        {
    ...
    case TV_DEC_SET_TYPE_H_POSITION:
		...
		if (ePorchTuneType == MTVDECEX_PORCHTUNE_SCPOS)//for DTV
		{
		       ...
		       MTVDECEX_SetPorch(ui1_path, MTVDECEX_HPORCH_CURRENT,pos);
		}
		else
		{
			if (eDecType == MTVDO_DEC_VGA)
    ...
    case TV_DEC_SET_TYPE_V_POSITION:
		...
		{
		       UINT16 def = 0;
			MTVDECEX_SetPorchTune((UINT8)ui1_path,MTVDECEX_PORCHTUNE_SCPOS);
			MTVDECEX_GetPorch(ui1_path,MTVDECEX_VPORCH_DEFAULT,&def);
			MTVDECEX_SetPorch(ui1_path, MTVDECEX_VPORCH_CURRENT, (INT32)pv_set_info-16+def);
		}
		else
		{
			MTVDECEX_SetPorch(ui1_path, MTVDECEX_VPORCH_CURRENT, (UINT16)(UINT32)pv_set_info);
		}
	}
	    u2GetTypeHPos =  (UINT16)(UINT32)pv_set_info;
	    i4RetCode = RMR_OK;
    ...
    case TV_DEC_SET_TYPE_AUTO_COLOR:
        ...
        #endif
        #if SUPPORT_POP
        MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_SCART, &u1IsPipScart);
        #endif
...
else
	{
#endif
               MTVDECEX_GetVideoDecType(ui1_path, &eDecType);
 	        if ((eDecType == MTVDO_DEC_VGA) || (eDecType == MTVDO_DEC_YPBPR))
	        {
	            if(MTVDECEX_StartAutoColor())
	            {
	                i4RetCode = RMR_ASYNC_NFY;
	        ...
	        else if(eDecType == MTVDO_DEC_TVD3D)
	        {
	            if(MTVDECEX_StartCVBSAutoColor())
	            {
	                i4RetCode = RMR_ASYNC_NFY;
	            }
		     else
    ...
    case TV_DEC_SET_TYPE_SCART_INP_MODE:
            ...
            (pv_set_info != NULL))
        {
            ...
            MTVDECEX_SCART_SetInputMode((*(TV_DEC_SCART_MODE_T*)pv_set_info)-1);
            i4RetCode = RMR_OK;            
        }
        else
        {
    ...
    case TV_DEC_SET_TYPE_PLAY_TTX:
             ...
             (((TV_DEC_TTX_NFY_INFO_T *)pv_set_info)->pf_ttx_nfy != NULL) )
        {
            ...
            {
                MTVDECEX_VBI_StartNotifyTTXOnOff(ui1_path, SV_ON, NULL);
            }
    ...
    case TV_DEC_SET_TYPE_STOP_TTX:
        {
            ...
            {
                MTVDECEX_VBI_StartNotifyTTXOnOff(ui1_path, SV_OFF, NULL);
            }
    ...
    case TV_DEC_SET_TYPE_MONITOR_VPS_DATA:
        if ( ( ui1_path == SV_VP_MAIN && (_rMChannelCompInfo.u4_Cap & TV_DEC_CAP_VPS) ) || 
             ( ui1_path == SV_VP_PIP  && (_rPChannelCompInfo.u4_Cap & TV_DEC_CAP_VPS) )  ) 
        {
                 ...
                 (((TV_DEC_VPS_DATA_NFY_INFO_T *)pv_set_info)->pf_vps_data_nfy != NULL) )
            {
                ...
                MTVDECEX_VBI_StartNotifyVPSOnOff(MTVDO_MAIN, SV_ON, NULL);
    ...
    case TV_DEC_SET_TYPE_STOP_VPS_DATA:
             ...
             ( ui1_path == SV_VP_PIP  && (_rPChannelCompInfo.u4_Cap & TV_DEC_CAP_VPS) )  ) 
        {
            ...
            MTVDECEX_VBI_StartNotifyVPSOnOff(MTVDO_MAIN, SV_OFF, NULL);
    ...
    case TV_DEC_SET_TYPE_PLAY_EU_WSS:
        if ( ( ui1_path == SV_VP_MAIN && (_rMChannelCompInfo.u4_Cap & TV_DEC_CAP_EU_WSS) ) || 
             ( ui1_path == SV_VP_PIP  && (_rPChannelCompInfo.u4_Cap & TV_DEC_CAP_EU_WSS) )  ) 
        {
    ...
    case TV_DEC_SET_TYPE_PLAY_US_WSS:
        if ( ( ui1_path == SV_VP_MAIN && (_rMChannelCompInfo.u4_Cap & TV_DEC_CAP_US_WSS) ) || 
             ( ui1_path == SV_VP_PIP  && (_rPChannelCompInfo.u4_Cap & TV_DEC_CAP_US_WSS) )  ) 
        {
    ...
    case TV_DEC_SET_TYPE_VGA_TIMING_OPTION:
        ...
        MTVDECEX_VGA_SetAmbiguousIndex(pt_tv_dec_vga_timing_option->ui2_opt_index, pt_tv_dec_vga_timing_option->b_is_default);
...
#ifdef SUPPORT_VGA_UI_AMBIGUOUS_TIMING_SELECT         
        MTVDECEX_VGA_SetAmbiguousUserSel((BOOL)(UINT32)pv_set_info);
#endif        
        i4RetCode = RMR_OK;
        break;
   ...
   #ifdef SUPPORT_VGA_UI_AMBIGUOUS_TIMING_SELECT 
        if ( MTVDECEX_VGA_MWIFResetAmbiguousTimingEEPROM() )
        	i4RetCode = RMR_OK;
        else
        	i4RetCode = RMR_DRV_SET_FAILED;
    #else
    ...
    case TV_DEC_SET_TYPE_VGA_DETECT:
        {
            ...
            {
                u1Mw_SetVideoSrc(SV_VP_PIP, SV_VS_VGA1);
            }
            else
            {
                u1Mw_SetVideoSrc(SV_VP_PIP, SV_VS_MAX);
            }
       	    i4RetCode = RMR_OK;
        } 
        break;
#ifdef CC_SUPPORT_ATV_DESCRAMBLE
	case TV_DEC_SET_TYPE_SCANADJ_INFO:
		{
			if (MTVDECEX_TVD_SetScanadjInfo((MTVDECEX_SCANADJ_INFO_T *)pv_set_info) == MTR_OK)
			{
				i4RetCode = RMR_OK;
			}
			else
		...
		break;
#endif
	case VID_HDMI_SET_VDO_TXTURE_MODE:
		u1Mw_HdmiSetVdoTxtureMode((BOOL)(UINT32)pv_set_info);
		i4RetCode = RMR_OK;
		break;
...
void _TvdNotifyAutoDone(UINT16 ui2_id, BOOL fgSuccess)
{
	...
	TVD_DBG_MSG( " _TvdNotifyAutoDone (ui2_id=%u  fgSuccess=%u)\n", ui2_id,fgSuccess);

  if(ui2_id > 1) ui2_id = 1;
  if((UINT8)fgSuccess>1)//MTAL use vApiNotifyVGAAutoProgress instead of vApiNotifyAutoDone,when get 100 mean auto done!
  {
	if((UINT8)fgSuccess!=100)return;
  }
  if(fgSuccess)
  {
...
void _TvdNotifyAutoColorDone(UINT16 ui2_id, BOOL fgSuccess)
{
  ...
  TVD_DBG_MSG(" _TvdNotifyAutoColorDone(ui2_id=%u  fgSuccess=%u)\n", ui2_id,fgSuccess);
...
void _TvdNotifyMVChg(UINT8 u1Path, BOOL fgStatus)
{
  ...
  TVD_DBG_MSG(" _TvdNotifyMVChg(path=%u  status=%u)\n", u1Path, fgStatus);
...
VOID _TvdNOtifyAutoDetectSource(UINT8 u1SrcIndex, UINT8 u1SrcStatus)
{
	...
	TVD_DBG_MSG( " _TvdNOtifyAutoDetectSource (u1SrcIndex=%u  u1SrcStatus=%u)\n", u1SrcIndex,u1SrcStatus);
	#if 1 //#ifndef LINUX_TURNKEY_SOLUTION
...
VOID _TvdNotifyResolutionChg(UINT8 u1Path, UINT8 u1SigState)
{
    ...
    UINT8 i;
    #if (defined CC_MT5360) || (defined CC_MT5363) ||(defined CC_MT5365) || (defined CC_MT5395) ||(defined CC_MT5399) || (defined CC_MT5880) || defined(CC_MT5881) || defined(CC_MT5882)
    UINT8 bHDCPStatus = 0;
    #endif
    #ifdef CC_AUD_HDMI_SPDIF_CFG
    UINT8 bAud_SPDIF_Status;
    #endif
    UINT8 bVideoIsSrcInterlace = SV_FALSE;
#ifdef CC_3D_SUPPORT    
    MTTDTV_TAG3D_TYPE_T eTag3d = MTVDO_TDTV_TAG3D_NOT_SUPPORT;
#endif    
    UINT8 bInputPolarity=0;
    UINT8 bInputTimingID=0;
    UINT8 u1HdmiScanInfo=0, u1HdmiAFD=0, u1HdmiInputType=0;
    MTVDECEX_HDMI_ARATIO_T eHdmiAspectRatio = MTVDECEX_HDMI_ARATIO_MAX;
    MTVDECEX_HDMI_INFOFRAME rHdmiInfoFrame = {0};    
    MTVDECEX_HDMI_MODE_T eHDMImode = MTVDECEX_HDMI_ARATIO_MAX;
	#if defined(CC_MT5399) || defined(CC_MT5890) || defined(CC_MT5882)
	MTVDECEX_HDMI_INFOFRAME rHdmiSpdInfoFrame = {0};
	UINT8 rHdmiSpdInfoUpSta = 0;
	UINT8 rHdmiSpdInfoUpstaChg = 0;
	UINT8 rHdmiSpdInfoSvSta = 0;
	UINT8 rHdmiSpdInfoSvStaChg = 0;
	#endif 
	#if defined(CC_MT5890)
	UINT8 u1HdmiSpd4KpsSta = 0;
	#endif
    TVD_DBG_MSG(" _TvdNotifyResolutionChg(path=%u  status=%u)\n", u1Path, u1SigState);
    ...
    MTVDECEX_GetVideoRefreshRate((UINT8)u1Path, &(t_resolution.ui2_frame_rate));
    MTVDECEX_GetVideoInputWidth((UINT8)u1Path, &(t_resolution.ui2_width));
    MTVDECEX_GetVideoInputHeight((UINT8)u1Path, &(t_resolution.ui2_height));
    MTVDECEX_IsVideoSrcInterlace((UINT8)u1Path, &bVideoIsSrcInterlace);
    t_resolution.b_is_progressive = (SV_FALSE == bVideoIsSrcInterlace);
    t_resolution.e_src_asp_ratio = TV_DEC_SRC_ASPECT_RATIO_UNKNOWN;
	MTVDECEX_IsMhlSignal(&t_resolution.b_is_mhl_signal);
#ifdef CC_3D_SUPPORT
    MTVDO_TDTV_GetTag3D((UINT8)u1Path, &eTag3d);
    switch(eTag3d)
    {
    ...
    {
        UINT8 bIsVgaTiming = SV_FALSE;
        MTVDECEX_IsPCTiming(u1Path, &bIsVgaTiming);
        t_resolution.e_timing_type = ( bIsVgaTiming?
                                       TV_DEC_TIMING_GRAPHIC:
                                       TV_DEC_TIMING_VIDEO );
    }
    MTVDECEX_GetVideoInputPolarity(u1Path, &bInputPolarity);
    MTVDECEX_GetVideoInputTimingID(u1Path, &bInputTimingID);
    t_resolution.ui4_res_idx = ( (((UINT32)bInputPolarity)<<24) | bInputTimingID );
    ...
    if ( e_curr_src_type == DRVT_AVC_HDMI )
    {
    	                ...
    	                MTVDECEX_HDMI_GetHDCPStatus(&bHDCPStatus);
    #if 1 //#ifndef LINUX_TURNKEY_SOLUTION
    ...
    {    	                
...
#endif
    #endif
        MTVDECEX_HDMI_GetAspectRatio(&eHdmiAspectRatio);
        MTVDECEX_HDMI_GetScanInfo(&u1HdmiScanInfo);
        MTVDECEX_HDMI_GetAFD(&u1HdmiAFD);
        MTVDECEX_HDMI_GetInputType(&u1HdmiInputType);
        TV_DEC_RES_EXT_SET_HDMI_SCAN_INFO( t_resolution.ui4_ext, u1HdmiScanInfo);
        TV_DEC_RES_EXT_SET_HDMI_PIC_ASP_RATIO( t_resolution.ui4_ext,  (UINT8)eHdmiAspectRatio);
        TV_DEC_RES_EXT_SET_HDMI_AFAR( t_resolution.ui4_ext, u1HdmiAFD);
        

        //Get HDMI AVI infoframe
        MTVDECEX_HDMI_GetMode(&eHDMImode);
        MTVDECEX_HDMI_GetInfoFrame(&rHdmiInfoFrame,MTHDMI_InfoFrame_AVI);
        	//Get HDMI SPD Infoframe
#if defined(CC_MT5399) || defined(CC_MT5890) || defined(CC_MT5882)
		MTVDECEX_HDMI_GetSPDUpSta(&rHdmiSpdInfoUpSta);
		MTVDECEX_HDMI_GetSPDUpStaChg(&rHdmiSpdInfoUpstaChg);
		MTVDECEX_HDMI_GetSPDSvSta(&rHdmiSpdInfoSvSta);
		MTVDECEX_HDMI_GetSPDSvStaChg(&rHdmiSpdInfoSvStaChg);
        MTVDECEX_HDMI_GetInfoFrame(&rHdmiSpdInfoFrame,MTHDMI_InfoFrame_SPD); 
#endif
#if defined(CC_MT5890)
		MTVDECEX_HDMI_GetSPD4KpsSta(&u1HdmiSpd4KpsSta);
#endif
		if (((rHdmiInfoFrame.HDMI_FrameData[0] & 0xff) != 0x82) ||(eHDMImode != MTVDECEX_HDMI_HDMI))
		{
			t_resolution.t_hdmi_infoframe.b_valid=0;
			for(i=0;i<TVD_HDMI_INFO_FRAME_LEN;i++)
			{
		...
		else
		{
			...
			t_resolution.t_hdmi_infoframe.aui1_info_frame_data[0]=rHdmiInfoFrame.HDMI_FrameData[1] & 0xff;
			t_resolution.t_hdmi_infoframe.aui1_info_frame_data[1]=(rHdmiInfoFrame.HDMI_FrameData[1] >> 8) & 0xff;
			t_resolution.t_hdmi_infoframe.aui1_info_frame_data[2]=(rHdmiInfoFrame.HDMI_FrameData[1] >> 16) & 0xff;
			t_resolution.t_hdmi_infoframe.aui1_info_frame_data[3]=(rHdmiInfoFrame.HDMI_FrameData[1] >> 24) & 0xff;
			t_resolution.t_hdmi_infoframe.aui1_info_frame_data[4]=rHdmiInfoFrame.HDMI_FrameData[2] & 0xff;
			t_resolution.t_hdmi_infoframe.aui1_info_frame_data[5]=(rHdmiInfoFrame.HDMI_FrameData[2] >> 8) & 0xff;
				
			t_AVI_infoframe.ui1_byte_1= rHdmiInfoFrame.HDMI_FrameData[1] & 0xff;
			t_AVI_infoframe.ui1_byte_2= (rHdmiInfoFrame.HDMI_FrameData[1] >> 8) & 0xff;
			t_AVI_infoframe.ui1_byte_3= (rHdmiInfoFrame.HDMI_FrameData[1] >> 16) & 0xff;
		}
	
	_MW_VDP_HDMI_AVI_INFOFRAME_CB(u1Path,t_AVI_infoframe);
 #if defined(CC_MT5399) || defined(CC_MT5890) || defined(CC_MT5882)
	   if(((rHdmiInfoFrame.HDMI_FrameData[0] & 0xff) != 0x83) || (eHDMImode != MTVDECEX_HDMI_HDMI))
	   {
		   t_resolution.t_hdmi_spd_info.SpdInfoData.VdrNam[0] = 0;
		   t_resolution.t_hdmi_spd_info.SpdInfoData.VdrNam[1] = 0;
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[0] = 0;
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[1] = 0;
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[2] = 0;
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[3] = 0;
		   t_resolution.t_hdmi_spd_info.SpdInfoData.SrcInfo= 0;
	   }
	   else
	   {
		   t_resolution.t_hdmi_spd_info.SpdInfoData.VdrNam[0] = rHdmiSpdInfoFrame.HDMI_FrameData[1];
		   t_resolution.t_hdmi_spd_info.SpdInfoData.VdrNam[1] = rHdmiSpdInfoFrame.HDMI_FrameData[2];
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[0] = rHdmiSpdInfoFrame.HDMI_FrameData[3];
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[1] = rHdmiSpdInfoFrame.HDMI_FrameData[4];
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[2] = rHdmiSpdInfoFrame.HDMI_FrameData[5];
		   t_resolution.t_hdmi_spd_info.SpdInfoData.PrdDspt[3] = rHdmiSpdInfoFrame.HDMI_FrameData[6];
		   t_resolution.t_hdmi_spd_info.SpdInfoData.SrcInfo= rHdmiSpdInfoFrame.HDMI_FrameData[7] && 0xff;
	   }
	   t_resolution.t_hdmi_spd_info.UPStatus = rHdmiSpdInfoUpSta;
	   t_resolution.t_hdmi_spd_info.UPStaChg = rHdmiSpdInfoUpstaChg;
	   t_resolution.t_hdmi_spd_info.SVStatus = rHdmiSpdInfoSvSta;
	   t_resolution.t_hdmi_spd_info.SVStaChg = rHdmiSpdInfoSvStaChg;
#if defined(CC_MT5890)  
	   t_resolution.t_hdmi_spd_info.u1Spd4KPsStatus = u1HdmiSpd4KpsSta;
#endif
	   
#endif		
        t_resolution.e_color_fmt = ((u1HdmiInputType==0)? 
...
#ifdef SYS_DRV_VALIDATION_DISABLE
    ...
    UINT8                   d2)
{
    ...
    MTVDECEX_GetVideoInternalDec(u1Path, &eDecType);
    if (eDecType==MTVDO_DEC_TVD3D)
    {
        MTVDECEX_TVD_GetNRLevel(&u2NRLCurr);
...
void TvdNotifyCCData(UINT8 u1Path, UINT8 d1, UINT8 d2, UINT8 field)
{
    ...
    if(_arCCNfyInfo[u1Path].pf_cc_nfy)
    {
        ...
        if (eDecType==MTVDO_DEC_TVD3D)
        {
            MTVDECEX_VBI_SetCCECCOnOff (_arCCStatus[u1Path].fgECCOnOff);
        }
#endif 
...
static void _TvdProcessTtxData(UINT8 u1Path, UINT16 u2NumPktAvail, UINT32 _dVbiReadPtr)
{
    ...
    if ( i4Ret == TTX_BUFR_OK )
    {
        ...
        #else
            rTtxData.t_data.ui2_num_pkt = u2NumPktAvail;
            rTtxData.t_data.at_pkts= ttx_addr_kernel_to_user(u1Path, (UINT32)parPkts);
...
void vSetCapibility(UINT8 u1Path)
{
  ...
  if(u1Path == SV_VP_MAIN)
  {
    ...
    _rMChannelCompInfo.u4_Cap = u4Cap;
  }
  else if(u1Path == SV_VP_PIP)
  {
    if(u1IsPipTvd)
    ...
    _rPChannelCompInfo.u4_Cap = u4Cap;
  }
...
VOID _DrvNotifySigChg(UINT8 u1Path, UINT8 u1SigState)
{
    ...
    UINT32 u4ScanMode = 0;
	TVD_DBG_MSG(" _DrvNotifySigChg(path=%u  status=%u)\n", u1Path, u1SigState);
	
    MTVDECEX_GetVideoSrcType(u1Path, &u1SrcType);
    if (VSS_MAJOR(u1SrcType)!=VSS_DTV)
    {   
        // callback for MW_if
        ...
        if(MTVDO_GetScanMode(&u4ScanMode)==MTR_OK && u4ScanMode!=SV_SCAN_MODE)
        {
...
void _TvdNotifyColorSystem(UINT8 u1Path, UINT8 u1SigState)
{
    ...
    TVD_DBG_MSG(" _TvdNotifyColorSystem(path=%u  status=%u)\n", u1Path, u1SigState);
    ...
    MTVDECEX_GetVideoDefStatus(MTVDO_IS_MAIN_TVD, &u1IsMainTvd);
    MTVDECEX_GetVideoDefStatus(MTVDO_IS_PIP_TVD, &u1IsPipTvd);
    if( ((u1Path == SV_VP_MAIN)&&(u1IsMainTvd)) ||
        ((u1Path == SV_VP_PIP)&&(u1IsPipTvd)) )
    {
...
{
	TVD_DBG_MSG(" _Tvd_NotifyTVSig(path=%u  status=%u)\n", u1Path, u1SigState);
         ...
         (pt_nfy_sts->u1NeedNotify == TRUE)) )
    {
            ...
            (pt_rv_channel->t_Tvd_Ctrl == TV_DEC_CTRL_PLAY) )
        {
            ...
            if (((u1SrcType==SV_VS_ATV1 || u1SrcType==SV_VS_ATD1) || t_scart_info.b_canal_plus_support != FALSE) &&
                u1SrcType == _TveGetScartOutSource(0) &&
            ...
            if (u1Path == SV_VP_MAIN)
            {
                if(u1SrcType == _TveGetScartOutSource(1) &&
                   u1SigState == SV_VDO_STABLE && /* ignore no signal notify to mw */
                   _arTVNfyInfo[2].pf_tv_dec_nfy != NULL)
                {
                #if 1 //#ifndef LINUX_TURNKEY_SOLUTION
                ...
                if(u1SrcType == _TveGetScartOutSource(0) &&
                   u1SigState == SV_VDO_STABLE && /* ignore no signal notify to mw */
                   _arTVNfyInfo[3].pf_tv_dec_nfy != NULL)
                {
                #if 1 //#ifndef LINUX_TURNKEY_SOLUTION
