...
{0x2D4,0x04},//PRA target H
{0x2D5,0x04},//PRA target L
{0x2D6,0x04},//DAGC setting
{0x2D7,0xEC},//AGC polarity inverse.RF_AGC  disable.IF_AGC polarity inverse
{0x2D8,0xF4},//DAGC Target level H
{0x2D9,0xF4},//DAGC Target level L
{0x2DA,0x7F},//PRA speed
{0x2DB,0x7F},//AGC polarity inverse.RF_AGC  disable.IF_AGC polarity inverse
{0x2DC,0x80},//DAGC Target level L
{0x2DD,0x80},//PRA speed
{0x2DE,0x80},//PRA speed
{0x2DF,0x51},//PRA PORI
{0x2B7,0x10},
{0x2B9,0x10},
{0x00,0x00}//end flag
    ...
    )
{
    ...
    if(Freq<470000)
    {   
        rb211_result=rb211_tuner_GPOCtrl(pTuner, 0, 0); // Disable FM trap
        if(rb211_result != RB211_RESULT_OK){
            return -1;
        }
    }
    ...
    {
        rb211_result=rb211_tuner_GPOCtrl(pTuner, 0, 1); // Enable FM trap
        if(rb211_result != RB211_RESULT_OK){
            return -1;
        }
    }
