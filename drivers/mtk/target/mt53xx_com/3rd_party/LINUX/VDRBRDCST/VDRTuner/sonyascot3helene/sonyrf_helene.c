...
------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------
 Based on HELENE application note 1.5.2
------------------------------------------------------------------------------*/
...
*/
static const sonyrf_helene_terr_adjust_param_t g_terr_param_table[SONYRF_HELENE_TERR_TV_SYSTEM_NUM] = {
    /*
    ...
    {AUTO, 0x05, 0x03, 0x06, 0x03, 0x01, 0x01, 0x01, 0x00, BW_8,  OFFSET(2),  OFFSET(-3), 0x00}, /**< SONYRF_HELENE_ATV_I         (System-I) */
    {AUTO, 0x05, 0x03, 0x06, 0x03, 0x01, 0x01, 0x01, 0x00, BW_8,  OFFSET(2),  OFFSET(-3), 0x00}, /**< SONYRF_HELENE_ATV_DK        (System-D/K) */
    {AUTO, 0x03, 0x04, 0x0A, 0x04, 0x04, 0x04, 0x04, 0x00, BW_8,  OFFSET(2),  OFFSET(-3), 0x00}, /**< SONYRF_HELENE_ATV_L         (System-L) */
    {AUTO, 0x03, 0x04, 0x0A, 0x04, 0x04, 0x04, 0x04, 0x00, BW_8,  OFFSET(-1), OFFSET(4),  0x00}, /**< SONYRF_HELENE_ATV_L_DASH    (System-L DASH) */
    /* Digital */
    {AUTO, 0x09, 0x0B, 0x0B, 0x0B, 0x03, 0x03, 0x03, 0x00, BW_6,  OFFSET(-6), OFFSET(-3), 0x00}, /**< SONYRF_HELENE_DTV_8VSB      (ATSC 8VSB) */
    {AUTO, 0x09, 0x0B, 0x0B, 0x0B, 0x02, 0x02, 0x02, 0x00, BW_6,  OFFSET(-6), OFFSET(-3), 0x00}, /**< SONYRF_HELENE_DTV_QAM       (US QAM) */
...
*/
static sonyrf_result_t X_pon(sonyrf_helene_t *pTuner);
/**
 @brief Configure the HELENE tuner for specified terrestrial broadcasting system.
*/
static sonyrf_result_t TER_tune(sonyrf_helene_t *pTuner, uint32_t frequencykHz,
    sonyrf_helene_tv_system_t tvSystem, uint8_t vcoCal);
/**
 @brief Configure the HELENE tuner for specified satellite broadcasting system.
*/
static sonyrf_result_t SAT_tune(sonyrf_helene_t *pTuner, uint32_t frequencykHz,
    sonyrf_helene_tv_system_t tvSystem,  uint32_t symbolRateksps, uint8_t vcoCal);
/**
 @brief The last part of terrestrial tuning sequence.
*/
static sonyrf_result_t TER_tune_end(sonyrf_helene_t *pTuner, sonyrf_helene_tv_system_t tvSystem);
/**
 @brief The last part of satellite tuning sequence.
*/
static sonyrf_result_t SAT_tune_end(sonyrf_helene_t *pTuner, sonyrf_helene_tv_system_t tvSystem);
/**
 @brief Configure the HELENE tuner from terrestrial to Power Save state.
*/
static sonyrf_result_t TER_fin(sonyrf_helene_t *pTuner);
/**
 @brief Configure the HELENE tuner from satellite to Power Save state.
*/
static sonyrf_result_t SAT_fin(sonyrf_helene_t *pTuner);
/**
 @brief Configure the HELENE tuner to Oscillation Stop state.
*/
static sonyrf_result_t X_oscdis(sonyrf_helene_t *pTuner);
/**
 @brief Configure the HELENE tuner back from Oscillation Stop state.
*/
static sonyrf_result_t X_oscen(sonyrf_helene_t *pTuner);
/**
 @brief Reading gain information to calculate IF and RF gain levels.
*/
static sonyrf_result_t X_read_agc(sonyrf_helene_t *pTuner, uint8_t *pIFAGCReg, uint8_t *pRFAGCReg);
...
*/
#define TERR_INTERNAL_LOOPFILTER_AVAILABLE(tvSystem) (SONYRF_HELENE_IS_DTV(tvSystem)\
    && ((tvSystem) != SONYRF_HELENE_DTV_DVBC_6) && ((tvSystem) != SONYRF_HELENE_DTV_DVBC_8)\
    && ((tvSystem) != SONYRF_HELENE_DTV_DVBC2_6) && ((tvSystem) != SONYRF_HELENE_DTV_DVBC2_8))

/*------------------------------------------------------------------------------
 Implementation of public functions.
------------------------------------------------------------------------------*/

sonyrf_result_t sonyrf_helene_Create(sonyrf_helene_t *pTuner, sonyrf_helene_xtal_t xtalFreq,
    uint8_t i2cAddress, sonyrf_i2c_t *pI2c, uint32_t flags)
{
    SONYRF_TRACE_ENTER("sonyrf_helene_Create");
    ...
    if((xtalFreq == SONYRF_HELENE_XTAL_41000KHz) && !(flags & SONYRF_HELENE_CONFIG_EXT_REF)){
        SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_NOSUPPORT);
    }

    /* SONYRF_HELENE_CONFIG_EXT_REF and SONYRF_HELENE_CONFIG_SLEEP_DISABLEXTAL cannot be used at the same time. */
    if((flags & SONYRF_HELENE_CONFIG_EXT_REF) && (flags & SONYRF_HELENE_CONFIG_SLEEP_DISABLEXTAL)){
        SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_NOSUPPORT);
    }

    pTuner->state = SONYRF_HELENE_STATE_UNKNOWN; /* Chip is not accessed for now. */
    ...
    pTuner->tvSystem = SONYRF_HELENE_TV_SYSTEM_UNKNOWN;
    pTuner->isFreesatMode = 0;
    pTuner->pTerrParamTable = g_terr_param_table;

    /* Xtal OSC reference value */
    if(flags & SONYRF_HELENE_CONFIG_EXT_REF){
        pTuner->xosc_sel = 0x00;
        pTuner->xosc_cap_set = 0x00;
    }else{
        switch(xtalFreq){
        case SONYRF_HELENE_XTAL_16000KHz:
            pTuner->xosc_sel = 0x04;     /* 4 x 25 = 100uA */
            pTuner->xosc_cap_set = 0x33; /* 51 x 0.25 = 12.75pF */
            break;
        case SONYRF_HELENE_XTAL_20500KHz:
        case SONYRF_HELENE_XTAL_24000KHz:
            pTuner->xosc_sel = 0x04;     /* 4 x 25 = 100uA */
            pTuner->xosc_cap_set = 0x26; /* 38 x 0.25 = 9.5pF */
            break;
        case SONYRF_HELENE_XTAL_41000KHz:  /* 41MHz is for external reference input only. */
        default:
            SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_NOSUPPORT);
        }
    }
...
sonyrf_result_t sonyrf_helene_Initialize(sonyrf_helene_t *pTuner)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_Initialize");
    ...
    if((pTuner->flags & SONYRF_HELENE_CONFIG_POWERSAVE_TERR_MASK) == SONYRF_HELENE_CONFIG_POWERSAVE_TERR_RF_ACTIVE){
        result = TER_tune(pTuner, 666000, SONYRF_HELENE_DTV_DVBT_8, 1);
        if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(result); }

        SONYRF_SLEEP(50);

        result = TER_tune_end(pTuner, SONYRF_HELENE_DTV_DVBT_8);
        if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(result); }
    ...
    if((pTuner->flags & SONYRF_HELENE_CONFIG_SLEEP_DISABLEXTAL) && !(pTuner->flags & SONYRF_HELENE_CONFIG_EXT_REF)){
        /* Disable Xtal */
        result = X_oscdis(pTuner);
        if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(result); }
    }

    pTuner->state = SONYRF_HELENE_STATE_SLEEP;
    pTuner->frequencykHz = 0;
    pTuner->tvSystem = SONYRF_HELENE_TV_SYSTEM_UNKNOWN;
    pTuner->symbolRateksps = 0;
...
sonyrf_result_t sonyrf_helene_terr_Tune(sonyrf_helene_t *pTuner, uint32_t frequencykHz,
    sonyrf_helene_tv_system_t tvSystem)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_terr_Tune");
    ...
    if(!SONYRF_HELENE_IS_ATV(tvSystem) && !SONYRF_HELENE_IS_DTV(tvSystem)){
        SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_ARG);
    }
    ...
    /* State check */
    switch(pTuner->state){
    case SONYRF_HELENE_STATE_SLEEP:
    ...
    case SONYRF_HELENE_STATE_ACTIVE_S:
        /* Set system to "Unknown". (for safe) */
        pTuner->tvSystem = SONYRF_HELENE_TV_SYSTEM_UNKNOWN;
    ...
    case SONYRF_HELENE_STATE_ACTIVE_T:
        break;
...
sonyrf_result_t sonyrf_helene_terr_TuneEnd(sonyrf_helene_t *pTuner)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_terr_TuneEnd");
...
sonyrf_result_t sonyrf_helene_sat_Tune(sonyrf_helene_t *pTuner, uint32_t frequencykHz,
    sonyrf_helene_tv_system_t tvSystem, uint32_t symbolRateksps)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_sat_Tune");
    ...
    if(!SONYRF_HELENE_IS_STV(tvSystem)){
        SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_ARG);
    }
    ...
    if(tvSystem == SONYRF_HELENE_STV_ISDBS){
...
sonyrf_result_t sonyrf_helene_ShiftFRF(sonyrf_helene_t *pTuner, uint32_t frequencykHz)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_ShiftFRF");
...
sonyrf_result_t sonyrf_helene_Sleep(sonyrf_helene_t *pTuner)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_Sleep");
...
sonyrf_result_t sonyrf_helene_SetGPO(sonyrf_helene_t *pTuner, uint8_t id, uint8_t val)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_SetGPO");
...
sonyrf_result_t sonyrf_helene_GetGPI1(sonyrf_helene_t *pTuner, uint8_t *pVal)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;
    uint8_t data = 0;

    SONYRF_TRACE_ENTER("sonyrf_helene_GetGPI1");
    ...
    /* GPIO1_IN_SEL = 1 (0x86) */
    result = sonyrf_i2c_SetRegisterBits(pTuner->pI2c, pTuner->i2cAddress, 0x86, 0x10, 0x10);
    if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_I2C); }
...
sonyrf_result_t sonyrf_helene_SetRfExtCtrl(sonyrf_helene_t *pTuner, uint8_t enable)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_SetRfExtCtrl");
...
sonyrf_result_t sonyrf_helene_SetFreesatMode(sonyrf_helene_t *pTuner, uint8_t enable)
{
    SONYRF_TRACE_ENTER("sonyrf_helene_SetFreesatMode");
...
sonyrf_result_t sonyrf_helene_ReadRssi(sonyrf_helene_t *pTuner, int32_t *pRssi)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;
    uint8_t ifagcreg = 0;
    uint8_t rfagcreg = 0;
    ...
    SONYRF_TRACE_ENTER("sonyrf_helene_ReadRssi");
...
sonyrf_result_t sonyrf_helene_RFFilterConfig(sonyrf_helene_t *pTuner, uint8_t coeff, uint8_t offset)
{
    sonyrf_result_t result = SONYRF_RESULT_OK;

    SONYRF_TRACE_ENTER("sonyrf_helene_RFFilterConfig");

    if((!pTuner) || (!pTuner->pI2c)){
        SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_ARG);
    }

    if((pTuner->state != SONYRF_HELENE_STATE_SLEEP) && (pTuner->state != SONYRF_HELENE_STATE_ACTIVE_T)
        && (pTuner->state != SONYRF_HELENE_STATE_ACTIVE_S)){
        SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_SW_STATE);
    }

    if(pTuner->state == SONYRF_HELENE_STATE_SLEEP){
        if((pTuner->flags & SONYRF_HELENE_CONFIG_SLEEP_DISABLEXTAL) && !(pTuner->flags & SONYRF_HELENE_CONFIG_EXT_REF)){
            /* Enable Xtal */
            result = X_oscen(pTuner);
            if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(result); }
...
static sonyrf_result_t X_pon(sonyrf_helene_t *pTuner)
{
    ...
    /* Power save setting for analog block (0x0C) */
    {
        ...
        switch(pTuner->flags & SONYRF_HELENE_CONFIG_POWERSAVE_TERR_MASK){
        case SONYRF_HELENE_CONFIG_POWERSAVE_TERR_NORMAL:
            dataT[0] = 0x15;
            dataT[1] = 0x00;
            dataT[2] = 0x00;
            dataT[3] = 0x00;
            break;
        case SONYRF_HELENE_CONFIG_POWERSAVE_TERR_RFIN_MATCHING:
            dataT[0] = 0x15;
            dataT[1] = 0x00;
            dataT[2] = 0x00;
            dataT[3] = 0x02;
            break;
        case SONYRF_HELENE_CONFIG_POWERSAVE_TERR_RF_ACTIVE:
            dataT[0] = 0x06;
            dataT[1] = 0x00;
            dataT[2] = 0x02;
            dataT[3] = 0x00;
        ...
        {
            uint8_t lnaOff = (pTuner->flags & SONYRF_HELENE_CONFIG_SAT_LNA_OFF) ? 1 : 0;
            uint8_t terrRfActive = ((pTuner->flags & SONYRF_HELENE_CONFIG_POWERSAVE_TERR_MASK)
                == SONYRF_HELENE_CONFIG_POWERSAVE_TERR_RF_ACTIVE) ? 1 : 0;

            switch(pTuner->flags & SONYRF_HELENE_CONFIG_POWERSAVE_SAT_MASK){
            case SONYRF_HELENE_CONFIG_POWERSAVE_SAT_NORMAL:
                dataS[0] = terrRfActive ? 0x04 : 0x14;
                dataS[1] = lnaOff       ? 0x05 : 0x07;
                break;
            case SONYRF_HELENE_CONFIG_POWERSAVE_SAT_RFIN_MATCHING:
                dataS[0] = terrRfActive ? 0x04 : 0x14;
                dataS[1] = lnaOff       ? 0x04 : 0x06;
                break;
            case SONYRF_HELENE_CONFIG_POWERSAVE_SAT_RF_ACTIVE:
                if(lnaOff){
    ...
    /* 0x81 - 0x94 */
    {
        ...
        case SONYRF_HELENE_XTAL_24000KHz:
            data[0] = 0x18;
            break;
        case SONYRF_HELENE_XTAL_41000KHz:
            data[0] = 0x69;
            break;
        default:
            SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_NOSUPPORT);
        ...
        if(pTuner->flags & SONYRF_HELENE_CONFIG_EXT_REF){
            /* XOSC_APC_EN = 0, XOSC_SEL= 0uA */
            data[1] = 0x00;
            /* XOSC_CALC_EN = 0, XOSC_CAP_SET = 0pF */
            data[2] = 0x00;
        }else{
            /* XOSC_APC_EN = 1, XOSC_SEL = xosc_sel (sonyrf_helene_t member) */
            data[1] = (uint8_t)(0x80 | (pTuner->xosc_sel & 0x1F));
            /* XOSC_CALC_EN = 1, XOSC_CAP_SET = xosc_cap_set (sonyrf_helene_t member) */
            data[2] = (uint8_t)(0x80 | (pTuner->xosc_cap_set & 0x7F));
        }

        /* Setting for REFOUT signal output (0x84) */
        switch(pTuner->flags & SONYRF_HELENE_CONFIG_REFOUT_MASK){
        case 0:
            data[3] = 0x00; /* REFOUT_EN = 0, REFOUT_CNT = 0 */
            break;
        case SONYRF_HELENE_CONFIG_REFOUT_500mVpp:
            data[3] = 0x80; /* REFOUT_EN = 1, REFOUT_CNT = 0 */
            break;
        case SONYRF_HELENE_CONFIG_REFOUT_400mVpp:
            data[3] = 0x81; /* REFOUT_EN = 1, REFOUT_CNT = 1 */
            break;
        case SONYRF_HELENE_CONFIG_REFOUT_600mVpp:
            data[3] = 0x82; /* REFOUT_EN = 1, REFOUT_CNT = 2 */
            break;
        case SONYRF_HELENE_CONFIG_REFOUT_800mVpp:
        ...
        /* GPIO setting should be done by sonyrf_helene_SetGPO/sonyrf_helene_GetGPI1 after initialization */
        data[4] = 0x00;
        data[5] = 0x00;
        ...
        if(pTuner->flags & SONYRF_HELENE_CONFIG_OVERLOAD_EXTENDED_TC){
            data[9] = 0x00;
            data[10] = 0x45;
            data[11] = 0x56;
        }else{
...
#ifndef SONYRF_HELENE_IGNORE_NVM_ERROR /* For no NVM tuner evaluation */
        if((rdata[1] & 0x3F) != 0x00){
            SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_HW_STATE); /* CPU_ERR[5:0] != 0x00 (NVM Error) */
        }
#endif /* SONYRF_HELENE_IGNORE_NVM_ERROR */
    }
    ...
    sonyrf_helene_tv_system_t tvSystem, uint8_t vcoCal)
{
    ...
    /* 0x85, 0x86 */
    /* GPIO0, GPIO1 is changed by sonyrf_helene_SetGPO/sonyrf_helene_GetGPI1 */
    ...
    /* 0x91, 0x92 */
    {
        ...
        if((tvSystem == SONYRF_HELENE_DTV_DVBC_6) || (tvSystem == SONYRF_HELENE_DTV_DVBC_8)){
            data[0] = 0x16;
            data[1] = 0x26;
        }else{
            data[0] = 0x10;
    ...
    /* 0x9C, 0x9D */
    {
        ...
        if((pTuner->flags & SONYRF_HELENE_CONFIG_LOOPFILTER_INTERNAL) && TERR_INTERNAL_LOOPFILTER_AVAILABLE(tvSystem)){
            data[0] = 0x90;
        }else{
            data[0] = 0x00;
        }
    ...
    /* 0x5E - 0x66 */
    {
        ...
        if(SONYRF_HELENE_IS_ATV(tvSystem)){
        ...
        }else if((tvSystem == SONYRF_HELENE_DTV_DVBC_6) || (tvSystem == SONYRF_HELENE_DTV_DVBC_8)){
            /* DVB-C (Fref = 1MHz) */
            switch(pTuner->xtalFreq){
            case SONYRF_HELENE_XTAL_16000KHz:
                data[4] = 0x10;
        ...
        if((pTuner->flags & SONYRF_HELENE_CONFIG_LOOPFILTER_INTERNAL) && TERR_INTERNAL_LOOPFILTER_AVAILABLE(tvSystem)){
            if(pTuner->xtalFreq == SONYRF_HELENE_XTAL_20500KHz){
                data[5] = 0x2C;
            }else{
                data[5] = 0x38;
            }
            ...
            data[7] = 0x02;
            data[8] = 0x24;
        }else if(SONYRF_HELENE_IS_ATV(tvSystem)){
            if(pTuner->xtalFreq == SONYRF_HELENE_XTAL_20500KHz){
    ...
    /* 0x68 - 0x78 */
    {
        ...
        if(SONYRF_HELENE_IS_ATV(tvSystem)){
            data[0] = (uint8_t)((pTuner->flags & SONYRF_HELENE_CONFIG_OUTLMT_ATV_1_2Vpp) ? 0x01 : 0x00);
        }else if(SONYRF_HELENE_IS_DTV(tvSystem)){
            data[0] = (uint8_t)((pTuner->flags & SONYRF_HELENE_CONFIG_OUTLMT_DTV_1_2Vpp) ? 0x01 : 0x00);
        }else{
            SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_ARG);
        }
        ...
        if(tvSystem == SONYRF_HELENE_ATV_L_DASH){
        ...
        if(SONYRF_HELENE_IS_ATV(tvSystem) || (tvSystem == SONYRF_HELENE_DTV_DVBC_6) || (tvSystem == SONYRF_HELENE_DTV_DVBC_8)){
            data[13] = 0xD9;
            data[14] = 0x0F;
            data[15] = 0x25;
            data[16] = 0x87;
...
static sonyrf_result_t TER_tune_end(sonyrf_helene_t *pTuner, sonyrf_helene_tv_system_t tvSystem)
{
    ...
    SONYRF_TRACE_ENTER("TER_tune_end");
    ...
    sonyrf_helene_tv_system_t tvSystem, uint32_t symbolRateksps, uint8_t vcoCal)
{
    ...
    result = pTuner->pI2c->WriteOneRegister(pTuner->pI2c, pTuner->i2cAddress, 0x43,
        (uint8_t)((pTuner->flags & SONYRF_HELENE_CONFIG_SAT_LNA_OFF) ? 0x04 : 0x06));
    if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_I2C); }
    ...
    /* 0x04 - 0x15 */
    {
        ...
        if(pTuner->flags & SONYRF_HELENE_CONFIG_LOOPFILTER_INTERNAL){
            data[3] = 0x80;
        }else{
            data[3] = 0x00;
        }
        ...
        if(pTuner->flags & SONYRF_HELENE_CONFIG_SAT_LNA_OFF){
            data[8] = 0x0E; /* Clear SAT_RF_LNA_EN (Bit[0]) */
        }else{
            data[8] = 0x0F;
        }

        if((pTuner->flags & SONYRF_HELENE_CONFIG_POWERSAVE_TERR_MASK)
            == SONYRF_HELENE_CONFIG_POWERSAVE_TERR_RF_ACTIVE){
        ...
        default:
            SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_ARG); /* Invalid system */
        }
        ...
        data[16] = (uint8_t)((pTuner->flags & SONYRF_HELENE_CONFIG_OUTLMT_STV_0_6Vpp) ? 0x01 : 0x00);
...
static sonyrf_result_t SAT_tune_end(sonyrf_helene_t *pTuner, sonyrf_helene_tv_system_t tvSystem)
{
    ...
    SONYRF_TRACE_ENTER("SAT_tune_end");
...
static sonyrf_result_t TER_fin(sonyrf_helene_t *pTuner)
{
    ...
    /* Power save setting for analog block (0x5E, 0x5F, 0x60) */
    {
        ...
        /* Keep RF_EXT bit */
        result = sonyrf_i2c_SetRegisterBits(pTuner->pI2c, pTuner->i2cAddress, 0x67, data[3], 0xFE);
        if(result != SONYRF_RESULT_OK){ SONYRF_TRACE_RETURN(SONYRF_RESULT_ERROR_I2C); }
