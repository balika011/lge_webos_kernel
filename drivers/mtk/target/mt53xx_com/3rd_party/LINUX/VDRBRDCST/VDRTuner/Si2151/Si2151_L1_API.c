...
unsigned char    Si2151_L1_API_Init      (L1_Si2151_Context *api, int add) {
    ...
    Si2151_L0_Init(api->i2c);               /* For Mediatek platform porting */
    Si2151_L0_SetAddress(api->i2c, add, 0); /* For Mediatek platform porting */
    ...
    /* Set the propertyWriteMode to Si2151_DOWNLOAD_ON_CHANGE to only download property settings on change (recommended) */
    /*      if propertyWriteMode is set to Si2151_DOWNLOAD_ALWAYS the properties will be downloaded regardless of change */
...
unsigned char    Si2151_L1_API_Patch     (L1_Si2151_Context *api, int iNbBytes, unsigned char *pucDataBuffer) {
    ...
    mcDBG_MSG_L3(("Si2151 Patch %d bytes\n",iNbBytes));

    res = Si2151_L0_WriteCommandBytes(api->i2c, iNbBytes, pucDataBuffer);
    if (res!=iNbBytes) {
      mcDBG_MSG_ERR(("Si2151_L1_API_Patch error writing bytes: %s\n", Si2151_L1_API_ERROR_TEXT(ERROR_Si2151_LOADING_FIRMWARE)));
      return ERROR_Si2151_LOADING_FIRMWARE;
    }

    res = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (res != NO_Si2151_ERROR) {
      mcDBG_MSG_ERR(("Si2151_L1_API_Patch error 0x%02x polling response: %s\n", res, Si2151_L1_API_ERROR_TEXT(res)));
...
/***********************************************************************************************************************
  ...
  Porting:    May not be required for the final application, can be removed if not used.
 ***********************************************************************************************************************/
char*            Si2151_L1_API_TAG_TEXT(void) { return (char *)"Si2151_ROM60_0Ab18_V0.1";}
