...
#define C_SONY_RE231_STR		"SONY_RE231_Dual 2014-01-15\n v1.0"
...
UINT8 fgTunerNormal_GW_re231 = 1; //wenming 2012-7-4:flag for tuner Normal or Gateway access, 0 for Normal, 1 for Gateway
UINT8 fgTunerI2C_SPI_re231 = 0;

typedef struct _SONY_RE231_MULTI_TUNER_CTX
{
    sonyrf_tuner_ctx g_sonyrf_tuner_ctx;
    BOOL TunerInited;
}SONY_RE231_MULTI_TUNER_CTX;

/* Tuner control data instance. */
static SONY_RE231_MULTI_TUNER_CTX tRe231MultiCtx[RE231_TUNER_CELLS];

static const char *apcRE231_STR[RE231_TUNER_CELLS+1] = {"RE231_CELL1","RE231_CELL2","RE231_INVALID_CELLID"};
#define RE231_CELL_STR(_x_) (_x_ > RE231_TUNER_CELLS)?apcRE231_STR[RE231_TUNER_CELLS]:apcRE231_STR[_x_]

static BOOL RE231_GetTunerCtx(ITUNER_CTX_T *pTCtx, SONY_RE231_MULTI_TUNER_CTX **ppMultiTunerCtx)
{
    UINT8 u1TunerCellID = pTCtx->u1TunerCellID;
    mcDBG_MSG_L4(("%s:u1TunerCellID:%d,u1GwDemodAddr:0x%02x  >>>>>>>\n",__FUNCTION__,u1TunerCellID,pTCtx->u1DemodI2cAddress));
    if(u1TunerCellID >= RE231_TUNER_CELLS)
    {
        mcDBG_MSG_L1(("%s Error:%d exceed max RE231 tuner cells(%d)!!\n",__FUNCTION__,u1TunerCellID,RE231_TUNER_CELLS));
        return FALSE;
    }
    *ppMultiTunerCtx = &tRe231MultiCtx[u1TunerCellID];
    return TRUE;
}
//-----------------------------------------------------------------------------
/** RE231_TunerInit
    ...
    )
{
    ...
    UINT8 u1GwDemodAddr = psTunerCtx->u1DemodI2cAddress;
    SONY_RE231_MULTI_TUNER_CTX *pMultiTunerCtx = NULL;
    sonyrf_tuner_ctx *pg_sonyrf_tuner_ctx = NULL;

    if(!RE231_GetTunerCtx(psTunerCtx,&pMultiTunerCtx))
    {
        return;
    }
    pg_sonyrf_tuner_ctx = &(pMultiTunerCtx->g_sonyrf_tuner_ctx);
    pMultiTunerCtx->TunerInited = FALSE;
    pI2c = &(pg_sonyrf_tuner_ctx->i2c);
   ...
   // pEUCtx->m_aucPara[7] = C_RE231_DEMOD_INPUT_POWER;
    ...
    {
        sonyrf_i2c_MTK_CreateI2cGw_re231(pI2c, u1GwDemodAddr, 0x09); // I2C gateway
    }

    mcDBG_MSG_L1(("[%s] %s \n", RE231_CELL_STR(pTCtx->u1TunerCellID),__FUNCTION__));

    /* Initialize tuner struct for ES2 for safe. */
    sonyrf_ascot3_re231_Create(&(pg_sonyrf_tuner_ctx->tuner.tunerEs2), pI2c);
    pg_sonyrf_tuner_ctx->isEs1 = 0;
}
...
static BOOL RE231_TunerGetStatus(ITUNER_CTX_T *pTCtx)
{
    ...
    UINT8 data = 0;
    SONY_RE231_MULTI_TUNER_CTX *pMultiTunerCtx = NULL;
    if(!RE231_GetTunerCtx(pTCtx,&pMultiTunerCtx))
    {
        return FALSE;
    }
    pI2c = &(pMultiTunerCtx->g_sonyrf_tuner_ctx.i2c);
...
{
    sonyrf_result_t sonyrf_result = SONYRF_RESULT_OK;
    sonyrf_ascot3es1_t *pTunerEs1 = NULL;
    sonyrf_ascot3_t *pTuner = NULL;
    ...
    pg_sonyrf_tuner_ctx = &(pMultiTunerCtx->g_sonyrf_tuner_ctx);
    pTunerEs1 = &(pg_sonyrf_tuner_ctx->tuner.tunerEs1);
    pTuner = &(pg_sonyrf_tuner_ctx->tuner.tunerEs2);

    mcDBG_MSG_L4(("[%s] %s \n", RE231_CELL_STR(pTCtx->u1TunerCellID),__FUNCTION__));
    pTCtx->u4RF_Freq = Freq; // Update current frequency to context

    mcDBG_MSG_L2(("[%s]RE231Tuner: Set Freq %d KHz\n",RE231_CELL_STR(pTCtx->u1TunerCellID),Freq));
    if (Freq < (C_RE231_FREQ_DBOUND_LOWER/1000) ||
        Freq > (C_RE231_FREQ_DBOUND_UPPER/1000))
    {
        mcDBG_MSG_L2(("[%s]RE231Tuner: Out of range for LO!\n",RE231_CELL_STR(pTCtx->u1TunerCellID)));
        //return 1;
    }

    if(!pMultiTunerCtx->TunerInited)
    {
        sonyrf_i2c_t *pI2c = &(pg_sonyrf_tuner_ctx->i2c);
        uint8_t data = 0x00;

        pMultiTunerCtx->TunerInited=TRUE;
        ...
        if((data & 0xF0) == 0xC0){
            // ES1
            mcDBG_MSG_L2(("[%s]RE231Tuner: WARNING!! Previous version of tuner (ES1) is detected!\n",RE231_CELL_STR(pTCtx->u1TunerCellID)));
            sonyrf_ascot3es1_re231_Create(pTunerEs1, pI2c);
            pg_sonyrf_tuner_ctx->isEs1 = 1;
        }else if((data & 0xF0) == 0xD0){
            // ES2
            mcDBG_MSG_L2(("[%s]RE231Tuner: Latest version of tuner (ES2) is detected.\n",RE231_CELL_STR(pTCtx->u1TunerCellID)));
            sonyrf_ascot3_re231_Create(pTuner, pI2c);
            pg_sonyrf_tuner_ctx->isEs1 = 0;
        }else{
            mcDBG_MSG_L1(("[%s]RE231Tuner: ERROR!! Not RE231 module!\n",RE231_CELL_STR(pTCtx->u1TunerCellID)));
        }

        if(pg_sonyrf_tuner_ctx->isEs1){
            sonyrf_result = sonyrf_ascot3es1_re231_Initialize(pTunerEs1);
            if(sonyrf_result != SONYRF_RESULT_OK){
                mcDBG_MSG_L2(("[%s]RE231Tuner: sonyrf_ascot3es1_re231_Initialize is failed. (%d)\n", RE231_CELL_STR(pTCtx->u1TunerCellID),sonyrf_result));
            }
        }else{
            sonyrf_result = sonyrf_ascot3_re231_Initialize(pTuner);
            if(sonyrf_result != SONYRF_RESULT_OK){
                mcDBG_MSG_L2(("[%s]RE231Tuner: sonyrf_ascot3_re231_Initialize is failed. (%d)\n",RE231_CELL_STR(pTCtx->u1TunerCellID), sonyrf_result));
            }
        }
    }
...
static void  RE231_SetSawBw(ITUNER_CTX_T * pTunerCtx, UINT8 sawbw)
{
    ...
    mcDBG_MSG_L2(("[%s]RE231Tuner: Saw BW = %d\n",RE231_CELL_STR(pTunerCtx->u1TunerCellID) ,sawbw));
}
 ...
 */
//-----------------------------------------------------------------------------
static INT32 RE231_GetRSSI(ITUNER_CTX_T* pTunerCtx)
{
    sonyrf_ascot3es1_t *pTunerEs1 = NULL;
    sonyrf_ascot3_t *pTuner = NULL;
    sonyrf_result_t sonyrf_result = SONYRF_RESULT_OK;
    ...
    SONY_RE231_MULTI_TUNER_CTX *pMultiTunerCtx = NULL;
    sonyrf_tuner_ctx *pg_sonyrf_tuner_ctx = NULL;
    if(!RE231_GetTunerCtx(pTunerCtx,&pMultiTunerCtx))
    {
        return 0;
    }
    pg_sonyrf_tuner_ctx = &(pMultiTunerCtx->g_sonyrf_tuner_ctx);
    ...
    mcDBG_MSG_L2(("[%s]RE231Tuner: RSSI (dB x 100) = %d\n",RE231_CELL_STR(pTunerCtx->u1TunerCellID) ,rssi_x100));
...
static UINT32 RE231_GetSSIIndex(UINT8 fgDVBTC,ITUNER_CTX_T* pTunerCtx)
{
    ...
    if(0 == fgDVBTC)
    {
        mcDBG_MSG_L2(("[%s]RE231_GETSSI_DVBT: Mod=%d, Code Rate=%d",RE231_CELL_STR(pTunerCtx->u1TunerCellID),ucQamMode,ucCodeRate));
        ucQamModeUnknown = 3;
    }
    else
    {
        mcDBG_MSG_L2(("[%s]RE231_GETSSI_DVBC: Mod=%d",RE231_CELL_STR(pTunerCtx->u1TunerCellID),ucQamMode));
        ucQamModeUnknown = 5;
    }
    if(ucQamMode == ucQamModeUnknown)
    {
        mcDBG_MSG_L1(("[%s]QAM mode unknown!\n",RE231_CELL_STR(pTunerCtx->u1TunerCellID)));
    ...
    i4Power_rec = (RE231_GetRSSI(pTunerCtx)-50)/100;  //*100->*1
    i4Power_rel = i4Power_rec - i4Power_ref;

    mcDBG_MSG_L2(("[%s]RE231_GETSSI: Power_rec=%d, Power_ref=%d, Power_rel=%d\n",RE231_CELL_STR(pTunerCtx->u1TunerCellID),i4Power_rec,i4Power_ref,i4Power_rel));
    if(i4Power_rel < -15)
    {
        u4SSI_Indicator = 0;
    }
