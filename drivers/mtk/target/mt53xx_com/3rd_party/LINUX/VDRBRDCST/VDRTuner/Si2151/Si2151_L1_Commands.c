...
unsigned char Si2151_pollForResetState (L1_Si2151_Context *api)
{
  ...
  start_time = Si2151_system_time();

  while (Si2151_system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (Si2151_L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      mcDBG_MSG_ERR(("Si2151_pollForResetState ERROR reading byte 0!\n"));
	...
	Si2151_system_wait(5); /* wait 5ms */
  }

  mcDBG_MSG_ERR(("Si2151_pollForResetState ERROR status Timeout!\n"));
...
unsigned char Si2151_GetStatus (L1_Si2151_Context *api)
{
    ...
    if (Si2151_L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1)
    {
      mcDBG_MSG_ERR(("Si2151_GetStatus ERROR reading byte 0!\n"));
      return ERROR_Si2151_POLLING_CTS;
    }
...
unsigned char Si2151_pollForCTS (L1_Si2151_Context *api)
{
  ...
  while (Si2151_system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if (Si2151_L0_ReadCommandBytes(api->i2c, 1, rspByteBuffer) != 1) {
      mcDBG_MSG_ERR(("Si2151_pollForCTS ERROR reading byte 0!\n"));
      return ERROR_Si2151_POLLING_CTS;
    }
  ...
  mcDBG_MSG_ERR(("Si2151_pollForCTS ERROR CTS Timeout!\n"));
  return ERROR_Si2151_CTS_TIMEOUT;
}
...
unsigned char Si2151_pollForResponse (L1_Si2151_Context *api, unsigned int nbBytes, unsigned char *pByteBuffer)
{
  ...
  while (Si2151_system_time() - start_time <1000)  { /* wait a maximum of 1000ms */
    if ((unsigned int)Si2151_L0_ReadCommandBytes(api->i2c, nbBytes, pByteBuffer) != nbBytes) {
      mcDBG_MSG_ERR(("Si2151_pollForResponse ERROR reading byte 0!\n"));
  ...
  mcDBG_MSG_ERR(("Si2151_pollForResponse ERROR CTS Timeout!\n"));
  return ERROR_Si2151_CTS_TIMEOUT;
}
                                         ...
                                         unsigned char   force_top_gain)
{
    ...
    mcDBG_MSG_L3(("Si2151 AGC_OVERRIDE"));
  #ifdef   DEBUG_RANGE_CHECK
    if ((force_max_gain > Si2151_AGC_OVERRIDE_CMD_FORCE_MAX_GAIN_MAX) ) {error_code++; mcDBG_MSG_ERR(("\n Out of range: "));}; mcDBG_MSG_L3(("FORCE_MAX_GAIN %d ", force_max_gain ));
    if ((force_top_gain > Si2151_AGC_OVERRIDE_CMD_FORCE_TOP_GAIN_MAX) ) {error_code++; mcDBG_MSG_ERR(("\n Out of range: "));}; mcDBG_MSG_L3(("FORCE_TOP_GAIN %d \n ", force_top_gain ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 2, cmdByteBuffer) != 2) {
      mcDBG_MSG_ERR(("Error writing AGC_OVERRIDE bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling AGC_OVERRIDE response\n"));
      return error_code;
    }
                                         ...
                                         unsigned long   freq)
{
    ...
    mcDBG_MSG_L3(("Si2151 ATV_SCAN_TUNE "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((freq > Si2151_ATV_SCAN_TUNE_CMD_FREQ_MAX)  || (freq < Si2151_ATV_SCAN_TUNE_CMD_FREQ_MIN) ) {error_code++; mcDBG_MSG_ERR(("\n Out of range: "));}; mcDBG_MSG_L3(("FREQ %d \n", freq ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 8, cmdByteBuffer) != 8) {
      mcDBG_MSG_ERR(("Error writing ATV_SCAN_TUNE bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling ATV_SCAN_TUNE response\n"));
      return error_code;
    }
                                         ...
                                         unsigned char   en_xout)
{
    ...
    mcDBG_MSG_L3(("Si2151 CONFIG_CLOCKS "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode    > Si2151_CONFIG_CLOCKS_CMD_SUBCODE_MAX   )                                                            ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("SUBCODE %d "   , subcode    ));
    if ((clock_mode > Si2151_CONFIG_CLOCKS_CMD_CLOCK_MODE_MAX)  || (clock_mode < Si2151_CONFIG_CLOCKS_CMD_CLOCK_MODE_MIN) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("CLOCK_MODE %d ", clock_mode ));
    if ((en_xout    > Si2151_CONFIG_CLOCKS_CMD_EN_XOUT_MAX   )                                                            ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("EN_XOUT %d\n "   , en_xout    ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 3, cmdByteBuffer) != 3) {
      mcDBG_MSG_ERR(("Error writing CONFIG_CLOCKS bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }
                                         ...
                                         unsigned char   i2c_broadcast)
{
    ...
    mcDBG_MSG_L3(("Si2151 CONFIG_I2C "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode       > Si2151_CONFIG_I2C_CMD_SUBCODE_MAX      )  || (subcode       < Si2151_CONFIG_I2C_CMD_SUBCODE_MIN      ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("SUBCODE %d "      , subcode       ));
    if ((i2c_broadcast > Si2151_CONFIG_I2C_CMD_I2C_BROADCAST_MAX)                                                               ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("I2C_BROADCAST %d \n", i2c_broadcast ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 3, cmdByteBuffer) != 3) {
      mcDBG_MSG_ERR(("Error writing CONFIG_I2C bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling CONFIG_I2C response\n"));
      return error_code;
    }
    return NO_Si2151_ERROR;
}
                                         ...
                                         unsigned char   xout_mode)
{
    ...
    mcDBG_MSG_L3(("Si2151 CONFIG_PINS "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((gpio1_mode > Si2151_CONFIG_PINS_CMD_GPIO1_MODE_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("GPIO1_MODE %d ", gpio1_mode ));
    if ((gpio1_read > Si2151_CONFIG_PINS_CMD_GPIO1_READ_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("GPIO1_READ %d ", gpio1_read ));
    if ((gpio2_mode > Si2151_CONFIG_PINS_CMD_GPIO2_MODE_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("GPIO2_MODE %d ", gpio2_mode ));
    if ((gpio2_read > Si2151_CONFIG_PINS_CMD_GPIO2_READ_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("GPIO2_READ %d ", gpio2_read ));
    if ((agc1_mode  > Si2151_CONFIG_PINS_CMD_AGC1_MODE_MAX ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("AGC1_MODE %d " , agc1_mode  ));
    if ((agc1_read  > Si2151_CONFIG_PINS_CMD_AGC1_READ_MAX ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("AGC1_READ %d " , agc1_read  ));
    if ((agc2_mode  > Si2151_CONFIG_PINS_CMD_AGC2_MODE_MAX ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("AGC2_MODE %d " , agc2_mode  ));
    if ((agc2_read  > Si2151_CONFIG_PINS_CMD_AGC2_READ_MAX ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("AGC2_READ %d " , agc2_read  ));
    if ((xout_mode  > Si2151_CONFIG_PINS_CMD_XOUT_MODE_MAX ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("XOUT_MODE %d \n" , xout_mode  ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 7, cmdByteBuffer) != 7) {
      mcDBG_MSG_ERR(("Error writing CONFIG_PINS bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 7, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling CONFIG_PINS response\n"));
      return error_code;
    }
                                         ...
                                         unsigned char   ctsien)
{
    ...
    mcDBG_MSG_L3(("Si2151 EXIT_BOOTLOADER "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((func   > Si2151_EXIT_BOOTLOADER_CMD_FUNC_MAX  )  || (func   < Si2151_EXIT_BOOTLOADER_CMD_FUNC_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("FUNC %d "  , func   ));
    if ((ctsien > Si2151_EXIT_BOOTLOADER_CMD_CTSIEN_MAX)                                                      ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("CTSIEN %d\n ", ctsien ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 2, cmdByteBuffer) != 2) {
      mcDBG_MSG_ERR(("Error writing EXIT_BOOTLOADER bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling EXIT_BOOTLOADER response\n"));
      return error_code;
    }
                                                   ...
                                                   int   offset_500hz)
{
    ...
    mcDBG_MSG_L3(("Si2151 FINE_TUNE "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((persistence  > Si2151_FINE_TUNE_CMD_PERSISTENCE_MAX )                                                            ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: "));}; mcDBG_MSG_L3(("\nPERSISTENCE %d " , persistence  ));
    if ((offset_500hz > Si2151_FINE_TUNE_CMD_OFFSET_500HZ_MAX)  || (offset_500hz < Si2151_FINE_TUNE_CMD_OFFSET_500HZ_MIN) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range:\n "));}; mcDBG_MSG_L3(("OFFSET_500HZ %d \n", offset_500hz ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 4, cmdByteBuffer) != 4) {
      mcDBG_MSG_ERR(("Error writing FINE_TUNE bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling FINE_TUNE response\n"));
      return error_code;
    }
                                         ...
                                         unsigned int    prop)
{
    ...
    mcDBG_MSG_L3(("Si2151 GET_PROPERTY "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((reserved > Si2151_GET_PROPERTY_CMD_RESERVED_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED %d \n", reserved ));
    mcDBG_MSG_L3(("PROP %d \n"    , prop     ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 4, cmdByteBuffer) != 4) {
      mcDBG_MSG_ERR(("Error writing GET_PROPERTY bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 4, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling GET_PROPERTY response\n"));
      return error_code;
    }
...
unsigned char Si2151_L1_GET_REV         (L1_Si2151_Context *api)
{
    ...
    mcDBG_MSG_L3(("Si2151 GET_REV \n"));

    cmdByteBuffer[0] = Si2151_GET_REV_CMD;

    if (Si2151_L0_WriteCommandBytes(api->i2c, 1, cmdByteBuffer) != 1) {
      mcDBG_MSG_ERR(("Error writing GET_REV bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 10, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling GET_REV response\n"));
      return error_code;
    }
...
unsigned char Si2151_L1_PART_INFO       (L1_Si2151_Context *api)
{
    ...
    mcDBG_MSG_L3(("Si2151 PART_INFO \n"));

    cmdByteBuffer[0] = Si2151_PART_INFO_CMD;

    if (Si2151_L0_WriteCommandBytes(api->i2c, 1, cmdByteBuffer) != 1) {
      mcDBG_MSG_ERR(("Error writing PART_INFO bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 13, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling PART_INFO response\n"));
      return error_code;
    }
                                         ...
                                         unsigned char   reserved9)
{
    ...
    mcDBG_MSG_L3(("Si2151 POWER_DOWN_HW "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode   > Si2151_POWER_DOWN_HW_CMD_SUBCODE_MAX  )                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("SUBCODE %d "  , subcode   ));
    if ((pdb_xo_osc > Si2151_POWER_DOWN_HW_CMD_PDB_XO_OSC_MAX)                                                        ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n "));}; mcDBG_MSG_L3(("PDB_XO_OSC %d ", pdb_xo_osc ));
    if ((reserved1 > Si2151_POWER_DOWN_HW_CMD_RESERVED1_MAX)                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED1 %d ", reserved1 ));
    if ((en_xout   > Si2151_POWER_DOWN_HW_CMD_EN_XOUT_MAX  )                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("EN_XOUT %d "  , en_xout   ));
    if ((reserved2 > Si2151_POWER_DOWN_HW_CMD_RESERVED2_MAX)  || (reserved2 < Si2151_POWER_DOWN_HW_CMD_RESERVED2_MIN) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED2 %d ", reserved2 ));
    if ((reserved3 > Si2151_POWER_DOWN_HW_CMD_RESERVED3_MAX)                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED3 %d ", reserved3 ));
    if ((reserved4  > Si2151_POWER_DOWN_HW_CMD_RESERVED4_MAX )                                                        ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED4 %d " , reserved4 ));
    if ((reserved5  > Si2151_POWER_DOWN_HW_CMD_RESERVED5_MAX )                                                        ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED5 %d " , reserved5 ));
    if ((reserved6 > Si2151_POWER_DOWN_HW_CMD_RESERVED6_MAX)                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED6 %d ", reserved6 ));
    if ((reserved7 > Si2151_POWER_DOWN_HW_CMD_RESERVED7_MAX)                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED7 %d ", reserved7 ));
    if ((reserved8 > Si2151_POWER_DOWN_HW_CMD_RESERVED8_MAX)                                                          ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED8 %d ", reserved8 ));
    if ((reserved9  > Si2151_POWER_DOWN_HW_CMD_RESERVED9_MAX )                                                        ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED9 %d \n" , reserved9 ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 10, cmdByteBuffer) != 10) {
      mcDBG_MSG_ERR(("Error writing POWER_DOWN_HW bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResetState(api);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling POWER_DOWN_HW reset state\n"));
      return error_code;
    }
    return NO_Si2151_ERROR;
}
                                         ...
                                         unsigned char   reserved8)
{
    ...
    mcDBG_MSG_L3(("Si2151 POWER_UP "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode     > Si2151_POWER_UP_CMD_SUBCODE_MAX    )                                                         ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("SUBCODE %d "    , subcode     ));
    if ((clock_mode  > Si2151_POWER_UP_CMD_CLOCK_MODE_MAX )  || (clock_mode  < Si2151_POWER_UP_CMD_CLOCK_MODE_MIN ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("CLOCK_MODE %d " , clock_mode  ));
    if ((en_xout     > Si2151_POWER_UP_CMD_EN_XOUT_MAX    )                                                         ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("EN_XOUT %d "    , en_xout     ));
    if ((config_1p8v > Si2151_POWER_UP_CMD_CONFIG_1P8V_MAX)  || (config_1p8v < Si2151_POWER_UP_CMD_CONFIG_1P8V_MIN) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("CONFIG_1P8V %d ", config_1p8v ));
    if ((reserved3   > Si2151_POWER_UP_CMD_RESERVED3_MAX  )                                                         ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED3 %d "  , reserved3   ));
    if ((reserved4   > Si2151_POWER_UP_CMD_RESERVED4_MAX  )  || (reserved4   < Si2151_POWER_UP_CMD_RESERVED4_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED4 %d "  , reserved4   ));
    if ((reserved5   > Si2151_POWER_UP_CMD_RESERVED5_MAX  )  || (reserved5   < Si2151_POWER_UP_CMD_RESERVED5_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED5 %d "  , reserved5   ));
    if ((reserved6   > Si2151_POWER_UP_CMD_RESERVED6_MAX  )  || (reserved6   < Si2151_POWER_UP_CMD_RESERVED6_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED6 %d "  , reserved6   ));
    if ((reserved7   > Si2151_POWER_UP_CMD_RESERVED7_MAX  )  || (reserved7   < Si2151_POWER_UP_CMD_RESERVED7_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED7 %d "  , reserved7   ));
    if ((reserved8   > Si2151_POWER_UP_CMD_RESERVED8_MAX  )  || (reserved8   < Si2151_POWER_UP_CMD_RESERVED8_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED8 %d \n"  , reserved8   ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 10, cmdByteBuffer) != 10) {
      mcDBG_MSG_ERR(("Error writing POWER_UP bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling POWER_UP response\n"));
      return error_code;
    }
                                         ...
                                         unsigned char   offset)
{
    ...
    mcDBG_MSG_L3(("Si2151 REREAD_REPLY "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((offset > Si2151_REREAD_REPLY_CMD_OFFSET_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("OFFSET %d \n", offset ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 2, cmdByteBuffer) != 2) {
      mcDBG_MSG_ERR(("Error writing REREAD_REPLY bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 16, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling REREAD_REPLY response\n"));
      return error_code;
    }
                                         ...
                                         unsigned char   reserved2)
{
    ...
    mcDBG_MSG_L3(("Si2151 RESET_HW "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode   > Si2151_RESET_HW_CMD_SUBCODE_MAX  )  || (subcode   < Si2151_RESET_HW_CMD_SUBCODE_MIN  ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("SUBCODE %d "  , subcode   ));
    if ((reserved1 > Si2151_RESET_HW_CMD_RESERVED1_MAX)                                                     ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED1 %d ", reserved1 ));
    if ((reserved2 > Si2151_RESET_HW_CMD_RESERVED2_MAX)                                                     ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED2 %d \n", reserved2 ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 4, cmdByteBuffer) != 4) {
      mcDBG_MSG_ERR(("Error writing RESET_HW bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }
    error_code = Si2151_pollForResetState(api);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling RESET_HW response\n"));
      return error_code;
    }
                                         ...
                                         unsigned int    data)
{
    ...
    mcDBG_MSG_L3(("Si2151 SET_PROPERTY \n"));
  #ifdef   DEBUG_RANGE_CHECK
    mcDBG_MSG_L3(("RESERVED %d \n", reserved ));
    mcDBG_MSG_L3(("PROP %d \n"    , prop     ));
    mcDBG_MSG_L3(("DATA %d \n"    , data     ));
    ...
    if ((data != shadowData)  || (api->propertyWriteMode == DOWNLOAD_ALWAYS) || (res== ERROR_Si2151_UNKNOWN_PROPERTY) )
    {
        ...
        if (Si2151_L0_WriteCommandBytes(api->i2c, 6, cmdByteBuffer) != 6) {
          mcDBG_MSG_ERR(("Error writing SET_PROPERTY bytes!\n"));
          return ERROR_Si2151_SENDING_COMMAND;
        }

        error_code = Si2151_pollForResponse(api, 4, rspByteBuffer);
        if (error_code) {
          mcDBG_MSG_ERR(("Error polling SET_PROPERTY response\n"));
          return error_code;
        }
                                         ...
                                         unsigned char   type)
{
    ...
    mcDBG_MSG_L3(("Si2151 STANDBY "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((type > Si2151_STANDBY_CMD_TYPE_MAX) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("TYPE %d \n", type ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 2, cmdByteBuffer) != 2) {
      mcDBG_MSG_ERR(("Error writing STANDBY bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling STANDBY response\n"));
      return error_code;
    }
...
unsigned char Si2151_L1_TUNER_STATUS    (L1_Si2151_Context *api)
{
    ...
    mcDBG_MSG_L3(("Si2151 TUNER_STATUS \n"));
    cmdByteBuffer[0] = Si2151_TUNER_STATUS_CMD;

    if (Si2151_L0_WriteCommandBytes(api->i2c, 1, cmdByteBuffer) != 1) {
      mcDBG_MSG_ERR(("Error writing TUNER_STATUS bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 12, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling TUNER_STATUS response\n"));
      return error_code;
    }
                                         ...
                                         unsigned long   freq)
{
    ...
    mcDBG_MSG_L3(("Si2151 TUNER_TUNE_FREQ "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((mode > Si2151_TUNER_TUNE_FREQ_CMD_MODE_MAX)                                                  ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("MODE %d ", mode ));
    if ((freq > Si2151_TUNER_TUNE_FREQ_CMD_FREQ_MAX)  || (freq < Si2151_TUNER_TUNE_FREQ_CMD_FREQ_MIN) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("FREQ %d \n", freq ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 8, cmdByteBuffer) != 8) {
      mcDBG_MSG_ERR(("Error writing TUNER_TUNE_FREQ bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }

    error_code = Si2151_pollForResponse(api, 1, rspByteBuffer);
    if (error_code) {
      mcDBG_MSG_ERR(("Error polling TUNER_TUNE_FREQ response\n"));
      return error_code;
    }
                                         ...
                                         unsigned char   wake_up)
{
    ...
    mcDBG_MSG_L3(("Si2151 WAKE_UP "));
  #ifdef   DEBUG_RANGE_CHECK
    if ((subcode     > Si2151_WAKE_UP_CMD_SUBCODE_MAX    )  || (subcode     < Si2151_WAKE_UP_CMD_SUBCODE_MIN    ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("SUBCODE %d "    , subcode     ));
    if ((reset       > Si2151_WAKE_UP_CMD_RESET_MAX      )  || (reset       < Si2151_WAKE_UP_CMD_RESET_MIN      ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESET %d "      , reset       ));
    if ((clock_freq  > Si2151_WAKE_UP_CMD_CLOCK_FREQ_MAX )  || (clock_freq  < Si2151_WAKE_UP_CMD_CLOCK_FREQ_MIN ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("CLOCK_FREQ %d " , clock_freq  ));
    if ((reserved9   > Si2151_WAKE_UP_CMD_RESERVED9_MAX  )                                                        ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("RESERVED9 %d "  , reserved9   ));
    if ((func        > Si2151_WAKE_UP_CMD_FUNC_MAX       )  || (func        < Si2151_WAKE_UP_CMD_FUNC_MIN       ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("FUNC %d "       , func        ));
    if ((wake_up     > Si2151_WAKE_UP_CMD_WAKE_UP_MAX    )  || (wake_up     < Si2151_WAKE_UP_CMD_WAKE_UP_MIN    ) ) {error_code++; mcDBG_MSG_ERR(("\nOut of range: \n"));}; mcDBG_MSG_L3(("WAKE_UP %d "    , wake_up     ));
    if (error_code) {
      mcDBG_MSG_ERR(("%d out of range parameters\n", error_code));
      return ERROR_Si2151_PARAMETER_OUT_OF_RANGE;
    }
    ...
    if (Si2151_L0_WriteCommandBytes(api->i2c, 7, cmdByteBuffer) != 7) {
      mcDBG_MSG_ERR(("Error writing WAKE_UP bytes!\n"));
      return ERROR_Si2151_SENDING_COMMAND;
    }
