...
#include "Si2151_firmware_0_A_build_18.h"       /* firmware compatible with Si2151-marking */
...
int Si2151_PowerUpWithPatch    (L1_Si2151_Context *api)
{
    ...
    /* always wait for CTS prior to POWER_UP command */
        if ((return_code = Si2151_pollForCTS  (api)) != NO_Si2151_ERROR) {
            mcDBG_MSG_ERR(("Si2151_pollForCTS error 0x%02x\n", return_code));
            return return_code;
        }
    ...
    do {
            ...
            {
                mcDBG_MSG_ERR(("Si2151_L1_POWER_UP error 0x%02x: %s\n", return_code, Si2151_L1_API_ERROR_TEXT(return_code)));
                return return_code;
            }
        ...
        {
            mcDBG_MSG_ERR(("Si2151_L1_WAKE_UP error 0x%02x: %s\n", return_code, Si2151_L1_API_ERROR_TEXT(return_code)));
            return return_code;
        }
    }
    ...
    if ((return_code = Si2151_L1_PART_INFO(api)) != NO_Si2151_ERROR) {
        mcDBG_MSG_ERR(("Si2151_L1_PART_INFO error 0x%02x: %s\n", return_code, Si2151_L1_API_ERROR_TEXT(return_code)));
        return return_code;
    }
    mcDBG_MSG_L2(("chiprev %d\n",        api->rsp->part_info.chiprev));
    mcDBG_MSG_L2(("part    Si21%d\n",    api->rsp->part_info.part   ));
    mcDBG_MSG_L2(("romid   %d\n",        api->rsp->part_info.romid  ));
    mcDBG_MSG_L2(("pmajor  %d\n",        api->rsp->part_info.pmajor ));
    mcDBG_MSG_L2(("pminor  %d\n",        api->rsp->part_info.pminor ));
    mcDBG_MSG_L2(("pbuild  %d\n",        api->rsp->part_info.pbuild ));
    if ((api->rsp->part_info.pmajor >= 0x30) & (api->rsp->part_info.pminor >= 0x30)) {
    mcDBG_MSG_L2(("pmajor  '%c'\n",    api->rsp->part_info.pmajor ));
    mcDBG_MSG_L2(("pminor  '%c'\n",    api->rsp->part_info.pminor ));
    mcDBG_MSG_L2(("Full Info       'Si21%02d-%c%c%c ROM%x NVM%c_%cb%d'\n\n\n", api->rsp->part_info.part, api->rsp->part_info.chiprev + 0x40, api->rsp->part_info.pmajor, api->rsp->part_info.pminor, api->rsp->part_info.romid, api->rsp->part_info.pmajor, api->rsp->part_info.pminor, api->rsp->part_info.pbuild ));
    }
     if (!(api->load_control & SKIP_LOADFIRMWARE))
     {
        /* Check part info values and load the proper firmware */
          if (api->rsp->part_info.romid == 0x60)
          {
             if ((return_code = Si2151_LoadFirmware_16(api, Si2151_FW_0_Ab18, FIRMWARE_LINES_0_Ab18)) != NO_Si2151_ERROR) {
               mcDBG_MSG_ERR(("Si2151_LoadFirmware error 0x%02x: %s\n", return_code, Si2151_L1_API_ERROR_TEXT(return_code)));
               return return_code;
             }
          }
          else
          {
              mcDBG_MSG_ERR(("INCOMPATIBLE PART error ROMID 0x%02x\n", api->rsp->part_info.romid));
...
int Si2151_PowerUpUsingBroadcastI2C    (L1_Si2151_Context *tuners[], int tuner_count )
{
  ...
  for (t = 0; t < tuner_count; t++)
  {
      ...
      if ( (return_code = Si2151_PowerUpWithPatch(tuners[t])) != NO_Si2151_ERROR) {
        mcDBG_MSG_ERR(("Tuner %d Si2151_PowerUpWithPatch error 0x%02x\n", return_code));
        return return_code;
      }
       ...
       {
          mcDBG_MSG_ERR(("Tuner %d L1_CONFIG_I2C error 0x%02x\n", t, return_code));
          return return_code;
       }
 ...
 /* Check CTS for all tuners */
    if ( (return_code = Si2151_pollForCTS(tuners[t])) != NO_Si2151_ERROR ) {
      mcDBG_MSG_ERR(("Tuner %d pollForCTS error 0x%02x\n", t, return_code));
      return return_code;
    }
...
int Si2151_LoadFirmware_16        (L1_Si2151_Context *api, firmware_struct fw_table[], int nbLines)
{
    ...
    mcDBG_MSG_L2(("Si2151_LoadFirmware_16 starting...\n"));
    mcDBG_MSG_L2(("Si2151_LoadFirmware_16 nbLines %d\n", nbLines));
    ...
    for (line = 0; line < nbLines; line++)
    {
      if (fw_table[line].firmware_len > 0)  /* don't download if length is 0 , e.g. dummy firmware */
      {
        ...
        {
          mcDBG_MSG_ERR(("Si2151_LoadFirmware_16 error 0x%02x patching line %d: %s\n", return_code, line, Si2151_L1_API_ERROR_TEXT(return_code)));
          if (line == 0) {
          mcDBG_MSG_ERR(("The firmware is incompatible with the part!\n"));
          }
          //SiTraceConfiguration((char*)"traces resume"); /* Comment out for Mediatek platform porting */
          return ERROR_Si2151_LOADING_FIRMWARE;
        }
        //if (line==3) SiTraceConfiguration("traces suspend"); /* Comment out for Mediatek platform porting */
      }
    }
    //SiTraceConfiguration("traces resume"); /* Comment out for Mediatek platform porting */
    ...
    mcDBG_MSG_L2(("Si2151_LoadFirmware_16 complete...\n"));
    return NO_Si2151_ERROR;
}
/************************************************************************************************************************
...
int Si2151_LoadFirmware        (L1_Si2151_Context *api, unsigned char fw_table[], int nbLines)
{
    ...
    mcDBG_MSG_L2(("Si2151_LoadFirmware starting...\n"));
    mcDBG_MSG_L2(("Si2151_LoadFirmware nbLines %d\n", nbLines));
    ...
    for (line = 0; line < nbLines; line++)
    {
        ...
        {
          mcDBG_MSG_ERR(("Si2151_LoadFirmware error 0x%02x patching line %d: %s\n", return_code, line, Si2151_L1_API_ERROR_TEXT(return_code)));
          if (line == 0) {
          mcDBG_MSG_ERR(("The firmware is incompatible with the part!\n"));
          }
          //SiTraceConfiguration((char*)"traces resume"); /* Comment out for Mediatek platform porting */
    ...
    mcDBG_MSG_L2(("Si2151_LoadFirmware complete...\n"));
...
{
    int return_code;
    mcDBG_MSG_L2(("Si2151_Init starting...\n"));

    if ((return_code = Si2151_PowerUpWithPatch(api)) != NO_Si2151_ERROR) {   /* PowerUp into bootloader */
        mcDBG_MSG_ERR(("Si2151_PowerUpWithPatch error 0x%02x: %s\n", return_code, Si2151_L1_API_ERROR_TEXT(return_code)));
        return return_code;
    }
	...
	if ((return_code = Si2151_storeUserConfigPins(api)) != NO_Si2151_ERROR) {
        mcDBG_MSG_ERR(("Si2151_storeUserConfigPins error 0x%02x: %s\n", return_code, Si2151_L1_API_ERROR_TEXT(return_code)));
        return return_code;
    }
	
    mcDBG_MSG_L2(("Si2151_Init complete...\n"));
    return NO_Si2151_ERROR;
}
 /************************************************************************************************************************
  NAME: Si2151_Tune
 ...
 int  Si2151_Tune              (L1_Si2151_Context *api, unsigned char mode, unsigned long freq)
{
    ...
    int          return_code = 0;
    int          timeout     = 41; //36; /* Add 5ms respectively for porting on Mediatek platform */
    ...
    while ( (Si2151_system_time() - start_time) < timeout )
    {
    ...
    if (!api->status->tunint) {
      mcDBG_MSG_ERR(("Timeout waiting for TUNINT\n"));
      return ERROR_Si2151_TUNINT_TIMEOUT;
    }

    /* wait for xTVINT, timeout is 110ms for ATVINT and 10 ms for DTVINT */
    start_time = Si2151_system_time();
    timeout    = ((mode==Si2151_TUNER_TUNE_FREQ_CMD_MODE_ATV) ? 115 : 15); /* Add 5ms respectively for porting on Mediatek platform */
    while ( (Si2151_system_time() - start_time) < timeout )
    {
        if ((return_code = Si2151_L1_CheckStatus(api)) != NO_Si2151_ERROR)
            return return_code;
    ...
    if (mode==Si2151_TUNER_TUNE_FREQ_CMD_MODE_ATV)
    {
      if (api->status->atvint)
      {
        mcDBG_MSG_L2(("ATV Tune Successful\n"));
        return NO_Si2151_ERROR;
      }
      else
        mcDBG_MSG_ERR(("Timeout waiting for ATVINT\n"));
    }
    else
    {
        if (api->status->dtvint)
        {
          mcDBG_MSG_L2(("DTV Tune Successful\n"));
          return NO_Si2151_ERROR;
        }
        else
          mcDBG_MSG_ERR(("Timeout waiting for DTVINT\n"));
    }
...
{
    int return_code;
    mcDBG_MSG_L2(("Si2151_AGC_Override: mode = %d\n",mode));
    switch (mode)
    {
...
{
    int return_code;
    mcDBG_MSG_L2(("Si2151_XoutOn:  Turning Xout ON\n"));
...
{
    int return_code;
    mcDBG_MSG_L2(("Si2151_XoutOff:  Turning Xout OFF\n"));
...
{
    int return_code;
    mcDBG_MSG_L2(("Si2151_Standby: Going to Standby. The part will wake on the next command\n"));
...
{
    int return_code;
    mcDBG_MSG_L2(("Si2151_Powerdown: Powering down the part. Select InitAndConfig to reload.\n"));
...
#if 0 /* Comment out for Mediatek platform porting */
...
#endif /* Comment out for Mediatek platform porting */
