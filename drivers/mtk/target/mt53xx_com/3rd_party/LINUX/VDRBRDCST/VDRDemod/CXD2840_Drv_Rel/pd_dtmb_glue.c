    ...
    case 'a':
        {
            ...
            sony_i2c_MTK_CreateI2cGw_DTMB(&i2cTuner, pDemod->i2cAddress, 0x09);
...
INT32 DTMB_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
  ...
  psTunerCtx->u1DemodI2cAddress_Drv=0xD8;
   ...
   // #ifdef DTMB_MODEL_41M_XTAL
               demod_xtal = SONY_CN_DTV_DEMOD_41000KHZ;
//	#else
	//demod_xtal = SONY_CN_DTV_DEMOD_20500KHZ;
//	#endif
   ...
   // is_parallel = FALSE;
    ...
    sony_result = sony_i2c_MTK_CreateI2c_DTMB(pI2c);
...

    {
       ...
       // sony_cn_dtv_demod_Create(pDemod, pI2c, 0xDA, demod_xtal, &iffreq_config);
    }
    ...
    // Use ASCOT2E
    sony_result = sony_cn_dtv_demod_SetConfig(pDemod, SONY_CN_DTV_DEMOD_CONFIG_IS_ASCOT, SONY_CN_DTV_DEMOD_TUNER_ASCOT2E);
    if(sony_result != SONY_RESULT_OK){// return DRVAPI_TUNER_ERROR; 
	}
    ...
    if(is_parallel){
    	...
    	//#ifdef DTMB_MODEL_TSDATA7
		/* TS Serial Output from TSDATA7 pin */
		//sony_result = sony_cn_dtv_demod_SetConfig(pDemod, SONY_CN_DTV_DEMOD_CONFIG_TS_OUTPUT_PARALLEL_SERIAL,
          //  SONY_CN_DTV_DEMOD_TS_OUTPUT_SERIAL_7);
	//	#else
        /* TS Serial Output from TSDATA0 pin */
        sony_result = sony_cn_dtv_demod_SetConfig(pDemod, SONY_CN_DTV_DEMOD_CONFIG_TS_OUTPUT_PARALLEL_SERIAL,
            SONY_CN_DTV_DEMOD_TS_OUTPUT_SERIAL_0);
	//	#endif
        if(sony_result != SONY_RESULT_OK){
            mcDBG_MSG_L1(("Error in sony_cn_dtv_demod_SetConfig (%s).\n", sony_FormatResult(sony_result)));
          //  return DRVAPI_TUNER_ERROR;
...
//#if 0 /* If TS ERR is necessary, please enable this part. */
