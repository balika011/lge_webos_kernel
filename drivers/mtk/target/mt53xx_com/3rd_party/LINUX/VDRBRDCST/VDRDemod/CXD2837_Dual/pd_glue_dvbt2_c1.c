...
#define SONY_CXD2837_DRIVER_VERSION_STR "Sony CXD2837/2842/2841_Dual v0.01 2014-01-16"
...
#define mcDBG_MSG_L1(_x_) do{\
    DBG_PRTINF(DBG_PRINTF_L1,(SONY_DEMOD_CELL_STR));\
    DBG_PRTINF(DBG_PRINTF_L1,_x_);\
    }while(0)
#define mcDBG_MSG_L2(_x_) do{\
        DBG_PRTINF(DBG_PRINTF_L2,(SONY_DEMOD_CELL_STR));\
        DBG_PRTINF(DBG_PRINTF_L2,_x_);\
    }while(0)
#define mcDBG_MSG_L4(_x_) do{\
        DBG_PRTINF(DBG_PRINTF_L4,(SONY_DEMOD_CELL_STR));\
...
static BOOL fgHWReset = SONY_DEMOD_RESET_DISABLE; /*wenming 12-3-24: HW reset flag*/
static BOOL fgChanScanforGetAtt = FALSE;
static BOOL fgDemodInited = FALSE;
static TUNER_DEVICE_CTX_T*  pDVBT2TunerDeviceCtx = NULL;
...
static sony_result_t sony_tuner_MTK_Initialize (sony_tuner_terr_cable_t * pTuner)
{
    ...
    pTCtx = ITunerGetCtx_Multi(ITUNER_CELL_ID);
    if(!pTCtx) //Got null pointer
    {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_ARG);
    }
    //Set tuner ID & gateway demod I2C address
    pTCtx->u1TunerCellID = ITUNER_CELL_ID;
    pTCtx->u1DemodI2cAddress = SONY_DEMOD_IIC_ADDRESS;
                                      ...
                                      sony_demod_bandwidth_t bandwidth)
{
...

    {
        ...
        ITuner_OP(ITunerGetCtx_Multi(ITUNER_CELL_ID), itSetSawBw, sawbw, NULL);

        // Set Tuner PLL
        if(ITuner_SetFreq(ITunerGetCtx_Multi(ITUNER_CELL_ID), &param))
        {
...
static sony_result_t sony_stopwatch_elapsed (sony_stopwatch_t * pStopwatch, uint32_t* pElapsed)
{
...
static UINT32 DVBT2_TunerGetSignalPER(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
...
static void DVBT2_TunerTestI2C(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
    //add your implementation here
...
static void DVBT2_TunerDVBT2Command(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    case 'c':{
        ...
        /*********NO5 .check system  config*********/
            ...
            #ifdef SONY_DEMOD_ENABLE_GATEWAY
    ...
    case 'a':
        {
            ...
            const UINT8 tunerI2cAddress = SONY_TUNER_II2_ADDRESS; /* Please fix it if tuner address is changed. */
            sony_i2c_t i2cTuner;

#ifndef SONY_DEMOD_ENABLE_GATEWAY
    ...
    case 'p':
        {
            ...
            mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
            sony_demod_I2cRepeaterEnable(pDemod, 0x01); // I2C repeater enable
            if(ITuner_OP(ITunerGetCtx_Multi(ITUNER_CELL_ID), itSetTrapThresold, isAnalog, &frequencyKHz) == ITUNER_OK){
                if(isAnalog){
                                ...
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    ...
    pTunerCtx = ITunerGetCtx_Multi(ITUNER_CELL_ID);
...
//    ITUNER_CTX_T *tTunerCtx;
    ...
    g_sony_demod_ctx.breakFct = NULL;
    g_sony_demod_ctx.breakFctArg = NULL;
    pInteg->pfStopwatch_elapsed = sony_stopwatch_elapsed;
    pInteg->u1Tuner_ID = ITUNER_CELL_ID;
