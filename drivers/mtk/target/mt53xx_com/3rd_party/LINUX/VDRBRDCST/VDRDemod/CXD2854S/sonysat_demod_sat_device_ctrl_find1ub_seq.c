                                                                 ...
                                                                 sonysat_singlecable_ub_slot_t targetUB)
{
    ...
    SONYSAT_TRACE_ENTER ("sonysat_demod_sat_device_ctrl_find1ub_seq_Initialize");
...
sonysat_result_t sonysat_demod_sat_device_ctrl_find1ub_seq_Sequence (sonysat_demod_sat_device_ctrl_find1ub_seq_t * pSeq)
{
    ...
    SONYSAT_TRACE_ENTER ("sonysat_demod_sat_device_ctrl_find1ub_seq_Sequence");
    ...
    if (pSeq->subseqSS.isEnable){
        ...
        case FIND1UB_SEQ_STATE_START:
            /* ODU_UbxSignal_ON */
            result = sonysat_singlecable_command_ODU_UbxSignal_ON (&pSeq->message, SONYSAT_SINGLECABLE_ADDRESS_ALL_DEVICES);
            if (result != SONYSAT_RESULT_OK) {return result;}
            pSeq->commandReq = 1;
            pSeq->commonParams.waitTime = SONYSAT_DEMOD_SAT_DEVICE_CTRL_FIND1UB_ODU_TX_WAIT;
        ...
        case FIND1UB_SEQ_STATE_SS_FIN:
            SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_POWERLIST("Power", pSeq->subseqSS.pPowerList);
        ...
        case FIND1UB_SEQ_STATE_TRIAL2:
            ...
            result = sonysat_demod_dvbs_s2_blindscan_subseq_pm_Start (&pSeq->subseqPM, pSeq->offsetKHz);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN (result);
            }
        ...
        case FIND1UB_SEQ_STATE_TRIAL3:
            ...
            result = sonysat_demod_dvbs_s2_blindscan_subseq_pm_Start (&pSeq->subseqPM, pSeq->offsetKHz - 500);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN (result);
            }
        ...
        case FIND1UB_SEQ_STATE_TRIAL5:
            ...
            result = sonysat_singlecable_command_ODU_PowerOFF (&pSeq->message, SONYSAT_SINGLECABLE_ADDRESS_ALL_DEVICES, pSeq->targetUB);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN (result);
            }
            pSeq->commandReq = 1;
        ...
        case FIND1UB_SEQ_STATE_TRIAL9:
            ...
            if((pSeq->powerOn - pSeq->powerOff) > 1000){
                ...
                /* Go to Finish State. */
                pSeq->ubNumber = SONYSAT_SINGLECABLE_UB_1;
        ...
        case FIND1UB_SEQ_STATE_FINISH:
            ...
            result = sonysat_singlecable_command_ODU_PowerOFF (&pSeq->message, 
                                                            SONYSAT_SINGLECABLE_ADDRESS_ALL_DEVICES,
                                                            (sonysat_singlecable_ub_slot_t)pSeq->ubNumber);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN (result);
            }
            pSeq->commandReq = 1;

            if (pSeq->ubNumber == SONYSAT_SINGLECABLE_UB_8) {
