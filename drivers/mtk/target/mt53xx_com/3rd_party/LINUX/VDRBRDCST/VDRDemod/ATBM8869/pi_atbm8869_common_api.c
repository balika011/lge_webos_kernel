...
static DEMOD_I2C_DATA_T TSOutFMTTbl2[]=
{
 ...
 {0x0b24,0x01},
 {0x0305,0x00},//1:serial TS mode, 0: parallel TS mode
 {0x031f,0x00},//1bit 1: TS output from TSDATA[7], bit 0: TS output from TSDATA[0]
};
...
static BOOL SetDemodIO(UINT8 u1I2cAddr,UINT8 portSts)
{
 ...
 if(ATBM_ChipID == 0x30)
  {fgRet&=ATBMRead(u1I2cAddr,0x10f7,&ui8GPOReg);
   fgRet&=ATBMRead(u1I2cAddr,0x10fb,&u1Data);
   ui8GPOReg|=0x80;
   u1Data|=0x04;
   fgRet&=ATBMWrite(u1I2cAddr,0x10f7,ui8GPOReg);
    switch(portSts)
 	{
	 case (TESTIO23_0):
	  { u1Data&=0xef;
	 	fgRet&=ATBMWrite(u1I2cAddr,0x10fb,u1Data);
	  }
	 break;
	 case (TSTTIO23_1):
	  {  u1Data|=0x10;
...
*/
 VOID ATBM_SwitchIfAGC(UINT8 u1I2cAddr,BOOL fgAnalog)
 {
  ...
  if(DRVCUST_OptQuery(eTunerRFAGCSwitchGpio,&u4SwitchPin)==0)
  {
	 if(fgAnalog)//ATV
	 {
	 SwitchValue=1;////20130514:Gpio set High for ATV
	 }
	 else
	 {
	 SwitchValue=0;
...
BOOL ATBMGetSigLvl(UINT8 u1I2cAddr,MOD_TYPE_T mode,UINT8 * pu1SigLvl)
{
 BOOL fgRet=TRUE;
 UINT16 u2IfagcCtrl;
 mcDBG_MSG_L2(("before tunerOp() U1I2cAddr==%d)\n,haniqng for debug",u1I2cAddr));
 ...
 mcDBG_MSG_L2(("Finishd ATBM GetSigLvl,U1I2cAddr==0x%x)\n,THQ",u1I2cAddr));
...
VOID ATBMDoHWReset(VOID)
{
  ...
  {
  mcDBG_MSG_L1(("Reset Demod GPIO %d 0->1 50 ms\n",u4ResetPin));
