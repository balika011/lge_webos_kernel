...
/*----------------------------------------------------------------------------
  Static Functions
----------------------------------------------------------------------------*/
static sonysat_result_t setProgress (sonysat_demod_dvbs_s2_blindscan_seq_t * pSeq,
                                                           ...
                                                           uint32_t maxSymbolRateKSps)
{
    ...
    SONYSAT_TRACE_ENTER ("sonysat_demod_dvbs_s2_blindscan_seq_Initialize");
    ...
    result = sonysat_demod_dvbs_s2_blindscan_AllocData(&(pSeq->commonParams.storage), &(pSeq->pBandList));
    if (result != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN(result);
    }
...
{
    sonysat_result_t result = SONYSAT_RESULT_OK;
    SONYSAT_TRACE_ENTER ("sonysat_demod_dvbs_s2_blindscan_seq_Sequence");
    ...
    if (pSeq->subseqSS.isEnable){
        ...
        if (pSeq->subseqBT.isEnable){
            ...
            /* FS seq */
            result = sonysat_demod_dvbs_s2_blindscan_subseq_fs_Sequence (&pSeq->subseqFS);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN(result);
            }
        }
        ...
        case BLINDSCAN_SEQ_STATE_START:
            if (pSeq->maxSymbolRateKSps >= 20000){
                ...
                result = sonysat_demod_dvbs_s2_blindscan_subseq_ss_Start(&pSeq->subseqSS,
                                                                      ...
                                                                      2000,
                                                                      20000,
                                                                      pSeq->commonParams.pDemod->spectrumMode,
                                                                      pSeq->commonParams.pDemod->spectrumCount);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
                pSeq->commonParams.waitTime = 0;
        ...
        case BLINDSCAN_SEQ_STATE_SS1_FIN:
            /* Get candidate (MP) */
            SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_POWERLIST("2MHz", pSeq->subseqSS.pPowerList);
            result = sonysat_demod_dvbs_s2_blindscan_algo_GetCandidateMp (&(pSeq->commonParams.storage),
            ...
            result = sonysat_demod_dvbs_s2_blindscan_algo_SortBySymbolrate (&(pSeq->commonParams.storage), pSeq->pCandList1, 20000);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN(result);
            }

            SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_CANDLIST("fs1", pSeq->pCandList1);
            ...
            result = sonysat_demod_dvbs_s2_blindscan_subseq_fs_Start(&pSeq->subseqFS, pSeq->pCandList1, 19900, 45000, pSeq->pDetectedList);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN(result);
            }
        ...
        case BLINDSCAN_SEQ_STATE_STAGE1_FIN:
            if (pSeq->minSymbolRateKSps < 20000){
                ...
                result = sonysat_demod_dvbs_s2_blindscan_ClearDataList (&(pSeq->commonParams.storage), pSeq->pCandList2);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
                result = sonysat_demod_dvbs_s2_blindscan_algo_GetNonDetectedBand (&(pSeq->commonParams.storage),
                                                                               pSeq->minPowerFreqKHz,
                                                                               pSeq->maxPowerFreqKHz,
                                                                               500,
                                                                               pSeq->pDetectedList,
        ...
        case BLINDSCAN_SEQ_STATE_SS2_START:
            ...
            if (pSeq->pBandCurrent){
                result = sonysat_demod_dvbs_s2_blindscan_ClearPowerList (&(pSeq->commonParams.storage), pSeq->subseqSS.pPowerList);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
                result = sonysat_demod_dvbs_s2_blindscan_subseq_ss_Start(&pSeq->subseqSS,
        ...
        case BLINDSCAN_SEQ_STATE_SS2_FIN:
            {
                ...
                sonysat_demod_dvbs_s2_blindscan_data_t * pLast1;
                sonysat_demod_dvbs_s2_blindscan_data_t * pLast2;

                result = sonysat_demod_dvbs_s2_blindscan_AllocData (&pSeq->commonParams.storage, &pList);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }

                SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_POWERLIST("500KHz", pSeq->subseqSS.pPowerList);
                ...
                result = sonysat_demod_dvbs_s2_blindscan_algo_ReduceCandidate (&(pSeq->commonParams.storage), pList, pSeq->pDetectedList);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }

                result = sonysat_demod_dvbs_s2_blindscan_algo_DeleteDuplicate (&(pSeq->commonParams.storage), pList);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
                ...
                result = sonysat_demod_dvbs_s2_blindscan_FreeData (&pSeq->commonParams.storage, pList);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
        ...
        case BLINDSCAN_SEQ_STATE_FS2_START:
            ...
            SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_CANDLIST("fs2", pSeq->pCandList1);
            ...
            result = sonysat_demod_dvbs_s2_blindscan_subseq_fs_Start (&pSeq->subseqFS, pSeq->pCandList1, 4975, 20000, pSeq->pDetectedList);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN(result);
            }
        ...
        case BLINDSCAN_SEQ_STATE_FS2_FIN:
            {
                ...
                result = sonysat_demod_dvbs_s2_blindscan_algo_SeparateCandidate (&(pSeq->commonParams.storage), pSeq->pCandList2);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }

                result = sonysat_demod_dvbs_s2_blindscan_algo_DeleteDuplicate2 (&(pSeq->commonParams.storage), pSeq->pCandList2);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }

                result = sonysat_demod_dvbs_s2_blindscan_algo_ReduceCandidate (&(pSeq->commonParams.storage), pSeq->pCandList2, pSeq->pDetectedList);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
        ...
        case BLINDSCAN_SEQ_STATE_CS_PREPARING:
            ...
            if (pSeq->pCandCurrent){
                ...
                sonysat_demod_dvbs_s2_blindscan_data_t * pCurrent = pSeq->pDetectedList->pNext;
                while(pCurrent){
        ...
        case BLINDSCAN_SEQ_STATE_CS_TUNED:
            {
                ...
                result = sonysat_demod_dvbs_s2_blindscan_subseq_cs_Start (&(pSeq->subseqCS), 0);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN(result);
                }
        ...
        case BLINDSCAN_SEQ_STATE_FS3_START:
            result = sonysat_demod_dvbs_s2_blindscan_algo_DeleteDuplicate (&(pSeq->commonParams.storage), pSeq->pCandList1);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN(result);
            }

            SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_CANDLIST("fs3", pSeq->pCandList1);
            ...
            result = sonysat_demod_dvbs_s2_blindscan_subseq_fs_Start(&pSeq->subseqFS, pSeq->pCandList1, 1000, 5000, pSeq->pDetectedList);
            if (result != SONYSAT_RESULT_OK){
                SONYSAT_TRACE_RETURN(result);
            }
...
static sonysat_result_t setProgress (sonysat_demod_dvbs_s2_blindscan_seq_t * pSeq,
                                  ...
                                  uint8_t minorProgress)
{
    SONYSAT_TRACE_ENTER ("setProgress");
