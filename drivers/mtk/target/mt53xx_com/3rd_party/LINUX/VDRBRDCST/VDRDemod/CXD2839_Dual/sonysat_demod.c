...
#ifndef SONYSAT_DEMOD_SUPPORT_TERR_OR_CABLE
...
sonysat_result_t sonysat_demod_InitializeT_C (sonysat_demod_t * pDemod)
{
    ...
    if ((pDemod->chipId != SONYSAT_DEMOD_CHIP_ID_CXD2837) && 
        (pDemod->chipId != SONYSAT_DEMOD_CHIP_ID_CXD2839) && 
        (pDemod->chipId != SONYSAT_DEMOD_CHIP_ID_CXD2841) &&
        (pDemod->chipId != SONYSAT_DEMOD_CHIP_ID_CXD2842) &&
        (pDemod->chipId != SONYSAT_DEMOD_CHIP_ID_CXD2843)) {
        pDemod->state = SONYSAT_DEMOD_STATE_INVALID;
                                    ...
                                    int32_t value)
{
    ...
    case SONYSAT_DEMOD_CONFIG_TSCLK_MASK:
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x00,  0xC6, (uint8_t) value, 0x1D);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
        break;
    ...
    case SONYSAT_DEMOD_CONFIG_TSVALID_MASK:
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC8, (uint8_t) value, 0x1D);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
        break;
    ...
    case SONYSAT_DEMOD_CONFIG_TSERR_MASK:
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x00, 0xC9, (uint8_t) value, 0x1D);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
        break;
    ...
    case SONYSAT_DEMOD_CONFIG_TS_CURRENT_10mA:
...

        {
            ...
            if ((data & 0x0F) == SONYSAT_DEMOD_GPIO_MODE_TS_ERROR) {
                 ...
                 */
                if (sonysat_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVX, 0x85, (uint8_t) (value ? 0x04 : 0x00), 0x04) != SONYSAT_RESULT_OK) {
                    SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
                }
            }
        }
    ...
    case SONYSAT_DEMOD_CONFIG_TS_BACKWARDS_COMPATIBLE:
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x00, 0xD3, (uint8_t)(value ? 0x01 : 0x00), 0x01);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x00, 0xDE, (uint8_t)(value ? 0x01 : 0x00), 0x01);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
        break;
...
#ifdef SONYSAT_DEMOD_SUPPORT_TERR_OR_CABLE
    case SONYSAT_DEMOD_CONFIG_TSIF_SDPR:
        if ((pDemod->state != SONYSAT_DEMOD_STATE_SLEEP_T_C) && (pDemod->state != SONYSAT_DEMOD_STATE_SLEEP_S)) {
...

        {
             ...
             */
            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x10, 0xE4, data[0], 0x3F);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }

            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x10, 0xE5, data[1], 0xFF);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }

            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x10, 0xE6, data[2], 0xFF);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }
        }

        {
             ...
             */
            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x20, 0xDC, data[0], 0x1F);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }

            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT,  0x20, 0xDD, data[1], 0xFF);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }

            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x20, 0xDE, data[2], 0xFF);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }

            result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x20, 0xDF, data[3], 0xFF);
            if (result != SONYSAT_RESULT_OK) {
                SONYSAT_TRACE_RETURN (result);
            }
        }
    ...
    case SONYSAT_DEMOD_CONFIG_TS_AUTO_RATE_ENABLE:
        if ((pDemod->state != SONYSAT_DEMOD_STATE_SLEEP_T_C) && (pDemod->state != SONYSAT_DEMOD_STATE_SLEEP_S)) {
            /* This api is accepted in SLEEP state only */
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_SW_STATE);
        }
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x10, 0xE3, (uint8_t)(value ? 0x01 : 0x00), 0x01);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
         ...
         */
        result = sonysat_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x20, 0xD8, (uint8_t)(value ? 0x01 : 0x00), 0x01);
        if (result != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (result);
        }
        break;
    ...
    case SONYSAT_DEMOD_CONFIG_SAT_MEAS_PERIOD1:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_RANGE);
        }
    ...
    case SONYSAT_DEMOD_CONFIG_SAT_MEAS_PERIOD2:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_RANGE);
        }
    ...
    case SONYSAT_DEMOD_CONFIG_SAT_MEAS_PERIOD3:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_RANGE);
        }
    ...
    case SONYSAT_DEMOD_CONFIG_SAT_MEAS_PERIOD4:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_RANGE);
        }
...
{
#ifndef SONYSAT_DEMOD_ENABLE_GATEWAY //Defined in Makefile, mark it if use I2C bypass
                                        ...
                                        sonysat_demod_gpio_mode_t mode)
{
     ...
     */
    if (sonysat_i2c_SetRegisterBits(pDemod->pI2c, pDemod->i2cAddressSLVX, gpioModeSelAddr, (uint8_t)mode, 0x0F) != SONYSAT_RESULT_OK) {
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }

    {
         ...
         */
        if (sonysat_i2c_SetRegisterBits(pDemod->pI2c, pDemod->i2cAddressSLVX, 0x82, enableHiZ, gpioBitSel) != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
        }
    }
    ...
    if (id == 2) {
         ...
         * <SLV-X>  00h     85h     [2]    1'b0        OREG_GPIO2_C
         */
        if (sonysat_i2c_SetRegisterBits(pDemod->pI2c, pDemod->i2cAddressSLVX, 0x85, drivability, 0x04) != SONYSAT_RESULT_OK){
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
        }        
    }
                                    ...
                                    uint8_t value)
{
    ...
    */
    if (sonysat_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVX, 0xA2, (uint8_t) (value ? (0x01 << id) : 0x00), (0x01 << id)) != SONYSAT_RESULT_OK) {
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
...
static sonysat_result_t SDtoSLT(sonysat_demod_t * pDemod)
{
...

    {
        ...
        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVX, 0x13, &data[0], 2) != SONYSAT_RESULT_OK) {
            SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
        }
    }
