                                                 ...
                                                 uint32_t * pBER)
{
     ...
     */
    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x5B, tmpPacketNum, 2) != SONY_RESULT_OK) {
        SLVT_UnFreezeReg(pDemod);
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    switch (target) {
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_A:
        /*
         *  slave     Bank   Addr    Bit          Name
         * ------------------------------------------------------------
         ...
         */
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x16, tmpBitError, 3) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg(pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_B:
        /*
         *  slave     Bank   Addr    Bit          Name
         * ------------------------------------------------------------
         ...
         */
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x19, tmpBitError, 3) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg(pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_C:
        /*
         *  slave     Bank   Addr    Bit          Name
         * ------------------------------------------------------------
         ...
         */
        if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x1C, tmpBitError, 3) != SONY_RESULT_OK) {
            SLVT_UnFreezeReg(pDemod);
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    ...
    packetNum = ((uint32_t)tmpPacketNum[0] << 8) | tmpPacketNum[1];
    bitError = ((uint32_t)(tmpBitError[0] & 0x7F) << 16) | ((uint32_t)tmpBitError[1] << 8) | tmpBitError[2];
                                                          ...
                                                          uint32_t * pPEN)
{
     ...
     */
    result = pDemod->pI2c->ReadRegister(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xA1, rdata, 1);
    if (result != SONY_RESULT_OK) {
        SLVT_UnFreezeReg(pDemod);
        SONY_TRACE_RETURN(result);
    }    
    ...
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_A:
         ...
         */
        result = pDemod->pI2c->ReadRegister(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xA2, rdata, 2);
        break;
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_B:
        /*
         * slave    Bank    Addr    Bit           Name
         ...
         */
        result = pDemod->pI2c->ReadRegister(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xA4, rdata, 2);
        break;
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_C:
        /*
         * slave    Bank    Addr    Bit           Name
         ...
         */
        result = pDemod->pI2c->ReadRegister(pDemod->pI2c, pDemod->i2cAddressSLVT, 0xA6, rdata, 2);
        break;
    default:
        SLVT_UnFreezeReg(pDemod);
        SONY_TRACE_RETURN(SONY_RESULT_ERROR_ARG);
                                            ...
                                            uint32_t * pPER)
{
    ...
    packetError = ((uint32_t)tmpPacketError[0] << 8) + tmpPacketError[1];
    packetNum = ((uint32_t)tmpPacketNum[0] << 8) + tmpPacketNum[1];
                                               ...
                                               uint32_t * pTsRateKbps)
{
    ...
    switch (target) {
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_A:
        modIndex = (uint8_t)tmccInfo.currentInfo.layerA.modulation;
        codeRateIndex = (uint8_t)tmccInfo.currentInfo.layerA.codingRate;
        segmentNum = (uint8_t)tmccInfo.currentInfo.layerA.segmentsNum;
        break;
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_B:
        modIndex = (uint8_t)tmccInfo.currentInfo.layerB.modulation;
        codeRateIndex = (uint8_t)tmccInfo.currentInfo.layerB.codingRate;
        segmentNum = (uint8_t)tmccInfo.currentInfo.layerB.segmentsNum;
        break;
    case SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_C:
        modIndex = (uint8_t)tmccInfo.currentInfo.layerC.modulation;
        codeRateIndex = (uint8_t)tmccInfo.currentInfo.layerC.codingRate;
        segmentNum = (uint8_t)tmccInfo.currentInfo.layerC.segmentsNum;
        break;
    default:
        SONY_TRACE_RETURN(SONY_RESULT_ERROR_ARG);
    }

    if((modIndex > (uint8_t)SONY_ISDBT_MODULATION_64QAM) || (codeRateIndex > SONY_ISDBT_CODING_RATE_7_8)
        || (segmentNum > 13)){
        /* Specified layer is unused or some error */
        SONY_TRACE_RETURN(SONY_RESULT_ERROR_HW_STATE);
    }
    ...
    switch(pDemod->bandwidth){
    case SONY_DTV_BW_6_MHZ:
        *pTsRateKbps = dataRate1Seg6MHzBW[modIndex][codeRateIndex][guard] * segmentNum;
        break;
    case SONY_DTV_BW_7_MHZ:
        *pTsRateKbps = dataRate1Seg7MHzBW[modIndex][codeRateIndex][guard] * segmentNum;
        break;
    case SONY_DTV_BW_8_MHZ:
        *pTsRateKbps = dataRate1Seg8MHzBW[modIndex][codeRateIndex][guard] * segmentNum;
        break;
    default:
        SONY_TRACE_RETURN(SONY_RESULT_ERROR_SW_STATE);
    }

    *pTsRateKbps = (*pTsRateKbps + 50) / 100;
