...
sonysat_result_t sonysat_demod_dvbs_s2_blindscan_Initialize(sonysat_demod_t * pDemod)
{
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xBD, 0x00) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
...
sonysat_result_t sonysat_demod_dvbs_s2_blindscan_PS_INIT (sonysat_demod_t * pDemod, uint8_t ocfr_csk)
{
    uint8_t iqInv = 0;
    sonysat_result_t result = SONYSAT_RESULT_OK;
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x57, (ocfr_csk & 0x07)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    /* Set SLV-T Bank : 0xAB */
