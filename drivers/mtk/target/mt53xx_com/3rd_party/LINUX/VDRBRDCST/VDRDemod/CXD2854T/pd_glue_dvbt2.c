...
#define SONY_CXD2854_DVBTT2CC2_DRIVER_VERSION_STR "Sony CXD2854 (DVB-T/T2/C/C2) v0.13 2014-07-31"
...
// Static functions
//-----------------------------------------------------------------------------
static UINT32 sony_GetUEC(UINT8 tunerID);
//-----------------------------------------------------------------------------
// Sony specific
//-----------------------------------------------------------------------------
...
STATIC void DVBT2_TunerGetModuInfor(UINT8 tunerID, void* pzAttribute)
{
...
STATIC VOID vResetDemod(UINT8 tunerID, UINT16 u2DelayTime)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    UINT32 u4RstPinNum=201;

    // Config GPIO to reset demod
    mcDBG_MSG_L2(("CXD2854[%d] : vResetDemod\n", tunerID));
    if(!pSonyDemodCtx->fgHWReset)
    {
        if (DRVCUST_OptQuery(eDemodResetGpio, &u4RstPinNum) == 0)
        {
            ...
            mcDBG_MSG_L2(("CXD2854[%d] : Reset CXD2854 Demod GPIO=0x%x, 0->1, %dms\r\n", tunerID, u4RstPinNum, u2DelayTime));
        }
    }
}
...
static sony_result_t sony_integ_dvbc2_monitor_SSI (sony_integ_t * pInteg, uint8_t * pSSI)
{
    ...
    /* Get estimated RF Level (for MTK) */
    result = sony_integ_dvbc2_monitor_RFLevel (pInteg, &rfLevel);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
static BOOL SetBreakMonitor(DVBT2_PD_CTX_T *psPdCtx)
{
    ...
    if(psPdCtx->fgPIMoniStatus){ //PI really doing PI Monitor
        while(psPdCtx->fgPIMoniStatus){ //wait PI Monitor really finish break
            if(u1Cnt>250){ //in case of deadlock
                break;
            }
            u1Cnt++;
            mcDBG_MSG_L1(("DVBT2 PD wait break PI monitor finish. u1Cnt=%d\r\n", u1Cnt));
            //  mcDELAY_MS(10);
...
static void MonitorProcess(VOID *pvArg)
{
    ...
    sony_demod_ctx_t * pDemodCtx = NULL;

    if (psPdCtx == g_sony_demod_ctx[1].pPdCtxDvb){
        tunerID = 1;
        pDemodCtx = &g_sony_demod_ctx[1];
    }else{
        tunerID = 0;
        pDemodCtx = &g_sony_demod_ctx[0];
    }
    mcDBG_MSG_L2(("CXD2854[%d] : MonitorProcess start!\n", tunerID));
    while(psPdCtx->fgMonitorThreadRunning){
        if (psPdCtx->fgBreakMonitor){
            mcDBG_MSG_L2(("Entering DVBT2 mon_suspend\n"));
            mcSEMA_LOCK_MTFE(psPdCtx->t_escape_mon_suspend);
            mcDBG_MSG_L2(("Escape DVBT2 mon_suspend\n"));
        }else{
            ...
            if(uNcount == 1){
                ...
                uNcount = 0;
                temp = (UINT64)(pDemodCtx->u4Uec) + (UINT64)sony_GetUEC(tunerID);
                    if(temp > 0xffffffff){
                        temp = 0xffffffff;
...
************************************************************************/
static INT32 PD_StartMonitorEngine(DVBT2_PD_CTX_T *psPdCtx)
{
    UINT8 tunerID;
    if(psPdCtx == g_sony_demod_ctx[1].pPdCtxDvb){
        tunerID = 1;
    ...
    if(tunerID == 0){ //Two threads are created
        if(x_thread_create(&psPdCtx->t_monitor_thread,
            ...
            mcDBG_MSG_ERR(("PD_StartMonitorEngine (x_thread_create): DRVAPI_TUNER_ERROR!\n"));
    ...
    mcDBG_MSG_L1(("DVBT2 PD_StartMonitorEngine success\n"));
...
************************************************************************/
static INT32 PD_StopMonitorEngine(DVBT2_PD_CTX_T *psPdCtx)
{
    while (psPdCtx->t_monitor_thread){
        x_thread_delay((UINT32)(10));
    }
    mcDBG_MSG_L2(("DVBT2 PD_StopMonitorEngine success\n"));
    return (DRVAPI_TUNER_OK);
}
 ...
 */
//-------------------------------------------------------------------------
static INT32 DVBT2_TunerClose(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    sony_integ_t *pInteg = &pSonyDemodCtx->integ;

    if(pSonyDemodCtx->fgDemodInited == TRUE)
    {
        pSonyDemodCtx->fgDemodInited = FALSE; 
    }
    else
    {
        mcDBG_MSG_L1(("CXD2854[%d] : CXD2854 (DVB) had been closed!\n", tunerID));
        return (DRVAPI_TUNER_OK);
    }
    
    mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
    mcDBG_MSG_L2(("CXD2854[%d] : %s \n", tunerID,  __FUNCTION__));
    pPdCtxDVBT2->fgBreakMonitor = TRUE;
    pPdCtxDVBT2->fgMonitorThreadRunning = FALSE;
    mcSEMA_UNLOCK_MTFE(pPdCtxDVBT2->t_escape_mon_suspend);
    PD_StopMonitorEngine(pPdCtxDVBT2);
    x_sema_delete(pPdCtxDVBT2->t_escape_mon_suspend); 
 ...
 */
//-------------------------------------------------------------------------
static BOOL DVBT2_TunerAcq(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, void* pv_conn_info,
                            SIZE_T z_conn_info_len, x_break_fct _BreakFct, void* pvArg)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    TUNER_DEVICE_CTX_T * pDvbt2TunerDeviceCtx = (TUNER_DEVICE_CTX_T*)pSonyDemodCtx->pTunerDeviceCtxDvb;
    ...
    mcDBG_MSG_L2(("CXD2854[%d] : DVBT2_TunerAcq--------->>>>>>>>>>>>\n", tunerID));

    //MUTEX lock
    mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
    
    //Pause monitor thread
    if(pDvbt2TunerDeviceCtx->u1MWIntf != cMTAL_IF && pDvbt2TunerDeviceCtx->u1MWIntf != cSDAL_IF){
    ...
    pSonyDemodCtx->breakFct = _BreakFct;
    pSonyDemodCtx->breakFctArg = pvArg;
    // Clear cancel flag
    ...
    if(pPdCtxDVBT2->fgDisStatus == TRUE){
        ...
        // REFOUT enable
        ITuner_OP(ITunerGetCtx_Multi(tunerID), itSetRefOut, 1, NULL);
        ...
        if(data == 0x01){
            ...
            if((data != 0x01) && (data != 0x02) && (data != 0x04) && (data != 0x05) && (data != 0x06)){
                ...
                mcDBG_MSG_L1(("CXD2854[%d] : Switch to Terrestrial/Cable.\n", tunerID));
                sony_result = sony_demod_ReinitializeT_C(pDemod);
                if(sony_result != SONY_RESULT_OK){
                    mcDBG_MSG_L1(("Error in sony_demod_ReinitializeT_C (%s)\n", sony_FormatResult(sony_result)));
                    fgLock = 0;
    ...
    if(e_conn_type == TUNER_CONN_TYPE_CAB_DIG){ // DVB-C/C2
        ...
        {
            fgChannelScan = TRUE;
            pSonyDemodCtx->fgChanScanforGetAtt = TRUE;
        }
        else
        {
            pSonyDemodCtx->fgChanScanforGetAtt = FALSE;
        }
        pSonyDemodCtx->frequency = pt_tuner_info->ui4_freq/1000;
        ...
        #endif
        {
            ...
            mcDBG_MSG_L2(("CXD2854[%d] : Freq = %d\n", tunerID, tune_param.centerFreqKHz));
            mcDBG_MSG_L2(("CXD2854[%d] : System = DVB-C\n", tunerID));
            mcDBG_MSG_L2(("CXD2854[%d] : Bandwidth = 8\n", tunerID));
            if(fgChannelScan){
                mcDBG_MSG_L2(("CXD2854[%d] : Channel Scan\n", tunerID));
            }else{
                mcDBG_MSG_L2(("CXD2854[%d] : Normal Tune\n", tunerID));
            }

            sony_result = sony_integ_dvbc_Tune(pInteg, &tune_param);
            switch(sony_result){
            case SONY_RESULT_OK:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-C TS Locked.\n", tunerID));
                fgLock = 1;
                break;
            case SONY_RESULT_ERROR_UNLOCK:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-C TS Unlocked.\n", tunerID));
                fgLock = 0;
                break;
            case SONY_RESULT_ERROR_TIMEOUT:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-C Wait TS Lock but Timeout.\n", tunerID));
                fgLock = 0;
                break;
            default:
                mcDBG_MSG_L1(("CXD2854[%d] : Error in sony_integ_dvbc_Tune (%s)\n", tunerID, sony_FormatResult(sony_result)));
                fgLock = 0;
                break;
            }
        }
        ...
        if((fgLock == 0) && ((pt_tuner_info->e_dvb_c_c2_mode == TUNER_DVB_C2) || (pt_tuner_info->e_dvb_c_c2_mode == TUNER_DVB_AUTO_C_C2))){
            pSonyDemodCtx->c2DsId = 0;
            pSonyDemodCtx->c2PlpId = 0;
            ...
            if(fgChannelScan){
                ...
                mcDBG_MSG_L2(("CXD2854[%d] : Freq = %d kHz\n", tunerID, frequencyKHz));
                mcDBG_MSG_L2(("CXD2854[%d] : System = DVB-C2 (Scan)\n", tunerID));
                ...
                switch(sony_result){
                case SONY_RESULT_OK:
                    mcDBG_MSG_L2(("CXD2854[%d] : DVB-C2 Locked. (Signal Found)\n", tunerID));
                    fgLock = 1;

                    mcDBG_MSG_L2(("CXD2854[%d] : Number of Tune Params = %d\n", tunerID, num_tune_param));
                    mcDBG_MSG_L2(("CXD2854[%d] : Next Freq = %d kHz\n", tunerID, next_frequency));

                    pSonyDemodCtx->c2DsPlpList.e_bw = pt_tuner_info->e_bw;
                    pSonyDemodCtx->c2DsPlpList.ui2_curr_num = (UINT16)num_tune_param;
                    pSonyDemodCtx->c2NextFrequency = (UINT32)(next_frequency * 1000);

                    for(i = 0; i < (UINT16)num_tune_param; i++){
                        mcDBG_MSG_L2(("CXD2854[%d] : [%d] Freq = %d kHz\n", tunerID, i, ptune_param[i].c2TuningFrequencyKHz));
                        mcDBG_MSG_L2(("CXD2854[%d] : [%d] DS ID = %d\n", tunerID, i, ptune_param[i].dataSliceId));
                        mcDBG_MSG_L2(("CXD2854[%d] : [%d] PLP ID = %d\n", tunerID, i, ptune_param[i].dataPLPId));
                        mcDBG_MSG_L2(("CXD2854[%d] : [%d] IsDependentStaticDS = %d\n", tunerID, i, ptune_param[i].isDependentStaticDS));

                        // Store the information to g_sony_demod_ctx
                        pSonyDemodCtx->c2DsPlpList.t_plp_info[i].ui4_c2_tuning_freq = (UINT32)(ptune_param[i].c2TuningFrequencyKHz * 1000);
                        pSonyDemodCtx->c2DsPlpList.t_plp_info[i].ui1_ds_id = (UINT8)ptune_param[i].dataSliceId;
                        pSonyDemodCtx->c2DsPlpList.t_plp_info[i].ui1_plp_id = (UINT8)ptune_param[i].dataPLPId;
                        pSonyDemodCtx->c2DsPlpList.t_plp_info[i].b_dependent_static_ds = (BOOL)ptune_param[i].isDependentStaticDS;
                    }
                    break;
                case SONY_RESULT_ERROR_UNLOCK:
                    mcDBG_MSG_L2(("CXD2854[%d] : DVB-C2 Unlocked. (Signal Not Found)\n", tunerID));
                    fgLock = 0;
                    pSonyDemodCtx->c2NextFrequency = (UINT32)(next_frequency * 1000);
                    break;
                case SONY_RESULT_ERROR_TIMEOUT:
                    mcDBG_MSG_L2(("CXD2854[%d] : DVB-C2 Wait Lock but Timeout. (Signal Not Found)\n", tunerID));
                    fgLock = 0;
                    pSonyDemodCtx->c2NextFrequency = (UINT32)(next_frequency * 1000);
                    break;
                default:
                    mcDBG_MSG_L1(("CXD2854[%d] : Error in sony_integ_dvbc2_GetChannelTuneParameters (%s)\n", tunerID, sony_FormatResult(sony_result)));
                    fgLock = 0;
                    break;
                }
                ...
                mcDBG_MSG_L2(("CXD2854[%d] : Freq = %d kHz\n", tunerID, tune_param.c2TuningFrequencyKHz));
                mcDBG_MSG_L2(("CXD2854[%d] : System = DVB-C2\n", tunerID));
                mcDBG_MSG_L2(("CXD2854[%d] : Bandwidth = %d\n", tunerID, tune_param.bandwidth));
                mcDBG_MSG_L2(("CXD2854[%d] : DS ID = %d\n", tunerID, tune_param.dataSliceId));
                mcDBG_MSG_L2(("CXD2854[%d] : PLP ID = %d\n", tunerID, tune_param.dataPLPId));
                mcDBG_MSG_L2(("CXD2854[%d] : IsDependentStaticDS = %d\n", tunerID, tune_param.isDependentStaticDS));
                mcDBG_MSG_L2(("CXD2854[%d] : Normal Tune\n", tunerID));

                // Store DS ID and PLP ID for DVBT2_TunerGetSignal
                pSonyDemodCtx->c2DsId = (UINT8)tune_param.dataSliceId;
                pSonyDemodCtx->c2PlpId = (UINT8)tune_param.dataPLPId;

                sony_result = sony_integ_dvbc2_Tune(pInteg, &tune_param);
                switch(sony_result){
                case SONY_RESULT_OK:
                    mcDBG_MSG_L2(("CXD2854[%d] : DVB-C2 TS Locked.\n", tunerID));
                    fgLock = 1;
                    break;
                case SONY_RESULT_ERROR_UNLOCK:
                    mcDBG_MSG_L2(("CXD2854[%d] : DVB-C2 TS Unlocked.\n", tunerID));
                    fgLock = 0;
                    break;
                case SONY_RESULT_ERROR_TIMEOUT:
                    mcDBG_MSG_L2(("CXD2854[%d] : DVB-C2 Wait TS Lock but Timeout.\n", tunerID));
                    fgLock = 0;
                    break;
                default:
                    mcDBG_MSG_L1(("CXD2854[%d] : Error in sony_integ_dvbc2_Tune (%s)\n", tunerID, sony_FormatResult(sony_result)));
                    fgLock = 0;
                    break;
                }
            }
        ...
        if((pt_tuner_info->e_dvb_t_t2_mode == TUNER_DVB_T) || (pt_tuner_info->e_dvb_t_t2_mode == TUNER_DVB_AUTO_T_T2)){
            ...
            mcDBG_MSG_L2(("CXD2854[%d] : Freq = %d\n", tunerID, tune_param.centerFreqKHz));
            mcDBG_MSG_L2(("CXD2854[%d] : System = DVB-T\n", tunerID));
            mcDBG_MSG_L2(("CXD2854[%d] : Bandwidth = %d\n", tunerID, tune_param.bandwidth));
            mcDBG_MSG_L2(("CXD2854[%d] : Profile = %d\n", tunerID, tune_param.profile));
            if(fgChannelScan){
                mcDBG_MSG_L2(("CXD2854[%d] : Channel Scan\n", tunerID));
            }else{
                mcDBG_MSG_L2(("CXD2854[%d] : Normal Tune\n", tunerID));
            ...
            switch(sony_result){
            case SONY_RESULT_OK:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-T Locked.\n", tunerID));
                fgLock = 1;
                break;
            case SONY_RESULT_ERROR_UNLOCK:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-T Unlocked.\n", tunerID));
                fgLock = 0;
                break;
            case SONY_RESULT_ERROR_TIMEOUT:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-T Wait Lock but Timeout.\n", tunerID));
                fgLock = 0;
                break;
            default:
                mcDBG_MSG_L2(("CXD2854[%d] : Error in sony_integ_dvbt_Tune (%s)\n", tunerID, sony_FormatResult(sony_result)));
                fgLock = 0;
                break;
            }
        }
        ...
        if((fgLock == 0) && ((pt_tuner_info->e_dvb_t_t2_mode == TUNER_DVB_T2) || (pt_tuner_info->e_dvb_t_t2_mode == TUNER_DVB_AUTO_T_T2))){
            ...
            mcDBG_MSG_L2(("CXD2854[%d] : Freq = %d\n", tunerID, tune_param.centerFreqKHz));
            mcDBG_MSG_L2(("CXD2854[%d] : System = DVB-T2\n", tunerID));
            mcDBG_MSG_L2(("CXD2854[%d] : Bandwidth = %d\n", tunerID, tune_param.bandwidth));
            mcDBG_MSG_L2(("CXD2854[%d] : PLP ID = %d\n", tunerID, tune_param.dataPlpId));
            if(fgChannelScan){
                mcDBG_MSG_L2(("CXD2854[%d] : Channel Scan\n", tunerID));
            ...
            switch(sony_result){
            case SONY_RESULT_OK:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-T2 Locked.\n", tunerID));
                fgLock = 1;
                break;
            case SONY_RESULT_ERROR_UNLOCK:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-T2 Unlocked.\n", tunerID));
                fgLock = 0;
                break;
            case SONY_RESULT_ERROR_TIMEOUT:
                mcDBG_MSG_L2(("CXD2854[%d] : DVB-T2 Wait Lock but Timeout.\n", tunerID));
                fgLock = 0;
                break;
            default:
                mcDBG_MSG_L2(("CXD2854[%d] : Error in sony_integ_dvbt2_Tune (%s)\n", tunerID, sony_FormatResult(sony_result)));
                fgLock = 0;
                break;
            }
            ...
            if(fgLock == 1){
                if(tune_param.tuneInfo == SONY_DEMOD_DVBT2_TUNE_INFO_INVALID_PLP_ID){
                    mcDBG_MSG_L2(("CXD2854[%d] : Warning: PLP ID is invalid.\n", tunerID));
                    if(!pSonyDemodCtx->autoPLP){
                        fgLock = 0;
                    }
                }
            }
 ...
 */
//-------------------------------------------------------------------------
BOOL DVBT2_TunerGetSync(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    UINT16 u2Ret = 0;
    ...
    if((pDemod->system == SONY_DTV_SYSTEM_DVBT2) && (!pSonyDemodCtx->autoPLP)){
        // Check PLP ID error
        UINT8 plpError = 0;
 ...
 */
//-------------------------------------------------------------------------
static VOID  DVBT2_TunerDisc(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    sony_integ_t *pInteg = &pSonyDemodCtx->integ;
    ...
    if(pPdCtxDVBT2->fgDisStatus == TRUE)
    {
        ...
        mcDBG_MSG_L2(("CXD2854[%d] : DVBT2 Has Disconnect!\n", tunerID));
        return;
    }
    mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
...
static void DVBT2_TunerBypassI2C(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bSwitchOn)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    sony_demod_t *pDemod = &pSonyDemodCtx->demod;
    ...
    mcDBG_MSG_L2(("CXD2854[%d] : Testlog:Now is set I2C bypass: %d!!\n", tunerID, bSwitchOn));
 ...
 */
//-------------------------------------------------------------------------
static void DVBT2_TunerGetSignal(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    uint32_t symRate = 0;
    ...
    // Frequency -----------------------------------------------------
    _pSignal->Frequency = pSonyDemodCtx->frequency;
    ...
    if(pDemod->system == SONY_DTV_SYSTEM_DVBT){
        ...
        _pSignal->ui1_ds_id = pSonyDemodCtx->c2DsId;
        _pSignal->ui1_plp_id = pSonyDemodCtx->c2PlpId;
        #endif
    }
    else{
        _pSignal->QAMSize = (UINT32)MOD_UNKNOWN;
 ...
 */
//-------------------------------------------------------------------------
static UINT8 DVBT2_TunerGetSignalLevel(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    UINT8 ssi = 0;
 ...
 */
//-------------------------------------------------------------------------
static INT16 DVBT2_TunerGetSignalLeveldBm(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    INT16 i2Ret = 0;
    ...
    case SONY_DTV_SYSTEM_DVBT:
    default:
        sony_result = sony_integ_dvbt_monitor_RFLevel(pInteg, &rfLevel);
        break;
    case SONY_DTV_SYSTEM_DVBT2:
        sony_result = sony_integ_dvbt2_monitor_RFLevel(pInteg, &rfLevel);
        break;
    case SONY_DTV_SYSTEM_DVBC:
        sony_result = sony_integ_dvbc_monitor_RFLevel(pInteg, &rfLevel);
        break;
    case SONY_DTV_SYSTEM_DVBC2:
        sony_result = sony_integ_dvbc2_monitor_RFLevel(pInteg, &rfLevel);
        break;
    }
    if(sony_result == SONY_RESULT_OK){
        if(rfLevel >= 0){
...
static UINT32 sony_GetPreBER(UINT8 tunerID)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
...
static UINT32 sony_GetPER(UINT8 tunerID)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    sony_result_t sony_result = SONY_RESULT_OK;
    sony_demod_t *pDemod = &pSonyDemodCtx->demod;
...
static UINT32 sony_GetBER(UINT8 tunerID)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    uint32_t ber = 100000; // unit is 1e-5
    sony_result_t sony_result = SONY_RESULT_OK;
...
#if 0
static UINT32 DVBT2_TunerGetSignalBER(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    UINT32 ber = 0;
...
static UINT32 DVBT2_TunerGetSignalPER(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
...
static UINT16 DVBT2_TunerGetSignalSNR(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    UINT16 u2_ret = 0;
...
static void DVBT2_TunerSetMpgFmt(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, MPEG_FMT_T  *pt_mpeg_fmt)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
...
static char *DVBT2_TunerGetVer(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
...
static INT32 DVBT2_TunerNimTest(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    INT32 i4Ret = 0;
...
static void DVBT2_TunerSetRegSetting(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, UCHAR ucRegSet, UCHAR ucRegAddr, UCHAR ucRegValue)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
...
static void DVBT2_TunerShowRegSetting(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
...
static void DVBT2_TunerTestI2C(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
...
static void DVBT2_TunerCommand(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, INT32 i4Argc, const CHAR** aszArgv)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    //UCHAR   ucCmdSize = 0;
...
static void DVBT2_TunerDVBT2Command(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    sony_result_t sony_result = SONY_RESULT_OK;
            ...
            if(aszArgv[0][1] == 's'){
                ...
                case SONY_RESULT_OK:
                    ...
                    if(bandwidth == SONY_DTV_BW_6_MHZ){
                        pSonyDemodCtx->c2DsPlpList.e_bw = BW_6_MHz;
                    }else{
                        pSonyDemodCtx->c2DsPlpList.e_bw = BW_8_MHz;
                    }
                    pSonyDemodCtx->c2DsPlpList.ui2_curr_num = (UINT16)num_tune_param;
                    pSonyDemodCtx->c2NextFrequency = (UINT32)(next_frequency * 1000);
    ...
    case 's':
        ...
        {
            TS_FMT_T tsfmt;
            if(sony_GetTsFmt(tunerID, pDemod, &tsfmt)){
    ...
    case 't':
        {
            ...
            if((pTuner->i2cAddress == 0x00) || (pTuner->pI2c == NULL)){
                mcDBG_MSG_L1(("CXD2854[%d] : Tuner struct instance is not correctly configured!\n", tunerID));
                return;
            }
            switch(aszArgv[0][1])
            {
                ...
                default:
                {
                    ...
                    if(pDemod->system == SONY_DTV_SYSTEM_DVBT){
                        ...
                        sony_result = sony_demod_dvbt_monitor_SyncStat(pDemod, &syncStat, &tsLock, &unlock);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L4(("DVBT Sync State : %d\n", syncStat));
                            mcDBG_MSG_L1(("DVBT TS Lock    : %d\n", tsLock));
                            mcDBG_MSG_L4(("DVBT Unlock Det : %d\n", unlock));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbt_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else if(pDemod->system == SONY_DTV_SYSTEM_DVBT2){
                        ...
                        sony_result = sony_demod_dvbt2_monitor_SyncStat(pDemod, &syncStat, &tsLock, &unlock);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L4(("DVBT2 Sync State : %d\n", syncStat));
                            mcDBG_MSG_L1(("DVBT2 TS Lock    : %d\n", tsLock));
                            mcDBG_MSG_L4(("DVBT2 Unlock Det : %d\n", unlock));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbt2_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else if(pDemod->system == SONY_DTV_SYSTEM_DVBC){
                        ...
                        if(sony_result == SONY_RESULT_OK){
                            ...
                            mcDBG_MSG_L4(("DVBC Unlock Det : %d\n", unlock));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbc_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else if(pDemod->system == SONY_DTV_SYSTEM_DVBC2){
                        ...
                        sony_result = sony_demod_dvbc2_monitor_SyncStat(pDemod, &syncStat, &tsLock, &unlock);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L4(("DVBC2 Sync State : %d\n", syncStat));
                            mcDBG_MSG_L1(("DVBC2 TS Lock    : %d\n", tsLock));
                            mcDBG_MSG_L4(("DVBC2 Unlock Det : %d\n", unlock));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbc2_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else{
            ...
            switch(aszArgv[0][1])
            {
                ...
                default:
                {
                    ...
                    if(pDemod->system == SONY_DTV_SYSTEM_DVBT){
                        ...
                        sony_result = sony_demod_dvbt_monitor_PreViterbiBER(pDemod, &ber);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L4(("DVBT PreViterbi BER (1e7) : %d\n", ber));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbt_monitor_PreViterbiBER (%s)\n", sony_FormatResult(sony_result)));
                        }
                        sony_result = sony_demod_dvbt_monitor_PreRSBER(pDemod, &ber);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L1(("DVBT PreRS BER (1e7) : %d\n", ber));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbt_monitor_PreRSBER (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else if(pDemod->system == SONY_DTV_SYSTEM_DVBT2){
                        ...
                        sony_result = sony_demod_dvbt2_monitor_PreLDPCBER(pDemod, &ber);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L4(("DVBT2 PreLDPC BER (1e7) : %d\n", ber));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbt2_monitor_PreLDPCBER (%s)\n", sony_FormatResult(sony_result)));
                        }
                        sony_result = sony_demod_dvbt2_monitor_PreBCHBER(pDemod, &ber);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L1(("DVBT2 PreBCH BER (1e9) : %d\n", ber));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbt2_monitor_PreBCHBER (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else if(pDemod->system == SONY_DTV_SYSTEM_DVBC){
                        ...
                        sony_result = sony_demod_dvbc2_monitor_PreLDPCBER(pDemod, &ber);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L4(("DVBC2 PreLDPC BER (1e7) : %d\n", ber));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbc2_monitor_PreLDPCBER (%s)\n", sony_FormatResult(sony_result)));
                        }
                        sony_result = sony_demod_dvbc2_monitor_PreBCHBER(pDemod, &ber);
                        if(sony_result == SONY_RESULT_OK){
                            mcDBG_MSG_L1(("DVBC2 PreBCH BER (1e9) : %d\n", ber));
                        }else{
                            mcDBG_MSG_L1(("Error in sony_demod_dvbc2_monitor_PreBCHBER (%s)\n", sony_FormatResult(sony_result)));
                        }
                    }else{
    ...
    case 'u':
        {
            UINT32 Uec = pSonyDemodCtx->u4Uec;
            switch(aszArgv[0][1])
            {
                case 'e':
                default:
    ...
    case 'p':
        {
            ...
            mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
            sony_demod_I2cRepeaterEnable(pDemod, 0x01); // I2C repeater enable
            if(ITuner_OP(ITunerGetCtx_Multi(tunerID), itSetTrapThresold, isAnalog, &frequencyKHz) == ITUNER_OK){
                if(isAnalog){
...
static void DVBT2_TunerDetachI2C(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bDetach)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
 ...
 */
//-------------------------------------------------------------------------
static INT16 DVBT2_TunerGetAttribute(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    ...
    case TUNER_GET_TYPE_BER:
        ...
        {
            UINT32 ber = sony_GetBER(tunerID);
            i4Temp = (INT32)ber;
        }
 ...
 */
//-------------------------------------------------------------------------
static INT16 DVBT2_TunerGetAttribute2(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen, DRV_CONN_TYPE_T e_conn_type)
{
    ...
    pTunerCtx = ITunerGetCtx_Multi(tunerID);
     ...
     if ((e_get_type==TUNER_GET_TYPE_SQI)&&(pSonyDemodCtx->fgChanScanforGetAtt == TRUE))
    {
    ...
    case TUNER_GET_TYPE_PRE_VBER:
        ...
        {
            UINT32 u4Preber = sony_GetPreBER(tunerID);
    ...
    case TUNER_GET_TYPE_UEC:
        {
            /*
            UINT16 UEC = (UINT16)sony_GetUEC(tunerID);
            ...
            *pzAttributeLen = sizeof(UINT32);
            x_memcpy(pzAttribute, &pSonyDemodCtx->u4Uec, *pzAttributeLen);
        }
        break;
    ...
    case TUNER_GET_TYPE_KHZ_CFO:
        {
            ...
            if(sony_result == SONY_RESULT_OK){
                ...
                mcDBG_MSG_L2(("Error in sony_demod_dvbx_monitor_CarrierOffset (%s)\n", sony_FormatResult(sony_result)));
            }
            *pzAttributeLen = sizeof(INT32);
    ...
    case TUNER_GET_TYPE_TS_LOCK_STATUS:
        {
            ...
            if(pDemod->system == SONY_DTV_SYSTEM_DVBT){
                sony_result = sony_demod_dvbt_monitor_SyncStat(pDemod, &syncStat, &tsLock, &unlock);
                if(sony_result == SONY_RESULT_OK){
                    mcDBG_MSG_L2(("Sync State : %d\n", syncStat));
                    mcDBG_MSG_L2(("TS Lock    : %d\n", tsLock));
                    mcDBG_MSG_L2(("Unlock Det : %d\n", unlock));
                }else{
                    mcDBG_MSG_L2(("Error in sony_demod_dvbt_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                }
            }else if(pDemod->system == SONY_DTV_SYSTEM_DVBT2){
                sony_result = sony_demod_dvbt2_monitor_SyncStat(pDemod, &syncStat, &tsLock, &unlock);
                if(sony_result == SONY_RESULT_OK){
                    ...
                    mcDBG_MSG_L2(("Error in sony_demod_dvbt2_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                }
            }else if(pDemod->system == SONY_DTV_SYSTEM_DVBC){
                sony_result = sony_demod_dvbc_monitor_SyncStat(pDemod, &arLock, &tsLock, &unlock);
                if(sony_result == SONY_RESULT_OK){
                    ...
                    mcDBG_MSG_L2(("Error in sony_demod_dvbc_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                }
            }else if(pDemod->system == SONY_DTV_SYSTEM_DVBC2){
                sony_result = sony_demod_dvbc2_monitor_SyncStat(pDemod, &syncStat, &tsLock, &unlock);
                if(sony_result == SONY_RESULT_OK){
                    ...
                    mcDBG_MSG_L2(("Error in sony_demod_dvbc2_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
                }
            }else{
    ...
    case TUNER_GET_TYPE_TUNER_STATUS:
        {
            BOOL pllLock = FALSE;
            UINT8 temp = 0;
            ITuner_OP(ITunerGetCtx_Multi(tunerID), itGetStatus, 0, &pllLock);
    ...
    case TUNER_GET_TYPE_DVBC2_DS_PLP_LIST:
        ...
        *pzAttributeLen = sizeof(TUNER_DVBC2_DS_PLP_LIST);
        x_memcpy(pzAttribute, &pSonyDemodCtx->c2DsPlpList, *pzAttributeLen);
        break;
    case TUNER_GET_TYPE_DVBC2_NEXT_FREQ:
        mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_DVBC2_NEXT_FREQ\r\n"));
        {
            UINT32 ui4Freq = pSonyDemodCtx->c2NextFrequency;
    ...
    case TUNER_GET_TYPE_DIG_DEMOD_STATUS:
        mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_DIG_DEMOD_STATUS\r\n"));
        {
            ...
            *pzAttributeLen = sizeof(TUNER_TER_DIG_TUNE_STATUS_T);
            DVBT2_TunerGetModuInfor(tunerID, &tuneStatus);
    ...
    case TUNER_GET_TYPE_PER_UPPER:
        {
            UINT16 perUpper;
            UINT32 per=sony_GetPER(tunerID);
 ...
 */
//-------------------------------------------------------------------------
static INT32 DVBT2_TunerSetAttribute(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_SET_TYPE_T  e_set_type,
    ...
    case TUNER_SET_TYPE_DEMOD_SHUTDOWN:
        {
            ...
            // Go shutdown state
            if(sony_demod_ForceShutdown(pDemod) != SONY_RESULT_OK){
                mcDBG_MSG_L1(("CXD2854[%d] : sony_demod_ForceShutdown failed!\n", tunerID));
                break;
            }

            // REFOUT disable
            ITuner_OP(ITunerGetCtx_Multi(tunerID), itSetRefOut, 0, NULL);
...
static INT32 DVBT2_TunerSetDiagCmd(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_SET_TYPE_T e_tuner_set_type, VOID* pv_set_info, SIZE_T z_size)
{
    INT32 retSts = RMR_OK;
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
    if(pv_set_info == NULL){
        mcDBG_MSG_L1(("[DVBT2_TunerSetDiagCmd] pv_set_info is NULL & set type can not support...\n"));
        return DRVAPI_TUNER_OK;
    }
    mcSEMA_LOCK_MTFE(pPdCtxDVBT2->hHalLock);
    switch(e_tuner_set_type)
    {
        ...
        {   
            ...
            ITUNER_CTX_T *pTCtx;
            pTCtx = ITunerGetCtx_Multi(tunerID);
...
static INT32 DVBT2_TunerGetDiagInfo(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_GET_TYPE_T e_tuner_get_type, VOID* pv_get_info, SIZE_T* pz_size)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T *pPdCtxDVBT2 = (DVBT2_PD_CTX_T*)pSonyDemodCtx->pPdCtxDvb;
...
//-----------------------------------------------------------------------------
// Tuner #0
static INT32 DVBT2_TunerClose_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return DVBT2_TunerClose(0, ptTDSpecificCtx);
}

static BOOL DVBT2_TunerAcq_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, void* pv_conn_info,
                            SIZE_T z_conn_info_len, x_break_fct _BreakFct, void* pvArg)
{
    return DVBT2_TunerAcq(0, ptTDSpecificCtx, e_conn_type, pv_conn_info, z_conn_info_len, _BreakFct, pvArg);
}

static BOOL DVBT2_TunerGetSync_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSync(0, ptTDSpecificCtx, e_conn_type);
}

static void DVBT2_TunerGetSignal_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    DVBT2_TunerGetSignal(0, ptTDSpecificCtx, e_conn_type, _pSignal);
}

static UINT8 DVBT2_TunerGetSignalLevel_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalLevel(0, ptTDSpecificCtx, e_conn_type);
}

static INT16 DVBT2_TunerGetSignalLeveldBm_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalLeveldBm(0, ptTDSpecificCtx, e_conn_type);
}

static UINT32 DVBT2_TunerGetSignalPER_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalPER(0, ptTDSpecificCtx,  e_conn_type);
}

static UINT16 DVBT2_TunerGetSignalSNR_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalSNR(0, ptTDSpecificCtx, e_conn_type);
}

static void DVBT2_TunerSetMpgFmt_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, MPEG_FMT_T *pt_mpeg_fmt)
{
    DVBT2_TunerSetMpgFmt(0, ptTDSpecificCtx, pt_mpeg_fmt);
}

static char *DVBT2_TunerGetVer_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return DVBT2_TunerGetVer(0, ptTDSpecificCtx);
}

static INT32 DVBT2_TunerNimTest_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return DVBT2_TunerNimTest(0, ptTDSpecificCtx);
}

static void DVBT2_TunerSetRegSetting_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, UCHAR ucRegSet, UCHAR ucRegAddr, UCHAR ucRegValue)
{
    DVBT2_TunerSetRegSetting(0, ptTDSpecificCtx, ucRegSet, ucRegAddr, ucRegValue);
}

static void DVBT2_TunerShowRegSetting_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    DVBT2_TunerShowRegSetting(0, ptTDSpecificCtx);
}

static void DVBT2_TunerTestI2C_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    DVBT2_TunerTestI2C(0, ptTDSpecificCtx);
}

static void DVBT2_TunerCommand_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, INT32 i4Argc, const CHAR** aszArgv)
{
    DVBT2_TunerCommand(0, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void DVBT2_TunerDVBT2Command_0(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    DVBT2_TunerDVBT2Command(0, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void DVBT2_TunerDetachI2C_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bDetach)
{
    DVBT2_TunerDetachI2C(0, ptTDSpecificCtx, bDetach);
}

static VOID  DVBT2_TunerDisc_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    DVBT2_TunerDisc(0, ptTDSpecificCtx, e_conn_type);
}

static INT32 DVBT2_TunerSetAttribute_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_SET_TYPE_T  e_set_type,
                                        const VOID *pvAttribute, SIZE_T zAttributeLen)
{
    return DVBT2_TunerSetAttribute(0, ptTDSpecificCtx, e_set_type, pvAttribute, zAttributeLen);
}

static INT16 DVBT2_TunerGetAttribute_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    return DVBT2_TunerGetAttribute(0, ptTDSpecificCtx, e_get_type, pzAttribute, pzAttributeLen);
}

static void DVBT2_TunerBypassI2C_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bSwitchOn)
{
    DVBT2_TunerBypassI2C(0, ptTDSpecificCtx, bSwitchOn);
}

static INT16 DVBT2_TunerGetAttribute2_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetAttribute2(0, ptTDSpecificCtx, e_get_type, pzAttribute, pzAttributeLen, e_conn_type);
}

#ifdef fcADD_DIAG_INFO
static INT32 DVBT2_TunerSetDiagCmd_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_SET_TYPE_T e_tuner_set_type, VOID* pv_set_info, SIZE_T z_size)
{
    return DVBT2_TunerSetDiagCmd(0, ptTDSpecificCtx, e_tuner_set_type, pv_set_info, z_size);
}

static INT32 DVBT2_TunerGetDiagInfo_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_GET_TYPE_T e_tuner_get_type, VOID* pv_get_info, SIZE_T* pz_size)
{
    return DVBT2_TunerGetDiagInfo(0, ptTDSpecificCtx, e_tuner_get_type, pv_get_info, pz_size);
}
#endif

// Tuner #1
static INT32 DVBT2_TunerClose_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return DVBT2_TunerClose(1, ptTDSpecificCtx);
}

static BOOL DVBT2_TunerAcq_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, void* pv_conn_info,
                            SIZE_T z_conn_info_len, x_break_fct _BreakFct, void* pvArg)
{
    return DVBT2_TunerAcq(1, ptTDSpecificCtx, e_conn_type, pv_conn_info, z_conn_info_len, _BreakFct, pvArg);
}

static BOOL DVBT2_TunerGetSync_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSync(1, ptTDSpecificCtx, e_conn_type);
}

static void DVBT2_TunerGetSignal_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    DVBT2_TunerGetSignal(1, ptTDSpecificCtx, e_conn_type, _pSignal);
}

static UINT8 DVBT2_TunerGetSignalLevel_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalLevel(1, ptTDSpecificCtx, e_conn_type);
}

static INT16 DVBT2_TunerGetSignalLeveldBm_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalLeveldBm(1, ptTDSpecificCtx, e_conn_type);
}

static UINT32 DVBT2_TunerGetSignalPER_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalPER(1, ptTDSpecificCtx,  e_conn_type);
}

static UINT16 DVBT2_TunerGetSignalSNR_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetSignalSNR(1, ptTDSpecificCtx, e_conn_type);
}

static void DVBT2_TunerSetMpgFmt_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, MPEG_FMT_T *pt_mpeg_fmt)
{
    DVBT2_TunerSetMpgFmt(1, ptTDSpecificCtx, pt_mpeg_fmt);
}

static char *DVBT2_TunerGetVer_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return DVBT2_TunerGetVer(1, ptTDSpecificCtx);
}

static INT32 DVBT2_TunerNimTest_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return DVBT2_TunerNimTest(1, ptTDSpecificCtx);
}

static void DVBT2_TunerSetRegSetting_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, UCHAR ucRegSet, UCHAR ucRegAddr, UCHAR ucRegValue)
{
    DVBT2_TunerSetRegSetting(1, ptTDSpecificCtx, ucRegSet, ucRegAddr, ucRegValue);
}

static void DVBT2_TunerShowRegSetting_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    DVBT2_TunerShowRegSetting(1, ptTDSpecificCtx);
}

static void DVBT2_TunerTestI2C_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    DVBT2_TunerTestI2C(1, ptTDSpecificCtx);
}

static void DVBT2_TunerCommand_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, INT32 i4Argc, const CHAR** aszArgv)
{
    DVBT2_TunerCommand(1, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void DVBT2_TunerDVBT2Command_1(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    DVBT2_TunerDVBT2Command(1, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void DVBT2_TunerDetachI2C_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bDetach)
{
    DVBT2_TunerDetachI2C(1, ptTDSpecificCtx, bDetach);
}

static VOID  DVBT2_TunerDisc_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    DVBT2_TunerDisc(1, ptTDSpecificCtx, e_conn_type);
}

static INT32 DVBT2_TunerSetAttribute_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_SET_TYPE_T  e_set_type,
                                        const VOID *pvAttribute, SIZE_T zAttributeLen)
{
    return DVBT2_TunerSetAttribute(1, ptTDSpecificCtx, e_set_type, pvAttribute, zAttributeLen);
}

static INT16 DVBT2_TunerGetAttribute_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    return DVBT2_TunerGetAttribute(1, ptTDSpecificCtx, e_get_type, pzAttribute, pzAttributeLen);
}

static void DVBT2_TunerBypassI2C_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bSwitchOn)
{
    DVBT2_TunerBypassI2C(1, ptTDSpecificCtx, bSwitchOn);
}

static INT16 DVBT2_TunerGetAttribute2_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen, DRV_CONN_TYPE_T e_conn_type)
{
    return DVBT2_TunerGetAttribute2(1, ptTDSpecificCtx, e_get_type, pzAttribute, pzAttributeLen, e_conn_type);
}

#ifdef fcADD_DIAG_INFO
static INT32 DVBT2_TunerSetDiagCmd_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_SET_TYPE_T e_tuner_set_type, VOID* pv_set_info, SIZE_T z_size)
{
    return DVBT2_TunerSetDiagCmd(1, ptTDSpecificCtx, e_tuner_set_type, pv_set_info, z_size);
}

static INT32 DVBT2_TunerGetDiagInfo_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_GET_TYPE_T e_tuner_get_type, VOID* pv_get_info, SIZE_T* pz_size)
{
    return DVBT2_TunerGetDiagInfo(1, ptTDSpecificCtx, e_tuner_get_type, pv_get_info, pz_size);
}
#endif
...
static void PD_DVBT2_Register_LTDIS_Fct(UINT8 tunerID, PTD_CTX ptTDCtx)
{
    TUNER_DEVICE_CTX_T*     pTunerDeviceCtx = (TUNER_DEVICE_CTX_T*)ptTDCtx;

    if(tunerID == 1){
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerClose   = DVBT2_TunerClose_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerAcq     = DVBT2_TunerAcq_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGerSync     = DVBT2_TunerGetSync_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignal   = DVBT2_TunerGetSignal_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLevel = DVBT2_TunerGetSignalLevel_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLeveldBm = DVBT2_TunerGetSignalLeveldBm_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalPER = DVBT2_TunerGetSignalPER_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalSNR = DVBT2_TunerGetSignalSNR_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetMpgFmt   = DVBT2_TunerSetMpgFmt_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetVer  = DVBT2_TunerGetVer_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerNimTest     = DVBT2_TunerNimTest_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetRegSetting = DVBT2_TunerSetRegSetting_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerShowRegSetting = DVBT2_TunerShowRegSetting_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerTestI2C     = DVBT2_TunerTestI2C_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerCommand     = DVBT2_TunerCommand_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDtdCommand  = DVBT2_TunerDVBT2Command_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDetachI2C   = DVBT2_TunerDetachI2C_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDisc    = DVBT2_TunerDisc_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetAttribute= DVBT2_TunerSetAttribute_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetAttribute= DVBT2_TunerGetAttribute_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerBypassI2C = DVBT2_TunerBypassI2C_1;
#if defined(PD_REMAP_TER_DIG) || defined(PD_REMAP_CAB_DIG) || defined(PD_REMAP_SAT_DIG) || defined(PD_NO_USE_REMAP)
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetAttribute2 =  DVBT2_TunerGetAttribute2_1;
#endif
        #ifdef fcADD_DIAG_INFO
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetDiagCmd = DVBT2_TunerSetDiagCmd_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetDiagInfo = DVBT2_TunerGetDiagInfo_1;
        #endif
    }else{
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerClose   = DVBT2_TunerClose_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerAcq     = DVBT2_TunerAcq_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGerSync     = DVBT2_TunerGetSync_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignal   = DVBT2_TunerGetSignal_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLevel = DVBT2_TunerGetSignalLevel_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLeveldBm = DVBT2_TunerGetSignalLeveldBm_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalPER = DVBT2_TunerGetSignalPER_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalSNR = DVBT2_TunerGetSignalSNR_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetMpgFmt   = DVBT2_TunerSetMpgFmt_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetVer  = DVBT2_TunerGetVer_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerNimTest     = DVBT2_TunerNimTest_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetRegSetting = DVBT2_TunerSetRegSetting_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerShowRegSetting = DVBT2_TunerShowRegSetting_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerTestI2C     = DVBT2_TunerTestI2C_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerCommand     = DVBT2_TunerCommand_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDtdCommand  = DVBT2_TunerDVBT2Command_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDetachI2C   = DVBT2_TunerDetachI2C_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDisc    = DVBT2_TunerDisc_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetAttribute= DVBT2_TunerSetAttribute_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetAttribute= DVBT2_TunerGetAttribute_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerBypassI2C = DVBT2_TunerBypassI2C_0;
#if defined(PD_REMAP_TER_DIG) || defined(PD_REMAP_CAB_DIG) || defined(PD_REMAP_SAT_DIG) || defined(PD_NO_USE_REMAP)
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetAttribute2 =  DVBT2_TunerGetAttribute2_0;
#endif
        #ifdef fcADD_DIAG_INFO
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetDiagCmd = DVBT2_TunerSetDiagCmd_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetDiagInfo = DVBT2_TunerGetDiagInfo_0;
        #endif
    }
}
 ...
 */
//-------------------------------------------------------------------------
static INT32 DVBT2_TunerOpen_Common(UINT8 tunerID, PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    DVBT2_PD_CTX_T * pPdCtxDVBT2 = NULL;
    TUNER_DEVICE_CTX_T * pDvbt2TunerDeviceCtx = NULL;
    INT32 retCode = 0;
    
    mcDBG_MSG_L2(("CXD2854[%d] : %s \n", tunerID, __FUNCTION__));
    
    pDvbt2TunerDeviceCtx = (TUNER_DEVICE_CTX_T*)ptTDCtx;

    //register LTDIS APIs to HAL
    PD_DVBT2_Register_LTDIS_Fct(tunerID, ptTDCtx);
    //Regisiter tuner driver
    ITunerRegistration(pDvbt2TunerDeviceCtx->tunerType);

    //Check Demod inited
    if (pSonyDemodCtx->fgDemodInited){
        mcDBG_MSG_L1(("CXD2854[%d] : CXD2854 had been initialized!\n", tunerID));
        return (DRVAPI_TUNER_OK);
    }
    pSonyDemodCtx->fgDemodInited = TRUE;
    ...
    // Store to sony_demod_ctx_t member.
    pSonyDemodCtx->pPdCtxDvb = pPdCtxDVBT2;
    pSonyDemodCtx->pTunerDeviceCtxDvb = pDvbt2TunerDeviceCtx;

    // demod instance initialization (to do only once)
    // NOTE: If Initialization is done from ISDB-T side, this is unnecessary.

    if(0 == tunerID) //MTK: HW reset on main initialize
    {
        pSonyDemodCtx->fgHWReset = FALSE;
    }
    else
    {
        pSonyDemodCtx->fgHWReset = TRUE;
    }
    vResetDemod(tunerID, 20);
    retCode = sony_mtk_glue_common_Initialize(tunerID);
    if (retCode != DRVAPI_TUNER_OK) {
        return retCode;
    }
    
    if(pDvbt2TunerDeviceCtx->u1MWIntf != cMTAL_IF && pDvbt2TunerDeviceCtx->u1MWIntf != cSDAL_IF){
        if (x_sema_create(&pPdCtxDVBT2->t_escape_mon_suspend, X_SEMA_TYPE_BINARY,X_SEMA_STATE_LOCK) != OSR_OK){
            mcDBG_MSG_ERR(("EN_DVBT2_TunerOpen (mcSEMA_CREATE): ERROR!\n"));
            return (DRVAPI_TUNER_ERROR);
        }
...
{
    return DVBT2_TunerOpen_Common(0, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
}

// Tuner #0
INT32 DVBT2_C1_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    return DVBT2_TunerOpen_Common(0, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
}

// Tuner #1
INT32 DVBT2_C2_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    return DVBT2_TunerOpen_Common(1, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
}


#ifdef CC_PROJECT_FOR_SONY
// NOTE: (by Sony) This API should be modified for dual tuner, but not modified because Sony member are not sure about this API.
void sony_cxd2854_Semalock(UINT8 u1TunerID, BOOL lock)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[u1TunerID];
    DVBT2_PD_CTX_T * pPdCtxDVBT2 = NULL;
    pPdCtxDVBT2 = (DVBT2_PD_CTX_T *)pSonyDemodCtx->pPdCtxDvb;
    if(pSonyDemodCtx->fgDemodInited == FALSE)
    {
        return;
    }
