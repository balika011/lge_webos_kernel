...
#elif defined(SONYSAT_LNBC_INTERSIL_ISL9492)
// For INTERSIL_ISL9492 LNBC
...
#define VENDOR_DRIVER_VER            "2014-01-16\nCXD2839_DualVer0.01"
...
//MTK_Dual_Tuner: Add cell information to all log
#undef mcDBG_MSG_L1
#undef mcDBG_MSG_L2
#undef mcDBG_MSG_L4
#undef PREFIXMSG
#define PREFIXMSG ""
#define mcDBG_MSG_L1(_x_) do{\
    DBG_PRTINF(DBG_PRINTF_L1,(SONYSAT_DEMOD_CELL_STR));\
    DBG_PRTINF(DBG_PRINTF_L1,_x_);\
    }while(0)
#define mcDBG_MSG_L2(_x_) do{\
        DBG_PRTINF(DBG_PRINTF_L2,(SONYSAT_DEMOD_CELL_STR));\
        DBG_PRTINF(DBG_PRINTF_L2,_x_);\
    }while(0)
#define mcDBG_MSG_L4(_x_) do{\
        DBG_PRTINF(DBG_PRINTF_L4,(SONYSAT_DEMOD_CELL_STR));\
        DBG_PRTINF(DBG_PRINTF_L4,_x_);\
    }while(0)
...
//-----------------------------------------------------------------------------
typedef struct _sonysat_demod_ctx_t // Sony CXD2839 Driver Specific
{
    // Instances to control CXD2839 driver
    sonysat_demod_t demod;
    sonysat_integ_t integ;
    sonysat_tuner_sat_t tuner;
    sonysat_lnbc_t lnbc;
    sonysat_i2c_t i2c;
#ifdef SONYSAT_DEMOD_ENABLE_GATEWAY
    sonysat_i2c_t i2cGw;
#endif
...
static VOID  DVBS_TunerConnect(void);
...
static void DVBS_OduPowerOff(DVBS_PD_CTX_T * pdCtx)
{
    ...
    if(sonysat_result == SONYSAT_RESULT_OK){
        if(tslock == 1){
            for(retrycnt = 0; retrycnt < 7; retrycnt++){
#ifdef SONYSAT_LNBC_MPS_MP8126
                sonysat_result = pInteg->pLnbc->SetVoltage (pInteg->pLnbc, SONYSAT_LNBC_VOLTAGE_AUTO);
                if(sonysat_result != SONYSAT_RESULT_OK){
                    mcDBG_MSG_L2(("[DVBS_TunerAcq] Error at pLnbc->SetVoltage(AUTO)\n"));
...
static void Demod_GPIO_Reset(void)
{
    ...
    if(DRVCUST_OptQuery(eDvbsDemodResetGpio,&u4ResetGpionum)==0)
    {
        ...
        {
            mcDBG_MSG_L2(("Do not Reset DVBS demod CXD2839\n"));
        }
    }
    else
    {
...
#ifndef CC_SONYDEMOD_SQI_FOR_SONY
// For SQI (NOTE: It's SONY original and tentative!!)
                                            ...
                                            uint8_t * pQuality)
{
    ...
    //uint8_t quality = 0;
    //uint32_t s2ber = 0;
    SONYSAT_TRACE_ENTER ("s_s2_monitor_Quality2");
...
#ifdef CC_SONYDEMOD_DVBS_FREQSKIP
static sonysat_result_t DVBS_TunerSetFreqSkip (BOOL bFreqSkip)
{
    sonysat_demod_t *pDemod = &g_sonysat_demod_ctx.demod;

    SONYSAT_TRACE_ENTER("DVBS_TunerSetFreqSkip");
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x80, ((bFreqSkip == TRUE) ? 0x01 : 0x00)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x4B, ((bFreqSkip == TRUE) ? 0x73 : 0x93)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x4C, ((bFreqSkip == TRUE) ? 0x73 : 0xC3)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xB4, ((bFreqSkip == TRUE) ? 0x01 : 0x00)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x64, ((bFreqSkip == TRUE) ? 0x73 : 0xA3)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x6C, ((bFreqSkip == TRUE) ? 0x7C : 0x7E)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x73, ((bFreqSkip == TRUE) ? 0x7C : 0xCE)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x7A, ((bFreqSkip == TRUE) ? 0x7C : 0xCE)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x83, ((bFreqSkip == TRUE) ? 0x7C : 0x9E)) != SONYSAT_RESULT_OK){
        SONYSAT_TRACE_RETURN (SONYSAT_RESULT_ERROR_I2C);
    }
    ...
    if(pCmd->ui4_control_mask & DISEQC_CMD_CTRL_UNICABLE_ODU_CH_CHANGE_MASK){
        ...
        if(pCmd->ui4_control_mask & DISEQC_CMD_CTRL_LNB_POWER_MASK){
            ...
            case 0: // Power Off
                ...
                break;
            case 1: // Normal
#if defined(SONYSAT_LNBC_INTERSIL_ISL9492)
                lnbcConfigVoltageLow = INTERSIL_ISL9492_CONFIG_VOLTAGE_LOW_13_5V;
                lnbcConfigVoltageHigh = INTERSIL_ISL9492_CONFIG_VOLTAGE_HIGH_18_5V;
#elif defined(SONYSAT_LNBC_MPS_MP8126)
            ...
            case 2: // Plus
#if defined(SONYSAT_LNBC_INTERSIL_ISL9492)
                lnbcConfigVoltageLow = INTERSIL_ISL9492_CONFIG_VOLTAGE_LOW_14_2V;
                lnbcConfigVoltageHigh = INTERSIL_ISL9492_CONFIG_VOLTAGE_HIGH_20_0V;
#elif defined(SONYSAT_LNBC_MPS_MP8126)
        ...
        /*
          CXD2839 has DiSEqC hardware and it automatically do ALL DiSEqC sequence.
                                     ...
                                     sonysat_integ_dvbs_s2_blindscan_result_t * pResult)
{
    ...
    if(pResult->eventId == SONYSAT_INTEG_DVBS_S2_BLINDSCAN_EVENT_DETECT){
        ...
        DVBS_AutoScanCallback(1,
            pInteg->pLnbc->voltage == SONYSAT_LNBC_VOLTAGE_HIGH ? POL_LIN_HORIZONTAL : POL_LIN_VERTICAL,
            pResult->tuneParam.centerFreqKHz,
            pResult->tuneParam.symbolRateKSps,
            FEC_I_UNKNOWN);
#endif
...
#endif
{
    ...
    //MTK_Dual_Tuner: Set stopwatch_elapsed function pointers
    pSeq->subseqBT.pfStopwatch_elapsed = pInteg->pfStopwatch_elapsed;
    pSeq->subseqPM.pfStopwatch_elapsed = pInteg->pfStopwatch_elapsed;
    pSeq->subseqSS.pfStopwatch_elapsed = pInteg->pfStopwatch_elapsed;
...
static sonysat_result_t sonysat_stopwatch_elapsed (sonysat_stopwatch_t * pStopwatch, uint32_t* pElapsed)
{
...
#else
    mcDBG_MSG_L1(("[DVBS_TunerConnect] Enable TS Output!\n"));
#endif
...
static INT32 DVBS_TunerClose(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    ...
    if(hMainSub != NULL_HANDLE){
        x_sema_delete(hMainSub);
        hMainSub = NULL_HANDLE;
    }
    ...
    case CONN_MODE_UNICABLE_SCAN:
        {
            ...
            bs_start_freq = pt_tuner_info->ui4_start_freq; // in MHz
            bs_stop_freq = pt_tuner_info->ui4_end_freq;    // in MHz
    ...
    case CONN_MODE_UNICABLE_SURF://unicable
        {
            ...
            if(tuneParam.symbolRateKSps == 0){
                ...
                tune_srs_param.freqRange = SONYSAT_DEMOD_DVBS_S2_TUNE_SRS_FREQ_RANGE_10MHz; // 10MHz range.
        ...
        if(system == SONYSAT_DTV_SYSTEM_DVBS){
            ...
            if(sonysat_result == SONYSAT_RESULT_OK){
                ...
                case SONYSAT_DVBS2_MODULATION_8PSK:
                    _pSignal->QAMSize = (UINT32)MOD_PSK_8; break;
                case SONYSAT_DVBS2_MODULATION_16APSK:
                    _pSignal->QAMSize = (UINT32)MOD_QAM_16; break; // MOD_APSK_16 is not defined.
                case SONYSAT_DVBS2_MODULATION_32APSK:
                    _pSignal->QAMSize = (UINT32)MOD_QAM_32; break; // MOD_APSK_32 is not defined.
                case SONYSAT_DVBS2_MODULATION_DUMMY_PLFRAME:
                    _pSignal->QAMSize = (UINT32)MOD_UNKNOWN; break; // Unknown
...
static UINT8 DVBS_TunerGetSignalLevel(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    ...
    if(sonysat_result == SONYSAT_RESULT_OK){
        // IFAGC range 0 - 8191(0x1FFF)
        u1Ret = (UINT8)(ifagcOut * 100 / 8191);
    }else{
        u1Ret = 0;
    }
    ...
    case 's':
        ...
        /* Quality */
        {
            ...
            if (sonysat_result == SONYSAT_RESULT_OK) {
                ...
                mcDBG_MSG_L1(("Error in s_s2_monitor_Quality2 (%s)\n", sonysat_FormatResult(sonysat_result)));
            }
        }
    ...
    case 'a':
        {
            ...
            const UINT8 tunerI2cAddress = SONYSAT_DEMOD_IIC_ADDRESS_TUNER; /* Please fix it if tuner address is changed. */
            sonysat_i2c_t i2cTuner;

            /*sonysat_i2c_MTK_CreateI2c(&i2cTuner);*/
            /* If tuner is accessed by I2C Gateway */
#ifndef SONYSAT_DEMOD_ENABLE_GATEWAY
            sonysat_i2c_MTK_CreateI2c(&i2cTuner);
#else
                                ...
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    ...
    case TUNER_GET_TYPE_LNB_SHORT_STATUS:
        {
          sonysat_lnbc_t *pLnbc = &g_sonysat_demod_ctx.lnbc;
          UINT8 u1OverLoad=0;
          LNB_SHORT_INFO * plnb_short_info=(LNB_SHORT_INFO *)pzAttribute;

          if(NULL!=pLnbc->MonitorOverload)
          {
           *pzAttributeLen = sizeof(LNB_SHORT_INFO);

           if(SONYSAT_RESULT_OK!=pLnbc->MonitorOverload(pLnbc,&u1OverLoad))
           {
            u1OverLoad=plnb_short_info->val;//previous value
           }
           if(plnb_short_info->val!=u1OverLoad)
           {
              plnb_short_info->isNotify=TRUE;
           }
           else
           {
              plnb_short_info->isNotify=FALSE;
           }
           plnb_short_info->val=u1OverLoad;
          }
        }
        break;
    ...
    case TUNER_GET_TYPE_SAT_DIG:
        {   /*maybe return Last_program_channel*/
            ...
            mcDBG_MSG_L1(("[DVBS_TunerGetAttribute]  ui4_freq:%d KHz SymbolRate=%dKS/s !\n",pAttri0->ui4_freq,pAttri0->ui4_sym_rate));
        }
        break;
    ...
    case TUNER_SET_TYPE_TS_TRISTATE:
        {
            ...
            // NOTE: This register is automatically controlled by the CXD2839 driver.
...
#if 1
static BOOL DVBS_TunerNotifyEnable(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
...
// public functions
//-----------------------------------------------------------------------------
static void PD_DVBS_Register_LTDIS_Fct(PTD_CTX ptTDCtx)
{
...
INT32 DVBS_C1_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    ...
    sonysat_demod_xtal_t demod_xtal;
    ...
    /* for dummy debug */
    demodSlave = SONYSAT_DEMOD_IIC_ADDRESS_2842;
    tunerSlave = SONYSAT_DEMOD_IIC_ADDRESS_TUNER;
...
#else
    /* For CXD2839 */
    demodSlave = SONYSAT_DEMOD_IIC_ADDRESS_2839;
    tunerSlave = SONYSAT_DEMOD_IIC_ADDRESS_TUNER;
    demod_xtal = SONYSAT_DEMOD_XTAL_41000KHz;
    is_iqinv = 0;
#endif
    ...
    *pptTDSpecificCtx = pPdCtxDVBS;/*There may be a question here.................................*/
    ...
    mcSEMA_UNLOCK_MTFE(pPdCtxDVBS->hHalLock);
    if(hMainSub == NULL_HANDLE){
        if (x_sema_create(&hMainSub, X_SEMA_TYPE_BINARY, X_SEMA_STATE_LOCK) != OSR_OK)
        {
            mcDBG_MSG_ERR(("%s:  x_sema_create(hMainSub) Fail!\n",__FUNCTION__));
            return (DRVAPI_TUNER_ERROR);
        }
    }
    mcSEMA_UNLOCK_MTFE(hMainSub);
    ...
    g_sonysat_demod_ctx.breakFct = NULL;
    g_sonysat_demod_ctx.breakFctArg = NULL;
    pInteg->pfStopwatch_elapsed = sonysat_stopwatch_elapsed;

    sonysat_result = sonysat_i2c_MTK_CreateI2c(pI2c);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }

    // I2C struct creation used in CXD2839 driver
#ifdef SONYSAT_DEMOD_ENABLE_GATEWAY //Defined in Makefile, mark it if use I2C bypass
    sonysat_result = sonysat_i2c_MTK_CreateI2cGw(pI2cGw, demodSlave, 0x09);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
    mcDBG_MSG_L2(("Using gateway, demodSlave is %d\n",demodSlave));

    // Read tuner product id
    sonysat_result = pI2cGw->ReadRegister (pI2cGw, tunerSlave, 0x7F, &data, 1);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
#else
    ...
    if(data == 0x07){
        ...
        mcDBG_MSG_ERR(("Error to access to tuner %d.\n",data));
        return DRVAPI_TUNER_ERROR;
    }

    // LNB struct creation ----------------------------------------------------
#if defined(SONYSAT_LNBC_INTERSIL_ISL9492)
    sonysat_result = intersil_isl9492_Create(pLnbc, SONYSAT_DEMOD_IIC_ADDRESS_LNB, pI2c);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
    // Check I2C communication to LNB controller first
    {
        ...
        if(sonysat_result != SONYSAT_RESULT_OK){
            mcDBG_MSG_L2(("Error to write to LNBC. LNBC will be ignored.\n"));
            ignore_lnbc = TRUE;
        } else {
            mcDBG_MSG_L2(("Success to write to LNBC.\n"));
        }
    }
#elif defined(SONYSAT_LNBC_MPS_MP8126)
    ignore_lnbc = FALSE;
...
#elif defined(SONYSAT_LNBC_ST_LNBH29)
    ignore_lnbc = FALSE;
    sonysat_result = st_lnbh29_Create (pLnbc, SONYSAT_DEMOD_IIC_ADDRESS_LNB, pI2c);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
#elif defined(SONYSAT_LNBC_ALLEGRO_A8304)
    ignore_lnbc = FALSE;
    sonysat_result = allegro_a8304_Create (pLnbc, SONYSAT_DEMOD_IIC_ADDRESS_LNB, pI2c);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
#endif

    // Integ and demod struct creation used in CXD2839 driver -----------------
    if(ignore_lnbc){
    ...
    mcDBG_MSG_L2(("Starting CXD2839 Demod Initialization.\n"));
