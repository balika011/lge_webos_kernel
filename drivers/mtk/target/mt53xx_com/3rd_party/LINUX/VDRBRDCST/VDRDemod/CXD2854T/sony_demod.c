                                 ...
                                 sony_i2c_t * pDemodI2c)
{
    ...
    pDemod->i2cRepeaterEnable = 1; /* For MTK */
...
sony_result_t sony_demod_InitializeT_C (sony_demod_t * pDemod)
{
    ...
    if ((pDemod->xtalFreq == SONY_DEMOD_XTAL_20500KHz) || (pDemod->xtalFreq == SONY_DEMOD_XTAL_41000KHz)) {
        if (!SONY_DEMOD_CHIP_ID_2k12_GENERATION (pDemod->chipId) &&
            !SONY_DEMOD_CHIP_ID_2k14_GENERATION (pDemod->chipId)) {
            pDemod->state = SONY_DEMOD_STATE_INVALID;
        ...
        if (!SONY_DEMOD_CHIP_ID_2k14_GENERATION (pDemod->chipId)) {
            pDemod->state = SONY_DEMOD_STATE_INVALID;
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_NOSUPPORT);
        }
    }
    ...
    case SONY_DEMOD_CONFIG_TS_CURRENT_10mA:
...

        {
            ...
            if ((data & 0x0F) == SONY_DEMOD_GPIO_MODE_TS_ERROR) {
                 ...
                 */
                if (sony_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x85, (uint8_t) (value ? 0x04 : 0x00), 0x04) != SONY_RESULT_OK) {
                    SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
                }
            }
        }
                                        ...
                                        sony_demod_gpio_mode_t mode)
{
    ...
    if (id == 2) {
         ...
         * <SLV-X>  00h     85h     [2]    1'b0        OREG_GPIO2_C
         */
        if (sony_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVX, 0x00, 0x85, drivability, 0x04) != SONY_RESULT_OK){
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }        
    }
                                    ...
                                    uint8_t value)
{
    ...
    */
    if (sony_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0xA2, (uint8_t) (value ? (0x01 << id) : 0x00), (0x01 << id)) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
...
sony_result_t sony_demod_SetTsClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
     ...
     */
    {
        ...
        if ((system == SONY_DTV_SYSTEM_ISDBT) || (system == SONY_DTV_SYSTEM_ISDBS)) {
...
static sony_result_t SDtoSLT(sony_demod_t * pDemod)
{
    ...
    if (pDemod->xtalFreq == SONY_DEMOD_XTAL_24000KHz) {
        ...
        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVX, 0x12, 0x00) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    }
    ...
    if (SONY_DEMOD_CHIP_ID_SUPPORT_SAT (pDemod->chipId)) {
        /* SADC Bias On */
...
static sony_result_t SDtoSLS(sony_demod_t * pDemod)
{
    ...
    if (SONY_DEMOD_CHIP_ID_SUPPORT_TERR_OR_CABLE (pDemod->chipId)) {
        /* TADC Bias On */
        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x43, 0x0A) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
...
/* For MTK */
sony_result_t sony_demod_ReinitializeT_C (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("sony_demod_ReinitializeT_C");
...
/* For MTK */
sony_result_t sony_demod_ForceShutdown (sony_demod_t * pDemod)
{
    uint8_t data = 0;
    SONY_TRACE_ENTER ("sony_demod_ForceShutdown");
    ...
    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVX, 0x15, &data, 1) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
