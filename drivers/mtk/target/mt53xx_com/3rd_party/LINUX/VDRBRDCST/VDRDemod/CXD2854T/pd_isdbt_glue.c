...
#define SONY_CXD2854_ISDBT_DRIVER_VERSION_STR "Sony CXD2854 (ISDB-T) v0.13 2014-07-31"
...
static VOID vResetDemod(UINT8 tunerID, UINT16 u2DelayTime)
{
    ...
    if (DRVCUST_OptQuery(eDemodResetGpio, &u4RstPinNum) == 0)
    {
        ...
        mcDBG_MSG_L2(("CXD2854[%d] : Reset ISDBT Demod GPIO=0x%x, 0->1, %dms\r\n", tunerID, u4RstPinNum, u2DelayTime));
    }
}
}
...
static sony_result_t sony_demod_isdbt_monitor_LayerNum (sony_demod_t * pDemod, uint8_t * pLayerNum)
{
    sony_result_t result = SONY_RESULT_OK;
    sony_isdbt_tmcc_info_t tmccInfo;

    SONY_TRACE_ENTER ("sony_demod_isdbt_monitor_LayerNum");
...
*/
static void ISDBT_TunerGetDemodData(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    sony_result_t sony_result = SONY_RESULT_OK;
...
static void ISDBT_TunerGetTMCC(UINT8 tunerID, void* pzAttribute)
{   
    SIGNAL t_signal;
    ...
    for(ii=0; ii<3; ii++){
        ...
        case LAYERA:
            ISDBT_TunerGetDemodData(tunerID, NULL, TUNER_CONN_TYPE_TER_DIG,&t_signal);
    ...
    {
	    uint8_t layerNum = 0;
	    sony_result = sony_demod_isdbt_monitor_LayerNum(pDemod, &layerNum);
	    if(sony_result == SONY_RESULT_OK){
	    mcDBG_MSG_L2(("Layer Num = %d\n", layerNum));
	    pt_tuner_info->e_LAYER_NUM= layerNum;
	    }else{
	    mcDBG_MSG_L2(("Error in sony_demod_isdbt_monitor_LayerNum (%s)\n", sony_FormatResult(sony_result)));
	    }
    }
    /* ewsFlag */
    {
        ...
        if(sony_result == SONY_RESULT_OK){
            ...
            ewsFlag = pSonyDemodCtx->fgEws; // Previous EWS flag
        }
        ...
        if(pSonyDemodCtx->fgEws != ewsFlag){
        ...
        // Save current value
        pSonyDemodCtx->fgEws = ewsFlag;
        pt_tuner_info->ewsFlag= ewsFlag;
    }
    
    /* 1.isPartial ; 2.Interleaving 3. segmentsNum */
    {
            const char* mode_str[] = {
                "1", "2", "3", "Unknown(3)"
            };
            ...
            if(sony_result == SONY_RESULT_OK){
                ...
                mcDBG_MSG_L4(("Error in sony_demod_isdbt_monitor_ModeGuard (%s)\n", sony_FormatResult(sony_result)));
            }

            sony_result = sony_demod_isdbt_monitor_TMCCInfo(pDemod, &tmccInfo);
            if(sony_result == SONY_RESULT_OK){
                pt_tuner_info->isPartial= tmccInfo.currentInfo.isPartial;
                
                pt_tuner_info->e_ISDBT_LAYERA.Interleaving=tmccInfo.currentInfo.layerA.ilLength;
                pt_tuner_info->e_ISDBT_LAYERB.Interleaving=tmccInfo.currentInfo.layerB.ilLength;
                pt_tuner_info->e_ISDBT_LAYERC.Interleaving=tmccInfo.currentInfo.layerC.ilLength;
                
                pt_tuner_info->e_ISDBT_LAYERA.SegmentsNum=tmccInfo.currentInfo.layerA.segmentsNum;
                pt_tuner_info->e_ISDBT_LAYERB.SegmentsNum=tmccInfo.currentInfo.layerB.segmentsNum;
                pt_tuner_info->e_ISDBT_LAYERC.SegmentsNum=tmccInfo.currentInfo.layerC.segmentsNum;
        ...
        /* TS Rate(DataRate) */
        {
            uint32_t tsRateKbps = 0;
            sony_result = sony_demod_isdbt_monitor_TsRate(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_A, &tsRateKbps);
            if(sony_result == SONY_RESULT_OK){
                pt_tuner_info->e_ISDBT_LAYERA.DataRateMbps= (tsRateKbps + 500)/1000;
                mcDBG_MSG_L1(("TS Rate (Layer A) = %d(kbps)\n", tsRateKbps));
                
            }else if(sony_result == SONY_RESULT_ERROR_HW_STATE){
                mcDBG_MSG_L1(("TS Rate (Layer A) : Unused\n"));
            }else{
                mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_TsRate (%s)\n", sony_FormatResult(sony_result)));
            }
            sony_result = sony_demod_isdbt_monitor_TsRate(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_B, &tsRateKbps);
            if(sony_result == SONY_RESULT_OK){
                pt_tuner_info->e_ISDBT_LAYERB.DataRateMbps= (tsRateKbps + 500)/1000;
                mcDBG_MSG_L1(("TS Rate (Layer B) = %d(kbps)\n", tsRateKbps));
            }else if(sony_result == SONY_RESULT_ERROR_HW_STATE){
                mcDBG_MSG_L1(("TS Rate (Layer B) : Unused\n"));
            }else{
                mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_TsRate (%s)\n", sony_FormatResult(sony_result)));
            }
            sony_result = sony_demod_isdbt_monitor_TsRate(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_C, &tsRateKbps);
            if(sony_result == SONY_RESULT_OK){
                pt_tuner_info->e_ISDBT_LAYERC.DataRateMbps= (tsRateKbps + 500)/1000;
                mcDBG_MSG_L1(("TS Rate (Layer C) = %d(kbps)\n", tsRateKbps));
            }else if(sony_result == SONY_RESULT_ERROR_HW_STATE){
                mcDBG_MSG_L1(("TS Rate (Layer C) : Unused\n"));
            }else{
                mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_TsRate (%s)\n", sony_FormatResult(sony_result)));
            }
        }
...
static void MonitorProcess(VOID *pvArg)
{
    ...
    sony_demod_ctx_t * pSonyDemodCtx = NULL;
    if (psPdCtx == g_sony_demod_ctx[1].pPdCtxIsdb){
        tunerID = 1;
        pSonyDemodCtx = &g_sony_demod_ctx[1];
    } else {
        tunerID = 0;
        pSonyDemodCtx = &g_sony_demod_ctx[0];
    }
    ...
    while (psPdCtx->fgMonitorThreadRunning)
    {
      //  Snr = 0;
        if (psPdCtx->fgBreakMonitor)
        {
            ...
            mcDBG_MSG_L1(("CXD2854[%d] : Entering ISDBT mon_suspend\n", tunerID));
            mcSEMA_LOCK_MTFE(psPdCtx->t_escape_mon_suspend);
            //mcMUTEX_LOCK(psPdCtx->t_escape_mon_suspend);
            mcDBG_MSG_L1(("CXD2854[%d] : Escape ISDBT mon_suspend\n", tunerID));
        }
        else
        {
           ...
           //UEC_ACC
        u4PreUecPktErr = pSonyDemodCtx->u4Uec;
        if(pSonyDemodCtx->u1UecAccDelay > 3){
            if(Count==1)
            {
                pSonyDemodCtx->u4Uec += sony_GetUEC(tunerID);
                if(u4PreUecPktErr > (pSonyDemodCtx->u4Uec))
                {
                    pSonyDemodCtx->u4Uec = 0xFFFFFFFF;
                    mcDBG_MSG_L2(("UECAcc[%d] has already overflowed \n",tunerID));
                }
                mcDBG_MSG_L2(("UECAcc[%d] =%lu\n",tunerID,pSonyDemodCtx->u4Uec));
                Count=0;
            }
            else
            {
...
************************************************************************/
static INT32 PD_StopMonitorEngine(ISDBT_PD_CTX_T *psPdCtx)
{
 ...
 */
//-------------------------------------------------------------------------
static INT32 ISDBT_TunerClose(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    ...
    PD_StopMonitorEngine(pPdCtx);  // stop monitor engine
 ...
 */
//-------------------------------------------------------------------------
static BOOL ISDBT_TunerAcq(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, void* pv_conn_info,
                            SIZE_T z_conn_info_len, x_break_fct _BreakFct, void* pvArg)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    TUNER_DEVICE_CTX_T *pIsdbtTunerDeviceCtx = (TUNER_DEVICE_CTX_T*)pSonyDemodCtx->pTunerDeviceCtxIsdb;
...
// CXD2854 driver specific
    ...
    mcDBG_MSG_L2(("CXD2854[%d] : ISDBT_TunerAcq--------->>>>>>>>>>>>\n", tunerID));

    //MUTEX lock
    mcSEMA_LOCK_MTFE(pPdCtx->hHalLock);
    pSonyDemodCtx->u4Uec=0;//Clear UEC ACC calue
    pSonyDemodCtx->u1UecAccDelay = 0;
    ...
    if(g_sonyproduct_typeinfo.eProjectID == 0x00){ // Sakura
        ...
        {
	        GPIO_SetOut(u4GpioforRFAGCSwitch,0);//d20140628_Xiaohong:Gpio set High for DTV
...
//Pause monitor thread
...

    {
        ...
        mcDBG_MSG_L2(("CXD2854[%d] : System = ISDB-T\n", tunerID));
        ...
        switch(sony_result){
        case SONY_RESULT_OK:
            mcDBG_MSG_L2(("CXD2854[%d] : ISDB-T Locked.\n", tunerID));
            fgLock = 1;
            break;
        case SONY_RESULT_ERROR_UNLOCK:
            mcDBG_MSG_L2(("CXD2854[%d] : ISDB-T Unlocked.\n", tunerID));
            fgLock = 0;
            break;
        case SONY_RESULT_ERROR_TIMEOUT:
            mcDBG_MSG_L2(("CXD2854[%d] : ISDB-T Wait Lock but Timeout.\n", tunerID));
            fgLock = 0;
            break;
        default:
            mcDBG_MSG_L2(("CXD2854[%d] : Error in sony_integ_isdbt_Tune (%s)\n", tunerID, sony_FormatResult(sony_result)));
            fgLock = 0;
            break;
        }
    }
 ...
 */
//-------------------------------------------------------------------------
static BOOL ISDBT_TunerGetSync(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    sony_result_t sony_result = SONY_RESULT_OK;
 ...
 */
//-------------------------------------------------------------------------
static VOID ISDBT_TunerDisc(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    sony_integ_t *pInteg = &pSonyDemodCtx->integ;
    ...
    if(pPdCtx->fgDisStatus == TRUE)
    {
        ...
        mcDBG_MSG_L2(("CXD2854[%d] : ISDBT Has Disconnect!\n", tunerID));
        return;
    }
    mcSEMA_LOCK_MTFE(pPdCtx->hHalLock);
 ...
 */
//-------------------------------------------------------------------------
static void ISDBT_TunerGetSignal(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    ...
    ISDBT_TunerGetDemodData(tunerID, ptTDSpecificCtx, e_conn_type, _pSignal);
...
static UINT8 sony_demod_isdbt_monitor_IFAGCOut_MTK(UINT8 tunerID)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    UINT8 u1Ret=0;
    sony_result_t sony_result = SONY_RESULT_OK;
 ...
 */
//-------------------------------------------------------------------------
static UINT8 ISDBT_TunerGetSignalLevel(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    UINT8  u1signallevel = 0;
    ...
    u1signallevel = sony_demod_isdbt_monitor_IFAGCOut_MTK(tunerID);
 ...
 */
//-------------------------------------------------------------------------
static INT16 ISDBT_TunerGetSignalLeveldBm(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    INT16 i2Ret=0;
    ...
    sony_result = sony_integ_isdbt_monitor_RFLevel(pInteg, &rfLevel);
    if(sony_result == SONY_RESULT_OK){
        if(rfLevel >= 0){
...
static UINT32 ISDBT_TunerGetSignalPER(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static UINT32 sony_GetUEC(UINT8 tunerID)
{
    ...
    sony_result = sony_demod_isdbt_monitor_PacketErrorNumber(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_A, &pen);
    if(sony_result == SONY_RESULT_OK){
        penTotal += pen;
    }

    sony_result = sony_demod_isdbt_monitor_PacketErrorNumber(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_B, &pen);
    if(sony_result == SONY_RESULT_OK){
        penTotal += pen;
    }

    sony_result = sony_demod_isdbt_monitor_PacketErrorNumber(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_C, &pen);
    if(sony_result == SONY_RESULT_OK){
        penTotal += pen;
    }
...
static UINT16 ISDBT_TunerGetSignalSNR(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    UINT16 u2_ret=0;
...
static void ISDBT_TunerSetMpgFmt(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, MPEG_FMT_T  *pt_mpeg_fmt)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static char *ISDBT_TunerGetVer(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
...
static INT32 ISDBT_TunerNimTest(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static void ISDBT_TunerSetRegSetting(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, UCHAR ucRegSet, UCHAR ucRegAddr, UCHAR ucRegValue)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static void ISDBT_TunerShowRegSetting(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static void ISDBT_TunerTestI2C(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static void ISDBT_TunerCommand(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, INT32 i4Argc, const CHAR** aszArgv)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
static void ISDBT_TunerDtdCommand(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    sony_result_t sony_result = SONY_RESULT_OK;
    ...
    case 's':
        ...
        /* RF level */
        {
            ...
            if(sony_result == SONY_RESULT_OK){
                ...
                mcDBG_MSG_L1(("Error in sony_integ_isdbt_monitor_RFLevel (%s)\n", sony_FormatResult(sony_result)));
            }
        }
        /* SQI (NOT FORMAL) */
        {
        ...
        /* Layer Num */
        {
            ...
            sony_result = sony_demod_isdbt_monitor_LayerNum(pDemod, &layerNum);
            if(sony_result == SONY_RESULT_OK){
                mcDBG_MSG_L1(("Layer Num = %d\n", layerNum));
            }else{
                mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_LayerNum (%s)\n", sony_FormatResult(sony_result)));
            }
        }
    ...
    case 'b':
            ...
            case 'e':
		{
			...
			sony_result = sony_demod_isdbt_monitor_PreRSBER(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_A, &ber);
			if(sony_result == SONY_RESULT_OK){
				mcDBG_MSG_L1(("BER[%d] (Layer A, unit: 1e-7) = %d\n", tunerID,ber));
			}else{
				mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_PreRSBER (%s)\n", sony_FormatResult(sony_result)));
			}
			sony_result = sony_demod_isdbt_monitor_PreRSBER(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_B, &ber);
			if(sony_result == SONY_RESULT_OK){
				mcDBG_MSG_L1(("BER[%d](Layer B, unit: 1e-7) = %d\n", tunerID,ber));
			}else{
				mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_PreRSBER (%s)\n", sony_FormatResult(sony_result)));
			}
				sony_result = sony_demod_isdbt_monitor_PreRSBER(pDemod, SONY_DEMOD_ISDBT_MONITOR_TARGET_LAYER_C, &ber);
			if(sony_result == SONY_RESULT_OK){
				mcDBG_MSG_L1(("BER[%d](Layer C, unit: 1e-7) = %d\n",tunerID, ber));
			}else{
				mcDBG_MSG_L1(("Error in sony_demod_isdbt_monitor_PreRSBER (%s)\n", sony_FormatResult(sony_result)));
			}
		}
    ...
    case 't':
	   ...
	   break;
            default:
		{
			...
			mcSEMA_LOCK_MTFE(pPdCtx->hHalLock);
			sony_demod_I2cRepeaterEnable(pDemod, 0x01); // I2C repeater enable
			u2OPRet = ITuner_OP(ITunerGetCtx_Multi(tunerID), itGetStatus, 0, &bTunerSts);
			sony_demod_I2cRepeaterEnable(pDemod, 0x00); // I2C repeater disable
			mcSEMA_UNLOCK_MTFE(pPdCtx->hHalLock);
			if(u2OPRet)
				{
    ...
    case 'u':
        ...
        switch(aszArgv[0][1]){
            case 'e':
                {
                    mcDBG_MSG_L1(("UEC_ACC[[%d] = %lu\n",tunerID,pSonyDemodCtx->u4Uec));
                }
             break;
            case 'c':
                {
                    pSonyDemodCtx->u4Uec = 0;
                    mcDBG_MSG_L1(("UEC_ACC value will reset to 0 \n"));
                }
            break;
            default:
            break;
                ...
                case 'r':
                    {
                        uint8_t tunerRev;
                        sony_demod_I2cRepeaterEnable(pDemod, 0x01); // I2C repeater enable
                        ITuner_OP(ITunerGetCtx_Multi(tunerID),itGetChipRev,0,&tunerRev);
                        sony_demod_I2cRepeaterEnable(pDemod, 0x00); // I2C repeater disable
                        
                        mcDBG_MSG_L2(("TUNER_CHIP_REVISION is 0x%x\n", tunerRev));
                        mcDBG_MSG_L2(("DEMOD_CHIP_REVISION is 0x%x\n", pDemod->chipId));
                    }
                 break;
                 default:
                 break;
...
static void ISDBT_TunerDetachI2C(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bDetach)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
 ...
 */
//-------------------------------------------------------------------------
static INT16 ISDBT_TunerGetAttribute(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    ...
    case TUNER_GET_TYPE_BER: 
        *(UINT32*)pzAttribute = sony_GetBER(tunerID);
        break;
    ...
    case TUNER_GET_TYPE_SSI:
        {
            ...
            *pzAttributeLen = sizeof(UINT8);
            ssi = sony_demod_isdbt_monitor_IFAGCOut_MTK(tunerID);        
    ...
    case TUNER_GET_TYPE_TMCC_INFO:
        mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_TMCC_INFO\r\n"));
        {
            ...
            *pzAttributeLen = sizeof(TUNER_TER_DIG_TMCC_INFO_T);
            ISDBT_TunerGetTMCC(tunerID, &TunerTmcc);
            x_memcpy(pzAttribute, &TunerTmcc, *pzAttributeLen);
        }
        break;
	case TUNER_GET_TYPE_TS_LOCK_STATUS:
		mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_TS_LOCK_STATUS\r\n"));
		{
			UINT8 dmdLock = 0;
			UINT8 tsLock = 0;
			UINT8 unlock = 0;
	...
	case TUNER_GET_TYPE_DEMOD_LOCK_STATUS:
		mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_DEMOD_LOCK_STATUS\r\n"));
		{
			UINT8 dmdLock = 0;
			UINT8 tsLock = 0;
			UINT8 unlock = 0;
			sony_result = sony_demod_isdbt_monitor_SyncStat(pDemod, &dmdLock, &tsLock, &unlock);
			if(sony_result == SONY_RESULT_OK){
			mcDBG_MSG_L2(("TS Lock[%d]	  = %d\n", tunerID,tsLock));
			}else{
			mcDBG_MSG_L2(("Error in sony_demod_isdbt_monitor_SyncStat (%s)\n", sony_FormatResult(sony_result)));
			}
			*pzAttributeLen = sizeof(UINT8);
	...
	case TUNER_GET_TYPE_TUNER_CHIP_REVISION:
		mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_TUNER_CHIP_REVISION\r\n"));
		{
			uint8_t tunerRev;
            sony_demod_I2cRepeaterEnable(pDemod, 0x01); // I2C repeater enable
            ITuner_OP(ITunerGetCtx_Multi(tunerID),itGetChipRev,0,&tunerRev);
            sony_demod_I2cRepeaterEnable(pDemod, 0x00); // I2C repeater disable
			mcDBG_MSG_L2(("TUNER_CHIP_REVISION is 0x%x\n", tunerRev));
			*pzAttributeLen = sizeof(uint8_t);
			x_memcpy(pzAttribute, &tunerRev, *pzAttributeLen);
		}
	break;
    
	case TUNER_GET_TYPE_DEMOD_CHIP_REVISION:
		mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_DEMOD_CHIP_REVISION\r\n"));
		{
			uint8_t demodRev;
            demodRev = pDemod->chipId;
			mcDBG_MSG_L2(("DEMOD_CHIP_REVISION is 0x%x\n", demodRev));
			*pzAttributeLen = sizeof(uint8_t);
			x_memcpy(pzAttribute, &demodRev, *pzAttributeLen);
		}
	break;
	
	case TUNER_GET_TYPE_KHZ_CFO:
		mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_KHZ_CFO\r\n"));
		{
			int32_t offset;
			sony_result = sony_demod_isdbt_monitor_CarrierOffset(pDemod, &offset);
			if(sony_result == SONY_RESULT_OK){
			mcDBG_MSG_L2(("Carrier Offset = %d(Hz)\n", offset));
			}else{
			mcDBG_MSG_L2(("Error in sony_demod_isdbt_monitor_CarrierOffset (%s)\n", sony_FormatResult(sony_result)));
			}
			*pzAttributeLen = sizeof(int32_t);
			x_memcpy(pzAttribute, &offset, *pzAttributeLen);
		}
	break;

	case TUNER_GET_TYPE_LAYER_NUM:
		mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_KHZ_CFO\r\n"));
		{
			uint8_t layerNum = 0;
			sony_result = sony_demod_isdbt_monitor_LayerNum(pDemod, &layerNum);
			...
			*pzAttributeLen = sizeof(uint8_t);
			x_memcpy(pzAttribute, &layerNum, *pzAttributeLen);
		}
	break;
...
case TUNER_GET_TYPE_TUNER_STATUS:
	{
		BOOL *u1RFStatus;
		ITuner_OP(ITunerGetCtx_Multi(tunerID),itGetStatus,0,&u1RFStatus);
		mcDBG_MSG_L2(("HELLO,e_get_type =TUNER_GET_TYPE_TUNER_STATUS\r\n"));
		*pzAttributeLen = sizeof(BOOL);
		x_memcpy(pzAttribute, &u1RFStatus, *pzAttributeLen);
	}
break;
#endif // CC_PROJECT_FOR_SONY
 ...
 */
//-------------------------------------------------------------------------
static INT32 ISDBT_TunerSetAttribute(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_SET_TYPE_T  e_set_type,
...
static INT32 ISDBT_TunerSetDiagCmd(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_SET_TYPE_T e_tuner_set_type, VOID* pv_set_info, SIZE_T z_size)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
    INT32   retSts = RMR_OK;
...
static INT32 ISDBT_TunerGetDiagInfo(UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_GET_TYPE_T e_tuner_get_type, VOID* pv_get_info, SIZE_T* pz_size)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx = (ISDBT_PD_CTX_T*)pSonyDemodCtx->pPdCtxIsdb;
...
*/
//-------------------------------------------------------------------------
static void ISDBT_TunerBypassI2C (UINT8 tunerID, PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bSwitchOn)
...
//-----------------------------------------------------------------------------
// Tuner #0
static INT32 ISDBT_TunerClose_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return ISDBT_TunerClose(0, ptTDSpecificCtx);
}

static BOOL ISDBT_TunerAcq_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, void* pv_conn_info,
                            SIZE_T z_conn_info_len, x_break_fct _BreakFct, void* pvArg)
{
    return ISDBT_TunerAcq(0, ptTDSpecificCtx, e_conn_type, pv_conn_info, z_conn_info_len, _BreakFct, pvArg);
}

static BOOL ISDBT_TunerGetSync_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSync(0, ptTDSpecificCtx, e_conn_type);
}

static void ISDBT_TunerGetSignal_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    ISDBT_TunerGetSignal(0, ptTDSpecificCtx, e_conn_type, _pSignal);
}

static UINT8 ISDBT_TunerGetSignalLevel_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalLevel(0, ptTDSpecificCtx, e_conn_type);
}

static INT16 ISDBT_TunerGetSignalLeveldBm_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalLeveldBm(0, ptTDSpecificCtx, e_conn_type);
}

static UINT32 ISDBT_TunerGetSignalPER_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalPER(0, ptTDSpecificCtx,  e_conn_type);
}

static UINT16 ISDBT_TunerGetSignalSNR_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalSNR(0, ptTDSpecificCtx, e_conn_type);
}

static void ISDBT_TunerSetMpgFmt_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, MPEG_FMT_T *pt_mpeg_fmt)
{
    ISDBT_TunerSetMpgFmt(0, ptTDSpecificCtx, pt_mpeg_fmt);
}

static char *ISDBT_TunerGetVer_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return ISDBT_TunerGetVer(0, ptTDSpecificCtx);
}

static INT32 ISDBT_TunerNimTest_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return ISDBT_TunerNimTest(0, ptTDSpecificCtx);
}

static void ISDBT_TunerSetRegSetting_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, UCHAR ucRegSet, UCHAR ucRegAddr, UCHAR ucRegValue)
{
    ISDBT_TunerSetRegSetting(0, ptTDSpecificCtx, ucRegSet, ucRegAddr, ucRegValue);
}

static void ISDBT_TunerShowRegSetting_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    ISDBT_TunerShowRegSetting(0, ptTDSpecificCtx);
}

static void ISDBT_TunerTestI2C_0(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    ISDBT_TunerTestI2C(0, ptTDSpecificCtx);
}

static void ISDBT_TunerCommand_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, INT32 i4Argc, const CHAR** aszArgv)
{
    ISDBT_TunerCommand(0, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void ISDBT_TunerDtdCommand_0(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    ISDBT_TunerDtdCommand(0, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void ISDBT_TunerDetachI2C_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bDetach)
{
    ISDBT_TunerDetachI2C(0, ptTDSpecificCtx, bDetach);
}

static VOID  ISDBT_TunerDisc_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    ISDBT_TunerDisc(0, ptTDSpecificCtx, e_conn_type);
}

static INT32 ISDBT_TunerSetAttribute_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_SET_TYPE_T  e_set_type,
                                        const VOID *pvAttribute, SIZE_T zAttributeLen)
{
    return ISDBT_TunerSetAttribute(0, ptTDSpecificCtx, e_set_type, pvAttribute, zAttributeLen);
}

static INT16 ISDBT_TunerGetAttribute_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    return ISDBT_TunerGetAttribute(0, ptTDSpecificCtx, e_get_type, pzAttribute, pzAttributeLen);
}

static void ISDBT_TunerBypassI2C_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bSwitchOn)
{
    ISDBT_TunerBypassI2C(0, ptTDSpecificCtx, bSwitchOn);
}

#ifdef fcADD_DIAG_INFO
static INT32 ISDBT_TunerSetDiagCmd_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_SET_TYPE_T e_tuner_set_type, VOID* pv_set_info, SIZE_T z_size)
{
    return ISDBT_TunerSetDiagCmd(0, ptTDSpecificCtx, e_tuner_set_type, pv_set_info, z_size);
}

static INT32 ISDBT_TunerGetDiagInfo_0(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_GET_TYPE_T e_tuner_get_type, VOID* pv_get_info, SIZE_T* pz_size)
{
    return ISDBT_TunerGetDiagInfo(0, ptTDSpecificCtx, e_tuner_get_type, pv_get_info, pz_size);
}
#endif

// Tuner #1
static INT32 ISDBT_TunerClose_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{	
    return ISDBT_TunerClose(1, ptTDSpecificCtx);
	
}

static BOOL ISDBT_TunerAcq_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, void* pv_conn_info,
                            SIZE_T z_conn_info_len, x_break_fct _BreakFct, void* pvArg)
{
    return ISDBT_TunerAcq(1, ptTDSpecificCtx, e_conn_type, pv_conn_info, z_conn_info_len, _BreakFct, pvArg);
}

static BOOL ISDBT_TunerGetSync_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSync(1, ptTDSpecificCtx, e_conn_type);
}

static void ISDBT_TunerGetSignal_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    ISDBT_TunerGetSignal(1, ptTDSpecificCtx, e_conn_type, _pSignal);
}

static UINT8 ISDBT_TunerGetSignalLevel_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalLevel(1, ptTDSpecificCtx, e_conn_type);
}

static INT16 ISDBT_TunerGetSignalLeveldBm_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalLeveldBm(1, ptTDSpecificCtx, e_conn_type);
}

static UINT32 ISDBT_TunerGetSignalPER_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalPER(1, ptTDSpecificCtx,  e_conn_type);
}

static UINT16 ISDBT_TunerGetSignalSNR_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    return ISDBT_TunerGetSignalSNR(1, ptTDSpecificCtx, e_conn_type);
}

static void ISDBT_TunerSetMpgFmt_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, MPEG_FMT_T *pt_mpeg_fmt)
{
    ISDBT_TunerSetMpgFmt(1, ptTDSpecificCtx, pt_mpeg_fmt);
}

static char *ISDBT_TunerGetVer_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return ISDBT_TunerGetVer(1, ptTDSpecificCtx);
}

static INT32 ISDBT_TunerNimTest_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    return ISDBT_TunerNimTest(1, ptTDSpecificCtx);
}

static void ISDBT_TunerSetRegSetting_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, UCHAR ucRegSet, UCHAR ucRegAddr, UCHAR ucRegValue)
{
    ISDBT_TunerSetRegSetting(1, ptTDSpecificCtx, ucRegSet, ucRegAddr, ucRegValue);
}

static void ISDBT_TunerShowRegSetting_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    ISDBT_TunerShowRegSetting(1, ptTDSpecificCtx);
}

static void ISDBT_TunerTestI2C_1(PTD_SPECIFIC_CTX ptTDSpecificCtx)
{
    ISDBT_TunerTestI2C(1, ptTDSpecificCtx);
}

static void ISDBT_TunerCommand_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, INT32 i4Argc, const CHAR** aszArgv)
{
    ISDBT_TunerCommand(1, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void ISDBT_TunerDtdCommand_1(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    ISDBT_TunerDtdCommand(1, ptTDSpecificCtx, i4Argc, aszArgv);
}

static void ISDBT_TunerDetachI2C_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bDetach)
{
    ISDBT_TunerDetachI2C(1, ptTDSpecificCtx, bDetach);
}

static VOID  ISDBT_TunerDisc_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    ISDBT_TunerDisc(1, ptTDSpecificCtx, e_conn_type);
}

static INT32 ISDBT_TunerSetAttribute_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_SET_TYPE_T  e_set_type,
                                        const VOID *pvAttribute, SIZE_T zAttributeLen)
{
    return ISDBT_TunerSetAttribute(1, ptTDSpecificCtx, e_set_type, pvAttribute, zAttributeLen);
}

static INT16 ISDBT_TunerGetAttribute_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_GET_TYPE_T  e_get_type,
                                VOID *pzAttribute, SIZE_T* pzAttributeLen)
{
    return ISDBT_TunerGetAttribute(1, ptTDSpecificCtx, e_get_type, pzAttribute, pzAttributeLen);
}

static void ISDBT_TunerBypassI2C_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, BOOL bSwitchOn)
{
    ISDBT_TunerBypassI2C(1, ptTDSpecificCtx, bSwitchOn);
}

#ifdef fcADD_DIAG_INFO
static INT32 ISDBT_TunerSetDiagCmd_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_SET_TYPE_T e_tuner_set_type, VOID* pv_set_info, SIZE_T z_size)
{
    return ISDBT_TunerSetDiagCmd(1, ptTDSpecificCtx, e_tuner_set_type, pv_set_info, z_size);
}

static INT32 ISDBT_TunerGetDiagInfo_1(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CUSTOM_TUNER_GET_TYPE_T e_tuner_get_type, VOID* pv_get_info, SIZE_T* pz_size)
{
    return ISDBT_TunerGetDiagInfo(1, ptTDSpecificCtx, e_tuner_get_type, pv_get_info, pz_size);
}
#endif
//-----------------------------------------------------------------------------
// public functions
//-----------------------------------------------------------------------------
static void PD_ISDBT_Register_LTDIS_Fct(UINT8 tunerID, PTD_CTX ptTDCtx)
{
    TUNER_DEVICE_CTX_T* pTunerDeviceCtx = (TUNER_DEVICE_CTX_T*)ptTDCtx;

    if(tunerID == 1){
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerClose       = ISDBT_TunerClose_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerAcq         = ISDBT_TunerAcq_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGerSync     = ISDBT_TunerGetSync_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignal   = ISDBT_TunerGetSignal_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLevel = ISDBT_TunerGetSignalLevel_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLeveldBm = ISDBT_TunerGetSignalLeveldBm_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalPER = ISDBT_TunerGetSignalPER_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalSNR = ISDBT_TunerGetSignalSNR_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetMpgFmt   = ISDBT_TunerSetMpgFmt_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetVer      = ISDBT_TunerGetVer_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerNimTest     = ISDBT_TunerNimTest_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetRegSetting = ISDBT_TunerSetRegSetting_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerShowRegSetting = ISDBT_TunerShowRegSetting_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerTestI2C     = ISDBT_TunerTestI2C_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerCommand     = ISDBT_TunerCommand_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDtdCommand  = ISDBT_TunerDtdCommand_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDetachI2C   = ISDBT_TunerDetachI2C_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDisc        = ISDBT_TunerDisc_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetAttribute= ISDBT_TunerSetAttribute_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetAttribute= ISDBT_TunerGetAttribute_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerBypassI2C= ISDBT_TunerBypassI2C_1;
#ifdef fcADD_DIAG_INFO
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetDiagCmd  = ISDBT_TunerSetDiagCmd_1;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetDiagInfo = ISDBT_TunerGetDiagInfo_1;
#endif
    }else{
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerClose       = ISDBT_TunerClose_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerAcq         = ISDBT_TunerAcq_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGerSync     = ISDBT_TunerGetSync_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignal   = ISDBT_TunerGetSignal_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLevel = ISDBT_TunerGetSignalLevel_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalLeveldBm = ISDBT_TunerGetSignalLeveldBm_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalPER = ISDBT_TunerGetSignalPER_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetSignalSNR = ISDBT_TunerGetSignalSNR_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetMpgFmt   = ISDBT_TunerSetMpgFmt_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetVer      = ISDBT_TunerGetVer_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerNimTest     = ISDBT_TunerNimTest_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetRegSetting = ISDBT_TunerSetRegSetting_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerShowRegSetting = ISDBT_TunerShowRegSetting_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerTestI2C     = ISDBT_TunerTestI2C_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerCommand     = ISDBT_TunerCommand_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDtdCommand  = ISDBT_TunerDtdCommand_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDetachI2C   = ISDBT_TunerDetachI2C_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerDisc        = ISDBT_TunerDisc_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetAttribute= ISDBT_TunerSetAttribute_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetAttribute= ISDBT_TunerGetAttribute_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerBypassI2C= ISDBT_TunerBypassI2C_0;
#ifdef fcADD_DIAG_INFO
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerSetDiagCmd  = ISDBT_TunerSetDiagCmd_0;
        pTunerDeviceCtx->tLTDIS_Fct_Tbl.pf_TunerGetDiagInfo = ISDBT_TunerGetDiagInfo_0;
#endif
    }
}
//-------------------------------------------------------------------------
 ...
 */
//-------------------------------------------------------------------------
static INT32 Vendor_TunerOpen_Common(UINT8 tunerID, PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    sony_demod_ctx_t *pSonyDemodCtx = &g_sony_demod_ctx[tunerID];
    ISDBT_PD_CTX_T *pPdCtx;
    TUNER_DEVICE_CTX_T *pIsdbtTunerDeviceCtx;
    ...
    *pptTDSpecificCtx = pPdCtx;
    ...
    PD_ISDBT_Register_LTDIS_Fct(tunerID, ptTDCtx);
    //Regisiter tuner driver
    ITunerRegistration(pIsdbtTunerDeviceCtx->tunerType);

    // Store to sony_demod_ctx_t member.
    pSonyDemodCtx->pPdCtxIsdb = pPdCtx;
    pSonyDemodCtx->pTunerDeviceCtxIsdb = pIsdbtTunerDeviceCtx;
...
INT32 Vendor_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
	...
	switch((int) g_sonyproduct_typeinfo.modelVariationID){
		case 0x06: // ISDBT Single & Dual Main source
			return Vendor_TunerOpen_Common(0, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
		break;
		case 0x07: // ISDBT Dual Sub source
			return Vendor_TunerOpen_Common(1, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
		break;
		default:
			return Vendor_TunerOpen_Common(0, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
...
// Tuner #0
INT32 Vendor_C1_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
    return Vendor_TunerOpen_Common(0, ptTDCtx, ptTunerCfgInfo, pptTDSpecificCtx);
}

// Tuner #1
INT32 Vendor_C2_TunerOpen(PTD_CTX ptTDCtx, TUNER_CONFIG_T * ptTunerCfgInfo, PTD_SPECIFIC_CTX* pptTDSpecificCtx)
{
	VDRBRDCST_GetSonyProductTypeInfo(&g_sonyproduct_typeinfo);
	switch((int) g_sonyproduct_typeinfo.modelVariationID){
		case 0x07: // ISDBT Dual Sub source
		...
		default:
			return (DRVAPI_TUNER_OK);
			mcDBG_MSG_L2(("Single Tuner Module,Second Source will Return OK Directly\n"));
		break;
	}
}
