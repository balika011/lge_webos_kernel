...
#ifdef SONYSAT_TUNERMODULE_PE231
// For module PE231
#define SONYSAT_TUNER_EXT_REF
#define SONYSAT_TUNER_LNA_ENABLE
#else
#ifdef SONYSAT_TUNERMODULE_ZE221
// For module ZE221
#else
#ifdef SONYSAT_TUNERMODULE_PE221
// For module PE221
#define SONYSAT_TUNER_EXT_REF
#else
...
EXTERN UINT8 CheckDemodTypeFromEeprom(void);
...
static void Demod_GPIO_Reset(void)
{
    ...
    mcDBG_MSG_L2(("Do not need to reset Demod"));
}
...
static BOOL sonysat_HandleDiseqcCommand(DISEQC_BUS_CMD_T *pCmd)
{
    ...
    if(pCmd->ui4_control_mask & DISEQC_CMD_CTRL_UNICABLE_ODU_CH_CHANGE_MASK){
        ...
        if(pCmd->ui4_control_mask & DISEQC_CMD_CTRL_LNB_POWER_MASK){
            ...
            if((pCmd->ui1_lnb_power == 0) || (pCmd->ui1_lnb_power == 1)){
...
#endif
{
    ...
    pSeq = (sonysat_demod_dvbs_s2_blindscan_seq_t*) x_mem_alloc(sizeof(sonysat_demod_dvbs_s2_blindscan_seq_t));
    if(pSeq == NULL){
...
static VOID  DVBS_TunerDisc(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type)
{
    ...
    if(!do_blindscan){
        // Go to Sleep state
        // If blind scan is running, demod also go to Sleep state by Monitor thread.
        sonysat_integ_t *pInteg = &g_sonysat_demod_ctx.integ;
        sonysat_integ_SleepS(pInteg);
    }
...
static void DVBS_TunerGetSignal(PTD_SPECIFIC_CTX ptTDSpecificCtx, DRV_CONN_TYPE_T e_conn_type, SIGNAL *_pSignal)
{
    ...
    if(_pSignal)
    {
        ...
        // Symbol rate
        {
            ...
            sonysat_result = sonysat_demod_dvbs_s2_monitor_SymbolRate(pDemod, &symbolRate);
            if(sonysat_result == SONYSAT_RESULT_OK){
                _pSignal->SymbolRate = symbolRate;
            }else{
...
static void DVBS_TunerDtdCommand(PTD_SPECIFIC_CTX ptTDSpecificCtx,INT32 i4Argc, const CHAR** aszArgv)
{
    ...
    case 's':
        ...
        /* Symbol Rate */
        {
            ...
            sonysat_result = sonysat_demod_dvbs_s2_monitor_SymbolRate(pDemod, &symbolRate);
            if(sonysat_result == SONYSAT_RESULT_OK){
                mcDBG_MSG_L1(("Symbol Rate (ksps) : %d\n", symbolRate));
            }else{
...
//    sonysat_i2c_t *pI2cGw = &g_sonysat_demod_ctx.i2cGw;
    ...
    u1DemodType = CheckDemodTypeFromEeprom();
   // (TUNER_DEVICE_CTX_T*)ptTDCtx->demodType = u1DemodType;
    ...
    is_iqinv = 0; /* Bernabeu : 0, SYSEVA : 1 */
        ...
        tunerSlave = 0xC6;
        is_iqinv = 0;
#ifdef CC_SONY_DEMOD_CRYSTAL_41M
...
/* For CXD2839 */
    ...
    *pptTDSpecificCtx = pPdCtxDVBS;/*There may be a question here.................................*/
    ...
    //if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
    mcDBG_MSG_L2(("pDVBSTunerDeviceCtx->demodType=0x%02x\n",u1DemodType));
#ifdef CC_6K_DVBS_DEMOD_BYPASS
     if(u1DemodType == 0x0B){
