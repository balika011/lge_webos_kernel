...
*/
static sonysat_result_t Initialize(sonysat_horus3a_t *pTuner);
/**
 @brief Configure the HORUS3A tuner to Active state.
*/
static sonysat_result_t Tune(sonysat_horus3a_t *pTuner, uint32_t frequencykHz,
    sonysat_horus3a_tv_system_t tvSystem, uint32_t symbolRateksps);
/**
 @brief Configure the HORUS3A tuner to Power Save state.
*/
static sonysat_result_t EnterPowerSave(sonysat_horus3a_t *pTuner);
/**
 @brief Configure the HORUS3A tuner to go out from Power Save state.
*/
static sonysat_result_t LeavePowerSave(sonysat_horus3a_t *pTuner);
...
{
    SONYSAT_TRACE_ENTER("sonysat_horus3a_Create");
    ...
    pTuner->state = SONYSAT_HORUS3A_STATE_UNKNOWN; /* Chip is not accessed for now */
    pTuner->xtalFreqMHz = xtalFreqMHz;
    pTuner->pI2c = pI2c;
    pTuner->i2cAddress = i2cAddress;
    pTuner->flags = flags;
    pTuner->frequencykHz = 0;
    pTuner->tvSystem = SONYSAT_HORUS3A_TV_SYSTEM_UNKNOWN;
...
sonysat_result_t sonysat_horus3a_Initialize(sonysat_horus3a_t *pTuner)
{
    ...
    SONYSAT_TRACE_ENTER("sonysat_horus3a_Initialize");
    ...
    pTuner->state = SONYSAT_HORUS3A_STATE_SLEEP;
    pTuner->frequencykHz = 0;
    pTuner->tvSystem = SONYSAT_HORUS3A_TV_SYSTEM_UNKNOWN;
    pTuner->symbolRateksps = 0;
    ...
    sonysat_horus3a_tv_system_t tvSystem, uint32_t symbolRateksps)
{
    ...
    SONYSAT_TRACE_ENTER("sonysat_horus3a_Tune");

    if((!pTuner) || (!pTuner->pI2c)){
        SONYSAT_TRACE_RETURN(SONYSAT_RESULT_ERROR_ARG);
    }

    if((pTuner->state != SONYSAT_HORUS3A_STATE_SLEEP) && (pTuner->state != SONYSAT_HORUS3A_STATE_ACTIVE)){
    ...
    if(pTuner->state == SONYSAT_HORUS3A_STATE_SLEEP){
    ...
    pTuner->state = SONYSAT_HORUS3A_STATE_ACTIVE;
...
sonysat_result_t sonysat_horus3a_Sleep(sonysat_horus3a_t *pTuner)
{
    ...
    SONYSAT_TRACE_ENTER("sonysat_horus3a_Sleep");
...
sonysat_result_t sonysat_horus3a_SetGPO(sonysat_horus3a_t *pTuner, uint8_t output)
{
    ...
    SONYSAT_TRACE_ENTER("sonysat_horus3a_SetGPO");
...
static sonysat_result_t Initialize(sonysat_horus3a_t *pTuner)
{
    ...
    switch(pTuner->flags & SONYSAT_HORUS3A_CONFIG_IQOUT_MASK){
    case SONYSAT_HORUS3A_CONFIG_IQOUT_DIFFERENTIAL:
        /* IQ Out = Differential */
        result = pTuner->pI2c->WriteOneRegister(pTuner->pI2c, pTuner->i2cAddress, 0x0A, 0x00);
        if(result != SONYSAT_RESULT_OK) { SONYSAT_TRACE_RETURN(SONYSAT_RESULT_ERROR_I2C); }
        break;
    case SONYSAT_HORUS3A_CONFIG_IQOUT_SINGLEEND:
        /* IQ Out = Single Ended */
        result = pTuner->pI2c->WriteOneRegister(pTuner->pI2c, pTuner->i2cAddress, 0x0A, 0x40);
        if(result != SONYSAT_RESULT_OK) { SONYSAT_TRACE_RETURN(SONYSAT_RESULT_ERROR_I2C); }
        break;
    case SONYSAT_HORUS3A_CONFIG_IQOUT_SINGLEEND_LOWGAIN:
...

    {
        ...
        /* XOSC_SEL setting */
        if(pTuner->flags & SONYSAT_HORUS3A_CONFIG_EXT_REF){
            data = 0x00;
        }else{
        ...
        if(!(pTuner->flags & SONYSAT_HORUS3A_CONFIG_REFOUT_OFF)){
            /* REFOUT ON  */
            data |= 0x80;
        }
    ...
    sonysat_horus3a_tv_system_t tvSystem, uint32_t symbolRateksps)
{
    ...
    switch(tvSystem){
    case SONYSAT_HORUS3A_STV_ISDBS:
        ...
        break;
    case SONYSAT_HORUS3A_STV_DVBS:
    case SONYSAT_HORUS3A_STV_DVBS2:
    ...
    if(pTuner->flags & SONYSAT_HORUS3A_CONFIG_LNA_ENABLE){
...
static sonysat_result_t EnterPowerSave(sonysat_horus3a_t *pTuner)
{
    ...
    {
        uint8_t data[2];
        if(pTuner->flags & SONYSAT_HORUS3A_CONFIG_POWERSAVE_ENABLEXTAL){
        ...
        if(pTuner->flags & SONYSAT_HORUS3A_CONFIG_LNA_ENABLE){
            if(pTuner->flags & SONYSAT_HORUS3A_CONFIG_POWERSAVE_STOPLNA){
                /* LNA is stopped */
                data[1] = 0x23;
            }else{
                /* LNA is not stopped */
