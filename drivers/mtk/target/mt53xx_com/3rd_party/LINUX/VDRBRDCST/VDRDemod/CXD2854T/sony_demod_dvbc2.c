...
static sony_result_t SLTtoAC2_24MHzXtalSetting (sony_demod_t * pDemod);
...
sony_result_t result = SONY_RESULT_OK;
    ...
    if (pDemod->xtalFreq == SONY_DEMOD_XTAL_24000KHz) {
        result = SLTtoAC2_24MHzXtalSetting (pDemod);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
...
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("SLTtoAC2_24MHzXtalSetting");
...

    {
         ...
         */ 
        const uint8_t data[4] = {0x05, 0xB8, 0xD8, 0x00};
            ...
            
    {
        /* Slave    Bank    Addr    Bit       Default    Value      Name
         * ---------------------------------------------------------------------------------     
         * <SLV-T>   5Eh     DBh    [7:0]     8'h14      8'h18      OREG_SEQC2_RESET_WTIME[7:0]
         * <SLV-T>   5Eh     DCh    [7:0]     8'h1A      8'h1E      OREG_SEQC2_CCOD_WTIME[15:8]
         * <SLV-T>   5Eh     DDh    [7:0]     8'h00      8'h71      OREG_SEQC2_CCOD_WTIME[7:0]
         * <SLV-T>   5Eh     DEh    [7:0]     8'h50      8'h5D      OREG_SEQC2_C2FS_WTIME[15:8]
         * <SLV-T>   5Eh     DFh    [7:0]     8'h00      8'hA9      OREG_SEQC2_C2FS_WTIME[7:0]
         * <SLV-T>   5Eh     E0h    [7:0]     8'h50      8'h5D      OREG_SEQC2_L1SEQ_WTIME[15:8]
         * <SLV-T>   5Eh     E1h    [7:0]     8'h00      8'hA9      OREG_SEQC2_L1SEQ_WTIME[7:0]
         * <SLV-T>   5Eh     E2h    [7:0]     8'h3C      8'h46      OREG_SEQC2_DMDOK_WTIME[15:8]
         * <SLV-T>   5Eh     E3h    [7:0]     8'h00      8'h3F      OREG_SEQC2_DMDOK_WTIME[7:0]
         */
        const uint8_t data[9] = {0x18, 0x1E, 0x71, 0x5D, 0xA9, 0x5D, 0xA9, 0x46, 0x3F};
        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDB, &data[0], sizeof (data)) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    }
...
static sony_result_t AC2toSLT(sony_demod_t * pDemod)
{
...

    {
        ...
        /* Slave     Bank    Addr   Bit     default    Name
         * ----------------------------------------------------------------------------------
         * <SLV-T>   2Dh     24h    [7:0]     8'h75    OREG_LDPCD_PLP_MIN_OUTTIME_N[7:0]
         * <SLV-T>   2Dh     25h    [7:0]     8'h75    OREG_LDPCD_PLP_MIN_OUTTIME_S[7:0]
         */
        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x24, data, sizeof (data)) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    ...
    for (q = 0; q < pGetTuneParamsSeq->numNotch; q++) {
        ...
        if (overlapStart < overlapEnd) {
            if ((overlapEnd - overlapStart) > (uint32_t) (pGetTuneParamsSeq->bnw * pGetTuneParamsSeq->dx - 1)) {
                *pIsDSDS = 1; /* DSlice is noted as Dependant Static DS so break out of notch loop */
                break;
            }
        }
    }
