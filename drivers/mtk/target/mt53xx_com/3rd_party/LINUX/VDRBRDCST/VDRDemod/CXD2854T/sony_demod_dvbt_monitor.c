                                                ...
                                                uint32_t * pBER)
{
    ...
    /**
     @brief The list of alpha values for corresponding Code Rate value.
    */
    static const int32_t alphaDVBT[5] = {
    /* 1/2, 2/3, 3/4, 5/6, 7/8 */
        1,  3,   30,  45,  120
    };
    ...
    if (SONY_DEMOD_CHIP_ID_2k14_GENERATION (pDemod->chipId)) {
        ...
        if ((tps.rateHP >= SONY_DVBT_CODERATE_RESERVED_5) || 
            (tps.rateLP >= SONY_DVBT_CODERATE_RESERVED_5)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_OTHER);
        }

        /* Obtain alpha value based on code rate and modulation */
        if (tps.hierarchy == SONY_DVBT_HIERARCHY_NON || profile == SONY_DVBT_PROFILE_HP) {
            alpha = alphaDVBT[tps.rateHP];
        }
        else if ( profile == SONY_DVBT_PROFILE_LP) {
            alpha = alphaDVBT[tps.rateLP];
        }
    }
    ...
    */
    /* In case of 2k14 Generation Demod Chips
      BER = (bitError - round(2^(N-11) * alpha)) * 10000000 / (2^N * 204 * 8)
          = (bitError - round(2^(N-11) * alpha)) * 312500 / (2^N * 51)
          = (bitError - round(2^(N-11) * alpha)) * 250 * 1250 / (2^N * 51)
          (Divide in 2 steps to prevent overflow.)
    */
    {
        uint32_t div = 0;
        ...
        if (SONY_DEMOD_CHIP_ID_2k14_GENERATION (pDemod->chipId)) {
            ...
            /* round(2^(N-11) * alpha) */
            if (periodExp >= 11) {
                roundAlpha = alpha << (periodExp - 11);
            }
            else {
                roundAlpha = (alpha + (1 << (10 - periodExp))) >> (11 - periodExp);
            }

            /* (bitError - round(2^(N-11) * alpha)) */
            if (bitError > roundAlpha) {
                bitError -= roundAlpha;
            }
            else {
                /* bitError cannot be less than 0. */
                bitError = 0;
            }
        }
