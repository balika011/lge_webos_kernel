...
sony_result_t sony_demod_dvbc_monitor_SymbolRate (sony_demod_t * pDemod, uint32_t * pSymRate)
{
    ...
    /* Symbol rate[MSymbol/sec] = (IREG_TRIALSRATE +( IREG_TLFINTEGRAL /( 2^( 24 + IREG_TSMDRATE )))) * ADC_Clock(MHz) / 16384
     *                          = ((IREG_TRIALSRATE * 4) + (IREG_TLFINTEGRAL / (2^(22 + IREG_TSMDRATE)))) * ADC_Clock(MHz) / 65536
     * Symbol rate[KSymbol/sec] = ((IREG_TRIALSRATE * 4) + (IREG_TLFINTEGRAL / (2^(22 + IREG_TSMDRATE)))) * ADC_Clock(MHz) * 1000 / 65536 
     *
     * For Demod XTal = 20.5MHz / 41MHz   ADC_Clock(MHz) = 41
     * For Demod XTal = 24MHz             ADC_Clock(MHz) = 48
     */
    if ((pDemod->xtalFreq == SONY_DEMOD_XTAL_20500KHz) || (pDemod->xtalFreq == SONY_DEMOD_XTAL_41000KHz)) {
        ...
        *pSymRate = ((trialsrate * 4 + (tlfIntegral / (1 << (22 + tsmDrate)))) * 48000 + 32768) / 65536;
    }
    else {
...
sony_result_t sony_demod_dvbc_monitor_CarrierOffset (sony_demod_t * pDemod, int32_t * pOffset)
{
    ...
    *pOffset = sony_Convert2SComplement (((rdata[0] & 0x3F) << 8) | rdata[1], 14);
     ...
     *
     * For Demod XTal = 20.5MHz / 41MHz   ADC_Clock(KHz) = 41000
     * For Demod XTal = 24MHz             ADC_Clock(KHz) = 48000
     */
    if ((pDemod->xtalFreq == SONY_DEMOD_XTAL_20500KHz) || (pDemod->xtalFreq == SONY_DEMOD_XTAL_41000KHz)) {
