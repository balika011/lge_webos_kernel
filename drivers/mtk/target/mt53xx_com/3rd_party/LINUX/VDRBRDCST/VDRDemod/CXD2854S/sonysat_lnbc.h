...
typedef enum {
    ...
    SONYSAT_LNBC_STATE_OVERLOAD    /**< Overload (Only for "CC_PROJECT_FOR_SONY") */
...
typedef struct sonysat_lnbc_t {
    ...
    */
    ...
    /**
     @brief Monitor Thermal Shutdown status.
     ...
     @return SONYSAT_RESULT_OK if successful.
    */
    sonysat_result_t (*MonitorThermalShutdown) (struct sonysat_lnbc_t * pLnbc, uint8_t * pIsDetect);

    /**
     @brief Monitor Disable  status.

     @param pLnbc Instance of the LNB controller driver.
     @param pDIS Address of the status.
                        - 0: Detect LNBC is Enable status.
                        - 1: Detect LNBC is Disable status.

     @return SONYSAT_RESULT_OK if successful.
    */
    sonysat_result_t (*MonitorDisable) (struct sonysat_lnbc_t * pLnbc, uint8_t * pDIS);
    /**
     @brief When detect LNBC is disable, controller should Restart LNBC every 1.5s.

     @param pLnbc Instance of the LNB controller driver.

     @return SONYSAT_RESULT_OK if successful.
    */
    sonysat_result_t (*Restart) (struct sonysat_lnbc_t * pLnbc);
