...
sonysat_result_t sonysat_demod_dvbs_s2_blindscan_subseq_bt_Sequence (sonysat_demod_dvbs_s2_blindscan_subseq_bt_t * pSeq)
{
    ...
    case BT_STATE_WAIT_TSLOCK:
        ...
        result = sonysat_demod_dvbs_s2_monitor_ScanInfo (pSeq->pCommonParams->pDemod, 
                                                      &tslock,
        ...
        if (tslock){
            ...
            if (elapsedTime > pSeq->timeout){
                ...
                result = sonysat_demod_dvbs_s2_monitor_Pilot (pSeq->pCommonParams->pDemod, &plscLock, &pilotOn);
                if (result != SONYSAT_RESULT_OK){
                    SONYSAT_TRACE_RETURN (result);
                }
    ...
    case BT_STATE_WAIT_TSLOCK2:
        ...
        if (tslock){
            ...
            /* Success */
            SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_BT_RESULT("OK2");
            result = finish_ok (pSeq);
            SONYSAT_TRACE_RETURN (result);
        } else {
            if (elapsedTime > pSeq->timeout){
                SONYSAT_DEMOD_DVBS_S2_BLINDSCAN_TRACE_BT_RESULT("NG2( TS lock timeout)");
                result = finish_ng (pSeq);
                SONYSAT_TRACE_RETURN (result);
            } else {
                /* Continue to wait */
