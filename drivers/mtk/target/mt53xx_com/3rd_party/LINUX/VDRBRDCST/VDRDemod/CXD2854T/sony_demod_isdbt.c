...
------------------------------------------------------------------------------*/
/**
 @brief Configure the demodulator from Sleep T/C to Active for ISDB-T
*/
static sony_result_t SLTtoAIT(sony_demod_t * pDemod);
...
/**
 @brief Configure the demodulator from Active ISDB-T to Sleep T/C.
*/
static sony_result_t AITtoSLT(sony_demod_t * pDemod);
...
/**
 @brief Configure the demodulator from Sleep T/C to EWS-IT.
*/
...
*/
static sony_result_t setDemodCommonParameter (sony_demod_t * pDemod);

/**
 @brief Cancel the demodulator for common settings .
*/
static sony_result_t cancelDemodCommonParameter (sony_demod_t * pDemod);

/**
 @brief Configure the demodulator for tuner optimisations and bandwidth specific
        settings.
*/
static sony_result_t setDemodBandParameter(sony_demod_t * pDemod);
                                     ...
                                     sony_isdbt_tune_param_t * pTuneParam)
{
    ...
    /* 20.5MHz/41MHz Xtal boards support only 6MHz Bandwidth */
    if ((pTuneParam->bandwidth != SONY_DTV_BW_6_MHZ) && (pDemod->xtalFreq != SONY_DEMOD_XTAL_24000KHz)) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_NOSUPPORT);
    }

    if ((pDemod->state == SONY_DEMOD_STATE_ACTIVE_T_C) && (pDemod->system == SONY_DTV_SYSTEM_ISDBT)) {
    ...
    else if((pDemod->state == SONY_DEMOD_STATE_ACTIVE_T_C) && (pDemod->system != SONY_DTV_SYSTEM_ISDBT)){
...
static sony_result_t SLTtoAIT(sony_demod_t * pDemod)
{
    ...
    result = setDemodCommonParameter (pDemod);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }

    /* Set tuner and bandwidth specific settings */
    result = setDemodBandParameter (pDemod);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
static sony_result_t SLTtoEWS(sony_demod_t * pDemod)
{
    ...
    /* Disable HiZ Setting  */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x80, 0x2F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
...
static sony_result_t AITtoSLT(sony_demod_t * pDemod)
{
    ...
    result = cancelDemodCommonParameter (pDemod);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("setDemodBandParameter");
    ...
    case SONY_DTV_BW_8_MHZ:
        /*  20.5/41MHz Xtal support is not available on ISDB-T 8MHzBW */
        if (pDemod->xtalFreq != SONY_DEMOD_XTAL_24000KHz) {
            result = SONY_RESULT_ERROR_NOSUPPORT;
            break;
        }
        {    /* TRCG Nominal Rate [37:0] */
            const uint8_t nominalRate[5] = {0x11, 0xB8, 0x00, 0x00, 0x00}; /* 24MHz */

            /* <Timing Recovery setting>
             * Slave     Bank    Addr   Bit       Default    Value      Name
             * -----------------------------------------------------------------------
             * <SLV-T>   10h     9Fh    [5:0]     8'h11      8'h11      OREG_TRCG_NOMINALRATE[37:32]
             * <SLV-T>   10h     A0h    [7:0]     8'hF0      8'hB8      OREG_TRCG_NOMINALRATE[31:24]
        ...
        if (pDemod->tunerOptimize == SONY_DEMOD_TUNER_OPTIMIZE_ASCOT2XR) {
            const uint8_t itbCoef[14] =
...

        {
             ...
             * --------------------------------------------------------------------
             * <SLV-T>   10h     D9h    [5:0]     8'h00      8'h13      OREG_CDRB_GTDOFST[13:8]
             * <SLV-T>   10h     DAh    [7:0]     8'hBC      8'hFC      OREG_CDRB_GTDOFST[7:0]
             */
        ...
        if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x71, 0x03, 0x07) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    ...
    case SONY_DTV_BW_7_MHZ:
        /*  20.5/41MHz Xtal support is not available on ISDB-T 7MHzBW */
        if (pDemod->xtalFreq != SONY_DEMOD_XTAL_24000KHz) {
            result = SONY_RESULT_ERROR_NOSUPPORT;
            break;
        }
        {   /* TRCG Nominal Rate [37:0] */
            const uint8_t nominalRate[5] = {0x14, 0x40, 0x00, 0x00, 0x00}; /* 24MHz XTal */
            /* <Timing Recovery setting>
             *    Slave    Bank    Addr   Bit      Default     Value      Name
             * -----------------------------------------------------------------------
             *   <SLV-T>   10h     9Fh    [5:0]     8'h11      8'h14      OREG_TRCG_NOMINALRATE[37:32]
             *   <SLV-T>   10h     A0h    [7:0]     8'hF0      8'h40      OREG_TRCG_NOMINALRATE[31:24]
             *   <SLV-T>   10h     A1h    [7:0]     8'h00      8'h00      OREG_TRCG_NOMINALRATE[23:16]
             *   <SLV-T>   10h     A2h    [7:0]     8'h00      8'h00      OREG_TRCG_NOMINALRATE[15:8]
             *   <SLV-T>   10h     A3h    [7:0]     8'h00      8'h00      OREG_TRCG_NOMINALRATE[7:0]
             */
...

        {
             ...
             * slave    Bank    Addr    Bit      default     value      Name
             * --------------------------------------------------------------------
             * <SLV-T>   10h     D9h    [5:0]     8'h00      8'h1A      OREG_CDRB_GTDOFST[13:8]
             * <SLV-T>   10h     DAh    [7:0]     8'hBC      8'hFA      OREG_CDRB_GTDOFST[7:0]
             */
    ...
    case SONY_DTV_BW_6_MHZ:
        ...
        }        
        {
             ...
             * Slave     Bank    Addr   Bit     Default   Name
             * -----------------------------------------------------------------------
             * <SLV-T>   10h     9Fh    [5:0]   8'h11     OREG_TRCG_NOMINALRATE[37:32]
             * <SLV-T>   10h     A0h    [7:0]   8'hF0     OREG_TRCG_NOMINALRATE[31:24]
...

        {
             ...
             *    slave    Bank    Addr    Bit    default   Value          Name
             *   ---------------------------------------------------------------------------------
             *   <SLV-T>   10h     D9h    [5:0]     8'h00      gtofst1    OREG_CDRB_GTDOFST[13:8]
             *   <SLV-T>   10h     DAh    [7:0]     8'hBC      gtofst2    OREG_CDRB_GTDOFST[7:0]
             *
             *         -------+---------+---------
             *          Xtal  | gtofst1 | gtofst2
             *         -------+---------+---------
             *         20.5MHz|  8'h1A  |  8'hE2
             *           41MHz|         |
             *         -------+---------+---------
             ...
             *         -------+---------+---------
             */
            if ((pDemod->xtalFreq == SONY_DEMOD_XTAL_20500KHz) || (pDemod->xtalFreq == SONY_DEMOD_XTAL_41000KHz)) {
                data[0] = 0x1A;
                data[1] = 0xE2;
            }
            else { /* SONY_DEMOD_XTAL_24000KHz */
                data[0] = 0x1F;
                data[1] = 0x79;
            }
        ...
        if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x71, 0x07, 0x07) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
...
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("setDemodCommonParameter");
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x31, 0x00, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
     ...
     */
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xCE, 0x00, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xCF, 0x00, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
     ...
     */
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x69, 0x04, 0x07) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x6B, 0x03, 0x07) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x9D, 0x50, 0xFF) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xD3, 0x06, 0x1F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xED, 0x00, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE2, 0xCE, 0x80) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xF2, 0x13, 0x10) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDE, 0x2E, 0x3F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    /*   slave    Bank    Addr    Bit    default   Value          Name
     *  ---------------------------------------------------------------------------------
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDE, 0x02, 0x03) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    /* Set SLV-T Bank : 0x1E */
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x73, 0x68, 0xFF) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    /* Set SLV-T Bank : 0x63 */
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x81, 0x00, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    ...
    if (pDemod->xtalFreq == SONY_DEMOD_XTAL_24000KHz) {
        ...
        {
            uint8_t data[2] = {0x60, 0x00};
            /* Slave     Bank    Addr   Bit     Default    Value      Name
             * ----------------------------------------------------------------------------------
             * <SLV-T>   10h     BFh    [7:0]     8'h52      8'h60      OREG_CKF_FREQ[15:8]
             * <SLV-T>   10h     C0h    [7:0]     8'h00      8'h00      OREG_CKF_FREQ[7:0]
             */
            if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xBF, &data[0], sizeof(data)) != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
            }
        }
...

        {
             ...
             */
            if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xA8, &data[0], sizeof(data)) != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
            }
        }
    }
...
{
    sony_result_t result = SONY_RESULT_OK;
    SONY_TRACE_ENTER ("cancelDemodCommonParameter");
    ...
    */
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x69, 0x05, 0x07) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x6B, 0x07, 0x07) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x9D, 0x14, 0xFF) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xD3, 0x00, 0x1F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xED, 0x01, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE2, 0x4E, 0x80) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xF2, 0x03, 0x10) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDE, 0x32, 0x3F) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    ...
    */
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xDE, 0x03, 0x03) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    /* Set SLV-T Bank : 0x1E */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x00, 0x1E) != SONY_RESULT_OK) {
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x73, 0x00, 0xFF) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    /* Set SLV-T Bank : 0x63 */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x00, 0x63) != SONY_RESULT_OK) {
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x81, 0x01, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
