...
#define VENDOR_DRIVER_VER            "2014-01-15\nCXD2839Ver2.19"
    ...
    case CONN_MODE_UNICABLE_SURF://unicable
        {
            ...
            if(tuneParam.symbolRateKSps == 0){
                ...
                // sonysat_demod_dvbs_s2_blindscan_seq_t size is big, so allocate in heap.
                pSeq = (sonysat_demod_dvbs_s2_tune_srs_seq_t*) x_mem_alloc_virtual(sizeof(sonysat_demod_dvbs_s2_tune_srs_seq_t));
                if(pSeq == NULL){
                    // Oops... memory allocation failed...
    ...
    case TUNER_GET_TYPE_LNB_SHORT_STATUS:
        {
            ...
            if((pLnbc != NULL) && (pLnbc->MonitorOverload != NULL))
            {
            ...
            {
                mcDBG_MSG_L4(("Err: TUNER_GET_TYPE_LNB_SHORT_STATUS fail, Null pointer\n"));
            }
        }
        break;
    ...
    case TUNER_GET_TYPE_ACTUAL_FREQ:
        {
            ...
            INT32 offset = 0;
            sonysat_result = sonysat_demod_dvbs_s2_monitor_CarrierOffset(pDemod, &offset);
            Frequency = g_sonysat_demod_ctx.tunedSignal.centerFreqKHz + offset;  //in KHz
            mcDBG_MSG_L2(("e_get_type = TUNER_GET_TYPE_ACTUAL_FREQ\n"));
            mcDBG_MSG_L2(("DVBS frequency is %ld\n", Frequency));
            *pzAttributeLen = sizeof(INT32);
    ...
    case TUNER_SET_TYPE_DISEQC_CMD:
        {
            ...
            if(pCmd->ui4_control_mask & DISEQC_CMD_CTRL_NOT_RESEND_FULL_CMD_MASK){
                //Not copy to resend
            }else{
...
//    sonysat_i2c_t *pI2cGw = &g_sonysat_demod_ctx.i2cGw;
    ...
    *pptTDSpecificCtx = pPdCtxDVBS;/*There may be a question here.................................*/
    ...
    // NOTE: Please set TS serial or parallel here!!
		...
		is_parallel = FALSE;
		mcDBG_MSG_L2(("CXD2839: Using SERIAL TS output!!\n"));
 #else
		is_parallel = TRUE;
		mcDBG_MSG_L2(("CXD2839: Using PARALLEL TS output!!\n"));
 #endif
    ...
    // LNB struct creation ----------------------------------------------------
#if defined(SONYSAT_LNBC_INTERSIL_ISL9492)
    sonysat_result = intersil_isl9492_Create(pLnbc, 0x10, pI2c);
    if(sonysat_result != SONYSAT_RESULT_OK){ return DRVAPI_TUNER_ERROR; }
