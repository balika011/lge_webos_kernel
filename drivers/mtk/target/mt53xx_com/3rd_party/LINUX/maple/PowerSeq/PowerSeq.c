...
void Custom60HzPowerOnSequence(void)
{
    ...
    //smith mark  x_thread_delay(1800);//add this for SOEM power seq sepc T34 > 2s  
    vDrvTurnOnPanel();
   ...
   //smith mark   x_thread_delay(_au2PowerSeqDelay[2]);
    
    // Send LVDS output
//    PRINT_TIMESTAMP_MSG("[Power On]", "LVDS_OUTPUT (Send LVDS output)\n");
    x_thread_delay(40);//smith Sakurau spec LVDS on delay 30~50 ms form panel power on
    vDrvLVDSConfig();
    vDrvLVDSOn();
    ...
    //smith markx_thread_delay(_au2PowerSeqDelay[6]);
    ...
    //smith mark x_thread_delay(_au2PowerSeqDelay[8]);
}
...
void CustomPowerOnSequence(void)
{
   ...
   //smith mark  CustomGetPowerSequenceDelay();
    
    vDrvLVDSOff();

    Custom60HzPowerOnSequence();

    //smith mark  x_thread_delay(_au2PowerSeqDelay[9]);
	...
	*/
	 x_thread_delay(510);//smith Sakurau spec backlight  on delay >550 ms form panel power on ,550-40=510
     vDrvTurnOnBacklt();
#ifdef CC_SUPPORT_MHL
			if(!_bfgMhlInit)
			{
	...
	//DRVCUST_SetScanPWM_High_Low(100); //need PQ
	
	BACKLT_SetDimming(BACKLT_GetDimmingFrequency(), 255);

	//smith mark x_thread_delay(_au2PowerSeqDelay[11]);
