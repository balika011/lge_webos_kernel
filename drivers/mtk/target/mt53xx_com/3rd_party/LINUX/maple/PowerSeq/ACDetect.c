...
#if 0
static BOOL _fgACDetReqSelfDiagPause=FALSE;
#endif
#ifdef CC_SOEM_ACDET_FLOW
static BOOL _u1fgACDetectErrorInitValMatch = FALSE;
#endif
//-----------------------------------------------------------------------------
// Functions
//-----------------------------------------------------------------------------
...
static void CustomACDetectClearShutdownState(void)
{
    ...
    UINT32 u4Value=0;
    // Clear AC_DET error into EEPROM
    u4Value = CUSTOM_POWER_SEQ_CLEAR_SHUTDOWN_STATE;
...
static void CustomACDetectBackNormal(HANDLE_T hHandle, VOID* pTag)
{
    ...
    if (bPowerOnIsDone)
    {
		...
		#if 0
		if(SELF_DIAG_INIT_STATUS() && _fgACDetReqSelfDiagPause) 
		{
          SELF_DIAG_RESUME();
		}
		#endif
...
static void CustomACDetectHandle(void *pvArg)
{
    ...
    UINT8  u1ShutdownState = 0;
    #ifdef CC_SOEM_ACDET_FLOW
	UINT8  u1ACDetInitVal = 0;
	UINT8  u1ACDetectErrorIniVal = 0;
	#endif
    UINT32 u4Counter = 0;
    ...
    UNUSED(EEPROM_Write(CUSTOM_SHUTDOWN_OCCUR_ADDR, 
                        (UINT32)(VOID *)&u1ShutdownState, sizeof(UINT8)));
	#ifdef CC_SOEM_ACDET_FLOW
    UNUSED(EEPROM_Read(POWER_SEQ_FORCE_AC_DET_INIT_VAL, 
                       (UINT32)(VOID *)&u1ACDetectErrorIniVal, sizeof(UINT8)));
	#endif
    _bfgEnableShutdownState = FALSE;
...
//	vIO32Write4B(PDWNC_RESRV0, 0x53965396);
    ...
    while (_u1fgACDetectGo == 1)
    {
		...
		#ifdef CC_SOEM_ACDET_FLOW
		if((i4Value == AC_DETECT_ERROR) && (!_u1fgACDetectErrorInitValMatch))
		#else
        ...
        #endif
        {
            ...
            #ifdef CC_SOEM_ACDET_FLOW
		    if (u1ACDetectErrorIniVal == AC_DETECT_ERROR_INIT_VAL)
            {
                #ifdef CUSTOM_ALL_AC_DET_LOG
                PRINT_TIMESTAMP_MSG("[AC Detect]", "Stop ACDET DUE TO ERROR INT\n");
                #endif
				_u1fgACDetectErrorInitValMatch = TRUE;
            }
			#endif
			
			#ifdef CC_SOEM_ACDET_FLOW
            if ((u4Counter == 0)&&(!_u1fgACDetectErrorInitValMatch))
			#else
            if (u4Counter == 0)
			#endif
            {
...
#endif /* CUSTOM_ALL_AC_DET_LOG */
                #if 0
                if(SELF_DIAG_INIT_STATUS())
				{
                  SELF_DIAG_PAUSE();
				  _fgACDetReqSelfDiagPause = TRUE;
                }
				else
				{
				  _fgACDetReqSelfDiagPause = FALSE;
				}
				#endif
            ...
            if (++u4Counter == _u4ACDetectRetryLimit)
            {
                ...
                // Go to panel off
                #ifdef CC_SOEM_ACDET_FLOW
                u1ACDetInitVal = AC_DETECT_ERROR_INIT_VAL; 
                UNUSED(EEPROM_Write(POWER_SEQ_FORCE_AC_DET_INIT_VAL, 
                        (UINT32)(VOID *)&u1ACDetInitVal, sizeof(UINT8)));
				#endif
        ...
        else
        {
            ...
            u4Counter = 0;
            #ifdef CC_SOEM_ACDET_FLOW
			if ((i4Value == AC_DETECT_NORMAL)&&(u1ACDetectErrorIniVal == AC_DETECT_ERROR_INIT_VAL))
            {
                #ifdef CUSTOM_ALL_AC_DET_LOG
                PRINT_TIMESTAMP_MSG("[AC Detect]", "back ACDET DUE TO AC_DETECT_NORMAL\n");
                #endif
                u1ACDetInitVal = AC_DETECT_NORMAL_INIT_VAL; 
                UNUSED(EEPROM_Write(POWER_SEQ_FORCE_AC_DET_INIT_VAL, 
                       (UINT32)(VOID *)&u1ACDetInitVal, sizeof(UINT8)));    
				_u1fgACDetectErrorInitValMatch = FALSE;
				u1ACDetectErrorIniVal = AC_DETECT_NORMAL_INIT_VAL;
            }
			#endif
        }
...
void ACErrorDetect(INT32 i4Gpio, BOOL fgStatus)
{
    UNUSED(i4Gpio);
    UNUSED(fgStatus);
    PRINT_TIMESTAMP_MSG("[AC Detect]", "AC_DROP = %d\n");

	#ifdef CC_SAKURA_CVT_SUPPORT
    GPIO_SetOut(213,0);//AC_OFF_INFO
    GPIO_SetOut(213,1);//AC_OFF_INFO
    GPIO_SetOut(247,1);//OPWRSB
    GPIO_SetOut(247,0);//OPWRSB
	#endif
    ...
    vDrvTurnOffBacklt(); // Backlight off
    AUD_PowerOffGpioMute(TRUE); //AMP/HP/Line out mute
    HAL_Delay_us(500);
    vDrvLVDSOff();
    HAL_Delay_us(500);
	vDrvTurnOffPanel();
