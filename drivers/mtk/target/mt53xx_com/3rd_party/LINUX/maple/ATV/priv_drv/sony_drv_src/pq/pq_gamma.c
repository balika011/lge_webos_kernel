...
/* ------------------------------------------- */
#if 0
void Sony_WB_Acfg_Pic_Update_Set (UINT16 write_data)
{
	ui2_acfg_update_pic_mode = write_data;
}
UINT16 Sony_WB_Acfg_Pic_Update_Get (void)
{
	return ui2_acfg_update_pic_mode;
}
#endif
...
UINT32 Sony_PQ_GAMMA_EDID_Write (UINT16 write_data)
{
	...
	u1_LastByte[1] = 0x54;
	u2_last_byte_addr = 0x180 + NVM_WB_SIZE;
...
*/
			...
			if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, 0x100,sony_pq_header, NVM_SONY_PQ_HEADER_SIZE))//header
			{
				u4_result = 1;
				break;
			}
	...
	x_thread_delay(20);
			...
			SIF_X_Read(1, 0x100, 0xaa, 2, 0x100,sony_pq_header, NVM_SONY_PQ_HEADER_SIZE);
	...
	x_thread_delay(20);
			if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, 0x120,luma_read, 66))
			{
				u4_result = 1;
				break;
			}

			SIF_X_Read(1,0x100,0xaa,2,0x120,luma_read,66);
	...
	x_thread_delay(20);
			if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, 0x180,wb_data_nvm, NVM_WB_SIZE))
			{
				u4_result = 1;
				break;
			}

			SIF_X_Read(1,0x100,0xaa,2,0x180,wb_data_nvm,60);
...
//======================		  write last byte 0x5754 to panel 	 ==================================
	x_thread_delay(20);
			if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, u2_last_byte_addr ,u1_LastByte, 2)) // write loop by per 16 byte
			{
					u4_result = 1;
					break;
			}
...
//======================		  confirm verify byte in panel		 ==================================
			...
			if(0 == SIF_X_Read(1, 0x100, 0xaa, 2, u2_last_byte_addr ,u1_LastByte, 2)) 
			{
				u4_result = 1;
				break;
			}
...
//======================		write PBM to EEP		=========================================
			...
			if (0 == SIF_X_Read(1, 0x100, 0xaa, 2, 0x100,sony_pq_header, NVM_SONY_PQ_HEADER_SIZE)) //header 
			{
				u4_result = 1;
				break;
			}
			...
			if (0 == SIF_X_Read(1, 0x100, 0xaa, 2, 0x120,luma_read, NVM_GAMMA_LVL_SIZE)) //luma
			{
				u4_result = 1;
				break;
			}
			...
			if (0 == SIF_X_Read(1, 0x100, 0xaa, 2, 0x180,wb_data_nvm, NVM_WB_SIZE)) //wb
			{
				u4_result = 1;
				break;
			}
