...
static void _apply_color_temp(UINT16 ct)
{
    ...
    ct_info.ui1CTIndex = ct;        //(ct | 0x80);   //force update gamma          weibin
...
UINT32 Sony_PBM_EDID_Write (UINT16 write_data)
{
	...
	UINT32 compare_panel_buf[EEP_WB_PBM_SIZE_ALL_TEMP]={0};
    UINT64  u8Offset;
	...
	u2_LastByte[1]=0x50;	
	u2_last_byte_addr = 0x1c0 + EEP_WB_PBM_SIZE_ALL_TEMP;
...
//======================      write PBM data to Panel   ===================================================
	    ...
	    x_thread_delay(20);
                if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, u2_temp_addr + u2_shift_addr ,&data[u2_shift_addr], 0x10)) // write loop by per 16 byte
                {
                    u4_return = 1;
                    break;
                }
		...
		x_thread_delay(20);
            ...
            u2_last_addr = ((0x1c0 + EEP_WB_PBM_SIZE_ALL_TEMP) / 0x10)*0x10;
            if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, u2_last_addr ,&data[u2_shift_addr +0x10 ], u2_last_data_len))
            {
                u4_return = 1;
                break;
			}
			SIF_X_Read(1,0x100,0xaa,2,0x1c0,data,602);
...
//======================          write last byte 0x5750 to panel      ===================================================
	...
	x_thread_delay(20);
            if (0 == SIF_X_Write(1, 0x100, 0xaa, 2, u2_last_byte_addr ,u2_LastByte,2)) // write loop by per 16 byte
            {
                u4_return = 1;
                break;
            }
...
//======================		  confirm verify byte in panel		 ==========================================
			...
			if (0 == SIF_X_Read(1, 0x100, 0xaa, 2, u2_last_byte_addr ,u2_LastByte, EEP_WB_VER_BYTE))
			{
				u4_return = 1;
				break;
			}
...
//======================		write PBM to EEP		===================================================
        ...
        u2_last_addr = EEP_WB_PBM_MAGIC_OFFSET + u2_Write_times *0x10;
			...
			if (0 == SIF_X_Read(1, 0x100, 0xaa, 2, u2_temp_addr,data, 602))
			{
				u4_return = 1;
				break;
			}
        ...
        u4_temp_address =(UINT32) &data[u2_Write_times * 0x10];
			...
			EEPROM_Read(EEP_WB_PBM_MAGIC_OFFSET,(UINT32)data,602);
