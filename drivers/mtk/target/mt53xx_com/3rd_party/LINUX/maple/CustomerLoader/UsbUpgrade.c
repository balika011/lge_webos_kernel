...
void vGetTagVersion(CHAR* ucTagName, CHAR * szVerStr)
{
    ...
    {
#if 1//defined(CC_MTK_LOADER)
        LDR_ENV_T* prLdrEnv = (LDR_ENV_T*)CC_LDR_ENV_OFFSET;
        x_strncpy(szVerStr, prLdrEnv->u.rHdr.szVersion, 8);
        szVerStr[8]='\0';
#else	
//        LoaderGetVersion(szVerStr);
//        Printf("vGetTagVersion:[%s, %s]\n", ucTagName, szVerStr);

        SIZE_T z_size = LOADER_VERSION_LENGTH;
        UINT32 u4CheckSum = 0;
	
        d_custom_misc_get (DRV_CUSTOM_MISC_TYPE_LOADER_INFO,
                                   (VOID *)(UCHAR*)szVerStr,
                                   &z_size);
        szVerStr[LOADER_VERSION_LENGTH] = '\0';
        Printf("vGetTagVersion:[%s, %s]\n", ucTagName, szVerStr);
                               ...
                               UCHAR *pauc_buffer, INT32* pi4_len)
{
	...
	UINT32	uiEraseMode=0;

	EEPROM_Header_Read(FALSE, (UINT64)16, (UINT32) &uiEraseMode, 1);
	
    if(uiEraseMode == 0x4E)
    {
        Printf("<_LdrEepromUpgrade>: Only vlidate init Header no need to update data!!\n");
		return DLM_OK;
    }
#endif
...
#ifdef CC_EEP2EMMC_DUAL_BANK_SUPPORT
        EEPROM_Init_Data_Read(TRUE, (UINT64)(u4i), (UINT32) &uiTmp, 1);
        
        uiTmp = ((uiTmp & (~pauc_buffer[u4i+u4MaskIndex])) | 
                 (pauc_buffer[u4DataIndex + u4i]& pauc_buffer[u4i + u4MaskIndex]));
        EEPROM_Init_Data_Write(TRUE, (UINT64)(u4i), (UINT32) &uiTmp, 1);
		
        EEPROM_Init_Data_Read(FALSE, (UINT64)(u4i), (UINT32) &uiTmp, 1);
        
        uiTmp = ((uiTmp & (~pauc_buffer[u4i+u4MaskIndex])) | 
                 (pauc_buffer[u4DataIndex + u4i]& pauc_buffer[u4i + u4MaskIndex]));
        EEPROM_Init_Data_Write(FALSE, (UINT64)(u4i), (UINT32) &uiTmp, 1);
...
#ifdef CC_EEP2EMMC_DUAL_BANK_SUPPORT
static INT32 _LdrEepInitHeaderUpgrade(DLM_ACCESS_T e_flag,
                               UCHAR *pauc_buffer, INT32* pi4_len)
{
    UINT32  u4i;
    UINT32  u4MemLen=0;
    UINT32  u4DataIndex, u4MaskIndex;
    UINT8   uiTmp = 0;
	UINT8   uiEraseHeader[512]={0};
    UINT32  u4BufLen;
	UINT32  uiEraseMode=0;
	UINT32  ui4EraseAddress=0;

    if (NULL == pauc_buffer)
    {
        Printf("\n<NVM initial header Upgrade>:pauc_buffer = NULL\n");
        return DLM_OPEN_FILE_FAILED;
    }

    if (NULL == pi4_len)
    {
        Printf("\n<NVM initial header Upgrade>:pi4_len = NULL\n");
        return DLM_OPEN_FILE_FAILED;
    }

    for (u4i = 0; u4i < 4; u4i++)
    {
        Printf("\n<NVM initial header Upgrade>:The Byte  %d is  = %d\n", u4i, pauc_buffer[u4i]);
        u4MemLen =((pauc_buffer[u4i] << (u4i * 8)) + u4MemLen);
    }

    u4BufLen = (UINT32)*pi4_len;

    Printf("\n<NVM initial header Upgrade>:The Data length is u4MemLen = %d\n", u4MemLen);
    Printf("\n  EEPROM Size is 0x19000 \n"); //100k for initial header
    Printf("\n  Buf Length is 0x%x \n", u4BufLen);

    if (u4MemLen > 0x19000)
    {
        Printf("\n<NVM initial header Upgrade>:The Buf  Size  ERROR!!!!\n");
        return DLM_OPEN_FILE_FAILED;
    }

    if (u4BufLen != ((u4MemLen * 2) + 4))
    {
        Printf("\n<NVM initial header Upgrade>:The Buf Length=%d  ERROR!!!!\n", u4BufLen);
        return DLM_OPEN_FILE_FAILED;
    }
    ...
    for (u4i = 0; u4i < u4MemLen; u4i++)
    {
        ...
        EEPROM_Header_Read(FALSE, (UINT64)(u4i), (UINT32) &uiTmp, 1);
        
        uiTmp = ((uiTmp & (~pauc_buffer[u4i+u4MaskIndex])) | 
                 (pauc_buffer[u4DataIndex + u4i]& pauc_buffer[u4i + u4MaskIndex]));
        EEPROM_Header_Write(FALSE, (UINT64)(u4i), (UINT32) &uiTmp, 1);
        
        Printf("<_LdrEepInitHeaderUpgrade>: Offset:  0x%2x;  Write Data: 0x%2x\n", u4i, uiTmp);
    }

    EEPROM_Header_Read(FALSE, (UINT64)4, (UINT32) &uiEraseMode, 1);

    if(uiEraseMode == 0x4E) //N-> no need to erase header
    {
        Printf("<_LdrEepInitHeaderUpgrade>: Only vlidate init Header\n");
    }
    else
    {
        for (u4i = 0; u4i < 600; u4i++)
        {
		    ui4EraseAddress = u4i*512;
			//Printf("<_LdrEepInitHeaderUpgrade>: Header Erease Offset: %d\n", ui4EraseAddress);
            EEPROM_Header_Write(TRUE, (UINT64)(ui4EraseAddress), (UINT32)uiEraseHeader, 512);
        }
		Printf("<_LdrEepInitHeaderUpgrade>: Header Erease Done\n");
    }
    Printf("\n<NVM initial header Upgrade>:DONE!!!!\n");
    return DLM_OK;
}
#endif
...
void _LdrUpgradeDlmCbFunc(UINT32 u4BufSize)
{
	...
	#ifdef CC_EEP2EMMC_DUAL_BANK_SUPPORT
    x_dlm_reg_append_callback("nvih", u4BufSize,  _LdrEepInitHeaderUpgrade);
	#endif
...
static void FlashPQGetVersion(CHAR *szCurVer)
{
    ...
    if (DRVCUST_InitGet(eFlagFlashPqEnable))
    {
	    ...
	    {
            #ifdef CC_EMMC_BOOT
	        Loader_ReadMsdc(DRVCUST_InitGet(eFlashPqBasePartition),
	            u8PqBaseAddr,
...
#ifdef CC_MTK_LOADER
    #ifdef CC_EMMC_BOOT
	Loader_ReadMsdc(u1AQPartition, FLASH_AQ_VER_OFFSET, (VOID*)szCurVer, FLASH_AQ_VER_LENGTH);
    #else
