...
#include "maple_drv/maple_pin_setup.h"
#include "ostg_if.h"
...
#include "ir_if.h"
#include "maple_drv/sirc_ir_tbl.h"
#endif
...
#ifndef IR_BZ
//for BZ model, these keys are removed
...
static LDR_DATA_T *prLoaderLogoData;
...
static BOOL bWakeupReasonRequestInit = TRUE;
...
static UINT32 u4GetLEDBlinkCode(SELF_DIAG_TABLE eErrorLog)
{
    ...
    if (eErrorLog.u4MainPowerError)
    {
        u4LEDBlinkTime = 2;
    }
    else if (eErrorLog.u4DcAlertError || eErrorLog.u4AudError || eErrorLog.u4TuDemodError ||
		     eErrorLog.u44kBeOutError || eErrorLog.u44kBeError || eErrorLog.u4HdmiEqError)
    {
        u4LEDBlinkTime = 3;
    }
    else if (eErrorLog.u4LdError || eErrorLog.u4VledError)
    {
        u4LEDBlinkTime = 4;
    }
    else if (eErrorLog.u4TconError || eErrorLog.u4PanelIdError|| eErrorLog.u4FRCTCI2CError)
    {
        u4LEDBlinkTime = 5;
    }
    else if (eErrorLog.u4BacklightError)
    {
        u4LEDBlinkTime = 6;
    }
    else if (eErrorLog.u4BacklightError)
...
static BOOL CustomCheckShutdown(void)
{
    ...
    {   
        ...
        if (u1Val & CUSTOM_SHUTDOWN_ERROR)
        {
			...
			CustomDisableStandbyLEDCtrl(FALSE);
...
#if defined(CC_SONY_MT7662)
#if defined(CONFIG_MT53XX_NATIVE_GPIO)
void Sony_BtGpioInt_Callback(INT32 i4Gpio, BOOL fgStatus)
{
	INT32 myValue = 0;
    //vIO32Write4B(PDWNC_ARM_INTEN, u4IO32Read4B(PDWNC_ARM_INTEN) & ~(0x04)); //OPCTRL2 enable interrupt
    //Printf("PURPLEARROW, GPIO send keyV2\n");
	
    if (u4IO32Read4B(0xf0028074) & (0x4000)) //gpio214 is high
    {
        myValue = 0;
        GPIO_Output(214, &myValue);
        myValue = 1;
        GPIO_Output(215, &myValue);
    }
    else
    {
        myValue = 1;
...
#if defined(CC_SONY_MT7662)
#if defined(CONFIG_MT53XX_NATIVE_GPIO)
	//GPIO_Reg(OPCTRL(2),GPIO_TYPE_INTR_BOTH,Sony_BtGpioInt_Callback);
#endif
#endif
    return;
}
...
void PDWNC_Custom_PowerResume(void)
{
	...
	i4Value = PDWNC_IsSuspend();
	GPIO_Output(SUSPEND_GPIO_NUMBER,&i4Value);
	...
	{
		Printf("%s: DTV EEPROM read failure.\n",__FUNCTION__);
	}  
...
void PDWNC_Custom_ReadWakeupReason(UINT32* pu4Reason)
{
    ...
    UINT32 u4Val;
	#if defined(CC_MT5890)
	UINT32 u4Val1,u4GpioNum;
	#endif
    DTVCFG_T rDtvCfg;
    UINT32 u4KeypadPowerKeyPressed = 0;  

    if (!bWakeupReasonRequestInit)
    {
	...
	u4Val1 = _PDWNC_ReadWakeupStatus(1);  
	...
	#if defined(CC_MT5890)
	else if(!PDWNC_GetWakeupGpioNum(&u4GpioNum))
	{
		//Printf("Get Wakeup Gpio Num : %d .\n",u4GpioNum);
		UINT32 u4Gpio;
		if(DRVCUST_InitQuery(eWakeupOnWLanGpio, (UINT32 *)&u4Gpio) == 0)
		{
			if(u4GpioNum == u4Gpio)
			{
				*pu4Reason = (UINT32)PDWNC_WAKE_UP_REASON_WIFI;
				return;
			}
		}
		if(DRVCUST_InitQuery(eBTWakeupGpio, (UINT32 *)&u4Gpio) == 0)
		{
			if(u4GpioNum == u4Gpio)
			{
				*pu4Reason = (UINT32)PDWNC_WAKE_UP_REASON_BT;
				return;
			}
		}	
	}
            ...
            if (PDWNC_T8032Cmd(&rCmd, &rResponse) == 0)
            {
                ...
                case CUSTOM_ACTIVE_HOTEL_MODE:
                    *pu4Reason = (UINT32)PDWNC_WAKE_UP_REASON_CUSTOM_3;
                    ...
                    break;
                case CUSTOM_EXIT_HOTEL_MODE: //joe_20140328
    ...
    bWakeupReasonRequestInit = FALSE;
...
static void _reset_mhl(void)
{
    //INT32 i4Ret = 0;
    INT32 i4ValTmp = 0;
    //UINT8 u1ValTmp = 0;

    UNUSED(GPIO_Output(204, &i4ValTmp));
    x_thread_delay(10);
    i4ValTmp = (i4ValTmp) ? 0 : 1;
    UNUSED(GPIO_Output(204, &i4ValTmp));
    x_thread_delay(2);
...
/******************************************************/     
	...
	vIO32WriteFldAlign(PDWNC_OPDELAY, WAKEUP_DISABLE_SEC_CNT(1),FLD_OPWRSB_DELAY);
#endif
...
static void LOADER_Custom_2K_TimerIsr(void)
{
    ...
    {        
        if(u4Count != 0)
        {
            ...
            LDR_DynamicLogoDisplay(prLoaderLogoData, u4Count);
			#endif
			#endif
			u4Count=0;
        }
...
static void LOADER_Custom_4K_TimerIsr(void)
{
    if (BIM_IsIrqPending(VECTOR_T1))
    {
        D_LED_TYPE eLedType;
...
// u4Val = 0;
        ...
        //GPIO_Input(USB0_OC_GPIO);
#endif

#if 0//def NDEBUG
        EEPROM_Read(CUSTOM_UART_SELECTION_ADDR, (UINT32)&u4Val, 1);
		Printf("Read UART mode---Check Addr 0x6c8 = 0x%x\n",(UINT8)u4Val);
      ...
      //  BACKLT_SetDimming(BACKLT_GetDimmingFrequency(), (UINT32)0x80);        
	    ...
	    if((u4Val&0x00000001) == 0x00000001)
	    {
	...
	UNUSED(PDWNC_Custom_GetCurrentState(&u4PowerState));

    Printf("LOADER_Custom_EnvInitFunc Get state = 0x%d!!\n",u4PowerState);
	
	if (((u4Value == PDWNC_WAKE_UP_REASON_AC_POWER) && (u4PowerState != STATE_POWER_ON)) || 
		(CustomPCBAQueryCountry() == DRV_CUSTOM_COUNTRY_COL))
    {
        UNUSED(EEPDTV_GetCfg(&rDtvCfg));
	    rDtvCfg.u1Flags4 |= (UINT8)DTVCFG_FLAG4_QUIET_HOT_BOOT;
	    prLoaderData->rDtvCfg.u1Flags4 |= (UINT8)DTVCFG_FLAG4_QUIET_HOT_BOOT;
	    UNUSED(EEPDTV_SetCfg(&rDtvCfg));
    }
#endif
...
static void Custom_InitGPIOTimer(void)
{
    ...
    {
      _pfnCustomNextIsr = RegisterIrqHandler(LOADER_Custom_2K_TimerIsr);
    }
	else
	{
	  _pfnCustomNextIsr = RegisterIrqHandler(LOADER_Custom_4K_TimerIsr);
	}
