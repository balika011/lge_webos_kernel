...
#define PANEL_I2C_SDA_GPIO   GPIO(93)
#define PANEL_I2C_SCL_GPIO   GPIO(94)
#define CI_DET				 0xffffffff
...
enum
{
   E_POWER_SEQ_T33,//panel power duration when read panel edid
   E_POWER_SEQ_T37,//time from panel power on to panel edid read start
   E_POWER_SEQ_T34,//time betwwen the 1st panel power off to next time panel power on
   E_POWER_SEQ_T35,//time from panel power on to LVDS on
   E_POWER_SEQ_T36,//time from LVDS ON to BL on
   E_POWER_SEQ_T41,//time from BL on to DIMMER output
   E_POWER_SEQ_T42=E_POWER_SEQ_T41,
   E_POWER_SEQ_T39,//time from BL off to LVDS off
   E_POWER_SEQ_T38,//time from LVDS off to Panel power off
};
...
#define POWER_SEQ_DO_REAL_DELAY(power_seq_idx) \
( \
{ \
    UINT32 u4RealDelay=0;\
	UINT32 u4TargetDelay=CustomQueryPowerSequenceDelay(power_seq_idx);\
	UINT32 u4start_tick=CustomQueryPowerSequenceStart(power_seq_idx);\
    UINT32 u4elapsedTm=(x_os_get_sys_tick()-u4start_tick)*x_os_get_sys_tick_period();\
    if(u4start_tick && (u4elapsedTm<u4TargetDelay))\
	 {\
	    u4RealDelay=u4TargetDelay-u4elapsedTm;\
		x_thread_delay(u4RealDelay); \
	 }\
}\
)
