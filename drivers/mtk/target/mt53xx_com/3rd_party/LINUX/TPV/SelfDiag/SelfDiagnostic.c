...
extern void PDWNC_Custom_DetermineNextState(UINT32 u4CurrentState, UINT32 u4PowerEvent,
...
static BOOL fgAudAmpError = FALSE;
//static UINT8 u1AudAmpErrorCount = 0;

void CustomAudioAMPErrorHandler(INT32 i4Gpio, BOOL fgStatus)
{
    UNUSED(i4Gpio);
    UNUSED(fgStatus);
    fgAudAmpError = TRUE;
...
static void SelfDiagHandle(void *pvArg)
{
    ...
    UINT32 gpio;
	UINT32 value;
    UINT32 u4ActValue = SELF_DIAG_ACT_REBOOT; 
    ...
    while (_bfgSelfDiagGo)
    {
        ...
        // Detect D_AUD_ERROR 
        CHK_SELF_DIAG_PAUSE();
        if ((_eSelfDiagEnable.u2AudError == SELF_DIAG_ENABLE)&&(fgAudAmpError == true)) 
        {
            u4Value = GPIO_GetIn(SELF_DIAG_AUD_AMP_ERROR_DET);
            LOG(9, "SELF_DIAG_AUD_AMP_ERROR_DET = %d\n", u4Value);
            CHK_SELF_DIAG(fgAudAmpError, _u4DetectAudTimeCnt,
...
void SelfDiagPause(void)
{
    ...
    if(_hSelfDiagTimer != NULL_HANDLE)
      {
        UNUSED(x_timer_stop(_hSelfDiagTimer));
      }
    // Disable Audio AMP interrupt    
    //GPIO_Reg(SELF_DIAG_AUD_AMP_ERROR_DET, GPIO_TYPE_NONE, 
    //    NULL);
}
...
void SelfDiagStart(D_SYSTEM_STATE eState)
{
    ...
    // Init Audio AMP interrupt
    if(_eSelfDiagEnable.u2AudError == SELF_DIAG_ENABLE)
	{
	VERIFY(GPIO_Reg(SELF_DIAG_AUD_AMP_ERROR_DET, GPIO_TYPE_INTR_FALL,CustomAudioAMPErrorHandler)>= 0);
	}
