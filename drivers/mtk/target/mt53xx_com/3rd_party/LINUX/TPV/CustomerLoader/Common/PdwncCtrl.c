...
{
#ifndef CC_MT5880
    UINT32 u4HDMIHPD0GPIO;
#endif
    UINT32 u4HDMIHPD1GPIO,u4HDMIHPD2GPIO,u4HDMIHPD3GPIO;
...
static void _enable_cbus(BOOL bEn) //Denny 2013/01/07 add
{
    INT32 i4Ret = 0;
    UINT8 u1ValTmp = 0;
    ...
    i4Ret = SIF_X_Read( 4, 0x120, 0xD0, 1, 0x87, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
    ...
    i4Ret = SIF_X_Write( 4, 0x120, 0xD0, 1 , 0x87, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
    ...
    i4Ret = SIF_X_Read( 4, 0x120, 0xD0, 1, 0x76, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
    ...
    i4Ret = SIF_X_Write( 4, 0x120, 0xD0, 1 , 0x76, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
    ...
    i4Ret = SIF_X_Read( 4, 0x120, 0xD0, 1, 0x73, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
    ...
    i4Ret = SIF_X_Write( 4, 0x120, 0xD0, 1 , 0x73, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
...
#ifdef CC_SUPPORT_MHL
static void _enabe_terminal(BOOL bEn) //Denny 2013/01/10 add
{
    INT32 i4Ret = 0;
    UINT8 u1ValTmp = 0;

    i4Ret = SIF_X_Read( 4, 0x120, 0xD0, 1, 0x70, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
    ...
    i4Ret = SIF_X_Write( 4, 0x120, 0xD0, 1 , 0x70, & u1ValTmp, 1);
    if (i4Ret == 0)
    {
...
#endif
static void _reset_mhl(void) //Denny 2013/01/07 modify
{
    //INT32 i4Ret = 0;
    INT32 i4ValTmp = 0;
    //UINT8 u1ValTmp = 0;
...
void PDWNC_Custom_SetupPowerDown(UINT32 u4PowerDownEvent, UINT32 *pu4PowerDownEn)
{
    ...
    //Denny 2012/12/06 add to reset MHL SiI1292
    _reset_mhl();
...
void PDWNC_Custom_EnterNextState(UINT32 u4NextState)
{
    ...
    if(u4Val == 0)
    {
        _enabe_terminal(FALSE); //Denny 2013/01/07 add
...
#ifdef CC_NO_LOG
void LOADER_CheckToDisableUARTPinmux(void)
{
    Printf("Read UART mode---Check Addr 0x6c8\n");
    UINT8 u1Val = 0;
    EEPROM_Read(CUSTOM_UART_SELECTION_ADDR, (UINT32)&u1Val, 1);
    if(u1Val == UART_SELECTION_SILENT ||
       u1Val == UART_SELECTION_HOTEL_SONY_STANDARD ||
       u1Val == UART_SELECTION_HOTEL_SONY_VDA ||
       u1Val == UART_SELECTION_HOTEL_SONY_LOCATEL)
    {
        Printf("Turn off UART log!\n");
        BSP_PinSet(PIN_U0TX, 1);
    }
}
void LOADER_CheckToEnableUART(void)
{
    UINT8 u1Val = 0;
    EEPROM_Read(CUSTOM_UART_SELECTION_ADDR, (UINT32)&u1Val, 1);
    if(u1Val != UART_SELECTION_SILENT &&
       ...
       u1Val != UART_SELECTION_HOTEL_SONY_LOCATEL)
    {
        IO_WRITE32(RS232_BASE, 0x18, 2); // risc active mode.
    }
}
#endif
#endif /* CC_MTK_LOADER */
...
void LOADER_Custom_EnvInitFunc(void *prLdrData, UINT32 fgValidFlag)
{
    ...
    if (fgValidFlag == 0)
    {
#if defined(CC_MTK_LOADER) && defined(CC_NO_LOG)
        LOADER_CheckToEnableUART();
#endif
...
// u4Val = 0;
    ...
    /* Reset MHL */          // Denny 2013/01/07 add
    //if (PDWNC_ReadWakeupReason() == PDWNC_WAKE_UP_REASON_AC_POWER)
    {
        _reset_mhl();
    }
