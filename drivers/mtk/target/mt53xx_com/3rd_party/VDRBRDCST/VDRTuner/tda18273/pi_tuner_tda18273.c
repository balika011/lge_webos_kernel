...
#define NXP_TDA18273_ADDRESS           ((UINT8)  0xC6) //zhilin_20121210: for sony lcct 2k13 sipanda tuner porting
#else
#define NXP_TDA18273_ADDRESS           ((UINT8)  0xC0)
#endif

#define C_TDA18273_IF_FREQUENCY          ((UINT16)  3250)  /* kHz */
//#define C_TDA18273_IF_FREQUENCY_ANA      ((UINT16)  38900)  /* kHz */
//#define C_TDA18273_IF_FREQUENCY_L1       ((UINT16)  33900)  /* kHz */
#define C_TDA18273_LO_DIVIDER_STEP       ((UINT32) 1000)  /* Hz */
#define C_TDA18273_LO_DIVIDER_STEP_ANA   ((UINT16)  1000)  /* Hz */
#define C_NXP_TDA18273_STR                        "NXP_TDA18273 2010-01-01\n 2012-12-28\n v1.15"
...
#define C_TDA18273_IF_FREQUENCY_ATSC_64QAM  ((UINT16)  3250) /*kHz*/  
#define C_TDA18273_IF_FREQUENCY_ISDBT    	((UINT16)  3250) /*kHz*/  
#define C_TDA18273_IF_FREQUENCY_DTMB 		((UINT16)  4000) /*kHz*/  

#define C_TDA18273_SAW_BW              SAW_BW_8M
#define C_TDA18273_TOP_SET_ANA         ((UINT8)   0x0A)

//for display signal level
#define C_U8_TDA18273_AGC_IF_LVL_MAX     0xc2  //no signal
#define C_U8_TDA18273_AGC_IF_LVL_MIN     0xa8  //-70dbm input
...
//MT5135 PRA 
#define C_TDA18273_AGC_IF_SLP             0.125 
#define C_TDA18273_AGC_IF_INT             -0.5
#define C_TDA18273_AGC_IF_MAX             0
#define C_TDA18273_AGC_IF_MIN            -1


#define C_U8_TDA18273_AGC_IF_SLP          (INT8)(C_TDA18273_AGC_IF_SLP *64 )
#define C_U8_TDA18273_AGC_IF_INT          (INT8)(C_TDA18273_AGC_IF_INT *16 )
#define C_U8_TDA18273_AGC_IF_MAX          (INT8)(C_TDA18273_AGC_IF_MAX *128)
#define C_U8_TDA18273_AGC_IF_MIN          (INT8)(C_TDA18273_AGC_IF_MIN *128)




static UINT8* pTDA18273EqNormal[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN] = {NULL};
static UINT8* pTDA18273EqWeak[MOD_ANA_TYPE_END - MOD_ANA_TYPE_BEGIN]= {NULL};
...
#define DRO_LP_CFG03 19
#define DRO_LP_CFG04 20

#define PGA_INDEX  0x06
    ...
    )
{
	...
	//for display signal level
	pEUCtx->m_ifAgc_lvl_max = (UINT8)C_U8_TDA18273_AGC_IF_LVL_MAX;
	pEUCtx->m_ifAgc_lvl_min = (UINT8)C_U8_TDA18273_AGC_IF_LVL_MIN;
	...
	pUSCtx->u2LO_Step = 100;  //*10 = 1KHz
  	...
  	pTDA18273EqNormal[MOD_PAL_BG - MOD_ANA_TYPE_BEGIN]=EQTDA18273B;
	pTDA18273EqNormal[MOD_PAL_DK - MOD_ANA_TYPE_BEGIN]=EQTDA18273DK;
	pTDA18273EqNormal[MOD_PAL_I - MOD_ANA_TYPE_BEGIN]=EQTDA18273I;
	pTDA18273EqNormal[MOD_SECAM_L - MOD_ANA_TYPE_BEGIN]=EQTDA18273L;
	pTDA18273EqNormal[MOD_SECAM_L1 - MOD_ANA_TYPE_BEGIN]=EQTDA18273L1;
	pTDA18273EqNormal[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]=EQTDA18273M;

	pTDA18273EqWeak[MOD_PAL_BG - MOD_ANA_TYPE_BEGIN]=EQTDA18273BGWeak;
	pTDA18273EqWeak[MOD_PAL_DK - MOD_ANA_TYPE_BEGIN]=EQTDA18273DKWeak;
	pTDA18273EqWeak[MOD_PAL_I - MOD_ANA_TYPE_BEGIN]=EQTDA18273IWeak;
	pTDA18273EqWeak[MOD_SECAM_L - MOD_ANA_TYPE_BEGIN]=EQTDA18273LWeak;
	pTDA18273EqWeak[MOD_SECAM_L1 - MOD_ANA_TYPE_BEGIN]=EQTDA18273L1Weak;
	pTDA18273EqWeak[MOD_NTSC_M - MOD_ANA_TYPE_BEGIN]=EQTDA18273MWeak;
	
	mcDBG_MSG_L2(("TDA18273_TunerInit\n"));
    ...
    )
{
 ...
 //  TuningParam.m_BandWidth = pTCtx->specific_member.eu_ctx.m_SAW_BW;
   ...
   mcDBG_MSG_L2((C_NXP_TDA18273_STR " Set Freq %d Hz\n",Freq));
  ...
  if(!fgTunerInit)
  {
   ...
   /* TDA18273 Driver initialization  */
    ...
    if(err == TM_OK)
   		{   
   		mcDBG_MSG_INFO(("tmbslTDA18273_Open OK\n"));
		/* TDA18273 Hardware initialization */
		err = tmbslTDA18273_HwInit(0);
   		}
   	 else
		mcDBG_MSG_INFO(("tmbslTDA18273_Open failed\n"));
	  if(err == TM_OK)
	  	{
	  	err=tmbslTDA18273_SetInternalVsync(0,TRUE);
	  	}
	  else
	  	{
	  	mcDBG_MSG_INFO(("tmbslTDA18273_HwInit failed\n"));
	  	}
		...
		mcDBG_MSG_INFO(("tmbslTDA18273_SetInternalVsync failed\n"));
	 
	 if(err == TM_OK)
	 	{ 	
		mcDBG_MSG_INFO(("tmbslTDA18273_SetStandardMode OK\n"));
		err=tmbslTDA18273_Write(0,&gTDA18273_Reg_Reference_Byte__XTout,0x00,Bus_NoRead);//disable 16MHZ XOUT 
	 	}
	 else
	   mcDBG_MSG_INFO(("tmbslTDA18273_SetStandardMode fail\n"));
	 
	 if(err == TM_OK)
	 	{
	 	mcDBG_MSG_INFO(("TDA18273 disable 16MHZ XOUT OK\n"));
	 	}
	 else	 	
	    mcDBG_MSG_INFO(("TDA18273 disable 16MHZ XOUT fail\n"));
	 fgTunerInit=TRUE;
...
if (Freq < (C_TDA18273_FREQ_DBOUND_LOWER) || 
 ...
 if( Mode == MOD_ATSC_8VSB) //digital reception
   ...
   {
	stdMode = TDA18273_ISDBT_6MHz;
	pTCtx->u2IF_Freq =C_TDA18273_IF_FREQUENCY_ISDBT;
   }
 else if(Mode ==MOD_DTMB)
   {
	stdMode = TDA18273_DMBT_8MHz;
	pTCtx->u2IF_Freq =C_TDA18273_IF_FREQUENCY_DTMB;
   }
 ...
 */
//-----------------------------------------------------------------------------
static VOID  TDA18273_SetSawBw(ITUNER_CTX_T * pTCtx, UINT8 sawbw)
{
 pTCtx->specific_member.eu_ctx.m_SAW_BW=sawbw;
 mcDBG_MSG_L2(("TDA18273_SetSawBw=%d\n",sawbw));
...
UINT32 TDA18273_GetSSI_DVBT(ITUNER_CTX_T* pTunerCtx)
{
    INT16 Power_Ref[] =
    {
        -93,
    ...
    /* Get Power_Rec Value */
	tmbslTDA18273_GetPowerLevel(0, &ucPower_rec_dbuv); /*wenming 11-12-9:api return in 0.5 dbuv term*/
...
UINT32 TDA18273_GetSSI_DVBC(ITUNER_CTX_T* pTunerCtx)
{
    INT16 Power_Ref[] =
    {
        -82,//16QAM
...
//-----------------------------------------------------------------------------
/** TDA18273_SetSSICondition
 *  Set SSI Condition for TH2603
 *  @param
 *  @retval void
 */
//-----------------------------------------------------------------------------
void TDA18273_SetSSICondition(ITUNER_CTX_T* pTunerCtx, VOID * pInOutVal)
{
...
INT16 TDA18273_TunerOP(ITUNER_CTX_T * pTCtx, ITUNEROP_T eOperation, UINT8 SetVal,  VOID * pInOutVal){
	...
	case itGetTunerRelReg://Get ATD Tuner Rel register setting
		{
			...
			UINT8 u1PRATar,u1Dagc,u1PRAConf,u1DagcTargetLevel_H,u1DagcTargetLevel_L,u1DroLpCfg03,u1DroLpCfg04;
			...
			//PRA target	
			  ...
			  arrAtdRelReg[DRO_LP_CFG03].u1RegVal=u1DroLpCfg03;
			  arrAtdRelReg[DRO_LP_CFG04].u1RegVal=u1DroLpCfg04;
    ...
    case itGetEqScriptNormal:
		...
		if((MOD_PAL_BG - MOD_ANA_TYPE_BEGIN) == SetVal)
		{
			...
			{ 
				pTDA18273EqNormal[SetVal] = EQTDA18273B;
			}
			else /*PAL G system*/
			{
				pTDA18273EqNormal[SetVal] = EQTDA18273G;
			}
		}
        *(UINT8**)pInOutVal = pTDA18273EqNormal[SetVal];
        break;
    case itGetEqScriptWeak: 
        *(UINT8**)pInOutVal = pTDA18273EqWeak[SetVal];
        break;
	case itSetSawBw:
		{
		TDA18273_SetSawBw(pTCtx,SetVal);
		}
		break;
    case itSetIF:
    	break;
	...
	case itSetSSICond:
    TDA18273_SetSSICondition(pTCtx, pInOutVal);
    break;
	case itGetSSIIndex:   /*wenming 11-12-9:0 for DVBT SSI, 1 for DVBC SSI*/
	{ if (SetVal==0)
		*((INT32*)pInOutVal) = TDA18273_GetSSI_DVBT(pTCtx);
     else
		 *((INT32*)pInOutVal) = TDA18273_GetSSI_DVBC(pTCtx);
	}
	 break;
	case itSetFineRF:
		{
		//call new api do fine-tune for nxp18273
		INT8 step=*(INT8* )pInOutVal;
		if(TM_OK!=tmbslTDA18273_SetFineRF(0,step))
		{
	...
	case itGetSignalLevel:
	{
	 ...
	 tmbslTDA18273_GetPowerLevel(0, &ucPower_rec_dbuv);
   ...
   case itGetPGAIdx://Get PGA register setting
   	...
   	break;
    default:
        mcDBG_MSG_L2(("NO case !Not implement in NXP18273 Tuner.\n"));
...
UInt32 ReadLen, UInt8* pData)
{
    ...
    /* ...*/
		...
		if(ucSPIRead(NXP_TDA18273_ADDRESS, *pAddr, pData, ReadLen) == 0)   /*wenming 110920*/
#else// tuner use i2c 
		if(ucI2cRead(NXP_TDA18273_ADDRESS, *pAddr, pData, ReadLen) == 0)   /*wenming 110920*/
#endif
		{
			return TM_OK;
		}
...
#ifdef TUNER_USE_SPI //spi-i2c control tuner 		
	if(ucSPIWriteOnly(NXP_TDA18273_ADDRESS,acBuffer,WriteLen+1)==0)
#else
	if(ucI2cWrite(NXP_TDA18273_ADDRESS,acBuffer[0],acBuffer+1,WriteLen)==0) 
#endif
