...
#define INTERRUPT_CBUS_INT_FLAG 0x4000000
...
#if ENABLE_CBUS
#define MCBUS_DIS_INT() vIO32Write1B(PDWNC_T8032_INTEN + 3, u1IO32Read1B(PDWNC_T8032_INTEN + 3) & 0xfb)
#define MCBUS_EN_INT() vIO32Write1B(PDWNC_T8032_INTEN + 3, u1IO32Read1B(PDWNC_T8032_INTEN + 3) | 0x04)
...
#define MReadGPIOInt() u4IO32Read4B(PDWNC_EXTINTSTA)
#define MReadGPIOIntEn() u4IO32Read4B(PDWNC_T8032_EXTINTEN)
#define MClrGPIOInt(x)  vIO32Write4B(PDWNC_EXTINTCLR, (x))

//just 5881
#define MOIRI_DIS_INT() vRegWriteFldAlign(PDWNC_T8032_EXTINTEN, 0, FLD_T8032_EXTIO_OIRI_INTEN)
#define MOIRI_EN_INT()  vRegWriteFldAlign(PDWNC_T8032_EXTINTEN, 1, FLD_T8032_EXTIO_OIRI_INTEN)
#define MOIRI_GPIO_BOTH_EDGE_SETUP() vRegWriteFldAlign(PDWNC_EXINT2ED, 1, FLD_EXINT_2ED_OIRI);\
						             vRegWriteFldAlign(PDWNC_EXINTLEV, 0, FLD_EXINT_LEV_OIRI);\
						             vRegWriteFldAlign(PDWNC_EXINTPOL, 0, FLD_EXINT_POL_OIRI);
...
#if defined(CC_MT5389)||defined(CC_MT5396)||defined(CC_MT5398)||defined(CC_MT5368)||defined(CC_MT5880)||defined(CC_MT5881)	//MT5389_TODO
    #define SetSERVOEN(x)
...
//channel c,d,e,f always on for por
#define MEnableSrvad(x) vRegWriteFldAlign(PDWNC_SRVCFG1, 0xF000 | RegReadFldAlign(PDWNC_SRVCFG1, FLD_SRVCH_EN) |(0x1 << x), FLD_SRVCH_EN)
//#define MSelectSrvad(x) vRegWriteFldAlign(PDWNC_ADCCFG0, x, FLD_SRV_SEL)
#define CORE_POWER_SRV (8)//(6)	//it is change to 8 from 5368/5396/5389--> add channel B since 5881  //(0xB + 1)
#define MUpdateSrvad(x) //servo ad can not write value ---vRegWriteFldAlign(PDWNC_ADOUT0 + ((x) << 2), 0, FLD_ADOUT0);//only support ad ch 0~9
...
#define SwitchUartPD_PORT0() vRegWrite1B(PDWNC_PINMUX0+2,((u1RegRead1B(PDWNC_PINMUX0+2) & 0xfc)|0x01))  //UART0 pad as T8032 log //bit 16~17
    ...
    #endif
#elif defined(CC_MT5880)
#define SwitchUartPD_PORT0() vRegWrite1B(PDWNC_PINMUX0+2,((u1RegRead1B(PDWNC_PINMUX0+2) & 0xf8)|0x01))  //UART0 pad as T8032 log //bit 16~17
    #if T8032_OCCUPIED_UART0_AT_NORMAL_POWER
        #define SwitchBack_PORT0()
    #else
        #define SwitchBack_PORT0() vRegWrite1B(PDWNC_PINMUX0+2, u1RegRead1B(PDWNC_PINMUX0+2) & 0xf8) //UART0 pad as ARM log 
    #endif
#elif defined(CC_MT5881)
#define SwitchUartPD_PORT0() vRegWrite2B(PDWNC_PINMUX1+2,((u2RegRead2B(PDWNC_PINMUX1+2) & 0xf03f)|0x480))  //UART0 pad as T8032 log //bit 16~17
    #if T8032_OCCUPIED_UART0_AT_NORMAL_POWER
        #define SwitchBack_PORT0()
    #else
        #define SwitchBack_PORT0() vRegWrite2B(PDWNC_PINMUX1+2,(u2RegRead2B(PDWNC_PINMUX1+2) & 0xf03f)) //UART0 pad as ARM log 
    #endif
