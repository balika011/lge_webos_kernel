...
UINT8 u1IRCheckData[DATA_LEN];
UINT8 u1BitMode;
UINT8 u1IrPulseTimeUnit;
UINT8 u1Update;

UINT32 u4WakeupKey;

UINT8 _IRRX_RCMMCheckInvalid(UINT32 u4Time,UINT32 u4Length)
{
	#if 1
	UINT32 u4Range =0;
...
void vTask6InitRCMMWakeup(void)
{
	UINT8 i;
	u4WakeupKey = 0;
	u1BitMode = 0;
...
void vTask6RCMMWakeup(void)
{
	UINT8 u1Invalid = FALSE;
	UINT8 i = 0;
	UINT32 u1DataMode = 0;
	UINT32 u4Time =0;
	if((MGET_ARM_POWER_STATE() != ARM_POWER_OFF)||(!u1Update))
	{	
		return;
	}
	MIR_DIS_INT();
	//check sync
	#if 0
	u4Time = u1IrPulseTimeUnit * u1IRCheckData[0];
	u1Invalid = _IRRX_RCMMCheckInvalid(u4Time,HEADER_HIGH_LEN);
	LogSD("sync0",u1IRCheckData[0]);

	u4Time = u1IrPulseTimeUnit * u1IRCheckData[1];
	u1Invalid = u1Invalid || _IRRX_RCMMCheckInvalid(u4Time,HEADER_LOW_LEN);
	LogSD("sync1",u1IRCheckData[1]);
	#endif
	//check data
	for(i=0;i<u1BitMode;)
	{
		u4Time = (u4WakeupKey>>i)&0x3;
		if(u4Time == 0)
		{
			u1DataMode = DATA_LOW00_LEN;
		}
		else if(u4Time == 1)
		{
			u1DataMode = DATA_LOW01_LEN;
		}
		else if(u4Time == 2)
		{
			u1DataMode = DATA_LOW10_LEN;
		}
		else if(u4Time == 3)
		{
			u1DataMode = DATA_LOW11_LEN;
		}
		LogSD("index",u1BitMode +1 -i);
		u4Time = u1IrPulseTimeUnit * u1IRCheckData[u1BitMode +1 -i];
		u1Invalid = u1Invalid || _IRRX_RCMMCheckInvalid(u4Time,u1DataMode);
		u4Time = u1IrPulseTimeUnit * u1IRCheckData[u1BitMode -i];//[u1BitMode +2 -i -1]
		u1Invalid = u1Invalid || _IRRX_RCMMCheckInvalid(u4Time,DATA_HIGHT_LEN);
		i = i + 2;
