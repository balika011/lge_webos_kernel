...
#if RCMM_SUPPORT
#define DATA_LEN 26
extern UINT8 u1Update;
extern UINT8 u1IRCheckData[DATA_LEN];
#endif
/*
#if defined SIR_TASK
#if SIR_TASK
...
void vPDWNC_ISR(void) small interrupt EX0INT  
{
	...
	if(u4PDWNC_IntStatus & INTERRUPT_IR_INT_FLAG)
	{
		...
		if(u1Update == FALSE)	
		{
			...
			{
				u1IRCheckData[i] = u1RegRead1B(PDWNC_CHKDATA0 + i*4);
			}
		}
		u1Update = TRUE;
		#else
	...
	u4PDWNC_IntStatus = MReadGPIOInt() & MReadGPIOIntEn();
	if(u4PDWNC_IntStatus)
	{
		vCustGpioIsr(u4PDWNC_IntStatus);
	}
	MClrGPIOInt(u4PDWNC_IntStatus);
#endif
