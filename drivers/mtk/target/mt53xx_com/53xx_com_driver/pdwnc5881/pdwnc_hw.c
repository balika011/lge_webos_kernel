...
#define VGAWAKEUP_VRANGE_L 0x1c
	...
	#define VGAWAKEUP_HRANGE_U	(0x199)//409
	#define VGAWAKEUP_HRANGE_L	(0x1c)//28
...
static void _PdwncHwIsr(UINT16 u2VctId)
{
	...
	// gpio init 
	u4ValGpio = u4IO32Read4B(PDWNC_EXTINTSTA) & u4IO32Read4B(PDWNC_EXTINTEN);
...
//-----------------------------------------------------------------------------
#if defined(CC_MT5398) || defined(CC_MT5880)
void _PDWNC_SetupEthernetWakeup(void)
{
    UINT32 u4Val;
    UINT32 MACADDRESS_H;
    UINT32 MACADDRESS_L;
...
#endif   
        PDWNC_INTEN(PDWNC_INTNO_AUX_INT_0);
        // Install GPIO ISR
        PDWNC_GPIO_Resume();
...
#if 0
void PDWNC_SetSuspendDelay(UINT8 u1Delay20ms)
{
    SysSuspendDelay = u1Delay20ms;
}

UINT8 PDWNC_GetSuspendDelay()
{
    return SysSuspendDelay;
}
#endif
...
UINT32 PDWNC_EnterSuspend(void)
{
	...
	//PDWNC_EnterPowerDown(u4SuspendEn,PDWNC_POWER_DOWN_UNKNOWN);
