...
//-----------------------------------------------------------------------------
/**   _PDWNC_RequestT8032EnterSuspend request T8032 to entern standby mode
 */
//-----------------------------------------------------------------------------
INT32 _PDWNC_RequestT8032EnterSuspend(void)
{
    INT32 i4Ret = 0;
    PDWNC_T8032_CMD_T rCmd;
	#if 0
...
//david : Switch operation of  CEC, UART, ENGINE are not necessary in MT5387/63
        ...
        u4DurationTime = (UINT32)DRVCUST_InitGet(eSuspendModeDurationTime);  // suspend mode duration time (0~65535)

        rCmd.au1Data[0] = (UINT8)u4DurationTime & 0xff;
        rCmd.au1Data[1] = (UINT8)((u4DurationTime>>8) & 0xff);
        rCmd.au1Data[2] = (UINT8)((u4DurationTime>>16) & 0xff);
        rCmd.au1Data[3] = (UINT8)((u4DurationTime>>24) & 0xff);
		
        Printf("eSuspendModeDurationTime = 0x%x,0x%x,0x%x,0x%x\n",rCmd.au1Data[3],rCmd.au1Data[2],rCmd.au1Data[1],rCmd.au1Data[0]);

        // Signal 8032 to enter standby
        i4Ret = _T8032Cmd(&rCmd, NULL);

        LOG(7, "Signal T8032 to enter suspend!\n");    
		#if 0 //remove this delay to have a test
...
//#endif    
    ...
    u4Reason = u4Val = PDWNC_ReadWakeupReason();
...
//#endif
...
#ifdef CC_SUPPORT_STR
        u1FrequencyPerStandby = (UINT32)DRVCUST_InitGet(eSuspendModeFrequencyPerStandby);  // how many times suspend and 1 standby (0~255) 
		rCmd.au1Data[0] = u1FrequencyPerStandby & 0xff;
        rCmd.au1Data[1] = (u1FrequencyPerStandby>>8) & 0xff;
		rCmd.au1Data[2] = (u1FrequencyPerStandby>>16) & 0xff;
		rCmd.au1Data[3] = (u1FrequencyPerStandby>>24) & 0xff;
#endif
    ...
    if (u4Reason == PDWNC_WAKE_UP_REASON_AC_POWER)
    {
#ifdef CC_ENABLE_REBOOT_REASON
#if !defined(CC_MT5398)
        // AC on, clear data. Must do it here.
        ...
        if (!DRVCUST_InitQuery(eT8032RebootOffset, &u4RebootOffset) && !DRVCUST_InitQuery(eT8032RebootSize, &u4RebootSize))
        {
            ...
            buf = x_mem_alloc(u4RebootSize);
            x_memset(buf, 0, sizeof(buf));
            PDWNC_Write_T8032_XData(u4RebootOffset, sizeof(buf), buf);
            x_mem_free(buf);
        }
#endif
