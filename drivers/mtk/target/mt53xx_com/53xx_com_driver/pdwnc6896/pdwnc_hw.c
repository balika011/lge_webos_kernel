...
//#ifdef CUST_SETQUIETBOOTREASON_1
...
static STR2KEY_T _aStrToKeyArray[] = {

	{ "vga", PDWNC_WAKE_UP_REASON_VGA },
    { "rtc", PDWNC_WAKE_UP_REASON_RTC },
    { "keypad", PDWNC_WAKE_UP_REASON_KEYPAD },
    { "ir", PDWNC_WAKE_UP_REASON_IRRC },
    { "uart", PDWNC_WAKE_UP_REASON_UART },
    { "ac_power_on", PDWNC_WAKE_UP_REASON_AC_POWER },
    { "hdmi", PDWNC_WAKE_UP_REASON_HDMI },
    { "uart_normal", PDWNC_WAKE_UP_REASON_UART_NORMAL },
    { "0", PDWNC_WAKE_UP_REASON_RC_DIGIT_0 },
    { "1", PDWNC_WAKE_UP_REASON_RC_DIGIT_1 },
    { "2", PDWNC_WAKE_UP_REASON_RC_DIGIT_2 },
    { "3", PDWNC_WAKE_UP_REASON_RC_DIGIT_3 },
    { "4", PDWNC_WAKE_UP_REASON_RC_DIGIT_4 },
    { "5", PDWNC_WAKE_UP_REASON_RC_DIGIT_5 },
    { "6", PDWNC_WAKE_UP_REASON_RC_DIGIT_6 },
    { "7", PDWNC_WAKE_UP_REASON_RC_DIGIT_7 },
    { "8", PDWNC_WAKE_UP_REASON_RC_DIGIT_8 },
    { "9", PDWNC_WAKE_UP_REASON_RC_DIGIT_9 },
    { "prg_up", PDWNC_WAKE_UP_REASON_RC_PRG_UP },
    { "prg_down", PDWNC_WAKE_UP_REASON_RC_PRG_DOWN },
    { "inp_src", PDWNC_WAKE_UP_REASON_RC_INP_SRC },
	{ "rc_analog", PDWNC_WAKE_UP_REASON_RC_ANALOG },
	{ "rc_digital", PDWNC_WAKE_UP_REASON_RC_DIGITAL },
	{ "rc_digital_analog", PDWNC_WAKE_UP_REASON_RC_DIGITAL_ANALOG },    
    { "fp_prg_up", PDWNC_WAKE_UP_REASON_FP_PRG_UP },
    { "fp_prg_down", PDWNC_WAKE_UP_REASON_FP_PRG_DOWN },
    { "fp_inp_src", PDWNC_WAKE_UP_REASON_FP_INP_SRC },
	{ "dvd", PDWNC_WAKE_UP_REASON_DVD },
    { "rtc_special", PDWNC_WAKE_UP_REASON_RTC_SPECIAL },
    { "watchdog", PDWNC_WAKE_UP_REASON_WATCHDOG },
	{ "scart", PDWNC_WAKE_UP_REASON_SCART },
	{ "ethernet", PDWNC_WAKE_UP_REASON_ETHERNET },
	{ "suspend", PDWNC_WAKE_UP_REASON_RESUME_FROM_SUSPEND },    
    { "custom_1", PDWNC_WAKE_UP_REASON_CUSTOM_1 },
    { "custom_2", PDWNC_WAKE_UP_REASON_CUSTOM_2 },
    { "custom_3", PDWNC_WAKE_UP_REASON_CUSTOM_3 },
    { "custom_4", PDWNC_WAKE_UP_REASON_CUSTOM_4 },
    { NULL, 0xFFFFFFFF },
};
//#endif

BOOL PDWNC_CUST_CheckQuietboot(void)
{
	 

    INT32       i4_ret, i, u4CustQuietBootSupportEeprom;
    DTVCFG_T    rDtvCfg;
    CHAR *      u4CustWakeupreason1;
    CHAR *      u4CustWakeupreason2;
    CHAR *      u4CustWakeupreason3;

    if(0 == (DRVCUST_InitQuery(eCustQuietBootSupportEeprom, (UINT32 *)&u4CustQuietBootSupportEeprom)))
    {
        ...
        if (i4_ret!=0)
        {
            Printf("PDWNC_CUST_ReadWakeupReason: DTV EEPROM read failure.\n");
            LOG(0, "%s(%d) DTV EEPROM read failure.\n", __FILE__, __LINE__);
            return FALSE;
        } 
        
        if(u4CustQuietBootSupportEeprom == 1)  // support eeprom quiet boot flag
        {
            if (!(rDtvCfg.u1Flags2 & DTVCFG_FLAG2_QUIET_BOOT))
            {
                Printf("EEPROM setting is not Quiet Boot!\n");
                return FALSE;
            }
        }
    }
    ...
    //#ifdef CUST_SETQUIETBOOTREASON_1
	if(0 == (DRVCUST_InitQuery(eCustWakeupreason1, (UINT32 *)&u4CustWakeupreason1)))
    {
    	for (i=0; ((_aStrToKeyArray[i].szString != NULL) &&
            (x_strcmp((CHAR *)u4CustWakeupreason1, _aStrToKeyArray[i].szString)!=0)); i++) { }
    	
    	if((_aStrToKeyArray[i].szString == NULL))
    	{
    	    Printf("CUST_SETQUIETBOOTREASON_1 is not in PDWNC_WAKE_UP_REASON key array!\n");
    		return FALSE;
    	}

        if(PDWNC_ReadWakeupReason() == _aStrToKeyArray[i].u4Key )
        {
    		LOG(0, "%s(%d) PDWNC_CUST_ReadWakeupReason: Quiet Boot\n", __FILE__, __LINE__);
    		return TRUE;
        }
    }
    else
    {
        Printf("CUST_SETQUIETBOOTREASON_1 is NULL\n");
        return FALSE;
    }

    //#ifdef CUST_SETQUIETBOOTREASON_2
    if(0 == (DRVCUST_InitQuery(eCustWakeupreason2, (UINT32 *)&u4CustWakeupreason2)))
    {
    	for (i=0; ((_aStrToKeyArray[i].szString != NULL) &&
    		(x_strcmp((CHAR *)u4CustWakeupreason2, _aStrToKeyArray[i].szString)!=0)); i++) { }
    	
    	if((_aStrToKeyArray[i].szString == NULL))
		{
		    Printf("CUST_SETQUIETBOOTREASON_2 is not in PDWNC_WAKE_UP_REASON key array!\n");
			return FALSE;
		}
    ...
    {
        Printf("CUST_SETQUIETBOOTREASON_2 is NULL\n");
        return FALSE;
    }

    //#ifdef CUST_SETQUIETBOOTREASON_3
    if(0 == (DRVCUST_InitQuery(eCustWakeupreason3, (UINT32 *)&u4CustWakeupreason3)))
	{
		for (i=0; ((_aStrToKeyArray[i].szString != NULL) &&
			(x_strcmp((CHAR *)u4CustWakeupreason3, _aStrToKeyArray[i].szString)!=0)); i++) { }
		
		if((_aStrToKeyArray[i].szString == NULL))
		{
		    Printf("CUST_SETQUIETBOOTREASON_3 is not in PDWNC_WAKE_UP_REASON key array!\n");
			return FALSE;
		}
    ...
    {
        Printf("CUST_SETQUIETBOOTREASON_3 is NULL\n");
        return FALSE;
    }

    Printf("All PDWNC_WAKE_UP_REASON Keys mismatch\n");
	return FALSE;
}
//#endif
#endif
...
static void _PDWNC_RebootHandle(void *pvArg)
{
    ...
    u4CPUPLL_FBDIV = IO32ReadFldAlign(REG_CPUPLL_AA_CFG0, FLD_RG_CPUPLL_FBDIV) + 4;
...
//    vDrvSYNCSlicerWakeup(1);
	...
	if((u4PowerDownEn & WAK_VGA) == 0)   // if(1)//if ((u4PowerDownEn & WAK_VGA) == 0)
    {
		...
		vIO32WriteFldAlign(PDWNC_VGACFG2, 0x0, FLD_RG_SYNC_PWD); 
...
#if defined(CC_SUPPORT_STR) && !defined(CC_MTK_LOADER)
...
void PDWNC_pm_resume(void)
{
	...
	LOG(0,"/////////////////////////////go in PDWNC_pm_resume(void)\n");
...
#if defined(CC_SUPPORT_STR) && !defined(CC_MTK_LOADER)
//-----------------------------------------------------------------------------
/** PDWNC_EnterPowerDown() Enter Suspend mode.
 */
//-----------------------------------------------------------------------------
void PDWNC_EnterSuspend(void)
{
    ...
    UINT32 u4PowerDownEvent = PDWNC_POWER_DOWN_UNKNOWN;
	...
	// for ypbpr not to wakeup & all the source can wakeup if vga input & can not input to _PdwncSetupVGA()
    ...
    _PDWNC_RequestT8032EnterSuspend();  // enter suspend mode

}

//EXTERN INT32 IRRX_SendMtkIr(UINT32 u4Key);
#ifdef LINUX_TURNKEY_SOLUTION
extern unsigned int skip_send_key; // drop key during suspend
#define IR_set_skip_send_key(x)   skip_send_key=x;
#else
#define IR_set_skip_send_key(x)
#endif
static UINT32 SysSuspendMode=0;
void PDWNC_Resume(void)
{
    IR_set_skip_send_key(0);
    // it is because 8032 consume the power key, the input driver can't get the key event
#ifndef CC_SUPPORT_STR_V1
    printk("(yjdbg) PDWNC_Resume send a fake power key\n");
    IRRX_SendMtkIr(BTN_POWER);
#else
    printk("(yjdbg) PDWNC_Resume send a fake key\n");
    IRRX_SendMtkIr(BTN_SELECT);
#endif
}

void PDWNC_SetSuspendMode(void)
{
    IR_set_skip_send_key(1);
    SysSuspendMode = 1;
}

void PDWNC_SetResumeMode(void)
{
    SysSuspendMode = 0;
}
