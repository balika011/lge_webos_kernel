...
UINT32 bIsWakeupKey(IR_DATA_TYPE irData)
{
    UINT32 i, u4IrExpEn, u4EnExpIRM;
    u4IrExpEn  = u4IO32Read4B(PDWNC_IREXP_EN);
    u4EnExpIRM = u4IO32Read4B(PDWNC_ENEXP_IRM);
    ...
    for(i=0; (i<MAX_EXP_KEY_NUM) && (u4IrExpEn&(1<<i)); i++)
    {
        //LogSD("i",i);
        //LogD(irData & u4EnExpIRM);
        //LogD(u4IO32Read4B(PDWNC_EXP_IR0 + (4*i)) & u4EnExpIRM);
        if((irData & u4EnExpIRM) == (u4IO32Read4B(PDWNC_EXP_IR0 + (4*i)) & u4EnExpIRM))
        {
            return 1;
        }
    }
...
void vIRTask_SuspendProcess(void)
{
...
//        return;    
    ...
    if(MGET_IR_WAKEUP_EN() && bIsWakeupKey(tmpIrData))   // turnkey remote power key
    {
