...
#endif   
#if defined(CC_MT5881) || defined(CC_MT5880)
		if(RegReadFldAlign(PDWNC_PIN_HDMI_CEC,HDMI_CEC_FUN) != CEC_PIN_FUNCTION )
		{
			vRegWriteFldAlign(PDWNC_PIN_HDMI_CEC,CEC_PIN_FUNCTION,HDMI_CEC_FUN);
			LogS("CEC_Init:set cec pin to cec function!!\n");
		}
#endif

    }
#if SUSPEND_TO_DRAM
    else if ((MGET_SYS_PRE_STANDBY_STATUS() == STBY_FROM_NORMAL_POWER)||(gSys_state.fgStandbyMode == SUSPEND_STATE))  //From normal to standby,only enable T8032 CEC interrupt
#else
...
#if SUSPEND_TO_DRAM
UINT8 CEC_getSuspendWakeEnStatus(void)
{
    return ui1_SuspendCecWakeen;
}
void CEC_clrSuspendWakeEnStatus(void)
{
    ui1_SuspendCecWakeen = 0;
}
#endif
