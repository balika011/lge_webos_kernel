...
#endif
#if SUSPEND_TO_DRAM
extern UINT32 u4DurationTime;
extern UINT32  u1FrequencyPerStandby;
#endif
...
//    Cmd_ReadT8032WakeUp is the COMMAND most frequestly issued from ARM, so place it at 1st decision...
    ...
    { 
        u1FrequencyPerStandby--;
        if(u1FrequencyPerStandby == 0)  // enter standby
        {
            #if UART_DBG
            LogS("EnterStandby");
            #endif   
        ...
        {   // enter suspend
            ...
            // suspend last for duration time and enter standby
            vSetTimer(WAKEUP_FROM_SUSPEND_TIMER_ID, 50);
            u4DurationTime = 0; // clear 0 
			u4DurationTime = ((UINT32)tmpCmd.au1Data[0] | ((UINT32)tmpCmd.au1Data[1] << 8) | ((UINT32)tmpCmd.au1Data[2] << 16) | ((UINT32)tmpCmd.au1Data[3] << 24));	  
#if UART_DBG
            LogS("EnterSuspend");
            LogSD("duration time",u4DurationTime);
            LogSD("u1FrequencyPerStandby",u1FrequencyPerStandby);
#endif

            // RTC : disable rtc interrupt to ARM and enable rtc interrupt to 8032
            vRegWriteFldAlign(PDWNC_ARM_INTEN, 0, FLD_RTC_INTEN);
            vRegWriteFldAlign(PDWNC_T8032_INTEN, 1, FLD_RTC_INTEN);
            
            vEnterSuspend();
        }
        
        vCMD_RESONSE(Cmd_EnterSuspend, 0, 0, 0);
    }
#endif
	#if HOTKEY_WAKEUP_SUPPORT
	else if(tmpCmd.u1Cmd == Cmd_GetIrWakeupValue)//
    {
		vCMD_RESONSE(Cmd_GetIrWakeupValue, HotKeyExp, 0, 0);
    }
	#endif
    else if(tmpCmd.u1Cmd == Cmd_EnterPowerNormal)
    {
        if(tmpCmd.u1SubCmd == Cmd_Sub_PowerStage_loader)
        {
			...
			#if HOTKEY_WAKEUP_SUPPORT
			vIrHotKeyWakeup_ExpValue();
			#endif
            
#if SUSPEND_TO_DRAM
            u1FrequencyPerStandby = 0; // clear 0 
			u1FrequencyPerStandby = ((UINT32)tmpCmd.au1Data[0] | ((UINT32)tmpCmd.au1Data[1] << 8) | ((UINT32)tmpCmd.au1Data[2] << 16) | ((UINT32)tmpCmd.au1Data[3] << 24));	  
#endif
        }
