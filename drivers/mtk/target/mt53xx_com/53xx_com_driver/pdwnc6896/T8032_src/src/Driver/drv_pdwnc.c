...
#if SUSPEND_TO_DRAM
    else if(u1state == SUSPEND_STATE)
    {
        gSys_state.fgStandbyMode = SUSPEND_STATE;   
        gSys_state.fgPowerOff = ARM_POWER_ON;
        gSys_state.fgNewIrData = NO_EVENT;        
        gSys_state.fgServiceMode = 0;
        gSys_state.fgNewCmdCome = NO_EVENT;
...
#if SUSPEND_TO_DRAM
void vWakeUpSystemFromSuspend(void)
{
    // IR : disable ir interrupt to 8032 and enable ir interrupt to ARM
    MDisableINT() ;
    MClrSysInt(0xffffffff); //clean all interrupt
    MIR_DIS_INT();
    vRegWriteFldAlign(PDWNC_ARM_INTEN, 1, FLD_IR_INTEN);
    MEnableINT() ;
    ...
    MWriteDataToARM(Cmd_WakeupARMFromSuspend); 
}

void vEnterSuspend(void)
{
#if UART_DBG    
    LogS("vEnterSuspend\n");
#endif   
    ...
    MClrSysInt(0xffffffff); //clean all interrupt
    // uart_pd bug
    vRegWriteFldAlign(PDWNC_UART_PD_INT_EN, 0, FLD_TBUFE);

#if IR_TASK   
    MIR_EN_INT();
    //MIR_DIS_HW_WAKEUP();    // 8032 will read wakeup enable register is suspend mode
    _MIR_FlushKey();
...
#if 1// RW_REG_INTERFACE	
    //vConfigSrvad(3);
#endif

    vResetSysStateVar(SUSPEND_STATE);
    ...
    vRegWrite4B(PDWNC_SHREG0, 0x2); // indicate ARM not to wakeup from suspend
...
#if SUSPEND_TO_DRAM
        u1RTCWakeupFlag = 1;  // rtc wakeup from suspend
#endif
	}
...
#if ETHERNET_TASK
    if((MReadSysIntEn() & (0x20000000)) && (MReadSysInt() & (0x20000000))) //bit29  ethernet interrupt enable/state bit
    {  
