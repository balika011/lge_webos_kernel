...
#if HOTKEY_SW_WAKEUP_SUPPORT 
UINT8 HotKeyExp;
static UINT8 vIrHotKeyWakeup_ExpValue(IR_DATA_TYPE irdata)
{
	UINT8 i;
	
	//LogSW("ChkHotKey:",irdata);
	for(i=0;i<MAX_EXP_KEY_NUM;i++)
	{
		 if(u4IO32Read4B(PDWNC_EXP_IR0 + i*4) == (UINT32)irdata)
		 {
			break;
		 }
	}
...
*/ 
  ...
  //  LogSB("BitCnt:",_MGetIRBitCnt());
	//LogSD("IR Data1:",_ReadIrDataReg());
	//LogSW("IR Data1:",irdata);
	...
	#if HOTKEY_SW_WAKEUP_SUPPORT 
	else if((gIRWakeupKeyList & T8032_IR_WAK_HOT_KEY) && (vIrHotKeyWakeup_ExpValue(irdata)<MAX_EXP_KEY_NUM))
	{
	   return 1;
	}
	#endif
...
#endif
#if HOTKEY_SW_WAKEUP_SUPPORT
      HotKeyExp=0xff;//restore the hotkey exp idx
#endif	
#if SONY_IR_DBG
...
UINT8 IsSonyPowerKey(IR_DATA_TYPE data u2key)
{
	...
	{/* 20101015, add u1WakeUp_waiting_idx = 2, since this idx will be changed to 2 while the 1st '5' comming and system will wakeup by 2nd '5' (5_) */
		...
		return 1;
#if HOTKEY_SW_WAKEUP_SUPPORT		
    else if((gIRWakeupKeyList & T8032_IR_WAK_HOT_KEY) && (vIrHotKeyWakeup_ExpValue(u2key)<MAX_EXP_KEY_NUM))
	{
	   // LogSB("HotKey!!:",HotKeyExp);
	    return 1;
	}   
#endif	
	return 0;
...
#else
			if(gSIRC_ctrl.u2PreIrRxData0 == irData)
#endif
			{
...
*/
				...
				else if((fgHotelCmdData.uRs232Data[0] == 0x82) && (fgHotelCmdData.uRs232Data[1] == 0x04) && (fgHotelCmdData.uRs232Data[2] == 0x80))
                {
                    ...
                    if((0x06 + fgHotelCmdData.uRs232Data[3]) == fgHotelCmdData.uRs232Data[4])
                    {
						...
						//LogB(fgHotelCmdData.uRs232Data[3]);
						//LogB(u1Module2AvSelectPW);
						//LogB(u1Module2AvSelectSrc);
						//LogB(u1Module2HdmiPortSel);
...
//				if(u4RegRead4B(PDWNC_RESRV0)!= 0x12345678)	
				...
				{    
					#if 1//power query in standby should not reply anything.
