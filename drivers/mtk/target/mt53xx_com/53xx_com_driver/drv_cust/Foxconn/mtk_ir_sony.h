...
#endif /* CC_MTK_LOADER */
#include "x_debug.h"
#include "Foxconn/mtk_ir_sony_tbl.h"
...
static UINT32 _IsTimeOffInvalid(UINT32 *pu4TimeSlot, UINT32 u4SlotNum)
{
    ...
    for (i=2; i<u4SlotNum; i+=2)
    {
        ...
        if ((pu4TimeSlot[i] < (g_u4TimeOffMin * _u4CpuClkMhz)) || // 0.45ms
            (pu4TimeSlot[i] > (g_u4TimeOffMax * _u4CpuClkMhz)))   // standard: 0.75ms, but Foxconn's receiver tend to have longer Toff, so use 0.65ms
        {
