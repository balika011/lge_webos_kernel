...
static void _EnableNewSeamless(UINT32 u4FbgId, UINT32 u4HSize, UINT32 u4VSize, FBM_CREATE_FBG_PAR_T *prPar);
...
static void _FbgStatus(UCHAR ucFbgId)
{
    ...
    LOG(1, "FBG(%d) Type(%d) CM(%d) Ns(%d) D(%d) F(%d) B(%d) Pool(0x%x, 0x%x) Resolution(0x%x,0x%x),Pitch(0x%x),Raster(%d),422Mode(%d)\n",
    ...
    for (ucIdx = 0; ucIdx < _arFbg[ucFbgId].ucFbNs; ucIdx++)
    {
        ...
        {
            LOG(0, "\tFB(%d) S(%d,%d) Y(0x%x) C(0x%x) PTS(0x%8x)\n",
                ucIdx,
    ...
    if(_arFbg[ucFbgId].fgEnableNewSeamless)
    {
        for (ucIdx = 0; ucIdx < _arFbg[ucFbgId].ucSeamlessFbNs; ucIdx++)
        {
            if(ucIdx < FBM_MAX_FB_NS_PER_GROUP)
            {
                LOG(1, "\tFB(%d) S(%d) O(%d) Y(0x%x) C(0x%x)\n",
                    ucIdx,
            ...
            if (_arFbg[u4FbgIdx].ucFbgId != FBM_FBG_ID_UNKNOWN && _arFbg[u4FbgIdx].u1FbgAppMode == FBM_FBG_APP_Frame_Pack_3D)
            {
                ...
                for (u4FbIdx = 0; ((u4FbIdx < _arFbg[u4ResizeFbgId].ucSeamlessFbNs)&&(u4FbIdx < FBM_MAX_FB_NS_PER_GROUP)); u4FbIdx++)
                {
                    ...
                    if (FBM_CHECK_CB_FUNC_VERIFY(_arFbmCbFunc.au4CbFunc[FBM_CB_FUNC_GFX_MEMSET], _arFbmCbFunc.au4CbFuncCRC[FBM_CB_FUNC_GFX_MEMSET]))
                    {
                        ...
                        ((UINT8*)u4Addr, FBM_FBG_1080HD_H, FBM_FBG_1080HD_V, 0x0);
                    }
                    else
                    {
                    ...
                    {
                        ((FBM_GFX_MEMSET_FUNC)_arFbmCbFunc.au4CbFunc[FBM_CB_FUNC_GFX_MEMSET])
                        ((UINT8*)u4Addr, FBM_FBG_1080HD_H, (FBM_FBG_1080HD_V >> 1), 0x80);
...
void _EnableNewSeamless(UINT32 u4FbgId, UINT32 u4HSize, UINT32 u4VSize, FBM_CREATE_FBG_PAR_T *prPar)
{
    if (!_arFbg[u4FbgId].fgEnableNewSeamless)
    {
...
#ifdef CC_ENABLE_SEAMLESS_FOR_MVC
        ...
        {
            UINT32 u4YSize;
            UINT32 u4HeightAlign = ((u4VSize + 31) & (~31));
            UINT32 u4WidthAlign =  ((u4HSize + 63) & (~64));
        ...
        // Allocate seamless working buffer.
        _arFbg[u4FbgId].u4FbResizeMemoryPoolSize = 0x600000;//FBM_EXTRA_FOR_SEAMLESS;

#if defined(CC_ENABLE_SEAMLESS_FOR_MVC) && defined(ENABLE_MULTIMEDIA)

        //_arFbg[u4FbgId].u4FbResizeMemoryPoolSize = 0x900000;//FBM_EXTRA_FOR_SEAMLESS; // 3 frame buffer for 3D.
        _arFbg[u4FbgId].u4FbResizeMemoryPoolSize = 0x600000;//FBM_EXTRA_FOR_SEAMLESS; // B2R is refined to only lock 2 frames buffer for 3D.
        
        prVdecEsInfo = _VDEC_GetEsInfo(_arFbg[u4FbgId].u1DecoderSrcId);
        if (prVdecEsInfo != NULL && prPar != NULL &&
           prVdecEsInfo->eMMSrcType == SWDMX_SRC_TYPE_NETWORK_NETFLIX /*&& prPar->u1AppMode == FBM_FBG_APP_Frame_Pack_3D*/)
        {
            _arFbg[u4FbgId].u4FbResizeMemoryPool = prFbmPool->u4Addr + prFbmPool->u4Size - _arFbg[u4FbgId].u4FbResizeMemoryPoolSize - FMB_EXTRA_FOR_SINGLE_H264_LV5;

            //if (_arFbg[u4FbgId].u1DecoderSrcId == ES0)
            if(prPar->u1AppMode == FBM_FBG_APP_Frame_Pack_3D)
            {
                _arFbg[u4FbgId].u4FbResizeMemoryPool -= _arFbg[u4FbgId].u4FbResizeMemoryPoolSize;
        ...
        _arFbg[u4FbgId].u4FbResizeMemoryPool = prFbmPool->u4Addr + prFbmPool->u4Size - _arFbg[u4FbgId].u4FbResizeMemoryPoolSize - FMB_EXTRA_FOR_SINGLE_H264_LV5;
#else
        _arFbg[u4FbgId].u4FbResizeMemoryPool = prFbmPool->u4Addr + FBM_GetRunningVdecMemSize(u4FbgId);
#endif
        }

        _arFbg[u4FbgId].ucSeamlessFbNs = (_arFbg[u4FbgId].u4FbResizeMemoryPoolSize / (u4SeamlessYSize + u4SeamlessCSize));

        LOG(1, "Seamless buffer FbNs %d.\n", _arFbg[u4FbgId].ucSeamlessFbNs);
        ...
        {
            UINT32 u4KR3DFbNs;
            // For testing. Reuse KR3D buffer.
            prFbmPool = FBM_GetPoolInfo((UINT8)FBM_POOL_TYPE_IMGRZ_3D_KR);
            u4Addr = (((prFbmPool->u4Addr) + 0x7FF) & (~0x7FF));
            
            u4KR3DFbNs = (FBM_IMGRZ_3D_KR_POOL_SIZE / (u4SeamlessYSize + u4SeamlessCSize));
            
            for (u4FbIdx = _arFbg[u4FbgId].ucSeamlessFbNs ; ((u4FbIdx < _arFbg[u4FbgId].ucSeamlessFbNs + u4KR3DFbNs)&&(u4FbIdx < FBM_MAX_FB_NS_PER_GROUP)); u4FbIdx++)
            {
                _arFbg[u4FbgId].au4ResizeAddrY[u4FbIdx] = u4Addr;
            ...
            _arFbg[u4FbgId].ucSeamlessFbNs += u4KR3DFbNs;
        }
#endif
                         ...
                         UINT32 u4HSize, UINT32 u4VSize, FBM_CREATE_FBG_PAR_T *prPar)
{
    ...
    // Check fgEnableSeamless
    _EnableNewSeamless(u4FbgId, u4HSize, u4VSize, prPar);
...
UCHAR FBM_CreateGroup(UCHAR ucFbgType, UINT32 u4VDecFmt, UINT32 u4HSize, UINT32 u4VSize)
{
    ...
    _arFbg[u4FbgId].ucFbgType = ucFbgType;
    _arFbg[u4FbgId].ucFbgId = (UCHAR)u4FbgId;
    _arFbg[u4FbgId].fgGotFirstSeqChg = FALSE;
                         ...
                         UINT32 u4VSize, FBM_CREATE_FBG_PAR_T *prPar)
{
    ...
    FBM_POOL_T* prFbmPool = NULL;
    UINT32 u4PoolSize = 0;
#if (defined(CC_FBM_TWO_FBP_SHARED_WITH_DFB)  || defined(CC_VOMX_TV_COEXIST)) && defined(CC_FBM_FULL_3D_SUPPORT)
...
#ifdef ENABLE_MULTIMEDIA
    prVdecEsInfo = _VDEC_GetEsInfo(_arFbg[ucFbgId].u1DecoderSrcId);
#endif
    u4YSize = 0;
    u4CSize = 0;
    for (u4ListIdx = 0; u4ListIdx < ucFbpListNs; u4ListIdx++)
    ...
    if (ucFbgCm == FBM_CM_420 || ucFbgCm == FBM_CM_422 || ucFbgCm == FBM_CM_444)
    {
            ...
            if (_arFbg[ucFbgId].u4VDecFmt == FBM_VDEC_H264)
            {
                ...
                switch(_arFbg[ucFbgId].ucFbgType)
                {
                    ...
                    case FBM_FBG_TYPE_TNBHD:
                        u4ReferenceNs = ((2 * (FBM_FBG_1080HD_H * FBM_FBG_1080HD_V) * FBM_FBG_H264_FHD_REFERNCE_NS) / (u4HSize * u4VSize));
                        break;
                    case FBM_FBG_TYPE_1080HD:
                        u4ReferenceNs = (((FBM_FBG_1080HD_H * FBM_FBG_1080HD_V) * FBM_FBG_TYPE_1080HD_H264_R_NS) / (u4HSize * u4VSize));                        
                        break;
                    case FBM_FBG_TYPE_720HD:
                        u4ReferenceNs = (((FBM_FBG_720HD_H * FBM_FBG_720HD_V) * FBM_FBG_TYPE_720HD_H264_R_NS) / (u4HSize * u4VSize));                                                
                        break;
                    case FBM_FBG_TYPE_PAL:
                        u4ReferenceNs = (((FBM_FBG_PAL_H * FBM_FBG_PAL_V) * FBM_FBG_TYPE_PAL_H264_R_NS) / (u4HSize * u4VSize));                                                
                        break;
                    default:
...
#ifdef CC_H264_LV5_SUPPORT_SHARE_POOL
                    if (_arFbg[ucFbgId].fgEnableH264V5Support)
                    {
                        prFbmPool = FBM_GetPoolInfo((UINT8)FBM_POOL_TYPE_MPEG);
                        u4Addr = FBM_ALIGN_MASK(prFbmPool->u4Addr, FBM_FMG_Y_ALIGMENT);
                        u4PoolSize = prFbmPool->u4Size - FBM_EXTRA_FOR_SEAMLESS;

                        u4FbNs = ((prFbmPool->u4Size - u4MVSize)/u4Size);
                    }
#endif
        ...
        if(_arFbg[ucFbgId].u4VDecFmt == FBM_VDEC_JPEG && ucFbgCm == FBM_CM_422 && prPar->u1AppMode != FBM_FBG_APP_MTIMAGE)
        {
            ...
            if (prFbmPool != NULL)
            {
                ...
                {
                    LOG(0, "Unexpected pool range of FBM_POOL_TYPE_JPG_VDP!\n");
                    ASSERT(0);
                }
            }
#endif
        ...
        else
        {
            ...
            if(_arFbg[ucFbgId].ucFbgType == FBM_FBG_TYPE_SBSHD  || _arFbg[ucFbgId].ucFbgType == FBM_FBG_TYPE_TNBHD)
            {
                ...
                {
                    u4Addr = FBM_ALIGN_MASK(prFbmPool->u4Addr, FBM_FMG_Y_ALIGMENT);
                    u4PoolSize = prFbmPool->u4Size; //include seamless buffer when full 3d.
                }
                else
                {
                ...
                ((u4FbIdx < _arFbg[ucFbgId].ucFbNs)&&(u4FbIdx < FBM_MAX_FB_NS_PER_GROUP)); u4FbIdx++)
        {
            ...
            if(_arFbg[ucFbgId].ucFbgType == FBM_FBG_TYPE_SBSHD  || _arFbg[ucFbgId].ucFbgType == FBM_FBG_TYPE_TNBHD)
            {
                if(u4PoolSize < (u4YSize+u4CSize) && prFbmPool2 != NULL)
                {
                    u4Addr = FBM_ALIGN_MASK(prFbmPool2->u4Addr, FBM_FMG_Y_ALIGMENT);
                    u4PoolSize = prFbmPool2->u4Size;
                }
                // Y
    ...
    //Handle extra reference frame, update FbNs & Fb partition here.
        ...
        if (_arFbg[ucFbgId].u4VDecFmt == FBM_VDEC_H264 && _arFbg[ucFbgId].fgEnableH264V5Support)
        {
            ...
            u4ExtraReferenceNs = ((u4PoolSize - u4MVSize)/u4Size);;
            ...
            u4ExtraReferenceNs = ((FMB_EXTRA_FOR_SINGLE_H264_LV5 - u4MVSize)/u4Size);;
            ...
            for (u4FbIdx = _arFbg[ucFbgId].ucFbNs; ((u4FbIdx < _arFbg[ucFbgId].ucFbNs + u4ExtraReferenceNs)&&(u4FbIdx < FBM_MAX_FB_NS_PER_GROUP)); u4FbIdx++)
            {
                // Y
                _arFbg[ucFbgId].au4AddrY[u4FbIdx] = u4Addr;
