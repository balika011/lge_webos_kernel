...
static BOOL _FbmVirtualCompareCRC_Picture(CHAR * pcDir,UINT32 u4Num)
{
    ...
    if ( _rB2rAutoTestFbgCfg.height > 1088 )
    {
	    _FbmVirtualCreateFbg(TRUE, PICTURE_2D, MPEG_FRAME_RATE_24);
    }
    else
    {
...
static BOOL _FbmVirtualSelectFbgConfig(UINT32 u4Pitch, UINT32 u4Width, UINT32 u4Height)
{
	...
	else if (u4Pitch <= 3840 && u4Width <= 3840 && u4Height <= 1088)
	{
		_rB2rAutoTestFbgCfg.width = 1920;
		_rB2rAutoTestFbgCfg.height = 1080;
		_rB2rAutoTestFbgCfg.active_width = u4Width;
	...
	{
		_rB2rAutoTestFbgCfg.width = 3840;
		_rB2rAutoTestFbgCfg.height = 2160;
		_rB2rAutoTestFbgCfg.active_width = u4Width;
		_rB2rAutoTestFbgCfg.active_height = u4Height;
		_rB2rAutoTestFbgCfg.pitch = u4Pitch;
		_rB2rAutoTestFbgCfg.frame_size = FBM_FBG_TYPE_2160HD_Y_SIZE;
	}
	else if (u4Pitch <= 4096 && u4Width <= 4096 && u4Height <= 2176)
	{
		_rB2rAutoTestFbgCfg.width = 4096;
		_rB2rAutoTestFbgCfg.height = 2160;
		_rB2rAutoTestFbgCfg.active_width = u4Width;
		_rB2rAutoTestFbgCfg.active_height = u4Height;
		_rB2rAutoTestFbgCfg.pitch = u4Pitch;
	...
	{
		_rB2rAutoTestFbgCfg.width = u4Width;
		_rB2rAutoTestFbgCfg.height = u4Height;
		_rB2rAutoTestFbgCfg.active_width = u4Width;
		_rB2rAutoTestFbgCfg.active_height = u4Height;
		_rB2rAutoTestFbgCfg.pitch = u4Pitch;
		_rB2rAutoTestFbgCfg.frame_size = FBM_ALIGN_MASK((u4Width * u4Height), FBM_FMG_Y_ALIGMENT);
	}
	return TRUE;
}
...
{ 
    ...
    while (_ucFbmVirtualMpegDestroy == 0)
    {
		...
		if (_i4B2rVerifyStatus == START || _i4B2rVerifyStatus == DOING)
		{
			...
			VDP_HalDoB2RAutoTestCRCReadSetting(CRC_READING_ENABLE, 0);
