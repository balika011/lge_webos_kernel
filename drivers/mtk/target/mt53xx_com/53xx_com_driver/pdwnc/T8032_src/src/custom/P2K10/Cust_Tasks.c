...
//Since we have not the BYTE1 information of P_COMP RC5 yellow/red/green, RC5 is not support MULIT RC now		
    ...
    //RC6 system 7 power code :0x03987043 (2010/01/25)
            if(u4Code == 0x03987000 && key_map_idx == IR_POWER_IDX)
                return 1;
...
#if P_ERROR_LED
PError_Code_Ctrl Phi_err_code_ctrl;
void vPSetCtrlLedEnable(void)
{
    if(Phi_err_code_ctrl.fgErrCodeLedType == 1)
    {
        SetSERVOEN((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk));
        SetSERVOPADEN((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk));
    }
    else
    {
        SetGPIOEN(Phi_err_code_ctrl.fgErrCodeLedMsk);
    }
}

//we assume the gpio is  enable before this task
static void PTurnOnCtrlLed(void)
{
    if(MGET_ARM_POWER_STATE() == ARM_POWER_OFF)
//if(1)
    {
        if(Phi_err_code_ctrl.fgErrCodeLedType == 1)
        {
            if(Phi_err_code_ctrl.fgErrCodeLedPolarity)
                SetSERVOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk));
            else
                ClrSERVOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk)); 
        }
        else
        {
            if(Phi_err_code_ctrl.fgErrCodeLedPolarity)
                SetGPIOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk));
            else
                ClrGPIOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk)); 
        }
    }
}

static void PTurnOffCtrlLed(void)
{
    if(MGET_ARM_POWER_STATE() == ARM_POWER_OFF)
//if(1)
    {
...
static void PToggleCtrlLed(void)
{
        if(MGET_ARM_POWER_STATE() == ARM_POWER_OFF)
//    if(1)
        {
            if(Phi_err_code_ctrl.fgErrCodeLedType == 1)
            {
                if(ReadSERVOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk)))
                {
                    ClrSERVOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk));
#if UART_DBG
LogS("LED 0")  ;                  
...
LogS("LED 1");                    
            ...
            {
                if(ReadGPIOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk)))
                {
                    ClrGPIOOUT((UINT8)(Phi_err_code_ctrl.fgErrCodeLedMsk));
                }
                else
...
void vPErrCodeLedReset(void)
{
    vSetTimer(P_ERR_LED_TIMER_ID, 0);
    Phi_err_code_ctrl.LED_Status = _ErrCode_Start;
}


void vTask5_P_Init_error_led_ctrl(void)
{
    memcpy(&(Phi_err_code_ctrl), 0, sizeof(PError_Code_Ctrl));
    vSetTimer(P_ERR_LED_TIMER_ID, 0);
}

void vTask5_P_WakeupProcess_error_led_ctrl(void)
{
//if (Phi_err_code_ctrl.fgErrCodeLedOn)
    if(1)
    {
        PTurnOffCtrlLed();
    }
...
void vTask5_P_StandbyProcess_error_led_ctrl(void)
{

    if((Phi_err_code_ctrl.LED_Array[0] == 0 ) && (Phi_err_code_ctrl.LED_Array[1] == 0) && (Phi_err_code_ctrl.LED_Array[2] == 0) && (Phi_err_code_ctrl.LED_Array[3] == 0) && (Phi_err_code_ctrl.LED_Array[4] == 0))
    {
        Phi_err_code_ctrl.fgErrCodeLedOn = 0;//force disable led function 
        vSetTimer(P_ERR_LED_TIMER_ID, 0);
    }
...
void vTask5_P_error_led_ctrl_state_machine(void)
{
    UINT8 i, j;
    
    if(fgIsTimerUp(P_ERR_LED_TIMER_ID) && (MGET_ARM_POWER_STATE() == ARM_POWER_OFF) && (Phi_err_code_ctrl.fgErrCodeLedOn))
    {
        switch (Phi_err_code_ctrl.LED_Status)
        {
             case _ErrCode_Start: //5000MS  only first time....
                Phi_err_code_ctrl.LED_Status = _ErrCode_Stop2;
                vSetTimer(P_ERR_LED_TIMER_ID, CNT_5000ms);
                PTurnOffCtrlLed();
#if UART_DBG
LogS("_ErrCode_Start");
#endif
                return;//break;
                case _ErrCode_Stop2: //3000MS
                {
#if UART_DBG
                    LogS("_ErrCode_Stop2");
#endif                    
                    Phi_err_code_ctrl.LED_ptr = 0;

                    while((Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]==0)&&(Phi_err_code_ctrl.LED_ptr<(LEDcodeMax-1)))
                    {
                        Phi_err_code_ctrl.LED_ptr++;
                    }

                    if((Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]==0)&&(Phi_err_code_ctrl.LED_ptr==(LEDcodeMax-1)))
                    {
                        Phi_err_code_ctrl.LED_ptr = 0;//none error code......
                        Phi_err_code_ctrl.LED_Status = _ErrCode_Stop1;
                        vSetTimer(P_ERR_LED_TIMER_ID, CNT_3000ms);
                        return;
                    }

                    if(Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]!=0)
                    {
                        i = ((Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]) >>4);
                        if( i == 0)
                        {
                            j = ((Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]) &0x0F);
                            Phi_err_code_ctrl.LED_cycle = (j*2)-1;
                            Phi_err_code_ctrl.LED_Status = _ErrCode_Byte01;
                            vSetTimer(P_ERR_LED_TIMER_ID, CNT_250ms);//Phi_err_code_ctrl.LED_Cnt = CNT_250ms;
                        }
                        else
                        {
                            Phi_err_code_ctrl.LED_cycle = (i*2)-1;
                            Phi_err_code_ctrl.LED_Status = _ErrCode_Byte10;
                            vSetTimer(P_ERR_LED_TIMER_ID, CNT_750ms);//Phi_err_code_ctrl.LED_Cnt = CNT_750ms;
                        }
                    }
                    break;

                    case _ErrCode_Byte10:
                    // 750ms
#if UART_DBG
        LogS("_ErrCode_Byte10");
#endif                    
                    Phi_err_code_ctrl.LED_cycle--;
                    if( Phi_err_code_ctrl.LED_cycle == 0)
                    {

                        j = ((Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]) &0x0F);
                        if( j == 0)
                        {
                            Phi_err_code_ctrl.LED_Status = _ErrCode_ByteGap;
                            vSetTimer(P_ERR_LED_TIMER_ID, CNT_3000ms);//Phi_err_code_ctrl.LED_Cnt = CNT_3000ms;
                        }
                        else
                        {
                            Phi_err_code_ctrl.LED_cycle = (j*2)-1;
                            Phi_err_code_ctrl.LED_Status = _ErrCode_NibbleGap;
                            vSetTimer(P_ERR_LED_TIMER_ID, CNT_1500ms);//Phi_err_code_ctrl.LED_Cnt = CNT_1500ms;
                        }
                    }
                    else
                        vSetTimer(P_ERR_LED_TIMER_ID, CNT_750ms);//Phi_err_code_ctrl.LED_Cnt = CNT_750ms;
                ...
                break;
                case _ErrCode_ByteGap://3000ms
                {
#if UART_DBG
                    LogS("_ErrCode_ByteGap");
#endif                    
                    Phi_err_code_ctrl.LED_ptr++;
                    if( Phi_err_code_ctrl.LED_ptr>=LEDcodeMax)
                    {
                        Phi_err_code_ctrl.LED_Status = _ErrCode_Stop1;
                        vSetTimer(P_ERR_LED_TIMER_ID, CNT_3000ms);//                        Phi_err_code_ctrl.LED_Cnt = CNT_3000ms;
                        Phi_err_code_ctrl.LED_ptr=0;
                ...
                case _ErrCode_NibbleGap:// 1500ms
                {
#if UART_DBG
        LogS("_ErrCode_NibbleGap");
#endif                 
                    
                    j = ((Phi_err_code_ctrl.LED_Array[Phi_err_code_ctrl.LED_ptr]) &0x0F);
                    if( j == 0)
                    Phi_err_code_ctrl.LED_cycle=1;
                    else
                    Phi_err_code_ctrl.LED_cycle = (j*2)-1;
                ...
                case _ErrCode_Byte01://250ms
                {
#if UART_DBG
        LogS("_ErrCode_Byte01");
#endif                       
                    Phi_err_code_ctrl.LED_cycle--;
                    if( Phi_err_code_ctrl.LED_cycle == 0)
                    {
        ...
        LogS("_ErrCode_Stop1");
                ...
                default:
                {
//                    Phi_err_code_ctrl.LED_Cnt = 0;
                    Phi_err_code_ctrl.LED_Status = 0;
//                    PTurnOffCtrlLed();
                    vSetTimer(P_ERR_LED_TIMER_ID, 0);
                }
                break;                
