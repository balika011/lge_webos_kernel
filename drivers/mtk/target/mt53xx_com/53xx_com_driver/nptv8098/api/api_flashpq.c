...
{    
    UINT16 i, j;
    UINT32 u4Temp, u4Addr=0, u4Value, u4Mask, u4Pos = 0;
...
{
    #ifndef CC_MTK_LOADER     
    STORG_SyncRead(DRVCUST_OptGet(aFlashPqFlashInfo[bIndex].dwFlashBasePartition) ,u8Offset, (VOID*)u4MemPtr, u4MemLen);
	#else
	...
	#else
    Loader_ReadNandFlash((UINT32)DRVCUST_InitGet(aFlashPqFlashInfo[bIndex].dwFlashBasePartition), u8Offset, (VOID*)u4MemPtr, u4MemLen);
    #endif
	#endif
}
...
{
#ifndef CUST_SOEM_DRV    //Salvia no need SCEbySource
...
UINT8 bApiFlashPqUpdateQtyMinMax(UINT8 u1VidPath, UINT8 bIndex)
{
    ...
    if (((aFlashPqFileHead[bIndex].bRes & RES_SUPPORT_QTY_MAX_MIN) == 0) ||
        ...
        (DRVCUST_OptGet(aFlashPqSWInfo[bIndex].dwSwQtyMinMaxEnable) == 0))
    {
...
UINT8 bApiFlashPqSetRawData(UINT16 u2RegNum, UINT32 *pRegTbl, UINT8* pDataTbl)
{
    ...
    if((pRegTbl==NULL)||(pDataTbl==NULL))
    {
        Printf("bApiFlashPqSetRawData, invalid parameter!!!\n");
        return SV_FALSE;
    }
...
UINT8 bApiFlashPqUpdateRawData(void)
{
    ...
    //Check Register data Case Index
    if(u2CaseIndex >= pRawDataSubHead[u2TblIndex].u2CaseNum)
    {
        LOG(0, "FlashPQ[%d] Register Data Case Index Error! %d >= %d\n",
            u1Index,u2CaseIndex,pRawDataSubHead[u2TblIndex].u2CaseNum);
        return (SV_FALSE);
    }
