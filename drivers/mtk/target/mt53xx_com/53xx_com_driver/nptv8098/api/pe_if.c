...
#ifdef CUST_SOEM_DRV
extern BOOL vApiEepromGammaFlag(void);
extern void DRVCUST_EepromLoadGammaTable(void);
#endif
...
UINT16 u2LEDMotionDuty = 0xFF;
UINT16 u2PreBacklightDuty = 0;
#endif
// === EXTERN VARIABLE ====================================================================
...
void vApiGammaInit(void)
{
    ...
    #ifdef CUST_SOEM_DRV
        #if 0    // ---Temp init gamma from flash pq for Sipadan.   dpa
        {
            EXTERN UINT16 u2GammaDecodedTable[3][257];
            EXTERN UINT16 wHwGammaCurve[3][257];
            vDrvGammaDecodeTable();
            x_memcpy(wHwGammaCurve, u2GammaDecodedTable, sizeof(u2GammaDecodedTable));
            ...
            && bRegisterList[bVdpArg][REGISTER_APPLY_TIME] == bApplyTime)
        {
            switch (bVdpArg)
            {
                ...
                case PE_EVENT_SCALER_PE:
                    #if defined(CC_MT5398)
                    vDrvUsPeiOnOff(_bSuperResolutionStats);
                    #endif
                    break;   
                case PE_EVENT_OD:
                     //vDrvODLoadODTable();
...
void vApiVideoDemoMode(UINT8 bmode, Region DemoRec, UINT8 bDEMOModules)
{
    ...
    #ifdef CUST_SOEM_DRV
	bDEMOModules = bDEMOModules & 0xBF; 						 // disable SCE demo.
	#endif
    //Set Demo modules	
...
void vHalSetSCEPat(UINT8 bOnOff, UINT16 wYdata)
{
    ...
    #ifndef CUST_SOEM_DRV
    if (IO32ReadFldAlign(MATRIX_00, MATRIX_PQ_ADJ) || ISSCALERRGB(SV_VP_MAIN))
    {
...
{
    if (bColor < SV_RGB_RGB)
    {
        vDrvSetColorFilter(bPath, bColor, bOnOff);
    }
}
...
void vHalVideoGammaLoadTable(void)
{
    ...
    {    
        ...
        DRVCUST_SET_GAMMA();
#endif
   	//#ifndef PQ_7K8K_SUPPORT
    vDrvSetGammaWritePingPongFlag(SV_TRUE);   
	Printf("++++ vDrvSetGammaWritePingPongFlag =true \n");
	//#endif
...
void vHalVideoGammaTblInit(void)
{
	...
	if(vApiEepromGammaFlag())
	{
	   DRVCUST_EepromLoadGammaTable();
	}
	#endif
    #endif
...
void vApiVideoAllPQItemOnOff(UINT8 bOnOff)
{
    ...
    {        
        ...
        // Scalar
        vHalVideoSuperResolutionOnOff(SV_VP_MAIN, SV_OFF);
...
{    
    EXTERN void DRVCUST_SetLEDMotion(UINT8 u1OnOff);
    LOG(3, "vHalVideoLEDMotion = %d\n",OnOff);
	if (OnOff)
	{
		u2PreBacklightDuty=0;
		u1LEDMotionFlag = SV_ON;
        DRVCUST_SetLEDMotion(SV_ON);
	}
	else
	{
		u1LEDMotionFlag = SV_OFF;
        DRVCUST_SetLEDMotion(SV_OFF);
	}	
}

void vHalVideoLEDMotionDuty(UINT16 u2BacklightDuty)
{
	LOG(4, "vHalVideoLEDMotionDuty() u2BacklightDuty = %d\n",u2BacklightDuty);

	u2LEDMotionDuty = (u2BacklightDuty > 450)? 450 : u2BacklightDuty;
}
#endif
