...
#ifdef CUST_SOEM_DRV     // linux system use app_driver_table_share.c not vid_quality_cust.c
#define APP_DRV_TABLE_SHARE
#endif
...
EXTERN UINT32 FlashPanel_GetPQIndex(UINT32 u4Idx);
#endif
...
PE_UI_RANGE_T aUiQtyItemMinMax[PE_ARG_NS] =
{
	// BEGIN   				END   					 	TYPE
	...
	#else
	{QUALITY_BRIGHTNESS,	QUALITY_BRIGHTNESS,    	R_QTYMAP | R_SCAN | R_NOSIG},				// PE_ARG_BRIGHTNESS
	{QUALITY_CONTRAST,  	QUALITY_CONTRAST,    	R_QTYMAP | R_SCAN | R_NOSIG},				// PE_ARG_CONTRAST
	{QUALITY_HUE,  			QUALITY_HUE,      		R_QTYMAP | R_SCAN | R_NOSIG},				// PE_ARG_HUE
	{QUALITY_SATURATION,  	QUALITY_SATURATION,  	R_QTYMAP | R_SCAN | R_NOSIG},				// PE_ARG_SATURATION
	#endif
	...
	#else
	{QUALITY_BS_BEGIN, 		QUALITY_BS_END, 		R_QTYMAP | R_MAIN | R_NOSIG | R_OFF},				// PE_ARG_BLUE_STRETCH,
	#endif
...
UINT8 gMPEGNRHigh;
#ifndef DEMO_BOARD
EXTERN void DRVCUST_BrightPlusProc(void);
#endif
#endif
...
{
	UINT8 i, j;
	UINT8 bCustomSrcTypTmg, bBegin, bEnd,bCustomSCETypTmg, bSCEBegin, bSCEEnd;
...
/**
 * @brief Translate source type/timing index into MHL input  index.
 * @param bSrcTypTmg SOURCE_TYPE_TIMING_DIGI_SD_I_60 ~ SOURCE_TYPE_TIMING_DIGI_PC
 * @return value is SOURCE_TYPE_TIMING_MHL_SD_I_60 ~ SOURCE_TYPE_TIMING_MHL_PC
 *
 */
#ifdef CUST_SOEM_DRV
UINT8 bApiRemappingMHLSrc(UINT8 bSrcTypTmg)
{
	//if (_bIsMhlDevice==1)     //DPA temp mark
	{
		switch(bSrcTypTmg)
		{
		case SOURCE_TYPE_TIMING_DIGI_SD_I_50:
			return SOURCE_TYPE_TIMING_MHL_SD_I_50;
			
		case SOURCE_TYPE_TIMING_DIGI_SD_P_50:
			return SOURCE_TYPE_TIMING_MHL_SD_P_50;
			
		case SOURCE_TYPE_TIMING_DIGI_SD_I_60:
			return SOURCE_TYPE_TIMING_MHL_SD_I_60;
			
		case SOURCE_TYPE_TIMING_DIGI_SD_P_60:
			return SOURCE_TYPE_TIMING_MHL_SD_P_60;
		
		case SOURCE_TYPE_TIMING_DIGI_720P_24:
			return SOURCE_TYPE_TIMING_MHL_720P_24;
			
    	case SOURCE_TYPE_TIMING_DIGI_720P_25:
			return SOURCE_TYPE_TIMING_MHL_720P_25;
			
    	case SOURCE_TYPE_TIMING_DIGI_720P_50:
			return SOURCE_TYPE_TIMING_MHL_720P_50;
			
   		case SOURCE_TYPE_TIMING_DIGI_720P_60:
			return SOURCE_TYPE_TIMING_MHL_720P_60;
		
		case SOURCE_TYPE_TIMING_DIGI_1080I_50:
			return SOURCE_TYPE_TIMING_MHL_1080I_50;
			
    	case SOURCE_TYPE_TIMING_DIGI_1080I_60:
			return SOURCE_TYPE_TIMING_MHL_1080I_60;
			
    	case SOURCE_TYPE_TIMING_DIGI_1080P_24:
			return SOURCE_TYPE_TIMING_MHL_1080P_24;
			
    	case SOURCE_TYPE_TIMING_DIGI_1080P_25:
			return SOURCE_TYPE_TIMING_MHL_1080P_25;
			
    	case SOURCE_TYPE_TIMING_DIGI_1080P_50:
			return SOURCE_TYPE_TIMING_MHL_1080P_50;
			
    	case SOURCE_TYPE_TIMING_DIGI_1080P_60:
			return SOURCE_TYPE_TIMING_MHL_1080P_60;
                     ...
                     INT16 const i2Dft, INT16 const i2Cur)
{
    ...
    UiRange = GetQtyRange(u1SrcTypTmg, u2QtyItem);
#ifdef CUST_SOEM_DRV
		bHdtv = (wDrvVideoInputHeight(SV_VP_MAIN) >= 720);
		if(gMPEGNRHigh)
		{
			if (bHdtv)
			{
				gSHPGain = 	IO32ReadFldAlign(TDSHPALL_ADAP_GAIN_00, TDSHARPALL_GAIN_HD);
			}
			else
			{
				gSHPGain = 	IO32ReadFldAlign(TDSHPALL_ADAP_GAIN_00, TDSHARPALL_GAIN_SD);
			}
			if (u2QtyItem == QUALITY_2DNR_NR_SNR_MANUAL_AUTO_RATIO)
			{
				return UiRange.i4Max;
			}
			if ((u2QtyItem >= QUALITY_2DNR_SNR_MAX_RTO_LVL_0)&&(u2QtyItem <= QUALITY_2DNR_SNR_MAX_RTO_LVL_9))
			{
				return UiRange.i4Max;
			}
		}
		...
		vIO32WriteFldAlign(TDSHPALL_ADAP_GAIN_00,gSHPGain ,TDSHARPALL_GAIN);
					
				
	        	

	if (u2QtyItem == QUALITY_TDSHARPALL_GAIN)
	{
		UiRange.i4Dft = UiRange.i4Dft * gSHPGain /100;
	}
#endif
    ...
    else
    {
        ...
        else
        {
            ...
            {
                ReturnVal = (UINT16) (MAX(0,(i2Cur - i2Min)) * (UiRange.i4Dft - UiRange.i4Min) /
    ...
    for (u4Item = 0; u4Item < VID_QA_ELEM_NUM; u4Item++)
    {
        switch (at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].e_vid_qa_type)
        {
			...
			case VID_QA_TYPE_MPEG_NR:
				u4PeArg = PE_ARG_MPEG_NR;
				break;   
			case VID_QA_TYPE_BLUE_STRETCH:
				u4PeArg = PE_ARG_BLUE_STRETCH;
				break;
            #endif
            default:
                continue;                       
        ...
        for (i4VisIdx = 0; i4VisIdx < EVDP_VIS_MAX; i4VisIdx++)
        {
            ...
            #ifdef CUST_SOEM_DRV
            if(u4PeArg == PE_ARG_R_GAIN || u4PeArg == PE_ARG_R_OFFSET )
            {
                aUiVqMinMaxDft[i4VisIdx][u4PeArg+1].i4Min = at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].i4_min;
                aUiVqMinMaxDft[i4VisIdx][u4PeArg+1].i4Max = at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].i4_max;
                aUiVqMinMaxDft[i4VisIdx][u4PeArg+1].i4Dft = at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].i4_default;
            
                aUiVqMinMaxDft[i4VisIdx][u4PeArg+2].i4Min = at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].i4_min;
                aUiVqMinMaxDft[i4VisIdx][u4PeArg+2].i4Max = at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].i4_max;
                aUiVqMinMaxDft[i4VisIdx][u4PeArg+2].i4Dft = at_drv_vdp_vid_qa_min_max_dft[u1PicMode][u4Item].i4_default;
            }
            #endif
        }
    }    
...
void vUi_3D_NR(UINT8 u1VidPath, UINT8 bOnOff, UINT8 u1Auto, INT16 i2Cur)
{
    if (u1VidPath == SV_VP_MAIN)
    {
        ...
        #ifdef CUST_SOEM_DRV
        if(bGetSignalType(SV_VP_MAIN)==SV_ST_TV)  //Masoem, ATV RNR Auto follow TNR
        {
...
{
	#ifdef CUST_SOEM_DRV
	UINT8 PQIdex = FlashPanel_GetPQIndex(0);
	#endif
    ...
    #ifdef CUST_SOEM_DRV
    if((PQIdex == 6) || (PQIdex == 5))// 32FHD/32WXGA panel, modify the LCN_LSAT_GAIN from 0x48 -> 0x3A
    {
    //	wWriteQualityTable(QUALITY_LCN_LSAT_GAIN, 0x3A);
    }
    #endif
    vDrvSCEParam();
...
{
#ifdef CUST_SOEM_DRV
    //UINT8 bInPhotoFrameMode = DRVCUST_GetPhotoFrameMode(); //diable AL when in Photo Frame mode
    //vHalVideoAdaptiveLumaOnOff(u1VidPath, (bInPhotoFrameMode ? SV_OFF : SV_ON));
    vHalVideoAdaptiveLumaOnOff(u1VidPath, SV_ON);
    wWriteQualityTable(QUALITY_AL_ON_1_OFF_0,bOnOff);
...
{
    #ifdef CUST_SOEM_DRV
    //Requirement from Asri [Brightness +1 when off && smart pic mode=cinema/sports/Standard/Photo_std]
    if(u1VidPath==SV_VP_MAIN)
    {
        UINT8 bSmartPicIndex = vApiFlashPqGetSmartPicId();
        if((bOnOff==SV_OFF)&&((bSmartPicIndex==4)||(bSmartPicIndex==5)||
            (bSmartPicIndex==2)||(bSmartPicIndex==7)))
        {
            vHalVideoBrightness(u1VidPath, (wReadQualityTable(QUALITY_BRIGHTNESS)+1));
        }
        else
        {
...
void vUi_SHARPNESS(UINT8 u1VidPath, UINT8 bOnOff)
{
	...
	vHalVideoSharpnessParam(u1VidPath);
    #ifdef CUST_SOEM_DRV
	vHalVideoCTIParam(u1VidPath);// update CTI data
	#endif
}
...
UINT8 bApiVideoProc(UINT8 u1VidPath, PE_ARG_TYPE u1UiItem)
{
    ...
    #ifdef CUST_SOEM_DRV
    INT16 i2SHPMin, i2SHPMax, i2SHPDft, i2SHPCur;
    PE_UI_RANGE_T UiSHPRange;
    #endif
    UINT8 u1SrcTypTmg;
    ...
    #ifdef CUST_SOEM_DRV
	//u1SrcTypTmg = bApiRemappingMHLSrc(u1SrcTypTmg);
	#endif
    ...
    #ifdef CUST_SOEM_DRV
    if (IO32ReadFldAlign(PEUI_DBG, PEUI_DBG_01))
    {
        LOG(0, "\n[Api_peui.c] Path(%d),SourceTiming->CustomTiming [%d]->[%d]\n",
            u1VidPath, bDrvVideoGetSourceTypeTiming(u1VidPath),u1SrcTypTmg);
    }
    #endif
    ...
    #ifdef CUST_SOEM_DRV
    if (IO32ReadFldAlign(PEUI_DBG, PEUI_DBG_00) && (u1UiItem <= PE_ARG_NS))
    {
        LOG(0, "[Front]PE UI: %s[%d]  Min %d  Max %d  Dft %d  Cur %d \n",
            ucPeUiType[u1UiItem], u1VidPath, i2Min, i2Max, i2Dft, i2Cur);
    }
	if(u1UiItem == PE_ARG_MPEG_NR)
	{
		if(i2Cur == 3)
		{
			gMPEGNRHigh = TRUE;
	...
	if (QtyRange.i4Dft & R_AUTO)
	{
        ...
        #ifdef CUST_SOEM_DRV
        i2Cur = ((i2Cur ==0)&&(bAuto==1)) ? 2 : i2Cur;    //Masoem, RF custom nr
        #endif
		i2Max -= 1;
	}
	...
	// Fill Quality Table
#ifdef CUST_SOEM_DRV
	if (QtyRange.i4Dft & R_QTYMAP)//((QtyRange.i4Dft & R_QTYMAP) && (bOnOff != 0))  enable pq mapping when UI show off
#else
...
#endif
	{
        ...
        {
            #ifdef CUST_SOEM_DRV
            if(!((u1UiItem == PE_ARG_ADAPTIVE_LUMA) && (u2QtyItem >=QUALITY_AL_BS_BEGIN ) &&(u2QtyItem <=QUALITY_AL_BS_END)))
            #endif
            {
        ...
        if(u1UiItem == PE_ARG_MPEG_NR)
        {
        	PE_GetArg(u1VidPath, PE_ARG_SHARPNESS, &i2SHPCur);

			 if (PE_GetUiRange(u1VidPath, PE_ARG_SHARPNESS, &UiSHPRange) == SV_FAIL)
		    {
		        LOG(6, "PE_GetUiRange %d FAIL\n", u1UiItem);
		        return SV_FALSE;
		    }
		    
			i2SHPMin = UiSHPRange.i4Min;
			i2SHPMax = UiSHPRange.i4Max;
			i2SHPDft = UiSHPRange.i4Dft;
			u2Value = bApiQtyMapping(u1SrcTypTmg, QUALITY_TDSHARPALL_GAIN, i2SHPMin, i2SHPMax, i2SHPDft, i2SHPCur);
		    wWriteQualityTable(QUALITY_TDSHARPALL_GAIN, u2Value);
        }
        #endif
	}
	...
	if(QtyRange.i4Dft & R_TIMEUI)
	{
		// Special Case: fill driver value from timing table directly
		// UI=0 -> table 0, UI=1 -> table 1, etc..
        for (u2QtyItem=QtyRange.i4Min; u2QtyItem <= QtyRange.i4Max; u2QtyItem++)
        {
            UINT16 u2Value = bApiQtyMapping(i2Cur, u2QtyItem, i2Min, i2Max, i2Dft, i2Cur);
        	wWriteQualityTable(u2QtyItem, u2Value);
        }
	}
    ...
    switch (u1UiItem)
    {
        ...
        case PE_ARG_GAMMA:
            ...
            #ifdef CUST_SOEM_DRV
            DRVCUST_BrightPlusProc();    //[Temp]Masoem, Update Brightplus setting
            #endif
            break;
