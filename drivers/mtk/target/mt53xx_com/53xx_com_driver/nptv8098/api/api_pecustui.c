                              ...
                              VOID * pv_set_info, SIZE_T z_size, BOOL b_store)
{
    ...
    case DRV_CUSTOM_VID_TYPE_REMAP_GAMMA:
    ...
    case DRV_CUSTOM_VID_TYPE_PRE_PATTERN:
    case DRV_CUSTOM_DISP_TYPE_REDUCE_BLUR:
    ...
    case DRV_CUSTOM_VID_TYPE_MUTE_PATTERN:
        ...
        break;
	#ifdef CUST_SOEM_DRV
	case DRV_CUSTOM_VID_TYPE_GAMMA_BUF:
		if (z_size != sizeof(DRV_GAMMA_BUF_T))
        {
            return (-1);
        }
        break;
    ...
    case DRV_CUSTOM_VID_TYPE_SCE_PATTERN:
        ...
        break;
	#ifdef CUST_SOEM_DRV
	case DRV_CUSTOM_VID_TYPE_DBC_DEBUG:
        if (z_size != sizeof(DRV_CUSTOM_DBC_DEBUG_T))
        {
            return (-1);
        }
        break;
    ...
    case DRV_CUSTOM_VID_TYPE_PQ_DEMO:
        ...
        break;         
	#ifdef CUST_SOEM_DRV
	case DRV_CUSTOM_VID_TYPE_EPG_FLAG:
        if (z_size != sizeof(BOOL))
        {
            return (-1);
        }
...
//	#endif	
	#endif	
    case DRV_CUSTOM_VID_TYPE_COLOR_FILTER:
        if(z_size!=sizeof(COLOR_FILTER_INFO_T))
        {
            return (-1);
        }
        break;         
    case DRV_CUSTOM_VID_TYPE_MANUAL_GAMMA:
        if(z_size!=sizeof(i4GammaCtrlPts))
        {
            return (-1);
        }
        break;         
    case DRV_CUSTOM_VID_TYPE_EXP_COLOR:
        if(z_size!=sizeof(PQ_EXP_COLOR_T))
        {
            return (-1);
        }
        break;         
    case DRV_CUSTOM_DISP_TYPE_FORCE_BACK_LIGHT:
        if(z_size!=sizeof(FORCE_BACK_LIGHT_T))
        {
            return (-1);
        }
        break;         
    ...
    case DRV_CUSTOM_DISP_TYPE_OUTRGB_COLOR:    /*Output specific test color in RGB color space */
        ...
        // Spread spectrum
    case DRV_CUSTOM_DISP_TYPE_SPREAD_SPECTRUM:
		#ifndef CC_SOEM_BOARD
        vDrvSetVOSpread((UINT16) ((UINT32) pv_set_info));
		#else
		{
		   	UINT32 u4Freq=0,u4Permillage=0;
			PANEL_GetSpreadSpectrum(&u4Freq, &u4Permillage);
			Printf("DDDS_SetSpreadSpectrum %d %d\n",u4Freq,(UINT16) ((UINT32) pv_set_info));
			DDDS_SetSpreadSpectrum(u4Freq, (UINT16) ((UINT32) pv_set_info));
		}
		#endif
        return 0;
	...
	#ifndef DEMO_BOARD
	case DRV_CUSTOM_VID_TYPE_GAMMA_BUF:
      	vDrvWriteGammaBuf(((DRV_GAMMA_BUF_T *)pv_set_info)->ui1_Index,
      		                       ((DRV_GAMMA_BUF_T *)pv_set_info)->ui2_rx,
        				((DRV_GAMMA_BUF_T *)pv_set_info)->ui2_gx,
        				((DRV_GAMMA_BUF_T *)pv_set_info)->ui2_bx);
      	return 0;
	#endif
	#endif
    case DRV_CUSTOM_VID_TYPE_MUTE_PATTERN:
        ...
        return 0;
    case DRV_CUSTOM_VID_TYPE_SCE_PATTERN:
        vApiRegisterVideoEvent(PE_EVENT_SCE_PAT, 
                                    ((DRV_CUSTOM_SCE_PATTERN_T *)pv_set_info)->ui1_OnOff,
                                    ((DRV_CUSTOM_SCE_PATTERN_T *)pv_set_info)->ui2_Ydata >> 2);
        return 0;
	#ifdef CUST_SOEM_DRV
	#ifndef DEMO_BOARD
	case DRV_CUSTOM_VID_TYPE_DBC_DEBUG:
        DRVCUST_DBC_DEBUG_MODE(((DRV_CUSTOM_DBC_DEBUG_T *)pv_set_info)->ui1DBC_MAGIC,
            ((DRV_CUSTOM_DBC_DEBUG_T *)pv_set_info)->ui1BrightLevel);        
        return 0;
	case DRV_CUSTOM_VID_TYPE_EPG_FLAG:  //When EPG on then turn off dynamic backlight
        DRVCUST_EPG_FLAG((UINT8)((UINT32)pv_set_info));
        return 0;
	#endif
	#endif
    case DRV_CUSTOM_DISP_TYPE_AUTO_BACK_LIGHT_SETS:
    ...
    case DRV_CUSTOM_VID_TYPE_PQ_DEMO:
    	...
    	vApiVideoDemoMode(((PQ_DEMO_INFO_T *)pv_set_info)->bDemoMode,
    	  ...
    	  return 0;        
    case DRV_CUSTOM_VID_TYPE_COLOR_FILTER:
        vHalSetColorFilter(SV_VP_MAIN, SV_RGB_R, ((COLOR_FILTER_INFO_T *)pv_set_info)->u1ROnOff);
        vHalSetColorFilter(SV_VP_MAIN, SV_RGB_G, ((COLOR_FILTER_INFO_T *)pv_set_info)->u1GOnOff);
        vHalSetColorFilter(SV_VP_MAIN, SV_RGB_B, ((COLOR_FILTER_INFO_T *)pv_set_info)->u1BOnOff);
        return 0;
    case DRV_CUSTOM_VID_TYPE_PRE_PATTERN:
        vDrvPreprocEnablePattern(SV_VP_MAIN, *(UINT8 *)pv_set_info);
        return 0;
    case DRV_CUSTOM_VID_TYPE_MANUAL_GAMMA:
        x_memcpy(i4GammaCtrlPts, (INT32*)pv_set_info, sizeof(i4GammaCtrlPts));
        DRVCUST_SET_MAUNAL_GAMMA(i4GammaCtrlPts);
        return 0;
    case DRV_CUSTOM_VID_TYPE_REMAP_GAMMA:
        vDrvSetGammaRemapIndex(*(UINT32*)pv_set_info);
        return 0;
    case DRV_CUSTOM_VID_TYPE_EXP_COLOR:
        vHalSetSCEAngular(((PQ_EXP_COLOR_T *)pv_set_info)->u1Module, 
                            ((PQ_EXP_COLOR_T *)pv_set_info)->u1Color,
                            (((PQ_EXP_COLOR_T *)pv_set_info)->u1Value+((PQ_EXP_COLOR_T *)pv_set_info)->u1Offset));
        return 0;
    case DRV_CUSTOM_DISP_TYPE_FORCE_BACK_LIGHT:
        BACKLT_ForceDimming(((FORCE_BACK_LIGHT_T *)pv_set_info)->u1Force, ((FORCE_BACK_LIGHT_T *)pv_set_info)->u1Duty);
        return 0;
    case DRV_CUSTOM_DISP_TYPE_REDUCE_BLUR:
        vDrvLcdimSpiSetFreqCtrl((UINT32)(*(UINT8 *)pv_set_info));
