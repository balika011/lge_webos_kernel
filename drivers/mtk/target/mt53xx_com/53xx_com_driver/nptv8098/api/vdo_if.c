...
UINT8 bApiVideoMainSubSrc(UINT8 bMainSrc, UINT8 bSubSrc)
{
...
#ifdef CC_SUPPORT_STR
    printk("bApiVideoMainSubSrc: 0x%x, 0x%x\n", bMainSrc, bSubSrc);
#endif
#ifndef CC_UP8032_ATV
    ...
    if(fgPipCh)
    {
	    ...
	    {
            //when both main and sub are hdmi source, donot send pip mode change
            if(!(bGetICInputType(SV_VP_PIP)==P_DVI && bGetICInputType(SV_VP_MAIN)==P_DVI))
            {
	...
	switch(bType)
	{
		case SV_VD_TVD3D:
			...
			*pu1CStripe = vTvd3dGetAnalogCopyProtectCStripe();
			Printf("bApiVideoGetAnalogCopyProtect SV_VD_TVD3D PSYNC(%d), CSTRIPE(%d)\n",
				*pu1Psp,*pu1CStripe);
			break;
		case SV_VD_YPBPR:
		    ...
		    *pu1CStripe =0;
			Printf("undefined ");
		#endif
		    Printf("bApiVideoGetAnalogCopyProtect SV_VD_YPBPR PSYNC(%d), CSTRIPE(%d)\n",
				*pu1Psp,*pu1CStripe);
			break;
		default:
			break;
