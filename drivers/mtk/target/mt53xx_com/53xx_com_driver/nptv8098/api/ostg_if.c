...
void BACKLT_SetDimming(UINT32 u4Frequency, UINT32 u4Dimming)
{
    ...
    else if(((vDrvGetLCDFreq() == 24) || (vDrvGetLCDFreq() == 48)) && (u1DrvGetForceOutputFrameRate() & FORCE_DCLK_FORCE_48HZ))
    {
    ...
    if ((u4Dimming>0)&&(u4Dimming<100))
    {
		u4High = u4Dimming/u4Devider;
		u4Low = (100-u4Dimming)/u4Devider;
    }
    else if (u4Dimming>=100)
    {
		u4High = 100;
    }
    LOG(3, "_|-|_ BACKLT_SetDimming freq:%dHz,dev:%d,start:%d%,high:%d%,low:%d%\n", u4Frequency, u4Devider, SCANPWM_START, u4High, u4Low);
...
void BACKLT_ForceDimming(UINT32 u4Force, UINT32 u4Duty)
{
#ifndef CC_MTK_LOADER
    vIO32WriteFldMulti(FORCE_BACKLIGHT_0, P_Fld(u4Force, FORCE_BACKLIGHT_EN) | P_Fld(u4Duty, FORCE_BACKLIGHT_DUTY));
#endif
}
...
UINT32 BACKLT_GetDimmingFrequency(void)
{
    ...
    #ifdef SUPPORT_PANEL_48HZ
    else if(((vDrvGetLCDFreq() == 24) || (vDrvGetLCDFreq() == 48) || (vDrvGetLCDFreq() == 96)) && (u1DrvGetForceOutputFrameRate() & FORCE_DCLK_FORCE_48HZ))
    {
...
void vApiSetPanelBright(UINT8 u1BrightLevel)
{
    ...
    {
        u1BrightLevel = IO32ReadFldAlign(FORCE_BACKLIGHT_0, FORCE_BACKLIGHT_DUTY);
    }
#endif
...
void vApiTCONPower(VIDEO_TCON_POWER_SEQ_T eTconPowSeq, BOOL fgEnable)
{
    ...
    {
        if ((fgEnable == TRUE)&&(_fgTCONOn == FALSE)) 
        {
            u1DrvTconInit();                
            _fgTCONOn = TRUE;
        }
        printf("vApiTCONPanelPower: %d",fgEnable);
        u1DrvTconOffOn(eTconPowSeq, fgEnable);
    }
