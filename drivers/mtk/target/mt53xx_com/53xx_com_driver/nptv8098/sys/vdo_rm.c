...
void vVRMDumpVRMInfo(UINT8 bPath, VRM_MODULE_E moduleID)
{
    ...
    LOG(2, "[%2d%8s  [%1d] X=%4d Y=%4d Width=%4d Height=%4d HT=%4d VT=%4d FR=%3d %c]\n",
            vrmInfo->u2VRMID,
...
void vVRMDumpAllVRMInfo(void)
{
    ...
    {   
        ...
        vDrvRegenInOutStatusDump(bPath);        
...
void vVRMUpdateOMUXResolution(UINT8 bPath)
{
    ...
    if(vDrvGetImportTriggerModule(bPath) == IMPORT_MODULE_B2R)
    {
        b2rVrmInfo.u4SrcWidth  = picInfo->wSrcW;
        b2rVrmInfo.u4SrcHeight = picInfo->wSrcH;    
        VDP_GetSeamlessInfo(bPath, &b2rVrmInfo);
        curVrmInfo.u2Width = b2rVrmInfo.u4SrcWidth;
        curVrmInfo.u2Height = b2rVrmInfo.u4SrcHeight; 
    }
    else
    {        
...
void vVRMUpdateMIBResolution(UINT8 bPath)
{
    ...
    extern UINT16 u2OverflowYoffset;
    #endif
    ...
    if((//422 case
        ...
        //444 can't support odd width in Cobra ES
        || IS_COBRA_E())
    {
    ...
    if(IS_COBRA_E() || IS_VIPER_E())
    {
        ...
        if(u4FixedWidth[bPath] != 0 && getScalerInfo(bPath)->u4IsDramFrontMode == SV_TRUE && u4HPDSFactor == DFT_HPD_FACTOR)
        {
            curVrmInfo.u2Width = curVrmInfo.u2Width - u4FixedWidth[bPath];        
            LOG(3, "fixed regen width(%d) u4FixedWidth(%d)\n",curVrmInfo.u2Width, u4FixedWidth[bPath]);
        }
        #endif  
    }

    #ifdef MIB_WIDTH_256N2_TO_256N_WA
    if(!bIsScalerInput444(bPath) && ((curVrmInfo.u2Width % 256) == 2))//DI quality is abnormal when input width=256*N+2,fix it to 256*N.
    {
        curVrmInfo.u2Width -= 2;
    }
    #endif
    ...
    #ifdef CC_MHEG5_POSITION_DRIVER_WA//mheg5 certification .the position is error.so software walk around
    if((curVrmInfo.u2Y > (mibIn->u2Height - curVrmInfo.u2Height))&&(mibIn->u2Height > curVrmInfo.u2Height))
    {
        if(SV_VP_MAIN == bPath)
        {
            if((E_TD_OUT_NATIVE == eMIBOUT)&&(bDrvVideoIsSrcInterlace(bPath) == SV_TRUE))
            {
                u2OverflowYoffset = curVrmInfo.u2Y -(mibIn->u2Height - curVrmInfo.u2Height);
            }
            else
            {
                u2OverflowYoffset = 0;    
...
void vVRMUpdateFSCResolution(UINT8 bPath)
{
    ...
    fscIn = VRMGetVRMInfo(bPath, VRM_MODULE_MIB);
...
inline void vVRMUpdateMixerResolutionWithMJC(UINT8 bPath)
{
    ...
    if((_u4ForceDispOff[bPath])||(bPath == SV_VP_PIP && curVrmInfo.u2Width==0 && curVrmInfo.u2Height==0))
    {
    ...
    if(BSP_GetIcVersion() == IC_VER_5398_AA || BSP_GetIcVersion() == IC_VER_5880_AA)
    {
        #if defined(SCALER_WA1_DRAM_WIDTH)
        if(u4FixedWidth[bPath] != 0 && getScalerInfo(bPath)->u4IsDramFrontMode == SV_FALSE)
        {
            curVrmInfo.u2Width = curVrmInfo.u2Width - u4FixedWidth[bPath];        
            LOG(3, "fixed outr width(%d) u4FixedWidth(%d)\n",curVrmInfo.u2Width, u4FixedWidth[bPath]);
        }
        #endif	
    }
    ...
    curVrmInfo.u2VTotal += 55;
    curVrmInfo.u2VTotal = MAX( curVrmInfo.u2VTotal , 120);

    //With MJC but flip/mirror not in MJC done
    #ifdef CC_FLIP_MIRROR_SUPPORT 
	if(u4GetFlipMirrorModule(bPath)!=FLIP_BY_MJC)
	{
...
inline void vVRMUpdateMixerResolutionWithoutMJC(UINT8 bPath)
{
  ...
  //  E_TD_OUT eMIBOUT = TD_MIB_OUT(u4MIB);
    ...
    {        
        ...
        if(curVrmInfo.u2Width < wDrvGetOutputHActive())
        {
        ...
        if(curVrmInfo.u2Height < wDrvGetOutputVActive())
        {
...
void vVrmAllImportEn(UINT8 bPath, UINT8 bOnOff)
{
    ...
    vRegenSetResolutionBypass(bPath, !bOnOff);
...
void vDrvSetImportTriggerModule(UINT8 bPath, IMPORT_MODULE_E module)
{
    ...
    vDrvRegenClrSetRes(bPath, SET_RES_BOTH);
    ...
    else
    {
        ...
        vRegenSetResolutionMode(bPath, SV_ON);
