...
BOOL fgIs4k2kScalerBypass(UINT8 bPath)
{
		...
		if((_fgHDMI4k2k && (bPath == SV_VP_MAIN)))
		{
			bRtn = SV_TRUE ;
		}
...
UINT8 bIsScalerInput444(UINT8 bPath) reentrant
{
    ...
    if((bPath == SV_VP_PIP) && bDrvVrmIsAppFlagSet(VRM_APP_VIDEO_ENC))   
    {
        return FALSE;
    }
	...
	#endif
	#ifdef CC_SUPPORT_4K2K
		//4k2k case forced to 444 for PQ tuning because no one can't do 422 to 444
...
void vDrvVideoConnect(UINT8 bPath, UINT8 bOnOff)
{
    ...
    switch(bType)
    {
        ...
        case SV_VD_DVI:
			...
			{
#ifdef CC_OSD_ENCODE
				if(bGetICInputType(SV_VP_PIP)==P_OSTGENC)
				{
				    vCCIROSDOnOff(SV_OFF);
					vCCIROSDConnect(bPath, bOnOff);
				}
				else if(bGetICInputType(SV_VP_PIP)==P_OSTGOSDENC)
				{
				    vCCIROSDOnOff(SV_ON);
					vCCIROSDConnect(bPath, bOnOff);
				}
...
void vShowOmuxStatus(void)
{
    Printf("=========================\n");
    vIO32WriteFldAlign(SYS_05, 1, RAND_EN);
    vIO32WriteFldAlign(SYS_05, 1, RAND_CH_SEL);
    vUtDelay10ms(5);
