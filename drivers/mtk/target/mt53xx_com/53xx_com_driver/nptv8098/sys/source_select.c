...
#ifdef CC_OSD_ENCODE
	/*VSS_OSTGOSDENC */	{SV_VD_DVI, SV_ST_DVI},
	/*VSS_OSTGENC */	{SV_VD_DVI, SV_ST_DVI}
#endif
};
...
void vDrvAvMux(UINT32 dMUX)
{
    ...
    if((dMUX>>24)==0x80 )	//CVBSINY
	{
    		vIO32WriteFldAlign(REG_CVBS_CFG1, 1, RG_CVBSINYA_EN);
		for(i=0;i<portmap_table_num; i++)
		{		
    			if((portmap_table[i].hdtv_ic_port==P_YP1))
    			{
     				vIO32WriteFldAlign(PDWNC_VGACFG4, 0, FLD_RG_CVBSIN_Y0_EN);  				
    				vIO32WriteFldAlign(PDWNC_VGACFG4, 1, FLD_RG_CVBSIN_Y1_EN);
    				i=portmap_table_num;
    			}
    			else if((portmap_table[i].hdtv_ic_port==P_YP0))
    			{
    				vIO32WriteFldAlign(PDWNC_VGACFG4, 1, FLD_RG_CVBSIN_Y0_EN);
    				vIO32WriteFldAlign(PDWNC_VGACFG4, 0, FLD_RG_CVBSIN_Y1_EN);
    				i=portmap_table_num;
			}
		}
	}
