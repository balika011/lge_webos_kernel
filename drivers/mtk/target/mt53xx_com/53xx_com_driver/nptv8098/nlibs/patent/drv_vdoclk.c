...
vIO32WriteFldAlign(VFE_18, 0, RG_SDDS_FIFO_START);//for suspend unlock issue
...
//CC_MT5365 according to Walter's suggestion
 ...
 vDrvCLKINFreeRun(ENABLE);
    ...
    //vIO32WriteFldMulti(CKGEN_AFEPLLCFG1,P_Fld(1,FLD_RG_VGAPLL_PD)|P_Fld(0,FLD_RG_VGAPLL_MONAIOEN)|P_Fld(0,FLD_RG_VGAPLL_MONCLKEN));
      ...
      vIO32WriteFldMulti(REG_PLL_GROUP_CFG8, P_Fld(1, FLD_RG_VGAPLL_SDDS_PD_PDB)|P_Fld(1,FLD_RG_VGAPLL_SDDS_PD_EN)|P_Fld(0,FLD_RG_VGATL_BIAS_PWD)|P_Fld(1,FLD_RG_XDDS_PWDB)|P_Fld(7,FLD_RG_XDDS_PI_C)|P_Fld(1,FLD_RG_XDDS_HF)|P_Fld(1,FLD_RG_VGAPLL_RTB_EN));
      vIO32WriteFldMulti(REG_PLL_GROUP_CFG0, P_Fld(0, FLD_RG_VGAPLL_MONCKEN)|P_Fld(0,FLD_RG_VGAPLL_MONEN)|P_Fld(1,FLD_RG_VGAPLL_RESERVE));
   ...
   //vIO32WriteFldMulti(REG_PLL_GROUP_CFG6, P_Fld(0x0, FLD_RG_VGAPLL_B_EN)|P_Fld(0x0,FLD_RG_VGAPLL_G_EN)|P_Fld(0x0,FLD_RG_VGAPLL_R_EN));
   	vIO32WriteFldMulti(REG_PLL_GROUP_CFG8, P_Fld(0x0, FLD_RG_VGAPLL_ENTL)|P_Fld(0x1,FLD_RG_VGAPLL_ENTL)|P_Fld(0x0,FLD_RG_VGAPLL_INTH_EN));
	vIO32WriteFldAlign(VFE_18, 1, RG_SDDS_FIFO_START);
}
...
void vDrvAllADCPLLPow(UINT8 bPow)
{
	...
	{    
       ...
       vIO32WriteFldMulti(REG_PLL_GROUP_CFG8, P_Fld(0x0, FLD_RG_VGAPLL_B_EN)|P_Fld(0x0,FLD_RG_VGAPLL_G_EN)|P_Fld(0x0,FLD_RG_VGAPLL_R_EN));
       }
}
...
void vPGA_LPF_BW(UINT8 bLPF)
{
    ...
    {
            vIO32WriteFldAlign(REG_VGA_Normal_CFG3,        LPF_Table_1[bLPF][0], MUXCAP_EN1);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG7,        LPF_Table_1[bLPF][1], MUXCAP_EN2);
             vIO32WriteFldAlign(REG_VGA_Normal_CFG11,       LPF_Table_1[bLPF][2], MUXCAP_EN3);
...
//new band selection
   ...
   //Setting ADCPLL Band
    if(wPixClk<450)
    {
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG0, 0x3, FLD_RG_VGAPLL_BS);       
    }
    else if((wPixClk<800)&&(wPixClk>=450))
    {
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG0, 0x2, FLD_RG_VGAPLL_BS);   
    }
    else
    {
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG0, 0x0, FLD_RG_VGAPLL_BS);   
    }
   ...
   //Setting before ADCPLL    
    if(wPixClk<300)
    {
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG8, 0x0, FLD_RG_XDDS_CKSEL);   
   }
    else if((wPixClk<650)&&(wPixClk>=300))			//(SDclk CW)/1, ADCPLLx1 
    {
      bADCPLL_NS = 1;
      //vIO32WriteFldAlign(CKGEN_AFEPLLCFG5,0, FLD_RG_VGAPLL_SDDSO_DIV12_SEL);
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG8, 0x1, FLD_RG_XDDS_CKSEL);   
    }
   ...
   //Setting before PFD    
    ...
    else
    {
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG8, 0x2, FLD_RG_VGAPLL_SDDS_FBKSEL);   

      //vIO32WriteFldAlign(CKGEN_AFEPLLCFG0, bADCPLL_NS, FLD_RG_VGAPLL_DIV);
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG0, bADCPLL_NS, FLD_RG_VGAPLL_FBDIV);   
...
//   vIO32WriteFldMulti(REG_PLL_GROUP_CFG6, P_Fld(0x0, FLD_RG_VGAPLL_G_EN)|P_Fld(0x0,FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
   ...
   vIO32WriteFldMulti(REG_PLL_GROUP_CFG8, P_Fld(0x1, FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
...
void vPhaseSearchCore(void)
{
	...
	if (_bCurPhase < 3)
	{
		_dSumTmp[_bCurPhase] = dVGAGetDiffValue(BD_RED);
	...
	else
	{
		...
		_dSumTmp[2] = dVGAGetDiffValue(BD_RED);
...
{
    vIO32WriteFldAlign(0xf0028300, 0x1, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf002890C, 0x00000300, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0028910, 0x40000000, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0028910, 0x50000000, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0028390, 0x0,  Fld(25,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0028904, 0x80000000, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0028300, 0x00000001, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0028908, 0x00000350, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf002890C, 0xF0000000, Fld(32,0,AC_FULLDW));
    vIO32WriteFldAlign(0xf0062118, 0x1, Fld(2,4,AC_MSKB0));  
    vIO32WriteFldAlign(0xf006213C, 0x0, Fld(1,23,AC_MSKB2));
    vIO32WriteFldAlign(0xf0062140, 0x0, Fld(1,23,AC_MSKB2));
    vIO32WriteFldAlign(0xf0062118, 0x1, Fld(1,23,AC_MSKB2));
    vIO32WriteFldAlign(0xf006211C, 0x0, Fld(1,23,AC_MSKB2));
    vIO32WriteFldAlign(0xf0062104, 0x0, Fld(1,16,AC_MSKB2));
    vIO32WriteFldAlign(0xf006203C, 0x0, Fld(1,20,AC_MSKB2));
    vIO32WriteFldAlign(0xf0062040, 0x0, Fld(1,20,AC_MSKB2));
    vIO32WriteFldAlign(0xf0062018, 0x0, Fld(1,23,AC_MSKB2));
    vIO32WriteFldAlign(0xf006201C, 0x0, Fld(1,23,AC_MSKB2));
    vIO32WriteFldAlign(0xf0062008, 0x1, Fld(1,0,AC_MSKB0));
    vIO32WriteFldAlign(0xf0062010, 0x1, Fld(1,19,AC_MSKB2));
    
    
    if(bType)
    {
        //PPLL電壓量測設定
        vIO32WriteFldAlign(0xf0062010, 0x1, Fld(1,22,AC_MSKB2)); //RG_PPLL_MONEN=1
        vIO32WriteFldAlign(0xf006200c, 0x2, Fld(3,29,AC_MSKB3)); //RG_VPLL_TST_SEL[1:0]=2'b10
    }
    else
    {
        //VPLL電壓量測設定
        vIO32WriteFldAlign(0xf0062010, 0x0, Fld(1,22,AC_MSKB2));  //RG_PPLL_MONEN=0
        vIO32WriteFldAlign(0xf0062008, 0x1, Fld(1,4,AC_MSKB0));  //RG_VPLL_MONEN=1
        vIO32WriteFldAlign(0xf006200c, 0x3, Fld(3,29,AC_MSKB3));  //RG_VPLL_TST_SEL[1:0]=2'b11
    }
