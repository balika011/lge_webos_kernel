...
void vDrvDBLSWInit(void)
{
    ...
    vIO32WriteFldAlign(OPC_12, 0x00, OPC_REMAP_P0);
    vIO32WriteFldAlign(OPC_12, 0x20, OPC_REMAP_P1);
    vIO32WriteFldAlign(OPC_12, 0x40, OPC_REMAP_P2);
    vIO32WriteFldAlign(OPC_12, 0x60, OPC_REMAP_P3);
    vIO32WriteFldAlign(OPC_13, 0x80, OPC_REMAP_P4);
    vIO32WriteFldAlign(OPC_13, 0xA0, OPC_REMAP_P5);
    vIO32WriteFldAlign(OPC_13, 0xC0, OPC_REMAP_P6);
    vIO32WriteFldAlign(OPC_13, 0xE0, OPC_REMAP_P7);
    vIO32WriteFldAlign(OPC_14, 0xFF, OPC_REMAP_P8);
    
#ifdef CUST_SOEM_DRV
	vIO32WriteFldAlign(OPC_0A, 990, OPC_MAX_CONTENT_TH);
	vIO32WriteFldAlign(OPC_0E, 6, RUN_QUE_SIZE); 
	
	vIO32WriteFldAlign(BRIGHT_PLUS_00, 0, BRIGHT_PLUS_CTRL);
	vIO32WriteFldAlign(BRIGHT_PLUS_01, 230, BRIGHT_PLUS_P1);
	vIO32WriteFldAlign(BRIGHT_PLUS_01, 255, BRIGHT_PLUS_P2);
	vIO32WriteFldAlign(BRIGHT_PLUS_01, 115, BRIGHT_PLUS_G1);
	vIO32WriteFldAlign(BRIGHT_PLUS_01, 104, BRIGHT_PLUS_G2);
	vIO32WriteFldAlign(BRIGHT_PLUS_02, 1, BRIGHT_PLUS_STEPUP);
	vIO32WriteFldAlign(BRIGHT_PLUS_02, 5, BRIGHT_PLUS_STEPDN);
	vIO32WriteFldAlign(BRIGHT_PLUS_02, 8, BRIGHT_PLUS_STEPUPCNT);
	vIO32WriteFldAlign(BRIGHT_PLUS_02, 2, BRIGHT_PLUS_STEPDNCNT);
#endif
}
...
UINT8 vDrvOPCPwmRemap(UINT8 u1PWM)
{
    UINT32 u4Index, u4Value, u4PtIdx, u4Res;
    static UINT8 u1PWMCurve[9] = {0x00, 0x20, 0x40, 0x60, 0x80, 0xA0, 0xC0, 0xE0, 0xFF};

    if (IO32ReadFldAlign(OPC_14, OPC_REMAP_EN))
    {
        // If update bit is set, read new mapping curve from SW register
        if (IO32ReadFldAlign(OPC_14, OPC_REMAP_UPDATE))
        {
            for(u4Index = 0; u4Index < 9; u4Index++)
            {
                u1PWMCurve[u4Index] = u1IO32Read1B(OPC_12 + u4Index);
            }
            vIO32WriteFldAlign(OPC_14, 0, OPC_REMAP_UPDATE);
        }
        
        // Re-map new PWM value
        u4PtIdx = u1PWM >> 5;
        u4Res = u1PWM % 32;
        u4Value = ((u1PWMCurve[u4PtIdx]*(32-u4Res)) + (u1PWMCurve[u4PtIdx+1]*u4Res)+31)>>5;
...
void vDrvOPCTableSearch(UINT16 u2LuminanceTable[3][LUMA_TABLE_SIZE][LUMA_TABLE_SIZE], UINT8 bMaxContent, OPC_INFO *OPCInfo)
{
    ...
    OPCInfo->OPC_CURVE_PWM = vDrvOPCPwmRemap(OPCInfo->OPC_CURVE_PWM);
    vIO32WriteFldAlign(OPC_0A, OPCInfo->OPC_CURVE_PWM, OPC_REMAP_PWM);
...
#ifdef CUST_SOEM_DRV
void vDrvBrightPlusSetOPCCurve(void)
{
    // write opc curve
	vIO32WriteFldMulti(DBL_01, 
	                    P_Fld(0x80, DBL_P1) |
	                    P_Fld(0x80, DBL_P2) |
	                    P_Fld(MAX(0x80,IO32ReadFldAlign(OPC_0B, OPC_SW_G1)), DBL_G1) |
	                    P_Fld(MAX(0x80,IO32ReadFldAlign(OPC_0B, OPC_SW_G2)), DBL_G2));    
	vIO32WriteFldAlign(DBL_00, 0, DBL_G_SLOPE);
}
#endif
...
#ifdef CUST_SOEM_DRV
UINT8 vDrvCalculateBPGain(UINT8 bMaxContent)    
{
	static UINT8 bCounterUP = 0;
	static UINT8 bCounterDN = 0;
	UINT8 u1LowBound, u1HighBound, u1Gain, u1G1, u1G2, u1StepUp,u1StepDn,u1StepUpCnt,u1StepDnCnt;
	u1LowBound = IO32ReadFldAlign(BRIGHT_PLUS_01, BRIGHT_PLUS_P1);
	u1HighBound = IO32ReadFldAlign(BRIGHT_PLUS_01, BRIGHT_PLUS_P2);
	u1G1 = IO32ReadFldAlign(BRIGHT_PLUS_01, BRIGHT_PLUS_G1);
	u1G2 = IO32ReadFldAlign(BRIGHT_PLUS_01, BRIGHT_PLUS_G2);
	u1StepUp = IO32ReadFldAlign(BRIGHT_PLUS_02, BRIGHT_PLUS_STEPUP);
	u1StepDn = IO32ReadFldAlign(BRIGHT_PLUS_02, BRIGHT_PLUS_STEPDN);
	u1StepUpCnt = IO32ReadFldAlign(BRIGHT_PLUS_02, BRIGHT_PLUS_STEPUPCNT);
	u1StepDnCnt = IO32ReadFldAlign(BRIGHT_PLUS_02, BRIGHT_PLUS_STEPDNCNT);

	u1G1 = MAX(1,u1G1);

	u1LowBound = MIN(u1LowBound,((u1HighBound*u1G2*2/u1G1 + 1) >> 1));

	vIO32WriteFldAlign(BRIGHT_PLUS_01, u1LowBound, BRIGHT_PLUS_P1);	
	
	u1StepUpCnt = MAX(1,u1StepUpCnt);
	u1StepDnCnt = MAX(1,u1StepDnCnt);

	u1G1 = MAX(MIN(u1G1 , 199),100);
	u1G2 = MAX(MIN(u1G2 , 199),100);

	if(bMaxContent <= u1LowBound)
	{
		u1Gain = u1G1;
	}
	else if(bMaxContent >= u1HighBound)
	{
		u1Gain = u1G2;
	}
	else
	{
		u1Gain = ((u1LowBound * u1G1) + (u1HighBound * u1G2 - u1LowBound * u1G1) *(bMaxContent - u1LowBound)/(u1HighBound - u1LowBound))/ bMaxContent;		
	}

	//vIO32WriteFldAlign(BRIGHT_PLUS_00, u1Gain, BRIGHT_PLUS_OUTPUT_GAIN);	

	//normalize to 128

	u1Gain = u1Gain *128/100;
    //Begin  add APL information to control--
    {
    UINT8 bAPL =bDrvGetAPL();
    UINT8 bAPLTHD = IO32ReadFldAlign(BRIGHT_PLUS_03, BRIGHT_PLUS_APLTHD);
    UINT8 tempgain=u1Gain;
    bAPLTHD = MAX(30,((bAPLTHD==0) ? 64 : bAPLTHD));
    if(IO32ReadFldAlign(BRIGHT_PLUS_03, BRIGHT_PLUS_APL_REF)==0)
    {
        if(u1Gain>128)
        {
            if((bAPL <= bAPLTHD)&&(bAPL > (bAPLTHD-30)))
            {
                u1Gain = MAX((u1Gain - ((bAPLTHD-bAPL)*25/30)),128);
            }
            else if(bAPL<=(bAPLTHD-30))
            {
                u1Gain = 128;
            }
            
            if(IO32ReadFldAlign(BRIGHT_PLUS_03, BRIGHT_PLUS_APL_LOG)==1)
            LOG(0,"\n[Bright plus]Gain before [%d] -> after [%d]\n",tempgain*100/128,u1Gain*100/128);
        }
    }
    
	vIO32WriteFldAlign(BRIGHT_PLUS_00, u1Gain*100/128, BRIGHT_PLUS_OUTPUT_GAIN);	
    }
    //End ----------------------------

	u1Gain = u4ReturnQueAverage(u1Gain, 0);

	if(u1Gain > bBPpreGain)
	{
		bCounterUP++;
		bCounterDN = 0;
		
	}
	else if(u1Gain < bBPpreGain)
	{
		bCounterDN++;
		bCounterUP = 0;
	}

	if(bCounterUP == u1StepUpCnt)
	{
		if(u1Gain > (bBPpreGain + u1StepUp))
		{
			u1Gain = bBPpreGain + u1StepUp;
		}
		bCounterUP = 0;
	}
	else if(bCounterDN == u1StepDnCnt)
	{
		if(u1Gain < (bBPpreGain - u1StepDn))
		{
			u1Gain = bBPpreGain - u1StepDn;
		}
		bCounterDN = 0;
	}
	else
...
#ifdef CUST_SOEM_DRV
void vDrvBrightPlusInfoToSwReg(OPC_INFO *OPCInfo)
{
    vIO32WriteFldAlign(OPC_0B, OPCInfo->OPC_CURVE_P1, OPC_SW_P1);
    vIO32WriteFldAlign(OPC_0B, OPCInfo->OPC_CURVE_P2, OPC_SW_P2);        
    vIO32WriteFldAlign(OPC_0B, OPCInfo->OPC_CURVE_G1, OPC_SW_G1);
...
void vDrvBrightPlusSetDefault(void)
{ 
    bBPpreGain = 128;
    vIO32WriteFldAlign(OPC_0B, 0x80, OPC_SW_P1);
    vIO32WriteFldAlign(OPC_0B, 0x80, OPC_SW_P2);        
...
void vDrvBrightPlusProc(void)
{
    ...
    if(IO32ReadFldAlign(BRIGHT_PLUS_00, BRIGHT_PLUS_CTRL))
    {

		u1DrvGetRGBHist(OPCRGBMaxNormHist, 0);
		...
		bBPGain = vDrvCalculateBPGain(bMaxContent);

        OPCInfo.OPC_CURVE_P1 = 0x80;
        OPCInfo.OPC_CURVE_P2 = 0x80;
        OPCInfo.OPC_CURVE_G1 = bBPGain;
        OPCInfo.OPC_CURVE_G2 = bBPGain;
        OPCInfo.OPC_CURVE_GSLP= 0x0;
        vDrvBrightPlusInfoToSwReg(&OPCInfo);
        //vDrvBrightPlusSetOPCCurve(0x80,0x80,bBPGain,bBPGain,0); 
    ...
    else
    {
    	vDrvClearTPF();
        // 7. set default
        vDrvBrightPlusSetDefault();      
		//vDrvBrightPlusSetOPCCurve(0x80,0x80,0x80,0x80,0);  
    }  
}
#endif
