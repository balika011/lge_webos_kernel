...
static UINT32 _u4EstRNRConTh(UINT32 u4NoiseLvl)
{
    UINT32 a = _SWR(NR_NM_05, SNR_THM_GAIN_A);
    UINT32 b = _SWR(NR_NM_05, SNR_THM_GAIN_B);
    UINT32 c = _SWR(NR_NM_06, SNR_THM_OFST_C);
...
void vDrvAnrSetSNR(void)
{
    ...
    if (_SWR(NR_CTRL_00, EN_MOTION_ANR) ==0)
    {
...
void vDrvAnrSetMOANR(void)
{
    ...
    NR_W(PBC_SHP3_04, _rNrWRegMoAnr.u1LPFLevel, PBC3_LPF_LEVEL);
#endif
}
...
#ifdef CC_MT5398
    NR_W(TDSHP_27, _rNrWRegMoAnr.u1TdsCorZero, TDS_COR_ZERO);
    NR_W(PBC_SHP1_04, _rNrWRegMoAnr.u1LPFLevel, PBC3_LPF_LEVEL);
#endif
}
...
void vDrvMoRnrProc(UINT8 u1StrLvl)
{
    ...
    u4RnrAggTh = _EN(EN_RNR_TH_ADAP) ? _u4EstRNRAggTh(u1StrLvl) : 16*_SWR(NR_PQ_13, RNR_MAX_TH);
    u4RnrConTh = _EN(EN_RNR_TH_ADAP) ? _u4EstRNRConTh(u1StrLvl) : 16*_SWR(NR_PQ_13, RNR_CON_TH);
    u4K = MIN(15, _rSnrTh.u1GeneralStr);
...
void vDrvAnrMoAnr2DProc(UINT8 u1StrLvl , UINT8 u1NoiseLvl)
{
    UINT32 u4SmMoidx;
    ...
    #ifdef CC_MT5398
    _rNrWRegMoAnr.u1LPFLevel =IO32ReadFldAlign(TDSHP_2F,TDS_PBC_BLUR_EN) ? LERP(u1LPFMax, u1LPFMin, u1NoiseLvl, 5) : u1LPFMin;
    #endif
   // _rNrWRegMoAnr.u1LPFLevel = LERP(u1LPFMax, u1LPFMin, u1StrLvl, 4);
    ...
    if (_SWR(NR_AUTO_02,BNR_AGAINST_PATCH_EN))
    {
        ...
        if (_rNrWRegMoAnr.u1BNRVAgst!=NR_R(NXNR_05,NX_VBLK_FLT_AGAINST))
        {
            _u1BNRVAgst = NR_R(NXNR_05,NX_VBLK_FLT_AGAINST);
        }
...
void vDrvAnrMoAnrProc(UINT32 u4NoiseLevel, UINT32 u4HomoSum)
{
   ...
   u4FinalMotionLvl =0;
    ...
    // 2. 2D settings
    vDrvAnrMoAnr2DProc(u4SnrStrIdx,u4NoiseLevel);
    // 3. 3D settings
