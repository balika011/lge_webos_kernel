...
#elif defined(CC_SUPPORT_ECO_NON_ECO_AUTO_CFG)

#include "drv_tdtv_5398_FS_tbl.h"
#include "drv_tdtv_5398_def_tbl.h"
...
const UINT64 DQC2XCFG_PR=
    TD_TBL_PACK(DEC(E_TDTV_DECODER_INPUT_SS_P,E_TDTV_DECODER_OUTPUT_3D_FS  ),DQC(E_TD_IN_SS_P ,E_TD_OUT_3D_FS)|(1<<(DQC_IN_W+DQC_OUT_W)),
    TTT(E_TD_IN_FS_P ,E_TD_OUT_NATIVE),MIB(E_TD_IN_FS_P,E_TD_OUT_NATIVE), SCL(E_TD_IN_FS_P
    ,E_TD_OUT_NATIVE),TTD(E_TD_IN_LI_P,E_TD_OUT_NATIVE),MJC(E_TD_IN_FS_P,E_TD_OUT_3D_LI));

const UINT64 DQC2XCFG_SG=
    TD_TBL_PACK(DEC(E_TDTV_DECODER_INPUT_SS_P,E_TDTV_DECODER_OUTPUT_3D_FS  ),DQC(E_TD_IN_SS_P ,E_TD_OUT_3D_FS)|(1<<(DQC_IN_W+DQC_OUT_W)),
    TTT(E_TD_IN_FS_P,E_TD_OUT_NATIVE),MIB(E_TD_IN_FS_P,E_TD_OUT_NATIVE), SCL(E_TD_IN_FS_P
    ,E_TD_OUT_NATIVE),TTD(E_TD_IN_FS_P,E_TD_OUT_NATIVE),MJC(E_TD_IN_FS_P,E_TD_OUT_3D_FS));

const UINT64 DQC2XCFG_322=
    TD_TBL_PACK(DEC(E_TDTV_DECODER_INPUT_SS_P,E_TDTV_DECODER_OUTPUT_3D_FS  ),DQC(E_TD_IN_SS_P ,E_TD_OUT_3D_FS)|(1<<(DQC_IN_W+DQC_OUT_W)),

    TTT(E_TD_IN_FS_P ,E_TD_OUT_3D_2_2D),MIB(E_TD_IN_2D_P,E_TD_OUT_NATIVE),SCL(E_TD_IN_2D_P 
    ,E_TD_OUT_NATIVE),TTD(E_TD_IN_2D_P,E_TD_OUT_NATIVE),MJC(E_TD_IN_2D_P,E_TD_OUT_NATIVE));

static UINT64 DQCNormalCfg;
static UINT64 DQCNormal322;
...
static inline E_TDTV_UI_3D_PACKING eDrvTDTVGetExternalUIPacking(UINT32 u4Path)
{
...
static inline void vDrvTDTVOSDConfig(void)
{
    ...
    if((E_TDTV_UI_3D_MODE_OFF==u4DrvTDTV3DModeQuery())|| (0x80==u1Depth) ||(SRM_IsEpgMode()))
    {
        u1OnOff = SV_OFF;
    }
    else
...
static inline void vDrvTDTVOSTGConfig(void)
{
	...
	printf("#############eTDTVMode:%d,eTDTVAutoMode:%d,u4FrameRate:%d \n",eTDTVMode,eTDTVAutoMode,u4FrameRate);
    switch(eTDTVMode)
    {
#ifdef SENSIO_LICENSE
        case E_TDTV_UI_3D_MODE_SS:  
        ...
        case E_TDTV_UI_3D_MODE_FS:	 
            ...
            else if(((24==u4FrameRate)||(48==u4FrameRate)))
            {   
                u4VTotal = 1125;
                u4Height = 1080;
                u4Width  = 1920;              
			...
			#if defined(SUPPORT_DISP_50TO120)
        	if (IS_SUPPORT_DISP_50TO120_3D)			
               {
	               	u4VTotal = 1125;
	                u4Height = 1080;
	                u4Width  = 1920;            
            ...
            //output frame rate = 120hz,96hz,force to 120hz
            else if(120==u4FrameRate || 96 ==u4FrameRate)
            {
                u4VTotal = 810;
                u4Height = 768;
                u4Width  = 1366;
            ...
            //output frame rate = 100hz,force to 120hz
            else
            {
				#if defined(SUPPORT_DISP_50TO120)
	        	if (IS_SUPPORT_DISP_50TO120_3D)			
            ...
            u4Clk    = PANEL_CLK_H;
            break;  
            //for 8283, force output frame rata to 60hz on 2D case.
        case E_TDTV_UI_3D_MODE_OFF: 
            printf("############enter 2D case.............\n");
            if(50==u4FrameRate || 25== u4FrameRate)
            {				
			#if defined(SUPPORT_DISP_50TO120)
        	if (IS_SUPPORT_DISP_50TO120_2D)			
               {
	                u4VTotal = 1125;
		            u4Height = 1080;
		            u4Width  = 1920;
            ...
            break;
        case E_TDTV_UI_3D_MODE_TTD: 
            printf("##############enter 2D ->3D case....\n");
            if( bIsScalerInput444(SV_VP_MAIN)==SV_TRUE) // PC timing
            {
            	if(u4FrameRate>=60)
            	    u4FrameRate=60;
            }
            //double output frame rate to TTD,then output shutter to 8283
            u4FrameRate = u4FrameRate << 1;

            if((24==u4FrameRate)||(48==u4FrameRate)||(30==u4FrameRate)||(60==u4FrameRate))
            {
                u4VTotal = 1125;
                u4Height = 1080;
                u4Width = 1920;
            ...
            //for 100hz, force output frame rate to 120hz 
            else
            {
				#if defined(SUPPORT_DISP_50TO120)
	        	if (IS_SUPPORT_DISP_50TO120_3D)			
...
UINT32 u4DrvTDTVMMModeQueryEx(UINT32 u4Path, E_TDTV_DECODER_INPUT eDetectedMode)
{
        ...
        if(E_TDTV_DECODER_INPUT_SS_P==eDetectedMode)
            u4Cfg =TD_2ENUM_TO_INT( E_TDTV_DECODER_OUTPUT_3D_FS, E_TDTV_DECODER_INPUT_SS_P);  
...
UINT32 u4DrvTDTV3DFMTCapQuery(UINT32 bPath)
{
	...
	if(fgIs4k2kPhoto3DSupport(bPath))
	{
		bInput444 = 0 ;
	}
	else
#endif
	{
		bInput444 = bIsScalerInput444(SV_VP_MAIN);
	}
    //3D UI Enable
    if((SV_VP_MAIN==bPath)&&(
            (bDrvVideoSignalStatus(SV_VP_MAIN)!=SV_VDO_STABLE)
...
void vDrvTDTVInit(UINT8 forceinit)
{
    ...
    {  
        ...
        if(IS_IC_5398()&&(BSP_GetIcVersion() > IC_VER_5398_AA))
        {
            x_memcpy((void*)u8VdoPathProcCfg
                , (void*)u8VdoPathProcCfgAfterECO, sizeof(u8VdoPathProcCfg));
            x_memcpy((void*)u8VdoPathProcCfgShutter
                , (void*)u8VdoPathProcCfgShutterAfterECO, sizeof(u8VdoPathProcCfg));
            x_memcpy((void*)u8VdoPathProcCfgWithoutMJC
                , (void*)u8VdoPathProcCfgWithoutMJCAfterECO, sizeof(u8VdoPathProcCfg));
            x_memcpy((void*)u8VdoPathProcCfgFullHDPR
                , (void*)u8VdoPathProcCfgFullHDPRAfterECO, sizeof(u8VdoPathProcCfg));            
        }
#endif
        ...
        DQCNormalCfg = u8VdoPathProcCfg[E_TDTV_DECODER_INPUT_SS_P];
        DQCNormal322 = u8VdoPathProc322Cfg[E_TDTV_DECODER_INPUT_SS_P];
        u1Initialized=1;
    }
}
...
#if defined(CC_MT5398)
    if((eDecDetectPack[E_DEC_MOD_MM]==E_TDTV_DECODER_INPUT_SS_P)
         &&(SV_ST_MPEG==bGetSignalType(VDP_1)))
    {

        u8VdoPathProcCfg[E_TDTV_DECODER_INPUT_SS_P]=(IS_LVDS_DISP_3D_SHUTTER)?  DQC2XCFG_SG : DQC2XCFG_PR;
        u8VdoPathProc322Cfg[E_TDTV_DECODER_INPUT_SS_P]=DQC2XCFG_322;

        if((!bDrvVideoIsSrcInterlace(VDP_1))&&
            (((60 <= bDrvVideoGetRefreshRate(VDP_1))&&(wDrvVideoInputWidth(VDP_1)<=960))
            ||((120 <= bDrvVideoGetRefreshRate(VDP_1))&&(wDrvVideoInputWidth(VDP_1)<=640))))
        {
            pu8VdoPathProcCfg[E_TDTV_DECODER_INPUT_SS_P] |= ((UINT64)1<<(TD_TBL_POS_TTT-1));
        }
    }
    else
#endif    
    {
        u8VdoPathProcCfg[E_TDTV_DECODER_INPUT_SS_P]=DQCNormalCfg;
        u8VdoPathProc322Cfg[E_TDTV_DECODER_INPUT_SS_P]=DQCNormal322;
    }
...
{    
    ...
    {    
        ...
        else if(E_TDTV_UI_3D_MODE_FS==eDrvTDTVGetCurCfg())
        {
        ...
        else
        {
            vDrvTTDSetLREyeInverse(SV_OFF);
            MJC_SetInputLRSwap(SV_OFF);            
            MJC_SetOutputLRSwap(((ePanelLRInv + eLRInv + u1LRInvForB2RFM)%2 == 1));
        }        
    }
#endif
