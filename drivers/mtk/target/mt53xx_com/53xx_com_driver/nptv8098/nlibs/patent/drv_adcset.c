...
{     
   ...
   // vIO32WriteFldAlign(VFE_E1, 5, RG_CLAMP_SINK); /*add for V2 */
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_AAF_BW_2); /*add for V2 */
...
//SIFP
   ...
   vIO32WriteFldAlign(VSRC_00, 0x04, TVD3D_MPGAC);
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG4, 0x08, RG_CLAMP_DRAIN_2);
...
BOOL vDrvSetSIFADC(void)
{
    ...
    else
    {
        ...
        vIO32WriteFldAlign(REG_CVBS_CFG4, 0x1, RG_BTM_EN_2);
        vIO32WriteFldAlign(REG_CVBS_CFG4, 0x08, RG_CLAMP_DRAIN_2);
        vIO32WriteFldAlign(REG_CVBS_CFG5, 0x11, RG_OFFCURB_2);
        vIO32WriteFldAlign(REG_CVBS_CFG5, 0x0D, RG_PGAGAINB_2);
        bUseChD = TRUE;
...
void vDrvHDTVADCDefaultSetting(void)
{
    ...
    vIO32WriteFldMulti(PDWNC_VGACFG1,P_Fld(6,FLD_RG_SOGY_VTL)|P_Fld(9,FLD_RG_SOGY_VTH));
	...
	for (bCh=0; bCh<3; bCh++)
	{
        vIO32WriteFldMulti(REG_VGA_Normal_CFG1+16*bCh,P_Fld(0,CLAMP_SOURCE_OFF_1)|P_Fld(0,CLAMP_SINK_OFF_1)
                              ...
                              |P_Fld(0x0A,CLAMP_SINK_1)|P_Fld(0x18,CLAMP_SOURCE_1));
    ...
    vIO32WriteFldMulti(REG_VGA_Normal_CFG8, P_Fld(2,AGSEL)|P_Fld(0,RESSEL)|P_Fld(7,CAP_EN)|P_Fld(1,OFFSET_EN)
     ...
     {    	
     	vIO32WriteFldMulti(REG_VGA_Normal_CFG12,P_Fld(0x04,ADC_VREFP_1)|P_Fld(0x04,ADC_VREFP_2)|P_Fld(0x04,ADC_VREFP_3)|P_Fld(0x03,ADC_VREFN));    	
     }    
     else if((IO32ReadFldAlign(0xf0008674, AC_FULLDW)>>24)==2)//gain too big    	
     {    	
     	vIO32WriteFldMulti(REG_VGA_Normal_CFG12,P_Fld(0x03,ADC_VREFP_1)|P_Fld(0x03,ADC_VREFP_2)|P_Fld(0x03,ADC_VREFP_3)|P_Fld(0x04,ADC_VREFN));    	
     }    
...
void vDrvSetHDTVADC(UINT8 bVDOINPUT)
{
 ...
 vIO32WriteFldMulti(PDWNC_VGACFG1, P_Fld(0x1B,FLD_RG_SOGY_SINK)|P_Fld(0xB,FLD_RG_SOGY_SOURCE)
    ...
    vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(1,FLD_RG_VSYNC_EN)|P_Fld(5,FLD_RG_SYNC_VLSEL)
                                          |P_Fld(3,FLD_RG_SYNC_VHSEL)|P_Fld(0,FLD_RG_SYNC_TSTSEL)
                                          |P_Fld(0,FLD_RG_SYNC_DESPK_EN)|P_Fld(0,FLD_RG_SYNC_PWD)
                                          |P_Fld(1,FLD_RG_FB_PWD));
   #else
    vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(1,FLD_RG_VSYNC_EN)|P_Fld(3,FLD_RG_SYNC_VLSEL)
...
void vCust_EBGain_OverRule_Update(UINT8 bNum) //kaloverrule
{
    ...
    if(_bcurrent_use_adc!=_bcurrent_use_adc)
    {
	if(bNum ==0xAA)
	{
		_bcurrent_use_adc=2;  //external ADC
...
void vCust_SOGY_VTH_Set(UINT32 dwLevel)
{
 ...
 //   vIO32WriteFldAlign(SyncSlice_Control_01, dwLevel,VTH_SEL);
        vIO32WriteFldAlign(PDWNC_VGACFG1,dwLevel,FLD_RG_SOGY_VTH);
}
...
void vCust_SOGY_VTL_Set(UINT32 dwLevel)
{
    ...
    vIO32WriteFldAlign(PDWNC_VGACFG1,dwLevel,FLD_RG_SOGY_VTL);
...
void MTAL_SOGY_VTH_Set(UINT8 bLevel)
{
	...
	rSlicerConfig.AP_SVTLH_ManuValue=(bLevel) |(rSlicerConfig.AP_SVTL_ManuValue<<4);
      ...
      vIO32WriteFldAlign(PDWNC_VGACFG1,bLevel,FLD_RG_SOGY_VTH);
}
...
void MTAL_SOGY_VTL_Set(UINT8 bLevel)
{
	...
	rSlicerConfig.AP_SVTLH_ManuValue=(bLevel<<4) |(rSlicerConfig.AP_SVTH_ManuValue);
      ...
      vIO32WriteFldAlign(PDWNC_VGACFG1,bLevel,FLD_RG_SOGY_VTL);
}
...
void vScartVfeConnect(void)
{
    ...
    vIO32WriteFldMulti(PDWNC_VGACFG1,P_Fld(0x06,FLD_RG_SOGY_VTL)|P_Fld(0x08,FLD_RG_SOGY_VTH));
...
void vVFESLT_slicer_test_verC(void)
{
	...
	for (i=0;i<sizeof(VFEverC_slicer);i++)
	{
		...
		if ((i%16)==0)
			LOG(1,"#SLT:[%2d] ",i/16);	
             ...
             vIO32WriteFldMulti(PDWNC_VGACFG1,  P_Fld((VFEverC_slicer[i]>>3), FLD_RG_SOGY_VTL)|P_Fld((VFEverC_slicer[i]), FLD_RG_SOGY_VTH));
	...
	//Change PGA setting
             ...
             vIO32WriteFldMulti(PDWNC_VGACFG1, P_Fld(8, FLD_RG_SOGY_VTL)|P_Fld(8, FLD_RG_SOGY_VTH));
...
void vQuerySlicer(void)
{
    ...
    DBG_Printf(VGA_Debug,"slicer Low/High %d/%d\n", IO32ReadFldAlign(PDWNC_VGACFG1,  FLD_RG_SOGY_VTL), IO32ReadFldAlign(PDWNC_VGACFG1,  FLD_RG_SOGY_VTH));
   #endif  
...
PRIVATE void vSetSlicer(UINT8 pair)//wu add
{
       ...
       vIO32WriteFldMulti(PDWNC_VGACFG1,P_Fld((pair & 0xF0) >>4,FLD_RG_SOGY_VTL)|P_Fld((pair & 0x0F),FLD_RG_SOGY_VTH));
}
...
#define SLICER_LV_LIST_NUM 6 // 4
#if ADAPTIVE_MONITOR_SLICER_MEASURE
//PRIVATE UINT8 code  MonSlicerSetting[] ={0x41, 0x41, 0xC9, 0xC9, 0x63}; // {0x9c, 0x14, 0x9C, 0x9C, 0x36};
PRIVATE UINT8 code  MonSlicerSetting[] ={0x14, 0x14, 0x9c, 0x9c, 0x36, 0x36}; // {0x9c, 0x14, 0x9C, 0x9C, 0x36};
#endif
...
{   
    ...
    {0x69, {0x69,0x7A,0x8B,0x9B}}, // 0x58, {0x58,0x69,0x79,0x89}, // 0xa8, {0x98,0xa8,0xb9,0xc9},
    {0x14, {0x14,0x15,0x27,0x38}}, // 0x04, {0x04,0x15,0x26,0x37},  // 480i HV Reduce
...
{
	UINT8 pair;
	pair=IO32ReadFldAlign(PDWNC_VGACFG1,FLD_RG_SOGY_VTL)<<4 |IO32ReadFldAlign(PDWNC_VGACFG1,FLD_RG_SOGY_VTH);
...
{
    UINT32 i=0, ADC_OUT_A=0,ADC_OUT_B=0,ADC_OUT_C=0;
    UINT8 offCura_sign=0,OffCur_delta=0,offCur_temp=0;
    Printf("vDrvCvbsoffsetCal\n");
    vIO32WriteFldAlign(DFE_03, 0x06, AGC2_MODE);
    vIO32WriteFldAlign(DFE_07, 0x1F, AGC2_MANUAL_ACODE);    
    vIO32WriteFldAlign(DFE_07, 0x800, AGC2_MANUAL_DCODE);
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_AISEL);    
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_AISELR);
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_CINRA_EN);
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_CINRB_EN);
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_UPDN);    
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_BTM_EN);
    vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_C_SELA);//
    vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_C_SELB);//
    vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_VAGSELA);//
    vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_VAGSELB);//
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CLAMP_PWD);    
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_MIX_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CVBS_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_INMUX_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_PGABUFNA_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_PGABUFNB_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CVBSADC_SEL_CKPLL);// CY 1->0
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CVBSADC_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 3, RG_CVBSADC_CHSEL);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_CVBS_REV);    
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0x0, RG_CVBS_REV);//
     vIO32WriteFldAlign(REG_CVBS_CFG1, 0x00, RG_OFFCURA);
     vUtDelay2us(70); 
    	ADC_OUT_A=IO32ReadFldAlign(DFE_STA_03, SYNC_LV);


        if( ADC_OUT_A<250 || ADC_OUT_A>265)
        {
       	vIO32WriteFldAlign(REG_CVBS_CFG1, 0x01, RG_OFFCURA);
              vUtDelay2us(70); 
   		ADC_OUT_B=IO32ReadFldAlign(DFE_STA_03, SYNC_LV);
       	vIO32WriteFldAlign(REG_CVBS_CFG1, 0x11, RG_OFFCURA);
              vUtDelay2us(70); 
  		ADC_OUT_C=IO32ReadFldAlign(DFE_STA_03, SYNC_LV);
    //Printf("vDrvCvbsoffsetCal  ADC_OUT_A=%d ,ADC_OUT_B=%d,ADC_OUT_C=%d \n",ADC_OUT_A,ADC_OUT_B,ADC_OUT_C);
		if(ADC_OUT_A>ADC_OUT_B && ADC_OUT_A<ADC_OUT_C)
		{
		  offCura_sign=0x10;
		}
		else
     ...
     //Printf("vDrvCvbsoffsetCal  offCura_sign=0x%x \n",offCura_sign);
      	vIO32WriteFldAlign(REG_CVBS_CFG1, offCura_sign, RG_OFFCURA);
                    vUtDelay2us(70); 
       }
    ...
    {
    	ADC_OUT_A=IO32ReadFldAlign(DFE_STA_03, SYNC_LV);
   // Printf("vDrvCvbsoffsetCal  222  ADC_OUT_A=%d \n",ADC_OUT_A);
        if( ADC_OUT_A>=250 && ADC_OUT_A<=265)
        {
        	i=16;
     //Printf("vDrvCvbsoffsetCal  End \n");
        }
        else
        {
       	offCur_temp=IO32ReadFldAlign(REG_CVBS_CFG1,  RG_OFFCURA);
      // Printf("vDrvCvbsoffsetCal  offCur_temp=0x%x \n",offCur_temp);
		if( ADC_OUT_A >265)
		{
		   		OffCur_delta=(ADC_OUT_A-255)/10;
		   		if (OffCur_delta<1)
		   		{
		   			OffCur_delta=OffCur_delta+1;
		   		}
		   		if((offCur_temp&&0xF)<OffCur_delta)
		   		{
		   			offCur_temp=offCura_sign;
		   		}
		   		else
	      				offCur_temp=offCur_temp-OffCur_delta;

		}
		else
		{
		   OffCur_delta=(255-ADC_OUT_A)/10;
		   		if (OffCur_delta<1)
		   		{
		   			OffCur_delta=OffCur_delta+1;
		   		}
		   if(((offCur_temp&&0xF)+OffCur_delta) >0x10)
		   	offCur_temp=offCura_sign+0xF;
		   else
		      offCur_temp=offCur_temp+OffCur_delta;
		}
      // Printf("vDrvCvbsoffsetCal  i=%d, OffCur_delta=0x%x ,offCur_temp=0x%x\n",i,OffCur_delta,offCur_temp);
       	vIO32WriteFldAlign(REG_CVBS_CFG1, offCur_temp, RG_OFFCURA);
       	i=i+1;
        vUtDelay2us(70);
    ...
    vIO32WriteFldAlign(DFE_03, 0x08, AGC2_MODE);
     //Printf("vDrvCvbsoffsetCal  finish \n");
    //rollback these registers for scart problem after finish offset cal
    vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_UPDN);    
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_C_SELA);//
	//vIO32WriteFldAlign(REG_CVBS_CFG1, 0x00, RG_OFFCURA);
}
