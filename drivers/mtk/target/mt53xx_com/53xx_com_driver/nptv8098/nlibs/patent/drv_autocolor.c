...
void vDrvSetHDADCDigitalOffset(UINT8 bOrder,UINT8 value)
{
       switch (bOrder)
       {
		...
		vIO32WriteFldAlign(HDFE_02, value, AD1_OFFSET);          
               break;
           case 1:
       	vIO32WriteFldAlign(HDFE_00, _bDigSignBit, SIGN_OFFSET2);
		vIO32WriteFldAlign(HDFE_02, value, AD2_OFFSET);          
               break;
           case 2:
...
static UINT8 bReadHDTVDigitalOffset(UINT8 bOrder)
{
    ...
    {
        case 0:
           return(IO32ReadFldAlign(HDFE_02, AD1_OFFSET));
        case 1:
           return(IO32ReadFldAlign(HDFE_02, AD2_OFFSET));
        case 2:
        default:
...
//	  vIO32WriteFldAlign(REG_VGA_Normal_CFG3+(bCh<<4), 1, ADC_1_PWD);	       //power down ADC  // [SA7_Nick] 080525 marked
    ...
    if(bOnOff==0)     //turn off
    {
              ...
              // vIO32WriteFldMulti(CKGEN_AFEPLLCFG4, P_Fld(0x1, FLD_RG_VGAPLL_CKO_SEL)|P_Fld(0x1, FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
               vIO32WriteFldMulti(REG_PLL_GROUP_CFG8, P_Fld(0x1, FLD_RG_VGAPLL_CKO_SEL)|P_Fld(0x1, FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
               ...
               vIO32WriteFldMulti(REG_PLL_GROUP_CFG8, P_Fld(0x2, FLD_RG_VGAPLL_CKO_SEL)|P_Fld(0x0, FLD_RG_VGAPLL_B_EN)|P_Fld(0x0,FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
       #endif
    ...
    else
    {
       ...
       vIO32WriteFldAlign(REG_PLL_GROUP_CFG8, 0x0, FLD_RG_VGAPLL_CKO_SEL);
          vIO32WriteFldAlign(REG_PLL_GROUP_CFG9, 0x1, FLD_RG_VGAPLL_X27IN_EN);
          vIO32WriteFldAlign(REG_PLL_GROUP_CFG11, 0x1, FLD_RG_TL_27M_SEL);
...
UINT8 bDigital_gain;
 ...
 if(_bOrder<3)  //for klocwork check
 {
    ...
    vDrvOnChipAutoColorModeSet(OFFSET_MODE,_bType);
        ...
        {
           case 0:
               bDigital_gain = (IO32ReadFldAlign(HDFE_02, AD1_OFFSET));
               break;
           case 1:
               bDigital_gain = (IO32ReadFldAlign(HDFE_02, AD2_OFFSET));
               break;
           case 2:
           default:
      ...
      else
      {
	      bCalibration_direction = EQUAL_OFFSET;
           // DBG_Printf(VGA_Debug,"=== Current channel %x BLANK LEVEL=0x%x is almost equal to Target level=0x%x : difference is 0x%x===\n",_bOrder,_wData_v1v2[_bOrder],_wADCTarget[_bOrder],bAdjOffset);
      }
...
UINT8 bTolenence;
  if((_bType <=INT_VGA_COMPOENT) && (bMode<=DIGITAL_OFFSET_MODE))   //for klocwork check
  {
     ...
     for (_bOrder=0;  _bOrder<3; _bOrder++)
    {
	 //DBG_Printf(VGA_Debug,"ch1=%x, ch2=%x, ch3=%x\n",_wData_v1v2[0] ,_wData_v1v2[1], _wData_v1v2[2] );
...
UINT8 bTmp2;
   ...
   switch (_bAutoColorState0)
   {
     ...
     case VDO_AUTO_COLOR_1P0_START:  //to set gain and offset for the corresponding signal type
          ...
          vIO32WriteFldAlign(HDFE_02, 0, AD1_OFFSET);          
          vIO32WriteFldAlign(HDFE_00, 0, SIGN_OFFSET2);
          vIO32WriteFldAlign(HDFE_02, 0, AD2_OFFSET);          
