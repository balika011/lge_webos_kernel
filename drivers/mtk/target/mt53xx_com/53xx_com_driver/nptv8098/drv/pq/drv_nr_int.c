...
static UINT32* _u4HBlkLineSumR = 0;
...
#define _SFTX(x) ((((x)&0x7)*2) + 16)
#define _SFTY(y) (((y)&0x7)*2)
#define RD_VBLK(x) ((_u4BLKPos[((x)>>3)] >> _SFTX(x)) & 0x3)
#define RD_HBLK(y) ((_u4BLKPos[((y)>>3)] >> _SFTY(y)) & 0x3)
#define WR_VBLK(x, v) _u4BLKPos[(x)>>3] = (_u4BLKPos[(x)>>3] & (~(0x3 << _SFTX(x)))) | ((v) << _SFTX(x))
#define WR_HBLK(y, v) _u4BLKPos[(y)>>3] = (_u4BLKPos[(y)>>3] & (~(0x3 << _SFTY(y)))) | ((v) << _SFTY(y))

#define V_LS(x) (((*((UINT32*)_u4VBlkLineSum+((x)>>1))) >> (((x) & 0x1) ? 14 : 0)) & 0x3FFF)
#define H_LS(y) (((*((UINT32*)_u4HBlkLineSum+((y)>>1))) >> (((y) & 0x1) ? 14 : 0)) & 0x3FFF)
#define V_LS_R(x) (((*((UINT32*)_u4VBlkLineSumR+((x)>>1))) >> (((x) & 0x1) ? 14 : 0)) & 0x3FFF)
#define H_LS_R(y) (((*((UINT32*)_u4HBlkLineSumR+((y)>>1))) >> (((y) & 0x1) ? 14 : 0)) & 0x3FFF)
...
static inline UINT32 *_pu4HBlkLSPtr(void)
{
    return ((_u1LRFreeRunCnt) ? _u4HBlkLineSumR : _u4HBlkLineSum);
}
...
void vDrvNXNRCopyLineSumTool(void)
{
    ...
    if (_u1Cnt >= 6)
    {
        ...
        x_memcpy((UINT32*)NR_HBLK_START, _SWR(NR_FWBNR_00, NR_FWBNR_READ_RIGHT) ? _u4HBlkLineSumR : _u4HBlkLineSum, sizeof(UINT32)*NX_HEIGHT/2);    
...
UINT8 u1DrvNXNRReadBlkLineSum(void)
{
    ...
    UINT32* pu4HBlkLineSum = _pu4HBlkLSPtr();
    UINT32* pu4VBlkAddr = (UINT32*) pu4VBlkLineSum + _u4ReadCnt;
    UINT32* pu4HBlkAddr = (UINT32*) pu4HBlkLineSum + _u4ReadCnt;

    if (pu4VBlkLineSum == NULL || pu4HBlkLineSum == NULL || !u1DrvNRGetBlockMeterSupport() || u4VBlkDataSize == 0)
        return 0;
...
UINT8 u1DrvNXNRWriteBlkPos(void)
{
    ...
    NR_W(NXNR_14, 1, NX_CPU_SEL_SRAM);
    NR_W(NXNR_15, 720, NX_INIT_SRAM_ADDR);
    NR_W(NXNR_14, 1, NX_SRAM_AUTO_INC_ADDR);
    ...
    else
    {
        ...
        if ((_u1ScnChg ==1) && (_rNmSta.u1NLScnChgCnt==0) &&(_rNmSta.u1ScnChgCnt==0)&& _SWR(NR_NM_07, NM_SC_MO_MTHD2_EN))
        {
            ...
            if (_rNmSta.u4HomoSum < 10)
            {
                ...
                {
                    _u1ScnChgMotion = (_SWR(NR_NMSTA_0E, NR_NM_STA_NL) < _SWR(NR_AUTO_02, MOTNR_MIN_NL_TH)) ? 0 : 1;
                }
                else
            {
                _u1ScnChgMotion = 0;
            ...
            LOG(1,"===============>Method2: NR Motion= %d ; u4HomoSum =%d ; current NL = %d \n", _u1ScnChgMotion ,_rNmSta.u4HomoSum  ,_SWR(NR_NMSTA_0E, NR_NM_STA_NL)); 
        }
        
    }

    //update NL second time when motion
    if ((_u1ScnChgMotion ==0) && (_rNmSta.u4HomoSum > 10))
    {
...
void vDrvNRPQInit(void)
{
    ...
    _u4HBlkLineSumR = (UINT32*) x_mem_alloc(sizeof(UINT32)*NX_WIDTH/2);

    if (_u4VBlkLineSum == NULL || _u4HBlkLineSum == NULL || _u4VBlkLineSumR == NULL || _u4HBlkLineSumR == NULL)
    {
    ...
    else
    {
        ...
        x_memset(_u4HBlkLineSumR, 0, sizeof(UINT32)*NX_WIDTH/2);
    }
...
UINT8 u1DrvGetNMVal(void)
{
    return IO32ReadFldAlign(NR_NMSTA_0E, NR_NM_STA_NL);
}

/*UINT16 u2DrvGetMPEGLineCnt(void)
{
    return IO32ReadFldAlign(NR_BLK_STA_13, NR_STA_VBLK_BEST_PCT)*IO32ReadFldAlign(NXNR_0D,NX_MANUAL_VBLK_CONFIDENCE);
}
*/
