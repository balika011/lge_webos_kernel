...
void vDrvDIChkStatus(UINT8 u1VdpId)
{
    ...
    if (rMIBState[u1VdpId].u1StatusCnt == DI_STATUS_CNT)
    {
...
{	
    ...
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_05, 1      , IPTG_HV_MODE);
	MDDI_WRITE_FLD(u1VdpId, MCVP_KC_03, (u1OnOff == 1)? ((u1VdpId == VDP_1)? 1 : 5):0, IPTG_PAT_TYPE);
...
void vDrvDIInitPscanCRC(UINT8 u1VdpId)
{
    ...
    if (IS_ECO_1_IC() || IS_ECO_2_IC())
    {
        MDDI_WRITE_FLD(u1VdpId, MCVP_KC_25, _arMDDiPrm[u1VdpId].u2Height-1, OUT_CRC_CLIP_V_END);
    }
    else// for slt to filer
    {
        MDDI_WRITE_FLD(u1VdpId, MCVP_KC_25, _arMDDiPrm[u1VdpId].u2Height-5, OUT_CRC_CLIP_V_END);
    }
...
void vDrvDIEnableISR(UINT8 u1VdpId, UINT8 u1Enable)
{
    ...
    if(u1Enable == SV_ON)
    {
        ...
        //Delay to trigger after FS 3D to 2D as vtotal is not stable , and should not effect 2D cases.
        if((TD_TTT_IN(u4DrvTDTVTTTModeQuery(u1VdpId)) == E_TD_IN_FS_I || 
            TD_TTT_IN(u4DrvTDTVTTTModeQuery(u1VdpId)) == E_TD_IN_FS_P) &&
            (TD_TTT_OUT(u4DrvTDTVTTTModeQuery(u1VdpId)) == E_TD_OUT_3D_2_2D))
        {
            u1DlyAfterISREn[u1VdpId] = DEL_TRIG_PREDOWN_AFTER_ISREN;
        }
        else
        {
            u1DlyAfterISREn[u1VdpId] = 0;
        }
    ...
    else
    {
        ...
        {   
            ...
            u2InputLine = 
                MAX(u2InputLine/4 ,
                    u2InputLine 
                    *(FRC_LINE_TOLERENCE_N * u1OutFR - FRC_LINE_TOLERENCE_M * u1InFR)
                    /(FRC_LINE_TOLERENCE_N * u1OutFR)
                );                
        }
        else
        {
...
void vDrvDIDispmodeLRLockISR(void)
{
    ...
    UINT32 u4SCLDispRIsInv    =IO32ReadFldAlign(SCPIP_SCSYS_09, SCSYS_09_SC_INV_OUTPUT_DISP_R);    
    ...
    {    
        ...
        vIO32WriteFldAlign(SCPIP_SCSYS_09, ((!u4CurrMIBINV) != u1DrvMJCIsNeedInverseLR()),SCSYS_09_SC_INV_OUTPUT_DISP_R);         
    }
    else
    {
