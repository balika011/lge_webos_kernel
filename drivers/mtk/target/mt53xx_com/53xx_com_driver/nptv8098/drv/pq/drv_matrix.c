...
extern UINT8 DRVCUST_IsEnablePedastal(UINT8 bPath);
            ...
            if (!fgIsExtSrcCCIR())
            {
				...
				if((bPath==SV_VP_PIP)&&
				    (bGetICInputType(SV_VP_PIP)==P_OSTGENC  ||
				     bGetICInputType(SV_VP_PIP)==P_OSTGOSDENC))
				{
					u4ColorSystem = u4HDTV ? SV_COLOR_RGB_HD : SV_COLOR_RGB_SD;
				}
#endif
...
void vDrvSetPedestalMatrix(UINT8 bTransIndex, UINT8 bPath)
{
    UINT8 i;
    INT32 i4Temp;
    ...
    if(DRVCUST_IsEnablePedastal(bPath) == SV_FALSE)
    {
        return;
    }
/*
    if(!GET_MATRIX_PED && (!DRVCUST_IsEnablePedastalInSubPath()))
    {
        return;
    }
*/
...
void vDrvColorTransform(UINT8 bPath)
{
    ...
    switch (bGetSignalType(bPath))
    {
        ...
        #ifdef CUST_SOEM_DRV
        case SV_ST_MPEG:
            if(VDP_GetPlayMode(bPath)==1)    //Masoem MM photo, force to Full range
            {
                u4VideoRange = (MPV_GetDecMode(0) != 0) ? SV_HDMI_RANGE_LIMIT : SV_HDMI_RANGE_FULL;
                //LOG(2,"\nDDDD ---%d---",MPV_GetDecMode(0));
            }
            else
            {
        ...
        default:
            u4VideoRange = SV_HDMI_RANGE_LIMIT;
            u4VideoRange = (u4VideoRange == SV_HDMI_RANGE_DEFAULT) ? 
                                    SV_HDMI_RANGE_LIMIT : u4VideoRange;
            break;
    }
    ...
    {
        vDrvSetPedestalMatrix(u4ColorTransIdx, bPath);
        vDrvSetMatrixLvlAdj();
        if (bPath == SV_VP_MAIN)
        {
                          ...
                          UINT16 wSatValue, UINT16 wHueValue)
{
    ...
    // Contrast Adjustment
    for (u4Index = 3; u4Index < 6; u4Index++)
    {
        COLOR_TRANSFORM_ADJ[u4Index] = dwTable[u4Index] * wContValue >> 7;
    }
    ...
    {
        COLOR_TRANSFORM_ADJ[u4Index] = (COLOR_TRANSFORM_ADJ[u4Index] * wSatValue) >> 7;
    }
    ...
    COLOR_TRANSFORM_ADJ[12] = ((dwTableY2R[12] * (INT32)wContValue) >> 7) + 
...
void vDrvSetColorFilter(UINT8 bPath, UINT8 bColor, UINT8 bOnOff)
{
    ...
    if (IO32ReadFldAlign(MATRIX_00, MATRIX_PQ_ADJ) || ISSCALERRGB(bPath) || (bPath == SV_VP_PIP))
    {
