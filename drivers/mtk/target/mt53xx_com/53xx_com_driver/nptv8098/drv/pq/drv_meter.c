...
UINT8 bDrvGetAPL(void)
{
    UINT32 u1OffsetLR;
#ifndef CC_MT5398
    if (TD_SCL_IN(u4DrvTDTVScalerModeQueryEx(SV_VP_MAIN)) == E_TD_IN_FS_P)
    {
...
#else
    u1OffsetLR = ((u43DGetOutputLR() == 0) || (u4DrvTDTV3DModeQuery() == E_TDTV_UI_3D_MODE_OFF) || (u4DrvTDTV3DModeQuery() == E_TDTV_UI_3D_MODE_TTD)) ? 0 : METER_OFFSET_LR;        
#endif    
...
{   
    ...
    *bAPLR = IS_UI_3D_MODE ? IO32ReadFldAlign(METER_INFO_00+u1OffsetLR, METER_APL_VAL) : 0;
}
...
/**
 * Set Luma Min / Max LPF
 *
 * @param bLPFSel    LPF Select
 * 0: {1}(default), 1: {1, 2, 1}, 2: {1, 4, 6, 4, 1}, 3: {1, 4, 7, 8, 7, 4, 1}
 */

/*void vDrvSetLumaMaxLPF(UINT8 bLPFSel)
{   
    vIO32WriteFldAlign(SCE_CFG_MAIN, bLPFSel, COLOR_YMAX_LPF_SEL);   
}

void vDrvSetLumaMinLPF(UINT8 bLPFSel)
{   
    vIO32WriteFldAlign(SCE_CFG_MAIN, bLPFSel, COLOR_YMIN_LPF_SEL);   
}
*/
// ====================================================================================================
// SCE Histogram 
...
void vDrvSetSatHistWindow(UINT16 * u2Window)
{
	...
	vIO32WriteFldMulti(SAT_HIST_CFG_MAIN_3,
	                  P_Fld(u2Window[0], COLOR_HIST_X_START) |
	                  P_Fld(u2Window[2], COLOR_HIST_X_END));
	vIO32WriteFldMulti(SAT_HIST_CFG_MAIN_4,
	                  P_Fld(u2Window[1], COLOR_HIST_Y_START) |
	                  P_Fld(u2Window[3], COLOR_HIST_Y_END));
}

void vDrvSetLumaHistWindow(UINT16 * u2Window)
{
	if (u2Window == NULL)
	{
		return;
...
void vDrvGetSatHistBoundary(UINT8 * u1Boundary)
{
	if (u1Boundary == NULL)
	{
		return;
	}
    u1Boundary[0] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN, SAT_BOUND_1);
    u1Boundary[1] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN, SAT_BOUND_2);
    u1Boundary[2] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN, SAT_BOUND_3);
    u1Boundary[3] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN, SAT_BOUND_4);
    u1Boundary[4] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN_2, SAT_BOUND_5);
    u1Boundary[5] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN_2, SAT_BOUND_6);
    u1Boundary[6] = IO32ReadFldAlign(SAT_HIST_CFG_MAIN_2, SAT_BOUND_7);    
}
...
static void vDrvLumaHistWinCalculate(UINT16 *u2Window)
{
...
static void vDrvLumaHistInit(void)
{
	UINT16 wActiveWin[4];
	...
	vDrvSetLumaHistWindow(wActiveWin);	
}
...
static void vDrvSatHistInit(void)
{
	...
	wActiveWin[0] = 0;
	wActiveWin[1] = 0;
	wActiveWin[2] = PANEL_GetPanelWidth();
	wActiveWin[3] = PANEL_GetPanelHeight();

	vDrvSetSatHistWindow(wActiveWin);
	vDrvSetSatHistBoundary(u1SatHistBoundary);
}

static void vDrvLumaHistWinUpdate(void)
{
	UINT16 wActiveWin[4];

	vDrvLumaHistWinCalculate(wActiveWin);
	vDrvSetLumaHistWindow(wActiveWin);
}
...
#ifndef CC_MT5398
    u13DGetOutputLRFlg = (TD_SCL_IN(u4DrvTDTVScalerModeQueryEx(SV_VP_MAIN)) == E_TD_IN_FS_P) ? 
                            (UINT8)u43DGetOutputLR() : 0;
...
void vDrvGetASLLR(UINT8* bASLL, UINT8* bASLR)
{
    ...
    *bASLR = IS_UI_3D_MODE ? IO32ReadFldAlign(METER_INFO_00+u1OffsetLR, METER_ASL_VAL) : 0;
}
...
/*void vDrvSetHueHistBoundaryWithTHD(UINT8* bBoundary)
{
	if (bBoundary == NULL)
	{
		return;
	}
    vIO32WriteFldAlign(C_BOOST_MAIN_2, bBoundary[0], HUE_HIST_STH);  

	vIO32WriteFldMulti(HUE_HIST_CFG_MAIN,
	                  P_Fld(bBoundary[1], HUE_BOUND_0) |
	                  P_Fld(bBoundary[2], HUE_BOUND_1) |
	                  P_Fld(bBoundary[3], HUE_BOUND_2) |
	                  P_Fld(bBoundary[4], HUE_BOUND_3));
	vIO32WriteFldMulti(HUE_HIST_CFG_MAIN_1,
	                  P_Fld(bBoundary[5], HUE_BOUND_4) |
	                  P_Fld(bBoundary[6], HUE_BOUND_5) |
	                  P_Fld(bBoundary[7], HUE_BOUND_6) |
	                  P_Fld(bBoundary[8], HUE_BOUND_7));
}
...
static void vDrvHueHistInit(void)
{
    ...
    UINT8 u1HueHistBoundary[HUE_HIST_NUM] = {0, 0x20, 0x40, 0x60, 0x80, 0xA0, 0xC0, 0xE0};
