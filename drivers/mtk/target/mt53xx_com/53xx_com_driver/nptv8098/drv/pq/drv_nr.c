...
void vDrvNRSet3DNRMode(E_3DNR_MODE e3DMode)
{
    switch (e3DMode)
    {
        ...
        case MCNR_MODE_MA:
            ...
            #ifdef CUST_SOEM_DRV
            if(e3DMode==MCNR_MODE_MA)    //Masoem, for special panel
            {   
                //extern UINT32 FlashPanel_GetPQIndex(UINT32 u4Idx);
                //UINT8 u1panelID = (UINT8)FlashPanel_GetPQIndex(0);
                //if((u1panelID==3)||(u1panelID==4)||(u1panelID==5)/*||(u1panelID==6)*/)    //Masoem 32 inch 1080p noise  issue
                //NR_WM(MCNR_00,/* P_Fld(1, MCNR_ENABLE) |*/ P_Fld((MCNR_MODE_MV-1), MCNR_MODE));
            }
            #endif
            break;
        case MCNR_MODE_OFF:
...
void vDrvNRSetWindow(UINT16 u2Width, UINT16 u2Height)
{
    ...
    NR_WM(NXNR_3F,
        P_Fld((((u2Width & 0x3) != 0) && (!IS_ECO_2_IC()) ? 1 : 0), NX_RNR_FORCE_AGG_RATIO_EN)|
...
void vDrvNRSetDotCrawlNROnOff(UINT8 u1OnOff)
{
	...
	{
		_SWW(NR_PQ_0A,u1OnOff , NR_DNR_WA_EN);
	}
}
