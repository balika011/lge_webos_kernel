...
void vDrvDIGetInfo(UINT8 u1VdpId, UINT8 u1DbgIdx)
{
    ...
    switch (u1DbgIdx)
    {
        ...
        case 1:
            ...
            Printf("DispX                   %d\n", _arMDDiPrm[u1VdpId].DispX);
            Printf("DispY                   %d\n", _arMDDiPrm[u1VdpId].DispY);
            Printf("DispW                   %d\n", _arMDDiPrm[u1VdpId].DispW);
            Printf("DispH                   %d\n\n", _arMDDiPrm[u1VdpId].DispH);
            ...
            Printf("u2FreezeWriteFilter       0x%x\n", rMIBState[u1VdpId].u2FreezeWriteFilter);
            Printf("u1VSyncCnt                %d\n", rMIBState[u1VdpId].u1VSyncCnt);
            Printf("u2Vcnt                    %d\n", rMIBState[u1VdpId].u2Vcnt);
            Printf("u1FreezeCnt               %d\n", rMIBState[u1VdpId].u1FreezeCnt);
            Printf("u1StatusCnt               %d\n", rMIBState[u1VdpId].u1StatusCnt);
            Printf("bAllowedAutoStop          %d\n", rMIBState[u1VdpId].bAllowedAutoStop);
            Printf("u1SRCWAutoStopDisableCnt  %d\n", rMIBState[u1VdpId].u1SRCWAutoStopDisableCnt);
...
UINT32 u4DrvDIMaxOutWidth(UINT8 u1VdpId,UINT32 u4InputVActive,UINT32 u4DispVActive,UINT32 u4DispVTotal,UINT32 u4OutFR)
{
    ...
    if(u1DrvDIIsSupportedSignal(u1VdpId)) // if DI support, need to calculate width; if not support, just return 1920
    {    
...
{   
    ...
    if((_bSrcMainNew == _bSrcSubNew) && (bGetSignalType(u1VdpId) == SV_ST_MPEG))
    {
        rMIBState[VDP_1].u1AutoStop = u1OnOff;

        if (rMIBState[VDP_1].bAllowedAutoStop)
        {
            vDrvDISetAutoStop(VDP_1, u1OnOff);
        }

        rMIBState[VDP_2].u1AutoStop = u1OnOff;

        if (rMIBState[VDP_2].bAllowedAutoStop)
        {
            vDrvDISetAutoStop(VDP_2, u1OnOff);
        }
...
void vDrvDIFreezeOnOff(UINT8 u1VdpId, UINT8 u1OnOff)
{
    ...
    if(IS_PROGRESSIVE(u1VdpId))
    {
        ...
        rMIBState[u1VdpId].u2FreezeWriteFilter = (u1OnOff) ? u2Disable : 0x1FF;
    }
	
	if((!IS_MIB_FRC(u1VdpId)) && IS_INTERLACE(u1VdpId))
	{
	    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, u1OnOff, INPUT_SYNC_FREEZE);
	}
	else
	{
	    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, SV_OFF, INPUT_SYNC_FREEZE);
	}
