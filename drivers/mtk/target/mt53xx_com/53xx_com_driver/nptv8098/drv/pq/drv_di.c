...
static inline void _vDrvDICSFlagCfg(UINT8 u1VdpId)
{
    ...
    if(IS_SD_TIMING(SV_VP_MAIN) && MDDI_READ_FLD(VDP_1, MCVP_KC_19, SRAM_MODE) <= E_SRAM_SHARE_SD_P_OS) //SD Timing
    {
...
static void _vDrvDIAutoStopCtrl(UINT8 u1VdpId,UINT32 u4OnOff)
{
    ...
    if((SV_OFF==u4OnOff)
        ||(!IS_MPEG(u1VdpId))
        ||(E_TD_IN_FS_I==_arMDDiPrm[u1VdpId].eTDInput)
        )
    {
...
static void _vDrvDISetMisc(UINT8 u1VdpId)
{
    ...
    {
        MDDI_WRITE_FLD(u1VdpId, MCVP_KC_00, ((u1VdpId == VDP_1)?0xB : 0xC), SRCW_V_BGN);
    }
    else
    {
...
static void inline _vDrvDISetResEnd(UINT8 u1VdpId)
{
    ...
    {    
        ...
        if(u1DelayCnt==0)
        {
            MDDI_WRITE_FLD(u1VdpId,MCVP_KC_33, (IS_PROGRESSIVE(u1VdpId) && !IS_MIB_FRC(u1VdpId)),PROG_SRC_RST_EN);
        }
        else
        {
            u1DelayCnt--;
    ...
    MDDI_WRITE_FLD(u1VdpId,MCVP_KC_33,SV_ON     ,DISABLE_PRE_HSYNC_REF);
...
static inline void _vDrvDICfgDeRatio(UINT8 u1VdpId)
{
    ...
    if(0!=u4OutSigClk && 0!=u4InSigClk)
    {
        ...
        {        
            ...
            {
                u4OutSigClk/=2;                
                MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, SV_ON, Fld(1,30,AC_MSKB3)); 
            }
            else
            {
                MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, SV_OFF, Fld(1,30,AC_MSKB3)); 
            }
...
static UINT8 _u1DrvDIMaskedOnOff(UINT8 u1VdpId)
{
    ...
    if ((u1IsChannelOn == SV_FALSE)
        ...
        || (bDrvVideoGetTiming(u1VdpId) == MODE_1080P_120)        
        || (bDrvVideoGetTiming(u1VdpId) == MODE_1080P_100)
        )    
    {
        return SV_OFF;
    }
...
{ 
...
#endif
    {
        ...
        {                        
            ...
            #ifdef MIB_PROG_IN_SYNC_WA
            MDDI_WRITE_FLD(u1VdpId,MCVP_KC_33, 0,PROG_SRC_RST_EN);
