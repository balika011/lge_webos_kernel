...
extern sLdmSpiCommandSet sLePower1887SpiFunc;
...
void SPI_NON_DEF_CMD_3_PARAM(UINT8 u1Param1, UINT8 u1Param2, UINT8 u1Param3){}  // for non-defined command in specific IC

void vDrvLcdimSpiSetFreqCtrl(UINT32 u4Value)
{
#ifndef CC_MTK_LOADER        	
        vIO32WriteFldAlign(SPI_FREQ_CTRL_0, u4Value, SPI_FREQ_CTRL);
#endif
}
...
void vDrvLcdimSpiSetDoubleVsyncFreq(UINT32 u4Double)
{
    UINT32 u4Latency = 0;
    UINT8 u1CurFreq;
    static UINT8 u1Freq = 0;

    u1CurFreq = vDrvGetLCDFreq();
    ...
    if (u1Freq != 0)
    {
        u4Latency = u4Double ? ((BSP_GetDomainClock(SRC_BUS_CLK)) / (u1Freq * 2)) : 0;
    }
...
void vDrvLcdimSpiSetVsyncFreq(UINT32 u4Factor)
{
    UINT32 u4LCDFreq = vDrvGetLCDFreq();
    UINT32 u4Latency;

    if ((u4Factor == 0) || (u4LCDFreq == 0))
    {
        Printf("[SPI] Invalid LCD frequency = %d, factor = %d\n", u4LCDFreq, u4Factor);
        return;
    }

    u4Latency = (BSP_GetDomainClock(SRC_BUS_CLK)) / (u4LCDFreq * u4Factor);

    vIO32WriteFldAlign(LED_CON, 1, SW_SET_DATA);
    vIO32WriteFldAlign(LED_VSCFG, u4Factor - 1, FRAME_RATE);
    vIO32WriteFldAlign(LED_VSCFG, u4Latency, VSYNC_LATENCY);
    vIO32WriteFldAlign(LED_CON, 0, SW_SET_DATA);
}
...
{
#ifndef CC_MTK_LOADER
    if (IO32ReadFldAlign(FORCE_BACKLIGHT_0, FORCE_BACKLIGHT_EN))
    {
        u2Duty = IO32ReadFldAlign(FORCE_BACKLIGHT_0, FORCE_BACKLIGHT_DUTY);
    }
    ...
    if (pParam == NULL)
    {
        ...
        {
            LOG(0, "[SPI] Fail To Retrieve Local Dimming Panel Attribute\n");
        }         
    }
    else
    {
    ...
    else
    {
        ...
        else if (pParam->u1ProtocolIndex == E_SPI_AMS_382X_PROTOCOL)
        {
            if ((u1Freq== 25) || (u1Freq == 50) || (u1Freq == 100))
            {
                vDrvSetPWM(HSYNC_SRC, (BSP_GetDomainClock(SRC_BUS_CLK)/255) / 102350, 0x80, 0xFF); 
            }
            else
            {
...
void vDrvLcdimSpiSetAttr(void)
{
    ...
    UINT16  Tch, Tcld, Tclg;                      // CS setting

    //-------------------------------------------------------
    // 0. set sw / hw mode
    //-------------------------------------------------------    
#ifdef LONG_TIME_LOADER_LOGO
    vIO32WriteFldAlign(LED_CON, SV_OFF, SRAM_HW_MODE);
    ...
    Tcld =  (LDM_BUS_CLOCK*(pParam->u2Tcld)+500)/1000;        
    Tclg =  (LDM_BUS_CLOCK*(pParam->u2Tclg)+500)/1000;        
...
{
#ifndef CC_MTK_LOADER
    UINT8 u2LcdimBlockNum = (UINT8)u2DrvLcdimSpiGetBlockNum();
#endif
    ...
    else if(pParam->u1ProtocolIndex == E_SPI_LEPOWER_1887_PROTOCOL)
    {
        sSpiFunc = &sLePower1887SpiFunc;
    }
    ...
    u1LcdimSpiHeader1 = (u1LcdimScanning | u1LcdimCurrentBoost | u1LcdimH1BSelect | u1LcdimReverse) | 0x01;
  	...
  	// 4. get panel order
#ifndef CC_MTK_LOADER        	
    u2LcdimBlockNum = u2DrvLcdimSpiGetBlockNum();
    DRVCUST_GetLcdimPanelOrder(u1DriverOrder, (UINT8)u2LcdimBlockNum);
    DRVCUST_GetLcdimPanelDimGain(u1BlockDimGain, (UINT8)u2LcdimBlockNum);
    DRVCUST_GetLcdimPanelDelay(u1PanelDelay, (UINT8)u2LcdimBlockNum);
    ...
    // 6. Set dimming lower bound
    u2DimmingLowBound = pParam->u2DimmingLowBound;

    // 7. Define HSYNC source
    HSYNC_SRC = pParam->u1HsyncPort;
...
BOOL bDrvLcdimSpiSetPinmux(UINT8 u1Pinmux)
{
        switch (u1Pinmux) 
        {
            case 0:
#ifdef CC_MT5398            
                BSP_PinSet( PIN_JTDO, PINMUX_FUNCTION2);
                BSP_PinSet( PIN_JTCK, PINMUX_FUNCTION2);
                BSP_PinSet( PIN_JTMS, PINMUX_FUNCTION2);
                BSP_PinSet( PIN_JTDI, PINMUX_FUNCTION2);
                BSP_PinSet( PIN_JTRST_, PINMUX_FUNCTION2);
                return SV_TRUE;      
#else
                return SV_FALSE;      
#endif
...
BOOL bDrvLcdimSpiHWInit(void)
{
    ...
    LOG(1, "[SPI] Init Success, Block H = %d, Block V = %d, Support Scanning = %d\n", pParam->u1BlockNumN, pParam->u1BlockNumM, pParam->u1ScanningOnoff);        
...
#ifdef LONG_TIME_LOADER_LOGO
    #ifdef CC_MT5398
    vIO32WriteFldAlign(LD2_MODE_DS, 0x6, LD_DS);
    #endif
#endif
...
void vDrvLcdimSpiSendDataSWMode(UINT8 *u1DataArray, UINT16 u2Datalen)
{
    ...
    {     
        vDrvLcdimSpiWaitReady(10000);  /* FIXME: changed from 1000 to 10000 */
...
void vDrvLcdimSpi1BReadMultiDev(UINT8 u1DevID, UINT8 u1Addr)
{
    sSpiFunc->vSpiMultiDevRead1BCmd(u1DevID, u1Addr);
}

void vDrvLcdimSpi1BWriteMultiDev(UINT8 u1DevID, UINT8 u1Addr, UINT8 u1Data)
{
    sSpiFunc->vSpiMultiDevWrite1BCmd(u1DevID, u1Addr, u1Data);
}
...
void vDrvLcdimSpiCommandGen(UINT8 u1CommandSet, UINT32 u4CommandParam)
{
    switch(u1CommandSet)
    {
        ...
        case E_SPI_READ_FAULT:             
            ...
            break;                
        case E_SPI_LIGHT_UP:               
             vIO32WriteFldAlign(SWRW_08, SV_ON, SPI_LIGHT_UP);
