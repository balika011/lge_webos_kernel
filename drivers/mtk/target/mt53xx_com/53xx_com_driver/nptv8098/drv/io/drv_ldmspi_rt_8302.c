...
void vDrvRT8302LightUpCmd(void)
{
    ...
    vDrvLcdimSpiSetLoopAllOnOff(SV_OFF);    
    vIO32WriteFldAlign(LED_CON, 1, OP_MODE);  
    vIO32WriteFldAlign(LED_CON, 0, SRAM_HW_MODE);
...
void vDrvRT8302LogOffCmd(void)
{
    ...
    // 2. turn on all current regulator
    vDrvRT8302Write1BCmd(LDM_RT8302_CUR_REGULATOR, LDM_RT8302_IC_VER << 4);
#endif
