...
void vPDSUpdateMddiFifo(UINT8 bPath)
{
    ...
    if (fbmInfo->u4MddiDramWidth != SCPOS_BYPASS_PSCANNR && fbmInfo->u4MddiDramWidth != 0)
    {
...
UINT32 u4PDSUpdatePDSHeight(UINT8 bPath)
{
    ...
    else
    {
        if(bDrvVideoIsSrcInterlace(bPath) && 
            !((bDrvVrmIsAppFlagSet(VRM_APP_VIDEO_ENC)&&(bPath == VDP_2)) || 
            (bDrvVrmIsAppFlagSet(VRM_APP_GRAPHIC_DUMP)&&(u1Scpip_GFX_GetSource() == bPath)))) //only venc and graphic dump do interlace (non-integer) predown
        {
...
{        
    ...
    u4GetHwVps = u4PDSGetVPSFactorReg(VDP_1);
