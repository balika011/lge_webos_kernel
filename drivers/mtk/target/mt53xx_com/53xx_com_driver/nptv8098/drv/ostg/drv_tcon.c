...
void vMiniLVDS_AnalogInit(void)
{
    ...
    else
    {
	 ...
	 #endif
	 vIO32WriteFldMulti(REG_LVDS_TX_CFG0, P_Fld(0x0A, RG_LVDSB_TVCM)|P_Fld(6, RG_LVDSB_NSRC)|P_Fld(1, RG_LVDSB_PSRC));
	 ...
	 #else
	 	#ifdef CC_MT5565
	 		vIO32WriteFldAlign(REG_LVDS_TX_CFG2, 0x0082, RG_LVDSB_CLK_IQ_SEL);
	 	#else
			vIO32WriteFldAlign(REG_LVDS_TX_CFG2, 0x2040, RG_LVDSB_CLK_IQ_SEL);
	 	#endif
	 #endif
...
#else	 
{
	...
	//B group clk regulator power on
...
void vEPI_AnalogOff(void)
{
    ...
    //B group pre driver power down
    vIO32WriteFldAlign(REG_LVDS_TX_CFG2, 0x3fff, RG_LVDSB_PRE_PWD);
    vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_EXT_EN_D11);
    //B group clk regulator power down
    vIO32WriteFldAlign(REG_LVDS_TX_CFG8, 1, RG_LVDSB_LDO_PWD_D11);
    //B group bias current power down
...
void vDrvMiniLVDSOff(void)
{
	...
	vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_EXT_EN_D11); //12bit 
	vIO32WriteFldAlign(REG_LVDS_TX_CFG1, 0xFFF, RG_LVDSB_DRV_PWD); //12bit 
    vIO32WriteFldAlign(REG_LVDS_TX_CFG8, 1, RG_LVDSB_LDO_PWD_D11);
    vIO32WriteFldMulti(REG_LVDS_TX_CFG0,P_Fld( 1, RG_LVDSB_BIAS_ODD_PWD)|P_Fld( 1, RG_LVDSB_BIAS_EVEN_PWD));

	//A Group Power Off Sequence
	vIO32WriteFldAlign(REG_LVDS_TX_CFG9, 0, RG_LVDSA_EXT_EN_D11); //12bit
    vIO32WriteFldAlign(REG_LVDS_TX_CFG10, 0xFFF, RG_LVDSA_DRV_PWD); //12bit
    vIO32WriteFldMulti(REG_LVDS_TX_CFG17, P_Fld( 1, RG_LVDSA_LDO_ODD_PWD_D11)|P_Fld( 1, RG_LVDSA_LDO_EVEN_PWD_D11));
    vIO32WriteFldMulti(REG_LVDS_TX_CFG17,P_Fld( 1, RG_LVDSA_LDO_BIAS_ODD_PWD_D11)|P_Fld( 1, RG_LVDSA_LDO_BIAS_EVEN_PWD_D11));
    vIO32WriteFldMulti(REG_LVDS_TX_CFG9,P_Fld( 1, RG_LVDSA_BIAS_ODD_PWD)|P_Fld( 1, RG_LVDSA_BIAS_EVEN_PWD));
	#else
	...
	//B Group Power Off Sequence
	vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_EXT_EN_D11); //14bit
	vIO32WriteFldAlign(REG_LVDS_TX_CFG1, 0x3FFF, RG_LVDSB_DRV_PWD); //14bit
    vIO32WriteFldAlign(REG_LVDS_TX_CFG8, 1, RG_LVDSB_LDO_PWD_D11);
    vIO32WriteFldMulti(REG_LVDS_TX_CFG0,P_Fld( 1, RG_LVDSB_BIAS_ODD_PWD)|P_Fld( 1, RG_LVDSB_BIAS_EVEN_PWD));
    #endif
...
{
    UINT16 wFlashPanelid=PANEL_TOTAL_NUMBER;

    // implement flash panel mapping here
...
UINT32 u4Data;
   ...
   vIO32WriteFldAlign(GR0, rFlashPanelTable.u2FPGr0TctgFcr, GR0_TCTG_FCR);
    ...
    //Swing and Skew
    //LVDS
    vIO32WriteFldMulti(REG_LVDS_TX_CFG0, P_Fld(rFlashPanelTable.u1FPRgLVDSBTvo, RG_LVDSB_TVO)|
        P_Fld(rFlashPanelTable.u1FPRgLVDSBTvcm, RG_LVDSB_TVCM));

#ifdef CC_MT5398
    vIO32WriteFldMulti(REG_LVDS_TX_CFG9, P_Fld(rFlashPanelTable.u1FPRgLVDSATvo, RG_LVDSA_TVO)|
...
#ifndef CC_MT5881 // MT5881_TODO : mark for 5881 build fail
    _vDrvTconSetPinMux();
#endif
...
UINT8 vDrvTCONRXCRC(UINT8 Mode)
{
	...
	vIO32WriteFldAlign(MLVDS_CTRL_46, 1, RG_TOP_CRC_NEW);
	vIO32WriteFldAlign(MLVDS_CTRL_09, Mode, RG_FIFO_SEL); // 0: LVDS 1: mini-lvds 4:EPI
	
	//CRC Check Frame Range
	vIO32WriteFldAlign(MLVDS_CTRL_46, 0xA, RG_TOP_CRC_V_CNT);
	
	//CRC Check Lines Range
	vIO32WriteFldAlign(MLVDS_CTRL_47, 1, RG_TOP_CRC_H_START); 

  	vIO32WriteFldAlign(MLVDS_CTRL_47, PANEL_GetPanelHeight(), RG_TOP_CRC_H_END);

	vIO32WriteFldAlign(MLVDS_CTRL_48, 0x1FFF, RG_CRC_HLEN_NEW); 
	...
	do
	{
		 ...
		 ||(IO32ReadFldAlign(MLVDS_CTRL_52, ST_TOP_CRC4_FAIL) == 1))
		{
		  ...
		  Printf("R Channel RX_CRC1 Value = %d\n",IO32ReadFldAlign(MLVDS_CTRL_49, ST_TOP_CRC1_VALUE));
		  Printf("R Channel RX_CRC2 Value = %d\n",IO32ReadFldAlign(MLVDS_CTRL_50, ST_TOP_CRC2_VALUE));
		  Printf("R Channel RX_CRC3 Value = %d\n",IO32ReadFldAlign(MLVDS_CTRL_51, ST_TOP_CRC3_VALUE));
		  Printf("R Channel RX_CRC4 Value = %d\n",IO32ReadFldAlign(MLVDS_CTRL_52, ST_TOP_CRC4_VALUE));
		  Printf("\n");
...
UINT8 vDrvMLVDSCRC(void)
{
	...
	vIO32WriteFldAlign(MLVDS_CTRL_46, 0, RG_TOP_CRC_ALG_SEL);
	vIO32WriteFldAlign(MLVDS_CTRL_46, 1, RG_TOP_CRC_NEW);

	//CRC Check Frame Range
	vIO32WriteFldAlign(MLVDS_CTRL_46, 0xC8, RG_TOP_CRC_V_CNT);
    ...
    vIO32WriteFldAlign(MLVDS_CTRL_48, 0xFFF, RG_CRC_HLEN_NEW);  
...
////////////////////////////R Channel CRC Check////////////////////////////////////////////
	...
	//Select L/R Channel to do CRC Check
	vIO32WriteFldAlign(MLVDS_CTRL_48, 0, RG_TOP_CRC_SEL_L); //Select R Channel
...
{	
	...
	UINT8 u1CurrentFcnt = 0;
		
    if(u2Vector == VECTOR_TCON_EH)  
    {
    ...
    VERIFY(BIM_ClearIrq(VECTOR_TCON_EH));
}
...
void vDrvTCONInitISR(void)
{
	...
	if (!u1TCONISRInited)
	{
		if (x_reg_isr(VECTOR_TCON_EH, vDrvTCONISR, &pfnOldIsr) != OSR_OK)
		{
