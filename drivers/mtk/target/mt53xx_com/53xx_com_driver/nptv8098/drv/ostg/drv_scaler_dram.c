...
void vScpipWriteCtrlOnOff(UCHAR ucVdpId, UCHAR ucOnOff)
{
    ...
    if(ucOnOff == SV_ON)
    {
        ...
        if(u4DrvVrmGetAppFlag() == VRM_APP_VIDEO_ENC && ucVdpId == VDP_2)
        {
            ucOnOff = SV_OFF;
        }
...
static void _vScpipUpdateLinePitch(UINT32 u4VdpId)
{
    ...
    UINT8 u1WidthFactor = (u4VdpId== VDP_1)&&fgIs2X ? 2: 1 ;
    ...
    {
        width = MIN(wDrvVideoInputWidth(bPath)*u1WidthFactor, fbmInfo->u4Width);
    }
...
void vScpipUpdateDramReg(UINT32 u4VdpId)
{
    ...
    if((dramInfo->YMsbAddr == SCPIP_READ_FLD(SCPIP_DRAM_M_04+dramOffset, DRAM_M_04_DA_ADDR_BASE_MSB_Y) &&
       ...
       dramInfo->FrameNum == u4ScpipGetBufNum(u4VdpId))||(bMode != VDP_SCPOS_DISPMODE_OFF))
    {
