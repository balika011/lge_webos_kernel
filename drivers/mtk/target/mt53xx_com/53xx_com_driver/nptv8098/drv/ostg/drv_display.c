...
static UINT16 u2DrvHsyncFpFixChk(UINT16 u2Fp) 
{
    if (u2Fp <= ODFPLimit)
    {
        u2Fp = ODFPLimit;
...
static UINT16 u2DrvHsyncBpFixChk(UINT16 u2Bp) 
{
...
void vDrvSetLCDTiming(void)
{
    ...
    u4HPorchF = u2DrvHsyncFpFixChk(u4HPorchF);
	u4HPorchB = u2DrvHsyncBpFixChk(u4HPorchB);
    #ifndef CC_MTK_LOADER        
...
#ifdef SUPPORT_PANEL_SCAN_PWM
	vDrvSetScanBLLatchMode(SCAN_PWM_LATCH_MODE_OFF);
...
void vDrvVideoDitherInit(void)
{
    ...
    if (u4PanelDither != 0xffffffff)
    {
        ...
        #ifdef CUST_SOEM_DRV
        #ifndef CC_MTK_LOADER                
        {    //for sony, enable dither by input timging
            UINT8 bSignalType = 0xFF;
            UINT8 bTV_EN,bAV_EN,bYP_EN,bHDMI_EN,bMM_EN;
            extern UINT8 bGetSignalType(UINT8 bPath);
            bSignalType = bGetSignalType(SV_VP_MAIN);
            
            bTV_EN =   (u4PanelDither & (1<<12))? 1 : 0;
            bAV_EN =   (u4PanelDither & (1<<13))? 1 : 0;
            bYP_EN =   (u4PanelDither & (1<<14))? 1 : 0;
            bHDMI_EN = (u4PanelDither & (1<<15))? 1 : 0;
            bMM_EN =   (u4PanelDither & (1<<16))? 1 : 0;
            if(!(((bSignalType==SV_ST_TV)  && (bTV_EN==SV_ON))
                ||((bSignalType==SV_ST_AV)  && (bAV_EN==SV_ON))
                ||((bSignalType==SV_ST_YP)  && (bYP_EN==SV_ON))
                ||((bSignalType==SV_ST_DVI) && (bHDMI_EN==SV_ON))
                ||((bSignalType==SV_ST_MPEG)&& (bMM_EN==SV_ON))))
            {
                u4Round = 0;
                u4RDither = 0;
                u4EDither = 0;
            ...
            vIO32WriteFldAlign(OS_DITHER_00, OSTG_FPHASE_VALUE, DITHER_FPHASE);  // Running order dithering frame phase increment.
        }
        #endif
        #endif
    }
    ...
    vIO32WriteFldAlign(OS_DITHER_00, 3, DITHER_FPHASE);             // Running order dithering frame phase increment.
    #endif                                                            // 1. If ROUND_EN = 1, the output will only rounding only 
...
// vpll init
	...
	//if ((GetCurrentPanelIndex() == PANEL_CMI_4K2K_FHD120))
	{
		...
		Panel_3DGlassesCTL(e3DGlasses_P_3DL,e3DGlasses_S_DISPR,e3DGlasses_POl_Bypass,e3DGlasses_POS_1stR,0,0);
...
void vDrvLCDSetFrame(UINT16 wFrameRate)
{
    ...
    {
        vDrvLCDFreqSet(SV_DCLK_60HZ, 30);
    }
    else if (wFrameRate==25)
    {
        vDrvLCDFreqSet(SV_DCLK_50HZ, 25);
    }
#ifdef SUPPORT_PANEL_48HZ
    else if (wFrameRate == 24)
    {
        vDrvLCDFreqSet(SV_DCLK_48HZ, 24);
    }
#endif
#endif
    else
...
//    UINT8 bRefreshRate;
    ...
    else if ((bRefreshRate == 24) && (DRVCUST_PanelGet(ePanel242530pOutput) == 0))
    {
            ...
            ((u4DrvTDTV3DModeQuery() != E_TDTV_UI_3D_MODE_OFF) && (u1DrvGetForceOutputFrameRate() & FORCE_DCLK_FORCE_3D_60HZ)))
        {
            bLCDFreqType = SV_DCLK_60HZ;
            bOutFrameRate = PANEL_IsSupport120Hz()? 120: 60;
            LOG(2, "vDDDSLCDSetting: enable 24/48-to-120 display.");
        }
        else
        #endif
        {
    ...
    if (_u14K2KMode==E_4K2K_NATIVE)
    {
        ...
        {
            bOutFrameRate = bRefreshRate;
			Printf("[4k2k]support 242530pOutput: %dHz\n", bOutFrameRate);
        }
    }
#endif
...
void vDddsSettingCheck(void)
{
    ...
    if (_bDddsDelayFlag && (!(_fgAutoSearch)) && SRM_IsMainVideoStrictStable())
    {
        vDDDSLCDSetting(wDrvVideoGetVTotal(SV_VP_MAIN), bDrvVideoGetRefreshRate(SV_VP_MAIN));
...
static void vDrvDisp3DModeCB(void)
{
    ...
    static S_TDTV_OSTG_CONFIG pre_rOstgCnf;
    BOOL u4Osd2, u4Osd3;
	UINT32 u4HTotal,u4FrameRate;
	u4HTotal = 0;
	u4FrameRate=0;
    ...
    {
        if(rOstgCnf.u4FrameRate == 50 || rOstgCnf.u4FrameRate == 100)
        {
            
            if (rOstgCnf.u4FrameRate == 50)
            {
               u4HTotal=(rOstgCnf.u4Clk/60/rOstgCnf.u4VTotal); // for panel table
               u4FrameRate = 60;
            }
            if (rOstgCnf.u4FrameRate == 100)
            {
                u4HTotal=(rOstgCnf.u4Clk/120/rOstgCnf.u4VTotal); // for panel table
                u4FrameRate = 120;
             }            
            u4HTotal &= ~0x1;       
            updatePanelAttribute(u4HTotal,
                             rOstgCnf.u4VTotal,
                             rOstgCnf.u4Width,
                             rOstgCnf.u4Height,
                             u4FrameRate);
        ...
        {
            if (rOstgCnf.u4FrameRate == 60)
            {
                u4HTotal=(rOstgCnf.u4Clk/50/rOstgCnf.u4VTotal); // for panel table
                u4FrameRate = 50;
            }
            if (rOstgCnf.u4FrameRate == 120)
            {
                u4HTotal=(rOstgCnf.u4Clk/100/rOstgCnf.u4VTotal); // for panel table
                u4FrameRate = 100;
             }
        ...
        bOUTPUT_REFRESH_RATE = u13DModeRefreshRate;
		Printf("#####output frame rate is %d\n",bOUTPUT_REFRESH_RATE);
        // clock range to cover 24/25/30Hz output clock
        PANEL_SetPixelClkMin(50000000);
    }
    if(rOstgCnf.u4VTotal != pre_rOstgCnf.u4VTotal || 
        rOstgCnf.u4HTotal != pre_rOstgCnf.u4HTotal||
        rOstgCnf.u4FrameRate!= pre_rOstgCnf.u4FrameRate)
     {
            Printf(" start to chang the panel paremater\n");
        	vDrvSetAllMute(SV_ON);
            OSD_PLA_GetEnable(OSD_PLANE_2, &u4Osd2);
            OSD_PLA_GetEnable(OSD_PLANE_3, &u4Osd3);
            ...
            vDrvVOFreqSet(u43DModeDclk);            
            fgIsVsyncIsrStart=0;
            Printf(" bSI_DISPLAY_DCLK_TYPE:%d\n",bSI_DISPLAY_DCLK_TYPE);
	        vDrvSetLCDTiming();
			x_thread_delay(160); // 20ms * 3vsync * 2 = 120ms
            vErrorHandleSetByPanel();                      
            Printf("[JW]vDrvLVDSConfig @ vDrvDisp3DModeCB\n");      
            OSD_PLA_Enable(OSD_PLANE_2, TRUE);
            OSD_PLA_Enable(OSD_PLANE_3, TRUE);
            OSD_TimingNfy(); 
           // _OSD_ResetFsm();
	        vDrvSetAllMute(SV_OFF);
            Printf(" end to chang the panel paremater\n");
     }
    pre_rOstgCnf = rOstgCnf;       
    #ifdef DRV_SUPPORT_EXTMJC
    vDrvExtmjcRedoTwoChipConnection( SV_VP_MAIN, SV_TRUE);
    #endif
#ifdef DRV_SUPPORT_MT8283
...
static void vSavePanelAttribute_4K2K(void)
{
	...
	// AUO_4K2K30
		...
		rPANEL_4K2K30[SV_DCLK_60HZ].u2Width = rPANEL_FHD120[SV_DCLK_60HZ].u2Width << 1;
		rPANEL_4K2K30[SV_DCLK_60HZ].u2Height = rPANEL_FHD120[SV_DCLK_60HZ].u2Height << 1;
		rPANEL_4K2K30[SV_DCLK_60HZ].u2HTotal = rPANEL_FHD120[SV_DCLK_60HZ].u2HTotal << 1;
		rPANEL_4K2K30[SV_DCLK_60HZ].u2VTotal = rPANEL_FHD120[SV_DCLK_60HZ].u2VTotal << 1;
		rPANEL_4K2K30[SV_DCLK_60HZ].u4PixelClk = rPANEL_FHD120[SV_DCLK_60HZ].u4PixelClk;
		rPANEL_4K2K30[SV_DCLK_60HZ].u4VsyncWidth = rPANEL_FHD120[SV_DCLK_60HZ].u4VsyncWidth;
		rPANEL_4K2K30[SV_DCLK_60HZ].u4VsyncBp = rPANEL_FHD120[SV_DCLK_60HZ].u4VsyncBp;
		...
		rPANEL_4K2K30[SV_DCLK_50HZ].u2Width = rPANEL_FHD120[SV_DCLK_50HZ].u2Width << 1;
		rPANEL_4K2K30[SV_DCLK_50HZ].u2Height = rPANEL_FHD120[SV_DCLK_50HZ].u2Height << 1;
		rPANEL_4K2K30[SV_DCLK_50HZ].u2HTotal = rPANEL_FHD120[SV_DCLK_50HZ].u2HTotal << 1;
		...
		rPANEL_4K2K30[SV_DCLK_48HZ].u2HTotal = rPANEL_FHD120[SV_DCLK_48HZ].u2HTotal << 1;
		...
		rPANEL_4K2K30_COMMON.u4PixleClockMax = rPANEL_FHD120_COMMON.u4PixleClockMax;
		rPANEL_4K2K30_COMMON.u4PixleClockMin = rPANEL_FHD120_COMMON.u4PixleClockMin;
		rPANEL_4K2K30_COMMON.u2HTotalMax = rPANEL_FHD120_COMMON.u2HTotalMax << 1;
		rPANEL_4K2K30_COMMON.u2HTotalMin = rPANEL_FHD120_COMMON.u2HTotalMin << 1;
		rPANEL_4K2K30_COMMON.u2VTotalMax = rPANEL_FHD120_COMMON.u2VTotalMax << 1;
		rPANEL_4K2K30_COMMON.u2VTotalMin = rPANEL_FHD120_COMMON.u2VTotalMin << 1;
		rPANEL_4K2K30_COMMON.u2VClkMax = 31;
		rPANEL_4K2K30_COMMON.u2VClkMin= 23; 
	}
...
void vDrv4K2KChk(UINT8 eCond, UINT8 bSignalMode)
{
	...
	else if(eCond == E_MODE_DONE)
	{
		...
		if(bSignalMode == SV_VDO_STABLE)
		{
			if (fgIs4k2kNativeTimingOut(SV_VP_MAIN))  // 4K2K native mode
			{
				SetPanel_4k2k(E_4K2K_NATIVE);					   
			}					
			else if (fgExt4k2k2D3DTimingChg() && (u4DrvTDTV3DModeQuery()!=E_TDTV_UI_3D_MODE_OFF))
			{
				// set in tdtv callback
			}
#if 0							
			else if ((prHdrInfo.fgIsCMI4k1kVideo == TRUE)&&(!prHdrInfo.fgHdrInvalid))
			{
					Printf("[4k2k][Mloop]###split vdo\n");
					BSP_PinSet(PIN_GPIO53, PINMUX_FUNCTION1);
					SetPanel_4k2k(E_4K2K_SPLIT_VDO);
			}
#endif							
...
void SetPanel_4k2k( UINT8 u1Mode)
{
	...
	if(_4k2kPanelType == i4panelType)
	{
		Printf("[4k2k] skip Set Panel type %d due to same as last time\n", i4panelType);
		x_sema_unlock(_hFrcSetPanelSema);
		return;
	}
	...
	if ((_4k2kPanelType != i4panelType)||(fg2530))
	{	
		...
		u4CtlWord2 = PANEL_GetControlWord2() & 0xFFFFFEFF;	// clean linebuffer bit first
...
#else
	    vDrvVOFreqSet(
	        (bDrvVideoGetRefreshRate(SV_VP_MAIN)==25 || bDrvVideoGetRefreshRate(SV_VP_MAIN)==50 || bDrvVideoGetRefreshRate(SV_VP_MAIN)==100)?
	            (PANEL_GetPixelClk50Hz()):
#ifdef SUPPORT_PANEL_48HZ
	            (bDrvVideoGetRefreshRate(SV_VP_MAIN)==24 || bDrvVideoGetRefreshRate(SV_VP_MAIN)==48)?
	            (PANEL_GetPixelClk48Hz()):
#endif
	            (PANEL_GetPixelClk60Hz()));
#endif 
