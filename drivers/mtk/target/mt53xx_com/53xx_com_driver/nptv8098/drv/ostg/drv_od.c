...
void vDrvODMloop(void)
{
...
#ifdef CC_SUPPORT_OD    
    static UINT8 _u1ODStillCnt=0;
    static UINT8 _ODState =0;
    UINT32 u4MotionLvl;
    UINT32 u4ODStillCntTh;
#endif    
...
#endif
#if OD_FW_WA3
    static UINT32 u4ODMuteCNT = 0;
#endif

#if OD_FW_WA3
    if ((u1ODMute == SV_OFF) && (IO32ReadFldAlign(OD_REG12, RG_OD_START)))
    {
        vDrvODOnOff(SV_OFF);
        u4ODMuteCNT = 0;
    }
    else if ((u1ODMute == SV_ON) && (!IO32ReadFldAlign(OD_REG12, RG_OD_START)))
    {
        if (u4ODMuteCNT > IO32ReadFldAlign(OD_SW_00, OD_SW_CNT))
        {
            vDrvODOnOff(SV_ON);
            u1ODMute = 0xFF;
        }
    ...
    if ((u4DrvTDTV3DModeQuery() != E_TDTV_UI_3D_MODE_OFF) && IO32ReadFldAlign(OD_SW_01, OD_STILL_OFF_EN))
    {
        u4MotionLvl = u4DrvDIGetMotionLevel();
        u4ODStillCntTh =  IO32ReadFldAlign(OD_SW_01, OD_STILL_CNT);
        
        if (u4MotionLvl < IO32ReadFldAlign(OD_SW_01, OD_MOTION_LVL))
        {
            _u1ODStillCnt = (_u1ODStillCnt==u4ODStillCntTh) ? u4ODStillCntTh : (_u1ODStillCnt+1);
            vIO32WriteFldAlign(OD_REG49, 1, ODT_BYPASS_FSYNC);
        }
        else
        {
             _u1ODStillCnt = (_u1ODStillCnt==0) ? 0 : (_u1ODStillCnt-1);
             vIO32WriteFldAlign(OD_REG49, 0, ODT_BYPASS_FSYNC);
        }
        _ODState =1;
    }
    else if (_ODState ==1)
...
void vDrvODInit(void)
{
    ...
    vDrvODMode(0);
        ...
        vDrvODLoadFBTable();
#endif
    vIO32WriteFldAlign(OD_SW_01, 100, OD_MOTION_LVL);
    vIO32WriteFldAlign(OD_SW_01, 60, OD_STILL_CNT);
    vIO32WriteFldAlign(OD_SW_01, 1, OD_STILL_OFF_EN);

    vIO32WriteFldAlign(OD_SW_00, 0x20, OD_SW_CNT);
    fgIsDitherInited = SV_FALSE;
}
