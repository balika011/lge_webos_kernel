...
{
    //vIO32WriteFldAlign(SCPIP_SYSTEM_17, 1, SYSTEM_17_COLOR_3X3_IN_EN);
    VssInited=1;
    LOG(4,"Vss Inited\n");
}
...
{	
    ...
    //vIO32WriteFldAlign(SCPIP_SYSTEM_17,bPath,SYSTEM_17_COLOR_3X3_IN_3DGFX_SOURCE_SEL);
...
UINT8 u1Scpip_GFX_GetCurrentFrameIdx(void)
{
    ...
    //((UINT8)(IO32ReadFldAlign(SCPIP_DRAM_M_03, DRAM_M_03_STA_GFX_WR_FRAME_CNT)));
}
...
UINT8 u1Scpip_GFX_GetWriteStatus(void)
{
    ...
    //(UINT8)(IO32ReadFldAlign(SCPIP_DRAM_M_14, DRAM_M_14_STA_GFX_WRITE_EN)));
}
...
UINT8 u1Scpip_GFX_Set_Frame_Num(UINT8 u1Number)
{
	...
	else
	{
		...
		//vIO32WriteFldAlign(SCPIP_DRAM_M_14,u1Number-1,DRAM_M_14_DA_GFX_FRAME_NUM);
...
{
    UINT8 u1Idx=0;
    UINT32 u4BaseAddr,u4FrameSize;

    if  ( VssInited == 0 ) 
    {
         LOG(4,"Vss not Inited\n");
         return;
    }
    u4BaseAddr=_arGfxInf.u4BassAddr0 << 4 ;
    u4FrameSize=_arGfxInf.u4BassAddrOffset << 4 ;
    
    LOG(4,"~~~ init GFX dram to 0 ~~~\n");

    for(u1Idx=0; u1Idx<_arGfxInf.u1FrameNumber; u1Idx++)
    {
        u4BaseAddr+=(u4FrameSize*u1Idx);
        LOG(4,"start adr=%x  , size=%x .\n",u4BaseAddr,u4FrameSize);
        x_memset((VOID*)(VIRTUAL(u4BaseAddr)) , 0, u4FrameSize);
    }
}
...
UINT8 u1Scpip_GFX_Set_Dram_Address(UINT32 u4BaseAddr, UINT32 u4FrameSize, UINT8 u1FrameNum)
{
    ...
    // update frame number.
    //vIO32WriteFldAlign(SCPIP_DRAM_M_14,u1FrameNum-1,DRAM_M_14_DA_GFX_FRAME_NUM);
    vScpipGfxSetFrameNum(u1FrameNum-1);

    // update frame addresses
    for(u1Idx=0; u1Idx<u1FrameNum; u1Idx++)
    {
        //vIO32WriteFldAlign(SCPIP_DRAM_M_0D+(0x4*u1Idx), u4BaseAddr+(u4FrameSize*u1Idx), DRAM_M_0D_DA_GFX_ADDR_BASE_0);
    ...
    //vIO32WriteFldAlign(SCPIP_DRAM_M_12,_arGfxInf.u4WAddrLow, DRAM_M_12_DA_GFX_WADDR_LO_LIMIT);
    //vIO32WriteFldAlign(SCPIP_DRAM_M_11,_arGfxInf.u4WAddrHigh,DRAM_M_11_DA_GFX_WADDR_HI_LIMIT);
...
UINT8 u1Scpip_GFX_Set_Write_Mode(UINT8 u1Select)
{
    ...
    //vIO32WriteFldAlign(SCPIP_DRAM_M_14,u1Select,DRAM_M_14_DA_GFX_WRITE_EN_MODE);
...
{
	//UINT8 u1Mode;
	//u1Mode = IO32ReadFldAlign(SCPIP_DRAM_M_14, DRAM_M_14_DA_GFX_WRITE_EN_MODE);
	if(_arGfxInf.u1WriteMode)
	{
		//vIO32WriteFldAlign(SCPIP_DRAM_M_14,0,DRAM_M_14_DA_GFX_WRITE_TRIGGER);
		//vIO32WriteFldAlign(SCPIP_DRAM_M_14,1,DRAM_M_14_DA_GFX_WRITE_TRIGGER);
		vScpipGfxWriteTrigger(0);
...
UINT8 u1Scpip_GFX_Write_Enable(UINT8 u1Enable)
{
    if(u1Enable==SV_ON)
    {
        ...
        //vIO32WriteFldAlign(SCPIP_SYSTEM_17, !(_arGfxInf.u1Sel444) ,SYSTEM_17_COLOR_3X3_IN_EN);
        //vIO32WriteFldAlign(SCPIP_SYSTEM_17, _arGfxInf.u1Sel444, SYSTEM_17_SEL444_3X3);
        if(_arGfxInf.u1PathIn == SV_VP_MAIN)
        {
             vScpipGfx422To444Enable(!(_arGfxInf.u1Sel444));
        }
        else
        {
            vScpipGfx422To444Enable(0); //sub already 444, don't need to do 422to444 again
        }

        vScpipGfxBypass3x3(_arGfxInf.u1Sel444);
        //vIO32WriteFldAlign(SCPIP_SYSTEM_17, _arGfxInf.u1HBoundEn, SYSTEM_17_REG_H_BOUND_EN_3X3);
        //vIO32WriteFldAlign(SCPIP_SYSTEM_17, _arGfxInf.u2HBoundValue, SYSTEM_17_REG_H_BOUND_3X3);
    }

    _arGfxInf.u1WriteEnable = u1Enable;
    //vIO32WriteFldAlign(SCPIP_DRAM_M_14,u1Enable,DRAM_M_14_DA_GFX_WRITE_ENABLE);
...
UINT8 u1Scpip_GFX_Set_3DTo2D_En(UINT8 u1VdpId, UINT8 u1Enable)
{
    ...
    //vIO32WriteFldAlign(SCPIP_DRAM_M_13,u1Enable,DRAM_M_13_DA_GFX_3D_TO_2D_EN);
...
UINT8 u1Scpip_GFX_Set_Frame_Drop(UINT8 u1VdpId, UINT8 u1Enable, UINT8 u1DropM, UINT8 u1DropN)
{
	...
	//vIO32WriteFldAlign(SCPIP_DRAM_M_04,u1Enable,DRAM_M_04_DA_GFX_FRAME_DROP_EN);
	//vIO32WriteFldAlign(SCPIP_DRAM_M_04,u1DropM,DRAM_M_04_DA_FRAME_DROP_M);
	//vIO32WriteFldAlign(SCPIP_DRAM_M_04,u1DropN,DRAM_M_04_DA_FRAME_DROP_N);
...
UINT8 u1Scpip_GFX_Set_Alpha(UINT8 u1Alpha)
{
    ...
    //vIO32WriteFldAlign(SCPIP_DRAM_M_14,u1Alpha,DRAM_M_14_DA_GFX_ALPHA);
...
UINT8 u1Scpip_GFX_Set_Dump_Format(SCPIP_GFX_FORMAT Format)
{
    ...
    {
        case RGBA8888:
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,1,DRAM_M_14_DA_GFX_MODE);
			vScpipGfxSetMode(1);
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,0,DRAM_M_14_DA_GFX_ALPHA_POS_SEL);
			vScpipGfxAlphaPosSel(0);
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,2,DRAM_M_14_DA_GFX_MUX_B);
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,1,DRAM_M_14_DA_GFX_MUX_R);
       ...
       case RGBA2101010:
        ...
        case RGB565:
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,2,DRAM_M_14_DA_GFX_MODE);
            vScpipGfxSetMode(2);
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,1,DRAM_M_14_DA_GFX_MUX_B);
            //vIO32WriteFldAlign(SCPIP_DRAM_M_14,2,DRAM_M_14_DA_GFX_MUX_R);
            vScpipGfxSetMux(2, 0, 1);
...
void vScpip_GFX_Query(void)
{
	...
	LOG(0,"path(%d),WEnable(%d),WMode(%d),u1Sel444(%d)\n",_arGfxInf.u1PathIn,_arGfxInf.u1WriteEnable,_arGfxInf.u1WriteMode,_arGfxInf.u1Sel444);
	...
	LOG(0,"3D22D(%d),boundary:en(%d),value(%d)\n",_arGfxInf.u13D22DEnable, _arGfxInf.u1HBoundEn,_arGfxInf.u2HBoundValue);
...
void vScposGfxDumpStart(UINT8 u1Path)
{
	...
	u1Scpip_GFX_Source_Sel(SV_VP_MAIN);
