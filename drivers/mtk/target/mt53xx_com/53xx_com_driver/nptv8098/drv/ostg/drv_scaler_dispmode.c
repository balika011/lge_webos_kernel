...
static void _vScpipUpdatePoclkDivider(UINT8 mode)
{
    ...
    {               
        ...
        {	        
            ...
            && eScpipGetTdtvInType(VDP_1)>SCALER_TDTV_IN_TYPE_2D
            && wDrvVideoGetHTotal(SV_VP_MAIN)*wDrvVideoGetVTotal(SV_VP_MAIN)*bDrvVideoGetRefreshRate(SV_VP_MAIN) >0)
            {
                u4HDMIInClk = (UINT64)dwHDMIPixelFreq();
                u4HDMIInFR = u8Div6464((UINT64)(u4HDMIInClk * 100000),(UINT64)(wDrvVideoGetHTotal(SV_VP_MAIN) * wDrvVideoGetVTotal(SV_VP_MAIN)), NULL);

                if(bDrvVideoIsSrcInterlace(SV_VP_MAIN) == SV_TRUE)
                {
                    u4HDMIInFR = u4HDMIInFR <<1;
                    LOG(0,"Input is I-timing. Double frame rate (%d)\n",u4HDMIInFR);
                    if(bCCIRCENStatus() || (1 == RegReadFldAlign(CCIR_00, CCH_CLK_DIV)))
                    {
                        u4HDMIInFR = u4HDMIInFR >>1;
                	    LOG(0,"I-timing with subsampling");
                    }
                }
                ...
                {
                    u4HDMIInFR = u4HDMIInFR <<1;
                    LOG(0,"Scaler do 3D transform. Double frame rate (%d)\n",u4HDMIInFR);
                }   
                #endif

                LOG(0,"_wHDMIPixelClk  = (%d)\n",_wHDMIPixelClk);
                LOG(0,"dwHDMIPixelFreq = (%d)\n",dwHDMIPixelFreq());
                LOG(0,"DSInputHTotal = (%d)\n",wDrvVideoGetHTotal(SV_VP_MAIN));
                LOG(0,"DSInputVTotal = (%d)\n",wDrvVideoGetVTotal(SV_VP_MAIN));
                LOG(0,"HDMI input frame rate = (%d)\n",u4HDMIInFR);
                LOG(0,"out/in frame rate = (%d)(%d)\n", u4FrameRate, bDrvVideoGetRefreshRate(SV_VP_MAIN));
                dividend = u8Div6464((u4HTotal * u4VTotal * u4HDMIInFR * 0x8000), POCLK_SOURCE_CLK, NULL);
                dividend = u8Div6464(dividend * u4FrameRate , 100 * bDrvVideoGetRefreshRate(SV_VP_MAIN), NULL);
                LOG(0,"dividend = (%d)\n",dividend);
            }
            else
            {
...
void _VDP_DispModeEnable(UCHAR ucVdpId, UCHAR ucMode)
{
    ...
    if (u1ScpipGetDispMode(bPath) != ucMode) 
    {
        ...
        _vDrvVideoSetMute(MUTE_MODULE_SCPOS_DRAM, ucVdpId, 10, TRUE);
