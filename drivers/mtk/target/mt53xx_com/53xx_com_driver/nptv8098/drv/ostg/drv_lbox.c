...
#define LETTER_BOX_LEFT_RIGHT_BOUND             35  ///< 15%   //change 15 to 35 for limit left and right cut max
...
UINT32 _u4VeryStableTime = 20; //20; //4;
...
UINT32 _u4LBOXLeftMaxBound;
UINT32 _u4LBOXRightMaxBound;
static UINT8 _uLBoxLeftRightUpdateFlag = 0;
#endif
...
static UINT32 _u4LBoxFinalUpdateData[4]={0,0,0,0};
...
const UINT32 _au4LboxThresholdValue[LETTER_BOX_APATIVED_THD_NS] = { 48, 48, 64, 74 };//{ 32, 48, 64, 80 };     ///< 4 adaptive threshold of Luma    //change for some 4:3 video HORIZONTAL cut wrong 4/14
...
static void vLBoxGetInputResolution(UINT32 *pu4Width, UINT32 *pu4Height)
{
    ...
    if (pu4Height)
    {
        ...
        u2Height = (UINT16)u4ScpipGetPreScaleHeight(SV_VP_MAIN);
...
{
  static UINT8 u1CounterFlg=0;

    _u4LBoxStableTime = _u4StableTime;
	
// add dectect up and  low	black bar may have noise  
   if( ((_u4LBoxFinalUpdateData[LBOX_IDX_DOWN]!=0)||(_u4LBoxFinalUpdateData[LBOX_IDX_UP] !=0) ))  
	{
		if(((_u4LBoxFinalUpdateData[LBOX_IDX_UP] + _u4LboxInvalidDraft2)<= _rBoundData[LBOX_IDX_UP].u4MaxBound)&&(_u4LBoxFinalUpdateData[LBOX_IDX_DOWN] >= (_rBoundData[LBOX_IDX_DOWN].u4MaxBound + _u4LboxInvalidDraft2)))
		{
			if((_rBoundData[LBOX_IDX_DOWN].u4FinalBound > (_u4LBoxFinalUpdateData[LBOX_IDX_DOWN] + _u4LboxInvalidDraft1))||((_rBoundData[LBOX_IDX_UP].u4FinalBound+ _u4LboxInvalidDraft1)< _u4LBoxFinalUpdateData[LBOX_IDX_UP]))
				{
					...
					LOG(0,"final not update for maybe have back bar noise,Befor(%d,%d),After(%d,%d) counter(%d)\n",_rBoundData[LBOX_IDX_DOWN].u4FinalBound,_rBoundData[LBOX_IDX_UP].u4FinalBound,_u4LBoxFinalUpdateData[LBOX_IDX_UP],_u4LBoxFinalUpdateData[LBOX_IDX_DOWN],u1CounterFlg);
				    if(u1CounterFlg != 0)
						{
								_rBoundData[LBOX_IDX_DOWN].u4FinalBound=_u4LBoxFinalUpdateData[LBOX_IDX_DOWN];
								_rBoundData[LBOX_IDX_UP].u4FinalBound=_u4LBoxFinalUpdateData[LBOX_IDX_UP];
								LOG(0,"back finalbound \n");
				    	}
				}
		}
	}
...
// add for dectect low black bar have caption   
    if( _u4LBoxDetectionValid ==0)
    	{
    		if( LboxIsBoundStable(_rBoundData[LBOX_IDX_UP].u4FinalBound, _u4LBoxFinalUpdateData[LBOX_IDX_UP], _u4LboxInvalidDraft1 ) == 1)
			{
				LOG(3,"in dectect subtitling _u4LboxInvalidDraft2(%d)\n", _u4LboxInvalidDraft2);
				if(_rBoundData[LBOX_IDX_DOWN].u4FinalBound> ( _u4LBoxFinalUpdateData[LBOX_IDX_DOWN] + _u4LboxInvalidDraft2))
				{
						LOG(3,"in dectect subtitling succeed\n");
					  	_u4LBoxDetectionValid =1;
				}
			}
			
			if(_uLBoxLeftRightUpdateFlag == 1)
			{
				_u4LBoxDetectionValid =1;
				
				LOG(3,"in dectect subtitling but left/right cut first\n");
			}
    	}
...
static void LboxReconfig(void)
{
	...
	vLBoxGetInputResolution(&_u4LBoxSrcWidth, &_u4LBoxSrcHeight);
		...
		_u4LBOXLeftMaxBound =( _u4LBoxSrcWidth*LETTER_BOX_LEFT_RIGHT_BOUND)/100;
		_u4LBOXRightMaxBound =(_u4LBoxSrcWidth*(100-LETTER_BOX_LEFT_RIGHT_BOUND))/100;
	#endif
...
static UINT8 LboxUpdateFinalData(void)
{
	...
	u4RightDiff = IO32ReadFldAlign(SIDE_LBOX_STA_00, STA_SUM_BLACK_RIGHT_POS_DIFF);
       ...
       for(i=0 ; i<LBOX_IDX_MAX; i++)
       {
			...
			{
				LOG(4,"in Updatefinaldata i(%d),u4LeftDiff(%d)_u4LBoxSumDiffThreshold(%d)u4RightDiff(%d)\n",i,u4LeftDiff,_u4LBoxSumDiffThreshold,u4RightDiff);
				break;
			}
			#endif 
...
static UINT8 LboxDetectionGetResult(UINT32 *u4YIsSmall, UINT32 *rBeforeIIR, UINT32* rAfterIIR)
{
    ...
    *u4YIsSmall = u4Small;
	...
	if (u4Low < _rBoundData[LBOX_IDX_DOWN].u4MaxBound)
	{
	    ...
	    if (u4Small)
			{
					u4LeftMin  =  _rBoundData[LBOX_IDX_LEFT].u4DetectBound;
					u4RightMax =  _rBoundData[LBOX_IDX_RIGHT].u4DetectBound;
					LOG(3, "Dark Frame Detected u4LeftMin(%d) u4RightMax(%d)\n",u4LeftMin,u4RightMax);
			}
		else
			{
                    u4LeftMin = IO32ReadFldAlign(SIDE_LBOX_STA_02, STA_BLACK_LEFT_POS_MIN);
                     ...
                     //u4RightMin = IO32ReadFldAlign(SIDE_LBOX_STA_02, STA_BLACK_RIGHT_POS_MIN);
                    u4RightMax = IO32ReadFldAlign(SIDE_LBOX_STA_01, STA_BLACK_RIGHT_POS_MAX);
					if(u4LeftMin >_u4LBOXLeftMaxBound)
					{
					   u4LeftMin= _rBoundData[LBOX_IDX_LEFT].u4DetectBound;
					}
					if(u4RightMax < _u4LBOXRightMaxBound)
					{
						u4RightMax =_rBoundData[LBOX_IDX_RIGHT].u4DetectBound;
					}
                     //u4LeftDiff = IO32ReadFldAlign(SIDE_LBOX_STA_00, STA_SUM_BLACK_LEFT_POS_DIFF);
        ...
        while ((_u4LBoxDetectionEnable != 0)||u1WindowLBoxEnable)
        {
            ...
            {            
                ...
                if (u1LBoxReconfigflg != 0)
  		  				{
  		      				LboxReconfig();
  		      				//LboxSetPosSize(); //mask  for  LBOX may can't effect when change channel becuse when Pre factor change LboxSourcechange while may it reconfig
  		 				 }
...
UINT8 LboxQueryRegion(UINT32 *pu4LBoxX, UINT32 *pu4LBoxY, UINT32 *pu4Width, UINT32 *pu4Height, UINT32 *pu4HValid, UINT32 *pu4VValid)
{
	...
	//tt add
	UINT32 _u4LBoxSrcWidth = (UINT32)u4ScpipGetPreScaleWidth(SV_VP_MAIN);	
	UINT32 _u4LBoxSrcHeight = (UINT32)u4ScpipGetPreScaleHeight(SV_VP_MAIN);
    ...
    if ((_u4LBoxDetectionEnable != 0) && (_u4LBoxDetectionValid != 0) && (!bLBOXIsInput444()))   // 444 can not use letterbox detection (detect G only!)
    {
                ...
                )
            {
                ...
                *pu4HValid = 1;
                ...
                u4LBoxY = u4LBoxY * u4Height/_u4LBoxSrcHeight;            
                u4Height= u4NewHeight * u4Height/_u4LBoxSrcHeight;
                //u4Height = u4NewHeight;
				_u4LBoxFinalUpdateData[LBOX_IDX_UP] =_rBoundData[LBOX_IDX_UP].u4FinalBound;
				_u4LBoxFinalUpdateData[LBOX_IDX_DOWN] =_rBoundData[LBOX_IDX_DOWN].u4FinalBound;
				_u4LBoxFinalUpdateData[LBOX_IDX_LEFT] =_rBoundData[LBOX_IDX_LEFT].u4FinalBound;
				_u4LBoxFinalUpdateData[LBOX_IDX_RIGHT] =_rBoundData[LBOX_IDX_RIGHT].u4FinalBound;
				_uLBoxLeftRightUpdateFlag=0;
                #if SUPPORT_HORIZONTAL_LBOX   
