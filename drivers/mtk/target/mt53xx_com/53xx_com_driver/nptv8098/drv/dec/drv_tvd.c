...
#define TVD_LLOCK_EN_PORCH_WA 1

// ---------- Sony bug patchs ---------------------//
#define TVD_PICTURE_WHITE_FLASH	0
#define TVD_BURST_ONOFF_TEST_WA 0
#define TVD_RTM1_CM 0
#ifdef CC_TVD_SUPPORT_ADJUST_HFREQUNCY
#define TVD_ADJUST_HFREQUNCY 1
#else
#define TVD_ADJUST_HFREQUNCY 0
#endif
...
static const INT8 _sTvd_ManualVPorch_Tbl[]=
     {-1, 0, 0, -1, -1, -1, 0, -1};//PAL60 and NTSC443 are Demod_Curf not GameMode
static INT8* _spCurrVPorchTbl = (INT8 *)_sTvd_ManualVPorch_Tbl;
static INT8 _su1UIVPorchDelay=0;
#define DFT_V_PORCH 0x40

#if TVD_LLOCK_EN_PORCH_WA
static INT8 _swIIR_LLOCK_EN_porch_delay_PAL = 0;
static INT8 _swIIR_LLOCK_EN_porch_delay_NTSC = 2;
#endif
...
#if TVD_ADJUST_HFREQUNCY

	if (!IO32ReadFldAlign(TG_STA_05, LINE_STDFH_FLAG))
					
		{
		 
		   vIO32WriteFldAlign(TG_04, SV_ON, LF_OFFSET_EN);
		   LOG(1,"[TVD_DBG_MSG] adjust the line frequency.\n");
		}
...
static void _svDrvTvdSetMenuAvDelay(UINT8 bTvdMode, UINT16 wAvDelay)
{
...
#if 0 // try how many porch delay with cli command
    if (bTvdCtrl(TCTL_FLAG,TC_GETEN,0))
    {
        UINT8 delay = bTvdCtrl(TCTL_FLAG,TC_GETVAL,0);
        switch(bTvdMode)
        {
            case SV_CS_PAL_60:
            case SV_CS_PAL_M:
            case SV_CS_NTSC358:
            case SV_CS_NTSC443:
                _swIIR_LLOCK_EN_porch_delay_NTSC= (delay & 0x7f);
                _swIIR_LLOCK_EN_porch_delay_NTSC*= (delay & 0x80) ? -1:1;
                break;

            case SV_CS_SECAM:
            case SV_CS_PAL:
            case SV_CS_PAL_N:
                _swIIR_LLOCK_EN_porch_delay_PAL = (delay & 0x7f);
                _swIIR_LLOCK_EN_porch_delay_PAL *= (delay & 0x80) ? -1:1;
                break;
    ...
    switch(bTvdMode)
    {
        ...
        case SV_CS_NTSC443:
        #if TVD_LLOCK_EN_PORCH_WA
            wAvDelay = IO32ReadFldAlign(TG_09, IIR_LLOCK_EN)
                ? ((UINT16)(((INT16)wAvDelay) + _swIIR_LLOCK_EN_porch_delay_NTSC))
                : wAvDelay;
        #endif
            LOG(3,">> _svDrvTvdSetMenuAvDelay AV delay(0x%x)\n",wAvDelay);
        ...
        #if TVD_LLOCK_EN_PORCH_WA
            wAvDelay = IO32ReadFldAlign(TG_09, IIR_LLOCK_EN)
                ? ((UINT16)(((INT16)wAvDelay) + _swIIR_LLOCK_EN_porch_delay_PAL))
                : wAvDelay;
        #endif
            LOG(3,">> _svDrvTvdSetMenuAvDelay AV delay(0x%x)\n",wAvDelay);
...
#ifdef	CC_TVD_SUPPORT_FORCE_NTSC_M 	 // this is only for sony only US and COL will force NTSC-3.58
		vIO32WriteFldAlign(CDET_00, SV_ON, TVD_MMODE); 
		vIO32WriteFldAlign(CDET_00, SV_CS_NTSC358, TVD_MODE);
#endif
    }
}
...
void TVD_pm_suspend(void)
{
    ...
    */
    ...
    x_memcpy((void *)pm_save_vsrc2_regs,(void *)VSRC_07,sizeof(pm_save_vsrc2_regs));
...
void TVD_pm_resume(void)
{
    ...
    */
    ...
    x_memcpy((void *)VSRC_07,(void *)(pm_save_vsrc2_regs),sizeof(pm_save_vsrc2_regs));
