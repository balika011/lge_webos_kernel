...
#ifdef CC_SUPPORT_4K2K
	_fgHDMI4k2k = FALSE;
	LOG(1,"[4K2K][HDMI]_fgHDMI4k2k=%d @vDviInitial\n", _fgHDMI4k2k);
#endif
    _bDviDeStableCnt = 0;
    _bDviDeChgCntTHR = 1;
...
UINT8 bHDMI3DIPCheck(void)
{
    if((_bDviTiming == MODE_3D_720p_50_FP)||(_bDviTiming == MODE_3D_720p_60_FP)||(_bDviTiming == MODE_3D_1080p_24_FP)||
       ...
       (_bDviTiming == MODE_3D_1440_240p_60_FP)||(_bDviTiming == MODE_3D_1440_288p_50_FP)||(_bDviTiming == MODE_3D_1440_576p_50_FP)||
    	(_bDviTiming == MODE_3D_720p_25_FP)||(_bDviTiming == MODE_3D_1080p_25_FP)||(_bDviTiming == MODE_3D_1440_480p_60_FP))
    {
...
void vDviConnect(UINT8 bchannel, UINT8 fgIsOn)
{
    ...
    if(fgIsOn == SV_ON)
    {
		...
		//when recording, main is hdmi, sub  follow main will reconnect, trigger mode change, lead to video flash
...
void vDviModeDetect(void)
{
    ...
    switch(_bDviModeDetState)
    {
        ...
        case DVI_SEARCH_STATE:
			...
			if (((_bDviTiming == MODE_2160p_24)||(_bDviTiming == MODE_2160p_25)||(_bDviTiming == MODE_2160p_30)||(_bDviTiming == MODE_4096_2160p_24))
				&& fgIsBypassFlgClr(BYPASS_NPTV_4K2K))
			{
				_fgHDMI4k2k = TRUE;
			}
			else
...
#ifdef CC_SOEM_BOARD
UINT16 wtimeTbl[] = {80,90,100,110,120,\
					 130,140,150,160,170,\
					 180,180,190,190,200,\
					 210,220,220,220,230,\
					 240,250,260,270,280};
UINT16 NoSigLongHpdChkTime(void)
{
	UINT16 wtime;
	wtime = (_wHDMI_OFFON_MUTE_COUNT - 500)/100;// hpd value total 25.
	if(wtime < 0)
	{
		return 3;
	}
	return wtimeTbl[wtime];
}
#endif
/**
...
#ifdef CC_SOEM_BOARD
        if((rCurTime.u4Seconds - _rDviModeChgTime.u4Seconds) > wWaitCnt + (NoSigLongHpdChkTime()/20))
#else
...
#else
	#ifdef CC_SOEM_BOARD
				if(_bDviModeChged && _bPWOFFCnt > NoSigLongHpdChkTime())
	#else
