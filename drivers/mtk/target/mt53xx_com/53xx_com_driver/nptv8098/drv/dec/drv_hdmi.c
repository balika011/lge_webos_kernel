...
//frame rate
UINT8 FRAME_RATE_TABLE_PPL[10]={0,6,12,15,20,24,25,30,50,60};
...
UINT32 u4XtalfreqCnt = FREQCOUNTS_27MHZ;
...
#define TMDS_CLOCK_HIGH1 5
#define TMDS_CLOCK_HIGH2 6
...
static void vHDMIPhyBandCtrl(E_HDMI_SWITCH_NUM ePortl, UINT8 u1Val)
{
    ...
    switch(u1Val)
    {
        ...
        case TMDS_CLOCK_LOW2:  // 30 ~ 40MHz
			...
			case TMDS_CLOCK_MIDDLE1:  // 40 ~ 100MHz
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4RegOffset, 0, RG_HDMI_0_BAND_SEL_SW);
				vIO32Write4B(REG_HDMI_RX_Normal_0_CFG3 + u4RegOffset, 0x00214310);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG4 + u4RegOffset, 0, RG_HDMI_0_PLL_DIVEN);
        ...
        case TMDS_CLOCK_MIDDLE2:  // 100 ~ 160MHz for Cobra ES2 or 40~160 for others 
            vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4RegOffset, 0, RG_HDMI_0_BAND_SEL_SW);
        ...
        case TMDS_CLOCK_HIGH1:  // 160 ~ 240MHz
            ...
            vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG4 + u4RegOffset, 1, RG_HDMI_0_PLL_DIVEN);
            vIO32Write4B(REG_HDMI_RX_Normal_0_CFG5 + u4RegOffset, 0x04000100);
                     if (_bEQTuning == 0)
                     {
			...
			default:
			case TMDS_CLOCK_HIGH2:  // > 240MHz
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4RegOffset, 1, RG_HDMI_0_BAND_SEL_SW);
				vIO32Write4B(REG_HDMI_RX_Normal_0_CFG3 + u4RegOffset, 0x09212310);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG4 + u4RegOffset, 1, RG_HDMI_0_PLL_DIVEN);
				vIO32Write4B(REG_HDMI_RX_Normal_0_CFG5 + u4RegOffset, 0x04000100);
						 if (_bEQTuning == 0)
						 {
...
static void vHdmiPhySetting(E_HDMI_SWITCH_NUM ePort)
{
    ...
    */
    if(BSP_GetIcVersion() == IC_VER_5398_AB)
    {
		...
		{
			u1ClkRate = TMDS_CLOCK_HIGH1;
		}
		else
		{
			u1ClkRate = TMDS_CLOCK_HIGH2;
		}
...
switch(eSel)
{
	...
	case HDMI_SWITCH_1:
		...
		// pip clk
	    if(bGetICInputType(SV_VP_PIP)!=P_OSTGOSDENC &&
...
void vHDMIInit(void)
{
    ...
    for(eHdmiPort = HDMI_SWITCH_1 ; eHdmiPort < HDMI_SWITCH_MAX; eHdmiPort++)
    {
		...
		_arHdmiRx[eHdmiPort]._HPDPULLLowFlag = HPDLOW_WAITNEXT_PULL;
    ...
    for(eHdmiPort = HDMI_SWITCH_1; eHdmiPort < HDMI_SWITCH_MAX; eHdmiPort++)
    {
        ...
        vHDMIPhyBandCtrl(eHdmiPort, TMDS_CLOCK_HIGH2);
        /*TMDS setting*/
...
#if defined(CC_MT5389) || defined(CC_MT5398) || defined(CC_MT5880)//CC_AUD_PLL_HALF
...
static PPL2Info_t info_HDMI_PPL;
PPL2Info_t* API_HDMI_PPL2_Info(void)
{
	PPL2Info_t* info_ppl2 = &info_HDMI_PPL;
	UINT8 tmp;
	UINT16 i,pixel_tmp;

	x_memset(info_ppl2, 0 ,sizeof(PPL2Info_t));
...
//byte 1
	//STG2-0
	if(RegReadFldAlign(SPDRX7 + u4ActiveHdmiRegBase, SPDRX7_0)==0x01)
	{
		info_ppl2->HDMI_ppl2_App1=1;
	}
	else if((RegReadFldAlign(SPDRX7 + u4ActiveHdmiRegBase, SPDRX7_0)==0x02)||(RegReadFldAlign(SPDRX7 + u4ActiveHdmiRegBase, SPDRX7_0)==0x0a))
	{
		info_ppl2->HDMI_ppl2_App1=2;
	}
	else if(RegReadFldAlign(SPDRX7 + u4ActiveHdmiRegBase, SPDRX7_0)==0x08)
	{
		info_ppl2->HDMI_ppl2_App1=4;
	}
	else
	{
		info_ppl2->HDMI_ppl2_App1=5;
	}

	//SC
	if (fgHDMIinterlaced())
	{
		info_ppl2->HDMI_ppl2_App1 &= (~0x08);	//interface signal
	}
	else
	{
		info_ppl2->HDMI_ppl2_App1 |= 0x08;	//progressive signal
	}

//byte2
	//FR3-0
	tmp=bHDMIRefreshRate();
	if(fgHDMIinterlaced())
	{
		tmp=tmp/2;
	}
	if(tmp<=60)
	{
		for(i=0;i<10;i++)
		{
			if(tmp==FRAME_RATE_TABLE_PPL[i])
			{
				info_ppl2->HDMI_ppl2_App2=(i<<4);
				break;
			}
		}
		if(i==10)
		{
			info_ppl2->HDMI_ppl2_App2=0xb0;	
		}
	}
	else
	{
		info_ppl2->HDMI_ppl2_App2=0xa0;
	}
	...
	{
		info_ppl2->HDMI_ppl2_App2|=0x08;	//3D
	}
	else
	{
		info_ppl2->HDMI_ppl2_App2&=(~0x08);	//2D
	}
	...
	if(bHDMIInputType())
	{
		if(bHDMIRgbRange()==2)
		{
			info_ppl2->HDMI_ppl2_App2|=0x04;	//RGB Full range
		}
		if(bHDMIRgbRange()==1)
		{
			info_ppl2->HDMI_ppl2_App2|=0x00;	//RGB Limited Range
		}
	}
	else
	{
	  if ((RegReadFldAlign(AVIRX2 + u4ActiveHdmiRegBase, AVIRX2_0) & 0x40) == 0x40)
	  {
	    info_ppl2->HDMI_ppl2_App2|=0x04;	//YCC Full range
	  }
	  if ((RegReadFldAlign(AVIRX2 + u4ActiveHdmiRegBase, AVIRX2_0)  & 0x40) == 0x00)
	  {
	    info_ppl2->HDMI_ppl2_App2|=0x00;	//YCC Limited Range
	  }
	}
	...
	if(bHDMIInputType())
	{
		info_ppl2->HDMI_ppl2_App2|=0x03;	//RGB color space
	}
	else
	{
		if(bHDMI422Input())
		{
			info_ppl2->HDMI_ppl2_App2|=0x02;	//YCbCr 422 
		}
		else
		{
			info_ppl2->HDMI_ppl2_App2|=0x03;	//YCbCr 444
		}
	}

//byte4	
	pixel_tmp= wHDMIResoWidth();	
	
	if((pixel_tmp!=0)&&((pixel_tmp%16)==0)&&(pixel_tmp<4096))
	{
		for(i=1;i<255;i++)
		{
			if((pixel_tmp/16)==i)
		...
		info_ppl2->HDMI_ppl2_App4=(UINT8)i;
	}
	else if(pixel_tmp>=4096)
	{
		info_ppl2->HDMI_ppl2_App4=255;
	}
	else
	{
		info_ppl2->HDMI_ppl2_App4=0;
	}
//byte5
	pixel_tmp = wHDMIResoHeight(); 
	if(fgHDMIinterlaced())
	{
		pixel_tmp = pixel_tmp*2;
	}

	if((pixel_tmp!=0)&&((pixel_tmp%8)==0)&&(pixel_tmp<2160))
	{
		for(i=1;i<255;i++)
		{
			if((pixel_tmp/8)==i)
		...
		info_ppl2->HDMI_ppl2_App5=(UINT8)i;
	}
	else if(pixel_tmp>=2160)
	{
		info_ppl2->HDMI_ppl2_App5=255;
	}
	else
	{
		info_ppl2->HDMI_ppl2_App5=0;
	}
...
void vHDMIMainLoop(void)
{
    ...
    if (u1EdidInit == 0)
    {
       return;
			...
			if((IO32ReadFldAlign(HDMI_PIP_SEL, REG_VIDEO_OUT_PIP_DATA_SEL) == 0)&&(IO32ReadFldAlign(HDMI_PIP_SEL, REG_VIDEO_OUT_PIP_MAVIS_SEL2) == 0)&&
				(IO32ReadFldAlign(HDMI_PIP_SEL, REG_VIDEO_OUT_PIP_MAVIS_SEL1) == 0)&&(IO32ReadFldAlign(HDMI_PIP_SEL, REG_VIDEO_OUT_PIP_MAVIS_SEL) == 0))
			{
				vCCIROSDSETVSTART();
			}
#elif defined(CC_MT5398)
			if(IO32ReadFldAlign(OMUX_00, OMUX_PIP_SEL) == 0x05)
			{
				vCCIROSDSETVSTART();
			}
#endif
...
BOOL fgHDMICRC(INT16 ntry)
{
    ...
    while(ntry > 0)
    {
        ...
        if((u1IO32Read1B(VID_CRC_CHK_2 + u4ActiveHdmiRegBase)&0x81) == 0x0)
        {
            ...
            {
                LOG(3, "CRC is not ready\n");
                return 0;
            }
        }
        else
        {
            LOG(3, "reset CRC fail");
            return 0;
        }
    }
    ...
    {
        LOG(3, "interlace signal\n");
    }
    else
    {
        LOG(3, "progressive signal\n");
    }
