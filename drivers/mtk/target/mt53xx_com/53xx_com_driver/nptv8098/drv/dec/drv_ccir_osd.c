...
{	
	...
	if((width == 0)&&(height == 0))
	{
		...
		width = wDrvGetOutputHActive();
		height = wDrvGetOutputVActive();
#endif	
...
void vCCIROSDConnect(UINT8 bchannel, UINT8 fgIsOn)
{
	...
	if(fgIsOn == SV_ON)
	{
		...
		//pip data/clk from OSTG
		vIO32WriteFldAlign(CCIR_04_NEW,1,CCH_DATA_CLK_SEL);
		//pip from CCIR1
		vIO32WriteFldAlign(OMUX_00, 0x5, OMUX_PIP_SEL);
	...
	{
		_bCcir2status= 0;
		//pip data/clk from hdmi
		vIO32WriteFldAlign(CCIR_04_NEW,0,CCH_DATA_CLK_SEL);
	}
#endif	
}
/**
