...
UINT8 bVideoUpdateSrcRegion(UINT8 bPath)
{
    ...
    //if input is SBS full, need to divide Source width by 2. Because source region is pixel-based and target width will be divide by 2.
    if (((MODE_3D_1080p_24_SBS_FULL <= bDrvVideoGetTiming(bPath)) &&
        (bDrvVideoGetTiming(bPath) <= MODE_3D_640_480p_60_SBS_FULL)) ||
        ((bPath == SV_VP_MAIN) && u4DrvTDTVIsDecoderContentFull()))
    {
        prPicInfo->wTargetW = prPicInfo->wTargetW << 1;
    }
