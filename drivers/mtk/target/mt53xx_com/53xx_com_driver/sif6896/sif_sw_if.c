...
#endif
        _Sifsw_Delay1us(bBitDelay + _bDelayOffset);		// LG_CHG (ieeum.lee)
...
static void _Sifsw_GetByte(UINT8 *prValue, UINT8 fgSeqRead, UINT8 bBitDelay)
{
    ...
    *prValue = 0;
    ...
    while (bBitMask)
    {
        ...
        _Sifsw_Delay1us(bBitDelay + (_bDelayOffset*4) );		// LG_CHG (ieeum.lee)
                  ...
                  UINT8 *pu1Buf, UINT16 u2ByteCnt)
{
	...
	// LG_CHG (ieeum.lee) for adjusting waveform in case of 100Khz(u2ClkDiv:0x10e).
	if(u2ClkDiv >= 0x100 && u2ClkDiv <= 0x11e)
		_bDelayOffset = 1;
	else
		_bDelayOffset = 0;
