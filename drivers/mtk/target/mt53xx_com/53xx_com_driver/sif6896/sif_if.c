...
#ifdef CC_SAT_USE_ISL_LNB
static UINT32 _fgDVBS_IsSet = 0;
#endif
                   ...
                   UINT8 *pu1Buf, UINT16 u2ByteCnt)
{
    ...
    {
        _SIF_Unlock(); 
		Printf("SIF_X_Write _fgSIFDisabled:\n");
        return u2ByteCnt;
    }
...
void SIF_pm_str_resume(void)
{
	...
	IO_WRITE32MSK(BIM_BASE, RW_SM0_CTRL0_REG + 0x30, SM0EN, SM0EN);
#ifdef CC_SAT_USE_ISL_LNB
	_fgDVBS_IsSet=*((volatile unsigned int *)0xF0029240);
	_fgDVBS_IsSet&=~(1<<6);//clear bit6
	*((volatile unsigned int *)0xF0029240) = _fgDVBS_IsSet;
#endif
