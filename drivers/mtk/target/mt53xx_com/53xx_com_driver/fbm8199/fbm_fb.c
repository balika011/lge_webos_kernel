...
#ifdef FBM_FB_LOG
static void _FbmFbLog(UCHAR u1FbIdx, UCHAR ucFbStatus, const CHAR* sFunc, const UINT32 u4Line);
#define  ASSERT_FBM(x) \
{\
    if(!(x))\
    {\
...
#else
#define _FbmFbLog(x, y, z, zz)     ((void)0)
...
static void _FbmFbLog(UCHAR u1FbIdx, UCHAR ucFbStatus, const CHAR* sFunc, const UINT32 u4Line)
{
    ...
    case FBM_FB_STATUS_EMPTY:
        tDebugLog[u4DebugIdx].cStatus = 'E';
        break;

    case FBM_FB_STATUS_DECODE:
        tDebugLog[u4DebugIdx].cStatus = 'D';
        break;

    case FBM_FB_STATUS_READY:
        tDebugLog[u4DebugIdx].cStatus = 'R';
        break;

    case FBM_FB_STATUS_DISPLAYQ:
        tDebugLog[u4DebugIdx].cStatus = 'Q';
        break;

    case FBM_FB_STATUS_LOCK:
        tDebugLog[u4DebugIdx].cStatus = 'L';
        break;

    case FBM_FB_STATUS_DIGEST:
        tDebugLog[u4DebugIdx].cStatus = 'G';
        break;

    case FBM_FB_STATUS_FIELD_READY:
        tDebugLog[u4DebugIdx].cStatus = 'F';
        break;

    case FBM_FB_STATUS_RESIZE:
        tDebugLog[u4DebugIdx].cStatus = 'S';
        break;

    case FBM_FB_STATUS_RESIZE_DONE:
        tDebugLog[u4DebugIdx].cStatus = 'Z';
        break;

    case FBM_FB_STATUS_RECYCLE:
        tDebugLog[u4DebugIdx].cStatus = 'Y';
        break;
    default:
        tDebugLog[u4DebugIdx].cStatus = 'U';
        break;
    }
    #endif
    ...
    if ((++u4DebugIdx >= FBM_FB_LOG_NS) && (u1FbIdx != 0xFC))
    {
        ...
        {
            LOG(6, "%d: [%d.%06d][%-32s-%4d] FB%d Status%d.\n", u4_idx,
    ...
    if(u1FbIdx == 0xFC)
    {
        ...
        {
            LOG(0, "%d: [%d.%06d][%-32s-%4d] FB%d Status%d.\n", u4_idx,
                tDebugLog[u4_idx].tCallTime.u4Seconds, tDebugLog[u4_idx].tCallTime.u4Micros,
                tDebugLog[u4_idx].sCallFunc, tDebugLog[u4_idx].u4CallLine,  
                tDebugLog[u4_idx].ucFBId, tDebugLog[u4_idx].ucStatus);
        }
...
VOID FBM_TriggerSmlsByResizer(UCHAR ucFbgId, UCHAR ucFbId)
{
    ...
    if (_prFbg[ucFbgId].prPicHdr[ucFbId].fgSeamlessDisp || _prFbg[ucFbgId].fgAutoSeamless || _prFbg[ucFbgId].prPicHdr[ucFbId].fgResizeDispFrm)
    {
        ...
        else if (!_prFbg[ucFbgId].fgEnableNewSeamless)
        {
            ...
            _prFbg[ucFbgId].ucSeamlessRzId = (ucRzId != VDP_PRESCALE_RZ_ID_UNKNOWN) ? ucRzId : VDP_PRESCALE_RZ_ID_DEFAULT;
        }
        ...
        _VDP_Prescale_Send(_prFbg[ucFbgId].ucSeamlessRzId, ucFbgId, ucFbId, _prFbg[ucFbgId].ucCurSeamlessTaskId);
    }
...
BOOL _FBM_2FBsChkDynamicDone(UCHAR ucFbgId)
{
    ...
    LOG(0, "FBG(%d) 2FBs Dynamic Mode(%d) NsOldBase(%d) End.\n", ucFbgId, _prFbg[ucFbgId].eDynamicMode, _prFbg[ucFbgId].ucFbNsOldBase);
 ...
 */
//-------------------------------------------------------------------------
void _FBM_PutFrameBufferToEmptyResizeQ(UCHAR ucFbgId, UCHAR ucFbId)
{
    UCHAR ucNextWriteIdx;
    ...
    if (_prFbg[ucFbgId].aucResizeBlockFb != FBM_FB_ID_UNKNOWN)
    {
        ...
        _prFbg[ucFbgId].prPicHdr[_prFbg[ucFbgId].aucResizeBlockFb].ucSeamlessResizeFbid = ucFbId;
        _prFbg[ucFbgId].aucResizeBlockFb = FBM_FB_ID_UNKNOWN;
    }

    _prFbg[ucFbgId].aucResizeFbStatus[ucFbId] = FBM_FB_STATUS_EMPTY;
    _prFbg[ucFbgId].aucResizeSrcFbId[ucFbId] = FBM_FB_ID_UNKNOWN;
    _prFbg[ucFbgId].aucResizeTaskId[ucFbId] = 0xFF;

    ucNextWriteIdx = (_prFbg[ucFbgId].rResizeEmptyQ.ucReadIdx + _prFbg[ucFbgId].rResizeEmptyQ.ucCount) % FBM_MAX_FB_NS_PER_GROUP;
    _prFbg[ucFbgId].rResizeEmptyQ.aucQueue[ucNextWriteIdx] = ucFbId;
    _prFbg[ucFbgId].rResizeEmptyQ.ucCount++;
}
 ...
 */
//-------------------------------------------------------------------------
EXTERN BOOL VdpChkSequenceInfo(UCHAR ucVdpId, BOOL fgPreChk);
...
void FBM_SetFrameBufferFlag(UCHAR ucFbgId, UINT32 u4Flag)
{
    ...
    if (u4Flag == FBM_FLAG_SEQ_CHG || u4Flag == FBM_FLAG_SEQ_CHG_SPEEDUP)
    {
...
#endif
        if (u4Flag == FBM_FLAG_SEQ_CHG_SPEEDUP && _prFbg[ucFbgId].fgGotFirstSeqChg == FALSE)
        {
        ...
        if (FBM_ChkSeamlessMode(ucFbgId, SEAMLESS_BY_NPTV) && _prFbg[ucFbgId].fgGotFirstSeqChg)
        {
            VdpChkSequenceInfo(0, TRUE);
        }
...
#endif
{
    ...
    _FbmFbLog(ucFbId, ucFbStatus, sFunc, u4Line);
...
//    LOG(7, "Fb(%d, %x) %d >> %d\n", ucFbId, _prFbg[ucFbgId].au4AddrY[ucFbId], _prFbg[ucFbgId].aucFbStatus[ucFbId], ucFbStatus);
    ...
    if (_prFbg[ucFbgId].fgCreateFromInst && ucFbStatus == FBM_FB_STATUS_EMPTY && _prFbg[ucFbgId].aucFbStatus[ucFbId] != FBM_FB_STATUS_EMPTY)
    {
        ...
        _FbmFbLog(ucFbId, ucFbStatus, __func__, __LINE__);
    }
#endif
    switch (ucFbStatus)
    {
...
UCHAR FBM_GetEmptyRefFrameBuffer(UCHAR ucFbgId, UINT32 u4Delay)
{
    ...
    if (ucFbId < FBM_MAX_FB_NS_PER_GROUP)
    {
        _FbmFbLog(ucFbId, FBM_FB_STATUS_DECODE, __func__, __LINE__);
...
UCHAR FBM_GetEmptyFrameBuffer(UCHAR ucFbgId, UINT32 u4Delay)
{
    ...
    else
    {
        ...
        if ((_prFbg[ucFbgId].rDisplayQ.ucCount > 0 && _prFbg[ucFbgId].u4VDecFmt != FBM_VDEC_H264) ||
            (_prFbg[ucFbgId].rDisplayQ.ucCount > 0 && FBM_ChkFrameBufferFlag(ucFbgId, FBM_FLAG_THUMBNAIL_MODE)))
        {
            UCHAR ucTmpId = 0;
...
{
    _FbmFbLog(0xFC, u4FbStatus, __func__, __LINE__);
}
...
BOOL FBM_FlushEmptyBufToDispQ(UCHAR ucFbgId)
{
    ...
    while (_prFbg[ucFbgId].rEmptyQ.ucCount > 0)
    {
        ...
        _FbmFbLog(ucFbId, _prFbg[ucFbgId].aucFbStatus[ucFbId], __func__, __LINE__);
        _prFbg[ucFbgId].prPicHdr[ucFbId].fgNotDisplay = TRUE;
        _FbmPutFrameBufferToDispQ(ucFbgId, ucFbId);
...
BOOL FBM_InsertFB(UCHAR ucFbgId, UINT32 u4Addr)
{
    ...
    if (_prFbg[ucFbgId].rEmptyQ.ucCount >= ucCount)
    {
        ...
        _FbmFbLog(ucFbId, FBM_FB_STATUS_DISPLAYQ, __func__, __LINE__);
        _FbmPutFrameBufferToDispQ(ucFbgId, ucFbId);
        FBM_MUTEX_UNLOCK(ucFbgId);
        return TRUE;
    }
    
    _FbmFbLog(ucFbId, FBM_FB_STATUS_EMPTY, __func__, __LINE__);
    _prFbg[ucFbgId].aucFbStatus[ucFbId] = FBM_FB_STATUS_EMPTY;
    _prFbg[ucFbgId].afgIsOwner[ucFbId] = TRUE;
...
VOID FBM_SetResizeFrameBufferStatus(UCHAR ucFbgId, UCHAR ucFbId, UCHAR ucFbStatus)
{
    ...
    case FBM_FB_STATUS_RESIZE_DONE:
        ...
        {
            LOG(3, "[Warning] Set ResizeDone to non-Resizing frame (%d, %d)!\n", ucFbId, _prFbg[ucFbgId].aucFbStatus[ucFbId]);
            ucFbStatus = FBM_FB_STATUS_EMPTY;
        }
        ...
        {
            LOG(3, "[Warning] Set ResizeDone with src (%d:%d) not in dispQ, id %d/%d\n",
...
UCHAR FBM_GetEmptyResizeFrameBuffer(UCHAR ucFbgId, UCHAR ucSrcFbId, UCHAR ucTaskId, UINT32 u4Delay, UCHAR* pucRetValue)
{
    ...
    *pucRetValue = 0;
    ...
    if (ucFbId < FBM_MAX_FB_NS_PER_GROUP)
    {
        _FbmFbLog(ucFbId, FBM_FB_STATUS_RESIZE, __func__, __LINE__);
...
BOOL FBM_SetDispTag(UCHAR ucFbgId, UCHAR ucFbId,UINT32 u4Tag)
{
    ...
    LOG(5,"FBM_SetDispTag fbg=%d,fb=%d,Tag=0x%x\n",ucFbgId,ucFbId,u4Tag);
    FBM_MUTEX_LOCK(ucFbgId);
...
BOOL FBM_GetFbByDispTag(UINT32 u4Tag, UCHAR *pucFbgId, UCHAR *pucFbId)
{
    ...
    *pucFbId = FBM_FB_ID_UNKNOWN;
            ...
            if(_prFbg[ucFbgId].u4DispTag[ucFbId] == u4Tag)
            {
                ...
                *pucFbId = ucFbId;
                _prFbg[ucFbgId].u4DispTag[ucFbId]=INVALID_DISPTAG;
                 LOG(5,"FBM_GetFbByDispTag fbg=%d,fb=%d,Tag=0x%x\n",ucFbgId,ucFbId,u4Tag);
                 FBM_MUTEX_UNLOCK(ucFbgId);
                 return TRUE;
            }
        }
    ...
    LOG(3,"FBM_GetFbByDispTag Fail Tag=0x%x\n",u4Tag);
    return FALSE;
}
    ...
    for (ucFbgId = 0; ucFbgId < FBG_MAX_NS; ucFbgId++)
    {
        if(VERIFY_FBG(ucFbgId)==FALSE)
        {
           ...
           for (ucFbId=_prFbg[ucFbgId].ucFbNsBase; ucFbId<_prFbg[ucFbgId].ucFbNs; ucFbId++)
           {
                ...
                {
                   _prFbg[ucFbgId].u4DispTag[ucFbId]=INVALID_DISPTAG;
                   LOG(5,"FBM_ReleaseBufferByDispTag fbg=%d,fb=%d,Tag=0x%x\n",ucFbgId,ucFbId,u4Tag);
