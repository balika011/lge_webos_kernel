...
{
    if( IO32ReadFldAlign(TDNAVI_FW_00, TDNAVI_SW_LOG)!= 0)
    {
    	printf(" SW result :%d\n",final_sta_det_type);
    }
...
//    min_vld_blk = (sbs_vld_blk<=tab_vld_blk)? sbs_vld_blk : tab_vld_blk;
    ...
    if((TD_NAVI_DETECT_SBS==g_u1TdNaviDetCurr)||(TD_NAVI_DETECT_TAB==g_u1TdNaviDetCurr))
    {
        c_d_th=c_d_th_out;    
    }
    else
	...
	if( IO32ReadFldAlign(TDNAVI_FW_00, TDNAVI_SW_LOG)!= 0)
    {
    	printf("like_sbs:%d, wt_sbs_vld_blk:%d,like_tab:%d,wt_tab_vld_blk:%d, SwNaviPlusDetected:%d\n",
			like_sbs,wt_sbs_vld_blk,like_tab,wt_tab_vld_blk,SwNaviPlusDetected);
    }
    ...
    //final decision
    valid_sbs_ratio = (sta_sbs_vld_cnt==0) ? 0 : sta_sbs_cnt*0x100 / sta_sbs_vld_cnt;
    valid_tab_ratio = (sta_tab_vld_cnt==0) ? 0 : sta_tab_cnt*0x100 / sta_tab_vld_cnt;

    hw_valid_sbs_ratio = (sta_hw_sbs_vld_cnt==0) ? 0 : sta_hw_sbs_cnt*0x100 / sta_hw_sbs_vld_cnt;
    hw_valid_tab_ratio = (sta_hw_tab_vld_cnt==0) ? 0 : sta_hw_tab_cnt*0x100 / sta_hw_tab_vld_cnt;

    if(IO32ReadFldAlign(TDNAVI_FW_00, TDNAVI_SBS_LOG))
    {
        LOG(0,"[Plus SBS]Fmt =%1x SbsCnt = %2x SbsVld = %2x sbsRat = %2x\n"
            , SwNaviPlusDetected ,sta_sbs_cnt,sta_sbs_vld_cnt, valid_sbs_ratio);
        LOG(0,"[sta_gmv_s0]%d [sta_gmv_s1]%d [sta_gmv2_s0]%d [sta_gmv2_s1]%d\n"
            ,gmv_s0,gmv_s1,gmv2_s0,gmv2_s1); 
    }

    if(IO32ReadFldAlign(TDNAVI_FW_00, TDNAVI_TAB_LOG))
    {
        LOG(0,"[Plus TAB]Fmt =%1x TabCnt = %2x TabVld = %2x TabRat = %2x\n"
            , SwNaviPlusDetected,sta_tab_cnt,sta_tab_vld_cnt, valid_tab_ratio);
        LOG(0,"[sta_gmv_t0]%d [sta_gmv_t1]%d [sta_gmv2_t0]%d [sta_gmv2_t1]%d\n"
            ,gmv_t0,gmv_t1,gmv2_t0,gmv2_t1);         
    }
    ...
    if((SwNaviPlusDetected==TD_NAVI_DETECT_UNKNOWN)||
         ((valid_sbs_ratio<c_fw_ratio_gain_th+hw_valid_sbs_ratio)&&
          ...
          (hw_valid_sbs_ratio*c_fw_hw_ratio_weight+valid_sbs_ratio*(4-c_fw_hw_ratio_weight))/4<c_d_th&&
          (hw_valid_tab_ratio*c_fw_hw_ratio_weight+valid_tab_ratio*(4-c_fw_hw_ratio_weight))/4<c_d_th))
...
void vDrvTDTVTDNAVIDumpBuf(void *pvArgs)
{
    ...
    while(1)
    {
        ...
        vRegWriteFldAlign(TDNVAI_0B     ,SV_ON  ,FW_READ_EN);        
        ...
        for(u4Index = 0; u4Index < 480; u4Index++)
        {
            ...
            VARBuf[u4Index] = RegReadFldAlign(TDNVAI_STA1,TDNVAI_FPGA_02_BLK_PIXEL_DIFF);
            APLBuf[u4Index] = RegReadFldAlign(TDNVAI_STA1,TDNVAI_FPGA_02_BLK_Y);           
        }
        
        vRegWriteFldAlign(TDNVAI_0B,SV_OFF,FW_READ_EN);
        vRegWriteFldAlign(TDNVAI_0B,0,FW_ADDRESS);
...
void vDrvTDTVTDNAVIFillBuf(void)
{
    ...
    for(j=0;j<NAVI_LINE_HEIGHT/2;j++)
    {
        ...
        for(i=0;i<30/2;i++)    
        {
            blk_var[0][j*NAVI_LINE_WIDTH/2 + i]   = VARBuf[j*30 + i];
            blk_var[1][j*NAVI_LINE_WIDTH/2 + i]   = VARBuf[j*30 + i + 30/2];
            blk_var[2][j*NAVI_LINE_WIDTH/2 + i]   = VARBuf[(j+NAVI_LINE_HEIGHT/2)*30 + i];
            blk_var[3][j*NAVI_LINE_WIDTH/2 + i]   = VARBuf[(j+NAVI_LINE_HEIGHT/2)*30 + i + 30/2];

            blk_buf[0][j*NAVI_LINE_WIDTH/2 + i]   = APLBuf[j*30 + i];
            blk_buf[1][j*NAVI_LINE_WIDTH/2 + i]   = APLBuf[j*30 + i + NAVI_LINE_WIDTH/2];
            blk_buf[2][j*NAVI_LINE_WIDTH/2 + i]   = APLBuf[(j+NAVI_LINE_HEIGHT/2)*30 + i];
...
void vDrvTDTVTDNAVIDumpInit(void)
{
    ...
    VERIFY(x_sema_create(&_hTDNAVIVLoopCheck       , X_SEMA_TYPE_BINARY, X_SEMA_STATE_LOCK) == OSR_OK);   	
    VERIFY(x_thread_create(&hThread, "TDNAVI Dump Thread", 1024, 90,vDrvTDTVTDNAVIDumpBuf, 0,NULL) == OSR_OK);    
}
