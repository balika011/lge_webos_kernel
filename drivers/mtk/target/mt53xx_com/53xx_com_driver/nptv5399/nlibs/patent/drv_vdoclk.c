...
#ifdef CC_MT5399
#define PIX_CLK_INV_Criterion 350
#endif

#ifdef CC_MT5882 
#define PIX_CLK_INV_Criterion 615
#endif
...
void vDrvADCPLLSet(UINT16 wPixClk, UINT16 wHtotal)
{
   ...
   //vIO32WriteFldAlign(CKGEN_DTDPLLCFG0, 0, FLD_RG_ADCPLL2_DE_FB_CN);
    ...
    #ifdef CC_MT5399// for 5399 108M vga auto problem
    if(wPixClk>=PIX_CLK_INV_Criterion)
	#else //for 5882 162M vga auto problem
	if(wPixClk<=PIX_CLK_INV_Criterion)	
	#endif
    {
...
void vYPbPrPhaseIsr_New(void)    //NEW_PHASE_ALGORITHM
{
		...
		{
			vApiNotifyAutoDone(0,TRUE);
			#if NOTIFY_PROGRESS
			vApiNotifyVGAAutoProgress(0,100);
			#endif
		}
		else 
		{
			vApiNotifyAutoDone(1,TRUE);
			#if NOTIFY_PROGRESS
			vApiNotifyVGAAutoProgress(1,100);
			#endif
		}
		#else
...
void vYPbPrPhaseIsr(void)//not used
{
	if (_bCurPhase <= 32)
	{
	       vPhaseSearchCore();
...
void vDrvDDDS_Response(void)
{
    ...
    switch (_bDDDSCaliState)
    {
        ...
        case DDDS_CALI_START:
             ...
             dwTmp= u4IO32Read4B(0xf000240c);     
             vIO32Write4B(0xf000240c, 0xffffffff);   //disable all interrupt
             ...
             CW[0] = _wControlWord;   
			 fglock[0] = IO32ReadFldAlign(STA_DDDS_00,STA_DDDS_00_DDDS_LOCK);
...
#ifndef CC_MT5399
#define CAPRI_SYSPLL_CLK_SS 648000000
#endif
...
void vDDDSInit(void)
{
    ...
    // Set error limit
	if ((IS_PANEL_L12R12)||(IS_PANEL_2D_N_3D_L12R12))
...
#define RG_PDREV_0_1 Fld(8,0,AC_FULLB0)//[7:0]
...
void vDrvSetVopllVcoMonitor(UINT8 bType)
{
	...
	#else
	// 	ADIN1_SRV
	vIO32WriteFldAlign(0xf0028900, 0x3C, RG_PDREV_0_1);
    #endif
    ...
    if(bType)
    {
        ...
        vIO32WriteFldAlign(REG_VOPLL_CFG4, 0x1, RG_PPLL_MONEN);
        vIO32WriteFldAlign(REG_VOPLL_CFG3, 0x2, RG_VPLL_TST_SEL);
    }
    else
    {
        Printf("VPLL Vtrl measure \n");
        vIO32WriteFldAlign(REG_VOPLL_CFG4, 0x0, RG_PPLL_MONEN);
        vIO32WriteFldAlign(REG_VOPLL_CFG2, 0x1, RG_VPLL_MONEN);
        vIO32WriteFldAlign(REG_VOPLL_CFG3, 0x3, RG_VPLL_TST_SEL);
    }
