...
#ifdef CC_MT5890
#define UART_VECTOR(u1Port)  (UINT32)((u1Port == (UART_PORT_PD) || (u1Port == (UART_PORT_HT) || (u1Port == (UART_PORT_BT)) ? (VECTOR_PDWNC) : (VECTOR_RS232))
#else
...
#define UART_TX_DMA_BLOCKSIZE    (256)
	#else
#define UART_TX_DMA_BLOCKSIZE    (1024)
	#endif
#endif
//---------------------------------------------------------------------------
// Imported functions
...
#ifdef CC_MT5881
static UINT8 _u1SentByOthers = 0;
#endif
 ...
 */
//-------------------------------------------------------------------------
static void _Uart_HT_Isr(UINT16 u2VctId)
{
    UINT32 i;
    UINT32 u4RxCnt, u4TxCnt;
    UINT32 u4RomeLeft, u4IntStatus;
    ...
    u4IntStatus = UART_HwGetIntStatus(UART_PORT_HT);

    UART_HwClearIntStatus(UART_PORT_HT,(~u4IntStatus));//clear uart pd int status, bit set 0 to clear
    ...
    // Case1: Overflow/Error 
    if (CHK_UART_RX_OVERFLOW(UART_PORT_HT, u4IntStatus) ||
        CHK_UART_RX_ERROR(UART_PORT_HT, u4IntStatus))
    {
        LOG(0, "_Uart_HT_Isr() overflow, error status = 0x%08X !!!\n",
               (unsigned int)u4IntStatus);
            
        // Clear Rx buffer
        UART_HwClearFifo(UART_PORT_HT, CLEAR_RX_BUFFER);
    }
    ...
    // if yes a. read data to software fifo 
    //        b. call notify function
    if (CHK_UART_DMA_RX_INT(UART_PORT_HT, u4IntStatus))
    {
        prRxFIFO = &_arRxFIFO[UART_PORT_HT];
            
        Buf_GetRoomLeft(prRxFIFO, u4RomeLeft);

        u4RxCnt = UART_HwGetRxDataCnt(UART_PORT_HT);
        //ASSERT(u4RxCnt);            
        u4RxCnt = MIN(u4RomeLeft, u4RxCnt);
        ...
        if (u4RxCnt > 0)
        {
            ...
            for (i = 0; i < u4RxCnt; i++)
            {
                u1Data = UART_READ_DATA(UART_PORT_HT);

                // push to software fifo.
                Buf_Push(prRxFIFO, u1Data);
            }
            _Uart_SendRcvDataEvent(UART_PORT_HT, u4RxCnt);
        }            
    }        
    ...
    if (CHK_UART_TX_INT(UART_PORT_HT, u4IntStatus))
    {
        prTxParam = &_arTxParam[UART_PORT_HT];

        ASSERT(prTxParam->u4Port == (UINT32)UART_PORT_HT);
        ASSERT(prTxParam->pData);

        u4TxCnt = UART_HwGetTxEmptyCnt(UART_PORT_HT);
        ASSERT(prTxParam->u4FifoLength <= u4TxCnt);
        UNUSED(u4TxCnt);
        ...
        if (prTxParam->u4TotalLength > prTxParam->u4ActualLength)
        {
            ...
            prTxParam->u4FifoLength = 
                MIN((prTxParam->u4TotalLength - prTxParam->u4ActualLength),
                    prTxParam->u4FifoLength);
            UART_HwDiableTxIntr(UART_PORT_HT);                                          
            UART_HwWrite(UART_PORT_HT, pData, prTxParam->u4FifoLength);
            if (prTxParam->u4FifoLength > 1)
            {
                UART_CLR_BIT(REG_SERPD_INT_STATUS, INT_SER1_TX_EMPTY);
            }
            UART_HwEnableTxIntr(UART_PORT_HT);            
        }
        else
        {
            ASSERT(prTxParam->u4TotalLength == prTxParam->u4ActualLength);
            ...
            if (prTxParam->fgBlockedMode)
            {
                VERIFY(x_sema_unlock(_ahTxNotifySema[UART_PORT_HT]) == OSR_OK);
            }
            else
            {
                ASSERT(_arRs232NfyInfo[UART_PORT_HT].pf_rs_232_nfy);
                _arRs232NfyInfo[UART_PORT_HT].pf_rs_232_nfy(
                    (VOID *)_arRs232NfyInfo[UART_PORT_HT].pv_tag, 
                    RS_232_COND_XMT_EMPTY, _arTxPrivateTag[UART_PORT_HT]);
            }
        }
    }
}
 ...
 */
//-------------------------------------------------------------------------
static void _Uart_BT_Isr(UINT16 u2VctId)
{
    UINT32 i;
    UINT32 u4RxCnt, u4TxCnt;
    UINT32 u4RomeLeft, u4IntStatus;
    ...
    u4IntStatus = UART_HwGetIntStatus(UART_PORT_BT);

    UART_HwClearIntStatus(UART_PORT_BT,(~u4IntStatus));//clear uart pd int status, bit set 0 to clear
    ...
    // Case1: Overflow/Error 
    if (CHK_UART_RX_OVERFLOW(UART_PORT_BT, u4IntStatus) ||
        CHK_UART_RX_ERROR(UART_PORT_PD, u4IntStatus))
    {
        LOG(0, "_Uart_BT_Isr() overflow, error status = 0x%08X !!!\n",
               (unsigned int)u4IntStatus);
            
        // Clear Rx buffer
        UART_HwClearFifo(UART_PORT_BT, CLEAR_RX_BUFFER);
    }
    ...
    // if yes a. read data to software fifo 
    //        b. call notify function
    if (CHK_UART_DMA_RX_INT(UART_PORT_BT, u4IntStatus))
    {
        prRxFIFO = &_arRxFIFO[UART_PORT_BT];
            
        Buf_GetRoomLeft(prRxFIFO, u4RomeLeft);

        u4RxCnt = UART_HwGetRxDataCnt(UART_PORT_BT);
        //ASSERT(u4RxCnt);            
        u4RxCnt = MIN(u4RomeLeft, u4RxCnt);
        ...
        if (u4RxCnt > 0)
        {
            ...
            for (i = 0; i < u4RxCnt; i++)
            {
                u1Data = UART_READ_DATA(UART_PORT_BT);

                // push to software fifo.
                Buf_Push(prRxFIFO, u1Data);
            }
            _Uart_SendRcvDataEvent(UART_PORT_BT, u4RxCnt);
        }            
    }        
    ...
    if (CHK_UART_TX_INT(UART_PORT_BT, u4IntStatus))
    {
        prTxParam = &_arTxParam[UART_PORT_BT];

        ASSERT(prTxParam->u4Port == (UINT32)UART_PORT_BT);
        ASSERT(prTxParam->pData);

        u4TxCnt = UART_HwGetTxEmptyCnt(UART_PORT_BT);
        ASSERT(prTxParam->u4FifoLength <= u4TxCnt);
        UNUSED(u4TxCnt);
        ...
        if (prTxParam->u4TotalLength > prTxParam->u4ActualLength)
        {
            ...
            prTxParam->u4FifoLength = 
                MIN((prTxParam->u4TotalLength - prTxParam->u4ActualLength),
                    prTxParam->u4FifoLength);
            UART_HwDiableTxIntr(UART_PORT_BT);                                          
            UART_HwWrite(UART_PORT_BT, pData, prTxParam->u4FifoLength);
            if (prTxParam->u4FifoLength > 1)
            {
                UART_CLR_BIT(REG_SERPD_INT_STATUS, INT_SER1_TX_EMPTY);
            }
            UART_HwEnableTxIntr(UART_PORT_BT);            
        }
        else
        {
            ASSERT(prTxParam->u4TotalLength == prTxParam->u4ActualLength);
            ...
            if (prTxParam->fgBlockedMode)
            {
                VERIFY(x_sema_unlock(_ahTxNotifySema[UART_PORT_BT]) == OSR_OK);
            }
            else
            {
                ASSERT(_arRs232NfyInfo[UART_PORT_BT].pf_rs_232_nfy);
                _arRs232NfyInfo[UART_PORT_BT].pf_rs_232_nfy(
                    (VOID *)_arRs232NfyInfo[UART_PORT_BT].pv_tag, 
                    RS_232_COND_XMT_EMPTY, _arTxPrivateTag[UART_PORT_BT]);
            }
        }
    }
}
...
static void _Uart_Isr(UINT16 u2Vector)
{
...
#ifndef CC_NO_KRL_UART_DRV
	if (u2Vector != VECTOR_RS232 || u2Vector != VECTOR_UART_DMA)
	{
		return;
	}
#else
	if (u2Vector != VECTOR_UART_DMA)
	{
		return;
	}
#endif
	...
	// for tx empty test, use gpio to compare //must put OPCTRL0 high before	
#if 0
	if( u4IntStatus & 0x80000 )// dbg_tx us 0x80000
	{
		vIO32Write4B(0xf0028074,0);// set OPCTRL0 low
		HAL_Delay_us(15000);// delay 15ms
		vIO32Write4B(0xf0028074,0);// set OPCTRL0 high
	}
#endif
...
#else
					HalFlushInvalidateDCacheMultipleLine(VIRTUAL((UINT32)pData), prTxParam->u4FifoLength);
#endif
...
static void _Uart_SwInit(void)
{
    ...
    for (i4Idx = 0; i4Idx < (INT32)UART_PORT_NUM; i4Idx++)
    {
	   ...
	   if(i4Idx == 1)
	   {
			Buf_init(&_arRxFIFO[i4Idx], (void *)_au1VirBuf, SW_FIFO_SIZE);
	   }
	   else
	   {
#endif   	
	...
	VERIFY(x_reg_isr(VECTOR_UART_DMA, _Uart_Isr, &pfnOldIsr) == OSR_OK);
#else
	VERIFY(x_reg_isr(VECTOR_UART_DMA, _Uart_Isr, &pfnOldIsr) == OSR_OK);
#endif
...
#ifdef CC_MT5890
	VERIFY(PDWNC_RegIsr((UINT16)5, _Uart_HT_Isr, &pfnOldIsr) == OSR_OK);
	VERIFY(PDWNC_RegIsr((UINT16)6, _Uart_BT_Isr, &pfnOldIsr) == OSR_OK);
#endif
#endif /* CC_UART_PD_SUPPORT */
}
...
UINT32 UART_Read(UINT8 u1Port, UINT8 *pBuffer, UINT32 u4NumToRead, BOOL fgBlockedMode)
{
    ...
    if (fgBlockedMode)
    {
        while (u4NumRead < u4NumToRead)
        {
            ...
            {
                *pBuffer = '?';  //just a character, the value is indifference
                u4DataAvail = 1;
                pBuffer += u4DataAvail;
                u4NumRead += u4DataAvail;            
                _u1SentByOthers = 0;
...
UINT32 UART_Write(UINT8 u1Port, const UINT8 *pData, UINT32 u4NumberToWrite, BOOL fgBlockedMode)
{
    ...
    {
#ifdef CC_NO_KRL_UART_DRV		
		LOG(5, "[UART_Write]-1 Port(%d)Num(%d)\n", UART_HwGetTxEmptyCnt(u1Port), u4NumberToWrite);
#endif
        UART_HwDiableTxIntr(u1Port);
    ...
    {
#ifdef CC_NO_KRL_UART_DRV		
		LOG(5, "[UART_Write]-2 Port(%d)Num(%d)\n", UART_HwGetTxEmptyCnt(u1Port), u4NumberToWrite);
#endif
        prTxParam = &_arTxParam[u1Port];
...
#else
		u4RxPhyaddr = BSP_AllocAlignedDmaMemory(SW_FIFO_SIZE,4);
		u4RxViraddr = VIRTUAL(u4RxPhyaddr);
		UART_HwDmaWriteDram(u4RxViraddr, SW_FIFO_SIZE, 1);
		_au1VirBuf=u4RxViraddr;
#endif
...
#ifdef CC_NO_KRL_UART_DRV
void UART_ModemCtr(BOOL fgEnable)
{
	UART_HwModemCtr(fgEnable);
}
#endif
...
#else
			pu1WBuf[u4i] = (UINT8)BIM_READ32(0x74);//TIMER_LOW_REG = 0x74
#endif
            pu1RBuf[u4i] = 0x00;
        }
...
#ifdef CC_MT5881
//[DTV00419251] USB-PL2303 support. porting nuclues support usb serial driver.
INT32 MUSB_Uart_InputSwitch(void)
{
	return UART_InputSwitch();
}

void MUSB_Uart_InOutQuery(void)
{
	_Uart_InOutQuery();
}

void MUSB_Uart_OutputSwitch(void)
{
	_Uart_OutputSwitch();
}

INT32 MUSB_Uart_Get_Input_Mode(void)
{
	return _u4InputMode;
}

void MUSB_Uart_Set_Input_Mode(INT32 u4Value)
{
	_u4InputMode = u4Value;
	return;
}

UINT32 MUSB_Uart_Get_FactoryFlag(void)
{
	return _u4FactoryFlag;
}

void MUSB_Uart_Set_FactoryFlag(UINT32 u4Value)
{
	_u4FactoryFlag = u4Value;
}

UINT32 MUSB_Uart_Get_MagicInputForceFactory(UINT8 bMaxfg)
{
	return bMaxfg ? _u4MagicMaxInputForceFactory : _u4MagicMinInputForceFactory;
}

void MUSB_Uart_Set_MagicInputForceFactory(UINT8 bMaxfg, UINT32 u4Value)
{
	if(bMaxfg)
		_u4MagicMaxInputForceFactory = u4Value;
	else
		_u4MagicMinInputForceFactory = u4Value;
}
#endif
