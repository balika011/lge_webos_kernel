...
#ifdef CC_MT5890        
	case UART_PORT_HT:								\
		u4RegAddr = REG_SERHT_CTR;					\
		break;										\
	case UART_PORT_BT:								\
		u4RegAddr = REG_SERBT_CTR;					\
		break;										\
#endif        
	default:										\
		return; 									\
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		UART_WRITE8(REG_SERHT_PORT, u1Data);
		break;
    case UART_PORT_BT:
        UART_WRITE8(REG_SERBT_PORT, u1Data);
        break;	
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		return (UART_READ8(REG_SERHT_PORT));
    case UART_PORT_BT:
        return (UART_READ8(REG_SERBT_PORT));
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		u4RegAddr = REG_SERHT_STATUS;
		break;
    case UART_PORT_BT:
        u4RegAddr = REG_SERBT_STATUS;
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		UART_SET_BIT(REG_SERHT_INT_EN, INT_SERPD_TX_EMPTY);
		break;
    case UART_PORT_BT:
        UART_SET_BIT(REG_SERBT_INT_EN, INT_SERPD_TX_EMPTY);
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		UART_CLR_BIT(REG_SERHT_INT_EN, INT_SERPD_TX_EMPTY);
		break;
    case UART_PORT_BT:
        UART_CLR_BIT(REG_SERBT_INT_EN, INT_SERPD_TX_EMPTY);
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
...
void UART_HwModemCtr(BOOL fgEnable)
{
    if (fgEnable)
    {
		...
		BSP_PinSet(PIN_GPIO49, PINMUX_FUNCTION3);//cts
		BSP_PinSet(PIN_GPIO50, PINMUX_FUNCTION3);//rts
		UART_WRITE32(REG_SER_MSR, MODEM_HW_RTS | MODEM_HW_CTS | MODEM_RTS_TRIGGER_VALUE);//2, 3, level
#endif
...
//    UINT32 u4ExtraUartSupport;
...
#ifdef CC_MT5890
	UART_HwClearFifo(UART_PORT_HT, CLEAR_TX_BUFFER | CLEAR_RX_BUFFER);
	UART_HwClearFifo(UART_PORT_BT, CLEAR_TX_BUFFER | CLEAR_RX_BUFFER);
#endif
#endif
...
#ifdef CC_MT5890
	UART_WRITE32(REG_SERHT_CTR, CTR_NORMAL | DIS_RX_ZERO_ON);
	UART_WRITE32(REG_SERBT_CTR, CTR_NORMAL | DIS_RX_ZERO_ON);
#endif
#endif
...
#ifdef CC_MT5890
	UART_WRITE32(REG_SERHT_BUF_CTL, 0x1F00 | (UARTHT_FIFO_SIZE - 5));
	UART_WRITE32(REG_SERBT_BUF_CTL, 0x1F00 | (UARTBT_FIFO_SIZE - 5));
#endif
#endif
...
#ifdef CC_MT5890
	UART_HwWaitTxBufClear(UART_PORT_HT);
	UART_HwWaitTxBufClear(UART_PORT_BT);
#endif
#endif
...
#else
	// do not clear UART0 int status, clear UART1 int status
	u4Data = UART_READ32(REG_SER_INT_STATUS);
	u4Data |= INT_SER0_MASK;
...
#ifdef CC_MT5890
	UART_WRITE32(REG_SERHT_INT_STATUS, 0);
	UART_WRITE32(REG_SERBT_INT_STATUS, 0);
#endif
#endif
    ...
    UART_WRITE32(REG_SER_INT_EN,
		...
		#else
		// Enable interrupt mask	
		u4Data = UART_READ32(REG_SER_INT_EN);
		u4Data |= INT_SER1_DMAW_TOUT | INT_SER1_DMAW_OVERFLOW | INT_SER1_DMAW_FULL;
		UART_WRITE32(REG_SER_INT_EN, u4Data);
		#endif
    #endif
#ifdef CC_UART_PD_SUPPORT    
    UART_WRITE32(REG_SERPD_INT_EN,
                 ...
                 INT_SERPD_RISC_DATA);
#ifdef CC_MT5890
	UART_WRITE32(REG_SERHT_INT_EN,
				INT_SERPD_RX_OVERFLOW | INT_SERPD_RX_TOUT | INT_SERPD_RX_FULL | 
				INT_SERPD_RISC_DATA);
	UART_WRITE32(REG_SERBT_INT_EN,
				 INT_SERPD_RX_OVERFLOW | INT_SERPD_RX_TOUT | INT_SERPD_RX_FULL | 
				 INT_SERPD_RISC_DATA);
#endif
#endif
    ...
    #ifndef CC_NO_KRL_UART_DRV // mark UART 0 's transparent mode
...
#ifdef CC_MT5890
	UART_WRITE32(REG_HT_NROMAL_MODE_BUF_SZ, 0);
	UART_WRITE32(REG_SERHT_CTL, SET_TRANS_MODE_ON);
	UART_WRITE32(REG_BT_NROMAL_MODE_BUF_SZ, 0);
	UART_WRITE32(REG_SERBT_CTL, SET_TRANS_MODE_ON);
#endif
#endif
}
...
UINT32 UART_GetRxDMABuf(UINT32 u4Port, UINT8 *pBuffer, UINT32 u4NumToRead)
{
    ...
    u4WPTR = UART_READ32(REG_SER1_DMAW_WPTR);//0x158
    u4RPTR = UART_READ32(REG_SER1_DMAW_RPTR);//0x15c
    u4SPTR = UART_READ32(REG_SER1_DMAW_SPTR);//0x150
    u4EPTR = UART_READ32(REG_SER1_DMAW_EPTR);//0x154

    if (u4WPTR > u4RPTR)
    {
#ifdef CC_NO_KRL_UART_DRV
		LOG(5, "[UART_GetRxDMABuf]-1Addr(0x%x)Read_P(0x%x)\n", (UINT32)pBuffer, u4RPTR);
		u4VirRPTR = VIRTUAL(u4RPTR);
		LOG(5, "[UART_GetRxDMABuf]-1 V_Addr(0x%x)\n", u4VirRPTR);
#endif
        u4Length = MIN((u4WPTR - u4RPTR), u4NumToRead);
#ifdef CC_NO_KRL_UART_DRV
		HalFlushInvalidateDCacheMultipleLine(u4VirRPTR, u4Length);
#endif

#ifndef CC_NO_KRL_UART_DRV
		x_memcpy((VOID *)pBuffer, (const VOID *)u4RPTR, u4Length);
#else
		x_memcpy((VOID *)pBuffer, (const VOID *)u4VirRPTR, u4Length);	 
#endif
    ...
    {
#ifdef CC_NO_KRL_UART_DRV
		LOG(5, "[UART_GetRxDMABuf]-2 Addr(0x%x)Read_P(0x%x)\n", (UINT32)pBuffer, u4RPTR);
		u4VirRPTR = VIRTUAL(u4RPTR);
		LOG(5, "[UART_GetRxDMABuf]-2 V_Addr(0x%x)\n", u4VirRPTR);
#endif		
        ...
        u4RxLength += u4Length;
#ifdef CC_NO_KRL_UART_DRV
		LOG(5, "[UART_GetRxDMABuf]-3 Addr(0x%x)Read_Start(0x%x)\n", (UINT32)pBuffer, u4SPTR);
		u4VirSPTR = VIRTUAL(u4SPTR);
		LOG(5, "[UART_GetRxDMABuf]-3 V_Addr(0x%x)\n", u4VirSPTR);
		HalFlushInvalidateDCacheMultipleLine(u4VirSPTR, u4Length);
#endif

#ifndef CC_NO_KRL_UART_DRV
		x_memcpy((VOID *)pBuffer, (const VOID *)u4SPTR, u4Length);
#else
		x_memcpy((VOID *)pBuffer, (const VOID *)u4VirSPTR, u4Length);
#endif
...
VOID UART_HwDmaWriteDram(UINT32 u4SPTR, UINT32 u4Size, UINT32 u4Threshold)
{
    ...
    u4RxPhyaddr = PHYSICAL(u4SPTR);
	LOG(5, "[UART_HwDmaWriteDram]P_Addr(0x%x)size(%d)\n", u4RxPhyaddr, u4Size);
#endif
...
#else
	UART_WRITE32(REG_SER1_DMAW_SPTR, u4RxPhyaddr);
	UART_WRITE32(REG_SER1_DMAW_EPTR, (u4RxPhyaddr + u4Size));
#endif
...
#else
	UART_WRITE32(REG_SER1_DMAR_SPTR, u4RxPhyaddr);
#endif
    u4Reg = 0;
    if (!fgCircular)
    {
...
#else
		UART_WRITE32(REG_SER1_DMAR_EPTR, (u4RxPhyaddr + u4Size + 1));
#endif
    }
    else
    {
...
#else
		UART_WRITE32(REG_SER1_DMAR_EPTR, (u4RxPhyaddr + u4Size));
#endif
        u4Reg |= DMAR_CIR;
    }
...
#else
		UART_WRITE32(REG_SER1_DMAR_WPTR, (u4RxPhyaddr+u4Size));
#endif
    }
}
...
#ifdef CC_MT5890
		u4Ctr |= (u4Port == UART_PORT_HT || u4Port == UART_PORT_BT) ? BUAD_RATE_PD_2400 : BUAD_RATE_2400;
#endif
#else
...
#ifdef CC_MT5890
		u4Ctr |= (u4Port == UART_PORT_HT || u4Port == UART_PORT_BT) ? BUAD_RATE_PD_9600 : BUAD_RATE_9600;
#endif
...
#ifdef CC_MT5890
		u4Ctr |= (u4Port == UART_PORT_HT || u4Port == UART_PORT_BT) ? BUAD_RATE_PD_19200 : BUAD_RATE_19200;
#endif
#else
...
#ifdef CC_MT5890
		u4Ctr |= (u4Port == UART_PORT_HT || u4Port == UART_PORT_BT) ? BUAD_RATE_PD_38400 : BUAD_RATE_38400;
#endif
#else
...
#ifdef CC_MT5890
		u4Ctr |= (u4Port == UART_PORT_HT || u4Port == UART_PORT_BT) ? BUAD_RATE_PD_57600 : BUAD_RATE_57600;
#endif
#else
...
UINT32 UART_HwGetRxDataCnt(UINT32 u4Port)
{
	...
	case UART_PORT_HT:
		...
		if (UART_HwCheckPortTransMode(u4Port))
		{
			u4RxCnt = (UART_READ32(REG_SERHT_STATUS) & SER_READ_ALLOW);
		}
		else
		{
			u4RxCnt = UART_READ32(UART_HT_BASE + REG_NROMAL_MODE_BUF_SZ) + 1;
		}
		break;
    case UART_PORT_BT:
        // In normal mode, get actual rx size from REG_NROMAL_MODE_BUF_SZ register.
        if (UART_HwCheckPortTransMode(u4Port))
        {
            u4RxCnt = (UART_READ32(REG_SERBT_STATUS) & SER_READ_ALLOW);
        }
        else
        {
            u4RxCnt = UART_READ32(UART_BT_BASE + REG_NROMAL_MODE_BUF_SZ) + 1;
        }
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		u4TxCnt = UART_READ32(REG_SERHT_STATUS);
		break;
    case UART_PORT_BT:
        u4TxCnt = UART_READ32(REG_SERBT_STATUS);
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
    }
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		u4Addr = REG_SERHT_BUF_CTL;
		break;
    case UART_PORT_BT:
        u4Addr = REG_SERBT_BUF_CTL;
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	else if (u4Port == UART_PORT_HT)
	{
		u4IntStatus  = UART_READ32(REG_SERHT_INT_STATUS);
		u4IntStatus &= UART_READ32(REG_SERHT_INT_EN);
		u4IntStatus &= INT_SERPD_MASK;
	}
	else if (u4Port == UART_PORT_BT)
	{
		u4IntStatus  = UART_READ32(REG_SERBT_INT_STATUS);
		u4IntStatus &= UART_READ32(REG_SERBT_INT_EN);
		u4IntStatus &= INT_SERPD_MASK;
	}
#endif	
    else
...
#else
		u4IntStatus &= (INT_SER1_MASK);// only set uart 1 interrupt
#endif
    }
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		UART_WRITE32(REG_SERHT_INT_STATUS, u4Data);
		break;
    case UART_PORT_BT:
        UART_WRITE32(REG_SERBT_INT_STATUS, u4Data);
        break;
#endif
#endif /* CC_UART_PD_SUPPORT */
...
#ifdef CC_MT5890
	case UART_PORT_HT:
		return ((UART_READ32(REG_SERHT_CTL) & (UINT32)TRANS_MODE_ON) > 0);
    case UART_PORT_BT:
        return ((UART_READ32(REG_SERBT_CTL) & (UINT32)TRANS_MODE_ON) > 0);
#endif
#endif /* CC_UART_PD_SUPPORT */
