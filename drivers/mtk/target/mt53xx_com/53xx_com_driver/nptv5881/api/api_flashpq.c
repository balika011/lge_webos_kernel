...
// === DEFINE =============================================================================
#define ENABLE_FLASH_PQ_DEBUG       0  //  1
...
void vApiFlashPqDataReadFlash(UINT8 bIndex, UINT64 u8Offset, UINT32 u4MemPtr, UINT32 u4MemLen)
{
	...
	Loader_ReadMsdc((UINT32)
	DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition)),u8Offset, (VOID*)u4MemPtr, u4MemLen);
	#else
...
void vApiFlashPqToDram(UINT8 bIndex)
{
    ...
    #endif
    {
        ...
        #ifndef CC_MTK_LOADER  
        if 
        ((((UINT64)DRVCUST_OptGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition))<<32) +
        ...
        #endif
        {
            #ifdef CC_MTK_LOADER
            u4BlockSize = 
            DRVCUST_InitGet((QUERY_TYPE_T)(eNANDFlashPartSize0+DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition))));
            #else
            u4BlockSize = 
            DRVCUST_OptGet((QUERY_TYPE_T)(eNANDFlashPartSize0+DRVCUST_OptGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition))));
            #endif
        }
        else
        {        
            #ifdef CC_MTK_LOADER
            u4BlockSize = 
            DRVCUST_InitGet((QUERY_TYPE_T)(eNorFlashPartSize0+DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition))));
            #else
            u4BlockSize = DRVCUST_OptGet((QUERY_TYPE_T)(eNorFlashPartSize0+DRVCUST_OptGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition))));
            #endif
        }
        ...
        u8PqBaseAddr[bIndex] = //((UINT64)DRVCUST_OptGet(aFlashPqFlashInfo[bIndex].dwFlashBasePartition)<<32) +
            DRVCUST_OptGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBaseAddress))+ 
            DRVCUST_OptGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashOffset));
		#else
		u8PqBaseAddr[bIndex] = //((UINT64)DRVCUST_InitGet(aFlashPqFlashInfo[bIndex].dwFlashBasePartition)<<32) +
            DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBaseAddress)) + 
            DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashOffset));
		#endif
...
UINT8 bApiFlashPqVerifyHeader(UINT8 bIndex)
{
    ...
    #endif
    {
        ...
        #else
		if 
		((((UINT64)DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBasePartition))<<32) +
            DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBaseAddress)) == 0) ||
            (DRVCUST_InitGet((QUERY_TYPE_T)(aFlashPqFlashInfo[bIndex].dwFlashBlockSize)) == 0))
		#endif
        {
...
UINT8 bApiFlashPqUpdatePanelTable(void)
{
    ...
    UINT16 u2PanelIndex = 1;//wApiFlashPqGetPanelId(); //fix build error,mark temporary 
    UINT8 u1Index = 0;
