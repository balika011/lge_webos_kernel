...
#if defined(CUST_LCCT_DRV) || defined(CC_FLIP_MIRROR_SUPPORT)
#include "fbm_drvif.h"
...
void vDrvSCEHWInit(void)
{
    ...
    vIO32WriteFldAlign(SCE_CFG_MAIN, 0x1, S_BY_Y_MODE);  
...
{
	vIO32WriteFldAlign(SCE_CFG_MAIN, ~bOnOff, ALLBP);
}
...
void vDrvSCEParam(void)
{
    ...
    vIO32WriteFldMulti(C_BOOST_MAIN,
              P_Fld(wReadQualityTable(QUALITY_ALCOM_CBOOST_ONOFF), NEW_CBOOST_EN) |
              P_Fld(wReadQualityTable(QUALITY_ALCOM_CBOOST_ONOFF), CBOOST_ENABLE) |
              P_Fld(wReadQualityTable(QUALITY_ALCOM_BOOST_GAIN), CBOOST_GAIN) |
...
#else
    vIO32WriteFldMulti(C_BOOST_MAIN,
              P_Fld(SV_OFF, NEW_CBOOST_EN) |
              P_Fld(wReadQualityTable(QUALITY_ALCOM_CBOOST_ONOFF), CBOOST_ENABLE) |
              P_Fld(wReadQualityTable(QUALITY_ALCOM_MINUS_BOOST_EN), MINUS_CBOOST_EN) |
              P_Fld(wReadQualityTable(QUALITY_ALCOM_BOOST_GAIN), CBOOST_GAIN) |
...
{
    #ifdef CC_FLIP_MIRROR_SUPPORT
	if((u4QueryFlipModule(SV_VP_MAIN) & 0x7)&&(u1GetFlipMirrorConfig()& 0x2))
	{rRegion.u1OnOff = !rRegion.u1OnOff;}
    #endif
    if (bOnOff == SV_ON) 
    {
        //Set region or off
        vIO32WriteFldAlign(DBG_CFG_MAIN, !rRegion.u1OnOff, SPLIT_SWAP);
        //Define demo region
