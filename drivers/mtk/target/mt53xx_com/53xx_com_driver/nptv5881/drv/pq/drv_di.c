...
#define MIB_KERNEL_SYNC_WA_CNT              (0xF)
#define MIB_SET_RES_FINISH_CNT              (0xB)
#define MIB_SET_RES_TIME_OUT                (0xF)
#define MIB_REMY_PROG_IN_SYNC_WA            
...
UINT8 rMIBForceModeChg[VDP_NS] = {0,0};

static HANDLE_T _hDIPQIsrSema;

#ifdef MIB_REMY_PROG_IN_SYNC_WA
UINT8 u1DelayCnt = 0;
#endif
...
{        
    ...
    else 
    {
        ...
        else
        {
            ...
            //PR Mode only support Y Only, No Chroma C NR
            if((E_TD_OUT_3D_LI==_arMDDiPrm[u1VdpId].eTDOutput)&&(E_MIB_NR_YC==eNRMode))                        
            {
                eNRMode=E_MIB_NR_Y;
            }                 
...
static inline void _vDrvDISetKernalCtrl(UINT8 u1VdpId, E_MIB_DMODE eDataMode)
{
    ...
    rMIBState[u1VdpId].u1OutbufCnt = MIB_KERNEL_SYNC_WA_CNT;
    ...
    if(VDP_2==u1VdpId)
    {
        MDDI_WRITE_FLD(VDP_2,MCVP_KC_1F ,_arMDDiPrm[u1VdpId].u1IsDualFPI ,MAIN_PIP_OUT_SYNC);                
    }
    #endif
...
static inline void _vDrvDICSFlagCfg(UINT8 u1VdpId)
{
    ...
    if(IS_SD_TIMING(SV_VP_MAIN)) //SD Timing
    {
    ...
    {
        MDDI_WRITE_FLD(u1VdpId,MCVP_CS_00, 1, ME_MODE);
        MDDI_WRITE_FLD(u1VdpId,MCVP_DRAM_01, 0, DA_FLAG_MV_HALF_MODE); 
    }
...
static inline void _vDrvDISetDramCtrl(UINT8 u1VdpId, E_MIB_DMODE eDataMode)
{
    ...
    //3D Mode WRITE CONFIGURATION
    if(IS_PR_MODE(u1VdpId))
    {
        u4DA_NR_YUV_MODE=MIB_YUV_MODE_Y_ONLY;

        if(IS_PROGRESSIVE(u1VdpId))
        {
            u4DA_MODE_YUV=MIB_YUV_MODE_C_ONLY;
        }        
    ...
    {
        rMIBState[u1VdpId].u1VSyncCnt=DRAM_REFILL_DELAY;  
    }    

    u4PreAF_TO_CF[u1VdpId] = u4AF_TO_CF;
    
    if((rMIBState[u1VdpId].u1VSyncCnt==0)||(SV_ON==u4AF_TO_CF))
    {
        MDDI_WRITE_FLD(u1VdpId,MCVP_KC_17,u4AF_TO_CF        ,AF_TO_CF);
    }

    u4DRAM_RW_EN &= (UINT32)rMIBState[u1VdpId].u2FreezeWriteFilter;
...
{   
            ...
            &&IS_PROGRESSIVE(u1VdpId))
        {
            switch(TdIn)
            {
                case E_TD_IN_FS_P:            
                    ...
                    break;            
                case E_TD_IN_SBS_P:
                    //SBS can't use 0x8, 0x9 0x10 will be o.k ???
    ...
    if(IS_ECO_2_IC())
    {
        MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_09,
            (((E_TD_OUT_3D_TB==TdOut)||(E_TD_IN_TB_P==TdIn))&&IS_420TO422(u1VdpId)), DA_420_TB_EN);
    }
    #endif
}
...
static void _vDrvDIAutoStopCtrl(UINT8 u1VdpId,UINT32 u4OnOff)
{
    ...
    {
        rMIBState[u1VdpId].u1SRCWAutoStopDisableCnt= DRAM_REFILL_DELAY;  
        rMIBState[u1VdpId].bAllowedAutoStop = SV_FALSE;
        vDrvDISetAutoStop(u1VdpId, SV_OFF);
    }
    else
    {
        if(rMIBState[u1VdpId].u1SRCWAutoStopDisableCnt > 0)
        {
            rMIBState[u1VdpId].u1SRCWAutoStopDisableCnt-- ;
        }
        else
        {
...
static void _vDrvDIISR(UINT16 u2Vector)
{
    ...
    if (MDDI_READ_FLD(VDP_1, MCVP_BC_02, INT_MIVS_R_STA))
    {
        ...
        rMIBState[VDP_1].u1VSyncCnt = rMIBState[VDP_1].u1VSyncCnt ? rMIBState[VDP_1].u1VSyncCnt -1 : 0;
    ...
    {
        rMIBState[VDP_2].u1VSyncCnt = rMIBState[VDP_2].u1VSyncCnt ? rMIBState[VDP_2].u1VSyncCnt -1 : 0;
#ifdef CC_SUPPORT_VENC
        vDrvDIVEncISR();
#endif
...
static void _vDrvDIInitISR(void)
{
    ...
    if (!u1Initialized)
    {
        ...
        {
            LOG(0, "Error: fail to register PSCAN ISR!\n");
        }
        else
        {
            LOG(0, "Register PSCAN ISR OK!\n");
        }

        u1Initialized = SV_TRUE;
        LOG(0, "PSCAN ISR registered!\n");
    }
}
...
static void _vDrvDISetMisc(UINT8 u1VdpId)
{
    ...
    // Reset SW counter
    rMIBState[u1VdpId].u1StatusCnt = 0;
    rMIBState[u1VdpId].u1FreezeCnt = 0;
    rMIBState[u1VdpId].u1SRCWAutoStopDisableCnt = 0;

    //Reset freeze config when mode change.
    rMIBState[u1VdpId].u2FreezeWriteFilter = 0x1FF;
    ...
    #endif
    if((SV_VP_MAIN==u1VdpId)||IS_ECO_2_IC())
    {   //Main Path need more blank for PQ Processing
        MDDI_WRITE_FLD(u1VdpId, MCVP_KC_2B, 0x80, DISPMODE_H_BLANK); 
    }
    else
    {   // Sub Path reduce blanking for FPI 120 mode
        MDDI_WRITE_FLD(u1VdpId, MCVP_KC_2B, 0x30, DISPMODE_H_BLANK); 
    }

    if (u1VdpId == SV_VP_PIP && _arMDDiPrm[u1VdpId].u1IsDualFPI)
    {
        MDDI_WRITE_FLD(u1VdpId, MCVP_KC_2B, 0x1d, I2P_H_BLANK); 
    }
    else
    {
   ...
   // MDDI_WRITE_FLD(u1VdpId, MCVP_KC_00, IS_PR_MODE(u1VdpId) && IS_PROGRESSIVE(u1VdpId) ? 0x5 : 0x8, SRCW_V_BGN);
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_00, IS_PR_MODE(u1VdpId) && IS_PROGRESSIVE(u1VdpId) ? 0x5 : ((u1VdpId == VDP_1)?0xB : 0xC), SRCW_V_BGN);

#ifdef MIB_COBRA_ECO1_PIP_GARBAGE
    if (u1VdpId == VDP_2 && IS_ECO_2_IC() && !_arMDDiPrm[u1VdpId].u1IsPOP)
    {
...
static void inline _vDrvDISetResStart(UINT8 u1VdpId)
{
    ...
    rMIBState[u1VdpId].u1SetResTimeOutCnt  = rMIBState[u1VdpId].u1DynRes
        ? MIB_SET_RES_TIME_OUT   : 0;
...
static void inline _vDrvDISetResEnd(UINT8 u1VdpId)
{
    ...
    if(rMIBState[u1VdpId].u1SetResTimeOutCnt > 0)
    {
        rMIBState[u1VdpId].u1SetResTimeOutCnt --;
    }
    else
    {
    ...
    {    
        ...
        if(u1DelayCnt==0)
        {
            MDDI_WRITE_FLD(u1VdpId,MCVP_KC_33, (IS_PROGRESSIVE(u1VdpId) && !IS_MIB_FRC(u1VdpId)),PROG_LAST_PIXEL_OPT);
        }
        else
        {
            u1DelayCnt--;
    ...
    MDDI_WRITE_FLD(u1VdpId,MCVP_KC_0B,SV_OFF    ,SIZE_UPDATE);        
...
static inline void _vDrvDICfgDeRatio(UINT8 u1VdpId)
{
    ...
    UINT32 u4OutHT =  _arMDDiPrm[u1VdpId].u2Width + MIB_DISP_HBLANK_MIN;
    UINT32 u2Height =  _arMDDiPrm[u1VdpId].u2Height;
    UINT16 u2NSTDRatio;
    ...
    if (IS_SD_TIMING(u1VdpId)&&IS_ECO_2_IC())
    {
    ...
    #if 0 // Experiment to improve PQ quality by boosting clk
    if(0!=u4OutSigClk && 0!=u4InSigClk)
    {
        ...
        {  
            ...
            {
                u4OutSigClk/=2;                
                MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, SV_ON, FAST_PRE_FETCH); 
            }
            else
            {
                MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, SV_OFF, FAST_PRE_FETCH); 
            }
            #endif
            u4DELmt =u4DrvDIGetClkRate(u1VdpId)/u4OutSigClk;
        }
        else
        {        
            MDDI_WRITE_FLD(u1VdpId, MCVP_KC_33, SV_OFF, FAST_PRE_FETCH);
...
static inline void _vDrvDISetActiveWindow(UINT8 u1VdpId, UINT16 wSrcXOffset, UINT16 wSrcYOffset, UINT16 wSrcWidth, UINT16 wSrcHeight)
{
    ...
    if(IS_IC_5881_ES2()&&IS_INTERLACE(u1VdpId)&&u1VdpId==VDP_1)
    {
        UINT16 u2HeightB  = wSrcHeight/32;
        UINT16 u2HeightA  = u2HeightB+(wSrcHeight%32+3)/4;
        UINT16 u2WidthB   = wSrcWidth/16;
        UINT16 u2WidthA   =u2WidthB+ (wSrcWidth%16+1)/2;
        if(u2WidthA*u2HeightA > 4095)
        {
            if(u2HeightB*u2WidthB > 4095)
            {
                vIO32WriteFldAlign(MCVP_CS_3F ,4095 , IF_REGION_MOTION_ECO_AREA);
            }
            else
            {
                vIO32WriteFldAlign(MCVP_CS_3F ,(u2HeightB*u2WidthB) , IF_REGION_MOTION_ECO_AREA);
            }
            vIO32WriteFldAlign(MCVP_CS_3F ,SV_ON , IF_REGION_MOTION_ECO_ON);
        }
        else
        {
            vIO32WriteFldAlign(MCVP_CS_3F ,SV_OFF, IF_REGION_MOTION_ECO_ON);
        }
    }
    //Update Self timing gen for debug pattern
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_01, (_arMDDiPrm[u1VdpId].u2VTotal), IPTG_SELF_VT);
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_01, (_arMDDiPrm[u1VdpId].u2HTotal), IPTG_SELF_HT);        
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_02, (_arMDDiPrm[u1VdpId].u2Height), IPTG_V_ACT);
...
{    
    return (IS_MPEG(u1VdpId)
            ...
            && !(!IS_ECO_2_IC()&&((E_TD_IN_TB_P==_arMDDiPrm[u1VdpId].eTDInput))))
...
{    
    ...
    if(SV_VP_MAIN==u1VdpId)
    {
        ...
        if( 
            ...
            //Before ECO , PR Mode can't support NXNR
            ||((E_TD_OUT_3D_LI==_arMDDiPrm[u1VdpId].eTDOutput)
                &&!IS_ECO_2_IC()
                &&IS_PROGRESSIVE(u1VdpId))
        ...
        else
        {
            ...
            {         
                case E_MIB_NR_YC:
                case E_MIB_NR_Y:    
                    if(IS_SD_TIMING(VDP_1)) // SD Timing
                    {
                        e3DNR = MCNR_MODE_MC;         
                    }
                    else if (rMIBState[VDP_1].u4SetResNewKDE == 0) // 1080p
                    {
                        e3DNR = MCNR_MODE_MA;
                    }
                    else // HD Timing
...
static inline UINT8 _u1DrvDIModeChange(UINT8 u1VdpId)
{
    ...
    rNewMIBCfg.u1IsDualFPI      = r3DInOutFormat[u1VdpId].u1IsDualFPI;
...
#endif
    {
        if (_u1DrvDIModeChange(u1VdpId)|| rMIBForceModeChg[u1VdpId])
        {  
            rMIBForceModeChg[u1VdpId] = SV_FALSE;
            ...
            u1DelayCnt = IO32ReadFldAlign(PSCAN_MISC_01, MDDI_DEBUG_CNT);
            #endif
    ...
    if(prPool != NULL)
    {
        ...
        if(u4DrvTDTVIsDualFPI())
        {
            if(VDP_1==u4Arg1)
            {
                ...
                {
                    rMIBForceModeChg[VDP_1] = SV_TRUE;
                ...
                {
                    rMIBForceModeChg[VDP_2] = SV_TRUE;
        ...
        {    
            ...
            {
                rMIBForceModeChg[u4Arg1] = SV_TRUE;
...
void vDrvDIInit(UINT8 u1VdpId)
{
    ...
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_00, 1, SRCW_CRC_SEL);
    #endif
