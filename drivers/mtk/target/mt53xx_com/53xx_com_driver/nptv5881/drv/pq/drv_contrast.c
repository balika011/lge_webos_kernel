...
static void vDrvADLSetLumaCurveHwExtend(const UINT16 * waLArray)
{
	UINT8 bi;
	...
	vIO32Write4B(Y_FTN_33_32_MAIN, waLArray[32]);
}
...
static void vDrvADLSetLumaCurveHw(const UINT16 * waLArray)
{
    #if 0
    UINT8 bi;
    UINT16 waLArray65[LUMA_CURVE_NUM_65];

	ASSERT(waLArray!=NULL);

	// Interpolate 33 point to 65 point
    for (bi = 0; bi < 32; bi++)
    {
        waLArray65[2 * bi] = waLArray[bi];
        waLArray65[2 * bi + 1] = (waLArray[bi] + waLArray[bi + 1])>>1;
    }
    waLArray65[64] = waLArray[32];
    #endif
    vDrvADLSetLumaCurveHwExtend(waLArray);
}
...
void vDrvADLSetSecondLumaCurveHw(const UINT16 * waSLArray)
{
    #if 0
    UINT8 bi;
    UINT16 waSLArray65[LUMA_CURVE_NUM_65];
    ...
    for (bi = 0; bi < 32; bi++)
    {
        waSLArray65[2 * bi] = waSLArray[bi];
        waSLArray65[2 * bi + 1] = (waSLArray[bi] + waSLArray[bi + 1])>>1;
    }
    waSLArray65[64] = waSLArray[32];
    #endif
    
    vDrvADLSetSecondLumaCurveHwExtend(waSLArray);
}
...
{
        UINT8 _bYGammaUI = IO32ReadFldAlign(ADAP_LUMA_09, ADL_UI_YGAMMA); 	
...
// Update Adaptive luma version 2 parameter
    ...
    vIO32WriteFldAlign(ADAP_LUMA_0C, wReadQualityTable(QUALITY_ALCOM_YGAMMA_DARK_SCENE_BRIT), Y_BRIGHT);
	...
	#ifdef CUST_LCCT_DRV
	// FleshTone ADL 
	vIO32WriteFldAlign(ADAP_LUMA_04, wReadQualityTable(QUALITY_AL_BS_COLOR_ON_1_OFF_0), SWREG_COLOR_BS_ON_1_OFF_0);     
	vIO32WriteFldAlign(ADAP_LUMA_0E, wReadQualityTable(QUALITY_AL_R_RATIO), BS_COLOR_KR);
	vIO32WriteFldAlign(ADAP_LUMA_0E, wReadQualityTable(QUALITY_AL_G_RATIO), BS_COLOR_KG);
	vIO32WriteFldAlign(ADAP_LUMA_0E, wReadQualityTable(QUALITY_AL_B_RATIO), BS_COLOR_KB);    
	vIO32WriteFldAlign(ADAP_LUMA_0F, wReadQualityTable(QUALITY_AL_R_SAT_L), BS_ASL_REMAP_R_LO);
	vIO32WriteFldAlign(ADAP_LUMA_0F, wReadQualityTable(QUALITY_AL_R_SAT_H), BS_ASL_REMAP_R_HI);
	vIO32WriteFldAlign(ADAP_LUMA_0F, wReadQualityTable(QUALITY_AL_G_SAT_L), BS_ASL_REMAP_G_LO);
	vIO32WriteFldAlign(ADAP_LUMA_0F, wReadQualityTable(QUALITY_AL_G_SAT_H), BS_ASL_REMAP_G_HI);
	vIO32WriteFldAlign(ADAP_LUMA_10, wReadQualityTable(QUALITY_AL_B_SAT_L), BS_ASL_REMAP_B_LO);
	vIO32WriteFldAlign(ADAP_LUMA_10, wReadQualityTable(QUALITY_AL_B_SAT_H), BS_ASL_REMAP_B_HI); 
	vIO32WriteFldAlign(ADAP_LUMA_10, wReadQualityTable(QUALITY_AL_WHITE_DECAY), BS_ASL_REMAP_W_DECAY);  
	#endif
...
extern void vDrvGetBlackWhite2BinSum(const UINT16 * waHistCur, UINT8 bMode); // MJC moving box patch of mt5396 changelsit:903459 ,checked to cd lee ,it also do in Cobra
