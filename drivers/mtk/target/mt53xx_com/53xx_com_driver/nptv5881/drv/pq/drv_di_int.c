...
{    
    {   //RF
        {MCVP_DARE_00, 0x87000130, 0x8F111130},  //   
    ...
    {   //HD
        {MCVP_DARE_00, 0x87000030, 0x8F111130},  //
        {MCVP_DARE_11, 0x41884000, 0xFFFFFFFF},  //Vertical Filter V-Variation Table
        {MCVP_DARE_12, 0x22084000, 0xFFFFFFFF},  //Vertical Filter H-Variation Table       
        {REGTBL_END    , 0x00000000, 0x00000000}
    }
...
const REGTBL_T CODE MDDI_PD_PARAM[3][14] =
{    
    {   //480i
...
static void _vDrvDISetPDMAQuality(UINT8 bPath)
{
    ...
    vIO32WriteFldAlign(MCVP_SB_01
        ,SV_VD_TVD3D==_arMDDiPrm[bPath].u1DecType, SB_NR_PAL_C_FILTER);                          
}
...
static void _vDrvDISetIFQuality(UINT8 bPath)
{
	...
	//for motion backgroud motion and preground still pattern, fix flick issue
	if((VDP_GetPlayMode(VDP_1) == FBM_FBG_DTV_MODE)&& (bDrvVideoGetTiming(VDP_1) == MODE_625I))
	{
	   vIO32WriteFldAlign(MCVP_FUSION_0D, 0x20,IF_RA_CLIP_FLICKER_TH);
	}
}
...
static void _vDrvDISetDARQuality(UINT8 bPath)
{
	...
	if(IS_MPEG(VDP_1)&&(VDP_GetPlayMode(VDP_1) == FBM_FBG_DTV_MODE)&& (bDrvVideoGetTiming(VDP_1) == MODE_625I))
	{
		vIO32WriteFldAlign(MCVP_DARE_13, 0xA, DAR_V_MOTION_O0);
		vIO32WriteFldAlign(MCVP_DARE_13, 0xD, DAR_V_MOTION_O1);
	}
...
void vSetPDInfoDelayAtProgDispMode(void)
{
    ...
    u1SCRPointer = MDDI_READ_FLD(VDP_1, MCVP_DRAM_13 ,STA_DEBUG) & 0x7;
    
    MDDI_WRITE_FLD(VDP_1,MCVP_DRAM_12, 2   ,DA_DEBUG);
    u1NRWPointerY = (MDDI_READ_FLD(VDP_1, MCVP_DRAM_13 ,STA_DEBUG)>>6) & 0x7;
...
static void _vDrvDIPDFwPatch(void)
{
	...
	if(IS_MPEG(VDP_1)&&(VDP_GetPlayMode(VDP_1) == FBM_FBG_DTV_MODE)&& (bDrvVideoGetTiming(VDP_1) == MODE_625I))
	{
		UINT32 u4BlockMotion=(IO32ReadFldAlign(MCVP_STATUS_17, BLOCK_MOTION_SUM_PRE_STATUS))&0xFFFF;
		UINT32 u4SawNorm=IO32ReadFldAlign(MCVP_FUSION_0B, IF_SIMPLE_SAW_NORMAL);
		UINT32 u4HEdgeCnt= DiSta.IfSta.u4HEdgeCnt;
		LOG(1,">>blockmotion=%x,sawnorm=%x,hedgecnt=%x \n",u4BlockMotion,u4SawNorm,u4HEdgeCnt);
		
		if((u4BlockMotion>=0x20 &&u4BlockMotion<=0x4F)&&(u4SawNorm>=0x10 && u4SawNorm<=0x40)
			&&(u4HEdgeCnt>=0xFF && u4HEdgeCnt<=0x2FF))
		{
			vIO32WriteFldAlign(MCVP_FILM_27, 0, R_22ENABLE);
		}
		else
		{
			vIO32WriteFldAlign(MCVP_FILM_27, 0x1, R_22ENABLE);
		}
	}
...
{        
    ...
    if (IO32ReadFldAlign(PSCAN_FWFILM_06, FILED_WEAVE_EN)
        && ((DiPar.PdPar.u2FilmMode > E_DI_FILM_22) || (DiPar.PdPar.bFilm24To50)) 
        && (DiPar.PdPar.bFwBadEdit == 0)
...
/**
 * @brief DI film adaptive adjustment semaphore unlock in ISR.
 * @param void
 */
...
/**
 * @brief DI film adaptive adjustment loop in single thread.
 * @param void
 */
...
UINT32 u4DrvDIGetMotionLevel(void)
{
    ...
    {
        u4Motion = IO32ReadFldAlign(MJCFD_0B, MJCFD_FD_DET_MOV_INFO);
    }
