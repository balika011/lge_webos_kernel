...
{
	vIO32WriteFldAlign(GAMMA_00, bOnOff, C_GM_EN);
}
...
void vDrvGammaWriteTable(void)
{
    ...
    {
		vIO32WriteFldAlign(GAMMA_03, u2GammaDecodedTable[2][u2GammaIdx] >> 2, R_WDATA_B); 		// B[11:8]
		vIO32WriteFldMulti(GAMMA_02,
						P_Fld(u2GammaDecodedTable[0][u2GammaIdx] >> 2, R_WDATA_R) |
						P_Fld(u2GammaDecodedTable[1][u2GammaIdx] >> 2, R_WDATA_G)); 		
    }											

    // write last point
    vIO32WriteFldAlign(GAMMA_09, MIN(0x400, u2GammaDecodedTable[0][u2GammaIdx] >> 2), C_GM_256_R);
    vIO32WriteFldAlign(GAMMA_10, MIN(0x400, u2GammaDecodedTable[1][u2GammaIdx] >> 2), C_GM_256_G);
    vIO32WriteFldAlign(GAMMA_10, MIN(0x400, u2GammaDecodedTable[2][u2GammaIdx] >> 2), C_GM_256_B);
...
UINT8 bDrvGammaVerifyTable(void)
{
    ...
    for (u2GammaIdx = 0; u2GammaIdx < 256; u2GammaIdx++)
    {        
        if(u2GammaReadBackTable[0][u2GammaIdx] != (u2GammaDecodedTable[0][u2GammaIdx] >> 2))
        {
            Printf("[FAIL] GAMMA [R] [%d] : [W] = %d, [R] = %d\n", u2GammaIdx, (u2GammaDecodedTable[0][u2GammaIdx] >> 2), u2GammaReadBackTable[0][u2GammaIdx]);
            bRet = SV_FALSE;
        }
        if(u2GammaReadBackTable[1][u2GammaIdx] != (u2GammaDecodedTable[1][u2GammaIdx] >> 2))
        {
            Printf("[FAIL] GAMMA [G] [%d] : [W] = %d, [R] = %d\n", u2GammaIdx, (u2GammaDecodedTable[1][u2GammaIdx] >> 2), u2GammaReadBackTable[1][u2GammaIdx]);
            bRet = SV_FALSE;
        }
        if(u2GammaReadBackTable[2][u2GammaIdx] != (u2GammaDecodedTable[2][u2GammaIdx] >> 2))
        {
            Printf("[FAIL] GAMMA [B] [%d] : [W] = %d, [R] = %d\n", u2GammaIdx, (u2GammaDecodedTable[2][u2GammaIdx] >> 2), u2GammaReadBackTable[2][u2GammaIdx]);
            bRet = SV_FALSE;
        }        
    }
...
void vDrvGammaMute(UINT16 wGammaMuteR, UINT16 wGammaMuteG, UINT16 wGammaMuteB)
{
	...
	vIO32WriteFldAlign(GAMMA_03, wGammaMuteB >> 2, R_WDATA_B); 		// B[11:8]
	vIO32WriteFldMulti(GAMMA_02,
						P_Fld(wGammaMuteR >> 2, R_WDATA_R) |
						P_Fld(wGammaMuteG >> 2, R_WDATA_G)); 
	bMuteOnOff = SV_ON;
...
{
	vIO32WriteFldAlign(AMBL_00, bOnOff, DEMO_EN);
}

void vDrvAmbilightSetWindow(UINT8 bWindowNum,
                            const structAmbilightWindow * pAmbiWindow)
{
	vIO32WriteFldAlign(AMBL_03 + bWindowNum * AMBILIGHT_OFFSET1,
	                  pAmbiWindow->wX_Start, W0_X_START);
	vIO32WriteFldAlign(AMBL_03 + bWindowNum * AMBILIGHT_OFFSET1,
	                  pAmbiWindow->wX_End, W0_X_END);
	vIO32WriteFldAlign(AMBL_04 + bWindowNum * AMBILIGHT_OFFSET1,
	                  pAmbiWindow->wY_Start, W0_Y_START);
	vIO32WriteFldAlign(AMBL_04 + bWindowNum * AMBILIGHT_OFFSET1,
	                  pAmbiWindow->wY_End, W0_Y_END);
	vIO32WriteFldAlign(AMBL_05 + bWindowNum * AMBILIGHT_OFFSET1,
...
UINT16 wDrvAmbilightGetRGB(UINT8 bWindowNum, UINT8 bRGBSel)
{
	...
	{
    	case AmbilightRSel:
    		wRet = IO32ReadFldAlign(AMBL_27 + bWindowNum * AMBILIGHT_OFFSET2, W0_R);
    		break;
    	case AmbilightGSel:
    		wRet = IO32ReadFldAlign(AMBL_27 + bWindowNum * AMBILIGHT_OFFSET2, W0_G);
    		break;
    	case AmbilightBSel:
    		wRet = IO32ReadFldAlign(AMBL_28 + bWindowNum * AMBILIGHT_OFFSET2, W0_B);
    		break;
    	default:
    		break;
	}
