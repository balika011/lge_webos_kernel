...
{
    vIO32WriteFldAlign(LOCAL_DIM_0D, u2LcDSGainCurve[0], C_DS_MULT_0);
    vIO32WriteFldAlign(LOCAL_DIM_0D, u2LcDSGainCurve[1], C_DS_MULT_1);
    vIO32WriteFldAlign(LOCAL_DIM_0D, u2LcDSGainCurve[2], C_DS_MULT_2);
    vIO32WriteFldAlign(LOCAL_DIM_0E, u2LcDSGainCurve[3], C_DS_MULT_3);
    vIO32WriteFldAlign(LOCAL_DIM_0E, u2LcDSGainCurve[4], C_DS_MULT_4);
    vIO32WriteFldAlign(LOCAL_DIM_0E, u2LcDSGainCurve[5], C_DS_MULT_5);
    vIO32WriteFldAlign(LOCAL_DIM_0F, u2LcDSGainCurve[6], C_DS_MULT_6);
    vIO32WriteFldAlign(LOCAL_DIM_0F, u2LcDSGainCurve[7], C_DS_MULT_7);
    vIO32WriteFldAlign(LOCAL_DIM_0F, u2LcDSGainCurve[8], C_DS_MULT_8);
}
...
void vDrvLcdimSetDemoEffect(UINT8 *bDimmingResult)
{
    ...
    if (u4DemoFrameCount <= 120)
    {
        // Set SPI dimming command
        for(i = 0; i < _u4BlockNumH  * _u4BlockNumV; i ++)
        {
    ...
    {
        if((++u4DemoFrameCount)%60 == 0)
        {
            if (u4CurrentDemoBlock == (_u4BlockNumH  * _u4BlockNumV))
            {
                u4DemoRound ++;
        ...
        for(i = 0; i < _u4BlockNumH * _u4BlockNumV; i ++)
        {
            bDimmingResult[(_u4BlockNumH * _u4BlockNumV - 1) - i] = (i == u4CurrentDemoBlock) ? 0xFF : 0x0;
        }
    }
}
...
void vDrvGetLcDimDimmingResult(UINT8 *bDimmingResult)
{
    ...
    {
        if(IO32ReadFldAlign(LOCAL_DIM_0A, DIM_RDY)==SV_TRUE)
        {
            vIO32WriteFldAlign(LOCAL_DIM_0A, 1, C_FRAME_DLY_CPU);         
            vIO32WriteFldAlign(LOCAL_DIM_0A, 0, C_FRAME_DLY_CPU_RADDR);         

            for(i = 0; i < u4InfoSize; i++)
            {
                bDimmingResult[i] = IO32ReadFldAlign(LOCAL_DIM_0A, DIM_DATA);
            }
            
            vIO32WriteFldAlign(LOCAL_DIM_0A, 0, C_FRAME_DLY_CPU); 
        	vIO32WriteFldAlign(LOCAL_DIM_0A, 0, C_FRAME_DLY_CPU_RADDR); 
            vIO32WriteFldAlign(LOCAL_DIM_0A, 0, DIM_RDY);            
            break;
        }
        else
        {
...
void vDrvUpdateLcDimContentInfo(UINT8 bContentSel)
{
    ...
    switch(bContentSel)
    {
        case 0:     //retrieve dimming
            ...
            for(i = 0; i < LCDIM_LOOP_WAIT; i++)
            {
                if(IO32ReadFldAlign(LOCAL_DIM_0A, DIM_RDY)==SV_TRUE)
                {
                    ...
                    for(i = 0; i < u4InfoSize; i++)
                    {
                        *(bLcDimInfo + (E_LDM_INFO_DIM*u4InfoSize) + i) = IO32ReadFldAlign(LOCAL_DIM_0A, DIM_DATA);
                    }            
                    vIO32WriteFldAlign(LOCAL_DIM_0A, 0, C_FRAME_DLY_CPU);         
                    vIO32WriteFldAlign(LOCAL_DIM_0A, 0, DIM_RDY);                    
                    break;
        ...
        case 1:     //retrieve content max / average / blending
           	vIO32WriteFldMulti(LOCAL_DIM_0B, P_Fld(1, C_BLK_INFO_CPU) |
	                                        P_Fld(0, C_BLK_INFO_CPU_RADDR));
            
            for(i = 0; i < u4InfoSize; i++)
            {
                u4LcDimDimmingInfo = u4IO32Read4B(LOCAL_DIM_0C);
            ...
            vIO32WriteFldAlign(LOCAL_DIM_0B, 0, C_BLK_INFO_CPU);               
            vIO32WriteFldAlign(LOCAL_DIM_0A, 0, DIM_RDY);       
            break;            
        default:
            break;
...
{
    vIO32WriteFldAlign(LOCAL_DIM_15, 1, R_MAPPING_FILL_TABLE_READY);         // set ping-pong tick
    vIO32WriteFldAlign(LOCAL_DIM_15, 0, R_MAPPING_FILL_TABLE_READY);         // set ping-pong tick

//    while(IO32ReadFldAlign(LOCAL_DIM_15, MAPPING_TABLE_WAIT_VSYNC));
}
...
void vDrvSetLcDimMappingCurve(UINT8 *bMappingCurve)
{
    ...
    vIO32WriteFldAlign(LOCAL_DIM_15, 1, R_MAPPING_WR_AUTO_INC_ADDR);         // set write auto-incremental
    vIO32WriteFldAlign(LOCAL_DIM_09, 0, R_MAPPING_ADDR);                     // set initial address to 0

    for (u4Index = 0; u4Index < 256; u4Index++)
    {
        vIO32Write4B(LOCAL_DIM_16, bMappingCurve[u4Index]);                  // R_MAPPING_WDATA
    }                                           

    vIO32WriteFldAlign(LOCAL_DIM_15, 0, R_MAPPING_WR_AUTO_INC_ADDR);         // set write auto-incremental   
    vIO32WriteFldAlign(LOCAL_DIM_09, 0, R_MAPPING_ADDR);                     // set initial address to 0
}
...
BOOL vDrvVerifyLcDimMappingCurve(UINT8 *bLcDimMappingCurve)
{
    ...
    vIO32WriteFldAlign(LOCAL_DIM_15, 1, R_MAPPING_RD_AUTO_INC_ADDR);       // set write auto-incremental
    vIO32WriteFldAlign(LOCAL_DIM_09, 0, R_MAPPING_ADDR);                   // set initial address to 0

    for (index = 0; index < 256; index++)
    {
        bLcDimMappingCurveReadBack[index] = (u4IO32Read4B(LOCAL_DIM_16) >> 16) & 0xFF;
    }
    ...
    vIO32WriteFldAlign(LOCAL_DIM_15, 0, R_MAPPING_RD_AUTO_INC_ADDR);         // set write auto-incremental   
    vIO32WriteFldAlign(LOCAL_DIM_09, 0, R_MAPPING_ADDR);                     // set initial address to 0
...
void vDrvLcDimCalDivider(UINT32 u4BlockPxlHMax, UINT32 u4BlockPxlVMax)
{
	...
	{
        vIO32WriteFldAlign(LOCAL_DIM_06, u4ShiftA, C_SHIFT_A);
        vIO32WriteFldAlign(LOCAL_DIM_06, u4MultiplyB, C_MULT_B);
        vIO32WriteFldAlign(LOCAL_DIM_06, u4ShiftC, C_SHIFT_C);
...
void vDrvLcDimSetMaxThd(UINT32 u4LcDimMaxThd)
{
    ...
    vIO32WriteFldAlign(LOCAL_DIM_07, u4MaxThd, C_MAX_THD);
    Printf("-> u4LcDimMaxThd = %d (Total Pixel of Block = %d)\n\n", u4MaxThd, u4BlockPxlSum);
}

void vDrvLcDimSetAveWeight(UINT8 bAveWeight)
{
    vIO32WriteFldAlign(LOCAL_DIM_07, bAveWeight, C_AVE_W);
}

void vDrvLcDimSetTPFParam(UINT8 bTPFCurWeight)
{
	vIO32WriteFldAlign(LOCAL_DIM_08, bTPFCurWeight, C_CUR_W_MIN);
}
...
BOOL bDrvLcDimSetPanelInfo(UINT32 u4BlockNumH, UINT32 u4BlockNumV)
{
    ...
    vIO32WriteFldAlign(LOCAL_DIM_03, u4BlockNumH-1, C_BLK_H);
    vIO32WriteFldAlign(LOCAL_DIM_03, u4BlockNumV-1, C_BLK_V); 
 
    vIO32WriteFldAlign(LOCAL_DIM_18, u4BlockPxlH0-1, C_PXL_H_0);
    vIO32WriteFldAlign(LOCAL_DIM_18, u4BlockPxlH1-1, C_PXL_H_1);
    vIO32WriteFldAlign(LOCAL_DIM_19, u4BlockPxlH2-1, C_PXL_H_2);
    vIO32WriteFldAlign(LOCAL_DIM_19, u4BlockPxlH3-1, C_PXL_H_3);  
    vIO32WriteFldAlign(LOCAL_DIM_02, u4BlockPxlHLast-1, C_PXL_H_LAST);

    vIO32WriteFldAlign(LOCAL_DIM_01, u4BlockPxlV-1, C_PXL_V);
    vIO32WriteFldAlign(LOCAL_DIM_02, u4BlockPxlVLast-1, C_PXL_V_LAST);
...
void vDrvLcdimSetDSDemo(UINT8 u1DemoType, BOOL bOnOff)
{
    if (u1DemoType == 0)     // Store Demo
    {
        ...
        {
            // Set ink mode
            vRegWriteFldAlign(LOCAL_DIM_00, SV_ON, C_INK_EN);
            //mark in mt5398
            //vRegWriteFldAlign(LOCAL_DIM_00, 3, C_INK_MODE);
            vRegWriteFldAlign(LOCAL_DIM_01, SV_ON, C_DS_DEMO_EN);
            vRegWriteFldAlign(LOCAL_DIM_01, SV_ON, C_DEMO_SWAP);

            // Set full white ink
            //mark in mt5398
            //vRegWriteFldAlign(LOCAL_DIM_07, 0xFF, C_LD_LEVEL);
            vRegWriteFldAlign(LOCAL_DIM_13, 0, C_DEMO_PXL_X_START);
            vRegWriteFldAlign(LOCAL_DIM_13, PANEL_GetPanelWidth(), C_DEMO_PXL_X_END);
            vRegWriteFldAlign(LOCAL_DIM_14, 0, C_DEMO_PXL_Y_START);
            vRegWriteFldAlign(LOCAL_DIM_14, PANEL_GetPanelHeight(), C_DEMO_PXL_Y_END);
        }
        else
        {
            vRegWriteFldAlign(LOCAL_DIM_00, SV_OFF, C_INK_EN);
            //mark in mt5398
            //vRegWriteFldAlign(LOCAL_DIM_00, 0, C_INK_MODE);
            vRegWriteFldAlign(LOCAL_DIM_01, SV_OFF, C_DS_DEMO_EN);
        }
    }
    else                    // Factory Line Demo
    {
        if (bOnOff == SV_TRUE)
        {
            ...
            vIO32WriteFldAlign(MATRIX_02, 0, COLOR_FILTER_R_GAIN);
            vIO32WriteFldAlign(MATRIX_02, 0, COLOR_FILTER_G_GAIN);
            vIO32WriteFldAlign(MATRIX_02, 0, COLOR_FILTER_B_GAIN);
            vIO32WriteFldAlign(PEUI_00, 1, PEUI_OS_CSC_MODE_CHANGE_M);
...
{
    vRegWriteFldAlign(LOCAL_DIM_00, bOnOff, C_DS_EN);    
}
