...
static void vDrvSatHistWinCalculate(UINT16 *u2Window)
{
    ...
    else if(u43DMode == E_TDTV_UI_3D_MODE_SBS)
    {
        if(bHistWindowLRflg == 1) //left
        {
            u2WindowRecord[0] = 16;
            u2WindowRecord[1] = 16;
            u2WindowRecord[2] = out.u2Width / 2 - 16;
            u2WindowRecord[3] = out.u2Height - 16;
            bHistWindowLRflg = 0;
        }
        else    //right
        {
            u2WindowRecord[0] = out.u2Width / 2 + 16;
            u2WindowRecord[1] = 16;
            u2WindowRecord[2] = out.u2Width - 16;
            u2WindowRecord[3] = out.u2Height - 16;
            bHistWindowLRflg = 1;
    ...
    else if(u43DMode == E_TDTV_UI_3D_MODE_TB)
    {
        if(bHistWindowTBflg == 1) //top
        {
            ...
            u2WindowRecord[1] = 16;
            u2WindowRecord[2] = out.u2Width - 16;
            u2WindowRecord[3] = out.u2Height / 2 - 16;
            bHistWindowTBflg = 0;
        }
        else    //bottom
        {
            u2WindowRecord[0] = 16;
            u2WindowRecord[1] = out.u2Height / 2 + 16;
            u2WindowRecord[2] = out.u2Width - 16;
            u2WindowRecord[3] = out.u2Height - 16;
            bHistWindowTBflg = 1;
        }
...
void vDrvGetLumaInfoHW(void)
{
    ...
    if((u4PixCnt != u4PixCntPre) && (u4PixCnt != 0))
    {
...
static void vDrvHueHistInit(void)
{
    ...
    UINT8 u1HueHistBoundary[HUE_HIST_NUM] = {4, 0xDC, 0xE4, 0xE9, 0xED, 0xF2, 0xF6, 0xFB};
