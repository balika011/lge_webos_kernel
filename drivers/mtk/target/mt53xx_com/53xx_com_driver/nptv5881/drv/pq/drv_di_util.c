...
void vDrvNRSetMCVPDitherOnOff(UINT8 u1OnOff)
{
	NR_W(MCVP_KC_3E, (u1OnOff ? 1 : 0), DI_MCNR_DITHER_EN);	
}
...
{
    //to use Rdither,disable error defusion,change by yuehao.qiu 2012-11-20  
    vDrvDISetWritePrecision(u1VdpId, u18bit);
    //vDrvDISetErrDefusion(u1VdpId, u18bit);
    vDrvNRSetMCVPDitherOnOff(u18bit);
    if (u1VdpId == VDP_1)
