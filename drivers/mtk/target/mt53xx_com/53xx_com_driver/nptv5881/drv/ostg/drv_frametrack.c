 ...
 *************************************************************************/
#ifdef CC_MT5398
UINT32 _u4FrameTrackMode = FRAME_TRACK_MODE_DDDS_BY_MJC;
#else
...
UINT32 u4GetFrameTrackOnOff(void)
{
    ...
    {
        return IO32ReadFldAlign(SCPIP_PIP_OA_0E, PIP_OA_0E_FRAME_TRACK_EN1);
    }
...
E_FRAME_TRACK_STAUS  eScpipGetFrameTrackStatus(void) 
{
    ...
    if (u4Mode == 1)  // HW Frame Track
    {
        u4Status = IO32ReadFldAlign(SCPIP_PIP_OA_RO_02, PIP_OA_RO_02_FRAME_TRACK_ST);
        switch(u4Status)
        {
            case 0:
...
E_FRAME_TRACK_STAUS  eScpipGetFrameTrackStatus(void) 
{
    ...
    if (u4Mode == 1)  // HW Frame Track
    {
        if(u4ScpipGetFrametrackLockStatus() == 1)
        {
             return SCPIP_FRAME_TRACK_LOCK;
        }
         else
...
{
	_u4FrameTrackMode = (u1FttMJC? FRAME_TRACK_MODE_DDDS_BY_MJC : FRAME_TRACK_MODE_DDDS_BY_SCALER);
}
#endif
...
void vDrvFrameTrackProc(void)
{
    ...
    if (u4FscOnOff != 0xFF)
    {
        ...
        if (bDrvCheckMTKGoodStatus() == SV_TRUE || u1InFR != u1OutFR)
        {
			...
			{
			    vDrvSetDDDSFrameTrackMode(DDDS_FT_FROM_MJC);
			...
			else
			{
				...
				vFrameTrackSetFlg(FRAME_TRACK_MODE_DDDS_BY_SCALER);
...
{
    UINT32 u4InOutGCD = GCD(bDrvVideoGetRefreshRate(SV_VP_MAIN), vDrvGetLCDFreq());
    ...
    #ifdef CC_MT5398
    if (bDrvVideoGetRefreshRate(SV_VP_MAIN) == 0)
    {
        vDrvMJCSetFrmMask(0, 0);
    }
    else
    {
        vDrvMJCSetFrmMask(bDrvVideoGetRefreshRate(SV_VP_MAIN)/u4InOutGCD, 0);
    }
    #else
    UNUSED(u4InOutGCD);
    #endif        
