...
{
	PSC_WRITE_FLD(PSCSYS_0A, u4Start, PSC_VMSK_STR_CROP_1);
	PSC_WRITE_FLD(PSCSYS_0A, u4End, PSC_VMSK_END_CROP_1);
}
//Hmsk crop for overscan
void vPscSetHmskCrop(UINT32 u4Start,UINT32 u4End)
{
	PSC_WRITE_FLD(PSCSYS_0A, u4Start, PSC_HMSK_STR_CROP_1);
	PSC_WRITE_FLD(PSCSYS_0A, u4End, PSC_HMSK_END_CROP_1);
}
void vPscSetHmskStartOfst(UINT32 u4HStart)
{
	PSC_WRITE_FLD(PSCSYS_06, u4HStart, PSC_HMSK_STR_OFST_1);
}
void vPscSetVmskStartOfst(UINT32 u4VStart)
{
	PSC_WRITE_FLD(PSCSYS_07, u4VStart, PSC_VMSK_STR_OFST_1);
}
...
{
	PSC_WRITE_FLD(PSCCTRL_1F, u4Htotal, PSC_MJC_DISP_INF_HTOTAL);
}
UINT32 vPscGetMjcDispInterfaceHtotal(void)
{
	return PSC_READ_FLD(PSCCTRL_1F, PSC_MJC_DISP_INF_HTOTAL);
}
