...
#define  vDrvScposReset( x )     ( x==ON ? (vIO32WriteFldAlign(CKGEN_BLOCK_RST_CFG1,  0x7 , FLD_SCALER_RST)):(vIO32WriteFldAlign(CKGEN_BLOCK_RST_CFG1, 0x00 , FLD_SCALER_RST)) )
#define  vDrvScposSetClock(x)    ( x==ON ? (vIO32WriteFldAlign(CKGEN_BLOCK_CKEN_CFG1, 0x7 , FLD_SCALER_CKEN)):(vIO32WriteFldAlign(CKGEN_BLOCK_CKEN_CFG1, 0x00 , FLD_SCALER_CKEN)) )
#endif
...
BOOL u4UpdateTgDly[VDP_NS] = {SV_OFF,SV_OFF};
...
UINT32 _u4FgDDDSTrackHalfStep = 0;
...
void vScpipSwInit(void)
{
    ...
    #endif
    getScalerInfo(SV_VP_MAIN)->bUpdateOutXYLater = SV_FALSE; 
    getScalerInfo(SV_VP_PIP)->bUpdateOutXYLater = SV_FALSE; 	
}
...
UINT8 u1ScpipGetClipInfo(UINT8 bPath,PSCAN_CLIP_INFO_T* clipInfo)
{
    ...
    if(in.u2Width == 0 || in.u2Height == 0 ||
        out.u2Width == 0 || out.u2Height == 0)
    {
        return SV_FAIL;
    }
    ...
    clipInfo->wX = in.u2X;
    clipInfo->wY = in.u2Y;
    clipInfo->wWidth = in.u2Width;
    clipInfo->wHeight = in.u2Height;
...
static void _vScpipUpdateDramFrnBckModePrm(UINT8 bPath, UINT32 inW, UINT32 inH, UINT32 outW, UINT32 outH)
{
    ...
    else if(IS_REMY_E() && scalerInfo->bOutrChanged == SV_TRUE && scalerInfo->u4DispMode==VDP_SCPOS_DISPMODE_OFF)
    {
        //Remy Before ECO Outwindow change should mute
        _vDrvVideoSetMute(MUTE_MODULE_SCPOS_DRAM, bPath, 15, TRUE);
        LOG(1,"bPath(%d) Remy before ECO OutWindow changed => mute\n",bPath);    
    }
...
static void _vScpipUpdateDramFrameDelay(UINT8 bPath)
{
    ...
    if(bDrvVideoSignalStatus(bPath)!=(UINT8)SV_VDO_STABLE || bScalerMode != VDP_SCPOS_DISPMODE_OFF)
    {
        scalerInfo->u4DramFrmDly = 0;
    }
    else if(bScalerMode == VDP_SCPOS_DISPMODE_OFF)
    {
...
static void _vScpipUpdateOutputResolution(UINT8 bPath, UINT32 outX, UINT32 outY, UINT32 outW, UINT32 outH)
{
    ...
    if(IS_COBRA_M() || IS_VIPER_M() || IS_PYTHON_M() || IS_REMY_M() || IS_REMY_E() )
    {
    ...
    //update monitor out, for output window change
    #ifdef MON_OUT_CLIP_BLACK_BAR
    if((scalerInfo->u4OutputX != outX || scalerInfo->u4OutputY != outY || scalerInfo->u4OutputWidth != outW || scalerInfo->u4OutputHeight != outH)
        &&(bPath == SV_VP_MAIN))
    {
        vMonOutSetUpdatePrmFlg(SV_TRUE);
    }
    #endif
    csState = x_crit_start();
...
static void _vScpipUpdateTgDly(UINT32 u4VdpId) 
{
    UINT32 u4InputV;
    UINT8 bPath;
    static UINT32 u4cnt[2]={0,0};
    static UINT32 u4InputVCnt[2]={0,0};
    ...
    bPath = getScalerPath(u4VdpId);
   if( u4UpdateTgDly[bPath]==SV_ON)
   {
        if(bPath==SV_VP_MAIN)
        {
            if(u4cnt[bPath]==0)
            {
                //clear main input status
                SCPIP_WRITE_FLD(SCPIP_SCSYS_01, 1, SCSYS_01_SC_CLR_INPUT_MEASURE_1);
                SCPIP_WRITE_FLD(SCPIP_SCSYS_01, 0, SCSYS_01_SC_CLR_INPUT_MEASURE_1);
            }
            u4InputV=(UINT32)SCPIP_READ_FLD(SCPIP_SCSYS_2E, SCSYS_2E_SC_STA_VTOTAL_1);
        ...
        else
        {
            if(u4cnt[bPath]==0)
            {
                //clear sub input status
                 SCPIP_WRITE_FLD(SCPIP_SCSYS_01, 1, SCSYS_01_SC_CLR_INPUT_MEASURE_2);
                 SCPIP_WRITE_FLD(SCPIP_SCSYS_01, 0, SCSYS_01_SC_CLR_INPUT_MEASURE_2);
            }
            u4InputV=(UINT32)SCPIP_READ_FLD(SCPIP_SCSYS_30, SCSYS_30_SC_STA_VTOTAL_2);
       ...
       //set Tg Dly after 10 input vsync
       if((u4InputV != 0 && u4InputVCnt[bPath]>10) || u4cnt[bPath]>100)
        {
            vScpipSetTimeGenDelay(bPath,SV_ON);
            u4UpdateTgDly[bPath]=SV_OFF;
            u4cnt[bPath]=0;
            u4InputVCnt[bPath]=0;
        }
   }
...
void vScpipSetForcedNonLinearFactor( UINT32 iInputTotal, UINT32 iInputMiddle, UINT32 iOutputTotal, UINT32 iOutputMiddle )
{
    ...
    UINT32 MiddlePhase = u8Div6464(((INT64) (iInputMiddle-1)<<DFT_FRAC_PHASE_BITS(SV_VP_MAIN)), (iOutputMiddle-1), NULL);
    UINT32 SidePhase   = u8Div6464(((INT64) (InputSide-1)   <<DFT_FRAC_PHASE_BITS(SV_VP_MAIN)), (OutputSide-1), NULL);
    ...
    if( _fscInfo[u4VdpId].u4NonLinearPhaseSign == 0 )
    {
        ...
        _fscInfo[u4VdpId].u4NonLinearPhaseFrac = u8Div6464(((INT64)(2*(InputSide*DFT_UNIT_PHASE(SV_VP_MAIN) - MiddlePhase*OutputSide ))<<12), ( OutputSide*OutputSide + OutputSide ) & ( 4096 -1 ), NULL);
    ...
    else
    {
        ...
        _fscInfo[u4VdpId].u4NonLinearPhaseFrac = u8Div6464(((INT64)(2*(MiddlePhase*OutputSide - InputSide*DFT_UNIT_PHASE(SV_VP_MAIN) ))<<12), ( OutputSide*OutputSide + OutputSide ) & ( 4096 -1 ), NULL);
        NonlinearPhase = MiddlePhase - OutputSide*_fscInfo[u4VdpId].u4NonLinearPhaseIntg - (( OutputSide*_fscInfo[u4VdpId].u4NonLinearPhaseFrac )>>12);
    }
...
void vUpdateRptGenCount(UINT32 u4VdpId, UINT32 u4DramHeight)
{
    ...
    _fscInfo[u4VdpId].u4RptGenCount = u4DramHeight  >> 1;
	return;
}
...
static void _vScpipUpdateRegenReg(UINT32 u4VdpId)
{
    ...
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_00+regenOffset, 1, RG_M_REGEN_ENABLE);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_00+regenOffset, 1, RG_M_CLIP_EN);    
    if(_fscInfo[u4VdpId].eRegenIn3DType == SCALER_TDTV_IN_TYPE_SBS)
    {
        b3DTypeRegValue = 1<<3;
    }
    ...
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_00+regenOffset, b3DTypeRegValue, RG_M_IN_FORMAT);        
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_03+regenOffset, _fscInfo[u4VdpId].u4RegenOutX, RG_M_H_START_L);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_03+regenOffset, _fscInfo[u4VdpId].u4RegenOutY, RG_M_V_START_L);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_04+regenOffset, _fscInfo[u4VdpId].u4RegenOutX, RG_M_H_START_R);    
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_04+regenOffset, _fscInfo[u4VdpId].u4RegenOutY, RG_M_V_START_R);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_02+regenOffset, _fscInfo[u4VdpId].u4RegenOutW, RG_M_OUT_ACTIVE_WIDTH);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_02+regenOffset, _fscInfo[u4VdpId].u4RegenOutH, RG_M_OUT_ACTIVE_HEIGHT);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_01+regenOffset, _fscInfo[u4VdpId].u4RegenInW, RG_M_IN_ACTIVE_WIDTH);
    SCPIP_WRITE_FLD(PRE_RE_GEN_MAIN_01+regenOffset, _fscInfo[u4VdpId].u4RegenInH, RG_M_IN_ACTIVE_HEIGHT);
}
...
{   
    ...
    //pre ds
    SCPIP_WRITE_FLD(PRE_DS_MAIN_00+pdsOffset, _fscInfo[u4VdpId].u4HPreDsFactor, PRE_DS_MAIN_00_PRE_DSCALE_H_1);	
    SCPIP_WRITE_FLD(PRE_DS_MAIN_00+pdsOffset, _fscInfo[u4VdpId].u4VPreDsFactor, PRE_DS_MAIN_00_PRE_DSCALER_V_1);
    SCPIP_WRITE_FLD(PRE_DS_MAIN_01+pdsOffset, DFT_PDS_FACTOR - _fscInfo[u4VdpId].u4HPreDsFactor, PRE_DS_MAIN_01_PRE_DSCALE_H_INIT_1);	
    SCPIP_WRITE_FLD(PRE_DS_MAIN_01+pdsOffset, DFT_PDS_FACTOR - _fscInfo[u4VdpId].u4VPreDsFactor, PRE_DS_MAIN_01_PRE_DSCALE_V_INIT_1);    
    SCPIP_WRITE_FLD(PRE_DS_MAIN_04+pdsOffset, _fscInfo[u4VdpId].u4PreDsInW, PRE_DS_MAIN_04_PREDS_MAIN_IN_H);	
    SCPIP_WRITE_FLD(PRE_DS_MAIN_04+pdsOffset, _fscInfo[u4VdpId].u4PreDsInH, PRE_DS_MAIN_04_PREDS_MAIN_IN_V);	
    SCPIP_WRITE_FLD(PRE_DS_MAIN_03+pdsOffset, _fscInfo[u4VdpId].u4PreDsOutW, PRE_DS_MAIN_03_PREDS_MAIN_OUT_H);	
    SCPIP_WRITE_FLD(PRE_DS_MAIN_03+pdsOffset, _fscInfo[u4VdpId].u4PreDsOutH, PRE_DS_MAIN_03_PREDS_MAIN_OUT_V);

    //regen register
    _vScpipUpdateRegenReg(u4VdpId);
   
	//add import
    SCPIP_WRITE_FLD(SCPIP_SCCTRL1_0A+ctrlOffset, 0, SCCTRL1_0A_SC_SET_RES_TOGGLE_1);	
	/* input timing */
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_09+ctrlOffset, _fscInfo[u4VdpId].u4InputWidth, SCCTRL1_09_SC_INPUT_WIDTH_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_09+ctrlOffset, _fscInfo[u4VdpId].u4InputHeight, SCCTRL1_09_SC_INPUT_HEIGHT_1);

	/* dram width or height */
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_0C+ctrlOffset, _fscInfo[u4VdpId].u4DramSRCWidth, SCCTRL1_0C_SC_DRAM_SRC_W_WIDTH_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_1C+ctrlOffset, _fscInfo[u4VdpId].u4DramWidth, SCCTRL1_1C_SC_DRAM_W_WIDTH_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_1F+ctrlOffset, _fscInfo[u4VdpId].u4DramHPitch, SCCTRL1_1F_SC_WR_DRAM_H_PITCH_1);

	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_1C+ctrlOffset, _fscInfo[u4VdpId].u4DramHeight, SCCTRL1_1C_SC_DRAM_W_HEIGHT_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_1E+ctrlOffset, _fscInfo[u4VdpId].u4DramVPitch, SCCTRL1_1E_SC_WR_DRAM_V_PITCH_1);

	/* scaling factor */
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_01+ctrlOffset, _fscInfo[u4VdpId].u4HDeltaPhaseIntg, SCCTRL1_01_SC_H_PHASE_DELTA_INTG_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_01+ctrlOffset, _fscInfo[u4VdpId].u4HDeltaPhaseFrac, (bPath==SV_VP_MAIN)?SCCTRL1_01_SC_H_PHASE_DELTA_FRAC_1: SCCTRL2_01_SC_H_PHASE_DELTA_FRAC_2);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_03+ctrlOffset, _fscInfo[u4VdpId].u4VDeltaPhaseIntg, SCCTRL1_03_SC_V_PHASE_DELTA_INTG_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_03+ctrlOffset, _fscInfo[u4VdpId].u4VDeltaPhaseFrac, (bPath==SV_VP_MAIN)?SCCTRL1_03_SC_V_PHASE_DELTA_FRAC_1: SCCTRL2_03_SC_V_PHASE_DELTA_FRAC_2);	

	/* initial phase */
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_00+ctrlOffset, _fscInfo[u4VdpId].u4HInitPhaseSign, SCCTRL1_00_SC_H_PHASE_INIT_SIGN_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_00+ctrlOffset, _fscInfo[u4VdpId].u4HInitPhaseIntg, SCCTRL1_00_SC_H_PHASE_INIT_INTG_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_00+ctrlOffset, _fscInfo[u4VdpId].u4HInitPhaseFrac, (bPath==SV_VP_MAIN)?SCCTRL1_00_SC_H_PHASE_INIT_FRAC_1:SCCTRL2_00_SC_H_PHASE_INIT_FRAC_2);

	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_02+ctrlOffset, _fscInfo[u4VdpId].u4VInitPhaseSign, SCCTRL1_02_SC_V_PHASE_INIT_SIGN_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_02+ctrlOffset, _fscInfo[u4VdpId].u4VInitPhaseIntg, SCCTRL1_02_SC_V_PHASE_INIT_INTG_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_02+ctrlOffset, _fscInfo[u4VdpId].u4VInitPhaseFrac, (bPath==SV_VP_MAIN)?SCCTRL1_02_SC_V_PHASE_INIT_FRAC_1:SCCTRL2_02_SC_V_PHASE_INIT_FRAC_2);

	if (u4VdpId == SV_VP_MAIN)
	{
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_04+ctrlOffset, _fscInfo[u4VdpId].u4HUVInitPhaseSign, SCCTRL1_04_SC_H_UV_PHASE_INIT_SIGN_1);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_04+ctrlOffset, _fscInfo[u4VdpId].u4HUVInitPhaseIntg, SCCTRL1_04_SC_H_UV_PHASE_INIT_INTG_1);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_04+ctrlOffset, _fscInfo[u4VdpId].u4HUVInitPhaseFrac, SCCTRL1_04_SC_H_UV_PHASE_INIT_FRAC_1);

    	/* non-linear */
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_05+mskOffset, _fscInfo[u4VdpId].u4NonLinear, SCCTRL1_05_SC_NONL_H_ENABLE);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_05+mskOffset, _fscInfo[u4VdpId].u4NonLinearPhaseSign, SCCTRL1_05_SC_NONL_H_ADD_MINUS_SEL);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_05+mskOffset, _fscInfo[u4VdpId].u4NonLinearStopPos, SCCTRL1_05_SC_NONL_H_STOP_POS);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_07+mskOffset, _fscInfo[u4VdpId].u4NonLinearMiddlePos, SCCTRL1_07_SC_NONL_H_MIDDLE_POS);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_06+mskOffset, _fscInfo[u4VdpId].u4NonLinearPhaseIntg, SCCTRL1_06_SC_NONL_H_PHASE_DELTA);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_06+mskOffset, _fscInfo[u4VdpId].u4NonLinearPhaseFrac, SCCTRL1_06_SC_NONL_H_PHASE_DELTA_FLOAT);
	}
	
	/* output timing*/
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_25+mskOffset, _fscInfo[u4VdpId].u4OutputX, SCCTRL1_25_SC_HMSK_STR_OFST_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_27+mskOffset, _fscInfo[u4VdpId].u4OutputY, SCCTRL1_27_SC_VMSK_STR_OFST_1);	
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_11+ctrlOffset, _fscInfo[u4VdpId].u4OutputWidth, SCCTRL1_11_SC_OUTPUT_WIDTH_1);
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_11+ctrlOffset, _fscInfo[u4VdpId].u4OutputHeight, SCCTRL1_11_SC_OUTPUT_HEIGHT_1);

	/* dram back mode || frn mode */
	if(_fscInfo[u4VdpId].u4Dispmode != VDP_SCPOS_DISPMODE_OFF)
	{
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_13+ctrlOffset, 0, SCCTRL1_13_SC_BCK_DRAM_MODE_1);
    	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_13+ctrlOffset, 0, SCCTRL1_13_SC_FRN_DRAM_MODE_1);
	}
	else
	{
    	...
    	{
        	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_13+ctrlOffset, 0, SCCTRL1_13_SC_BCK_DRAM_MODE_1);
        	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_13+ctrlOffset, 1, SCCTRL1_13_SC_FRN_DRAM_MODE_1);
    	}
    	else
    	{
        	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_13+ctrlOffset, 1, SCCTRL1_13_SC_BCK_DRAM_MODE_1);
        	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_13+ctrlOffset, 0, SCCTRL1_13_SC_FRN_DRAM_MODE_1);
    	}
	}	
	/* the time gen read pointer */
	SCPIP_WRITE_FLD(SCPIP_SCCTRL1_1E+ctrlOffset, _fscInfo[u4VdpId].u4RptGenCount, SCCTRL1_1E_SC_RPT_GEN_VCNT_1);

    SCPIP_WRITE_FLD(SCPIP_SCCTRL1_0A+ctrlOffset, 1, SCCTRL1_0A_SC_SET_RES_TOGGLE_1);	
    SCPIP_WRITE_FLD(SCPIP_SCSYS_01, 1, SCSYS_01_SC_SET_DATA);    
	return;
}
...
void vOnScalerInfoChg(UINT32 u4VdpId)
{
	...
	//OK~ 3. Update dram mode, dram widht height setting
