...
{
    UINT32 u4WidthLimitation, u4HeightLimitation, u4DispWidth;
    ...
    if ((u1ScpipGetDispMode(getScalerPath(u4VdpId)) == VDP_SCPOS_DISPMODE_OFF) && _bSRMStable[u4VdpId])  // dram mode
    {
        ...
        if(u4WidthLimitation> _arScalePrm[u4VdpId].rFbmInfo.u4Width)
            u4WidthLimitation = _arScalePrm[u4VdpId].rFbmInfo.u4Width;

        // check if current size setting exceeds FBM buffer size
        if((u4WidthLimitation * u4HeightLimitation) 
                > (_arScalePrm[u4VdpId].rFbmInfo.u4Width * _arScalePrm[u4VdpId].rFbmInfo.u4Height))
        {
...
void vScpipWriteCtrlOnOff(UCHAR ucVdpId, UCHAR ucOnOff)
{
    ...
    if (ucOnOff == SV_ON)
    {
        ...
        if(_u1ReadEnCnt[ucVdpId]==READ_DELAY_COUNTER )
        {
           ...
           #ifdef CUST_LCCT_DRV //for sony remy change to TV when without input signal, There is a unmute transient status show snow and black and then snow again
		   _vDrvVideoSetMute(MUTE_MODULE_SCPOS_DRAM, ucVdpId,(vScpipReCalDelayCounter(ucVdpId, 3)+2 + 40), TRUE);// + 40
		   #else
...
static void _vScpipUpdateLinePitch(UINT32 u4VdpId)
{
    ...
    LOG(1, "Line Pitch(%d) Width (%d)\n", scalerInfo->u4DramLinePitch, width);
}
/**
 *	@cobra porting done
**/
