...
{    
    ...
    #ifdef CC_MT5399
    vIO32WriteFldAlign(REG_LVDS_TX_CFG9, (bDriving+4), RG_LVDSA_TVO);
    #endif
...
UINT8 vDrvLVDSGetDriving(void)
{
    ...
    #else
    bDriving =( (IO32ReadFldAlign(REG_LVDS_TX_CFG3,  RG_LVDSB_TVO_EVEN_D11) )>>24)/2+3;
    #endif
	bDriving &= 0xf;  //0~15
    return (bDriving);
}
...
void u4ReadAPad(void)
{
	...
	printf("LVDSB_EXT_EN:  <0x%3x>\n", IO32ReadFldAlign(REG_LVDS_TX_CFG0,RG_LVDSB_EXT_EN_D11));
	printf("LVDSB_DRV_PWD: <0x%3x>\n", IO32ReadFldAlign(REG_LVDS_TX_CFG1,RG_LVDSB_DRV_PWD));
	printf("LVDSA_EXT_EN:  <0x%3x>\n", IO32ReadFldAlign(REG_LVDS_TX_CFG9,RG_LVDSA_EXT_EN_D11));
	printf("LVDSA_DRV_PWD: <0x%3x>\n", IO32ReadFldAlign(REG_LVDS_TX_CFG10,RG_LVDSA_DRV_PWD));
#else
	#ifndef CC_MT5881
	printf("			   ECK E5 | E4 E3 E2 E1 | E0 OCK O5 O4 | O3 O2 O1 O0\n");
	printf("LVDSB_EXT_EN:  <0x%4x>\n", IO32ReadFldAlign(REG_LVDS_TX_CFG0,RG_LVDSB_EXT_EN_D11));
	printf("LVDSB_DRV_PWD: <0x%4x>\n", IO32ReadFldAlign(REG_LVDS_TX_CFG1,RG_LVDSB_DRV_PWD));
	#endif
#endif
}
void vSetLVDSAPad(UINT8 u1OnOff, UINT16 u4BExtEn, UINT16 u4BDrvPwd,UINT16 u4AExtEn, UINT16 u4ADrvPwd)
{
	switch (u1OnOff)
	{
		...
		case 2: // user-defined
		#ifdef CC_MT5398
			vIO32WriteFldAlign(REG_LVDS_TX_CFG9, u4AExtEn, RG_LVDSA_EXT_EN_D11);
   			vIO32WriteFldAlign(REG_LVDS_TX_CFG0, u4BExtEn, RG_LVDSB_EXT_EN_D11);
   			vIO32WriteFldAlign(REG_LVDS_TX_CFG9, 0x1, RG_LVDSA_EXT_EN_D11_E0);
   			vIO32WriteFldAlign(REG_LVDS_TX_CFG10, u4ADrvPwd, RG_LVDSA_DRV_PWD);
   			vIO32WriteFldAlign(REG_LVDS_TX_CFG1, u4BDrvPwd, RG_LVDSB_DRV_PWD);
		#else
			#ifndef CC_MT5881
			vIO32WriteFldAlign(REG_LVDS_TX_CFG0, u4BExtEn, RG_LVDSB_EXT_EN_D11);
   			vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0x1, RG_LVDSB_EXT_EN_D11_E0);
...
void vDrvSetVOPLLClockSchema(UINT8 u1ClockSrc)
{
	...
	switch (u1ClockSrc)
	{
		case VOPLL_CLK_SRC_DDDS:
			...
			if ((IS_PANEL_L12R12)||(IS_PANEL_WXGA240))
