...
#ifdef CC_MHL_CBUS_CMOS_CTRL
#define CBUS_CMOS_CTRL_GPIO 0
#else
#define CBUS_CMOS_CTRL_GPIO 0
#endif
...
static UINT8 u1AbortTimerFlg = 0;
...
UINT8 bHpdInitFlg = 0;//for state change from discovery to connected
...
static UINT8 _Cbus_SendMsgEx(UINT16 *pMsgData, UINT8 dataSize)
{
	...
	if(u11Ktest)
	{
#if CBUS_CMOS_CTRL_GPIO    
		GPIO_SetOut(CBUS_CMOS_CTRL_GPIO, 0);
#endif		
	}
	...
	if(u1DebugLvl == 2)
	{	
	    LOG(0," >> tx [%d. %d]  iCnt: %d data: ",
		tTimer.u4Seconds, tTimer.u4Micros, dataSize);
		
			for(i = 0; i < dataSize; i++)
			{
				LOG(0, "0x%x ", (*(pMsgData+i)));
			}
			LOG(0,"\n");
	}
	else
...
static UINT8 _Cbus_SendMsg(void)
{
	...
	if(i>0)
	{
	   ...
	   {
		    LOG(0," >> tx [%d. %d]  iCnt: %d data: ",
		    tTimer.u4Seconds, tTimer.u4Micros,u1dataidx);
		    for(i = 0; i<u1dataidx; i++)
		    {
		        LOG(0,"0x%x ", aui2data[i]);
		    }
		    LOG(0,"\n");
	    }
#endif
...
void vCbus_HwInit(void)
{
    ...
    vIO32WriteFldAlign(PDWNC_PADCFG8, 1,FLD_HDMI_2_HPD_CBUS_E8);
	//disable cbus RX			
	...
	vIO32WriteFldAlign(PDWNC_PADCFG8,0,FLD_HDMI_2_HPD_CBUS_PU);//280e8 bit 3 PU  --- SET TO 0
    //cbus test 
    if((u4WakeUpReason == (UINT32)PDWNC_WAKE_UP_REASON_AC_POWER) || 
	   (u4WakeUpReason == (UINT32)PDWNC_WAKE_UP_REASON_WATCHDOG))
    {
	    vIO32WriteFldAlign(PDWNC_PADCFG8,0,FLD_HDMI_2_HPD_CBUS_PUPD);//280E8 bit 9 PUPD ---  SET to 1
	...
	vIO32WriteFldAlign(PDWNC_PADCFG8,0,FLD_HDMI_2_HPD_CBUS_SMT);//SMT ---  SET to 0
	//vIO32WriteFldAlign(PDWNC_CBUS_LINK_07, 2, FLD_ZCBUS_SINK_ON_CTRL);// PD0 -- 28d50 bit 23 set to 1 enable 1k resister
	//vIO32WriteFldAlign(PDWNC_CBUS_LINK_07, 5, FLD_ZCBUS_SINK_ON_CTRL);																  //pd1 / pd -- bit 25:24 set to 10, 100k resister
...
#endif		
						if(_arHdmiRx[HDMI_SWITCH_3]._bHDMIState == 6)
						{
							if(u1AnReady == 2)
							{
							    u1Bcaps = 0xa1;
...
{  
    ...
    x_memset(test, 0, 16*sizeof(UINT8));
	...
	UNUSED(u2Vector);
     ...
     u4IntStat = u4IO32Read4B(PDWNC_CBUS_STA_00);
		...
		if (u4IntStat & 0x1)
		{
			...
			// pull rx sense
			// TODO
			if(eActiveHdmiPort == HDMI_SWITCH_2)
			{
			   vCbusSetRapContentOff();
			}
			else
			{
				MhlSetHpd = 1;
	...
	if (u4IntStat & 0x40)// 	6
	{
        ...
        {
#if CBUS_CMOS_CTRL_GPIO    
		    GPIO_SetOut(CBUS_CMOS_CTRL_GPIO, 1);
#endif		    
	    }
		u4ClrInt |= 0x40;
    ...
    if(u1DebugLvl == 2)
    {
		if((test_idx > 0)&&(test_idx <= 16))
		{		
			LOG(0," << rx [%d. %d]  iCnt: %d data: ",
			tRxTime.u4Seconds, tRxTime.u4Micros,u1RxMsgCnts);
...
UINT8 u1CableDisconnectTimer = 100;
void vCbusCmdTest(UINT8 cmd_id, UINT8 sub_cmd_id)
{
	...
	case 10:
		...
		LOG(0,"Write burst test command\n");
		//_cbusWriteBurst();
		break;
	case 11:
...
{
	LOG(1,"[cbus] boot up timer out, %d\n", u1BootUp);
    ...
    if(u1BootUp == 2)
    {
       ...
       vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,0, FLD_ZCBUS_DISCOVER_EN);
 	    ...
 	    GPIO_SetOut(VBUS_CTRL_GPIO, 0);//disable vbus
#endif
		if(fgHDMIActiveportNum()!=HDMI_SWITCH_2)
		{
		  vMHLOpenRxTermination(0);
		}
    ...
    else
    {
		...
		//cbus test 
		vIO32WriteFldAlign(PDWNC_PADCFG8,1,FLD_HDMI_2_HPD_CBUS_PUPD);//280E8 bit 9 PUPD ---  SET to 1
...
void vMhlEmucatorCableDisconnect(void)
{
	vIO32WriteFldAlign(PDWNC_CBUS_LINK_0C,0,FLD_CDSENSE_FORCE1);
	vIO32WriteFldAlign(PDWNC_CBUS_LINK_0C,1,FLD_CDSENSE_GATED);
	vUtDelay1ms(u1CableDisconnectTimer);
	vIO32WriteFldAlign(PDWNC_CBUS_LINK_0C,0,FLD_CDSENSE_GATED);	
}
	...
	switch (iMhlStateOld)
	{
		...
		case MHL_STATE_INIT:
           ...
           _Cbus_OcpStopTimer();
			...
			{
#if CBUS_CMOS_CTRL_GPIO    		    
			    GPIO_SetOut(CBUS_CMOS_CTRL_GPIO, 1);//for cts test
#endif	
			}
			//disable cbus RX			
			vIO32WriteFldAlign(PDWNC_CBUS_LINK_02,0,FLD_LINKRX_EN);
			...
			if(fgHDMIActiveportNum() != HDMI_SWITCH_3)
			{
				vSelMHLPort(0);
			}
		break;
...
void vCbusInit(void)
{
	...
	if(bInitFlg == 0)
	{
	    ...
	    VERIFY(x_thread_create(&phCbusThreadHdl, 
	        "CbusThread", 2048,  59, _Cbus_Thread, 0, NULL) == OSR_OK);
        VERIFY(x_thread_create(&phCbusHdcpThreadHdl, 
        "HdcpThread", 2048,  59, _Hdcp_Thread, 0, NULL) == OSR_OK);
	  //  if(IS_SupportMHL())
	    {
	        VERIFY(PDWNC_RegIsr(PDWNC_INTNO_CBUS_INT, _Cbus_Interrupt, &pfnOldIsr) == PDWNC_OK);
	    }
    }
}
...
{
#ifdef VBUS_CTRL_GPIO
     //vbus is auto control by gpio for sony spec
    INT32 VbusValue;
	  VbusValue = (INT32)u1VbusState;
    GPIO_SetOut(VBUS_CTRL_GPIO,VbusValue);//enable vbus
#endif    
#ifdef CBUS_DMSG
...
{
	if((vCbusGetState()==MHL_STATE_CONNECTED)&&(eActiveHdmiPort == HDMI_SWITCH_3))
	{
	    bHpdInitFlg = 1;
		MhlSetHpd = 1;
		MhlSetPathen = 0;
...
void vCbusSetRapContentOff(void)
{
    MHL_MSG_T t_Msg;
    UINT8 i;
	arTxMscMsgs[0] = CBUS_MSC_CTRL_MSC_MSG;
