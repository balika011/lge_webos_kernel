...
static void vScartUpdateAutoMode(void)
{
    ...
    {
        if(IO32ReadFldAlign(SCART_STA_00, FLD_FB_OFF_CNT) < 10
           && IO32ReadFldAlign(SCART_STA_00, FLD_FB_ON_CNT) > 512)
        {
            /* almost all RGB */
            _bScartAutoMode = SCART_COLOR_RGB;
        }
        else if(IO32ReadFldAlign(SCART_STA_00, FLD_FB_ON_CNT) < 10)
        {
...
void vScartReadFsSrvAdc(void)
{
    ...
    {
        DBG_Printf(SCART_LOG,"Read Scart1 FS hi threshold = %d\n", ui4ScartThre);
    }

    if(0 == DRVCUST_OptQuery(eSCARTPin8LowThres,&ui4ScartThre))
    {
        DBG_Printf(SCART_LOG,"Read Scart2 FS hi threshold = %d\n", ui4ScartThre);
    }
}
