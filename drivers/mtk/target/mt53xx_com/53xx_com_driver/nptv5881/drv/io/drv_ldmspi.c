...
BOOL bDrvLcdimSpiSWInit(void)
{
    ...
    else    //fix klockwork warning
    {
  	...
  	// 4. get panel order
#ifndef CC_MTK_LOADER
    u1TempblockNum = (u2DrvLcdimSpiGetBlockNum()<200) ? u2DrvLcdimSpiGetBlockNum() : 200;
    DRVCUST_GetLcdimPanelOrder(u1DriverOrder, u1TempblockNum);
    DRVCUST_GetLcdimPanelDimGain(u1BlockDimGain, (UINT8)u2DrvLcdimSpiGetBlockNum());
#endif //CC_MTK_LOADER   
