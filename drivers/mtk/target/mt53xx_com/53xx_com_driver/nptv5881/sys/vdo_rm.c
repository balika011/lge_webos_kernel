...
EXTERN void vDrvTDTVTriggerCfgCheck(void);
...
void vVRMUpdateMIBResolution(UINT8 bPath)
{
    ...
    if(curVrmInfo.u2ScanMode && !bDrvDIIsScalerBob(bPath)) // interlaced input, de-interlaced at MIB
    {        
...
void vVRMUpdateFSCResolution(UINT8 bPath)
{
    ...
    if(bDrvDIIsScalerBob(getScalerVDP(bPath)) == SV_TRUE)
    {
        curVrmInfo.u2Height /= 2;
    }
...
void vDrvFireImportTrigger(UINT8 bPath)
{
    ...
    switch(module)
    {
        ...
        case IMPORT_MODULE_PDS:
            vDrvFirePDSImportTrigger(bPath);//remy have fix the problem about abnormal trigger signal in pre-down 
