...
#ifdef CC_MT5398// reserved for 120Hz Panel
...
void vVdoOutISRCheckSetDataTrigger(void)
{
	_u4VSyncCountSD ++;
	#if defined(CC_MT5398)  
	if((fgApplyScalerSetData) || (bPscIsNeedSetData() == SV_TRUE) || (fgApplyScanPWMSetData))       //PSC
	#else
	if((fgApplyScalerSetData) || (bScpipIsNeedSetData() == SV_TRUE) || (fgApplyScanPWMSetData))    //FSC
    #endif
	{	
        if(_u4VSyncCountSD%3 == 0)
        {
            ...
            {
                vPscUpdateReg_SETDATA();                                //set PSC reg
                vPscSetNeedSetDataFlag(SV_FALSE);                       //clear PSC flag
            }
            #else
