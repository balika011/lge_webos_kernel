...
#ifdef CUST_LCCT_DRV
extern UINT8 ui1_karaoke_status; //0: no device 1: yes
#endif
...
extern E_TDTV_UI_3D_PACKING u4DrvTDTV3DModeQuery(void);
...
LOG(1, "***** MAIN_FLG_MODE_CHG*****\n");
                ...
                //for Remy play the MM CH+- semiauto maintain pre 3D mode
                //init the 3DNaviResult when mode change
                u1DrvTdNaviModeChange();
                _fgFRS_ENA = SV_OFF;
        ...
        case VDO_STATE_WAIT_MODE_DET:
            if(fgIsMainDec())
            {
				...
				#ifdef FORCE_3D_60HZ_OUTPUT_ENABLE
					//if((u4DrvTDTV3DModeQuery()>=E_TDTV_UI_3D_MODE_TTD)&&(u4DrvTDTV3DModeQuery()<E_TDTV_UI_3D_MODE_END))
					if (u4DrvTDTV3DModeQuery()!=E_TDTV_UI_3D_MODE_OFF)
					{
						_bForceFrameRate=60;
						LOG(3, "====#3 TDTV 3DMode: %d -->Force-60Hz\n", u4DrvTDTV3DModeQuery());
					}
					//else if (u4DrvTDTV3DModeQuery()==E_TDTV_UI_3D_MODE_OFF)
					else
					{
						_bForceFrameRate=0;
						LOG(3, "====#4 cancel Force-60Hz\n");
					}
				#endif
...
void vApiVideoMainloop(void)
{
...
#if defined(CUST_LCCT_DRV) && defined(SYS_MHL_SUPPORT)
	static BOOL b_cur_stt = FALSE;
	if((ui1_karaoke_status)&&((_IsMhlDevice() == 1)&&fgIsHDMI()))
	{
		if(b_cur_stt == FALSE)
		{
			b_cur_stt = TRUE;
#ifdef KARAOKE_MIC_LINE_IN2  
			AUD_MicInput_Enable(TRUE);  //wait for integration test.	
#endif  
		}
	}
	else
	{
		if(b_cur_stt == TRUE)
		{
			b_cur_stt = FALSE;
#ifdef KARAOKE_MIC_LINE_IN2  
			AUD_MicInput_Enable(FALSE);	//wait for integration test.	
#endif  
		}
	}
#endif
...
//    }
    ...
    {
        vRegenLoop(SV_VP_MAIN);
        #if SUPPORT_POP
        vRegenLoop(SV_VP_PIP);   
        #endif
    }
