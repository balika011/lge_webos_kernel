...
VOID vDrvVideoHandleUnMute(VOID)
{
    ...
    for(u4Path = SV_VP_MAIN; u4Path <= SV_VP_PIP; u4Path++)
    {
        ...
        if(_au4LastMuteState[u4Path] != u4MuteOnOFF)
        {
            ...
            {
                if ((_rVideoMuteInfo[SV_VP_MAIN][MUTE_MODULE_MJC].u4Delay < 8) && (((SRM_IsPipVideo() && (_eApplicationMode == SRM_TV_MODE_TYPE_PIP)) || SRM_IsPopVideo()) == 0))
                {
                    _rVideoMuteInfo[SV_VP_MAIN][MUTE_MODULE_MJC].u4Delay = 8;
                }
            }
            #endif
