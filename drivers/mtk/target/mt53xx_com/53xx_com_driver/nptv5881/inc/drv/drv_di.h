...
//#define MIB_DISP_MODE_ON_WA
#define MIB_LI_INPUT_WA
//#define MIB_WIDTH_256N2_TO_256N_WA
...
#define IS_ECO_1_IC(void)                ((IS_IC_5398()&&(BSP_GetIcVersion() > IC_VER_5398_AA)) \
                                                ||IS_IC_5880()||IS_IC_5860()||IS_IC_5881())

#define IS_ECO_2_IC(void)                ((IS_IC_5398()&&(BSP_GetIcVersion() > IC_VER_5398_AA)) \
                                                ||(IS_IC_5880()&&(BSP_GetIcVersion() > IC_VER_5880_AA)) \
                                                ||(IS_IC_5860()&&(BSP_GetIcVersion() > IC_VER_5860_AA))\
                                                ||(IS_IC_5881()))
...
#define IS_PR_MODE(u1Id)                 (((E_TD_OUT_3D_LI==_arMDDiPrm[u1Id].eTDOutput)\
                                                &&(IS_INTERLACE(u1Id)||((!IS_ECO_2_IC(u1Id))&& IS_420TO422(u1Id)))) \
...
#define MIB_SYS_DYN_ELE_LIST \
    MIB_ELE_CMD(UINT16,u2FreezeWriteFilter)\
    MIB_ELE_CMD(UINT8,u1VSyncCnt)\
    MIB_ELE_CMD(UINT16,u2Vcnt)\
    MIB_ELE_CMD(UINT8,u1FreezeCnt)\
    MIB_ELE_CMD(UINT8,u1StatusCnt)\
    MIB_ELE_CMD(UINT8,bAllowedAutoStop)\
    MIB_ELE_CMD(UINT8,u1AutoStop)\
    MIB_ELE_CMD(UINT8,u1SRCWAutoStopDisableCnt)\
    ...
    MIB_ELE_CMD(UINT8,u1OutbufCnt)\
    MIB_ELE_CMD(UINT8,u1SetResFinishedCnt)\
    MIB_ELE_CMD(UINT8,u1SetResTimeOutCnt)
