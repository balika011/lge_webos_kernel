...
{     
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIX_PWD);//
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIXGAIN);//
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIXINPUT);//
    vIO32WriteFldAlign(VFE_E0, 0, RG_BTM_ENA); /*add for V2 */
    vIO32WriteFldAlign(VFE_E0, 0, RG_BTM_ENB); /*add for V2 */
    ...
    vIO32WriteFldAlign(VFE_E1, 1, RG_UPDNA);//
    vIO32WriteFldAlign(VFE_E1, 1, RG_UPDNB);//
    vIO32WriteFldAlign(VFE_E1, 1, RG_VAGSELA);//
    vIO32WriteFldAlign(VFE_E1, 1, RG_VAGSELB);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 0, RG_CALPGAA);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 0, RG_CALPGAB);//
    vIO32WriteFldAlign(VFE_E1, 0, RG_C_SELA);//
    vIO32WriteFldAlign(VFE_E1, 0, RG_C_SELB);//
    ...
    vIO32WriteFldAlign(VFE_E2, 0, RG_PGABUFNA_PWD);//
    vIO32WriteFldAlign(VFE_E2, 0, RG_PGABUFNB_PWD);//
    ...
    vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFTA_PWD);//
    vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFTB_PWD);//
    vIO32WriteFldAlign(VFE_E2, 0, RG_OFFCURA_PWD); /*add for V2 */
    vIO32WriteFldAlign(VFE_E2, 0, RG_OFFCURB_PWD);  /*add for V2 */
    ...
    vIO32WriteFldAlign(VFE_E3, 0x20, RG_AISEL);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E4, 0, RG_FE_SELA);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E4, 0, RG_FE_SELB);//
    vIO32WriteFldAlign(VFE_E3, 0, RG_AISELR);//
    vIO32WriteFldAlign(VFE_E3, 3, RG_AAF_BW); /*add for V2 */
    ...
    vIO32WriteFldAlign(VFE_E4, 2, RG_MIXVOCM_SEL);//
    ...
    vIO32WriteFldAlign(VFE_E5, 1, RG_20U_MIXOFF);//
    ...
    vIO32WriteFldAlign(VFE_E7, 0, RG_CINRA_EN);//
    vIO32WriteFldAlign(VFE_E7, 0, RG_CINRB_EN);//
    vIO32WriteFldAlign(VFE_E7, 0x61, RG_CVBS_REV);//

    vIO32WriteFldAlign(VFE_E6, 0, RG_CVBSADC_SEL_CKPLL);// CY 1->0
    vIO32WriteFldAlign(VFE_E6, 0, RG_CVBSADC_CKCTRL);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E0, 0, RG_BG_PD);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E6, 0, RG_CVBSADC_VRSEL1); //
    vIO32WriteFldAlign(VFE_E6, 1, RG_CVBSADC_BIASSEL);//
    vIO32WriteFldAlign(VFE_E5, 0, RG_CVBSADC_PWD);//
    vIO32WriteFldAlign(VFE_E5, 3, RG_CVBSADC_CHSEL);//

    vIO32WriteFldAlign(VFE_E4, 1, RG_50U_CLAMPA);//
    vIO32WriteFldAlign(VFE_E4, 1, RG_50U_CLAMPB);//
...
void vDrvADCOffsetCal(void)
{
    ...
    vIO32WriteFldAlign(HDFE_02, 0x00, AD1_OFFSET);  //set digital offset =1 for CH 1
    vIO32WriteFldAlign(HDFE_02, 0x00, AD2_OFFSET);	 //set digital offset =1 for CH 2 
...
void vDrvBypPD(void)
{
    vIO32WriteFldAlign(VFE_E0, 1, RG_MIX_PWD);// //power down mixer
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIXINPUT);// CH A
}
// => bApiVideoMainSubSrc
/**
...
void vDrvAvByp(void)
{
    ...
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIXINPUT);//   //input ChA
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIX_PWD);//      // Power on  Mixer0
    vIO32WriteFldAlign(VFE_E0, 5, RG_MIXGAIN);//      //MIX0 gain = 1.65X
...
void vDrvSVdoByp(void)  //MT5365 can not support A+B channel
{
    ...
    //Use mixer 0 for AV bypass
    //vIO32WriteFldAlign(VFE_E0, 3, RG_MIXINPUT);
    //vIO32WriteFldAlign(VFE_E0, 0, RG_MIX_PWD);//      // Power on  Mixer0
    //vIO32WriteFldAlign(VFE_E0, 5, RG_MIXGAIN);//      //MIX0 gain = 1.33X
	...
	switch (VSS_MAJOR(vsrc))
	{
		...
		case VSS_CVBS:
            ...
            if (((_bMainICIn == P_FB0) || (_bSubICIn == P_FB0) || (_bMainICIn == P_FB1) || (_bSubICIn == P_FB1))  && (bINT_BST_DET == 1)) //internal path
			{
			    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E0, 0, RG_CLAMP_SINK_ENB);//
			    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E0, 0, RG_CLAMP_SOURCE_ENB);//
                vIO32WriteFldAlign(VFE_E2, 1,  RG_SHIFTB_PWD);//
		...
		case VSS_SV:
			...
			/*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_PGAB_PWD);//
			/*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_CTINB);//
			vIO32WriteFldAlign(VFE_E3, 0, RG_OFFCURB);//
...
{//FLUKE white pattern pure AV
    ...
    for(bCALCLAMP=0;bCALCLAMP<=0x0f;bCALCLAMP++)
        for(bOFFCUR=0;bOFFCUR<=0x1f;bOFFCUR++)
        {
            ...
            vIO32WriteFldAlign(VFE_E3, bOFFCUR, RG_OFFCURA);//
            vUtDelay1ms(50);
    ...
    vIO32WriteFldAlign(VFE_E3, 0x00, RG_OFFCURA);//     //default value
}
...
{//plug AV and SV
    ...
    IO32ReadFldAlign(VFE_E3,(IO32ReadFldAlign(VFE_E3, RG_AISELR)|0x001), RG_AISELR);//
...
void vDrvCvbsAdcPD(void)
{
    ...
    #endif
     vIO32WriteFldAlign(VFE_E5, 1, RG_CVBSADC_PWD);//
   #endif
}
...
{
   #if PD_ADC_ENABLE
    vIO32WriteFldAlign(VFE_E0, 1, RG_MIX_PWD);//
    vIO32WriteFldAlign(VFE_E0, 0, RG_MIXINPUT);//
    ...
    vIO32WriteFldAlign(VFE_E2, 1, RG_PGABUFNA_PWD);//
    ...
    vIO32WriteFldAlign(VFE_E2, 1, RG_PGABUFNB_PWD);//
...
void vDrvCvbsVfePWON(void)
{
    ...
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_BUFB_PWD);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_CTBUFB_PWD);//
    vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFTA_PWD);    /*add for V2*/
    vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFTB_PWD);    /*add for V2*/
    vIO32WriteFldAlign(VFE_E2, 0, RG_OFFCURA_PWD); /*add for V2*/
    vIO32WriteFldAlign(VFE_E2, 0, RG_OFFCURB_PWD); /*add for V2*/
    vIO32WriteFldAlign(VFE_E4, 0, RG_CVBS_PWD);       /*add for V2*/
...
void vDrvHDTVADCDefaultSetting(void)
{
    ...
    //vIO32WriteFldAlign(VFE_00, 1, SELF_EN); //5365 mark
       
    vIO32WriteFldMulti(VFE_00, P_Fld(0,ALWAYSAG)|P_Fld(0,ACMONEN)|P_Fld(5,IBG_SEL)|P_Fld(0x0B,VBG_SEL)
	...
	for (bCh=0; bCh<3; bCh++)
	{
		//vIO32WriteFldAlign(VFE_01+16*bCh, 0x00000A0A, VFE_01_All);   //VFE_01, VFE_05, VFE_09 //20090323 37.5uA->62.5uA for SAG
        vIO32WriteFldMulti(VFE_01+16*bCh,P_Fld(0,CLAMP_SOURCE_OFF_1)|P_Fld(0,CLAMP_SINK_OFF_1)
                              |P_Fld(1,AG_GATE_1)|P_Fld(0,PGA_1_PWD)|P_Fld(0,SHORTPGAO_1)
                              |P_Fld(0x0A,CLAMP_SINK_1)|P_Fld(0x0A,CLAMP_SOURCE_1));

		//vIO32WriteFldAlign(VFE_02+16*bCh, 0x40000802, VFE_02_All);   //VFE_02, VFE_06, VFE_0A //0x40000804 reduce PGA bias current from 50uA to 25uA  
        vIO32WriteFldMulti(VFE_02+16*bCh, P_Fld(0,AGMONEN_1)|P_Fld(0,ADCTESTEN1)|P_Fld(0,PGATEST_1)
                              |P_Fld(2,NCKCTRL_1)|P_Fld(0,VCLAMP_MONEN)|P_Fld(0,GBIAS_1_PWD)|P_Fld(2,PGABIAS_1));

		//vIO32WriteFldAlign(VFE_04+16*bCh, 0x00410000, VFE_04_All);	//VFE_04, VFE_08, VFE_0C // enable AG Buffer for MT5387_88 and set AG 600mV(recommended)
        vIO32WriteFldMulti(VFE_03+16*bCh, P_Fld(0,DIVSEL_1)|P_Fld(2,IFLASH_1)|P_Fld(2,IGBIAS_1)
    ...
    vIO32WriteFldAlign(VFE_03, 0, TRGSEL);

    vIO32WriteFldMulti(VFE_04, P_Fld(0,ADCBIAS_PWD)|P_Fld(0,AGGEN_PWD)|P_Fld(0,VREFGEN_PWD)
                        |P_Fld(1,ACBUF_PWD)|P_Fld(0,ACHIN)|P_Fld(1,ACRES_EN)|P_Fld(0,ACCH1IN)
                        |P_Fld(0,VGA_ABIST_EN)|P_Fld(0,VGA_ABIST_IN));
    vIO32WriteFldMulti(VFE_08, P_Fld(1,AGSEL)|P_Fld(0,RESSEL)|P_Fld(7,CAP_EN)|P_Fld(1,OFFSET_EN)
                        |P_Fld(3,OFFSET)|P_Fld(0,ACCH2IN)|P_Fld(0,SYNC_ABIST_EN)|P_Fld(0,SYNC_ABIST_IN));
#ifdef CC_MT5365_V2
    vIO32WriteFldMulti(VFE_0C,P_Fld(0x06,ADC_VREFP)|P_Fld(0x03,ADC_VREFN));
#else
    vIO32WriteFldMulti(VFE_0C,P_Fld(0x03,ADC_VREFP)|P_Fld(0x03,ADC_VREFN));
#endif

   #if Phase3Channel
    vIO32WriteFldMulti(VFE_0C, P_Fld(1,CLKINV_EN)|P_Fld(1,RELATCH_EN)|P_Fld(0,ACCH3IN));
   #else
    vIO32WriteFldMulti(VFE_0C, P_Fld(0,CLKINV_EN)|P_Fld(0,RELATCH_EN)|P_Fld(0,ACCH3IN));
   #endif
       
   #if 1	
    // Slove VGA ADC no data
	vIO32WriteFldAlign(VFE_03, 0, CLR_1);vUtDelay1ms(1);
	vIO32WriteFldAlign(VFE_03, 1, CLR_1);vUtDelay1ms(1);
	vIO32WriteFldAlign(VFE_03, 0, CLR_1);vUtDelay1ms(1);
    // Slove VGA ADC no data
   #endif
...
void vDrvSetHDTVADC(UINT8 bVDOINPUT)
{
    ...
    {
        //vIO32WriteFldAlign(VFE_02+(bLoopCnt<<4), 0, DCLAMP_EN_1);      //5365 mark //disable digital clamp of CH 1/2/3
        vIO32WriteFldAlign(VFE_01+(bLoopCnt<<4), 0, CLAMP_SOURCE_OFF_1);
        vIO32WriteFldAlign(VFE_01+(bLoopCnt<<4), 0, CLAMP_SINK_OFF_1); 	
    }
    ...
    vIO32WriteFldAlign(HDFE_00, 0x0F, IDX1_COMP_EN); 	//[SA7_Michael] enable mapping table for CH1,2,3
    ...
    switch (bVDOINPUT)
    {
        case P_YP0:   //HDTV
        case P_YP1:
            vIO32WriteFldAlign(VFE_01, 0, CLAMP_MIDDLE_1);
            vIO32WriteFldAlign(VFE_05, 1, CLAMP_MIDDLE_2);
            vIO32WriteFldAlign(VFE_09, 1, CLAMP_MIDDLE_3);		
            bInputType = 0;
        break;
        case P_VGA:
            vIO32WriteFldAlign(VFE_01, 0, CLAMP_MIDDLE_1);
            vIO32WriteFldAlign(VFE_05, 0, CLAMP_MIDDLE_2);
            vIO32WriteFldAlign(VFE_09, 0, CLAMP_MIDDLE_3);
            bInputType = 1;
        break;
        ...
        case P_VGACOMP:
        	vIO32WriteFldAlign(VFE_01, 1, CLAMP_MIDDLE_1);
        	vIO32WriteFldAlign(VFE_05, 0, CLAMP_MIDDLE_2);
        	vIO32WriteFldAlign(VFE_09, 1, CLAMP_MIDDLE_3);
        	bInputType = 3;
            break;    
    ...
    if(bInputType != 0xFF)
    {
		...
		vIO32WriteFldAlign(VFE_02+(0x10*bLoopCnt), bTmpLowByte , PGAOFFSET_1L);  //PGAOFFSET_1,PGAOFFSET_2,PGAOFFSET_3
		vIO32WriteFldAlign(VFE_02+(0x10*bLoopCnt), bTmpHighByte , PGAOFFSET_1H);  //PGAOFFSET_1,PGAOFFSET_2,PGAOFFSET_3
        }
		vIO32WriteFldAlign(HDFE_01, _bAutoColorGain[bInputType][0], AD1_GAIN);	// reset the Digital gain to unit gain
		vIO32WriteFldAlign(HDFE_01, _bAutoColorGain[bInputType][1], AD2_GAIN);
		vIO32WriteFldAlign(HDFE_02, _bAutoColorGain[bInputType][2], AD3_GAIN);
     #else
        for(bLoopCnt=0; bLoopCnt<3; bLoopCnt++)
        {
            vIO32WriteFldAlign(VFE_02+(0x10*bLoopCnt), _bAutoColorGain[bInputType][bLoopCnt], PGAGAIN_1);      //PGAGAIN_1,PGAGAIN_2,PGAGAIN_3
            vIO32WriteFldAlign(VFE_02+(0x10*bLoopCnt), _bAutoColorOffset[bInputType][bLoopCnt], PGAOFFSET_1);  //PGAOFFSET_1,PGAOFFSET_2,PGAOFFSET_3
        }
...
void vDrvAllHDADCPow(UINT8 bPow)
{
	...
	if (bPow == SV_TRUE)//Power on
	{
       #if PD_ADC_ENABLE
        //vIO32WriteFldAlign(VFE_00, 1, SELF_EN); //5365 mark  
		vIO32WriteFldAlign(VFE_00,0,BG_PWD);
        vUtDelay1ms(2);
		for (bCh=0; bCh<3; bCh++)
		{
            //vIO32WriteFldAlign(VFE_01+(bCh<<4), 0, CLAMP_1_PWD); //5365 mark  
            vIO32WriteFldAlign(VFE_01+(bCh<<4), 0, CLAMP_SOURCE_OFF_1);		
            vIO32WriteFldAlign(VFE_01+(bCh<<4), 0, CLAMP_SINK_OFF_1);		
            vIO32WriteFldAlign(VFE_01+(bCh<<4), 0, PGA_1_PWD);		
            vIO32WriteFldAlign(VFE_03+(bCh<<4), 0, ADC_1_PWD);	
            vIO32WriteFldAlign(VFE_03+(bCh<<4), 0, ADCCK_1_PWD);
		}
        vUtDelay1ms(2);
        //vIO32WriteFldAlign(VFE_00, 0, SELF_EN); //5365 mark    
       #endif 
	...
	{
       #if PD_ADC_ENABLE
		vIO32WriteFldAlign(VFE_00,1,BG_PWD);
		for (bCh=0; bCh<3; bCh++)
		{
            //vIO32WriteFldAlign(VFE_01,1,CLAMP_1_PWD); //5365 mark  
            vIO32WriteFldAlign(VFE_01+(bCh<<4),1,CLAMP_SOURCE_OFF_1);	   
            vIO32WriteFldAlign(VFE_01+(bCh<<4),1,CLAMP_SINK_OFF_1); 	   
            vIO32WriteFldAlign(VFE_01+(bCh<<4),1,PGA_1_PWD);	   
            vIO32WriteFldAlign(VFE_03+(bCh<<4),1,ADC_1_PWD);			   
            //vIO32WriteFldAlign(VFE_03+(bCh<<4), 1, ADCCK_1_PWD);
		}
       #endif 
...
UINT8 vCust_R_Offset_Read(void)
{
       ...
       return 0;
#else
	return (UINT8)(IO32ReadFldAlign(VFE_0A, PGAOFFSET_3));
#endif	
}
...
UINT8 vCust_G_Offset_Read(void)
{
       ...
       return 0;
#else
	return (UINT8)(IO32ReadFldAlign(VFE_06, PGAOFFSET_2));
#endif
}
...
UINT8 vCust_B_Offset_Read(void)
{
       ...
       return 0;
#else
	return (UINT8)(IO32ReadFldAlign(VFE_02, PGAOFFSET_1));
#endif	
}
...
UINT8 vCust_R_Gain_Read(void)
{
       ...
       return 0;
#else
	return (UINT8)(IO32ReadFldAlign(VFE_0A, PGAGAIN_3));
#endif	
}
...
UINT8 vCust_G_Gain_Read(void)
{
       ...
       return 0;
#else
	return (UINT8)(IO32ReadFldAlign(VFE_06, PGAGAIN_2));
#endif	
}
...
UINT8 vCust_B_Gain_Read(void)
{
       ...
       return 0;
#else
	return (UINT8)(IO32ReadFldAlign(VFE_02, PGAGAIN_1));
#endif	
}
...
void vScartVfeConnect(void)
{
    ...
    // 1. Select YC mixer
    vIO32WriteFldMulti(VFE_E0, P_Fld(0,RG_MIX_PWD)|P_Fld(0,RG_MIXINPUT)|P_Fld(5,RG_MIXGAIN));//power on mixer 0
        
    //vIO32WriteFldMulti(VFE_E0, P_Fld(0,RG_MIX1_PWD)|P_Fld(1,RG_MIX1_INPUT)|P_Fld(2,RG_MIX1_GAIN));//power on mixer 1
    
    vIO32WriteFldMulti(VFE_E5, P_Fld(1,RG_MIX_10U));//
    vIO32WriteFldMulti(VFE_E4, /*mark for V2: P_Fld(1,RG_MIX_60U_EN) |P_Fld(0,RG_MIX_SHORT)|*/P_Fld(2,RG_MIXVOCM_SEL));//
   ...
   // vIO32WriteFldMulti(VGACFG2,P_Fld(0,FLD_RG_DIV2_EN)|P_Fld(0,FLD_RG_DIV3_EN)|P_Fld(0,FLD_RG_DIV5_EN)|P_Fld(0,FLD_RG_CKINV_SOGY_EN)|P_Fld(0,FLD_RG_CLKSEL_SOGY));//use 27Mhz 
	...
	{		
	  ...
	  if ((IO32ReadFldAlign(VFE_E3, Fld(32, 0, AC_FULLDW))&CHA)>=1)
	  bINT_BST_DET=1;
	  else
	  bINT_BST_DET=0;
    if(bINT_BST_DET==1) // Internal Path, set from source_table.c
    {            
             vIO32WriteFldAlign(VFE_E7, 0x01, RG_CINRB_EN);//connect SC from VGA VFE to CVBS VFE in CVBS part             
             if((_bMainICIn == P_FB0) || (_bSubICIn == P_FB0))//SCART1
             {
...
{ //Michael Modify :20100308
    ...
    vIO32WriteFldAlign(VFE_E7, 0x00, RG_CINRA_EN);// //disconnect C in R at CHB
    vIO32WriteFldAlign(VFE_E7, 0x00, RG_CINRB_EN);// //disconnect C in R at CHB
...
void vPGA_LPF_BW(UINT8 bw)
{
    ...
    {
        case 0: 
            vIO32WriteFldAlign(VFE_08, 0x00, CAP_EN);//no setting any LPF
            vIO32WriteFldAlign(VGACFG2, 0x00, FLD_RG_MUXCAP_EN);
            vIO32WriteFldAlign(SYNCFG3, 0x00, FLD_RG_INPUT_CAP_5PF);
            break;

        case 1: 
            vIO32WriteFldAlign(VFE_08, 0x0F, CAP_EN);//  pixel clock <41MHz
            vIO32WriteFldAlign(VGACFG2, 0x03, FLD_RG_MUXCAP_EN);
            vIO32WriteFldAlign(SYNCFG3, 0x01, FLD_RG_INPUT_CAP_5PF);
            break;

        case 2: 
            vIO32WriteFldAlign(VFE_08, 0x0A, CAP_EN);//60MHz > pixel clock >=41MHz
            vIO32WriteFldAlign(VGACFG2, 0x03, FLD_RG_MUXCAP_EN);
            vIO32WriteFldAlign(SYNCFG3, 0x00, FLD_RG_INPUT_CAP_5PF);
            break;
            
        case 3: 
            vIO32WriteFldAlign(VFE_08, 0x05, CAP_EN);// 90MHz > pixel clock >60MHz
            vIO32WriteFldAlign(VGACFG2, 0x03, FLD_RG_MUXCAP_EN);
            vIO32WriteFldAlign(SYNCFG3, 0x00, FLD_RG_INPUT_CAP_5PF);
            break;
            
        case 4: 
            vIO32WriteFldAlign(VFE_08, 0x02, CAP_EN);// 130MHz > pixel clock >90MHz
            vIO32WriteFldAlign(VGACFG2, 0x03, FLD_RG_MUXCAP_EN);
            vIO32WriteFldAlign(SYNCFG3, 0x00, FLD_RG_INPUT_CAP_5PF);
            break;
        case 5: 
            vIO32WriteFldAlign(VFE_08, 0x00, CAP_EN);//pixel clock >130MHz
