...
static const REGTBL_T TD_NAVI_INIT[] =
{
    ...
    #if defined(TDNAVI_DET_WA1_TOO_FEW)
    {TDNVAI_01, 0x4004DA00, 0xFFFFFFFE},
    #else
...
void vDrvTdNaviSetPicSize(void)
{
	...
	vIO32WriteFldAlign(TDNVAI_03, u2RestX, REST_X);
	vIO32WriteFldAlign(TDNVAI_03, u2RestY, REST_Y);
...
static void vDrvTdNaviDet(void)
{
    ...
    //control the g_u1TdNaviAdaptOnOff when open the flow
    if(IO32ReadFldAlign(TDNAVI_FW_00, TDNAVI_DEBUG) && g_u1TdNaviDetCurr == TD_NAVI_DETECT_2D)
    {
         g_u1TdNaviAdaptOnOff = 0;
    }
    else
    {
         g_u1TdNaviAdaptOnOff = 1 ;
    }
    ...
    else //(TD_NAVI_DETECT_2D == g_u1TdNaviDetCurr)
	{
		...
		// Ink Black
		vIO32WriteFldAlign(TDNVAI_04, 0, FW_DBG_INK_Y);
		vIO32WriteFldAlign(TDNVAI_04, 128, FW_DBG_INK_C);		
...
static UINT8 u1DrvTdNaviGetFwFrameDetType(void)
{
    ...
    #if defined(TDNAVI_DET_WA1_TOO_FEW)
    UINT8 u1StaSbsVldBlk1 = vDrvTdNaviGetSbsVldBlk();
	UINT8 u1StaTabVldBlk1 = vDrvTdNaviGetTabVldBlk();
    UINT8 VALID_BLK_THR_WA1 = 10;

    if(u1StaSbsVldBlk1 < VALID_BLK_THR_WA1 || u1StaTabVldBlk1 < VALID_BLK_THR_WA1 )
    {
...
static void vDrvTdNaviGetAdaptSetting(void)
{
    ...
    {
        g_u1In3DSimilarityRatio  = 0xDA;   //0xF8; for remy  248  setting difficult to 3D
        g_u1In3DSimilarThr       = 0x5;
        
        g_u1Out3DSimilarityRatio = 0xA0;   //for remy  160  setting will 3D->2D
        g_u1Out3DSimilarThr      = 0xA;

        vIO32WriteFldAlign(TDNVAI_01, 0x4, AMBIG_THR);//0x2 for remy  0x2 setting difficult to 3D
...
UINT8 vDrvTdNaviGetCurrDetType(void)
{
    ...
    {
        LOG(0,"[NAVI_RST] CURRENT RST TYPE IS %d\n",g_u1TdNaviDetRst);
    }
...
void vDrvTdNaviMainloop(void)
{
    ...
    if(u1IsInited)
    {
        ...
        if((E_TDNAVI_DUMPING==u1State)
            &&(x_sema_lock(_hTDNAVIDumpLock , X_SEMA_OPTION_NOWAIT)==SV_ON))
        {
