...
#if 0
#define WAIT_FOR_STA_RDY() u4Dummy = RegReadFldAlign(TDNVAI_FPGA_01,TDNVAI_FPGA_01_READ_POS);\
    u4Dummy = RegReadFldAlign(TDNVAI_FPGA_01,TDNVAI_FPGA_01_READ_POS);\
    u4Dummy = RegReadFldAlign(TDNVAI_FPGA_01,TDNVAI_FPGA_01_READ_POS);\
    u4Dummy = RegReadFldAlign(TDNVAI_FPGA_01,TDNVAI_FPGA_01_READ_POS);\
    u4Dummy = RegReadFldAlign(TDNVAI_FPGA_01,TDNVAI_FPGA_01_READ_POS);\
    u4Dummy = RegReadFldAlign(TDNVAI_FPGA_01,TDNVAI_FPGA_01_READ_POS);\
...
void vDrvTDTVTDNAVIStressTest(UINT32 count)
{
    ...
    UINT32 u4Compare = 0;
    HAL_RAW_TIME_T u4RawTime;
    UINT32 u4ClkTmp;
    UINT32 u4ClkMin = 0xFFFFFFFF;
    ...
    while(count--)
    {
        vRegWriteFldAlign(TDNVAI_FPGA_01,1,TDNVAI_FPGA_01_READ_EN);
        x_thread_delay(16);
        ...
        for(u4Index = 0; u4Index < 480; u4Index++)
        {
            ...
            HAL_GetRawTime(&u4RawTime); 
            u4ClkTmp = u4RawTime.u4Cycles;
            WAIT_FOR_STA_RDY();
            HAL_GetRawTime(&u4RawTime); 
            u4ClkTmp = u4RawTime.u4Cycles - u4ClkTmp;
            u4ClkMin = MIN(u4ClkMin,u4ClkTmp);
                
            if(u4Compare == 0)
            {
                a4BlkPixelDiff[u4Index] = RegReadFldAlign(TDNVAI_FPGA_02,TDNVAI_FPGA_02_BLK_PIXEL_DIFF);
            ...
            {
                u4TempPixel = RegReadFldAlign(TDNVAI_FPGA_02,TDNVAI_FPGA_02_BLK_PIXEL_DIFF);
        ...
        for(u4Index = 0; u4Index < 480; u4Index++)
        {
            ...
            if(u4Compare == 0)
            {
                a4BlkYValue[u4Index] = RegReadFldAlign(TDNVAI_FPGA_02,TDNVAI_FPGA_02_BLK_Y);
            ...
            {                    
                u4TempYValue = RegReadFldAlign(TDNVAI_FPGA_02,TDNVAI_FPGA_02_BLK_Y);
        ...
        vRegWriteFldAlign(TDNVAI_FPGA_01,0,TDNVAI_FPGA_01_READ_EN);             
        x_thread_delay(16);
        u4Compare=1;
    }
