    ...
    else
    {
        ...
        if (_u1Count == 0)
        {
                ...
                (_rNmPrm.rNrErfb.u1SysSta < _rNmPrm.rNrErfb.u1StaMax))
            {
                if (u4ErfbMode == 0)
                {
                    _rNmPrm.rNrErfb.u1SysSta = MIN((_rNmPrm.rNrErfb.u1SysSta + _rNmPrm.rNrErfb.u1IncOfst), _rNmPrm.rNrErfb.u1StaMax);
                    _rNmPrm.rNrErfb.i2DiffAccm = 0;
                }
                else
                {
            ...
            else if ((_rNmPrm.rNrErfb.i2DiffAccm < _rNmPrm.rNrErfb.i2DecTh) && _rNmPrm.rNrErfb.u1SysSta > 0)
            {
                if (u4ErfbMode == 0)
                {
                    _rNmPrm.rNrErfb.u1SysSta = MAX(_rNmPrm.rNrErfb.u1SysSta, _rNmPrm.rNrErfb.u1IncOfst) - _rNmPrm.rNrErfb.u1IncOfst;
                    _rNmPrm.rNrErfb.i2DiffAccm = 0;
                }
                else
                {
...
static UINT8 _u1DrvMCNRMoFbckFact(UINT32 u4MoPixCnt)
{
	...
	if (u4MoPixCnt >= u4Dft)
		...
		u1MBlend = LERP_F_C(0, 0, u4Max, 8, u4MoPixCnt);
...
static void _vDrvMCNRCleanDiffLvl(const NR_NM_STA_T* prNmSta)
{
...
//        u4NoiseLvl -= 4;
    ...
    if (prNmSta->u1GmvValid && (prNmSta->u1GmvX == 0) && (prNmSta->u1GmvY == 0)
...
static void _vDrvMCNRSADAnalysis (const NR_NM_STA_T* prNmSta)
{
    ...
    if (_rNrQ1B[E_NR_Q1B_CENTROID].u4Idx == 0)
    {
        ...
        _rNmAna.u1MoFbckBlend = _u1DrvMCNRMoFbckFact(GET_Q_AVG(_rNrQ2B[E_NR_Q2B_MOPIX])) + _rNmPrm.u1MoFbkOfst;
    ...
    u1NoiseLevel = (_rNmPrm.u4Cfg & NM_CFG(NM_CFG_UPDATE_NL_SC)) ? _u1DrvNRGetNLScnChgRaw()
        /*_u1DrvNRGetNLScnChg(prNmSta->u1NLScnChgCnt)*/ : MIN(_u1DrvNRGetNLForQ(), 60);
    ...
    {
    case 1:
        u1NoiseLevel = MAX(_u4EstTVDNL(), u1NoiseLevel);
        break;
    case 2:
