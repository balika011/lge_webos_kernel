...
//    vIO32WriteFldAlign(REG_CVBS_CFG7, 3, RG_CVBSADC_CHSEL_2);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 2, RG_50U_CLAMP);//
...
void vDrvSetTVADC(UINT8 vsrc)
{
	...
	vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_CVBSPGA_CHAB_DIS);//enable CHB manual gain 
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_CVBSADC2_PWD_33);//
		...
		case VSS_SV:
			...
			vTvd3dATDInit(SV_OFF);
			vDrvCvbsVfePWON();
			vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_CVBSPGA_CHAB_DIS);//Disable CHB manual gain
...
vDrvVoDDDSRST(1);
    ...
    switch (bVDOINPUT)
    {
        ...
        case P_FB1:   //SCART1
            ...
            vIO32WriteFldMulti(PDWNC_VGACFG4,P_Fld(1,FLD_RG_SYNC2_VTH)|P_Fld(0,FLD_RG_SYNC2_VTL)); //FB 
            bInputType = 2;
        break;
...
void vCust_G_Gain_Update(UINT8 bGain)
{
    ...
    if(bType!=P_FA && bType<4)
    {
        ...
        if(_bEEPROM_ready==1)
        {
               // bTmpHighByte = (bGain>>8) & 0xff;   //if uint16
               // bTmpLowByte =  bGain& 0xff;
				bTmpHighByte = bGain & 0xff;   
                bTmpLowByte =  0;
        if((bType==INT_SCART) && (fgIsCLKLock()))
...
void vScartVfeConnect(void)
{
 ...
 //   vIO32WriteFldMulti(REG_CVBS_CFG1, /*mark for V2: P_Fld(1,RG_MIX_60U_EN) |P_Fld(0,RG_MIX_SHORT)|*/P_Fld(2,RG_MIXVOCM_SEL));//
    ...
    vIO32WriteFldAlign(PDWNC_VGACFG1, 0   , FLD_RG_VSYNC_EN); 
...
PRIVATE UINT8 code  MonSlicerSetting[] ={0xAA, 0x43, 0xAA, 0xAA, 0x65}; // {0x9c, 0x14, 0x9C, 0x9C, 0x36};//HL
#endif
