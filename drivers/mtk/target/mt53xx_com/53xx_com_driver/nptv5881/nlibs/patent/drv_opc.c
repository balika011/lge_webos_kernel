...
#else
#define OPC_TOTAL_DIFF_THD   80000
#endif
...
UINT8 bDrvOPCFindMaxContent(const UINT16 *u2aNormHist)
{
	...
	#else
	u4HistCount = ((u2HEnd - u2HStart) * (u2VEnd - u2VStart)) >> 5 ;

	u4MaxThd = u4HistCount * u2MaxThdRatio / 1000;

    // todo : adaptive max threshold here
	u4HistCount = MAX(u4HistCount, 1);
	#endif

    for(bi = RGB_MAX_HIST_NUM - 1; bi > 0; bi--)
    {
    	#ifdef CUST_LCCT_DRV
		if(u4HistCount < u2aNormHist[bi])
        break;
		#endif
	...
	#else
		u2MaxContent = MIN(bi<<3, 255); 
    
	if(bi)
	{
		if((u2aNormHist[bi]+u2aNormHist[bi-1]) > u4MaxThd)
		{
			u2MaxContent = bi*8 - ((u2aNormHist[bi-1]*17+u2aNormHist[bi])/(u2aNormHist[bi]+u2aNormHist[bi-1])/2);
			u2MaxContent = MIN(u2MaxContent, 255);
		}
	}
    
    vIO32WriteFldAlign(OPC_01,  u2MaxContent, OPC_MAX_CONTENT);   
...
#ifdef CUST_LCCT_DRV
void vDrvBrightPlusSetOPCCurve(UINT8 u1P1, UINT8 u1P2, UINT8 u1G1, UINT8 u1G2, UINT8 u1GSLOPE)
{
    // write opc curve
	vIO32WriteFldMulti(DBL_01, 
	                    P_Fld(u1P1, DBL_P1) |
	                    ...
	                    P_Fld(u1G1, DBL_G1) |
	                    P_Fld(u1G2, DBL_G2));    
	vIO32WriteFldAlign(DBL_00, u1GSLOPE, DBL_G_SLOPE);      
}
#endif
...
UINT8 vDrvCalculateBPGain(UINT8 bMaxContent)    
{
	...
	u1LowBound = (u1HighBound*u1G2*2/u1G1 + 1) >> 1;
