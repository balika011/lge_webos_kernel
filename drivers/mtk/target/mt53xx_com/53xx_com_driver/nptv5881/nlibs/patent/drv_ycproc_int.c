...
#if 0
    // enable  ink  set
    vIO32WriteFldAlign(DBG_CFG, 1, INK_EN);
    vIO32WriteFldAlign(DBG_CFG, 0, Y_INK_MODE);
    vIO32WriteFldAlign(DBG_CFG, 0, CB_INK_MODE);
    vIO32WriteFldAlign(DBG_CFG, 0, CR_INK_MODE);

    // ink color
    vIO32WriteFldMulti(INK_DATA_MAIN,
    				P_Fld(0x200, INK_DATA_Y) |
    				P_Fld(0, INK_DATA_CB));
    vIO32WriteFldAlign(INK_DATA_MAIN_CR, 0, INK_DATA_CR);

    x_thread_delay(20);

    vIO32WriteFldAlign(DBG_CFG, 0, INK_EN);
#endif

    //ink position
    vIO32WriteFldMulti(POS_MAIN, P_Fld(u2PosX, POS_X)|P_Fld(u2PosY, POS_Y));
