...
static void _PdwncHwIsr(UINT16 u2VctId)
{
	...
	u4ValGpio = u4IO32Read4B(PDWNC_EXT_INTSTA_0) & u4IO32Read4B(PDWNC_ARM_INTEN_0);
	if(u4ValGpio)
    for (i = 0 ; i < 0x20; i++)
    {
        if (u4ValGpio & _PINT(i))
        {
			...
			_PdwncGpioIsr(i);
			//isr function will do clear status.
			//vIO32Write4B(PDWNC_EXTINTCLR, _PINT(i)); 
        }
    }
	u4ValGpio = u4IO32Read4B(PDWNC_EXT_INTSTA_1) & u4IO32Read4B(PDWNC_ARM_INTEN_1);
	if(u4ValGpio)
    for (i = 0 ; i < (TOTAL_PDWNC_GPIO_INT_NUM - 0x20); i++)
    {
        if (u4ValGpio & _PINT(i))
        {
			//gpio interrupt always call this function
...
void PDWNC_WatchDogPreInit(void)
{
	...
	for(i=0; i<3; i++)
	{
		...
		{
			LOG(0,"Reg WDT%d interrupt callback fail..\n", i);
			return;
		}
	}
	LOG(0,"Reg WDT interrupt callback successful..\n");
}
#endif
//-----------------------------------------------------------------------------
...
/*static*/ void _PdwncSetupVGA(UINT32 u4PowerDownEn)
{
	...
	if((u4PowerDownEn & WAK_VGA) == 0)  
    {
		...
		vIO32WriteFldAlign(PDWNC_VGACFG1, 0x0, FLD_RG_FB_AB_EN);
...
#ifndef CC_MTK_LOADER
static void _PdwncSetupRTC(UINT32* pu4PowerDownEn)
{
    UINT32 u4RtcHw;
 ...
 * 
 *  @param u4Pdstat  register 0/1.
 *  @retval   u4Status0/u4Status1.		
 */
//-----------------------------------------------------------------------------
UINT32 _PDWNC_ReadWakeupStatus(UINT32 u4Pdstat)
{
    if(u4Pdstat == 1)
	{
		return u4IO32Read4B(PDWNC_PDSTAT1);
	}else{
		return u4IO32Read4B(PDWNC_PDSTAT0);
	}
}
...
{
    return (u4IO32Read4B(PDWNC_WDTCTL) & FLD_WDTALL_RST);
}
...
void _PDWNC_SetupEthernetWakeup(UINT32 *pu4PowerDownEn)
{
...
#endif
	{
		...
		PDWNC_SetupGpioWakeup(u4Gpio,TRUE,u4Pol,FALSE);
	}
}
//-----------------------------------------------------------------------------
// Public functions
...
UINT32 PDWNC_ReadWakeupReason(void)
{
    ...
    if((_fgSuspendWakeupReasonUp == TRUE) && (u4Reason == PDWNC_WAKE_UP_REASON_AC_POWER))
	{
		u4Reason = PDWNC_WAKE_UP_REASON_UNKNOWN;
	}
	fgHasRead = TRUE;
...
#ifndef CC_MTK_LOADER
#ifdef CC_PDWNC_REBOOT_NOTIFY
static HANDLE_T _StandbyLock = NULL_HANDLE;
void wait_for_unlock_standby (void)
{
    INT32 i4Ret;
    i4Ret = x_sema_lock_timeout(_StandbyLock, 10000);
    if (i4Ret != OSR_OK)
    {
        Printf("wait_for_unlock_standby timeout");
    }
}
void unlock_standby(void)
{
    x_sema_unlock(_StandbyLock);
}

extern unsigned int isAndroidRecoveryMode(void);
//static int pdwnc_reboot_notify(struct notifier_block* nb, unsigned long code, void *unused) // ng
//static int pdwnc_reboot_notify(void *unused) // ok
//static int pdwnc_reboot_notify(unsigned long code, void *unused) // ng //cannot use the keyword: code
//static int pdwnc_reboot_notify(UINT32 event, void *unused) // ok
static int pdwnc_reboot_notify(struct notifier_block* nb, unsigned long event, void *unused)
{
    static int reboot_event_handled = 0;
    MTPDWNC_STANDBY_NOTIFY_T t_cb_info;
    char* cmd = unused;

    printk(KERN_EMERG "pdwnc_reboot_notify-0: %s\n", cmd);
    if (!reboot_event_handled)
    {
        /* only do this once */
        reboot_event_handled = 1;
        t_cb_info.u4Block = TRUE;
        if (t_cb_info.u4Block)
        {
#if 0
            if (cmd && *cmd != '\0')
            {
                printk(KERN_EMERG "pdwnc_reboot_notify-1: %s\n", cmd);
                if (!x_strcmp(cmd, "recovery"))
                {
                    return NOTIFY_DONE;
                }
            }
#endif
            if (isAndroidRecoveryMode())
            {
                printk(KERN_EMERG "pdwnc_reboot_notify: isAndroidRecoveryMode\n");
                return NOTIFY_DONE;
            }
            _CB_PutEvent(CB_DRV_PDWNC_STANDBY_NFY, sizeof(MTPDWNC_STANDBY_NOTIFY_T), &t_cb_info);
            printk(KERN_EMERG "pdwnc_reboot_notify: wait-1\n");
            wait_for_unlock_standby();
            printk(KERN_EMERG "pdwnc_reboot_notify: wait-2\n");
        }
    }
    return NOTIFY_DONE;
}

static struct notifier_block pdwnc_notifier = {
    .notifier_call  = pdwnc_reboot_notify,
    .next           = NULL,
    .priority       = 0,
};
#endif
...
void mtk_pdwnc_entertandby_func(void)
{
	unsigned int _powerdown_en;
	unsigned int _powerdown_event;

	_powerdown_en = WAK_IRRX;
	_powerdown_event = 0;
	UNUSED(PDWNC_EnterPowerDown(_powerdown_en,_powerdown_event));
}
#endif
//-----------------------------------------------------------------------------
...
INT32 PDWNC_Init(void)
{
...
#else
	vIO32Write4B(PDWNC_WAKEN,0x00000000); //just clean wakeup enable bit, for MT5890(Oryx),wangliang.wang,20140830.
	vIO32Write4B(PDWNC_WAKEN1,0x00000000);
    u4Val = PDWNC_ReadWakeupReason();
    if(u4Val == PDWNC_WAKE_UP_REASON_AC_POWER)
    {   
        UINT8 u1Year = 0xff;
    ...
    if (_StandbyLock == NULL_HANDLE)
    {
	VERIFY(x_sema_create(&_StandbyLock, X_SEMA_TYPE_BINARY, 
		    X_SEMA_STATE_LOCK) == OSR_OK);
    }
    printk(KERN_EMERG "register_reboot_notifier: pdwnc_notifier\n");
    register_reboot_notifier(&pdwnc_notifier);
#endif

	//Registration mtk power off function.
	#if !defined(CC_DRIVER_BUILD)
	pm_power_off = mtk_pdwnc_entertandby_func;
	#endif 
#endif /* CC_MTK_LOADER */
...
void PDWNC_Reboot()
{
	...
	vIO32WriteFldAlign(PDWNC_WDTCTL,0xFF,FLD_WDT0E);//	use watchdog 0 trigger wakeup.
    while (1) { }
}
...
UINT32 PDWNC_IsWakeupByPowerButton(void)
{
    ...
    _PDWNC_ReadServoADCWakeupStatus(&u4Val);
	...
	//u4Val = (_PDWNC_ReadWakeupStatus(0) & ~(0xff << 20)) | u4Servo;
    ...
    if (i == 0)
    {
            ...
            (prSingleGpioKeyList[i].u4Key != DKBIT_NONE))
        {
            if (prSingleGpioKeyList[i].u4Key == DKBIT_POWER)
            {
				...
				if(!PDWNC_GetWakeupGpioNum(&u4Val))
				{
					if (prSingleGpioKeyList[i].u4Gpio == u4Val)
	                {
	                    return 1;
	                }else{
	                    return 0;
...
#ifdef SYS_ANDROID_TV_TIF_SUPPORT
#ifndef CC_MTK_LOADER
	rCmd.au1Data[1] = (drvMhlStandbyNeedCharge())?1:0;
#endif
#endif
...
#endif
#ifndef PDWNC_RESUME_METHOD

#ifdef CC_ANDROID_TWO_WORLDS
#define PDWNC_RESUME_METHOD 0x1    //By Application control
#else
#define PDWNC_RESUME_METHOD 0x0
#endif

#endif
UINT32 u4PDWNC_TestWakelock = PDWNC_RESUME_METHOD;
static void _PDWNC_AWAKE_PMS(void)
{
    IRRC_WAKKEY_T pWakeupKey;

    if (u4PDWNC_TestWakelock == 0)
    {
        IRRX_SendMtkIr(DRVCUST_InitGet(ePdwncResumeHandleKey)); // trigger a user-activity via a fixed-key
    }
    else
    {
        if (u4PDWNC_TestWakelock & 0x02)
        {
            UINT32 u4Reason = PDWNC_ReadWakeupReason();
            if (u4Reason != PDWNC_WAKE_UP_REASON_RTC)
            {
                if (u4Reason == PDWNC_WAKE_UP_REASON_IRRC)
                {
                    IRRX_QueryWakeupKey(&pWakeupKey);
                    IRRX_SendMtkIr(pWakeupKey.u4WakeupKey);
                    printk("(yjdbg) IRRX_SendMtkIr.1: 0x%x\n", pWakeupKey.u4WakeupKey);
                }
                else
                {
                    IRRX_SendMtkIr(DRVCUST_InitGet(ePdwncResumeHandleKey)); // trigger a user-activity via a fixed-key
                    printk("(yjdbg) IRRX_SendMtkIr.2: 0x%x\n", BTN_POWER);
                }
            }
        }
    }
...
static void _PDWNC_TestOnOFF(void *pvArg)
{
...
void PDWNC_Resume(void)
{
    ...
    {
	if (u4PDWNC_TestWakelock & 0x01)
	{
	    t_cb_info.u4Reason = PDWNC_ReadWakeupReason();
	    t_cb_info.u4Other = 0xA5A5A5A5;
	    _CB_PutEvent(CB_DRV_PDWNC_RESUME_NFY, sizeof(MTPDWNC_RESUME_REASON_T), &t_cb_info);
	}
	if (u4PDWNC_TestWakelock & 0x02)
	{
	    _PDWNC_AWAKE_PMS();
...
UINT32 PDWNC_RequestSuspendEvent(UINT32 u4SuspendEn, UINT32 u4SuspendEvent)
{
	...
	vIO32WriteFldAlign(PDWNC_PDSTCLR, 0x1, FLD_STCLR);
	//clear watchdog flag
    vIO32Write4B(PDWNC_WDTCTL,u4IO32Read4B(PDWNC_WDTCTL));
	_fgSuspendWakeupReasonUp = TRUE;
#endif
	return 0;
}
...
UINT32 PDWNC_EnterPowerDown(UINT32 u4PowerDownEn, UINT32 u4PowerDownEvent)
{
    ...
    vIO32Write4B(PDWNC_WAKEN, (u4IO32Read4B(PDWNC_WAKEN) | u4PowerDownEn));
	Printf("PDWNC_EnterPowerDown(%x,%x) \n",u4IO32Read4B(PDWNC_WAKEN),u4PowerDownEvent);
