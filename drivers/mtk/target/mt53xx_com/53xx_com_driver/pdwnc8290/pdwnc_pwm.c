...
#if defined(CC_SUPPORT_PDWNC_PWM)

#define PDWNC_PD_PWMC0(X) ((X==0)?PDWNC_PD_PWM0C0:((X==1)?PDWNC_PD_PWM1C0:((X==2)?PDWNC_PD_PWM2C0:PDWNC_PD_PWM3C0)))
#define PDWNC_PD_PWMC1(X) ((X==0)?PDWNC_PD_PWM0C1:((X==1)?PDWNC_PD_PWM1C1:((X==2)?PDWNC_PD_PWM2C1:PDWNC_PD_PWM3C1)))


void PDWNC_PwmSetResolution(UINT32 u4Path, UINT32 u4Arg)
{
    vIO32WriteFldAlign(PDWNC_PD_PWMC0(u4Path) ,u4Arg,FLD_PWM0RSN);
}

void PDWNC_PwmSetHightPeriod(UINT32 u4Path, UINT32 u4Arg)
{
    vIO32WriteFldAlign(PDWNC_PD_PWMC0(u4Path) ,u4Arg,FLD_PWM0H);
}

void PDWNC_PwmSetRepeatCycle(UINT32 u4Path, UINT32 u4Arg)
{
    vIO32WriteFldAlign(PDWNC_PD_PWMC0(u4Path) ,u4Arg,FLD_PWM0CYC);
}

void PDWNC_PwmSetAutoEnable(UINT32 u4Path, UINT32 u4Arg)
{
    vIO32WriteFldAlign(PDWNC_PD_PWMC0(u4Path) ,u4Arg,FLD_PWM0AUTO);
}

void PDWNC_PwmSetPwmEnable(UINT32 u4Path, UINT32 u4Arg)
{
    vIO32WriteFldAlign(PDWNC_PD_PWMC0(u4Path) ,u4Arg,FLD_PWM0E);
}

void PDWNC_PwmSetPwmPrescaler(UINT32 u4Path, UINT32 u4Arg)
{
    vIO32WriteFldAlign(PDWNC_PD_PWMC1(u4Path) ,u4Arg,FLD_PWM0P);
}
 ...
 *  @param u4Frequence  pwm frequence, (1hz)
 *  @param u4DutyCycle  pdwm u4DutyCycle, it will be set as u4DutyCycle% high (0~100)
 */
//-----------------------------------------------------------------------------
UINT32 PDWNC_PwmSetPwm(UINT32 u4Path, UINT32 u4Enable,UINT32 u4Frequence, UINT32 u4DutyCycle)
{
	...
	PDWNC_PwmSetResolution(u4Path,255); //fix resolution as 99
	...
	else
	{
		if(IS_XTAL_27MHZ())
		{
			u4Factor = 27000000/(256 * u4Frequence) - 1;
		}
		else if(IS_XTAL_24MHZ())
		{
			u4Factor = 24000000/(256 * u4Frequence) - 1;
		}
		else
		{
			//printf("Set pwm get system clock error \n");
			return FALSE;
		}
	}
