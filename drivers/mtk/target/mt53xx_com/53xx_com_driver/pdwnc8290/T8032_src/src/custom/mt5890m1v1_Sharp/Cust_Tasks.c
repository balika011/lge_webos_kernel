...
#if SHARP_AC_DET_SUPPORT
#define SHARP_AC_DET_GPIO  (OPCTRL0)
#define SHARP_MUTE_A_ALL_GPIO (OPCTRL11)
#define SHARP_STB_GPIO		(OPCTRL14)
UINT16 u1AcDetFilter;
UINT8  u1Task3Status;
#define  TASK3_RUN 	 	1
#define  TASK3_ERROR 	2
#define  TASK3_REBOOT   3
#define  TASK3_END	    4

void vWatchdogReboot()
{
    vIO32Write4B(PDWNC_WDTRST0,0x0fffffff);	
    vIO32Write4B(PDWNC_WDT0,0x7fff0000);
	vIO32Write4B(PDWNC_WDTCTL, (u4IO32Read4B(PDWNC_WDTCTL) & 0x00ffffff) | 0xFF);//	use watchdog 0 trigger wakeup.
    while (1) {}
}
void vTask3Init_SharpAcDet(void)
{
	vSetTimer1(SHARP_AC_DET_TIMER_ID, 100);//delay 100ms.
	vSetTimer(SHARP_AC_DET_RESET_TIMER_ID, 0);
	u1AcDetFilter = 0;
	u1Task3Status = TASK3_RUN;
}
void vTask3_SharpAcDet_state_machine(void)
{
	if(u1AcDetFilter >= 50 && (u1Task3Status == TASK3_ERROR))
	{
		u1AcDetFilter = 0;
		u1Task3Status = TASK3_END;
		//LogS("Task3 status : TASK2_END.");
		vWatchdogReboot();
	}
	if((u1AcDetFilter >= 10) && (u1Task3Status == TASK3_RUN))
	{
		GPIO_SetOut(SHARP_MUTE_A_ALL_GPIO,1); //mute.
		GPIO_SetOut(SHARP_STB_GPIO,0);//when detect the momentary power failure,uP set STB pin to low.
		vEnterPowerStandby();
		u1Task3Status = TASK3_ERROR;
		u1AcDetFilter = 0;
		//LogS("Task3 status : TASK3_ERROR .");
	}
	if(fgIsTimer1Up(SHARP_AC_DET_TIMER_ID) && (u1Task3Status == TASK3_RUN))
	{
		vSetTimer1(SHARP_AC_DET_TIMER_ID, 2);
		
		if(!GPIO_GetIn(SHARP_AC_DET_GPIO))
		{
			u1AcDetFilter++;
			//LogS("AC_DET is LOW .");
		}
	...
	if(fgIsTimerUp(SHARP_AC_DET_RESET_TIMER_ID) && (u1Task3Status == TASK3_ERROR))
	{
		vSetTimer(SHARP_AC_DET_RESET_TIMER_ID, 5);
