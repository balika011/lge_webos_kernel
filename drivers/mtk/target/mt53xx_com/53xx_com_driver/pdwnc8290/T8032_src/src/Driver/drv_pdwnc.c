...
void vWakeUpSystem(UINT8 u1Reason, UINT8 u1SubReason) //reentrant
{
        ...
        fgCecStandbyToOn = 1; 
		 ...
		 //? FLD_HDMI_0_HPD_CBUS_PUPD undedined //vIO32WriteFldAlign(PDWNC_PADCFG14,0,FLD_HDMI_0_HPD_CBUS_PUPD);//280E8 bit 9 PUPD ---  SET to 1
...
{
    vIO32Write4B(PDWNC_ETHERNET_INT_STATUS, u4IO32Read4B(PDWNC_ETHERNET_INT_STATUS));
}
#endif
/*
#if defined SIR_TASK
...
void vPDWNC_ISR(void) small interrupt EX0INT  
{
    ...
    {  
        UINT32 value = u4IO32Read4B(PDWNC_ETHERNET_INT_STATUS);
        _vEthernetIntClr();
        if (value & INTERRUPT_ETHERNET_WOL_INT_FLAG)
        {
...
#if GPIO_INTERRUPT_ENABLE
	u4PDWNC_IntStatus2 = u4IO32Read4B(PDWNC_EXT_INTSTA_0) & u4IO32Read4B(PDWNC_T8032_EXT_INTEN_0);
	for (i = 0 ; i < 0x1F; i++)
    {
        if (u4PDWNC_IntStatus2 & ((UINT32)0x1 << i))
        {
			//gpio interrupt always call this function
			vCustGpioIsr(i);
        }
    }
	u4PDWNC_IntStatus2 = u4IO32Read4B(PDWNC_EXT_INTSTA_1) & u4IO32Read4B(PDWNC_T8032_EXT_INTEN_1);
	for (i = 0 ; i < TOTAL_PDWNC_GPIO_INT_NUM - 0x1F; i++)
    {
        if (u4PDWNC_IntStatus2 & ((UINT32)0x1 << i))
        {
			//gpio interrupt always call this function
...
{
    vExInit();
  	u4IcVer = u4IO32Read4B(PDWNC_RESRV0);
