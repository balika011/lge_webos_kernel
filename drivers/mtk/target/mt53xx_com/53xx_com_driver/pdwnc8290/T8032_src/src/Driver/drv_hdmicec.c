...
static void _Cec_CustRxFrame(CEC_FRAME_DESCRIPTION *frame)
{
    ...
    case OPCODE_USER_CONTROL_PRESSED:
        if (frame->blocks.header.destination != CEC_LOG_BROADCAST)
        {
            ...
            else if(frame->blocks.operand[0] == 0x6C)
            {
				//do nothing.
            }
			else
...
#ifdef PDWN_ANDROID_WW_CEC_SUPPORT //for sakura request.
		else
		{
			rCECTxFrame.size = 4;
			rCECTxFrame.sendidx = 0;
    ...
    case OPCODE_ACTIVE_SOURCE:
        ...
        break;
#ifndef PDWN_ANDROID_WW_CEC_SUPPORT //sakura standby request, do nothing.  
    case OPCODE_ROUTING_CHANGE:
    case OPCODE_ROUTING_INFORMATION:
        if ((frame->blocks.header.destination == CEC_LOG_BROADCAST) && 
             (fgCecEnOneTouchPlay == 1)) //Buffer Only when enable One Touch Play
...
#ifndef PDWN_ANDROID_WW_CEC_SUPPORT // for sakura 
...
void CEC_Init(void)
{
    ...
    u1CecVersion = 0x05;    /*support HDMI version 1.4a/b*/
...
#elif defined (CC_MT5881) || defined(CC_MT8290)
...
#ifdef PDWN_ANDROID_WW_CEC_SUPPORT // Sakura
		vRegWriteFldAlign(TX_ARB, 0xC, BCNT_NEW_MSG);
#else
