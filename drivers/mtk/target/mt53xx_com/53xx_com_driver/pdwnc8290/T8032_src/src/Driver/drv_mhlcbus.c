...
UINT8 xdata fgCbus8032VbusCharge                          _at_ HDMI_BASE+0x1ED; //0xFA01;
#endif
...
static void vCbusExternal1KControl(UINT8 u1Ctrl) reentrant;
static void vCbusHdcpControl(UINT8 u1Ctrl) reentrant;
...
#if 0 //GPIO_SetOut() and GPIO_GetIn() have resolved "reentrant" problem, if you find any problem, please contact wangliang.wang .
...
static UINT8 _CbusStandbyChargeCtrl(void)
{
	return fgCbus8032VbusCharge?1:0;
}
static void _CbusEnableVbus(UINT8 enable)
{
#if ENABLE_TIF_MHL_VBUS_CTRL
	if(!_CbusStandbyChargeCtrl())
	{	
		enable = 0;
	}
#endif
	GPIO_SetOut(VBUS_CTRL_GPIO, enable?1:0);
}
...
void vCbus_HwInit(void)
{
    ...
    {
        u1BootUp = 1;
		vCbusExternal1KControl(PDWNC_CBUS_1K_MHL_MODE);
		vCbusHdcpControl(PDWNC_CBUS_1K_MHL_MODE);
    }
    else //cable disconnected
    {
        u1BootUp = 0;
		vCbusExternal1KControl(PDWNC_CBUS_1K_HDMI_MODE);
		vCbusHdcpControl(PDWNC_CBUS_1K_HDMI_MODE);
    }
    if (MGET_SYS_PRE_STANDBY_STATUS() != STBY_FROM_NORMAL_POWER)	  //From AC on, need to initialize CEC register
    {    
#if VBUS_CTRL_GPIO
...
{  
    ...
    if((u1BootupWaitFlg == 6)&&(fgIsTimer1Up(MHL_BOOTUP_TIMER_ID)))
    {
		...
		vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,1, FLD_ZCBUS_DISCOVER_EN);
		vIO32WriteFldAlign(PDWNC_CBUS_LINK_0C,1, FLD_CBUS_1K_CTRL);
...
void vCbusMainLoop(void)
{
	...
	switch (iMhlStateOld)
	{
		...
		case MHL_STATE_INIT:	
			...
			{
				vIO32WriteFldAlign(PDWNC_PADCFG17,1,FLD_CBUS_PUPD);//280E8 bit 9 PUPD ---  SET to 0, high z state
				vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,0, FLD_ZCBUS_DISCOVER_EN);
...
static void vCbusHdcpControl(UINT8 u1Ctrl) reentrant
{
#if defined(CC_MT8290)//MT5861 share the define with MT5890.
		if(IS_IC_5861() || (vGetIcVersion() >= IC_VER_5890_AC))//only 5861.
		{
			if(u1Ctrl)
			{
