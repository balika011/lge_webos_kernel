    ...
    UINT32 u4PoolSize, UINT32 u4YSize, UINT32 u4CSize, UINT8 u1AppMode, UINT32* pu4ExtFbNs)
{
    ...
    UINT32 u4ExtraFbNsStageFright = 0;
    FBM_POOL_T* prFbmPool = NULL;
    ...
    if(u1AppMode == FBM_FBG_APP_MHP)
    {
     ...
     //Add extra reference frame.
#if defined(CC_SUPPORT_TV_STAGEFRIGHT) && defined(CC_SHRINK_FBM_FOR_512MB)
    if ((u1AppMode == FBM_FBG_APP_OMX_DISP) && ((prFbmPool = FBM_GetPoolInfo((UINT8)FBM_POOL_TYPE_TV_STAGEFRIGHT)) != NULL))
    {
        u4ExtraFbNsStageFright = prFbmPool->u4Size/u4FbSize;
    }
#endif
    ...
    switch (u4VDecFmt)
    {
        ...
        case FBM_VDEC_MPEG2:
        {
            ...
            if (u4ExtraFbNsStageFright)
            {
                UINT32 u4TmpNs = ((u1AppMode == FBM_FBG_APP_OMX_DISP) ? 10 :6) - u4FbCount;
                u4ExtraFbNsStageFright = (u4ExtraFbNsStageFright > u4TmpNs) ? u4TmpNs : u4ExtraFbNsStageFright;
            }
        ...
        case FBM_VDEC_VC1:
        {
            ...
            if (u4ExtraFbNsStageFright)
            {
                UINT32 u4TmpNs = ((u1AppMode == FBM_FBG_APP_OMX_DISP) ? 10 :5) - u4FbCount;
                u4ExtraFbNsStageFright = (u4ExtraFbNsStageFright > u4TmpNs) ? u4TmpNs : u4ExtraFbNsStageFright;
            }
        }
        break;
    ...
    u4ExtraFbNs += u4ExtraFbNsStageFright;
                         ...
                         UINT32 u4HSize, UINT32 u4VSize, FBM_CREATE_FBG_PAR_T *prPar)
{
...
#if (defined(CC_FBM_TWO_FBP_SHARED_WITH_DFB) || defined(CC_VOMX_TV_COEXIST)) && defined(CC_FBM_FULL_3D_SUPPORT) || defined (FBM_4K1K_VIDEO_SUPPORT) || defined(CC_SUPPORT_FULL_3D_PHOTO)
    ...
    // Config color mode
    LOG(1, "Config FBG(%d): DMode(%d) FBNs(%d-%d %d-%d) Pool(0x%x + 0x%x) EmptyCnt(%d).\n", 
            u4FbgId, _arFbg[u4FbgId].eDynamicMode, _arFbg[u4FbgId].ucFbNsOldBase, _arFbg[u4FbgId].ucFbNsOld, _arFbg[u4FbgId].ucFbNsBase, _arFbg[u4FbgId].ucFbNs,
...
void FBM_ReleaseGroup(UCHAR ucFbgId)
{
    ...
    {
#if ((defined(CC_FBM_TWO_FBP_SHARED_WITH_DFB)  || defined(CC_VOMX_TV_COEXIST)) && defined(CC_FBM_FULL_3D_SUPPORT)) || defined(FBM_4K2K_JPG_SUPPORT) ||defined(FBM_4K2K_SUPPORT) ||defined(FBM_4K1K_VIDEO_SUPPORT) || defined(CC_SUPPORT_FULL_3D_PHOTO)
                         ...
                         UINT32 u4VSize, FBM_CREATE_FBG_PAR_T *prPar)
{
    ...
    //for not-standard timing such as 1440*900 to support much more reference.
    {
        ...
        if(_arFbg[ucFbgId].u4VDecFmt == FBM_VDEC_JPEG && ucFbgCm == FBM_CM_422 && prPar->u1AppMode != FBM_FBG_APP_MTIMAGE)
        {
            u4Addr = _arFbg[ucFbgId].u4FbMemoryPool;
#ifdef CC_FBM_3D_OSD
#if !defined(FBM_4K2K_JPG_SUPPORT) && !defined(FBM_4K2K_SUPPORT) && !defined(CC_SUPPORT_FULL_3D_PHOTO)
            ...
            LOG(3, "FBM CONFIG HV(%d, %d) Ns(%d) Mv(%d) Addr(%d) Size(%d)\n",
                u4HSize, u4VSize, u4FbNs, _arFbg[ucFbgId].ucMvBufNs, u4Addr, u4PoolSize);
        ...
        if(_arFbg[ucFbgId].ucFbgType == FBM_FBG_TYPE_SBSHD  || _arFbg[ucFbgId].ucFbgType == FBM_FBG_TYPE_TNBHD)
        {
            ...
            {
                if((u4FbIdx == _arFbg[ucFbgId].ucFbNs - 1) && !(_arFbg[ucFbgId].u4VDecFmt == FBM_VDEC_JPEG && ucFbgCm == FBM_CM_422))
                {
    ...
    //Handle extra reference frame, update FbNs & Fb partition here.
        ...
        #if defined(CC_SUPPORT_TV_STAGEFRIGHT) && defined(CC_SHRINK_FBM_FOR_512MB)
        if (u4ExtraFbNs && (prPar->u1AppMode == FBM_FBG_APP_OMX_DISP) && ((prFbmPool = FBM_GetPoolInfo((UINT8)FBM_POOL_TYPE_TV_STAGEFRIGHT)) != NULL))
        {
            UINT32 u4ExtraReferenceNs = 0;
            u4Addr = FBM_ALIGN_MASK(prFbmPool->u4Addr , FBM_FMG_Y_ALIGMENT);
            u4PoolSize = prFbmPool->u4Size;
            u4ExtraReferenceNs = FBM_MIN(u4PoolSize/(u4YSize + u4CSize), u4ExtraFbNs);
            ...
            LOG(2, "Extra %d FB from TV_STAGEFRIGHT.\n", u4ExtraReferenceNs);
        }
        #endif
