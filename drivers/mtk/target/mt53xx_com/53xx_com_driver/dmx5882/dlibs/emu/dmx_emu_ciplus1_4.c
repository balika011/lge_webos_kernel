...
#define SYNC_PATTERN 				(0x40414243)
...
typedef enum
{
    TEST_TYPE_REMUX = 0,            // remux only
    TEST_TYPE_REMUX_MM = 1,         // remux+1mm
    TEST_TYPE_REMUX_TIMESHIFT= 2    // remux+1timeshift
}CIPLUS_TEST_TYPE;
...
typedef struct
{
  ...
  UINT32 DDI_TS_TO_FRAMER_GOLDEN_TS_Start;
  UINT32 DDI_TS_TO_FRAMER_GOLDEN_TS_Offset;
  UINT32 DDI_TS_TO_FRAMER_GOLDEN_TS_Len;
}DDI_TS_TO_FRAMER_T;
...
{/*pes_start_addr,pes_offset_addr,pes_len, ts_start_addr, ts_offset_addr,ts_len*/
   {0x5200000,0x5200000,0, 0x5400000, 0x5400000, 0},
   {0x5600000,0x5600000,0, 0x5800000, 0x5800000, 0}
};
                                       ...
                                       UINT16 u2pidnum,DMX_PID_TYPE_T e_type)
{
	...
	if (!_DMX_SetPid(u1pidx, DMX_PID_FLAG_ALL, 
		 &rPid, FALSE))
	{
		return FALSE;
	}
	                            ...
	                            UINT32 u4TimeToNotify)
{
	...
	rGblPid.u4TimeToNotify = u4TimeToNotify;
    if (!_FVR_SetGBL(u1Index, FVR_GBL_FLAG_ALL, &rGblPid))
    {
        return FALSE;
    }
...
static BOOL _SetPVRGBLEx3(UINT8 u1Index, BOOL fgTimestamp,UINT8 remux_flag)
{
    ...
    rGblPid.u4BufStart = Record_Addr_T[u1Index].u4_RecordAddr;
    rGblPid.u4BufSize = Record_Addr_T[u1Index].u4_RecordSize;
    rGblPid.u4SyncPattern=SYNC_PATTERN;//0x40414243,for debug used.
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SetPVRGBLTimerNotify(UINT32 ui4_timer_notify)
{
    FVR_GBL_T rGblPid;
    ...
    if (!_FVR_SetGBL(0, FVR_GBL_FLAG_TIME_INFO, &rGblPid))
    {
        return FALSE;
    }
...
static BOOL _DMX_EMU_CustPvrPath(PVR_TYPE_T e_pvr_type,UINT8 u1_tsidx,BOOL fg_disable_filter)
{
    UINT32 u4Reg;
	UINT8 i=0x0;
	if (u1_tsidx >= DMX_FRAMER_COUNT)
	{
		Printf("[%s]Invalid tsidx(%d).\r\n", __FUNCTION__, u1_tsidx);
		return FALSE;
	}
	switch(e_pvr_type)
    {
      case TIMESHIFT_PVR:
		  ...
		  //for timeshift used
	  	  if(u1_tsidx == 2)
	  	  {
	  	     _DMX_PVRPlay_SetPort(1,DMX_PVRPLAY_PORT_FRAMER2_BYPASS);
	  	  }
		  else if (u1_tsidx == 4)
		  {
		     _DMX_PVRPlay_SetPort(1,DMX_PVRPLAY_PORT_DBM);
		  }
		  else
		  {
	  ...
	  case CI_REMUX_PVR:
	  	{
		  //for  cam-out ,the path is :pvr->ts_idx3->
	  	  _DMX_PVRPlay_SetPort(0, DMX_PVRPLAY_PORT_FRAMER3);
	  	}break;
	  default: 
	  	{
...
static void _DMX_EMU_TS_Parameter_Init(void)
{
  ...
  for(i=0;i<DDI_TS_TO_FRAMER_TOTAL_NUM;i++)
  {
	...
	DDI_TS_TO_FRAMER_GOLDEN_FILES[i].DDI_TS_TO_FRAMER_GOLDEN_TS_Offset =
	DDI_TS_TO_FRAMER_GOLDEN_FILES[i].DDI_TS_TO_FRAMER_GOLDEN_TS_Start;
  }
...
//-----------------------------------------------------------------------------
/** _CreateThread to update the pvr
 */
//-----------------------------------------------------------------------------
static VOID _FvrThread(VOID* pvArg)
{
...
static UINT32 _CreateTSPacket(INT8  u1pididx,DMX_TS_INPUT_SOURCE e_source_mode,UINT32 u4PesAddr, UINT32 u4PesSize)
{
    ...
    while (u4PesSize > 0)
    {
		...
		{
			if(PID_FRAMER_TABLE[u1pididx] == DDI_TS_TO_FRAMER0)
			{
				x_memcpy((VOID*)(DDI_TS_TO_FRAMER_GOLDEN_FILES[0].DDI_TS_TO_FRAMER_GOLDEN_TS_Offset),
					(VOID*)(_arTsGoldenFile[e_source_mode].TS_Addr_Offset-188), 188);
				DDI_TS_TO_FRAMER_GOLDEN_FILES[0].DDI_TS_TO_FRAMER_GOLDEN_TS_Offset += 188;
			}
			else if (PID_FRAMER_TABLE[u1pididx] == DDI_TS_TO_FRAMER1)
			{
				x_memcpy((VOID*)(DDI_TS_TO_FRAMER_GOLDEN_FILES[1].DDI_TS_TO_FRAMER_GOLDEN_TS_Offset),
					(VOID*)(_arTsGoldenFile[e_source_mode].TS_Addr_Offset-188), 188);
				DDI_TS_TO_FRAMER_GOLDEN_FILES[1].DDI_TS_TO_FRAMER_GOLDEN_TS_Offset += 188;
			}
		}
...
static BOOL _DmxEmuGenRemuxData(BOOL fgon,UINT32 pattern)
{
	  ...
	  UINT32 SrcSize = (_arTsGoldenFile[DMX_TS_FROM_DDI].TS_Addr_Offset- \
	   ...
	   /*enable DDI engine*/
	  if(!_DMX_EMU_DDI_Transfer(6,SrcAddr,SrcSize))
	  {
	  ...
	  /*pvr2*/
	   ...
	   if (TRUE)
	   {
   			UINT8* pRecAddr = NULL;
			UINT8* pu1Test = NULL;
			UINT32 u1SyncByte = 0xFF;
			UINT32 i = 0;
			UINT32 j = 0;
			UINT32 u4RecSize = 0;
			FVR_GBL_T rGbl;
			UINT32 u4Pos[4] = {0};
			_FVR_GetGBL(4, FVR_GBL_FLAG_BUFFER|FVR_GBL_FLAG_SYNC_MASK, &rGbl);
			u4RecSize = rGbl.u4Wp - rGbl.u4Rp;
			pRecAddr = (UINT8*)rGbl.u4Rp;

			if (u4RecSize == 0)
			{
				Printf("no data in remux buffer!\n");
				return FALSE;
			}
			else 
			{
				Printf("record data size of remux buffer is:0x%x\n", u4RecSize);
			}
			while (i<u4RecSize)
			{
				// skip timestamp
				...
				if (pRecAddr[i] == (SYNC_PATTERN&0xFF)
					&& (rGbl.u1SyncMask & 0x1))
				{
					pu1Test = (UINT8*)DDI_TS_TO_FRAMER_GOLDEN_FILES[0].DDI_TS_TO_FRAMER_GOLDEN_TS_Start;
					j = i+188;
					i++; u4Pos[0]++;
					for(;i<j; i++,u4Pos[0]++)
					{
						if (pRecAddr[i] != pu1Test[u4Pos[0]])
						{
						    Printf("Compare error: idx:%d, %x!=%x\n", i, pRecAddr[i], pu1Test[u4Pos[0]]);
        					return FALSE;
						}
					}
				}
				else if (pRecAddr[i] == ((SYNC_PATTERN>>8)&0xFF) 
					&& (rGbl.u1SyncMask & 0x2))
				{
					pu1Test = (UINT8*)DDI_TS_TO_FRAMER_GOLDEN_FILES[1].DDI_TS_TO_FRAMER_GOLDEN_TS_Start;
					j = i+188;
					i++; u4Pos[1]++;
					for(;i<j; i++,u4Pos[1]++)
					{
						if (pRecAddr[i] != pu1Test[u4Pos[1]])
						{
						    Printf("Compare error: idx:%d  %x!=%x\n", i,  pRecAddr[i], pu1Test[u4Pos[1]]);
        					return FALSE;
						}
					}
				}
				else if (pRecAddr[i] == ((SYNC_PATTERN>>16)&0xFF) 
					&& (rGbl.u1SyncMask & 0x4))
				{
				
					pu1Test = (UINT8*)_arTsGoldenFile[2].TS_Addr_Start;
					j = i+188;
					i++; u4Pos[2]++;
					for(;i<j; i++,u4Pos[2]++)
					{
						if (pRecAddr[i] != pu1Test[u4Pos[2]])
						{
							Printf("Compare error: idx:%d, %x!=%x\n", i, pRecAddr[i], pu1Test[u4Pos[2]]);
							return FALSE;
						}
					}
				}
				else if (pRecAddr[i] == ((SYNC_PATTERN>>24)&0xFF) 
				&& (rGbl.u1SyncMask & 0x8))
				{
					pu1Test = (UINT8*)_arTsGoldenFile[3].TS_Addr_Start;
					j = i+188;
					i++; u4Pos[3]++;
					for(;i<j; i++,u4Pos[3]++)
					{
						if (pRecAddr[i] != pu1Test[u4Pos[3]])
						{
							Printf("Compare error: idx:%d, %x!=%x\n", i, pRecAddr[i], pu1Test[u4Pos[3]]);
							return FALSE;
						}
					}
				}
	   ...
	   Printf("Remux data compare is ok.\r\n");
...
static INT32 _DMXCI_MultiStream_CaOut_Test(INT32 i4Argc, const CHAR ** aszArgv)
{
	  ...
	  _DmxEmuGenRemuxData(FALSE,SYNC_PATTERN);
      /*set hardware path*/
	  //Set DBM bypass mode
	  if(fg_bypass_pidfilter)
	  {
	   ...
	   _DMX_EMU_CustPvrPath(CI_REMUX_PVR,3,FALSE); // pvr 0->tsidx 3, dbm input
	  }
	  else
	  { /*init hardaware path*/
	      DMXCMD_WRITE32(DMX_REG_DBM_CONTROL,(DMXCMD_READ32(DMX_REG_DBM_CONTROL)&~(1<<29)));
	    ...
	    /*set ts_idx3 for multi-stream playback settings*/
	     ...
	     DMXSetLocalTSID(3,SYNC_PATTERN,0x7);
	  }
	      // Set Full pull mode
	...
	if (!DMX_TsOut_SetDataWidthEx(0, DMX_TSOUT_DATAWIDTH_2))
	{
	...
	{/*set pid*/
		...
		// pid for remux data from tsindex0(ddi data)
	    _DMX_EMU_SetPlayPidEx(3,0,0,PID_TABLE[0],DMX_PID_TYPE_PES);
		// pid for remux data from tsindex1(ddi data)
	    _DMX_EMU_SetPlayPidEx(3,1,1,PID_TABLE[1],DMX_PID_TYPE_PES);
		// pid for remux data from tsindex2(ip data)
	    u1pidx=_AllocatePididx_By_InputSource(DMX_TS_FROM_PVR2);
...
//----------------------------------------------------------------
 /*   description:check whether record for every ts_idx  is ok
 *     test path:ddi_ts(framer0),ddi_ts(framer1),IP(pvr2->2)->Remux Record Buffer.
...
static INT32 _DMXCIPLUS_Multi_Play_Test(INT32 i4Argc, const CHAR ** aszArgv)
{
	  ...
	  CIPLUS_TEST_TYPE testType=TEST_TYPE_REMUX;
	  ...
	  UINT32 u4Reg,i=0;
	  MM_MSG_T  rMsg;
	  UINT32 u4SyncPattern = 0x47474647;
	  UINT32 u4SyncMask = 0x3;
	  
	  if((i4Argc != 3) && (i4Argc != 4))
	  {
		Printf("%s,Test command:play timestamp[0|1], testType[0|1|2], bScramble[0|1]\n"
			   "	testType 0: remux only; 1: remux+mm; 2: remux+timeshift\n"
			   "	bScramble 0: FALSE; 1: TRUE\n",aszArgv[0]);
		return	-1;
	  }
      fg_time_on = StrToInt(aszArgv[1]);

	  if (StrToInt(aszArgv[2]) == 1)
	  {
	  	testType = TEST_TYPE_REMUX_MM;
	  }
	  else if(StrToInt(aszArgv[2]) == 2)
	  {
	  	testType = TEST_TYPE_REMUX_TIMESHIFT;
	  }

	  if ( i4Argc == 4 && (StrToInt(aszArgv[3]) == 1))
	  {
	  	bScramble = TRUE;
	  }
	  Printf("timestamp=%s, testType=%s, bScramble=%s\n", fg_time_on ? "TRUE" : "FALSE",
	  	testType==TEST_TYPE_REMUX ? "TEST_TYPE_REMUX":
	  	(testType==TEST_TYPE_REMUX_MM ? "TEST_TYPE_REMUX_MM" : "TEST_TYPE_REMUX_TIMESHIFT"),
			bScramble ? "TRUE":"FALSE");
	  ...
	  if(fg_init_first == FALSE)
	 {
	  ...
	  // Create  MM  thread 
	   ...
	   if (testType == TEST_TYPE_REMUX)
	   {
		  /*generate pes data for IP*/
		  _DmxEmuGenPES(DMX_TS_FROM_PVR2,TRUE);
	   }
	   else if (testType == TEST_TYPE_REMUX_MM)
	   {
	   ...
	   if (bScramble)
	   {
		   //init des_parameter for encrypt/decrypt
			_InitDesParam(TV_PLAY_KEY,PID_TABLE[0],PID_TABLE[1]);
	   ...
	   _DMX_EMU_CIPLUS_Init();
	   /*Set  Full pull mode*/
	   _DMX_SetInputMode(DMX_PULL);

	   /*set DDI to demuxrate*/
	   
		/*path1:for multi-stream record/playback settings*/
		...
		_SetPVRGBLEx5(4,fg_time_on,u4SyncPattern,u4SyncMask,500);
		_FVR_Start(4);
	   
		/*3 multi-stream playback settings*/
		...
		DMXSetLocalTSID(3,u4SyncPattern,u4SyncMask); //set local_ts_id valid.
		/*do pid filter*/
		DMXCMD_WRITE32(DMX_REG_DBM_CONTROL,(DMXCMD_READ32(DMX_REG_DBM_CONTROL)&~(1<<29)));
		...
		if (bScramble)
		{
			/*set multi-stream playback pid,use DMX_PID_TYPE_TS_RAW to debug it.*/
		...
		/*4, set the connection between pvr1 and ts_idx3*/
		_DMX_EMU_CustPvrPath(CI_REMUX_PVR,3,FALSE); 
		if (!DMX_TsOut_SetDataWidthEx(0, DMX_TSOUT_DATAWIDTH_2))
		{
		 	Printf("Failed to set data width!\n");
		...
		if (testType == TEST_TYPE_REMUX)
		{
			/*remux only, IP input setting*/
		    /*customize the IP path	*/
		   _DMX_EMU_CustPvrPath(IP_PVR,2,FALSE);
		   u1pidx=_AllocatePididx_By_InputSource(DMX_TS_FROM_PVR2);
	       _DMX_EMU_SetPlayPidEx1(3,2,u1pidx,PID_TABLE[u1pidx],DMX_PID_TYPE_PES);
		   	/*set mini-pvr fro timeshift*/
			if(_DmxEmuSetPVRPlayExt1(IP_PVR,FALSE,FALSE) == FALSE)
			{
			    Printf("%s ,set IP pvr buffer fails.\r\n",__FUNCTION__);
				return FALSE;
			}
		}
		else if (testType == TEST_TYPE_REMUX_MM)
		{
		    /*path2: mm playback settings*/
		...
		else if (testType == TEST_TYPE_REMUX_TIMESHIFT)
		{
		
			/*path2: timeshift settings*/
			_SetPVRGBLEx5(0,fg_time_on,0x47474647,0x3,500);
			_FVR_Start(0);
			/*set the connecton between pvr2 and ts_idx2 for timeshift*/
			_DMX_EMU_CustPvrPath(TIMESHIFT_PVR,2,TRUE); 

			if (bScramble)
			{
			/*timeshift :set the playback for ts_idx2.*/
			_DMX_EMU_SetPlayPidEx2(2,0,2,PID_TABLE[0],DMX_PID_TYPE_PES);	
			}
			else
			{
			/*timeshift :set the playback for ts_idx2.*/
	  ...
	  /*part4: start  engine*/
	  /* enable the pvr1 playback engine*/
	  _DmxEmuStartPVRPlay(CI_REMUX_PVR);

	  if (testType == TEST_TYPE_REMUX)
	  {
		  /*enable the IP input engine*/
		  _DmxEmuStartPVRPlay(IP_PVR);
	  }
	  else if(testType == TEST_TYPE_REMUX_MM)
	  {
		  /*enable the mm playback engine*/
		  _DmxEmuStartPVRPlay(MM_PVR);
	  }
	  else if (testType == TEST_TYPE_REMUX_TIMESHIFT)
	  {
	  	/*enable the pvr playback engine*/
	    _DmxEmuStartPVRPlay(TIMESHIFT_PVR);
	  }
	  
	  /*enable dma engine ,step by step*/
      /*get src address and src size*/
	  SrcAddr= _arTsGoldenFile[DMX_TS_FROM_DDI].TS_Addr_Start;
	  SrcSize = (_arTsGoldenFile[DMX_TS_FROM_DDI].TS_Addr_Offset- \
						_arTsGoldenFile[DMX_TS_FROM_DDI].TS_Addr_Start);
	   /*enable DDI engine, don't wait complete*/
	  if(!_DMX_EMU_DDI_Transfer(6,SrcAddr,SrcSize))
	  {
		Printf("DDI move datas error,test Fail.\r\n");
		return -1;	
...
static INT32 _DMXCIPLUS_Multi_Stop_Test(INT32 i4Argc, const CHAR ** aszArgv)
{
  ...
  if(i4Argc != 2)
  {
	Printf("%s,Test command:stop testType[0|1|2]\n",aszArgv[0]);
	return	-1;
  }
  if (StrToInt(aszArgv[2]) == 1)
  {
  ...
  Printf("testType=%s\n", testType==TEST_TYPE_REMUX ? "TEST_TYPE_REMUX":
   ...
   if (!_MemCmp(SrcpesAddr, DstpesAddr,u4size, &u4SrcDiff, &u4DstDiff))
   {
	...
	if (testType==TEST_TYPE_REMUX)
	{
	   // for IP 
	   SrcpesAddr=_arTsGoldenFile[DMX_TS_FROM_PVR2].Pes_Addr_Start;
	   DstpesAddr=_arPidBuffer[3].u4BufAddr;
	   u4size=(_arTsGoldenFile[DMX_TS_FROM_PVR2].Pes_Addr_Offset- \
	           _arTsGoldenFile[DMX_TS_FROM_PVR2].Pes_Addr_Start);
	...
	else
	{
	 ...
	 {
	 	_DmxEmuStopPVRPlay(IP_PVR);
		_DMX_EMU_FreePlaypid(3); /*free pid for IP playback.*/
		_DMX_EmuPVRPlay_FreeBuf(IP_PVR);
	 }
	 else if (testType==TEST_TYPE_REMUX_MM)
	 {
...
static INT32 _DMXCI_Remux_Interrupt_Test(INT32 i4Argc, const CHAR ** aszArgv)
{
	  ...
	  _SetPVRGBLTimerNotify(ui4_timer_notify);
	  _FVR_Start(4);
...
static INT32 _DMXCI_Remux_Test(INT32 i4Argc, const CHAR ** aszArgv)
{
	  ...
	  /*init ts parameter*/
	  _DMX_EMU_TS_Parameter_Init();
	  /*generate pes data for ddi_ts*/
	  _DmxEmuGenPES(DMX_TS_FROM_DDI,TRUE);
	  /*generate pes data for IP*/
	  _DmxEmuGenPES(DMX_TS_FROM_PVR2,TRUE);
	   /*init hw path */
	   /*default is pvr1->ts2; pvr0->ts3*/
	   _DMX_EMU_CIPLUS_Init();
	   /*customize the IP path	*/
	   _DMX_EMU_CustPvrPath(IP_PVR,2,FALSE);
	  /*set mini-pvr*/
...
static INT32 _DMXCI_IP_TS_TO_DEMUX(INT32 i4Argc, const CHAR ** aszArgv)
{
  ...
  if(i4Argc != 5)
  {
    Printf("%s,Test command:IP ts_idx[2] disable_filter[0,1]  \
