 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SetTestFilter(UINT8 u1TsIdx,UINT8 u1PatIdx)
{
    DMX_FILTER_T rFilter;
            ...
            (rFilter.eMode == DMX_FILTER_MODE_GENERIC))
    {
	    ...
	    if (!DMX_SetFilter((_arPat[u1PatIdx].u1FilterIdx+96), 1, DMX_FILTER_FLAG_ALL, &rFilter))
        {
            return FALSE;
        }
    }
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SetTestGenericFilter(UINT8 u1TsIdx, UINT8 u1PatIdx)
{
    DMX_FILTER_GENERIC_T rFilter;
    UINT32 i;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSI_Test(UINT8 u1TsIdx, BOOL fgGenPat, DMX_FILTER_MODE_T eMode, BOOL fgBypassFramer)
{
    UINT32 i, u4PidCount;
    UINT8 *pu1Golden;
    BOOL fgRet = TRUE;
    UINT32 u4DstStart,u4DstEnd;
   //just has 64 playback pid ,but there's 128 sectin filter ,so set the max num=64.
    #if 1
    ...
    for (i=0; i<u4PidCount; i++)
    {
        ...
        if (eMode == DMX_FILTER_MODE_GENERIC)
        {
            _SetTestGenericFilter(u1TsIdx, i);
        }
        else
        {
            _SetTestFilter(u1TsIdx, i);
        }
    }
...
BOOL _DMX_EMU_PSI_Rec_Test(UINT8 u1TsIdx, BOOL fgGenPat, DMX_FILTER_MODE_T eMode, BOOL fgBypassFramer)
{
	...
	Printf("Not support from Oryx, no need to test!\n");
	return FALSE;

	//There are 128 playback pid filters and 128 sectin filters ,
	//so set the max num=128
#if 0
    _u1DmxEmuTestPidNum = DMX_NUM_PID_INDEX;
    u4PidCount = DMX_NUM_PID_INDEX;
#else 
...
BOOL _DMX_EMU_PSI_PlayRec_Test(UINT8 u1TsIdex, BOOL fgGenPat, DMX_FILTER_MODE_T eMode, BOOL fgBypassFramer)
{
    ...
    u4PidCount = DMX_NUM_FILTER_INDEX;
    ...
    for (i=0; i<u4PidCount; i++)
    {
        ...
        if (eMode == DMX_FILTER_MODE_GENERIC)
        {
            _SetTestGenericFilter(u1TsIdex, i);
        }
        else
        {
            _SetTestFilter(u1TsIdex ,i);
        }
    }
...
BOOL _DMX_EMU_PSICombo_Test(UINT8 u1TsIdx, BOOL fgGenPat, DMX_FILTER_MODE_T eMode, UINT32 u4Hw, BOOL fgBypassFramer)
{
    if (u4Hw == 0)
    {
        return _DMX_EMU_PSI_Test(u1TsIdx, fgGenPat, eMode, fgBypassFramer);
    }
    else if (u4Hw == 1)
    {
...
BOOL DMX_EMU_PSICALoopback_Test(UINT8 u1TsIdex, BOOL fgGenPat, DMX_FILTER_MODE_T eMode, UINT8 u1TsOutIdx)
{
	...
	//There are 64 playback pid filters and 128 sectin filters ,
	//so set the max num=64
#if 1
		_u1DmxEmuTestPidNum = DMX_NUM_PID_INDEX;
		u4PidCount = DMX_NUM_PID_INDEX;
#else 
