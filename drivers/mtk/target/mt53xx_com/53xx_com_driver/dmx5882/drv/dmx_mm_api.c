...
static BOOL _DMXMMAllocBuf(UINT8 u1Idx, const DMX_MM_T *prMM)
{
    ...
    if(prMM->fgAllocBuf)
    {
        ...
        #endif
        u4PhyBufStart = (UINT32)BSP_AllocAlignedDmaMemory(prMM->u4BufSize,
...
BOOL _DMX_MM_SetSecureAES(UINT8 u1Idx, UINT32 u4Flags, const DMX_MM_DRM_AES_T *prDRM, BOOL fgReload)
{
    ...
    if(fgReload)
    {
		...
		{
			if(!TZ_DMX_SetSecureIV((UINT8 *)prDRM->au1IV, sizeof(prDRM->au1IV)))
			{
        ...
        if(!TZ_DMX_ReloadSecureKeyIv(u1Idx))
        {
