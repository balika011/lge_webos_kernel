...
//  Not use now. Use logical mapping
    ...
    if (u4Flags & FVR_PID_FLAG_TYPE)
    {
        ...
        if (prPid->ePidType == DMX_PID_TYPE_ES_VIDEO)
        {
            ...
            FVR_PER_PID_S_W(u1Pidx, 3) = 0xFFFFFFFF;        // previous start code pattern
        }
        ...
        if (prPid->fgDoSFilter)
        {
           LOG(0,"don't need do section filter .\r\n");
        }
    }
    ...
    if (u4Flags & FVR_PID_FLAG_ENABLE)
    {
        ...
        {
            _FVR_Lock();
            FVR_PER_PID_S_W(u1Pidx, 2) |= 0xFF00;       // Set scramble state to unknown
