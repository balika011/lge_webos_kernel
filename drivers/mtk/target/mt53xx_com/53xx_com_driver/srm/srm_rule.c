...
static UINT32 _SrmScposRule5396_Shrink(SRM_VDP_INFO_T* prSrmVdpInfo, SRM_VDP_CRTL_T* prSrmVdpCtrl)
{
    UINT32 u4Mode = 0;
    UINT32 u4CtrlMode = SRM_CTRL_MODE_NONE;
    UINT32 u4PDSize = PANEL_GetPanelWidth();
             ...
             (SRM_GetMmMode() != SRM_MM_MODE_TYPE_THUMBNAIL))
    {
        ...
        {
			if((u4DrvTDTV3DModeQuery() == E_TDTV_UI_3D_MODE_OFF) && (prSrmVdpInfo[VDP_1].u4OutputHeight < SrmGetMaxVdpHeight()))	
			{							
					u4PDSize = 1278;						
					u4Mode |= FBM_POOL_MODE_SCPOS_FULL; 
				}
...
static UINT32 _SrmScposRule5365_128Mx1(SRM_VDP_INFO_T* prSrmVdpInfo, SRM_VDP_CRTL_T* prSrmVdpCtrl)
{
             ...
             (SRM_GetMmMode() != SRM_MM_MODE_TYPE_THUMBNAIL))
    {
        ...
        if ((prSrmVdpInfo[VDP_1].u4VgaTiming == 1) ||
            ...
            SRM_IsMainVerticalFull())
        {
            if ((prSrmVdpInfo[VDP_1].u4Resolution > SRM_VDP_SD_RESOLUTION) &&( !SRM_IsPopVideo()) && (!SRM_IsMainVerticalFull()))
            {
                u4Mode |= FBM_POOL_MODE_SCPOS_PSCAN_DISP;
            }
            else
...
#if defined(CC_MT5395) || defined(CC_MT5396)
static UINT32 _SrmMddiRule5396_Shrink(SRM_VDP_INFO_T* prSrmVdpInfo, SRM_VDP_CRTL_T* prSrmVdpCtrl)
{
    UINT32 u4Mode = 0;
    UINT32 u4CtrlMode = SRM_CTRL_MODE_NONE;
        ...
        )
    {
        ...
        if (prSrmVdpInfo[VDP_1].u4Interlace == 1)
        {
			if(((SRM_GetCurrFBMMode()== FBM_MMMODE))&&(prSrmVdpInfo[VDP_1].u4Resolution > SRM_VDP_720HD_RESOLUTION))
			{
			    u4Mode |= FBM_POOL_MODE_MDDI_CBOB;
				u4Mode &= ~FBM_POOL_MODE_10BIT;
				u4Mode |= FBM_POOL_MODE_8BIT;
		...
		if ((u4DrvTDTV3DModeQuery() != E_TDTV_UI_3D_MODE_OFF) && (prSrmVdpInfo[VDP_1].u4Source != (UINT32)VSS_DTV))	
			{	
			    u4Mode &= ~FBM_POOL_MODE_MDDI_CBOB;
                u4Mode &= ~FBM_POOL_MODE_MDDI_BOB;
				u4Mode |= FBM_POOL_MODE_MDDI_FULL;	
        ...
        (prSrmVdpInfo[VDP_2].u4VgaTiming == 0))
    {
	 ...
	 {
		if(prSrmVdpInfo[VDP_2].u4Resolution > SRM_VDP_SD_RESOLUTION)
			{
...
static UINT32 _SrmMddiRule5501_net(SRM_VDP_INFO_T* prSrmVdpInfo, SRM_VDP_CRTL_T* prSrmVdpCtrl)
{
        ...
        )
    {
        ...
        #ifdef CC_SCPOS_3DTV_SUPPORT
        if ((u4DrvTDTVForceDIDramModeQuery() == E_TDTV_DI_DRAM_0FB)||((u4DrvTDTVForceDramModeQuery()!=0)&&(prSrmVdpInfo[VDP_1].u4Resolution > SRM_VDP_SD_RESOLUTION)))
        {
			u4Mode &= ~FBM_POOL_MODE_10BIT;
			u4Mode |= FBM_POOL_MODE_8BIT;
            u4Mode &= ~FBM_POOL_MODE_MDDI_NR_Y_C;
...
static UINT32 _SrmMJCRule5395(SRM_VDP_INFO_T* prSrmVdpInfo, SRM_VDP_CRTL_T* prSrmVdpCtrl)
{
...
#endif
    {
        ...
        if (SRM_IsMJCWidthLimit() || SRM_IsGameMode(VDP_1) || (SRM_IsSubVideoDisplay()==1) || (prSrmVdpInfo[VDP_1].u4VgaTiming == 1))
    	{
    	    u4Mode = FBM_POOL_MODE_FRC_Y10_C10_444;
    	}
                  ...
                  FN_SRN_DB_RULE **papfNotStableRule, UINT32 *pu4NotStableRuleNs)
{
	...
	else if(SRMFBM_GetConf() == FBM_MEM_CFG_MT5396_3DTV_CHA_NOMJC_SHRINK)
	{
		...
		UINT8 idx = 0;
		papfStableRule[idx++] = (FN_SRN_DB_RULE*)_SrmScposRule5396_Shrink;
		papfStableRule[idx++] = (FN_SRN_DB_RULE*)_SrmMddiRule5396_Shrink;
	    papfStableRule[idx++] = (FN_SRN_DB_RULE*)_SrmMJCRule5395;
        papfStableRule[idx++] = (FN_SRN_DB_RULE*)_SrmStableRule;
        *pu4StableRuleNs = idx;
