...
BOOL _DMX_TsIn_SetRountMux(UINT8 u1TsIdx, UINT8 u1TsOutIdx)
{
    ...
    u4Reg &= ~(1 << (u1TsIdx + 4));
    u4Reg |= ((u1TsOutIdx == 1) ? 1 : 0) << (u1TsIdx + 4);
...
BOOL _DMX_TsOut_SetRoute(UINT8 u1TsIndex, UINT8 u1TsoutIdx, DMX_TSOUT_ROUTE_T eRoute)
{
    ...
    case DMX_TSOUT_ROUTE_DBM_LOOPBACK:
        ...
        ASSERT((u1TsIndex == 2) || (u1TsIndex == 3));
    ...
    case DMX_TSOUT_ROUTE_EXT_LOOPBACK:
        ...
        i4Ret = BSP_PinSet(PIN_GPIO15, 1);
        i4Ret = BSP_PinSet(PIN_GPIO18, 1);
        i4Ret = BSP_PinSet(PIN_GPIO19, 1);
        i4Ret = BSP_PinSet(PIN_GPIO20, 1);
        i4Ret = BSP_PinSet(PIN_GPIO21, 1);
        i4Ret = BSP_PinSet(PIN_GPIO22, 1);

        //CI address output enable
        u4Reg = CKGEN_READ32(CKGEN_TSIF_CFG) | (1<<7);
        CKGEN_WRITE32(CKGEN_TSIF_CFG, u4Reg);
        ...
        {
            u4Reg = CKGEN_READ32(CKGEN_REG_EXTTS_CKCKG) & ~0x700;
            u4Reg |= 0x200; // //  dmxout ts clk
            CKGEN_WRITE32(CKGEN_REG_EXTTS_CKCKG, u4Reg);
        }
        else
        {
            u4Reg = CKGEN_READ32(CKGEN_REG_EXTTS_CKCKG) & ~0x700;
            u4Reg |= 0x300; // dmxout2 ts clk
            CKGEN_WRITE32(CKGEN_REG_EXTTS_CKCKG, u4Reg);
        }
#endif
