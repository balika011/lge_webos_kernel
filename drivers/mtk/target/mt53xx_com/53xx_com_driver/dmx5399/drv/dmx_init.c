...
static void _DmxSetExtPinmux(DMX_PINMUX_SEL_T ePin)
{
    ...
    case DMX_PINMUX_EXT2_4_S:
        ...
        //PAD_GPIO44   --> SYNC
        //PAD_GPIO48   --> DATA0
        i4Ret = BSP_PinSet(PIN_GPIO42, 3);
        i4Ret = BSP_PinSet(PIN_GPIO43, 3);
        i4Ret = BSP_PinSet(PIN_GPIO44, 3);
        i4Ret = BSP_PinSet(PIN_GPIO48, 3);
        break;
    ...
    case DMX_PINMUX_EXT3_1_S:
        ...
        //PAD_GPIO51 -->SYNC
        //PAD_GPIO52 -->DATA0
        i4Ret = BSP_PinSet(PIN_GPIO49, 7);
        i4Ret = BSP_PinSet(PIN_GPIO50, 7);
        i4Ret = BSP_PinSet(PIN_GPIO51, 7);
        i4Ret = BSP_PinSet(PIN_GPIO52, 7);
        break;
    ...
    case DMX_PINMUX_EXT5_S:
        //PAD_PVR_TSSYNC -->CLK
        //PAD_PVR_TSCLK -->VALID
        //PAD_CI_TSDATA0 -->SYNC
        //PAD_GPIO34 -->DATA0
        i4Ret = BSP_PinSet(PIN_PVR_TSSYNC, 3);
        i4Ret = BSP_PinSet(PIN_CI_TSDATA0, 3);
        i4Ret = BSP_PinSet(PIN_GPIO34, 1);
        break;

    case DMX_PINMUX_EXT5_P:
        //PAD_PVR_TSSYNC -->CLK
        //PAD_PVR_TSCLK -->VALID
        //PAD_CI_TSDATA0 -->SYNC
        //PAD_GPIO34-->DATA0
...
static BOOL _DmxTSSoftInit(void)
{
    ...
    //---------------------------------------------------------
    // Clear pid replaced globe region
    //---------------------------------------------------------
    ...
    x_memset((void*)DMX_SECTION_POSNEG, 0, 8 * 2 * DMX_NUM_FILTER_INDEX);
    x_memset((void*)DMX_SECTION_CONTROL, 0, 4 * 2 * DMX_NUM_FILTER_INDEX);
    x_memset((void*)DMX_SECTION_PATTERN, 0, 8 * 2 * DMX_NUM_FILTER_INDEX);
    x_memset((void*)DMX_SECTION_MASK, 0, 8 * 2 * DMX_NUM_FILTER_INDEX);

    //second 96 section filter
    x_memset((void*)DMX_SECTION_POSNEG2, 0, 8 * 2 * DMX_NUM_FILTER_INDEX);
    x_memset((void*)DMX_SECTION_CONTROL2, 0, 4 * 2 * DMX_NUM_FILTER_INDEX);
    x_memset((void*)DMX_SECTION_PATTERN2, 0, 8 * 2 * DMX_NUM_FILTER_INDEX);
    x_memset((void*)DMX_SECTION_MASK2, 0, 8 * 2 * DMX_NUM_FILTER_INDEX);

    x_memset((void*)DMX_ONEBYTE_FILTER_BASE, 0, 2 * DMX_NUM_ONEBYTE_FILTER_INDEX);

    //Set section filter group
    u4Ctrl = SECTIONFILTER_SETTING & (~0xFF);
    u4Ctrl |= (0 << 0); //section filter group 0 for ts index 0
    u4Ctrl |= (1 << 2); //section filter group 1 for ts index 1
    u4Ctrl |= (2 << 4); //section filter group 2 for ts index 2
    u4Ctrl |= (3 << 6); //section filter group 3 for ts index 3
...
static BOOL _DmxInitHWPath(void)
{
    ...
    // PVR path
    //----------------------------------
    DMX_PVRPlay_SetPortEx(0,DMX_PVRPLAY_PORT_FRAMER2_BYPASS); // PVR for TS file
...
BOOL _DMX_SetFrontEndEx(UINT8 u1Idx, DMX_FRONTEND_T eFrontEnd)
{
    ...
    if ((u1Idx == 2) || (u1Idx == 3))
    {
        ...
        {
            _DMX_SetDbm_InputSource(u1Idx, DMX_DBM_INPUT_DDI);
        }
    }
...
void _DMX_SetDbm_InputSource(UINT8 u1TsIdx, DMX_DBM_INPUT_SOURCE_T eSource)
{
    ...
    {
        LOG(0, "Tsindex Error, _DMX_SetDbm_InputSource only support framer 2 and 3.\n");
        return;
    }
