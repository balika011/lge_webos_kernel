...
static FILTER_STRUCT_T _arFilterStructEx[DMX_FRAMER_COUNT][DMX_NUM_FILTER_INDEX];
                          ...
                          UINT32* pu4PidWord0, PID_STRUCT_T* prPidStruct, BOOL fgKeepScramble)
{
    ...
    if (fgEnable)
    {
		...
		if (prPidStruct->ePidType == DMX_PID_TYPE_ES_VIDEO)
		{
		DMXPIDLOG(DMX_LOG_AV 1, u1Pidx, "(%u) DMX Enable V-PID(0x%x)", u1Pidx, prPidStruct->u2Pid);
		_fgWaitForFirstI = TRUE;
		}
#endif  // TIME_MEASUREMENT
    ...
    else
    {
		...
		if (prPidStruct->ePidType == DMX_PID_TYPE_ES_VIDEO)
        {
          DMXPIDLOG(DMX_LOG_AV 1, u1Pidx, "(%u) DMX Disable V-PID(0x%x)", u1Pidx, prPidStruct->u2Pid);
        }
                    ...
                    const DMX_FILTER_T* prFilter)
{
    ...
    u4FilterWord = SECTION_FILTER_CTRL(prFilter->u1Tsidx, u1FilterIndex);

    prFilterStruct = &_arFilterStructEx[prFilter->u1Tsidx][u1FilterIndex];
    ...
    if ((u4Flags & DMX_FILTER_FLAG_PATTERN) != 0)
    {
        ...
        SECTION_FILTER_CTRL(prFilter->u1Tsidx, u1FilterIndex) &= ~(1U << 31);

        au4Pattern[0] = SECTION_FILTER_PAT(prFilter->u1Tsidx, u1FilterIndex, 0);
        au4Pattern[1] = SECTION_FILTER_PAT(prFilter->u1Tsidx, u1FilterIndex, 1);

        au4Mask[0] = SECTION_FILTER_MASK(prFilter->u1Tsidx, u1FilterIndex, 0);
        au4Mask[1] = SECTION_FILTER_MASK(prFilter->u1Tsidx, u1FilterIndex, 1);
        ...
        if (prFilterStruct->eMode == DMX_FILTER_MODE_SHORT)
        {
            ...
            SECTION_FILTER_POSNEG(prFilter->u1Tsidx, u1FilterIndex, 0) = 0x0;
            SECTION_FILTER_POSNEG(prFilter->u1Tsidx, u1FilterIndex, 1) = 0x0;
        }
        ...
        else if (prFilterStruct->eMode == DMX_FILTER_MODE_POS_NEG)
        {
            ...
            else if (u1Bank == 1)
            {
                ...
                SECTION_FILTER_POSNEG(prFilter->u1Tsidx, u1FilterIndex, 0) = prFilter->au4Mask[0];
                SECTION_FILTER_POSNEG(prFilter->u1Tsidx, u1FilterIndex, 1) = prFilter->au4Mask[1];
            }
        }
        else
        {
        ...
        SECTION_FILTER_PAT(prFilter->u1Tsidx, u1FilterIndex, 0) = au4Pattern[0];
        SECTION_FILTER_PAT(prFilter->u1Tsidx, u1FilterIndex, 1) = au4Pattern[1];

        SECTION_FILTER_MASK(prFilter->u1Tsidx, u1FilterIndex, 0) = au4Mask[0];
        SECTION_FILTER_MASK(prFilter->u1Tsidx, u1FilterIndex, 1) = au4Mask[1];
    ...
    SECTION_FILTER_CTRL(prFilter->u1Tsidx,u1FilterIndex) = u4FilterWord;
                       ...
                       const DMX_FILTER_GENERIC_T* prFilter)
{
    ...
    if ((u4Flags & DMX_FILTER_FLAG_PATTERN) != 0)
    {
        ...
        SECTION_FILTER_PAT(prFilter->u1Tsidx,u1FilterIndex, 0) = (prFilter->au1Data[0] << 24) |
                (prFilter->au1Data[1] << 16) |
                (prFilter->au1Data[2] << 8)  |
                (prFilter->au1Data[3]) ;
        SECTION_FILTER_PAT(prFilter->u1Tsidx,u1FilterIndex, 1) = (prFilter->au1Data[4] << 24) |
        ...
        {
            SECTION_FILTER_MASK(prFilter->u1Tsidx ,u1FilterIndex, 0) = (prFilter->au1Mask[0] << 24) |
                    (prFilter->au1Mask[1] << 16) |
                    (prFilter->au1Mask[2] << 8)  |
                    (prFilter->au1Mask[3]);
            SECTION_FILTER_MASK(prFilter->u1Tsidx ,u1FilterIndex, 1) =  (prFilter->au1Mask[4] << 24) |
        ...
        {
            //Only Table ID
            SECTION_FILTER_MASK(prFilter->u1Tsidx ,u1FilterIndex, 0) = (UINT32)(prFilter->au1Mask[0] << 24);
            SECTION_FILTER_MASK(prFilter->u1Tsidx ,u1FilterIndex, 1) =  0;
        }

        SECTION_FILTER_POSNEG(prFilter->u1Tsidx ,u1FilterIndex, 0) = (prFilter->au1PosNeg[0] << 24) |
                (prFilter->au1PosNeg[1] << 16) |
                (prFilter->au1PosNeg[2] << 8)  |
                (prFilter->au1PosNeg[3]);
        SECTION_FILTER_POSNEG(prFilter->u1Tsidx ,u1FilterIndex, 1) = (prFilter->au1PosNeg[4] << 24) |
...
FILTER_STRUCT_T* _DMX_GetFilterStruct(UINT8 u1Idx, UINT32 u4FilterIndex)
{
    ...
    return &_arFilterStructEx[u1Idx][u4FilterIndex];
}
