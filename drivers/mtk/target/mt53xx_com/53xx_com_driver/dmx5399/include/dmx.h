...
#define DMX_ONEBYTE_FILTER_BASE                 (DEMUX5_BASE + 0x174) //1C174

#define DMX_PATTERN_MATCH_BASE0                 (DEMUX5_BASE) //0x1C000
#define DMX_PATTERN_MATCH_BASE1                 (DEMUX5_BASE + 0xA8) //0x1C0A8


#define DMX_DMEM_CA_KEY_BASE                    (DEMUX7_BASE) //54000
#define DMX_DMEM_CA_CTRL_BASE                   (DEMUX7_BASE + 0xC00) //54C00

#define DMX_DMEM_MM_KEY_SIZE                    (18) //72 byte
#define DMX_DMEM_MM_KEY_BASE                    (DEMUX7_BASE + 0xE00) //54E00
#define DMX_DMEM_MM_IV_BASE                     (DEMUX7_BASE + 0xE20) //54E20
#define DMX_DMEM_MM_KEY_BASE1                   (DEMUX7_BASE + 0xE80) //54E80
#define DMX_DMEM_MM_IV_BASE1                    (DEMUX7_BASE + 0xEA0) //54EA0
...
#define SECTION_FILTER_APPENDDATA_SIZE          (8) //48bit filter match result + 1 byte ts index  + 1byte serial number
...
#define REG_IOMMU_CFG5              0x5//0x010        // performance meter
#define REG_IOMMU_CFG6              0x6//0x010       //monitor result 0
#define REG_IOMMU_CFG7              0x7//0x010        //monitor result 1
#define REG_IOMMU_CFG8              0x8//0x010        //monitor result 2
#define REG_IOMMU_CFG9              0x9//0x010         // overread 0
#define REG_IOMMU_CFGA              0xA//0x010         //overread 1
#define REG_IOMMU_CFGB              0xb//0x010          //overread 2
#define REG_IOMMU_CFGC              0xc//0x010         //overread 3
#define REG_IOMMU_CFGD              0xd//0x010         //overread 4
...
typedef enum
{
    ...
    DMX_PINMUX_EXT4_S = 0x90,          // Parallel

    DMX_PINMUX_EXT5_S = 0xA0,          // Parallel
    DMX_PINMUX_EXT5_P = 0xA1,          // Parallel
#endif
} DMX_PINMUX_SEL_T;
...
//New Section Filter access Command
...
#define SECTION_FILTER_PAT(Tsidx, Filteridx, word) \
        SECTION_FILTER_BASE(((Tsidx == 0) ? DMX_SECTION_PATTERN : \
                            (Tsidx == 1) ? (DMX_SECTION_PATTERN + 0x180) : \
                            (Tsidx == 2) ? (DMX_SECTION_PATTERN2) : \
                            (DMX_SECTION_PATTERN2 + 0x180)\
                            ),Filteridx,word)

#define SECTION_FILTER_MASK(Tsidx, Filteridx, word) \
        SECTION_FILTER_BASE(((Tsidx == 0) ? DMX_SECTION_MASK : \
                            (Tsidx == 1) ? (DMX_SECTION_MASK + 0x180) : \
                            (Tsidx == 2) ? (DMX_SECTION_MASK2) : \
                            (DMX_SECTION_MASK2 + 0x180)\
                            ),Filteridx,word)

#define SECTION_FILTER_POSNEG(Tsidx, Filteridx, word) \
        SECTION_FILTER_BASE(((Tsidx == 0) ? DMX_SECTION_POSNEG : \
                            (Tsidx == 1) ? (DMX_SECTION_POSNEG + 0x180) : \
                            (Tsidx == 2) ? (DMX_SECTION_POSNEG2) : \
                            (DMX_SECTION_POSNEG2 + 0x180)\
                            ),Filteridx,word)

#define SECTION_FILTER_CTRL(Tsidx, Filteridx) \
        SECTION_FILTER_BASE_CTRL(((Tsidx == 0) ? DMX_SECTION_CONTROL : \
                            (Tsidx == 1) ? (DMX_SECTION_CONTROL + 0xC0) : \
                            (Tsidx == 2) ? (DMX_SECTION_CONTROL2) : \
                            (DMX_SECTION_CONTROL2 + 0xC0)\
                            ),Filteridx)
/// Function entry log
...
#define PLAYBACK_GBL_BASE       (DEMUX5_BASE + 0x150)
#define CT_SETTING              (*(volatile UINT32*)(DEMUX5_BASE + 0x150))
#define MICROCODE_WATCHDOG      (*(volatile UINT32*)(DEMUX5_BASE + 0x154))
#define CT_TSIDX_SETTING        (*(volatile UINT32*)(DEMUX5_BASE + 0x158))
#define SECTIONFILTER_SETTING   (*(volatile UINT32*)(DEMUX5_BASE + 0x15C))

#define B_FRAME_DROP_ENABLE     (*(volatile UINT32*)(DEMUX5_BASE + 0x160))

#define RV_SETTING_SIZE         (33)//132 byte
#define RV_SETTING_BASE         (DEMUX5_BASE + 0x370) //0x1C370
