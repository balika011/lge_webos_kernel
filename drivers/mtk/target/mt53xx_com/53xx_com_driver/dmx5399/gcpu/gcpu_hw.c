...
#include "gcpu_iram0_98.h"
#include "gcpu_iram0_viper.h"
#include "gcpu_iram0_python.h"
#include "gcpu_iram0_99.h"
...
static BOOL _Gcpu_Hw_GetRamCode(BYTE **ppbRamCode, UINT32 *pu4RamCodeSize, BYTE **ppbTrapping, UINT32 *pu4TrapSize)
{
    ...
    // for Viper/Python 1st verstion
    if(BSP_GetIcVersion() == IC_VER_5880_AA)
    {
        *ppbRamCode = (BYTE*) GCPU_IRAMV_CODE;
        *ppbTrapping = (BYTE*) GCPU_IRAMV_TRAPPING;

        *pu4RamCodeSize = sizeof(GCPU_IRAMV_CODE);
        *pu4TrapSize = sizeof(GCPU_IRAMV_TRAPPING);
    }
    else if(BSP_GetIcVersion() == IC_VER_5860_AA)
    {
        *ppbRamCode = (BYTE*) GCPU_IRAMP_CODE;
        *ppbTrapping = (BYTE*) GCPU_IRAMP_TRAPPING;
        
        *pu4RamCodeSize = sizeof(GCPU_IRAMP_CODE);
        *pu4TrapSize = sizeof(GCPU_IRAMP_TRAPPING);
    }
    else if (BSP_GetIcVersion() == IC_VER_5398_AA)
    {
    ...
    else if (BSP_GetIcVersion() == IC_VER_5399_AA)
    {
        *ppbRamCode = (BYTE*) GCPU_IRAMM_CODE;
        *ppbTrapping = (BYTE*) GCPU_IRAMM_TRAPPING;
         
        *pu4RamCodeSize = sizeof(GCPU_IRAMM_CODE);
        *pu4TrapSize = sizeof(GCPU_IRAMM_TRAPPING);
    }
...
INT32 _GCPU_Hw_CmdEx(GCPU_HW_CMD_T *prHwCmd, const GCPU_IO_BUF_T *prBuf, UINT32 u4Flags) 
{
    ...
    else if (u4Flags & GCPU_HWCMD_FLAG_DMA_BUF)
    {
