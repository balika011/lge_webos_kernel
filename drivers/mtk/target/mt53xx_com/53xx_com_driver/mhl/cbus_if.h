...
#define SetCBUSStatus(arg) { \
    CRIT_STATE_T rCritState; \
    rCritState = x_crit_start(); \
    (_Cbus_Status |= ((UINT16)arg)); \
    x_crit_end(rCritState); }

#define ClrCBUSStatus(arg) { \
    CRIT_STATE_T rCritState; \
    rCritState = x_crit_start(); \
    (_Cbus_Status &= (~((UINT16)arg))); \
    x_crit_end(rCritState); }

#define IsCBUSStatus(arg) 	((_Cbus_Status & ((UINT16)arg)) > 0)
#endif
...
void _Cbus_NotifyTxResult(UINT8 result, CEC_FRAME_DESCRIPTION *frame,void *tag);
