...
static INT32 _GfxSwRectFill(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    switch (prsGfxReg->fg_OP_MODE)
    {
        ...
        u4Width = prsGfxReg->fg_SRC_WIDTH;
        #if GFX_4KX2K_TEST
        u4Height = prsGfxReg->fg_SRC_HEIGHT + ((prsGfxReg->fg_SRC_HEIGHT_12 & 0x01)<< 11) + 1;
        prsGfxReg->fg_SRC_HEIGHT_12 = 0;
        #else
        u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
        #endif
        break;
...
static INT32 _GfxSwBitBlt(const MI_DIF_FIELD_T *prsGfxReg)
{
    ...
    UINT32 u4ConEn;
    UINT32 u4wrAorder, u4wrRorder, u4wrGorder, u4wrBorder;
   ...
   // u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
    ...
    u4ConEn = prsGfxReg->fg_YuyvConv;
    u4wrAorder = prsGfxReg->fg_DST_WR_A_ORDER;
    u4wrRorder = prsGfxReg->fg_DST_WR_R_ORDER;
    u4wrGorder = prsGfxReg->fg_DST_WR_G_ORDER;
    u4wrBorder = prsGfxReg->fg_DST_WR_B_ORDER;
    ...
    {
       return GFX_SwYUYVToRaster(pu1SrcBase, pu1DstBase, u4SrcPitch, u4DstPitch,
                u4SrcCM, u4DstCM, u4Width, u4Height, u4wrAorder, u4wrRorder,
               u4wrGorder, u4wrBorder);   
    }
    else
    {
...
static INT32 _GfxSwYCrCb2Rgb(const MI_DIF_FIELD_T *prsGfxReg)
{
   ...
   // rData.u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
  #if GFX_4KX2K_TEST
    rData.u4Height = prsGfxReg->fg_SRC_HEIGHT + ((prsGfxReg->fg_SRC_HEIGHT_12 & 0x01)<< 11) + 1;
    prsGfxReg->fg_SRC_HEIGHT_12 = 0;
  #else
    rData.u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
  #endif
...
//-------------------------------------------------------------------------
/** _GfxSwYBlockToYRaster
 *  y block to y raster
 */
//-------------------------------------------------------------------------
/*lint -save -e613 */
static _GfxSwYBlockToYRaster(const MI_DIF_FIELD_T *prsGfxReg)
{
    UINT8 *pu1SrcBase, *pu1DstBase, *pu1CBCRBase;
    UINT32 u4SrcPitch, u4DstPitch, u4CBCRPitch, u4WritePitchEn;
    UINT32 u4SrcCM, u4DstCM;
    UINT32 u4Color;
    UINT32 u4Width, u4Height;
    UINT32 u4Vfull, u4SwapMode;
    ...
    pu1DstBase = (UINT8 *)prsGfxReg->fg_DST_BSAD;
    u4DstPitch = prsGfxReg->fg_OSD_WIDTH << 4; // no value
    u4DstCM = prsGfxReg->fg_CM;
    u4Width = prsGfxReg->fg_SRC_WIDTH;
   // u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
    #if GFX_4KX2K_TEST
    ...
    u4Color = prsGfxReg->fg_RECT_COLOR;
    u4WritePitchEn = prsGfxReg->fg_WRITE_NO_PITCH;

    pu1CBCRBase = (UINT8 *)prsGfxReg->fg_SRCCBCR_BSAD;
    u4CBCRPitch  =  prsGfxReg->fg_SRCCBCR_PITCH;
    u4Vfull =   prsGfxReg->fg_VFULL;
    u4SwapMode =   prsGfxReg->fg_SWAP_MODE;
    ...
    return GfxSwYBlockToYRaste(pu1SrcBase, pu1DstBase, u4SrcPitch, u4DstPitch,
            u4SrcCM, u4DstCM, u4Width, u4Height, pu1CBCRBase, u4CBCRPitch,
           u4Vfull, u4SwapMode, u4WritePitchEn);   
...
//-------------------------------------------------------------------------
/*lint -save -e613 */
static _GfxSwCBCRBlockTo1CBCRRaster(const MI_DIF_FIELD_T *prsGfxReg)
{
    UINT8 *pu1SrcBase, *pu1DstBase, *pu1CBCRBase;
    UINT32 u4SrcPitch, u4DstPitch, u4CBCRPitch, u4WritePitchEn;
    UINT32 u4SrcCM, u4DstCM;
    UINT32 u4Color;
    UINT32 u4Width, u4Height;
    UINT32 u4Vfull, u4SwapMode, u4CBCROrder;
  ...
  //  u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
    ...
    u4Vfull =   prsGfxReg->fg_VFULL;
    u4SwapMode =   prsGfxReg->fg_SWAP_MODE;
    u4CBCROrder = prsGfxReg->fg_WRITE_CBCR_ORDER;
    ...
    pu1DstBase = VIRTUAL(pu1DstBase);
    // virtual address
    pu1CBCRBase = VIRTUAL(pu1CBCRBase);

    return GfxSwCBCRBlockTo1CBCRRaster(pu1SrcBase, pu1DstBase, u4SrcPitch, u4DstPitch,
            u4SrcCM, u4DstCM, u4Width, u4Height, pu1CBCRBase, u4CBCRPitch,
           u4Vfull, u4SwapMode, u4WritePitchEn, u4CBCROrder);   
...
//-------------------------------------------------------------------------
/*lint -save -e613 */
static _GfxSwCBCRBlockTo2CBCRRaster(const MI_DIF_FIELD_T *prsGfxReg)
{
    UINT8 *pu1SrcBase, *pu1DstBase, *pu1CBCRBase;
    UINT32 u4SrcPitch, u4DstPitch, u4CBCRPitch, u4WritePitchEn;
    UINT32 u4SrcCM, u4DstCM;
  ...
  //  u4Height = prsGfxReg->fg_SRC_HEIGHT + 1;
    ...
    return GfxSwCBCRBlockTo2CBCRRaster(pu1SrcBase, pu1DstBase, u4SrcPitch, u4DstPitch,
            u4SrcCM, u4DstCM, u4Width, u4Height, pu1CBCRBase, u4CBCRPitch,
           u4Vfull, u4SwapMode, u4WritePitchEn, u4CBCROrder);   
...
void GFX_SwAction(void)
{
    ...
    switch (prGfxReg->fg_OP_MODE)
    {
        ...
        case OP_YBLOCK_TO_YRASTER:
        i4Ret = _GfxSwYBlockToYRaster(prGfxReg);
        break;
        
        case OP_CBCRBLOCK_TO_1CBCRRASTER:
        i4Ret = _GfxSwCBCRBlockTo1CBCRRaster(prGfxReg);
        break;
        
        case OP_CBCRBLOCK_TO_2CBCRRASTER:
        i4Ret = _GfxSwCBCRBlockTo2CBCRRaster(prGfxReg);
        break;
...
//-------------------------------------------------------------------------
/*lint -save -e613 -e740 */
INT32 GfxSwYBlockToYRaste(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, UINT32 u4SrcPitch, UINT32 u4DstPitch,
            UINT32 u4SrcCM, UINT32 u4DstCM, UINT32 u4Width, UINT32 u4Height, UINT32 pu1CBCRBase, UINT32 u4CBCRPitch,
            UINT32 u4Vfull, UINT32 u4SwapMode, UINT32 u4WritePitchEn)
{
    INT32 ai4SrcColorComp[4] =
    {   0};
    INT32 ai4DstColorComp[4] =
    ...
    u4SrcCM = CM_RGB_CLUT8;
    u4DstCM = CM_RGB_CLUT8;
...
//-------------------------------------------------------------------------
/** GFX_SwGetARGBOrder
 *  change read argb order
 */
//-------------------------------------------------------------------------
/*lint -save -e613 -e740 */
INT32 GFX_SwGetARGBOrder(UINT32 u4Aorder, UINT32 u4Rorder, UINT32 u4Gorder, UINT32 u4Border, INT32 * ai4SrcColorComp)
{   
    INT32 i4Q, i4W, i4E, i4R;
    ...
    {
        case 0:
            i4Q = ai4SrcColorComp[3]; //A
            break;
        case 1:
            i4Q = ai4SrcColorComp[2];
            break;
        case 2: 
            i4Q = ai4SrcColorComp[1];
            break;
        case 3:
            i4Q = ai4SrcColorComp[0];
            break;
    }
    ...
    {
        case 0:
            i4W = ai4SrcColorComp[3];
            break;
        case 1:
            i4W = ai4SrcColorComp[2];
            break;
        case 2: 
            i4W = ai4SrcColorComp[1];
            break;
        case 3:
            i4W = ai4SrcColorComp[0];
            break;
    }
    ...
    {
        case 0:
            i4E = ai4SrcColorComp[3];
            break;
        case 1:
            i4E = ai4SrcColorComp[2];
            break;
        case 2: 
            i4E = ai4SrcColorComp[1];
            break;
        case 3:
            i4E = ai4SrcColorComp[0];
            break;
    }
    ...
    {
        case 0:
            i4R = ai4SrcColorComp[3];
            break;
        case 1:
            i4R = ai4SrcColorComp[2];
            break;
        case 2: 
            i4R = ai4SrcColorComp[1];
            break;
        case 3:
            i4R = ai4SrcColorComp[0];
            break;
    }
    //chose 1,3
    ai4SrcColorComp[0] = i4Q; //A
    ai4SrcColorComp[1] = i4W; //R
    ai4SrcColorComp[2] = i4E; //G
    ai4SrcColorComp[3] = i4R; //B
...
//-------------------------------------------------------------------------
/*lint -save -e613 -e740 */
INT32 GfxSwCBCRBlockTo1CBCRRaster(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, UINT32 u4SrcPitch, UINT32 u4DstPitch,
            UINT32 u4SrcCM, UINT32 u4DstCM, UINT32 u4Width, UINT32 u4Height, UINT32 pu1CBCRBase, UINT32 u4CBCRPitch,
            UINT32 u4Vfull, UINT32 u4SwapMode, UINT32 u4WritePitchEn, UINT32 u4CBCROrder)
{
    INT32 ai4SrcColorComp[4] =
    {   0};
    INT32 ai4DstColorComp[4] =
    ...
    for (y = 0; y < u4Height/2; y++)
    {
        ...
        for (x = 0; x < u4Width/4; x++) //cbcr use cm_8888 to read.
        {
            GFX_SwGetColorComponent(&pu1SrcRead, CM_ARGB8888_DIRECT32, ai4SrcColorComp);
            if(u4CBCROrder && (u4DstCM == CM_ARGB8888_DIRECT32))
            {
                GFX_SwGetARGBOrder(R_ORDER, A_ORDER, B_ORDER, G_ORDER, ai4SrcColorComp);
            }
         
            GFX_SwSetColorComponent(&pu1DstWrite, CM_ARGB8888_DIRECT32, ai4SrcColorComp);
        } // ~for
...
//-------------------------------------------------------------------------
/*lint -save -e613 -e740 */
INT32 GfxSwCBCRBlockTo2CBCRRaster(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, UINT32 u4SrcPitch, UINT32 u4DstPitch,
            UINT32 u4SrcCM, UINT32 u4DstCM, UINT32 u4Width, UINT32 u4Height, UINT32 pu1CBCRBase, UINT32 u4CBCRPitch,
            UINT32 u4Vfull, UINT32 u4SwapMode, UINT32 u4WritePitchEn, UINT32 u4CBCROrder)
{
    INT32 ai4SrcColorComp[4] =
    ...
    UINT32 *pu4Color;
    UINT32 x, y;
    UINT8 *pu1SrcLine, *pu1DstLine, *pu1DstLine1 ;
    UINT8 *pu1SrcRead, *pu1DstRead, *pu1DstWrite, *pu1DstWrite1;

    pu1SrcLine = pu1CBCRBase;
    pu1DstLine = pu1DstBase; //cb dst
    pu1DstLine1 = pu1SrcBase; //cr dst
    
    for (y = 0; y < u4Height/2; y++)
    {
        pu1SrcRead = pu1SrcLine;
        pu1DstWrite = pu1DstLine;
        pu1DstWrite1 = pu1DstLine1;

        for (x = 0; x < u4Width/2; x++)
        {
            GFX_SwGetColorComponent(&pu1SrcRead, CM_ARGB4444_DIRECT16, ai4SrcColorComp);
            
            ai4DstColorComp[3] = (ai4SrcColorComp[2] & 0xf0) |  (ai4SrcColorComp[3] & 0x0f);
            GFX_SwSetColorComponent(&pu1DstWrite, CM_RGB_CLUT8, ai4DstColorComp);
            ai4DstColorComp[3] = (ai4SrcColorComp[0] & 0xf0) |  (ai4SrcColorComp[1] & 0x0f);
            GFX_SwSetColorComponent(&pu1DstWrite1, CM_RGB_CLUT8, ai4DstColorComp);
            
        } // ~for

        if(u4WritePitchEn)
        {
            pu1DstLine += u4Width/2;
            pu1DstLine1 += u4Width/2;
        }
        else
        {
            pu1DstLine += u4DstPitch;
            pu1DstLine1 += u4DstPitch;
        }
...
//-------------------------------------------------------------------------
/*lint -save -e613 -e740 */
INT32 GFX_SwYUYVToRaster(UINT8 *pu1SrcBase, UINT8 *pu1DstBase, UINT32 u4SrcPitch,
        UINT32 u4DstPitch, UINT32 u4SrcCM, UINT32 u4DstCM, UINT32 u4Width,
        UINT32 u4Height, UINT32 u4wrAorder, UINT32 u4wrRorder, UINT32 u4wrGorder,UINT32 u4wrBorder)
{
    INT32 ai4SrcColorComp[4] =
    {   0};
    INT32 ai4DstColorComp[4] =
    ...
    for (y = 0; y < u4Height; y++)
    {
        ...
        for (x = 0; x < u4Width; x++)
        {
            ...
            GFX_SwGetARGBOrder(u4wrAorder, u4wrRorder, u4wrGorder, u4wrBorder, ai4SrcColorComp);
            
            //chose 1,3
            i4Q =  ai4SrcColorComp[0] ; //A
            i4W =  ai4SrcColorComp[1] ; //R
            i4E =  ai4SrcColorComp[2] ; //G
            i4R =  ai4SrcColorComp[3] ; //B
            
            {
                    ...
                    *((UINT16 *)pu1DstWrite) = u2Color;
                    pu1DstWrite += 2;
            }

         //   GFX_SwSetColorComponent(&pu1DstWrite, u4DstCM, ai4DstColorComp);
            
        } // ~for

        pu1SrcLine += u4SrcPitch;
        
        //do not have dst pitch
        pu1DstLine += u4Width*2 ;
