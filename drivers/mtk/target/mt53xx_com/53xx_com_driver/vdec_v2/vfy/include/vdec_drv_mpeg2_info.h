...
typedef struct _MPEG2_STEREO_VID_FORMAT_SIGNAL_FIELD_T
{
	UINT32								:   16; //reserved   fixed 0x04ff
	UINT32		u4StereoVFormatSigType  :   7;  //stereo video format signaling type  3--3D(side by side),8--2D,other--reserved  default  8(2D)   bslbf
	UINT32								:   1;  //reserved   fixed 1
	UINT32      u4StereoVFormatSigLen   :   8;  //stereo video format signaling length   uimsbf
}MPEG2_STEREO_VID_FORMAT_SIGNAL_FIELD_T;
typedef union _MPEG2_STEREO_VID_FORMAT_SIGNAL_UNION_T
{
	UINT32									au4Reg[1];
	MPEG2_STEREO_VID_FORMAT_SIGNAL_FIELD_T  rField;
}MPEG2_STEREO_VID_FORMAT_SIGNAL_UNION_T;
...
typedef struct _MEPG2_DEC_PRM_T
{
    ...
    UINT16 ucOverSpecNtfyCnt;   // skip not support profile_level
...
#endif
	MPEG2_STEREO_VID_FORMAT_SIGNAL_UNION_T rStereoVFormatSig; //stereo video format signaling type
} MEPG2_DRV_INFO_T;
