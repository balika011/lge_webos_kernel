...
extern int vVP8DumpMem( UINT32 u4InstID, unsigned char* buf, unsigned int size , int  frame_num , unsigned int type , bool isTimeout);
...
static unsigned long long lSavedBitCnt[MAX_VP8_DATAPARTITION];
static BOOL  fgNewPartionStart = FALSE;
static BOOL fgLastPartionNumOneGroup = FALSE;


//static VOID vVdecWebpHWWebpReset(VDEC_WEBP_BUF_INFO_T *prBufInfo,BOOL fgDecStarted)
static VOID vVdecWebpHWWebpReset(UINT32 u4InstID,UINT32 u4CurCoeffRtprBase,UINT32 u4CurParNum)
{
    VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr = &_rVDecVp8FrmHdr[u4InstID];
    UINT32 u4CurBitCnt=0, dShiftBit= 0;
    UINT32 u4NextParNum = u4CurParNum+1;
    unsigned long long lNextBitCnt=0;

    u4CurBitCnt = u4VDecReadVP8VLD(0, RO_VP8_PART_BITCNT);
    #if 1
    UTIL_Printf("Ring_Fifo: vp8_vld_79= 0x%08x\n", u4CurBitCnt);
    UTIL_Printf("Ring_Fifo: coeff_rtpr_base= 0x%08x\n", u4CurCoeffRtprBase);
    #endif

    if (fgLastPartionNumOneGroup)
    {
        lNextBitCnt = lSavedBitCnt[prVDecVp8FrmHdr->rVp8DecParam.uDataPartitionNum-1-u4CurParNum];
        fgLastPartionNumOneGroup = FALSE;
        fgNewPartionStart  = TRUE;
        //UTIL_Printf("LastPartionNumOneGroup is TRUE; Part Num %d, lNextBitCnt is %0x%x, uDataPartitionNum is 0x%x\n", u4CurParNum, lNextBitCnt, prVDecVp8FrmHdr->rVp8DecParam.uDataPartitionNum);
    }
    else if (fgNewPartionStart)
    {
        lNextBitCnt = lSavedBitCnt[u4CurParNum+1];
        //UTIL_Printf("NewPartionStart is TRUE: Part Num %d\n", u4CurParNum);
    }
    else
    {
        lNextBitCnt = (u4VDecReadVP8VLD(u4InstID, RW_VP8_COEFR0+u4NextParNum*4));
		//UTIL_Printf("Ring_Fifo: next_bit_cnt= 0x%llx\n", lNextBitCnt);
		lNextBitCnt = lNextBitCnt<<3;
		//UTIL_Printf("Ring_Fifo: next_bit_cnt= 0x%llx\n", lNextBitCnt);
    }

	lSavedBitCnt[u4CurParNum] = u4CurCoeffRtprBase;
	
    lSavedBitCnt[u4CurParNum] = (lSavedBitCnt[u4CurParNum]<<3)+u4CurBitCnt;//The next same partition start position
    #if 1
    UTIL_Printf("Ring_Fifo: saved_bit_cnt= 0x%llx\n",lSavedBitCnt[u4CurParNum]);
    UTIL_Printf("Ring_Fifo: next_bit_cnt= 0x%llx\n", lNextBitCnt);
    UTIL_Printf("Ring_Fifo, CurParNum: %d, lNextBitCnt>>3: 0x%llx,\n", u4CurParNum,lNextBitCnt>>3);
   #endif
    //vVDEC_HAL_VP8_VldReset(VP8_VLD2);
    vVDecWriteVP8VLD(u4InstID, RW_VP8_STR, 0x100, 0);
    vVDecWriteVP8VLD(u4InstID, RW_VP8_STR, 0x0, 0);
    vVDecWriteVP8VLD2(u4InstID, RW_VLD_RDY_SWTICH, READY_TO_RISC_1);

    UTIL_Printf("Start Init_Barrel_Shifter_Sec\n");
    #if 1
    vVDecWriteVP8VLD2(u4InstID, RW_VLD_VSTART, ((PHYSICAL(prVDecVp8FrmHdr->u4FifoStart))>> 6));
    vVDecWriteVP8VLD2(u4InstID, RW_VLD_VEND, ((PHYSICAL(prVDecVp8FrmHdr->u4FifoEnd))>> 6));
    #else
    vVDecVP8WriteVLD2(u4InstID, RW_VLD_VSTART, ((PHYSICAL(prBufInfo->u4Start))>> 6));
    vVDecVP8WriteVLD2(u4InstID, RW_VLD_VEND, ((PHYSICAL(prBufInfo->u4End))>> 6));
    #endif

    #if 1
    while(!_VDEC_VP8_InitBarrelShifter_Webp(VP8_VLD2,
                 (UINT32)(lNextBitCnt>>3), prVDecVp8FrmHdr->u4WritePos))
    #else
    while(!_VDEC_VP8_InitBarrelShifter_Webp(VP8_VLD2,
                 prBufInfo->u4ReadPos, prBufInfo->u4RealWritePos))
    #endif
    {
      UTIL_Printf("<Webp>HWreset WebP Init VLD2 BarrelShifter fail\n");
      vVDEC_VP8_VldReset(VP8_VLD2);
    }
    //End Init Barrel shifter
#if 1
    UTIL_Printf("End INIT_BARREL_SHIFT_SEC\n");
    UTIL_Printf("Ring_Fifo,input_window[2]: 0x%x, \n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR1));
    UTIL_Printf("Ring_Fifo,input_window_sec after init_barrel: 0x%x, \n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR2));
#endif
   dShiftBit= ((lNextBitCnt-((u4VDecReadVLD(u4InstID,RW_VLD_RPTR)/*+0xc*/)<<3))&0x7);
   //UTIL_Printf("shift %ld bits\n", dShiftBit);
   u4VDecVP8VLDGetBits(VP8_VLD2, u4InstID, dShiftBit);
#if 0
     UTIL_Printf("Ring_Fifo,input_window_sec after shift: 0x%x, \n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR2));
#endif
   vVDecWriteVP8VLD(u4InstID, RW_VP8_DEC_RESUME,0x1, 0);
...
void vVDEC_Vp8DumpReg(void)
{
    ...
    UTIL_Printf("VLD1 ReadPtr: 0x%08x\n", u4VDecReadVLD(0, RW_VLD_RPTR));
    UTIL_Printf("VLD2 ReadPtr: 0x%08x\n", u4VDecReadVP8VLD2(0, RW_VLD_RPTR));
    
    /* VP8 VLD */
    UTIL_Printf("******* VP8 VLD Registers *******\n");
    for (i = 41; i < 68; i++)
    {
        UTIL_Printf("VP8 VLD %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVP8VLD(0,i * 4));
    }
    ...
    /* TOP VLD */
    UTIL_Printf("******* TOP VLD Registers *******\n");
    for (i = 0; i < 100; i++)
    {
        UTIL_Printf("TOP VLD %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVLDTOP(0, i * 4));
    }
    ...
    for (i = 0; i < 735; i++)
    {
        UTIL_Printf("MC %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadMC(0, i * 4));
    }
    
    /* MV */
    UTIL_Printf("******* MV Registers *******\n");
    UTIL_Printf("MV 181 (0x0x02D4): 0x%08x\n", u4VDecReadVP8MV(0, i * 4));

    
    /* PP */
    UTIL_Printf("******* PP Registers *******\n");
    for (i = 0; i < 600; i++)
    {
        UTIL_Printf("PP %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVP8PP(0, i * 4));
    }
	/*Misc base */
	UTIL_Printf("******* Misc Registers *******\n");
    for (i = 0; i < 128; i++)
    {
        UTIL_Printf("DV %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadMISC(0, i * 4));
    }

	/*VLD base */
	for (i = 33; i < 253; i++)
    {
        UTIL_Printf("VLD %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVLD(0, i * 4));
    }

	UTIL_Printf("******* End dump Registers *******\n");
#endif
}
...
//    VDEC_INFO_VP8_VFIFO_PRM_T rVp8VFifoInitPrm;
   ...
   #if VDEC_SUPPORT_POWER_SAVING
   vVDEC_HAL_PowerSave(u4InstID, VDEC_FMT_VP8);
   #endif
...
{             
    ...
    rVp8VDecInitPrm.u4VFifoEa = (UINT32)_pucVFifo[u4InstID] + V_FIFO_SZ;
...
//    i4VDEC_HAL_VP8_InitVDecHW(u4InstID, &rVp8VDecInitPrm);
    ...
    prVDecVp8FrmHdr->u4FifoEnd = (UINT32)_pucVFifo[u4InstID] + V_FIFO_SZ;
...
INT32 i4VP8_Parse_Frame_Header(UINT32 u4BsId, UINT32 u4InstID, VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr)
{
  ...
  x_memcpy(&prVDecVp8FrmHdrPhysical,prVDecVp8FrmHdr,sizeof(VDEC_INFO_VP8_FRM_HDR_T));
  #if VDEC_SUPPORT_IOMMU
   prVDecVp8FrmHdrPhysical.u4FifoStart = (prVDecVp8FrmHdr->u4FifoStart);
   prVDecVp8FrmHdrPhysical.u4FifoEnd = (prVDecVp8FrmHdr->u4FifoEnd);
   prVDecVp8FrmHdrPhysical.u4VldStartPos = (prVDecVp8FrmHdr->u4VldStartPos);
   prVDecVp8FrmHdrPhysical.u4WritePos = (prVDecVp8FrmHdr->u4WritePos);
  #else
  ...
  {
      UTIL_Printf("Frame Type %d, I\n", _u4FileCnt[u4InstID]);
  }
  else
  {
      UTIL_Printf("Frame Type %d, P\n", _u4FileCnt[u4InstID]);
  }
#endif
  if(prVDecVp8FrmHdr->uFrameType==VP8_I_FRM)
  {
     ...
     {
         UTIL_Printf("VP8 Header Identify error (0x%x)\n",u4HalValue);
         return FALSE;
     }
     ...
     {
        UTIL_Printf("VP8 Header W/H error \n");
        return FALSE;
     }
     u4ReadOffset+=7;
  ...
  #if VDEC_SUPPORT_IOMMU
  prVDecVp8FrmHdr->u4VldStartPos = (prVDecVp8FrmHdrPhysical.u4VldStartPos);
  prVDecVp8FrmHdr->u4FifoStart = (prVDecVp8FrmHdrPhysical.u4FifoStart);
  prVDecVp8FrmHdr->u4FifoEnd = (prVDecVp8FrmHdrPhysical.u4FifoEnd);
  #else
  ...
  if(prVDecVp8FrmHdr->uFrameType==VP8_I_FRM)
  {
    ...
    x_memcpy(prVDecVp8FrmHdr->rVp8DecParam.rCurFc.CoefProbs,default_coef_probs,sizeof(default_coef_probs));
    ...
    {
      UTIL_Printf("VP8 Header Color Space unsupport\n");
      return FALSE;
    }
  }
...
//  UTIL_Printf("eLoopFilterType is %d, iLoopFilterLvl is %d, iSharpLvl is %d\n", prVp8DecParam->eLoopFilterType, prVp8DecParam->iLoopFilterLvl,  prVp8DecParam->iSharpLvl);
  ...
  {
      UTIL_Printf("VP8 Init BarrelShifter fail\n");
      vVDEC_VP8_VldReset(VP8_VLD2);
  }
...
/*    x_memcpy(_pucWorkYBuf[u4InstID],&prVDecVp8FrmHdr->u4CurYAddr,sizeof(UINT32));
    x_memcpy(_pucDumpLstYBuf[u4InstID],&prVDecVp8FrmHdr->u4LstYAddr,sizeof(UINT32));
    x_memcpy(_pucDumpGldYBuf[u4InstID],&prVDecVp8FrmHdr->u4GldYAddr,sizeof(UINT32));
    x_memcpy(_pucDumpArfYBuf[u4InstID],&prVDecVp8FrmHdr->u4AlfYAddr,sizeof(UINT32));
*/
#if VDEC_VP8_WEBP_SUPPORT
...
//    VDEC_INFO_DEC_PRM_T *prDecPrm = &_tVerMpvDecPrm[u4InstID];
    ...
    x_memcpy(&_rVDecVp8FrmHdrAddr[u4InstID],prVDecVp8FrmHdr,sizeof(VDEC_INFO_VP8_FRM_HDR_T));
    prVDecVP8DecPrm->prFrmHdr = &_rVDecVp8FrmHdrAddr[u4InstID];
    _fgFirstMBRow = TRUE;
    #if VP8_MULTI_RING_BUFFER
    fgLastPartionNumOneGroup = FALSE;
    fgNewPartionStart = FALSE;
    #endif
    
    prVDecVP8DecPrm->prFrmHdr->u4CurCAddr = prVDecVP8DecPrm->prFrmHdr->u4CurYAddr +PIC_Y_SZ;

    x_memset((UCHAR *)prVDecVP8DecPrm->prFrmHdr->u4CurYAddr, 0x0, PIC_Y_SZ);
    x_memset((UCHAR *)prVDecVP8DecPrm->prFrmHdr->u4CurCAddr, 0x0, PIC_C_SZ);
#if VDEC_VP8_WEBP_SUPPORT
    x_memset((UCHAR *)prVDecVP8DecPrm->prFrmHdr->u4ImgReszY, 0x0, PIC_Y_SZ);
    x_memset((UCHAR *)prVDecVP8DecPrm->prFrmHdr->u4ImgReszC, 0x0, PIC_C_SZ);
#endif

    HalInvalidateDCacheMultipleLine(prVDecVP8DecPrm->prFrmHdr->u4CurYAddr, PIC_Y_SZ);
	HalInvalidateDCacheMultipleLine(prVDecVP8DecPrm->prFrmHdr->u4CurCAddr, PIC_C_SZ);
#if VDEC_VP8_WEBP_SUPPORT
    HalInvalidateDCacheMultipleLine(prVDecVP8DecPrm->prFrmHdr->u4ImgReszY, PIC_Y_SZ);
    HalInvalidateDCacheMultipleLine(prVDecVP8DecPrm->prFrmHdr->u4ImgReszC, PIC_C_SZ);
#endif
#if VDEC_SUPPORT_YC_SEPARATE
    prVDecVP8DecPrm->prFrmHdr->u4GldCAddr = prVDecVP8DecPrm->prFrmHdr->u4GldYAddr +PIC_Y_SZ;
    prVDecVP8DecPrm->prFrmHdr->u4AlfCAddr = prVDecVP8DecPrm->prFrmHdr->u4AlfYAddr +PIC_Y_SZ;
	prVDecVP8DecPrm->prFrmHdr->u4LstCAddr = prVDecVP8DecPrm->prFrmHdr->u4LstYAddr +PIC_Y_SZ;
...
#if 1
    {
        ...
        UTIL_Printf("# %d , Input Window 1 is 0x%x, \n", _u4FileCnt[u4InstID], u4InputWindow);

		u4InputWindow = u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR2);

		UTIL_Printf("# %d , Input Window 2 is 0x%x, \n", _u4FileCnt[u4InstID], u4InputWindow);
    }

#endif
		
    i4VDEC_HAL_VP8_DecStart(u4InstID, prVDecVP8DecPrm->prFrmHdr);
    x_memcpy(prVDecVp8FrmHdr,prVDecVP8DecPrm->prFrmHdr,sizeof(VDEC_INFO_VP8_FRM_HDR_T));
...
//Searh path name
    ...
    //UTIL_Printf("u4HWCRC_Y0 is 0x%lx, u4HWCRC_C3 is 0x%lx \n", u4HWCRC_Y0, u4HWCRC_C3);
      ...
      )
    {
       fgDecErr = TRUE;
	   UTIL_Printf("u4CRCValueY0 is 0x%lx, u4CRCValueC3 is 0x%lx \n", u4CRCValueY0, u4CRCValueC3);
	   UTIL_Printf("u4CRCValueY0 is 0x%lx, u4CRCValueY1 is 0x%x, u4CRCValueY2 is 0x%x, u4CRCValueY3 is 0x%x, u4CRCValueC0 is 0x%x, u4CRCValueC1 is 0x%x, u4CRCValueC2 is 0x%x, u4CRCValueC3 is 0x%lx \n", \
	   	            u4CRCValueY0, u4CRCValueY1,u4CRCValueY2, u4CRCValueY3, u4CRCValueC0, u4CRCValueC1, u4CRCValueC2, u4CRCValueC3);
       UTIL_Printf("u4HWCRC_Y0 is 0x%lx, u4HWCRC_Y1 is 0x%x, u4HWCRC_Y2 is 0x%x, u4HWCRC_Y3 is 0x%x, u4HWCRC_C0 is 0x%x, u4HWCRC_C1 is 0x%x, u4HWCRC_C2 is 0x%x, u4HWCRC_C3 is 0x%lx \n", \
    ...
    if((_u4FileCnt[u4InstID]%50) == 0 || _fgVP8DumpReg)
    {
        vVDecOutputDebugString("Pic count to [%d]\n", _u4FileCnt[u4InstID]); 
        sprintf(strMessage,"[VP8 Pic CRC count %d], \n", _u4FileCnt[u4InstID]);  
    ...
    UTIL_Printf("Get VP8 time duration, %d, %d \n", _u4FileCnt[u4InstID], rTimeDecDuration[u4InstID].u4Micros);
    ...
    if(fgCompare)
    {
        ...
        #endif    
        {
         ...
         //   u4NonSwapYBase = (UINT32)_pucWorkYBuf[u4InstID];
           ...
           UTIL_Printf("VP8 ImgReszY (0x%08x), ImgReszC (0x%08x)\n",prVDecVp8FrmHdr->u4ImgReszY, prVDecVp8FrmHdr->u4ImgReszC);
           UTIL_Printf("VP8 VdecY (0x%08x), VdecC (0x%08x)\n",prVDecVp8FrmHdr->u4CurYAddr, prVDecVp8FrmHdr->u4CurYAddr+ PIC_Y_SZ);
            #else
            ...
            UTIL_Printf("TRC-vVP8WrData2PC +2.4.1, u4NonSwapYLengthBase=0x%x, u4NonSwapCLengthBase=0x%x\n", u4NonSwapYLengthBase, u4NonSwapCLengthBase);
            UTIL_Printf("TRC-vVP8WrData2PC +2.4.2, u4PIC_SIZE_BS=%d, u4UFO_LEN_SIZE_Y=%d, u4PIC_SIZE_Y_BS=%d\n",
...
//		HalFlushInvalidateDCache();
 ...
 //               Printf("MPV emu VP8 inverse swap mode %d\n", _tVerMpvDecPrm[u4InstID].ucAddrSwapMode);
  ...
  //      BSP_dma_unmap_single((UINT32)_pu1VP8VDecYGoldPhy, GOLD_Y_SZ, FROM_DEVICE);
        ...
        #else
        HalInvalidateDCacheMultipleLine(prVDecVp8FrmHdr->u4CurYAddr, PIC_Y_SZ);
        HalInvalidateDCacheMultipleLine(prVDecVp8FrmHdr->u4CurCAddr, PIC_C_SZ);
        #if VDEC_VP8_WEBP_SUPPORT
        HalInvalidateDCacheMultipleLine(prVDecVp8FrmHdr->u4ImgReszY, PIC_Y_SZ);
        HalInvalidateDCacheMultipleLine(prVDecVp8FrmHdr->u4ImgReszC, PIC_C_SZ);
        #endif
        #if 0
    	{
	        ...
	        UTIL_Printf("Before imageResize VP8 Y Data\n");
			ucYCBase = (UCHAR *)prVDecVp8FrmHdr->u4CurYAddr;
       	    for(i=0; i < 176*144 ; i=i+16)
       	    {
				a = *(UINT8 *)(ucYCBase + i);
			...
			ucYCBase = (UCHAR *)prVDecVp8FrmHdr->u4CurCAddr;
            ...
            UTIL_Printf("After imageResize VP8 Y Data\n");
			ucYCBase = (UCHAR *)prVDecVp8FrmHdr->u4ImgReszY;
       	    for(i=0; i < 176*144 ; i=i+16)
       	    {
				a = *(UINT8 *)(ucYCBase + i);
				b = *(UINT8 *)(ucYCBase + i + 1);
			...
			ucYCBase = (UCHAR *)prVDecVp8FrmHdr->u4ImgReszC;
		...
		UTIL_Printf("[VP8]u4YBase is 0x%lx, width is %d, height is %d\n",u4YBase, u4Width, u4Height);
        #endif  
  ...
  //      BSP_dma_unmap_single((UINT32)_pu1VP8VDecCGoldPhy, GOLD_C_SZ, FROM_DEVICE);
        ...
        _tFBufFileInfo[u4InstID].u4TargetSz = GOLD_Y_SZ;
        _tFBufFileInfo[u4InstID].u4FileLength = 0;
        vConcateStr(_bFileStr1[u4InstID][5], _bPatternPath, _bFileAddStrYLen, _u4FileCnt[u4InstID]);

        printk("TRC-vVP8WrData2PC +6.1, _bFileStr1[%d][5]=%s\n", u4InstID, _bFileStr1[u4InstID][5]);

#ifdef DIRECT_DEC
        if (_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID] && _u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID])
#endif
        {
            fgOpenFile(u4InstID, _bFileStr1[u4InstID][5],"r+b", &_tFBufFileInfo[u4InstID]);
            _u4GoldenYSize[u4InstID] = _tFBufFileInfo[u4InstID].u4RealGetBytes;

            {
                ...
                printk("\n<========= Y LEN compare end =====>\n");
				printk("Golden Y Length Size is 0x%d, u4CCompare size if 0x%d\n",_u4GoldenYSize[u4InstID],u4YCompare );
            }
            ...
            if (FALSE == fgDecErr) {
                printk("Y Length data compare OK\n");
            }
        }
        ...
        _tFBufFileInfo[u4InstID].u4TargetSz = GOLD_C_SZ;
        _tFBufFileInfo[u4InstID].u4FileLength = 0;
        vConcateStr(_bFileStr1[u4InstID][6], _bPatternPath, _bFileAddStrCLen, _u4FileCnt[u4InstID]);

        printk("TRC-vVP8WrData2PC +6.2, _bFileStr1[%d][6]=%s\n", u4InstID, _bFileStr1[u4InstID][6]);

#ifdef DIRECT_DEC
        if (_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID] && _u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID])
#endif
        {
            fgOpenFile(u4InstID, _bFileStr1[u4InstID][6],"r+b", &_tFBufFileInfo[u4InstID]);
            _u4GoldenCSize[u4InstID] = _tFBufFileInfo[u4InstID].u4RealGetBytes;
            {
                ...
                printk("\n<========= C LEN compare end =====>\n");
				printk("Golden C Length Size is %d, u4CCompare size if 0x%d\n",_u4GoldenCSize[u4InstID],u4CCompare );
            }
            ...
            for (u4Idx = 0; u4Idx < _u4GoldenCSize[u4InstID] >> 2; u4Idx++) {
                ...
                if (pu4GoldenBuf[u4Idx] != pu4WorkBuf[u4Idx]) {
                    u4Cnt++;
                    sprintf(strMessage,"C Length Data Mismatch at [%d] = 0x%X, Golden = 0x%X , u4Cnt is %d!!\n", u4Idx, pu4WorkBuf[u4Idx],pu4GoldenBuf[u4Idx],u4Cnt);
                    printk("%s",strMessage);
                    fgDecErr = TRUE;
                    //vVDEC_HAL_H264_VDec_DumpReg(u4InstID);  // mark by ginny
                    break;
            ...
            if (FALSE == fgDecErr) {
                printk("C Length data compare OK\n");
            }
        }
#endif
    ...
    #endif
    {
        ...
        vVDecOutputDebugString("Pic count to [%d]\n", _u4FileCnt[u4InstID]);  
        #endif
        sprintf(strMessage,"[Success pic count %d], \n", _u4FileCnt[u4InstID]); 
        fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tFileListRecInfo[u4InstID]);
    }
    #endif

	#if VDEC_VP8_WEBP_SUPPORT //For WEBP picure mode
	UTIL_Printf("[Success pic count]\n");
	#endif
    #ifdef REDEC    
       if(_u4ReDecCnt[u4InstID] == 0)
    #endif    
...
//Searh path name
    ...
    fgDecErr = FALSE;
        ...
        // Y decoded data Compare   
         vConcateStr(_bFileStr1[u4InstID][3], _bPatternPath, _bFileAddStrY, _u4FileCnt[u4InstID]);
        _pu1VP8VDecYGoldPhy = (UINT8 *)BSP_dma_map_single((UINT32)_pucDumpYBuf[u4InstID], GOLD_Y_SZ, TO_DEVICE);
        ...
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYGoldPhy, GOLD_Y_SZ, TO_DEVICE);
        ...
        _pu1VP8VDecCGoldPhy = (UINT8 *)BSP_dma_map_single((UINT32)_pucDumpCBuf[u4InstID], GOLD_C_SZ, TO_DEVICE);
        #ifdef DIRECT_DEC
        if((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID]) && ( _u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID]))
        #endif    
        {  
        ...
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecCGoldPhy, GOLD_C_SZ, TO_DEVICE);
...
void vVP8WEBPPixComp(UINT32 u4InstID)
{
	...
	UTIL_Printf("Compare Error==> MB Row Count to [%d] \n", _u4FileCnt[u4InstID] - 1);
    fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tFileListRecInfo[u4InstID]);
    }
    #endif
    if(_u4FileCnt[u4InstID] >= _u4EndCompPicNum[u4InstID])
...
#else
   vVP8WrData2PC(u4InstID, _pucDumpYBuf[u4InstID], ((((prVDecVp8FrmHdr->u4Width+ 15) >> 4) * ((prVDecVp8FrmHdr->u4Height + 31) >> 5)) << 9),  &fgNextFrameExist);
#endif
...
//#endif
  ...
  #else
//    rVp8BSInitPrm.u4WritePos = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
    rVp8BSInitPrm.u4WritePos = (UINT32)_pucVFifo[u4InstID] + ((_u4LoadBitstreamCnt[u4InstID]%2)?(V_FIFO_SZ):(V_FIFO_SZ>>1));
  #endif
    i4VDEC_HAL_VP8_InitBarrelShifter(0, u4InstID, &rVp8BSInitPrm);  
  ...
  #ifdef SEMI_RING_FIFO
  _tInFileInfo[u4InstID].u4BSLenConsumed = _u4VP8ByteCount[u4InstID];
  #endif
...
//    rVp8BSInitPrm.u4WritePos = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
    rVp8BSInitPrm.u4WritePos = (UINT32)_pucVFifo[u4InstID] + ((_u4LoadBitstreamCnt[u4InstID]%2)?(V_FIFO_SZ):(V_FIFO_SZ>>1));
  #endif
//    i4VDEC_HAL_VP8_InitBarrelShifter(_u4BSID[u4InstID], u4InstID, &rVp8BSInitPrm);
  ...
  if ( (_u4LoadBitstreamCnt[u4InstID]&0x1) 
  	 && (rVp8BSInitPrm.u4ReadPointer  > ((UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ/2))) )
  {
...
BOOL fgIsVP8VDecComplete(UINT32 u4InstID)
{
  ...
  {
  	 //UTIL_Printf("_fgVDecComplete\n");
      vVDEC_HAL_VP8_GetMbxMby(u4InstID, &u4MbX, &u4MbY);
      #if 0 //VP8_MB_ROW_MODE_SUPPORT
      if ((u4VDecReadVP8VLD(0,RO_VP8_VOKR)>>8))// for mb row mode check decode done a mb row.
      	{
        UTIL_Printf("u4MbX = 0x%x, u4MbY = 0x%x\n", u4MbX, u4MbY);
		UTIL_Printf("Test00\n");
      	#if (CONFIG_DRV_VERIFY_SUPPORT)    
      	  BSP_dma_unmap_single((UINT32)_pu1VP8VDecYAddrPhy, PIC_Y_SZ+PIC_C_SZ, FROM_DEVICE);
      	#endif
		UTIL_Printf("Test01 , _testCnt is %d\n", _testCnt);
		HalInvalidateDCacheMultipleLine(prVDecVp8FrmHdr->u4CurYAddr, PIC_Y_SZ);
	    HalInvalidateDCacheMultipleLine(prVDecVp8FrmHdr->u4CurCAddr, PIC_C_SZ);

		UTIL_Printf("DV value is 0x%x\n",u4VDecReadDV(0, RW_VP8_CTRL));
		...
		UTIL_Printf("Test02\n");
      	...
      	_testCnt++;
      	#if (CONFIG_DRV_VERIFY_SUPPORT)    
      	  _pu1VP8VDecYAddrPhy = (UINT8 *)BSP_dma_map_single(prVDecVp8FrmHdr->u4CurYAddr, PIC_Y_SZ+PIC_C_SZ, FROM_DEVICE);
      	#endif
		UTIL_Printf("Test0\n");
...
// because get MbX and MbY only have 9 bits, so we only compare 9bits
      ...
      {
          #if 0 //VP8_MB_ROW_MODE_SUPPORT
          /// Left 2 MB Row need to compare.
      	  vVP8WEBPPixComp(u4InstID);
          if (prVDecVp8FrmHdr->u4Height > 16)
      	  {
      	      vVP8WEBPPixComp(u4InstID);
      	  }
          #endif
...
void vVerVP8DecEndProc(UINT32 u4InstID)
{
  ...
  #if VP8_MULTI_RING_BUFFER
  UINT32 u4Index = 0;
  UINT32 u4CurParNum= 0, u4CurCoeffRtprBase = 0;
  #if VP8_MB_ROW_MODE_SUPPORT
  UINT32 u4PicPitch = 0;
  UINT32 u4VdoPPOn = 0;
  ...
  while(u4CntTimeChk < DEC_RETRY_NUM) 
  {
    ...
    {        
        ...
        vVDEC_HAL_VP8_GetMbxMby(u4InstID, &u4MbX, &u4MbY);
        #if 0//log
        UTIL_Printf("Total Mbx = %d, Total Mby = %d\n", (u4AlignWidth+15)/16, (u4ALignHeight+15)/16);
        UTIL_Printf("Mbx = %d, Mby = %d\n", u4MbX, u4MbY);
        #endif
        #endif
          if(u4VDEC_HAL_VP8_VDec_ReadFinishFlag(u4InstID))
          {
            ...
            if (fgIsLastRow)
            {
                ...
                while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
                {
                    UTIL_Printf("22Wait img_resz on\n");
                }
                
                UTIL_Printf("********Last row end\n");
            }
            #endif
              BIM_ClearIrq(VECTOR_VDFUL);
              vVDecWriteVLDTOP(0, RW_VLD_OK_CLR, 1);
              vVDecWriteVLDTOP(0, RW_VLD_OK_CLR, 0);
              UTIL_Printf("@@@@Pic Decode Finish: vld_93 %d, vld_92 %d\n", 
                (u4VDecReadVP8VLD(0,RO_VP8_PFR)&1), (u4VDecReadVP8VLD(0,RO_VP8_VOKR)&1));
              break;
          }
          #if VP8_MB_ROW_MODE_SUPPORT
          else if((u4VDecReadVP8VLD(0,RO_VP8_VOKR)>>8)&0x1)//brc init
          {
            u4MbX_last = u4MbX;
            u4MbY_last = u4MbY;
            #if 0//Get Mby may be incorrect with MC_11
            vVDEC_HAL_VP8_GetMbxMby(u4InstID, &u4MbX, &u4MbY);
            #else
            ...
            #if 1//log
            UTIL_Printf("Total Mbx = %d, Total Mby = %d\n", (u4AlignWidth+15)/16, (u4ALignHeight+15)/16);
            UTIL_Printf("Mby = 0x%x, Toal Mby 0x%x\n", u4VDecReadVP8VLD(0,RO_VP8_XY)&0xFFFF, (u4ALignHeight+15)/16);
            UTIL_Printf("@@@@MCU row will decode:MB_Y 0x%x, Total MB_row 0x%x\n", u4MbY, (u4ALignHeight+15)/16);
            UTIL_Printf("brc init: vp8_vld_92: %d\n", u4VDecReadVP8VLD(u4InstID,RO_VP8_VOKR));
            UTIL_Printf("brc init: multi_token_partition: %d\n", (u4VDecReadVP8VLD(u4InstID,RW_VP8_HEADER3)>>12)&0x3);
            UTIL_Printf("Ring_Fifo, brc_int input_window: 0x%x\n", u4VDecReadVP8VLD(u4InstID, RO_VP8_VBSR1));
            #endif

			#if 1
			UTIL_Printf("Misc_41 Value is 0x%x\n", u4VDecReadDV(0, RW_VP8_CTRL));
            while(0 == ((u4VDecReadDV(0, RW_VP8_CTRL)>>16)&0x1))//misc_41[16]
            {
                UTIL_Printf("44Wait vdec_int done\n");
            }
			#endif
            ...
            while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
            {
                UTIL_Printf("55Wait img_resz on\n");
            }
            if (!_fgFirstMBRow)
            {
                while(0 == (u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)&0x1))//imgresz_done
                {
                    UTIL_Printf("66Wait ImgResz Done\n");
                }
            }
            ...
            {
                UTIL_Printf("Last MB Row\n");
                fgIsLastRow = TRUE;
                while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
                {
                    UTIL_Printf("Wait img_resz on\n");
                }
                while(0 == (u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)&0x1))//imgresz_done
                {
                    UTIL_Printf("Wait ImgResz Done\n");
                }
            }
           #if 0// print input window
           UTIL_Printf("Ring_Fifo, row_trig input_window: 0x%x\n", u4VDecVP8ReadVLD(u4InstID, RO_VLD_VP8_VBSR1));
           UTIL_Printf("row_trig_vdec input_window_sec: 0x%x\n", u4VDecVP8ReadVLD(u4InstID, RO_VLD_VP8_VBSR2));
           #endif
          ...
          #endif
          if ((u4VDecReadVP8VLD(0,RO_VP8_VOKR)>>4)&0x1)//vld_pause(vld_92[4])
          {
              UTIL_Printf("..........Receive VLD_pause: Ring_Fifo mb_y= %d............., uDataPartitionNum is %d\n", u4MbY, prVDecVp8FrmHdr->rVp8DecParam.uDataPartitionNum);
              if (u4Index<=prVDecVp8FrmHdr->rVp8DecParam.uDataPartitionNum-1)
              {
                u4CurParNum = u4Index;
                u4CurCoeffRtprBase = u4VDecReadVP8VLD(u4InstID, RW_VP8_COEFR0+u4CurParNum*4);
                UTIL_Printf("..........Ring_Fifo mb_y= %dVLD_[%d]----0x%x.............\n",u4MbY,(RW_VP8_COEFR0+u4CurParNum*4)>>2,  u4CurCoeffRtprBase);
                u4Index++;
                if (u4Index == prVDecVp8FrmHdr->rVp8DecParam.uDataPartitionNum)
                {
                    u4Index = 0;
                    fgLastPartionNumOneGroup = TRUE;
                    UTIL_Printf("One group last partition\n");
                }
              }

			  UTIL_Printf("u4CurCoeffRtprBase is 0x%x, u4CurParNum is %d\n", u4CurCoeffRtprBase, u4CurParNum);
              vVdecWebpHWWebpReset(u4InstID, u4CurCoeffRtprBase,u4CurParNum);
              #if 0
              while( 0 == ((u4VDecReadDV(0, RW_VLD_VP8_CTRL)>>16)&0x1))
              {
                  UTIL_Printf("Wait vdec_int done\n");
              }
              vVDecWriteDV(0, RW_VLD_VP8_CTRL,0x1011);//clear vdec_init
              vVDecWriteDV(0, RW_VLD_VP8_CTRL,0x1001);
              #endif
             if((u4MbX == u4MbX_last) && (u4MbY == u4MbY_last))
             {
               // UTIL_Printf("(u4MbX == u4MbX_last) && (u4MbY == u4MbY_last)\n");
                u4CntTimeChk ++;
             }
             else
            {
    ...
    {    
      ...
      if((u4Cnt & 0x3f)== 0x3f)
      {
        ...
        {
         // UTIL_Printf("VP8 Test 100\n");

          #if (VDEC_VP8_WEBP_SUPPORT && !VP8_MB_ROW_MODE_SUPPORT)
		   while(0 == ((u4VDecVP8ReadImgResz(0, 4 * IMG_RESZ_DONE)>>4)&0x1))//imgrsz_63[4]
           {
               UTIL_Printf("Wait img_resz on\n");
           }
		  ...
		  UTIL_Printf("VP8 Decode finish, MC_478(DRAM_Clk) value is 0x%x \n", u4Value_478);
		  UTIL_Printf("VP8 Decode finish, MC_632(VDec_Clk) value is 0x%x \n", u4Value_632); 
		  #endif
          u4CntTimeChk = 0;
          break;
        }
        ...
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYAddrPhy, PIC_Y_SZ+PIC_C_SZ, FROM_DEVICE);
  #if VDEC_VP8_WEBP_SUPPORT
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYAltPhy, DEC_PP_Y_SZ+DEC_PP_C_SZ, TO_DEVICE);
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYLstPhy, DEC_PP_Y_SZ+DEC_PP_C_SZ, TO_DEVICE);
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYGldPhy, DEC_PP_Y_SZ+DEC_PP_C_SZ, TO_DEVICE);
  #else
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYAltPhy, GOLD_Y_SZ+GOLD_C_SZ, TO_DEVICE);
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYLstPhy, GOLD_Y_SZ+GOLD_C_SZ, TO_DEVICE);
        BSP_dma_unmap_single((UINT32)_pu1VP8VDecYGldPhy, GOLD_Y_SZ+GOLD_C_SZ, TO_DEVICE);
  #endif    
  #endif    
...
//      UTIL_Printf("u4MbX = 0x%x, u4MbY = 0x%x\n", u4MbX, u4MbY);
//      UTIL_Printf("_testCnt = 0x%x\n", (UINT32)_testCnt);
      #if VP8_MB_ROW_MODE_SUPPORT
      if (_u4ErrCnt !=0)
      {UTIL_Printf("Webp Compare NG, ErrCnt = %d\n", _u4ErrCnt);}
      	else
      {UTIL_Printf("Webp Compare OK, MB Count = %d\n", _u4MbH);}

     // UTIL_Printf("Trigger Decode Count = %d\n", _testCnt);
      _u4MbH =0;
      _testCnt =0;
      _fgOpenfile = FALSE;
  //   _fgLastMBRow =FALSE;
...
//Print LOG
  ...
  Printf("%s", strMessage);
	  ...
	  {
		  UINT32 u4DramValue = 0;
			  
		  //vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_DRAM, PHYSICAL((UINT32)_pucSegIdWrapper[u4InstID]));
		  vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_DRAM, PHYSICAL(prVDecVp8FrmHdr->u4SegIdWrapper));
		  vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_SRAM, 0x2200400);
  
		  vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_LARB_CONTROL, 1);
		  vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_DRAM_TRIGGER, 3);
  
		  u4DramValue = u4VDecReadVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_DRAM_TRIGGER);
		  UTIL_Printf("End u4DramValue is 0x%x\n", u4DramValue);
  
		  while((u4DramValue & 0x4) != 0)
		  {
			  u4DramValue = u4VDecReadVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_DRAM_TRIGGER);
		  ...
		  vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_VP8_SEGID_LARB_CONTROL, 0);
	  }
  ...
  #if VDEC_SUPPORT_POWER_SAVING
  vVDEC_HAL_PowerSaveClear(u4InstID,VDEC_FMT_VP8);
  #endif
