...
UINT32 u4RM_VDecRead_ImgResz(UINT32 u4VDecID, UINT32 u4Addr)
{
    ...
    {
        UINT32 u4Val;
		u4Val = u4ReadReg(IMG_RESZ_REG_OFFSET0 + u4Addr);
  ...
  UINT32 u4CurrPicDispWidth, UINT32 u4CurrPicDispHeight)
{
  ...
  
  {
	...
	u4Value = u4RM_VDecRead_ImgResz(0, IMG_RESZ_JPG_MODE * 4);

	UTIL_Printf("u4RM_VDecRead_ImgResz is 0x%lx\n", u4Value);
  }
  ...
  if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.fgRPRRacingModeEnable)
  {
...

	{
		...
		UTIL_Printf("u4RM_VDecRead_ImgResz test1 is 0x%lx\n", u4Value);
    }
  }
  else	
  {
...

	{
		...
		UTIL_Printf("u4RM_VDecRead_ImgResz test2 is 0x%lx\n", u4Value);
    }
  }
  ...
  #ifdef RM_DDR3MODE_ENABLE
  {
   ...
   {
		vRM_VDecWrite_ImgResz(0, RZ_TG_BUF_LEN * 4, (qw + (u4AlignSize << 12)) | 0xC0000000); 
		UTIL_Printf("RZ_TG_BUF_LEN value is 0x%lx\n", (qw + (u4AlignSize << 12)) | 0xC0000000);
   }
#else
...
//  qw = (u4CurrPicDecWidth) << 12 | u4CurrPicDecHeight;
  ...
  {
      vRM_VDecWrite_ImgResz(0, IMG_RESZ_START * 4, 0xC3); // activate                
      vRM_VDecWrite_ImgResz(0, IMG_RESZ_START * 4, 0xC2); 
  }
  ...
  {
    while ((u4RM_VDecRead_ImgResz(0, IMG_RESZ_DONE * 4) & 0x1) == 0);
    UTIL_Printf("IMGresz Done!, value is 0x%lx \n",u4RM_VDecRead_ImgResz(0, IMG_RESZ_DONE * 4));
  }
}
