...
void vRM_GenerateDownScalerGolden(UINT32 u4InstID, UINT32 DecYAddr,UINT32 DecCAddr, UINT32 u4Size, 
	                                                                  BOOL lk_en, UINT32 luma_key);
extern UINT32 u4RM_VDecRead_ImgResz(UINT32 u4VDecID, UINT32 u4Addr);
...
UINT32 u4GetField(UINT32 u4Val, int b0, int b1)
{
    int i;
    UINT32 u4Mask = 0;
...
BOOL fgGetRvPic(UINT32 u4InstID)
{
    UINT32 u4Parm;
    int i;
    ...
    UTIL_Printf("4th Input Window %x \n", u4InputWindow);
   }
	#endif
    u4Parm = *(UINT32 *)(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.u4RMFrmInfoBuf +
    	                               _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.u4RMFrmInfoRPtr);
...
void vRM_TriggerDecode(UINT32 u4InstID, VDEC_INFO_RM_PICINFO_T *prParsingPic)
{
  ...
  {
    UTIL_Printf("MC Output Mode - Buffer Addr Error \n");
    ASSERT(0);
  }
  #endif //RM_MCOutPut_ENABLE
  
  _tBufPVAddrInfo[u4InstID].u4PhyRMOutBufY= BSP_dma_map_single(prParsingPic->u4OutBufY, PIC_Y_SZ, FROM_DEVICE); 
  _tBufPVAddrInfo[u4InstID].u4PhyRMOutBufC= BSP_dma_map_single(prParsingPic->u4OutBufC, PIC_C_SZ, FROM_DEVICE);
...
#else
    vVDecResetHW(u4InstID, VDEC_FMT_RV);
#endif
...
*/
  ...
  vVDEC_HAL_RM_EnableDramTable(u4InstID, _pucRMIntraTableBuf[u4InstID], _pucRMInterTableBuf[u4InstID]);
  ...
  //Pre-Parsing Picture Information
  {
    ...
    x_memcpy((void*) (&_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMPicInfo_Preparsing), 
...
//Set BwdIsI
  ...
  {
    UTIL_Printf("=====> Disable Deblocking \n");
  }
  ...
  //if (fgFirstTime)
  {
    if (prParsingPic->ePtype == RM_INTRAPIC || prParsingPic->ePtype == RM_FORCED_INTRAPIC)
    {
      ...
      if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.fgRPRRacingModeEnable)
      {
        ...
        UTIL_Printf("=====> I Racing Mode, About to trigger image resizer in racing mode...\n");
      ...
      UTIL_Printf("=====> I Racing Mode, Image resizer is now waiting for vdec...\n");
      }
      #endif //RM_RPR_RACINGMODE_ENABLE
/*
...
*/
    ...
    else if (prPic->ePtype == RM_INTERPIC)
    {
      ...
      if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.fgRPRRacingModeEnable)
      {
        ...
        UTIL_Printf("=====> P Racing Mode, About to trigger image resizer in racing mode...\n");
        vRprImgResz(u4InstID,
            prPic->u4Width, prPic->u4Height,
            prPic->u4OrgWidth, prPic->u4OrgHeight,
            rPreParsingPicInfo->u4Width, rPreParsingPicInfo->u4Height,
          ...
          UTIL_Printf("=====> P Racing Mode, Image resizer is now waiting for vdec...\n");
      }
      #endif //RM_RPR_RACINGMODE_ENABLE
/*      
#ifdef RPR_RACING
...
*/
      ...
      if (prParsingPic->u4OrgWidth != prParsingPic->u4PrevDispWidth || prParsingPic->u4OrgHeight != prParsingPic->u4PrevDispHeight)  // RPR
      {
        ...
        UTIL_Printf("Normal RPR, About to trigger image resizer...\n");
        vRprImgResz(
           u4InstID,
        ...
        UTIL_Printf("Normal RPR, Image has been resized!\n");
        #endif //RM_RPR_RACINGMODE_ENABLE
    ...
    {
		UTIL_Printf("prParsingPic->ePtype is 0x%lx\n",prParsingPic->ePtype);
		UTIL_Printf("Current picture count is %d, 0 base\n",_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt);
...
//Sync Pic Info
  ...
  //Clear Y, C Memory
  //x_memset((void*) prParsingPic->u4OutBufY, 0xFF, PIC_Y_SZ);
  //x_memset((void*) prParsingPic->u4OutBufC, 0xFF, PIC_C_SZ);
...
//VDec HW Init Flow  //////////////////////////////////////////////////////////
  ...
  {
    UTIL_Printf("RM Error Data in AU FIFO \n");
    while (1)
    {
      x_thread_delay(1);
    }
...
UINT32 mb_to_line_y(UINT32 buf, int width, int x, int y)
{
  UINT32 val = 0;
  int mb_x;
  int mb_y;
...
BOOL fgRM_CheckYCGolden(UINT32 u4InstID)
{
  ...
  x_memset(_pucDumpYBuf_1[u4InstID], 0x0, GOLD_Y_SZ);
  x_memset(_pucDumpCBuf_1[u4InstID], 0x0, GOLD_C_SZ);
  ...
  u4GoldYUSz = u4GoldYSz + u4GoldYSz / 4;
#if 0
  	{
		    ...
		    UTIL_Printf("Printf Y Data\n");
           	for(i=0; i < u4GoldYSz ; i=i+16)
           	{
				a = *(UINT8 *)(u4OutBufY + i);
				b = *(UINT8 *)(u4OutBufY + i + 1);
				c = *(UINT8 *)(u4OutBufY + i + 2);
				d = *(UINT8 *)(u4OutBufY + i + 3);
				e = *(UINT8 *)(u4OutBufY + i + 4);
				f = *(UINT8 *)(u4OutBufY + i + 5);
				g = *(UINT8 *)(u4OutBufY + i + 6);
				h = *(UINT8 *)(u4OutBufY + i + 7);
				j = *(UINT8 *)(u4OutBufY + i + 8);
				k = *(UINT8 *)(u4OutBufY + i + 9);
				l = *(UINT8 *)(u4OutBufY + i + 10);
				m = *(UINT8 *)(u4OutBufY + i + 11);
				n = *(UINT8 *)(u4OutBufY + i + 12);
				o = *(UINT8 *)(u4OutBufY + i + 13);
				p = *(UINT8 *)(u4OutBufY + i + 14);
				q = *(UINT8 *)(u4OutBufY + i + 15);
		    ...
		    UTIL_Printf("Printf C Data\n");
           	for(i=0; i < u4GoldYSz/4 ; i=i+16)
           	{
				a = *(UINT8 *)(u4OutBufC + i);
				b = *(UINT8 *)(u4OutBufC + i + 1);
				c = *(UINT8 *)(u4OutBufC + i + 2);
				d = *(UINT8 *)(u4OutBufC + i + 3);
				e = *(UINT8 *)(u4OutBufC + i + 4);
				f = *(UINT8 *)(u4OutBufC + i + 5);
				g = *(UINT8 *)(u4OutBufC + i + 6);
				h = *(UINT8 *)(u4OutBufC + i + 7);
				j = *(UINT8 *)(u4OutBufC + i + 8);
				k = *(UINT8 *)(u4OutBufC + i + 9);
				l = *(UINT8 *)(u4OutBufC + i + 10);
				m = *(UINT8 *)(u4OutBufC + i + 11);
				n = *(UINT8 *)(u4OutBufC + i + 12);
				o = *(UINT8 *)(u4OutBufC + i + 13);
				p = *(UINT8 *)(u4OutBufC + i + 14);
				q = *(UINT8 *)(u4OutBufC + i + 15);
  ...
  for (y = 0; y < u4Height; y++)
  {
    for (x = 0; x < u4Width; x += 4)
    {
      ...
      if (u4ErrVal != u4GolVal)
      {
        ...
        UTIL_Printf("Check Golden Y Error \n");
        UTIL_Printf("Check Golden Y Error, u4ErrVal is %lx, u4GolVal is %lx \n", u4ErrVal, u4GolVal);
        UTIL_Printf("Golden address = %x.size = %x, u4OutBufY address is 0x%lx\n",u4GoldenBuf,(u4Height*u4Width), u4OutBufY);
        UTIL_Printf("HW address = %x.size = %x\n",u4OutBufY,(u4Height*u4Width));
		UTIL_Printf("y = 0x%x. x = 0x%x, u4Height is 0x%x, u4Width is 0x%x\n",y,x, u4Height, u4Width);
        {
  ...
  for (y = 0; y < u4Height/2; y++)
  {
    for (x = 0; x < u4Width/2; x += 4)
    {
      ...
      if (u4ErrVal != u4GolVal)
      {
        ...
        UTIL_Printf("Check Golden U Error \n");
        {
           //vRMVfyBufDeBlock(u4OutBufY,u4GoldenBuf,u4DecWidthAlign,u4DecHeight,16,32);
           vDumpVldCkSum();
           vDumpAvcMvCkSum();
      ...
      if (u4ErrVal2 != u4GolVal2)
      {
        ...
        UTIL_Printf("Check Golden V Error\n");
        ASSERT(0);
      }
    }
  }
...
BOOL fgRM_CheckYCDownScalerGolden(UINT32 u4InstID)
{
 ...
 // HalFlushInvalidateDCache();
  ...
  x_memset(_pucDumpYBuf_1[u4InstID], 0x0, GOLD_Y_SZ);
  x_memset(_pucDumpCBuf_1[u4InstID], 0x0, GOLD_C_SZ);
  //x_memset(_pucVDSCLBuf_1[u4InstID], 0x0, VDSCL_BUF_SZ_1);
      ...
      for (u4Pix = 0; u4Pix < u4Width*(u4Height>>1); u4Pix++)
      {
        ...
        //if((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.fgDec2ndFld) || (_tVerMpvDecPrm[u4InstID].ucPicStruct == FRM_PIC))
        {
          ...
          //     (*(pbDecBuf)) < (*(pbGoldenBuf)) && (*(pbDecBuf))>= (*(pbGoldenBuf) - DSCL_TOLERATED_RANGE) )
          {
            ...
            //UTIL_Printf("Chroma round off floating number: %x %x \n", *(pbDecBuf), *(pbGoldenBuf));
            u4ErrorCnt++;
          }
...
// component: 0 => Y, 1 => C
{
  int srcbufLen;
    ...
    #if 0
    UTIL_Printf("=====> vdscl parameter Log - Start \n");
    UTIL_Printf("srcWidth, srcHeight, targetWidth, targetHeight %x %x %x %x \n", srcWidth, srcHeight, targetWidth, targetHeight);
    UTIL_Printf("srcData, targetData, component, pic_stru %x %x %x %x \n", srcData, targetData, component, pic_stru);
    UTIL_Printf("tgbufLen, h_ofst, v_ofst %x %x %x \n", tgbufLen, h_ofst, v_ofst);
    UTIL_Printf("src_agent, spec_type, mbaff, fg_src %x %x %x %x \n", src_agent, spec_type, mbaff, fg_src);
    UTIL_Printf("src_h_ofst, src_v_ofst, scl_h_ofst, scl_v_ofst %x %x %x %x \n", src_h_ofst, src_v_ofst, scl_h_ofst, scl_v_ofst);
    UTIL_Printf("=====> vdscl parameter Log - End \n");
    #endif //0
...
static void vDumpReg(void)
{
  ...
  UTIL_Printf("RV VLD window: 0x%08X\n", _tVerMpvDecPrm[0].SpecDecPrm.rVDecRMDecPrm.u4HWInputWindow);
   ...
   for (i=0; i<700; i++)
   {
      UTIL_Printf("MC%04d (0x%04X) = 0x%08X\n", i, (i<<2), u4VDecReadMC(0, (i<<2)));
   }

   UTIL_Printf("PP Registers\n");
   for (i=0; i<30; i++)
   {
      UTIL_Printf("PP%04d (0x%04X) = 0x%08X\n", i, (i<<2), u4ReadReg(RM_VDEC_PP_BASE + (i<<2)));
   }
   UTIL_Printf("Register for check sum\n");
  UTIL_Printf("RV VLD 72: 0x%08X\n", u4ReadReg(RM_VLD_REG_OFFSET0 + 72 * 4));
  UTIL_Printf("RV VLD 73: 0x%08X\n", u4ReadReg(RM_VLD_REG_OFFSET0 + 73 * 4));
  UTIL_Printf("RV VLD 74: 0x%08X\n", u4ReadReg(RM_VLD_REG_OFFSET0 + 74 * 4));
  UTIL_Printf("RV VLD 75: 0x%08X\n", u4ReadReg(RM_VLD_REG_OFFSET0 + 75 * 4));
  ...
  UTIL_Printf("AVC_MV 153: 0x%08X\n", u4ReadReg(AVC_MV_REG_OFFSET0 + 153 * 4));
  UTIL_Printf("AVC_MV 154: 0x%08X\n", u4ReadReg(AVC_MV_REG_OFFSET0 + 154 * 4));

  UTIL_Printf("PP+0x100+3: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x100 +  3 * 4));
  UTIL_Printf("PP+0x100+4: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x100 +  4 * 4));
  UTIL_Printf("PP+0x100+5: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x100 +  5 * 4));
  UTIL_Printf("PP+0x100+7: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x100 +  7 * 4));
  UTIL_Printf("PP+0x400+2: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x400 +  2 * 4));
  UTIL_Printf("PP+0x400+3: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x400 +  3 * 4));
  UTIL_Printf("PP+0x600+9: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x600 +  9 * 4));
  UTIL_Printf("PP+0x800+2: 0x%08X\n", u4ReadReg(RM_VDEC_PP_BASE + 0x800 +  2 * 4));
}
...
//Keep in Momery
  ...
  x_memcpy(pbChecksumBuf, &u4DecPicCnt, 4);

  //UTIL_Printf("Keep Checksum Data to Temp Buffer %x \n", (UINT32) pbChecksumBuf);

  //Keep Checksum Data
  pbChecksumBuf = pbChecksumBuf + 4;
  x_memcpy(pbChecksumBuf, au4ChecksumData, 40);
  ...
  //UTIL_Printf("Check HW Checksum with Golden Checksum \n");

  pbChecksumBuf = pbChecksumBuf + 40*u4DecPicCnt;
  pbChecksumBuf = pbChecksumBuf + 4*u4DecPicCnt + 4;

  if (x_memcmp(pbChecksumBuf, au4ChecksumData, 40) == 0)
  {
    // TODO:
  }
  else
...
void vRM_CheckCRCResult(UINT32 u4InstID, UINT32 u4DecFrameCnt, UINT32 u4CRCResBuf)
{
  ...
  for (i=0; i<4; i++)
  {
	...
	//UTIL_Printf("Y u4CRCResult is 0x%lx, SWResult is 0x%lx\n",u4CRCResult, u4SWResult);
    if (u4SWResult != u4CRCResult)
    {
	  UTIL_Printf("Y u4CRCResult is 0x%lx, SWResult is 0x%lx\n",u4CRCResult, u4SWResult);
      UTIL_Printf("Y CRC Compare Golden Error\n");
      UTIL_Printf("RM Dec Cnt = %d is error, \n", u4DecFrameCnt);   
      ASSERT(0);
    }
  }
  ...
  for (i=0; i<4; i++)
  {
	...
	//UTIL_Printf("C u4CRCResult is 0x%lx, SWResult is 0x%lx\n",u4CRCResult, u4SWResult);
    if (u4SWResult != u4CRCResult)
    {
      UTIL_Printf("Y u4CRCResult is 0x%lx, SWResult is 0x%lx\n",u4CRCResult, u4SWResult);
  ...
  for (i=0; i<4; i++)
  {
    ...
    if (u4SWResult != u4CRCResult)
    {
      UTIL_Printf("CRC Compare Golden Error\n");
      ASSERT(0);
    }
  }
...
void vRM_VDecDecEndProc(UINT32 u4InstID)
{
  ...
  {    
    ...
    if((u4Cnt & 0x3f)== 0x3f)
    {
      ...
      {
	  	//UTIL_Printf("RM Test 100\n");
        u4CntTimeChk = 0;
        break;
      }
      else
      {
	  	//UTIL_Printf("RM Test 101\n");
        u4MbX_last = u4MbX;
        u4MbY_last = u4MbY;
        vVDEC_HAL_RM_GetMbxMby(u4InstID, &u4MbX, &u4MbY);

		//UTIL_Printf("u4MbX is %d, u4MbY is %d\n", u4MbX, u4MbY);
...
//Check Error Condition
  if (u4CntTimeChk == DEC_RETRY_NUM)
  {
    ...
    BSP_dma_unmap_single(_tBufPVAddrInfo[u4InstID].u4PhyRMOutBufY, PIC_Y_SZ, FROM_DEVICE);      
    BSP_dma_unmap_single(_tBufPVAddrInfo[u4InstID].u4PhyRMOutBufC, PIC_C_SZ, FROM_DEVICE);      
...
//Add Decoded Pic Cnt
  #ifdef RM_ADDRSWAP_ENABLE
  #ifdef RM_ADDRSWAP_DSCL_ENABLE
  UTIL_Printf("RM Dec Cnt = %d, AdrSwap = %d, DSCL AdrSwap On \n", _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt, 
                                               _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.u4RMAddrSwapMode);
  #else //RM_ADDRSWAP_DSCL_ENABLE
  UTIL_Printf("RM Dec Cnt = %d, AdrSwap = %d \n", _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt, 
                                               _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.u4RMAddrSwapMode);
  #endif //RM_ADDRSWAP_DSCL_ENABLE
  #else //RM_ADDRSWAP_ENABLE
  if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt % 10 == 0)
  UTIL_Printf("RM Dec Cnt = %d \n", _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt);
  #endif //RM_ADDRSWAP_ENABLE
...
//Update Src Bitstream and Golden Data
  ...
  UTIL_Printf("Test File name is  %s\n", strEmuFileName);
  if((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt >= _u4StartCompPicNum[u4InstID])&&
     (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt <= _u4EndCompPicNum[u4InstID]))
  {
 ...
 _tBufPVAddrInfo[u4InstID].u4PhyVififoSa = BSP_dma_map_single((UINT32) _pucVFifo[u4InstID], V_FIFO_SZ, TO_DEVICE); 
  ...
  if(fgOpen == FALSE)
  {
   //UTIL_Printf("Emulation Finish, change to next sequence \n");
    UTIL_Printf("%s Emulation Finish at %d, change to next sequence \n",strEmuFileName,_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.rRMParsPicInfo.u4DecodedPicCnt);
    _tVerDec[u4InstID].ucState = 4;
  ...
  #ifdef RM_RINGVIFO_FLOW
  {
    ...
    {
      UTIL_Printf("VFIFO Size Error (< One Picture) \n");
      ASSERT(0);
    }
