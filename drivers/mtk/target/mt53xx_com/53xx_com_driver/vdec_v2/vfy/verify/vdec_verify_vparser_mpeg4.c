...
#endif
extern	vWrite2PC(UINT32 u4InstID, UCHAR bType, UCHAR *pbAddr);
extern BOOL fgWrData2PC(void* pvAddr, UINT32 u4Size, UINT32 u4Mode, char *strFileName);

void vWrite2PC_before(UINT32 u4InstID, UCHAR bType, UCHAR *pbAddr)
{
#if 1
    UINT32 u4Temp;
    char strMessage[256];
    ...
    switch(bType)
    {
        ...
        case 2:
			...
			_tFBufFileInfo[u4InstID].u4FileLength = 640*256;
        ...
        case 3:
			...
			_tFBufFileInfo[u4InstID].u4FileLength = 640*256/2;
            fgWrData2PC((void*)pbAddr,_tFBufFileInfo[u4InstID].u4FileLength,7,_bTempStr1[u4InstID]);
            break;
        case 4:
            u4Temp += sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_VDec%d_%d_MV_before.err", u4InstID, _u4FileCnt[u4InstID]);
			_tFBufFileInfo[u4InstID].u4FileLength = 10240;
            fgWrData2PC((void*)pbAddr,_tFBufFileInfo[u4InstID].u4FileLength,7,_bTempStr1[u4InstID]);
            break;
        case 5:
...
UINT32 u4VParserMPEG4(UINT32 u4InstID, BOOL fgInquiry)
{
  ...
  switch(_u4CodecVer[u4InstID])
  {
    ...
    case VDEC_H263:
      ...
      //UTIL_Printf("input window before parsing = 0x%08x\n", _u4Datain[u4InstID]);
...
UINT32 u4ShVParser(UINT32 u4InstID, BOOL fgInquiry)
{
  ...
  if(_u4CodecVer[u4InstID] == VDEC_H263)
  {
      ...
      if((_ucVopCdTp[u4InstID] == 0x02) || (_ucVopCdTp[u4InstID] == 0x03))//set disposable inter as P frame mtk40343
      {
        _ucVopCdTp[u4InstID] = 0x01;
      }
  ...
  if(fgInquiry)
  {
	UTIL_Printf("[ShortHeader]: fgInquiry is TRUE!\n");
    return(0);
  }
...
void vVerifyCalGmcMv(UINT32 u4InstID)
{
  ...
  //s = 2 << _ucSpriteEnable[u4InstID];// use enable. xiaolei.li _ucSpriteWarpingAccuracy[u4InstID]; 
  s = 2 << _ucSpriteWarpingAccuracy[u4InstID]; //  xiaolei.li for bitstream vcon-asp14L1 in mpeg4 mode. mustang emu.
  if(fgBadDivX)
  {
...
//    ptMp4Prm->u4QPelType = TYPE_14496;
  ...
  else
  {
    ...
    if(_ucVopCdTp[u4InstID] != VCT_B)
    {
      ...
      if(_u4CodecVer[u4InstID] == VDEC_DIVX3)
      {
        ...
        ptMp4Prm->u4CMvType = (ptMp4Prm->u4CMvType & 0xfffffffe) | QPEL_4MV_CMV_14496;  // Follow DivX source code
      }
    ...
    {
      if((_u4UserDataCodecVersion[u4InstID] == 0) || /*(_u4UserDataCodecVersion[u4InstID] >= 503)*/(_u4UserDataCodecVersion[u4InstID] >= 500))
      {
        //ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_DIVX503;
        ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_14496;
      }
  ...
  {
	  if (u4MVWorkaround & BUG_MMS_BVOP_CHR && ptMp4Prm->rDep.rM4vDecPrm.prVol->ucQuarterSample)
	  {
		  ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_14496;
	  }
	  else
  ...
  #endif
//  UTIL_Printf("u4QPelType = 0x%x,u4CMvType = 0x%x codetype = 0x%x\n",ptMp4Prm->u4QPelType,ptMp4Prm->u4CMvType,_ucVopCdTp[u4InstID]);
}
...
void vVerifyDx3SetPicLayerPrm(UINT32 u4InstID)
{
  UINT32 u4SliceSize,u4MBHeigh,i,j = 0;
  ...
  #if (1)//add chip version config
  {
    u4MBHeigh = ((_u4DIVX3Height[u4InstID] + 15)/16);
    u4SliceSize = u4MBHeigh/(_ucFrameMode[u4InstID] - 22);
    for(i = 0; i < u4MBHeigh; i++)
    {
        if(!(i%u4SliceSize))
        {
            switch(j)
            {
                case 0:
                    ptDx3Prm->ucSliceBoundary[0] = i;
                    break;
                case 1:
                    ptDx3Prm->ucSliceBoundary[1] = i;
                    break;
                case 2:
                    ptDx3Prm->ucSliceBoundary[2] = i;
                    break;
                case 3:
                    ptDx3Prm->ucSliceBoundary[3] = i;
                    break;
                case 4:
                    ptDx3Prm->ucSliceBoundary[4] = i;
                    break;
            }
            j++;
        }
...
*/
  ...
  else if((_u4Datain[u4InstID] & H263_VIDEO_START_MASK) == SHORT_VIDEO_START_MARKER)
  {
  ...
  if(_fgVerQuantType[u4InstID])
  {
    ...
    if(_fgVerLoadIntraMatrix[u4InstID])
    {
	  ...
	  //		rMpegBSInitPrm.u4WritePointer = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2)));
		  rMpegBSInitPrm.u4WritePointer = (UINT32)_pucVFifo[u4InstID] + ((_u4LoadBitstreamCnt[u4InstID]%2)?(V_FIFO_SZ):(V_FIFO_SZ>>1));//mtk40343
      #endif
        i4VDEC_HAL_MPEG4_InitBarrelShifter(1, u4InstID, &rMpegBSInitPrm);
...
BOOL fgVPrsValidStartCode(UINT32 u4InstID, BOOL fgMp4, UINT32 *pdwSC)
{
  if (fgMp4)
  {
    ...
    {// check video plane with short header
	  ...
	  // modified by xiaolei.li for python verification
      else if(((((*pdwSC)& H263_VIDEO_START_MASK) == H263_VIDEO_START_MARKER) && (_u4CodecVer[u4InstID] == VDEC_H263))
	  	     || ((((*pdwSC)& SHORT_VIDEO_START_MASK) == SHORT_VIDEO_START_MARKER) && (_u4CodecVer[u4InstID] == VDEC_MPEG4)))
      {
        return TRUE;
      }
    }
