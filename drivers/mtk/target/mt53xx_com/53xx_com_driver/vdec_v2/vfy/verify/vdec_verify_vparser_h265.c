...
UINT32 vHEVCParseSliceHeader(UINT32 u4InstID,UCHAR ucVldID);  
...
void vHEVCVerifyInitSPS(UINT32 u4InstID,H265_SPS_Data *prSPS);
void vHEVCVerifyInitTilesInfo(UINT32 u4VDecID,H265_SPS_Data *prSPS,H265_PPS_Data *prPPS,VDEC_INFO_H265_PIC_INFO_T *prH265PicInfo);
...
//	            _ptH265FBufInfo[u4InstID][u4LostBFindex] = _ptH265FBufInfo[u4InstID][j];
            memcpy(_ptH265FBufInfo[u4InstID][u4LostBFindex].u4YStartAddr, _ptH265FBufInfo[u4InstID][j].u4YStartAddr, _ptH265FBufInfo[u4InstID][j].u4DramYSize);
            memcpy(_ptH265FBufInfo[u4InstID][u4LostBFindex].u4CStartAddr, _ptH265FBufInfo[u4InstID][j].u4CStartAddr, _ptH265FBufInfo[u4InstID][j].u4DramCSize);
            if(_u4UFOMode)
            {
                memcpy(_ptH265FBufInfo[u4InstID][u4LostBFindex].u4YLenStartAddr, _ptH265FBufInfo[u4InstID][j].u4YLenStartAddr, _ptH265FBufInfo[u4InstID][j].u4UFOLenYsize);
                memcpy(_ptH265FBufInfo[u4InstID][u4LostBFindex].u4CLenStartAddr, _ptH265FBufInfo[u4InstID][j].u4CLenStartAddr, _ptH265FBufInfo[u4InstID][j].u4UFOLenCsize);                
            }            
...
*/           
    ...
    for( i = 0; i < pCurrRPS->u4NumberOfNegativePictures + pCurrRPS->u4NumberOfPositivePictures ; i++)
    {
        ...
        {
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            UTIL_Printf("VDEC[%d][%d][%d] usedLT %d fbuf Poc %d slice POC %d deltaPoc %d slice referened %d\n",u4InstID,i,j,
                                                    _ptH265FBufInfo[u4InstID][j].bUsedAsLongTerm,
                                                    _ptH265FBufInfo[u4InstID][j].i4POC,
                                                    pCurrSliceHdr->i4POC,
                                                    pCurrRPS->i4DeltaPOC[i],
                                                    _ptH265FBufInfo[u4InstID][j].bFirstSliceReferenced);
            #endif
...
// Return    : 
// *********************************************************************
void vHEVCAssignQuantParam(UINT32 u4InstID,UCHAR ucVldID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
    int  i,size, list, qp;
    ...
    // SQT part
    vVDecWriteHEVCPP(ucVldID, HEVC_IQ_SL_CTRL, prSPS->bScalingListFlag );
    ...
    if ( prSPS->bScalingListFlag )
    {
        ...
        if (prPPS->bPPSScalingListPresentFlag) 
        {
            ...
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            printk("use pps scalinglist 0x%x\n",(UINT32)pScalingList);
            #endif
        }
        else
        {
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            printk("use sps scalinglist 0x%x\n",(UINT32)pScalingList);
            #endif
        }
        ...
        vVDEC_HAL_H265_SetSLPP( ucVldID, pScalingList );
        vVDecWriteHEVCPP(ucVldID, HEVC_IQ_SRAM_32BITS_CTRL_REG, 0x1 );
        ...
        for(size=0;size<SCALING_LIST_SIZE_NUM;size++)
        {
            for(list = 0; list < H265_scalingListNum[size]; list++)
            {
                for(qp=0;qp<SCALING_LIST_REM_NUM;qp++)
                {
                    ...
                    vVDEC_HAL_H265_SetSLVLD(ucVldID,coeff , width, height, H265_invQuantScales[qp],&u4IQSramAddr);
                }
            }
        }
        
        vVDecWriteHEVCPP(ucVldID, HEVC_IQ_SRAM_32BITS_CTRL_REG, 0 );
...
// *********************************************************************
// Function    : UINT32 vHEVCVerifyVDecSetPicInfo(UINT32 u4InstID, UCHAR ucVldID,VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
// Description : Set Pic related info before reordering
// Parameter   : None
// Return      : UINT32
// *********************************************************************
UINT32 vHEVCVerifyVDecSetPicInfo(UINT32 u4InstID, UCHAR ucVldID,VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
    BOOL bIsUFO;  
    UCHAR u2ECFbIndex = 0xFF;
    VDEC_INFO_H265_FBUF_INFO_T *prTempFBufInfo = NULL;
    prTempFBufInfo = _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prFBufInfo;
    
    bIsUFO = _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.bIsUFOMode;
    
    if (bIsUFO)        
    {
        _ptH265CurrFBufInfo[u4InstID]->bIsUFOEncoded = 1;
    }
    
    if(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.ucVDecStatus == VDEC_LAE_ST)
    {
        printk("[flow info] LAE Process !\n");
        vVDEC_HAL_H265_SetLAEDecAddr(ucVldID,PHYSICAL(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.rLaeInfo.u4LaeCabacAddr),
                                            PHYSICAL(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.rLaeInfo.u4LaeEcAddr));
        // leon
        if(_u4DualCoreEnable[0] == 2)
        {
            vVDEC_HAL_H265_SetLAEDecCore(0, 0x0);
        }

        vVDEC_HAL_Set_ErrConceal(u4InstID,ucVldID);
        vVDEC_HAL_H265_SetSHDRHEVLD(ucVldID, &(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        vVDEC_HAL_H265_SetSPSHEVLD(ucVldID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
        vVDEC_HAL_H265_SetPPSHEVLD(ucVldID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);

        vHEVCVerifyInitTilesInfo( u4InstID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS,&_rH265PicInfo[u4InstID]);
        vVDEC_HAL_H265_SetTilesInfo(u4InstID,ucVldID,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);

    }
    #if 0
    else if(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.ucVDecStatus == VDEC_CORE1_ST)
    {
        printk("Core1 register part0 step start!\n");
        //copy hevc param from core0
        x_memcpy(&_tVerMpvDecPrm[u4InstID],&_tVerMpvDecPrm[0],sizeof(VDEC_INFO_DEC_PRM_T));
        //copy  fb info
        x_memcpy(&_ptH265FBufInfo[u4InstID],&_ptH265FBufInfo[0],(sizeof(VDEC_INFO_H265_FBUF_INFO_T) * 25));

        //copy current fb info
        _ptH265CurrFBufInfo[u4InstID] = _ptH265CurrFBufInfo[0];

        //copy pic info 
        x_memcpy(&_rH265PicInfo[u4InstID],&_rH265PicInfo[0],sizeof(VDEC_INFO_H265_PIC_INFO_T));
        //assign SQT condition is "u4SliceCnt == 0"
        _rH265PicInfo[u4InstID].u4SliceCnt = 0;
        //copy
        x_memcpy(&_ptH265RefPicList[u4InstID],&_ptH265RefPicList[u4InstID],sizeof(VDEC_INFO_H265_REF_PIC_LIST_T));

        //set core1 register
        u2ECFbIndex = ucSearchNearestPOCFbufIdx(u4InstID,&(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        u4VDEC_HAL_H265_SetECInfo(ucVldID,&(prTempFBufInfo[u2ECFbIndex]),&(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        u4VDEC_HAL_H265_SetUFO(ucVldID,&(_ptH265CurrFBufInfo[u4InstID]));
        vVDEC_HAL_H265_SetRefPicListReg(ucVldID,&_rH265PicInfo[u4InstID],&_ptH265FBufInfo[u4InstID]);
        vVDEC_HAL_H265_InitPicInfo(ucVldID,&(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        vHEVCAssignQuantParam(u4InstID, ucVldID,ptVerMpvDecPrm);
        vVDecWriteMC(ucVldID, 702*4,(u4VDecReadMC(ucVldID,702*4) | (1 << 8)));
        vVDEC_HAL_H265_SetSHDRHEVLD(ucVldID, &(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        vVDEC_HAL_H265_SetSPSHEVLD(ucVldID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
        vVDEC_HAL_H265_SetPPSHEVLD(ucVldID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
        ...
        vHEVCVerifyInitTilesInfo( u4InstID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS,&_rH265PicInfo[u4InstID]);
        vVDEC_HAL_H265_SetTilesInfo(u4InstID,ucVldID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
        ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.ucVDecStatus = VDEC_NORMAL_ST;
        printk("Core1 register part0 step done!\n");
    }
    else
    {
        u2ECFbIndex = ucSearchNearestPOCFbufIdx(u4InstID,&(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        ...
        vHEVCVerifyInitTilesInfo( u4InstID, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS,&_rH265PicInfo[u4InstID]);
        vVDEC_HAL_H265_SetTilesInfo(u4InstID, ucVldID,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS, ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS);
        //single core decode and non LAE decode.
        if(_u4DualCoreEnable[u4InstID] != 1)
        {
          vVDEC_HAL_H265_VDec_SetPipComReg(ucVldID,ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS);
        }
        printk("Core0 register part0 step done!\n");
    }
    #else
    else
    {
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        UTIL_Printf("Set register %d hw %d \n",u4InstID,ucVldID);
        #endif
        vVDecH265SetRegister(u4InstID,ucVldID,&(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        if(_u4DualCoreEnable[0] == 1)
        {
            vVDecWriteHEVCVLD(VDEC_VLD1, RW_HEVC_SLICE_0, u4VDecReadHEVCVLD(ucVldID,RW_HEVC_SLICE_0));
            if ( ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSliceHdr->u4SliceType != HEVC_I_SLICE )
            {
                vVDecWriteHEVCVLD(VDEC_VLD1, RW_HEVC_SLICE_2, u4VDecReadHEVCVLD(ucVldID,RW_HEVC_SLICE_2)); 
                vVDecWriteHEVCVLD(VDEC_VLD1, RW_HEVC_REF_PIC_LIST_MOD, u4VDecReadHEVCVLD(ucVldID,RW_HEVC_REF_PIC_LIST_MOD));  
            }
            vVDecH265SetRegister(u4InstID,VDEC_VLD1,&(ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm));
        }
    }
    #endif
...
void vHEVCVerifyPrepareFBufInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
  ...
  if((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples != _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicWidthInLumaSamples) \
     ||(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples != _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicHeightInLumaSamples))
  {
    printk("[INFO] Active width hegith from [%d,%d] to [%d,%d]\n",_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicWidthInLumaSamples,_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.rLastInfo.u4LastPicHeightInLumaSamples,
                                        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicWidthInLumaSamples,_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prSPS->u4PicHeightInLumaSamples);
    

    if(_u4CRCMode == 0 || _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.prPPS->bTilesEnabledFlag)
    {
...
void vHEVCResetFBAddr(UINT32 u4InstID, UINT32 u4FBindex)
{
    ...
    _ptH265FBufInfo[u4InstID][u4FBindex].u4Addr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4Addr;
    _ptH265FBufInfo[u4InstID][u4FBindex].u4YStartAddr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4YStartAddr;
    _ptH265FBufInfo[u4InstID][u4FBindex].u4CAddrOffset = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4CAddrOffset;

    if(0)
    {
        _ptH265FBufInfo[u4InstID][u4FBindex].u4Addr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4Addr;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4YStartAddr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4YStartAddr;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4CStartAddr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4CStartAddr;    
        _ptH265FBufInfo[u4InstID][u4FBindex].u4YLenStartAddr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4YLenStartAddr;
        _ptH265FBufInfo[u4InstID][u4FBindex].u4CLenStartAddr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4CLenStartAddr;
    }
    _ptH265FBufInfo[u4InstID][u4FBindex].u4MvStartAddr = _ptH265FBufInfoTmp[u4InstID][u4FBindex].u4MvStartAddr;  
}
...
void vHEVCPartitionDPB(UINT32 u4InstID)
{
    ...
    else 
    {
        ...
        if(bIsUFO)
        {    
            u4UFOLenYsize = (((((u4DramPicSize + 1023 ) >> 10 ) << 10) + 255) >>8);
//	            u4UFOLenYsize = ((u4DramPicYSize + 255) >> 8);
    ...
    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum = (DPB_SZ /u4DramPicArea > 200) ? 200 : DPB_SZ /u4DramPicArea;
    ...
    for(i=0; i<_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++)
    {
        ...
        //x_memcpy(&_ptH265FBufInfoTmp[i],&_ptH265FBufInfo[u4InstID][i],sizeof(_ptH265FBufInfo[u4InstID][i]));
...
UINT32 vHEVCAllocateFBuf(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm, BOOL fgFillCurrFBuf)
{
  ...
  {
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    printk("[INFO] vHEVCAllocateFBuf() MinPoc status %d\n",_ptH265FBufInfo[u4InstID][u4MinPOCFBufIdx].ucFBufStatus);
    #endif
    printk("[Warning] Force output one reference buffer as current decode buffer!\n");
    vVerifyClrFBufInfo(u4InstID, u4MinPOCFBufIdx);
  }

  #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        printk("[INFO] vHEVCAllocateFBuf() MaxFB IndexNum = %d; get fb index = %d \n" , _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum ,u4MinPOCFBufIdx);
  #endif
  
  if(fgFillCurrFBuf)
  {
      _tVerMpvDecPrm[u4InstID].ucDecFBufIdx = u4MinPOCFBufIdx;
      _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucDecFBufId = u4MinPOCFBufIdx;
...
void vHEVCSetCurrFBufIdx(UINT32 u4InstID, UINT32 u4DecFBufIdx)
{
    ...
    #ifdef HEVC_VP_MODE //to make sure vp mode is move data or not
    if(_u4DualCoreEnable[u4InstID] == 0)
    {
        memset(_ptH265CurrFBufInfo[u4InstID]->u4YStartAddr , 0, _ptH265CurrFBufInfo[u4InstID]->u4DramYSize);
        memset(_ptH265CurrFBufInfo[u4InstID]->u4CStartAddr , 0, _ptH265CurrFBufInfo[u4InstID]->u4DramCSize);
        HalFlushInvalidateDCache();
    }
    #endif

    //UFO mode clear Dram for golden compare
...
// *********************************************************************
// Function    : UINT32 vHEVCSearchRealPic(UINT32 u4InstID,UCHAR ucVldID)
// Description : Search for the real pic then to dec
// Parameter   : None
// Return      : UINT32
// *********************************************************************
UINT32 vHEVCSearchRealPic(UINT32 u4InstID,UCHAR ucVldID)
{
  ...
  VDEC_INFO_H265_BS_INIT_PRM_T rH265BSInitPrm = {0,0,0,0,0,0};
  UINT32 u4Bits = 0;    
  ...
  {    
    ...
    {
        #ifdef HEVC_VP_MODE  
        printk("[INFO] update Current rptr here 0x%x!\n",_u4CurrPicStartAddr[u4InstID]);
        ...
        {
            _u4CurrPicStartAddr[u4InstID] = u4VDEC_HAL_H265_ReadRdPtr(_u4BSID[u4InstID], ucVldID, (UINT32)_pucVFifo[u4InstID], &u4Bits);
        }
        #endif
    }
    else 
    ...
    {
        //reset dual core HW
        vVDecResetMultiCore(ucVldID);
        vVDecConfigMultiCore(ucVldID,PHYSICAL(_pucBSCABACBuff[u4InstID]),PHYSICAL(_pucECInfoBuff[u4InstID]), 0);
    }
    else if(_u4DualCoreEnable[u4InstID] == 2 ) // LAE + MCore resethw
    {
        vVDecResetLAEMultiCore(ucVldID);
        vVDecConfigMultiCore(ucVldID,PHYSICAL(_pucBSCABACBuff[u4InstID]),PHYSICAL(_pucECInfoBuff[u4InstID]), 1);
    }
    
    i4VDEC_HAL_H265_InitVDecHW(ucVldID);

    if (bSkipToDecodeFlag == 1)
    {
        i4VDEC_HAL_H265_InitBarrelShifter(_u4BSID[u4InstID], ucVldID, &rH265PreBSInitPrm);
    ...
    {
        i4VDEC_HAL_H265_InitBarrelShifter(_u4BSID[u4InstID], ucVldID, &rH265BSInitPrm);
    }

    u4Temp = u4VDEC_HAL_H265_GetStartCode_PicStart(_u4BSID[u4InstID], ucVldID) & 0xffff;
    ...
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    UTIL_Printf("//[VDEC %d] NalHdr: 0x%04X  _ucNalUnitType=%d  _u4NuhLayerId=%d  _u4NuhTemporalId=%d Pic #%d \n",u4InstID,u4Temp,_ucNalUnitType[u4InstID], _u4NuhLayerId[u4InstID], _u4NuhTemporalId[u4InstID],_u4PicCnt[u4InstID] );
    #endif
    u4Temp = u4VDEC_HAL_H265_ShiftGetBitStream(_u4BSID[u4InstID], ucVldID, 0);
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    UTIL_Printf("[INFO] NAL Current input window: 0x%08x \n",u4Temp );
    UTIL_Printf("Init BS rptr[0x%08x] Cur rptr [0x%08x]\n",_u4CurrPicStartAddr[u4InstID],u4VDEC_HAL_H265_ReadRdPtr(_u4BSID[u4InstID], u4InstID, (UINT32)_pucVFifo[u4InstID], &u4Bits));
    #endif      

    if(_u4DualCoreEnable[u4InstID] == 1)
    {
        i4VDEC_HAL_H265_InitVDecHW(1);
        i4VDEC_HAL_H265_InitBarrelShifter(_u4BSID[u4InstID], 1, &rH265BSInitPrm);
        u4Temp = u4VDEC_HAL_H265_GetStartCode_PicStart(_u4BSID[u4InstID], 1) & 0xffff;
        fgForbidenZeroBits = ((u4Temp >> 15) & 0x01); // bit 15
        if(fgForbidenZeroBits != 0)
        {
            vHEVCErrInfo(VER_FORBIDEN_ERR);
        ...
        _ucNalUnitType[1] = ((u4Temp>>9) & 0x3f); // bit 9 10 11 12 13 14
        _u4NuhLayerId[1] =  ((u4Temp>>3) & 0x3f); // bit 3 4 5 6 7 8
        _u4NuhTemporalId[1] = (u4Temp & 0x07)-1; // bit 0 1 2
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        UTIL_Printf("//[VDEC %d] NalHdr: 0x%04X  _ucNalUnitType=%d  _u4NuhLayerId=%d  _u4NuhTemporalId=%d Pic #%d\n",VDEC_VLD1,u4Temp,_ucNalUnitType[1], _u4NuhLayerId[1], _u4NuhTemporalId[1],_u4PicCnt[u4InstID] );
        #endif
        u4Temp = u4VDEC_HAL_H265_ShiftGetBitStream(_u4BSID[u4InstID], 1, 0);
    }
    ...
    switch(_ucNalUnitType[u4InstID])
    {
        ...
        //record au address for core1 usage.
            _u4AUParseStartAddr[u4InstID] = _u4CurrPicStartAddr[u4InstID];
            if (1)//( _u4SkipPicNum[u4InstID] >= _u4StartCompPicNum[u4InstID] )
            {
                u4RetVal = vHEVCParseSliceHeader(u4InstID,ucVldID);
                if(0)//(_u4DualCoreEnable[u4InstID] == 1)
                {
                    UINT32 u4RetValCore1;
                    UINT32 u4VDecID = VDEC_VLD1;
                    UINT32 u4McoreInst = 1;
                    _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH265DecPrm.ucVDecStatus = VDEC_CORE1_ST;
                    u4RetValCore1 = vHEVCParseSliceHeader(u4McoreInst,u4VDecID);
                }
            }
            else
            {
        ...
        case NAL_UNIT_SPS:
            ...
            if(_u4DualCoreEnable[u4InstID] == 1)
            {
                memcpy(_rH265SPS[1],_rH265SPS[0],sizeof(H265_SPS_Data)*32);
            }
            break;      
        case NAL_UNIT_PPS:
            ...
            if(_u4DualCoreEnable[u4InstID] == 1)
            {
                memcpy(_rH265PPS[1],_rH265PPS[0],sizeof(H265_PPS_Data)*256);
            }
            break;         
			break;
    ...
    if (u4RetVal != PARSE_OK){
        UTIL_Printf("Parser faile RetVal 0x%x\n",u4RetVal);
        break;
    }
...
UINT32 vHEVCLAEParseSliceNal(UINT32 u4InstID,UCHAR ucVldID)
{
    UINT32 u4RetVal = PARSE_OK;
    UINT32 u4PreIDRPicCnt = 0;
    UINT32 u4RollBackBitCnt,u4RollBackPrePtr, u4RollBackBC;
    ...
    rH265BSInitPrm.u4VFifoSa = (UINT32)_pucVFifo[u4InstID];
    rH265BSInitPrm.u4VFifoEa = (UINT32)_pucVFifo[u4InstID] + V_FIFO_SZ;
    rH265BSInitPrm.u4VLDRdPtr = (UINT32)_pucVFifo[u4InstID] + _u4LAEParseStartAddr[u4InstID];
    ...
    i4VDEC_HAL_H265_LAE_InitVDecHW(ucVldID);
    i4VDEC_HAL_H265_LAE_InitBarrelShifter(_u4BSID[u4InstID], ucVldID, &rH265BSInitPrm);
    u4Temp = u4VDEC_HAL_H265_LAE_GetStartCode_PicStart(_u4BSID[u4InstID], ucVldID) & 0xffff;
    ...
    _ucNalUnitType[u4LAEID] = ((u4Temp>>9) & 0x3f); // bit 9 10 11 12 13 14
    _u4NuhLayerId[u4LAEID] =  ((u4Temp>>3) & 0x3f); // bit 3 4 5 6 7 8
    _u4NuhTemporalId[u4LAEID] = (u4Temp & 0x07)-1; // bit 0 1 2
    //printk("\n[ERROR] ER2!!!_ucNalUnitType[u4InstID] 0x%08x============================= \n", _ucNalUnitType[u4InstID] );
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    printk("//[INFO-LAE] NalHdr: 0x%04X  _ucNalUnitType=%d  _u4NuhLayerId=%d  _u4NuhTemporalId=%d\n",u4Temp,_ucNalUnitType[u4LAEID], _u4NuhLayerId[u4LAEID], _u4NuhTemporalId[u4LAEID] );
    #endif
    u4Temp = u4VDEC_HAL_H265_ShiftGetBitStream(_u4BSID[u4InstID], ucVldID, 0);
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    switch(_ucNalUnitType[u4LAEID])
    {
        ...
        case NAL_UNIT_CODED_SLICE_RASL_N:
        case NAL_UNIT_CODED_SLICE_TFD:
            u4RetVal = vHEVCParseSliceHeader(VDEC_LAE0,ucVldID);
            break;
        case NAL_UNIT_SEI:
            vHEVCVerifySEI_Rbsp(u4LAEID);
            break;
        case NAL_UNIT_SPS:
            u4RetVal = vHEVCVerifySPS_Rbsp(u4LAEID);
            break;      
        case NAL_UNIT_PPS:
            u4RetVal = vHEVCVerifyPPS_Rbsp(u4LAEID);
            break;         
        default:
            break;
        }
        //search next start code start point.
        if (u4RetVal != PARSE_OK){
            UTIL_Printf("Parser faile RetVal 0x%x\n",u4RetVal);
            break;
        }
        _u4LAEParseStartAddr[u4InstID] = u4VDEC_HAL_H265_ReadRdPtr(_u4BSID[u4InstID], ucVldID, (UINT32)_pucVFifo[u4InstID], &u4Bits);
    }while(!((0<=_ucNalUnitType[u4LAEID] && _ucNalUnitType[u4LAEID] <=9)||(16<=_ucNalUnitType[u4LAEID] && _ucNalUnitType[u4LAEID] <=21) ));

    return u4RetVal;
}


UINT32 vHEVCLAETrigDec(UINT32 u4InstID)
{
    i4VDEC_HAL_H265_DecStart(u4InstID,NULL);
}

BOOL fgHEVCDecIsFinish(UINT32 u4InstID, unsigned long  start_time)
{
    UINT32 u4Timeout = 0;
    UINT32 u4Loop = 0;
    ...
    while(!fgDecFinish)
    {
        if(((u4VDecReadMCORETOP(u4InstID, 0xC) >> (24 + u4InstID - VDEC_LAE0)) & 0x1))
        {
            fgDecFinish = TRUE;
            break;
        }
...
{    
    ...
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            DBG_H265_PRINTF(pfLogFile,"================== PTL parameters[%d] ==================\n",u4InstID );
    #endif
    ...
    {
        DBG_H265_PRINTF(pfLogFile, "Main Still Picture Profile");
...
#if DEBUG_LEVEL >= DBG_LEVEL_INFO
  printk ("[INFO] vHEVCVerifySPS_Rbsp(%d) start!!\n",u4InstID);
#endif

    if((u4InstID == VDEC_LAE0) || (u4InstID == VDEC_VLD1) || (u4InstID == VDEC_LAE1))
    {
        _u4BSID[u4InstID] = u4InstID;
    }
    ...
    if ( u4TmpMaxTLayersMinus1==0 ){
     ...
     if(u4SeqParameterSetId < 32){
         ...
         pH265_SPS_DataInst->u4SPSId = u4SeqParameterSetId;
     ...
     vHEVCVerifyInitSPS(u4InstID,pH265_SPS_DataInst);
     ...
     else if(pH265_SPS_DataInst->u4BitDepthLumaMinus8 == 2 && pH265_SPS_DataInst->u4BitDepthChromaMinus8 == 2)
     {
        ...
        _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMain10Mode = HEVC_COMPACT_MODE;
        if(_u4DualCoreEnable[u4InstID] == 1)
        {
            _u4Main10Mode = HEVC_COMPACT_MODE;
            _tVerMpvDecPrm[VDEC_VLD1].SpecDecPrm.rVDecH265DecPrm.ucMain10Mode = HEVC_COMPACT_MODE;
        }
     }
    ...
    {   
        #if DEBUG_LEVEL >= DBG_LEVEL_BUG1
        DBG_H265_PRINTF(pfLogFile,"u4Idx %d pH265_SPS_DataInst 0x%x 0X%X\n",u4Idx,(UINT32)pH265_SPS_DataInst,(UINT32)(pH265_SPS_DataInst->pShortTermRefPicSets[u4Idx]) );
        #endif
        ASSERT(pH265_SPS_DataInst->pShortTermRefPicSets[u4Idx] != NULL);
...
#if DEBUG_LEVEL >= DBG_LEVEL_INFO
  printk ("[INFO] vHEVCVerifyPPS_Rbsp(%d) start!!\n",u4InstID);
#endif
    ...
    for( sizeId = 0; sizeId < SCALING_LIST_SIZE_NUM; sizeId++)
    {
        for( listId = 0; listId < H265_scalingListNum[sizeId]; listId++)
        {
            pH265_PPS_DataInst->rPPS_ScalingList.pScalingListDeltaCoef[sizeId][listId] = _rPPSScalingListDeltaCoef[u4InstID][u4PicParameterSetId][sizeId][listId];
        }
    }
    ...
    {
        if(u4PicParameterSetId > MAX_H265_PPS_THD)
        {
            printk("Scaling list array not enough assert!\n");
            ASSERT(0);
        }
...
// *********************************************************************
// Function    : void vHEVCVerifyInitTilesInfo(UINT32 u4VDecID,H265_SPS_Data *prSPS,H265_PPS_Data *prPPS,VDEC_INFO_H265_PIC_INFO_T *prH265PicInfo)
...
void vHEVCVerifyInitTilesInfo(UINT32 u4VDecID,H265_SPS_Data *prSPS,H265_PPS_Data *prPPS,VDEC_INFO_H265_PIC_INFO_T *prH265PicInfo)
{
    ...
    else
    {
        ...
        for(j = 0; j < prPPS->u4NumRowsMinus1+1; j++)
        {
            ...
            prH265PicInfo->rTileInfo[j * (prPPS->u4NumColumnsMinus1+1) + i].u4TileWidth = ( i4Lcu_pic_width - uiCummulativeTileWidth );
        }
        ...
        for(j = 0; j < prPPS->u4NumColumnsMinus1+1; j++)
        {
            ...
            prH265PicInfo->rTileInfo[i * (prPPS->u4NumColumnsMinus1+1) + j].u4TileHeight = ( i4Lcu_pic_height - uiCummulativeTileHeight );
        }
    }
    ...
    for( uiRowIdx = 0; uiRowIdx < prPPS->u4NumRowsMinus1+1; uiRowIdx++ )
    {
        for( uiColumnIdx = 0; uiColumnIdx < prPPS->u4NumColumnsMinus1+1; uiColumnIdx++ )
        {
            ...
            {
                uiRightEdgePosInCU += prH265PicInfo->rTileInfo[uiRowIdx * (prPPS->u4NumColumnsMinus1+1) + i].u4TileWidth;
            }
            prH265PicInfo->rTileInfo[uiTileIdx].u4RightEdgePosInCU = (uiRightEdgePosInCU-1);

            //initialize the BottomEdgePosInCU for each tile
            uiBottomEdgePosInCU = 0;
            for( i = 0; i <= uiRowIdx; i++ )
            {
                uiBottomEdgePosInCU += prH265PicInfo->rTileInfo[i * (prPPS->u4NumColumnsMinus1+1) + uiColumnIdx].u4TileHeight;
            }
            prH265PicInfo->rTileInfo[uiTileIdx].u4BottomEdgePosInCU = (uiBottomEdgePosInCU-1);

            //initialize the FirstCUAddr for each tile
            prH265PicInfo->rTileInfo[uiTileIdx].u4FirstCUAddr = ( (prH265PicInfo->rTileInfo[uiTileIdx].u4BottomEdgePosInCU - 
            prH265PicInfo->rTileInfo[uiTileIdx].u4TileHeight +1)*i4Lcu_pic_width + 
            prH265PicInfo->rTileInfo[uiTileIdx].u4RightEdgePosInCU - prH265PicInfo->rTileInfo[uiTileIdx].u4TileWidth + 1);
...
void vHEVCVerifyInitSPS(UINT32 u4InstID,H265_SPS_Data *prSPS)
{
    ...
    // init scaling list
    if(prSPS->u4SPSId > MAX_H265_SPS_THD)
    {
        printk("Sps ID too large, reallocate scaling list arry!\n");
        ASSERT(0);
    }
    ...
    for( sizeId = 0; sizeId < SCALING_LIST_SIZE_NUM; sizeId++)
    {
        for( listId = 0; listId < H265_scalingListNum[sizeId]; listId++)
        {
            prSPS->rSPS_ScalingList.pScalingListDeltaCoef[sizeId][listId] = _rSPSScalingListDeltaCoef[u4InstID][prSPS->u4SPSId][sizeId][listId];
        }
    }
...
UINT32 vHEVCParseSliceHeader(UINT32 u4InstID,UCHAR ucVldID)
{
    ...
    if(ucVldID == VDEC_VLD1)
    {
        _u4BSID[u4InstID] = VDEC_VLD1;
    }
    ...
    ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPicInfo = &_rH265PicInfo[u4InstID];
    ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prFBufInfo = &_ptH265FBufInfo[u4InstID][0];
    ...
    pH265_Slice_Hdr_DataInst->bFirstSliceSegmentInPic = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    {      //IRAP picture
        i4tmp = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            DBG_H265_PRINTF(pfLogFile,"no_output_of_prior_pics_flag  %d\n", i4tmp );
        #endif
    }
    pH265_Slice_Hdr_DataInst->u4PPSID = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    else
    {
        ...
        vErrMessage(u4InstID, (CHAR *)_bTempStr1[u4InstID]);
        if ( _rH265PPS[u4InstID][pH265_Slice_Hdr_DataInst->u4PPSID].bPPSValid )
        {       // given PPS id is valid
            ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS = &_rH265PPS[u4InstID][pH265_Slice_Hdr_DataInst->u4PPSID];
            pH265_PPS_DataInst = ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prPPS;
        }
        else if ( _rH265PPS[u4InstID][0].bPPSValid )
        {                                                 // PPS id = 0 is valid
        ...
        if ( _rH265SPS[u4InstID][_rH265PPS[u4InstID][pH265_Slice_Hdr_DataInst->u4PPSID].u4SeqParameterSetId].bSPSValid )
        {       // given SPS id is valid
            ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS = &_rH265SPS[u4InstID][pH265_PPS_DataInst->u4SeqParameterSetId];
            pH265_SPS_DataInst = ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.prSPS;

        }
        else if ( _rH265SPS[u4InstID][0].bSPSValid )
        {                                                   // SPS id = 0 is valid
    ...
    {
        pH265_Slice_Hdr_DataInst->bDependentSliceSegmentFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    {
        pH265_Slice_Hdr_DataInst->u4SliceSegmentAddress  = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID],ucVldID,i4BitsSliceSegmentAddress);//u(v)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    if ( 0==pH265_Slice_Hdr_DataInst->bDependentSliceSegmentFlag )
    {
        ...
        pH265_Slice_Hdr_DataInst->u4SliceType = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {
            pH265_Slice_Hdr_DataInst->bPicOutputFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        if ( pH265_Slice_Hdr_DataInst->bFirstSliceSegmentInPic )
        {
            ...
            hevc_slice_0_reg = (( 0 & 0xfffff) << 0) |
                                            ...
                                            ((isIDRflag & 0x1) << 31); 
            vVDecWriteHEVCVLD(ucVldID, RW_HEVC_SLICE_0, hevc_slice_0_reg);
            vHEVCVerifyInitPicInfo(u4InstID);
        }
        
        if ( pH265_SPS_DataInst->bSeparateColourPlaneFlag )
        {
            pH265_Slice_Hdr_DataInst->u4ColourPlaneID = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID,2);//u(2)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        else
        {
            ...
            pH265_Slice_Hdr_DataInst->i4POCLsb = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID,pH265_SPS_DataInst->u4Log2MaxPicOrderCntLsbMinus4 + 4);//u(v)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            ...
            else
            {
                ...
                if ( i4NumBits > 0 )
                {
                    i4tmp = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID,i4NumBits);//u(v)
                    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            ...
            if ( 1 == pH265_SPS_DataInst->bLongTermRefsPresent )
            {
                ...
                {
                    pH265_Slice_Hdr_DataInst->u4NumOfLongTermSPS = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
                    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                ...
                pH265_Slice_Hdr_DataInst->u4NumOfLongTermPics = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
                #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                ...
                for ( i4Indexer2 = iOffset+pSliceRPS->u4NumberOfLongtermPictures-1, i4Indexer3 = 0; i4Indexer3 < i4NumOfLtrp; i4Indexer2--, i4Indexer3++)
                {
                    ...
                    {
                        i4tmp = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID,pH265_SPS_DataInst->u4Log2MaxPicOrderCntLsbMinus4+4);//u(v)
                        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                    ...
                    {       
                        //SBPresentFlag
                        i4tmp = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
                        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                ...
                || u4NalType == NAL_UNIT_CODED_SLICE_BLA_N_LP ) 
           {
            ...
            {
                pH265_Slice_Hdr_DataInst->bTMVPFlagsPresent = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
                #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {
            pH265_Slice_Hdr_DataInst->bSaoEnabledFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                DBG_H265_PRINTF(pfLogFile,"slice_sao_luma_flag %d\n", pH265_Slice_Hdr_DataInst->bSaoEnabledFlag);
            #endif
            pH265_Slice_Hdr_DataInst->bSaoEnabledFlagChroma = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        if ( pH265_Slice_Hdr_DataInst->u4SliceType != HEVC_I_SLICE )
        {  
            // not I slice
            i4tmp = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {  
            ...
            if ( pH265_Slice_Hdr_DataInst->bFirstSliceSegmentInPic )
            {
                    ...
                    vVDecWriteHEVCVLD(ucVldID, RW_HEVC_SLICE_2, hevc_slice_2_reg);  
            }
            ...
            if ( pH265_PPS_DataInst->bListsModificationPresentFlag && i4NumRpsCurrTempList > 1 )
            {
                ...
                if ( pH265_Slice_Hdr_DataInst->bFirstSliceSegmentInPic )
                {
                    ...
                    hevc_ref_pic_list_mod_reg = ((i4NumRpsCurrTempList & 0xf) << 0) |
								      ((length & 0x7) << 4);
                    vVDecWriteHEVCVLD(ucVldID, RW_HEVC_REF_PIC_LIST_MOD, hevc_ref_pic_list_mod_reg);  

                    /////////////////////////////////////////////////////////////////////
                    vHEVCVerifyRef_Pic_List_Modification(ucVldID, pH265_Slice_Hdr_DataInst);
                    /////////////////////////////////////////////////////////////////////
            ...
            if ( pH265_Slice_Hdr_DataInst->u4SliceType == HEVC_B_SLICE )
            {
                pH265_Slice_Hdr_DataInst->bMvdL1ZeroFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
                #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            ...
            {
                pH265_Slice_Hdr_DataInst->bCabacInitFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
                #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            ...
            if ( 1==pH265_Slice_Hdr_DataInst->bTMVPFlagsPresent )
            {
                if ( pH265_Slice_Hdr_DataInst->u4SliceType == HEVC_B_SLICE )
                {
                    pH265_Slice_Hdr_DataInst->bColFromL0Flag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
                    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                ...
                {
                    pH265_Slice_Hdr_DataInst->u4ColRefIdx = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
                    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            ...
            {     
                /////////////////////////////////////////////////////////////////////
                vVDEC_HAL_H265_PredWeightTable(ucVldID);
                /////////////////////////////////////////////////////////////////////
            }
            pH265_Slice_Hdr_DataInst->u4FiveMinusMaxNumMergeCand = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                DBG_H265_PRINTF(pfLogFile,"five_minus_max_num_merge_cand %d\n", pH265_Slice_Hdr_DataInst->u4FiveMinusMaxNumMergeCand );
            #endif
        }

        i4tmp= i4VDEC_HAL_H265_SeCodeNum(_u4BSID[u4InstID], ucVldID);//se(v)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {
            pH265_Slice_Hdr_DataInst->i4SliceQpDeltaCb = i4VDEC_HAL_H265_SeCodeNum(_u4BSID[u4InstID], ucVldID);//se(v)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                DBG_H265_PRINTF(pfLogFile,"slice_qp_delta_cb %d\n", pH265_Slice_Hdr_DataInst->i4SliceQpDeltaCb );
            #endif
            pH265_Slice_Hdr_DataInst->i4SliceQpDeltaCr = i4VDEC_HAL_H265_SeCodeNum(_u4BSID[u4InstID], ucVldID);//se(v)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        if (1==pH265_PPS_DataInst->bDeblockingFilterControlPresentFlag)
        {
            ...
            {
                pH265_Slice_Hdr_DataInst->bDeblockingFilterOverrideFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
                #if DEBUG_LEVEL >= DBG_LEVEL_INFO
            ...
            {
                pH265_Slice_Hdr_DataInst->bDeblockingFilterDisableFlag = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID, 1);//u(1)
                #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                ...
                {
                    pH265_Slice_Hdr_DataInst->i4DeblockingFilterBetaOffsetDiv2 = i4VDEC_HAL_H265_SeCodeNum(_u4BSID[u4InstID], ucVldID);//se(v)
                    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
                        DBG_H265_PRINTF(pfLogFile,"beta_offset_div2 %d\n", pH265_Slice_Hdr_DataInst->i4DeblockingFilterBetaOffsetDiv2 );
                    #endif
                    pH265_Slice_Hdr_DataInst->i4DeblockingFilterTcOffsetDiv2 = i4VDEC_HAL_H265_SeCodeNum(_u4BSID[u4InstID], ucVldID);//se(v)
                    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    {
        pH265_Slice_Hdr_DataInst->u4NumEntryPointOffsets = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {
            pH265_Slice_Hdr_DataInst->u4OffsetLenMinus1 = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {
            i4tmp = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID,pH265_Slice_Hdr_DataInst->u4OffsetLenMinus1+1);//u(v)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    if((ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.ucVDecStatus != VDEC_LAE_ST) &&
       (ptVerMpvDecPrm->SpecDecPrm.rVDecH265DecPrm.ucVDecStatus != VDEC_CORE1_ST))
    {
     ...
     ////////////////////////////////set Pic INFO ////////////////////////////////
     if ( PARSE_OK != vHEVCVerifyVDecSetPicInfo(u4InstID,ucVldID,ptVerMpvDecPrm) )
     {
        #if DEBUG_LEVEL >= DBG_LEVEL_BUG1
    ...
    {
        pH265_Slice_Hdr_DataInst->u4SliceHeaderExtensionLength = u4VDEC_HAL_H265_UeCodeNum(_u4BSID[u4InstID], ucVldID);//ue(v)
        #if DEBUG_LEVEL >= DBG_LEVEL_INFO
        ...
        {
            i4tmp = u4VDEC_HAL_H265_GetRealBitStream(_u4BSID[u4InstID], ucVldID,8);//u(8)
            #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    ...
    vVDEC_HAL_H265_TrailingBits(_u4BSID[u4InstID], ucVldID);
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    printk ("[INFO] vHEVCParseSliceHeader() done!!\n");
#endif
    printk("[H265-%d] input window is 0x%08x\n",u4InstID,u4VDEC_HAL_H265_GetBitStreamShift(_u4BSID[u4InstID],ucVldID,0));
    return PARSE_OK;
}

// *********************************************************************
// Function    : void vHEVCVerifyRef_Pic_List_Modification(UINT32 u4InstID, VDEC_INFO_H265_SLICE_HDR_T *prSliceHdr)
...
void vVDecH265SetRegister(UINT32 u4InstID,UINT32 ucVldID,VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm)
{
    UCHAR u2ECFbIndex = 0xFF;
    VDEC_INFO_H265_FBUF_INFO_T *prTempFBufInfo = NULL;
    VDEC_INFO_DEC_PRM_T  *ptVerMpvDecPrm;
...
void vAllocateScaleList(UINT32 u4InstID)
{
    int u4InstId,u4SizeId,u4ListId;
    UINT32 u4PpsId;
    UINT32 u4TotalSize = 0;
    printk("%s %d ",__FUNCTION__,u4InstID);
    //Allocate scaling list array
//    for(u4InstId = 0; u4InstId < 3; u4InstId += 2)
    {
        for(u4PpsId = 0; u4PpsId < MAX_H265_PPS_THD; u4PpsId ++)
        {
            for( u4SizeId = 0; u4SizeId < SCALING_LIST_SIZE_NUM; u4SizeId++)
            {
                for( u4ListId = 0; u4ListId < H265_scalingListNum[u4SizeId]; u4ListId++)
                {
                    int size = (MAX_MATRIX_COEF_NUM < H265_scalingListSize[u4SizeId])? MAX_MATRIX_COEF_NUM : H265_scalingListSize[u4SizeId];
                    //prSPS->rSPS_ScalingList.pScalingListDeltaCoef[u4SizeId][u4ListId] = (INT32*)vmalloc( size*sizeof(INT32) );
                    if(_rPPSScalingListDeltaCoef[u4InstID][u4PpsId][u4SizeId][u4ListId] == NULL)
                    {
                        if(u4PpsId < MAX_H265_SPS_THD)
                        {
                            u4TotalSize += size;
                            #if 1
                            _rSPSScalingListDeltaCoef[u4InstID][u4PpsId][u4SizeId][u4ListId] = (INT32*)x_alloc_aligned_verify_mem(size*sizeof(INT32),16,0);
                            #else
                            _rSPSScalingListDeltaCoef[u4InstId][u4PpsId][u4SizeId][u4ListId] = (INT32*)vmalloc( size*sizeof(INT32) );
                            #endif
                        }
                        u4TotalSize += size;
                        #if 1
                        _rPPSScalingListDeltaCoef[u4InstID][u4PpsId][u4SizeId][u4ListId] = (INT32*)x_alloc_aligned_verify_mem(size*sizeof(INT32),16,0);
                        #else
                        _rPPSScalingListDeltaCoef[u4InstId][u4PpsId][u4SizeId][u4ListId] = (INT32*)vmalloc( size*sizeof(INT32) );
                        #endif
                    }
                }
            }
        }
        printk("Total allocate 0x%x byte\n",u4TotalSize*4);
    }
}

void vAllocateRPS(UINT32 u4InstID)
{
    UINT32 i,j;
    UINT32 u4TotalSize = 0;
    printk("%s %d  ",__FUNCTION__,u4InstID);
    for (i = 0; i< 32 ; i++)
    {
        ...
        for (j = 0; j < 64; j++)
        {
            ...
            {
                _rH265SPS[u4InstID][i].pShortTermRefPicSets[j] = (pH265_RPS_Data) x_alloc_aligned_verify_mem(sizeof(H265_RPS_Data),16,0);
                if((i < 5) && (j < 5))
                    printk("RPS[%d][%d][%d] = 0x%x\n",u4InstID,i,j,_rH265SPS[u4InstID][i].pShortTermRefPicSets[j]);

            }

        }
    }
    u4TotalSize = sizeof(H265_RPS_Data)*32*64;
    printk("Total allocate 0x%x byte\n",u4TotalSize);
}
...
void vFreeScaleList(UINT32 u4InstID)
{
    UINT32 u4PpsID,i,j;
...
void vHEVCVerifySEI_Rbsp(UINT32 u4InstID)
{
    UINT32 u4PayLoadType = 0,u4Temp = 0;
    while((u4Temp = (u4VDEC_HAL_H265_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 8) & 0xFF)) == 0xFF)
    {
        u4PayLoadType += u4Temp;
    }
    u4PayLoadType += u4Temp;
    #if DEBUG_LEVEL >= DBG_LEVEL_INFO
    printk("// SEI message payload type 0x%x\n",u4PayLoadType);
    #endif
}
...
#define  START_CODE_TAG		0x000001
#define RESET_OF_BYTE	0x10

UINT32 u4SearStartCode(UINT32 u4InstID,CHAR *pBuf,UCHAR *pNalType)
{
	CHAR *temp = pBuf;
	BOOL fgStartCode = false;
	UINT32 u4Val,u4Ret = 0;
	UCHAR u4NalType = 0xFF;
	*pNalType = 0xFF;
	while(!fgStartCode)
	{
		u4Val = INVERSE_ENDIAN(*(UINT32 *)temp);
		if((u4Val >> 8) == START_CODE_TAG)
		{
			fgStartCode = true;
			u4Ret = (UINT32)temp - (UINT32)pBuf;
			//temp ++;
			temp ++;
			u4Val = INVERSE_ENDIAN(*(UINT32 *)temp);
            //printf("value0 0x%08x \n",u4Val);
			u4NalType = (u4Val >> 9) & 0x3F; 
			*pNalType = u4NalType;
            temp ++;
			u4Val = INVERSE_ENDIAN(*(UINT32 *)temp);
            //printf("value1 0x%08x \n",u4Val);
            //multi-slice case
            if(((u4NalType < 10) || (u4NalType > 15 && u4NalType < 22)) && ((u4Val & 0x80) == 0))
            {
                fgStartCode = false;
            }
		}
		...
		temp ++;
		if(((UINT32)temp - (UINT32)_pucVFifo[u4InstID]) > (_tVerMpvDecPrm[u4InstID].u4BitstreamLen - RESET_OF_BYTE))
		{
			break;
		}
	}
...
void vHEVCFastVirtualParse(UINT32 u4InstID)
{
    #if 0
    UINT32 u4Temp,u4PicCnt = 0;
    UINT32 u4Rdptr = 0,u4Bits = 0;
    UCHAR ucNalUnitType;
    UINT32 u4PicHeader = 0x00;
    BOOL fgFirstNonVclNal = TRUE,fgFirstVclNal = TRUE;
    ...
    while(u4Rdptr < _tVerMpvDecPrm[u4InstID].u4BitstreamLen)
    {
        u4Temp = u4VDEC_HAL_H265_GetStartCode_PicStart(_u4BSID[u4InstID], u4InstID) & 0xffff;
        ucNalUnitType = ((u4Temp>>9) & 0x3f); // bit 9 10 11 12 13 14
        u4Rdptr = u4VDEC_HAL_H265_ReadRdPtr(_u4BSID[u4InstID], u4InstID, (UINT32)_pucVFifo[u4InstID], &u4Bits);

        if(u4Rdptr < _u4PicInfo[u4PicCnt - 1])
        {
            break;
        }
        UTIL_Printf("NalType = 0x%x rdptr = 0x%x @cnt %d\n",ucNalUnitType,u4Rdptr,u4PicCnt);
        if(fgFirstNonVclNal)
        {
            _u4PicInfo[u4PicCnt] = u4Rdptr - 4;
            UTIL_Printf("Picture %d @ rdptr 0x%x\n",u4PicCnt,_u4PicInfo[u4PicCnt]);
            fgFirstNonVclNal = FALSE;
            u4PicCnt++;
        }
        
        if(ucNalUnitType >= NAL_UNIT_VPS)
        {
            
        }
        else
        {
            fgFirstNonVclNal = TRUE;
        }
        
    }
    #else
    UINT32 u4FileOffset = 0,u4Size = 0,u4PicCnt = 0;
    INT32 iFileLen = 0;
    CHAR *pTempBuf = NULL;
    CHAR ucNalType;
    BOOL fgFirstNonVclNal = TRUE,fgFirstVclNal = TRUE;
    while(u4FileOffset < (iFileLen - RESET_OF_BYTE))
	{
		pTempBuf = (CHAR *)(_pucVFifo[u4InstID] + u4FileOffset);
		u4Size = u4SearStartCode(u4InstID,pTempBuf,&ucNalType);
		if(ucNalType == 0xFF)
		{
			break;
		}
		...
		if(fgFirstNonVclNal)
        {
            _u4PicInfo[u4PicCnt] = u4FileOffset - 2;
            UTIL_Printf("Picture %d @ rdptr 0x%x nal %d\n",u4PicCnt,_u4PicInfo[u4PicCnt],ucNalType);
            fgFirstNonVclNal = FALSE;
            u4PicCnt++;
        }
        
        if(ucNalType >= NAL_UNIT_VPS)
        {
	...
	_u4EndCompPicNum[u4InstID] = u4PicCnt - 1;
    #endif
