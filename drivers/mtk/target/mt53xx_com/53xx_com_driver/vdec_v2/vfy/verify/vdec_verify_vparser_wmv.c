...
void vRCVFileHeader(UINT32 u4InstID)
{
  ...
  UTIL_Printf("Width:%d, height:%d\n", prWMVSPS->u4PicWidthSrc, prWMVSPS->u4PicHeightSrc);
...
void vWMVSearchSliceStartCode(UINT32 u4InstID)
{
   ...
   while (1)
   {
        ...
        if ( ((UINT32) pVideoData) >=   ((UINT32) _pucVFifo[u4InstID]) /*((_tInFileInfo[u4InstID].u4FileLength << 3) + ((UINT32) _pucVFifo[u4InstID]))*/ )
        {
           break;
        }
...
if (!prWmvDecBpPrm->fgWmvMode)
{
  ...
  UTIL_Printf("vVdecWmvRecBpType xu4DcacSa:0x%x\n", (UINT32)_pucDcac[u4InstID]);
}
else
{
