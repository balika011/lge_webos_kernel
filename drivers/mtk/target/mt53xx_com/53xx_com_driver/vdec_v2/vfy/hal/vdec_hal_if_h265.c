...
void u4VDEC_HAL_H265_SetUFO(UINT32 u4VDecID,VDEC_INFO_H265_FBUF_INFO_T *prCurrentFBuf)
{
    ...
    { 
        ...
        #ifdef MT5890_E3_SUPPORT
        vVDecWriteHEVCPP(u4VDecID,(806 << 2),((u4VDecReadHEVCPP(u4VDecID,(806 << 2)) & 0xFFFFFF00) | 0x96));
        #endif
...
void u4VDEC_HAL_H265_SetECInfo(UINT32 u4VDecID,VDEC_INFO_H265_FBUF_INFO_T *prECRefFBuf,VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm)
{
    ...
    UCHAR i;
    #ifdef VDEC_SIM_DUMP
    LOG(0,"// [INFO] EC settings Y addr 0x%x\n",prECRefFBuf->u4YStartAddr);
    #endif
    // yc seperate enable
    ...
    #ifdef MT5890_E3_SUPPORT
    vVDecWriteHEVCMV(u4VDecID, 132*4, 0 << 4 );
    #endif
}
...
void vVDEC_HAL_H265_InitPicInfo(UINT32 u4VDecID,VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm)
{
    ...
    // 10 bit setting
    vVDecWriteMC(u4VDecID, HEVC_MC_CMPTMODE, (_u4Main10Mode == 3)? 0x1:0x0);
    if(_u4DualCoreEnable[0] == DUAL_CORE_MODE || _u4DualCoreEnable[0] == M10TILE_DUAL_CORE_MODE)
    {
    ...
    {        
        printk("PP Setting For Still Picture Profile!\n");
        vVDecWriteHEVCPP(u4VDecID, HEVC_SWITCH_DRAM_MODE, 0x1);
        vVDecWriteHEVCPP(u4VDecID, HEVC_SAO_WRAPPER_ADDR, PHYSICAL(_pucPPSAOBuf[0]));
        vVDecWriteHEVCPP(u4VDecID, HEVC_DBK_WRAPPER_ADDR, PHYSICAL(_pucPPDBKBuf[0]));
        ...
        vVDecWriteVLDTOP(u4VDecID, (16 << 2), 0x3);
        vVDecWriteVLDTOP(u4VDecID, HEVC_VLD_PRED_ADDR, PHYSICAL(_pucVLDPredBuf[0]));
        vVDecWriteVLDTOP(u4VDecID, HEVC_VLD_IP_ADDR, PHYSICAL(_pucVLDIPBuf[0]));
        vVDecWriteVLDTOP(u4VDecID, HEVC_VLD_VBUF_ADDR, PHYSICAL(_pucVLDVBuf[0]));
    }

    // _u4CRCMode CRC enable register.    
    vVDecWriteMCORETOP(u4VDecID,(192<<2),0x1);
    
    // Single
    if(_u4DualCoreEnable[u4VDecID] == 0)
        vVDecWriteHEVCMISC(u4VDecID, 4, 0x100);
}
...
void vVDEC_HAL_H265_SetRefPicListReg(UINT32 u4VDecID,VDEC_INFO_H265_PIC_INFO_T *prH265PicInfo,VDEC_INFO_H265_FBUF_INFO_T *prH265FBBufInfo)
{
    ...
    minDPOC = MAX_INT;
    for(i=0; i < prH265PicInfo->i4RefListNum; i++)
    { 
        if (abs(prH265PicInfo->i4PocDiffList0[i])<minDPOC)
        {
            minDPOC = abs(prH265PicInfo->i4PocDiffList0[i]);
            u4Value = i;
        }
    }
    vVDecWriteHEVCMV(u4VDecID, 132*4, u4Value<<4 );
...
void vVDEC_HAL_H265_SetSPSHEVLD(UINT32 u4VDecID, H265_SPS_Data *prSPS, H265_PPS_Data *prPPS)
{
    ...
    if(prSPS->u4BitDepthLumaMinus8 != 0 && prSPS->u4BitDepthChromaMinus8!= 0)
    {
       if(prSPS->u4BitDepthLumaMinus8 == 0x2 && prSPS->u4BitDepthChromaMinus8 == 0x2) 
       {
            ...
            #ifdef MT5890_E3_SUPPORT
            vVDecWriteHEVCPP(u4VDecID, HEVC_2BIT_REARRANGE, 0x1);
            #endif
       }
...
void vVDEC_HAL_H265_SetSHDRHEVLD(UINT32 u4VDecID, VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm)
{
    UINT32 u4SHDRInfo;
    INT32 i4Indexer;
    INT32 i4NumRpsCurrTempList = 0;
    INT32 numRpsCurrTempList0 = 0;
    INT32 length = 0;
    BOOL bUseSAO; 
...
// **************************************************************************
// Function : void vVDEC_HAL_H265_SetTilesInfo(UINT32 u4InstID, UCHAR ucVldID,H265_SPS_Data *prSPS, H265_PPS_Data *prPPS)
...
// Return      : None
// **************************************************************************
void vVDEC_HAL_H265_SetTilesInfo(UINT32 u4InstID,UCHAR ucVldID, H265_SPS_Data *prSPS, H265_PPS_Data *prPPS)
{
    ...
    {
      uiRightEdgePosInCU = _rH265PicInfo[u4InstID].rTileInfo[uiColumnIdx].u4RightEdgePosInCU;
    ...
    {
        uiBottomEdgePosInCU = _rH265PicInfo[u4InstID].rTileInfo[uiRowIdx*(prPPS->u4NumColumnsMinus1+1)].u4BottomEdgePosInCU;
      ...
      if (uiBottomEdgePosInCU >= 512)
       ...
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_0,last_lcu_x_in_tile_0_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_1,last_lcu_x_in_tile_1_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_2,last_lcu_x_in_tile_2_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_3,last_lcu_x_in_tile_3_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_4,last_lcu_x_in_tile_4_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_5,last_lcu_x_in_tile_5_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_6,last_lcu_x_in_tile_6_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_7,last_lcu_x_in_tile_7_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_8,last_lcu_x_in_tile_8_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_9,last_lcu_x_in_tile_9_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_10,last_lcu_x_in_tile_10_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_11,last_lcu_x_in_tile_11_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_12,last_lcu_x_in_tile_12_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_13,last_lcu_x_in_tile_13_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_14,last_lcu_x_in_tile_14_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_15,last_lcu_x_in_tile_15_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_X_16,last_lcu_x_in_tile_16_reg);
   
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_0,last_lcu_y_in_tile_0_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_1,last_lcu_y_in_tile_1_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_2,last_lcu_y_in_tile_2_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_3,last_lcu_y_in_tile_3_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_4,last_lcu_y_in_tile_4_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_5,last_lcu_y_in_tile_5_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_6,last_lcu_y_in_tile_6_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_7,last_lcu_y_in_tile_7_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_8,last_lcu_y_in_tile_8_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_9,last_lcu_y_in_tile_9_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_10,last_lcu_y_in_tile_10_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_11,last_lcu_y_in_tile_11_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_12,last_lcu_y_in_tile_12_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_13,last_lcu_y_in_tile_13_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_14,last_lcu_y_in_tile_14_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_15,last_lcu_y_in_tile_15_reg);
       vVDecWriteHEVCVLD(ucVldID,RW_HEVC_TILE_Y_16,last_lcu_y_in_tile_16_reg);
...
void u4VDEC_HAL_InitMVMC(UINT32 u4InstID,UINT32 u4VDecID,VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm)
{
    UINT32 i,u4BufId;
    u4BufId = _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum - 1;
    for(i = 0; i < 16; i++)
    {
        vVDecWriteMC(u4VDecID,(HEVC_MC_REF_B_LIST0_Y + (i<<2)),PHYSICAL(_ptH265FBufInfo[u4InstID][u4BufId].u4YStartAddr));
        vVDecWriteMC(u4VDecID,(HEVC_MC_REF_B_LIST1_Y + (i<<2)),PHYSICAL(_ptH265FBufInfo[u4InstID][u4BufId].u4YStartAddr));
        vVDecWriteMC(u4VDecID,(HEVC_MC_REF_B_LIST0_C + (i<<2)),PHYSICAL(_ptH265FBufInfo[u4InstID][u4BufId].u4CStartAddr));
        vVDecWriteMC(u4VDecID,(HEVC_MC_REF_B_LIST1_C + (i<<2)),PHYSICAL(_ptH265FBufInfo[u4InstID][u4BufId].u4CStartAddr));
        vVDecWriteHEVCMV(u4VDecID,(i << 2),PHYSICAL(_ptH265FBufInfo[u4InstID][u4BufId].u4MvStartAddr) >> 4);
    }
}
...
UINT32 vVDEC_HAL_H265_VDec_VPmode(UINT32 u4VDecID)
{
    ...
    if( risc_val1 & 0x1 )
    {
        ...
        for (i=0; i<_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH265DecPrm.ucMaxFBufNum; i++ ){
            ...
            UTIL_Printf("CurrentFB POC %d vs FbufPOC %d\n",_ptH265CurrFBufInfo[u4VDecID]->i4POC,_ptH265FBufInfo[u4VDecID][i].i4POC);
        }
        if (minDPOC==9999999){
...
//        rH265VDecInitPrm.u4FGSeedbase = (UINT32)_pucFGSeedbase[u4VDecID];
        ...
        UTIL_Printf("Copy from x/y 0x%x,0x%x to y/c 0x%x,0x%x\n",(_ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4YStartAddr),(_ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4CStartAddr),
                                                                 (_ptH265CurrFBufInfo[u4VDecID]->u4YStartAddr),(_ptH265CurrFBufInfo[u4VDecID]->u4CStartAddr));
...
//        vVDecWriteMC(u4VDecID, 3*4, (PHYSICAL(_ptH265CurrFBufInfo[u4VDecID]->u4YStartAddr + _ptH265CurrFBufInfo[u4VDecID]->u4CAddrOffset)+ u4W_Dram*SliceStartLCURow*u4LCUsize/2)>>8);
        //YC sperate
        vVDecWriteMC(u4VDecID, 0*4, (PHYSICAL(_ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4YStartAddr) + u4W_Dram*SliceStartLCURow*u4LCUsize)>>9);
        vVDecWriteMC(u4VDecID, 1*4, (PHYSICAL(_ptH265FBufInfo[u4VDecID][ucRefFBIndex].u4CStartAddr)+ u4W_Dram*SliceStartLCURow*u4LCUsize/2)>>8);
        vVDecWriteMC(u4VDecID, 2*4, (PHYSICAL(_ptH265CurrFBufInfo[u4VDecID]->u4YStartAddr)+ u4W_Dram*SliceStartLCURow*u4LCUsize)>>9);
        vVDecWriteMC(u4VDecID, 3*4, (PHYSICAL(_ptH265CurrFBufInfo[u4VDecID]->u4CStartAddr)+ u4W_Dram*SliceStartLCURow*u4LCUsize/2)>>8);
...
void vVDEC_HAL_H265_VDec_DumpReg(UINT32 u4VDecID, BOOL bDecodeDone)
{
    ...
    if(bDecodeDone)
    {
        ...
        #if DUAL_VDEC_TEST
        Dump_reg (VLD_TOP_REG_OFFSET0, 53, 54, pBitstream_name, _u4PicCnt[u4VDecID], 1);       //PP
        #endif
