...
void vVDecVP6SetVLD2Wptr(UINT32 u4WPtr)
{
   ...
   UTIL_Printf("<vdec> u4VStart is 0x%x, u4VEnd is 0x%x, u4WPtrAlign is 0x%x\n",u4VStart, u4VEnd, u4WPtrAlign);
   #if 0
   ...
   UTIL_Printf("<vdec> u4WPtrAlign is 0x%x\n",u4WPtrAlign);
    #if VDEC_SUPPORT_IOMMU
    vVDecVP6WriteVLD2(WO_VLD_WPTR, (u4WPtrAlign));
	#else
    vVDecVP6WriteVLD2(WO_VLD_WPTR, PHYSICAL(u4WPtrAlign));
	#endif

	UTIL_Printf("<vdec> vVDecVP6SetVLD2Wptr end\n");
}
