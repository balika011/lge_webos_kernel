...
void vVDecWriteAVCVLD(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
	...
	{
        vWriteReg(LAE_AVCVLD_OFFSET0 + u4Addr, u4Val);
#ifdef VDEC_SIM_DUMP
        vVDecSimDumpW(u4VDecID, LAE_AVCVLD_OFFSET0, u4Addr, u4Val);
#endif
    }
	else
	{
		vWriteReg(LAE_AVCVLD_OFFSET1 + u4Addr, u4Val);
#ifdef VDEC_SIM_DUMP
        vVDecSimDumpW(u4VDecID, LAE_AVCVLD_OFFSET1, u4Addr, u4Val);
#endif
	}
}
...
UINT32 u4VDecReadAVCVLD(UINT32 u4VDecID, UINT32 u4Addr)
{
    ...
    {    	
        UINT32 u4Val = u4ReadReg(AVC_VLD_REG_OFFSET1 + u4Addr);	
#ifdef VDEC_SIM_DUMP
	...
	{
		UINT32 u4Val = u4ReadReg(LAE_AVCVLD_OFFSET0 + u4Addr); 
        
        #ifdef VDEC_SIM_DUMP
        vVDecSimDumpR(u4VDecID, LAE_AVCVLD_OFFSET0, u4Addr, u4Val);
        #endif
	...
	{
		UINT32 u4Val = u4ReadReg(LAE_AVCVLD_OFFSET1 + u4Addr); 
        
        #ifdef VDEC_SIM_DUMP
        vVDecSimDumpR(u4VDecID, LAE_AVCVLD_OFFSET1, u4Addr, u4Val);
        #endif
...
UINT32 u4VDecReadAVCMV(UINT32 u4VDecID, UINT32 u4Addr)
{
    ...
    {
        UINT32 u4Val = u4ReadReg(AVC_MV_REG_OFFSET1 + u4Addr);     
        #ifdef VDEC_SIM_DUMP
...
void vVDecWriteAVCFG(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    if (u4VDecID == 0)
    {
        ...
        #ifdef VDEC_SIM_DUMP
        vVDecSimDumpR(u4VDecID, AVC_FG_REG_OFFSET0, u4Addr, u4Val);
        #endif
    }
    else
    {
        ...
        #ifdef VDEC_SIM_DUMP
        vVDecSimDumpR(u4VDecID, AVC_FG_REG_OFFSET1, u4Addr, u4Val);
        #endif
    }
}
...
UINT32 u4VDecReadAVCFG(UINT32 u4VDecID, UINT32 u4Addr)
{
    ...
    if (u4VDecID == 0)
    {
        UINT32 u4Val = u4ReadReg(AVC_FG_REG_OFFSET0 + u4Addr); 
    ...
    {
        UINT32 u4Val = u4ReadReg(AVC_FG_REG_OFFSET1 + u4Addr); 
...
BOOL fgH264VLDInitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4FIFOSa, UINT32 u4VLDRdPtr, UINT32 u4VLDWrPtr)
{
  ...
  vVDecWriteVLD(u4VDecID, RW_VLD_ASYNC + (u4BSID << 10), u4VDecReadVLD(u4VDecID, RW_VLD_ASYNC) | VLD_WR_ENABLE);
    ...
    if (!fgFetchOK)
    {
        UTIL_Printf("Init fetch fail\n");
		for (i = 66; i < 80; i++)
		{
			u4VDecReadMISC(u4VDecID, i*4);
		}
        return (FALSE);
    }
    //mb();
...
BOOL fgInitH264BarrelShift1(UINT32 u4VDecID, VDEC_INFO_H264_BS_INIT_PRM_T *prH264BSInitPrm)
{
    ...
    if(1 == prH264BSInitPrm->fgDualCoreEn)
    {
...
vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_TIMEOUT_SW, u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_TIMEOUT_SW)|VLD_TOP_DEC_CYCLE_EN);
	...
	if(1 == prH264BSInitPrm->fgDualCoreEn)
    {
        vVDecSetBS2VFIFO(0, u4VDecID, PHYSICAL(prH264BSInitPrm->u4VFifoSa), PHYSICAL((UINT32) prH264BSInitPrm->u4VFifoEa));
    }
#if CONFIG_DRV_VIRTUAL_ADDR
#if ((CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)||(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561))
  ...
  vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, 0x80000000); // bit[30]: vld wrapper bypass on
  #endif
#else
...
UINT32 u4VDecReadH264VldRPtr(UINT32 u4BSID, UINT32 u4VDecID, UINT32 *pu4Bits, UINT32 u4VFIFOSa)
{
    ...
    UINT32 u4SramDataSz;
    UINT32 u4ByteAddr;
    #if ((CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)||(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561))
    UINT32 u4Cnt = 0;
    #endif
	#ifdef VDEC_SIM_DUMP
	UTIL_Printf("[H264]u4VDEC_HAL_H264_ReadRdPtr Start\n");
	#endif
    // HW issue, wait for read pointer stable
      ...
      { 
        ...
        UTIL_Printf("[H264] Inst%d, ReadH264VldRPtr, u4VEnd:0x%x, time out!\n", 
          u4VDecID, u4VEnd);
      }
    ...
    while (!(u4VDecReadVLD(u4VDecID, RO_VLD_SRAMCTRL + (u4BSID << 10))&0x1))
    {
        ...
        if (u4Cnt >= 0x1000)
        {
            UTIL_Printf("[MTK40343] Wait HW stable timeout!\n");
            break;
        }
    }
#else
...
#endif
#ifdef VDEC_SIM_DUMP
	UTIL_Printf("[H264]u4VDEC_HAL_H264_ReadRdPtr End 0x%x\n",u4ByteAddr);
#endif
    return (u4ByteAddr);
