...
/// Set LAE HW registers related with B reference list
/// \return B0 list and B1 list are equal or not
BOOL bVDEC_HAL_H264_LAE_SetBRefPicListReg(
	UINT32 u4VDecID, 
...
/// Set video LAE decoder hardware registers to decode
/// \return If return value < 0, it's failed. Please reference hal_vdec_errcode.h.
INT32 i4VDEC_HAL_H264_LAEDecStart(
	UINT32 u4VDecID,
	VDEC_INFO_DEC_PRM_T *prDecPrm
);
...
/// trigger dual core decoded start
/// \return None
void vVDEC_HAL_H264_TriggerDualCore(
	UINT32 u4VDecID
);

/// trigger decoded start
/// \return None
void vVDEC_HAL_H264_TriggerDecode(
	UINT32 u4VDecID
);
