...
#else
        UTIL_Printf("[VDEC]WMV: Wrong VDecDecID\n");
        //return 0;
#endif
    }
}
...
INT32 i4VDEC_HAL_WMV_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_WMV_VFIFO_PRM_T *prWmvVFifoInitPrm)
{
    ...
    vVDecResetHW(u4VDecID, VDEC_FMT_WMV);
#else
    vVDecResetHW(u4VDecID, VDEC_UNKNOWN);
#endif

    #if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890) //turn on wmv mode 
    vVDecWriteVLD(u4VDecID, RW_VLD_WMV_MODE , 0x1);
    #endif
...
UINT32 u4VDEC_HAL_WMV_GetRealBitStream(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4ShiftBits)
{
    ...
    return (dRegVal0 >> (32-u4ShiftBits));
}
...
INT32 i4VDEC_HAL_WMV_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_WMV_BS_INIT_PRM_T *prWmvBSInitPrm, BOOL fgIsVC1)
{
   ...
   if((u4VDecReadVLD(u4VDecID,RO_VLD_SRAMCTRL) & (0x1<<15)))
    ...
    #if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890) //turn on barrel shifter by firmware
     vVDecWriteVLD(u4VDecID, RW_VLD_LDSH , (u4VDecReadVLD(u4VDecID, RW_VLD_LDSH) | (0x1 << 8)));
    #endif
    ...
    if (!fgFetchOK)
    {
    	UTIL_Printf("Fetch data fail, fgFetchOK:%d\n", fgFetchOK);
...
INT32 i4VDEC_HAL_WMV_HWDecBP(UINT32 u4VDecID, UINT32 u4BpNum, VDEC_INFO_WMV_DEC_BP_PRM_T *prWmvDecBpPrm)
{
    ...
    #else
    INT32 height = (prWmvDecBpPrm->ucFrameCodingMode == INTERLACEFIELD)? 
        prWmvDecBpPrm->u4PicHeightSrc/2 : prWmvDecBpPrm->u4PicHeightDec/*Main Profile only*/;
  ...
  /*  vVDecWriteVLD(u4VDecID, RW_VLD_ADDREXTEND, 
                                   (u4VDecReadVLD(u4VDecID, RW_VLD_ADDREXTEND) & 0xFFFFFFC0) 
                                | (((u4AbsDramANc((UINT32) prWmvDecBpPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >> 22) & 0x3F)); //194 Mv_1#1(frame/top field)
    ...
    #if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561)// Step 1 : Set Picture Size
  ...
  //-----------------------------------------------------
   ...
   if (prWmvDecBpPrm->fgWmvMode)
   {
    ...
    #if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890) //turn off firmware header parsing flag
     vVDecWriteVLD(u4VDecID, RW_VLD_LDSH , u4VDecReadVLD(u4VDecID, RW_VLD_LDSH)&(~ (0x00000001 << 8)));
    #endif
   ...
   UTIL_Printf("BP Input Window = 0x%x\n",u4VDecReadVLD(u4VDecID,0xf0));
...
void vVDEC_HAL_WMV_GetRealMby(UINT32 u4VDecID, UINT32 *pu4Mby)
{
	*pu4Mby = (u4VDecReadVLD(u4VDecID, 148*4)>>24);
}
...
INT32 i4VDEC_HAL_WMV_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	//UTIL_Printf("global setting decode buffer\n");
    //-------------------------------------------------
  ...
  vVDecWriteVLD(u4VDecID, RW_VLD_PIC_W_MB, ((prDecPrm->u4PicBW + 15)>> 4));  
    ...
    // initialize prediction
   // UTIL_Printf("initialize prediction!\n");
    ...
    //MPEG-4 use    WriteREG(RW_VLD_DIRE_MD_IL, 0x00000000); //199 set once
//     UTIL_Printf("initialize prediction!----01\n");
    ...
    // UTIL_Printf("initialize prediction!----02\n");
    vVDecWriteVLD(u4VDecID, RW_VLD_MBDRAM_SEL, a201); //201 set once
  ...
  //vVDecWriteMC(u4VDecID, VLD_REG_OFST + 0x258, 1000);
	...
	switch(prWmvDecPrm->prPPS->ucPicType)
	{
		case 0xa0:
			UTIL_Printf("[WMV]: vop type = I\n");
		break;
		case 0xa1:
			UTIL_Printf("[WMV]: vop type = P\n");
		break;
		case 0xa2:
			UTIL_Printf("[WMV]: vop type = B\n");
		break;
		case 0xa3:
			UTIL_Printf("[WMV]: vop type = BI\n");
		break;
		case 0xa4:
			UTIL_Printf("[WMV]: vop type = Skip\n");
		break;
		default:
			UTIL_Printf("[WMV]: vop type error\n");
		break;	
	}	
#endif
    ...
    //--------------------------------------------------------------------
    // UTIL_Printf("initialize prediction!----04\n");
	...
	#ifdef MT5881_WMV_NO_WRAPPER
	vVDecWriteMC(u4VDecID, RW_MC_WRAPPER_SWITCH, u4VDecReadMC(u4VDecID, RW_MC_WRAPPER_SWITCH) | (1<<0));
    ...
    if (prWmvDecPrm->prSPS->u4PicWidthSrc > 1920)
    {
		...
		#elif(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890)  //For files that have a resolution more than 1920
		vVDecWriteMC(u4VDecID, RW_MC_WRAPPER_SWITCH, 1);
		vVDecWriteMC(u4VDecID, RW_MC_MVDCAC_SEL, (u4VDecReadMC(u4VDecID,RW_MC_MVDCAC_SEL) & 0xFFFFFFEF));
		vVDecWriteMC(u4VDecID, RW_MC_DDR3_EN, (u4VDecReadMC(u4VDecID,RW_MC_DDR3_EN) | 1));
		//vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_ADDR, u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Vld_Wrapper_Sa));
		//vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, ((u4VDecReadVLDTOP(u4VDecID,RW_VLD_TOP_PRED_SRAM_CFG) & 0xBFFFFFFF) | (0x1<< 30)));
		//UTIL_Printf("[VC1] Size > 1920 \n");		
        #else
  ...
  // UTIL_Printf("initialize prediction!----05\n");
    //--------------------------------------------------------------------
	...
	// UTIL_Printf("initialize prediction!----06\n");
    if (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3)
    {
  ...
  // UTIL_Printf("initialize prediction!----07\n");
    //
...
/*
  ...
  */
  ...
  // UTIL_Printf("initialize prediction!----08\n");
  ...
  // UTIL_Printf("initialize prediction!----09\n");
    //------------------------------
   ...
   //UTIL_Printf("initialize prediction!----0a\n");
    //-----------------------------------------------------
    ...
    //  UTIL_Printf("initialize prediction!----0b\n"); 
    //-----------------------------------------------------
  ...
  // UTIL_Printf("initialize prediction!----0c\n");
    //-----------------------------------------------------
 ...
 //  UTIL_Printf("initialize prediction!----0d\n");
    //-----------------------------------------------------
  ...
  //          ((prWmvDecPrm->u4PicWidthSrc + 15)>>4); //picture_width_in_MB);
   ...
   //UTIL_Printf("initialize prediction!----0f\n");
    //-------------------------------------------------
  ...
  vVDecWriteVLD(u4VDecID, RW_VLD_WMV_OVC, a134 );
    ...
    //-------------------------------------------------
    // VLD_reg_138 : VLD mode setting
    //-------------------------------------------------
#if (CONFIG_CHIP_VER_CURR <= CONFIG_CHIP_VER_MT8560)
    a138 = 0 +
          (0 << 25);
#else //MT8580B, MT8561,and later
    {
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8580)
		    BOOL fgRunBelowSegmentAfter8580B = fgRunBelowSegmentAfter(IC_8580_VER_B);
    	    if (fgRunBelowSegmentAfter8580B)
        	{
            	a138 = 1 << 20;  /* HW default setting. HW fix the wrokaround in fgWMVResultChk: u4WmvErrType == 0x800 && fgIsHwCheckErr */
        	}
#elif (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890)
			a138 = 1 << 20;
#else
        	{
            	a138 = 0 + (0 << 25);
...
#endif
  // UTIL_Printf("initialize prediction!----020\n");
...
#endif
//UTIL_Printf("initialize prediction!----021\n");	 
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
    ...
    if (prWmvDecPrm->i4CodecVersion == VDEC_VC1)
    {
        ...
        UINT32 a158 = 0;
        UINT32 a159 = 0;
       // UTIL_Printf("initialize prediction!----021xx\n");	
      #if (CONFIG_DRV_VERIFY_SUPPORT)
        UCHAR ucCnt = 0;
        UCHAR ucbit = 0;
		...
		//UTIL_Printf("initialize prediction!----02100\n");
		...
		//UTIL_Printf("initialize prediction!----02101\n");
		...
		//UTIL_Printf("initialize prediction!----02102\n");
      ...
      #endif        
	  //UTIL_Printf("initialize prediction!----02103\n");
    ...
    #endif
#endif
//UTIL_Printf("initialize prediction!----0001xxx\n");	 
    ...
    //UTIL_Printf("initialize prediction!----0001xxx--01\n");	 
    #ifndef VDEC_PIP_WITH_ONE_HW
	//UTIL_Printf("initialize prediction!----0001xxx--VDEC_PIP_WITH_ONE_HW\n");
   ...
   // UTIL_Printf("initialize prediction!----0001xxx--02\n");	
    #endif
#endif
//UTIL_Printf("initialize prediction!----0001\n");	 
    ...
    if (prWmvDecPrm->fgWmvMode)
    {
        ...
        aAvcMv131 =u4AbsDramANc(aAvcMAddr) >> 4;    //fixed by Lu Sun
        ...
        if (prWmvDecPrm->prPPS->fgFieldMode == TRUE && (prWmvDecPrm->i4CodecVersion == VDEC_VC1))
       {
        ...
        //UTIL_Printf("AVCMV_131 = %d\n", aAvcMv131);
        //UTIL_Printf("AVCMV_134 = %d\n", aAvcMv134);
        //UTIL_Printf("AVCMV_135 = %d\n", aAvcMv135);
        //UTIL_Printf("AVCMV_137 = %d\n", aAvcMv137);
        ...
        //UTIL_Printf("VLD_163 = %d\n", a163);
        //UTIL_Printf("VLD_164 = %d\n", a164);
        //UTIL_Printf("VLD_167 = %d\n", a167);
        //UTIL_Printf("VLD_168 = %d\n", a168);
        //UTIL_Printf("VLD_169 = %d\n", a169);
        //UTIL_Printf("VLD_170 = %d\n", a170);
        //UTIL_Printf("VLD_171 = %d\n", a171);
        ...
        //UTIL_Printf("DCAC_0 = %d\n", aWmvDcac0);
        //UTIL_Printf("DCAC_1 = %d\n", aWmvDcac1);
    }
#endif
//UTIL_Printf("initialize prediction!----0002\n");	
    ...
    //Writeback by PP
    vVDecWriteMC(u4VDecID, RW_MC_PP_WB_BY_POST, 0x00000001);
	//UTIL_Printf("initialize prediction!----022\n");

 if (prWmvDecPrm->prSPS->u4PicWidthSrc >= 1920)
    {
     /*if FPGA emulation, due to bitfile limitation, if HD bitsream, may be need to turn off PP wrapper & pp output buffer*/	
   //  vVDecWriteMC(u4VDecID, RW_MC_WRAPPER_SWITCH, 1); //sun for temp
...
#endif
//UTIL_Printf("initialize prediction!----0003\n");	
...
#endif
//UTIL_Printf("initialize prediction!----0004\n");	
...
#if 0
{
    ...
    if (_VDecNeedDumpRegister(u4VDecID))
    {
        ...
        UTIL_Printf("VLDTOP Before Setting\n");
        for(reg = 0; reg < 100; reg++)
        {
            u4Val = u4VDecReadTopVLD(u4VDecID, (reg << 2));
            UTIL_Printf("%d (0x%x) = 0x%4x\n", reg, (reg<<2), u4Val);
        }
        UTIL_Printf("DSCL Before Setting\n");
...
#endif
//UTIL_Printf("initialize prediction!----0006\n");	

#ifdef DEC_END_POLLING
vVDecWriteVLD(u4VDecID, 172*4, u4VDecReadVLD(u4VDecID, 172*4)|(1<<0)); //clear decode finish
vVDecWriteVLD(u4VDecID, 172*4, u4VDecReadVLD(u4VDecID, 172*4)&(0xFFFFFFFE));
#endif
...
#endif
//UTIL_Printf("initialize prediction!----0005\n");	
...
/*sun for enable wmv crc*/
    ...
    u4CRC_Agent = u4VDecReadMC(u4VDecID, RW_VLD_WMV_PARA3);
 	if(u4CRC_Agent == 1)
	{
		UTIL_Printf("CRC PP out\n");
	}else
	{
		UTIL_Printf("CRC MC out\n");
	}
    vVDecWriteCRC(u4VDecID,VDEC_CRC_REG_EN, ((u4CRC_Agent << 1)+ 0x1));
 #endif
 ...
 //vVDecWriteVLD(u4VDecID, RW_VLD_WMV_OVC, (u4VDecReadVLD(u4VDecID, RW_VLD_WMV_OVC) | 0x1)); //Close Error Concelment
 UTIL_Printf("Input Window = 0x%x\n",u4VDecReadVLD(u4VDecID,0xf0));
   
 //vVDecWriteVLD(u4VDecID, RW_VLD_TIMEOUT , 0xFFFFFFFF); //Set HW timeout
 //vVDecWriteMC(u4VDecID, (409*4) , (u4VDecReadMC(u4VDecID, (409*4)) | 0x1B1)); //Enable HW to stop at MB_X &MB_Y
  
#if(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890)
	  if(5 >= u4VDecReadMC(u4VDecID,RW_MC_PIC_W_MB))
		  vVDecWriteMC(u4VDecID,RW_MC_L2NBM_ER_TYPE, 0);  //Close L2NBM when the resolution is too small (MB_X<=5)
#endif
#if CONFIG_DRV_VERIFY_SUPPORT
   if (_u4FileCnt[u4VDecID] == _u4DumpRegPicNum[u4VDecID])
   {
   	UTIL_Printf("_u4FileCnt[u4VDecID]==_u4DumpRegPicNum[u4VDecID]\n");
   }
#endif
#if 0
 {					
			   UINT32 length;
			   UCHAR * pbVFifo = _pucVFifo[u4VDecID];
			   UTIL_Printf("DUMP VFIFO\n\n\n");
			   for( length = 0; length < 1483092; length ++,pbVFifo ++)
			   {						
				   if(length % 16 == 0)
				   {
					   UTIL_Printf("\n%02x", *pbVFifo );
				   ...
				   {
					   UTIL_Printf(" %02x", *pbVFifo);
				   }						
			   }	
			   UTIL_Printf("\n");
 }
#endif
 UTIL_Printf("Trigger  HW\n");
 //vVDEC_HAL_WMV_VDec_DumpReg(u4VDecID, TRUE); //sun for temp
...
{
    // load wmv-sum to mpeg2-sum before 8561
     //load wmv-sum to firmware-sum for 8561
    vVDecWriteVLD(u4VDecID, RW_VLD_LDSH, 0x1);
    mb();
    vVDecWriteVLD(u4VDecID, RW_VLD_LDSH, 0x0);

    /*Due to caculate rptr which need correct sum for next frame, we need turn on bit[8] to make firmware sum valid*/
   #if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890) //turn on barrel shifter by firmware
     vVDecWriteVLD(u4VDecID, RW_VLD_LDSH , (u4VDecReadVLD(u4VDecID, RW_VLD_LDSH) | (0x1 << 8)));//sun for temp
    #endif
  ...
  for (i=i4McStart; i<i4McEnd; i++)
  {
   ...
   for (i=i4TopVldStart; i<i4TopVldEnd; i++)
   {
      ...
      UTIL_Printf("%04d (0x%04X) = 0x%08X\n", i, (i<<2), u4Data);
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8555)
        UTIL_Printf("Y CRC Registers\n");
        for (i=0; i<4; i++)
        {
          u4Data = u4ReadReg(VDEC_CRC_REG_OFFSET0 + ((2+i)*4));
          UTIL_Printf("%04d (0x%04X) = 0x%08X\n", (2+i), ((2+i)<<2), u4Data);      
         }
        UTIL_Printf("C CRC Registers\n");
        for (i=0; i<4; i++)
        {
        ...
        UTIL_Printf("IS Registers\n");
        for (i=128; i<192; i++)
        {
          u4Data =  u4ReadReg(AVS_PP_REG_OFFSET0 + (i*4));
          UTIL_Printf("%04d (0x%04X) = 0x%08X\n", i, (i<<2), u4Data);      
         }
        UTIL_Printf("IQ Registers\n");
         for (i=320; i<384; i++)
        {
          u4Data =  u4ReadReg(AVS_PP_REG_OFFSET0 + (i*4));
          UTIL_Printf("%04d (0x%04X) = 0x%08X\n", i, (i<<2), u4Data);      
         }
         UTIL_Printf("IT Registers\n");
         for (i=576; i<640; i++)
        {
          u4Data =  u4ReadReg(AVS_PP_REG_OFFSET0 + (i*4));
          UTIL_Printf("%04d (0x%04X) = 0x%08X\n", i, (i<<2), u4Data);      
...
{
  #ifdef DEC_END_POLLING
  return u4VDecReadVLD(u4VDecID, 147*4);
  #else
...
UINT32 u4VDecWMVReadVldRPtr(UINT32 u4BSID, UINT32 u4VDecID, UINT32 *pu4Bits, UINT32 u4VFIFOSa)
{
	UINT32 u4DramRptr;
