...
const UINT32 DUAL_FORMAT_ComSetting[11] = 
{
...
INT32 i4VDEC_HAL_MPEG_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_MPEG_VFIFO_PRM_T *prMpegVFifoPrm)
{
    ...
    if((prMpegVFifoPrm->u4CodeType == VDEC_MPEG4)||(prMpegVFifoPrm->u4CodeType == VDEC_DIVX3)||(prMpegVFifoPrm->u4CodeType == VDEC_H263))
        vVDecResetHW(u4VDecID, VDEC_FMT_MP4);
    else // mpeg2
        vVDecResetHW(u4VDecID, VDEC_FMT_MPV);
#else
    vVDecResetHW(u4VDecID, VDEC_UNKNOWN);
#endif
    #if VDEC_SUPPORT_IOMMU
    vDEC_HAL_COMMON_IOMMUInit(VDEC_FMT_MPV);
    ...
    #if VDEC_SUPPORT_IOMMU
    vVDecSetVLDVFIFO(0, u4VDecID, (prMpegVFifoPrm->u4VFifoSa), (prMpegVFifoPrm->u4VFifoEa));
    #else
    ...
    //UTIL_Printf("InitVDecHW, after reset HW, Codec:%d, FIFO[0x%x, 0x%x]\n", 
    //    prMpegVFifoPrm->u4CodeType, prMpegVFifoPrm->u4VFifoSa, prMpegVFifoPrm->u4VFifoEa);

    if((prMpegVFifoPrm->u4CodeType == VDEC_MPEG2)||(prMpegVFifoPrm->u4CodeType == VDEC_MPEG1)||(prMpegVFifoPrm->u4CodeType == VDEC_MPEG))
    {
        //UTIL_Printf("InitVDecHW, Codec:%d, cur VLD59:0x%x\n", 
        //    prMpegVFifoPrm->u4CodeType, u4VDecReadVLD(u4VDecID, 59 * 4));
...
INT32 i4VDEC_HAL_MPEG_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_MPEG_BS_INIT_PRM_T *prMpegBSInitPrm)
{
   ...
   if((u4VDecReadVLD(u4VDecID,RO_VLD_SRAMCTRL) & (0x1<<15)))
    ...
    #if VDEC_SUPPORT_IOMMU    
    vVDecSetVLDVFIFO(u4BSID, u4VDecID, ((UINT32) prMpegBSInitPrm->u4VFifoSa), ((UINT32) prMpegBSInitPrm->u4VFifoEa));
    #else
    ...
    u4VLDRemainByte =  ((prMpegBSInitPrm->u4ReadPointer)) & 0xf;
     ...
     #if VDEC_SUPPORT_IOMMU   
     vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), ((UINT32) prMpegBSInitPrm->u4ReadPointer));
     vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),((UINT32) prMpegBSInitPrm->u4WritePointer));
     #else
...
//      vVDecWriteVLD(u4VDecID, 0xCC , 0x00); //Just for debug
      ...
      #ifdef VDEC_SIM_DUMP
	    UTIL_Printf("wait(`VDEC_INI_FETCH_RDY == 1); u4BSID:%d\n", u4BSID, u4VLDRemainByte);
      UTIL_Printf("[BS%d] Remain:%d, Read:0x%x, Write:0x%x\n", u4BSID, u4VLDRemainByte, prMpegBSInitPrm->u4ReadPointer, prMpegBSInitPrm->u4WritePointer);
	    #endif
      
      if (fgVDecWaitVldFetchOk(u4BSID, u4VDecID))
      {
          fgFetchOK = TRUE;
          //UTIL_Printf("wait(`VDEC_INI_FETCH_RDY == 1); fetch OK!\n");
       }
#else
    vVDecWriteVLD(u4VDecID,RW_VLD_RDY_SWTICH + (u4BSID << 10), READY_TO_RISC_1);

    #if VDEC_SUPPORT_IOMMU 
    vVDecSetVLDVFIFO(u4BSID, u4VDecID, (prMpegBSInitPrm->u4VFifoSa), (prMpegBSInitPrm->u4VFifoEa));
    #else
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), (prMpegBSInitPrm->u4ReadPointer));
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), (prMpegBSInitPrm->u4ReadPointer));       
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10), (prMpegBSInitPrm->u4WritePointer));
        vVDecWriteVLD(u4VDecID, RW_VLD_ASYNC + (u4BSID << 10), u4VDecReadVLD(u4VDecID, RW_VLD_ASYNC) | VLD_WR_ENABLE);
#else
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),(((prMpegBSInitPrm->u4WritePointer) << 4) | 0x2));
#endif
        #else
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR, u4VDecReadVLD(u4VDecID, WO_VLD_WPTR) | VLD_CLEAR_PROCESS_EN);
    ...
    if (!fgFetchOK)
    {
        UTIL_Printf("wait(`VDEC_INI_FETCH_RDY == 1); fetch NG!\n");
...
// **************************************************************************
// Function : INT32 i4VDEC_HAL_MPEG4_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_MPEG_BS_INIT_PRM_T *prMpegBSInitPrm);
// Description :Initialize barrel shifter with byte alignment
// Parameter : u4ReadPointer : set read pointer value
//                 u4WrtePointer : set write pointer value
//                 u4BSID  : barrelshifter ID
...
//                  <0: fail.
// **************************************************************************
INT32 i4VDEC_HAL_MPEG4_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_MPEG_BS_INIT_PRM_T *prMpegBSInitPrm)
{
    BOOL fgFetchOK = FALSE;
...
//      vVDecWriteVLD(u4VDecID, 0xCC , 0x00); //Just for debug
	  ...
	  vVDecWriteVLD(u4VDecID,RW_VLD_LDSH,u4VDecReadVLD(u4VDecID,RW_VLD_LDSH) | 0x100);
	  #endif
      // start to fetch data
      vVDecWriteVLD(u4VDecID, RW_VLD_PROC + (u4BSID << 10), VLD_INIFET);
...
#if (CONFIG_DRV_LINUX_DATA_CONSISTENCY)
    //return (UINT32)(VIRTUAL((u4VDecReadVldRPtr(u4BSID, u4VDecID, pu4Bits, (u4VFIFOSa)))));
    return (UINT32)(((u4VDecReadVldRPtr(u4BSID, u4VDecID, pu4Bits, (u4VFIFOSa)))));
#else
    return u4VDecReadVldRPtr(u4BSID, u4VDecID, pu4Bits, (u4VFIFOSa));
#endif

#else //VDEC_SUPPORT_IOMMU
        ...
        VDEC_INFO_MPEG_BS_INIT_PRM_T *prMpegBSInitPrm, VDEC_INFO_MPEG_QANTMATRIX_T *prQuantMatrix)
{
    ...
    #if VDEC_SUPPORT_IOMMU
    u4OldByte = u4VDecReadVldRPtr(0, u4VDecID, &u4Bit, (prMpegBSInitPrm->u4VFifoSa));
    #else
    ...
    if (i4VDEC_HAL_MPEG4_InitBarrelShifter(0, u4VDecID, prMpegBSInitPrm) < 0)
    {
        return (FALSE);
    }
...
INT32 i4VDEC_HAL_MPEG_VPStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    vVDecWriteVLD(u4VDecID, RW_VLD_PARA, 0xC0500000); //Frame Picture + P picture type
	#endif
...
//	vVDecWriteTopVLD(u4VDecID,RW_TOPVLD_WMV_PICSIZE_MB,(((prDecPrm->u4PicW+ 15)>>4) -1) | ((((prDecPrm->u4PicH + 15)>>4) - 1)<<16));
	...
	#ifndef VPMODE_FOR_5881
	#ifdef CONFIG_TV_DRV_VFY
	vVDecWriteVLD(u4VDecID, RW_VLD_PICSZ, (u4VDecReadVLD(u4VDecID, RW_VLD_PICSZ)|(1<<30))); //set mpeg2_flag=1
	#endif
	#endif
    // addr swap mode
...
void vVDEC_HAL_MPEG_GetErrInfo(UINT32 u4VDecID, VDEC_INFO_MPEG_ERR_INFO_T *prMpegErrInfo)
{
    ...
    u4RegVal = u4VDecReadVLD(u4VDecID, RO_VLD_SUM);
    prMpegErrInfo->u4MpegErrType = ((u4RegVal >>8) & 0xFFFFBF);//VLD_69[31:8](mask bit 14) !=0-->decode error
...
INT32 vVDEC_HAL_MPEG2_DisableMVOverflowDetection(UINT32 u4VDecID)
{
    ...
    //   vVDecWriteVLD(u4VDecID, WO_VLD_BBUFRST, u4VDecReadVLD(u4VDecID, WO_VLD_BBUFRST) | ((unsigned)0x1 << 14));// for error bitstream, byte align firstly, then find start code
                                                                                                                                                                                                                                                                            //DE considers still set bit 14 default to 0
    return HAL_HANDLE_OK;
}
...
UINT32 u4VDEC_HAL_MPEG_VDec_ReadFinishFlag(UINT32 u4VDecID, BOOL fgMpeg4)
{
    ...
    {
        return (u4VDecReadMISC(u4VDecID, 41*4)>>16);
    }
    #else
    if(fgMpeg4)
    {
...
void vVDEC_HAL_MPEG_BANDWID_MEAS_DumpReg(UINT32 u4VDecID)
{
	
	UTIL_Printf("NBM_DLE_NUM,ESA_REQ_DATA_NUM,MC_MBX,MC_MBY,CYC_SYS,INTRA_CNT,LAT_BUF_BYPASS\n");
	UTIL_Printf("%d,%d,%d,%d,%d,%d,%d\n",
...
void vVDEC_HAL_MPEG_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
  ...
  INT32 i4McStart = 0;
  INT32 i4McEnd = 700;
  INT32 i4VldTopStart = 0;
  INT32 i4VldTopEnd = 100;
  INT32 i4PPStart = 0;
  INT32 i4PPEnd = 32;
  INT32 i4AVCMVStart = 131;
  INT32 i4AVCMVEnd = 182;
 ...
 else
 {
...
#if 1
  sprintf(strMessage, "%s", "MPEG VLD setting\n");     
  fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tRecFileInfo[u4VDecID]);
  ...
  sprintf(strMessage,"%s", "MPEG VLD TOP setting\n");
  for(i = i4VldTopStart; i < i4VldTopEnd; i++)
  {
    u4Temp = i;
    sprintf(strMessage, "%d (0x%.4x) = 0x%.8x\n", u4Temp, (u4Temp << 2), u4VDecReadVLDTOP(u4VDecID, (u4Temp << 2)));
    fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tRecFileInfo[u4VDecID]);
    //fprintf(_tRecFileInfo.fpFile, "%d (0x%.4x) = 0x%.8x\n", u4Temp, 0xf800+(u4Temp << 2), u4VDecReadVLD(u4VDecID, (u4Temp << 2)));
  }

  sprintf(strMessage, "%s", "MPEG MC setting\n");     
  fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tRecFileInfo[u4VDecID]);
  //fprintf(_tRecFileInfo.fpFile,"WMV VLD setting\n");     
  for(i=i4McStart; i<i4McEnd; i++) //
  {
    ...
    sprintf(strMessage, "%s", "MPEG PP setting\n");
  fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tRecFileInfo[u4VDecID]);
  for(i = i4PPStart; i < i4PPEnd; i++)
  {
    u4Temp = i;
    sprintf(strMessage, "%d (0x%.4x) = 0x%.8x\n", u4Temp, (u4Temp << 2), vVDecReadPP(u4VDecID, (u4Temp << 2)));
    fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tRecFileInfo[u4VDecID]);
  }
  for(i = 45; i< 53; i++)
  {
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580 || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 ||(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890))
void vVDEC_HAL_MPEG_WriteSram(UINT32 u4InstID,UINT32 u4SramAddr,UINT32 u4SramValue,BOOL fgNewSram)
{
    UINT32 u4Temp;

    if (fgNewSram)
    {
        u4Temp = (u4VDecReadMC(u4InstID,0x93C) & (0xffffc000)) | (1 << 16);
        u4Temp |= u4SramAddr;
...
UINT32 u4VDEC_HAL_MPEG_ReadSram(UINT32 u4InstID,UINT32 u4SramAddr,BOOL fgNewSram) 
{
    UINT32 u4RegVal,u4Temp = 0;

    if(fgNewSram)
    {
        u4Temp = u4VDecReadMC(u4InstID,0x93C) & (~(1 << 16)) & (0xffffc000);
        u4Temp |= u4SramAddr;
        vVDecWriteMC(u4InstID,0x93C,u4Temp);
...
UINT32 u4VDecMPEG2ReadVldRPtr(UINT32 u4BSID, UINT32 u4VDecID, UINT32 *pu4Bits, UINT32 u4VFIFOSa)
{
	UINT32 u4DramRptr;
	...
	// HW issue, wait for read pointer stable
...
#if ((CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580) || CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8561 ||(CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT5890))
		if((u4VDecReadVLD(u4VDecID, RO_VLD_FETCHOK + (u4BSID << 10)) & (1<<28)))
		...
		*pu4Bits &= 0x7;
		...
		//UTIL_Printf("Read Pointer = 0x%x\n",u4ByteAddr);
		return (u4ByteAddr);
}

UINT32 u4VDecMPEG4ReadVldRPtr(UINT32 u4BSID, UINT32 u4VDecID, UINT32 *pu4Bits, UINT32 u4VFIFOSa)
{
	UINT32 u4DramRptr;
	UINT32 u4SramRptr, u4SramWptr;
	UINT32 u4SramDataSz;
...
void vVDEC_HAL_MPEG_SetDualFormatComReg(UINT32 u4VDecID, UINT32 u4Pic_width, UINT32 u4Pic_height)
{
    UINT32 u4PPStartReg, u4Index;
    u4Index = 0;
    if((u4Pic_width <= 1920) && (u4Pic_height <= 1088))
    {
        for(u4PPStartReg = 45; u4PPStartReg < 53; u4PPStartReg++)
        {
           vVDecWritePP(u4VDecID,(u4PPStartReg << 2),(vVDecReadPP(u4VDecID,(u4PPStartReg << 2)) & 0xFFFF0000) | DUAL_FORMAT_ComSetting[u4Index]);
            u4Index ++;
           vVDecReadPP(u4VDecID,(u4PPStartReg << 2));
        }
        ...
        if(u4VDecID == 1)
        {
            vVDEC_HAL_MPEG_SetCore1DualFormatComReg(u4VDecID);
        }
    }
    
}
void vVDEC_HAL_MPEG_SetCore1DualFormatComReg(UINT32 u4VDecID)
{
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_CORE1_SRAM_OFFSET,0x2080800);
    u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_CORE1_SRAM_OFFSET);
    vVDecWritePP(u4VDecID,HEVC_PP_DBK_SRAM_OFFSET,0x800);
    vVDecReadPP(u4VDecID, HEVC_PP_DBK_SRAM_OFFSET);
}
