...
UINT32 u4VDec_HAL_VP6_Patch(UINT32 u4VDecID, VDEC_INFO_VP6_BS_INIT_PRM_T *prVp6BSInitPrm, VDEC_INFO_VP6_FRM_HDR_T *prFrmHdr);
...
void vVDEC_HAL_VP6_InitCtx(UINT32 u4VDecID, UINT32 u4BSID, VDEC_INFO_VP6_BS_INIT_PRM_T * prVp6BSInitPrm, VDEC_INFO_VP6_FRM_HDR_T *prFrmHdr)
{
    UINT32 i, j, u4Cnt, u4Addr, u4Addr2, u4Tmp;
    UINT32 coeff_model[4];
    //UTIL_Printf("<vdec> entry %s\n", __FUNCTION__);
    ...
    if (prFrmHdr->fgUseHuffman)
    {
        ...
        // Trigger Prob Update
        u4Cnt = 0;
        // Patch for MT8555, MT8580, MT8580B.
        #if 0 //To DTV verify
        if (prFrmHdr->fgMultiStream)
        {
            u4VDec_HAL_VP6_Patch(u4VDecID, prVp6BSInitPrm, prFrmHdr);
        }
        #endif
        ...
        // AC Coeff
        //UTIL_Printf("<vdec> init AC Huff Tree\n");
        ...
        // DC Coeff & Run Coeff
        //UTIL_Printf("<vdec> init DC Huff Tree and Zero Run Huff Tree\n");
        for (j = 0; j < 4; j++)
        {
            for (i = 0; i < 4; i++)
            {
        ...
        // EOB Run
        //UTIL_Printf("<vdec> init EOB Run Huff Tree\n");
...
INT32 i4VDEC_HAL_VP6_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_VP6_VFIFO_PRM_T *prVp6VFifoInitPrm)
{
    ...
    vVDecResetHW(u4VDecID, VDEC_FMT_VP6);
#else
    vVDecResetHW(u4VDecID, VDEC_UNKNOWN);
#endif

    #if VDEC_SUPPORT_IOMMU
    vDEC_HAL_COMMON_IOMMUInit(VDEC_FMT_VP6);
	vDEC_HAL_COMMON_MMUFlush();

	vDEC_HAL_COMMON_PrintfMMUStatus();
	#endif
    #if VDEC_SUPPORT_IOMMU
    vVDecSetVLDVFIFO(0, u4VDecID, ((UINT32) prVp6VFifoInitPrm->u4VFifoSa), ((UINT32) prVp6VFifoInitPrm->u4VFifoEa));
	#else
...
#ifdef VDEC_SIM_DUMP
    UTIL_Printf("<vdec> VP6_VLD_SHIFT_BIT(%d);\n", u4ShiftBits);
#endif
...
INT32 i4VDEC_HAL_VP6_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_VP6_BS_INIT_PRM_T *prVp6BSInitPrm)
{
    ...
    #if VDEC_SUPPORT_IOMMU
    vVDecSetVLDVFIFO(u4BSID, u4VDecID, ((UINT32) prVp6BSInitPrm->u4VFifoSa), ((UINT32) prVp6BSInitPrm->u4VFifoEa));
	#else
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        #if VDEC_SUPPORT_IOMMU
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), ((UINT32) prVp6BSInitPrm->u4ReadPointer));
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), ((UINT32) prVp6BSInitPrm->u4ReadPointer));
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10), ((UINT32) prVp6BSInitPrm->u4WritePointer));
		#else
...
INT32 i4VDEC_HAL_VP6_InitBarrelShifter2(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_VP6_BS_INIT_PRM_T *prVp6BSInitPrm, VDEC_INFO_VP6_FRM_HDR_T *prVDecVp6FrmHdr)
{
    ...
    #if VDEC_SUPPORT_IOMMU
    vVDecVP6SetVLD2VFIFO(((UINT32) prVp6BSInitPrm->u4VFifoSa), ((UINT32) prVp6BSInitPrm->u4VFifoEa));
	#else
    ...
    if(u4VDecVP6ReadVLD2(RO_VLD_SRAMCTRL) & PROCESS_FLAG)
    {
        while((u4VDecVP6ReadVLD2(RO_VLD_SRAMCTRL) & AA_FIT_TARGET_SCLK) == 0)
        {
            ...
            if(u4Cnt >= WAIT_THRD)
            {
                UTIL_Printf("<vdec> [VP6] BARRELSHIFTER 2 INITIL FAIL\n");
                break;
            }
        }
    }
    ...
    #if VDEC_SUPPORT_IOMMU
    vVDecVP6WriteVLD2(RW_VLD_RPTR, ((UINT32) prVp6BSInitPrm->u4ReadPointer));
	#else
    vVDecVP6WriteVLD2(RW_VLD_RPTR, PHYSICAL((UINT32) prVp6BSInitPrm->u4ReadPointer));
	#endif
	UTIL_Printf("<vdec> u4WritePointer value is 0x%x\n", prVp6BSInitPrm->u4WritePointer);
    ...
    {
        UTIL_Printf("<vdec> [VP6] BARRELSHIFTER 2 FETCH FAIL\n");
        return FALSE;
    }
...
void vVDEC_HAL_VP6_GetErrInfo(UINT32 u4VDecID, VDEC_INFO_VP6_ERR_INFO_T *prVp6ErrInfo)
{
    ...
    u4RegVal = u4VDecReadVP6VLD(u4VDecID, RO_VP6_VLD_ERR);
...
UINT32 u4VDec_HAL_VP6_Patch(UINT32 u4VDecID, VDEC_INFO_VP6_BS_INIT_PRM_T *prVp6BSInitPrm, VDEC_INFO_VP6_FRM_HDR_T *prFrmHdr)
{
    ...
    BOOL fgRunBelowSegmentAfter8580B = FALSE;
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    fgRunBelowSegmentAfter8580B = fgRunBelowSegmentAfter(IC_8580_VER_B);
#endif

    u4BSID = 0;
    //UTIL_Printf("<vdec> // VP6 Workaround, key_frame = %d, input window=%08x\n", prFrmHdr->ucFrameType == VP6_I_FRM,u4VDecReadVP6VLD(u4VDecID, 60*4));

    if (prFrmHdr->ucFrameType == VP6_P_FRM)
    {
        //UTIL_Printf("<vdec> // Trigger Mode Prob. Update, input window=%08x\n", u4VDecReadVP6VLD(u4VDecID, 60*4));
        vVDecWriteVP6VLD(u4VDecID, 75*4, 0x1, u4BSID); // only need MT8555 & MT8580A; MT8580B can be removed or not.
        vVDecWriteVP6VLD(u4VDecID, 39*4, 0x1, u4BSID);
        while ((u4VDecReadVP6VLD(u4VDecID, 66*4) & 0xFFFFF) != 0x1)
        { //x_thread_delay(1);
        }

        //`ifdef MT8555 or MT5396_A or MT8580_A
        if (!fgRunBelowSegmentAfter8580B)
        {
            //---------- idx = 0 ----------
            vVDecWriteVP6VLD(u4VDecID, 50*4, 0x348, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 51*4, 0x09200709, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 52*4, 0x010e030e, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 53*4, 0x03000000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 54*4, 0x0, u4BSID);
            //---------- idx = 0 ----------
            vVDecWriteVP6VLD(u4VDecID, 50*4, 0x349, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 51*4, 0x0f191315, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 52*4, 0x0c0c1217, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 53*4, 0x0a040000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 54*4, 0x0, u4BSID);
            //---------- idx = 0 ----------
            vVDecWriteVP6VLD(u4VDecID, 50*4, 0x34a, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 51*4, 0x29010100, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 52*4, 0x00000100, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 53*4, 0x62040000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 54*4, 0x0, u4BSID);
            //---------- idx = 0 ----------
            vVDecWriteVP6VLD(u4VDecID, 50*4, 0x34b, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 51*4, 0x16001f00, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 52*4, 0x00010701, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 53*4, 0x190a0000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 54*4, 0x0, u4BSID);
            //---------- idx = 0 ----------
            vVDecWriteVP6VLD(u4VDecID, 50*4, 0x34c, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 51*4, 0x02020000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 52*4, 0x000b0100, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 53*4, 0x03000000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 54*4, 0x0, u4BSID);
            //---------- idx = 0 ----------
            vVDecWriteVP6VLD(u4VDecID, 50*4, 0x34d, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 51*4, 0x03030202, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 52*4, 0x00040402, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 53*4, 0x02040000, u4BSID);
            vVDecWriteVP6VLD(u4VDecID, 54*4, 0x0, u4BSID);
        }
        else
        {
            //---------- idx = 0 ----------
            vVDecWriteMC(u4VDecID, 591*4, 0x10d23);
            vVDecWriteMC(u4VDecID, 592*4, 0x09200709);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d22);
            vVDecWriteMC(u4VDecID, 592*4, 0x010e030e);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d21);
            vVDecWriteMC(u4VDecID, 592*4, 0x03000000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d20);
            vVDecWriteMC(u4VDecID, 592*4, 0x0);
            //---------- idx = 0 ----------
            vVDecWriteMC(u4VDecID, 591*4, 0x10d27);
            vVDecWriteMC(u4VDecID, 592*4, 0x0f191315);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d26);
            vVDecWriteMC(u4VDecID, 592*4, 0x0c0c1217);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d25);
            vVDecWriteMC(u4VDecID, 592*4, 0x0a040000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d24);
            vVDecWriteMC(u4VDecID, 592*4, 0x0);
            //---------- idx = 0 ----------
            vVDecWriteMC(u4VDecID, 591*4, 0x10d2b);
            vVDecWriteMC(u4VDecID, 592*4, 0x29010100);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d2a);
            vVDecWriteMC(u4VDecID, 592*4, 0x00000100);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d29);
            vVDecWriteMC(u4VDecID, 592*4, 0x62040000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d28);
            vVDecWriteMC(u4VDecID, 592*4, 0x0);
            //---------- idx = 0 ----------
            vVDecWriteMC(u4VDecID, 591*4, 0x10d2f);
            vVDecWriteMC(u4VDecID, 592*4, 0x16001f00);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d2e);
            vVDecWriteMC(u4VDecID, 592*4, 0x00010701);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d2d);
            vVDecWriteMC(u4VDecID, 592*4, 0x190a0000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d2c);
            vVDecWriteMC(u4VDecID, 592*4, 0x0);
            //---------- idx = 0 ----------
            vVDecWriteMC(u4VDecID, 591*4, 0x10d33);
            vVDecWriteMC(u4VDecID, 592*4, 0x02020000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d32);
            vVDecWriteMC(u4VDecID, 592*4, 0x000b0100);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d31);
            vVDecWriteMC(u4VDecID, 592*4, 0x03000000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d30);
            vVDecWriteMC(u4VDecID, 592*4, 0x0);
            //---------- idx = 0 ----------
            vVDecWriteMC(u4VDecID, 591*4, 0x10d37);
            vVDecWriteMC(u4VDecID, 592*4, 0x03030202);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d36);
            vVDecWriteMC(u4VDecID, 592*4, 0x00040402);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d35);
            vVDecWriteMC(u4VDecID, 592*4, 0x02040000);
            vVDecWriteMC(u4VDecID, 591*4, 0x10d34);
            vVDecWriteMC(u4VDecID, 592*4, 0x0);
        }
        ...
        for(i = 0; i<34; i++)
        {
            vVDecWriteVP6VLD(u4VDecID, 34*4, ((1 << 16) + ( ((UINT32)mv_prob[i]) << 8) + (1)), u4BSID);
            while ((u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x1) == 0)
            {//x_thread_delay(1);
            }

            if(u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x100)
            {
                vVDecWriteVP6VLD(u4VDecID, 34*4, 7, u4BSID);
                while ((u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x1) == 0)
                {//x_thread_delay(1);
                }
            }
    ...
    for(i = 0; i<22; i++)
    {
        vVDecWriteVP6VLD(u4VDecID, 34*4, ((1 << 16) + ( ((UINT32)dc_prob[i]) << 8) + (1)), u4BSID);
        while ((u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x1) == 0)
        {//x_thread_delay(1);
        }
    ...
    vVDecWriteVP6VLD(u4VDecID, 34*4, 1, u4BSID);
    while ((u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x1) == 0)
    {//x_thread_delay(1);
    }
    ...
    if(u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x100)
    {
        for(i = 1; i < 64; i++)
        {
            vVDecWriteVP6VLD(u4VDecID, 34*4, ((1 << 16) + ( ((UINT32)reorder_prob[i]) << 8) + (1)), u4BSID);
            while ((u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x1) == 0)
            {//x_thread_delay(1);
            }
            ...
            if(u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x100)
            {
                vVDecWriteVP6VLD(u4VDecID, 34*4, 4, u4BSID);
                while ((u4VDecReadVP6VLD(u4VDecID, 37*4) & 0x1) == 0)
                {//x_thread_delay(1);
                }

                coef_reorder = (u4VDecReadVP6VLD(u4VDecID, 37*4) >> 8) & 0xFF;

                vVDecWriteVLD(u4VDecID, 152*4, 0x100|((i>>2)<<2));
                reg_153 = u4VDecReadVLD(u4VDecID, 153*4);

                tmp = 0xFFFFFFFF ^ (0x000000FF << (8*(3-i%4)));
                reg_153 = (reg_153 & tmp) | (coef_reorder << (8*(3-i%4)));
                vVDecWriteVLD(u4VDecID, 152*4, (i>>2)<<2);
                vVDecWriteVLD(u4VDecID, 153*4, reg_153);
    ...
    reg_35 = u4VDecReadVP6VLD(u4VDecID, 35*4);
    i4VDEC_HAL_VP6_InitBarrelShifter(u4BSID, u4VDecID, prVp6BSInitPrm);
    vVDecWriteVP6VLD(u4VDecID, RW_VP6_PCI_PAR + (u4BSID << 10), reg_35, u4BSID);

    // shift bit
    u4Loop = prFrmHdr->u4ShiftBitsCnt / 32;
    u4Remain = prFrmHdr->u4ShiftBitsCnt % 32;
    //UTIL_Printf("<vdec> shit_bits_cnt =%d\n",prFrmHdr->u4ShiftBitsCnt);
    for (i = 0; i < u4Loop; i++)
    {
        u4VDEC_HAL_VP6_GetBitStreamShift(u4BSID, u4VDecID, 32);
    }
    if (u4Remain)
    {
        u4VDEC_HAL_VP6_GetBitStreamShift(u4BSID, u4VDecID, u4Remain);
    }

    // initial range
    u4VDEC_HAL_VP6_InitBoolCoder(u4BSID, u4VDecID, 0);

    // shift bool
    u4Loop = prFrmHdr->u4BoolShiftBitsCnt / 7;
    u4Remain = prFrmHdr->u4BoolShiftBitsCnt % 7;
    //UTIL_Printf("<vdec> bool_shit_bits_cnt =%d\n",prFrmHdr->u4BoolShiftBitsCnt);
    for (i = 0; i < u4Loop; i++)
    {
        u4VDEC_HAL_VP6_GetBoolCoderShift(u4BSID, u4VDecID, 7);
    }
    if (u4Remain)
    {
        u4VDEC_HAL_VP6_GetBoolCoderShift(u4BSID, u4VDecID, u4Remain);
    }
...
INT32 i4VDEC_HAL_VP6_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    UINT32 u4PicBW;
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    UINT32 u4KeyFrame, u4Huffman, u4Multi, u4Alpha;
#endif
#if (CONFIG_DRV_VERIFY_SUPPORT)
    UINT32 u4TargFrm = 10000;
#endif
    BOOL fgRunBelowSegmentAfter8580B = FALSE;
    BOOL fgRunBelowSegmentAfter8580A = FALSE;

#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    fgRunBelowSegmentAfter8580B = fgRunBelowSegmentAfter(IC_8580_VER_B);
    fgRunBelowSegmentAfter8580A = fgRunBelowSegmentAfter(IC_8580_VER_A);
#endif
    ...
    if (0 == fgRunBelowSegmentAfter8580A)
    {
        ucKeyFrame = (prVDecVP6DecPrm->prFrmHdr->ucFrameType ==VP6_I_FRM)? 1: 0;
        u4Vp68C = ( (u4VertMb << 16) | (u4HoriMb << 8) | (ucKeyFrame << 1) | (RW_VP6_FLAG));
    ...
    else
    {
        ...
        if (fgRunBelowSegmentAfter8580B)
        {
            ...
            u4Alpha = (prVDecVP6DecPrm->u4AlphaFlag & VP6_ALPHA_FRAME) ? RW_VP6_ALPHA : 0;
            u4Vp68C     = (u4KeyFrame | u4Huffman | u4Multi | u4Alpha | RW_VP6_FLAG | RW_VP6_RTN_ERR);
            vVDecWriteVP6VLD(u4VDecID, RW_VP6_PCI_PAR + (u4BSID << 10), u4Vp68C, u4BSID);
        ...
        else
        {
            ...
            vVDecWriteVP6VLD(u4VDecID, RW_VP6_PCI_PAR + (u4BSID << 10), u4Vp68C, u4BSID);
            vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PIC_MB_SIZE_M1, (((u4VertMb - 1)<< 16) | (u4HoriMb - 1)));
            vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PIC_PIX_SIZE, ((u4VertMb << 20) | (u4HoriMb << 4)));
        }
#endif
    }
    ...
    if (0 == fgRunBelowSegmentAfter8580B)
    {
    ...
    else
    {
		...
		vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_ETC, 0xFFFFFFFF, u4BSID); //Yi Feng Modify to open all error type@20140710
    }
    ...
    u4PicBW = ((prDecPrm->u4PicBW + 15)>> 4);
    vVDecWriteVLD(u4VDecID, RW_VLD_PIC_W_MB, u4PicBW);
#else
#if VDEC_DDR3_SUPPORT
    u4PicBW = ((prDecPrm->u4PicBW + 63)>> 4);
#else
    u4PicBW = ((prDecPrm->u4PicBW + 15)>> 4);
#endif
    if (fgRunBelowSegmentAfter8580A)
    {
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
        vVDecWriteMC(u4VDecID, RW_MC_PIC_W_MB, u4PicBW);
#endif
    }
    else
    {
    ...
    if (fgRunBelowSegmentAfter8580B)
    {
        ...
        if(prVDecVP6DecPrm->prFrmHdr->u2Buff2Offset || (prVDecVP6DecPrm->u4AlphaFlag & VP6_ALPHA_ENABLE))
        {
            vVDEC_HAL_VP6_InitCtx(u4VDecID, u4BSID, prVDecVP6DecPrm->prVp6BSInitPrm, prVDecVP6DecPrm->prFrmHdr);
        }
    ...
    #if 0
    {
		UINT32 u4VLD35Value = 0;
        u4VLD35Value = u4VDecReadVP6VLD(u4VDecID, 35*4);

		UTIL_Printf("<vdec> Before u4VLD35Value is 0x%x\n", u4VLD35Value);

		vVDecWriteVP6VLD(u4VDecID, 35*4, u4VLD35Value | 0x1000000, u4BSID);

		
		u4VLD35Value = u4VDecReadVP6VLD(u4VDecID, 35*4);
		
		UTIL_Printf("<vdec> After u4VLD35Value is 0x%x\n", u4VLD35Value);
    ...
    #if 0 //To DTV verify
    // Patch for MT8555, MT8580, MT8580B.
    if (prVDecVP6DecPrm->prFrmHdr->fgMultiStream == FALSE)
    {
        u4VDec_HAL_VP6_Patch(u4VDecID, prVDecVP6DecPrm->prVp6BSInitPrm, prVDecVP6DecPrm->prFrmHdr);
    }
    #endif 


    vVDecWriteDV(u4VDecID, 60 *4, 1);
	
	{
		UINT32 u4InputWindow = 0;

        u4InputWindow = u4VDecReadVP6VLD(u4VDecID, RO_VP6_VLD_BARL);
    
        UTIL_Printf("# %d , Input Window 0x%x \n", _u4FileCnt[u4VDecID], u4InputWindow);	

		//vVDEC_HAL_VP6_VDec_DumpReg(u4VDecID, TRUE);
...
void vVDEC_HAL_VP6_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
    ...
    Printf("******* VLD VP6 Registers *******\n");
    for (i=33; i<256; i++)
	{
        u4VDecReadVP6VLD(0, i*4);
	}

    Printf("******* VLD TOP Registers *******\n");
    for (i=0; i<128; i++)
    {
        u4VDecReadVLDTOP(0, i*4);
		//Printf("VLD TOP %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVLDTOP(0, i*4));
    }

    Printf("******* VLD Registers *******\n");
    for (i=33; i<256; i++)
    {
        u4VDecReadVLD(0, i*4);
		//Printf("VLD %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadVLD(0, i*4));
    }
    ...
    for (i=0; i<1024; i++)
    {
        u4VDecReadVP6PP(0, i*4);
		//Printf("AVCMV %d (0x%04x): 0x%08x\n", i, i * 4, u4VDecReadAVCMV(0, i*4));
    }
...
UINT32 u4VDEC_HAL_VP6_InitBoolCoder(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4ShiftBits)
{
    ...
    if (0 == fgRunBelowSegmentAfter8580B)
    {
        vVDecWriteVP6VLD(u4VDecID, RW_VP6_INIT_BOOL + (u4BSID << 10), RW_VP6_BOOL_EN, u4BSID);
    }
    else
    {
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
...
void vVDEC_HAL_VP6_SetErrorFlag(UINT32 u4VDecID)
{
	UINT32 u4ErrDection = 0;
	u4ErrDection = u4VDecReadVP6VLD(u4VDecID, RW_VP6_PCI_PAR);
	UTIL_Printf("u4ErrDection is  0x%x \n", u4ErrDection);
	u4ErrDection = u4ErrDection & 0xFFFFFFFB;
    vVDecWriteVP6VLD(u4VDecID, RW_VP6_PCI_PAR, u4ErrDection, 0);
