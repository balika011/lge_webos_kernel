...
extern void vVDEC_Vp8DumpReg(void);
#if VDEC_VP8_WEBP_SUPPORT
...
void vVDecVP8WriteImgResz(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    if (u4VDecID == 0)
    {
        ...
        UTIL_Printf("RISCWrite('IMG_RESZ_REG_OFFSET0 + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);
    }
    else
    {
        vWriteReg(IMG_RESZ_REG_OFFSET0 + u4Addr, u4Val);
...
UINT32 u4VDecVP8ReadImgResz(UINT32 u4VDecID, UINT32 u4Addr)
{
    if (u4VDecID == 0)
    {
       ...
       UTIL_Printf("RISCRead('IMG_RESZ_REG_OFFSET0 + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);
       return (u4Val);
    }
    else
    {
...
INT32 i4VDEC_HAL_VP8_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_VP8_VFIFO_PRM_T *prVp8VFifoInitPrm)
{
...
#else
    vVDecResetHW(u4VDecID, VDEC_FMT_VP8);
#endif
    #if VDEC_SUPPORT_IOMMU
    vDEC_HAL_COMMON_IOMMUInit(VDEC_FMT_VP8);
    ...
    #if VDEC_SUPPORT_IOMMU
    vVDecSetVLDVFIFO(0, u4VDecID, ((UINT32) prVp8VFifoInitPrm->u4VFifoSa), ((UINT32) prVp8VFifoInitPrm->u4VFifoEa));
	#else
    ...
    if(u4VldId==VP8_VLD1)
    {
        if(u4VDecReadVLD(0, RO_VLD_SRAMCTRL) & PROCESS_FLAG)
        {
           while((u4VDecReadVLD(0, RO_VLD_SRAMCTRL) & AA_FIT_TARGET_SCLK) == 0)
           {
              ...
              if(u4Cnt >= WAIT_THRD)
              {
                UTIL_Printf("VP8 VLD1 HWReset WaitSramStable Fail\n");
                break;
              }
           }
        }
...
BOOL fgVDEC_Vp8DecReset(UINT32 u4InstID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo,BOOL fgInit)
{
   ...
   if(!fgInit)
   {
       ...
       {
           UTIL_Printf("VP8 Init VLD1 BarrelShifter fail\n");
           vVDEC_VP8_VldReset(VP8_VLD1);
       }
   ...
   if(!fgInit)
   {
	   ...
	   #if VDEC_SUPPORT_IOMMU
       vVDecWriteVP8VLD2(0,RW_VLD_VSTART, ((UINT32)(pVp8DecInfo->u4FifoStart ))>> 6);
       vVDecWriteVP8VLD2(0,RW_VLD_VEND, ((UINT32)(pVp8DecInfo->u4FifoEnd))>> 6);
	   #else
...
INT32 i4VDEC_HAL_VP8_InitBarrelShifter(UINT32 u1VP8VLD, UINT32 u4VDecID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
 ...
 //   HalDisableDCache();
  ...
  if (u1VP8VLD == VP8_VLD1)
	...
	#if VDEC_SUPPORT_IOMMU
    vVDecWriteVLD(u4VDecID, RW_VLD_VSTART , ((UINT32)pVp8DecInfo->u4FifoStart) >> 6);
    vVDecWriteVLD(u4VDecID, RW_VLD_VEND, ((UINT32)pVp8DecInfo->u4FifoEnd)>> 6);
	#else
	...
	#if VDEC_SUPPORT_IOMMU
    vVDecWriteVP8VLD2(u4VDecID, RW_VLD_VSTART , ((UINT32)pVp8DecInfo->u4FifoStart) >> 6);
    vVDecWriteVP8VLD2(u4VDecID, RW_VLD_VEND, ((UINT32)pVp8DecInfo->u4FifoEnd)>> 6);
	#else
    vVDecWriteVLD(u4VDecID, RW_VLD_VSTART , PHYSICAL((UINT32)pVp8DecInfo->u4FifoStart) >> 6);
    vVDecWriteVLD(u4VDecID, RW_VLD_VEND, PHYSICAL((UINT32)pVp8DecInfo->u4FifoEnd)>> 6);
	#endif
  ...
  if (u1VP8VLD == VP8_VLD1)
  	{
		...
		#if VDEC_SUPPORT_IOMMU
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR, ((UINT32) pVp8DecInfo->u4VldStartPos));
        vVDecWriteVLD(u4VDecID, RW_VLD_RPTR, ((UINT32) pVp8DecInfo->u4VldStartPos));
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR, ((UINT32) pVp8DecInfo->u4WritePos));
		#else
  ...
  else
  	{
		...
		#if VDEC_SUPPORT_IOMMU
        vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR, ((UINT32)( pVp8DecInfo->u4VldStartPos)));
        vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR, ((UINT32)( pVp8DecInfo->u4VldStartPos)));
        vVDecWriteVP8VLD2(u4VDecID, WO_VLD_WPTR, ((UINT32)( pVp8DecInfo->u4WritePos)));
		#else
...
{
    #if VDEC_SUPPORT_IOMMU
    return u4VDecReadVldRPtr(u4BSID, u4VDecID, pu4Bits, ((UINT32) u4VFIFOSa));
	#else
...
BOOL _VDEC_VP8_InitBarrelShifter_Webp(UINT32 u4VldId,
                                   ...
                                   UINT32 u4WritePointer)
{
    ...
    UTIL_Printf("_VDEC_VP8_InitBarrelShifter_Webp: u4ReadPointer 0x%x, u4WritePointer 0x%x, u4VLDRemainByte 0x%x\n", u4ReadPointer, u4WritePointer, u4VLDRemainByte);

    if(u4VDecReadVP8VLD2(u4VDecID, RO_VLD_SRAMCTRL) & PROCESS_FLAG)
    {
        while((u4VDecReadVP8VLD2(u4VDecID, RO_VLD_SRAMCTRL) & AA_FIT_TARGET_SCLK) == 0)
        {
            u4Cnt++;
            if(u4Cnt >= 0x1000)
            {
                UTIL_Printf("<vdec> [VP8] BARRELSHIFTER 2 INITIL FAIL\n");
                break;
            }
        }
    }
    if(u4VldId == VP8_VLD2)
    {
        for (i = 0; i < 5; i++)
        {
          ...
          //vVDecVP8WriteVLD2(u4VDecID, WO_VLD_WPTR, u4VDecVP8ReadVLD2(u4VDecID, WO_VLD_WPTR) | VLD_CLEAR_PROCESS_EN);
          //vVDecVP8WriteVLD2(u4VDecID, RW_VLD_RPTR, u4ReadPointer/*PHYSICAL((UINT32)u4ReadPointer)*/);
          vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR, u4ReadPointer/*PHYSICAL((UINT32)u4ReadPointer)*/);
          //vVDecVP8WriteVLD2(u4VDecID, WO_VLD_WPTR, PHYSICAL((UINT32)u4WritePointer));
          vVDecWriteVP8VLD2(u4VDecID, WO_VLD_SRST, 0x100);
          vVDecWriteVP8VLD2(u4VDecID, WO_VLD_SRST, 0x0);

          // start to fetch data
          vVDecWriteVP8VLD2(u4VDecID,RW_VLD_PROC, VLD_INIFET);
          if(fgVDecWaitVP8VldFetchOk(0, u4VDecID,VP8_VLD2))
          {
             fgFetchOK = TRUE;
             break;
          }
...
/*
 *u4PhyWritePtr is phyical address.it will compare with VStart,VEnd.
 */
VOID _VDEC_VP8_VLDSetWritePtr(UINT32 u4PhyWritePtr)
{
    ...
    u4VStart = u4VDecReadVP8VLD2(u4VDecID, RW_VLD_VSTART)<<6;
    u4VEnd = u4VDecReadVP8VLD2(u4VDecID, RW_VLD_VEND)<<6;

    ASSERT((u4PhyWritePtr&WPTR_ALIGN_MARK)==0);
    if(u4PhyWritePtr < u4VStart)
    {
        u4PhyWritePtr = u4VEnd - (u4VStart - u4PhyWritePtr);
        //ASSERT(0);
        UTIL_Printf("<VP8>SetWrPtr Vstart Err 0x%x 0x%x\n", u4PhyWritePtr, u4VStart);
    }

    if(u4PhyWritePtr >= u4VEnd)
    {
        u4PhyWritePtr = u4VStart + (u4PhyWritePtr - u4VEnd);
        //ASSERT(0);
        UTIL_Printf("<VP8>SetWrPtr VEnd Err 0x%x 0x%x\n", u4PhyWritePtr, u4VStart);
    }

    vVDecWriteVP8VLD2(u4VDecID, WO_VLD_WPTR, u4PhyWritePtr);
...
VOID _Vdec_VP8HwResetWapper(VOID)
{
    ...
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_SEGID_DRAM_ADDR, 0x4000);
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, 0x80000000);
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE, 0x3);

    vVDecWriteVP8MC(u4VDecID, RW_MC_DDR3_EN, 1);//0x00200000);//1);
    vVDecWriteVP8MC(u4VDecID, RW_MC_PP_DBLK_Y_ADDR, 0);
    vVDecWriteVP8MC(u4VDecID, RW_MC_PP_DBLK_C_ADDR, 0);
...
void vVDEC_HAL_VP8_AlignRdPtr(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4VFIFOSa, UINT32 u4AlignType)
{
    ...
    #if VDEC_SUPPORT_IOMMU
    u4VLDByte= u4VDecReadVldRPtr(u4BSID, u4VDecID, &u4VLDbits, (u4VFIFOSa));
	#else
...
#ifdef VDEC_UFO_SUPPORT
{
    ...
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_Y[%d]=%d\n", u4VDecID, u4PIC_SIZE_Y[u4VDecID]);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE[%d]=%d\n", u4VDecID, u4PIC_SIZE[u4VDecID]);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4UFO_LEN_SIZE_Y[%d]=%d\n", u4VDecID, u4UFO_LEN_SIZE_Y[u4VDecID]);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4UFO_LEN_SIZE_C[%d]=%d\n", u4VDecID, u4UFO_LEN_SIZE_C[u4VDecID]);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_Y_BS[%d]=%d\n", u4VDecID, u4PIC_SIZE_Y_BS[u4VDecID]);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_BS[%d]=%d\n", u4VDecID, u4PIC_SIZE_BS[u4VDecID]);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_REF[%d]=%d\n", u4VDecID, u4PIC_SIZE_REF[u4VDecID]);

    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4DramPicSize=%d\n", u4DramPicSize);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4DramPicArea=%d\n", u4DramPicArea);
    ...
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_UFO_WIDTH=%d\n", u4PIC_UFO_WIDTH);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_UFO_HEIGHT=%d\n", u4PIC_UFO_HEIGHT);
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_W_H=%d\n", u4PIC_W_H);
    ...
    x_memset(pVp8DecInfo->u4CurYAddr, 0, u4DramPicArea);

    pVp8DecInfo->u4CurCAddr = pVp8DecInfo->u4CurYAddr + u4PIC_SIZE_Y_BS[u4VDecID];

	//open ufo
	u4PPValue = u4VDecReadVP8PP(u4VDecID, 16 * 4);
	vVDecWriteVP8PP(u4VDecID, 16 *4, u4PPValue | 0x1000000);

	UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +0, 16PP value is 0x%x\n", u4PPValue);
}
#endif
   //MC setting
/*
   ...
   */
   ...
   //UTIL_Printf("MC 727 value is  0x%x\n", u4VDecReadVP8MC(u4VDecID, 727 * 4));
   vVDecWriteVP8MC(u4VDecID, 727 * 4, 1);

   //UTIL_Printf("Physical LstYAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4LstYAddr));
   //UTIL_Printf("Physical GldYAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4GldYAddr));
   //UTIL_Printf("Physical AlfYAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4AlfYAddr));
   //UTIL_Printf("Physical CurYAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4CurYAddr));
   
   //UTIL_Printf("Physical LstCAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4LstCAddr));
   //UTIL_Printf("Physical GldCAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4GldCAddr));
   //UTIL_Printf("Physical AlfCAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4AlfCAddr));
   //UTIL_Printf("Physical CurCAddr  0x%x\n", PHYSICAL((UINT32)pVp8DecInfo->u4CurCAddr));

   vVDecWriteVP8MC(u4VDecID, 729 * 4, PHYSICAL((UINT32)pVp8DecInfo->u4LstCAddr));
   vVDecWriteVP8MC(u4VDecID, 730 * 4, PHYSICAL((UINT32)pVp8DecInfo->u4GldCAddr));
   vVDecWriteVP8MC(u4VDecID, 731 * 4, PHYSICAL((UINT32)pVp8DecInfo->u4AlfCAddr));
   #endif

#if (!VDEC_DDR3_SUPPORT && !VDEC_VP8_WEBP_SUPPORT)
...
//   else
   ...
   vVDecWriteVP8MC(u4VDecID, 0x660, (u4VDecReadVP8MC(u4VDecID, 0x660) |(0x80000000)));
...

	{
	    ...
	    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_VP8_SEGID_DRAM, PHYSICAL(pVp8DecInfo->u4SegIdWrapper));
	    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_VP8_SEGID_SRAM, 0x2200400);

		vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_VP8_SEGID_LARB_CONTROL, 1);
		vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_VP8_SEGID_DRAM_TRIGGER, 1);

		u4DramValue = u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_VP8_SEGID_DRAM_TRIGGER);
		UTIL_Printf("u4DramValue is 0x%x\n", u4DramValue);
		...
		vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_VP8_SEGID_LARB_CONTROL, 0);
	}
   ...
   //vVDecWriteMC(u4VDecID, RW_MC_PP_WB_BY_POST, 0x00000001);
...

       {
	       ...
	       if(pVp8DecInfo->u4Width>16/* && pVp8DecInfo->u2Height>=16*/)
	       {
            ...
            #endif
	          UTIL_Printf("Picture Mode u4HalValue is 0x%x\n", u4HalValue);
           ...
           // UTIL_Printf("pic width-0x%x,pic height-0x%x\n", pVp8DecStartInfo->u4Width, pVp8DecStartInfo->u4Height);
            ...
            UTIL_Printf("MB Row Size is 0x%x, \n", u4VDecReadVP8PP(u4VDecID, 0x6C));
	       }
       }
#endif
#if VP8_MULTI_RING_BUFFER 
   vVDecWriteVP8VLD(u4VDecID,RW_VP8_PART_CHG_PAUSE,0x10001, 0);
#endif
    ...
    if (_fgVP8DumpReg)
    {
    	UTIL_Printf("******* INPUT Window *******\n");
    	UTIL_Printf("VP8_VLD1 VBSR1: 0x%08x\n", u4VDecReadVP8VLD(0, RO_VP8_VBSR1));
    	UTIL_Printf("VP8_VLD2 VBSR2: 0x%08x\n", u4VDecReadVP8VLD(0, RO_VP8_VBSR2));
    }
#endif
#endif    
...
#if (CONFIG_DRV_VERIFY_SUPPORT)    
        _pu1VP8VDecYAddrPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4CurYAddr, PIC_Y_SZ+PIC_C_SZ, FROM_DEVICE);
  #if VDEC_VP8_WEBP_SUPPORT
        _pu1VP8VDecYAltPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4LstYAddr, DEC_PP_Y_SZ+DEC_PP_C_SZ, TO_DEVICE);
        _pu1VP8VDecYLstPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4AlfYAddr, DEC_PP_Y_SZ+DEC_PP_C_SZ, TO_DEVICE);
        _pu1VP8VDecYGldPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4GldYAddr, DEC_PP_Y_SZ+DEC_PP_C_SZ, TO_DEVICE);
  #else
        _pu1VP8VDecYAltPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4LstYAddr, GOLD_Y_SZ+GOLD_C_SZ, TO_DEVICE);
        _pu1VP8VDecYLstPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4AlfYAddr, GOLD_Y_SZ+GOLD_C_SZ, TO_DEVICE);
        _pu1VP8VDecYGldPhy = (UINT8 *)BSP_dma_map_single(pVp8DecInfo->u4GldYAddr, GOLD_Y_SZ+GOLD_C_SZ, TO_DEVICE);
  #endif    
#endif    

   if (pVp8DecInfo->u4Width < 176)
   {
       vVDecWriteDV(u4VDecID, 60 * 4, 0x1);
   }
#ifdef VDEC_UFO_SUPPORT
    // Debug only
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 700 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 700 * 4));
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 664 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 664 * 4));
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 698 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 698 * 4));
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 699 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 699 * 4));

    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 663 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 663 * 4));
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 701 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 701 * 4));
    UTIL_Printf("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 343 * 4)=0x%x\n", u4VDecReadMC(u4VDecID, 343 * 4));
#endif
   
   //vWriteReg(CKGEN_REG_VDEC_SYS_CKCFG,VDEC_SYS_CK_SEL_MID);
   //Trigger decoder
  // UTIL_Printf("******* INPUT Window *******\n");
  // UTIL_Printf("VP8_VLD1 VBSR1: 0x%08x\n", u4VDecReadVP8VLD(0, RO_VP8_VBSR1));
  // UTIL_Printf("VP8_VLD2 VBSR2: 0x%08x\n", u4VDecReadVP8VLD(0, RO_VP8_VBSR2));
  ...
  // vVDEC_Vp8DumpReg();
    ...
    UTIL_Printf("******* Start Triggle *******\n");
   //vVDEC_HAL_VP8_VDec_DumpReg(0, TRUE);
...
{
#if VP8_MB_ROW_MODE_SUPPORT && !VP8_MULTI_RING_BUFFER
   return (BOOL )((u4VDecReadVP8VLD(0,RO_VP8_PFR)&1) ||(u4VDecReadVP8VLD(0,RO_VP8_VOKR)&1) || (u4VDecReadVP8VLD(0,RO_VP8_VOKR)>>8));  
#else
   return (BOOL )((u4VDecReadVP8VLD(0,RO_VP8_PFR)&1) ||(u4VDecReadVP8VLD(0,RO_VP8_VOKR)&1));
#endif
}
