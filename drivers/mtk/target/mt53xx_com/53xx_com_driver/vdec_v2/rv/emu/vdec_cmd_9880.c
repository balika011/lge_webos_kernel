...
#define FRMINFO_BASE_DIR "g:/RM/rm_emu/frminf"
#define GOLDEN_BASE_DIR  "g:/RM/rm_emu/golden"

#define DUMP_BASE_DIR    "g:/RM/rm_emu"
...
static BOOL fgCompareGold(UINT32 n, char *szDir, UINT32 u4DecWidth, UINT32 u4DecHeight, UINT32 u4Width, UINT32 u4Height, BOOL fgFirstTime)
{
  ...
  if (u4Val != n)
  {
      printf("!!! CRC frame number mismatch(%d, %d) !!!\n", u4Val, n);
      return FALSE;
  }
...
static void vVldSoftReset(void)
{
    ...
    if (!BSP_IsFPGA())
    {
		...
		_VDEC_CkgenSel(0,VDEC_FMT_RV,0);
    }
#ifdef POWER_SAVING_TEST
    RISCWrite(MISC_BASE + 0xC8, 0xEF);
    RISCWrite(MISC_BASE + 0xCC, 0x3BA41100);
#endif
    if (!BSP_IsFPGA())
    {
        //RISCWrite(MISC_BASE + 0x84, 0);
		_VDEC_SysClkSel(0,VDEC_FMT_RV,0);
    }
