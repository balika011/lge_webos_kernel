...
extern UCHAR _aucVp9LogOnOff[4];

//vp9 config defines
/////////////////////////////////////////////////////////
#define VP9_RISC_LOG 1
// vp9 internal used.
#define VP9_CONFIG_CHIP_VER_MT5890 8590 // Oryx
#define VP9_CONFIG_CHIP_VER_MT5891 8591 // WuKong
#define VP9_CONFIG_CHIP_VER_CURR VP9_CONFIG_CHIP_VER_MT5890
//////////////////////////////////////////////////////////

#define VP9_ALIGN_DEC(addr,align) (addr & (~(align-1)))
#define VP9_ALIGN_INC(addr,align) ((addr+align-1) & (~(align-1)))
...
#define VP9_ERROR(x, ...)  LOG(0, "[vp9] !!error-> "x" \n", ##__VA_ARGS__)
#define VP9_INFO(x, ...)  LOG(0, "[vp9] "x" \n", ##__VA_ARGS__)

#define VP9_DEBUG(x, ...) \
            do {                           \
                if(_aucVp9LogOnOff[VP9_LOG_DEBUG])                \
                    LOG(0, "[vp9] "x" \n", ##__VA_ARGS__);  \
            }while(0)

#define VP9_VERBOSE(x, ...) \
            do {                           \
                if(_aucVp9LogOnOff[VP9_LOG_VERBOSE])                \
                    LOG(0, "[vp9] "x" \n", ##__VA_ARGS__);  \
            }while(0)

#define VP9_PRINT_ERROR(esid, x, ...)  LOG(0, "[vp9-%d] !!error-> "x" \n", esid, ##__VA_ARGS__)
#define VP9_PRINT_INFO(esid, x, ...)  LOG(0, "[vp9-%d] "x" \n", esid, ##__VA_ARGS__)

#define VP9_PRINT_DEBUG(esid, x, ...)   \
            do {                           \
                if(_aucVp9LogOnOff[VP9_LOG_DEBUG])                \
                    LOG(0, "[vp9-%d] "x" \n", esid, ##__VA_ARGS__);  \
            }while(0)

#define VP9_PRINT_VERBOSE(esid, x, ...) \
            do {                           \
                if(_aucVp9LogOnOff[VP9_LOG_VERBOSE])                \
                    LOG(0, "[vp9-%d] "x" \n", esid, ##__VA_ARGS__);  \
            }while(0)

#define VP9_PRINT_IO(esid, x, ...) \
            do {                           \
                if(_aucVp9LogOnOff[VP9_LOG_IO])                \
                    LOG(0, "[vp9-%d] "x" \n", esid, ##__VA_ARGS__);  \
            }while(0)

#if VP9_RISC_LOG
#define VP9_PRINT_RISC(x, ...) \
    do {                           \
        if(_aucVp9LogOnOff[VP9_LOG_RISC])                \
            LOG(0, "[vp9-reg]      "x"", ##__VA_ARGS__);  \
    }while(0)
#else
#define VP9_PRINT_RISC(x, ...)
#endif
...
typedef struct TileBuffer {
    ...
    UINT32 u4TileRisc[4]; // only used with multi-threaded decoding
    BOOL fgValidTile;
    BOOL fgLastTile;
} TileBuffer;

typedef struct _VP9_LAE_INFO_T_
{
...
typedef struct _VP9_COMMON_T_
{
    ...
    VP9_LAE_INFO_T* prVp9LaeInput;
    //Dram Buffer
    VP9_DRAM_BUF_T rSegIdBuffer;
    VP9_DRAM_BUF_T rMVBuffer;
    ...
    UINT32 u4JifMark;
    UINT64 u4LstOutJif;
    UINT32 u4MaxDecodeTime;
    UINT32 u4AvgDecodeTime;
    UINT32 u4FrameNum;
    UINT32 u4TotalDecoded;
    UINT32 u4TotalDecodeTime;
}VP9_COMMON_T;
