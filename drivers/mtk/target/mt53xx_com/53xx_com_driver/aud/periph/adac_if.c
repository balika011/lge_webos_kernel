    ...
    if ((fgEnableFlag != _fgSpkEnable) || (!_fgSpeakerInit))
    {
        if (fgEnableFlag) //unmute amp
        {
            ...
            {   
                ...
                AUD_DspChannelMute(AUD_DEC_MAIN, AUD_CH_FRONT_LEFT, FALSE);
                AUD_DspChannelMute(AUD_DEC_MAIN, AUD_CH_FRONT_RIGHT, FALSE);
            #else
                ADAC_GpioAmpEnable(fgEnableFlag);
            #endif
            }
...
#if defined(CC_MT5398) || defined(CC_MT5399) || defined(CC_MT5890) //|| defined(CC_MT5861) || defined(CC_MT5882)
        if (fgEnable)
        {
...
void ADAC_GpioAmpEnable(BOOL fgEnable)
{
    ...
    if (u4GpioAmp != 0xffffffff)
    {
        ...
        LOG(8, "[AUD] Aud AMP Enable GPIO : %d\n", i4RegAoutSwitch);
...
#endif
    u4GpioAmp = DRVCUST_OptGet(eAudioAmpEnable1GpioNum);
    if (u4GpioAmp != 0xffffffff)
    {
        eAmpPolariy = (AUD_AMP_GPIO_POLARITY_T)DRVCUST_OptGet(eAudioAmp1GpioPolarity);
        if (eAmpPolariy == AUD_AMP_GPIO_HIGH_ENALBE)
        {
            i4RegAoutSwitch = fgEnable ? 1 : 0;
        }
        ...
        LOG(8, "[AUD] Aud AMP1 Enable GPIO : %d\n", i4RegAoutSwitch);
        AUD_GPIOOutput(u4GpioAmp, i4RegAoutSwitch);
    }
}
#endif
