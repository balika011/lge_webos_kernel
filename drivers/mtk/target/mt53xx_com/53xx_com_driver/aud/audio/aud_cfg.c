...
static void AUD_LineinLRDetect(UINT8 u1DecId, BOOL fgEnable);
...
static void _AudClkSrcSel(UINT8 u1DecId, AUD_DEC_STREAM_FROM_T eStreamFrom, AUD_FMT_T eFmt)
{
...
#ifdef CC_AUD_I2S_IN
        ||(eStreamFrom == AUD_STREAM_FROM_I2S) || (ePreviouStream == AUD_STREAM_FROM_I2S)
#endif
        )
    {
        bHDMIClockDetect();
    ...
    {
        /// Set ext_hdmi_bck/lrck/sdata0 for Line-in 3 use.
        ///AUD_WRITE32(REG_DEC3_LIN_CFG0, (AUD_READ32(REG_DEC3_LIN_CFG0) & (~LIN_CLK_SEL_MASK)) | AOUT_HDMI);

        /// To Do, Mustang without Line-in 3, use Line-in 2 first
        AUD_WRITE32(REG_DEC2_LIN_CFG0, (AUD_READ32(REG_DEC2_LIN_CFG0) & (~LIN_CLK_SEL_MASK)) | AOUT_HDMI);
    }
#endif
    ...
    switch (eStreamFrom)
    {
        ...
        case AUD_STREAM_FROM_LINE_IN_3:
            /// To Do
            /// Set Dsp2 Dec0 Control Line-in 3
            ///AUD_CLR_BIT(REG_ACK2_CFG, DEC3_LIN_CLK_AUD3_SEL);
            // Slave mode
            ///AUD_SET_BIT(REG_ACK_CFG, LIN3_MS);

            /// To Do, Mustang without Line-in 3, use Line-in 2 first
            AUD_CLR_BIT(REG_ACK2_CFG, DEC2_LIN_CLK_AUD3_SEL);
            // Slave mode
            AUD_SET_BIT(REG_ACK_CFG, LIN2_MS);
            break;
#endif
#ifdef CC_AUD_I2S_IN
...
static void _AUD_SetCustSpec2GpioEnable(BOOL fgEnable)
{
    ...
    if (u4CustSpecGpio != 0xffffffff)
    {
        ...
        LOG(8, "Aud Cust Spec2 Gpio Switch : %d\n", i4RegGpioSwitch);
        AUD_GPIOOutput(u4CustSpecGpio, i4RegGpioSwitch);
    }
    else
    {
...
static void _AUD_SetCustHpGpioEnable(BOOL fgEnable)
{
    ...
    if (u4CustHpGpio != 0xffffffff)
    {
        ...
        LOG(8, "Aud Cust Spec HP Gpio Switch : %d\n", i4RegGpioSwitch);
...
void AUD_PowerOffGpioMute(BOOL fgEnable)
{
    ...
    x_thread_delay(5);
    //Printf("[DEBUG] AMP Rst delay 30ms\n");
...
void _AUD_BitStrPtrSel(UINT8 u1DecId, AUD_DEC_STREAM_FROM_T eStreamFrom)
{
    ...
    switch (eStreamFrom)
    {
        ...
        {
            u4Format = u4Format | PTR_SEL_BANK2_DEFINED0;
            //u4Format = u4Format | PTR_SEL_BANK2_DMX;
        }
        AUD_WRITE32(REG_BS_CFG, u4Format); //AOMX2_TEST_TODO
        
        #if 0
        AUD_WRITE32(REG_BS_CFG, u4Format | 
            ((u1DecId == AUD_DEC_MAIN) ? PTR_SEL_BANK0_PSR : PTR_SEL_BANK1_DMX)); //AOMX2_TEST_TODO
        #endif
#else
            AUD_WRITE32(REG_BS_CFG, u4Format |
                        ((u1DecId == AUD_DEC_MAIN) ? PTR_SEL_BANK0_PSR : PTR_SEL_BANK1_PSR));
...
//#ifdef CC_AUD_HDMI_PARSER_3_0_SUPPORT
        ...
        case AUD_STREAM_FROM_I2S: 
...
#elif defined(DSP_TEST_LINE3)
            AUD_WRITE32(REG_BS_CFG, u4Format |
                        ((u1DecId == AUD_DEC_MAIN) ?
                         PTR_SEL_BANK0_DEC3_SPDIF_LINE_IN : PTR_SEL_BANK1_DEC3_SPDIF_LINE_IN));
#else
            if (u1DecId == AUD_DEC_MAIN)
            {
                u4Format |= PTR_SEL_BANK0_SPDIF_LINE_IN;
            ...
            else if (u1DecId == AUD_DEC_THIRD) // THIRD Decoder, on-line sound/phone-ring
            {
                ...
                u4Format |= PTR_SEL_BANK2_SPDIF_LINE_IN;
#else
                u4Format |= PTR_SEL_BANK2_DEC2_SPDIF_LINE_IN;
                //u4Format |= PTR_SEL_BANK2_DEC3_SPDIF_LINE_IN;     //DEC3_USE_LINEIN3
#endif
            }
            else if (u1DecId == AUD_DEC_4TH)
            {
    ...
    case AUD_STREAM_FROM_LINE_IN_3:
        /// To Do
        /// Set Line-in 3 for Dsp2 Dec0

        /// Set Line-in 3 for Dsp1 Dec0 first
        //AUD_WRITE32(REG_BS_CFG, u4Format | 
        //    ((u1DecId == AUD_DEC_MAIN) ? 
        //    PTR_SEL_BANK0_DEC3_SPDIF_LINE_IN : PTR_SEL_BANK1_DEC3_SPDIF_LINE_IN));
...
void AUD_StrInputEnable(UINT8 u1DecId, AUD_DEC_STREAM_FROM_T eStreamFrom, AUD_FMT_T eFmt, BOOL fgEnable, UINT32 u4ReceivePesCnt)
{
    ...
    {
        vWriteShmUINT8 (B_DELAY_MODE_BYPASS, 0x1); 
    }
    if((eStreamFrom != AUD_STREAM_FROM_LINE_IN) && (u1DecId == AUD_DEC_MAIN)&& fgEnable)
    {
        vWriteShmUINT8 (B_DELAY_MODE_BYPASS, 0x0); 
    }
    ...
    if (fgEnable)
    {
        switch (eStreamFrom)
        {
            ...
            case AUD_STREAM_FROM_FEEDER:
#ifdef CC_ENABLE_AOMX
                if (u1DecId == AUD_DEC_AUX) //AOMX2_TEST_TODO
                {
                    AUD_WRITE32(REG_DMX_STR, DSP_INTERNAL_ADDR(u4GetAFIFOStart(u1DecId) >> 2));
                    AUD_WRITE32(REG_DMX_END, DSP_INTERNAL_ADDR(u4GetAFIFOEnd(u1DecId) >> 2));
                }
                else if(u1DecId == AUD_DEC_THIRD)
                {
                    #if 1
            ...
            case AUD_STREAM_FROM_LINE_IN_2: 
                ...
                AUD_SET_BIT(REG_DEC2_LIN_CFG0, LR_DETECT_ON);
            ...
            case AUD_STREAM_FROM_LINE_IN_3: 
                ...
                /// To Do
                /// Need change for Oryx PAD & Pinmux.
                /// Use PAD_AOSDATA1~4 be ext_hdmi_mclk/_lrck/_bck/_sdata0
                vIO32WriteFldAlign(CKGEN_PMUX3, 3, FLD_PAD_AOSDATA1);
                vIO32WriteFldAlign(CKGEN_PMUX8, 3, FLD_PAD_AOSDATA4);
                AUD_SET_BIT(REG_TEST_CFG, HDMI_SPDIF_SEL);
                ...
                AUD_SET_BIT(REG_DEC3_LIN_CFG0, LR_DETECT_ON);
...
//#ifdef CC_AUD_HDMI_PARSER_3_0_SUPPORT
            ...
            case AUD_STREAM_FROM_MULTI_MEDIA:
                if ((eStreamFrom == AUD_STREAM_FROM_MULTI_MEDIA) &&
                    (AUD_GetMultimediaMode(u1DecId) == AUD_MM_MODE_GAME))
                {
                    // Setup audio fifo
                    //vSetSPDIFInFIFO(u1DecId, u4GetAFIFOStart(AUD_DEC_THIRD), u4GetAFIFOEnd(AUD_DEC_THIRD));
#ifndef CC_MT5391_AUD_3_DECODER
                    VERIFY(PSR_SoftSetAudioFIFO(u1DecId, u4GetAFIFOStart(u1DecId), u4GetAFIFOEnd(u1DecId)));
#else
#ifndef CC_AUD_SKYPE_SUPPORT
...
#endif
                    // Turn off input when config format
                    //AUD_AinEnable(u1DecId, AUD_STREAM_FROM_LINE_IN, FALSE);
                    // Format config
                    //AUD_AinSrcSel(u1DecId, AUD_STREAM_FROM_LINE_IN, eFmt);
                    // Turn on input
                    //AUD_AinEnable(u1DecId, AUD_STREAM_FROM_LINE_IN, TRUE);
                }
                else
                {
#ifndef CC_AUD_SKYPE_SUPPORT
    ...
    else // Turn off stream input
    {
        switch (eStreamFrom)
        {
            ...
            case AUD_STREAM_FROM_LINE_IN_2:
                ...
                AUD_CLR_BIT(REG_DEC2_LIN_CFG0, LR_DETECT_ON);
            ...
            case AUD_STREAM_FROM_LINE_IN_3:
                ...
                /// To Do, Mustang without Line-in 3, use Line-in 2 first
                ///AUD_CLR_BIT(REG_DEC3_LIN_CFG0, LR_DETECT_ON);
                AUD_CLR_BIT(REG_DEC2_LIN_CFG0, LR_DETECT_ON);
...
void  AUD_SetSampleFreq(UINT8 u1DecId, SAMPLE_FREQ_T eSmpFreq)
{
    ...
    {
        DRVCUST_SendAudEvent(E_CUST_AUD_FORMAT_CHANGE, u1DecId); //BE_FIXME
            ...
            ((u1DecId == AUD_DEC_THIRD) && _IsTriOnlyDecMode() && (_aeStrSrcConf[AUD_DEC_THIRD] != AUD_STREAM_FROM_HDMI)))
        {
            ...
            if((_aeStrSrcConf[AUD_DEC_MAIN] == AUD_STREAM_FROM_DIGITAL_TUNER)&&(_AudGetStrFormat(AUD_DEC_AUX) == AUD_FMT_AC3)&&(AUD_DRVGetAudioState(AUD_DEC_AUX)==AUD_ON_PLAY))
            {
...
void AUD_AinSrcSel(UINT8 u1DecId, AUD_DEC_STREAM_FROM_T eStreamFrom, AUD_FMT_T eAudFmt)
{
...
#elif defined(DSP_TEST_LINE3)
        u4Format1 |= ( PTR_SEL_BANK0_DEC3_SPDIF_LINE_IN);
#else
        if (u1DecId == AUD_DEC_MAIN)
        {
            u4Format1 |= ( PTR_SEL_BANK0_SPDIF_LINE_IN);
...
#else
            u4Format1 |= ( PTR_SEL_BANK2_DEC2_SPDIF_LINE_IN);
            //u4Format1 |= ( PTR_SEL_BANK2_DEC3_SPDIF_LINE_IN);     //DEC3_USE_LINEIN3
#endif
        }
        else if (u1DecId == AUD_DEC_4TH)
        {
...
#ifdef KARAOKE_MIC_LINE_IN2
    else if (eStreamFrom == AUD_STREAM_FROM_LINE_IN_2) //CC_AUD_SUPPORT_MIC_INPUT
    {
#ifdef USE_16BIT_ALIGNMENT        
...
#ifdef CC_AUD_LINE_IN3
    else if (eStreamFrom == AUD_STREAM_FROM_LINE_IN_3) //CC_AUD_SUPPORT_MIC_INPUT
    {
        /// To Do
    ...
    switch (eStreamFrom)
    {
        ...
        case AUD_STREAM_FROM_LINE_IN_3:
#endif
            AUD_AinCfg(u1DecId, eStreamFrom, (const AIN_CFG_T* )&_rLineInCfg);
            break;
        case AUD_STREAM_FROM_HDMI:
...
{
    UINT32 u4AUDCFG = 0;
#if defined (CC_AUD_LINE_IN3) || defined (KARAOKE_MIC_LINE_IN2)
    UINT32 u4AUDCFG_0 = 0;
#endif
    ...
    {    
        ///u4AUDCFG = AUD_READ32(REG_EXTADC_CFG2) & (~(AUD_IN_CFG_MASK << 16)); 
        /// To Do, Mustang without Line-in 3, use Line-in 2 first
        u4AUDCFG_0 = AUD_READ32(REG_EXTADC_CFG) & (~(AUD_IN_CFG_MASK << 16)); 
        u4AUDCFG = (AUD_READ32(REG_EXTADC_CFG)>>16) & (~AUD_IN_CFG_MASK);
    }
    ...
    if (prAinCfg != NULL)
    {
        ...
        if (eStrSrc == AUD_STREAM_FROM_LINE_IN_2)
        {   
            u4AUDCFG &= (~SPLIN_LR_INVT);  //MIC2 (line_in_2) should not set this bit, since it's external I2S data input.
            AUD_WRITE32(REG_EXTADC_CFG, ((u4AUDCFG<<16) | u4AUDCFG_0) );
        }
        else
        {
            AUD_WRITE32(REG_EXTADC_CFG, u4AUDCFG);
        }
        AUD_CLR_BIT(u4REG_LIN_CFG, SPDIF_LINE_IN_ENABLE);
        AUD_SET_BIT(u4REG_LIN_CFG, SPDIF_LINE_IN_ENABLE);
#else
        AUD_WRITE32(REG_EXTADC_CFG, u4AUDCFG);
        AUD_CLR_BIT(REG_LIN_CFG0, SPDIF_LINE_IN_ENABLE);
        AUD_SET_BIT(REG_LIN_CFG0, SPDIF_LINE_IN_ENABLE);
...
#ifdef CC_AUD_LINE_IN3
        else if (eStrSrc == AUD_STREAM_FROM_LINE_IN_3)
        {   
            u4AUDCFG &= (~SPLIN_LR_INVT);  //MIC2 (line_in_2) should not set this bit, since it's external I2S data input.
            ///AUD_WRITE32(REG_EXTADC_CFG2, u4AUDCFG);
            /// To Do, Mustang without Line-in 3, use Line-in 2 first
            AUD_WRITE32(REG_EXTADC_CFG, ((u4AUDCFG<<16) | u4AUDCFG_0) );
            AUD_CLR_BIT(REG_DEC2_LIN_CFG0, SPDIF_LINE_IN_ENABLE);
            AUD_SET_BIT(REG_DEC2_LIN_CFG0, SPDIF_LINE_IN_ENABLE);
...
#elif defined(DSP_TEST_LINE3)
        AUD_SET_BIT(REG_DEC3_LIN_CFG0, SPDIF_LINE_IN_ENABLE);           //laihui
#else
#ifndef DEC3_USE_LINEIN2
...
#elif defined(DSP_TEST_LINE3)
        AUD_CLR_BIT(REG_DEC3_LIN_CFG0, SPDIF_LINE_IN_ENABLE);       //laihui
#else
#ifndef DEC3_USE_LINEIN2
...
BOOL  _fgApolloOcdCmdLog = 0;
UINT32 AUD_ApolloProbePause(void)
{
    //AUD_DspProbeEnable();
    APOLLO_WRITE32(APOLLO_REG_CORE_CFG,0x1);
    return 0;
}

UINT32 AUD_ApolloProbeGo(void)
{
    //AUD_DspProbeEnable();
    APOLLO_WRITE32(APOLLO_REG_CORE_CFG,0x0);
    return 0;
}
UINT32 AUD_ApolloProbeOcdOpen(void)
{
      APOLLO_SET_BIT(APOLLO_OCD_BYP_JTAG,OCD_BYP_OPEN);
    return 0;
}
UINT32 AUD_ApolloProbeOcdClose(void)
{
      APOLLO_CLR_BIT(APOLLO_OCD_BYP_JTAG,OCD_BYP_OPEN);
    return 0;
}
UINT32 AUD_ApolloProbeOcdIW(UINT16 u2Cmd)
{
      int i;
      UINT16 u2Tmp;
      ...
      u2Tmp = u2Cmd;
      i = 0;
      APOLLO_WRITE32(APOLLO_OCD_INSTR_IN,u2Cmd);
      APOLLO_WRITE32(APOLLO_OCD_INSTR_EXEC,1);
      while(APOLLO_READ32(APOLLO_OCD_BYP_JTAG) &0x00000002)
      {
          x_thread_delay(10);
          i++;
          if (i>100)
          {
              i = 0;
              LOG(0,"AUD_ApolloProbeOcdIW no response...\n");
      }
      }
      if (_fgApolloOcdCmdLog) LOG(1, "IW(%x)\n", u2Tmp);
      return 0;
}
UINT32 AUD_ApolloProbeOcdDR(UINT16 u2Cmd)
{
      UINT32 u4Data;
      UINT16 u2Tmp;
      ...
      APOLLO_WRITE32(APOLLO_OCD_INSTR_IN,u2Cmd);
      //AUD_WRITE32(APOLLO_OCD_INSTR_EXEC,1);
      while(APOLLO_READ32(APOLLO_OCD_BYP_JTAG) &0x00000002)
      {
          x_thread_delay(10);
      }
      //return APOLLO_READ32(APOLLO_OCD_DATA_PO);
      u4Data = APOLLO_READ32(APOLLO_OCD_DATA_PO);
      if (_fgApolloOcdCmdLog) LOG(1, "%x = DR(%x)\n", u4Data, u2Tmp);
      return u4Data;
}
UINT32 AUD_ApolloProbeOcdDW(UINT16 u2Cmd, UINT32 u4Data)
{
      UINT16 u2Tmp;
      
      u2Tmp = u2Cmd;
      APOLLO_WRITE32(APOLLO_OCD_INSTR_IN,u2Cmd);
      APOLLO_WRITE32(APOLLO_OCD_DATA_PI,u4Data);
      APOLLO_WRITE32(APOLLO_OCD_INSTR_WE_IN,1);
      while(APOLLO_READ32(APOLLO_OCD_BYP_JTAG) &0x00000002)
      {
          x_thread_delay(10);
      }
      if (_fgApolloOcdCmdLog) LOG(1, "DW cmd=%x, data = %x\n", u2Tmp, u4Data);
      return 0;
}
UINT32 AUD_ApolloProbeOcdDRW(UINT16 u2Cmd, UINT32 u4Data)
{
      UINT32 u4Result;
      UINT16 u2Tmp;
      ...
      //return AUD_READ32(APOLLO_OCD_DATA_PO);
      u4Result = APOLLO_READ32(APOLLO_OCD_DATA_PO);
      if (_fgApolloOcdCmdLog) LOG(1, "%x = DRW(%x, %x)\n", u4Result, u2Tmp, u4Data);
      return u4Data;
}
BOOL AUD_ApolloProbeOpenChk(void)
{
      if ((APOLLO_READ32(APOLLO_OCD_BYP_JTAG)&OCD_BYP_OPEN)==1) return 1;
    else return 0;
}
BOOL AUD_ApolloProbeOcdPauseChk(void)
{
      if ((APOLLO_READ32(APOLLO_OCD_BYP_JTAG)&OCD_MODE)==OCD_MODE) return 1;
    else return 0;
}
UINT32 u4AUD_ApolloProbeStopOnWrite(UINT32 u4Addr, UINT32 u4Data, UINT32 u4AddrMask, UINT32 u4DataMask, UINT32 u4Type)
{
    ...
    if (u4Addr != 0)
    {
        APOLLO_WRITE32(APOLLO_REG_APL_RCDBG_WDMH_ADR,u4Addr);
        APOLLO_WRITE32(APOLLO_REG_APL_RCDBG_WDMH_ADRMASK,u4AddrMask);
        APOLLO_WRITE32(APOLLO_REG_APL_RCDBG_WDMH_DATA,u4Data); //data:0x12345678
        APOLLO_WRITE32(APOLLO_REG_APL_RCDBG_WDMH_DATAMASK,u4DataMask); // data masking
        
        u4Temp = APOLLO_READ32(APOLLO_REG_APL_RCDBG_CTRL);
        u4Temp &= 0xFFFFCFFF;
        u4Temp |= (u4Type<<12);
        u4Temp |= WDMH_EN;
        APOLLO_WRITE32(APOLLO_REG_APL_RCDBG_CTRL,u4Temp);
        
    }
    else
    {
        u4Temp = APOLLO_READ32(APOLLO_REG_APL_RCDBG_CTRL);
        APOLLO_WRITE32(APOLLO_REG_APL_RCDBG_CTRL,u4Temp&(~WDMH_EN));
        printf("Address can't be zero for Apollo DM hold\n");
    }
...
void AUD_ApolloProbeChkStopOnWrite(BOOL* fgEnable, UINT32* u4Addr, UINT32* u4Data, UINT32* u4AddrMask, UINT32* u4DataMask, UINT32* u4Type)
{

    *fgEnable = (APOLLO_READ32(APOLLO_REG_APL_RCDBG_CTRL) & (0x010)) >> 4;
    *u4Addr = APOLLO_READ32(APOLLO_REG_APL_RCDBG_WDMH_ADR);
    *u4Data = APOLLO_READ32(APOLLO_REG_APL_RCDBG_WDMH_DATA);
    *u4AddrMask = APOLLO_READ32(APOLLO_REG_APL_RCDBG_WDMH_ADRMASK);
    *u4DataMask = APOLLO_READ32(APOLLO_REG_APL_RCDBG_WDMH_DATAMASK);
    *u4Type = (APOLLO_READ32(APOLLO_REG_APL_RCDBG_CTRL) & (0x3000)) >> 12;
