...
static UINT32 _au4AudCtrlType[AUD_DEC_MAX] = { AUD_INVALID_CTRL_TYPE, AUD_INVALID_CTRL_TYPE, AUD_INVALID_CTRL_TYPE, AUD_INVALID_CTRL_TYPE };
static AUD_DEC_CMD_T _aeAudCtrlCmd[AUD_DEC_MAX] = { AUD_CMD_NULL, AUD_CMD_NULL, AUD_CMD_NULL, AUD_CMD_NULL };
...
void AUD_CommandDone(UINT8 u1DecId,  AUD_DEC_CMD_T eAudDecCmd, UINT32 u4Result)
{
    ...
    if ((pfAudDecNfy != NULL) && ((_au4AudCtrlType[u1DecId] != AUD_INVALID_CTRL_TYPE) || (eAudDecCmd == AUD_CMD_STOPSTREAM)))
    {
        if (_aeAudCtrlCmd[u1DecId] == eAudDecCmd)
        {
            _aeAudCtrlCmd[u1DecId] = AUD_CMD_NULL;
...
#ifdef CC_AUD_DDI
BOOL AUD_GetMMAudioOnly(void)
{
    return AUD_DrvGetMMAudioOnly();
}
#endif
//-----------------------------------------------------------------------------
...
INT32 AUD_SetDecType(UINT8 u1DecId,  AUD_DEC_STREAM_FROM_T eStreamFrom, AUD_FMT_T eDecFmt)
{
    ...
    if ((eStreamFrom == AUD_STREAM_FROM_DIGITAL_TUNER) && (eDecFmt==AUD_FMT_PCM)&&(u1DecId==AUD_DEC_AUX))
    {
        LOG(5,"eDecFmt = AUD_FMT_LPCM\n");
        eDecFmt = AUD_FMT_LPCM;
    }
	...
	// notify
	AUD_GetDecodeType(u1DecId, &eFrom, &eFmt);
	VERIFY(_AUD_GetNotifyFunc(&pfAudDecNfy));
    ...
    if (!AUD_IsDecoderRealPlay(u1DecId))
    {
...
INT32 AUD_SetSourceFrom(UINT8 u1DecId,  AUD_DEC_STREAM_FROM_T eStreamFrom)
{
    ...
    AUD_DEC_ID_VALIDATE(u1DecId);
    VERIFY(x_sema_lock(_hSema, X_SEMA_OPTION_WAIT) == OSR_OK);
    AUD_DRVSetStreamFrom(u1DecId, eStreamFrom);
    VERIFY(x_sema_unlock(_hSema) == OSR_OK);
    return AUD_OK;
}
...
UINT8 AUD_GetResource(AUD_DEC_STREAM_FROM_T eStreamFrom, BOOL fgIsAD, BOOL fgForRec)
{
    ...
    AUD_GetStreamFrom(AUD_DEC_AUX, &eStreamFromCur1);
    AUD_GetStreamFrom(AUD_DEC_THIRD, &eStreamFromCur2);
...
void AUD_PlayAudClip(UINT8 u1DecId, UINT32 u4Channel, BOOL fgPlayTestTone)
{
    ...
    for (i = (UINT8)AUD_CH_FRONT_LEFT; i <= (UINT8)AUD_CH_DMX_RIGHT; i++)
    {
        _au1ChannelDelay[u1DecId][i] =
            AUD_DspGetChannelDelay((AUD_CH_T)i);
		AUD_DspChannelDelay(0,(AUD_CH_T)i,u1DecId);
    }
...
void AUD_StopAudClip(UINT8 u1DecId)
{
    ...
    if (_fgClipChnVolChanged)
    {
        ...
        for (i = (UINT8)AUD_CH_FRONT_LEFT; i <= (UINT8)AUD_CH_DMX_RIGHT; i++)
        {
			AUD_DspChannelDelay(_au1ChannelDelay[u1DecId][i], (AUD_CH_T)i, u1DecId);
        }
    }
 ...
 */
//-----------------------------------------------------------------------------
void AUD_SetCtrlType(UINT8 u1DecId, UINT32 u4MWSetCtrlType)
{
    _au4AudCtrlType[u1DecId] = u4MWSetCtrlType;
}
 ...
 */
//-----------------------------------------------------------------------------
void AUD_SetCtrlCmd(UINT8 u1DecId, AUD_DEC_CMD_T eMWSetCtrlCmd)
{
    _aeAudCtrlCmd[u1DecId]= eMWSetCtrlCmd;
}
...
void AUD_HEAACLevelAdjust (UINT8 u1Level)
{
    AUD_DrvHEAACLevelAdjust(u1Level);
}
#endif
...
void AUD_LPcmSetting(UINT8 u1DecId, AUD_LPCM_INFO_T* prLPcmInfo)
{
    ...
    LOG(1, "prLPcmInfo, (%d, %d, %d, %d, %d, %d, %d, %d, %d)\n", 
    u1DecId,
    prLPcmInfo->b_big_endian, 
    prLPcmInfo->b_info_not_specific, 
    prLPcmInfo->b_signed_data, 
    prLPcmInfo->eLPcmType, 
    prLPcmInfo->eSampleFreq, 
    prLPcmInfo->ui2_bits_per_sample, 
    prLPcmInfo->ui2_block_align, 
    prLPcmInfo->ui2_num_ch);
...
void AUD_MM_Set_Dec_Fmt_Conflict(UINT8 u1DecId, BOOL fgFlag)
{
    ...
    AUD_DRVGetStreamFrom(u1DecId, &eAudDecStreamFrom);
...
#if defined(DSP_FORCE_RESET_ENABLE) || defined(CC_FAST_INIT) || defined(CC_SUPPORT_STR)
...
void AUD_pm_str_suspend(void)
{
    ...
    LOG(0, "[AUDS] AUD_pm_suspend. (20120621a)\n");
    {
        int i;
...
#endif
            if (!fgDecoderStopped(i))
            {
                AUD_DSPCmdStop(i);
                while (!fgDecoderStopped(i))
                {
                    LOG(0, "[AUDS] waiting decoder %d stop\n", i);
                    x_thread_delay(10);
                }
            }
        }
    ...
    /////// Power Down Audio Module ///////
    /// Disable audio output stage.
    LOG(0, "AUD_pm_str_suspend - Power Down. (20120618a)\n");
    if (!(u4AudPmStrSkip & 0x1))
    {
        ADAC_Mute(TRUE);
    }
    if (!(u4AudPmStrSkip & 0x2))
    {
        ADAC_GpioCodecMute(TRUE);
    }
    if (!(u4AudPmStrSkip & 0x4))
    {
        ADAC_GpioAmpEnable(FALSE);
    }
    if (!(u4AudPmStrSkip & 0x8))
    {
        ADAC_CodecMute(TRUE);
    }
#if 0
    if (!(u4AudPmStrSkip & 0x10))
    {
        ADAC_MuteCLK(TRUE);    ///PWM CLK Fade-out to 0V.
    }
#endif
    ...
    /// Disable MCLK Output
    if (!(u4AudPmStrSkip & 0x20))
    {
        _AUD_OutPadEnable(AUD_DEC_MAIN, FALSE);
    }
    if (!(u4AudPmStrSkip & 0x40))
    {
        _AUD_OutPadEnable(AUD_DEC_AUX, FALSE);
    }
    ...
    /// Disable AADC
    if (!(u4AudPmStrSkip & 0x80))
    {
        ADAC_ADCDown(TRUE);
    }
    /// Disable PWMDAC
    if (!(u4AudPmStrSkip & 0x100))
    {
        ADAC_ADCDown(TRUE);
    }
#endif
    ...
    /// Power down Aud Pll
    if (!(u4AudPmStrSkip & 0x200))
    {
        AudPllPowerDown(APLL_ID_MAIN, 1);
    }
    if (!(u4AudPmStrSkip & 0x400))
    {
        AudPllPowerDown(APLL_ID_AUX, 1);
    }

    /// Power down DSP
    if (!(u4AudPmStrSkip & 0x800))
    {
        vDspPowerOff();
...
void AUD_pm_str_resume(void)
{
...
#else
    /////// Power On Audio Module ///////
    LOG(0, "AUD_pm_str_resume - Power On (20120618a).\n");

    /// Power on DSP
    if (!(u4AudPmStrSkip & 0x800))
    {
        u4ADSP_DspReset();
        //vIO32WriteFldAlign(CKGEN_REG_DSP_CKCFG, 0, FLD_DSP_PD);
    }
    ...
    if (!(u4AudPmStrSkip & 0x10))
    {
        ADAC_MuteCLK(FALSE);    ///PWM CLK Fade-out to 0V.
    }
#endif
    if (!(u4AudPmStrSkip & 0x8))
    {
...
void AUD_OpenGamePcm(UINT8 u1DecId, AUD_FMT_T u1DecFmt, SAMPLE_FREQ_T SampleRate)
{
    ...
    u4AFifoWp = VIRTUAL(u4GetAFIFOStart(u1DecId));
    ...
    AUD_SetDecType(u1DecId, AUD_STREAM_FROM_MULTI_MEDIA, u1DecFmt);
    DSP_SetPcmFs(u1DecId, SampleRate);
    //AUD_PcmSetting(u1DecId, (const AUD_PCM_SETTING_T *)&rPcmSetting);
    //_AUD_IgnoreDecNotify(u1DecId, TRUE);
    //AUD_DSPCmdPlay(u1DecId);
    PSR_RiscSetAudioWp(u1DecId, PHYSICAL(u4AFifoWp));
    //_AUD_IgnoreDecNotify(u1DecId, FALSE);
}
...
void AUD_GetPcmDelay(UINT32 u1DecId, UINT32* pdelay2)
{
    ...
    u4AFifoSize = u4GetAFIFOEnd(u1DecId) - u4GetAFIFOStart(u1DecId);
    u4AFifoRp = VIRTUAL(DSP_GetDspReadPtrPhysicalAddr(u1DecId));
    u4AFifoWp = VIRTUAL(DSP_PHYSICAL_ADDR(PSR_SoftGetAudioWp()));
...
UINT32 AUD_FillPcmBufer(UINT8 u1DecId, UINT32* lpBuffer2, UINT32 size2)
{
    ...
    if (size2 < u4AFfioFreeSize)
    {
        VERIFY(AUD_PcmBufferWrite(u4AFifoWp, lpBuffer2, size2,
                                  VIRTUAL(u4GetAFIFOStart(u1DecId)), VIRTUAL(u4GetAFIFOEnd(u1DecId))));

        u4AFifoWp = ((u4AFifoWp + size2) < VIRTUAL(u4GetAFIFOEnd(u1DecId))) ?
...
BOOL AUD_PlayMuteSetChannelDelayReady(UINT16 u2ChannId)
{
    ...
    bRet = AUD_PlayMuteCmd(AUD_PM_CMD_SET_DELAY_READY, u2ChannId);
    #endif
    return bRet;
}
#endif
...
BOOL AUD_PlayMuteSetAoutEnableReady(UINT16 u2Timeout)
{
    BOOL bRet = TRUE;
    #ifdef CC_AUD_ENABLE_PLAY_MUTE_CONTROL
    bRet = AUD_PlayMuteCmd(AUD_PM_CMD_AOUT_ENABLE, u2Timeout);
    #endif
    return bRet;
}
#endif
...
#ifdef CC_AUD_LINE_IN3
void AUD_Line3Input_Enable(BOOL fgEnable) //CC_AUD_SUPPORT_MIC_INPUT
{
    UNUSED(AUD_Line3Input_Enable);
    /// To Do
    /// Use Dsp1 Dec0 first, need change to Dsp2 Dec0.
    if (fgEnable)
    {
        // Disable notify
        ...
        // Set Main Dec Source from Line_in_3
        AUD_SetDecType(AUD_DEC_MAIN, AUD_STREAM_FROM_LINE_IN_3, AUD_FMT_PCM);    
...
{
    if (AUD_IsDSPRealPlay(AUD_DEC_MAIN))
    {
        AUD_DSPCmdStop (AUD_DEC_MAIN);
        if (fgEnable) 
        { 
            vAprocS_Enable(0);                    // if DD_banner enable => disable upload_post
        }    
        else                                      // else                => depends on bIsSupportAproc
        {
            if (bIsSupportAproc () == TRUE) vAprocS_Enable(1);
            else vAprocS_Enable(0);
        }
        AUD_DSPCmdPlay (AUD_DEC_MAIN);
    }
    else
