...
// APE decoder
    ...
    case AUD_DTS_DEC3:
        ...
        //DTS Encoder Table ,
        dwDspCmptBuf = dwGetDSPCSADR (AUD_DTSENC);
...
//    BOOL fgWaitDsp;
    ...
    fgLoadCode  = FALSE;
    fgLoadTable = FALSE;
    dwFlashOfst = u4GetDspImgAddr_new (u4Type);   // set to dsp image addr
    ...
    case TYPE_AUD_DEC1:
        ...
        if (pAUD2COD1[u4Type - TYPE_AUD_DEC1] == DSPF_AAC_DEC1_SA)
        {
            ...
            if (dwSz)
            {
                dwDECICAddr = (dwGetDspIcacheAdr (TYPE_ENC2_ICACHE) + AACEXT_PRAM_ADR);
                vFlash2DramDma ((dwDECICAddr * 4), dwFlashOfst + (dwOfst * 4), (dwSz - 1) * 4);
            }
#endif
#ifdef CC_AUD_SUPPORT_MS10
            dwFlashOfstTmp = u4GetDspImgAddr_new (AUD_DDCO);   // set to dsp image addr
    ...
    case TYPE_AUD_DEC3:
        ...
        vDSPLoadDataSram(AUD_DEC_THIRD, pAUD2DROM3[u4Type-TYPE_AUD_DEC3]);
#endif
        break;
#endif
#ifdef CC_AUD_4_DECODER_SUPPORT
    case TYPE_AUD_DEC4:
        ...
        fgLoadCode = TRUE;
#ifdef CC_AUD_DATASRAM //Replace DATA ROM to DATA SRAM
        vDSPLoadDataSram(AUD_DEC_4TH, pAUD2DROM4[u4Type-TYPE_AUD_DEC4]);    /// MP3ENC_SUPPORT
#endif
        break;
#endif
    case TYPE_AUD_CMM:
        ...
        case AUD_POSTRAM:
            ...
            dwOfst= u4GetFlashData(u4GetDspImgAddr()+(DSPF_SRS_WOWHD_SA<<2));
            dwSz  = u4GetFlashData(u4GetDspImgAddr()+(DSPF_SRS_WOWHD_SZ<<2));
    #elif defined(DSP_SUPPORT_SRSCC3D)
            dwOfst= u4GetFlashData(u4GetDspImgAddr()+(DSPF_SRS_CC3D_SA<<2));
            dwSz  = u4GetFlashData(u4GetDspImgAddr()+(DSPF_SRS_CC3D_SZ<<2));
    #else
       ...
       case AUD_DTSENC:
        ...
        case AUD_COMMRAM:
            ...
            if (dwSz)
            {
                dwDECICAddr = dwGetDspIcacheAdr (TYPE_COMMAEXT_ICACHE);
                vFlash2DramDma ((dwDECICAddr * 4), dwFlashOfst+ (dwOfst * 4), (dwSz - 1) * 4);
            }
            ...
            if (dwSz)
            {
                dwDECICAddr = dwGetDspIcacheAdr (TYPE_COMMB_ICACHE);
                vFlash2DramDma ((dwDECICAddr * 4), dwFlashOfst+ (dwOfst * 4), (dwSz - 1) * 4);
            }
        ...
        case AUD_ROMCOD:
            ...
            if (dwSz)
            {
                dwDECICAddr = dwGetDspIcacheAdr (TYPE_ROM2_ICACHE);
                vFlash2DramDma ((dwDECICAddr * 4), dwFlashOfst + (dwOfst * 4), (dwSz - 1) * 4);
            }
#endif
...
void vDspSetSyncMode(UCHAR ucDecId, AV_SYNC_MODE_T eMode) //parson MM3
{
    ...
    u2SystemSetupValue = (u2ReadDspShmWORD(u4ShareInfo) & 0x7fff );
    ...
    vWriteDspShmWORD(u4ShareInfo, (UINT16)u2SystemSetupValue);
    vWriteDspWORD (u4ShareDram,   u2ReadDspShmWORD (u4ShareInfo));
...
AV_SYNC_MODE_T eDspGetSyncMode(UINT8 ucDecId)
{
    ...
    u4Ret = (dReadDspCommDram(u4Addr))>>23;
...
INT32 i4DspSendPts (UINT8 u1DecId, UINT32 u4Pts, UINT32 u4ParPnt)
{
    ...
    {
        vSendDspLongInt(INT_RC2D_PTS, u4ParPnt, u4Pts, FALSE);
    }
    else if (u1DecId == AUD_DEC_AUX)
    {
        vSendDspLongInt(INT_RC2D_PTS_DEC2, u4ParPnt, u4Pts, FALSE);
    }
#ifdef CC_MT5391_AUD_3_DECODER
    else if (u1DecId == AUD_DEC_THIRD)
    {
        vSendDspLongInt(INT_RC2D_PTS_DEC3, u4ParPnt, u4Pts, FALSE);
    }
#endif
...
/***************************************************************************
  ...
  Description :
    ...
    Return    :
***************************************************************************/
INT32 i4DspResetPts (UINT8 u1DecId)
{
    if (fgDspBusy ())
    {
        return DSP_BUSY;
    ...
    if (u1DecId == AUD_DEC_MAIN)
    {
        vSendDspLongInt(INT_RC2D_RESET_PTS,0,0, FALSE);
    }
...
BOOL fgCheckDspCmd(UINT32 u4DspCmd)
{
    ...
    if (u4DspCmd == DSP_PLAY)
    {
        ...
        else
        {
            ...
            _rDspStatus[AUD_DEC_MAIN].fgDspResume = FALSE;
            _rDspStatus[AUD_DEC_MAIN].fgPausePending = FALSE;
        }
    }
    ...
    else if (u4DspCmd == DSP_PAUSE)
    {
        ...
        else
        {
            ...
            if (_rDspStatus[AUD_DEC_MAIN].fgDspResume)
            {
                LOG(1, "Pending pause command due to DSP resume isn't done\n"); 
                _rDspStatus[AUD_DEC_MAIN].fgPausePending = TRUE;
                i4Ret = FALSE;
            }
            _rDspStatus[AUD_DEC_MAIN].fgDspPtsSet = FALSE;
            _rDspStatus[AUD_DEC_MAIN].fgDspPlay = FALSE;
    ...
    else if (u4DspCmd == DSP_RESUME)
    {
        ...
        else
        {
            ...
            if (_rDspStatus[AUD_DEC_MAIN].fgPausePending)
            {
                _rDspStatus[AUD_DEC_MAIN].fgPausePending = FALSE;
                i4Ret = FALSE;
            }
            ...
            _rDspStatus[AUD_DEC_MAIN].fgDspPlay = TRUE;
            _rDspStatus[AUD_DEC_MAIN].fgDspPause = FALSE;
            _rDspStatus[AUD_DEC_MAIN].fgDspResume = TRUE;
        }
    }
    ...
    else if (u4DspCmd == DSP_PLAY_DEC2)
    {
        ...
        else
        {
            ...
            _rDspStatus[AUD_DEC_AUX].fgDspResume = FALSE;
            _rDspStatus[AUD_DEC_AUX].fgPausePending = FALSE;            
        }
    }
    ...
    else if (u4DspCmd == DSP_PAUSE_DEC2)
    {
        ...
        {
            //Pending Pause when the previous resume isn't done!!!
            if (_rDspStatus[AUD_DEC_AUX].fgDspResume)
            {
                LOG(1, "Pending pause command due to DSP resume isn't done\n"); 
                _rDspStatus[AUD_DEC_AUX].fgPausePending = TRUE;
                i4Ret = FALSE;
            }            
            _rDspStatus[AUD_DEC_AUX].fgDspPtsSet = FALSE;
            _rDspStatus[AUD_DEC_AUX].fgDspPlay = FALSE;
    ...
    else if (u4DspCmd == DSP_RESUME_DEC2)
    {
        ...
        {
            //Clear Pending Pause and don't send resume command
            if (_rDspStatus[AUD_DEC_AUX].fgPausePending)
            {
                _rDspStatus[AUD_DEC_AUX].fgPausePending = FALSE;
                i4Ret = FALSE;
            }            
            ...
            _rDspStatus[AUD_DEC_AUX].fgDspResume = TRUE;      
        }
    }
#ifdef CC_MT5391_AUD_3_DECODER
    else if (u4DspCmd == DSP_PLAY_DEC3)
    {
        ...
        else
        {
            ...
            _rDspStatus[AUD_DEC_THIRD].fgDspResume = FALSE;
            _rDspStatus[AUD_DEC_THIRD].fgPausePending = FALSE;                  
        }
    }
    ...
    else if (u4DspCmd == DSP_PAUSE_DEC3)
    {
        ...
        {
            //Pending Pause when the previous resume isn't done!!!
            if (_rDspStatus[AUD_DEC_THIRD].fgDspResume)
            {
                LOG(1, "Pending pause command due to DSP resume isn't done\n"); 
                _rDspStatus[AUD_DEC_THIRD].fgPausePending = TRUE;
                i4Ret = FALSE;
            }                       
            _rDspStatus[AUD_DEC_THIRD].fgDspPtsSet = FALSE;
            _rDspStatus[AUD_DEC_THIRD].fgDspPlay = FALSE;
    ...
    else if (u4DspCmd == DSP_RESUME_DEC3)
    {
        ...
        {
            //Clear Pending Pause and don't send resume command
            if (_rDspStatus[AUD_DEC_THIRD].fgPausePending)
            {
                _rDspStatus[AUD_DEC_THIRD].fgPausePending = FALSE;
                i4Ret = FALSE;
            }            
            _rDspStatus[AUD_DEC_THIRD].fgDspResume = TRUE;      
            _rDspStatus[AUD_DEC_THIRD].fgDspPtsSet = FALSE;
            _rDspStatus[AUD_DEC_THIRD].fgDspPlay = TRUE;
    ...
    else if (u4DspCmd == DSP_PLAY_DEC4)
    {
        ...
        else
        {
            ...
            _rDspStatus[AUD_DEC_4TH].fgDspResume = FALSE;
            _rDspStatus[AUD_DEC_4TH].fgPausePending = FALSE;               
        }
    }
    ...
    else if (u4DspCmd == DSP_PAUSE_DEC4)
    {
        ...
        {
            //Pending Pause when the previous resume isn't done!!!
            if (_rDspStatus[AUD_DEC_4TH].fgDspResume)
            {
                LOG(1, "Pending pause command due to DSP resume isn't done\n"); 
                _rDspStatus[AUD_DEC_4TH].fgPausePending = TRUE;
                i4Ret = FALSE;
            }                
            _rDspStatus[AUD_DEC_4TH].fgDspPtsSet = FALSE;
            _rDspStatus[AUD_DEC_4TH].fgDspPlay = FALSE;
    ...
    else if (u4DspCmd == DSP_RESUME_DEC4)
    {
        ...
        {
            //Clear Pending Pause and don't send resume command
            if (_rDspStatus[AUD_DEC_4TH].fgPausePending)
            {
                _rDspStatus[AUD_DEC_4TH].fgPausePending = FALSE;
                i4Ret = FALSE;
            }            
            _rDspStatus[AUD_DEC_4TH].fgDspResume = TRUE;  
            _rDspStatus[AUD_DEC_4TH].fgDspPtsSet = FALSE;
            _rDspStatus[AUD_DEC_4TH].fgDspPlay = TRUE;
...
UINT32 u4DspGetSampleRate(UINT8 u1DecId)
{
    ...
    {
        dwSFreq = dReadDspCommDram(ADDR_D2RC_RISC_INFO_SAMPLING_RATE_DEC3);
    }
    else
    {
...
void vDspBlockPtrReset(UINT8 u1DecId)
{
    ...
    while (fgDspPlay)
    {
        ...
        if (!fgDspBusy ())
        {
            vSendDspLongInt(INT_RC2D_WRITE_DSP_MEMORY, dwAFifo, dwBufPNTReg, FALSE);
            break;
        }
...
//        bStrType = _bDspStrTypDec2;
    ...
    {
        dwDspInfo = u4ReadDspShmDWRD (D_SPKCFG_DEC3);
...
void vWriteDspSram (UINT32 u4Addr, UINT32 u4Value)
{
    ...
    {
        vSendDspLongInt(INT_RC2D_WRITE_DSP_MEMORY, (u4Addr<<8), ((u4Value)<<8), TRUE);
...
void vWriteDspSram32 (UINT32 u4Addr, UINT32 u4Value)
{
    ...
    {
        vSendDspLongInt(INT_RC2D_WRITE_DSP_MEMORY, (u4Addr<<8), ((u4Value)), TRUE);
    }
}
...
BOOL fgWriteDspSramInISR (UINT32 u4Addr, UINT32 u4Value)
{
    ...
    if (!fgDspBusy ())
    {
        vSendDspLongInt(INT_RC2D_WRITE_DSP_MEMORY, (u4Addr<<8), ((u4Value<<8)), FALSE);
...
BOOL fgWriteDspSramInISR32 (UINT32 u4Addr, UINT32 u4Value)
{
    ...
    if (!fgDspBusy ())
    {
        vSendDspLongInt(INT_RC2D_WRITE_DSP_MEMORY, (u4Addr<<8), ((u4Value)), FALSE);
        fgRet = TRUE;
    }
    else
    {
...
UINT32 u4ReadDspSram(UINT32 u4Addr)
{
    ...
    else
    {
        ...
        vSendDspLongInt(INT_RC2D_READ_DSP_MEMORY, ((u4Addr) << 8), 0, TRUE);
        x_thread_delay(1);
...
void vResetIecConfig(void)
{
    ...
    {
        u1OutBitNum = (uReadDspShmBYTE(B_IEC_WORD_LENGTH) & 0x03);
    }
    else
    {
    ...
    vWriteDspSram(REG_DSP_IEC_CHANNEL_CFG0, u4IecChannelCfg0 << 8);
    vWriteDspSram(REG_DSP_IEC_CHANNEL_CFG1, u4IecChannelCfg1 << 8);
#if (defined(CC_MT5881) || defined(CC_MT5399))
    vWriteDspSram32(REG_DSP_IEC_CHANNEL_CFG12, (u4ReadDspSram32(REG_DSP_IEC_CHANNEL_CFG12) | 0x80000000));
#endif
...
void vSetIecConfig(UINT8 u1DecId, UINT32 u4FreqIdx)
{
    ...
    if((_rDspStatus[AUD_DEC_MAIN].fgDspRealPlay == FALSE) && (_AudGetStrSource(AUD_DEC_MAIN) == AUD_STREAM_FROM_DIGITAL_TUNER))
    {
        vResetIecConfig();
    }
}
...
BOOL fgTriggerDspErrorLogs(void)
{
    ...
    if (!fgDspBusy ())
    {
        vSendDspLongInt(INT_RC2D_OR, ((LOG_DRAM_REFRESH) << 8),SRAM_SYSTEM_CTRL, FALSE);
        return TRUE;
    }
    else
    {
...
UINT32 u4ReadD2RInfo_AACVersion(UINT8 u1DecId)
{
    ...
    DSP_FlushInvalidateDCacheSmall(CACHE_FLUSH_TYPE_DATA, TYPE_COMM_DRAM_IDX, (u4Idx<<2), 1);

    return (UINT32)(dReadDspCommDram(u4Idx))>>8;
}
#endif
 ...
 */
//-----------------------------------------------------------------------------
UINT32 u4ReadD2RInfo_PtsCurPnt(UINT8 u1DecId)
{
    UNUSED(u1DecId);
    //DSP_FlushInvalidateDCache(CACHE_FLUSH_TYPE_DATA, TYPE_COMM_DRAM_IDX);
    DSP_FlushInvalidateDCacheSmall(CACHE_FLUSH_TYPE_DATA, TYPE_COMM_DRAM_IDX,
    ...
    return (UINT32)dReadDspCommDram(ADDR_D2RC_PTS_CUR_PNT);
}
//-----------------------------------------------------------------------------
 ...
 */
//-----------------------------------------------------------------------------
UINT32 u4ReadD2RInfo_TotalBank(UINT8 u1DecId)
{
    UNUSED(u1DecId);
    //DSP_FlushInvalidateDCache(CACHE_FLUSH_TYPE_DATA, TYPE_COMM_DRAM_IDX);
    DSP_FlushInvalidateDCacheSmall(CACHE_FLUSH_TYPE_DATA, TYPE_COMM_DRAM_IDX,
    ...
    return (UINT32)dReadDspCommDram(ADDR_D2RC_TOTAL_PUTBANK);
}
#endif
...
UINT32 u4ReadInputUnderrun(UINT8 u1DecId)
{
    if (u1DecId == AUD_DEC_MAIN)
    {
        return u4ReadDspSram(SRAM_DECODER_INPUT_UNDERRUN);
    }
    else if (u1DecId == AUD_DEC_AUX)
    {
        return u4ReadDspSram(SRAM_DECODER_INPUT_UNDERRUN_DEC2);
    }
    else if (u1DecId == AUD_DEC_THIRD)
    {
        return u4ReadDspSram(SRAM_DECODER_INPUT_UNDERRUN_DEC3);
    }
    else// if (u1DecId == AUD_DEC_4TH)
    {
        return u4ReadDspSram(SRAM_DECODER_INPUT_UNDERRUN_DEC4);
    }
}
...
void vResetUploadFlag(void)
{
    vWriteDspSram(SRAM_UPLOAD_FLAG,0); //TODO, 0xa78 is upload flag in 5396
}
#endif
...
UINT32 u4AprocReg_Read (UINT32 u4Offset)
{
    ...
    HalInvalidateDCacheMultipleLine ((UINT32)(VIRTUAL((UINT32)u4Addr)), 4+0x20);
...
void vAprocReg_Write (UINT32 u4Offset, UINT32 u4Val)
{
    ...
    *u4Addr = u4Val;
    ...
    HalFlushInvalidateDCacheMultipleLine ((UINT32)(VIRTUAL((UINT32)u4Addr)), 4+0x20);
}
...
UINT32 u4AprocCommSignal_send (UINT32 u4Idx, UINT32 u4Cmd, UINT32 u4Data0, UINT32 u4Data1)
{
    ...
    while (1)
    {
        u4TryCnt++; 
        rCritState = x_crit_start();
        if (!fgRC2ApolloBusy (RC2APOLLO_INT_0))
        {
            APOLLO_WRITE32(APROC_M2A_SIG_REG,APROC_SIGNAL(u4Idx, u4Cmd)); 
            APOLLO_WRITE32(APROC_M2A_DATA0_REG,u4Data0); 
            APOLLO_WRITE32(APROC_M2A_DATA1_REG,u4Data1); 

            APOLLO_WRITE32(APOLLO_RREG_RC2AP_INTRVEC,RC2APOLLO_INT_0);//enable RC2AP interrupt bit0
            x_crit_end(rCritState);
            break;
        }
        else        
...
/***************************************************************************
  ...
  Description : 
    ...
    Return    : 
***************************************************************************/
UINT32 u4ApolloReg_Read (UINT32 u4Offset)
{
    UINT32 *u4Addr;

    u4Addr = (UINT32 *) (u4GetApolloWorkBufNonInit(1)+(APOLLO_SHARE_MEMORY_REG_SADR-APOLLO_EXT_MEM_SADR)+ (u4Offset * APROC_REGMAP_REGSIZE));
    
    HalInvalidateDCacheMultipleLine ((UINT32)(VIRTUAL((UINT32)u4Addr)), 4+0x20);    

    return *((UINT32*)(VIRTUAL((UINT32)u4Addr)));
    //return (UINT32) *u4Addr;
}
...
/***************************************************************************
  ...
  Description : 
    ...
    Return    : 
***************************************************************************/
void vApolloReg_Write (UINT32 u4Offset, UINT32 u4Val)
{
    UINT32 *u4Addr;
    ...
    //*u4Addr = u4Val;
    u4Addr = (UINT32 *) (u4GetApolloWorkBufNonInit(1)+ (APOLLO_SHARE_MEMORY_REG_SADR-APOLLO_EXT_MEM_SADR) + (u4Offset * APROC_REGMAP_REGSIZE));
    *(UINT32 *)(VIRTUAL((UINT32)u4Addr)) = u4Val;
...
UINT32 u4ReadProc(UINT32 u4Addr)
{
...
#if 0 //waiting for CC_AUD_ARM_SUUPPORT 
    if (!_gu4AprocEnable)
    {
        return 0;
    }
    ...
    u4Value=APOLLO_READ32(APROC_A2M_READ_REG); 
...
UINT32 AUD_ProcGetPc(void)
{
    return APOLLO_READ32(APOLLO_REG_CORE_PC); // audio processor PC
}

UINT32 Aud_ProcGetBhist(void)
{
    return APOLLO_READ32(APOLLO_REG_DBGMON_BHIST_ORY);
}    

//-----------------------------------------------------------------------------
/** AUD_ProcGetPc
 *  Get audio processor PC Value
 *
 *  @param
 *  @retval
 */
//-----------------------------------------------------------------------------
UINT32 AUD_ProcGetPcValue(void)
{
    return APOLLO_READ32(APOLLO_REG_CORE_PC_VALUE); // audio processor PC
}
#endif
