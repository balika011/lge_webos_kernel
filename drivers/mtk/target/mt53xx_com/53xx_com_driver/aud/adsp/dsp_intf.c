...
// Imported functions
//-----------------------------------------------------------------------------
extern BOOL AUD_DRVGetDecodeType(UINT8 u1DecId,  AUD_DEC_STREAM_FROM_T * peStreamFrom,AUD_FMT_T * peDecType);
...
static void _GetFlacAudInfo(UINT8 u1DecId, AUD_DEC_INFO_T * prAudInfo);//paul_flac
...
UINT32 _u4DspICacheAddr[13]; //5368/96 // DSP instruction cache start address for each page
#else
...
// Audio processor
UINT32 _gu4AprocMemMap[APROC_MEMMAP_SIZE] = {0, 0, 0};
#endif
#ifdef CC_AUD_APOLLO_SUPPORT
// Audio processor
UINT32 _gu4ApolloMemMap[2] = {0, 0};
#endif

DSP_CMD_HIS_T   _rDspCmdHistory;
DSP_INT_T _rDspIntHistory;
...
BOOL _GetMpegAudInfo(UINT8 u1DecId, AUD_DEC_INFO_T * prAudInfo)
{
    ...
    if (prAudInfo != NULL)
    {
        ...
        if (u1DecId == AUD_DEC_MAIN)
        {
            ...
            prAudInfo->e_dec_type = (AUD_DECODE_TYPE_T)(dReadDspCommDram(ADDR_D2RC_MPEG_STREAM_TYPE)+ AUD_DECODE_TYPE_MPEG1_LAYER3);
        }
#ifdef CC_MT5391_AUD_3_DECODER
...
#endif
        {
            ...
            prAudInfo->e_dec_type = (AUD_DECODE_TYPE_T)(dReadDspCommDram(ADDR_D2RC_MPEG_STREAM_TYPE_DEC2)+ AUD_DECODE_TYPE_MPEG1_LAYER3);
        }
#ifdef CC_MT5391_AUD_3_DECODER
        else
        {
            ...
            prAudInfo->e_dec_type = (AUD_DECODE_TYPE_T)(dReadDspCommDram(ADDR_D2RC_MPEG_STREAM_TYPE_DEC3)+ AUD_DECODE_TYPE_MPEG1_LAYER3);
        }
#endif
...
static void _GetAacAudInfo(UINT8 u1DecId, AUD_DEC_INFO_T * prAudInfo)
{
    ...
    if (prAudInfo != NULL)
    {
        ...
        else if (u1DecId == AUD_DEC_THIRD)
        {
            ...
            // Get AAC version
            u4Version = dReadDspCommDram(ADDR_D2RC_AAC_VERSION_DEC3) >> 8;
            if (u4Version > 2)
            {
                prAudInfo->e_dec_type = AUD_DECODE_TYPE_UNKNOWN;
            }
            ...
            // Get AAC transmission format
            u4Format = dReadDspCommDram(ADDR_D2RC_AAC_TRANS_FORMAT_DEC3) >> 8;
            if (u4Format == 0)
            {
                LOG(5,"This is LATM/LOAS format\n");
            }
...
//paul_flac
static void _GetFlacAudInfo(UINT8 u1DecId, AUD_DEC_INFO_T * prAudInfo)
{
	...
	#endif
	//need review!--paul
	ui4_sample_rate = dReadDspCommDram(ADDR_D2RC_FLAC_SAMPLE_RATE);
	ui1_bit_depth = (UINT8)dReadDspCommDram(ADDR_D2RC_FLAC_BIT_PER_SAMPLE);
	ui1_ch_asm = (UINT8)dReadDspCommDram(ADDR_D2RC_FLAC_CHANNEL_ASM);
	...
	if (NULL != prAudInfo)
    {
		...
		{
			case 0:
				prAudInfo->ui4_sample_rate = u4ReadDspShmDWRD(D_FLAC_SMP_RATE);//0 for get from streaminfo metadata block
				break;
			case 1:
				prAudInfo->ui4_sample_rate = 88200;
				break;
		...
		{
			case 0:
				prAudInfo->ui1_bit_depth = u2ReadDspShmWORD(W_FLAC_BPS);//0 for get from streaminfo metadata block
				break;
			case 1:
...
{
    return ((u2ReadShmUINT16(W_SYSTEM_CTRL) & 0x20) == SHM_SYSTEM_CTRL_DEC3ONLY_MODE);
}
...
void vDspSendSample(UINT8 u1DecId)
{
    ...
    if (_rDspStatus[u1DecId].fgDspRealPlay && !_rDspStatus[u1DecId].fgDspResume)
    {
    ...
    else
    {
        ...
        if (_rDspStatus[u1DecId].fgDspResume)
        {
            LOG(5, "%s: u1DecId(%d), Resume\n", __FUNCTION__, u1DecId);
            AUD_DrvMMAoutEnableNotify(u1DecId, TRUE);
        }
    }

    //Clear Resume flag
    _rDspStatus[u1DecId].fgDspResume = FALSE;
    if (_rDspStatus[u1DecId].fgPausePending)
    {
        LOG(1, "vDspSendSample send pending pause command to DSP!!!\n");
        _rDspStatus[u1DecId].fgPausePending = FALSE;
        _u4DspSIntSD += FLOW_CONTROL_PAUSE;
    }
}
...
void vDspFlushDone(UINT8 u1DecId)
{
    ...
    {
        vWriteDspShmWORD(W_SPEED_DEC3,0x0);
...
// APE decoder
    if ( ((_rDspVars[u1DecId].bDspStrTyp == WMA_STREAM) ||
           (_rDspVars[u1DecId].bDspStrTyp == WMAPRO_STREAM) ||
           (_rDspVars[u1DecId].bDspStrTyp == APE_STREAM))
...
BOOL Dsp_WaitResume(UINT8 u1DecId)
{
    return _rDspStatus[u1DecId].fgDspResume;
}
...
void vDspMemInit(UINT8* puWorkingBuffer)
{
...
#else   //dual decode
    _u4DspICacheAddr[TYPE_ROM_ICACHE] =  ((DSP_AREA_IC_0_SA+u4DramAdr )>>8);   // Set rom as post cache
    _u4DspICacheAddr[TYPE_COMMA_ICACHE] = ((DSP_AREA_IC_1_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_CRI_ICACHE] = ((DSP_AREA_IC_2_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_COMMAEXT_ICACHE] = ((DSP_AREA_IC_3_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_DEC3_ICACHE] = ((DSP_AREA_IC_4_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_DEC1_ICACHE] = ((DSP_AREA_IC_5_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_DEC2_ICACHE] = ((DSP_AREA_IC_6_SA+u4DramAdr )>>8);

    _u4DspICacheAddr[TYPE_ROM2_ICACHE] = ((DSP_AREA_IC2_0_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_POST_ICACHE] = ((DSP_AREA_IC2_1_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_COMMB_ICACHE] = ((DSP_AREA_IC2_2_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_RESER_ICACHE] = ((DSP_AREA_IC2_3_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_DEC4_ICACHE] = ((DSP_AREA_IC2_4_SA+u4DramAdr )>>8);
    _u4DspICacheAddr[TYPE_ENC2_ICACHE] = ((DSP_AREA_IC2_5_SA+u4DramAdr )>>8);
#endif  //dual decode
    ...
    _u4DspCmptBuf[TYPE_APROC_IDX] = 0; //modify by ling
    ...
    _u4DspDramBuf[TYPE_APROC_IDX] = 0;  //modify by ling
...
{
    if (u4Size)
        _u4AFIFO[AUD_DEC_MAIN][1]=_u4AFIFO[AUD_DEC_MAIN][0]+u4Size;
    else
        _u4AFIFO[AUD_DEC_MAIN][1]=_u4AFIFO[AUD_DEC_MAIN][0]+DSP_AFIFO_SZ;

    {
        AUD_DECODER_T* parDecStatus;
        parDecStatus = AUD_DrvGetDecodeState(AUD_DEC_MAIN);
...
void vAdspBinMemInit(void)
{
    ...
    {
        LOG(0, "Error:fill adspbin buffer fail!!\n");
        ASSERT(0);
    }
...
static INT32 i4AdspBinMemFill(UINT32 u4BufAddr, UINT32 u4BufSize)
{
    ...
    STORG_SyncRead(u4PartitionId, ADSP_BIN_HDR_OFFSET, (VOID*)a1DataBuf, ADSP_BIN_HDR_SIZE);
    if (x_strncmp((CHAR*)a1DataBuf, "adspbin", ADSP_UNIT_NAME_LEN) != 0)
    {
        //PartitionID wrong/Data corrupted/Not exist
        LOG(0, "Error: ADSP bin is corrupted or doesn't exist, partition %d\n", u4PartitionId);
        return -1;
    }
...
UINT32 u4GetDspImgAddr_new(UINT32 u4Type)
{
  static BOOL    fgAlignedImage = FALSE;
  UINT32 u4ImageAddr;
  UINT32 u4Unaligned;
...
static BOOL _fgDspIntHistoryInit = FALSE;
void vLogDSPIntHistory(UINT32 u4Addr, UINT32 u4ShortD, UINT32 u4LongD, BOOL fgwait)
{
   if(!_fgDspIntHistoryInit)
   	{
   	    _rDspIntHistory.u4Index=0;
		_fgDspIntHistoryInit= TRUE;
   	}

   _rDspIntHistory.u4Addr[_rDspIntHistory.u4Index]  = u4Addr;
   _rDspIntHistory.u4LongD[_rDspIntHistory.u4Index] = u4LongD;
   _rDspIntHistory.u4ShortD[_rDspIntHistory.u4Index]= u4ShortD;
   _rDspIntHistory.fgwait[_rDspIntHistory.u4Index]  = fgwait;
   _rDspIntHistory.u4Index = (_rDspIntHistory.u4Index +1 )%DSP_INT_HIS_QUEUE_LENGTH;
...
*/
    ...
    //if (_rDspStatus[AUD_DEC_AUX].fgDspPlay) //when do ATV scan, if the 2nd decoder is still played, then it must be detector
    {
        ...
        ReadDspDec2CacheDram(0x810,u4LogVar5);
        ReadDspDec2CacheDram(0x811,u4LogVar6);
        ReadDspDec2CacheDram(0x812,u4LogVar7);
        ReadDspDec2CacheDram(0x813,u4LogVar8);
        ReadDspDec2CacheDram(0x804,u4LogVar9);
        ReadDspDec2CacheDram(0x806,u4LogVar10);
    }
    else if (u1DecId == AUD_DEC_THIRD)
    {
        ReadDspDec3CacheDram(0x800,u4Energy45M);
        ReadDspDec3CacheDram(0x801,u4Energy55M);
        ReadDspDec3CacheDram(0x802,u4Energy60M);
        ReadDspDec3CacheDram(0x803,u4Energy65M);
        ReadDspDec3CacheDram(0x805,u4Energy655M);

        ReadDspDec3CacheDram(0x814,u4LogVar1);
        ReadDspDec3CacheDram(0x815,u4LogVar2);
        ReadDspDec3CacheDram(0x816,u4LogVar3);
        ReadDspDec3CacheDram(0x817,u4LogVar4);

        ReadDspDec3CacheDram(0x810,u4LogVar5);
        ReadDspDec3CacheDram(0x811,u4LogVar6);
        ReadDspDec3CacheDram(0x812,u4LogVar7);
        ReadDspDec3CacheDram(0x813,u4LogVar8);
        ReadDspDec3CacheDram(0x804,u4LogVar9);
        ReadDspDec3CacheDram(0x806,u4LogVar10);
	}
	else
	{
        ...
        ReadDspDecCacheDram(0x810,u4LogVar5);
        ReadDspDecCacheDram(0x811,u4LogVar6);
        ReadDspDecCacheDram(0x812,u4LogVar7);
        ReadDspDecCacheDram(0x813,u4LogVar8);
        ReadDspDecCacheDram(0x804,u4LogVar9);
        ReadDspDecCacheDram(0x806,u4LogVar10);
	}
...
#ifdef CC_AUD_OVM_FORCE_MONO_SUPPORT
    if ((_arAudDecOvmMode == AUD_DEC_OVM_HIGH) && (_rDspVars[u1DecId].bDspStrTyp != FMRDO_STREAM))
    {
...
#ifdef CC_AUD_OVM_FORCE_MONO_SUPPORT
    if ((_arAudDecOvmMode == AUD_DEC_OVM_HIGH) && (_rDspVars[AUD_DEC_AUX].bDspStrTyp != FMRDO_STREAM))
    {
        if ((eTvSys == SV_A2_DK1) || (eTvSys == SV_A2_DK2) || (eTvSys == SV_A2_DK3))
        {
...
void vDspFlowControlNotify(UINT32 u4DspRIntData)
{
    ...
    case D2RC_FLOW_CONTROL_AOUT_ENABLE:
                ...
                )
             {
                  ...
                  Aud_AddLogDump("## For AV sync , UN-MUTE Audio !!\n", 0);
             }
             else
             {
                  LOG(7,"[SRM_AVSYNC] D2RC_FLOW_CONTROL_AOUT_ENABLE, without UNMUTE \n");
                  Aud_AddLogDump("AOUT_ENABLE, without UNMUTE\n", 0);
             }
#endif

#ifdef CC_AUD_ENABLE_PLAY_MUTE_CONTROL
        VERIFY(AUD_PlayMuteCmd(AUD_PM_CMD_UNMUTE_WITH_VDP, 0));
#endif
            ...
            if (pfAudDecNfy != NULL)
            {
                ...
                // CANAL+ / CANAL+ back to ATV mode
                {
                        ...
                        (_rDspVars[AUD_DEC_AUX].bDspStrTyp == NTSC_STREAM))
                    {
                        ...
                        if (_rDspVars[AUD_DEC_AUX].bDspStrTyp == NTSC_STREAM)
                        {
...
#endif
        else if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp == APE_STREAM)
        {
        	if (pfAudDecNfy != NULL)
        	{
        ...
        else if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp == WMAPRO_STREAM)
        {
         ...
         else if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp == FLAC_STREAM)//paul_flac
		{
			if (NULL != pfAudDecNfy)
			{
...
#endif
        else if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp == AMR_STREAM)
        {
            if (pfAudDecNfy != NULL)
            {
                UNUSED(_GetVorbisAudInfo(AUD_DEC_MAIN, &rAudDecInfo));
        ...
        else if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp == AWB_STREAM)
        {
            if (pfAudDecNfy != NULL)
            {
                UNUSED(_GetVorbisAudInfo(AUD_DEC_MAIN, &rAudDecInfo));
        ...
        else if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp == G726_STREAM)
        {
            if (pfAudDecNfy != NULL)
            {
                UNUSED(_GetVorbisAudInfo(AUD_DEC_MAIN, &rAudDecInfo));
    ...
    case D2RC_FLOW_CONTROL_AOUT_ENABLE_DEC2:
        ...
        else if (_rDspVars[AUD_DEC_AUX].bDspStrTyp == WMAPRO_STREAM)
        {
        	if (pfAudDecNfy != NULL)
        	{
                    rAudDecInfo.e_aud_fmt = AUD_FMT_WMAPRO;
...
#if 1///def CC_VORBIS_SUPPORT
        else if (_rDspVars[AUD_DEC_AUX].bDspStrTyp == VORBIS_STREAM)
        {
            if (pfAudDecNfy != NULL)
            {
    ...
    case D2RC_FLOW_CONTROL_AOUT_ENABLE_DEC3:
        ...
        else if (_rDspVars[AUD_DEC_THIRD].bDspStrTyp == WMAPRO_STREAM)
        {
        	if (pfAudDecNfy != NULL)
        	{
                    rAudDecInfo.e_aud_fmt = AUD_FMT_WMAPRO;
...
#if 1///def CC_VORBIS_SUPPORT
        else if (_rDspVars[AUD_DEC_THIRD].bDspStrTyp == VORBIS_STREAM)
        {
            if (pfAudDecNfy != NULL)
            {
...
UINT32 DSP_GetAtvTvSystemStrength(TV_AUD_SYS_T eAudSys)
{
    ...
    case SV_NTSC_M:
#ifdef CC_AUD_DDI
		u4Noise45 = u4ReadDspSram32(0x3638);
		u4Noise55 = u4ReadDspSram32(0x3639);
		u4Noise60 = u4ReadDspSram32(0x363a);
		u4Noise65 = u4ReadDspSram32(0x363b);
...
UINT32 DSP_Dec3DramRead(UINT32 u4Address)
{
    UINT32 u4DramValue;
    ReadDspDec3CacheDram(u4Address, u4DramValue);
    return u4DramValue;
}
...
INT32 DSP_ResetPts(UINT8 u1DecId)
{
	AUD_DEC_ID_VALIDATE(u1DecId);

    return i4DspResetPts(u1DecId);
}
...
void vDspDemodDetModeNotify(UINT32 u4IntGroup , UINT32 u4IntVector)
{
	...
	if (_rDspVars[AUD_DEC_MAIN].bDspStrTyp != FMRDO_STREAM)
	{
		...
		_aeAudType[AUD_DEC_THIRD] = rAudDecInfo.e_aud_type;
	}
...
void DSP_SetTvSrcMode (TV_AUD_SYS_T eAudSys)
{
	...
	DSP_UpdateDemodMode (u2ReadDspShmWORD (W_USER_MODE), u2SrcMode,u2ReadDspShmWORD (W_ADC_GAIN));
}
...
UINT8 DSP_GetAcmod(UINT8 u1DecId)
{
   ...
   {
   case AUD_DEC_AUX:
        u1Acmode = (dReadDspCommDram(ADDR_D2RC_AC3_ACMOD_DEC2)>>8);
        break;
   case AUD_DEC_THIRD:
        u1Acmode = (dReadDspCommDram(ADDR_D2RC_AC3_ACMOD_DEC3)>>8);
        break;
    case AUD_DEC_MAIN:
    default:
        u1Acmode = (dReadDspCommDram(ADDR_D2RC_AC3_ACMOD)>>8);
        break;
   }
...
UINT8 DSP_GetAc3DecType(UINT8 u1DecId)
{
   ...
   {
   case AUD_DEC_AUX:
        u1Ac3DecType = (dReadDspCommDram(ADDR_D2RC_AC3_DECTYPE_DEC2)>>8);
        break;
   case AUD_DEC_THIRD:
        u1Ac3DecType = (dReadDspCommDram(ADDR_D2RC_AC3_DECTYPE_DEC3)>>8);
        break;
    case AUD_DEC_MAIN:
    default:
        u1Ac3DecType = (dReadDspCommDram(ADDR_D2RC_AC3_DECTYPE)>>8);
        break;
   }
...
void DSP_MixSndDec3Mode(BOOL fgEnable)
{
    ...
    u2Data = u2ReadShmUINT16(W_SYSTEM_CTRL2);
	u4InputFlag = u4ReadDspSram(SRAM_INTBUF_INPUT_FLAG);
	u4MixsndStatus = u4ReadDspSram(SRAM_MIXSOUND_STATUS);
	u4InputFlag >>= 8;
	u4MixsndStatus >>= 8;
    if (fgEnable)
    {
    ...
    vWriteShmUINT16(W_SYSTEM_CTRL2, u2Data);
	vWriteDspSram(SRAM_INTBUF_INPUT_FLAG,u4InputFlag<<8);
	vWriteDspSram(SRAM_MIXSOUND_STATUS,u4MixsndStatus<<8);
    DSP_SendDspTaskCmd(UOP_DSP_MIXSOUND_DEC3);
}
...
UINT32 DSP_GetDataRate(UINT8 u1DecId)    //Andrew : For enhance DD Banner
{
    ...
    {
    case AUD_DEC_AUX:
        u4DataRate = dReadDspCommDram(ADDR_D2RC_RISC_INFO_BIT_RATE_DEC2);
        break;
    case AUD_DEC_THIRD:
        u4DataRate = dReadDspCommDram(ADDR_D2RC_RISC_INFO_BIT_RATE_DEC3);
        break;
    case AUD_DEC_MAIN:
    default:
    u4DataRate = dReadDspCommDram(ADDR_D2RC_RISC_INFO_BIT_RATE);
        break;
    }
...
// Return    : void
// *********************************************************************
void DSP_SetPcmChNum(UINT8 u1DecId, UINT8 u1ChNum)
{
    UINT32 u4Address = 0;
    UINT16 u2ChNum = 0;
    ...
    vWriteShmUINT16(u4Address, u2ChNum);
}
#endif
...
{
    if (!_rDspStatus[u1DecId].fgDspStopIssue && _rDspStatus[u1DecId].fgDspStop)	//stop without issue DSP_STOP command && DSP already flush, it's WMA error
    {
        if (u1DecId == AUD_DEC_MAIN)
        {
        ...
        {
            return( (dReadDspCommDram(ADDR_D2RC_WMA_ERROR_MODE_DEC2)>>8)+1);	//shift error code by 1 because there is error code=0
        }
        else
        {
            return( (dReadDspCommDram(ADDR_D2RC_WMA_ERROR_MODE_DEC3)>>8)+1);	//shift error code by 1 because there is error code=0
        }
    }
    else        //normal stop
        return(0);
...
UINT32 DSP_GetWmaFrameSize(void)
{
    ...
    if (_rDspStatus[AUD_DEC_MAIN].fgDspRealPlay || _rDspStatus[AUD_DEC_AUX].fgDspRealPlay || _rDspStatus[AUD_DEC_THIRD].fgDspRealPlay)
    {
...
UINT32 DSP_GetAPEErrorCode(void)
{
    ...
    */
    if (!_rDspStatus[AUD_DEC_MAIN].fgDspStopIssue && _rDspStatus[AUD_DEC_MAIN].fgDspStop)	//stop without issue DSP_STOP command && DSP already flush, it's WMA error
        return( (dReadDspCommDram(ADDR_D2RC_APE_ERROR_MODE)>>8));
    else        //normal stop
        return(0);
}
#endif
...
UINT32 DSP_GetVorbisErrorCode(UINT8 u1DecId)
{
    ...
    if (!_rDspStatus[u1DecId].fgDspStopIssue)
    {
...
#endif
        {
            ...
            {
                ///LOG(0,"########### Address = 0x%x. ################\n.", u4Temp1);
                u4Temp2 += (DSP_Dec2DramRead(u4Temp1)>>8);
            }
            return(u4Temp2);   /// +1 for decode have stop.
        }
#ifdef CC_MT5391_AUD_3_DECODER
        else
        {
            ...
            {
                ///LOG(0,"########### Address = 0x%x. ################\n.", u4Temp1);
                u4Temp2 += (DSP_Dec3DramRead(u4Temp1)>>8);
            }
            return(u4Temp2);   /// +1 for decode have stop.
        }
#endif
...
UINT8 u1GetAacChNum(UINT8 u1DecId)
{
    ...
    if (u1DecId == AUD_DEC_MAIN)
    {
        u1Mode = (UINT8)(dReadDspCommDram(ADDR_D2RC_AAC_CH_MODE) >> 8);
    }
    else if (u1DecId == AUD_DEC_AUX)
    {
    ...
    {
        u1Mode = (UINT8)(dReadDspCommDram(ADDR_D2RC_AAC_CH_MODE_DEC3) >> 8);
    }
    else
    {
        u1Mode = 0xff;
...
UINT8 u1GetAacVersion(UINT8 u1DecId)
{
    ...
    {
        return((UINT8)(dReadDspCommDram(ADDR_D2RC_AAC_VERSION_DEC3) >> 8));
    }
    else
    {
        return 0xff;
...
{
	if(dReadDspCommDram(AADR_D2RC_RISC_INFO_INPUT_ISSIF_EXIST))
    {
    	return TRUE;
    }
	else
...
UINT8 u1GetAacTSFormat(UINT8 u1DecId)
{
    ...
    {
	return((UINT8)(dReadDspCommDram(ADDR_D2RC_AAC_TRANS_FORMAT_DEC3) >> 8));
    }
    else
    {
        return 0xff;
...
void DspVorbisCodeBookResult (BOOL fgStatus)
{
    if (fgStatus == TRUE)
    {
        ...
        {
            _u4DspSIntSD = (UINT32)VORBIS_CODEBOOK_READY_DEC3;
        }
    }
    else
    {
        ...
        if ((_uVorbisIntDec == 3) && (_rDspStatus[AUD_DEC_THIRD].fgDspPlay) && (_rDspVars[AUD_DEC_THIRD].bDspStrTyp == VORBIS_STREAM))
        {
...
#define AUD_APROC_ALIGN_BYTE 4096
// Get Audio processor work space offset from ADSP
UINT32 u4ApolloGetWorkSpaceOffset (void)
{
    return DSP_APROC_CODE_SA;
}

// Get Audio processor work space size
UINT32 u4ApolloGetWorkSpaceSize (void)
{
    return DSP_APROC_CODE_SZ;
}
// Audio Apollo init
void vApolloMemInit (UINT8* puWorkingBuffer)
{
    UINT32 u4Addr, u4Align;
    UINT32 u4DramAdr = PHYSICAL((UINT32)(puWorkingBuffer));

    u4Addr = u4DramAdr + u4ApolloGetWorkSpaceOffset();
    ...
    _gu4ApolloMemMap[0] = u4Addr;
    _gu4ApolloMemMap[1] = u4Addr+DSP_APROC_CODE_SZ;
    //Note that for Apollo, the physical base address must be 4kB alignment
    // Check 256 Bytes alignment
    VERIFY(_AudAlign(VIRTUAL(u4Addr), AUD_APROC_ALIGN_BYTE) == VIRTUAL(u4Addr));

    //to do
#if 1

    _gu4AprocMemMap[APROC_MEMMAP_WORKBUF_PHY] = _gu4ApolloMemMap[1];
    u4Addr = VIRTUAL(_gu4ApolloMemMap[1]);
    _gu4AprocMemMap[APROC_MEMMAP_WORKBUF] = u4Addr;

    // init register map
    _gu4AprocMemMap[APROC_MEMMAP_REGMAP] = (UINT32)(u4Addr + (UINT32)(APOLLO_SHARE_MEMORY_REG_SADR-APOLLO_EXT_MEM_SADR));
...
// Audio ARM load code
UINT32 u4AudApolloLoadCode (UINT32 u4Base, UINT32 u4TblAddr, UINT32 u4Size)
{
#if 0 // no MID
    UINT8* puDest=(UINT8*)VIRTUAL(u4Base);
    UINT8* puSrc=(UINT8*)VIRTUAL((UINT32)&_pbA[0]);
    UINT32 u4Index=0, i;
    UINT32 u4Len = _Apollo_GetBinarySize();
    //BOOL fgPreviousPause;
    UINT32 u4Mask;
    ...
    DSP_FlushInvalidateDCacheFree(u4Base, (u4Len*64));
#else
    vFlash2DramDma (u4Base, u4TblAddr,  u4Size);
#endif
...
void DSP_GetLpcmStatus(UINT8 u1DecId) //LPCM_DBG
{
    ...
    LOG(0, "\n== LPCM Info of DEC%d==\n", u1DecId);
    u4Tmp = dReadDspCommDram(u4Addr1)>>8;
    LOG(0, " Channel = %d ", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr1+1)>>8;
    LOG(0, " Freq = 0x%x \n", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr1+2)>>8;
    LOG(0, " Bit_Shift = %d ", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr1+3)>>8;
    LOG(0, " DRC_Value = 0x%x \n", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr1+4)>>8;
    LOG(0, " Q = %d ", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr1+5)>>8;
    LOG(0, " DRC_Flag = 0x%x ", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr1+6)>>8;
    LOG(0, " Type = 0x%x \n", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr2)>>8;
    LOG(0, " ADPCM_Block_Align = %d ", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr2+1)>>8;
    LOG(0, " ADPCM_Channel = %d \n", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr2+2)>>8;
    LOG(0, " ADPCM_Type = 0x%x ", u4Tmp);
    u4Tmp = dReadDspCommDram(u4Addr2+3)>>8;
...
void DSP_SetEncMode(UINT8 u1DecId, AUD_ENC_MODE_T eEncMode)
{
    ...
    u2SystemCtrl2 = u2ReadDspShmWORD(W_SYSTEM_CTRL2) & (~SHM_ENC_INPUT_JOB_SEL_MASKING);

    u2SystemCtrl2 = u2SystemCtrl2 | ((u1DecId + 1) << 10);

    vWriteDspShmWORD (W_SYSTEM_CTRL2, u2SystemCtrl2);

    // Set intbuf4 input flag
    vWriteDspSram(SRAM_INTBUF4_INPUT_FLAG, (UINT16)(eEncMode<<8));
}

void DSP_GetEncMode(UINT8 *u1DecId, AUD_ENC_MODE_T *eEncMode)
{
    // Get AV sync mode
    *u1DecId = (UINT8)(((u2ReadDspShmWORD(W_SYSTEM_CTRL2) & (SHM_ENC_INPUT_JOB_SEL_MASKING)) >> 10) - 1);
    *eEncMode = (AUD_ENC_MODE_T)(u4ReadDspSram (SRAM_INTBUF4_INPUT_FLAG) >> 8);
    return;
}
...
{
    UINT32 u4_stc_adjust_flag = 0x90b;
    UINT32 u4_stc_adjust_frame = 0x90d;
    ...
    if (u4FrameNum > 10) //Skip too many frame is abnormal
    {
        u4FrameNum = 10;
    }
    ...
    {
    case AUD_DEC_MAIN:
        u4_stc_adjust_flag = 0x90b;
        u4_stc_adjust_frame = 0x90d;
        break;
    case AUD_DEC_AUX:
        u4_stc_adjust_flag = 0x368b;
        u4_stc_adjust_frame = 0x368d;
        break;
    case AUD_DEC_THIRD:
        u4_stc_adjust_flag = 0x98b;
        u4_stc_adjust_frame = 0x98d;
        break;
    }

    vWriteDspSram(u4_stc_adjust_flag, 0x900);            //stc_adjust_flag, bit3 and bit0, Force Skip
    vWriteDspSram(u4_stc_adjust_frame, u4FrameNum<<8);  //stc_adjust_frame, align @ bit16
