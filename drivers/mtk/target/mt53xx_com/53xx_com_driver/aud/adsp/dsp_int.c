...
void vDspSetSamplingRate(UINT8 u1DecId)
{
    ...
    if (_rDspVars[u1DecId].bDspStrTyp == MPEG3_STREAM)
    {
    ...
    //Extract SFreq
     u4FreqIdx = u4DspGetSampleRate(u1DecId);
    ...
    if (((u1DecId == AUD_DEC_MAIN) || (u1DecId == AUD_DEC_AUX) || (u1DecId == AUD_DEC_THIRD)) && fgValidFreq)            // && _bDspStrTyp != SACD_STREAM)
    {
        ...
        // IEC RAW delay
        vSetIECRawDelay (_rDspVars[u1DecId].dwDacFreq,u1DecId);
    }
...
{
    //srs headroom
    WriteDspCommDram32bit (ADDR_SRS_HEADROOM, u4ReadDspShmDWRD (D_SRS_HEADROOM) << 16);
    WriteDspCommDram32bit (ADDR_SRS_HEADROOM + 1, u4ReadDspShmDWRD (D_SRS_COMPENSATE) << 16);
...
//================ MXM table ================
        ...
        {
            WriteDspCommDram ((DRAM_DRC2_COMMON_COEF_BASE + iLoopVar), pdwTable[iLoopVar]);
        }
        // 1 band AVC iir coefficients
        pdwTable = (UINT32 *) &drc2_1_band_tab[0];
        for (iLoopVar = 0; iLoopVar < 5; iLoopVar ++)
        {
            WriteDspCommDram ((DRAM_DRC2_IIR_COEF_AVC_BASE + iLoopVar), pdwTable[iLoopVar]);
        }
        // 2 band AVC iir coefficients
        pdwTable = (UINT32 *) &drc2_2_band_tab[0];
        for (iLoopVar = 0; iLoopVar < 2*5*2; iLoopVar ++)
        {
            WriteDspCommDram ((DRAM_DRC2_IIR_COEF_AVC_BAND0_BASE + iLoopVar), pdwTable[iLoopVar]);
        }
        // 2 band DRC iir coefficients
        pdwTable = (UINT32 *) &drc2_2_band_tab[0];
        for (iLoopVar = 0; iLoopVar < 2*5*2; iLoopVar ++)
        {
            WriteDspCommDram ((DRAM_DRC2_IIR_COEF_DRC_BAND0_BASE + iLoopVar), pdwTable[iLoopVar]);
        }
#endif
...
void vSetBassManageTable(UINT8 u1DecId)
{
    ...
    if (u1DecId == AUD_DEC_MAIN)
    {
        iCutFreq = u4ReadDspShmDWRD (D_CUTOFF_FREQ_LPF);
    }
    else
    {
        iCutFreq = u4ReadDspShmDWRD (D_CUTOFF_FREQ_LPF_DEC2);
    }
...
UINT32 VorbisReadDecDram(UINT32 u4Addr)
{
    ...
    {
        #ifdef FEW_DRAM_FLUSH
        u4Data = DspDec3CacheDramData(u4Addr);
        #else
        ReadDspDec3CacheDram(u4Addr, u4Data);
        #endif
    }
    else
    {
...
VOID VorbisWriteDecDram(UINT32 u4Addr, UINT32 u4Data)
{
    ...
    {
        #ifdef FEW_DRAM_FLUSH
        DspDec3CacheDramWriteData(u4Addr, u4Data);
        #else
        WriteDspDec3CacheDram(u4Addr, u4Data);
        #endif
    }
    else
    {
...
UINT32 VorbisReadDecDram32(UINT32 u4Addr)
{
    ...
    {
        #ifdef FEW_DRAM_FLUSH
        u4Data = DspDec3CacheDramData32(u4Addr);
        #else
        ReadDspDec3CacheDram32(u4Addr, u4Data);
        #endif
    }
    else
    {
...
VOID VorbisWriteDecDram32(UINT32 u4Addr, UINT32 u4Data)
{
    ...
    {
        #ifdef FEW_DRAM_FLUSH
        DspDec3CacheDramWriteData32(u4Addr, u4Data);
        #else
        WriteDspDec3CacheDram32(u4Addr, u4Data);
        #endif
    }
    else
    {
...
#ifdef CC_AUD_DDI
        u4ChDelay = (UINT32)(u2ReadDspShmWORD(W_CHDELAY_CH7) * u4Factor * 5) / 34; // samples
#else
