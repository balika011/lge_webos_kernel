...
void vDecCommonInit (void)
{
    ...
    u2BypassMode = uReadDspShmBYTE (B_DELAY_MODE_BYPASS);
	u2BypassMode = (u2BypassMode & uReadDspShmBYTE (B_DELAY_MODE));
    vWriteDspWORD (ADDR_RC2D_DELAY_MODE, u2BypassMode);
    ...
    //Reset DMX rpnt
	WriteDspCommDram(ADDR_D2RC_RISC_INFO_REG_BUF_PNT , _u4AFIFO[AUD_DEC_MAIN][0]);   //(AUD_READ32(REG_DMX_STR) << 2)
    ...
    case APE_STREAM:
        ...
        vWriteDspWORD (ADDR_RC2D_CTRL_3_NUM_1, (UINT16)CTRL_3_NUM_1);
        // 1
        vWriteDspWORD (ADDR_RC2D_APE_PARSE_AUDIO_ONLY,
                       u2ReadDspShmWORD (W_APE_PARSE_AUDIO_ONLY));
        // 2
        vWriteDspWORD (ADDR_RC2D_APE_FILE_VESION,
                       u2ReadDspShmWORD (W_APE_FILE_VESION));
        // 3
        WriteDspCommDram (ADDR_RC2D_APE_COMPRESS_LEVEL,
                       ((u4ReadDspShmDWRD (D_APE_COMPRESS_LEVEL))<<8));
        // 4
        WriteDspCommDram32bit(ADDR_RC2D_APE_BLOCK_PER_FRAME,
                       (u4ReadDspShmDWRD (D_APE_BLOCK_PER_FRAME)));
        // 5
        WriteDspCommDram (ADDR_RC2D_APE_FINAL_FRAME_BLOCK,
                       (u4ReadDspShmDWRD (D_APE_FINAL_FRAME_BLOCK)));
        // 6
        //notes: it should set (((high << 16)+low) & 0xFFFF) to ADDR_RC2D_APE_TOTAL_FRAME_NUM_LOW
        vWriteDspWORD (ADDR_RC2D_APE_TOTAL_FRAME_NUM_LOW,
                       u2ReadDspShmWORD (W_APE_TOTAL_FRAME_NUM_LOW));
        // 7
        vWriteDspWORD (ADDR_RC2D_APE_TOTAL_FRAME_NUM_HIGH,
                       u2ReadDspShmWORD (W_APE_TOTAL_FRAME_NUM_HIGH));
        // 8
        vWriteDspWORD (ADDR_RC2D_APE_BITS_PER_SAMPLE,
                       u2ReadDspShmWORD (W_APE_BITS_PER_SAMPLE));
        // 9
        vWriteDspWORD (ADDR_RC2D_APE_CHANNEL_NUM,
                       (u2ReadDspShmWORD (W_APE_CHANNEL_NUM) -1));
        // A
        WriteDspCommDram (ADDR_RC2D_APE_INPUT_SAMPLING_RATE,
                       u4ReadDspShmDWRD (D_APE_INPUT_SAMPLING_RATE));
        // B
        vWriteDspWORD (ADDR_RC2D_APE_MUTE_BANK_NUMBERS,
                       u2ReadDspShmWORD (W_APE_MUTE_BANK_NUMBERS));
        // C
        vWriteDspWORD (ADDR_RC2D_APE_INVALID_BYTES,
                       u2ReadDspShmWORD (W_APE_INVALID_BYTES));
        break;
#endif
    case WMAPRO_STREAM:
    ...
    case NTSC_STREAM:
        ...
        vWriteDspWORD (ADDR_RC2D_MTS_LR_AMP_M, 0x14b);
        vWriteDspWORD (ADDR_RC2D_MTS_LR_AMP_M_576K, 0x124);
        vWriteDspWORD (ADDR_RC2D_MTS_LR_AMP_N, 0x140);
        vWriteDspWORD (ADDR_RC2D_MTS_LR_AMP_N_576K, 0x15a);
#ifndef CC_AUD_WAFFLE_OVM_SUPPORT
        if ((uReadDspShmBYTE (B_HDEV_MODE) & 0x10) == 0x10)
        {
            vWriteDspWORD (ADDR_HDEV_MODE, 0x2);  // set bit 1 for 576k mode
        }
	 else
	 {
...
#else
        // 0 =288, 1=768, 2=576
        vWriteDspWORD (ADDR_HDEV_MODE, (uReadDspShmBYTE (B_HDEV_MODE) & NEW_MTS_MASK)>>NEW_MTS_SHIFT);
#endif      
...
#else
        vWriteDspWORD (ADDR_DEFAULT_MTS_MODE, 1);  // default mode: Stereo
#endif
        ...
        vWriteDspWORD (ADDR_MTS_SAP_MUTE_CTRL,uReadDspShmBYTE (B_MTS_SAP_MUTE_CTRL));
        vWriteDspWORD (ADDR_MTS_HDEV_LVL_REDUCE,uReadDspShmBYTE (B_MTS_HDEV_LVL_REDUCE));
        break;
    case FMFM_STREAM:
        vWriteDspWORD (ADDR_DEM_ADC_GAIN,u2ReadDspShmWORD (W_ADC_GAIN));
        vWriteDspWORD (ADDR_DEM_USER_MODE,u2ReadDspShmWORD (W_USER_MODE));
...
#else
        vWriteDspWORD (ADDR_HDEV_MODE, (uReadDspShmBYTE (B_HDEV_MODE) & NEW_FMFM_MASK)>>NEW_FMFM_SHIFT);
#endif
...
#else
        vWriteDspWORD (ADDR_HDEV_MODE, 0);  //A2 HDEV_MODE protection
#endif
    ...
    case PAL_STREAM:
...
#else
#ifdef CC_AUD_DDI
		vWriteDspWORD (ADDR_HDEV_MODE, ((uReadDspShmBYTE (B_HDEV_MODE) & 0x01) << 2) );  // set bit 2
#else
        vWriteDspWORD (ADDR_HDEV_MODE, (uReadDspShmBYTE (B_HDEV_MODE) & 0x01));
#endif
#endif
#else
        switch ((uReadDspShmBYTE (B_HDEV_MODE) & NEW_PAL_MASK)>>NEW_PAL_SHIFT)
        {
        case 0:
            vWriteDspWORD (ADDR_HDEV_MODE, 0x0);
            break;
        case 1:
            vWriteDspWORD (ADDR_HDEV_MODE, 0x1);
            break;
        case 2:
            vWriteDspWORD (ADDR_HDEV_MODE, 0x4);
            break;
        default:
            break;
        }
        ...
        if (((uReadDspShmBYTE (B_HDEV_MODE) & NEW_PAL_MASK)>>NEW_PAL_SHIFT)==2)
        {
            ...
            vWriteDspWORD(ADDR_FM_CARRIER_MUTE_THRESHOLD_HIGHT, uReadDspShmBYTE(B_FM_MUTE_THRESHOLD_HIGHT));
            vWriteDspWORD(ADDR_FM_CARRIER_MUTE_THRESHOLD_LOW,uReadDspShmBYTE(B_FM_MUTE_THRESHOLD_LOW));
        }
        else
        {
            vWriteDspWORD (ADDR_FM_CARRIER_MUTE_THRESHOLD_HIGHT, uReadDspShmBYTE (B_FM_MUTE_THRESHOLD_HIGHT_PAL));
    ...
    case DETECTOR_STREAM:
        ...
        vWriteDspWORD (ADDR_DETECTOR_A2_PILOT_CHK, uReadDspShmBYTE(B_DETECTOR_A2_PILOT_CHK));
        break;
    case FMRDODET_STREAM:
        ...
        break;
    case FMRDO_STREAM:
        vWriteDspWORD (ADDR_MTS_NUM_CHECK, 0);   //from shawn
...
//#ifndef CC_AUD_SUPPORT_MS10
    ...
    case VORBIS_STREAM:
     ...
     case FLAC_STREAM://paul_flac
		WriteDspCommDram(ADDR_RC2D_FLAC_MIN_BLK_SZ, u2ReadDspShmWORD(W_FLAC_MIN_BLK_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_MAX_BLK_SZ, u2ReadDspShmWORD(W_FLAC_MAX_BLK_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_CH_NUM_1, u2ReadDspShmWORD(W_FLAC_CHN_NUM));
		WriteDspCommDram(ADDR_RC2D_FLAC_BPS_1, u2ReadDspShmWORD(W_FLAC_BPS));
		WriteDspCommDram(ADDR_RC2D_FLAC_MIN_FRM_SZ, u4ReadDspShmDWRD(D_FLAC_MIN_FRM_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_MAX_FRM_SZ, u4ReadDspShmDWRD(D_FLAC_MAX_FRM_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_SAMP_RATE, u4ReadDspShmDWRD(D_FLAC_SMP_RATE));
        break;
     case G726_STREAM:
		WriteDspCommDram(ADDR_RC2D_G726_FRAME_SZ, u2ReadDspShmWORD(W_G726_FRAME)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_CH_NUM, uReadDspShmBYTE(B_G726_CH)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_RATE, uReadDspShmBYTE(B_G726_RATE)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_LAW, uReadDspShmBYTE(B_G726_LAW)<<8);
        WriteDspCommDram(ADDR_RC2D_G726_SRATE, uReadDspShmBYTE(B_G726_SRATE)<<8);
        break;
    default:
        break;
    }
...
void vDecCommonInitDec2 (void)
{
    ...
    case A2_STREAM:
        ...
        vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME,u4ReadDspShmDWRD  (D_A2_FINE_TUNE_VOLUME)>>8);
#else
        vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME,AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_A2_GAIN, eAudNvmAtvFmt)>>8);
#endif
        vWriteDspWORD (ADDR_MANUAL_MTS_SW, uReadDspShmBYTE (B_MANUAL_MTS_SW));
        //vWriteDspWORD (ADDR_MANUAL_OUTPUT_MATRIX, uReadDspShmBYTE (B_MANUAL_OUTPUT_MATRIX));
        vWriteDspWORD (ADDR_FM_NON_EU_MODE,uReadDspShmBYTE  (B_NON_EU_FM_MODE));
...
#else
        vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME_DEC2,AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_A2_GAIN, eAudNvmAtvFmt)>>8);
#endif
        if (u2ChannelSrcSet == SOURCE_NTSC_M)
        {
            vWriteDspWORD (ADDR_A2_STEREO_MONO_THRES, u2ReadDspShmWORD(W_A2_STEREO_MONO_THRES_M));
    ...
    case PAL_STREAM:
        ...
        vWriteDspWORD (ADDR_DEM_AM_FINE_TUNE_VOLUME,u4ReadDspShmDWRD  (D_AM_FINE_TUNE_VOLUME)>>8);
#else
         vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME, AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_PAL_GAIN, eAudNvmAtvFmt)>>8);
         vWriteDspWORD (ADDR_DEM_NICAM_FINE_TUNE_VOLUME, AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_NICAM_GAIN, eAudNvmAtvFmt)>>8);
         vWriteDspWORD (ADDR_DEM_AM_FINE_TUNE_VOLUME, AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_AM_GAIN, eAudNvmAtvFmt)>>8);
#endif
...
#else
        vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME_DEC2, AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_PAL_GAIN, eAudNvmAtvFmt)>>8);
        vWriteDspWORD (ADDR_DEM_NICAM_FINE_TUNE_VOLUME_DEC2, AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_NICAM_GAIN, eAudNvmAtvFmt)>>8);
        vWriteDspWORD (ADDR_DEM_AM_FINE_TUNE_VOLUME_DEC2, AUD_NVM_QRY_ATV_GAIN(AUD_NVM_QRY_AM_GAIN, eAudNvmAtvFmt)>>8);
#endif
    ...
    case DETECTOR_STREAM:
        ...
        vWriteDspWORD (ADDR_DETECTOR_A2_PILOT_CHK_DEC2, uReadDspShmBYTE(B_DETECTOR_A2_PILOT_CHK));
        break;
#endif
...
//#ifndef CC_AUD_SUPPORT_MS10
...
#else
	  	vWriteDspWORD (ADDR_RC2D_COOK_SAMP_PER_FRAME_DEC2,
		  	u2ReadDspShmWORD(W_COOK_SAMP_PER_FRAME));
	  	vWriteDspWORD (ADDR_RC2D_COOK_CHANNEL_NUM_DEC2,
		  	u2ReadDspShmWORD(W_COOK_CHANNEL_NUM));
	  	vWriteDspWORD (ADDR_RC2D_COOK_INPUT_SAMPLING_RATE_DEC2,
		  	u2ReadDspShmWORD(W_COOK_INPUT_SAMPLING_RATE));
	  	vWriteDspWORD (ADDR_RC2D_COOK_FRM_SZ_IN_BYTES_DEC2,
		  	u2ReadDspShmWORD(W_COOK_FRM_SZ_IN_BYTES));
	  	vWriteDspWORD (ADDR_RC2D_COOK_REGION_NUM_DEC2,
		  	u2ReadDspShmWORD(W_COOK_REGION_NUM));
	  	vWriteDspWORD (ADDR_RC2D_COOK_COUPLING_START_REGN_DEC2,
		  	u2ReadDspShmWORD(W_COOK_COUPLING_START_REGN));
	  	vWriteDspWORD (ADDR_RC2D_COOK_COUPLING_QBITS_DEC2,
		  	u2ReadDspShmWORD(W_COOK_COUPLING_QBITS));
#endif
	  break;
    ...
    case VORBIS_STREAM:
        ...
        case WMA_STREAM:
            ...
            vWriteDspWORD (ADDR_RC2D_CTRL_5_NUM_1, (UINT16)0xB);
            //WNA Packet No
            vWriteDspWORD (ADDR_RC2D_WMA_PACKET_NO_DEC2,
                           u2ReadDspShmWORD (W_WMA_PACKET_NO_DEC2));
            vWriteDspWORD (ADDR_RC2D_WMA_HEADER_SOURCE_DEC2,
                           u2ReadDspShmWORD (W_WMA_HEADER_SOURCE_DEC2));
            WriteDspCommDram (ADDR_RC2D_WMA_PACKET_COUNT_DEC2,
                           ((u4ReadDspShmDWRD (D_WMA_PACKET_COUNT_DEC2))<<8));
            WriteDspCommDram (ADDR_RC2D_WMA_PACKET_SIZE_DEC2,
                           ((u4ReadDspShmDWRD (D_WMA_PACKET_SIZE_DEC2))<<8));
            vWriteDspWORD (ADDR_RC2D_WMA_VERSION_DEC2,
                           u2ReadDspShmWORD (W_WMA_VERSION_DEC2));
            vWriteDspWORD (ADDR_RC2D_WMA_NUM_CHANNELS_DEC2,
                           u2ReadDspShmWORD (W_WMA_NUM_CH_DEC2));
            WriteDspCommDram (ADDR_RC2D_WMA_SAMPLE_PER_SECOND_DEC2,
                           ((u4ReadDspShmDWRD (D_WMA_SAMPLE_PER_SEC_DEC2))<<8));
            WriteDspCommDram (ADDR_RC2D_WMA_BYTES_PER_SECOND_DEC2,
                           ((u4ReadDspShmDWRD (D_WMA_BYTE_PER_SEC_DEC2))<<8));
            WriteDspCommDram (ADDR_RC2D_WMA_BLOCK_SIZE_DEC2,
                           ((u4ReadDspShmDWRD (D_WMA_BLOCK_SIZE_DEC2))<<8));
            vWriteDspWORD (ADDR_RC2D_WMA_ENCODER_OPTIONS_DEC2,
                           u2ReadDspShmWORD (W_WMA_ENCODER_OPTION_DEC2));
            //koro : if issue play without setting audio format (seek mode), wma need initial table
            vLoadDspDRAMTable (AUD_WMA_DEC2);
    ...
    case WMAPRO_STREAM:
        ...
        vWriteDspWORD (ADDR_RC2D_CTRL_5_NUM_1, (UINT16)0xC);
        //WNA Packet No
        vWriteDspWORD (ADDR_RC2D_WMAPRO_PACKET_NO_DEC2,
                       u2ReadDspShmWORD (W_WMAPRO_PACKET_NO_DEC2));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_HEADER_SOURCE_DEC2,
                       u2ReadDspShmWORD (W_WMAPRO_HEADER_SOURCE_DEC2));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_NUM_CHANNELS_DEC2,
                       u2ReadDspShmWORD (W_WMAPRO_NUM_CH_DEC2));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_SAMPLE_PER_SECOND_DEC2,
                       ((u4ReadDspShmDWRD (D_WMAPRO_SAMPLE_PER_SEC_DEC2))<<8));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_BYTES_PER_SECOND_DEC2,
                       ((u4ReadDspShmDWRD (D_WMAPRO_BYTE_PER_SEC_DEC2))<<8));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_BLOCK_SIZE_DEC2,
                       ((u4ReadDspShmDWRD (D_WMAPRO_BLOCK_SIZE_DEC2))<<8));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_ENCODER_OPTIONS_DEC2,
                       u2ReadDspShmWORD (W_WMAPRO_ENCODER_OPTION_DEC2));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_CH_MASK_DEC2,
                       ((u4ReadDspShmDWRD (D_WMAPRO_CH_MASK_DEC2))<<8));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_VALID_BITS_DEC2,
                       u2ReadDspShmWORD (W_WMAPRO_VALID_BITS_DEC2));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_ADV_OPTION_DEC2,
                       u2ReadDspShmWORD (W_WMAPRO_ADV_OPTION_DEC2));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_ADV_OPTION2_DEC2,
                       ((u4ReadDspShmDWRD (D_WMAPRO_ADV_OPTION2_DEC2))<<8));
    ...
    #else
        WriteDspCommDram(ADDR_RC2D_FLAC_MIN_BLK_SZ_DEC2, u2ReadDspShmWORD(W_FLAC_MIN_BLK_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_MAX_BLK_SZ_DEC2, u2ReadDspShmWORD(W_FLAC_MAX_BLK_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_CH_NUM_1_DEC2, u2ReadDspShmWORD(W_FLAC_CHN_NUM));
		WriteDspCommDram(ADDR_RC2D_FLAC_BPS_1_DEC2, u2ReadDspShmWORD(W_FLAC_BPS));
		WriteDspCommDram(ADDR_RC2D_FLAC_MIN_FRM_SZ_DEC2, u4ReadDspShmDWRD(D_FLAC_MIN_FRM_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_MAX_FRM_SZ_DEC2, u4ReadDspShmDWRD(D_FLAC_MAX_FRM_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_SAMP_RATE_DEC2, u4ReadDspShmDWRD(D_FLAC_SMP_RATE));
    #endif
        break;
    case G726_STREAM:
		WriteDspCommDram(ADDR_RC2D_G726_FRAME_SZ_DEC2, u2ReadDspShmWORD(W_G726_FRAME_DEC2)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_CH_NUM_DEC2, uReadDspShmBYTE(B_G726_CH_DEC2)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_RATE_DEC2, uReadDspShmBYTE(B_G726_RATE_DEC2)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_LAW_DEC2, uReadDspShmBYTE(B_G726_LAW_DEC2)<<8);
        WriteDspCommDram(ADDR_RC2D_G726_SRATE_DEC2, uReadDspShmBYTE(B_G726_SRATE_DEC2)<<8);
        break;
    default:
        break;
    }
...
//      vWriteDspWORD (ADDR_RC2D_MPEG_SYNC_MODE_DEC2, 0x1);
    ...
    case NTSC_STREAM:
        ...
        // Decoder 3
        vWriteDspWORD (ADDR_DEM_SOURCE_MODE_DEC3,u2ReadDspShmWORD (W_SOURCE_MODE));
        vWriteDspWORD (ADDR_DEM_USER_MODE_DEC3,u2ReadDspShmWORD (W_USER_MODE));
        vWriteDspWORD (ADDR_DEM_ADC_GAIN_DEC3,u2ReadDspShmWORD (W_ADC_GAIN));
        vWriteDspWORD (ADDR_DEM_SAP_FINE_TUNE_VOLUME_DEC3,u4ReadDspShmDWRD (D_SAP_FINE_TUNE_VOLUME)>>8);
        vWriteDspWORD (ADDR_DEM_MTS_FINE_TUNE_VOLUME_DEC3,u4ReadDspShmDWRD  (D_MTS_FINE_TUNE_VOLUME)>>8);
        break;
    case A2_STREAM:
        vWriteDspWORD (ADDR_DEM_ADC_GAIN,u2ReadDspShmWORD (W_ADC_GAIN));
        vWriteDspWORD (ADDR_DEM_USER_MODE, u2ReadDspShmWORD (W_USER_MODE));
        ...
        vWriteDspWORD (ADDR_FM_NON_EU_MODE,uReadDspShmBYTE  (B_NON_EU_FM_MODE));
        // Decoder 3
        vWriteDspWORD (ADDR_DEM_SOURCE_MODE_DEC3, u2ChannelSrcSet);
        vWriteDspWORD (ADDR_DEM_USER_MODE_DEC3, u2ReadDspShmWORD (W_USER_MODE));
        vWriteDspWORD (ADDR_DEM_ADC_GAIN_DEC3,u2ReadDspShmWORD (W_ADC_GAIN));
        vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME_DEC3,u4ReadDspShmDWRD  (D_A2_FINE_TUNE_VOLUME)>>8);
        if (u2ChannelSrcSet == SOURCE_NTSC_M)
        {
            vWriteDspWORD (ADDR_A2_STEREO_MONO_THRES, u2ReadDspShmWORD(W_A2_STEREO_MONO_THRES_M));
            vWriteDspWORD (ADDR_A2_MONO_STEREO_THRES, u2ReadDspShmWORD(W_A2_MONO_STEREO_THRES_M));
    ...
    case PAL_STREAM:
        ...
        vWriteDspWORD (ADDR_DEM_USER_MODE_DEC3,u2ReadDspShmWORD (W_USER_MODE));
        vWriteDspWORD (ADDR_DEM_ADC_GAIN_DEC3,u2ReadDspShmWORD (W_ADC_GAIN));
        vWriteDspWORD (ADDR_DEM_ENFORCED_MODE_DEC3,u2ReadDspShmWORD (W_ENFORCED_MODE));

        vWriteDspWORD (ADDR_DEM_PAL_FINE_TUNE_VOLUME_DEC3,u4ReadDspShmDWRD  (D_PAL_FINE_TUNE_VOLUME)>>8);
        vWriteDspWORD (ADDR_DEM_NICAM_FINE_TUNE_VOLUME_DEC3,u4ReadDspShmDWRD (D_NICAM_FINE_TUNE_VOLUME)>>8);
        vWriteDspWORD (ADDR_DEM_AM_FINE_TUNE_VOLUME_DEC3,u4ReadDspShmDWRD  (D_AM_FINE_TUNE_VOLUME)>>8);

        vWriteDspWORD (ADDR_SECAM_L_MUTE_MODE_DEC3,  uReadDspShmBYTE (B_AM_MUTE_MODE));
        vWriteDspWORD (ADDR_SECAM_L_MUTE_THRESHOLD_HIGHT_DEC3, uReadDspShmBYTE (B_AM_MUTE_THRESHOLD_HIGHT));
        vWriteDspWORD (ADDR_SECAM_L_MUTE_THRESHOLD_LOW_DEC3,uReadDspShmBYTE (B_AM_MUTE_THRESHOLD_LOW));
        break;

    case DETECTOR_STREAM:
        vWriteDspWORD (ADDR_DETECTOR_DEFAULT_MODE_DEC3, uReadDspShmBYTE(B_DETECT_DEFAULT));
        vWriteDspWORD (ADDR_DETECTOR_START_FLAG_DEC3, 1); // default turn on detection
        WriteDspCommDram (ADDR_DETECTOR_FM_NOISE_TH_DEC3, u4ReadDspShmDWRD (D_DETECTOR_FM_NOISE_TH));
        vWriteDspWORD (ADDR_DETECTOR_A2_PILOT_CHK_DEC3, uReadDspShmBYTE(B_DETECTOR_A2_PILOT_CHK));
        break;
#endif
...
//#ifndef CC_AUD_SUPPORT_MS10
    ...
    case DTSDEC_STREAM:
        ...
        {
        	//Big Endian
            vWriteDspWORD (ADDR_RC2D_DTS_DRC_FLAG_DEC3, 0xffd9);
        }
        else
        {
        	//Little Endian
        	vWriteDspWORD (ADDR_RC2D_DTS_DRC_FLAG_DEC3, 0xfffb);
        }
        AUD_SetDTSInfo(DEC_BIG_ENDIAN); //reset to Big Endian
        vWriteDspWORD (ADDR_RC2D_DTS_LFE_mixto_FRONT_DEC3, 0);
        vWriteDspWORD (ADDR_RC2D_DTS_DRC_ONOFF_DEC3,       9);
        vWriteDspWORD (ADDR_RC2D_DTS_DIALNORM_ONOFF_DEC3,  9);
        WriteDspCommDram (ADDR_RC2D_DTS_DRC_LOW_DEC3,      u4ReadDspShmDWRD (D_DTS_BC_DRC_LOW));
        WriteDspCommDram (ADDR_RC2D_DTS_DRC_MEDIUM_DEC3,   u4ReadDspShmDWRD (D_DTS_BC_DRC_MEDIUM));
        WriteDspCommDram (ADDR_RC2D_DTS_DRC_HIGH_DEC3,     u4ReadDspShmDWRD (D_DTS_BC_DRC_HIGH));
        WriteDspCommDram (ADDR_RC2D_DTS_DIALNORM_LOW_DEC3, u4ReadDspShmDWRD (D_DTS_BC_Dialnorm_LOW));
        WriteDspCommDram (ADDR_RC2D_DTS_DIALNORM_MEDIUM_DEC3, u4ReadDspShmDWRD (D_DTS_BC_Dialnorm_MEDIUM));
        WriteDspCommDram (ADDR_RC2D_DTS_DIALNORM_HIGH_DEC3, u4ReadDspShmDWRD (D_DTS_BC_Dialnorm_HIGH));
        break;
    case MPEG3_STREAM:
        // Normal play
        vWriteDspWORD (ADDR_RC2D_MP3_NORMAL_PLAY_DEC3, 0x00);
        // MP3 sync mode
        vWriteDspWORD (ADDR_RC2D_MP3_SYNC_MODE_DEC3, 0x01);
        // MP3 deemphasis mode
        vWriteDspWORD (ADDR_RC2D_MP3_DEEMPHASIS_MODE_DEC3, 0x00);
        // MP3 preparsing mode
        vWriteDspWORD (ADDR_RC2D_MP3_PREPARSING_MODE_DEC3, 0x00);
        break;
#if 1///def CC_VORBIS_SUPPORT
    case VORBIS_STREAM:
        vWriteDspWORD(ADDR_RC2D_VORBIS_PACKET_NO, u2ReadDspShmWORD(W_VORBISPACKET));
        vWriteDspWORD(ADDR_D2RC_VORBIS_TABLE_OK_DEC3, 0);
        vWriteDspWORD(ADDR_D2RC_VORBIS_ABS_GRANULE_POS_LL16_DEC3, 0);
        vWriteDspWORD(ADDR_D2RC_VORBIS_ABS_GRANULE_POS_LH16_DEC3, 0);
        vWriteDspWORD(ADDR_D2RC_VORBIS_ABS_GRANULE_POS_OK_DEC3, 0);
        /// Decode Dram is big enough
...
#else
  		vWriteDspWORD (ADDR_RC2D_COOK_SAMP_PER_FRAME_DEC3,
	  		u2ReadDspShmWORD(W_COOK_SAMP_PER_FRAME));
  		vWriteDspWORD (ADDR_RC2D_COOK_CHANNEL_NUM_DEC3,
	  		u2ReadDspShmWORD(W_COOK_CHANNEL_NUM));
  		vWriteDspWORD (ADDR_RC2D_COOK_INPUT_SAMPLING_RATE_DEC3,
	  		u2ReadDspShmWORD(W_COOK_INPUT_SAMPLING_RATE));
  		vWriteDspWORD (ADDR_RC2D_COOK_FRM_SZ_IN_BYTES_DEC3,
	  		u2ReadDspShmWORD(W_COOK_FRM_SZ_IN_BYTES));
  		vWriteDspWORD (ADDR_RC2D_COOK_REGION_NUM_DEC3,
	  		u2ReadDspShmWORD(W_COOK_REGION_NUM));
  		vWriteDspWORD (ADDR_RC2D_COOK_COUPLING_START_REGN_DEC3,
	  		u2ReadDspShmWORD(W_COOK_COUPLING_START_REGN));
  		vWriteDspWORD (ADDR_RC2D_COOK_COUPLING_QBITS_DEC3,
	  		u2ReadDspShmWORD(W_COOK_COUPLING_QBITS));
#endif
  		break;
    case LPCM_STREAM:
        u1Temp1 = (u2ReadDspShmWORD(W_LPCM_FREQUENCY_DEC3)>>8) & 0xFF;
        u1Temp2 = u2ReadDspShmWORD(W_LPCM_FREQUENCY_DEC3) & 0xFF;

        vWriteDspWORD(ADDR_RC2D_LPCM_CHANNEL_ASSIGNMENT_DEC3, uReadDspShmBYTE(B_LPCM_CH_ASSIGNMENT_DEC3));
        WriteDspCommDram(ADDR_RC2D_LPCM_FREQUENCY_DEC3,  (u1Temp1<<12)+u1Temp2);
        vWriteDspWORD(ADDR_RC2D_LPCM_BIT_SHIFT_DEC3,  uReadDspShmBYTE(B_LPCM_BIT_SHIFT_DEC3));
        vWriteDspWORD(ADDR_RC2D_LPCM_DRC_VALUE_DEC3,  uReadDspShmBYTE(B_LPCM_DRC_VALUE_DEC3));
        WriteDspCommDram(ADDR_RC2D_LPCM_Q_DEC3,  u2ReadDspShmWORD(W_LPCM_Q_VALUE_DEC3));
        vWriteDspWORD(ADDR_RC2D_LPCM_DRC_FLAG_DEC3,  uReadDspShmBYTE(B_LPCM_DRC_FLAG_DEC3));
        vWriteDspWORD(ADDR_RC2D_LPCM_DECODING_TYPE_DEC3,  uReadDspShmBYTE(W_LPCM_DECODING_TYPE_DEC3));

        if (uReadDspShmBYTE(W_ADPCM_DECODING_TYPE_DEC3)!=0) //ADPCM
        {
            vWriteDspWORD(ADDR_RC2D_LPCM_ADPCM_DEC_TYPE_DEC3, uReadDspShmBYTE(W_ADPCM_DECODING_TYPE_DEC3));
            vWriteDspWORD(ADDR_RC2D_LPCM_ADPCM_BITS_PER_SAMPLE_DEC3, uReadDspShmBYTE(B_ADPCM_BIT_PERSAMPLE_DEC3));
            vWriteDspWORD(ADDR_RC2D_ADPCM_BLOCK_ALIGN_DEC3, u2ReadDspShmWORD(W_ADPCM_BLOCK_ALIGN_DEC3));
            vWriteDspWORD(ADDR_RC2D_ADPCM_CHANNEL_NUM_DEC3, uReadDspShmBYTE(B_ADPCM_CHANNEL_NUM_DEC3));
        }
        break;
    ...
    case WMA_STREAM:
        //WNA Packet No
        vWriteDspWORD (ADDR_RC2D_WMA_PACKET_NO_DEC3,
                       u2ReadDspShmWORD (W_WMA_PACKET_NO_DEC3));
        vWriteDspWORD (ADDR_RC2D_WMA_HEADER_SOURCE_DEC3,
                       u2ReadDspShmWORD (W_WMA_HEADER_SOURCE_DEC3));
        WriteDspCommDram (ADDR_RC2D_WMA_PACKET_COUNT_DEC3,
                       ((u4ReadDspShmDWRD (D_WMA_PACKET_COUNT_DEC3))<<8));
        WriteDspCommDram (ADDR_RC2D_WMA_PACKET_SIZE_DEC3,
                       ((u4ReadDspShmDWRD (D_WMA_PACKET_SIZE_DEC3))<<8));
        vWriteDspWORD (ADDR_RC2D_WMA_VERSION_DEC3,
                       u2ReadDspShmWORD (W_WMA_VERSION_DEC3));
        vWriteDspWORD (ADDR_RC2D_WMA_NUM_CHANNELS_DEC3,
                       u2ReadDspShmWORD (W_WMA_NUM_CH_DEC3));
        WriteDspCommDram (ADDR_RC2D_WMA_SAMPLE_PER_SECOND_DEC3,
                       ((u4ReadDspShmDWRD (D_WMA_SAMPLE_PER_SEC_DEC3))<<8));
        WriteDspCommDram (ADDR_RC2D_WMA_BYTES_PER_SECOND_DEC3,
                       ((u4ReadDspShmDWRD (D_WMA_BYTE_PER_SEC_DEC3))<<8));
        WriteDspCommDram (ADDR_RC2D_WMA_BLOCK_SIZE_DEC3,
                       ((u4ReadDspShmDWRD (D_WMA_BLOCK_SIZE_DEC3))<<8));
        vWriteDspWORD (ADDR_RC2D_WMA_ENCODER_OPTIONS_DEC3,
                       u2ReadDspShmWORD (W_WMA_ENCODER_OPTION_DEC3));
        //koro : if issue play without setting audio format (seek mode), wma need initial table
        vLoadDspDRAMTable (AUD_WMA_DEC3);
    ...
    case WMAPRO_STREAM:
        ...
        vWriteDspWORD (ADDR_RC2D_CTRL_6_NUM_1, (UINT16)0xC);
        //WNA Packet No
        vWriteDspWORD (ADDR_RC2D_WMAPRO_PACKET_NO_DEC3,
                       u2ReadDspShmWORD (W_WMAPRO_PACKET_NO_DEC3));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_HEADER_SOURCE_DEC3,
                       u2ReadDspShmWORD (W_WMAPRO_HEADER_SOURCE_DEC3));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_NUM_CHANNELS_DEC3,
                       u2ReadDspShmWORD (W_WMAPRO_NUM_CH_DEC3));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_SAMPLE_PER_SECOND_DEC3,
                       ((u4ReadDspShmDWRD (D_WMAPRO_SAMPLE_PER_SEC_DEC3))<<8));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_BYTES_PER_SECOND_DEC3,
                       ((u4ReadDspShmDWRD (D_WMAPRO_BYTE_PER_SEC_DEC3))<<8));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_BLOCK_SIZE_DEC3,
                       ((u4ReadDspShmDWRD (D_WMAPRO_BLOCK_SIZE_DEC3))<<8));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_ENCODER_OPTIONS_DEC3,
                       u2ReadDspShmWORD (W_WMAPRO_ENCODER_OPTION_DEC3));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_CH_MASK_DEC3,
                       ((u4ReadDspShmDWRD (D_WMAPRO_CH_MASK_DEC3))<<8));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_VALID_BITS_DEC3,
                       u2ReadDspShmWORD (W_WMAPRO_VALID_BITS_DEC3));
        vWriteDspWORD (ADDR_RC2D_WMAPRO_ADV_OPTION_DEC3,
                       u2ReadDspShmWORD (W_WMAPRO_ADV_OPTION_DEC3));
        WriteDspCommDram (ADDR_RC2D_WMAPRO_ADV_OPTION2_DEC3,
                       ((u4ReadDspShmDWRD (D_WMAPRO_ADV_OPTION2_DEC3))<<8));
    ...
    #else
        WriteDspCommDram(ADDR_RC2D_FLAC_MIN_BLK_SZ_DEC3, u2ReadDspShmWORD(W_FLAC_MIN_BLK_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_MAX_BLK_SZ_DEC3, u2ReadDspShmWORD(W_FLAC_MAX_BLK_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_CH_NUM_1_DEC3, u2ReadDspShmWORD(W_FLAC_CHN_NUM));
		WriteDspCommDram(ADDR_RC2D_FLAC_BPS_1_DEC3, u2ReadDspShmWORD(W_FLAC_BPS));
		WriteDspCommDram(ADDR_RC2D_FLAC_MIN_FRM_SZ_DEC3, u4ReadDspShmDWRD(D_FLAC_MIN_FRM_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_MAX_FRM_SZ_DEC3, u4ReadDspShmDWRD(D_FLAC_MAX_FRM_SZ));
		WriteDspCommDram(ADDR_RC2D_FLAC_SAMP_RATE_DEC3, u4ReadDspShmDWRD(D_FLAC_SMP_RATE));
    #endif
        break;
    case G726_STREAM:
		WriteDspCommDram(ADDR_RC2D_G726_FRAME_SZ_DEC3, u2ReadDspShmWORD(W_G726_FRAME_DEC3)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_CH_NUM_DEC3, uReadDspShmBYTE(B_G726_CH_DEC3)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_RATE_DEC3, uReadDspShmBYTE(B_G726_RATE_DEC3)<<8);
		WriteDspCommDram(ADDR_RC2D_G726_LAW_DEC3, uReadDspShmBYTE(B_G726_LAW_DEC3)<<8);
        WriteDspCommDram(ADDR_RC2D_G726_SRATE_DEC3, uReadDspShmBYTE(B_G726_SRATE_DEC3)<<8);
        break;
    default:
        break;
    }
...
void vDecCommonInitDec4 (void)
{
	...
	case MP3ENC_STREAM:		///[Joel]20090327
		...
		WriteDspCommDram(ADDR_RC2D_ENCODER_OUTBUF_READ_PNT_DEC4, _tPbsBlk[0].wSblk << 8);
		WriteDspCommDram(ADDR_RC2D_ENCODER_OUTBUF_WRITE_PNT_DEC4, _tPbsBlk[0].wSblk << 8);
		WriteDspCommDram(ADDR_RC2D_ENCODER_OUTBUF_SIZE_DEC4, (_tPbsBlk[0].wEblk - _tPbsBlk[0].wSblk) << 8);

		WriteDspCommDram(ADDR_RC2D_ENCODER_SFREQ_DEC4, 0x0100);			///48KHz
		WriteDspCommDram(ADDR_RC2D_ENCODER_BITRATE_DEC4, (UINT16)MP3ENC_BR<<8);		///0x0E00, 320kbps / 0x0900, 128Kbps / 0x0B00, 192Jbps
		WriteDspCommDram(ADDR_RC2D_ENCODER_CHANNEL_MODE_DEC4, 0x0000);	///Stereo
		WriteDspCommDram(ADDR_RC2D_ENCODER_MPEG_FLAG_DEC4, 0x0000); /// Set padding bit to never for fix frame size.
...
void vDspShareInfoInit (void)
{
...
#endif
    // for debug use
    vWriteDspShmDWRD (D_SPKCFG, _u4DspGetSpkConfig() | 0x40); // full channel, set auto downmix by default (bit6)

    vWriteDspShmDWRD (D_SPKCFG_DEC2,     0x40); // LtRt, set auto downmix by default (bit6)
    vWriteDspShmDWRD (D_SPKCFG_DEC3,     0x40); // LtRt, set auto downmix by default (bit6)
    ...
    vWriteDspShmWORD (W_CHDELAY_DEC3,     0);
    ...
    vWriteDspShmWORD (W_PROCMOD,    IS_BYPASS_PROC() ? 0x2c00 : (0xc00 | (IS_SPKCFG_SSLN()<<7)) ); // ch9/10 downmix 0x2c00 yi.xiao for A2
#else
...
#ifdef CC_AUD_FY12_LOUDNESS
    vWriteDspShmDWRD(D_SBASSBOOSTGAIN2,  (BASS_TREBLE_ATTN_GAIN_MAX-1)/2); //0dB
    vWriteDspShmDWRD(D_SBASSCLEARGAIN2,  (BASS_TREBLE_ATTN_GAIN_MAX-1)/2);	//0dB
#endif
...
*/
	...
	vWriteDspShmBYTE (B_BANK576NUM, 	  9+13+22); // for mp3 modification
	vWriteDspShmBYTE (B_BANK384NUM,		  12+20+32);
	vWriteDspShmBYTE (B_BANK320NUM,		  14+24+38);// 320*B_BANK320NUM_DECx must be multiples of 256
	vWriteDspShmBYTE (B_BANK256NUM,		  18+30+48);//sunman for 500ms (48kHz) buffer
    ...
    vWriteDspShmBYTE (B_BANK320NUM_DEC2,    12);// 320*B_BANK320NUM_DECx must be multiples of 256
    ...
    vWriteDspShmBYTE (B_BANK320NUM_DEC3,    12);// 320*B_BANK320NUM_DECx must be multiples of 256
    vWriteDspShmBYTE (B_BANK256NUM_DEC3,    18);
    vWriteDspShmWORD (W_SPEED_DEC3,         0);
#endif
	...
	vWriteDspShmDWRD (D_UPLOAD_VOL,     CH_VOL_SHM_DEFAULT);
    ...
    vWriteDspShmDWRD (D_VOL_L,          CH_VOL_SHM_DEFAULT);
    vWriteDspShmDWRD (D_VOL_R,          CH_VOL_SHM_DEFAULT);

    //FOR DSP PAN FUNCTION INITIAL VALUES, CC_AUD_PANFADE_SUPPORT
    vWriteDspShmDWRD (D_FADEVALUE,      CH_VOL_SHM_DEFAULT);

    // Operation mode
    vWriteDspShmDWRD (W_AC3KARAMOD, 1); // 1: disable
    vWriteDspShmDWRD (W_AC3KARAMOD_DEC2, 1);
    vWriteDspShmDWRD (W_AC3KARAMOD_DEC3, 1);
    if (AUD_GetTargetCountry() == COUNTRY_EU)
    {
    ...
    vWriteDspShmDWRD (D_AC3DYN_LOW_DEC3, 0x007FFFFF); // 1dB
    vWriteDspShmDWRD (D_AC3DYN_HIGH_DEC3, 0x007FFFFF); // 1dB
    ...
    vWriteDspShmBYTE (B_DDT_CONTROL, 0); //bit0: bypass MDC/DDCO, bit1: output AAC RAW
#else
    #if defined(CC_AUD_DOLBY_SUPPORT_DDCO) || defined(CC_AUD_DOLBY_SUPPORT_DDT)
    //should not happen, set to 3
    vWriteDspShmBYTE (B_DDT_CONTROL, 3); //bit0: bypass MDC/DDCO, bit1: output AAC RAW
    #else
    vWriteDspShmBYTE (B_DDT_CONTROL, 2); //bit0: bypass MDC/DDCO, bit1: output AAC RAW
    #endif
#endif
    ...
    // New AVC
    vWriteDspShmWORD (W_AVC_FLAG, 0x3); // default is enable
    vWriteDspShmWORD (W_AVC_FC, 26); // 26 = 270 Hz
    vWriteDspShmDWRD (D_AVC_TARGET_LEVEL, 0x10000); // -24dB
    vWriteDspShmDWRD (D_AVC_ATTACK_RATE, 0x200);
    vWriteDspShmDWRD (D_AVC_RELEASE_RATE, 0x28);
    vWriteDspShmDWRD (D_AVC_RATIO, 0x0);
    vWriteDspShmDWRD (D_AVC_SILENCE_LEVEL, 0x10); // -96dB
    vWriteDspShmDWRD (D_AVC_MAX_EXPAND, 0x300); // 3dB
    vWriteDspShmDWRD (D_AVC_POST_GAIN, 0x0); // off
    vWriteDspShmDWRD (D_AVC_HOLD, 0x0); // 0ms
    vWriteDspShmDWRD (D_AVC_TARGET_LEVEL_1, 0x10000); // -24dB
    vWriteDspShmDWRD (D_AVC_ATTACK_RATE_1, 0x200);
    vWriteDspShmDWRD (D_AVC_RELEASE_RATE_1, 0x28);
    vWriteDspShmDWRD (D_AVC_RATIO_1, 0x0);
    vWriteDspShmDWRD (D_AVC_SILENCE_LEVEL_1, 0x10); // -96dB
    vWriteDspShmDWRD (D_AVC_MAX_EXPAND_1, 0x300); // 3dB
    vWriteDspShmDWRD (D_AVC_POST_GAIN_1, 0x0); // off
    vWriteDspShmDWRD (D_AVC_HOLD_1, 0x0); // 0ms
#else
    // Light added for AVC
...
#ifdef CC_S_SUBWOFFER_SUPPORT  ///CC_BASS_MANGER_INDEP_LPF
    vWriteDspShmDWRD (D_CUTOFF_FREQ_LPF, 300);
    vWriteDspShmDWRD (D_CUTOFF_FREQ_LPF_DEC2, 300);
#endif
	...
	*/
    ...
    vWriteDspShmDWRD(D_SRS_TSHD_TB_COMPRCtrl, DRVCUST_OptGet(eAudioTSHDComprCtrl));      //25%
    ...
    vWriteDspShmBYTE(B_SRS_TSHD_ANASIZE, (UINT8)DRVCUST_OptGet(eAudioTSHDAnaSize)); //150Hz
    vWriteDspShmBYTE(B_SRS_TSHD_TBFLAG, 0x0f);  //EANBLE
    vWriteDspShmBYTE(B_SRS_TSHD_HPFSIZE, 0); //80Hz
    ...
    */
     vWriteDspShmDWRD(D_SRS_TSHD_FLAGS, 0x0000151f);
    ...
    vWriteDspShmWORD(W_SRS_CC3D_FLAG, (UINT16)DRVCUST_OptGet(eAudioCC3Dflag)); //0x00007bfb
    //vWriteDspShmBYTE(B_SRS_CC3D_MODE, (UINT8)DRVCUST_OptGet(eAudioCC3Dmode)); // 1
    vWriteDspShmBYTE(B_SRS_CC3D_MODE, 0); // 0
    vWriteDspShmWORD(W_SRS_CC3D_INPUT_GAIN, 0x4000); //-8dB
    vWriteDspShmDWRD(D_SRS_CC3D_OUTPUT_GAIN, DRVCUST_OptGet(eAudioCC3DOutputGain)); //0dB

    //vWriteDspShmBYTE(B_SRS_CC3D_INMODE, 0x1); //SRS_IO_2_0_0
    vWriteDspShmBYTE(B_SRS_CC3D_INMODE, 17); //SRS_IO_LtRt
    vWriteDspShmBYTE(B_SRS_CC3D_3DMODE, 0x2); //SRS_SRS3D_STEREO

    vWriteDspShmBYTE(B_SRS_CC3D_SPKSIZE, (UINT8)DRVCUST_OptGet(eAudioCC3DSpkSize)); //150Hz
    //vWriteDspShmDWRD(D_SRS_CC3D_TB_TBCtrl, DRVCUST_OptGet(eAudioCC3DTbCtrl));      //50%
    vWriteDspShmDWRD(D_SRS_CC3D_TB_TBCtrl, 0x33333330);      //0.4
    vWriteDspShmDWRD(D_SRS_CC3D_TB_COMPRCtrl, DRVCUST_OptGet(eAudioCC3DComprCtrl));      //25%

    vWriteDspShmBYTE(B_SRS_CC3D_ANASIZE, (UINT8)DRVCUST_OptGet(eAudioCC3DAnaSize)); //150Hz
    vWriteDspShmBYTE(B_SRS_CC3D_TBFLAG, 0x0f);  //

    vWriteDspShmBYTE(B_SRS_CC3D_HPFSIZE, (UINT8)DRVCUST_OptGet(eAudioCC3DHpfSize)); //80HZ
    vWriteDspShmBYTE(B_SRS_CC3D_INISIZE, (UINT8)DRVCUST_OptGet(eAudioCC3DIniSize)); //InitNHFilter
    vWriteDspShmBYTE(B_SRS_CC3D_INIDEPTH, 0x20); //DelayLineInitialDepth
    ...
    vWriteDspShmWORD(W_SRS_FRONT_SPACECTRL, 0x50a4); //
    vWriteDspShmWORD(W_SRS_REAR_SPACECTRL, 0x2d71); //
    vWriteDspShmWORD(W_SRS_REAR_CENTERCTRL, 0x2d71); //Not Used
    vWriteDspShmWORD(W_SRS_TRUSUR_LEVEL, 0x4ccd); //

    //vWriteDspShmDWRD(D_SRS_MIX_FADECTRLEXT, 0x10002000); //
    vWriteDspShmDWRD(D_SRS_MIX_FADECTRLEXT, 0x40000000); //


    vWriteDspShmWORD(W_SRS_SRS3D_CENTERCTRL, 0x4000); //  SRS_FXP32(0.5, 1)
    vWriteDspShmWORD(W_SRS_SRS3D_SPACECTRL, 0x6666); //  SRS_FXP32(0.8f, 1)

    vWriteDspShmWORD(W_SRS_FOCUS_LEVEL, 0x3333); //
    vWriteDspShmWORD(W_SRS_LFOCUS_LEVEL, 0x3333); //
    vWriteDspShmWORD(W_SRS_RFOCUS_LEVEL, 0x3333); //
    vWriteDspShmWORD(W_SRS_LDEF_LEVEL, 0x0ccd); //
    vWriteDspShmWORD(W_SRS_RDEF_LEVEL, 0x0ccd); //

    vWriteDspShmWORD(W_SRS_CC3D_REF_LEV, 0x1400); //REF LEVEL
    vWriteDspShmWORD(W_SRS_CC3D_TBQ, 0x4000); //TBQ
    vWriteDspShmWORD(W_SRS_CC3D_DUCLEV, 0x1999); //DcuLevel
    ...
    vWriteDspShmDWRD (D_VIQ_NOISE_TH, DRVCUST_OptGet(eAudioVIQNoiseTh)); //sunman_viq2
       ...
       vWriteDspShmWORD (W_SRS_FLAG,0xA00);                                //ON/OFF
       //SRS HeadRoom
       vWriteDspShmDWRD(D_SRS_HEADROOM, 3); //0: 0db,1: 6db,2: 12db,3: 18db,4: 24db,5: 30db
       vWriteDspShmDWRD(D_SRS_COMPENSATE, 3); //0: 0db,1: 6db,2: 12db,3: 18db,4: 24db,5: 30db
#endif
#ifdef DSP_SUPPORT_SRS_PURESOUND
       vWriteDspShmWORD (W_SRS_FLAG,0x3800);                            //ON/OFF
       //SRS HeadRoom
       vWriteDspShmDWRD(D_SRS_HEADROOM, 3); //0: 0db,1: 6db,2: 12db,3: 18db,4: 24db,5: 30db
       vWriteDspShmDWRD(D_SRS_COMPENSATE, 3); //0: 0db,1: 6db,2: 12db,3: 18db,4: 24db,5: 30db
#endif
#ifdef DSP_SUPPORT_SRS_SS3D
       vWriteDspShmWORD (W_SRS_FLAG,0x2E00);                                //ON/OFF
       //SRS HeadRoom
       vWriteDspShmDWRD(D_SRS_HEADROOM, 3); //0: 0db,1: 6db,2: 12db,3: 18db,4: 24db,5: 30db
       vWriteDspShmDWRD(D_SRS_COMPENSATE, 3); //0: 0db,1: 6db,2: 12db,3: 18db,4: 24db,5: 30db
       Printf("fubofeng shm \n");
    ...
    vWriteDspShmWORD (W_SRS_PEQ_BANDS_1, (DRVCUST_OptGet(eAudioSRSPEQBands)<<8));
       ...
       vWriteDspShmDWRD (D_SRS_LMT_INPUT_GAIN, 0x7fffffff);
	   vWriteDspShmDWRD (D_SRS_LMT_OUTPUT_GAIN, 0x7fffffff);
	   vWriteDspShmDWRD (D_SRS_LMT_BOOST, 0x0a000a00);//
	   vWriteDspShmDWRD (D_SRS_LMT_LEVEL, 0x7fffffff);
	   vWriteDspShmDWRD (D_SRS_LMT_DELAY, 14 << 16);
	   vWriteDspShmDWRD (D_SRS_LMT_COEFF, 0x0010624e);
	   vWriteDspShmDWRD (D_SRS_LMT_THRESH, 0);
#endif

#ifdef DSP_SUPPORT_TCL_VBASS

   vWriteDspShmBYTE(B_TCLBASS_ENABLE     ,  1);
   vWriteDspShmDWRD(D_TCL_AO_PEAK        ,  5);
   vWriteDspShmDWRD(D_TCL_A1_PEAK        ,  143558);
   vWriteDspShmDWRD(D_TCL_A2_PEAK        ,  4601674);
   vWriteDspShmDWRD(D_TCL_BO_PEAK        ,  24);
   vWriteDspShmDWRD(D_TCL_B1_PEAK        ,  315945);
   vWriteDspShmDWRD(D_TCL_B2_PEAK        ,  4429268);
   vWriteDspShmDWRD(D_TCL_AT             ,  3112);
   vWriteDspShmDWRD(D_TCL_RT             ,  3);
   vWriteDspShmDWRD(D_TCL_A_PEAK         ,  3277);
   vWriteDspShmDWRD(D_TCL_GAIN_HAMONICGEN,  256);
   vWriteDspShmDWRD(D_TCL_G_OUT          ,  2);
   vWriteDspShmDWRD(D_TCL_GAIN_BACK      ,  1);
   vWriteDspShmDWRD(D_TCL_GAIN_OUT       ,  1);
   vWriteDspShmDWRD(D_TCL_GAINHAMONICOUT ,  1);
   vWriteDspShmDWRD(D_TCL_COEFF_HAM      ,  0);
   vWriteDspShmDWRD(D_TCL_X1PEAK         ,  0);
   vWriteDspShmDWRD(D_TCL_X2PEAK         ,  0);
   vWriteDspShmDWRD(D_TCL_C1PEAK         ,  0);
   vWriteDspShmDWRD(D_TCL_C2PEAK         ,  0);
   vWriteDspShmDWRD(D_TCL_COUT_PEAK      ,  0);
   vWriteDspShmDWRD(D_TCL_Y_OUT          ,  0);
   ...
   //all ATV audio shared shm
...
#ifdef CC_AUD_DDI
    vWriteDspShmBYTE (B_SAP_HP_MUTE_HIGH,   90);
    vWriteDspShmBYTE (B_SAP_HP_MUTE_LOW,  176);
#else
...
#else
    vWriteDspShmBYTE (B_SAP_FILTER_SEL,   3); //0->3 for COL FT IDW_2000
#endif
    vWriteDspShmBYTE (B_SAP_RATIO,   0x16); //Original :0x1C  //US FT issue ... SAP sensitivity
    vWriteDspShmBYTE (B_MTS_SAP_MUTE_CTRL,   1); // 1 to enable "real sap noise mute function"
    vWriteDspShmBYTE (B_MTS_HDEV_LVL_REDUCE, 1); // 1 to reduce mono output level by 6dB under 576k/768k mode
#ifdef CC_AUD_MTS_OUTPUT_LEVEL_GAIN_SUPPORT
...
#ifdef CC_AUD_SX1_FEATURE
    vWriteDspShmBYTE(B_FM_MUTE_THRESHOLD_HIGHT,   prPalFactory->u1CarrierMuteHdevHigherThd);
    vWriteDspShmBYTE(B_FM_MUTE_THRESHOLD_LOW, prPalFactory->u1CarrierMuteHdevLowerThd);
#endif
    ...
    vWriteDspShmDWRD(D_DETECTOR_FM_NOISE_TH  , 0x46000);//Sony FT detect BG as M because noise level is about 0x40000
#else
    vWriteDspShmDWRD(D_DETECTOR_FM_NOISE_TH  , 0x16000);
#endif
    vWriteDspShmBYTE(B_DETECTOR_A2_PILOT_CHK, 0x00);
    ...
    vWriteShmUINT8(B_ADFLAG , (uReadShmUINT8(B_ADFLAG)) | 0x10);
    #endif
    ...
    vWriteDspShmWORD (W_LIMITER_FLAG, 0x0); //limiter[5] before volume compensate
    ...
    vWriteDspShmDWRD (D_LIMITER_ATTACK_RATE, 0x200);  // 2 dB/ms, AQ?
    vWriteDspShmDWRD (D_LIMITER_RELEASE_RATE, 0x14);  // 0.08 dB/ms, AQ?
    vWriteDspShmDWRD (D_LIMITER_GAIN_RATIO, 0x0); // default is 0x0

    // DRC
    vWriteDspShmWORD (W_DRC_FLAG, 0x0); // default is disabled
    vWriteDspShmWORD (W_DRC_FC, 30); // 30 = 390 Hz
    vWriteDspShmDWRD (D_DRC_TARGET_LEVEL, 0x20000); // -12dB
    vWriteDspShmDWRD (D_DRC_ATTACK_RATE, 0x200);
    vWriteDspShmDWRD (D_DRC_RELEASE_RATE, 0x14);
    vWriteDspShmDWRD (D_DRC_RATIO, 0x0);
    vWriteDspShmDWRD (D_DRC_SILENCE_LEVEL, 0x10); // -96dB
    vWriteDspShmDWRD (D_DRC_MAX_EXPAND, 0x300); // 3dB
    vWriteDspShmDWRD (D_DRC_HYSTERESIS, 0x0); // 0dB
    vWriteDspShmDWRD (D_DRC_HOLD, 0x0); // 0ms
    vWriteDspShmDWRD (D_DRC_TARGET_LEVEL_1, 0x20000); // -12dB
    vWriteDspShmDWRD (D_DRC_ATTACK_RATE_1, 0x200);
    vWriteDspShmDWRD (D_DRC_RELEASE_RATE_1, 0x14);
    vWriteDspShmDWRD (D_DRC_RATIO_1, 0x0);
    vWriteDspShmDWRD (D_DRC_SILENCE_LEVEL_1, 0x10); // -96dB
    vWriteDspShmDWRD (D_DRC_MAX_EXPAND_1, 0x300); // 3dB
    vWriteDspShmDWRD (D_DRC_HYSTERESIS_1, 0x0); // 0dB
    vWriteDspShmDWRD (D_DRC_HOLD_1, 0x0); // 0ms

    // Limiter post drc mode
    vWriteDspShmDWRD (W_DRC_LIMITER_POST_DRC_MODE, 0x1);//default is ON
#endif
...
#ifdef CC_AUD_PEQ_LR
    vWriteDspShmWORD(W_PEQ_SET0_FC_R	, 50);
    vWriteDspShmWORD(W_PEQ_SET1_FC_R	, 100);
    vWriteDspShmWORD(W_PEQ_SET2_FC_R	, 200);
    vWriteDspShmWORD(W_PEQ_SET3_FC_R	, 300);
    vWriteDspShmWORD(W_PEQ_SET4_FC_R	, 1000);
    vWriteDspShmWORD(W_PEQ_SET5_FC_R	, 3000);
    vWriteDspShmWORD(W_PEQ_SET6_FC_R	, 5000);
    vWriteDspShmWORD(W_PEQ_SET7_FC_R	, 15000);
    vWriteDspShmWORD(W_PEQ_SET8_FC_R	, 18000);

    for (i=0; i<8; i++)
    {
    	vWriteDspShmDWRD(D_PEQ_SET0_GAIN_R + i*8, 0x200000);
    	vWriteDspShmWORD(W_PEQ_SET0_BW_R + i*8, 500);
    }
    vWriteDspShmDWRD(D_PEQ_SET8_GAIN_R, 0x200000);
    vWriteDspShmWORD(W_PEQ_SET8_BW_R, 500);
#endif
#endif
    ...
    vWriteDspShmDWRD(D_VIR_BASS_GAIN_2, 0x800);
#endif
...
//#ifdef ENABLE_AUDIO_RECORD_SUPPORT
	...
	vWriteDspShmWORD (W_BLUETOOTH_DATA_FLAG, 0x0);
	// Block Number 	PCM=128,	 MP3=128
	vWriteDspShmBYTE (B_BLUETOOTH_BLOCK_NUM, 128);
	// Block Size,		PCM=0x400,	MP3=0x3C0(320kbps)/0x240(192kbps)/0x180(128kbps)
	vWriteDspShmWORD (W_BLUETOOTH_BLOCK_SIZE, 0x400)
	vWriteDspShmBYTE (B_BLUETOOTH_BLK_INT_CNT, 16);
//#endif
...
#ifdef CC_VOCAL_MUTE
    /// Vocal Cancel
    vWriteDspShmBYTE(B_VC_DIFF_PRE,     0x00);
#endif
    ...
    // g726dec
    vWriteDspShmWORD (W_G726_FRAME,256);
    vWriteDspShmBYTE (B_G726_CH,1);
    vWriteDspShmBYTE (B_G726_RATE,32);          // rate: 40, 32, 24, 16, default:32
    vWriteDspShmBYTE (B_G726_LAW,0);            // 0:u-law, 1:a-law, 2: linear, unused for PCM in/out
    vWriteDspShmBYTE (B_G726_SRATE, 0x4);       // only support 8kHz, 4:SFREQ_8K
    vWriteDspShmWORD (W_G726_FRAME_DEC2,256);
    vWriteDspShmBYTE (B_G726_CH_DEC2,1);
    vWriteDspShmBYTE (B_G726_RATE_DEC2,32);
    vWriteDspShmBYTE (B_G726_LAW_DEC2,0);       // 0:u-law, 1:a-law, 2: linear, unused for PCM in/out
    vWriteDspShmBYTE (B_G726_SRATE_DEC2, 0x4);  // only support 8kHz, 4:SFREQ_8K
    vWriteDspShmWORD (W_G726_FRAME_DEC3,256);
    vWriteDspShmBYTE (B_G726_CH_DEC3,1);
    vWriteDspShmBYTE (B_G726_RATE_DEC3,32);
    vWriteDspShmBYTE (B_G726_LAW_DEC3,0);       // 0:u-law, 1:a-law, 2: linear, unused for PCM in/out
    vWriteDspShmBYTE (B_G726_SRATE_DEC3, 0x4);  // only support 8kHz, 4:SFREQ_8K

#ifndef CC_AUD_FAST_STOP_BY_HW_GAIN
    vWriteDspShmWORD (W_SYSTEM_CTRL2, u2ReadDspShmWORD(W_SYSTEM_CTRL2) | 0x4);
#else
    vWriteDspShmWORD (W_SYSTEM_CTRL2, u2ReadDspShmWORD(W_SYSTEM_CTRL2) | 0x204);    //bit9 enable FAST_STOP_BY_HW_GAIN
#endif
#ifdef CC_AUD_FIR_SUPPORT
    vWriteDspShmBYTE(B_FIR_ENABLE, 1);
#endif
...
void vDspInit (void)
{
    ...
    WriteDspCommDram (ADDR_RC2D_SPEED_DEC3, 0);
#endif
    ...
    WriteDspCommDram (ADDR_RC2D_L_CH_VOL, u4ReadDspShmDWRD(D_VOL_L));
    WriteDspCommDram (ADDR_RC2D_R_CH_VOL, u4ReadDspShmDWRD(D_VOL_R));

    WriteDspCommDram (ADDR_RC2D_L_CH_LAST_VOL, u4ReadDspShmDWRD(D_VOL_L));
    WriteDspCommDram (ADDR_RC2D_R_CH_LAST_VOL, u4ReadDspShmDWRD(D_VOL_R));
...
#ifdef CC_AUD_AVC_V20
    // New AVC
    vWriteDspWORD (DRAM_DRC2_AVC_FLAG, u2ReadDspShmWORD(W_AVC_FLAG));
...
#endif
    vWriteDspWORD (ADDR_RC2D_EFFVOL_SMOOTH_ORDER_DEC3, (UINT16) 10);

    vWriteDspWORD (ADDR_RC2D_CH_DELAY_DEC3, 0);
...
#ifdef CC_VOCAL_MUTE
    /// Voice Cancel
    vWriteDspWORD (ADDR_RC2D_POWER_DIFF_PRE, uReadDspShmBYTE(B_VC_DIFF_PRE));
    for(iLoopVar=0;iLoopVar<10;iLoopVar++)
    {
        WriteDspCommDram ((ADDR_RC2D_KARA_VOICE_MUTE_FILTER_COEFF+iLoopVar), VC_BSF_BPF_COEF[iLoopVar]);
    }
    WriteDspCommDram (ADDR_RC2D_KARA_VOICE_MUTE_BSF_GAIN, 0x20000); /// 0x20000 is 0dB
    WriteDspCommDram (ADDR_RC2D_KARA_VOICE_MUTE_BPF_GAIN, 0x20000); /// 0x20000 is 0dB
#endif
}
...
void vDspPlayCommonInit (void)
{
    ...
    WriteDspCommDram (ADDR_VSURR_DELAY_ON, 0x0);//off by xiaoyi20110525,actually is bypass in DSP
    ...
    WriteDspCommDram ((u4DramBase + ADDR_SPKHIGH_PARA4), 0x100000);
...
#ifdef CC_AUD_DRC_V20
    vWriteDspWORD (DRAM_DRC2_LIMITER_MODE,  u2ReadDspShmWORD (W_LIMITER_MODE));
    vWriteDspWORD (DRAM_DRC2_LIMITER_FLAG, u2ReadDspShmWORD (W_LIMITER_FLAG));
    ...
    // DRC
    vWriteDspWORD (DRAM_DRC2_DRC_FLAG, u2ReadDspShmWORD(W_DRC_FLAG));
    ...
    // Limiter post drc mode
    WriteDspCommDram (DRAM_DRC2_LIMITER_POST_DRC_MODE, u2ReadDspShmWORD(W_DRC_LIMITER_POST_DRC_MODE));
...
//#ifdef ENABLE_AUDIO_RECORD_SUPPORT
    ...
    vWriteDspWORD (ADDR_RC2D_UPLOAD_BLOCK_SIZE, u2ReadDspShmWORD(W_UPLOAD_BLOCK_SIZE));
    vWriteDspWORD (ADDR_RC2D_UPLOAD_BLK_INT_CNT, uReadDspShmBYTE(B_UPLOAD_BLK_INT_CNT));
    WriteDspCommDram(ADDR_D2RC_UPLOAD_WRITE_PTR,((_u4DspCmptBuf[TYPE_COMM_DRAM_IDX] + AUD_UPLOAD_BUFFER_BASE)&0xffffff));
    WriteDspCommDram(ADDR_RC2D_UPLOAD_READ_PTR,((_u4DspCmptBuf[TYPE_COMM_DRAM_IDX] + AUD_UPLOAD_BUFFER_BASE)&0xffffff));                                                              
    ...
    vWriteDspWORD (ADDR_RC2D_BLUETOOTH_BLK_INT_CNT, uReadDspShmBYTE(B_BLUETOOTH_BLK_INT_CNT));
    WriteDspCommDram (ADDR_RC2D_BLUETOOTH_WRITE_PTR, ((_u4DspCmptBuf[TYPE_COMM_DRAM_IDX] + AUD_BLUETOOTH_BUFFER_BASE)&0xffffff));
//#endif
#ifdef CC_AUD_FIR_SUPPORT
    vWriteDspWORD (ADDR_RC2D_NEWPOST_FLAG,
...
#ifdef DSP_SUPPORT_DUAL_DECODE
    //Reset Speed to 0
    vWriteDspWORD (ADDR_RC2D_SPEED_DEC3, u2ReadDspShmWORD (W_SPEED_DEC3));
#endif
    // Reset aout bank number to default value for some decoding will change it
    vWriteDspWORD (ADDR_RC2D_AOUT_BANK_576_NUM_DEC3,
        uReadDspShmBYTE (B_BANK576NUM_DEC3));
