...
static void _VdpIfStatus(UCHAR ucVdpId)
{
    VDP_CFG_T *_prVdpCfg = NULL;
    if(ucVdpId >= VDP_NS)
    {
        return;
    }
    _prVdpCfg = B2R_GetVdpConf(ucVdpId);

    if (_prVdpCfg)
    {
        LOG(1, "VDP(%d, On:%d) B2R(%d, On:%d) M(%d) O(%d) ES(%d) Bg(%x) Fz(%d)\n",
            ucVdpId,
            _prVdpCfg->ucVdpEnable,
            _prVdpCfg->ucB2rId,
            _prVdpCfg->ucEnable,
            //_prVdpCfg->ucMode,
            //_prVdpCfg->ucDispMode,
            u1ScpipGetDispMode(ucVdpId),
            _prVdpCfg->rOutInfo.ucPmxMode,
            _prVdpCfg->ucInputPort[0],
            _prVdpCfg->u4BgColor,
    ...
    LOG(1, "  --Src(W:%d H:%d Fr:%d LineSize:%d) FrmCrop(T:%d B:%d) Crop(E:%d X:%d Y:%d W:%d H:%d) Afd(Afd:%d AR:%d W:%d H:%d Idc:%d)\n",
        _prVdpCfg->u4SrcWidth,
        _prVdpCfg->u4SrcHeight,
        _prVdpCfg->u4FrameRate,
        _prVdpCfg->u4SrcLineSize,
        _prVdpCfg->u4FrmCropTop,
        _prVdpCfg->u4FrmCropBtm,
        _prVdpCfg->fgCropping,
        _prVdpCfg->u4CropX,
        _prVdpCfg->u4CropY,
        _prVdpCfg->u4CropWidth,
        _prVdpCfg->u4CropHeight,
        _prVdpCfg->u4Afd,
        _prVdpCfg->u4AspectRatio,
        _prVdpCfg->u4AspectRatioWidth,
        _prVdpCfg->u4AspectRatioHeight,
        _prVdpCfg->u4AspectRatioIdc);
    LOG(1, "  --Total(%d %d) Active(%d %d) Fr(%d) In-Fr(%d) Prg(%d) DoubleClk(%d) ClkMode(%d)\n",
        _prVdpCfg->rOutInfo.u4TotalWidth,
        _prVdpCfg->rOutInfo.u4TotalHeight,
        _prVdpCfg->rOutInfo.u4Width,
        _prVdpCfg->rOutInfo.u4Height,
        _prVdpCfg->rOutInfo.ucFrameRate,
        _prVdpCfg->rOutInfo.ucInputFrameRate,
        _prVdpCfg->rOutInfo.ucPrg,
        _prVdpCfg->rOutInfo.ucDoubleClk,
        _prVdpCfg->rOutInfo.u4ClkMode);
    }
    ...
    LOG(1, "-- VDP % d Output Window Ctl BY %s\n",ucVdpId,VDP_GetOutRegionControlByBE(ucVdpId) == FALSE ? "MTK":"BE IC");
    LOG(1, "-- VDP % d Source Regen  Ctl BY %s\n",ucVdpId,VDP_GetSrcRegionControlByBE(ucVdpId) == FALSE ? "MTK":"BE IC");
    #endif
...
#ifdef CC_MT5890
    // show bypass path information
    LOG(0, "%s,enable(%d) O(%d, %d)\n", "VDP_BYPASS",bGetPlaneEnable(VDP_BYPASS), u4GetPlaneWidth(VDP_BYPASS), u4GetPlaneHeight(VDP_BYPASS));
    LOG(0, "%s,enable(%d) O(%d, %d)\n", "VDP_PQPATH",bGetPlaneEnable(VDP_PQPATH), u4GetPlaneWidth(VDP_PQPATH), u4GetPlaneHeight(VDP_PQPATH));
    LOG(0, "%s,enable(%d) O(%d, %d)\n", "VDP_GFXOUT",bGetPlaneEnable(VDP_GFXOUT), u4GetPlaneWidth(VDP_GFXOUT), u4GetPlaneHeight(VDP_GFXOUT));
#endif
    for(u4VdpId = 0; u4VdpId < VDP_NS; u4VdpId++)
    {
...
UINT32 VDP_SetEnable(UCHAR ucVdpId, UCHAR ucEnable)
{
    ...
    else
    {
        ...
        if((B2R_GetImgConnect(ucVdpId)!= 0) &&
           (t_b2r_status.ucStatus == VDP_STATUS_NOSIGNAL))
        {
...
UINT32 VDP_SetMode(UCHAR ucVdpId, UCHAR ucMode)
{
    ...
    else if(_arVdpConf[ucVdpId].ucVdpEnable)
    {
        ...
        _vDrvVideoSetMute(MUTE_MODULE_API_FORCE, ucVdpId, (_u4DrvVideoGetMute(ucVdpId))?10:0, FALSE); //change 5 to 10 when change need to check CR DTV00612212 DTV00583481
    }
...
UINT32 VDP_SetSrcRegion(UCHAR ucVdpId, UCHAR ucSrcFullRegion, VDP_REGION_T rSrcRegion)
{
    ...
    #if defined(CC_OUT_OR_SRC_REGION_CHG_NOTIFY)
    VDP_DrvSrcRangeChgNotify(ucVdpId,_arVdpConf[ucVdpId].rSrcRegion);
    #endif
    VDP_MUTEX_UNLOCK;
    return VDP_SET_OK;
#else
...
UINT32 VDP_SetOutRegion(UCHAR ucVdpId, UCHAR ucOutFullRegion, VDP_REGION_T rOutRegion)
{
    ...
    #if defined(CC_OUT_OR_SRC_REGION_CHG_NOTIFY)
    VDP_DrvOutputRangeChgNotify(ucVdpId,_arVdpConf[ucVdpId].rOutRegion);
    #endif
...
static UINT32 _VDP_CheckSourceChangeCallback(UCHAR ucVdpId, VDP_SOURCE_INFO_T rVdpSrcInfo)
{
    ...
    ///////////////////////Decode Dual Mode   callback when it's not stable or main/sub both stable/////////
    if (bDrvVideoIsDecDualMode())
    {
        static UINT8 u1SigStateRec[VDP_NS];
        UINT8 u1IsStable = !(rVdpSrcInfo.u4FrameRate == 0 ||  rVdpSrcInfo.u4SrcWidth == 0 || rVdpSrcInfo.u4SrcHeight == 0);
        
        u1SigStateRec[ucVdpId] = u1IsStable;
        ...
        else if (TRUE == u1IsStable && u1SigStateRec[VDP_1] == u1SigStateRec[VDP_2])
        {
            ucVdpId = VDP_1;
            //continue
        }
...
UINT32 VDP_SourceChangeNotify(UINT8 bPath)
{
    ...
    rVdpSrcInfo.u4SrcWidth = wApiVideoGetInputWidth(bPath);
    rVdpSrcInfo.u4SrcHeight = wApiVideoGetInputHeight(bPath);
    ...
    // Update Scaler Info
    _arScalerInfo[bPath].u4SrcWidth = wDrvVideoInputWidth(bPath);
    _arScalerInfo[bPath].u4SrcHeight = wDrvVideoInputHeight(bPath);
...
*/
UINT32 VDP_Drv3DModeChgNotify(UINT8 bPath, VDP_3D_MODE_CHG_INFO_T rMode,UINT8 u1BeforeAfter)
{
    if(bPath >= VDP_NS)
    {
        return 0;
    ...
    LOG(0,"------- in VDP_Drv3DModeChgNotify ---IsBefore=%d-------\n",u1BeforeAfter);

    if(u1BeforeAfter==1)
	{
	    if(VDP_CHECK_CB_FUNC_VERIFY(_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_AFTER_3D_MODE_CHGED_IND], _rVdpCbFunc.au4CbFuncCRC[VDP_CB_FUNC_AFTER_3D_MODE_CHGED_IND]))
	    {
	        if(_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_AFTER_3D_MODE_CHGED_IND])
	        {
	            if(((PFN_VDP_3D_Mode_CHG_NOTIFY)_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_AFTER_3D_MODE_CHGED_IND])(bPath, rMode))
	            {
	                LOG(0,"after VDP_Drv3DModeChgNotify and (srcWid,srcHei,framerate,u1Output3DFormat,input3Dfor,colorformat,colorspace)(%d,%d,%d,%d,%d,%d,%d)\n",
						rMode.u4SrcWidth,rMode.u4SrcHeight,rMode.u4InOutFrameRate,rMode.u1Output3DFormat,rMode.u1Input3DFormat,rMode.u1ColorFormat,rMode.u1ColorSpace);
	                return 1;
	            }
	        }
	    }
	    ...
	    {
	        ASSERT(_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_AFTER_3D_MODE_CHGED_IND] == 0);
	    }
	}
	else if(u1BeforeAfter==0)
	{
	    if(VDP_CHECK_CB_FUNC_VERIFY(_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_3D_MODE_CHG_IND], _rVdpCbFunc.au4CbFuncCRC[VDP_CB_FUNC_3D_MODE_CHG_IND]))
	    {
	        if(_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_3D_MODE_CHG_IND])
	        {
	            if(((PFN_VDP_3D_Mode_CHG_NOTIFY)_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_3D_MODE_CHG_IND])(bPath, rMode))
	            {
	                LOG(0,"before VDP_Drv3DModeChgNotify and (srcWid,srcHei,framerate,u1Output3DFormat,input3Dfor,colorformat,colorspace)(%d,%d,%d,%d,%d,%d,%d)\n",
						rMode.u4SrcWidth,rMode.u4SrcHeight,rMode.u4InOutFrameRate,rMode.u1Output3DFormat,rMode.u1Input3DFormat,rMode.u1ColorFormat,rMode.u1ColorSpace);
	                return 1;
	            }
	        }
	    ...
	    {
	        ASSERT(_rVdpCbFunc.au4CbFunc[VDP_CB_FUNC_3D_MODE_CHG_IND] == 0);
	    }
	}
	else
		return 0;
...
//#if defined(CC_MT5398) || defined(CC_MT5368) || defined(CC_MT5389) || defined(CC_MT5399)
#if	defined(CC_MT5890)
...
UINT32 VDP_SetGameMode(UINT32 u4VdpId, UINT8 u1OnOff)
{
    ...
    if (SRM_IsGameMode(u4VdpId) != u1OnOff)
    {
		...
		DRVCUST_SendEvent(E_CUST_VDO_DELAY_CHANGE, 0);
    }   
#else
    UNUSED(u1OnOff);
#endif
...
void VDP_GetDramInf(UINT8 bPath,DRAM_INFO_T *inf)
{
    ...
    {
#if defined(CC_MT5890)
       VDP_HalGetB2rDispInfo(bPath,(B2R_DISP_INFO_T *)inf);
...
UINT8 VDP_GetVdpId(UINT8 u1B2rId)
{
	...
	for (u1Idx = 0; u1Idx < VDP_NS; u1Idx++)
	{
		...
		if (t_b2r_status.ucB2rId == u1B2rId)
		{
			return u1Idx;
		}
	}
