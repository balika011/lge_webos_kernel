                                 ...
                                 UINT32 u4BufStart, UINT32 u4BufEnd)
{
    ...
    if(_u4Dmx130Remainder > 0)
    {
        ...
        if(fgLock)
        {
            ...
            if(!_DmxTs130SendPacket(_u4Dmx130PktBufStart, 1, _u4Dmx130PktBufStart, _u4Dmx130PktBufEnd))
            {
    ...
    while(u4RemainSize >= 130)
    {
        ...
        else
        {
            if(fgPreLock)
            {
                ...
                if(!_DmxTs130SendPacket(u4PreDataPtr, (u4Count / 130), u4BufStart, u4BufEnd))
               	{
...
BOOL _DMX_TS130_SendDummyPacket(UINT8 u1TsIdx, UINT8 u1Pidx)
{
	...
	if(u1TsIdx == 0)
	{
		_DMX_DDI_SetPortEx(DMX_DDI_PORT_FRAMER0, _u1Dmx130Pidx, TRUE, FALSE);
	}
	else if(u1TsIdx == 1)
	{
		_DMX_DDI_SetPortEx(DMX_DDI_PORT_FRAMER1, _u1Dmx130Pidx, TRUE, FALSE);
	}
	else
	{
	...
	if(prPidStruct != NULL)
    {
        ...
        if(!_DmxTs130DDIMove(_u4Dmx130DummyBufStart, 188 * DMX_TS130_DUMMY_PKTCOUNT, 
...
BOOL _DMX_TS134_MoveData(DMX_MM_DATA_T *prData)
{
    ...
    {
        LOG(3,"Framer size is not 134 byte align\n");
        return FALSE;
    }
