...
static BOOL _Dmx_Multi2_Cleanup(void)
{
    ...
    {
        VERIFY(BSP_FreeAlignedDmaMemory((void*)_u4SourceBufAddr));
    }

    if (_u4VideoBufAddr != 0)
    {
        VERIFY(BSP_FreeAlignedDmaMemory((void*)_u4VideoBufAddr));
    }

    if (_u4AudioBufAddr != 0)
    {
        VERIFY(BSP_FreeAlignedDmaMemory((void*)_u4AudioBufAddr));
    }

    if (_u4BypassBufAddr != 0)
    {
        VERIFY(BSP_FreeAlignedDmaMemory((void*)_u4BypassBufAddr));
    }
...
static BOOL _DmxEmu_Multi2RecTest(UINT8 u1TsIndex)
{
    ...
    u4SourceSize = _u4SourceBufSize;
    u4DstAddr = 0x3400000;
    x_memset(u4DstAddr, 0, _u4GoldenBufAddr);
