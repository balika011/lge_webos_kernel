...
//    *pu2Ptr = *pu4PesSize - 6;
    ...
    if(fgPicpat)
    {
        ...
        // first 5 means Ts packet header (plus pointer)
        // second 5 means start code count
        u4StartEnd = 4 + _u4FVRPesHeaderLen + 9 + _u4StartOffset + 5;    
        pu4Result = (UINT32*)_u4TSResultAddr;
        pu4Result[_u4TSResultCount] = u4PktCount + (u4StartEnd / 188);
        _u4TSResultCount++;
    }
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SetPVRPESTestPid(UINT8 u1PidIdx, BOOL fgPicSearch)
{
    FVR_PID_T rFVRPid;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_FVR_PES_Test(UINT8 u1TsIndex)
{
    BOOL fgRet = TRUE;
    ...
    VERIFY(FVR_Init());    
    VERIFY(_SetPVRPESTestPid(0, TRUE));
...
static BOOL _FVREmuGenTS(UINT8 u1TsIndex, UINT32 *pu4DataSize)
{
    ...
    //-----------------------------------------
    VERIFY(FVR_Init());
    VERIFY(_SetPVRPESTestPid(0, FALSE));
...
static BOOL _FVREmuSetPlayPid(UINT8 u1TsIdx)
{
    ...
    _DMX_SetToDecoder(FALSE);

    return  TRUE;
}

//-----------------------------------------------------------------------------
static BOOL _FVREmuSetPVRPlay(UINT32 u4DataSize)
{
...
//-----------------------------------------------------------------------------
/** _DMX_EMU_FVR_TimestampOnOffTest
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_FVR_TimestampOnOffTest(UINT8 u1TsIndex, BOOL fgTimeStampOn)
{
    DMX_MM_DATA_T rData;
    DMX_PVR_PLAY_T rPlay;
    ...
    VERIFY(_FVREmuSetPVRPlay(u4DataSize));
    ...
    if(!fgTimeStampOn)
    {
        ...
        while(u4SendSize > 0)
        {
            ...
            VERIFY(_DMX_PVRPlay_SingleMove(&rData));
    ...
    {
        VERIFY(_FVREmuSetTimestamp(TRUE));

        VERIFY(DMX_PVRPlay_SetBuffer(TS_FVR_RECORD_ADDR, TS_FVR_RECORD_ADDR + TS_FVR_PID_SIZE - 1, 
...
//-----------------------------------------------------------------------------
/** _DMX_EMU_FVR_TimestampPauseTest
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_FVR_TimestampPauseTest(UINT8 u1TsIndex)
{
    DMX_PVR_PLAY_T rPlay;
    ...
    while(1)
    {
        ...
        VERIFY(_DMX_PVRPlay_Pause(&rPtr));
        u4PauseCount++;
        Printf("Pause: %d\n", u4PauseCount);
        ...
        x_thread_delay(50);
        VERIFY(_DMX_PVRPlay_Resume(&rPtr));        
    }
...
//-----------------------------------------------------------------------------
/** _DMX_EMU_FVR_TimestampResetTest
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_FVR_TimestampResetTest(UINT8 u1TsIndex)
{
    DMX_PVR_PLAY_T rPlay;
    UINT32 u4DataSize;
    DMX_PID_T rPid;
    ...
    VERIFY(_DMX_PVRPlay_SetBuffer(TS_FVR_RECORD_ADDR, TS_FVR_RECORD_ADDR + TS_FVR_PID_SIZE - 1, 
                              TS_FVR_RECORD_ADDR + u4DataSize, TS_FVR_RECORD_ADDR));
