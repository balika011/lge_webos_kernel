...
static BOOL _Dmx_Emu_DvbReleaseBuffers(void)
{
    ...
    for (i = 0; i < DMX_DVB_PID_NUM; i++)
    {
        ...
        {
            VERIFY(BSP_FreeAlignedDmaMemory((void*)_rDvbData[i].u4BufStart));
    ...
    if (_u4DvbSrcAddr != 0)
    {
        VERIFY(BSP_FreeAlignedDmaMemory((void*)_u4DvbSrcAddr));
        _u4DvbSrcAddr = 0;
        _u4DvbSrcSize = 0;
    }
    ...
    {
        VERIFY(BSP_FreeAlignedDmaMemory((void*)_u4DvbGoldenAddr));
        _u4DvbGoldenAddr = 0;
        _u4DvbGoldenSize = 0;
    }
...
static BOOL _DmxEmu_RecDvbMain(UINT8 u1TsIndex)
{
    ...
    if(!FVR_SetGBL(FVR_GBL_FLAG_ALL, &rGblPid))
    {
        fgRet = FALSE;
        goto _exit;
    }    
