...
/*
        for(j=8; j<16; j++)
        {
            _arPat[i].au1Pat[j] = 0; //random(256);
            _arPat[i].au1Mask[j] = 0; //random(256);
            _arPat[i].au1PosNeg[j] = 0; //random(256);
        }
*/
    }
}
...
{
    UINT32 i;
    static UINT32 _u4FilterIndex = 0;
    ...
    for(i=0; i<_u1DmxEmuTestPidNum; i++)
    {
        ...
        {
            // generic section filter only support 32 filter yet
            _arPat[i].u1FilterIdx = i;
        }
        else
        {
        _arPat[i].u1FilterIdx = _u4FilterIndex;
            }
        ...
        _u4FilterIndex++;
        _u4FilterIndex %= DMX_NUM_FILTER_INDEX;
    }
    ...
    UINT32 u4Data, const void* pvNotifyTag)
{
    ...
    if(u4Len > 0)
    {
        ...
        for(j=0; j<u4Len; j++)
        {
            ...
            {
                Printf("Compare error, %d, %x != %x\n", u1Pidx, pu1Test[j], pu1Golden[i+j]);
                _afgCompareResult[u1Pidx] = FALSE;
                return TRUE;
            }
        }
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _ClearTestFilter(void)
{
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SetTestFilter(UINT8 u1PatIdx)
{
    DMX_FILTER_T rFilter;
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _ClearTestGenericFilter(void)
{
 ...
 */
//-----------------------------------------------------------------------------
static BOOL _SetTestGenericFilter(UINT8 u1PatIdx)
{
    DMX_FILTER_GENERIC_T rFilter;
    UINT32 i;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSI_Test(UINT8 u1TsIdx, BOOL fgGenPat, DMX_FILTER_MODE_T eMode)
{
    UINT32 i, u4PidCount;
    UINT8 *pu1Golden;
    BOOL fgRet = TRUE;
    ...
    if(eMode == DMX_FILTER_MODE_GENERIC)
    {
        _u1DmxEmuTestPidNum = 1; //32;
        u4PidCount = 1; //32;
    }
    else
    {
        _u1DmxEmuTestPidNum = 80;
        u4PidCount = 80;
    }
    ...
    if(eMode == DMX_FILTER_MODE_GENERIC)
    {
        VERIFY(_ClearTestGenericFilter());
    }
    else
    {
        VERIFY(_ClearTestFilter());
    }        
    ...
    // Software parsing table, so can not input data to DMX too fast
    // N/M Mbits/sec
/*    
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSI_Rec_Test(UINT8 u1TsIdx, BOOL fgGenPat, DMX_FILTER_MODE_T eMode)
{
    UINT32 i;
    UINT8 *pu1Test, *pu1Golden;
    UINT32 u4DataSize;
    ...
    if(eMode == DMX_FILTER_MODE_GENERIC)
    {
        _u1DmxEmuTestPidNum = 32;
        u4PidCount = 32;
    }
    else
    {
        _u1DmxEmuTestPidNum = 64;
        u4PidCount = 64;
    }
...
/*
    // N/M Mbits/sec
    DDI_WRITE32(DDI_REG_PERIOD_M, 1 * 188 * 8 * 27);
    DDI_WRITE32(DDI_REG_PERIOD_N, 4);
    DDI_WRITE32(DDI_REG_RATE_CMD, 0x2);     // Update M and N
*/
    if(!_DMX_EMU_DDI_Transfer(0, TS_PACKET_ADDR, _u4TSPktOffset - _u4TSPktAddr))
    {
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSI_PlayRec_Test(UINT8 u1TsIdex, BOOL fgGenPat, DMX_FILTER_MODE_T eMode)
{
    UINT32 i;
    UINT8 *pu1Test, *pu1Golden;
    UINT32 u4DataSize;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSICombo_Test(UINT8 u1TsIdx, BOOL fgGenPat, DMX_FILTER_MODE_T eMode, UINT32 u4Hw)
{
    if(u4Hw == 0)
    {
        return _DMX_EMU_PSI_Test(u1TsIdx, fgGenPat, eMode);
    }
    else if(u4Hw == 1)
    {
        return _DMX_EMU_PSI_Rec_Test(u1TsIdx, fgGenPat, eMode);
    }
    else if(u4Hw == 2)
    {
        return _DMX_EMU_PSI_PlayRec_Test(u1TsIdx, fgGenPat, eMode);
    }
    else
    {
        return FALSE;
...
//-----------------------------------------------------------------------------
static BOOL _FVR_LPSet(UINT8 u1TsIndex)
{
    ...
    if((_FVR_IsRecording(u1TsIndex)) || (_DMX_PVRPlay_GetState() == DMX_PVRPLAY_STATE_PLAYING))
    {
...
//-----------------------------------------------------------------------------
static BOOL _FVR_LPEnable(UINT8 u1TsIndex, BOOL fgEnable)
{
    if(fgEnable)
    {        
...
//-----------------------------------------------------------------------------
static BOOL _FVR_LPFree(UINT8 u1TsIndex)
{
...
static BOOL _FVR_LPSetKey(void)
{
    ...
    if(fgCBC)
    {
        ...
        for(i=0; i<4; i++)
        {
            _DMX_SetAesIVEx2(i, au4IV, TRUE);
            _DMX_SetAesIVEx2(i, au4IV, FALSE);
    ...
    for(i=0; i<_u1DmxEmuTestPidNum; i++)
    {
        ...
        rFvrPid.eKeyType = 0; //(random(2) == 1) ? FVR_KEY_EVEN : FVR_KEY_ODD;
...
//-----------------------------------------------------------------------------
/** _DMX_EMU_PSILoopback_Test
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSILoopback_Test(UINT8 u1TsIdex, BOOL fgGenPat, DMX_FILTER_MODE_T eMode)
{    
    UINT32 i;
    BOOL fgRet = TRUE;
    UINT32 u4PidCount;
    ...
    VERIFY(_FVR_LPEnable(u1TsIdex, TRUE));
    ...
    VERIFY(_FVR_LPEnable(u1TsIdex, FALSE));
...
//-----------------------------------------------------------------------------
/** _DMX_EMU_PSICALoopback_Test
 */
//-----------------------------------------------------------------------------
BOOL _DMX_EMU_PSICALoopback_Test(UINT8 u1TsIdex, BOOL fgGenPat, DMX_FILTER_MODE_T eMode)
{    
    UINT32 i;
    BOOL fgRet = TRUE;
    UINT32 u4PidCount;
    ...
    // Set keys
    VERIFY(_DMX_SetAesKey(11, TRUE, au4EvenKey));
    VERIFY(_DMX_SetAesKey(11, FALSE, au4OddKey));
    VERIFY(_DMX_SetAesKeyLen(11, rParam.u2KeyLen));
    VERIFY(_DMX_SetCA_Mode(11, DMX_CA_MODE_AES, DMX_CA_FB_ECB, FALSE));
    _DMX_SetAesRtbMode(11, rParam.eRtb);
