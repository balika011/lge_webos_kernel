...
UINT8 _DMX_MUL_GetAvailableInst(DMX_CONN_TYPE_T eConnType)
{
    ...
    // if this is old IC and open two inst, show warning msg
    if(BSP_GetIcVersion() == IC_VER_5396_AA)
    {
        UINT8 u1Counter = 0, j;

        for(j = 0; j < DMX_MULTIPLE_INSTANCE_MAXNUM; j++)
        {
            if(_arDmxMulInstInfo[j].fgUsed == TRUE)
            {
                u1Counter++;
            }
        }
        ...
        if(u1Counter > 2)
        {
            LOG(0, "\n\n\n 5396 E1 IC only support for one instance\n Please use E4 IC\n\n\n");
        }
    }
...
BOOL _DMX_MUL_MoveData(UINT8 u1Inst, DMX_MM_DATA_T *prData)
{
    ...
    case DMX_IN_PLAYBACK_PS:
        ...
        if(!_DMX_SetVideoType(DMX_MM_MOVE_TSIDX, eVideoType))
        {
            return FALSE;
        }

        prData->u1Idx = u1Inst;
        if(!_DMX_SetFramerPktSize(DMX_MM_MOVE_TSIDX, 188))
        {
            return FALSE;
        }

        if(!_DMX_DDI_SetPacketSize(DMX_MM_MOVE_TSIDX, 188))
        {
            return FALSE;
        }
...
BOOL _DMX_MUL_SetFifoOutput(UINT8 u1Inst, DMX_PID_TYPE_T ePidType, UINT8 u1ChannelId, BOOL fgEnable)
{
    ...
    {
        LOG(6, "%s Only support audio and video type\n", __FUNCTION__);
        return FALSE;
    }
    ...
    if(i == DMX_NUM_PID_INDEX)
    {
        LOG(6, "%s can't find any matched pidx.\n", __FUNCTION__);
        return FALSE;
    }
