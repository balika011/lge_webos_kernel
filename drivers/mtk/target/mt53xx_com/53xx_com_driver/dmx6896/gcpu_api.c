	...
	case GCPU_TSSCRM:
        {
            ...
            rHwCmd.u4Param[GCPU_PARAM_PTR + 2] = (rLParam.eScheme << 28) | GCPU_PARAM_TSSCRM_EVEN;
            rHwCmd.u4Param[GCPU_PARAM_PTR + 3] = 0;

            rHwCmd.u4Param[GCPU_PARAM_TSSCRM_EVEN] = 0x4 + 0x0; // ECB mode + 128 bits

            for(i = 0; i < 4; i++)
            {
                p = rLParam.au1Key[0] + i * 4;
                rHwCmd.u4Param[GCPU_PARAM_TSSCRM_EVEN + 1 + i] = KEY_WRAPPER(p);
            }

            for(i = 0; i < 4; i++)
            {
                p = rLParam.au1Iv[0] + i * 4;
                rHwCmd.u4Param[GCPU_PARAM_TSSCRM_EVEN + 1 + 4 + i] = KEY_WRAPPER(p);
            }

            // odd key
            rHwCmd.u4Param[GCPU_PARAM_PTR + 4] = 0x00000047;
            rHwCmd.u4Param[GCPU_PARAM_PTR + 5] = 0x00000000;
            rHwCmd.u4Param[GCPU_PARAM_PTR + 6] = (rLParam.eScheme << 28) | GCPU_PARAM_TSSCRM_ODD;
            rHwCmd.u4Param[GCPU_PARAM_PTR + 7] = 0;

            rHwCmd.u4Param[GCPU_PARAM_TSSCRM_ODD] = 0x4 + 0x0; // ECB mode + 128 bits

            for(i = 0; i < 4; i++)
            {
                p = rLParam.au1Key[1] + i * 4;
                rHwCmd.u4Param[GCPU_PARAM_TSSCRM_ODD + 1 + i] = KEY_WRAPPER(p);
            }

            for(i = 0; i < 4; i++)
            {
                p = rLParam.au1Iv[1] + i * 4;
                rHwCmd.u4Param[GCPU_PARAM_TSSCRM_ODD + 1 + 4 + i] = KEY_WRAPPER(p);
            }
