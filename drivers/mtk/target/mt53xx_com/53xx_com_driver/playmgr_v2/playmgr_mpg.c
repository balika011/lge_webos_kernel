...
BOOL _PLAYMGR_OpenMpgMediaInfo(PLAYMGR_STATUS_T* prMediaStatus, VOID* pvMediaInfo)
{
        ...
        (prMediaInfo->rPlayerStatus.eMediaType == PLAYMGR_MEDIA_TYPE_MPEG2_PS))
    {
        ...
        if (prMediaInfo->u2VidStmNum > 0)
        {
            ...
            for(i=0;i<prMediaInfo->rPlayerStatus.u2VideoTracks;i++)
            {
                ...
                else if(prPsInfo->rStmAttribute[i].u.t_ps_video_stm_attr.e_enc == MINFO_INFO_VID_ENC_H265)
                {
                    prMediaInfo->rPlayerMpgInfo.rPgm[0].aeVideo[i] = ENUM_SWDMX_VDEC_H265;
                    //prMediaStatus->aeVidFormat[i] =
                    prMediaInfo->rPlayerStatus.aeVidFormat[i] = PLAYMGR_VDEC_H265;
                }
                else
                {
    ...
    else //if(1)
    {
        ...
        if ( prformat->e_media_type == MEDIA_TYPE_TS) //HEVC case
        {
            LOG(0, "HEVC slt open mpg\n");
            prMediaInfo->rPlayerMpgInfo.rPgm[0].au2VideoId[0] = 0x121;
            prMediaInfo->rPlayerMpgInfo.rPgm[0].aeVideo[0] = ENUM_SWDMX_VDEC_H265;            
        }
        else //AVS case
        {
            LOG(0, "AVS slt open mpg\n");
        ...
        if(prMediaInfo->rPlayerMpgInfo.rPgm[0].u2VideoNum > 0)
        {
            ...
            for(i=0;i<prMediaInfo->rPlayerMpgInfo.rPgm[0].u2VideoNum;i++)
            {
                ...
                else if(prMediaInfo->rPlayerMpgInfo.rPgm[0].aeVideo[i] == ENUM_SWDMX_VDEC_H265)
                {
                    //prMediaStatus->aeVidFormat[i] =
                    prMediaInfo->rPlayerStatus.aeVidFormat[i] = PLAYMGR_VDEC_H265;
                }				
