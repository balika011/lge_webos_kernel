...
INT32 VLDJPG_SetGlobDcHuffTbl		(JPEGHANDLE hInstance, UINT8 u1DcTblNo, const UINT8 *au1Bits, const UINT8 *au1Huffval)
{
	...
	if (u1DcTblNo < D_MAX_JPEG_HUFF_TBL)
	{
        ...
        VLDJPG_TransHuffTblToHwFormat((UINT8 *)VIRTUAL((UINT32)(INSTANCE->pu1HwDcHuffTbl + (u1DcTblNo * 256))), au1Huffval);
	}
	else
	{
...
INT32 VLDJPG_SetGlobAcHuffTbl		(JPEGHANDLE hInstance, UINT8 u1AcTblNo, const UINT8 *au1Bits, const UINT8 *au1Huffval)
{
	...
	if (u1AcTblNo < D_MAX_JPEG_HUFF_TBL)
	{
        ...
        VLDJPG_TransHuffTblToHwFormat((UINT8 *)VIRTUAL((UINT32)(INSTANCE->pu1HwAcHuffTbl + (u1AcTblNo * 256))), au1Huffval);
	}
	else
	{
		i4Ret = -(INT32)E_HWJPG_ERR_TBL_RANGE;
...
INT32 VLDJPG_ValidateGlobData		(JPEGHANDLE hInstance)
{
	...
	M_JPEG_HANDLE_VALIDATE();
		...
		if ((fgFlagsCheck & INSTANCE->fgGlobSet) != fgFlagsCheck)
		{
			...
			Printf("Glob validation failed : flags_set 0x%08lx flags_check 0x%08lx\n",
		...
		*/
		if ((INSTANCE->u1MaxBlkWidth < 1) || (INSTANCE->u1MaxBlkWidth > 4))
		{
...
INT32 VLDJPG_ValidateScanData		(JPEGHANDLE hInstance)
{
	...
	M_JPEG_HANDLE_VALIDATE();
		...
		*/
		...
		for (u1Count = 0; u1Count < D_MAX_JPEG_QTBL; u1Count++)
		{
			if (u1QtblAlloc & D_BIT(u1Count))
			{
				...
				if (u1QtblAlloc/*u1QtblChange*/ & D_BIT(D_QTBL_HW1))	// hardcoded due to only 2 hw entry
				{
					...
					if(VLDJPG_LoadTblQ(D_QTBL_HW1, INSTANCE->apu1Qtbl[u1Count]) != (INT32)E_HWJPG_OK)
					{
						LOG(0, "E_HWJPG_ERR_LOAD_Q error 1\n");
				...
				else if (u1QtblAlloc/*u1QtblChange*/ & (UINT8)D_BIT(D_QTBL_HW2))	// second entry
				{
					...
					if(VLDJPG_LoadTblQ(D_QTBL_HW2, INSTANCE->apu1Qtbl[u1Count]) != (INT32)E_HWJPG_OK)
					{
						LOG(0, "E_HWJPG_ERR_LOAD_Q error 2\n");						
						return -(INT32)E_HWJPG_ERR_LOAD_Q;						
					}						
				...
				else
				{
                                    if(u1QtblAlloc & D_BIT(2))
                                    {
                                        ...
                                        if(VLDJPG_LoadTblQ(D_QTBL_HW1, INSTANCE->apu1Qtbl[u1Count]) != (INT32)E_HWJPG_OK)
										{
											LOG(0, "E_HWJPG_ERR_LOAD_Q error 3\n");										
											return -(INT32)E_HWJPG_ERR_LOAD_Q;						
										}
                                        INSTANCE->afgQtblLoad[D_QTBL_HW1] = TRUE;
                                        INSTANCE->au1QtblLoadIndex[D_QTBL_HW1] = u1Count;
                                    ...
                                    else if(u1QtblAlloc & D_BIT(3))
                                    {
                                        ...
                                        if(VLDJPG_LoadTblQ(D_QTBL_HW2, INSTANCE->apu1Qtbl[u1Count]) != (INT32)E_HWJPG_OK)
										{
											LOG(0, "E_HWJPG_ERR_LOAD_Q error 4\n");										
											return -(INT32)E_HWJPG_ERR_LOAD_Q;						
										}
                                        INSTANCE->afgQtblLoad[D_QTBL_HW2] = TRUE;
                                        INSTANCE->au1QtblLoadIndex[D_QTBL_HW2] = u1Count;
		...
		{
			VERIFY(VLDJPG_SetVideoOutMode(TRUE, TRUE,INSTANCE->au1CompBlkWidth[0], INSTANCE->au1CompBlkWidth[1], INSTANCE->au1CompBlkHeight[0], INSTANCE->au1CompBlkHeight[1]) == (INT32)E_HWJPG_OK);
			VLDJPG_SetPicturePixelSize(INSTANCE->u4PicWidth);
		}
		#endif
        ...
        LOG(6, "R=%d, S=0x%x, Z=0x%x\n", INSTANCE->fgRingBuffer, INSTANCE->pu1RingBufStart, INSTANCE->u4RingBufferSize);
    ...
    UINT32 u4MCUsPerRow, UINT32 u4TotalRows)
{
	...
	M_JPEG_HANDLE_VALIDATE();
		...
		*/
...
#if defined(JPEG_VLD_V4) && (!defined(MJPEG_MCU_ROW_ENABLE))
        ...
        VLDJPG_SetPartialMode_StartRow((UINT32)INSTANCE->u4SkipMcuRowNum);
		#endif
...
INT32 VLDJPG_SetSkipMcuRowNum		(JPEGHANDLE hInstance, UINT32 u4SkipMcuRowNum)
{
	...
	M_JPEG_HANDLE_VALIDATE();
		INSTANCE->u4SkipMcuRowNum				= u4SkipMcuRowNum;
	M_JPEG_INTERFACE_END();
}
#endif
