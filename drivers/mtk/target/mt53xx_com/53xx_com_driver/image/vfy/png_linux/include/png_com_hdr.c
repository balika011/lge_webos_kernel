...
INT32 i4PngHdrPLTE(PNG_HDR_INFO_T *ptPngHdr)
{
    ...
    //skip chunk
    #if CONFIG_DRV_LINUX_DATA_CONSISTENCY && (!PNG_CACHE_MAP_ENABLE)
    BSP_FlushDCacheRange(ptPngHdr->tPngPlte.u4PlteData, ptPngHdr->tPngChunkInfo.u4ChunkSize);
    #endif
    vPngHdrSkipBytes(ptPngHdrFifo, u4ChunkTotalSize);
    return PNG_HDR_OK;
}
...
INT32 i4PngHdrtRNS(PNG_HDR_INFO_T *ptPngHdr)
{
    ...
    BSP_FlushDCacheRange(ptPngHdr->tPngTrns.u4TrnsData, ptPngHdr->tPngChunkInfo.u4ChunkSize);
    #endif
    vPngHdrSkipBytes(ptPngHdrFifo, u4ChunkTotalSize);
    return PNG_HDR_OK;
}
