...
UINT8 u1IsSPDFlag;
UINT8 u1SpdTest;
UINT8 u14KPsDetStatus;
#ifdef CC_COMBINE_4K_2K 	
UINT32 _wIsHDMI2KModel;
#endif
...
E_HDMI_SWITCH_NUM eActiveHdmiPort1 = HDMI_SWITCH_INIT;
...
#elif defined(CC_MT5396) || defined(CC_MT5398) || defined(CC_MT5399) || defined(CC_MT5890)
...
UINT32 u4GetHdmiRegBase(E_HDMI_SWITCH_NUM ePort)
{
...
#elif defined(CC_MT5396) || defined(CC_MT5398) || defined(CC_MT5880) || defined(CC_MT5399) || defined(CC_MT5890)
...
UINT32 u4GetAnalogRegBase(E_HDMI_SWITCH_NUM ePort)
{
...
#if HDMI_Audio_NewFlow
void vHDMIAudIntClear(void)
{
	if(IO32ReadFldAlign(INTR_STATE1 + u4ActiveHdmiRegBase, INTR5_FN_CHG) == 1)
	{
		vIO32Write4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR5_FN_CHG));
	}
	if(IO32ReadFldAlign(INTR_STATE1 + u4ActiveHdmiRegBase, INTR4_CTS_REUSED_ERR) == 1)
	{
		vIO32Write4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR4_CTS_REUSED_ERR));
	}
	if(IO32ReadFldAlign(INTR_STATE1 + u4ActiveHdmiRegBase, INTR4_CTS_DROPPED_ERR) == 1)
	{
		vIO32Write4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR4_CTS_DROPPED_ERR));
	}
	if(IO32ReadFldAlign(INTR_STATE1 + u4ActiveHdmiRegBase, INTR4_T4_PKT_ERR_ALERT) == 1)
	{
		vIO32Write4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR4_T4_PKT_ERR_ALERT));
	}
	if(IO32ReadFldAlign(INTR_STATE1 + u4ActiveHdmiRegBase, INTR5_BCH_AUDIO_ALERT) == 1)
	{
		vIO32Write4B(INTR_STATE1 + u4ActiveHdmiRegBase, Fld2Msk32(INTR5_BCH_AUDIO_ALERT));
	}
	// Clear interrupt
...
void vHDMIAudErrorHandler(void)
{
    ...
    if((_bHdmiMode == 0) || (_arHdmiRx[eActiveHdmiPort]._bHDMIState != HDMI_STATE_AUTH) || _bDviModeChged)
    {
		...
		#elif defined(CC_MT5398) || defined(CC_MT5880) || defined(CC_MT5399)
        ...
        if(fgHDMIAudFifoFault() || (eCurrHdmiPort != eActivePort))
        {
            ...
            if(IO32ReadFldAlign(INTR_STATE0 + u4ActiveHdmiRegBase, INTR2_NEW_AUD_PKT))
            {
                ...
                if(u1ACR_DPLL_LOCK > 1)    // no clock: 243MHz, normal 270 / 293 MHz.
                {
                    ...
                    {
                        vHDMIHandleAudFmtChange();
                        LOG(1,">>>[HDMI Aud]Sample Rate Changed,Re-Config!!!\n");
                    }
    ...
    {
		_bHdmiAudFreq = AUD_FS_48K;//AUD_FS_48K;   // default: 48k
    }
    else
    {
...
UINT8 bHDMIInputType(void)
{
    ...
    if(_bHdmiModeActive)
    {
        ...
        {
/**BEGIN: DTV00598876 George.yang 2014-7-31**/
...
#ifdef CC_HDMI_1_4_SUPPORT
    UINT16 u2HDMI2AnaBaseAddr;
#endif
    u4RegOffset = u4AnalogRegBase;
#ifdef CC_HDMI_1_4_SUPPORT

	if(eSrc == HDMI_SWITCH_4)
	{
	    u2HDMI2AnaBaseAddr = (UINT16)u4GetHdmi2AnalogRegBase(eSrc);
        vHDMI2ComAnaTermCtrl(u2HDMI2AnaBaseAddr,bOnOff);
	}
	else
#endif
	{
...
void vHDMIEQCIRCUITCTRL(E_HDMI_SWITCH_NUM ePort, UINT8 bOnOff)
{
	...
	if(bOnOff)
	{
		...
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7 + u4RegOffset, 1, RG_HDMI_0_PLL_EN);
...
//#ifndef __HDMI_SLT__
    else
    {
		...
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7 + u4RegOffset, 0, RG_HDMI_0_PLL_EN);		
...
UINT8 bHDMISampleRate44KHz(void)
{
...
void bHDMIAudioSigmaDeltaSetting(UINT32 value1, UINT8 value2, UINT8 bSDINOVR)
{
 ...
 //   vIO32WriteFldAlign(CTS_HW_ADP4 + u4ActiveHdmiRegBase, value2, SDINOVRVAL_39_32);
    ...
    //value1=0 means HDMI source, so get FS from HDMI driver
    //otherwise it is not HDMI source, FS refers to value1
    #if 1
    if((value1 == 0) && bHDMISampleRate44KHz() == 1)
    {
        value1 = 0x205bc01a;//add for giving the PCW the initial value ,when sample is changed between 44.1 and 32/48.
		uValFlg = 1;
    }
	#endif
    ...
    //    }
	  //  else if(IS_XTAL_24MHZ())
	    {
	        vIO32WriteFldAlign(DDS + u4HADDS2RegBase, 0x5A5119CE, REG_HADDS2_PCW_SW);
			#if 1
			if(uValFlg == 1)
			{
				uValFlg = 0;
    ...
    else
    {
        //48KHz
        //for DDS mode HDMI setting
        vIO32WriteFldAlign(DDS + u4HADDS2RegBase, 0x624DD2F1, REG_HADDS2_PCW_SW); //fix 27MHz reference clock
#if 0
        if(IS_XTAL_27MHZ())
        {
    ...
    vIO32WriteFldAlign(ACR_CTRL3 + u4HADDS2RegBase, 3, REG_CTRL_RESERVED);
...
#define TMDS_CLOCK_MIDDLE1 3
#define TMDS_CLOCK_MIDDLE2 4
#define TMDS_CLOCK_MIDDLE 5
#define TMDS_CLOCK_HIGH 6
...
static void vHDMIPhyBandCtrl(E_HDMI_SWITCH_NUM ePortl, UINT8 u1Val)
{
	...
	if((_bIsMhlDevice == 1) && (ePortl == HDMI_SWITCH_1))
	{
		...
		//vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset ,1, RG_HDMI_0_ZSEN_EN);	
		if(_bIsMhlPPMode == 1)
		{
			vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, 0, RG_HDMI_0_MHLMODE);
			vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,2, RG_HDMI_0_PLL_DIVCTRL);
		...
		else
		{
			...
			vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,3, RG_HDMI_0_PLL_DIVCTRL);
		...
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset ,1, RG_HDMI_0_TERM_MHL_SEL);
		//vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset ,0, RG_HDMI_0_TERM_CM_ENB);
	...
	else
	{
		...
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9 + u4RegOffset, 0, RG_HDMI_0_PLL_DIVCTRL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset ,0, RG_HDMI_0_TERM_MHL_SEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, 1, RG_HDMI_0_TERM_CM_ENB);//open DIP and DIN
	...
	if(_bIsMhlDevice == 1)
	{
		...
		vIO32WriteFldAlign(MHL_REG, 1, REG_MHL_CLK_INV);
	}
	else
	{
		...
		vIO32WriteFldAlign(MHL_REG, 0, REG_MHL_CLK_INV);
	}
    ...
    switch(u1Val)
    {
			case TMDS_CLOCK_LOW1:  // < 30MHz
				...
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0x1f, RG_HDMI_0_PLL_VCO_BAND);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf0, RG_HDMI_0_PLL_RESERVE);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_IC);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,4,    RG_HDMI_0_PLL_IR);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1,    RG_HDMI_0_PLL_BC);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,2,    RG_HDMI_0_PLL_BR);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0xf,  RG_HDMI_0_PLL_BP);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,2,    RG_HDMI_0_PLL_KVSEL);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1,    RG_HDMI_0_PLL_AUTOK_KS);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1,    RG_HDMI_0_PLL_AUTOK_KF);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0,    RG_HDMI_0_PLL_RST_DLY);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0,    RG_HDMI_0_PLL_BCA);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1, RG_HDMI_0_PLL_AUTOK_EN);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1, RG_HDMI_0_PLL_AUTOK_LOAD);
                 ...
                 vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, _bRXMode, RG_HDMI_0_RXMODE);
   				 vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, 3, RG_HDMI_0_EQ_GAIN);
			...
			case TMDS_CLOCK_LOW2:  // 30 ~ 40MHz
				if((_bIsMhlDevice == 1) && (ePortl == HDMI_SWITCH_1))
				{
					...
					if(_bIsMhlPPMode == 1)
					{
						...
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_IC);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,1, RG_HDMI_0_PLL_VCO_BAND);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,2, RG_HDMI_0_PLL_DIVCTRL);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf0, RG_HDMI_0_PLL_RESERVE);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0x27, RG_HDMI_0_PLL_FBKDIV);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,5,    RG_HDMI_0_PLL_IR);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,3, RG_HDMI_0_PLL_AUTOK_KS);
				        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,3, RG_HDMI_0_PLL_AUTOK_KF);
				        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,2, RG_HDMI_0_PLL_RST_DLY);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0,    RG_HDMI_0_PLL_BCA);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3,    RG_HDMI_0_PLL_BC);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3,    RG_HDMI_0_PLL_BR);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0xf,  RG_HDMI_0_PLL_BP);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,1,    RG_HDMI_0_PLL_KVSEL);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1, RG_HDMI_0_PLL_AUTOK_KS);
				        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1, RG_HDMI_0_PLL_AUTOK_KF);
				        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0, RG_HDMI_0_PLL_RST_DLY);
					}
					...
					{
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2  + u4RegOffset, 3, RG_HDMI_0_EQBAND);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset, 3, RG_HDMI_0_PLL_VCO_BAND);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset, 3, RG_HDMI_0_PLL_DIVCTRL);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf0, RG_HDMI_0_PLL_RESERVE);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0x1d, RG_HDMI_0_PLL_FBKDIV);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, 	  RG_HDMI_0_PLL_IC);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,5,    RG_HDMI_0_PLL_IR);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3,    RG_HDMI_0_PLL_BC);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3,    RG_HDMI_0_PLL_BR);
						...
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,1,    RG_HDMI_0_PLL_KVSEL);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0,    RG_HDMI_0_PLL_BCA);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,2, RG_HDMI_0_PLL_AUTOK_KS);
				        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,2, RG_HDMI_0_PLL_AUTOK_KF);
				        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1, RG_HDMI_0_PLL_RST_DLY);
				...
				else
				{
					...
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0, RG_HDMI_0_PLL_DIVCTRL);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf0, RG_HDMI_0_PLL_RESERVE);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,9, RG_HDMI_0_PLL_FBKDIV);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_IC);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,4, RG_HDMI_0_PLL_IR);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3, RG_HDMI_0_PLL_BC);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,2, RG_HDMI_0_PLL_BR);
			...
			case TMDS_CLOCK_MIDDLE1:
				...
				else
				{
				 ...
				 vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_EN);
				 vUtDelay2us(100);
				 if((_bIsMhlDevice == 1) && (ePortl == HDMI_SWITCH_1) && (!_bIsMhlPPMode))
				 {
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7 + u4RegOffset, 1, RG_HDMI_0_PLL_POSDIV);
				 }
				break;
			case TMDS_CLOCK_MIDDLE2:
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2  + u4RegOffset, 0, RG_HDMI_0_EQBAND);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset, 1, RG_HDMI_0_PLL_VCO_BAND);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset, 3, RG_HDMI_0_PLL_DIVCTRL);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf0, RG_HDMI_0_PLL_RESERVE);
			...
			case TMDS_CLOCK_MIDDLE:  // 40 ~ 160MHz
				...
				if((_bIsMhlDevice == 1) && (ePortl == HDMI_SWITCH_1))
				{
					...
					{
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2  + u4RegOffset, 0,   RG_HDMI_0_EQBAND);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset, 0, RG_HDMI_0_PLL_VCO_BAND);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_IC);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset, 2, RG_HDMI_0_PLL_DIVCTRL);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0x27, RG_HDMI_0_PLL_FBKDIV);
						vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf0, RG_HDMI_0_PLL_RESERVE);
				...
				{
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4RegOffset, 2, RG_HDMI_0_EQBAND);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0x1, RG_HDMI_0_PLL_VCO_BAND);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0, RG_HDMI_0_PLL_DIVCTRL);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xf3, RG_HDMI_0_PLL_RESERVE);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_IC);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,6, RG_HDMI_0_PLL_IR);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0, RG_HDMI_0_PLL_BC);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3, RG_HDMI_0_PLL_BR);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,3, RG_HDMI_0_PLL_BP);
					vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,1, RG_HDMI_0_PLL_KVSEL);
				}
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0, RG_HDMI_0_PLL_BCA);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,3, RG_HDMI_0_PLL_AUTOK_KS);
				...
				//vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_EN);
				 ...
				 if(_bIsMhlPPMode ==0)
				 {
				 	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,2, RG_HDMI_0_PLL_POSDIV);
				 }
			...
			case TMDS_CLOCK_HIGH:  // > 160MHz
				...
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0, RG_HDMI_0_PLL_VCO_BAND);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4RegOffset,0, RG_HDMI_0_PLL_DIVCTRL);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,0xfa, RG_HDMI_0_PLL_RESERVE);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,9, RG_HDMI_0_PLL_FBKDIV);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,1, RG_HDMI_0_PLL_IC);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,5, RG_HDMI_0_PLL_IR);
				vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4RegOffset,0, RG_HDMI_0_PLL_BC);
	...
	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4RegOffset,1, RG_HDMI_0_PLL_MONCK_EN);
	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG3  + u4RegOffset,0x410, RG_HDMI_0_FM_SEL);

	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG6  + u4RegOffset,0, RG_HDMI_0_REV);
    ...
    bCH0EQ = IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGA + u4RegOffset,RGS_HDMI_0_CH0_EQERR);
...
#ifdef CC_HDMI_1_4_SUPPORT
HdmiTmdsClkRate_t bHDMI2GetClkRate(void)
{
	UINT8 u1Val=0;
...
void vHDMI2AnaSetBand(E_HDMI_SWITCH_NUM ePort, HdmiTmdsClkRate_t eBand, BOOL b40xMode)
{
    HDMI2ANA_LEQ_ITEM tLEQItem;
    UINT16 u2HDMI2AnaBaseAddr;
    u2HDMI2AnaBaseAddr = (UINT16)u4GetHdmi2AnalogRegBase(ePort);
    ...
    vHDMI2ComAnaEQBandSet(u2HDMI2AnaBaseAddr,eBand,b40xMode);
    vHDMI2ComAnaPLLSet(u2HDMI2AnaBaseAddr,eBand,b40xMode);
    vHDMI2ComAnaDeepPLLSet(u2HDMI2AnaBaseAddr,eBand,b40xMode);
    
    vHDMI2ComAnaPLLEnable(u2HDMI2AnaBaseAddr);
    vHDMI2ComAnaEQMacroEnable(u2HDMI2AnaBaseAddr);

	vHDMI2ComAnaSAOSC(u2HDMI2AnaBaseAddr,FALSE);
    vHDMI2ComAnaEQOSC(u2HDMI2AnaBaseAddr,FALSE);
    
    vHDMI2ComAnaHalfRateSet(u2HDMI2AnaBaseAddr,b40xMode);

    vHDMI2ComAnaCDRLock(u2HDMI2AnaBaseAddr);

    vHDMI2ComAnaEQLock(u2HDMI2AnaBaseAddr,b40xMode,&tLEQItem,FALSE);
    if(b40xMode)
    {
        vHDMI2ComAnaEQIncreaseEnable(u2HDMI2AnaBaseAddr,FALSE);
        if(bHDMI2ComAnaAdjustRatioSet(u2HDMI2AnaBaseAddr))
        {
            vHDMI2ComAnaAdjustEnable(u2HDMI2AnaBaseAddr);
        }
        else
        {
            LOG(0, "Adjust ratio invalid!\n");
        }
    }
    vHDMI2ComAnaDeepPLLEnable(u2HDMI2AnaBaseAddr);
    
    return;
}

void vHDMI2BandCtrl(E_HDMI_SWITCH_NUM ePort)
{
	HdmiTmdsClkRate_t eBand = HDMI_TMDS_CLOCK_UNKNOW;
	
    eBand = bHDMI2GetClkRate();
	
	if(u1HdmiGetTmdsClkRate(ePort) != eBand)
	{
		vHdmiSaveTmdsClkRate(ePort, eBand);
		vHDMI2AnaSetBand(ePort,eBand,0);
		bHdmiPhyReset(ePort,HDMI_RST_DIGITALPHY);
	}
}
#endif
/**
...
static void vHdmiPhySetting(E_HDMI_SWITCH_NUM ePort)
{
	...
	#ifdef CC_HDMI_1_4_SUPPORT
	if(ePort == HDMI_SWITCH_4)
	{
		vHDMI2BandCtrl(ePort);
    ...
    if(IS_XTAL_24MHZ())
   	{
		if((_bIsMhlDevice == 1) && (ePort == HDMI_SWITCH_1))
		{
			if(_bIsMhlPPMode == 0)//noormal mode
			{
				...
				{
					u1ClkRate = TMDS_CLOCK_MIDDLE1;
				}
				else if(u1Val > 0x17)
				{
					u1ClkRate = TMDS_CLOCK_MIDDLE2;
				}
				else
				{
...
void bHdmi2PhyReset(E_HDMI_SWITCH_NUM ePort,UINT8 resettype)
{
	HDMI2ANA_LEQ_ITEM tLEQItem;
	UINT32 u4TmpHdmiRegBase = 0;
    UINT16 u2HDMI2AnaBaseAddr;
	...
	if(resettype == HDMI_RST_ALL)
	{
		...
		vHDMI2ComAnaEQMacroEnable(u2HDMI2AnaBaseAddr);
	    vHDMI2ComAnaEQOSC(u2HDMI2AnaBaseAddr,FALSE);
	    vHDMI2ComAnaHalfRateSet(u2HDMI2AnaBaseAddr,0);
	    vHDMI2ComAnaCDRLock(u2HDMI2AnaBaseAddr);
	    vHDMI2ComAnaEQLock(u2HDMI2AnaBaseAddr,0,&tLEQItem,FALSE);
	    vHDMI2ComAnaDeepPLLEnable(u2HDMI2AnaBaseAddr);
...
void bHdmiPhyReset(E_HDMI_SWITCH_NUM ePort, UINT8 resettype)
{
    ...
    if(resettype == HDMI_RST_ALL)
    {
    	...
    	if(ePort == HDMI_SWITCH_4)
    	{
    		bHdmi2PhyReset(HDMI_SWITCH_4,HDMI_RST_ALL);
			return;
    	}
	#endif
        bCH0EQ = IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGA + u4RegOffset,RGS_HDMI_0_CH0_EQERR);
    ...
    if(resettype == HDMI_RST_EQ)
    {
    	...
    	if(ePort == HDMI_SWITCH_4)
    	{
    		bHdmi2PhyReset(HDMI_SWITCH_4,HDMI_RST_EQ);
			return;
    	}
	#endif	
        bCH0EQ = IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGA + u4RegOffset,RGS_HDMI_0_CH0_EQERR);
...
static void vHdmiEqTracking(E_HDMI_SWITCH_NUM ePort)
{
    ...
    if((IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGA + u4AnalogRegBase,RGS_HDMI_0_CH0_STATUS)&0x100) == 0x100)
    {
        bEQCH0_tmp = IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGA + u4AnalogRegBase, RGS_HDMI_0_CH0_EQERR);
        bEQCH1_tmp = IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGB + u4AnalogRegBase, RGS_HDMI_0_CH1_EQERR);
        bEQCH2_tmp = IO32ReadFldAlign(RGS_HDMI_RX_Normal_0_CFGC + u4AnalogRegBase, RGS_HDMI_0_CH2_EQERR);
...
#if defined(CC_MT5396) || defined(CC_MT5398) || defined(CC_MT5880) || defined(CC_MT5399) || defined(CC_MT5890)
...
void vHDMIInit(void)
{
	...
	E_HDMI_SWITCH_NUM eHdmiPortMax = HDMI_SWITCH_MAX;
    UINT32 u4RegBase = 0;
    UINT32 u4AnaRegBase = 0;
	UINT8 u1term = 0;
#ifdef CC_HDMI_1_4_SUPPORT
    UINT16 u2HDMI2AnaRegBase;
#endif
...
#if defined(CC_HDMI_2_0_SUPPORT)
#if defined(CC_HDMI_OLD_NEW_SUPPORT) || defined(CC_HDMI_1_4_SUPPORT)
	eHdmiPortMax = HDMI_SWITCH_MAX;
#else 
	eHdmiPortMax = u1GetMinHDMI20Port();
#endif
#endif
    ///////////////////////////////////////////////////////
    // 2. Software Initial

    for(eHdmiPort = HDMI_SWITCH_1 ; eHdmiPort < eHdmiPortMax; eHdmiPort++)
    {
	...
	u1IsSPDFlag = 0;
	u1SpdTest = 0;
	memset(&u4SpdData,0,sizeof(HDMI_SPD_DATA_STRUCT));
#ifdef CC_Support_Sil9285
...
#ifdef CC_COMBINE_4K_2K 	
    UNUSED(DRVCUST_OptQuery(eGet2KModelSupport, &_wIsHDMI2KModel));
#endif	
    ...
    for(eHdmiPort = HDMI_SWITCH_1; eHdmiPort < eHdmiPortMax; eHdmiPort++)
    {
        ...
        /*analog setting*/
		//vIO32WriteFldAlign(USBPHYACR0,1,RG_USB20_BGR_EN);  //analog current

#if defined(CC_HDMI_2_0_SUPPORT)
    if(eHdmiPort < u1GetMinHDMI20Port())     
#endif
    {
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 1, RG_HDMI_0_CDR_RST);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_CDR_STOP);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_ATEST_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_ANAMON);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_CH0_CDRLPI);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_CH1_CDRLPI);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_CH2_CDRLPI);

		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 1, RG_HDMI_0_CKDT_AEN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_CKDT_SET);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 2, RG_HDMI_0_HYBIAS);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 1, RG_HDMI_0_CLKBUF_EN);
        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 1, RG_HDMI_0_DATA_RST);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_EQ_PD);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 2, RG_HDMI_0_D2SBIAS);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_EQ_FILT);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_EQ_FIL_SEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_EQ_PRBSCK_SEL); //change from 1 to 0
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 1, RG_HDMI_0_EQ_SWRSTSEL);   //add
        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 1, RG_HDMI_0_EQ_RST);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 2, RG_HDMI_0_HY);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_EQ_PATTERN_SEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 2, RG_HDMI_0_EQBIAS);      //add 
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 1, RG_HDMI_0_FULL_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 2, RG_HDMI_0_VREF);

		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_MHLEN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_MHLMODE);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_FTMODE);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_PI_EN);
        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_R2R_CK_SEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 3, RG_HDMI_0_KPGAIN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_PLLCK_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_RXAFE_EN_SW);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_RXAFECH0_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_RXAFECH1_EN);
        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_RXAFECH2_EN);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_TERM_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_ZSEN_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_TERM_MODE);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_RXMODE);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_EQBAND);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_TEST_SEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 3, RG_HDMI_0_EQ_GAIN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_TERM_MHL_SEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_TERM_CM_ENB);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_DEEPCLRCLK_PDB);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 0, RG_HDMI_0_DEEPCLRCLK_RSTN);
		
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4AnaRegBase,0, RG_HDMI_0_PLL_VCO_BAND);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4AnaRegBase,0, RG_HDMI_0_PLL_DIVCTRL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,0xfa, RG_HDMI_0_PLL_RESERVE);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,9, RG_HDMI_0_PLL_FBKDIV);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,1, RG_HDMI_0_PLL_IC);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,5, RG_HDMI_0_PLL_IR);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,0, RG_HDMI_0_PLL_BC);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,3, RG_HDMI_0_PLL_BR);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,3, RG_HDMI_0_PLL_BP);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4AnaRegBase,1, RG_HDMI_0_PLL_KVSEL);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG9  + u4AnaRegBase,0, RG_HDMI_0_PLL_BCA);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,3, RG_HDMI_0_PLL_AUTOK_KS);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,3, RG_HDMI_0_PLL_AUTOK_KF);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,2, RG_HDMI_0_PLL_RST_DLY);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,1, RG_HDMI_0_PLL_AUTOK_EN);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,1, RG_HDMI_0_PLL_AUTOK_LOAD);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,1, RG_HDMI_0_PLL_EN);
		vUtDelay2us(75);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4AnaRegBase,2, RG_HDMI_0_PLL_POSDIV);	 //change from CFG8 to CFG7
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,1, RG_HDMI_0_PLL_MONCK_EN);		
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG8  + u4AnaRegBase,1, RG_HDMI_0_PLL_MONREF_EN);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG3  + u4AnaRegBase,0x410, RG_HDMI_0_FM_SEL);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG6  + u4AnaRegBase,0, RG_HDMI_0_REV);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG0 + u4AnaRegBase, 0, RG_HDMI_0_CDR_RST);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_EQ_RST);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2 + u4AnaRegBase, 1, RG_HDMI_0_DEEPCLRCLK_RSTN);
	    vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4AnaRegBase, 0, RG_HDMI_0_DATA_RST);
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG5 + u4AnaRegBase ,0,RG_HDMI_0_EQ_CALEN);
		//u1HDMIEQOffsetCali(eHdmiPort);
        // pll auto reset
        vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG4 + u4AnaRegBase, 1, RG_HDMI_0_PLL_DETEN);
        vHDMIPhyBandCtrl(eHdmiPort, TMDS_CLOCK_LOW2);
    }
        /*TMDS setting*/
        vIO32Write4B(TMDS_CTRL0 + u4RegBase, 0x00000000);//reset TMDS PHY digital part
        vIO32Write4B(TMDS_CTRL0 + u4RegBase, 0x07000002);//invert CH 1,2,3 and TMDS_PHY normal operation.
        vIO32WriteFldAlign(TMDS_CTRL0 + u4RegBase, 1, C_DATA_SYNC_AUTO);//In the end, put on the auto reset the PHY_FIFO_read_pointer
         ...
         */
        vIO32Write1B(ACR_CTRL3_0 + u4RegBase, 0x5C);             //Enable MCLK loop_back; CTS's minimue B
#if defined(CC_MT5389) || defined(CC_MT5398) || defined(CC_MT5880) || defined(CC_MT5399) || defined(CC_MT5890) //CC_AUD_PLL_HALF
        ...
        /*LKTHRESH
          PLL Lock Stability Threshold  [19:0]
         */
        ...
        /**
		 **   Audio out I2S:
		 **   INVALID_EN[23]: 0 Send valid data only.
		 **   CLK_EDGE[22]:   1 Sample edge is falling
		 **   SIZE[21]:            0 32-bit word size
		 **   MSB[20]:            0 Enable most-significant bit.
		 **   WS[19]:              1 Left polarity When Word Select is high
		 **   JUSTIFY[18]:       0 Data is left justify.
		 **   DATA_DIR[17]:    0 MSB First.
		 **   1ST_BIT[16]:       1 No shift.
		 **/
        ...
        vIO32WriteFldAlign(PDWNC_PADCFG0, 1, FLD_HDMI_0_SDA_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG1, 1, FLD_HDMI_0_SCL_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG1, 1, FLD_HDMI_1_SDA_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG2, 1, FLD_HDMI_1_SCL_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG2, 1, FLD_HDMI_2_SDA_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG3, 1, FLD_HDMI_2_SCL_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG3, 1, FLD_HDMI_3_SDA_SMT);
        vIO32WriteFldAlign(PDWNC_PADCFG4, 1, FLD_HDMI_3_SCL_SMT);
...
#ifdef CC_HDMI_1_4_SUPPORT
    u2HDMI2AnaRegBase = (UINT16)u4GetHdmi2AnalogRegBase(HDMI_SWITCH_4);
	vHDMI2ComAnaInit(u2HDMI2AnaRegBase);
#endif
...
u1HDMI_SetEdidData(_au1Edid, sizeof(_au1Edid));
	...
	vIO32WriteFldAlign(MHL_REG, u1term, REG_HDMI_IMPSEL);
...
/*If the IC has ECO version ,you must check this setting*/
	if(IS_IC_5861() || (BSP_GetIcVersion() >= IC_VER_5890_AC))//MT5861 the MHL_SENSE pin should pull down 75K.
	{
		vIO32WriteFldAlign(PDWNC_PADCFG0,1,FLD_MHL_SENCE_PD);//pull down 75K to ground.
		//5861 MHL_SENSE can't not drop the voltage to 0 because of the external circuit canceled.

		vIO32WriteFldAlign(PDWNC_PADCFG17,1,FLD_CBUS_EX1K);//HDMI Mode default 
		vIO32WriteFldAlign(PDWNC_PDMISC,0,FLD_HDMI1_MHL_HDCP_SEL);//HDCP default HDMI Mode 
	}
...
PRIVATE void vHDMIVideoHdmiSetting(void)
{
    ...
    if(u1IO32Read1B(AVIRX0_0 + u4ActiveHdmiRegBase) != 0x82)
    {
        ...
        vIO32Write1B(VID_MODE_3 + u4ActiveHdmiRegBase, 0x10);//B
        vIO32Write1B(VID_BLANK_0 + u4ActiveHdmiRegBase, 0x10);//G
        vIO32Write1B(VID_BLANK_1 + u4ActiveHdmiRegBase, 0x10);//R
        return;
    }
    ...
    */
    ...
    else // YCbCR
    {
        ...
        vIO32Write1B(VID_BLANK_0 + u4ActiveHdmiRegBase, 0x10);// Y
    }
...
void vHdmiVsdbInfoCheck(void)
{
    if (1 == IO32ReadFldAlign(INTR_STATE0 + u4ActiveHdmiRegBase, INTR3_NEW_UNREC))
    {
		if((u1IO32Read1B(UNRECRX0_0 + u4ActiveHdmiRegBase) == 0x81))
		{
			...
			|| ((u1IO32Read1B(UNRECRX1_3 + u4ActiveHdmiRegBase) & 0x1) == 0x1))
			{
				u1VsdbInfo = 1;
				u1NoVsdbCnt = 0;
			}
		}
		 vIO32Write4BMsk(INTR_STATE0 + u4ActiveHdmiRegBase, 1,Fld2Msk32(INTR3_NEW_UNREC));
    }
...
void  vHDMIGetInfoFrame(UINT8 frame)
{
    ...
    for(i = 0; i < num; i++)
    {
        if(i < 10)
        {
            wHDMIInfoFrame[i] = u4IO32Read4B(startaddr + u4ActiveHdmiRegBase + (i * 4));
...
void vHDMIGetSPDData(void)
{
	...
	if(IO32ReadFldAlign(INTR_STATE0 + u4ActiveHdmiRegBase, INTR3_NEW_SPD) == 1)
	{
		...
		vIO32Write4BMsk(INTR_STATE0 + u4ActiveHdmiRegBase, 1,Fld2Msk32(INTR3_NEW_SPD));
	}
...
UINT8 u1HdmiGetUPStaChg(void)
{
	...
	if(_bHdmiMode && u1IsSPDFlag)
	{
		if((u1UPStaOld == HDMI_SPD_UP_UNDET) && (u1UPStaNew == HDMI_SPD_UP_DETED))
		{
			u1UPStaOld = u1UPStaNew;
			return HDMI_SPD_CHG_TOUP;
		}
		else if((u1UPStaOld == HDMI_SPD_UP_DETED) && (u1UPStaNew == HDMI_SPD_UP_UNDET))
		{
			u1UPStaOld = u1UPStaNew;
...
UINT8 u1HdmiGetSVStaChg(void)
{
	...
	if(_bHdmiMode && u1IsSPDFlag )
	{
		if((u1SVStaOld == HDMI_SPD_SV_UNDET) && (u1UPStaNew == HDMI_SPD_SV_DETED))
		{
			u1SVStaOld = u1SVStaNew;
			return HDMI_SPD_CHG_TOSV;
		}
		else if((u1SVStaOld == HDMI_SPD_SV_DETED) && (u1SVStaNew == HDMI_SPD_SV_UNDET))
		{
			u1SVStaOld = u1SVStaNew;
...
UINT8 u1HDMI_Get4KPStatus(void)
{
	...
	static UINT8 u1Is4KPs = 0;
	static UINT8 u1No4KPsCnt = 0;
	...
	if(IO32ReadFldAlign(INTR_STATE0 + u4ActiveHdmiRegBase, INTR3_NEW_SPD) && (u1IO32Read1B(SPDRX0_0 + u4ActiveHdmiRegBase) == 0x83))
	{
		...
		if((u1VendorName == SV_PATTERN_SCEI) &&((u1ProductDescription == SV_PATTERN_SCEI_PS3D) || (u1ProductDescription == SV_PATTERN_SCEI_PS3P)))
		{
			u1Is4KPs =1;
			u1No4KPsCnt = 0;
		}
...
static UINT16 _wHdmiOldHtotal;
static UINT16 _wHdmiOldVtotal;
static UINT8 u1HdmiResolutionUnStableCnt=0;
static UINT8 u1HdmiResolutionStableCnt=0;
static UINT8 u1HdmiResolutionZeroCnt=0;
static UINT8 u1HdmiResolutionStableFlg=0;
void vHDMIMainLoop(void)
{
	...
	UINT16 _wHdmiNewHtotal;
	UINT16 _wHdmiNewVtotal;
    ...
    for(eHdmiPort = HDMI_SWITCH_1 ; eHdmiPort < HDMI_SWITCH_MAX ; eHdmiPort++)
    {
		...
		if((eHdmiPort >= u1GetMinHDMI20Port()) && (fgHDMI2GetDuleMode((HdmiRxPort)eActiveHdmiPort)))
#else
		if(eHdmiPort >= u1GetMinHDMI20Port())
#endif
		{
    ...
    else
    {
        ...
        {
        #ifdef CECNFY5VSTATUS
			if(_arHdmiRx[eHdmiPort]._bNonStdDev != 1)//non-standard device no 5V
			{
			...
			{
				_Cec_Notify5VStatus(eHdmiPort,0);
				_arHdmiRx[eHdmiPort]._u1Tx5VActive = 0;//non-standard device set to 0
			}
		#endif
        if (_arHdmiRx[eHdmiPort]._bHDMIState == HDMI_STATE_PWOFF)
        {
		...
		#ifdef CC_HDMI_1_4_SUPPORT
            if(u1IsHdmiEqTracking(eHdmiPort) &&(eHdmiPort != HDMI_SWITCH_4))
		#else
		...
		if(eActiveHdmiPort == HDMI_SWITCH_1)
		{
        ...
        switch(_arHdmiRx[eHdmiPort]._bHDMIState)
        {
...
#endif
				u1HdmiResolutionZeroCnt =0;
				u1HdmiResolutionUnStableCnt =0;
				u1HdmiResolutionStableFlg =0;
#if HDMI_DEBUG
            ...
            case HDMI_STATE_SCDT:
				            ...
				            RegReadFldAlign(SRST + u4HdmiRegBase, CKDT))
                {
					...
					_wHdmiNewHtotal = wHDMIHTotal();
					_wHdmiNewVtotal = wHDMIVTotal();

					if(((_wHdmiNewHtotal <= (_wHdmiOldHtotal + 2)) && (_wHdmiNewHtotal >= (_wHdmiOldHtotal - 2)))
					  && ((_wHdmiNewVtotal <= (_wHdmiOldVtotal + 2)) && (_wHdmiNewVtotal >= (_wHdmiOldVtotal - 2))))
					{
						u1HdmiResolutionUnStableCnt =0;
						u1HdmiResolutionStableCnt ++;
					}
					else
					{
						u1HdmiResolutionUnStableCnt ++;
						u1HdmiResolutionStableCnt =0;
						u1HdmiResolutionStableFlg =0;
					}

					if((_wHdmiNewHtotal ==0) &&(_wHdmiNewVtotal ==0))
					{
						u1HdmiResolutionZeroCnt ++;
						u1HdmiResolutionStableCnt =0;
						u1HdmiResolutionStableFlg =0;
					}
					else
					...
					if((u1HdmiResolutionZeroCnt >=5) || (u1HdmiResolutionUnStableCnt >=5))
					{
						u1HdmiResolutionStableFlg=0;
						u1HdmiResolutionZeroCnt =0;
						u1HdmiResolutionUnStableCnt =0;
						vHDMIResetPLL(eHdmiPort);
						bHdmiPhyReset(eHdmiPort,HDMI_RST_ALL);
					}
					if(u1HdmiResolutionStableCnt >= 5)
					{
						u1HdmiResolutionStableFlg =1;
						u1HdmiResolutionStableCnt =0;
					}

					if(u1HdmiResolutionStableFlg)
					{
					...
					_wHdmiOldHtotal = _wHdmiNewHtotal;
					_wHdmiOldVtotal = _wHdmiNewVtotal;
                }
                else
                {
                    if(RegReadFldAlign(SRST + u4HdmiRegBase, CKDT))
                	{
                		vHDMIResetPLL(eHdmiPort);
                	}
            ...
            case HDMI_STATE_AUTH:
	         ...
	         if (eHdmiPort == eActiveHdmiPort)
                ...
                {
                #ifdef CC_HDMI_OLD_NEW_SUPPORT
				    if((eActiveHdmiPort == HDMI_SWITCH_4)&& (u1HDMI2GetDviModeCnt(eActiveHdmiPort) == 0))
				    {
				    	vHDMI2ComDdcSel(0,1); 
						vHDMI2ComNewDigSel(0,1);
						LOG(1,"________new digital drv_hdmi.c___________\n");
				    }
				#endif
                ...
                else
                {
					...
					if(eHdmiPort != eActiveHdmiPort)
					{
						vHdmiUnMute(eHdmiPort);  // modify for w2d feature,phenomenon is one half video is  mute
					}
                }
                ...
                {
                 #ifdef CC_HDMI_2_0_SUPPORT
                	if(eActiveHdmiPort != HDMI_SWITCH_4)
				    {
				    	vHDMI2ComDdcSel(0,0); 
						vHDMI2ComNewDigSel(0,0);
						LOG(1,"________old digital drv_hdmi.c___________\n");
...
BOOL fgHDMICRCMulti(INT16 ntry)
{
    ...
    while (ntry > 0)
    {
		...
		for ( i = 0 ; i < 3 ; i++)// mustang HDMI1 address +0xc00, no need to crc			
#else
		for ( i = 0 ; i < 4 ; i++)
#endif
       {
...
#endif
   {
        ...
        {
             LOG(0, "[Port%d]interlace signal %d\n",(4 - i),i);
        }
        else
        {
             LOG(0, "[Port%d]progressive signal %d\n",(4 - i),i);
        }

        if (idx[i] == 1)
        {
			LOG(0, "[Port%d]assume progressive signal %d\n",(4 - i),i);
			LOG(0, "[Port%d]CRC result %d:\n",(4 - i),i);
			LOG(0, "[Port%d]%x %x %x\n", (4 - i),result[i][0][0], result[i][0][1], result[i][0][2]);
        }
        else if (idx[i] == 2)
        {
            LOG(0, "[Port%d]assume interlaced signal %d\n",(4 - i),i);
            LOG(0, "[Port%d]CRC result %d:\n",(4 - i),i);
            LOG(0, "[Port%d]%x %x %x\n",(4 - i),result[i][0][0], result[i][0][1], result[i][0][2]);
            LOG(0, "[Port%d]%x %x %x\n",(4 - i),result[i][1][0], result[i][1][1], result[i][1][2]);
        }
    }
			...
			for ( i = 1 ; i < 3 ; i++)
#elif defined(CC_MT5399) || defined(CC_MT5890)
			for ( i = 0 ; i < 2 ; i++)// mustang HDMI1 address +0xc00, no need to crc
#else
			for ( i = 0 ; i < 3 ; i++)
#endif
    {
...
BOOL fgHDMICRC(INT16 ntry)
{
	...
	{
		ntry--;
		vUtDelay10ms(2); // NOTE: IT IS NECESSARY,the interlaced time is need to be enough
		...
		vIO32Write1B(VID_CRC_CHK_2 + u4ActiveHdmiRegBase, 0x8c);// clr,for the clearing of the CRC Status bit.If the value is 0, indicate the CRC Ready.
																//CRC Mode is 3, VSYNC
		LOG(3, "CRC clear status\n");

		while(IO32ReadFldAlign(VID_CRC_CHK + u4ActiveHdmiRegBase, STA_CRC_RDY) != 0x0)//read the CRC_RDY bit, if the 1, indicate the CRC Check Data is ready. STA_CRC_RDY is read only.
		{
			vUtDelay1ms(1);
			if (RegReadFldAlign(SRST +u4ActiveHdmiRegBase, SCDT) == 0)//read if the SCDT is active. If the 0, break. If the 1, wate for the STA_CRC_RDY.
			{
				break;
			}
		}
		while(u1IO32Read1B(VID_CRC_CHK_3 + u4ActiveHdmiRegBase) != 0x00)//read the value of the CRC Check out data[7:0].
		{
			vUtDelay1ms(1);
		...
		while(u1IO32Read1B(VID_CRC_CHK_0 + u4ActiveHdmiRegBase)  != 0x00)//read the value of the CRC Check out data[15:8]
		{
			vUtDelay1ms(1);
			if (RegReadFldAlign(SRST +u4ActiveHdmiRegBase, SCDT) == 0)
			{
		...
		while(u1IO32Read1B(VID_CRC_CHK_1 + u4ActiveHdmiRegBase) != 0x00)//read the value of the CRC Check out data[23:16].
		{
			vUtDelay1ms(1);
			if (RegReadFldAlign(SRST +u4ActiveHdmiRegBase, SCDT) == 0)
			{
		...
		if((u1IO32Read1B(VID_CRC_CHK_2 + u4ActiveHdmiRegBase)&0x81) == 0x0)//adjust the CRC_Start_bit and the CRC_RDY_bit. Both are 0, is active.
		{
			...
			vIO32Write1B(VID_CRC_CHK_2 + u4ActiveHdmiRegBase, 0x0c);//write the CRC_Start  0.
...
BOOL fgCCIR2(INT16 ntry)
{
	...
	if(ntry == 1)//open ccir2 for wifi push
	{
		...
		vIO32WriteFldAlign(CCIR_HDTV_08,hight,CCH_VDE_WIDTH_PIP);
...
#ifdef CC_SUPPORT_W2DR2R
UINT16 wHDMIResoWidthByPort(E_HDMI_SWITCH_NUM u1HdmiPort)
{
    UINT16 tmp;
...
UINT16 wHDMIResoHeightByPort(E_HDMI_SWITCH_NUM u1HdmiPort)
{
    UINT16 tmp;
...
UINT16 wHDMIHTotalByPort(E_HDMI_SWITCH_NUM u1HdmiPort)
{
    UINT16 tmp;
...
UINT16 wHDMIVTotalByPort(E_HDMI_SWITCH_NUM u1HdmiPort)
{
    UINT16 tmp;
...
BOOL fgHDMIinterlacedByPort(E_HDMI_SWITCH_NUM u1HdmiPort)
{
    UINT8 u1Vid = 0;
    UINT8 u1Vid392D = 0;
	UINT32 u4HdmiBasebyPort;
    ...
    if (u1IO32Read1B(AVIRX0_0 + u4HdmiBasebyPort) == 0x82)
    {
      ...
      if (((u1Vid == 20) || (u1Vid == 39)) && (wHDMIResoHeightByPort(u1HdmiPort) == 1080) && (wHDMIHTotalByPort(u1HdmiPort) == 2304))
     {
          u1Vid392D = 1;
     }
      else
...
UINT8 bHDMIRefreshRateByPort(E_HDMI_SWITCH_NUM u1HdmiPort)

{
    ...
    dwtmp = wHDMIHTotalByPort(u1HdmiPort) * wHDMIVTotalByPort(u1HdmiPort);
    ...
    pfreq = dwHDMIPixelFreqByPort(u1HdmiPort);
...
UINT32 dwHDMILineFreqByPort(E_HDMI_SWITCH_NUM u1HdmiPort)
{
    UINT32 ret;
    UINT16 wDiv;
    wDiv = wHDMIHTotalByPort(u1HdmiPort);
    ...
    ret = ((dwHDMIPixelFreqByPort(u1HdmiPort) * 10) / (wDiv));
    return ret;
}
#endif
...
PRIVATE void fgHDMIAVIFrame(void)
{
    ...
    else
    {
		...
		else if((u1IO32Read1B(AVIRX1_0 + u4ActiveHdmiRegBase)  & 0x60) == 0x60)
        {
		...
		else if((u1IO32Read1B(AVIRX1_1 + u4ActiveHdmiRegBase) & 0xc0) == 0xC0)
		{
			UINT8 u1ExtendColorMtry;
			printf("Extended Colorimetry:");
			u1ExtendColorMtry = (((u1IO32Read1B(AVIRX1_2 + u4ActiveHdmiRegBase)) & 0x70)>>4);
			switch(u1ExtendColorMtry)
			{
				case 0:
					printf("xvYCC 601 \n");
					break;
				...
				case 2:
					...
					break;
				case 3:
					printf("Adobe Ycc 601 \n");
					break;
				case 4:
					printf("Adobe RGB \n");
					break;
				case 5:
					printf("BT2020 Y'cC'bcC'rc \n");
					break;
				case 6:
					printf("BT2020 R'G'B' or Y'C'bC'r \n");
				case 7:
				default:
					printf("reserved \n");
					break;
...
void vHDMIAudDbgMsg(void)
{
    UINT32 wCTS_HW, wN_HW;
	printf(">>>>>>Audio Error Status Check Start<<<<<<\n");
	printf(">>>u1AudFifoFault = %d\n",u1AudFifoFault);
	printf(">>>_bHDMIAudFIFOflag = %d\n",_bHDMIAudFIFOflag); 
	if(u1AudPacket == AUDPACKET_GOOD)
	{
		printf(">>>AUD PKT GOOD!u1AudPacket = %d\n",u1AudPacket);
	}
	else
	{
		printf(">>>AUD PKT NG!u1AudPacket = %d\n",u1AudPacket);
	}
	printf(">>>>>>Audio Error Status Check End<<<<<<\n");
	if(IO32ReadFldAlign(INTR_STATE0 + u4ActiveHdmiRegBase, INTR2_NEW_AUD_PKT))
	{
		printf(">>>>>>Received New Audio Packet\n");
	}
	else
	{
		printf(">>>>>>Not Received New Audio Packet\n");
	}
	if(IO32ReadFldAlign(APLL1 + u4ActiveHdmiRegBase, ACR_DPLL_LOCK))
	{
		printf(">>>>>>APLL Locked\n");
	}
	else
	{
		printf(">>>>>>APLL Not Locked\n");
	}
    wCTS_HW = (IO32ReadFldAlign(CTS_HVAL + u4ActiveHdmiRegBase, CTS_VAL_HW19_16) << 16) | (IO32ReadFldAlign(CTS_HVAL + u4ActiveHdmiRegBase, CTS_VAL_HW15_8) << 8) | IO32ReadFldAlign(CTS_HVAL + u4ActiveHdmiRegBase, CTS_VAL_HW7_0);
    wN_HW = (IO32ReadFldAlign(N_HVAL + u4ActiveHdmiRegBase, N_VAL_HW) << 16) | (IO32ReadFldAlign(N_SVAL + u4ActiveHdmiRegBase, N_VAL_HW15_8) << 8) | IO32ReadFldAlign(N_SVAL + u4ActiveHdmiRegBase, N_VAL_HW7_0);
    printf(" HW CTS =%d,0x%x and N =%d,0x%x\n", wCTS_HW,wCTS_HW, wN_HW,wN_HW);
	printf(">>>>>>Current Sample Rate:%d\n",bHDMIGetAudSampleFreq());
}
...
#ifdef HDCP_USE_RAW_KEY

	vIO32Write4B(HD_KM0, 0x00001800);
	vIO32Write4B(HD_KS0, 0x00000000);
	vIO32Write4B(HD_KM1, 0x00001800);
	vIO32Write4B(HD_KS1, 0x00000000);
	vIO32Write4B(HD_KM2, 0x00001800);
	vIO32Write4B(HD_KS2, 0x00000000);
	vIO32Write4B(HD_KM3, 0x00001800);
	vIO32Write4B(HD_KS3, 0x00000000);
	
    vIO32Write4B(HD_KM0, 0x00201800);
    vIO32Write4B(HD_KM1, 0x00201800);
    vIO32Write4B(HD_KM2, 0x00201800);
    vIO32Write4B(HD_KM3, 0x00201800);
    vIO32Write4B(HD_KM0, 0x00001800);
    vIO32Write4B(HD_KM1, 0x00001800);
    vIO32Write4B(HD_KM2, 0x00001800);
    vIO32Write4B(HD_KM3, 0x00001800);
...
UINT8 u1HDMI_SetEdidDataByPort(UINT8 port, UINT8 * pData)
{
	...
	else
	{
		...
		{
			vIO32WriteFldAlign(PDWNC_EDID_CTL1, pData[u1PAOffset], FLD_PHY_ADDR1);
		...
		{
			vIO32WriteFldAlign(PDWNC_EDID_CTL2, pData[u1PAOffset], FLD_PHY_ADDR2);
		...
		{
			vIO32WriteFldAlign(PDWNC_EDID_CTL3, pData[u1PAOffset], FLD_PHY_ADDR3);
...
#else
#ifdef CC_MTK_PCB_SPECIFIC
    //Printf("hdmi 1-2 to Jtag \n");
    BSP_PinSet(PIN_GPIO56, 1);
    BSP_PinSet(PIN_GPIO57, 1);
    BSP_PinSet(PIN_GPIO58, 1);
    BSP_PinSet(PIN_GPIO59, 1);
    BSP_PinSet(PIN_HDMI_0_SCL, 1);
    BSP_PinSet(PIN_HDMI_0_SDA, 1);
    BSP_PinSet(PIN_HDMI_1_SCL, 2);
    BSP_PinSet(PIN_HDMI_1_SDA, 2);
    BSP_PinSet(PIN_HDMI_2_SCL, 2);
    BSP_PinSet(PIN_HDMI_2_SDA, 2);
    BSP_PinSet(PIN_HDMI_3_SDA, 1);
    BSP_PinSet(PIN_HDMI_3_SCL, 1);
    BSP_PinSet(PIN_HDMI_1_HPD, 0);
    BSP_PinSet(PIN_HDMI_2_HPD, 1);
    BSP_PinSet(PIN_USB_DM_P0, 0);
#else
//        Printf("hdmi 2-3 to Jtag \n");
    BSP_PinSet(PIN_GPIO56, 1);
    BSP_PinSet(PIN_GPIO57, 1);
    BSP_PinSet(PIN_GPIO58, 1);
    BSP_PinSet(PIN_GPIO59, 1);
    ...
    BSP_PinSet(PIN_HDMI_1_SCL, 1);
    BSP_PinSet(PIN_HDMI_1_SDA, 1);
    BSP_PinSet(PIN_HDMI_2_SCL, 4);
    BSP_PinSet(PIN_HDMI_2_SDA, 4);
    BSP_PinSet(PIN_HDMI_3_SDA, 4);
    BSP_PinSet(PIN_HDMI_3_SCL, 4);
    BSP_PinSet(PIN_HDMI_1_HPD, 0);
    BSP_PinSet(PIN_HDMI_2_HPD, 1);
    BSP_PinSet(PIN_USB_DM_P0, 0);
#endif
...
UINT8 u1HDMI_GetHdcpKey(UINT8 *pData, UINT16 u2Length)
{
    ...
    for (u4Idx = 0; u4Idx < u2Length; u4Idx += 16)
    {
  	 u4Data[0] = u4IO32Read4B(HDCP_DATA);
        ...
        pData[u4Idx] = (UINT8)u4Data[0] & 0xff;
        pData[u4Idx + 1] = (UINT8)(u4Data[0] >> 8) & 0xff;
        pData[u4Idx + 2] = (UINT8)(u4Data[0] >> 16) & 0xff;
        pData[u4Idx + 3] = (UINT8)(u4Data[0] >> 24) & 0xff;

        pData[u4Idx + 4] = (UINT8)u4Data[1] & 0xff;
        pData[u4Idx + 5] = (UINT8)(u4Data[1] >> 8) & 0xff;
        pData[u4Idx + 6] = (UINT8)(u4Data[1] >> 16) & 0xff;
        pData[u4Idx + 7] = (UINT8)(u4Data[1] >> 24) & 0xff;

        pData[u4Idx + 8] = (UINT8)u4Data[2] & 0xff;
        pData[u4Idx + 9] = (UINT8)(u4Data[2] >> 8) & 0xff;
        pData[u4Idx + 10] = (UINT8)(u4Data[2] >> 16) & 0xff;
        pData[u4Idx + 11] = (UINT8)(u4Data[2] >> 24) & 0xff;

        pData[u4Idx + 12] = (UINT8)u4Data[3] & 0xff;
        pData[u4Idx + 13] = (UINT8)(u4Data[3] >> 8) & 0xff;
        pData[u4Idx + 14] = (UINT8)(u4Data[3] >> 16) & 0xff;
        pData[u4Idx + 15] = (UINT8)(u4Data[3] >> 24) & 0xff;
    }
...
void u1HDMIEQOffsetCali(E_HDMI_SWITCH_NUM ePort)
{
	...
	for(i=7; i>0; i--)
	{
		...
		if((IO32ReadFldAlign((RGS_HDMI_RX_Normal_0_CFGD + u4RegOffset), RGS_HDMI_0_CH0_EQMM) == 1) && (u1ch_locked[0] == 0))
		{
			u1ch_lock_value[0] = i;
			u1ch_locked[0] = 1;
		...
		if((IO32ReadFldAlign((RGS_HDMI_RX_Normal_0_CFGD + u4RegOffset), RGS_HDMI_0_CH1_EQMM) == 1) && (u1ch_locked[1] == 0))
		{
			u1ch_lock_value[1] = i;
			u1ch_locked[1] = 1;
		...
		if((IO32ReadFldAlign((RGS_HDMI_RX_Normal_0_CFGD + u4RegOffset), RGS_HDMI_0_CH2_EQMM) == 1) && (u1ch_locked[2] == 0))
		{
			u1ch_lock_value[2] = i;
			u1ch_locked[2] = 1;
...
static UINT8 u1set_to_hdmi_port = 0;
UINT8 u1HDMI_Connect_Port(UINT8 port)
{
	u1set_to_hdmi_port = port;
	printf("set to hdmi port %d\n", u1set_to_hdmi_port);
    return 0;
}
...
void vMHLOpenRxTermination(BOOL bOnoff)
{
	if(bOnoff)
	{
		...
		if(_bIsMhlDevice == 1)
		{
			vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2, 0, RG_HDMI_0_TERM_CM_ENB);//connect 300ohm between DIP and DIN
		}
	}
	else
	{
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2, 0, RG_HDMI_0_TERM_EN);   //61108bit16
		vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG2, 1, RG_HDMI_0_TERM_CM_ENB);//disconnect 300ohm between DIP and DIN
	}
}
...
void vMHLSetHdcpAksv(UINT8* pu1Aksv)
{
    ...
    if(pu1Aksv)
    {
        ...
        for (i=0;i<5;i++)
		{
             ...
             vUtDelay1ms(1);   // T.B.D, need some time for writing into HDCP engine.
             // TODO, read back for checking.
             // note, TMDS clock must be active while writing Aksv values to hdcp engine
...
void vHDMIResetPLL(E_HDMI_SWITCH_NUM u1Port)
{
	UINT32 u4LocalAnaBase;
#ifdef CC_HDMI_1_4_SUPPORT
    UINT16 u2HDMI2AnaBaseAddr;
	if(u1Port == HDMI_SWITCH_4)
	{
	    u2HDMI2AnaBaseAddr = (UINT16)u4GetHdmi2AnalogRegBase(u1Port);
		vHDMI2ComAnaPLL_EN(u2HDMI2AnaBaseAddr,0);
		vUtDelay2us(25);
		vHDMI2ComAnaPLL_EN(u2HDMI2AnaBaseAddr,1);
		vUtDelay2us(100);
        vHDMI2ComAnaDeepPLL_EN(u2HDMI2AnaBaseAddr,0);
		vUtDelay2us(1);
        vHDMI2ComAnaDeepPLL_EN(u2HDMI2AnaBaseAddr,1);
        vUtDelay2us(100); //200us
		return;
	}
#endif
	u4LocalAnaBase = u4GetAnalogRegBase(u1Port);
	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4LocalAnaBase,0, RG_HDMI_0_PLL_EN);
	vUtDelay2us(25);
	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG7  + u4LocalAnaBase,1, RG_HDMI_0_PLL_EN);
	vUtDelay2us(25);
}
...
void vHDMIHDCPEN(UINT8 u1En)
{
	vHDMIDDCBusCTRL(u1En);
}
#ifdef CC_COMBINE_4K_2K
UINT8 u1GetHDMI2KModel(void)
{
	return ((_wIsHDMI2KModel&0x01) == 0x00)?1:0;
}
#endif
//AUD_HdmiSpeakerConfig(AUD_DEC_MAIN,u1GetHdmiSpeakerConfig())
//AUD_HdmiDownmixSetting(AUD_DEC_MAIN,u1GetHdmiDownmixSetting())
UINT8 u1GetHdmiSpeakerConfig(void)
{	
	if(_bHdmiModeActive)
	{
		return u1IO32Read1B(AUDRX1_3 + u4ActiveHdmiRegBase);
	}
	return 0;
}
UINT8 u1GetHdmiDownmixSetting(void)
{
	if(_bHdmiModeActive)
	{
		return u1IO32Read1B(AUDRX2_0 + u4ActiveHdmiRegBase);
	}
	return 0;
}
UINT8 u1HdmiResetCkdtRset(void)
{
	UINT32 u4LocalAnaBase;
	u4LocalAnaBase = u4GetAnalogRegBase(eActiveHdmiPort);
	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4LocalAnaBase, 0,RG_HDMI_0_CKDT_SET);
	vUtDelay2us(25);
	vIO32WriteFldAlign(REG_HDMI_RX_Normal_0_CFG1 + u4LocalAnaBase, 1,RG_HDMI_0_CKDT_SET);
	return 0;
}
UINT32 dwHDMIBitRate(void)
{
	UINT32 dwBitRate;
	UINT32 dwPixelFreq;

	dwPixelFreq = dwHDMIPixelFreq();
	dwBitRate = (dwPixelFreq * 10)/1024;

	return dwBitRate;
}

UINT8 u1HdmiGetActivePortTimingVic(void)
{
	UINT32 u4LocalBase;
	UINT8 vicCode = 0;
	u4LocalBase = u4GetHdmiRegBase(eActiveHdmiPort);
	vicCode =( u1IO32Read1B(AVIRX1_3 + u4LocalBase) & 0x7f);
	return vicCode;
}
