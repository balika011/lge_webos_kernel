...
#ifdef CC_SUPPORT_W2DR2R
#define wDVIGetHtotalByPort(x)	wHDMIHTotalByPort(x)
#define bDVIGetHeightByPort(x) wHDMIResoHeightByPort(x)
#endif
...
/********************************************************************************
* BEGIN: 2014-5-9 George.Yang For modifying the CCIR-AVStart\AVWidth\VSStart\VDEWidth    *
*********************************************************************************/
CCIR_Blank_Value_T CCIRBlankValue;

static UINT8 isThePatternChanged(void)
{
	return _bDviModeChged;
}

static UINT8 isThePatternInterlaced(void)
{
	return fgHDMIinterlaced();
}


static UINT16 wGetTheVsActiveValue(void)
{
	UINT8 num;
	num = _bDviTiming;
	if(num == MODE_DE_MODE)
	...
	return (isThePatternInterlaced())?Get_VGAMODE_IPV_LEN(num)/2:Get_VGAMODE_IPV_LEN(num);
}
...
UINT16 wCCIRGetAvWidth(void)
{
	...
	data = IO32ReadFldAlign(DVI_03, CCH_AV_WIDTH);
	data *= 2;
...
{
/**BEGIN: DTV00605656 George.yang 2014-9-16 10:46:32**/
	if (isThePatternChanged()){
//			CCIRBlankValue.Cur_VDEWidth_Value = 0;
		CCIRBlankValue.Cur_VSStart_Value = 0;
//			CCIRBlankValue.Old_VDEWidth_Value = 0;
		CCIRBlankValue.Old_VSStart_Value = 0;
	}
	if (0 == CCIRBlankValue.Old_VSStart_Value){
		/**BEGIN:DTV00598373: Georage.yang 2014-7-24**/
		CCIRBlankValue.Cur_VSStart_Value = wVSyncWidth -3;
		/**END**/
	}
/**END: George.yang 2014-9-16 10:46:32**/
	
	return CCIRBlankValue.Cur_VSStart_Value;
}
...
void vCCIRSetVsStart(UINT16 wStart)
{
	...
	{
		wCCIRGetVsStart();
		CCIRBlankValue.Old_VSStart_Value = CCIRBlankValue.Cur_VSStart_Value;
		CCIRBlankValue.Cur_VSStart_Value = wStart;
		
		if (0 == CCIRBlankValue.Old_VDEWidth_Value){
			
			CCIRBlankValue.Cur_VDEWidth_Value = wGetTheVsActiveValue();
			
			CCIRBlankValue.Old_VDEWidth_Value = CCIRBlankValue.Cur_VDEWidth_Value;
			value = (UINT32)(CCIRBlankValue.Cur_VDEWidth_Value << 16)+(UINT32)(CCIRBlankValue.Cur_VSStart_Value);
			vIO32Write4B(CCIR_HDTV_06,value);
		} else{
			value = (UINT32)(CCIRBlankValue.Cur_VDEWidth_Value << 16)+(UINT32)(CCIRBlankValue.Cur_VSStart_Value);
			vIO32Write4B(CCIR_HDTV_06,value);
		}
...
//			CCIRBlankValue.Old_VSStart_Value = 0;
    ...
    return CCIRBlankValue.Cur_VDEWidth_Value;
}
...
void vCCIRSetVsWidth(UINT16 wStart)
{
	...
	else
	{
		wCCIRGetVsWidth();
		CCIRBlankValue.Old_VDEWidth_Value = CCIRBlankValue.Cur_VDEWidth_Value;
		CCIRBlankValue.Cur_VDEWidth_Value = wStart;
		value = (UINT32)(CCIRBlankValue.Cur_VDEWidth_Value<< 16)+(UINT32)(CCIRBlankValue.Cur_VSStart_Value);
		vIO32Write4B(CCIR_HDTV_06,value);
	}
}
...
/**********************************************************************************
**END: 2014-5-9 George.Yang For modifying the CCIR-AVStart\AVWidth\VSStart\VDEWidth        **
***********************************************************************************/
...
/**BEGIN: DTV00605656 George.yang 2014-9-16 10:53:12**/
	memset(&CCIRBlankValue, 0, sizeof(CCIR_Blank_Value_T));
/**END: George.yang 2014-9-16 10:53:12**/
	...
	#endif
	//u1HdmiResetCkdtRset();  //for ckdt being set to 1, not support issue
    // Hardware Initialize
    if(bEnDVIDE == 1)
    {
        vEnterDEMode(SV_ON);
...
{
    if((_bDviTiming == MODE_3D_720p_50_FP)||(_bDviTiming == MODE_3D_720p_60_FP)||(_bDviTiming == MODE_3D_1080p_24_FP)|| (_bDviTiming == MODE_3D_1080p_60_FP)||
       (_bDviTiming == MODE_3D_480p_60_FP)||(_bDviTiming == MODE_3D_576p_50_FP)||(_bDviTiming == MODE_3D_720p_24_FP)|| (_bDviTiming == MODE_3D_1080p_50_FP) ||
...
{
#ifdef CC_HDMI_2_0_SUPPORT
    if(eActiveHdmiPort >= u1GetMinHDMI20Port())
	{
		_bCurQueryMode = wDvi2QueryAP3DSetting(SV_VP_MAIN);
	}
	else
#endif
	{
...
void vHDMISetDefaultPorchAndWidth(void)
{
    ...
    if((0 == bHDMI3DPacketVaild()) || (1 == fg3DSettingFailed))
    {
        if(1==bDviBypassAp3DRule())
        {
            ...
            //vCCIRSetAvWidth(wDviGetTableHactive());
			u4VPortch = (UINT32)(wVSyncWidth - 3);
			u4VPortch |= (((UINT32)_wDviHeight) << 16);
           // vDviSetVsStart(wVSyncWidth - 3);
            //vCCIRSetVsStart(wVSyncWidth - 3);
            //vDviSetVsWidth(_wDviHeight);
            //vCCIRSetVsWidth(_wDviHeight);
            vIO32Write4B(CCIR_HDTV_06,u4VPortch);
		   //vIO32WriteFldAlign(CCIR_HDTV_06, wStart, CCH_VDE_WIDTH);
        }
...
#ifdef CC_SUPPORT_W2DR2R
UINT8 bDviStdTimingSearchByPort(UINT8 bMode,UINT8 u1HdmiPort)
{
    UINT8 bSearch;
    UINT8 bSearchEnd;
    UINT16 wTmp;
	
	UINT32 u4HdmiRefreshRate = bHDMIRefreshRateByPort(u1HdmiPort);
	UINT32 u4HdmiLineFreq = dwHDMILineFreqByPort(u1HdmiPort);
	UINT32 u4HdmiHtotal = wHDMIHTotalByPort(u1HdmiPort);
	UINT32 u4HdmiHeight = bDVIGetHeightByPort(u1HdmiPort);
	UINT32 u4HdmiInterlace = fgHDMIinterlacedByPort(u1HdmiPort);
    if(bMode)
    {
        // Video Mode
        bSearch = HDTV_SEARCH_START;
...
#if DVI_DEBUG
    LOG(3, "vsync frequency: %d\n", bHDMIRefreshRateByPort(u1HdmiPort));
    LOG(3, "hsync frequency: %d\n", dwHDMILineFreqByPort(u1HdmiPort));
    LOG(3, "Htotal: %d\n", wHDMIHTotalByPort(u1HdmiPort));
#endif
...
#endif
        {
            if((u4HdmiRefreshRate >= (Get_VGAMODE_IVF(bSearch) - 2)) &&
               (u4HdmiRefreshRate <= (Get_VGAMODE_IVF(bSearch) + 2)))
            {
                if ((u4HdmiLineFreq >= (Get_VGAMODE_IHF(bSearch) - 8)) &&
                (u4HdmiLineFreq <= (Get_VGAMODE_IHF(bSearch) + 8)))
                {
                    // check interlaced
                    if((/*!bHDMI3DPacketVaild() && */Get_VGAMODE_INTERLACE(bSearch)) != (u4HdmiInterlace))
                    {
                        LOG(3, "Get_VGAMODE_INTERLACE(bSearch): %d\n", Get_VGAMODE_INTERLACE(bSearch));
                        LOG(3, "fgHDMIinterlaced: %d\n", u4HdmiInterlace);
                        continue;
                        ...
                        LOG(3, "bDVIGetHeightByPort()= %d\n", bDVIGetHeightByPort(u1HdmiPort));
                        LOG(3, "Get_VGAMODE_IPV_LEN=%d\n", Get_VGAMODE_IPV_LEN(bSearch));

                        if((wTmp > (Get_VGAMODE_IHTOTAL(bSearch) - 40)) &&
                           (wTmp < (Get_VGAMODE_IHTOTAL(bSearch) + 40)))
                        {
                            if(((u4HdmiHeight<<u4HdmiInterlace) > (Get_VGAMODE_IPV_LEN(bSearch) - 3)) &&
                               ((u4HdmiHeight<<u4HdmiInterlace) < (Get_VGAMODE_IPV_LEN(bSearch) + 3)))
                            {
                                LOG(3,"bSearch=%d\n",bSearch);
                                LOG(3,"UNRECRX0_0=%x\n",u1IO32Read1B(UNRECRX0_0 + u1HdmiPort));
                                LOG(3,"INTR_STATE0=%x\n",IO32ReadFldAlign(INTR_STATE0 + u1HdmiPort, INTR3_NEW_UNREC));
                                LOG(3,"UNRECRX1_3=%x\n",u1IO32Read1B(UNRECRX1_3 + u1HdmiPort) >> 5);
								
                                if(bHDMI3DPacketVaild())
                                {
                                    if((u1IO32Read1B(UNRECRX2_0 + u1HdmiPort) >> 4)==HDMI_3D_Structure_FramePacking)
                                    {
                                        if(bSearch==MODE_576P_OVERSAMPLE)
                                        {
                                            bSearch=MODE_3D_1440_576p_50_FP;
                                    ...
                                    else if((u1IO32Read1B(UNRECRX2_0 + u1HdmiPort) >> 4)==HDMI_3D_Structure_SideBySideFull)
                                    {
                                        if(bSearch==MODE_525I_OVERSAMPLE)
                                        {
                                            bSearch=MODE_3D_480I_60_SBS_FULL;
									...
									else if((u1IO32Read1B(UNRECRX2_0 + u1HdmiPort) >> 4)==HDMI_3D_Structure_LineAlternative)
                                    {
                                        if(bSearch==MODE_480P_OVERSAMPLE)
                                        {
                                            bSearch=MODE_3D_240p_60_LA;
									...
									else if ((u1IO32Read1B(UNRECRX2_0 + u1HdmiPort) >> 4)==HDMI_3D_Structure_FieldAlternative)
									{
										if(bSearch==MODE_3D_1440_240p_60_FP)
										{
											bSearch=MODE_3D_480I_60_FA;
									...
									else if ((u1IO32Read1B(UNRECRX2_0 + u1HdmiPort) >> 4)==HDMI_3D_Structure_SideBySideHalf)
									{
										if(bSearch==MODE_525I)
										{
											bSearch=MODE_3D_480I_60_SBS_HALF;
...
void vDviConnect(UINT8 bchannel, UINT8 fgIsOn)
{
    ...
    if(fgIsOn == SV_ON)
    {
        ...
        if(bchannel == SV_VP_MAIN)
        {
			...
			//Printf("__________________________3______________________\n");
        ...
        else
        {
			...
			// vIO32WriteFldAlign(OMUX_00, 1, VDOIN_MPCLK_SEL);
			 //Printf("__________________________1______________________\n");
...
/**BEGIN: 4K2K George.yang 2014-8-25 14:39:45**/
extern SRM_TV_MODE_T _eApplicationMode;
/**END: George.yang 2014-8-25 14:39:45**/
...
void vDviModeDetect(void)
{
    ...
    switch(_bDviModeDetState)
    {
        ...
        case DVI_MODE_CHG_DONE_STATE:
			...
			/* add because the ckdt being forced to avoid not support detected*/
            ...
            // set input capture
#ifdef CC_SUPPORT_W2DR2R
	/*if((IS_DISP_4K2K_TYPE4) && \
		((SRM_GetTvMode() == SRM_TV_MODE_TYPE_PIP) || (SRM_GetTvMode() == SRM_TV_MODE_TYPE_POP)))
	{
		if(wHDMIVTotal() >= 2000)
		{
			_bDviTiming = MODE_NOSUPPORT;
			LOG(0,"don't support 4K2K timing \n");
		}
	}*/

	/**BEGIN: 4k2k George.yang 2014-8-25 14:40:24**/
			if (_eApplicationMode == SRM_TV_MODE_TYPE_PIP ||
				_eApplicationMode == SRM_TV_MODE_TYPE_POP){
				if (wHDMIVTotal() >= 2000){
					_bDviTiming = MODE_NOSUPPORT;
					LOG(0 , "test Don't support 4K2K timing \n");
				}
			}
	/**END: George.yang 2014-8-25 14:40:24**/


	if(fgIs4K2KInBypassPath())
		{
			if(vDviTimingNeedW2d(_bDviTiming) == W2D_USE_LEFT_AND_RIGHT)
			{
				E_HDMI_SWITCH_NUM eHDMIPortIndex;
				for(eHDMIPortIndex = HDMI_SWITCH_1;eHDMIPortIndex < HDMI_SWITCH_MAX;eHDMIPortIndex++)
				{
					if(eHDMIPortIndex != eActiveHdmiPort)
					{
						if(_bDviTiming == bDviStdTimingSearchByPort(1,eHDMIPortIndex))
						{
							fgHDMIW2DR2REnable = TRUE;
							LOG(0,"timing search the same, apply w2d setting............\n");
							vHDMIW2DR2RSetting((UINT8)eActiveHdmiPort,eHDMIPortIndex);	
							break;
						}
					}
			...
			else if(vDviTimingNeedW2d(_bDviTiming) == W2D_USE_LEFT_ONLY)
			{
				fgHDMIW2DR2REnable = TRUE;
				LOG(0,"4k timing input, apply w2d setting............\n");
				vHDMIW2DR2RSetting((UINT8)eActiveHdmiPort,(UINT8)eActiveHdmiPort);	
			}
			if(!fgHDMIW2DR2REnable)
			{
				vR2RSetOmuxOutput(0,0);  //data is from rx to CCIR
				vR2RSetOmuxOutput(1,0);  //data is from rx to CCIR
				vCCIRSelMainCLK(); //clk is from rx to CCIR
				//vR2RSelSubCLK();  //clk is from rx to sub CCIR
			}
			if((fg2k2ktoOmux) && (vDviTimingNeedW2d(_bDviTiming) == W2D_USE_LEFT_AND_RIGHT)&&(fgHDMIW2DR2REnable))
			{
				_bDviTiming = 99; // 2X 2k2k to 1 4k2k  omux
			}
		}	
#endif
        ...
        case DVI_CHK_MODECHG:
               ...
               ||(aud_ch_info.eDataType!=(HDMI_DATATYPE)bHDMI_IsPCM())||(aud_ch_info.eSampeFreq!=(HDMI_AUD_FS)bHDMIGetAudSampleFreq()))
            {
                ...
                if((_bHDMIScanInfo != bHDMIScanInfo()) || (_bHDMIAspectRatio != bHDMIAspectRatio()) || (_bHDMIAFD != bHDMIAFD())
                   ...
                   || (_bHDMIHDCPStatus != bHDMIHDCPStatusGet()) || (u1HdmiSpdUPStatChk != u1HdmiGetUPStaChg()) || (u14KPsDetStatus != u1HDMI_Get4KPStatus()))
                {
                    if(_bInfoChgCnt++ > 3)
                    {
                        _bInfoChgCnt = 0;
						...
						u14KPsDetStatus = u1HDMI_Get4KPStatus();
						//u1HdmiSpdSVStatChk = u1HdmiGetSVStaChg();
            ...
            if (_bDviDeStableCnt < DE_STABLE_COUNT)
            {
                ...
                //remove this modechange check for timing 2880X480I and 2880X576I input
            ...
            case DVI_WAIT_STABLE:
				...
				if ((_bVStableCnt > VRES_STB_COUNTS && _bHStableCnt > HRES_STB_COUNTS ) || \
					(u1IsHdmiFastSw(eActiveHdmiPort) && _bVStableCnt > 20))
                    {
...
#endif
void vSetVPortch(void)
{
	UINT32 u4VPortch =0;

	u4VPortch = wVSyncWidth - 3;
	u4VPortch |= (_wDviHeight << 16);
    Printf("_______u4VPortch =0x%x_________________\n");
    vIO32Write4B(CCIR_HDTV_06,u4VPortch);
}
