 ...
 */
//-----------------------------------------------------------------------------
UINT16 u2MpegHdInputWidth(UINT8 bPath) 
{
    UCHAR ucB2rId;
    ucB2rId =VDP_Vdp2B2rId(bPath);
    if(ucB2rId >= B2R_NS)
 ...
 */
//-----------------------------------------------------------------------------
UINT16 u2MpegSdInputWidth(UINT8 bPath) 
{
#ifdef SUPPORT_2_B2R
    UCHAR ucB2rId;
    ucB2rId =VDP_Vdp2B2rId(bPath);
...
UINT16 u2MpegHdInputValidDataWidth(UINT8 bPath) 
{
    ...
    if (_prB2rConf[ucB2rId])
    {
        ...
        if (VDP_GetSeamlessResizeSize(ucB2rId, &u4ResizeWidth, &u4ResizeHeight))
        {
 ...
 */
//-----------------------------------------------------------------------------
UINT32 u4MpegHdPreciseRefreshRate(UINT8 bPath)
{
    UINT32 u4FrameRate;
    UCHAR ucB2rId;
    ucB2rId =VDP_Vdp2B2rId(bPath);
    ...
    if (_prB2rConf[ucB2rId])
    {
        ...
        if( (_prB2rConf[ucB2rId]->rOutInfo.ucInputFrameRate == MPEG_FRAME_RATE_24)
            || (_prB2rConf[ucB2rId]->rOutInfo.ucInputFrameRate == MPEG_FRAME_RATE_25)
            || (_prB2rConf[ucB2rId]->rOutInfo.ucInputFrameRate == MPEG_FRAME_RATE_30)
            || (_prB2rConf[ucB2rId]->rOutInfo.ucInputFrameRate == MPEG_FRAME_RATE_50)
            || (_prB2rConf[ucB2rId]->rOutInfo.ucInputFrameRate == MPEG_FRAME_RATE_60)
            || (_prB2rConf[ucB2rId]->rOutInfo.ucInputFrameRate == MPEG_FRAME_RATE_UNKNOWN))
        {
            u4FrameRate = ((UINT32)(_prB2rConf[ucB2rId]->rOutInfo.ucFrameRate) * 1000) ;
        }
        else
        {
            u4FrameRate = ((UINT32)(_prB2rConf[ucB2rId]->rOutInfo.ucFrameRate) * 1000000) / 1001 ;
        }
        return u4FrameRate;
    }
 ...
 */
//-----------------------------------------------------------------------------
UINT32 u4MpegSdPreciseRefreshRate(UINT8 bPath)
{
#ifdef SUPPORT_2_B2R
    UINT32 u4FrameRate;
    UCHAR ucB2rId;
    ...
    if (_prB2rConf[ucB2rId])
    {
        ...
        switch (_prB2rConf[B2R_1]->rOutInfo.ucPmxMode)
        {
               ...
               (_prB2rConf[ucB2rId]->rOutInfo.ucFrameRate == 15))
            {
               ...
               (_prB2rConf[ucB2rId]->rOutInfo.ucFrameRate == 50))
            {
                return MODE_3840_2160P_50;
            }
            else if((_prB2rConf[ucB2rId]->rOutInfo.u4Height == 2160) &&
               (_prB2rConf[ucB2rId]->rOutInfo.ucPrg == 1) &&
               (_prB2rConf[ucB2rId]->rOutInfo.ucFrameRate == 60))
            {
                return MODE_3840_2160P_60;
            } 
#endif            
            else
            {
...
UINT8 u1MpegHdClock(UINT8 bPath)
{
    UCHAR ucB2rId;
    ucB2rId =VDP_Vdp2B2rId(bPath);
    if(ucB2rId >= B2R_NS)
    ...
    if (_prB2rConf[ucB2rId])
    {
        switch (_prB2rConf[ucB2rId]->rOutInfo.u4ClkMode)
        {            
...
UINT8 u1MpegSdClock(UINT8 bPath)
{
#ifdef SUPPORT_2_B2R
    UCHAR ucB2rId;
    ucB2rId =VDP_Vdp2B2rId(bPath);
