...
#define MHL_OCP_GPIO   232
#define CBUS_1K_HDMI_MODE 1
#define CBUS_1K_MHL_MODE 0
...
volatile MHL_CBUS_SINK_CONTROL		stSinkCtr;
...
static volatile UINT16 ReadDevCapOffset = CBUS_MSC_DEVCAP_DEV_STATE;
...
static void vCbusExternal1KControl(UINT8 u1Ctrl);
static void vCbusHdcpControl(UINT8 u1Ctrl);
	...
	switch(Impedance)
	{
		case 1: //1K
			...
			if(IS_IC_5861())
			{
				...
				ENABLE_GZL_PINMUX_TO_CBUS();//For 5861
			}
			else
			{
				ENABLE_PINMUX_TO_CBUS();//For 5890
			}
		#endif
			vIO32WriteFldAlign(PDWNC_PADCFG17,1,FLD_CBUS_PUPD);//280E8 bit 9 PUPD ---	SET to 1
		...
		#endif
			vIO32WriteFldAlign(PDWNC_PADCFG17,0,FLD_CBUS_PUPD);//280E8 bit 9 PUPD ---	SET to 1
		    vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,0, FLD_ZCBUS_DISCOVER_EN);
		    vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,0, FLD_ZCBUS_SINK_ON_CTRL);
			vIO32WriteFldAlign(PDWNC_CBUS_LINK_0C,0, FLD_CBUS_1K_CTRL);
		...
		case 4://hpd
			...
			if(IS_IC_5861())
			{
				...
				DISABLE_GZL_PINMUX_TO_CBUS();//For 5861
			}
			else
			{
				DISABLE_PINMUX_TO_CBUS();//For 5890
			}
		#endif
			break;
		default:
...
void vCbusSetDiscoveryOK(BOOL fgIsMHLDiscoveryOK)
{
	...
	vCBusEnHwMode();//enable DDC HW mode
...
void vCbus_HwInit(void)
{
    ...
    vIO32WriteFldAlign(PDWNC_PADCFG17, 1,FLD_CBUS_E8);
	vCbusDisable();
...
#endif
	vIO32WriteFldAlign(PDWNC_PADCFG17,0,FLD_CBUS_PU);//280e8 bit 3 PU  --- SET TO 0
    //cbus test 
    if((u4WakeUpReason == (UINT32)PDWNC_WAKE_UP_REASON_AC_POWER) || 
	   (u4WakeUpReason == (UINT32)PDWNC_WAKE_UP_REASON_WATCHDOG))
    {
	    vIO32WriteFldAlign(PDWNC_PADCFG17,0,FLD_CBUS_PUPD);//280E8 bit 9 PUPD ---  SET to 0, high Z state
        vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,0, FLD_ZCBUS_DISCOVER_EN);
        vIO32WriteFldAlign(PDWNC_CBUS_LINK_07,0, FLD_ZCBUS_SINK_ON_CTRL);
    }	

	vIO32WriteFldAlign(PDWNC_PADCFG17,0,FLD_CBUS_SMT);//SMT ---  SET to 0		
    ...
    if(u1GetCdsenseStatus())//cable connected,when power on
    {
		...
		vCbusExternal1KControl(CBUS_1K_MHL_MODE);
		vCbusHdcpControl(CBUS_1K_MHL_MODE);
    }
    else //cable disconnected
    {
        _bIsMhlDevice = 0;
        u1BootUp = 0;
		vCbusExternal1KControl(CBUS_1K_HDMI_MODE);
		vCbusHdcpControl(CBUS_1K_HDMI_MODE);
    }
    if((u4WakeUpReason == (UINT32)PDWNC_WAKE_UP_REASON_AC_POWER) || 
    (u4WakeUpReason == (UINT32)PDWNC_WAKE_UP_REASON_WATCHDOG))
     {
...
static void vCbusExternal1KControl(UINT8 u1Ctrl)
{
		...
		if(IS_IC_5861() || (BSP_GetIcVersion() >= IC_VER_5890_AC))//only 5861.
		{
			if(u1Ctrl)
			{
				vIO32WriteFldAlign(PDWNC_PADCFG17,1,FLD_CBUS_EX1K);//HDMI Mode 
			}
			else
			{
				vIO32WriteFldAlign(PDWNC_PADCFG17,0,FLD_CBUS_EX1K);//MHL Mode 
			}
		}
...
static void vCbusHdcpControl(UINT8 u1Ctrl)
{
		...
		if(IS_IC_5861() || (BSP_GetIcVersion() >= IC_VER_5890_AC))//only 5861.
		{
			if(u1Ctrl)
			{
				vIO32WriteFldAlign(PDWNC_PDMISC,0,FLD_HDMI1_MHL_HDCP_SEL);//HDMI Mode 
			}
			else
			{
				vIO32WriteFldAlign(PDWNC_PDMISC,1,FLD_HDMI1_MHL_HDCP_SEL);//MHL Mode 
			}
		}
...
/*
@vCbusTriggerCmpInterrupt: Trigger HW interrupt according to write different data
to FLD_MON_CMP_DATA.
*/
...
void vCbusReadDcap(void)
{
	if(u1CbusMscAbortDelay2S)
		return;
	stSinkRequster.req_state = SINK_CBUS_REQ_BEGIN;
	stSinkRequster.u2ReqBuf[0] = CBUS_MSC_CTRL_READ_DEVCAP;
	stSinkRequster.u2ReqBuf[1] = ReadDevCapOffset;
	stSinkRequster.u1Len = 2;
	while(stSinkRequster.req_state == SINK_CBUS_REQ_BEGIN)
	{
		vCbusTriggerCmpInterrupt();
		x_thread_delay(4);
...
void vCbusSetPathEn(UINT8 key)
{
	...
	stSinkRequster.u2ReqBuf[2] = (key << 3) | 0x400;
	stSinkRequster.u1Len = 3;
...
void vCbusMainLoop(void)
{
	...
	switch (iMhlStateOld)
	{
		...
		case MHL_STATE_INIT:
			...
			if(u1DebugLvl == 2)
			{
				LOG(0,"iMhlStateOld = MHL_STATE_INIT!\n");
			}
		break;
...
void vCbusCommandLoop(void)
{
	...
	if(vCbusGetMHLState() == MHL_STATE_CONNECTED)
	{
		...
		{
			vCbusSetPathEn(PATH_EN_POSITIVE);
		...
		if(stSinkCtr.skIsRChg.u1RdDcap == 1)
		{
			stSinkCtr.skIsRChg.u1RdDcap = 0;
			vCbusReadDcap();
		}
		...
		else if(stSinkCtr.skIsSendWriteBurst == WRITE_BURST_HAL_REQ)
		{
			stSinkCtr.skIsSendWriteBurst = WRITE_BURST_END;
			vCbusRequestWriteBurst();
		}
		else if(stSinkCtr.skIsSendWriteBurst == WRITE_BURST_BEGIN)
		{
		#ifdef CC_MHL_3D_SUPPORT
			LOG(0,"___-send write burst,VicWriteburstIdx =%d,VicWriteburstCnt=%d,DtdWriteburstIdx=%d,DtdWriteburstCnt=%d_____________\n",VicWriteburstIdx,VicWriteburstCnt,DtdWriteburstIdx,DtdWriteburstCnt);
			stSinkCtr.skIsSendWriteBurst = WRITE_BURST_END;
#ifdef SYS_ANDROID_TV_TIF_SUPPORT
			if(stSinkCtr.skIsSendWriteBurstByAp == 1)
			{
				vCbusSendWriteBurstHalCmd();
				//stSinkCtr.skIsSendWriteBurstByAp = 0;
			}
			else
#endif
			{
		...
		else if(stSinkCtr.skIsRChg.u1DscrChg)
		{
			...
			else if((VicWriteburstIdx < VicWriteburstCnt) || (DtdWriteburstIdx < DtdWriteburstCnt ))
			{
				
				stSinkCtr.skIsSendWriteBurst = WRITE_BURST_CONT;
			}
			else
			{
				VicWriteburstIdx = 0;
		...
		else if(stSinkCtr.skIsSendWriteBurst == WRITE_BURST_CONT)
		{
			LOG(0,"____write burst again_________________\n");
			stSinkCtr.skIsSendWriteBurst = WRITE_BURST_END;
			vCbusRequestWriteBurst();
		}
...
void vCBusHandleReqAckCmd(UINT16 u2RxMsg)
{
	...
	else
	{
		...
		if(u1DebugLvl == 2)
		{
			LOG(0,"MSC Error :%s,%d.CMD:0x%x\n",__FUNCTION__,__LINE__,u2RxMsg);
		}
	}
}
#if 0
	...
	switch(stCbusCmd.stReqMsc.u1MscSta)
	{
		case SINK_CBUS_STATE_NONE:
			if(fgCbusMscAck(u2RxMsg))
			{
				...
				if(u1DebugLvl == 2)
				{
					LOG(0,"READ_DCAP ACK Received:%s,%d\n",__FUNCTION__,__LINE__);
				}
			}
			else if(fgCbusMscNack(u2RxMsg)||fgCbusMscAbort(u2RxMsg))
			{
		...
		case SINK_CBUS_STATE_OFFSET:
			if(fgCbusMscDate(u2RxMsg))
			{
				_arMhlSrcDevCap[ReadDevCapOffset & 0xFF] = vCbusGetMscData(u2RxMsg);
				ReadDevCapOffset ++;
				stSinkCtr.skIsRChg.u1RdDcap = 1;//Ready for next reading
				if(ReadDevCapOffset == CBUS_MSC_DEVCAP_INT_STATE_SIZE + 1 )
				{
					stSinkCtr.skIsRChg.u1RdDcap = 0;//Stop reading.
					ReadDevCapOffset = CBUS_MSC_DEVCAP_DEV_STATE;//for nexting reading.
				}
			}
			else if(fgCbusMscNack(u2RxMsg)||fgCbusMscAbort(u2RxMsg))
			{
			...
			vCbusRstMscState();// this command is finished.
			break;
		default:
			break;
	}
		...
		case SINK_CBUS_STATE_DATA:
			if(fgCbusMscDate(u2RxMsg))
			{
				...
				else if(u1Data0 == CBUS_MSC_REG_REGCHANGE_INT)
				{
					 ...
					 {
				 		//write burst finished
						stSinkCtr.skIsSendWriteBurst = WRITE_BURST_BEGIN;
					 }						
					 else
...
{  
	...
	if(u4IntStat & 0x400)
	{
		...
		iMhlStateNew = MHL_STATE_INIT;
		vCBusSetDiscoveryNG();
		vCbusSetVbus(FALSE);//disable vbus
	...
	if(fgCBusGetMscTimeOut())
	{
		...
		vCbusRstMscState();		
		if(stCbusCmd.stReqMsc.u2MscCmd == CBUS_MSC_CTRL_READ_DEVCAP)
		{
			stSinkCtr.skIsRChg.u1RdDcap = 1;
			LOG(0,"Current READ_DCAP Fail,Try Again\n");
		}
		LOG(0,"mto\n");
	}
...
void vCbusStatusQuery(void)
{
	...
	LOG(0,"stSinkCtr.skIsSendWriteBurst = %d\n",stSinkCtr.skIsSendWriteBurst);
#ifdef CC_MHL_3D_SUPPORT
	LOG(0,"VicWriteburstCnt = %d,DtdWriteburstCnt = %d\n",VicWriteburstCnt,DtdWriteburstCnt);
#endif
}
...
void vCbusRequestSendSratchPad(CBUS_MSG_DESCRIPTION_T *msg)
{
	...
	//vCbusRequestWriteBurst();
	stSinkCtr.skIsSendWriteBurst = WRITE_BURST_HAL_REQ;
...
/*
@vCbusSendWriteBurstHalCmd: the command from the MHL sevice on Top Layer.
*/
void vCbusSendWriteBurstHalCmd(void)
{
	if(u1CbusMscAbortDelay2S)
		return;
	stSinkRequster.req_state = SINK_CBUS_REQ_BEGIN;
	stSinkRequster.u2ReqBuf[0] = CBUS_MSC_CTRL_WRITE_BURST;
	stSinkRequster.u2ReqBuf[1] = scratchPadData.offset | 0x400;// + (VicWriteburstIdx << 4);//40~7F
	stSinkRequster.u2ReqBuf[2] = scratchPadData.Data[0] | 0x400;
	stSinkRequster.u2ReqBuf[3] = scratchPadData.Data[1] | 0x400;
	stSinkRequster.u2ReqBuf[4] = scratchPadData.Data[2] | 0x400;
	stSinkRequster.u2ReqBuf[5] = scratchPadData.Data[3] | 0x400;
	stSinkRequster.u2ReqBuf[6] = scratchPadData.Data[4] | 0x400;
	stSinkRequster.u2ReqBuf[7] = scratchPadData.Data[5] | 0x400;
	stSinkRequster.u2ReqBuf[8] = scratchPadData.Data[6] | 0x400;
	stSinkRequster.u2ReqBuf[9] = scratchPadData.Data[7] | 0x400;
	stSinkRequster.u2ReqBuf[10] = scratchPadData.Data[8] | 0x400;
	stSinkRequster.u2ReqBuf[11] = scratchPadData.Data[9] | 0x400;
	stSinkRequster.u2ReqBuf[12] = scratchPadData.Data[10] | 0x400;
	stSinkRequster.u2ReqBuf[13] = scratchPadData.Data[11] | 0x400;
	stSinkRequster.u2ReqBuf[14] = scratchPadData.Data[12] | 0x400;
	stSinkRequster.u2ReqBuf[15] = scratchPadData.Data[13] | 0x400;
	stSinkRequster.u2ReqBuf[16] = scratchPadData.Data[14] | 0x400;
	stSinkRequster.u2ReqBuf[17] = scratchPadData.Data[15] | 0x400;
	stSinkRequster.u2ReqBuf[18] = CBUS_MSC_CTRL_EOF;
	stSinkRequster.u1Len = 19;
	while(stSinkRequster.req_state == SINK_CBUS_REQ_BEGIN)
	{
		vCbusTriggerCmpInterrupt();
		x_thread_delay(4);
...
void vDeviceCapShow(void)
{
	UINT8 i;
	for(i = 0;i < 16;i += 4)
	{
		LOG(0,"SRC_DCAP 0x%08x: 0x%02x,0x%02x,0x%02x,0x%02x\n",
			CBUS_MSC_DEVCAP_DEV_STATE + i,
			_arMhlSrcDevCap[i],
			_arMhlSrcDevCap[i + 1],
			_arMhlSrcDevCap[i + 2],
			_arMhlSrcDevCap[i + 3]);
...
void vCbusCmdTest(UINT8 cmd_id, UINT8 sub_cmd_id)
{
	...
	// set path en
		...
		{
			LOG(0,"SET PATH EN{0} \n"); 
			vCbusSetPathEn(PATH_EN_NAGTIVE);
		}
	break;
	...
	break;
	case 11:
	    LOG(0,"RAP polling test \n");
		vCbusSendRapKeys((UINT8)(MHL_RAP_CMD_POLL & 0xFF));
		break;
	case 12:	
		LOG(0,"RAP content on test \n");
		vCbusSendRapKeys((UINT8)(MHL_RAP_CMD_CONTENTON & 0xFF));
	    break;
	case 13:
		LOG(0,"RAP content off test \n");
		vCbusSendRapKeys((UINT8)(MHL_RAP_CMD_CONTENTOFF & 0xFF));
	    break;
	case 23:
		if(sub_cmd_id == 1)
		{
			VERIFY( x_sema_unlock(_hCbusCommandSema)==OSR_OK);
			LOG(0,"Wake up vCbusCommandLoop() mannually!\n");
		}
		else
		{
			VERIFY( x_sema_unlock(_hCbusSema)==OSR_OK);
			LOG(0,"Wake up vCbusMainLoop() mannually!\n");
		}
		break;
	case 24:
		LOG(0,"Print the source's capability:\n");
		vDeviceCapShow();
		break;
	case 25:
		LOG(0,"MHL Term Dis/EN(0/1):\n");
		if(sub_cmd_id == 1)
		{
			vMHLOpenRxTermination(1);
		}
