...
#define TVD_CTRL_STABLE_MCNT 0   //TVD need 12 vsync modechangedone time 6vsync will have side effect
 ...
 *************************************************************************/
#ifdef CC_SOURCE_AUTO_DETECT
	BOOL _fgClampOnBottomToggle = TRUE;
	UINT8 sbClampOnBottomTogCnt = 0;
	UINT8 sbVpresSVFCnt = 0;
#endif
...
static void _svDrvTvdSetMenuPvDelay(UINT8 bTvdMode, UINT16 wPvDelay)
{

    vIO32WriteFldAlign(TG_0A, wPvDelay, TG_SAF_ADJ);
}
...
void vTvd3dVSyncISR(void)
{
    ...
    */
    ...
    if (_rTvd3dStatus.eSourceType==SV_ST_TV)
    {
        if(_fgClampOnBottomToggle == TRUE)
        {
            if(fgHwTvdVPresSVF() ==FALSE)
            {
                if(sbClampOnBottomTogCnt <= 80)
                {
                    sbClampOnBottomTogCnt = sbClampOnBottomTogCnt + 1;
                    vRegWriteFldAlign(VSRC_07, 1, VSRC_SWAP_AD01);
                }
                else if(sbClampOnBottomTogCnt <= 160)
                {
                    sbClampOnBottomTogCnt = sbClampOnBottomTogCnt + 1;
                    vRegWriteFldAlign(VSRC_07, 0, VSRC_SWAP_AD01);
                }
                else
                {
                    sbClampOnBottomTogCnt = 0;
                }
            }
            else
            {
               sbVpresSVFCnt = sbVpresSVFCnt + 1;
               if(sbClampOnBottomTogCnt <= 80)
               {
                    sbClampOnBottomTogCnt = 0;
               }
               ...
               {
                    sbClampOnBottomTogCnt = 81;
               }
               if(sbVpresSVFCnt == 2)
               {
                    _fgClampOnBottomToggle =FALSE;
               }
            }
        }
    }
...
#ifdef CC_SOURCE_AUTO_DETECT
	   vRegWriteFldAlign(VSRC_07, 0x01, VSRC_SWAP_AD01);
...
void vAVCompAutoDet(void)
{
    ...
    if(u1Status == _u1AVCompStatus)
    {
        ...
        if(fgDrvTvdIsAVCompCnt(TVD_AVCOMP_CNT))
        {
            ...
            if(fgHwTvdBLock()&&(bHwTvdCAGC()<0x40)) //10% burst level
            {
                u1Status_2 = SV_VDO_NOSIGNAL;
            }
            else
            {
                u1Status_2 = u1DrvGetCompStatus() ;
            }

            LOG(3, "TVD AVCompo Notify(%d)\n", u1Status_2);
            vApiNotifyAVCOMPAuto(path, u1Status_2);
        }
    }
    else
    {
...
#ifdef CC_SOURCE_AUTO_DETECT
    vIO32WriteFldAlign(VSRC_02, 0x0, CVBS_ADC_IDX_SEL); //for Source detect by zuyu
#else
...
void vTVDAVDelay(UINT8 bDelay)        // For CLI
{
	...
	LOG(0, "_svDrvTvdSet_HPORCH_Delay HPORCH_Delay = 0x%02X\n", (bDelay));
}

void vTVDPVDelay(UINT8 bDelay)        // For CLI
{
    _svDrvTvdSetMenuPvDelay(_rTvd3dStatus.bTvdMode, bDelay);
	LOG(0, "_svDrvTvdSet_VPORCH_Delay VPORCH_Delay = 0x%02X\n", (bDelay));
}
