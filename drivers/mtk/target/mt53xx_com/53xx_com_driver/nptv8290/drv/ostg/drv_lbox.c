...
static void vLBoxSetToneDetectionRange(UINT32 u4Left, UINT32 u4LeftSeperate, UINT32 u4Right, UINT32 u4RightSeperate)
{
    if (u4Left != LBOX_IGNORE)
    {
      if (IS_IC_5861())
      {
         vIO32WriteFldAlign(SIDE_LBOX_07, u4Left, C_LEFT_BOUND_4K);
      }
	  else
	  {
    ...
    if (u4Right != LBOX_IGNORE)
    {
	    if (IS_IC_5861())
	    {
			vIO32WriteFldAlign(SIDE_LBOX_07, u4Right, C_RIGHT_BOUND_4K);
	    }
		else
		{
...
static void LboxCheckHorizontalChroma(UINT32 _u4LBoxLeftCurrent,UINT32 _u4LBoxRightCurrent)
{
    ...
    if ( IS_IC_5861())
    {
    	u4UpBound = IO32ReadFldAlign(SIDE_LBOX_00, C_UP_BOUND_4K);
    	u4LowBound = IO32ReadFldAlign(SIDE_LBOX_00, C_LOW_BOUND_4K);
	}
	else
	{
...
static void LboxCheckVerticalChroma(UINT32 _u4LBoxUpCurrent,UINT32 _u4LBoxLowCurrent)
{
	...
	if ( IS_IC_5861())
	{
    	vIO32WriteFldAlign(SIDE_LBOX_00, u4Upbound, C_UP_BOUND_4K);
    	vIO32WriteFldAlign(SIDE_LBOX_00, _rBoundData[LBOX_IDX_UP].u4FinalBound, C_LOW_BOUND_4K);
	}
	else
	{
	...
	if ( IS_IC_5861())
	{
    	vIO32WriteFldAlign(SIDE_LBOX_00, _rBoundData[LBOX_IDX_DOWN].u4FinalBound, C_UP_BOUND_4K);
    	vIO32WriteFldAlign(SIDE_LBOX_00, u4Lowbound, C_LOW_BOUND_4K);	   
	}
	else
	{
										...
										)
{
    if (bLBOXIsInput444() == 0)    
    {
        ...
        if ( IS_IC_5861())
        {
			u4LeftBound = IO32ReadFldAlign(SIDE_LBOX_07, C_LEFT_BOUND_4K);
			u4RightBound = IO32ReadFldAlign(SIDE_LBOX_07, C_RIGHT_BOUND_4K);
        }
		else
		{
        ...
        if ( IS_IC_5861())
        {
			vIO32WriteFldAlign(SIDE_LBOX_07, u4LeftBound, C_LEFT_BOUND_4K);
			vIO32WriteFldAlign(SIDE_LBOX_07, u4RightBound, C_RIGHT_BOUND_4K);
        }
		else
		{
...
static void LboxReconfig(void)
{
	...
	vLBoxGetInputResolution(&_u4LBoxSrcWidth, &_u4LBoxSrcHeight);
        ...
        if ( IS_IC_5861())   //lbox_4k
        {
		  vIO32WriteFldAlign(LBOX_00, _rBoundData[LBOX_IDX_UP].u4DetectBound, LBOX_UP_BOUND_4K);
		  vIO32WriteFldAlign(LBOX_00, _rBoundData[LBOX_IDX_DOWN].u4DetectBound, LBOX_LOW_BOUND_4K);
		  vIO32WriteFldAlign(LBOX_04, u4LeftBound, LBOX_LEFT_BOUND_4K);
		  vIO32WriteFldAlign(LBOX_04, u4RightBound, LBOX_RIGHT_BOUND_4K);
        }
		else
		{
        ...
        // Update Config Register
	 	...
	 	if (IS_IC_5861())
	 	{
        	vIO32WriteFldAlign(SIDE_LBOX_00, u4UpBound, C_UP_BOUND_4K);
	        vIO32WriteFldAlign(SIDE_LBOX_00, u4LowBound, C_LOW_BOUND_4K);
			vIO32WriteFldAlign(SIDE_LBOX_07, _rBoundData[LBOX_IDX_LEFT].u4DetectBound, C_LEFT_BOUND_4K);
			vIO32WriteFldAlign(SIDE_LBOX_07, _rBoundData[LBOX_IDX_RIGHT].u4DetectBound, C_RIGHT_BOUND_4K);
	 	}
		else
		{
        	vIO32WriteFldAlign(SIDE_LBOX_00, u4UpBound, C_UP_BOUND);
...
static UINT8 LboxDetectionGetResult(UINT32 *u4YIsSmall, UINT32 *rBeforeIIR, UINT32* rAfterIIR)
{
    ...
    *u4YIsSmall = u4Small;
    ...
    else
    {
    	...
    	if ( IS_IC_5861())
    	{
    		u4Low= IO32ReadFldAlign(LBOX_STA_00, STA_LBOX_Y_LOW_LINE_4K);
    		u4Up= IO32ReadFldAlign(LBOX_STA_00, STA_LBOX_Y_UP_LINE_4K);
    	}
		else
		{
    ...
    if (IS_IC_5861())
    {    
    	u4LeftMin = IO32ReadFldAlign(SIDE_LBOX_STA_02, STA_BLACK_LEFT_POS_MIN_4K);
		u4RightMax = IO32ReadFldAlign(SIDE_LBOX_STA_01, STA_BLACK_RIGHT_POS_MAX_4K);
    }
	else
	{
		u4LeftMin = IO32ReadFldAlign(SIDE_LBOX_STA_02, STA_BLACK_LEFT_POS_MIN);
