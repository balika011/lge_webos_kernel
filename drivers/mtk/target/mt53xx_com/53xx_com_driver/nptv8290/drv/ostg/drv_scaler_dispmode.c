...
void vScpipModeSwitch(UINT8 bPath)
{
    ...
    if ( bPath == SV_VP_MAIN)
    {
        if (HYBRID_MODE) //mode == VDP_SCPOS_HYBRID_MODE )	
        {
            //it is the sam as front dram mode
            vScpipSetDramMode(bPath,SV_ON);
            vScpipSetLineSyncDispmode(bPath,SV_OFF);
            vScpipSetPSCANDispmode(bPath,SV_OFF);
            //hybrid mode only
            vScpipFscFrcLinePulseSel(1);  //FRC line pulse select to MIB
    	     vScpipFscDramReadPtrSel(1);   //dram read pointer select to MIB
            vScpipFscMainVsync_Sel(ON);   ////in hybrid mode, the set resolution vsync is come from MIB
            vScpipDisableDramWriteARB(SV_VP_MAIN, SV_OFF);  // disable dram write  workaround because write should be enabled for set resoluiton   
    	    return;
       }
    	else
    	{	
    	    vScpipFscFrcLinePulseSel(0);  //FRC line pulse select to FSC
    	    vScpipFscDramReadPtrSel(0);   //dram read pointer select to FSC
    	    vScpipFscMainVsync_Sel(OFF);   ////in normal mode, the set resolution vsync is come from FSC
    	    vScpipDisableDramWriteARB(SV_VP_MAIN, SV_ON);  // disable dram write  workaround because write should be enabled for set resoluiton   
    	}
    }

    if (mode == VDP_SCPOS_PSCAN_DISPMODE) 	//pscan dispmode && hybrid mode
    {
...
UINT32 vScpipGetPoclk( void )
{
   return POCLK_SOURCE_CLK;
}




void vScpipSetSCTimeGen(E_TG_MODULE eModule, UINT16 u2HTotal,UINT16 u2HActive, UINT16 u2VTotal, UINT16 u2VActive)
{    
     //update FSC output time gen.
     if(u4ScpipGetPanelHtotal( eModule ) != u2HTotal)
     {
          LOG(3,"Update module_%d PANEL HTOTAL from (%d) to (%d)\n",eModule, u4ScpipGetPanelHtotal(eModule), u2HTotal);     
          vScpipSetPanelHtotal(eModule,u2HTotal);
     }
    
     if(u4ScpipGetPanelWidth(eModule) != u2HActive)
     {
         LOG(3,"Update module_%d PANEL HACTIVE from (%d) to (%d)\n",eModule, u4ScpipGetPanelWidth(eModule), u2HActive);
         vScpipSetPanelWidth(eModule,u2HActive);            
     }
    
     if(u4ScpipGetPanelVtotal(eModule) != u2VTotal)
     {
         LOG(3,"Update module_%d PANEL VTOTAL from (%d) to (%d)\n",eModule, u4ScpipGetPanelVtotal(eModule), u2VTotal);
         vScpipSetPanelVtotal(eModule, u2VTotal);           
     }
        
     if(u4ScpipGetPanelHeight(eModule) != u2VActive)
     {
         LOG(3,"Update module_%d PANEL VACTIVE from (%d) to (%d)\n",eModule, u4ScpipGetPanelHeight(eModule), u2VActive);
         vScpipSetPanelHeight(eModule,u2VActive);           
     }
}



void vScpipSetSCTimeGenPorch(E_TG_MODULE eModule,UINT16 u2HFPorch,UINT16 u2HBPorch, UINT16 u2VFPorch, UINT16 u2VBPorch)
{
    //update FSC output time gen.
    if(u4ScpipGetHsyncFp(eModule) != u2HFPorch)
    {
    	LOG(3,"Update module=%d PANEL HF Porch from (%d) to (%d)\n",eModule, u4ScpipGetHsyncFp(eModule), u2HFPorch);     
		vScpipSetHsyncFp(eModule,u2HFPorch);
    }

    if(u4ScpipGetHsyncBp(eModule) != u2HBPorch)
    {
    	LOG(3,"Update module=%d PANEL HB Porch from (%d) to (%d)\n",eModule, u4ScpipGetHsyncBp(eModule), u2HBPorch);     
		vScpipSetHsyncBp(eModule,u2HBPorch);
    }    

    if(u4ScpipGetVsyncFp(eModule) != u2VFPorch)
    {
    	LOG(3,"Update module=%d PANEL VF Porch from (%d) to (%d)\n",eModule, u4ScpipGetVsyncFp(eModule), u2VFPorch);     
		vScpipSetVsyncFp(eModule,u2VFPorch);
    } 

    if(u4ScpipGetVsyncBp(eModule) != u2VBPorch)
    {
    	LOG(3,"Update module=%d PANEL VB Porch from (%d) to (%d)\n",eModule, u4ScpipGetVsyncBp(eModule), u2VBPorch);     
		vScpipSetVsyncBp(eModule,u2VBPorch);
    }     
}
...
SC time generator
*/
void vScpipSetTimeGenEnable( E_TG_MODULE u1Module , UINT8 u1OnOff )
{
    switch ( u1Module )
    {
        case E_FSC_TG:
            vScpipFscTGEnable(u1OnOff);
        break;    
        case E_PSC_TG:
            vPscTGEnable(u1OnOff);
        break;    
        case E_GFX_TG:
            vScpipGfxTGEnable(u1OnOff);
        break;  
        case E_ALL_TG:
            vScpipFscTGEnable(u1OnOff);
            vPscTGEnable(u1OnOff);
        ...
        break;
        default:
             LOG(3,"Error:Input wrong TimeGen module!");
        break;
    }
}
/*
0: Master mode (Enable at anytime)
1: Slave mode (Enable wait till next reference VSYNC pulse)
*/
void vScpipSetSlaveMode( E_TG_MODULE u1Module , UINT8 u1OnOff )
{

   switch ( u1Module )
   {
      case E_FSC_TG:
        vScpipFscTGSalveMode(u1OnOff);
      break;    
      case E_PSC_TG:
        vPscTGSlaveMode(u1OnOff);
      break;    
      case E_GFX_TG:
        vScpipGfxTGSlaveMode(u1OnOff);
      break;        
      case E_ALL_TG:
        vScpipFscTGSalveMode(u1OnOff);
        vPscTGSlaveMode(u1OnOff);
      ...
      break;
      default:
         LOG(3,"Error: slave mode Input module is wrong!");
      break;
  }
}
/*
Sync method in slave mode
0: Sync once at enable moment   <<  suggestion setting>>
1: Sync at every reference VSYNC pulse
*/
void vScpipSetSlaveSyncMode( E_TG_MODULE u1Module , UINT8 u1OnOff )
{
       switch ( u1Module )
       {
          case E_FSC_TG:
            vScpipFscTGSalveSyncMode(u1OnOff);
          break;    
          case E_PSC_TG:
            vPscTGSlaveSyncMode(u1OnOff);
          break;    
          case E_GFX_TG:
            vScpipGfxTGEnable(u1OnOff);
          break;  
