...
#ifdef  CC_SUPPORT_4K2K    //CC_MT5890 
...
UINT8 _fgFrametrackBaseOnLRByCLI =0;   

#ifdef   USE_FRAME_TRACK_WA
UINT32 u4FrmTrkStep,u4BkFrmTrkStep;
#endif
...
VDP_SCALER_PRM_T _arScalePrm[VDP_NS] =
{
    {                                                      //SV_VP_MAIN
        ...
        0,                                                 //u4Bypass;
        #endif
        0,                                                 //Output planee: VDP_PQPATH or VDP_GFXOUT
...
#endif
void  vScpipSetFrmTrkRefVsync( void );
...
void vScpipPrintCurrentScale(UCHAR ucVdpId)
{
    ...
    if (isChannelOn != SV_OFF)
    {
        ...
        Printf("Channel Info Path(%d) ChOn(%d) GoFz(%d) Fz(%d) 444(%d) RGB(%d) DecT(%d)\n",
                ucVdpId, 
                channel->bIsChannelOn,
...
static void _vScpipConfigFrameTrack(UINT32 u4Step)
{
    ...
    if ( IS_IC_5861())
       inRes = VRMGetVRMInfo(SV_VP_MAIN, (((bMode == VDP_SCPOS_PSCAN_DISPMODE)||(HYBRID_MODE))?VRM_MODULE_MIB:VRM_MODULE_FSC));
    else
    ...
    else
    {
        ...
        if(VDP_RTNeedSuperGameMode(VDP_1))
        {
    ...
    //6. Calculate Lock/Tor window size
    ...
    //7. Determine track based on L/R
    u4Scl  = u4DrvTDTVScalerModeQuery();
	eSclIn = TD_SCL_IN(u4Scl);
	eScOut = TD_SCL_OUT(u4Scl);
    fgLRTrack=0;

    if ( TD_MIB_IN(u4DrvTDTVDIModeQuery() > E_TDTV_DECODER_INPUT_2D_P ) )
    {  //if 3d input signal....
		  if ( HYBRID_MODE )
		  {
			vScpipFscMainDispR_Sel(ON); //in hybrid mode, LR signal is come from MIB
		  }
		  else
    	  {
			vScpipFscMainDispR_Sel(OFF); //in normal mode, LR signal is come from FSC
		  } 
        if(_fgFrametrackBaseOnLRByCLI)
    	{
    	    fgLRTrack = _fgFrametrackBaseOnLRByCLI;
    	}
		else
		{
		    if((E_TD_IN_FS_I==TD_MIB_IN(u4DrvTDTVDIModeQuery())) || (E_TD_IN_FS_P==TD_MIB_IN(u4DrvTDTVDIModeQuery())))
		     {
		        fgLRTrack|=(FRM_TRK_MASK_IN_EN | FRM_TRK_MASK_IN_RIGHT);
		     }
			if( (eSclIn == E_TD_IN_FS_I || eSclIn == E_TD_IN_FS_P) && ( eScOut == E_TD_OUT_NATIVE ))
		     {
		        fgLRTrack|=( FRM_TRK_3D_EN | FRM_TRK_3D_LOCK_AT_L );
		     }
		}
   }
	//
...
//    }
    ...
    //7. set frame trk check in input vsync rising /falling edge
    vScpipSelFrmTrkVsyncInEdge(0);  //0 falling edge, q: risng edge

    //8. set frame trk not cross the vsync
    vScpipFrmTrkCrossVsync(0);  //0: not cross Vsync, 1: frm_trk in  shortest path
}
...
void  vScpipSetFrmTrkRefVsync( void )
{
    ...
    if (vDrvGetTconTopVdoSel())  //if by_pass path
    {  //in  4k input (for HDMI only, B2R is no need ) , reference to HDMI input vsync  
		if(fgIsMainDVI())
       {
         vScpipSelFrmTrkRefVsync(SelFrmTrkRefHDMI);  //FRM SYNC to HDMI 
       }
	   else
	   {
	     vScpipSelFrmTrkRefVsync(SelFrmTrkRefB2R); //b2r
	   }
    }
    else
#endif        
    {   
      bMode = getScalerMode(SV_VP_MAIN);
       if ( HYBRID_MODE ) //bMode == VDP_SCPOS_HYBRID_MODE) HYBRID_MODE
       {
          vScpipSelFrmTrkRefVsync(SelFrmTrkRefMIB);  
		  return;
       }
      if(bMode == VDP_SCPOS_PSCAN_DISPMODE)
      {
      ...
      {
          //in other mode, reference vsync of scaler input
          vScpipSelFrmTrkRefVsync(SelFrmTrkRefScaler); 
      }
    }
}
/*
...
*/
void vScpipFrmTrkInLREnable(UINT8 fgLRTrack  )
{
      if ( fgLRTrack & FRM_TRK_3D_EN )  // if input is 3D format 
      {
          vScpipFrmTrk3DEnable( SV_ON ); 
          if (  fgLRTrack & FRM_TRK_3D_LOCK_AT_L )
             vScpipFrmTrkDispR( L_FRAME );
          else            
             vScpipFrmTrkDispR( R_FRAME );
      }
      else
          vScpipFrmTrk3DEnable( SV_OFF ); 
  
      //FS_3D_mode need mask input frame.
      if ( fgLRTrack & FRM_TRK_MASK_IN_EN )
      {
          vScpipFrmTrkMaskInLREnable(SV_ON);
          if ( fgLRTrack & FRM_TRK_MASK_IN_RIGHT )
              vScpipFrmTrkIMaskInLRSel( R_FRAME );
          else            
              vScpipFrmTrkIMaskInLRSel( L_FRAME );
      }
      else
      {
          vScpipFrmTrkMaskInLREnable(SV_OFF);
      }
}
...
void vScpipSwitchFrameTrackOnOff(UINT32 u4FscOnOff, UINT32 u4PscOnOff) 
{
	...
	static UINT32 u4PreFscTrackMode = 0xFF;
	static UINT32 u4FscTrackMode = 0xFF;

	if(fgFrameTrackIsFlgSet(FRAME_TRACK_MODE_DDDS_BY_FNTSCALER)) // DDDS frame track mode
	{
		u4FscTrackMode = FRAME_TRACK_MODE_DDDS_BY_FNTSCALER;
	}
	else if(fgFrameTrackIsFlgSet(FRAME_TRACK_MODE_VTOTAL_BY_FNTSCALER)) // Vtotal frame track mode
	{
		u4FscTrackMode = FRAME_TRACK_MODE_VTOTAL_BY_FNTSCALER;
	}

    if (u4FscStatus != u4FscOnOff || u4PscStatus != u4PscOnOff || u4PreFscTrackMode != u4FscTrackMode)
    {
        u4FscStatus = u4FscOnOff;
        u4PscStatus = u4PscOnOff;
		u4PreFscTrackMode = u4FscTrackMode;
...
void vScpipSwInit(void)
{
    UINT32 u4PlaneW;
	UINT32 u4PlaneH;
	u4PlaneW = PANEL_GetPanelWidth();
	u4PlaneH = PANEL_GetPanelHeight();
    vSetPlaneEnable(VDP_PQPATH, SV_ON);
#ifdef CC_SUPPORT_4K2K
	if(IS_SUPPORT_4K2K_DISPLAY)
	{
		if(GetPANEL4K2K_TYPE() != PANEL_TYPE_MAX)
		{
			u4PlaneW = u4GetPlaneWidth(VDP_PQPATH);
			u4PlaneH = u4GetPlaneHeight(VDP_PQPATH);
		}
		else if(IS_DISP_4K2K_TYPE3 && (u4PlaneW == 3840) && (u4PlaneH == 1080))//type3 default work on 4k1k
		{
		    u4PlaneW = 1920;
			u4PlaneH = 1080;
		}
...
#endif
    vSetPlaneWidth(VDP_PQPATH, u4PlaneW);
    vSetPlaneHeight(VDP_PQPATH, u4PlaneH);
...
/**
 * @brief Set Super game mode (DRVCUST)
 * @param ucVdpId VDP_1/VDP_2
 * @param ucOnOff On/Off
 * @6896 porting done
 */
void vScpipSetSuperGameMode(UCHAR ucVdpId, UCHAR ucOnOff)
{
    ...
    if (_arScalePrm[ucVdpId].u4IsSuperGameMode != ucOnOff)
    {
        LOG(3, "VDP(%d) SuperGame Mode(%d)\n", ucVdpId, ucOnOff);
        _arScalePrm[ucVdpId].u4IsSuperGameMode = ucOnOff;
        vScpipReconfig(ucVdpId);
    }
}
...
void vScpipSetForced444Mode(UINT32 u4VdpId,UINT32 u4Mode)
{
    ...
    if((u4VdpId>= VDP_NS)||(HYBRID_MODE != FALSE ))
    {
        return;
    }
...
UINT32 u4ScpipUpdateDramInputFmt(UINT8 bPath)
{
    ...
    if ( ( bPath==VDP_1 ) && HYBRID_MODE )
    {
        return 0;  //  422 
    }

    else if(getScalerMode(bPath) == VDP_SCPOS_DISPMODE_OFF)
    {
...
static void _vScpipSetMirrorFlipReg(UINT32 u4VdpId)
{
    ...
    if(HYBRID_MODE)
    {
    	vScpipSetHybridModeFlipMirror((UINT8)u4VdpId, bMirrorFlipMode);
    }   
	else
    {
    	vScpipSetFlipMirror((UINT8)u4VdpId, bMirrorFlipMode);
...
static void _vScpipUpdateInOutPrm(UINT8 bPath)
{
    ...
    //get time gen info
    VRMGetModuleVRMInfo(bPath, VRM_MODULE_VDP, &(dummy), &(scalerInfo->fscTimeGen));

    if (SRM_GetScposMode(bPath) & FBM_POOL_MODE_SCPOS_HYBRID_DISP)
    {
        VRM_INFO_T tMIBIn;
        VRMGetModuleVRMInfo(bPath, VRM_MODULE_MIB, &(tMIBIn), &(dummy));
        scalerInfo->fscIn = tMIBIn;
    }
}
...
void _vScpipUpdateDramWidthHeightPrm(UINT8 bPath, UINT32 inW, UINT32 inH, UINT32 outW, UINT32 outH)
{
	...
	{
	    SCALER_TDTV_IN_TYPE     tTDInType   = eScpipGetTdtvInType(bPath);
           SCALER_TDTV_OUT_TYPE  tTDOutType = eScpipGetTdtvOutType(bPath);
...
static void _vScpipUpdateDramFrameDelay(UINT8 bPath)
{
    ...
    if(bDrvVideoSignalStatus(bPath)==(UINT8)SV_VDO_STABLE && 
        (bScalerMode == VDP_SCPOS_DISPMODE_OFF    || 
         bScalerMode == VDP_SCPOS_HYBRID_MODE  )  &&
       _bSRMStable[bPath]==1 &&
...
void vScpipClearNeedUpdateFlag(UINT8 bPath)
{
    VDP_SCALER_PRM_T* scalerInfo;
...
static void _vScpipUpdateDispPrm(UINT32 u4VdpId) 
{
    ...
    _vScpipUpdateDramWidthHeightPrm(bPath, scalerInfo->fscIn.u2Width, scalerInfo->fscIn.u2Height,
                                             scalerInfo->u4OutputWidth,scalerInfo->u4OutputHeight);
    ...
    {    
        ...
        _vScpipUpdateVScalingModePrm(bPath,scalerInfo->u4InputHeight,scalerInfo->u4OutputHeight);
        //9. Update V scaling factor of partial sum
        _vScpipUpdateVPartialSumFactomPrm(bPath, scalerInfo->u4InputHeight,scalerInfo->u4OutputHeight);
    }
    else
    {
         //7. Update scaling factor
...
void vScpipSetRptGen3D( UINT32 u4VdpId)
{
	if(u4VdpId ==  SV_VP_PIP)
	{
		return ;
	}
	
	if(TD_MIB_IN(u4DrvTDTVDIModeQuery() > E_TDTV_DECODER_INPUT_2D_P))	//main && 3d input
	{
		vScpipSetScRptDisp_R_EN(SV_ON);
		vScpipSetScRptDisp_R(SV_ON);
	}
	else
	{
		vScpipSetScRptDisp_R_EN(SV_OFF);
		vScpipSetScRptDisp_R(SV_OFF);	
	}
}
...
{
    LOG(1, "_vScpipSet4K2KDramMode: %d\n", u2Width);

    if (u2Width > 1920 || HYBRID_MODE)
    {
	...
	{
	   vScpipForce2dMode(SV_ON);
	}
	else
	{
		vScpipForce2dMode(SV_OFF);
	}
}
...
static void _vScpipSetDispReg(UINT32 u4VdpId)
{
    ...
    vScpipSetReadPosGenAuto(bPath, 0);
     if ( IS_IC_5861())
     {
	 	vScpipSetRptGenVtotalSel(bPath, 0);  // this added register do not work well in gazelle,overscan cause garbage
	 	vScpipSetRptGen3D(bPath);
     }
    ...
    if(u1SetScalingCoef == 1&& bPath==SV_VP_MAIN)
    {
		...
		DRVCUST_SendEvent(E_CUST_OUT_SCALING_CHANGE, SV_VP_MAIN);
	...
	if(getScalerInfo(bPath)->u4DispMode == VDP_SCPOS_HYBRID_MODE)
	{
	 	vScpipSetFrontMode(bPath,SV_ON);
	 	vScpipSetBackMode(bPath,SV_OFF);
	}
    else if(getScalerInfo(bPath)->u4DispMode == VDP_SCPOS_DISPMODE_OFF)
    {
        if(scalerInfo->u4IsDramFrontMode==SV_TRUE)
        {
            vScpipSetFrontMode(bPath,SV_ON);
...
static void _vScpipUpdateDramPrm(UINT32 u4VdpId)
{
    ...
    if (u4FbmTotalSize == 0 || (getScalerMode(bPath) != VDP_SCPOS_DISPMODE_OFF) || (_bSRMStable[bPath] != SV_TRUE))
    {
        return;
    }
    ...
    LOG(1, "Scaler[%d] width (%d) height (%d)\n", bPath, scalerInfo->u4DramHPitch, scalerInfo->u4DramVPitch);
    ...
    {
        LOG(0, "After scaling FBM buffer[%d] to small!!! %d < %d + %d\n", bPath, u4FbmTotalSize, u4ScaledPage, u4Padding);
    }
...
extern UINT8 pscStatus2Line;
extern UINT8 vPsc2MjcGetMcOriStatus( void );
#endif
...
static void _vDrvScpipISR(UINT16 u2Vector)
{
    ...
    if(u4ScpipGetISRstatus(ISR_FSC_VSYNC_OUT_FALLING)) // 5
    {
        ...
        if( u1Mchg == 1)
        {
            LOG(4, "CRC @ ISR %8X %8X changed\n", CRC_Min, CRC_Mout);    
        }
        
        LOG(7, "ISR_FSC_VSYNC_OUT_FALLING %d\n", u4ScpipGetOutVCnt());
        
        if(bScpipIsNeedSetUpdate(SV_VP_MAIN) == SV_TRUE)
        {
           vScpipSetOutputResolution(SV_VP_MAIN, getScalerInfo(SV_VP_MAIN)->u4OutputWidth, getScalerInfo(SV_VP_MAIN)->u4OutputHeight);
           vScpipSetMaskStart(SV_VP_MAIN, getScalerInfo(SV_VP_MAIN)->fscOut.u2X, getScalerInfo(SV_VP_MAIN)->fscOut.u2Y); 
           vScpipClearNeedUpdateFlag(SV_VP_MAIN);
        }

        if(bScpipIsNeedSetUpdate(SV_VP_PIP) == SV_TRUE)
        {
           vScpipSetOutputResolution(SV_VP_PIP, getScalerInfo(SV_VP_PIP)->u4OutputWidth, getScalerInfo(SV_VP_PIP)->u4OutputHeight);
           vScpipSetMaskStart(SV_VP_PIP, getScalerInfo(SV_VP_PIP)->fscOut.u2X, getScalerInfo(SV_VP_PIP)->fscOut.u2Y);
           vScpipClearNeedUpdateFlag(SV_VP_PIP);
        }
        
#ifdef	USE_FRAME_TRACK_WA
		if ( u4FrmTrkStep != u4BkFrmTrkStep)
	    {
	        u4BkFrmTrkStep=u4FrmTrkStep;
		    SCPIP_WRITE_FLD(SCPIP_SCSYS_14, u4FrmTrkStep, SCSYS_14_SC_FRM_TRK_STEP);
	        LOG(4,"FrmTrk Chg = %d \n", u4FrmTrkStep );
		}
#endif	        
...
#ifdef SUPPORT_2LINE_DRV
         UINT8 bframe;
         if ( pscStatus2Line == ON )
         {
             bframe = vPsc2MjcGetMcOriStatus();
             LOG(2, "2line--%d\n",bframe) ;//vPsc2MjcGetMcOriStatus());
             if ( bframe )
             {
                 OSD_PLA_SetFastMute(0,1);
                 OSD_PLA_SetFastMute(1,1);
                 OSD_PLA_SetFastMute(2,1);
             }
			 else
			 {
				 OSD_PLA_SetFastMute(0,0);
				 OSD_PLA_SetFastMute(1,0);
				 OSD_PLA_SetFastMute(2,0);			    
			 }
         }
#endif
    ...
    if(u4ScpipGetISRstatus(ISR_DRAM_SUB_WRITE)) // 13
    {
        ...
        {
            eScalerVencFldInfo = (E_SCALER_VENC_INPUT_FLE_INFO)u1ScpipGetDramFldInfo(SV_VP_PIP);
        }
        vDrvScpipVEncISR();
        #endif
    ...
    if(u4ScpipGetISRstatus(ISR_DRAM_MON_WRITE)) // 15
    {
        ...
        {
           eScalerVencFldInfo = (E_SCALER_VENC_INPUT_FLE_INFO)u1ScpipGetDramFldInfo(2);//get monitor out field info
        }
	...
	{
	    if (u4ScpipGetISRstatus(ISR_GFX_VSYNC_RISING)) // 16
	    {
	        LOG(7, "ISR_GFX_VSYNC_RISING %d\n", u4ScpipGetGFXOutVCnt());
	        vScpipClearISRstatus(ISR_GFX_VSYNC_RISING);
	    }
	    
	    if (u4ScpipGetISRstatus(ISR_GFX_VSYNC_FALLING)) // 17
	    {
	        LOG(7, "ISR_GFX_VSYNC_FALLING %d\n", u4ScpipGetGFXOutVCnt());
	        vScpipClearISRstatus(ISR_GFX_VSYNC_FALLING);
	    }
	    
	    if (u4ScpipGetISRstatus(ISR_GFX_VDE_RISING)) // 18
	    {
	        LOG(7, "ISR_GFX_VDE_RISING %d\n", u4ScpipGetGFXOutVCnt());
	        vScpipClearISRstatus(ISR_GFX_VDE_RISING);
	    }
	        
	    if (u4ScpipGetISRstatus(ISR_GFX_VDE_FALLING)) // 19
	    {
	        LOG(7, "ISR_GFX_VDE_FALLING %d\n", u4ScpipGetGFXOutVCnt());
	        OSD_UnlockVsyncSema();
	        OSD_DoOutISR();
	        
	        if (GFXTimingInfo.u1TriggerState)
	        {
	            vDrvApplyGFXTiming();
	        }
	        vScpipClearISRstatus(ISR_GFX_VDE_FALLING);
	    }
	}
    #endif

    if(u4ScpipGetISRstatus(ISR_REF_FSC_PULSE)) // 24
    {
        LOG(7, "ISR_REF_FSC_PULSE %d\n", u4ScpipGetREFOutVCnt());
        vScpipClearISRstatus(ISR_REF_FSC_PULSE);
    }

    if(u4ScpipGetISRstatus(ISR_REF_PSC_PULSE)) // 25
    {
        LOG(7, "ISR_REF_PSC_PULSE %d\n", u4ScpipGetREFOutVCnt());
        vScpipClearISRstatus(ISR_REF_PSC_PULSE);
    }

    if(u4ScpipGetISRstatus(ISR_REF_GFX_PULSE)) // 26
    {
        LOG(7, "ISR_REF_GFX_PULSE %d\n", u4ScpipGetREFOutVCnt());
        vScpipClearISRstatus(ISR_REF_GFX_PULSE);
    }

    if(u4ScpipGetISRstatus(ISR_REF_B2R_PULSE)) // 27
    {
        LOG(7, "ISR_REF_B2R_PULSE %d\n", u4ScpipGetREFOutVCnt());
        vScpipClearISRstatus(ISR_REF_B2R_PULSE);
    }

    if(u4ScpipGetISRstatus(ISR_REF_R2R_PULSE)) // 28
    {
        LOG(7, "ISR_REF_R2R_PULSE %d\n", u4ScpipGetREFOutVCnt());
        vScpipClearISRstatus(ISR_REF_R2R_PULSE);
    }
    #endif
...
void vScpipSetISR()
{
    _vDrvScpipInitISR();
    // ISR enable
    vScpipSetISREn(ISR_FSC_VSYNC_OUT_RISING, SV_ON);
    vScpipSetISREn(ISR_FSC_VSYNC_OUT_FALLING,SV_ON);

    vScpipSetISREn(ISR_DRAM_GFX_WRITE, SV_ON); // enable graphic dump interrupt

#ifdef SUPPORT_2LINE_DRV
    vScpipSetISREn(ISR_PSC_VSYNC_OUT_FALLING,SV_ON);   //turn PSC vsync int ON
#endif



	#ifdef CC_MAPLE_CUST_DRV
	vScpipSetISREn(ISR_FSC_VDE_OUT_FALLING, SV_TRUE);
	#endif

    #if defined(CC_MT5890) && defined(SUPPORT_PANEL_GRAPH_MODE)
	if (IS_SUPPORT_VB1_GRAPH_MODE)
	{
    	vScpipSetISREn(ISR_GFX_VDE_FALLING, SV_ON);
	}
    #endif
}
