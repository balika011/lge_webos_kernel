...
UINT32 vRegenGetRegenOutClk(UINT8 bPath,REGEN_MODULE_E mod)
{ 
    REGEN_INFO_T* regenInfo;
    UINT32 u4InputClk;
    
    regenInfo = getRegenInfo(bPath,mod);
    u4InputClk=(regenInfo->regenOut.u2HTotal * regenInfo->regenOut.u2VTotal * regenInfo->regenOut.u2FrameRate);
...
static void _vRegenUpdateReg(UINT8 bPath,REGEN_MODULE_E mod)
{
    ...
    {  
        ...
        else if(mod==REGEN_MODULE_FSC_REGEN)
        {
            switch(TD_MIB_OUT(u4DrvTDTVDIModeQuery()))
            {
                case E_TD_OUT_3D_FS:
                    eInFmt =REGEN_IN_FMT_FS;
                    break;            
                case E_TD_OUT_3D_LI:
                    eInFmt =REGEN_IN_FMT_LI;
                    break;  
