...
#define OSD_HSYNC_DLY     0
#define OSD_VSYNC_DLY     0xFFFD
...
#endif
static void vPscSetOsdHVSyncDly( UINT16 u2Osd_HsyncDly, UINT16 u2Osd_VsyncDly );
...
static void _vPscUpdateOutputResolution(UINT32 outX, UINT32 outY, UINT32 outW, UINT32 outH)
{
    ...
    if(pscInfo.u4OutputX != outX || pscInfo.u4OutputY != outY)
    {
        ...
        LOG(3,"PSC SET DATA FLG = SV_TRUE\n");            
    }
...
static void _vPscUpdate2LineDrivePrm(void)
{
    if ((pscInfo.pscOut.u2FrameRate >> 1) == pscInfo.pscIn.u2FrameRate)
    {
        vPscSet2LineDrvOnOff(SV_ON);
    }
    else
    {
        vPscSet2LineDrvOnOff(SV_OFF);
    }
}
...
{    
    ...
    //5. update 2 line drive mode
    _vPscUpdate2LineDrivePrm();
}
/******************************************
*  @Cobra porting done
******************************************/
...
void vPscHwInit(void)
{
    ...
    vPscSetOsdHVSyncDly( OSD_HSYNC_DLY, OSD_VSYNC_DLY );
    vPscTGSlaveSyncMode(0); //set to recomend vale '0'.
...
static void vPscSetOsdHVSyncDly( UINT16 u2Osd_HsyncDly, UINT16 u2Osd_VsyncDly )
{
   vPscSetOsdHsyncDly( u2Osd_HsyncDly );
   vPscSetOsdVsyncDly( u2Osd_VsyncDly );
}
...
#ifdef  SUPPORT_2LINE_DRV
void vPscSet2LineDrv(UINT8 u1OnOff)
{
   pscStatus2Line=u1OnOff;
   vPscSet2LineDrvOnOff( u1OnOff); 
   vPsc2MjcSetMcOri( u1OnOff );  //if "1": from PSC, "0: form MJC
}
#endif
