...
void vScpipUpdateTdtvInfo(UINT32 u4VdpId, UINT32 u4TdtvPrm, UINT32 u4ScalerMode)
{
    ...
    SCALER_TDTV_OUT_TYPE sclOut, sclNativeOut;
    UINT8 bUpdate = SV_FALSE;
    UINT8 fgSingleHybrid, fgDualHybrid;
    UINT8 u1Scaler3DConv = TRUE;
    ...
    fgSingleHybrid = (UINT8)( (u4ScalerMode & FBM_POOL_MODE_SCPOS_HYBRID_DISP) > 0 );
    fgDualHybrid   = (UINT8)( (u4ScalerMode & FBM_POOL_MODE_SCPOS_DUAL_HYBRID_DISP) > 0 );
        
    tdtvIn = TD_SCL_IN(u4TdtvPrm);
    tdtvOut = TD_SCL_OUT(u4TdtvPrm);    
    
    tdMIBout = TD_MIB_OUT(u4DrvTDTVDIModeQuery());
    if ( fgSingleHybrid && E_TD_OUT_NATIVE != tdMIBout )
    {
        tdtvIn   = TD_MIB_IN(u4DrvTDTVDIModeQuery());
        tdtvOut = tdMIBout;
    }
    else if (fgDualHybrid)
    {
        tdtvIn   = E_TD_IN_FS_P;
        tdtvOut = E_TD_OUT_3D_SBS;
    }
    ...
    switch(tdtvIn)
    {
        ...
        case E_TD_IN_2D_P:
            ...
            sclNativeOut = SCALER_TDTV_OUT_TYPE_2D;
            u1Scaler3DConv = FALSE;
            break;
        case E_TD_IN_FS_I:   
        case E_TD_IN_FS_P:   
        case E_TD_IN_FP_I:   
        case E_TD_IN_FP_P:
            ...
            sclNativeOut = SCALER_TDTV_OUT_TYPE_SG;
            break;
        case E_TD_IN_TB_I:
        case E_TD_IN_TB_P: 
        ...
        case E_TD_IN_SBS_P:
            ...
            sclNativeOut = SCALER_TDTV_OUT_TYPE_SBS;
            break;
        case E_TD_IN_LI_I:   
        case E_TD_IN_LI_P:
            sclIn = SCALER_TDTV_IN_TYPE_LI;
            sclNativeOut = SCALER_TDTV_OUT_TYPE_PR;
            break;
        default:
            sclIn = SCALER_TDTV_IN_TYPE_2D;
            sclNativeOut = SCALER_TDTV_OUT_TYPE_2D;
    ...
    getTdtvInfo(u4VdpId)->u1Is3DDoneByScaler = u1Scaler3DConv;
...
UINT8 vScpipIs3DDoneByScaler(UINT32 u4VdpId)
{
    ...
    return getTdtvInfo(bPath)->u1Is3DDoneByScaler;
...
void vScpipSet3DModeReg(UINT32 u4VdpId)
{
    ...
    outType = eScpipGetTdtvOutType(u4VdpId);

    switch(outType)
    {
        case SCALER_TDTV_OUT_TYPE_2D:
    ...
    //  4k2k use two line buffers 
    if (getScalerInfo(u4VdpId)->u4InputWidth > 1920)
    {
        value = E_HW_SC_MODE_SBS;
    }
...
void vScpipUpdateDisprFreerunPrm(UINT32 u4VdpId)
{
    ...
    else if (HYBRID_MODE && out == SCALER_TDTV_OUT_TYPE_SG)
    {
        tdtvInfo->u4IsDisprFreerun = SV_TRUE;
     }     
#if defined(CC_MT5880)
...
{    
    ...
    if(getScalerMode(bPath) == VDP_SCPOS_PSCAN_DISPMODE)  //not in dram mode
    {
    ...
    else                                                //input is 3D & scaler is in dram mode
    {
        ...
        {
            if ( HYBRID_MODE )
				vRegWriteFldAlign(SCPIP_DRAM_M_12+dramOffset, 0, DRAM_M_12_DA_3D_INPUT_EN);    //it must clr to 0 in GFX function trigger mode @dual hybrid mode.
			else	
