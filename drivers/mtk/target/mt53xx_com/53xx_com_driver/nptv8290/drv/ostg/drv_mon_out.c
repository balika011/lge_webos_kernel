...
UINT8 vMonOutGetBypassRGB2YUV (void)
{
	return MONOUT_READ_FLD(SCPIP_MON_CTRL_03, SCPIP_MON_OUT_MON_RGB2YUV_BYPASS);
}
...
UINT8 vMonOutGetTVETG422to444_1(void)
{
	return MONOUT_READ_FLD(SCPIP_MON_CTRL_05, SCPIP_MON_OUT_TVE_YC2YUV_EN);
}
...
{
	return  (UINT8)MONOUT_READ_FLD(SCPIP_DRAM_MON_0C, SCPIP_MON_OUT_STA_WR_FLD) ; 
}
...
//
//---------------------------------------------------------------------------------------------
UINT8 vMonOutGetDaFrameNum(void)
{
   UINT8 u1FrameNum;
   u1FrameNum = MONOUT_READ_FLD(SCPIP_DRAM_MON_00,SCPIP_MON_OUT_DA_FRAME_NUM) + 1;
   return u1FrameNum;	
}
...
UINT8 vGetMonOutInputIsRGB(void)
{
    ...
    //muxer fix the hw bug,after background color and mute color,and output is RGB &444
	if(pMonOutInfo->u1MonOutInSel == E_MON_OUT_PANEL || pMonOutInfo->u1MonOutInSel == E_MON_OUT_MIXER)
	{
		bIsRGB = SV_TRUE;
	}	
    ...
    {  
		...
		switch( pMonOutInfo->u1MonOutInSel )
		{
			...
			case  E_MON_OUT_SUB_MIB_OUT:
                ...
                if(vGetMonOutInputIsRGB())//if input is RGB, MIB output is 444&RGB because it will bypass
        		{
        			vMonOutYc2yuvBeforeMonRegen(0);                     //YC2YUV  be fore MON_OUT REGEN 
        		}
        		else
        		{
        			vMonOutYc2yuvBeforeMonRegen(1);                     
        		}
...
void vMonOutUpdatePreDownParameter(void)
{
	...
	{
		u2pre_dscale_h_1 = DIVIDE_WITH_ROUND(pMonOutInfo->wPdsOutWidth * DFT_DS_FACTOR, pMonOutInfo->wPdsInWidth);
	}
	else
	{
	...
	{
		u2pre_dscale_v_1 = DIVIDE_WITH_ROUND(pMonOutInfo->wPdsOutHeight * DFT_DS_FACTOR, pMonOutInfo->wPdsInHeight);
	}
	else
	{
	...
	if(IS_IC_5861() && pMonOutInfo->wPdsOutWidth > 1920)
	{
		vMonOutSetHscalerbypass(1);
		vMonOutSetVscalerbypass(1);
	}
	else if (IS_IC_5890() && pMonOutInfo->wPdsOutWidth > 1280)
	{
		vMonOutSetHscalerbypass(1);
		vMonOutSetVscalerbypass(1);
	}
...
void vMonOutSetEnable(UINT8 Enable)
{
	...
	#endif	
		if((u4DrvVrmGetAppFlag() & VRM_APP_GRAPHIC_DUMP) && (u1Scpip_GFX_GetSource() == GFX_MON_OUT))
		{
...
{
	MONOUT_WRITE_FLD(SCPIP_DRAM_MON_00, 0, SCPIP_MON_OUT_DA_SNAKE_MAPPING);
	...
	vRegenSetResolutionBypass(0,REGEN_MODULE_MONOUT_REGEN, SV_ON);  //bypass setresolution,monitor out do not use setresolution
