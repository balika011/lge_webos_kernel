 ...
 * @8290
 */
void vScpipSetInputPattern(UINT32 u4VdpId, UINT32 u4PtType, UINT32 u4Width, UINT32 u4Height)
{
    UINT8 bPath;
    VDP_SCALER_PRM_T* scalerInfo;
    ...
    if (u4PtType == SCPIP_PTGEN_AUTO || u4PtType == SCPIP_PTGEN_STILL || u4PtType == SCPIP_PTGEN_MANUAL)
    {
        if (u4PtType == SCPIP_PTGEN_AUTO || u4PtType == SCPIP_PTGEN_STILL)
        {
            UINT32 u4YUV;
            //get width, height
            u4Width = scalerInfo->fscIn.u2Width;
            u4Height = scalerInfo->fscIn.u2Height;

            u4YUV = (bIsScalerInput444(bPath)? 0: 1);

            LOG(3, "VDP(%d), SC input size(%d, %d) YUV(%d)\n", u4VdpId, u4Width, u4Height, u4YUV);
        }
        
        if (u4PtType == SCPIP_PTGEN_STILL)
        {
            vScpipSetInPtgStyle(u4VdpId, 0); // not moving X
        }
        else
        {
            vScpipSetInPtgStyle(u4VdpId, SCL_PTG_TYPE_MOVING);
        }
 ...
 * @8290
 */
void vScpipSetOutputPattern(UINT32 u4VdpId, UINT32 u4PtType, UINT32 u4Width, UINT32 u4Height)
{
    UINT8 bPath;    
    VDP_SCALER_PRM_T* scalerInfo;
    ...
    if ((u4PtType == SCPIP_PTGEN_AUTO) || (u4PtType == SCPIP_PTGEN_STILL) || (u4PtType == SCPIP_PTGEN_MANUAL))
    {
        ...
        if (u4PtType == SCPIP_PTGEN_STILL)
        {
		    vScpipSetOutPtgStyle(u4VdpId, 0); // not moving
        }
        else
        {
            vScpipSetOutPtgStyle(u4VdpId, SCL_PTG_TYPE_MOVING);
        }
