...
{	
	...
	if( HYBRID_MODE )
	{
		if(bDrvVideoIsDecDualMode())  
   		{
   			if (bPath == GFX_MAIN_SCL  || bPath == GFX_MAIN_DI || bPath == GFX_SUB_SCL || bPath == GFX_SUB_DI)
			{
				bPath = GFX_HYBRID_4K;
			}
   		}
   		...
   		{
   			if (bPath == GFX_MAIN_SCL  || bPath == GFX_MAIN_DI)
			{
				bPath = GFX_HYBRID_4K;
			}
   		}	
    ...
    else if (_arGfxInf.u1PathIn == GFX_HYBRID_4K)
   	{
   		vScpipSetGfxCLKSelReg(FLD_POCLK_SEL_MIB_OCLK);  		
   	}
	else
    {
    	//to do if the panel is 4K MIB_OCLK=648,MCVP_CK=MIB_OCLK/2
        vScpipSetGfxCLKSelReg(FLD_POCLK_SEL_MIB_OCLK);
    }
    ...
    if(u4DrvVrmGetAppFlag() & VRM_APP_GRAPHIC_DUMP) // VRM trigger
    {
        ...
        else if(bPath == GFX_MON_OUT) // monitor out
        {
...
UINT8 u1Scpip_GFX_Write_Trigger(void)
{
	...
	{	
		LOG(0,"warning: GFX not trigger mode \n");
		return FALSE;
	}
}
...
UINT8 u1Scpip_GFX_Set_Dump_Format(SCPIP_GFX_FORMAT Format)
{
    ...
    case RGBA1010102:
	     ...
	     vScpipGfxSetMux(2, 0, 1);
            ...
            break;
    case YUV422:
            vScpipGfxSetMode(0);  //vRegWriteFldAlign(SCPIP_DRAM_M_14,0,DRAM_M_14_DA_GFX_MODE);
            vScpipGfxAlphaPosSel(0);
            vScpipGfxSetMux(1, 0, 2);
            _arGfxInf.Format = YUV422;
            _arGfxInf.u1AlphaPos = 0;  //Scaler format( 0:ARGB , 1:RGBA) different order with OpenGL.
            _arGfxInf.u1MuxB = 2;
            _arGfxInf.u1MuxR = 1;
            _arGfxInf.u1MuxG = 0;
