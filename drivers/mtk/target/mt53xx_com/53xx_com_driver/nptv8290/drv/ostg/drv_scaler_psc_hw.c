 ...
 */
 ...
 void vPscSetFrameTrackCrossVsync( UINT8 u1OnOff )
 {
	 PSC_WRITE_FLD(PSCSYS_0F, u1OnOff, SC_FRM_TRK_CROSS_VSYNC_EN);
 }

void vPscSetFrameTrack3DEnable( UINT8 u1OnOff )
{
    PSC_WRITE_FLD(PSCSYS_0F, u1OnOff, SC_FRM_TRK_3D_EN);
}
...
/*
Select psc2mjc_vsync:
0: From vsync
1: From vde
2: From vmsk
*/
void vPsc2MjcVsyncSelect(UINT8  u1VsyncSel)
{
      PSC_WRITE_FLD(PSCCTRL_09, u1VsyncSel, PSC2MJC_VSYNC_SEL);
}
...
void vPscSetOsdHsyncDly(UINT16 u2OsdHsyncDly)
{
	PSC_WRITE_FLD(PSCSYS_12, u2OsdHsyncDly, PSC_OSD_HSYNC_DLY);
}

void vPscSetOsdVsyncDly(UINT16 u2OsdVsyncDly)
{
	PSC_WRITE_FLD(PSCSYS_12, u2OsdVsyncDly, PSC_OSD_VSYNC_DLY);
}

void vPscSet2LineDrvOnOff(UINT8 u1OnOff)
{
	PSC_WRITE_FLD(PSCSYS_09, u1OnOff, PSC_2LINE_DRIVE_EN);
}

void vPscSet2LineDrvInv(UINT8 u12LineDrvInv)
{
	PSC_WRITE_FLD(PSCSYS_09, u12LineDrvInv, PSC_2LINE_DRIVE_INV);
}

void vPscSet2LineBframeYUV(UINT16 u2LineDrvY,UINT16 u2LineDrvU,UINT16 u2LineDrvV)
{
	PSC_WRITE_FLD(PSCSYS_0A, u2LineDrvY, PSC_2LINE_DRIVE_Y);
	PSC_WRITE_FLD(PSCSYS_0A, u2LineDrvU, PSC_2LINE_DRIVE_U);
	PSC_WRITE_FLD(PSCSYS_0B, u2LineDrvV, PSC_2LINE_DRIVE_V);
}
...
/*
   - the dly H and Vsync while postSc output timing to OSD.
*/
...
void vPscTGEnable( UINT8 u1OnOf )
{
    PSC_WRITE_FLD(PSCSYS_06, u1OnOf, PSC_TG_ENABLE);   
}

void vPscTGSlaveMode(UINT8 u1OnOff )
{
    PSC_WRITE_FLD(PSCSYS_06, u1OnOff, PSC_TG_SLAVE_MODE);   
}

/*
-	Sync method in slave mode
-	    0: Sync once at enable moment
 	         Reset Tg_v_cnt/tg_h_cnt at ebable.
 	         Free_run mode. <<Suggestion setting>>
-	    1: Sync at every reference VSYNC pulse
 	        Every Ref_TG Vsync will force reset the TG_H_COUNT/TG_V_COUNT
*/
void vPscTGSlaveSyncMode(UINT8 u1OnOff )
{
    PSC_WRITE_FLD(PSCSYS_06, u1OnOff, PSC_TG_SLAVE_SYNC_MODE);   
}

/*
Select psc2mjc_mc_ori:
0: Use mc_ori from MJC
1: Use 2-line drive L-B-R-B signal
*/
void vPsc2MjcSetMcOri(UINT8 u1McOriSel )
{
    PSC_WRITE_FLD(PSCSYS_06, u1McOriSel, PSC_MC_ORI_SEL);   
}


UINT8 vPsc2MjcGetMcOriStatus( void )
{
   return( PSC_READ_FLD(PSCSYS_06,  PSC_STA_MC_ORI ) );   
}
