...
BOOL fgTXPath2CHMode(void)
{
	if((PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_VB1)&&(LVDS_OUTPUT_PORT == OCTA_PORT)
		&&(GetPANEL4K2K_TYPE() == PANEL_TYPE_4K2K_60)
		)
	{
	    return (TRUE);
	}
	else if (PANEL_IsSupport4K2KDD())
	{
	    return (TRUE);
	}
	else
...
void vDrvMLVDSPatternGen(UINT8 u1OnOff)
{
    ...
    vIO32WriteFldAlign(VINTF_B1, PANEL_GetPanelWidth()>> u1DIV_2CH, VINTF_PTGEN_H_ACTIVE);
	if (PANEL_IsSupport4K2KDD())
    vIO32WriteFldAlign(VINTF_B1, wDrvGetOutputHTotal()>> u1DIV_2CH, VINTF_PTGEN_H_TOTAL);    
	else
	...
	if(PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_VB1)
	{
		if(fgTXPath2CHMode())
		{
			vIO32WriteFldAlign(VB1SYS6, u1OnOff, VB1SYS6_TSET_VB1_4567);
		}

		vIO32WriteFldAlign(VB1OSDSYS6,u1OnOff,VB1OSDSYS6_TEST);
...
// Pixel Reorder Pattern only for MLVDS & EPI
...
void vErrorHandleSetByPanel(void)
{
	...
	H_HIGH_TH = PANEL_GetHTotalMax() >> u1DIV_2CH;
	if (PANEL_IsSupport4K2KDD())
	H_LOW_TH = PANEL_GetHTotalMin() >> u1DIV_2CH;
	else
	...
	DE_HIGH_TH = (PANEL_GetPanelWidth() >> u1DIV_2CH)+7;
	DE_LOW_TH = (PANEL_GetPanelWidth() >> u1DIV_2CH)-8;

	if (PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_EPI)
	{
		VDE_HIGH_TH = PANEL_GetPanelHeight() + 3;
		VDE_LOW_TH = PANEL_GetPanelHeight() - 4;
	}
	else
	{
	...
	if (IS_XTAL_24MHZ())
	{
		ASSERT(PANEL_GetPixelClkMin()!=0);
		EH_CLK_HIGH_TH = (1024*24000/ ((PANEL_GetPixelClkMin())/ 1000));
		ASSERT(PANEL_GetPixelClkMax()!=0);
		EH_CLK_LOW_TH = (1024*24000/ ((PANEL_GetPixelClkMax())/ 1000));
		if (PANEL_IsSupport4K2KDD())
		{
			EH_CLK_HIGH_TH = EH_CLK_HIGH_TH << 1;
			EH_CLK_LOW_TH = EH_CLK_LOW_TH << 1;			
		}		
	}
	...
	vIO32WriteFldAlign(EH_10,(PANEL_GetPanelWidth() >> u1DIV_2CH),EH_PTGEN_H_ACTIVE);			
	...
	vIO32WriteFldAlign(EH_1A,(PANEL_GetPanelWidth() >> u1DIV_2CH)-20,EH_FIX_VDE_TH);			
	...
	if((PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_MLVDS) && (PANEL_GetPanelWidth() == 1366))
	{
		...
		vIO32WriteFldAlign(EH_1A,(PANEL_GetPanelWidth() >> u1DIV_2CH) + 2,EH_FIXED_DE_TOATL); 
	}
	else
	{		  
		vIO32WriteFldAlign(EH_1A,(PANEL_GetPanelWidth() >> u1DIV_2CH),EH_FIXED_DE_TOATL); 
	} 
...
void vErrorHandleSetByTiming(void)
{
	...
	else
	{
	 ...
	 if (PANEL_IsSupport4K2KDD())
	 {
		 vIO32WriteFldAlign(EH_19,(wDrvGetOutputHActive()>>1) + 80,EH_FIX_HTOTAL_VALUE);
		 vIO32WriteFldAlign(EH_19,(wDrvGetOutputHActive()>>1) + 80,EH_FIX_HTOTAL_TH);
	 }
	 else
	 {
...
static void vErrorHandleOSDInit(void)
{
#if defined(SUPPORT_PANEL_GRAPH_MODE)
	if (IS_SUPPORT_VB1_GRAPH_MODE)
	{
		vIO32WriteFldAlign(EH_00, 1 ,EH_FRAME_METER_EN_OSD);
		
		vIO32WriteFldMulti(EH_0F, P_Fld(0x0, EH_OSD_B_SWAP)
								| P_Fld(0x0, EH_OSD_G_SWAP)
								| P_Fld(0x0, EH_OSD_R_SWAP)								
								| P_Fld(0x0, EH_OSD_A_SWAP));
	}
#endif
}
...
void vErrorHandleInit(void)
{
    ...
    else  if ((LVDS_OUTPUT_PORT == DUAL_PORT)||(LVDS_OUTPUT_PORT == FOUR_PORT)||(LVDS_OUTPUT_PORT == OCTA_PORT)) // 1080P/60
    {
    ...
    if ((LVDS_OUTPUT_PORT == FOUR_PORT) || (LVDS_OUTPUT_PORT == OCTA_PORT))
    {
...
void vErrorHandleStatus(void)
{
	...
	if (IS_SUPPORT_VB1_GRAPH_MODE)
	{
		Printf("[Ouput Status from OSD]\n");			
		Printf("(Ouput)DE_TOTAL_MON=%d, H_TOTAL_MON=%d, VDE_TOTAL_MON=%d, V_TOTAL_MON=%d\n",
				IO32ReadFldAlign(EH_STATUS_12,EH_HACTIVE_STATUS),IO32ReadFldAlign(EH_STATUS_10,EH_HTOTAL_STATUS),
				IO32ReadFldAlign(EH_STATUS_12,EH_VACTIVE_STATUS), IO32ReadFldAlign(EH_STATUS_10,EH_VTOTAL_STATUS));	
		Printf("(Ouput)H_START=%d, V_START=%d\n",
				IO32ReadFldAlign(EH_STATUS_11,EH_HSTART_STATUS),IO32ReadFldAlign(EH_STATUS_11,EH_VSTART_STATUS));

		Printf("[Frame Rate]\n"); 
		u2Temp = IO32ReadFldAlign(EH_STATUS_13,EH_OSD_FRATE_STATUS);	
		if (!u2Temp)
		{
			u2Temp = 1;
		}
...
void vVByOneOSDFormatCustConfig(void)
{
	vIO32WriteFldMulti(EH_1B, P_Fld(0x0, EH_OSD_A7_SEL)
							| P_Fld(0x0, EH_OSD_A6_SEL)
							| P_Fld(0x0, EH_OSD_A5_SEL)
							| P_Fld(0x0, EH_OSD_A4_SEL)
							| P_Fld(0x0, EH_OSD_A3_SEL)
							| P_Fld(0x0, EH_OSD_A2_SEL)							
							| P_Fld(0x0, EH_OSD_A1_SEL)									
							| P_Fld(0x0, EH_OSD_A0_SEL));
	
	vIO32WriteFldMulti(EH_1C, P_Fld(0x0, EH_OSD_R7_SEL)
							| P_Fld(0x0, EH_OSD_R6_SEL)
							| P_Fld(0x0, EH_OSD_R5_SEL)
							| P_Fld(0x0, EH_OSD_R4_SEL)
							| P_Fld(0x0, EH_OSD_R3_SEL)
							| P_Fld(0x0, EH_OSD_R2_SEL)							
							| P_Fld(0x0, EH_OSD_R1_SEL)									
							| P_Fld(0x0, EH_OSD_R0_SEL));	

	vIO32WriteFldMulti(EH_1E, P_Fld(0x0, EH_OSD_G7_SEL)
							| P_Fld(0x0, EH_OSD_G6_SEL)
							| P_Fld(0x0, EH_OSD_G5_SEL)
							| P_Fld(0x0, EH_OSD_G4_SEL)
							| P_Fld(0x0, EH_OSD_G3_SEL)
							| P_Fld(0x0, EH_OSD_G2_SEL)							
							| P_Fld(0x0, EH_OSD_G1_SEL)									
							| P_Fld(0x0, EH_OSD_G0_SEL));	

	vIO32WriteFldMulti(EH_1F, P_Fld(0x0, EH_OSD_B7_SEL)
							| P_Fld(0x0, EH_OSD_B6_SEL)
							| P_Fld(0x0, EH_OSD_B5_SEL)
							| P_Fld(0x0, EH_OSD_B4_SEL)
							| P_Fld(0x0, EH_OSD_B3_SEL)
							| P_Fld(0x0, EH_OSD_B2_SEL)							
							| P_Fld(0x0, EH_OSD_B1_SEL)									
							| P_Fld(0x0, EH_OSD_B0_SEL));
...
UINT8 u1DrvEPIPostInit(void)
{
	...
	if (IS_EPI_8LANE)
	{
		vIO32WriteFldAlign(FIFO_CTRL_01, 7, TCLK_FIFO_ACTIVE_PORT); 
	}
...
void vDrvEPI_AnalogInit(void)
{
	...
	//B group pre emphasize power down
	...
	vIO32WriteFldAlign(REG_LVDS_TX_CFG3, 0, RG_LVDSB_TVO_EVEN_D11);
    vIO32WriteFldAlign(REG_LVDS_TX_CFG4, 0, RG_LVDSB_TVO_ODD_D11);
	vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_BIAS_EN_D11 );

	//A group ch1~8 AE4 AE3 AE2 AE0 AO4 AOCK AO1 AO0	
	if(fgIsSupport120HzPanel())
	{
		vIO32WriteFldMulti(REG_LVDS_TX_CFG12, P_Fld(0xA, RG_LVDSA_TVO)|P_Fld(4, RG_LVDSA_TVCM)|P_Fld(4, RG_LVDSA_NSRC)|
											 P_Fld(4, RG_LVDSA_PSRC));
		vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 1, RG_LVDSA_TVO_PRE);
		vIO32WriteFldAlign(REG_LVDS_TX_CFG14, 0x88C, RG_LVDSA_PRE_PWD);
	}
	else
	{
		vIO32WriteFldMulti(REG_LVDS_TX_CFG12, P_Fld(0x8, RG_LVDSA_TVO)|P_Fld(0x0B, RG_LVDSA_TVCM)|P_Fld(4, RG_LVDSA_NSRC)|
										 P_Fld(4, RG_LVDSA_PSRC));
		vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 0, RG_LVDSA_TVO_PRE);
		vIO32WriteFldAlign(REG_LVDS_TX_CFG14, 0xFFF, RG_LVDSA_PRE_PWD);
	}

	vIO32WriteFldMulti(REG_LVDS_TX_CFG13, P_Fld(0xC, RG_LVDSA_BIAS_SEL)|P_Fld(0xC, RG_LVDSA_NSRC_PRE)|P_Fld(0xC, RG_LVDSA_PSRC_PRE));
...
void vDrvEPI_AnalogOn(void)
{
    ...
    vIO32WriteFldMulti(REG_LVDS_TX_CFG20, P_Fld(0, RG_LVDSA_LDO_ODD_PWD_D11)|P_Fld(0, RG_LVDSA_LDO_BIAS_ODD_PWD_D11)|
                                          P_Fld(0, RG_LVDSA_LDO_EVEN_PWD_D11)|P_Fld(0, RG_LVDSA_LDO_BIAS_EVEN_PWD_D11));

	if (IS_EPI_8LANE)
	{
		vIO32WriteFldAlign(REG_LVDS_TX_CFG12, 0x773, RG_LVDSA_EXT_EN_D11); //Selest EPI Output
		vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 0x88C, RG_LVDSA_DRV_PWD);
	}
	else if(IS_EPI_4LANE)
	{
		vIO32WriteFldAlign(REG_LVDS_TX_CFG12, 0x630, RG_LVDSA_EXT_EN_D11); //Selest EPI Output
		vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 0x9CF, RG_LVDSA_DRV_PWD);
	}
	else 
	{
		vIO32WriteFldAlign(REG_LVDS_TX_CFG12, 0x770, RG_LVDSA_EXT_EN_D11); //Selest EPI Output
		vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 0x88F, RG_LVDSA_DRV_PWD);
	}
...
{	
	...
	vIO32WriteFldAlign(REG_LVDS_TX_CFG1, 0x7F,RG_LVDSB_DRV_EN_ODD_D11);
...
void vDrvEPI_AnalogOff(void)
{
	Printf("[EPI] vDrvEPI_AnalogOff \n");
		...
		vIO32WriteFldMulti(REG_LVDS_TX_CFG20, P_Fld(1, RG_LVDSA_LDO_ODD_PWD_D11)|P_Fld(1, RG_LVDSA_LDO_BIAS_ODD_PWD_D11)|
		...
		vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG0, 1, REG_LVDSTX_PLL_CFG0_VPLL_BG_PWD);		
#endif
...
void vDrvMiniLVDS_AnalogInit(void)
{
	...
	Printf("[LVDS] vDrvMiniLVDS_AnalogInit \n");	
    ...
    else
    {
	 ...
	 // Set VCM (TVCM)
	 #if defined(CC_MT5890)
	 vIO32WriteFldMulti(REG_LVDS_TX_CFG12, P_Fld(0x0A, RG_LVDSA_TVCM)|P_Fld(1, RG_LVDSA_NSRC)|P_Fld(1, RG_LVDSA_PSRC));
	 #endif	 
	 ...
	 #if defined(CC_MT5890)
	 vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 4, RG_LVDSA_BIAS_SEL); // for mini LVDS BIAS
	 //vIO32WriteFldAlign(REG_LVDS_TX_CFG10, 0x0C, RG_LVDSA_BIAS_SEL); // for EPI BIAS
	 #endif
	
	 //Set CLK_IQ SEL
	 #if defined(CC_MT5890)
	 vIO32WriteFldAlign(REG_LVDS_TX_CFG14, 0x008, RG_LVDSA_CLK_IQ_SEL);
...
{
    Printf("[LVDS] vDrvMiniLVDS_PostInit \n");
...
void vDrvMiniLvdsOn(void)
{
	...
	vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 0, RG_LVDSA_DRV_PWD);    
...
void vDrvMiniLVDSOff(void)
{
	...
	vIO32WriteFldMulti(REG_LVDS_TX_CFG1, P_Fld(0, RG_LVDSB_DRV_EN_EVEN_D11)|P_Fld(0,RG_LVDSB_DRV_EN_ODD_D11));
    vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_BIAS_EN_D11);
	//vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_EXT_EN_D11); //12 lane  
	//vIO32WriteFldAlign(REG_LVDS_TX_CFG8, 1, RG_LVDSB_LDO_PWD_D11);  
	//A Group Power Off Sequence
	vIO32WriteFldAlign(REG_LVDS_TX_CFG12, 0, RG_LVDSA_EXT_EN_D11); //12 lane
    vIO32WriteFldAlign(REG_LVDS_TX_CFG13, 0xFFF, RG_LVDSA_DRV_PWD); //12 lane
	...
	vIO32WriteFldMulti(REG_LVDS_TX_CFG1, P_Fld(0, RG_LVDSB_DRV_EN_EVEN_D11)|P_Fld(0,RG_LVDSB_DRV_EN_ODD_D11));
    vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_BIAS_EN_D11);
	//vIO32WriteFldAlign(REG_LVDS_TX_CFG0, 0, RG_LVDSB_EXT_EN_D11); //14 lane
	//vIO32WriteFldAlign(REG_LVDS_TX_CFG8, 1, RG_LVDSB_LDO_PWD_D11);  
#endif
...
UINT32 u4Data;
   ...
   vIO32WriteFldAlign(GR0, rFlashPanelTable.u2FPGr0TctgFcr, GR0_TCTG_FCR);
    ...
    //Swing and Skew
    //LVDS
    vIO32WriteFldMulti(REG_LVDS_TX_CFG3, P_Fld(rFlashPanelTable.u1FPRgLVDSBTvo, RG_LVDSB_TVO_ODD_D11)|
        P_Fld(rFlashPanelTable.u1FPRgLVDSBTvo, RG_LVDSB_TVO_EVEN_D11));
    vIO32WriteFldAlign(REG_LVDS_TX_CFG0, rFlashPanelTable.u1FPRgLVDSBTvcm, RG_LVDSB_TVCM_D11);

#if defined(CC_MT5890)
    vIO32WriteFldMulti(REG_LVDS_TX_CFG12, P_Fld(rFlashPanelTable.u1FPRgLVDSATvo, RG_LVDSA_TVO)|
        P_Fld(rFlashPanelTable.u1FPRgLVDSATvcm, RG_LVDSA_TVCM));
#endif

    vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG2, rFlashPanelTable.u1FPRgVpllDlyData, REG_LVDSTX_PLL_CFG2_VPLL_DLY_DATA);
}
#endif
...
void vDrvTCONInitISR(void)
{
	...
	{		
		if(PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_VB1)
		{
		    //if (!(PANEL_IsSupport4K2KDD()&& IS_IC_5861_ES1()))   // none-FIB
		    if (!IS_IC_5861_ES1())   // FIB and normal
...
void vDrvTCLKPLLRst(void)
{
	vIO32WriteFldAlign(TMGR2, 0, TMGR2_TCLK_PLLRST);
	HAL_Delay_us(200);
	vIO32WriteFldAlign(TMGR2, 1, TMGR2_TCLK_PLLRST);
	HAL_Delay_us(200);
	vIO32WriteFldAlign(TMGR2, 0, TMGR2_TCLK_PLLRST);
}
