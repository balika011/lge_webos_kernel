...
EXTERN void vScpipUpdateTdtvInfo(UINT32 u4VdpId, UINT32 u4TdtvPrm, UINT32 u4ScalerMode);
...
void vScpipWriteCtrlOnOff(UCHAR ucVdpId, UCHAR ucOnOff)
{
    ...
    if(ucOnOff == SV_ON)
    {
        ...
        {
            LOG(5," 2.[WRITE OFF]  Scaler Not Enabled\n");
            ucOnOff = SV_OFF;
        }
        #endif
        ...
        {
            LOG(5," 3.[WRITE OFF]  Channel Off\n");
            ucOnOff = SV_OFF;
        }
        //condition4. channel is Freezed
        if(prChannel->bIsFreeze == SV_ON)
        {
            LOG(5," 4.[WRITE OFF]  Scaler Freeze\n");
            ucOnOff = SV_OFF;
        }
        //condition5. Scaler is NOT in Dram mode
        if(u1ScpipGetDispMode(getScalerPath(ucVdpId))  == VDP_SCPOS_PSCAN_DISPMODE)
        {
            LOG(5," 5.[WRITE OFF]  Pscan Display Mode\n");
            ucOnOff = SV_OFF;
        }
        ...
        {
            LOG(5," 6.[WRITE OFF]  Dram Width Height Is Zero\n");
            ucOnOff = SV_OFF;
        }
        ...
        {
            LOG(5," 8.[WRITE OFF]  Scaler Bypassed\n");
            ucOnOff = SV_OFF;
        }
        #endif

#if defined(CC_MT5890)
		if((vDrvGetTconTopVdoSel() == E_VB1_PATH_BYPASS) && (ucVdpId == SV_VP_MAIN))
		{
		    LOG(5," 9.[WRITE OFF]  VB1 Bypass Path\n");
		    ucOnOff = SV_OFF;
		}
#endif
...
static void _vScpipSrmUpdateDramInfo(UINT32 u4VdpId, FBM_POOL_T* prPool, FBM_POOL_T* prMddiPool, UINT32 u4Mode)
{
    ...
    _arScalePrm[u4VdpId].rFbmInfo.u4MddiDramSize = prMddiPool->u4Size;
...
static void _vDrvScpipSrmUpdateDispmode(UINT32 u4VdpId, UINT32 u4Mode)
{
	...
	{
		_VDP_DispModeEnable((UCHAR)u4VdpId, (UCHAR)VDP_SCPOS_HYBRID_MODE);
	}
	else if (u4Mode & FBM_POOL_MODE_SCPOS_FULL)
    {
...
static void _vScpipUpdatePageNum(UINT32 u4VdpId)
{
    ...
    if( vScpipIs3DDoneByScaler(u4VdpId))
    {// 3D case
        u4FrameNum <<= 1;
    }
    switch (eScpipGetTdtvInType(u4VdpId))
    {
    ...
    {
        LOG(0, "Path(%d) FBM buffer to small!!! %d < %d + %d\n", u4VdpId, u4FbTotalSize, u4UsedPage, u4Padding);
    }
}
...
static void _vScpipCalBufferAddress(UINT32 u4VdpId)
{
    ...
    LOG(1,"Path(%d) Update Scaler Dram Info\n",u4VdpId);
    LOG(1,"==========================\n");    
...
static void _vScpipUpdateFrameNumber(UINT32 u4VdpId)
{
    ...
    //channel = getChannel(u4VdpId);

	
    // 1. scaler dram mode flip (4 frames)
    if (  HYBRID_MODE)   // in hybrid mode , the frame number must = MIB frame number
    {
        u4FrameNum= FBM_POOL_HYBRID_DRAM_FRAME_NUM;
    }
    ...
    if (FLIP_BY_SCALER == u4GetFlipMirrorModule(u4VdpId))
    {	
        u4FrameNum ++;       
    }
    else if (HYBRID_MODE && FLIP_BY_PSCAN== u4GetFlipMirrorModule(u4VdpId) )
    {
        u4FrameNum ++;       
    }
#endif
    ...
    if ((_arScalePrm[u4VdpId].u4IsGameMode==SV_TRUE) && (u4FrameNum > 2))   //game mode, force to 2 frame
    {
        LOG(1, "Path(%d) GameMode: Force scalar to 2 frame mode! (%d)\n", u4VdpId, u4FrameNum);
        u4FrameNum = 2;
    }
...
void vScpipUpdateDramReg(UINT32 u4VdpId)
{
    ...
    if(dramInfo->YMsbAddr == SCPIP_READ_FLD(SCPIP_DRAM_M_04+dramOffset, DRAM_M_04_DA_ADDR_BASE_MSB_Y) &&
       ...
       ((dramInfo->FrameNum == u4ScpipGetBufNum(u4VdpId))||(bMode == VDP_SCPOS_PSCAN_DISPMODE)))
...
void vDrvScpipSrmDispCb(UINT32 u4VdpId, UINT32 u4Mode, UINT32 u4PDSize, UINT32 u4SramMode)
{
    ...
    //2. update TDTV info
    vScpipUpdateTdtvInfo(u4VdpId,u4SramMode, u4Mode);
    ...
    if (u4VdpId == VDP_1)
    {
        ...
        #if 0//def CC_SUPPORT_4K2K//only for mustang 4k2k,because mustang scaler dram have move to CHB share with MJC
...
void vScpipSetBufferAddress(UINT32 u4VdpId, UINT32 u4Start, UINT32 u4Width, UINT32 u4Height, UINT32 u4Frames)
{
    SCALER_DRAM_ADDR_T* dramAddrInfo = getDramAddrInfo(u4VdpId);
    SCALER_FBM_POOL_T*  fbmInfo      = getFbmPoolInfo(u4VdpId);
    UINT16 dramOffset = getDramOffset(u4VdpId);

    UINT32 u4FbmWidth  = fbmInfo->u4Width;
    UINT32 u4FbmHeight = fbmInfo->u4Height;
    UINT32 u4FrameNum  = dramAddrInfo->FrameNum;
    
    UINT32 MsbPageNum = 0;
    UINT32 LsbPageNum = 0;
    UINT32 StartAddr = fbmInfo->u4Addr >> 11;    // starting address from FBM
    UINT32 TotalSize = fbmInfo->u4FbmSize >> 11; // total page number from FBM
    UINT32 YMsbAddr, YLsbAddr, UMsbAddr, ULsbAddr, VMsbAddr, VLsbAddr;

    if (u4Start != 0)
    {
        StartAddr = u4Start >> 11;
    }

    if (u4Width != 0 && u4Height != 0 && u4Frames != 0)
    {
        u4FbmWidth  = u4Width;
        u4FbmHeight = u4Height;
        u4FrameNum  = u4Frames;
    }

    MsbPageNum = ((u4FbmWidth + 255) / 256) * (((u4FbmHeight* u4FrameNum) + 7) / 8);
    if (fbmInfo->fg10bit == SV_TRUE)
    {
        LsbPageNum = (((u4FbmWidth + 255) / 256) * (((u4FbmHeight* u4FrameNum) + 255) / 256)) * 8;
    }

    if (MsbPageNum != dramAddrInfo->MsbPageNum)
    {
        LOG(0, "Path(%d) DRAM MSB changed from %d to %d\n", u4VdpId, dramAddrInfo->MsbPageNum, MsbPageNum);
    }

    if (LsbPageNum != dramAddrInfo->LsbPageNum)
    {
        LOG(0, "Path(%d) DRAM LSB changed from %d to %d\n", u4VdpId, dramAddrInfo->LsbPageNum, LsbPageNum);
    }    

    //1. Y Msb
    YMsbAddr = StartAddr; 
    //2. Y Lsb = Y Msb + Msb Total Page
    YLsbAddr = MAKE_ALIGN_TO((YMsbAddr + MsbPageNum), 3, ((StartAddr%8)+2)%8);
    //3. U Msb = Y Lsb + Lsb Total Page
    UMsbAddr = MAKE_ALIGN_TO((YLsbAddr + LsbPageNum), 3, ((StartAddr%8)+4)%8);
    //4. U Lsb = U Msb + Msb Total Page
    ULsbAddr = MAKE_ALIGN_TO((UMsbAddr + MsbPageNum), 3, ((StartAddr%8)+6)%8);
    //5&6. V Msb & V Lsb
    if(fbmInfo->fg422 == SV_FALSE)  //444
    {
        VMsbAddr = MAKE_ALIGN_TO((ULsbAddr + LsbPageNum), 3, ((StartAddr)+0)%8);
        VLsbAddr = MAKE_ALIGN_TO((VMsbAddr + MsbPageNum), 3, ((StartAddr)+2)%8);
    }
    else    //422
    {
        VMsbAddr = UMsbAddr;
    ...
    //2. update base address
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_04+dramOffset, YMsbAddr, DRAM_M_04_DA_ADDR_BASE_MSB_Y);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_05+dramOffset, YLsbAddr, DRAM_M_05_DA_ADDR_BASE_LSB_Y);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_06+dramOffset, UMsbAddr, DRAM_M_06_DA_ADDR_BASE_MSB_U);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_07+dramOffset, ULsbAddr, DRAM_M_07_DA_ADDR_BASE_LSB_U);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_08+dramOffset, VMsbAddr, DRAM_M_08_DA_ADDR_BASE_MSB_V);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_09+dramOffset, VLsbAddr, DRAM_M_09_DA_ADDR_BASE_LSB_V);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_0A+dramOffset, StartAddr + TotalSize,  DRAM_M_0A_DA_WADDR_HI_LIMIT);
    SCPIP_WRITE_FLD(SCPIP_DRAM_M_0B+dramOffset, StartAddr,              DRAM_M_0B_DA_WADDR_LO_LIMIT);
}
