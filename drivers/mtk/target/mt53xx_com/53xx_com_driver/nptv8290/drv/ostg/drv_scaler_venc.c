...
void vDrvScpipVEncISR(void)
{
    ...
    {
        LOG(0, "Error:encoder Module %d %s Retrun \n", _tScalerVencPrm.u1Src, __FUNCTION__);
        return;
    }
    ...
    {
        LOG(0, "Error: %s: _hScalerVEnc is NULL!!\n", __FUNCTION__);
        return;
    }
    ...
    {
        LOG(0, "Error: %s Send Address to VENC Fail\n", __FUNCTION__);
        ASSERT(0);
    }
...
{   
    ...
    LOG(2, "Set Venc Fld Record Mode %d\n",eScalerVencFldMode);
}
...
void u1DrvScpipH264Enable(UINT8 u1Mode)
{
    ...
    if (u1Mode == SCALER_H264_MODE)
    {
        ...
        {
           vScpipH264SetFrameMode(_tScalerVencPrm.u1Src, DRAM_FRAME_MODE);              //encoder one field or Progressive  Frame mode 
        }
        else if(eScalerVencFldMode == E_SCALER_VENC_BOTH_FLD)
        {
           ...
           vScpipSetFieldISREn(_tScalerVencPrm.u1Src,ON);
        }
    }
    ...
    { 
        ...
        vScpipSetFieldISREn(_tScalerVencPrm.u1Src,OFF);
    }
