...
void _vPDSHybridModeBypass(UINT8 bPath);
...
void vPDSUpdateMddiFifo(UINT8 bPath)
{
    ...
    UINT32 u4DIClkratio  = 0;
    UINT32 u4InSigClk  = 0;
    UINT32 u4DispMode = SRM_GetScposMode(bPath);
    ...
    {                
     ...
     //DI clock limitation: input clock * di_width/input width < di clock
    // so 297m * di_widht /1920 <162m , di_width < 162m*1920/297m
    if(u4DispMode & FBM_POOL_MODE_SCPOS_PSCAN_DISP)
    {
        if((wDrvVideoInputWidth(u4VdpId) > 1920) || (wDrvVideoInputHeight(u4VdpId) > 1088))
        {
            #define SHIFT_FOR_DIClk_RATIO_OVERFLOW   4
            u4DIClkratio =u4DrvDIGetClkRate(u4VdpId);
            u4InSigClk = wDrvVideoGetVTotal(u4VdpId) * wDrvVideoGetHTotal(u4VdpId) * bDrvVideoGetRefreshRate(u4VdpId);
            u4InSigClk = u4InSigClk>>SHIFT_FOR_DIClk_RATIO_OVERFLOW;
            u4DIClkratio = u4DIClkratio/u4InSigClk;      
            if(((wDrvVideoInputWidth(u4VdpId)*u4DIClkratio) >>SHIFT_FOR_DIClk_RATIO_OVERFLOW) < u4MddiFifoMax)
            {
                u4MddiFifoMax = (wDrvVideoInputWidth(u4VdpId)*u4DIClkratio) >>SHIFT_FOR_DIClk_RATIO_OVERFLOW;
                LOG(2,"DI clock limitation: u4MaxInputWidth =%d\n",u4MddiFifoMax);
            }
        }
    }
...
UINT32 u4PDSUpdatePDSWidth(UINT8 bPath)
{
    ...
    {
        u2DispWidth = getTveDispInfo()->wWidth;
    }
    else
    {
        if (bDrvVideoIsDecDualMode())
        {
            u2DispWidth = (getDispInfo(VDP_1)->wWidth >> 1);
        }
        else
        {
            u2DispWidth = getDispInfo(bPath)->wWidth;
        }
    }
    pdsInfo = getPDSInfo(bPath);
    u4VdpId = getScalerVDP(bPath);
    ...
    //error handle
    if(picInfo->wSrcW ==0 || picInfo->wTargetW == 0 || u2DispWidth == 0 || inRes->u2Width == 0)
    {
        return ret;
    }

    // whether there is fifo dram limit
    bIsFifoScalerDramLimit = bDrvVrmIsAppScalerFifoDramLimit(bPath);
    ...
    if ((E_TD_IN_SBS_P == eMIBIn || E_TD_IN_SBS_I == eMIBIn) && 
          E_TD_OUT_NATIVE != TD_MIB_OUT(u4MIB))
    {
        u4MddiFifoMax      <<= 1;
        u2DispWidth         <<= 1;
    }
    ...
    #endif
    {
        u4Bound = MIN(u2DispWidth, u4MddiFifoMax);
    }  

    LOG(3,"dispInfo->wWidth(%d) u4MddiFifoMax(%d) u4Bound(%d)\n", u2DispWidth, u4MddiFifoMax, u4Bound);
    
    //predown will be bounded to MIN(u2DispWidth, u4MddiFifoMax)
    ...
    if((SRM_GetScposMode(bPath) & FBM_POOL_MODE_SCPOS_FULL) && bIsFifoScalerDramLimit && !bDrvVideoIsDecDualMode() &&
...
UINT32 u4PDSUpdatePDSHeight(UINT8 bPath)
{
    ...
    E_TD_IN eMJCIn   = TD_MJC_IN(u4MJC);
    E_TD_OUT eMJCOUT = TD_MJC_OUT(u4MJC);
    UINT32 u4DispHeight = 0;
	
    pdsInfo = getPDSInfo(bPath);
    picInfo = getPicInfo(bPath);
    if((bPath == SV_VP_PIP) && bDrvVrmIsAppFlagSet(VRM_APP_SCART_OUT))
    {
        u4DispHeight = getTveDispInfo()->wHeight;
    }
    else
    {
        if (bDrvVideoIsDecDualMode())
        {
            u4DispHeight = getDispInfo(VDP_1)->wHeight;
        }
        else
        {
            u4DispHeight = getDispInfo(bPath)->wHeight;
        }
    }
    inRes = VRMGetVRMInfo(bPath, VRM_MODULE_PDS);
    ...
    if(picInfo->wSrcH ==0 || picInfo->wTargetH == 0 || u4DispHeight == 0 || inRes->u2Height == 0)
    {
        return ret;
    }
    ...
    u4Bound = u4DispHeight;
    LOG(3,"dispInfo->wHeight(%d) u4Bound(%d)\n", u4DispHeight, u4Bound);
    ...
    else
    {
        ...
        if(bDrvVideoIsSrcInterlace(bPath) && 
            !((bDrvVrmIsAppFlagSet(VRM_APP_VIDEO_ENC))&&(bPath == VDP_2))) //only venc alone can do predown + graphic dump			
        {
...
#ifdef PREDOWN_4K2K_TO_FHD_LOST_PIXEL_WA
void vPDSSetPredwnHblankReg(UINT32 u4VdpId, UINT32 u4Value)
{
    if(u4VdpId == SV_VP_MAIN)
    {
        PDS_WRITE_FLD(PRE_DS_MAIN_05, u4Value, PRE_DS_MAIN_05_MAIN_HCEN_ACT_AT_SYNC);    
    }
    else
    {
        PDS_WRITE_FLD(PRE_DS_MAIN_04, u4Value, PRE_DS_PIP_04_PIP_HCEN_ACT_AT_SYNC);
    }
}
#endif
/**
...
static void _vPDSUpdateHPDSFirFactorPrm(UINT8 bPath, UINT32 inW, UINT32 outW)
{
    ...
    if ( (inW < outW) || (outW == 0 ) )
    {
       LOG(2,"bPath %d input width =%d < output width=%d \n",bPath,inW,outW);
       return;
    }
...
static void _vPDSUpdateVPDSFactorPrm(UINT8 bPath, UINT16 inH, UINT16 outH)
{
    ...
    {
        u4Factor = ((DFT_PDS_FACTOR * outH + (inH-1)) / inH);   //for fixed the predown from 0x8000 to 0x1200 the MIB lost last line.
    }
...
#ifdef PREDOWN_4K2K_TO_FHD_LOST_PIXEL_WA
/**
 * @brief Set predown h blank parameter.
 * @param bPath VDP_1/VDP_2  
 * @the parameters will set to predown h blank.
 */
static void _vPDSUpdatePredownHblankPrm(UINT8 bPath)
{
    PREDOWN_INFO_T* pdsInfo;
    pdsInfo = getPDSInfo(bPath);

    //When 4k timing input and pre-down to 1920, the pre-down width will lost 1 or 2 pixels. 
    //so it need to predown h blanking, 
    if(((wDrvVideoInputWidth(bPath) > 1920) || (wDrvVideoInputHeight(bPath) > 1088)) 
        &&(pdsInfo->u4HPDSFactor <= DFT_PDS_FACTOR/2))
    {
        pdsInfo->u1PredownBlank = 0;
    }
    else
    {
        pdsInfo->u1PredownBlank = 1;
    }    
}
#endif
#if 0 
...
void _vPDSUpdateDispPrm(UINT8 bPath)
{
    ...
    #ifdef PREDOWN_4K2K_TO_FHD_LOST_PIXEL_WA
    //8.update predown h blanking register.
    _vPDSUpdatePredownHblankPrm(bPath);
    #endif

    //9, hybrid mode --> 10 bit bypass 
    _vPDSHybridModeBypass(bPath);
}
...
static void _vPDSUpdateReg(UINT8 bPath)
{
    ...
    #ifdef PREDOWN_4K2K_TO_FHD_LOST_PIXEL_WA
    vPDSSetPredwnHblankReg(bPath, pdsInfo->u1PredownBlank);
    #endif
    //6. import
    if(vDrvGetImportTriggerModule(bPath) == IMPORT_MODULE_NONE)
    {
...
void vDrvPDSPIPByPass10Bit(UINT8 bOnOff)
{
    if ( bOnOff == 0 ) 
	{
	    PDS_WRITE_FLD(PRE_DS_PIP_09, 0, PRE_DS_PIP_09_PREDS_PIP_10BIT_BYPASS);   //bypass_off  10bitmode  in pIP path
    }
	else
	{
	    PDS_WRITE_FLD(PRE_DS_PIP_09, 1, PRE_DS_PIP_09_PREDS_PIP_10BIT_BYPASS);   //bypass_off  8bitmode  in pIP path	   
	}
}

//-------------------------------------------------------------------------
//bypass sub_pre_dn module for dual hybrid mode
//-------------------------------------------------------------------------
void _vPDSHybridModeBypass(UINT8 bPath)
{
    if ( IS_IC_5861() )
    {
    	if ( bPath == SV_VP_PIP )
    	{
	   		if (bDrvVideoIsDecDualMode())   //HYBRID_MODE == HYBRID_MODE_DUAL ) 
	   		{
	   		       if (getPDSInfo(SV_VP_PIP)->u4InputWidth > getPDSInfo(SV_VP_PIP)->u4OutputWidth)
	   		       {
	   		            vDrvPDSPIPByPass10Bit( OFF );   //bypass OFF
	   		       }
	   		       else
	   		       {
		   		      vDrvPDSPIPByPass10Bit( ON );   //bypass ON
	   		       }   
	   		}
	   		else
	   		{
		   		vDrvPDSPIPByPass10Bit( OFF );   //by_pass OFF
	   		}
    	}
	}
}
