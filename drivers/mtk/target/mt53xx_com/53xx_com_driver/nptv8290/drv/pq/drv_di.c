...
static void _vDrvDICfgPQ(UINT8 u1VdpId, E_MIB_DMODE eDataMode);
...
const rDataMode aDataMode[]=
{
    ...
    DATAMODE_ARA(E(P,NR_Y    ,DI_OFF   ,FRC_SCL), 64, DMOD_M),
    ...
    //Hybrid
    DATAMODE_ARA(E(P,NR_OFF  ,DI_OFF    ,FRC_HYBRID), 64, DMOD_M),
    DATAMODE_ARA(E(P,NR_Y      ,DI_OFF   ,FRC_HYBRID), 64, DMOD_M),
    DATAMODE_ARA(E(P,NR_YC    ,DI_OFF   ,FRC_HYBRID), 64, DMOD_M),
    DATAMODE_ARA(E(P,NR_OFF  ,DI_OFF    ,FRC_DUAL_HYBRID), 64, DMOD_M | DMOD_S),
...
{        
    ...
    if (u4DrvTDTVIsDualFPI()  ||  bIsScalerInput444(u1VdpId)  ||
...
static inline UINT8 _u1DrvDIGetFrcMode(UINT8 u1VdpId)
{
    ...
    else if (rMIBFbmPool[u1VdpId].u4Mode & FBM_POOL_MODE_MDDI_HYBRID)
    {
        return E_MIB_FRC_HYBRID;
    }
    else if (rMIBFbmPool[VDP_1].u4Mode & FBM_POOL_MODE_MDDI_DUAL_HYBRID)
    {
        return E_MIB_FRC_DUAL_HYBRID;
    }
    else
    {
...
static inline UINT8 _u1DrvDIGetDIDataMode(UINT8 u1VdpId)
{
    ...
    if (u4DrvTDTVIsDualFPI() ||
        (rMIBFbmPool[u1VdpId].u4Mode & FBM_POOL_MODE_MDDI_DUAL_HYBRID))
    {
        u1VdpId=VDP_1;         
    }
...
static inline E_MIB_DMODE _u1DrvDIGetMIBDataMode(UINT8 u1VdpId)
{
    ...
    if (SV_VP_PIP==u1VdpId &&
         (u4DrvTDTVIsDualFPI() || (rMIBFbmPool[VDP_1].u4Mode & FBM_POOL_MODE_MDDI_DUAL_HYBRID)) 
       )
    {
...
static inline void _vDrvDISetKernalCtrl(UINT8 u1VdpId, E_MIB_DMODE eDataMode)
{
    ...
    {                          
        ...
        case E(P,NR_YC    ,DI_OFF ,FRC_MIB):            
            ...
            u4DA_PROG_MODE  =SV_ON;
            break;            
         //Progressive Hybrid Dram mode
        case E(P,NR_OFF    , DI_OFF  ,FRC_HYBRID):
        case E(P,NR_Y       , DI_OFF   ,FRC_HYBRID):          
        case E(P,NR_YC     , DI_OFF   ,FRC_HYBRID):            
            u4AF_FROM_SRC   =HW_MIB_AF_FROM_SRC;
            u4I2P_EN        =SV_OFF;                               
            u4KERNEL_SYNC   =E_MIB_SYNC_INPUT;
            u4OUTBUF_SYNC   =E_MIB_SYNC_INPUT;    // no influence
            u4DA_SYNC_MODE  =E_MIB_SYNC_INPUT;   //// not sure ??
            u4OUT_TG_MODE   =E_TG_FREE_RUN;
            u4DA_SC_READ_ENABLE=SV_OFF;
            u4DA_PROG_MODE  =SV_OFF;
            break;       
         //Progressive Dual Hybrid Dram mode
        case E(P,NR_OFF   ,DI_OFF ,FRC_DUAL_HYBRID):        
            u4AF_FROM_SRC   =HW_MIB_AF_FROM_SRC;
            u4I2P_EN        =SV_OFF;                               
            u4KERNEL_SYNC   =E_MIB_SYNC_INPUT;
            u4OUTBUF_SYNC   =E_MIB_SYNC_INPUT;     
            u4DA_SYNC_MODE  =E_MIB_SYNC_OUTPUT;  //use src_w
            u4OUT_TG_MODE   =E_TG_FREE_RUN;
            u4DA_SC_READ_ENABLE=SV_OFF;
            u4DA_PROG_MODE  =SV_OFF;
            break;  
    ...
    MDDI_WRITE_FLD(u1VdpId,MCVP_KC_00   , ( u4BYPASS_EN || IS_HYBRID(u1VdpId) ),BYPASS_EN);    
    ...
    //u4InSigClk = vRegenGetRegenOutClk(u1VdpId,REGEN_MODULE_PRE_REGEN);
    u1NotSupporedSignal = !u1DrvDIIsSupportedSignal(u1VdpId, E_DI_CLK_KERNEL); //  >  162M
    
    if(VDP_1==u1VdpId)
    {
        MDDI_WRITE_FLD(VDP_1,MCVP_GC_00 , (u4BYPASS_EN || u1NotSupporedSignal), M_SYNC_FULL_SPEED);
        vIO32WriteFldAlign(TOP_MAIN_00,(u4BYPASS_EN || u1NotSupporedSignal), BYPASS_DIV2_REGEN_M);
        if ( (IS_HYBRID(u1VdpId) || IS_DUAL_HYBRID(u1VdpId)) && u1NotSupporedSignal)
        {  //  single hybrid mode (NR) and 4k timing
            MDDI_WRITE_FLD(VDP_1, MCVP_GC_00 , 1, MIB_4K_MODE);
            MDDI_WRITE_FLD(VDP_1, MCVP_GC_00 , 1, MIB_FULL_DATA_RATE);
        }
        else
        {
            MDDI_WRITE_FLD(VDP_1, MCVP_GC_00 , 0, MIB_4K_MODE);
            MDDI_WRITE_FLD(VDP_1, MCVP_GC_00 , 0, MIB_FULL_DATA_RATE);
        }
    }
    else
    {
        MDDI_WRITE_FLD(VDP_1,MCVP_GC_00 , (u4BYPASS_EN || u1NotSupporedSignal), P_SYNC_FULL_SPEED);
        vIO32WriteFldAlign(TOP_PIP_00 , (u4BYPASS_EN || u1NotSupporedSignal), BYPASS_DIV2_REGEN_P);
    }   

#if defined(CC_FPGA_SUPPORT)    
        MDDI_WRITE_FLD(VDP_1,MCVP_GC_00 , 0, M_SYNC_FULL_SPEED);
        vIO32WriteFldAlign(TOP_MAIN_00, 0, BYPASS_DIV2_REGEN_M);
#endif
...
static  void _vDrvDISetDramCtrl(UINT8 u1VdpId, E_MIB_DMODE eDataMode)
{
    ...
    if (IS_PROGRESSIVE(u1VdpId) && IS_SCL_FRC(u1VdpId))    
    {
    ...
    if (rMIBState[u1VdpId].u1ModeChg)
    {
        if (IS_DUAL_HYBRID(u1VdpId))
        {
            UINT8 u1FixLR = 0;

            u1FixLR = (u1VdpId == SV_VP_PIP);
            if (IS_FLIP_MIRROR(SV_VP_MAIN))
            {
                u1FixLR = !u1FixLR;
            }
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, 1, DA_3D_FIX_LR_EN);    
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, u1FixLR, DA_3D_FIX_LR);    
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, 1, DA_3D_FRAME_INC_EN);    
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, (u1VdpId == SV_VP_MAIN), DA_FRAME_IDX_SEL);   
            
            MDDI_WRITE_FLD(u1VdpId, MCVP_BC_06, 1, HYBRID_V_START_SEL);    
            MDDI_WRITE_FLD(u1VdpId, MCVP_BC_06, 1, HYBRID_FRP_SEL);    
        }
        else
        {
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, 0, DA_3D_FIX_LR_EN);  
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, 0, DA_3D_FRAME_INC_EN);    
            MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_15, 0, DA_FRAME_IDX_SEL);   

            MDDI_WRITE_FLD(u1VdpId, MCVP_BC_06, 0, HYBRID_V_START_SEL);    
            MDDI_WRITE_FLD(u1VdpId, MCVP_BC_06, 0, HYBRID_FRP_SEL);  
        }
    }
...
{
    UINT32 u4MuteSWReg = IO32ReadFldAlign(PSCAN_MISC_01, MDDI_MUTE_PERIOD_TUNE_EN);
    if(!u4MuteSWReg)
    {
        #define MDDI_MUTE_DI_I        4 
        ...
        if (IS_HYBRID(u1VdpId) || IS_DUAL_HYBRID(u1VdpId))
        {
            u4Offset = MDDI_MUTE_SCALER;
        }
        else
        {
...
static void _vDrvDISetMisc(UINT8 u1VdpId)
{
    ...
    else if (IS_SCL_FRC(u1VdpId))// input sync mode, no transition garbage when changing display window
    {
        if(u1VdpId == VDP_2)
        {
            //DTV00618160 DTV00620996
            MDDI_WRITE_FLD(u1VdpId, MCVP_KC_00, 0xA, SRCW_V_BGN);
        }
        else
        {
            //change from C to B because C causes another bug from B2R
            MDDI_WRITE_FLD(u1VdpId, MCVP_KC_00, 0xB, SRCW_V_BGN);
        }
    }
    else
    {
    ...
    if (VDP_2 == u1VdpId)
    {
        ...
        {
            MDDI_WRITE_FLD(VDP_2,MCVP_KC_00, 1, FRAME420);      
        }
        else
        {
            MDDI_WRITE_FLD(VDP_2,MCVP_KC_00, 0, FRAME420);      
        }   
    }
}
...
static void inline _vDrvDISetResEnd(UINT8 u1VdpId)
{
    ...
    {
        MDDI_WRITE_FLD(u1VdpId,MCVP_KC_33, (IS_PROGRESSIVE(u1VdpId) && IS_SCL_FRC(u1VdpId)) || IS_SCL_BOB(_arMDDiPrm[u1VdpId].eDataMode),PROG_LAST_PIXEL_OPT);
    }
    #endif
...
static inline void _vDrvDISetActiveWindow(UINT8 u1VdpId, UINT16 wSrcXOffset, UINT16 wSrcYOffset, UINT16 wSrcWidth, UINT16 wSrcHeight)
{
    ...
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_1E, 
        (IS_FRAME_SEQ(u1VdpId) && IS_INTERLACE(u1VdpId) ) ?
    ...
    /*if((VDP_2==u1VdpId) && (_arMDDiPrm[VDP_2].u1VencOn) &&IS_PROGRESSIVE(u1VdpId))
    ...
    if (VDP_1==u1VdpId)
    {
        ...
        // Demo workaround for 420 SD progressive bottom line chroma error issue
...
static UINT8 _u1DrvDIMaskedOnOff(UINT8 u1VdpId)
{
    ...
    if ((u1IsChannelOn == SV_FALSE)
        ...
        || ((!u1DrvDIIsSupportedSignal(u1VdpId, E_DI_CLK_KERNEL)) && (IS_SCL_FRC(u1VdpId)))
...
{    
    return (IS_MPEG(u1VdpId)
            ...
            && !(IS_HYBRID(u1VdpId)||IS_DUAL_HYBRID(u1VdpId)))
...
{    
    ...
    if(u4PrevDMODE[u1VdpId] == eDataMode && 
        !rMIBState[u1VdpId].u1ModeChg           &&
        !(u1PreTvdMode != bHwTvdMode() && u1VdpId == SV_VP_MAIN) )
    {
        return; //Nothing changes.  datamode not changed && no modechange && Main tvd mode not changed
    }
    ...
    if(SV_VP_MAIN==u1VdpId)
    {
        ...
        if( 
            ...
            //NXNR doesn't work on 4k
            || (IS_HYBRID(u1VdpId))
            || (IS_DUAL_HYBRID(u1VdpId))
...
static inline UINT8 _u1DrvDIModeChange(UINT8 u1VdpId)
{
    ...
    if(IS_MIB_FRC(u1VdpId) || IS_HYBRID(u1VdpId) || IS_DUAL_HYBRID(u1VdpId))
    {   
    ...
    //For  interlace MM/DTV field repeat, switch aspect mode with only front regen/output window change.
    ...
    VRMGetModuleVRMInfo(u1VdpId, VRM_MODULE_FSC_REGEN, &in, &out);
    rNewMIBCfg.FrontRegenX    = out.u2X;
    rNewMIBCfg.FrontRegenY    = out.u2Y;
    rNewMIBCfg.FrontRegenW    = out.u2Width;
    rNewMIBCfg.FrontRegenH    = out.u2Height;
...
#ifdef CC_FLIP_MIRROR_SUPPORT
    rNewMIBCfg.u1HybridSCFlipMorror = 
    (FLIP_BY_SCALER==u4GetFlipMirrorModule(u1VdpId) && E_MIB_FRC_HYBRID == rNewMIBCfg.u1FrcMode);
#else
    rNewMIBCfg.u1HybridSCFlipMorror = SV_OFF;
#endif
    ...
    if (((rMIBState[u1VdpId].u1DynRes)
            &&((_arMDDiPrm[u1VdpId].u2HClip != rNewMIBCfg.u2HClip)
                ...
                || (_arMDDiPrm[u1VdpId].FrontRegenX != rNewMIBCfg.FrontRegenX)
                || (_arMDDiPrm[u1VdpId].FrontRegenY != rNewMIBCfg.FrontRegenY)
                || (_arMDDiPrm[u1VdpId].FrontRegenW != rNewMIBCfg.FrontRegenW)
                || (_arMDDiPrm[u1VdpId].FrontRegenH != rNewMIBCfg.FrontRegenH)))
        ...
        UINT32 u4Address, UINT32 u4Size, UINT32 u4Width, UINT32 u4Height)
{
    ...
    if (VDP_1 == u1VdpId && (u4Mode & FBM_POOL_MODE_MDDI_DUAL_HYBRID))
    {
        rMIBFbmPool[VDP_2].u4Addr = rMIBFbmPool[VDP_1].u4Addr;
        rMIBFbmPool[VDP_2].u4Size = rMIBFbmPool[VDP_1].u4Size;
        rMIBFbmPool[VDP_2].u4Width = rMIBFbmPool[VDP_1].u4Width;
        rMIBFbmPool[VDP_2].u4Height = rMIBFbmPool[VDP_1].u4Height;
        rMIBFbmPool[VDP_2].fg10bit = rMIBFbmPool[VDP_1].fg10bit;
        rMIBFbmPool[VDP_2].fg422 = rMIBFbmPool[VDP_1].fg422;
        rMIBFbmPool[VDP_2].u4Mode = rMIBFbmPool[VDP_1].u4Mode;
    }
}
 ...
 * @param u1VdpId VDP_1/VDP_2
 */
static void  _vDrvDIChkImpl(UINT8 u1VdpId)
{ 
    PATH_CHECK(u1VdpId);
...
#endif
    {
        ...
        _vDrvDICfgPQ(u1VdpId, rMIBState[u1VdpId].eBandwidthDataMode);
...
static inline void _vDrvDISramShareCfg(void)
{
    ...
    if (IS_IC_5861())
    {
        u4Width = (MDDI_READ_FLD(SV_VP_MAIN, MCVP_DRAM_09, DA_DRAM_LINE_LENGTH)+1) * MDDI_LINE_LENGHT_UNIT;
    }
    else
    {
        u4Width = MDDI_READ_FLD(SV_VP_MAIN, MCVP_DRAM_09, DA_DRAM_LINE_LENGTH) * MDDI_LINE_LENGHT_UNIT;
    }
...
void vDrvDISrmCb(UINT32 u4Arg1, UINT32 u4Arg2, UINT32 u4PDSize, UINT32 u4TDTV)
{
    ...
    UINT8  fgIsDualMode = ((u4Arg2 & FBM_POOL_MODE_MDDI_DUAL_HYBRID) > 0);
    ...
    if (u4Arg1 == VDP_1)
    {
        ...
        if (fgIsDualMode)
        {
            r3DInOutFormat[VDP_1].eTDInput    = E_TD_IN_FS_P;
            r3DInOutFormat[VDP_1].eTDOutput  = E_TD_OUT_3D_FS;
            r3DInOutFormat[VDP_2].eTDInput    = E_TD_IN_FS_P;
            r3DInOutFormat[VDP_2].eTDOutput  = E_TD_OUT_3D_FS;
        }
    }
    else
    {
    ...
    if (VDP_2 == u4Arg1 && (rMIBFbmPool[VDP_1].u4Mode & FBM_POOL_MODE_MDDI_DUAL_HYBRID) )
    {
        return;
    }
    ...
    if(prPool != NULL)
    {
        ...
        if ( u4DrvTDTVIsDualFPI() || fgIsDualMode)
        {
            if(VDP_1==u4Arg1)
            {  
...
void vDrvDIHwInit(UINT8 u1VdpId)
{
    ...
    MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_09, 1, DA_READ_REQ_SPACING);
    ...
    MDDI_WRITE_FLD(u1VdpId, MCVP_KC_07, 0, IPTG_HV_NO_ALIGN);
}
