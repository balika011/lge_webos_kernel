...
UINT16 waLumaHist_65[2][LUMA_HIST_NUM_65];
...
void vDrvGetLuma65HistLR(UINT16 waHistL[LUMA_HIST_NUM_65], UINT16 waHistR[LUMA_HIST_NUM_65], UINT16 u2NormBase)
{
    UINT8 bi;
    
	for (bi = 0; bi < LUMA_HIST_NUM_65; bi++)
	{	
		waHistL[bi] = u2IO32Read2B(LUMA_HIST_START+2*bi);
        waHistR[bi] = IS_UI_3D_MODE ? u2IO32Read2B(LUMA_HIST_START+METER_OFFSET_LR+2*bi): 0;
    }     
	...
	if (u2NormBase)
	{
	    bHistogramNormalization(waHistL, LUMA_HIST_NUM_65, u2NormBase);
	    bHistogramNormalization(waHistR, LUMA_HIST_NUM_65, u2NormBase);   
	}
}
...
void vDrvGetLumaInfoHW(void)
{
	...
	vIO32WriteFldMulti(METER_INFO_00+u1OffsetLR,
                        P_Fld(((u4PixCnt) ? ((((u4LumaSum+(1<<19))>>20)*u4PixCntDiv + (1<<7))>>8) : 0), METER_APL_VAL) 
	...
	for (i = 0; i < LUMA_HIST_NUM_65-1; i+=2)
	{
		if(i < LUMA_HIST_NUM)
		{
	        waLumaHist[u1OffsetLRFlg][i] = u4IO32Read4B(LUMA_HIST_00+(i*4))>>7;
		...
		waLumaHist_65[u1OffsetLRFlg][i] = u4IO32Read4B(LUMA_HIST_00+(i*4))>>7;
        waLumaHist_65[u1OffsetLRFlg][i+1] = u4IO32Read4B(LUMA_HIST_00+(i*4+4))>>7;

		vIO32Write4B(LUMA_HIST_START+u1OffsetLR+2*i, (waLumaHist_65[u1OffsetLRFlg][i] + (waLumaHist_65[u1OffsetLRFlg][i+1]<<16)));
	}
	
    waLumaHist[u1OffsetLRFlg][LUMA_HIST_NUM] = u4IO32Read4B(LUMA_HIST_32)>>7;
	waLumaHist_65[u1OffsetLRFlg][LUMA_HIST_NUM_65-1] = u4IO32Read4B(LUMA_HIST_64)>>7;
    vIO32Write4B(LUMA_HIST_END+u1OffsetLR, waLumaHist_65[u1OffsetLRFlg][LUMA_HIST_NUM_65-1]);
}
...
void vDrvMeterHWInit(void)
{
	...
	vIO32WriteFldAlign(LUMA_MIN_MAX_MAIN, 1, LUMA_SUM_SHIFT);
}
