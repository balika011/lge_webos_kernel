...
UINT8 u1AmbiHBlkNum;
UINT8 u1AmbiVBlkNum;
UINT8 u1LineArray[20];
UINT8 u1LineBlkNum[640];
UINT8 u1AmbiLIsrInitialized = SV_FALSE;
#endif
...
extern UINT8 bIsBT2020Timing;
extern UINT8 bIsBT2020CLPath;
...
void vDrvSetGammaRemapIndex(UINT32 u4Index)
{
    u4Index = MIN(u4Index, 0xFF);
	vIO32WriteFldAlign(GAMMA_REMAP_00, u4Index, GAMMA_REMAP_INDEX);
}
...
void vDrvxvYCCOnOff(UINT8 bOnOff)
{
    ...
    vApiRegisterVideoEvent(PE_EVENT_XVYCC, SV_VP_MAIN, SV_ON);
#else
...
// TODO_BT2020
void vDrvBT2020CLPathEnable(UINT8 bEnable)
{
#if defined(CC_MT5890)
    // pass Y channel into xvYCC
    vIO32WriteFldAlign(XVYCC_00, bEnable, REG_XVYCC_BT2020_Y_EN);    
    // enable extra 2 coef in output matrix
    vIO32WriteFldAlign(YCBCR2RGB_09, bEnable, R_BT2020_CL_EN);
#endif
...
void vDrvSetxvYCCGammaType(UINT8 u1Type)
{
    vIO32WriteFldAlign(XVYCC_00, u1Type, REG_XVYCC_GM_SRGB);
    vIO32WriteFldAlign(XVYCC_00, u1Type, REG_XVYCC_AGM_SRGB);    
}
...
void vDrvAmbiLitSysInit(void)
{
	...
	UINT8 u1BlockNumH=32;
	UINT8 u1BlockNumV=20;

	u4PanelWidth = PANEL_GetPanelWidth();
	u4PanelHeight = PANEL_GetPanelHeight();

    u4BlockWidth = u4PanelWidth / u1BlockNumH;
    u4BlockHeight = u4PanelHeight / u1BlockNumV;    
	u4BlockPxlHLast =u4BlockWidth + (u4PanelWidth % u1BlockNumH);
	u4BlockPxlVLast = u4BlockHeight + (u4PanelHeight % u1BlockNumV);
    // Set System Info
    ...
    if((u4PanelWidth<=1920)&&(u4PanelHeight<=1080))
	{	
	    vIO32WriteFldAlign(AMBL_00_ES2, 4, R_AMBI_SHIFT_N);            
	}
	else
	{	
		vIO32WriteFldAlign(AMBL_00_ES2, 6, R_AMBI_SHIFT_N);		   
	}
    vIO32WriteFldAlign(AMBL_01_ES2, u4BlockWidth, R_AMBI_BLK_W);    
    vIO32WriteFldAlign(AMBL_01_ES2, u4BlockHeight, R_AMBI_BLK_H);    
    vIO32WriteFldAlign(AMBL_01_ES2, u4BlockPxlHLast, R_AMBI_BLK_W_LAST);    
    vIO32WriteFldAlign(AMBL_01_ES2, u4BlockPxlVLast, R_AMBI_BLK_H_LAST); 
	u1AmbiHBlkNum=u1BlockNumH;
	u1AmbiVBlkNum=u1BlockNumV;
}
...
void vDrvAmbiLitUpdateLine(UINT16 * u2AmbiLitArray, UINT8 * u1AmbilBlk, UINT8 u1Line)
{
	...
	static UINT8 i=0;
	u1LineArray[i]=u1Line;//for debug
	++i;
	i%=20;

	//vIO32WriteFldAlign(AMBL_00_ES2,u1StartBlkNum,R_AMBI_START_BLKNUM);
    for (u1Block = 0; u1Block < u1AmbiHBlkNum; u1Block++)
    {
        ...
        u2AmbiLitArray[AMBILIT_ROFFSET + u1Line*u1AmbiHBlkNum + u1Block] = u2BlockResultR;
        u2AmbiLitArray[AMBILIT_GOFFSET + u1Line*u1AmbiHBlkNum + u1Block] = (UINT16)(u4BlockResultGB & 0xFFFF);
        u2AmbiLitArray[AMBILIT_BOFFSET + u1Line*u1AmbiHBlkNum + u1Block] = (UINT16)((u4BlockResultGB>>16) & 0xFFFF);        

        // for debug
       u1AmbilBlk[u1Line*u1AmbiHBlkNum + u1Block] = u1CurBlock;
	   u1LineBlkNum[u1Block]=u1CurBlock;
...
void vDrvAmbiLPrintContent(UINT8 u1Color)
{
    ...
    if(u1Color < 3)
    {
        for (u1Line = 0; u1Line < u1AmbiVBlkNum; u1Line++)
        {
            Printf("{%d}\n", u1Line);
            for (u1Block = 0; u1Block < u1AmbiHBlkNum; u1Block++)
            {
                Printf("%d ", u2AmbiLitArray[u1Color*u1AmbiVBlkNum*u1AmbiHBlkNum + u1Line*u1AmbiHBlkNum + u1Block]);
            }
            Printf("\n");
        }
    }
...
void vDrvAmbiLPrintBlkInfo(UINT8 u1Color,UINT8 id)
{
	UINT8 u1Block, u1Line;
	...
	if(u1Color < 3)
	{
		for (u1Line = 0; u1Line < u1AmbiVBlkNum; u1Line++)
		{
			...
			else if(id==2)
			{
				...
				for (u1Block = 0; u1Block < u1AmbiHBlkNum; u1Block++)
				{
					Printf("%d ", u1AmbiLitBLKArray[u1Color*u1AmbiVBlkNum*u1AmbiHBlkNum + u1Line*u1AmbiHBlkNum + u1Block]);
				}
				Printf("\n");
			}
...
void _vDrvAmbiLISR(UINT16 u2Vector)
{
    ...
    { 
		...
		u1LineArray[i]=u1AmbiLitLineID;//for debug
		++i;
		i%=20;
...
void vDrvAmbiLGetAllContent(UINT16 *u2AmbiLResult)
{
    if(u1AmbiLIsrInitialized)
    {
        x_memcpy(u2AmbiLResult, u2AmbiLitArray, 3840); // copy 640 * 2byte
    }
}
void vDrvAmbiLGetContent(UINT8 u1Color, UINT16 *u2AmbiLResult)
{
    if((u1Color <= 2)&&(u1AmbiLIsrInitialized))
    {
        x_memcpy(u2AmbiLResult, u2AmbiLitArray + (640*u1Color), 1280); // copy 640 * 2byte
    }
    else
    {
        ASSERT(0);
...
void vDrvSetDBLParam(UINT8 bP1, UINT8 bP2, UINT8 bG1, UINT8 bG2)
{
    vRegWriteFldMulti(DBL_01, 
	                    ...
	                    P_Fld(bG1, DBL_G1) |
	                    P_Fld(bG2, DBL_G2));
    vRegWriteFldAlign(DBL_00, 0, DBL_G_SLOPE);    
}

UINT8 u1GetDBLGain(void)
{
    UINT8 u1Gain = IO32ReadFldAlign(DBL_01, DBL_G1);
