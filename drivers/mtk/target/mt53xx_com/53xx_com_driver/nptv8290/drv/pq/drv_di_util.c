...
{    
    ...
    else if (IS_DUAL_HYBRID(u1VdpId))
    {
        MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_08, u1Mode, DA_DISPLAY_MODE); //420            
        MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_18, DI_NORMAL_MODE, DA_SC_DISPLAY_MODE);
    }
...
void vDrvDISetClkReset(UINT8 u1VdpId, UINT8 u1OnOff)
{
    ...
    UINT8 u1DualHybrid = (rMIBFbmPool[u1VdpId].u4Mode & FBM_POOL_MODE_MDDI_DUAL_HYBRID);

    if ((u1VdpId == VDP_1)  ||  (u4DrvTDTVIsDualFPI())  ||  u1DualHybrid)
    {        
    ...
    if ((u1VdpId == VDP_2)  ||  (u4DrvTDTVIsDualFPI())  ||  u1DualHybrid)
    {
...
void vDrvDISetClock(void)
{
    ...
    if ( IS_IC_5861() )
    {
        ...
        {
            vIO32WriteFldAlign(CKGEN_REG_MIB_OCLK_CKCFG, MIB_CLK_TYPESYSPLL_D1_CK, FLD_MIB_OCLK_SEL);
            vIO32WriteFldAlign(CKGEN_REG_MIB_OCLK_CKCFG, MIB_CLK_MIB_OCLK_D2_CK, FLD_MCVP_CK_SEL);
        }
        else
        {
            vIO32WriteFldAlign(CKGEN_REG_MIB_OCLK_CKCFG, MIB_CLK_TYPESYSPLL_D2_CK, FLD_MIB_OCLK_SEL);
            vIO32WriteFldAlign(CKGEN_REG_MIB_OCLK_CKCFG, MIB_CLK_MIB_OCLK_CK, FLD_MCVP_CK_SEL);
        }   
    }
    else
    #endif
