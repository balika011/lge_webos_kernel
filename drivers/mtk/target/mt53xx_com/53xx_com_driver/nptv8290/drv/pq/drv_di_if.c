...
UINT32 u4DrvDIMaxInputWidth(UINT8 u1VdpId)
{
    ...
    else if ( SRM_IsHybridMode(u1VdpId) )
    {
        u4MaxInputWidth = 3840;
    }
    else
    {
...
UINT32 u4DrvDIMaxOutWidth(UINT8 u1VdpId,UINT32 u4InputVActive,UINT32 u4DispVActive,UINT32 u4DispVTotal,UINT32 u4OutFR)
{
    ...
    UINT32 u4MDDIMode = SRM_GetMddiMode(u1VdpId);

#define INTERLACE_CLK_TOLERANCE 0x40
#define LI_INPUT_UPPER_BOUND 960

    if ( (u4MDDIMode & FBM_POOL_MODE_MDDI_HYBRID) ||
          (u4MDDIMode & FBM_POOL_MODE_MDDI_DUAL_HYBRID) )
    {
        return 3840;
    }
    
    if ( (u4MDDIMode & FBM_POOL_MODE_MDDI_DISP) &&
         (0!=u4DispVTotal)&&(0!=u4OutFR)&&(0!=u4InputVActive) 
       )
    {
        u4MaxOutputWidth = 
...
{   
    ...
    if ( (u4MDDIMode & FBM_POOL_MODE_MDDI_FULL) || (u4MDDIMode & FBM_POOL_MODE_MDDI_CBOB) )
    {
        u2Delay += 20;
    }

    if ( (u4MDDIMode & FBM_POOL_MODE_MDDI_DISP)      ||
          (u4MDDIMode & FBM_POOL_MODE_MDDI_HYBRID)    ||
          (u4MDDIMode & FBM_POOL_MODE_MDDI_DUAL_HYBRID)  
       )
    {
        u2Delay += 5;
...
{
    UINT8   u1ClkType; 
    UINT32 u4Clk, u4RightShift = 0;
    u1ClkType = IO32ReadFldAlign(CKGEN_REG_MIB_OCLK_CKCFG, FLD_MIB_OCLK_SEL);    

    #if defined(CC_MT5890)
    if ( IS_IC_5861() &&
         MIB_CLK_MIB_OCLK_D2_CK == IO32ReadFldAlign(CKGEN_REG_MIB_OCLK_CKCFG, FLD_MCVP_CK_SEL) )
    {
        u4RightShift ++;
    }
    #endif
    ...
    switch(u1ClkType)
    {
        case MIB_CLK_TYPESYSPLL_D1_CK:  
            u4Clk = MIB_SYSTEM_PLL;
            break;
        case MIB_CLK_TYPEENETPLL_D1_CK:
            u4Clk = MIB_ETHERNET_PLL;
            break;
...
UINT8 u1DrvDIIsSupportedSignal(UINT8 u1VdpId, E_DI_CLK_TYPE eClkType)
{
    ...
    u4DIClk = u4DrvDIGetClkRate(u1VdpId);

    if (eClkType == E_DI_CLK_SRCW)
    {
        u4DIClk <<= 1;
    }
    ...
    if(u4InputClk >= u4DIClk) // && TDTV_TAG3D_2D == u4DrvTDTVGetTag3D(u1VdpId)) 
    {
        return SV_OFF;
    }
    else
...
void u1DrvDIForceBypass(UINT8 u1VdpId, UINT8 u1BypassOnOff)
{
    ...
    //Notify VRM resolution change and disable set resolution.
