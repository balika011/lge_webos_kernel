...
UINT16 waLumaArray_65[LUMA_CURVE_NUM_65]; //luma curve
UINT16 waSecondLumaArray_65[LUMA_CURVE_SKIN_NUM_65];	//Second luma curve
...
static void vDrvADLSetLumaCurve65Hw(const UINT16 * waLArray)
{
	UINT8 bi;
...
void vDrvADLSetSecondLumaCurve65Hw(const UINT16 * waSLArray)
{
	UINT8 bi;
...
{
	UINT32 i;
	UINT8 u1CurveMode = vDrvADLGetLumaCurveMode();
	...
	{
		for (i = 0; i < LUMA_CURVE_NUM_65-1; i++)
		{
			waLumaArray_65[i] = (UINT16)(i << 4);
		}

		waLumaArray_65[LUMA_CURVE_NUM_65-1] = 1023;
	    vDrvADLSetLumaCurve65Hw(waLumaArray_65);
	}
...
void vDrvADLSetLinearSecondCurve(void)
{
	...
	else
	{
		for (i = 0; i < LUMA_CURVE_NUM_65-1; i++)
		{
			waSecondLumaArray_65[i] = (UINT16)(i << 4);
		}

		waSecondLumaArray_65[LUMA_CURVE_NUM_65-1] = 1023;
	    vDrvADLSetSecondLumaCurve65Hw(waSecondLumaArray_65);
	}
...
void vDrvADLGetLumaCurve65(UINT16* waLArray)
{
    if (waLArray == NULL)
    {
        return;
	...
	x_memcpy(waLArray, waLumaArray_65, LUMA_CURVE_NUM_65*sizeof(UINT16));
}

void vDrvADLSetLumaCurve65(const UINT16* waLArray)
{
    if (waLArray == NULL)
    {
        return;
    ...
    x_memcpy(waLumaArray_65, waLArray, LUMA_CURVE_NUM_65*sizeof(UINT16));
}
...
void vDrvADLSetLumaSecCurve65(const UINT16* waLArray)
{
    if (waLArray == NULL)
    {
        return;
    ...
    x_memcpy(waSecondLumaArray_65, waLArray, LUMA_CURVE_SKIN_NUM_65*sizeof(UINT16));
}
...
void vDrvADLSetLumaCurveMode(UINT8 u1Mode)
{
    vIO32WriteFldAlign(SCE_CFG_MAIN, u1Mode, YCURVE_33_EN);
}

void vDrvADLSetYHistogramMode(UINT8 u1Mode)
{
    vIO32WriteFldAlign(SCE_CFG_MAIN, u1Mode, COLOR_YHIST_TYPE);
}

UINT8 vDrvADLGetLumaCurveMode(void)
{
	return (UINT8)IO32ReadFldAlign(SCE_CFG_MAIN, YCURVE_33_EN);
}

UINT8 vDrvADLGetYHistogramMode(void)
{
	return (UINT8)IO32ReadFldAlign(SCE_CFG_MAIN, COLOR_YHIST_TYPE);
}
...
// Update Adaptive luma version 2 parameter
	...
	vIO32WriteFldAlign(ADAP_LUMA_0A, wReadQualityTable(QUALITY_ALCOM_SKIN_LUMA_BYP), ADL_SECOND_LUMA_EN);
...
{
	if (IO32ReadFldAlign(ADAPTIVE_REG, ADAPTIVE_LUMA) == 1)
	{
