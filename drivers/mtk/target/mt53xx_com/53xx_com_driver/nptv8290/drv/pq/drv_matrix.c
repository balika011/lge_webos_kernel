...
#define SUPPORT_BT2020_AUTO_DETECT SV_FALSE
#define SUPPORT_BT2020_UI_CONTROL SV_FALSE
...
UINT8 bIsBT2020Timing = SV_FALSE;
UINT8 bIsBT2020CLPath = SV_FALSE;
...
static INT32 const SRC_BT2020_YUV_YUV2RGB_CL[15] =
{
// TODO_BT2020
	0x0000,    0x0000,    0x0000,
	0x2000,    0x0000,    0x0000,
...
void vVdoSetColorTransform(UINT8 bPath)
{
    ...
    bIsBT2020Timing = SV_FALSE;
    bIsBT2020CLPath = SV_FALSE;
    ...
    switch (u1DecType)
    {
        ...
        case SV_VD_DVI:
            ...
            {                
				...
				// TODO_BT2020 : put BT2020 flag here
				bIsBT2020Timing = SV_FALSE;
				bIsBT2020CLPath = SV_FALSE;
#endif
            }
...
#if SUPPORT_BT2020_AUTO_DETECT
    // driver auto control
    vDrvxvYCCOnOff(bIsBT2020Timing);        
    vDrvxvYCCGammaOnOff(bIsBT2020Timing);
    vDrvxvYCCAntiCGammaOnOff(bIsBT2020Timing);
    vDrvxvYCCLoad3x3Matrix(DEFAULT_XVYCC_MATRIX);

    vDrvBT2020CLPathEnable(bIsBT2020CLPath); 

    if(bIsBT2020CLPath == SV_TRUE)
    {
        vDrvSetBlueStretchOnOff(SV_VP_MAIN, SV_OFF);
    }    
#elif SUPPORT_BT2020_UI_CONTROL
    // from UI control
    vDrvBT2020CLPathEnable(bIsBT2020CLPath); 
...
void vDrvSetColorFilter(UINT8 bPath, UINT8 bColor, UINT8 bOnOff)
{
    UINT32 u4Value = bOnOff ? 0x200 : 0x0;

    switch(bColor)
    {
        case SV_RGB_R:
            vIO32WriteFldAlign(MATRIX_02, u4Value, COLOR_FILTER_R_GAIN);
            break;
        case SV_RGB_G:
            vIO32WriteFldAlign(MATRIX_02, u4Value, COLOR_FILTER_G_GAIN);
            break;
        case SV_RGB_B:
            vIO32WriteFldAlign(MATRIX_02, u4Value, COLOR_FILTER_B_GAIN);
            break;
        default:
            return;
    }
    ...
    if (bPath == SV_VP_MAIN)
    {
        vIO32WriteFldAlign(PEUI_00, SV_ON, PEUI_OS_CSC_MODE_CHANGE_M);
    }
    else
    {
        vIO32WriteFldAlign(PEUI_00, SV_ON, PEUI_OS_CSC_MODE_CHANGE_P);
    }
}
                          ...
                          UINT16 wSatValue, UINT16 wHueValue)
{
    ...
    else
    {
        x_memcpy(dwTableY2R, (bIsScalerInputRGB(bPath))? SRC_RGB_YUV2RGB : 
                                (u4ExtLvl ? 
                                    (u4Matrix709 ? SRC_YUV_YUV2RGBEXTLVL_HD : 
#if SUPPORT_BT2020_AUTO_DETECT                                    
                                        ( bIsBT2020CLPath ? SRC_BT2020_YUV_YUV2RGB_CL : SRC_YUV_YUV2RGBEXTLVL)) : // TODO_BT2020 : add CL case matrix here
#else                                        
                                        SRC_YUV_YUV2RGBEXTLVL) : 
#endif                                        
    ...
    // TODO_BT2020 : add extra 2 coef here.
}
