...
const REGTBL_T CODE MDDI_QTY_INIT_MAIN[] =
{
    ...
    {MCVP_FUSION_2C     , 0x00140004, 0x00FF0004},
    {MCVP_FUSION_2D     , 0x01720408, 0x03FFFFFF},
    //CS
...
static void _vDrvDISetPDMAQuality(UINT8 bPath)
{
    ...
    #ifdef __MODEL_slt__
	vIO32WriteFldAlign(MCVP_SB_01, 0, SB_NR_PAL_C_FILTER);                          
	#else
...
static void _vDrvDISetIFQuality(UINT8 bPath)
{
    ...
    if (bPath == VDP_1)
    {
        ...
        // Set frequency detection
        vIO32WriteFldAlign(MCVP_FUSION_2D, IS_PROGRESSIVE(VDP_1), IF_PROG_MODE);
    }
...
{   
    *u1FreqHpfV = IO32ReadFldAlign(MCVP_FUSION_2B, IF_HD_FIELD_HI_F_SUM);
	*u1FreqHpfH = IO32ReadFldAlign(MCVP_FUSION_2B, IF_HD_HOR_HI_F_SUM);
	*u1FreqBpfH = IO32ReadFldAlign(MCVP_FUSION_2C, IF_HD_HOR_MID_F_SUM);
