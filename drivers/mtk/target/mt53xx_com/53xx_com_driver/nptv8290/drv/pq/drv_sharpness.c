...
static UINT32 TDS_CURVE_REGADDR[TDS_BAND_NUM]={};
...
static REGTBL_T REGTBL_TDSHARP[] =
{
	{0xF0027000, 0x00000000, 0x00000040},   // SHARP_00
	{0xF00270C4, 0x00050000, 0x000F0000},   // SHARP_31
	{0xF00270C8, 0x00020050, 0x0003F0FF},   // SHARP_32
	{0xF00270D8, 0x00020000, 0x00021F1F},   // SHARP_36
    {0xF0027108, 0x0073F1F4, 0xFFFFFFFF},   // SHARP_42
    {0xF002710C, 0x10000000, 0x1F000000},   // SHARP_43
    {0xF0027110, 0x00001000, 0x00001F00},   // SHARP_44
    {0xF0027114, 0x80FF80FF, 0xFFFFFFFF},   // SHARP_45
    {0xF0027118, 0x80FF80FF, 0xFFFFFFFF},   // SHARP_46
    {0xF0027130, 0x84284080, 0xBFFFFFC0},   // SHARP_4C
    {0xF0027134, 0x830C628A, 0xBFFFFFFF},   // SHARP_4D
    {0xF0027138, 0x28A20800, 0xFFFFFF00},   // SHARP_4E
    {0xF0027170, 0x80FF80FF, 0xFFFFFFFF},   // SHARP_5C
    {0xF0027174, 0x000080FF, 0x0000DFFF},   // SHARP_5D
	{REGTBL_END, 0x00000000, 0x00000000}, 
};

static REGTBL_T REGTBL_PBC[] =
{
	//PBC1 color range initial
	{0xF0027600, 0x80800A20, 0x0000FFFF},   // CDS_00
	{0xF0027604, 0x7D048199, 0xFFFFFFFF},   // CDS_01
	{0xF0027608, 0xC2CAC080, 0xFFFFF0FF},   // CDS_02
	{0xF002760C, 0x00C03220, 0xFFF00000},   // CDS_03
	//PBC2 color range initial
	{0xF00276A8, 0x800C0A0A, 0x0000FFFF},   // CDS_2A
	{0xF00276AC, 0x5406000C, 0xFFFFFFFF},   // CDS_2B
	{0xF00276B0, 0x62C85080, 0xFFFFF0FF},   // CDS_2C
	{0xF00276B4, 0x00C82020, 0xFFFFFFFF},   // CDS_2D
	//PBC3 color range initial
	{0xF0027750, 0x800C0A0A, 0x0000FFFF},   // CDS_54
	{0xF0027754, 0xB806000C, 0xFFFFFFFF},   // CDS_55
	{0xF0027758, 0xE3987080, 0xFFFFF0FF},   // CDS_56
	{0xF002775C, 0x00C03220, 0xFFF00000},   // CDS_57
	{REGTBL_END, 0x00000000, 0x00000000},
};
...
static REGTBL_T REGTBL_ECTIR[] =
{
	{0xF0027280, 0x00001A61, 0x10001A79},   // CTIR_00
	{0xF0027284, 0x00003714, 0x07777FFF},   // CTIR_01
	{0xF0027288, 0x1A0C0A05, 0xFFFFFF07},   // CTIR_02
	{0xF002728C, 0x1A0C0A05, 0xFFFFFF07},   // CTIR_03
	{0xF0027290, 0x40000001, 0xFFFFFF77},   // CTIR_04
	{0xF0027294, 0x0000000B, 0x000000FF},   // CTIR_05
	{REGTBL_END, 0x00000000, 0x00000000},
};

static REGTBL_T REGTBL_CBE[] = 
{
	{0xF00272C0, 0x00004040, 0x0F7F7F7F},   // CBE_00
	{0xF00272C4, 0x00000416, 0x00007F7F},   // CBE_01
	{0xF00272C8, 0x2020083F, 0x00007F7F},   // CBE_02
	{REGTBL_END, 0x00000000, 0x00000000}
};
...
void vDrvDDCTIRInit(void)
{
    REGTBL_ECTIR[0].u4Reg = CTIR_00;
    REGTBL_ECTIR[1].u4Reg = CTIR_01;
    REGTBL_ECTIR[2].u4Reg = CTIR_02;
    REGTBL_ECTIR[3].u4Reg = CTIR_03;
    REGTBL_ECTIR[4].u4Reg = CTIR_04;
    REGTBL_ECTIR[5].u4Reg = CTIR_05;
}
...
void vDrvDDCBEInit(void)
{
#ifdef CC_SUPPORT_UHD
    if (PQ_DD_MACRO)
    {
        vIO32WriteFldAlign(0xF00272D4, 1, BYPASS_CBE);
    }

    REGTBL_CBE[0].u4Reg = CBE_00;
    REGTBL_CBE[1].u4Reg = CBE_01;
    REGTBL_CBE[2].u4Reg = CBE_02;
#endif
}
...
UINT8 vDrvTDSharpOffset(UINT8 bBand, UINT8 bItem, UINT8 bValue)
{
	...
	if((bBand > TDS_LTI_V) || (bItem > TDS_CURVE_SW_REG_COR_ZERO))
	{
		return bValue;
	}
	...
	else if(bBand < TDS_LTI_HL)
	{
		bOffset = (UINT8)(u4RegRead4B(u4SWReg(VI_SHP_EXT_OFST_02 + bItem*12)) >> (bBand - TDS_D1)*8 );
	}
	else
	{
		bOffset = (UINT8)(u4RegRead4B(u4SWReg(VI_SHP_EXT_OFST_0B + bItem*4)) >> (bBand - TDS_LTI_HL)*8 );
	}
...
static void vDrvSharpDynCtrl(void)
{
	static UINT8 bPreHSHPGainLowP = 0x80;
	static UINT8 bPreHSHPGainLowN = 0x80;
	static UINT8 bPreHSHPGainMidP = 0x80;
	static UINT8 bPreHSHPGainMidN = 0x80;
	static UINT8 bPreHSHPGainHighP = 0x80;
	static UINT8 bPreHSHPGainHighN = 0x80;	
	static UINT8 bPreHSHPCorLow = 0x80;
	static UINT8 bPreHSHPCorMid = 0x80;
	static UINT8 bPreHSHPCorHigh = 0x80;
	static UINT8 bPreTDSHPEdgeGainH = 0x80;
	static UINT8 bPreTDSHPEdgeGainD = 0x80;
	static UINT8 bPreTDSHPEdgeGainV = 0x80;
	static UINT8 bPreTDSHPDetailGainH = 0x80;
	static UINT8 bPreTDSHPDetailGainD = 0x80;
	static UINT8 bPreTDSHPDetailGainV = 0x80;

	UINT8 bHSHPGainLowP, bHSHPGainLowN, bHSHPGainMidP, bHSHPGainMidN, bHSHPGainHighP, bHSHPGainHighN;
	UINT8 bHSHPCorLow, bHSHPCorMid, bHSHPCorHigh;
	UINT8 bTDSHPEdgeGainH, bTDSHPEdgeGainD, bTDSHPEdgeGainV;
	UINT8 bTDSHPDetailGainH, bTDSHPDetailGainD, bTDSHPDetailGainV;

	bHSHPGainLowP 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_02, HSHP_GAIN_LOW_POS) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_06, HSHP_GAIN_LOWP_OFST) - 0x80), 0, 0xFF);
	bHSHPGainLowN 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_02, HSHP_GAIN_LOW_NEG) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_06, HSHP_GAIN_LOWN_OFST) - 0x80), 0, 0xFF);
	bHSHPGainMidP 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_02, HSHP_GAIN_MID_POS) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_06, HSHP_GAIN_MIDP_OFST) - 0x80), 0, 0xFF);
	bHSHPGainMidN 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_02, HSHP_GAIN_MID_NEG) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_06, HSHP_GAIN_MIDN_OFST) - 0x80), 0, 0xFF);
	bHSHPGainHighP 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_03, HSHP_GAIN_HIGH_POS) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_07, HSHP_GAIN_HIGHP_OFST) - 0x80), 0, 0xFF);
	bHSHPGainHighN 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_03, HSHP_GAIN_HIGH_NEG) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_07, HSHP_GAIN_HIGHN_OFST) - 0x80), 0, 0xFF);
	bHSHPCorLow 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_04, HSHP_COR_LOW) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_08, HSHP_CORZ_LOW_OFST) - 0x80), 0, 0xFF);
	bHSHPCorMid 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_04, HSHP_COR_MID) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_08, HSHP_CORZ_MID_OFST) - 0x80), 0, 0xFF);
	bHSHPCorHigh 	= CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_04, HSHP_COR_HIGH) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_08, HSHP_CORZ_HIGH_OFST) - 0x80), 0, 0xFF);
	bTDSHPEdgeGainH = CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_00, TDSHP_EDGE_GAIN_H) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_09, TDS_EDGE_GAIN_H_OFST) - 0x80), 0, 0xFF);
	bTDSHPEdgeGainD = CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_00, TDSHP_EDGE_GAIN_D) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_09, TDS_EDGE_GAIN_D_OFST) - 0x80), 0, 0xFF);
	bTDSHPEdgeGainV = CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_00, TDSHP_EDGE_GAIN_V) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_09, TDS_EDGE_GAIN_V_OFST) - 0x80), 0, 0xFF);
	bTDSHPDetailGainH = CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_01, TDSHP_DETAIL_GAIN_H) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_0A, TDS_DETAIL_GAIN_H_OFST) - 0x80), 0, 0xFF);
	bTDSHPDetailGainD = CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_01, TDSHP_DETAIL_GAIN_D) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_0A, TDS_DETAIL_GAIN_D_OFST) - 0x80), 0, 0xFF);
	bTDSHPDetailGainV = CLIP(((UINT8)IO32ReadFldAlign(SHARPNESS_ADPT_01, TDSHP_DETAIL_GAIN_V) + (UINT8)IO32ReadFldAlign(TDSHP_EXT_OFST_0A, TDS_DETAIL_GAIN_V_OFST) - 0x80), 0, 0xFF);
    
	if(bPreHSHPGainLowP != bHSHPGainLowP)
	{
		vIO32WriteFldAlign(HSHARP_00, bHSHPGainLowP , SHARP_LOW);			  
		bPreHSHPGainLowP = bHSHPGainLowP;
	}
	if(bPreHSHPGainLowN != bHSHPGainLowN)
	{
		vIO32WriteFldAlign(HSHARP_01, bHSHPGainLowN , SHARP_LOW_NEG);			  
		bPreHSHPGainLowN = bHSHPGainLowN;
	}
	if(bPreHSHPGainMidP != bHSHPGainMidP)
	{
		vIO32WriteFldAlign(HSHARP_00, bHSHPGainMidP , SHARP_MID);			  
		bPreHSHPGainMidP = bHSHPGainMidP;
	}
	if(bPreHSHPGainMidN != bHSHPGainMidN)
	{
		vIO32WriteFldAlign(HSHARP_01, bHSHPGainMidN , SHARP_MID_NEG);			  
		bPreHSHPGainMidN = bHSHPGainMidN;
	}
	if(bPreHSHPGainHighP != bHSHPGainHighP)
	{
		vIO32WriteFldAlign(HSHARP_00, bHSHPGainHighP , SHARP_HIGH);			  
		bPreHSHPGainHighP = bHSHPGainHighP;
	}
	if(bPreHSHPGainHighN != bHSHPGainHighN)
	{
		vIO32WriteFldAlign(HSHARP_01, bHSHPGainHighN , SHARP_HIGH_NEG);			  
		bPreHSHPGainHighN = bHSHPGainHighN;
	}
	if(bPreHSHPCorLow != bHSHPCorLow)
	{
		vIO32WriteFldAlign(HSHARP_02, bHSHPCorLow , SHARP_LOW_CORING);			  
		bPreHSHPCorLow = bHSHPCorLow;
	}
	if(bPreHSHPCorMid != bHSHPCorMid)
	{
		vIO32WriteFldAlign(HSHARP_02, bHSHPCorMid , SHARP_MID_CORING);			  
		bPreHSHPCorMid = bHSHPCorMid;
	}
	if(bPreHSHPCorHigh != bHSHPCorHigh)
	{
		vIO32WriteFldAlign(HSHARP_02, bHSHPCorHigh , SHARP_HIGH_CORING);			  
		bPreHSHPCorHigh = bHSHPCorHigh;
	}
	if(bPreTDSHPEdgeGainH != bTDSHPEdgeGainH)
	{
		vIO32WriteFldAlign(SHARP_30, bTDSHPEdgeGainH , SHP_EDGE_GAIN_LB);			  
		bPreTDSHPEdgeGainH = bTDSHPEdgeGainH;
	}
	if(bPreTDSHPEdgeGainD != bTDSHPEdgeGainD)
	{
		vIO32WriteFldAlign(SHARP_30, bTDSHPEdgeGainD , SHP_EDGE_GAIN_MB);			  
		bPreTDSHPEdgeGainD = bTDSHPEdgeGainD;
	}
	if(bPreTDSHPEdgeGainV != bTDSHPEdgeGainV)
	{
		vIO32WriteFldAlign(SHARP_31, bTDSHPEdgeGainV , SHP_EDGE_GAIN_HB);			  
		bPreTDSHPEdgeGainV = bTDSHPEdgeGainV;
	}
	if(bPreTDSHPDetailGainH != bTDSHPDetailGainH)
	{
		vIO32WriteFldAlign(SHARP_30, bTDSHPDetailGainH , SHP_DETAIL_GAIN_LB);			  
		bPreTDSHPDetailGainH = bTDSHPDetailGainH;
	}
	if(bPreTDSHPDetailGainD != bTDSHPDetailGainD)
	{
		vIO32WriteFldAlign(SHARP_30, bTDSHPDetailGainD , SHP_DETAIL_GAIN_MB);			  
		bPreTDSHPDetailGainD = bTDSHPDetailGainD;
	}
	if(bPreTDSHPDetailGainV != bTDSHPDetailGainV)
	{
		vIO32WriteFldAlign(SHARP_31, bTDSHPDetailGainV , SHP_DETAIL_GAIN_HB);			  
		bPreTDSHPDetailGainV = bTDSHPDetailGainV;
	}
...
void vDrvPreSharpnessSWRegInit(void)
{
    vIO32WriteFldAlign(SHARPNESS_ADPT_03 ,0x40, HSHP_GAIN_HIGH_POS);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,0x40, HSHP_GAIN_MID_POS);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,0x43, HSHP_GAIN_LOW_POS);
	vIO32WriteFldAlign(SHARPNESS_ADPT_03 ,0x43, HSHP_GAIN_HIGH_NEG);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,0x45, HSHP_GAIN_MID_NEG);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,0x45, HSHP_GAIN_LOW_NEG);
}

void vDrvPreSharpInit(void)
{
    vDrvLoadRegTbl(REGTBL_HSHARP);
    //init sharpness sw register
    vDrvPreSharpnessSWRegInit();
}

void vDrvPreSharpParam(UINT8 bPath)
{
//  Global Gain
	vIO32WriteFldAlign(SHARPNESS_ADPT_03 ,
	                  wReadQualityTable(QUALITY_HSHARP_HIGH), HSHP_GAIN_HIGH_POS);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,
	                  wReadQualityTable(QUALITY_HSHARP_MID), HSHP_GAIN_MID_POS);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,
	                  wReadQualityTable(QUALITY_HSHARP_LOW), HSHP_GAIN_LOW_POS);
	vIO32WriteFldAlign(SHARPNESS_ADPT_03 ,
	                  wReadQualityTable(QUALITY_HSHARP_HIGH_NEG), HSHP_GAIN_HIGH_NEG);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,
	                  wReadQualityTable(QUALITY_HSHARP_MID_NEG), HSHP_GAIN_MID_NEG);
	vIO32WriteFldAlign(SHARPNESS_ADPT_02 ,
	                  wReadQualityTable(QUALITY_HSHARP_LOW_NEG), HSHP_GAIN_LOW_NEG);

//  Global Coring
	vIO32WriteFldAlign(SHARPNESS_ADPT_04 ,
...
void vDrvPostSharpParam(void)
{
	...
	vIO32WriteFldAlign(SHARP_00, wReadQualityTable(QUALITY_TDSHARP_COMBINE_MODE), SHP_COMBINE_MODE); 
	vIO32WriteFldMulti(SHARPNESS_ADPT_00, 	P_Fld(wReadQualityTable(QUALITY_TDSHARP_EDGE_GAIN_H), TDSHP_EDGE_GAIN_H)| 
											P_Fld(wReadQualityTable(QUALITY_TDSHARP_EDGE_GAIN_D), TDSHP_EDGE_GAIN_D)| 
								 			P_Fld(wReadQualityTable(QUALITY_TDSHARP_EDGE_GAIN_V), TDSHP_EDGE_GAIN_V));  
	vIO32WriteFldMulti(SHARPNESS_ADPT_01, 	P_Fld(wReadQualityTable(QUALITY_TDSHARP_DETAIL_GAIN_H), TDSHP_DETAIL_GAIN_H)|     		 
											P_Fld(wReadQualityTable(QUALITY_TDSHARP_DETAIL_GAIN_D), TDSHP_DETAIL_GAIN_D)|     
											P_Fld(wReadQualityTable(QUALITY_TDSHARP_DETAIL_GAIN_V), TDSHP_DETAIL_GAIN_V));  
    ...
    // TDSHARPALL
    vIO32WriteFldAlign(SHARP_27, wReadQualityTable(QUALITY_TDSHARPALL_GAIN_POS), SHP_COMBINED_POS_GAIN); 
    vIO32WriteFldAlign(SHARP_27, wReadQualityTable(QUALITY_TDSHARPALL_GAIN_NEG), SHP_COMBINED_NEG_GAIN); 
    ...
    // Y Level 
    vIO32WriteFldAlign(SHARP_4B, wReadQualityTable(QUALITY_TDSHARPALL_YLEV_ALPHA), TDS_YLEV_ALPHA);
	vIO32WriteFldAlign(SHARP_4B, wReadQualityTable(QUALITY_TDSHARPALL_YLEV_SEL), TDS_YLEV_SEL);
...
void vDrvTDSharpOffsetInit(void)
{
	...
	vDrvHSharpGainOffset(0x80,0x80,0x80,0x80,0x80,0x80);
	vDrvHSharpCorzOffset(0x80,0x80,0x80);
	vDrvPostSharpEdgeGainOffset(0x80,0x80,0x80);
	vDrvPostSharpDetailGainOffset(0x80,0x80,0x80);
	vDrvPostSharpLTIGainOffset(0x80,0x80,0x80);
	vDrvPostSharpLTICorzOffset(0x80,0x80,0x80);
}
...
void vDrvHSharpGainOffset(UINT8 u1GainLowP, UINT8 u1GainLowN, UINT8 u1GainMidP, UINT8 u1GainMidN, UINT8 u1GainHighP, UINT8 u1GainHighN)
{
    vIO32WriteFldMulti(TDSHP_EXT_OFST_06, 
        P_Fld(u1GainLowP, HSHP_GAIN_LOWP_OFST) | 
        P_Fld(u1GainLowN, HSHP_GAIN_LOWN_OFST) | 
        P_Fld(u1GainMidP, HSHP_GAIN_MIDP_OFST) | 
        P_Fld(u1GainMidN, HSHP_GAIN_MIDN_OFST));  
	
	vIO32WriteFldMulti(TDSHP_EXT_OFST_07, 
        P_Fld(u1GainHighP, HSHP_GAIN_HIGHP_OFST) | 
        P_Fld(u1GainHighN, HSHP_GAIN_HIGHN_OFST));  
}

void vDrvHSharpCorzOffset(UINT8 u1HSHPCorzLow, UINT8 u1HSHPCorzMid, UINT8 u1HSHPCorzHigh)
{
    vIO32WriteFldMulti(TDSHP_EXT_OFST_08, 
        P_Fld(u1HSHPCorzLow, HSHP_CORZ_LOW_OFST) | 
        P_Fld(u1HSHPCorzMid, HSHP_CORZ_MID_OFST) | 
        P_Fld(u1HSHPCorzHigh, HSHP_CORZ_HIGH_OFST));  
}

void vDrvPostSharpEdgeGainOffset(UINT8 u1EdgeGainH, UINT8 u1EdgeGainV, UINT8 u1EdgeGainD)
{
    vIO32WriteFldMulti(TDSHP_EXT_OFST_09, 
        P_Fld(u1EdgeGainH, TDS_EDGE_GAIN_H_OFST) | 
        P_Fld(u1EdgeGainV, TDS_EDGE_GAIN_V_OFST) | 
        P_Fld(u1EdgeGainD, TDS_EDGE_GAIN_D_OFST));  
}

void vDrvPostSharpDetailGainOffset(UINT8 u1DetailGainH, UINT8 u1DetailGainV, UINT8 u1DetailGainD)
{
    vIO32WriteFldMulti(TDSHP_EXT_OFST_0A, 
        P_Fld(u1DetailGainH, TDS_DETAIL_GAIN_H_OFST) | 
        P_Fld(u1DetailGainV, TDS_DETAIL_GAIN_V_OFST) | 
        P_Fld(u1DetailGainD, TDS_DETAIL_GAIN_D_OFST));  
}

void vDrvPostSharpLTIGainOffset(UINT8 u1LTIGainH1, UINT8 u1LTIGainH2, UINT8 u1LTIGainV)
{
    vIO32WriteFldMulti(TDSHP_EXT_OFST_0B, 
        P_Fld(u1LTIGainH1, LTI_GAIN_H1_OFST) | 
        P_Fld(u1LTIGainH2, LTI_GAIN_H2_OFST) | 
        P_Fld(u1LTIGainV, LTI_GAIN_V_OFST));  
}

void vDrvPostSharpLTICorzOffset(UINT8 u1LTICorzH1, UINT8 u1LTICorzH2, UINT8 u1LTICorzV)
{
    vIO32WriteFldMulti(TDSHP_EXT_OFST_0C, 
        P_Fld(u1LTICorzH1, LTI_CORZ_H1_OFST) | 
        P_Fld(u1LTICorzH2, LTI_CORZ_H2_OFST) | 
        P_Fld(u1LTICorzV, LTI_CORZ_V_OFST));  
}
...
void vDrvSharpProc(void)
{
	...
	vDrvPBCMNRProc();
	//Sharpness offset dynamic control 
	vDrvSharpDynCtrl();
}


void vDrvDDPostSharpInit(void)
{
#ifdef CC_SUPPORT_UHD
    if (PQ_DD_MACRO)
    {
        vIO32WriteFldAlign(0xF0027350, 1, TDPROC_BYPASS_ALL);
    }

    REGTBL_TDSHARP[0].u4Reg = SHARP_00;
    REGTBL_TDSHARP[1].u4Reg = SHARP_31;
    REGTBL_TDSHARP[2].u4Reg = SHARP_32;
    REGTBL_TDSHARP[3].u4Reg = SHARP_36;
    REGTBL_TDSHARP[4].u4Reg = SHARP_42;
    REGTBL_TDSHARP[5].u4Reg = SHARP_43;
    REGTBL_TDSHARP[6].u4Reg = SHARP_44;
    REGTBL_TDSHARP[7].u4Reg = SHARP_45;
    REGTBL_TDSHARP[8].u4Reg = SHARP_46;
    REGTBL_TDSHARP[9].u4Reg = SHARP_4C;
    REGTBL_TDSHARP[10].u4Reg = SHARP_4D;
    REGTBL_TDSHARP[11].u4Reg = SHARP_4E;
    REGTBL_TDSHARP[12].u4Reg = SHARP_5C;
    REGTBL_TDSHARP[13].u4Reg = SHARP_5D;

    REGTBL_PBC[0].u4Reg = CDS_00;
    REGTBL_PBC[1].u4Reg = CDS_01;
    REGTBL_PBC[2].u4Reg = CDS_02;
    REGTBL_PBC[3].u4Reg = CDS_03;
    REGTBL_PBC[4].u4Reg = CDS_2A;
    REGTBL_PBC[5].u4Reg = CDS_2B;
    REGTBL_PBC[6].u4Reg = CDS_2C;
    REGTBL_PBC[7].u4Reg = CDS_2D;
    REGTBL_PBC[8].u4Reg = CDS_54;
    REGTBL_PBC[9].u4Reg = CDS_55;
    REGTBL_PBC[10].u4Reg = CDS_56;
    REGTBL_PBC[11].u4Reg = CDS_57;
#endif

    TDS_CURVE_REGADDR[0] = SHARP_03; //H1
    TDS_CURVE_REGADDR[1] = SHARP_07; //H2
    TDS_CURVE_REGADDR[2] = SHARP_0B; //H3
    TDS_CURVE_REGADDR[3] = SHARP_0F; //V1
    TDS_CURVE_REGADDR[4] = SHARP_13; //V2
    TDS_CURVE_REGADDR[5] = SHARP_17; //V3
    TDS_CURVE_REGADDR[6] = SHARP_1B; //D1
    TDS_CURVE_REGADDR[7] = SHARP_1F; //D2
    TDS_CURVE_REGADDR[8] = SHARP_23; //D3
    TDS_CURVE_REGADDR[9] = SHARP_50; //LTI_HL
    TDS_CURVE_REGADDR[10] = SHARP_54; //LTI_HH
    TDS_CURVE_REGADDR[11] = SHARP_58; //LTI_V
    TDS_CURVE_REGADDR[12] = CDS_06;   //CDS1_H1
    TDS_CURVE_REGADDR[13] = CDS_0A;   //CDS1_H2
    TDS_CURVE_REGADDR[14] = CDS_0E;   //CDS1_H3
    TDS_CURVE_REGADDR[15] = CDS_12;   //CDS1_V1
    TDS_CURVE_REGADDR[16] = CDS_16;   //CDS1_V2
    TDS_CURVE_REGADDR[17] = CDS_1A;   //CDS1_V3
    TDS_CURVE_REGADDR[18] = CDS_1E;   //CDS1_D1
    TDS_CURVE_REGADDR[19] = CDS_22;   //CDS1_D2
    TDS_CURVE_REGADDR[20] = CDS_26;   //CDS1_D3
    TDS_CURVE_REGADDR[21] = CDS_30;   //CDS2_H1
    TDS_CURVE_REGADDR[22] = CDS_34;   //CDS2_H2
    TDS_CURVE_REGADDR[23] = CDS_38;   //CDS2_H3
    TDS_CURVE_REGADDR[24] = CDS_3C;   //CDS2_V1
    TDS_CURVE_REGADDR[25] = CDS_40;   //CDS2_V2
    TDS_CURVE_REGADDR[26] = CDS_44;   //CDS2_V3
    TDS_CURVE_REGADDR[27] = CDS_48;   //CDS2_D1
    TDS_CURVE_REGADDR[28] = CDS_4C;   //CDS2_D2
    TDS_CURVE_REGADDR[29] = CDS_50;   //CDS2_D3
    TDS_CURVE_REGADDR[30] = CDS_59;   //CDS3_H1
    TDS_CURVE_REGADDR[31] = CDS_5D;   //CDS3_H2
    TDS_CURVE_REGADDR[32] = CDS_61;   //CDS3_H3
    TDS_CURVE_REGADDR[33] = CDS_65;   //CDS3_V1
    TDS_CURVE_REGADDR[34] = CDS_69;   //CDS3_V2
    TDS_CURVE_REGADDR[35] = CDS_6D;   //CDS3_V3
    TDS_CURVE_REGADDR[36] = CDS_71;   //CDS3_D1
    TDS_CURVE_REGADDR[37] = CDS_75;   //CDS3_D2
    TDS_CURVE_REGADDR[38] = CDS_79;   //CDS3_D3
}
...
{	
	...
	DRVCUST_SharpnessEnhanceCFGInit();
