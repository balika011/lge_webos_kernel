...
static inline void _vDrvDISetFrameNum(UINT8 u1VdpId)
{
    ...
    {    
        ...
        case E(I, NR_OFF ,DI_Y1C1  ,FRC_MIB):	
            ...
            {//add one more buffer to avoid tear
            	u1YFieldNum = 2;
            	u1CFieldNum = 2;
            }
			break;
        ...
        case E(P,NR_OFF   ,DI_OFF , FRC_DUAL_HYBRID):
            u1YFieldNum = FBM_POOL_HYBRID_DRAM_FRAME_NUM; 
            u1CFieldNum = FBM_POOL_HYBRID_DRAM_FRAME_NUM;
            break; 
        default:
            u1YFieldNum = 0;
            u1CFieldNum = 0;
    ...
    if ( IS_FLIP_MIRROR(u1VdpId) ||  _arMDDiPrm[u1VdpId].u1HybridSCFlipMorror)
    {
        u1YFieldNum++;
        u1CFieldNum++;
    }
    ...
    rMIBFbmPool[u1VdpId].u1YNum  = u1YFieldNum;
    rMIBFbmPool[u1VdpId].u1CNum = u1CFieldNum;
        ...
        UINT32 u4StartAddr, UINT32 u4EndAddr)
{
    ...
    rMIBFbmPool[u1VdpId].u4LoAddr  = u4StartAddr>>11;
    rMIBFbmPool[u1VdpId].u4HiAddr  = u4FlgEndAddr>>11;
    rMIBFbmPool[u1VdpId].u4YMsbAddr  = u4StartAddr >> 11;
    rMIBFbmPool[u1VdpId].u4YLsbAddr   = u4YAddrLsb    >> 11;
    rMIBFbmPool[u1VdpId].u4UMsbAddr  = u4UAddrMsb   >> 11;
    rMIBFbmPool[u1VdpId].u4ULsbAddr   = u4UAddrLsb    >> 11;
    rMIBFbmPool[u1VdpId].u4VMsbAddr  = u4VAddrMsb   >> 11;
    rMIBFbmPool[u1VdpId].u4VLsbAddr   = u4VAddrLsb    >> 11;   
...
//    u4Width = MIN(2048,u4Width);
    ...
    u1Pitch = (UINT8) MIN(u4Width /MDDI_LINE_LENGHT_UNIT,0xF);
    if (IS_IC_5861())
    {
        u1Pitch--;
    }
    MDDI_WRITE_FLD(u1VdpId, MCVP_DRAM_09, u1Pitch, DA_DRAM_LINE_LENGTH);     
...
void vDrvDIWritePixelValue(UINT8 bPath, UINT16 x, UINT16 y, DI_BUFFER_DATA* value)
{
	...
	{			
		...
		{
		        u1Clr = ~(0x3 << Lsbshf);
		    TmpLineNum = (lineNumLsb) % u4VPxlPerPgMsb;
