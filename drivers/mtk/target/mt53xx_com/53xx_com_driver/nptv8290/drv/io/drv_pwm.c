                ...
                UINT32 u4DutyAll)
{
    ...
    u4Frequency = u4BusClock / ((u4TimeBase + 1) * (u4DutyAll + 1));
    u4DutyCycle = (u4DutyON * 100) / (u4DutyAll + 1);
    Printf("MTK_PWM Set(%lu): PWMP [0x%lx],PWMH [0x%lx], PWMRSN [0x%lx], Frequency[%luHz]m DutyCycle[%lu%%]\n",
        u4Src, u4TimeBase, u4DutyON, u4DutyAll, u4Frequency, u4DutyCycle);
...
void vDrvInitSCANPWM(UINT8 u1Src)
{
	...
	vDrvSetScanPWMStartHigh(u1Src,SV_ON);
	...
	if(IS_IC_5861())  // need to review after ECO
	{
		
		vIO32WriteFldAlign(PWM_SCAN_32, 0xF0000, REG_PWM_SCAN7_LOW);
		vIO32WriteFldAlign(PWM_SCAN_39, 0xF0000, REG_PWM_SCAN8_LOW);

		vIO32WriteFldAlign(PWM_SCAN_2F, 0, REG_PWM_SCAN7_START_PHASE);
		vIO32WriteFldAlign(PWM_SCAN_36, 0, REG_PWM_SCAN8_START_PHASE);
 ...
 * @return void
 */
#define D_DRV_SCAN_PWM_DUTY_MAX 1000
UINT32 _u4ScanPWMTestPer=D_DRV_SCAN_PWM_DUTY_MAX;
void vDrvSetScanPWM(UINT8 u1Src, UINT32 u4Start, UINT32 u4High, UINT32 u4Low)
{
    UINT16 u2HTotal, u2VTotal;
    UINT32 u4Val;

	u4Val = u4Start + u4High + u4Low;
	
	if (u4Val > _u4ScanPWMTestPer)
	{
		return;
	}
    ...
    u4Val = u2HTotal * u2VTotal / _u4ScanPWMTestPer;
...
/** Brief
 * This driver control the scanning PWM high frame mode enable
 * @param u1OnOff
 * @return void
 */
void vDrvSetScanPWMHFOnOff(UINT32 u4Src, UINT8 u1OnOff)
{
	switch(u4Src)    
	{        
		case SrcPWM1:            
			vIO32WriteFldAlign(PWM_SCAN_00, u1OnOff, REG_PWM_SCAN1_HIGH_FRAME_MODE);            
			break;        
		case SrcPWM2:            
			vIO32WriteFldAlign(PWM_SCAN_03, u1OnOff, REG_PWM_SCAN2_HIGH_FRAME_MODE);            
			break;        
		case SrcPWM3:            
			vIO32WriteFldAlign(PWM_SCAN_07, u1OnOff, REG_PWM_SCAN3_HIGH_FRAME_MODE);           
			break;        
		case SrcPWM4:            
			vIO32WriteFldAlign(PWM_SCAN_0A, u1OnOff, REG_PWM_SCAN4_HIGH_FRAME_MODE);            
			break;        
		case SrcPWM5:            
			vIO32WriteFldAlign(PWM_SCAN_0D, u1OnOff, REG_PWM_SCAN5_HIGH_FRAME_MODE);            
			break;		
		case SrcPWM6:			
			vIO32WriteFldAlign(PWM_SCAN_23, u1OnOff, REG_PWM_SCAN6_HIGH_FRAME_MODE);            
			break;
		case SrcPWM7:			
			vIO32WriteFldAlign(PWM_SCAN_30, u1OnOff, REG_PWM_SCAN7_HIGH_FRAME_MODE);            
			break;
		case SrcPWM8:			
			vIO32WriteFldAlign(PWM_SCAN_37, u1OnOff, REG_PWM_SCAN8_HIGH_FRAME_MODE);            
			break;	
		default:		    
			ASSERT(u4Src <= SrcPWM8);    
	}
...
/** Brief
 * This driver control the scanning PWM high frame mode select
 * @param u1Mode 1:One Frame ON/One Frame OFF 2:One Frame OFF/One Frame ON
 * @return void
 */
void vDrvSetScanPWMHFMode(UINT32 u4Src, UINT8 u1Mode)
{
	switch(u4Src)    
	{        
		case SrcPWM1:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_00, 0x0A, REG_PWM_SCAN1_HIGH_FRAME_MODE);
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_00, 0x14, REG_PWM_SCAN1_HIGH_FRAME_MODE);
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_00, 0x1F, REG_PWM_SCAN1_HIGH_FRAME_MODE);
			}
			break;        
		case SrcPWM2:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_03, 0x0A, REG_PWM_SCAN2_HIGH_FRAME_MODE);
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_03, 0x14, REG_PWM_SCAN2_HIGH_FRAME_MODE);
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_03, 0x1F, REG_PWM_SCAN2_HIGH_FRAME_MODE);
			}
			break;        
		case SrcPWM3:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_07, 0x0A, REG_PWM_SCAN3_HIGH_FRAME_MODE);
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_07, 0x14, REG_PWM_SCAN3_HIGH_FRAME_MODE);
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_07, 0x1F, REG_PWM_SCAN3_HIGH_FRAME_MODE);
			}
			break;        
		case SrcPWM4:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_0A, 0x0A, REG_PWM_SCAN4_HIGH_FRAME_MODE);
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_0A, 0x14, REG_PWM_SCAN4_HIGH_FRAME_MODE);
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_0A, 0x14, REG_PWM_SCAN4_HIGH_FRAME_MODE);
		...
		case SrcPWM5:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_0D, 0x0A, REG_PWM_SCAN5_HIGH_FRAME_MODE);
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_0D, 0x14, REG_PWM_SCAN5_HIGH_FRAME_MODE);
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_0D, 0x1F, REG_PWM_SCAN5_HIGH_FRAME_MODE);
			}
			break;		
		case SrcPWM6:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_23, 0x0A, REG_PWM_SCAN6_HIGH_FRAME_MODE); 
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_23, 0x14, REG_PWM_SCAN6_HIGH_FRAME_MODE); 
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_23, 0x1F, REG_PWM_SCAN6_HIGH_FRAME_MODE);
			}
			break;
		case SrcPWM7:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_30, 0x0A, REG_PWM_SCAN7_HIGH_FRAME_MODE); 
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_30, 0x14, REG_PWM_SCAN7_HIGH_FRAME_MODE); 
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_30, 0x1F, REG_PWM_SCAN7_HIGH_FRAME_MODE);
			}
			break;
		case SrcPWM8:
			if(u1Mode == 1)
			{
				vIO32WriteFldAlign(PWM_SCAN_37, 0x0A, REG_PWM_SCAN8_HIGH_FRAME_MODE); 
			}
			else if(u1Mode == 2)
			{
				vIO32WriteFldAlign(PWM_SCAN_37, 0x14, REG_PWM_SCAN8_HIGH_FRAME_MODE); 
			}
			else
			{
				vIO32WriteFldAlign(PWM_SCAN_37, 0x1F, REG_PWM_SCAN8_HIGH_FRAME_MODE);
			}
			break;	
		default:		    
			ASSERT(u4Src <= SrcPWM8);    
...
/** Brief
 * This driver control the scanning PWM start with high or low
 * @param 0: scan PWM channel
 * @param 1: On/Off. On:start with high. Off:start with low
 * @return void
 */
void vDrvSetScanPWMStartHigh(UINT32 u4Src, UINT8 u1OnOff)
{
	UINT32 u4Val=0;

	if (u1OnOff)  // start with high
	{
		u4Val = 0;
	}
	else
    ...
    {
        case SrcPWM1:
            vIO32WriteFldAlign(PWM_SCAN_06, u4Val, REG_PWM_SCAN1_START_HIGH);
            break;

        case SrcPWM2:
            vIO32WriteFldAlign(PWM_SCAN_06, u4Val, REG_PWM_SCAN2_START_HIGH);
            break;

        case SrcPWM3:
            vIO32WriteFldAlign(PWM_SCAN_10, u4Val, REG_PWM_SCAN3_START_HIGH);
            break;

        case SrcPWM4:
            vIO32WriteFldAlign(PWM_SCAN_10, u4Val, REG_PWM_SCAN4_START_HIGH);
            break;

        case SrcPWM5:
            vIO32WriteFldAlign(PWM_SCAN_11, u4Val, REG_PWM_SCAN5_START_HIGH);
            break;

		case SrcPWM6:
			vIO32WriteFldAlign(PWM_SCAN_11, u4Val, REG_PWM_SCAN6_START_HIGH);
            break;
			
		case SrcPWM7:
			vIO32WriteFldAlign(PWM_SCAN_3D, u4Val, REG_PWM_SCAN7_START_HIGH);
			break;
			
		case SrcPWM8:
			vIO32WriteFldAlign(PWM_SCAN_3D, u4Val, REG_PWM_SCAN8_START_HIGH);
			break;
...
{
	if (((PWMSCAN_SETDATA[1].u1CrossZone == 2)&&(PWMSCAN_SETDATA[1].u1Initialed == 1))
		||((PWMSCAN_SETDATA[2].u1CrossZone == 2)&&(PWMSCAN_SETDATA[2].u1Initialed == 1)))
	{
		u1ScanPwmCrossFlg = 2;
	}	
	else if (((PWMSCAN_SETDATA[1].u1CrossZone == 1)&&(PWMSCAN_SETDATA[1].u1Initialed == 1))
		||((PWMSCAN_SETDATA[2].u1CrossZone ==1)&&(PWMSCAN_SETDATA[2].u1Initialed ==1)))
	{
		u1ScanPwmCrossFlg = 1;
	}	
	else
...
{
	if (((PWMSCAN_SETDATA[1].u1CrossZone == 0)&&(PWMSCAN_SETDATA[1].u1Initialed == 1))
		||((PWMSCAN_SETDATA[2].u1CrossZone == 0)&&(PWMSCAN_SETDATA[2].u1Initialed == 1)))
	{
		u1ScanPwmCrossFlg = 0;
	}	
	else
...
UINT8 u1DrvGetScanPWMStruct_Ready(void)
{
	...
	#if SCANPWM_DEBUGLOG
	LOG(_bScanPWMDbgLogLev+3, "[ScanPWM]u1Ready = %d\n", u1Ready);
	if(!u1Ready)
	{
...
{
	UINT16 u2Range;
	if (_u4ScanPWMTestPer==255)
		u2Range = 2;
	else  // _u4ScanPWMTestPer=1000 or 1024
		u2Range = 10;

	if (u2Value >= u2Range)
		u2PWMCrossZoneup = u2Value-u2Range;
	else
		u2PWMCrossZoneup = 81;
}
...
static void vDrvSetScanPWMStruct(UINT8 u1Src, UINT32 u4Start, UINT32 u4High, UINT32 u4Low)
{
   ...
   if (u1Src <= PWMSCAN_MAX_CH)
   {
   		...
   		if (((u4High+u4Low)<510) || ((u4High+u4Low)<137))
   		{
     		u1DoubleFrame =1;
   		}  
   		...
   		if(u1DoubleFrame)
  		{
			...
			if(u4LineTotalCur>_u4ScanPWMTestPer)
			{
				#if SCANPWM_DEBUGLOG
				LOG(_bScanPWMDbgLogLev+3, "[ScanPWM]DoubleFrame #1\n");
				#endif
				u4LineTotalCur = (u4LineTotalCur - _u4ScanPWMTestPer);
			}
			else
			{
				#if SCANPWM_DEBUGLOG
				LOG(_bScanPWMDbgLogLev+3, "[ScanPWM]DoubleFrame #2\n");
				#endif
   		...
   		else
   		{
			...
			{
				#if SCANPWM_DEBUGLOG
				LOG(_bScanPWMDbgLogLev+3, "[ScanPWM]SingleFrame #0\n");
				#endif
				u4LineTotalCur = (u4LineTotalCur - _u4ScanPWMTestPer);
			}			
   		}   
		...
		u2CrossLine = wDrvGetOutputVTotal()-wDrvGetOutputVActive()-wDrvGetVsyncFp(); // [Apply point1 position] VDE Rising
   		...
   		#endif
		u2CrossLine = u2CrossLine * _u4ScanPWMTestPer / wDrvGetOutputVTotal();
		#if SCANPWM_DEBUGLOG
   		LOG(_bScanPWMDbgLogLev, "[ScanPWM]u2CrossLine=%d(%)\n",u2CrossLine);
   		#endif
		if (_u4ScanPWMTestPer==255)
			u2CrossLine_top = u2CrossLine + 12;
		else  // _u4ScanPWMTestPer=1000 or 1024
		...
		if((u4LineTotalCur >= 0) && (u4LineTotalCur <= u2CrossLine_top))
		{
 			...
 			PWMSCAN_SETDATA[u1Src].u1CrossZone = 1;  // OR 2
			#if SCANPWM_DEBUGLOG
...
void vDrvSetScanPWMSW_StepControl(void)
{
	...
	for (u1Ch = SrcPWM1; u1Ch <= PWMSCAN_MAX_CH; u1Ch++)
	{
		...
		{
			#if SCANPWM_DEBUGLOG
			LOG(_bScanPWMDbgLogLev+3, "[ScanPWM]High_step=%d, Low_step=%d \n",PWMSCAN_SETDATA[u1Ch].u4High,PWMSCAN_SETDATA[u1Ch].u4Low);
			#endif
			...
			#if SCANPWM_DEBUGLOG
			LOG(_bScanPWMDbgLogLev+3, "[ScanPWM]u4HighChg=%d, u4LowChg=%d \n",u4HighChg,u4LowChg);
			#endif
