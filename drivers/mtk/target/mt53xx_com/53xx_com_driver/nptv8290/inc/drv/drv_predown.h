 ...
 * $Revision: #1 $
*************************************************************************/
#ifndef _DRV_PREDOWN_H_
#define _DRV_PREDOWN_H_
#include "general.h"
#include "video_def.h"
#include "vdp_if.h"
#include "vdo_rm.h"

#define PREDOWN_4K2K_TO_FHD_LOST_PIXEL_WA

typedef struct
{
    UINT8 bPDSModeChanged;
    VRM_INFO_T pdsIn;
    VRM_INFO_T pdsOut;
    UINT8  fgPredsFirEn;
    UINT8  u1PredsFirDlySel;
    
    UINT32 u4PdsFirHDSFactorIntg;
    UINT32 u4PdsFirHDSFactorFrac;
    UINT32 u4PdsFirHDeltaPhaseIntg;
    UINT32 u4PdsFirHDeltaPhaseFrac;
    UINT8  u4PdsFirHDeltaPhaseSign;
    UINT8  fgPDSInterlace;
    UINT32 u4ForcedHPDS;
    UINT32 u4ForcedVPDS;
    UINT32 u4HPDSFactor;
    UINT32 u4VPDSFactor;
    UINT32 u4VPDSPhase;
    UINT32 u4VPDSPhaseBottom;
    UINT32 u4VPDSPhaseBottomEn;
...
#define PDS_WRITE_FLD(reg,value,fld) vIO32WriteFldAlign(reg, value, fld)
#define PDS_READ_FLD(reg,fld) IO32ReadFldAlign(reg,fld)
#define getPDSInfo(bPath) (&_pdsInfo[bPath])

EXTERN PREDOWN_INFO_T _pdsInfo[VDP_MAX_NS];
