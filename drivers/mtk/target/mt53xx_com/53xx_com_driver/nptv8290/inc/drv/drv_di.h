...
#define IS_SCL_FRC(u1Id)                 ((MIB_FRC_MODE(u1Id) == E_MIB_FRC_SCL)&&(E_MIB_DMODE_UNKNOWN!=rMIBState[u1Id].eBandwidthDataMode))

#define IS_HYBRID(u1Id)                   ((MIB_FRC_MODE(u1Id) == E_MIB_FRC_HYBRID)&&(E_MIB_DMODE_UNKNOWN!=rMIBState[u1Id].eBandwidthDataMode))

#define IS_DUAL_HYBRID(u1Id)       ((MIB_FRC_MODE(u1Id) == E_MIB_FRC_DUAL_HYBRID)&&(E_MIB_DMODE_UNKNOWN!=rMIBState[u1Id].eBandwidthDataMode))
...
typedef enum
{
    MIB_CLK_TYPEXTAL_D1_CK        =0,
    MIB_CLK_TYPESYSPLL_D2_CK     =1,
    MIB_CLK_TYPEENETPLL_D2_CK   =2,
    MIB_CLK_TYPESYSPLL_D1_CK     =3,
    MIB_CLK_TYPEENETPLL_D1_CK   =4,
    MIB_CLK_RSV                               =5,
} HW_MIB_OUTPUT_CLK_TYPE;

typedef enum
{
    MIB_CLK_MIB_OCLK_CK           =0,
    MIB_CLK_MIB_OCLK_D2_CK     =1,
} HW_MIB_CLK_TYPE;
...
typedef enum
{
    E_MIB_FRC_SCL                   =0x0,
    E_MIB_FRC_MIB                   =0x1,
    E_MIB_FRC_HYBRID             =0x2,
    E_MIB_FRC_DUAL_HYBRID   =0x3,
    E_MIB_FRC_UNKNOWN         =0x4,
} E_MIB_FRC_DMODE;
...
typedef enum
{
    ...
    //HYBRID DRAM MODE
    E_DCL(P,NR_OFF   ,DI_OFF ,FRC_HYBRID),
    E_DCL(P,NR_Y      ,DI_OFF ,FRC_HYBRID),
    E_DCL(P,NR_YC    ,DI_OFF ,FRC_HYBRID),
    //DUAL HYBRID DRAM MODE
    E_DCL(P,NR_OFF   ,DI_OFF ,FRC_DUAL_HYBRID),
...
#define MIB_FBM_ELE_LIST \
    ...
    MIB_ELE_CMD(UINT8,u1YNum)\
    MIB_ELE_CMD(UINT8,u1CNum)\
    MIB_ELE_CMD(UINT32,u4YMsbAddr)\
    MIB_ELE_CMD(UINT32,u4YLsbAddr)\
    MIB_ELE_CMD(UINT32,u4UMsbAddr)\
    MIB_ELE_CMD(UINT32,u4ULsbAddr)\
    MIB_ELE_CMD(UINT32,u4VMsbAddr)\
    MIB_ELE_CMD(UINT32,u4VLsbAddr)\
    MIB_ELE_CMD(UINT32,u4LoAddr)\
    MIB_ELE_CMD(UINT32,u4HiAddr)
...
#define MIB_SYS_STATIC_ELE_LIST \
    ...
    MIB_ELE_CMD(UINT8,u1HybridSCFlipMorror)\
    ...
    MIB_ELE_CMD(UINT32,FrontRegenX)\
    MIB_ELE_CMD(UINT32,FrontRegenY)\
    MIB_ELE_CMD(UINT32,FrontRegenW)\
    MIB_ELE_CMD(UINT32,FrontRegenH)\
...
void vDrvDIGetFreqStatus(UINT8* u1FreqHpfV, UINT8* u1FreqHpfH, UINT8* u1FreqBpfH);
...
UINT8 u1DrvDIIsSupportedSignal(UINT8 u1VdpId, E_DI_CLK_TYPE eClkType);
