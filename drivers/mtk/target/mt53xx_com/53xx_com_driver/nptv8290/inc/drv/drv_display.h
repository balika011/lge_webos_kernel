...
enum eVB1_ControlBit_Edge{
	eVB1_ControlBit_FALLING = 0,
	eVB1_ControlBit_RISING
};

typedef enum {
    E_TIMEGEN_PATH_PQ = 0,
    E_TIMEGEN_PATH_BYPASS_HDMI,
    E_TIMEGEN_PATH_BYPASS_MM,
    E_TIMEGEN_PATH_GFX,
	E_TIMEGEN_PATH_MAX,
}E_TIMEGEN_PATH;
...
typedef enum {
    E_TIMING_TRIGGER_STATE_IDLE     = 0,
    E_TIMING_TRIGGER_STATE_CTRL_BIT = 1,
    E_TIMING_TRIGGER_STATE_TIMING   = 2,
    E_TIMING_TRIGGER_STATE_WAITING  = 3,
    E_TIMING_TRIGGER_STATE_VALID    = 4,
}E_TIMING_TRIGGER_STATE;

typedef enum {
	E_TIMEGEN_TRIGGER_STATE_WAITING =0,
    E_TIMEGEN_TRIGGER_VB1_PATH_WAITING =1,
    E_TIMEGEN_TRIGGER_VB1_PATH = 2,
    E_TIMEGEN_TRIGGER_DECODE_TIMEGEN_WAINTING  = 3,
    E_TIMEGEN_TRIGGER_DECODE_TIMEGEN   = 4,
}E_TIMEGEN_TRIGGER_STATE;

typedef struct
{
    UINT8  u1TriggerState;
    UINT8  u1DCLKType;
    UINT8  u1FrameRate;
    UINT32 u4HTotal;
    UINT32 u4HActive;
...
typedef void (*vDisplayTimingCB)(UINT16 u2Arg);

typedef enum {
    E_DISPLAY_TIMING_CB_H_ACTIVE = 0,
    E_DISPLAY_TIMING_CB_H_TOTAL  = 1,
    E_DISPLAY_TIMING_CB_H_FP     = 2,
    E_DISPLAY_TIMING_CB_H_BP     = 3,
    E_DISPLAY_TIMING_CB_V_ACTIVE = 4,
    E_DISPLAY_TIMING_CB_V_TOTAL  = 5,
    E_DISPLAY_TIMING_CB_V_FP     = 6,
    E_DISPLAY_TIMING_CB_V_BP     = 7,
    E_DISPLAY_TIMING_NUM_OF_CB
}E_DISPLAY_TIMING_CB;


typedef enum {
    E_CONTROL_BIT_CB_3DST = 0,  // 3D Structure
    E_CONTROL_BIT_CB_HSZ,       // H Size
    E_CONTROL_BIT_CB_VSZ,       // V Size
    E_CONTROL_BIT_CB_CRHP,      // Crop H Position
    E_CONTROL_BIT_CB_CRVP,      // Crop V Position
    E_CONTROL_BIT_CB_CRHSZ,     // Crop H Size
    E_CONTROL_BIT_CB_CRVSZ,     // Crop V Size
    E_CONTROL_BIT_CB_AT,        // Aspect Type
    E_CONTROL_BIT_CB_AR,        // Aspect Ratio
    E_CONTROL_BIT_CB_SHSZ,      // SAR H Size
    E_CONTROL_BIT_CB_SVSZ,      // SAR V Size
    E_CONTROL_BIT_CB_CT,        // Codec Type
    E_CONTROL_BIT_CB_RF,        // Double Repeat Flag
    E_CONTROL_BIT_CB_VP,        // Video Path
    E_CONTROL_BIT_CB_CF,        // Color Format
    E_CONTROL_BIT_CB_RA,        // Range
    E_CONTROL_BIT_CB_CBV,       // Contro Bit Valid Flag
    E_CONTROL_BIT_NUM_OF_CB
}E_CONTROL_BIT_VB1_CB;

typedef enum {
    E_CONTROL_BIT_CB_HTOTAL = 0,   // HTOTAL
    E_CONTROL_BIT_CB_VTOTAL,       // V TOTAL
    E_CONTROL_BIT_CB_HSPos,        // H START POS
    E_CONTROL_BIT_CB_VSPos,        // H START POS
    E_CONTROL_BIT_CB_GF,           // Graphic format
    E_CONTROL_BIT_CB_VF,           // valid flag
    E_CONTROL_BIT_NUM_OF_GFX_CB
}E_CONTROL_BIT_GFX_CB;


typedef struct
{
    UINT8  u13DStructure;  //2D:0 SBS:01 TAB:10 FS:11
    UINT32 u4PicHSize;
    UINT32 u4PicVSize;
    UINT32 u4CropHPosition;
    UINT32 u4CropVPosition;
    UINT32 u4CropHSize;
    UINT32 u4CropVSize;
    UINT8  u1AspectType;    //SAR:0  DAR:1
    UINT8  u1AspectRatio;   //0:invalid 1:1.000 2:0.6735.....
    UINT32 u4SarHSize;      //SAR Hsize (valid only when aspect type == 0(SAR) && aspect ratio ==0(invalid) )
    UINT32 u4SarVSize;      //SAR Hsize (valid only when aspect type == 0(SAR) && aspect ratio ==0(invalid) )
    UINT8  u1CodeType;      //0:inknow 1:MPEG 2:MPEG2/AVS 3:MPEG4/Xvid 4:AVC/MVC 5:VC-1 6:RealVideo/VP8/MotionJPEG 7:HEVC 8:VP9
    UINT8  u1DoubleRF;      //1:24hz to 48 0:other
    UINT8  u1VideoPath;     // 0:PQ Path 1:Bypass Path
    UINT8  u1ColorFormat;   // 0:YUV 709 1:YUV 601 2:YUV2020 3:RGB
    UINT8  u1Range;         // 0:limit Range 1:full Range
    UINT8  u1CtrlBitValid;  // 0:invalid 1:valid
    
}CONTROL_BIT_VB1_T;

typedef struct
    {
    UINT32 u4CBHTOTAL;       // HTOTAL
    UINT32 u4CBVTOTAL;       // V TOTAL
    UINT32 u4CBHSPos;        // H START POS
    UINT32 u4CBVSPos;        // H START POS
    UINT32 u4CBGF;           // Graphic format
    UINT32 u4CBVF;           // valid flag
}CONTROL_BIT_GFX_T;

extern BOOL fgPhotoRasterModeIn4k;//4k photo in video path
extern void vDrvDisplayInit(void) ;
extern void vDrvRegisterDisplayCB(void);
extern void vDrvSetPanelTiming(void);
extern void vDrvApplyPanelTiming(void);
extern void vDrvVB1VideoMuteTiming(void);
...
extern void vScanGoodDclkRegion(UINT16 u2StartHsyncFreg, UINT16 u2EndHsyncFreg, UINT16 u2step);
...
extern TIMING_INFO_T GFXTimingInfo;

#endif
extern void vDrvSetVB1ControlBit_Valid(BOOL fgValid);
extern void vDrvSetVB1ControlBit(void);
extern void vDrvTconTopVdoSel(UINT8 u1Mode);
extern UINT8 vDrvGetTconTopVdoSel(void);
...
#define SUPPORT_FILM22_48TO120_2D   (1U << 4)
#define SUPPORT_3D_OUTPUT_60   		(1U << 5)
...
#define INTR_AMBILIGHT_ISR      (1U << 7)
