...
#define FLD_POCLK_SEL_MIB_OCLK_DIV2 4
...
#define SUB_HMSK_DLY	        0xD1
#define SUB_VMSK_DLY	        0x4
#define SUB_HMSK_DLY_Cobra      0xDB
#else    //for 5880 and 5860
#define SUB_HMSK_DLY_Viper     0x1D5
#define SUB_HMSK_DLY_Python    0x1D7
#define SUB_HMSK_DLY           0x1D7
#define SUB_VMSK_DLY           0x3
#endif


//3D frame trk mode
#define FRM_TRK_3D_EN           (0x1<<0)
#define FRM_TRK_3D_LOCK_AT_R    (0x1<<1)
#define FRM_TRK_3D_LOCK_AT_L    (0x1<<2)
#define FRM_TRK_MASK_IN_EN      (0x1<<3)
#define FRM_TRK_MASK_IN_RIGHT   (0x1<<4)
#define FRM_TRK_MASK_IN_LEFT    (0x1<<5)

#define SelFrmTrkRefMIB 0
#define SelFrmTrkRefScaler 1
#define SelFrmTrkRefHDMI 2
#define SelFrmTrkRefB2R 3


extern UINT8 _fgFrametrackBaseOnLRByCLI ;   
...
#define SCL_PTG_TYPE        0
#define SCL_PTG_TYPE_MOVING (0x1<<0)
...
typedef enum {
    ...
    ISR_GFX_VSYNC_RISING             =16,
    ISR_GFX_VSYNC_FALLING            =17,
    ISR_GFX_VDE_RISING               =18,
    ISR_GFX_VDE_FALLING              =19,
    ISR_GFX_OSD_VSYNC_RISING         =20,
    ISR_GFX_OSD_VSYNC_FALLING        =21,
    ISR_GFX_OSD_VDE_RISING           =22,
    ISR_GFX_OSD_VDE_FALLING          =23,
    ISR_REF_FSC_PULSE                =24,
    ISR_REF_PSC_PULSE                =25,
    ISR_REF_GFX_PULSE                =26,
    ISR_REF_B2R_PULSE                =27,
    ISR_REF_R2R_PULSE                =28,
	ISR_PQ_TDSHARP					 =29,  
...
typedef struct
{
    ...
    #endif
    UINT32 u4OutputPlane;                   //Output plane: VDP_PQPATH or VDP_GFXOUT
...
/* @param 3d dram input Mode 
*     0: frame packing/frame sequential
...
{
    E_DRAM_SC_IN_FS      =0x0,
    E_DRAM_SC_IN_SBS     =0x1,
    E_DRAM_SC_IN_TB      =0x2,
    E_DRAM_SC_IN_LI      =0x3
}E_DRAM_SC_3D_IN;


/* @param 3d dram output Mode 
*  @brief set scaler output 3D mode
*   0: PR 1: FPR 2: side by side 3: Shutter
*/
typedef enum
{
    E_DRAM_SC_OUT_PR      =0x0,
    E_DRAM_SC_OUT_FPR     =0x1,
    E_DRAM_SC_OUT_SBS     =0x2,
    E_DRAM_SC_OUT_FS      =0x3
}E_DRAM_SC_3D_OUT;
...
void vScpipSet3DModeReg(UINT32 u4VdpId);
...
void vScpipSetFrmTrackBaseOn(UINT8 fgLRTrack , E_3D_FRAME  eLockFrame );
...
EXTERN void vScpipForce2dMode( UINT8 u1OnOff );
EXTERN void vScpipSetGfxOsdDly_H( UINT32 u4Dly_H );
EXTERN void vScpipSetGfxOsdDly_V( UINT32 u4Dly_V );


#if defined(CC_MT5890)    
void vScpipRefSetData(UINT32 u4OnOff);
void vScpipRefSetTgImportOnOff(UINT32 u4OnOff);
void vScpipRefSetTgImportVCnt(UINT32 u4Vcunt);
void vScpipGfxSetData(UINT32 u4OnOff);
void vScpipGfxSetTgImportOnOff(UINT32 u4OnOff);
void vScpipGfxSetTgImportVCnt(UINT32 u4Vcunt);
...
#define HYBRID_MODE       ( SRM_IsHybridMode(VDP_1))
