...
static UINT8 fgIsPanelSupport3d=SV_OFF;
...
static UINT8 fgMaple3D22DMute = FALSE;
#endif
...
{
    #ifdef CC_MAPLE_CUST_DRV
    E_TDTV_UI_3D_2_2D e322 =  eDrvTDTVGet3D22D(SV_VP_MAIN);
    #endif
    ...
    #ifdef CC_MAPLE_CUST_DRV
    if(e322 != E_TDTV_UI_3D_2_2D_OFF) 
    {
        if(eDetectPack >= E_TDTV_DECODER_INPUT_TB_I && eDetectPack <= E_TDTV_DECODER_INPUT_SBS_P)
        {
           return E_TDTV_UI_3D_MODE_OFF;
        }
        else if(eDetectPack >= AUTO_ONLY_START && eDetectPack <= AUTO_ONLY_END)
        {
            _vDrvVideoSetMute(MUTE_MODULE_SCPOS_TV3D, SV_VP_MAIN, FOREVER_MUTE, FALSE);
            fgMaple3D22DMute = TRUE;
        }
    }
    else
    {
        if(fgMaple3D22DMute == TRUE)
        {
            _vDrvVideoSetMute(MUTE_MODULE_SCPOS_TV3D, SV_VP_MAIN, 0, FALSE); 
            fgMaple3D22DMute = FALSE;
        } 
    }
    #endif
...
E_TDTV_UI_3D_PACKING eDrvTDTVGetCurCfg(void)
{       
    E_DEC_MOD eDecMod=eGetDecModule(SV_VP_MAIN);
...
// judge whether is 3d mode or not
UINT8 u1DrvTDTVCurIs3DMode(void)
{
	if (eDrvTDTVGetCurCfg() != E_TDTV_UI_3D_MODE_OFF && eDrvTDTVGet3D22D(SV_VP_MAIN) == E_TDTV_UI_3D_2_2D_OFF)
	{
		return TRUE;
	}
	return FALSE;
}
// include  all of detected by decode , navi, UI mode , auto mode, 3D to 2D  case
E_TDTV_UI_3D_PACKING u1DrvTDTVGetCur3DMode(void)
{
	if (eDrvTDTVGetCurCfg() != E_TDTV_UI_3D_MODE_OFF && eDrvTDTVGet3D22D(SV_VP_MAIN) == E_TDTV_UI_3D_2_2D_OFF)
	{
...
#ifndef SUPPORT_MIB_3D_CONVERT_444_TO_422
...
static inline void vDrvTDTVDecConfig(E_DEC_MOD eMode)
{
	...
	//work around for output disp_r sync
...
static inline UINT32 u4DrvTDTVDecoderModeQuery(E_DEC_MOD eDec,E_TDTV_DECODER_INPUT eDetectedMode)
{
	...
	LOG(2,"decode detect 3D result: DEC type:%d, 3D mode :%d \n",eDec,eDetectedMode);
    ...
    if((ePreDetectedMode &(~0x1))!= (eDetectedMode&(~0x1)))
    {
            ...
            &&(IS_Support3D()==SV_TRUE))
        {
            if (fgIs3DAutoDisplay)
            {
				...
				LOG(2, "auto 3D display turn on already\n");
            }
            else
            {
...
// for query info about current output format
...
UINT32 u4DrvTDTV3DFMTCapQuery(UINT32 bPath)
{
    ...
    {  
        ...
        else
        {
            ...
            //if(SV_VD_TVD3D!=bGetVideoDecType(bPath)&& u1DrvDIIsSupportedSignal(bPath, E_DI_CLK_KERNEL))
    ...
    {
        u4Cap |= (UINT32)E_TDTV_UI_ENABLE<<((UINT8)E_TDTV_UI_3D_MODE_AUTO*2);
        if(u4DrvTDTVGetTag3D(SV_VP_MAIN)!=TDTV_TAG3D_FS&&u4DrvTDTVGetTag3D(SV_VP_MAIN)!=TDTV_TAG3D_FP)
        u4Cap |= (UINT32)E_TDTV_UI_ENABLE<<((UINT8)E_TDTV_UI_3D_MODE_OFF*2);    
    }
...
UINT32 u4DrvTDTV3DCtrlCapQuery(UINT32 bPath)
{
    ...
    if((!((E_TDTV_UI_NAVI_FULL_AUTO==eUITDNavi)&&(E_TDTV_UI_3D_MODE_AUTO==eDrvTDTVGetExternalUIPacking(bPath))))
        &&(!(TDTV_TAG3D_FP==u43DTag||TDTV_TAG3D_FS==u43DTag)||(E_TDTV_UI_3D_MODE_AUTO!=eDrvTDTVGetExternalUIPacking(bPath))))
    { 
...
{   
    /*
    if((!u1DrvDIIsSupportedSignal(u4Path, E_DI_CLK_KERNEL)))
    {
...
void vDrvTDTVPanelSupport3Ddbg(UINT8 u1support3d)
{
    if(IS_Support3D()==SV_TRUE)
    {    
        LOCK_TDTV_STATE();
        
        fgIsPanelSupport3d=u1support3d;
        
        fgIs3DAutoDisplay = u1support3d;
        fgTDTVModeChgTrigger=SV_ON;
        UNLOCK_TDTV_STATE();
...
void vDrvTDTVScaler3DPattern(UINT8 u1InOut,UINT8 u1OnOff)
{
    if(IS_Support3D()==SV_TRUE)
    {    
        vScpipInOut3DPtgEnable(u1InOut, u1OnOff);
        return;
    }
}
...
{   
	...
	printf("3D Panel                : %s\n",IS_LVDS_DISP_3D_SHUTTER?"shutter panel":IS_LVDS_DISP_3D_POLARIZED?"PR panel":"2D panel" );
    ...
    printf("Force Panel support 3d  : %d \n",fgIsPanelSupport3d);
...
void vPrintfMIBSCStatus(void)
{
#define ARA_SIZE2(x) (sizeof((x))/sizeof((x)[0]))
#define ARA_ASSEES2(array,idx) (array)[(idx)<ARA_SIZE2((array)) ? (idx) :(ARA_SIZE2(array)-1)]

#define VDO_IN_NAME2(x)	ARA_ASSEES2(szVdoInName ,x)
#define VDO_OUT_NAME2(x) ARA_ASSEES2(szVdoOutName,x)

	printf("[Main Path DI] In: %s\tOut: %s\n",
		VDO_IN_NAME2(TD_MIB_IN(sVdoCfg[SV_VP_MAIN].DI)),
		VDO_OUT_NAME2(TD_MIB_OUT(sVdoCfg[SV_VP_MAIN].DI)));
		
	printf("[Main Path SC] In: %s\tOut: %s\n",
		VDO_IN_NAME2(TD_SCL_IN(sVdoCfg[SV_VP_MAIN].SC)), 	
		VDO_OUT_NAME2(TD_SCL_OUT(sVdoCfg[SV_VP_MAIN].SC)));
...
void vDrvTDTVPacking(E_TDTV_UI_3D_PACKING eTDTVUI3DMode)
{
    ...
    if((IS_Support3D()!=SV_TRUE)
         || !(IS_LVDS_DISP_3D_SHUTTER || IS_LVDS_DISP_3D_POLARIZED|| fgIsPanelSupport3d)
        )
    {  
        eInternalUIPacking = E_TDTV_UI_3D_MODE_OFF;
        return;
    ...
    LOG(2,"========vDrvTDTVPacking Set 3D Packing %d\n",eInternalUIPacking);
...
void vDrvTDTVLRInverse(E_TDTV_UI_LR eTDTVLRInverse)
{
    ...
    fgTDTVModeChgTrigger=SV_ON;    
	LOG(2,"========vDrvTDTVLRInverse Set :LR inverse %d\n",eUILRInverse);
    UNLOCK_TDTV_STATE();
    return;
}
/**
...
void vDrvTDTVDepth(UINT32 u4DepthField)
{
	...
	LOG(2,"========vDrvTDTVDepth Set :depth %d\n",u4DepthField);
...
void vDrvTDTV3D22D(E_TDTV_UI_3D_2_2D e3D22DMode)
{
    ...
    {
        eUI3DTO2D=e3D22DMode;
        LOG(2,"========vDrvTDTV3D22D Set 3D22D %d\n",eUI3DTO2D);
    }    
    fgTDTVModeChgTrigger=SV_ON;
    UNLOCK_TDTV_STATE();
    return;
...
{
#if defined(CC_3DTV_FPR_SUPPORT) && defined(SUPPORT_MJC)
    UINT32 j;
    ...
    LOG(2,"========vDrvTDTV3DFPR is called %d==\n",eUI3DFPR);
    fgTDTVModeChgTrigger=SV_ON;
...
void vDrvTDTVProtrude(UINT32 u4Protrude)
{
	...
	LOG(2,"========vDrvTDTVProtrude Set Protrude %d\n",u4Protrude);
}
...
void vDrvTDTVDistanceFromTV(UINT32 u4Distant)
{
	...
	LOG(2,"========vDrvTDTVDistanceFromTV Set Distance %d\n",u4Distant);
}
...
void vDrvTDTVOSDDepth(UINT32 u4OSDDepth)
{
    ...
    fgTDTVModeChgTrigger=SV_ON;
	LOG(2,"========vDrvTDTVOSDDepth Set OSDDepth %d\n",u4OSDDepth);
    UNLOCK_TDTV_STATE();
...
{
    eUITDNavi = eTDTVNavi;   
    LOG(2,"========vDrvTDTVTDNAVI Set 3D NaviMode %d\n",eTDTVNavi);
}
...
{    
	...
	LOG(2,"========vDrvTDTVTDDC Set 3D TDDC %d\n",u1OnOff);
}
...
void vDrvTDTVTImgSafety(UINT8 u1Level)
{
	...
	LOG(2,"========vDrvTDTVTImgSafety Set 3D ImgSafety %d\n",u1ImgSafetyLvl);
...
static inline void vDrvEable3DLRSyncGPIO(void)
{
	UINT32 u4Ext3DEnableGPIO, u4Ext3DLRPin, u4Ext3DLRPinFunc;
	if(IS_LVDS_DISP_3D_SHUTTER )
	{	
		...
		{	
			...
			if (DRVCUST_OptQuery(e3DEnableGPIO,&u4Ext3DEnableGPIO)==0)
			{
				LOG(3, "[3D]3D_EN: GPIO%d output 1\n", u4Ext3DEnableGPIO);
			...
			{
				LOG(3, "[3D]3D_LR: 0x%x set to FUNCTION%d\n", u4Ext3DLRPin, u4Ext3DLRPinFunc);
				BSP_PinSet(u4Ext3DLRPin, u4Ext3DLRPinFunc);
			}
		}
		else
		{
			//Disable 3D LR Signal
			if (DRVCUST_OptQuery(e3DLRPin,&u4Ext3DLRPin)==0)
			{
				LOG(3, "[3D]3D_LR: 0x%x set to FUNCTION0\n", u4Ext3DLRPin);
				BSP_PinSet(u4Ext3DLRPin, 0);
			}
			//Disable 3D Enable Signal
			if (DRVCUST_OptQuery(e3DEnableGPIO,&u4Ext3DEnableGPIO)==0)
			{
				LOG(3, "[3D]3D_EN: GPIO%d output 0\n", u4Ext3DEnableGPIO);
...
{    
	...
	LOG(2,"ePanelLRInv:%d,eLRInv:%d,u1LRInvForB2RFM:%d,3D(0:2D,2:TTD,other is real 3D):%d \n",ePanelLRInv,eLRInv,u1LRInvForB2RFM, eDrvTDTVGetCurCfg());
#endif
}
...
{    
    ...
    {                
        ...
        {        
			...
			// for 3D need 50 ->60 or ,60 ->50 case
			vDrvCheck3DOuputFrameRate();
			//check  how many times changing bettwen 3D and 2D 
            LOG(2,"Current 3D mode is %d\n",u1DrvTDTVGetCur3DMode());
