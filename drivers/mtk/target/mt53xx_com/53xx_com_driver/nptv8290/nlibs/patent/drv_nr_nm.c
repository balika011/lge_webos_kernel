...
void _vDrvMCNRCleanDiffLvl(void)
{
    ...
    if ((_rNmSta.u1GmvX == 0) && (_rNmSta.u1GmvY == 0)
...
static void _vDrvMCNRSADAnalysis (const NR_NM_STA_T* prNmSta)
{
    ...
    u4Centroid = _u1DrvMCNRCalcSADCentroid(1, _rNmAna.u1Terminate, SADHIST, &_rNmAna.u4HistMoment);
    ...
    {
    u1NoiseLevel = (_rNmPrm.u4Cfg & NM_CFG(NM_CFG_UPDATE_NL_SC)) ?
        (prNmSta->u1NLScnChgCnt > 1 ?  u4Centroid : _u1DrvNRGetNLScnChgRaw()): MIN(_u1DrvNRGetNLForQ(), 60);
    }
    else
    {
        u1NoiseLevel = (_rNmPrm.u4Cfg & NM_CFG(NM_CFG_UPDATE_NL_SC)) ? _u1DrvNRGetNLScnChgRaw() : MIN(_u1DrvNRGetNLForQ(), 60);    
    }
    ...
    _rNmAna.u1TargetNL = u1NoiseLevel;
    
    if (_rNmPrm.u4Cfg & NM_CFG(NM_CFG_STABLIZER_IIR))
    {
        if (_SWR(NR_NM_13,NR_NM_NOIIR_CH_CHG)&&(bGetVideoDecType(SV_VP_MAIN) == SV_VD_TVD3D))
        {
    ...
    //For Sony Maple
    _rNmAna.u1NoiseLevel = (_SWR(NR_NM_16, CUST_FORCE_NOISE_LVL_EN)) ?
                            _SWR(NR_NM_16, CUST_FORCE_NOISE_LVL) : _rNmAna.u1NoiseLevel;
