...
//    vIO32WriteFldAlign(REG_CVBS_CFG1, 1, RG_MIXINPUT);//
   // vIO32WriteFldAlign(VFE_E0, 0, RG_BTM_ENA); /*add for V2 */
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG3, 2, RG_CVBSADC1_VREF_SEL); /*add for V2 */
    vIO32WriteFldAlign(REG_CVBS_CFG4, 2, RG_CVBSADC2_VREF_SEL); /*add for V2 */
   ...
   // vIO32WriteFldAlign(VFE_E1, 5, RG_CLAMP_SINK); /*add for V2 */
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_CVBSADC1_DC_EN);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_CVBSADC2_DC_EN);//
...
//    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_BTM_EN_2);
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0, RG_CLKTOVGA_EN);
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0D, RG_PGAGAINB);//Setting CHB gain
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_PGABUFNA_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_PGABUFNB_PWD);//
 ...
 //   vIO32WriteFldAlign(REG_CVBS_CFG7, 0, RG_PGABUFNB_PWD_2);//
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_SHIFTA_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_SHIFTB_PWD);//
...
//    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_CINRB_EN);//
   // vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_CVBS_REV);//
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_SOURCE_SEL);//
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_GROUND_SEL );//
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_SHORT_PGAA);//
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_SHORT_PGAB);//
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_BP_PGABUFA );//
    vIO32WriteFldAlign(REG_CVBS_CFG1, 0x0, RG_BP_PGABUFB );//
//    #ifdef CC_MT5389
//	vIO32WriteFldAlign(REG_CVBS_CFG2, 0x1, RG_GCOUPLE_ENB);//
//    #endif
    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_CVBSADC1_CK_SEL);// CY 1->0
    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_CVBSADC2_CK_SEL);// CY 1->0
...
//    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_CVBSADC_BIASSEL);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CVBSADC1_CORE_PWD );//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CVBSADC1_IO_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 0, RG_CVBSADC2_CORE_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 0, RG_CVBSADC2_IO_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_CVBSADC1_CK_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 0, RG_CVBSADC2_CK_PWD);//
...
//    vIO32WriteFldAlign(REG_CVBS_CFG7, 3, RG_CVBSADC_CHSEL_2);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 2, RG_50U_CLAMP);//
    ...
    //vIO32WriteFldAlign(VFE_E4, 2, RG_50U_CLAMPB);//
     ...
     vIO32WriteFldAlign(REG_CVBS_CFG1, 1, RG_MANUALGAIN_EN);//Enable CHB PGA manual gain
    #else
     vIO32WriteFldAlign(REG_CVBS_CFG1, 0, RG_MANUALGAIN_EN);//disable CHB PGA manual gain
    #endif
...
void vDrvSetTVADC(UINT8 vsrc)
{
	...
	switch (VSS_MAJOR(vsrc))
	{
		...
		case VSS_CVBS:
            ...
            if (((_bMainICIn == P_FB0) || (_bSubICIn == P_FB0) || (_bMainICIn == P_FB1) || (_bSubICIn == P_FB1))  && (bINT_BST_DET == 1)) //internal path
			{
			    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E0, 0, RG_CLAMP_SINK_ENB);//
			    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E0, 0, RG_CLAMP_SOURCE_ENB);//
                vIO32WriteFldAlign(REG_CVBS_CFG2, 1,  RG_SHIFTB_PWD);//
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG4, 0, RG_CVBSADC2_CK_PWD);//
		...
		case VSS_SV:
			...
			/*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_PGAB_PWD);//
			/*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_CTINB);//
			vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_OFFCURB);//
...
{//FLUKE white pattern pure AV
    ...
    for(bCALCLAMP=0;bCALCLAMP<=0x0f;bCALCLAMP++)
        for(bOFFCUR=0;bOFFCUR<=0x1f;bOFFCUR++)
        {
            ...
            vIO32WriteFldAlign(REG_CVBS_CFG2, bOFFCUR, RG_OFFCURA);//
            vUtDelay1ms(50);
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0x00, RG_OFFCURA);//     //default value
}
...
BOOL vDrvSetSIFADC(void)
{
	...
	{
          vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_PGABUFNB_PWD);
          vIO32WriteFldAlign(REG_CVBS_CFG2, 1,RG_SHIFTB_PWD);
         // vIO32WriteFldAlign(REG_CVBS_CFG3, 1,RG_OFFCURB_PWD);
          vIO32WriteFldAlign(REG_CVBS_CFG1, 0, RG_CLKTOVGA_EN);//disable 108M to VGA.Scart SV C use CVBS CHB
          bUseChD = FALSE;
	}
	else
	{
  ...
  //        	vIO32WriteFldAlign(REG_CVBS_CFG4, (CHA>>16)|0x40, RG_AISEL_2);//write 31:24
          vIO32WriteFldAlign(REG_CVBS_CFG1, 1, RG_CLKTOVGA_EN);//enable 108M to VGA.Scart SV C use VGA AD
          vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_PGABUFNB_PWD);
          vIO32WriteFldAlign(REG_CVBS_CFG2, 0,RG_SHIFTB_PWD);
...
void vScartSVInRFE(UINT8 status)
	{
		if(status)
			{
				if(IO32ReadFldAlign(REG_CVBS_CFG1, RG_CLKTOVGA_EN)==0)
				{
         				...
         				vIO32WriteFldAlign(OMUX_VDOIN_CLK_SEL, 1, FLD_VDOIN_CLK_SEL_9);//enable CVBS2_CLK108
             				vIO32WriteFldAlign(REG_CVBS_CFG1, 1, RG_CLKTOVGA_EN);//enable 108M to VGA.Scart SV C use VGA AD
	     				vIO32WriteFldMulti(REG_VGA_Normal_CFG7,P_Fld(1,RG_CLKOSEL_3)|P_Fld(0,RG_CLKOSEL_2)|P_Fld(0,RG_CLKOSEL_1));
	     				vIO32WriteFldAlign(REG_VGA_Normal_CFG6,0,RG_VGAADC3_CK_SEL);
			...
			{
				if(IO32ReadFldAlign(REG_CVBS_CFG1, RG_CLKTOVGA_EN)==1)
				{
        				...
        				vIO32WriteFldAlign(REG_CVBS_CFG1, 0, RG_CLKTOVGA_EN);//enable 108M to VGA.Scart SV C use VGA AD
					vIO32WriteFldMulti(REG_VGA_Normal_CFG7,P_Fld(0,RG_CLKOSEL_3)|P_Fld(0,RG_CLKOSEL_2)|P_Fld(1,RG_CLKOSEL_1));
					vIO32WriteFldAlign(REG_VGA_Normal_CFG6,1,RG_VGAADC3_CK_SEL);
...
void vDrvCvbsAdcPD(void)
{
    ...
    #endif
    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_CVBSADC1_CORE_PWD );//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_CVBSADC1_IO_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_CVBSADC2_CORE_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_CVBSADC2_IO_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG3, 1, RG_CVBSADC1_CK_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG4, 1, RG_CVBSADC2_CK_PWD);//
   #endif
}
...
void vDrvCvbsVfePD(void)
{
 ...
 //   vIO32WriteFldAlign(REG_CVBS_CFG1, 1, RG_MIXINPUT);//
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_PGABUFNA_PWD);//
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_SHIFTA_PWD);    /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_SHIFTB_PWD);    /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_CVBS_PWD);       /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_CORE_PWD);       /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_IGEN_PWD);       /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_CLAMP_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 1, RG_INMUX_PWD);//
 //  #ifdef CC_SUPPORT_SIF_BYPASS
//          vIO32WriteFldAlign(REG_CVBS_CFG7, 0, RG_PGABUFNB_PWD_2);
//          vIO32WriteFldAlign(REG_CVBS_CFG7, 0,RG_SHIFTB_PWD_2);
...
void vDrvCvbsVfePWON(void)
{
   ...
   // vIO32WriteFldAlign(REG_CVBS_CFG3, 0, RG_MIX_PWD);
    ...
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_BUFB_PWD);//
    /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_CTBUFB_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_SHIFTA_PWD);    /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_SHIFTB_PWD);    /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_CVBS_PWD);       /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_CORE_PWD);       /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_IGEN_PWD);       /*add for V2*/
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_CLAMP_PWD);//
    vIO32WriteFldAlign(REG_CVBS_CFG2, 0, RG_INMUX_PWD);//
...
void vDrvHDTVADCDefaultSetting(void)
{
    ...
    #endif
	LOG(0,"==== vDrvHDTVADCDefaultSetting  ===\n");
   #if EEPROM_READY
    _bEEPROM_ready = 1;
   #else
    _bEEPROM_ready = 0;
    ...
    //vIO32WriteFldAlign(REG_VGA_Normal_CFG0, 1, SELF_EN); //5365 mark
          	vIO32WriteFldAlign(REG_CVBS_CFG1, 0, RG_CLKTOVGA_EN);//disable 108M to VGA.Scart SV C use VGA AD
...
vIO32WriteFldMulti(REG_VGA_Normal_CFG7,P_Fld(0,RG_CLKOSEL_3)|P_Fld(0,RG_CLKOSEL_2)|P_Fld(1,RG_CLKOSEL_1)|P_Fld(0,RG_RGB_REV ));

   //vUtDelay1ms(2);
    vIO32WriteFldAlign(PDWNC_VGACFG2, 7,FLD_RG_OFFCUR);
	...
	for (bCh=0; bCh<3; bCh++)
	{
	 ...
	 vIO32WriteFldMulti(REG_VGA_Normal_CFG4+(4*bCh),P_Fld(1,RG_VGAADC1_CK_SEL )|P_Fld(0,RG_VGAADC1_DIV_SEL)
                                                                              ...
                                                                              |P_Fld(0xF,RG_VGAADC1_REV1 )|P_Fld(2,RG_VGAADC1_VREF_SEL )
                                                                              |P_Fld(0,RG_VGAADC1_IO_PWD));

	}

   // vIO32WriteFldAlign(REG_VGA_Normal_CFG3, 0, TRGSEL);
//vIO32WriteFldMulti(REG_VGA_Normal_CFG4,P_Fld(0,RG_VGAADC1_MON_SEL)|P_Fld(0,RG_VGAADC1_IO_PWD)|P_Fld(0,RG_VGAADC2_IO_PWD)|P_Fld(0,RG_VGAADC3_IO_PWD));
    vIO32WriteFldMulti(REG_VGA_Normal_CFG0, P_Fld(0,RG_VREFGEN4FE_PWD )|P_Fld(0,RG_VDC_P_EN )|P_Fld(0,RG_RESSEL )
     	...
     	vIO32WriteFldAlign(REG_VGA_Normal_CFG0,0x02,RG_ADCVREFP );
   #if Phase3Channel
    vIO32WriteFldMulti(REG_VGA_Normal_CFG7, P_Fld(1,RG_CLKINV_EN )|P_Fld(1,RG_RELATCH_EN ));
   #else
    vIO32WriteFldMulti(REG_VGA_Normal_CFG7, P_Fld(0,RG_CLKINV_EN )|P_Fld(0,RG_RELATCH_EN ));
   #endif
...
//vDrvVoDDDSRST(1);
    ...
    switch (bVDOINPUT)
    {
        ...
        case P_YP1:
            ...
            vIO32WriteFldAlign(PDWNC_VGACFG4, 0,FLD_RG_SYNC2_VTH);
            vIO32WriteFldAlign(PDWNC_VGACFG5, 0,FLD_RG_SYNC2_VTL);
        ...
        case P_VGA:
            ...
            vIO32WriteFldAlign(PDWNC_VGACFG4, 4,FLD_RG_SYNC2_VTH);
            vIO32WriteFldAlign(PDWNC_VGACFG5, 2,FLD_RG_SYNC2_VTL);
        ...
        case P_FB1:   //SCART1
            ...
            vIO32WriteFldAlign(PDWNC_VGACFG4, 2,FLD_RG_SYNC2_VTH);
            vIO32WriteFldAlign(PDWNC_VGACFG5, 2,FLD_RG_SYNC2_VTL);
            bInputType = 2;
        break;
...
{
	UINT8 bCh;
	DBG_Printf(DBG_MDET, "vDrvAllHDADCPow(%d)\n",bPow);
	...
	//	vIO32WriteFldAlign(REG_VGA_Normal_CFG4,0,ADCBIAS_PWD);
		...
		for (bCh=0; bCh<3; bCh++)
		{
          ...
          //  vIO32WriteFldAlign(REG_VGA_Normal_CFG1+(bCh<<4), 0, PGA_1_PWD);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 0, RG_VGAADC1_CORE_PWD);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 0, RG_VGAADC1_DIV_SEL );
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 0, RG_VGAADC1_CK_PWD);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 0, RG_VGAADC1_IO_PWD);
	...
	else if (bPow == SV_FALSE)//Power down
	{
		...
		for (bCh=0; bCh<3; bCh++)
		{
           ...
           // vIO32WriteFldAlign(REG_VGA_Normal_CFG1+(bCh<<4),1,PGA_1_PWD);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh),1,RG_VGAADC1_CORE_PWD);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 1, RG_VGAADC1_DIV_SEL);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 1, RG_VGAADC1_CK_PWD);
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(4*bCh), 1, RG_VGAADC1_IO_PWD);
...
void vScartVfeConnect(void)
{
 ...
 //   vIO32WriteFldMulti(REG_CVBS_CFG1, /*mark for V2: P_Fld(1,RG_MIX_60U_EN) |P_Fld(0,RG_MIX_SHORT)|*/P_Fld(2,RG_MIXVOCM_SEL));//
    ...
    vIO32WriteFldAlign(REG_CVBS_CFG0, 1, RG_SYNCTOVGA_EN );//
    // 3. Set FB path
    ...
    vIO32WriteFldAlign(PDWNC_VGACFG2, 1   , FLD_RG_VREF_CVBS); //power down SOGY source clamp
    // 5. Set slicer BW,gain and VH,VL
...
{ //Michael Modify :20100308
   ...
   // vIO32WriteFldAlign(REG_CVBS_CFG1, 0x00, RG_CVBS_REV);//
    vIO32WriteFldAlign(REG_CVBS_CFG0, 0, RG_SYNCTOVGA_EN );//
...
void vVFESLT_slicer_test_verC(void)
{
	...
	for (i=0;i<sizeof(VFEverC_slicer);i++)
	{
		...
		if ((i%16)==0)
			LOG(1,"#SLT:[%2d] ",i/16);
		vIO32WriteFldAlign(PDWNC_VGACFG5,(VFEverC_slicer[i]>>3), FLD_RG_SYNC2_VTL);
		vIO32WriteFldAlign(PDWNC_VGACFG4,(VFEverC_slicer[i]), FLD_RG_SYNC2_VTH);
		vResetVLenSP2();
		do
		{
			HAL_GetTime(&cur_time[1]);
...
void vSetMONSlicer(UINT8 pair)
{
    if(bHdtvOpt05_AdaptiveSlicer)
    {
        vIO32WriteFldAlign(PDWNC_VGACFG4, (pair & 0xF0) >>4, FLD_RG_SYNC2_VTH);
        vIO32WriteFldAlign(PDWNC_VGACFG5, (pair & 0x0F), FLD_RG_SYNC2_VTL);
...
{
	UINT8 pair;
	pair=IO32ReadFldAlign(PDWNC_VGACFG4,FLD_RG_SYNC2_VTH)<<4 |IO32ReadFldAlign(PDWNC_VGACFG5,FLD_RG_SYNC2_VTL);
    return(pair);
}
...
void vDrvOutputOffsetCalNew(UINT16 wCLAMP)
{
	...
	// **************************
       ...
       vIO32WriteFldAlign(REG_CVBS_CFG2, 0x1F, RG_OFFCURA);//
       vUtDelay1ms(u4Delay);
       iA= wDrvReadADCBlankAve();
       vIO32WriteFldAlign(REG_CVBS_CFG2, 0x0F, RG_OFFCURA);//
       vUtDelay1ms(u4Delay);
       iB= wDrvReadADCBlankAve();
       ...
       else
	{
       ...
       vIO32WriteFldAlign(REG_CVBS_CFG2, 0x00, RG_OFFCURA);////MT536X //recover to default setting
       vIO32WriteFldAlign(REG_CVBS_CFG2, 0x00, RG_OFFCURB);////MT536X //recover to default setting
