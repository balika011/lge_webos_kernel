...
#ifdef CC_MT5890
#define PIX_CLK_INV_Criterion 535
#endif

#ifdef CC_MT5861
#define PIX_CLK_INV_Criterion 750
#endif
...
static UINT32 _u4SSFreq = 0;//for modulation rate
#endif
...
//CC_MT5365 according to Walter's suggestion
 ...
 vDrvCLKINFreeRun(ENABLE);
    ...
    vIO32WriteFldAlign(CKGEN_DISP_CKCFG1, 1, FLD_RESYNC_PCLK_SEL);
        ...
        vIO32WriteFldMulti(REG_VGA_Normal_CFG7, P_Fld(1,RG_CLKINV_EN)|P_Fld(1,RG_RELATCH_EN)); //be careful the define for this bit is different between MT5365 AND MT8223		
    #else//single phase
        ...
        vIO32WriteFldMulti(REG_VGA_Normal_CFG7, P_Fld(0,RG_CLKINV_EN)|P_Fld(0,RG_RELATCH_EN)); //be careful the define for this bit is different between MT5365 AND MT8223			
    #endif
    //vIO32WriteFldMulti(CKGEN_AFEPLLCFG1,P_Fld(1,FLD_RG_VGAPLL_PD)|P_Fld(0,FLD_RG_VGAPLL_MONAIOEN)|P_Fld(0,FLD_RG_VGAPLL_MONCLKEN));
      vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 0, FLD_RG_VGAPLL_EN );
      vIO32WriteFldMulti(REG_PLL_GROUP_CFG7, P_Fld(1, FLD_RG_VGAPLL_SDDS_PD_PDB)|P_Fld(1,FLD_RG_VGAPLL_SDDS_PD_EN)|P_Fld(1,FLD_RG_VGATL_BIAS_EN )|P_Fld(1,FLD_RG_XDDS_PWDB)|P_Fld(7,FLD_RG_XDDS_PI_C)|P_Fld(1,FLD_RG_XDDS_HF)|P_Fld(1,FLD_RG_VGAPLL_RTB_EN));
      vIO32WriteFldMulti(REG_PLL_GROUP_CFG4, P_Fld(0, FLD_RG_VGAPLL_MONCK_EN )|P_Fld(1,FLD_RG_VGAPLL_RESERVE));
...
void vDrvAllADCPLLPow(UINT8 bPow)
{
	...
	{    
       ...
       vIO32WriteFldMulti(REG_PLL_GROUP_CFG7, P_Fld(0x0, FLD_RG_VGAPLL_B_EN)|P_Fld(0x0,FLD_RG_VGAPLL_G_EN)|P_Fld(0x0,FLD_RG_VGAPLL_R_EN));
       }
}
...
void vDrvADCPLLSet(UINT16 wPixClk, UINT16 wHtotal)
{
    UINT32 dTmp;
    UINT8 bMcode;
    UINT8 bADCPLL_NS, bADCPLL_POSTDIV;
   // UINT32 delay_count;
   // UINT8 bmode;
   DBG_Printf(VGA_Debug,"vDrvADCPLLSet(%d, %d)",wPixClk,wHtotal);	 
...
vIO32WriteFldAlign(VFE_18, 0, RG_SDDS_FIFO_START);
...
//new band selection
    //SDDS Setting			//Let SDclk CW range:    324/5 ~ 324/14
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 0x0, FLD_RG_VGAPLL_SDM_PCW_CHG);   //free run VGAPLL
   //Setting ADCPLL Band
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 0x0, FLD_RG_VGAPLL_BS);   
   ...
   //Setting before ADCPLL    
    if(wPixClk<300)
    {
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG7, 0x0, FLD_RG_XDDS_CKSEL);   
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 0, FLD_RG_VGAPLL_PREDIV);   
   }
    else if((wPixClk<650)&&(wPixClk>=300))			//(SDclk CW)/1, ADCPLLx1 
    {
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG7, 0x1, FLD_RG_XDDS_CKSEL);   
	vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 1, FLD_RG_VGAPLL_PREDIV);   
    }
	...
	vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 1, FLD_RG_VGAPLL_PREDIV);   
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, bADCPLL_NS, FLD_RG_VGAPLL_SDM_PCW );   //FBDIV
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, bADCPLL_POSTDIV, FLD_RG_VGAPLL_POSDIV );   ///4
...
//    vIO32WriteFldAlign(REG_PLL_GROUP_CFG0, 1, RG_VGAPLL_EN );
...
//   vIO32WriteFldMulti(REG_PLL_GROUP_CFG6, P_Fld(0x0, FLD_RG_VGAPLL_G_EN)|P_Fld(0x0,FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
   ...
   vIO32WriteFldMulti(REG_PLL_GROUP_CFG7, P_Fld(0x1, FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
    //vIO32WriteFldMulti(CKGEN_AFEPLLCFG1,P_Fld(0,FLD_RG_VGAPLL_PD)|P_Fld(1,FLD_RG_VGAPLL_VCOCAL_EN));
     ...
     vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 0x1, FLD_RG_VGAPLL_SDM_PCW_CHG);     //close VGAPLL loop
       vIO32WriteFldAlign(REG_PLL_GROUP_CFG4, 1, FLD_RG_VGAPLL_EN );
...
UINT32 dwTmp[MAX_PHASE_ARRAY],dwTmpMAX;
       ...
       if (fgIsMainYPbPr() || fgIsPipYPbPr())                  //wu
       {
       ...
       else
        {
           ...
           {
                case 0:
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 1, RG_CLKOSEL_1);	
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 0, RG_CLKOSEL_2);	
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 0, RG_CLKOSEL_3);	
                case 1:
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 1, RG_CLKOSEL_2);	
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 0, RG_CLKOSEL_1);	
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 0, RG_CLKOSEL_3);	
                case 2:
                default:
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 1, RG_CLKOSEL_3);	
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 0, RG_CLKOSEL_1);	
                vIO32WriteFldAlign(REG_VGA_Normal_CFG7, 0, RG_CLKOSEL_2);	
                break;
            }
...
void vDrvEnable_DDDS_Calibration(UINT8 bKP0_START,UINT8 bKP0_END, UINT8 bKI0_START, UINT8 bKI0_END, UINT8 ERROR_LIM,UINT32 u4InitCW)
{
#if EnableDDDSCalibration
...
void vDrvDDDS_Response(void)
{
    ...
    switch (_bDDDSCaliState)
    {
        ...
        case DDDS_CALI_START:
             ...
             vIO32WriteFldAlign(DDDS_0C, 0, DDDS_0C_VSYNC_TRACK_EN); // disable  Frame Track            
			 ...
			 LOG(3,"DDDS initial_CW=%7x, KP0=%u,  KI0=%u\n", CW_old,_bDKP0, _bDKI0);
             ...
             for(i=1; i<(DMEM_SIZE/4);i++)
             {
                        ...
                        vUtDelay2us(40);  // 480i Hsync=15.7kHz
                 	#if 0
                        if (CW[i] ==CW_old)
                        {
                              i=i-1;
	        ...
	        mode=_bHdtvTiming;
             ...
             #if SIMPLE_METHOD
             #else
			 LOG(3,"Repeat rate=%u, Hsync=%u Hz,  Pixel rate=%u MHz\n", wRepeatimes, (100*VGATIMING_TABLE[mode].IHF) ,CW_old);
             #endif
           #endif
             x_mem_free(CW);
             x_mem_free(fglock);
        ...
        case DDDS_CALI_END:
            if((_bDKI0== _bDKI0_MAX) && (_bDKP0== _bDKP0_MAX))
            {
                ...
                LOG(3,"End of DDDS identification\n");
            }
            else
            {
...
{
	if (PANEL_IsSupport4K2KDD()||((GetPANEL4K2K_TYPE() == PANEL_TYPE_4K2K_60) && fgIsMainDec()))
	{
		return (_u4Permillage << 1);
	}
	else
	{
...
void DDDS_SetSpreadSpectrum(UINT32 u4Freq, UINT32 u4Range)
{
	...
	{
		u4Range = 0;
     	LOG(3, "[SS] ENGINEER_DISPLAY_MODE \n");		
	}
    ...
    if ((u4Freq == _u4SSFreq) && (u4Range == _u4Permillage) && (u4SSVpllFreq == _u4VPllFrequency))
    {
     LOG(3, "[SS] Same SS\n");
     return;
    }
	...
	if (PANEL_IsSupport4K2KDD()||((GetPANEL4K2K_TYPE() == PANEL_TYPE_4K2K_60) && fgIsMainDec()))
	{
		_u4Permillage = u4Range >> 1;
	}
	else
	{
    	_u4Permillage = u4Range;
	}
	LOG(3, "[SS]##_u4Permillage=%d\n", _u4Permillage);
    ...
    {
        _u4SSFreq = 30000; // 62973;
    }

    if(IO32ReadFldAlign(DDDS_04,DDDS_04_STEP_FREQ_SEL))
    {
     u4Tick =  ( DDDS_SS_STEP_UNIT * 2 / 4 ) / _u4SSFreq;
    }
    else
    {
     u4Tick =  ( DDDS_SS_STEP_UNIT / 4 ) / _u4SSFreq;
    }
    ...
    u8Step = ORYX_SYSPLL_CLK_SS; // (UINT32)BSP_GetPLLClock(SRC_SYSPLL_CLK);
    ...
    {
       u8Step = u8Step * 8;  //four : VOPLL -> 8x by OCLK view
    } 
...
void vDDDSInit(void)
{
    ...
    LOG(1,"DDDSinitial only set error limit\n");    
	vIO32WriteFldAlign(DDDS_03, 1,DDDS_03_DDDS_ERR_LIM_RES);

    // Set error limit
    if (IS_VBYONE)
    {
		_u4ErrorLimit = ERROR_LIMIT_FOR_VB1;
	}
	else if ((IS_PANEL_L12R12)||(IS_PANEL_2D_N_3D_L12R12))
	{
		_u4ErrorLimit = ERROR_LIMIT_FOR_PDP;
	}
	...
	if ((_u4ErrorLimit & 0xff) <= 0x07)
	{
		_fgFrTrkHalfStep = TRUE;
		LOG(1,"DDDSinitial --> Half step!\n");
	}

#ifndef CC_MTK_LOADER	
#ifdef SUPPORT_FREERUN_DCLK_BY_STEP
    TargetErrlimit = (_u4ErrorLimit&0xff);
#endif
#endif
    #ifndef CC_MTK_LOADER
 ...
 * @brief vDrvTconTopVdoSel

 * Select vdo source to TconTop

 * @param  1(eTCONTOP_BYPASS_Mode): BYPASS Mode 0(eTCONTOP_PQ_Mode_):PQ Mode
 * @retval void
 */

void vDrvTconTopVdoSel(UINT8 u1Mode)
{
#if defined(CC_MT5890)
	if( u1Mode == E_VB1_PATH_BYPASS)
	{
		if (!IS_Support4KDisplay())
		{
			Printf("Not support 4K2K60Hz panel \n");
			vIO32WriteFldAlign(TMGR4, 0, TMGR4_VDO_SRC_SEL);  // from PQ path
			#if defined(CC_MT5399) || defined(CC_MT5890)
		...
		{
			vIO32WriteFldAlign(TMGR4, 1, TMGR4_VDO_SRC_SEL);  // from bypass path
		}	
	}
	else // ( u1Mode == eTCONTOP_PQ_Mode_)
	{
		vIO32WriteFldAlign(TMGR4, 0, TMGR4_VDO_SRC_SEL);  // from PQ path
...
UINT8 vDrvGetTconTopVdoSel(void)
{
   #if defined(CC_MT5890)
   	   if (PANEL_IsSupport4K2KDD()&& IS_IC_5861())
	   return 0;
	   else
       return(IO32ReadFldAlign(TMGR4, TMGR4_VDO_SRC_SEL));
   #else
       return 0;
   #endif
}
...
void vDrvSetOCLKClockSchemaInit(void)
{
	...
	if (((!IS_Support120HZ())&&(LVDS_OUTPUT_PORT == FOUR_PORT))
		...
		|| ((!IS_Support4KDisplay())&&(LVDS_OUTPUT_PORT == OCTA_PORT)))	
	{
	 // 1. Change OCLK, OCLK2, TCLK source from GPIO pin.  and power down oclk, tclk
	 #if defined(CC_MT5399) || defined(CC_MT5890)
	 _u1BondingEn = 1;
     Printf("Not support 120Hz panel or 21:9 panel or 4K2K60Hz panel \n"); 	
	 vIO32WriteFldMulti(CKGEN_TCLK_TEST, P_Fld(1,FLD_TCLK_TSTSEL)|P_Fld(1,FLD_OCLK_TSTSEL));
	 #else
	 _u1BondingEn = 0;
	 #endif
...
extern BOOL bR2rSelVB1Path(void);
#endif
...
#ifndef CC_MTK_LOADER
		#ifdef CC_SUPPORT_W2DR2R
			if (bR2rSelVB1Path())  // bypass path
			{	
				vIO32WriteFldAlign(DDDS_1C, 1, DDDS_1C_R2R_HSYNC_SEL);  // Hsync from PIP
			}
			else
			{
				vIO32WriteFldAlign(DDDS_1C, 0, DDDS_1C_R2R_HSYNC_SEL);  // Hsync from MAIN
			}
		#endif
#endif
			break;
...
void vDrvSetVopllVcoMonitor(UINT8 bType)
{
    ...
    vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG3, 0x1, REG_LVDSTX_PLL_CFG3_VPLL_TST_CKENB);
    vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG4, 0x1, REG_LVDSTX_PLL_CFG4_PPLL_TSTEN);    

    if(bType)
    {
        Printf("PPLL Vtrl measure \n");
        vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG4, 0x1, REG_LVDSTX_PLL_CFG4_PPLL_MONEN);
        vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG3, 0x2, REG_LVDSTX_PLL_CFG3_VPLL_TST_SEL);
    }
    else
    {
        Printf("VPLL Vtrl measure \n");
        vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG4, 0x0, REG_LVDSTX_PLL_CFG4_PPLL_MONEN);
        vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG3, 0x1, REG_LVDSTX_PLL_CFG3_VPLL_MONEN);
        vIO32WriteFldAlign(REG_LVDSTX_PLL_CFG3, 0x3, REG_LVDSTX_PLL_CFG3_VPLL_TST_SEL);
    }
...
void vDrvTconOn(void) //Efuse support control
{
    ...
    {
        //enable tcon feature
        vIO32WriteFldAlign(DDDS_08,1,FLAG_TCON_ONOFF);
    }
    else
    {
        //disable Tcon feature
        vIO32WriteFldAlign(DDDS_08,0,FLAG_TCON_ONOFF);
        // Tg off
...
{
    if (IO32ReadFldAlign(DDDS_08, FLAG_TCON_ONOFF) == 0) //not TCON panel
    {
    ...
    else
    {
        ...
        return (IO32ReadFldAlign(DDDS_08, FLAG_TCON_ONOFF));
    }
}

#ifdef CC_SUPPORT_UHD

#define UHD_PWR_ON   (BIM_BASE + 0x42c)
	#define UHD_4KDD_DATA_SELECT  Fld(1,18,AC_MSKB2)//[18:18]
	#define UHD_4KDD_SELECT       Fld(1,17,AC_MSKB2)//[17:17]
	#define UHD_PRE_ON_S_PRE      Fld(1,14,AC_MSKB1)//[14:14]
	#define UHD_PWR_ON_PRE        Fld(1,13,AC_MSKB1)//[13:13]
	#define UHD_PWR_ISO_EN_PRE    Fld(1,12,AC_MSKB1)//[12:12]
	#define UHD_PWR_CLK_DIS_PRE   Fld(1,11,AC_MSKB1)//[11:11]
	#define UHD_PWR_ON_RST_B_PRE  Fld(1,10,AC_MSKB1)//[10:10]
	#define UHD_PWR_MEM_PD_PRE	  Fld(10,0,AC_MSKW10)//[9:0]

UINT8  UhdPowerOn(void)
{

  if ((BSP_GetIcVersion() >= IC_VER_5890_AC) && IS_Support4KDD()&& PANEL_IsSupport4K2KDD() )
  {
	vIO32WriteFldAlign(UHD_PWR_ON, 0x0, UHD_PWR_MEM_PD_PRE);    //bit 9:0
	vIO32WriteFldAlign(UHD_PWR_ON, 0x1, UHD_PWR_ON_RST_B_PRE);  //bit 10	 
	vIO32WriteFldAlign(UHD_PWR_ON, 0x0, UHD_PWR_CLK_DIS_PRE);   //bit 11	 
	vIO32WriteFldAlign(UHD_PWR_ON, 0x0, UHD_PWR_ISO_EN_PRE);    //bit 12	 

	vIO32WriteFldAlign(UHD_PWR_ON, 0x1, UHD_PWR_ON_PRE);        //bit 13	 
	vIO32WriteFldAlign(UHD_PWR_ON, 0x1, UHD_PRE_ON_S_PRE);      //bit 14	 

	vIO32WriteFldAlign(UHD_PWR_ON, 0x1, UHD_4KDD_SELECT);      //bit 17	 
	vIO32WriteFldAlign(UHD_PWR_ON, 0x1, UHD_4KDD_DATA_SELECT); //bit 18
	return TRUE;
  }
  else
    return FALSE;
