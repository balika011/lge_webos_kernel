...
void vDrvChACalonBlankEnb(void)
{
         ...
         /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 1, RG_CALPGAA);//  //MT8222 this is Cal mode.
         ...
         /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_BP_PGABUFA);// // -->Robin 建議 mt8222 改 0
         vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFTA_PWD);//
          /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 1, RG_SHIFT2A_PWD);//// -->Robin 建議 mt8222 改 1
         vIO32WriteFldAlign(VFE_E3, 0, RG_AISEL);//
...
void vDrvChACalonBlankDsb(void)
{
      //recovery the setting of chA in "vDrvTVADCDefaultSetting() ==> operation in clamping on blank"
         ...
         vIO32WriteFldAlign(VFE_E3, 0x0110, RG_AISELR);//
         vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFTA_PWD);////
          /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 0, RG_SHIFT2A_PWD);////
...
void vDrvInputOffsetCal(void)
{
	...
	bCALCLAMP=0x0f;//mt5360, 15->0, slope = positive
       ...
       vIO32WriteFldAlign(VFE_E3, bOFFCUR, RG_OFFCURA);//
            ...
            /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 0x0f, RG_CALCLAMPA); //
            vUtDelay1ms(u4Delay);  
            iA= wDrvReadADCBlankAve();
            /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 0x00, RG_CALCLAMPA); //
            vUtDelay1ms(u4Delay);  
	...
	//vIO32WriteFldAlign(VFE_E1, bCALCLAMP, RG_CALCLAMPA);//original setting

       /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 0x07, RG_CALCLAMPA);////recover to default setting 
       /*mark for V2*/ //vIO32WriteFldAlign(VFE_E1, 0x07, RG_CALCLAMPB);//
...
void vDrvOutputOffsetCalNew(UINT16 wCLAMP)
{
	...
	// **************************
       ...
       vIO32WriteFldAlign(VFE_E3, 0x1F, RG_OFFCURA);//
       vUtDelay1ms(u4Delay);  
       iA= wDrvReadADCBlankAve();
       vIO32WriteFldAlign(VFE_E3, 0x0F, RG_OFFCURA);//
       vUtDelay1ms(u4Delay);  
       iB= wDrvReadADCBlankAve();
       ...
       else
	{
 	 ...
 	 LOG(3,"Calibration Done ,  CVBS output offset value 0x%2x\n",bOFFCUR);
       ...
       vIO32WriteFldAlign(VFE_E3, 0x00, RG_OFFCURA);////MT536X //recover to default setting 
       vIO32WriteFldAlign(VFE_E3, 0x00, RG_OFFCURB);////MT536X //recover to default setting 
...
{  
    ...
    vIO32WriteFldAlign(VFE_E3, 0x00, RG_OFFCURA);//recover to default setting 
    vIO32WriteFldAlign(VFE_E3, 0x00, RG_OFFCURB);//recover to default setting 
    vDrvDFE(256);
...
void vDrvCvbsAdcCal(void) 
{
          ...
          /*mark for V2*/ //vIO32WriteFldAlign(VFE_E2, 3, RG_AAFB_PWD);////???
