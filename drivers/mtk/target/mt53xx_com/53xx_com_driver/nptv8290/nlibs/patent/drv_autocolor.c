...
UINT16 _bInitOffset;
UINT16 _bInitGain;
UINT16 _bAutoColorGain_for_EFuse[4][3];
UINT8 _bInitDigSignBit;
...
UINT8 _bAutoColorGain_for_EFuse[4][3];
  ...
  UINT16 _wADCTarget[3],_wData_v1v2[3],_wADC_Diff_value[3],_wDiff_mim[3],_wBlank[3];
 ...
 */
PRIVATE UINT16 bDrvGetHDADCOffset_Digital(UINT8 bOrder)
{
	switch (bOrder)
	{
		case 0:
			return (IO32ReadFldAlign(HDFE_00, AD1_OFFSET));	//R channel maximum value in a frame
		case 1:
			return (IO32ReadFldAlign(HDFE_00, AD2_OFFSET));	//G channel maximum value in a frame
		case 2:
		default:
			return (IO32ReadFldAlign(HDFE_00, AD3_OFFSET));	//B channel maximum value in a frame
	}
}
...
PRIVATE UINT16 bDrvGetHDADCGain_Digital(UINT8 bOrder)
{
    ...
    {
        case 0:
            return (IO32ReadFldAlign(HDFE_01, AD1_GAIN));	//R channel maximum value in a frame
        case 1:
            return (IO32ReadFldAlign(HDFE_01, AD2_GAIN));	//G channel maximum value in a frame
        case 2:
        default:
            return (IO32ReadFldAlign(HDFE_02, AD3_GAIN));	//B channel maximum value in a frame
    }
}
 void bDrvSetHDADCGain_Digital(UINT8 bOrder, UINT16 value)
{
    ...
    {
        case 0:
            vIO32WriteFldAlign(HDFE_01, value, AD1_GAIN);
            break;
        case 1:
            vIO32WriteFldAlign(HDFE_01, value, AD2_GAIN);
            break;
        case 2:
        default:
            vIO32WriteFldAlign(HDFE_02, value, AD3_GAIN);
            break;
    }
}
...
UINT8 bGainWeighting;
      ...
      if(bUpDown==0)
      {
                    ...
                    {
                       case 0:
                              vIO32WriteFldAlign(HDFE_01, 0x8000-(bGain*bGainWeighting), AD1_GAIN);  // reset the Digital gain to uint gain
                           break;
                       case 1:
                              vIO32WriteFldAlign(HDFE_01, 0x8000-(bGain*bGainWeighting), AD2_GAIN);  // reset the Digital gain to uint gain
                           break;
                       case 2:
                       default:
                              vIO32WriteFldAlign(HDFE_02, 0x8000-(bGain*bGainWeighting), AD3_GAIN);  // reset the Digital gain to uint gain
                              break;
                   }
     }
     else
...
UINT16 wBlankValue;
    ...
    bDataOrder = IO32ReadFldAlign(HDTV_03,HDTV_DATA_SEL);
    if ((bDataOrder<8)&&(bOrder<3))
    {
    wBlankValue= IO32ReadFldAlign(HDTV_STA_06,wHDTV_Data_Sel[bDataOrder][bOrder]);
    return (wBlankValue);
    }
    else
    {
...
PRIVATE void vDrvPreSetToReadBlank(UINT8 bOnOff)
{
    #if ALWAYS_BLANK
      bOnOff = 0;
      vIO32WriteFldAlign(HDTV_03, bOnOff, HDTV_BLAK_SET);
    #else
      vIO32WriteFldAlign(HDTV_03, bOnOff, HDTV_BLAK_SET);
    #endif
...
void vDrvSetHDADCDigitalOffset(UINT8 bOrder,UINT16 value)
{
       ...
       {
           case 0:
       	//vIO32WriteFldAlign(HDFE_00, _bDigSignBit, SIGN_OFFSET1);
       	//vIO32WriteFldAlign(HDFE_00, 0, SIGN_OFFSET1);
		vIO32WriteFldAlign(HDFE_00, value, AD1_OFFSET);    
               break;
           case 1:
       	//vIO32WriteFldAlign(HDFE_00, _bDigSignBit, SIGN_OFFSET2);
       	//vIO32WriteFldAlign(HDFE_00, 0, SIGN_OFFSET2);
		vIO32WriteFldAlign(HDFE_00, value, AD2_OFFSET);          
               break;
           case 2:
       	//vIO32WriteFldAlign(HDFE_00, _bDigSignBit, SIGN_OFFSET3);
       	//vIO32WriteFldAlign(HDFE_00, 0, SIGN_OFFSET3);
		vIO32WriteFldAlign(HDFE_00, value, AD3_OFFSET);          
               break;
       }
}
...
/*static UINT8 vReadSignBit(UINT8 bOrder)
{
    ...
    {
        case 0:
           return(IO32ReadFldAlign(HDFE_00, SIGN_OFFSET1));
        case 1:
           return(IO32ReadFldAlign(HDFE_00, SIGN_OFFSET2));
        case 2:
        default:
           return(IO32ReadFldAlign(HDFE_00, SIGN_OFFSET3));
   }
}*/

static UINT16 bReadHDTVDigitalOffset(UINT8 bOrder)
{
    switch (bOrder)
    {
        case 0:
           return(IO32ReadFldAlign(HDFE_00, AD1_OFFSET));
        case 1:
           return(IO32ReadFldAlign(HDFE_00, AD2_OFFSET));
        case 2:
        default:
           return(IO32ReadFldAlign(HDFE_00, AD3_OFFSET));
   }
}
...
static UINT16 wReadHDTVDigitalGain(UINT8 bOrder)
{
    ...
    {
        case 0:
           return(IO32ReadFldAlign(HDFE_01, AD1_GAIN));
        case 1:
           return(IO32ReadFldAlign(HDFE_01, AD2_GAIN));
        case 2:
        default:
           return(IO32ReadFldAlign(HDFE_02, AD3_GAIN));
   }
}
#endif
UINT16 vCalculateDigOffset(UINT16 bOffset,UINT16 bAdjOffset,UINT8 bCmpResult)
{
    if(bCmpResult==0)
    {
	if((bOffset + bAdjOffset)>0x1FE)
             {
		bOffset=0x1FE;
             }
	else
...
{
  vIO32WriteFldAlign(ASYNC_0F, bMode , AS_PHASE_MAXMIN_SEL);
  switch (bOrder)
  {
    case 0:
	  return (IO32ReadFldAlign(STA_SYNC0_07, AS_RMAXMIND));	//R channel maximum value in a frame
    case 1:
	  return (IO32ReadFldAlign(STA_SYNC0_08, AS_GMAXMIND));	//G channel maximum value in a frame
    case 2:
    default:
	  return (IO32ReadFldAlign(STA_SYNC0_09, AS_BMAXMIND));	//B channel maximum value in a frame
  }
}
...
UINT8 bDrvOnChipGetVFESignalType(UINT8 bAutoInput)
{
  UINT8 bColor_Type;
    ...
    //DBG_Printf(VGA_Debug,"---bAutoInput=%x,  bColor_Type=%x   \n",bAutoInput,bColor_Type);
    return (bColor_Type);
}
  ...
  else
  {
   for(bOrder=0; bOrder<3; bOrder++)
   {
        ...
        if (_bEEPROM_ready==1)
        {
                            ...
                            if(bNextType==INT_SCART)
                            	{
                 UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_SCART0_LOW_BYTE + bOrder) , bTmpLowByte));
                 UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_SCART0_HIGH_BYTE +bOrder) , bTmpHighByte));
                            	
                            	}
                            else
                            	{
                 UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_HDTV0_LOW_BYTE + (bNextType*6)+ (2*bOrder)) , bTmpLowByte));
                 UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_HDTV0_HIGH_BYTE + (bNextType*6)+ (2*bOrder)) , bTmpHighByte));
                            	}
        }
        else
        {
...
UINT8 bGain,bOffset,bType,bOrder;
  for(bType=0; bType<4; bType++)
  {
    ...
    DBG_Printf(VGA_Debug,"check Type=%u, bOrder=%u, bGain=%x, bOffset=%x\n",bType,bOrder,bGain,bOffset);
    }
  }
}
#endif
...
{
UINT8 bCh;
    vIO32WriteFldAlign(PDWNC_VGACFG0, bOnOff, FLD_RG_VMUX_PWD); //to power down VGA MUX    

    vIO32WriteFldAlign(REG_VGA_Normal_CFG0, bOnOff, RG_VDC_N_EN ); // 1:enable "PGA Negative AG Always Connect" 
    for (bCh=0; bCh<3; bCh++)
    {
...
//	  vIO32WriteFldAlign(REG_VGA_Normal_CFG3+(bCh<<4), 1, ADC_1_PWD);	       //power down ADC  // [SA7_Nick] 080525 marked
	  ...
	  vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(bCh*4), 0, RG_VGAADC1_CORE_PWD);	       //power on ADC
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(bCh*4), 0, RG_VGAADC1_DIV_SEL);	
            vIO32WriteFldAlign(REG_VGA_Normal_CFG4+(bCh*4), 0, RG_VGAADC1_IO_PWD);	
// vIO32WriteFldMulti(REG_VGA_Normal_CFG4,P_Fld(0,RG_VGAADC1_IO_PWD)|P_Fld(0,RG_VGAADC2_IO_PWD)|P_Fld(0,RG_VGAADC3_IO_PWD));
      }
    }
 
    vIO32WriteFldAlign(HDTV_03, bOnOff, HDTV_EN);    
    ...
    if(bOnOff==0)     //turn off
    {
              ...
              // vIO32WriteFldMulti(CKGEN_AFEPLLCFG4, P_Fld(0x1, FLD_RG_VGAPLL_CKO_SEL)|P_Fld(0x1, FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
               vIO32WriteFldMulti(REG_PLL_GROUP_CFG7, P_Fld(0x1, FLD_RG_VGAPLL_CKO_SEL)|P_Fld(0x1, FLD_RG_VGAPLL_B_EN)|P_Fld(0x1,FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
               ...
               vIO32WriteFldMulti(REG_PLL_GROUP_CFG7, P_Fld(0x2, FLD_RG_VGAPLL_CKO_SEL)|P_Fld(0x0, FLD_RG_VGAPLL_B_EN)|P_Fld(0x0,FLD_RG_VGAPLL_G_EN)|P_Fld(0x1,FLD_RG_VGAPLL_R_EN));
       #endif
          //vIO32WriteFldAlign(CKGEN_AFEPLLCFG7, 0x0, FLD_RG_VGAPLL_X27IN_EN);
          //vIO32WriteFldAlign(REG_PLL_GROUP_CFG9, 0x0, FLD_RG_VGAPLL_X27IN_EN);
          vIO32WriteFldAlign(REG_PLL_GROUP_CFG7, 0x0, FLD_RG_TL_REFCK_SEL);
    ...
    else
    {
       ...
       vIO32WriteFldAlign(REG_PLL_GROUP_CFG7, 0x0, FLD_RG_VGAPLL_CKO_SEL);
          //vIO32WriteFldAlign(REG_PLL_GROUP_CFG9, 0x1, FLD_RG_VGAPLL_X27IN_EN);
          vIO32WriteFldAlign(REG_PLL_GROUP_CFG7, 0x1, FLD_RG_TL_REFCK_SEL);
       //CKGEN_WRITE32(REG_ADCPLL_CFG0, ((CKGEN_READ32(REG_ADCPLL_CFG0) & 0xFFFFFF3F) | 0x00000000)); //Set ADC clock=27MHz
       vIO32WriteFldAlign(REG_VGA_Normal_CFG0, 1, RG_VDC_P_EN  );   //to power down "ON CHIP auto color buffer and disable 825/450mv,
    }
}
...
void vDrvPreSetToReadBlank(UINT8 bOnOff)
{
    UINT8 bValue;
    bValue = (bOnOff==0) ? 2 : 0 ;
    vIO32WriteFldAlign(HDTV_03, bValue, HDTV_BLAK_SET);
}
*/

PRIVATE void vDrvSelectAutoColorBufferOutputChannel(UINT8 bOrder)
{
  vIO32WriteFldAlign(REG_VGA_Normal_CFG0, 1, RG_VDC_P_EN ); //5360 select channel buffer of ON CHIP AUTO COLOR 
}
   ...
   switch (bMode)
   {
     case ON_CHIP_GAIN_MODE:             //On chip Gain calibration mode
          ...
          vDrvOnChipModeOnOff(1);
                ...
                #endif
          //DBG_Printf(VGA_Debug,"Gain order=%2u, target=0x%x\n",_bOrder,_wADCTarget[_bOrder]);
          break;
	 case OFFSET_MODE:    
           _wADCTarget[_bOrder] = wColorBlankValueNew[bType][_bOrder];
          _bInitOffset= bReadHDTVDigitalOffset(_bOrder);//_bAutoColorOffset[bType][_bOrder];
        ...
        //DBG_Printf(VGA_Debug,"offset order=%2u, target=0x%x\n",_bOrder,_wADCTarget[_bOrder]);
	      break;
     ...
     case GAIN_MODE:    
          ...
          _wADCTarget[_bOrder]  = bDrvGetColorTargetValue(bType, _bOrder)-(_wBlank[_bOrder]>>5); //bColorMaxTypeNew[bType][_bOrder];
          ...
          bDrvSetHDADCGain(_bOrder,_bGain);
                #endif
          vDrvPreSetToReadBlank(AS_BLANK_ALWAYS);
          //DBG_Printf(VGA_Debug,"Gain order=%2u, _bGain=%x, _bInitOffset%x,target=0x%x\n",_bOrder,_bGain,_bInitOffset,_wADCTarget[_bOrder]);
          break;
   #if DIGITAL_NEW_OFFSET
...
void vDrvHDTVMeasureSetting(UINT8 bField_Number)
{
    ...
    vIO32WriteFldAlign(HDTV_02, bField_Number  , HDTV_BLK_CALI_FCNT);
...
UINT8 bTMP1[3];
    ...
    if(bType<=INT_VGA_COMPOENT)   //for klocwork check
    {
            ...
            vIO32WriteFldAlign(HDTV_02, bTMP1[0] , HDTV_BLK_CALI_Y_TAR); 
            vIO32WriteFldAlign(HDTV_02, bTMP1[1] , HDTV_BLK_CALI_PB_TAR); 
            vIO32WriteFldAlign(HDTV_02, bTMP1[2] , HDTV_BLK_CALI_PR_TAR); 
            vIO32WriteFldAlign(HDTV_00, 0x02  , HDTV_BLK_CALI_MIN);    //mimumu:2 code 10bit
            vIO32WriteFldAlign(HDTV_02, 0x3f  , HDTV_BLK_CALI_THRES); //maximum:16 code 8bit
            vIO32WriteFldAlign(HDTV_02, CALI_ENABLE  , HDTV_BLK_CALI_EN);          //Enable line blank average measurement function
            ...
            vIO32WriteFldAlign(HDTV_02, CALI_ENABLE  , HDTV_BLK_CALI_EN);           //Enable line blank average measurement function
            vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_ADJ_ON); //Disable HW blank Adjust Function
            vIO32WriteFldAlign(HDTV_00, CALI_DISABLE   , HDTV_LCLAMP_EN);           //Disable line clamp
            vIO32WriteFldAlign(HDTV_00, BEFORE_ADJ, HDTV_BLK_STA_SEL);
      #else                             //No HDTV clamping function and No HDTV HW blank level measurement method
            vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_EN);         //Disable line blank average measurement function
            vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_ADJ_ON); //Disable HW blank Adjust Function
            vIO32WriteFldAlign(HDTV_00, CALI_DISABLE   , HDTV_LCLAMP_EN);           //Disable line clamp
...
UINT8 bTMP1[3];
       ...
       vIO32WriteFldAlign(HDTV_02, bMode  , HDTV_BLK_CALI_ADJ_ON);                      //Enable or disable  HW blank Adjust Function
       vIO32WriteFldAlign(HDTV_02, bMode, HDTV_BLK_CALI_EN);                             //Enable line blank average measurement function
       vIO32WriteFldAlign(HDTV_00, AFTER_ADJ, HDTV_BLK_STA_SEL);   //blank level after calibration
}
#endif
...
void vDrvHDTVSelBlankDataSource(void)
{
       ...
       vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_ADJ_ON);                      //Disable HW blank Adjust Function
       #if HW_MEASURE_BLANK
       vIO32WriteFldAlign(HDTV_00, BEFORE_ADJ   , HDTV_BLK_STA_SEL);                                 //blank level after calibration
       #else
       vIO32WriteFldAlign(HDTV_00, AFTER_ADJ   , HDTV_BLK_STA_SEL);                                 //blank level after calibration
       #endif
       //vIO32WriteFldAlign(HDTV_02, CALI_ENABLE  , HDTV_BLK_CALI_ADJ_ON);                      //Disable HW blank Adjust Function
       //vIO32WriteFldAlign(HDTV_00, BEFORE_ADJ   , HDTV_BLK_STA_SEL);                                 //blank level after calibration
       vIO32WriteFldAlign(HDTV_02, CALI_ENABLE  , HDTV_BLK_CALI_EN);                             //Enable line blank average measurement function
          ...
          #if HW_MEASURE_BLANK
                vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_ADJ_ON);                          //disable HW blank Adjust Function
                vIO32WriteFldAlign(HDTV_00, BEFORE_ADJ   , HDTV_BLK_STA_SEL); //output line average value
                vIO32WriteFldAlign(HDTV_02, CALI_ENABLE  , HDTV_BLK_CALI_EN);                             //Enable line blank average measurement function
          #else
                vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_ADJ_ON);                          //disable HW blank Adjust Function
                vIO32WriteFldAlign(HDTV_00, AFTER_ADJ   , HDTV_BLK_STA_SEL);
                vIO32WriteFldAlign(HDTV_02, CALI_DISABLE  , HDTV_BLK_CALI_EN);                           //disable line blank average measurement function
          #endif
#endif          
}
...
{
     if ((IO32ReadFldAlign(HDTV_00,HDTV_BLK_STA_SEL)==BEFORE_ADJ)  && (IO32ReadFldAlign(HDTV_02,HDTV_BLK_CALI_EN)==CALI_ENABLE))
     {
       return 1;
     }
     else
...
PRIVATE void vDrvOnChipAutoColorGetADCData(UINT8 bMode)
{
 ...
 UINT8  bLoopCnt,bSelV1V2;//,bGainOffsetMode;  //dwSumADCRawData2 = u4IO32Read4B(REG_VGA_Normal_CFG0+(4*bIndexCnt));
 if(_bOrder<3)  //for klocwork check
 {
    switch (bMode)
    {
	case ON_CHIP_GAIN_MODE:
	        //DBG_Printf(VGA_Debug,"-----ON CHIP GAIN MODE----------\n");
	        ...
	        //DBG_Printf(VGA_Debug,"Select buffer=%x, register=0x%x, data=0x%x\n",_bOrder,REG_VGA_Normal_CFG3,dwSumADCRawData2);
            ...
            //DBG_Printf(VGA_Debug,"Channel=%x, SUM1=%x, SUM2=%x\n",_bOrder,wSumADC[_bOrder][0],wSumADC[_bOrder][1]);
	...
	case GAIN_MODE:
              dwSumADCRawData = 0;
               for(bLoopCnt = 0; bLoopCnt<32; bLoopCnt++)
                 {
                      dwSumADCRawData += bDrvOnChipGetADCMaxMinValue(_bOrder,AS_PHASE_MAX_SEL);   //to read the blank level of Y, Pb, Pr respective
	              vUtDelay2us(80);
                }
           _wData_v1v2[_bOrder] = dwSumADCRawData/32; //bDrvOnChipGetADCMaxMinValue(_bOrder,AS_PHASE_MAX_SEL);
            break;

	default:
	    break;
    }
    //DBG_Printf(VGA_Debug,"-- channel %u sample data=%x\n",_bOrder,_wData_v1v2[_bOrder]);
  }
}
...
if((_bOrder<3) && (bMode<=DIGITAL_OFFSET_MODE))  //for Kclockwork check
{
       ...
       if(bMode==GAIN_MODE)
       {
			wAdcBlankLevel=bDrvGetBlankVal(_bOrder);
                 if((wAdcBlankLevel<=(_wBlank[_bOrder]>>3)+(bOnChipCheckTolerance[OFFSET_MODE][1]+bTolenence))&&(wAdcBlankLevel>=(_wBlank[_bOrder]>>3)-(bOnChipCheckTolerance[OFFSET_MODE][1]+bTolenence)))
                  	{
          ...
          switch (bGainOffsetMode)
          {
			...
			if(_bGainIncDir[_bOrder]==0)
			{
				...
				/*else
				{
	       			...
	       			{
	             				_bCaliStep[_bOrder] = (_wADCTarget[_bOrder]-(_wData_v1v2[_bOrder]>>1)); //transfer to 10 bit base
	       			}
	       			else
	       			{
	            				_bCaliStep[_bOrder] = (_wADCTarget[_bOrder]-_wData_v1v2[_bOrder])>>3; //transfer to 10 bit base
	       			}
...
//				}
			...
			if (bMode == GAIN_MODE)
			{
						_bCaliStep[_bOrder]=((_wADCTarget[_bOrder] <<2)-(_wData_v1v2[_bOrder]<<2))*40;
			...
			else
			{
       			...
       			else
       			{
       				//if(_bOrder>0)
       					{
	             				_bCaliStep[_bOrder] = ((_wADCTarget[_bOrder]-_wData_v1v2[_bOrder])>>3); //transfer to 8 bit base
       				}
       				//else
       				//	{
	             			//	_bCaliStep[_bOrder] = ((_wADCTarget[_bOrder]-_wData_v1v2[_bOrder])>>3)<<1; //transfer to 8 bit base
       				//	}
       				}
	       			//if(_wData_v1v2[_bOrder]<5)
	       			//{
...
/*				else
				{
       				...
       				{
           						_bCaliStep[_bOrder] =_bGain/3*2;// (_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>1; //transfer to 10 bit base
           					
       				}
       				else if (_wData_v1v2[_bOrder]>(_wADCTarget[_bOrder]<<1))
       				{
           						_bCaliStep[_bOrder] =_wData_v1v2[_bOrder]>>5;// (_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>1; //transfer to 10 bit base
           					
       				}
       				else
       				{
          					 _bCaliStep[_bOrder] = (_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>3; //transfer to 10 bit base
       				}
		      ...
		      _bGainIncDir[_bOrder] = 0;
                    if (bMode == GAIN_MODE)
                    {
        			_bCaliStep[_bOrder]=((_wData_v1v2[_bOrder] <<2)-(_wADCTarget[_bOrder]<<2))*40;
				...
				else
				{
				  _bGain -=0x80;
       			...
       			else
       			{
       				//if(_bOrder>0)
       				{
	             				_bCaliStep[_bOrder] = ((_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>3); //transfer to 8 bit base
       				}
       				//else
       				//{
	             			//	_bCaliStep[_bOrder] = ((_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>3)<<1; //transfer to 8 bit base
       				//}

       			}
      				//if(_wData_v1v2[_bOrder]>(_wADCTarget[_bOrder]<<2))
       			//	{
           			//			_bCaliStep[_bOrder] =(_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])/10;// (_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>1; //transfer to 10 bit base
       			...
       			//	}
       				//else if (_wData_v1v2[_bOrder]>(_wADCTarget[_bOrder]<<1))
       				//{
           				//		_bCaliStep[_bOrder] =(_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])/5;// (_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>1; //transfer to 10 bit base
...
void vDrvMeasureMaxBlankLevel(void)
{
   ...
   _dwMaxLevelAvg[2] += _wData_v1v2[2] ;
    ...
    #if AUTO_BLANK_MEASURE
      DBG_Printf(VGA_Debug,"MAX 1=%3u, Max2=%3u, Max3=%3u\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
   #endif
...
void vDrvShowDigGainOffset(UINT8 bOrder)
   {
         ...
         if(_bDigSignBit==0)
         {
             ...
             {
                 DBG_Printf(VGA_Debug,"After BlkAdj: Digital gain & offset: CH=%u, DGain=0x%x, Doffset=%d\n",bOrder,wDgain,(INT8)(0-bDOffset));
             }
             else if(_bAdjBlkState0 == BLK_START)
             {
                 DBG_Printf(VGA_Debug,"Before BlkAdj: Digital gain & offset: CH=%u, DGain=0x%x, Doffset=%d\n",bOrder,wDgain,(INT8)(0-bDOffset));
             }
             else
            {
                 DBG_Printf(VGA_Debug,"Digital gain & offset: CH=%u, DGain=0x%x, Doffset=%d\n",bOrder,wDgain,(INT8)(0-bDOffset));
            }
         }
         else
        {
...
void vDrvShowAnalogGainOffset(UINT8 bOrder)
{
         ...
         if (_bAdjBlkState0 == BLK_MEASURE)
         {
             DBG_Printf(VGA_Debug,"After BlkAdj: analog offset : CH=%u, Aoffset=0x%x\n",bOrder,bAOffset);
         }
         else if (_bAdjBlkState0 == BLK_START)
         {
             DBG_Printf(VGA_Debug,"Before BlkAdj: analog offset : CH=%u, Aoffset=0x%x\n",bOrder,bAOffset);
          }
         else
         {
             DBG_Printf(VGA_Debug,"analog offset : CH=%u, Aoffset=0x%x\n",bOrder,bAOffset);
        }
         #else
         UINT8 bAgain;
         UINT8 bAOffset;
         ...
         if (_bAdjBlkState0 == BLK_MEASURE)
         {
             DBG_Printf(VGA_Debug,"After BlkAdj: Analog gain & offset : CH=%u, AGain=0x%x, Aoffset=0x%x\n",bOrder,bAgain,bAOffset);
         }
         else if (_bAdjBlkState0 == BLK_START)
         {
             DBG_Printf(VGA_Debug,"Before BlkAdj: Analog gain & offset : CH=%u, AGain=0x%x, Aoffset=0x%x\n",bOrder,bAgain,bAOffset);
          }
         else
         {
             DBG_Printf(VGA_Debug,"Analog gain & offset : CH=%u, AGain=0x%x, Aoffset=0x%x\n",bOrder,bAgain,bAOffset);
        }
     #endif
}                  
#endif
...
UINT8 bDigital_gain;
 ...
 if(_bOrder<3)  //for klocwork check
 {
    ...
    //DBG_Printf(VGA_Debug,"Channel %x ,Current offset=%x\n",_bOrder, bTempOffset [_bOrder]);
    vDrvOnChipAutoColorGetADCData(OFFSET_MODE);
    vDrvOnChipAutoColorModeSet(OFFSET_MODE,_bType);
        ...
        {
           case 0:
               bDigital_gain = (IO32ReadFldAlign(HDFE_00, AD1_OFFSET));
               break;
           case 1:
               bDigital_gain = (IO32ReadFldAlign(HDFE_00, AD2_OFFSET));
               break;
           case 2:
           default:
               bDigital_gain = (IO32ReadFldAlign(HDFE_00, AD3_OFFSET));
               break;
        }
      DBG_Printf(VGA_Debug,"AnalogAdjBlk : CH %u, target=0x%x, current=0x%x, Digital offset=0x%x, Analog offset=0x%x\n",_bOrder, _wADCTarget[_bOrder],_wData_v1v2[_bOrder],bDigital_gain,bTempOffset [_bOrder]);
    #endif
      ...
      if(_wData_v1v2[_bOrder]>(_wADCTarget[_bOrder]+_bDiffOffset)) // 0X08=10 bit 1 code
      {
	      ...
	      bCalibration_direction = DECREASE_OFFSET;
            DBG_Printf(VGA_Debug,"=== Current channel %x BLANK LEVEL=0x%x is higher than Target level=0x%x : difference is 0x%x===\n",_bOrder,_wData_v1v2[_bOrder],_wADCTarget[_bOrder],bAdjOffset);
            }
            else
            {
            bAdjOffset = (UINT8)((_wADCTarget[_bOrder]-_wData_v1v2[_bOrder])>>3);
	      bCalibration_direction = INCREASE_OFFSET;
            DBG_Printf(VGA_Debug,"=== Current channel %x BLANK LEVEL=0x%x is smaller than Target level=0x%x : difference is 0x%x===\n",_bOrder,_wData_v1v2[_bOrder],_wADCTarget[_bOrder],bAdjOffset);
      }
      ...
      if(bCalibration_direction==DECREASE_OFFSET) // 0X08=10 bit 1 code
      {
         ...
         {
           //bAdjOffset = (UINT8)((_wData_v1v2[_bOrder]-_wADCTarget[_bOrder])>>3);  //change to 10bit base
            DBG_Printf(VGA_Debug,"To decrease channel %x, bAdjOffset=%x\n",_bOrder,bAdjOffset);
      ...
      else if(bCalibration_direction==INCREASE_OFFSET)
      {
         ...
         {
            //bAdjOffset = (UINT8)((_wADCTarget[_bOrder]-_wData_v1v2[_bOrder])>>3);
            DBG_Printf(VGA_Debug,"To increase channel%x, bAdjOffset=%x\n",_bOrder,bAdjOffset);
      ...
      else
      {
         if(_bDoubleChkFlag[_bOrder]==1)
         {
          ...
          _bGainOffset_channel |= (0x01 << _bOrder);
            #ifdef FULLY_HW_AUTO_CALIBRATION
                 DBG_Printf(VGA_Debug,"After vDrvAnalogOffsetAdjBlk, Blank leve is : CH=%u,  blank level=%u\n", _bOrder,(_wData_v1v2[_bOrder]>>3));
            #endif
         }
         else
         {
...
UINT8 bCmpResult,bAdjOffset;
      ...
      //DBG_Printf(VGA_Debug,"  vDrvDigitalOffsetAdjBlk, CH=%u, Current blank level=0x%x\n",_bOrder,(_wData_v1v2[_bOrder]>>3));
if(_bOrder<3)  //only for Klocwork check
{
    ...
    {     
      #if AUTO_BLANK_DEBUG
      DBG_Printf(VGA_Debug,"vDrvDigitalOffsetAdjBlk : CH %u, target=0x%x, current=0x%x, Digital offset=0x%x, Analog offset=0x%x\n",_bOrder,_wADCTarget[_bOrder],_wData_v1v2[_bOrder],bReadHDTVDigitalOffset(_bOrder),bDrvGetHDADCOffset(_bOrder));
      #endif       
       if(_wData_v1v2[_bOrder]>(_wADCTarget[_bOrder]+0x8)) // 0X10=10 bit 2 code
       {
...
void vDrvResetAllGainOffset(void)
{
     if(_bType<=INT_VGA_COMPOENT)   //for klocwork check
     {
       ...
       #ifdef AUTOCOLOR_NEW_METHOD
       #else
              vIO32WriteFldAlign(HDFE_01, 0x8000, AD1_GAIN);  // reset the Digital gain to uint gain
...
void vPrint_Input_Type(void)
{
            ...
            {
                case INT_HDTV:
                    DBG_Printf(VGA_Debug,"Input sourec is YPbPr\n");
                    break;
                case INT_VGA:
                    DBG_Printf(VGA_Debug,"Input sourec is VGA\n");
                    break;
                case INT_SCART:
                    DBG_Printf(VGA_Debug,"Input sourec is SCART\n");
                    break;
                 default:
                    DBG_Printf(VGA_Debug,"Input sourec is Other\n");
                    break;
            }
}
       ...
       if(_bType==INT_VGA)
       {
           for(_bOrder=0; _bOrder<3; _bOrder++)
	    {
		        ...
		        dwOrgGain = bDrvGetHDADCGain(_bOrder);
                 ...
                 #if COMPENSATE_BY_ANALOG_GAIN
                      //dwTmpGain = (UINT32)((255+dwOrgGain)*(512-(wColorBlankValueNew[_bType][_bOrder]>>3))*0x1000/(512-_wBlanklevel_before_adj[_bOrder])-(255*0x1000));
                      //DBG_Printf(VGA_Debug,"_bOrder=%2u, Blanklevel_before_adj=0x%3x\n",_bOrder,_wBlanklevel_before_adj[_bOrder]);
	               ...
	               bGAIN=dwTmpGain;
                      ...
                      if(_bOrder==0)
                      {
                           vIO32WriteFldAlign(HDFE_01, dwTmpGain, AD1_GAIN);  // reset the Digital gain to uint gain
                      }
                      else if(_bOrder==1)
                      {
                           vIO32WriteFldAlign(HDFE_01, dwTmpGain, AD2_GAIN);	
                      }
                      else
                      {
                           vIO32WriteFldAlign(HDFE_02, dwTmpGain, AD3_GAIN);
                      }
                 #endif
                  }
                  #if COMPENSATE_BY_ANALOG_GAIN
                  //DBG_Printf(VGA_Debug,"  \n");
                  //DBG_Printf(VGA_Debug,"ch=%u, autocolor_gain=%u, Final gain=%u\n",_bOrder,dwOrgGain,bDrvGetHDADCGain(_bOrder));
                  #endif
            }
            #if COMPENSATE_BY_ANALOG_GAIN
            #else
            //DBG_Printf(VGA_Debug,"  \n");
            //DBG_Printf(VGA_Debug,"Final digital gain CH1=0x%x,CH2=0x%x,CH3=0x%x\n",IO32ReadFldAlign(HDFE_01,AD1_GAIN),IO32ReadFldAlign(HDFE_01,AD2_GAIN),IO32ReadFldAlign(HDFE_02,AD3_GAIN));
            #endif
        }
}
#endif
...
//#endif
   ...
   else
   {
       ...
       else
       {
           ...
           {
               DBG_Printf(VGA_Debug,"---Input type is changed. to restart the adjustment flow ----\n");
    ...
    {
        DBG_Printf(VGA_Debug,"=== state of adjust blank =%x\n",_bAdjBlkState0);
    ...
    {  
           ...
           else
           {
			 ...
			 _fgblankfinish=1;
               ...
               else
               {
                          ...
                          {
                             DBG_Printf(VGA_Debug,"Pre-check, Blank level: CH1=%3u,  CH2=%3u, CH3=%3u\n",_dwBlankAvg[0],_dwBlankAvg[1],_dwBlankAvg[2]);
                    ...
                    if( (_bAutoColorTimeOutCNT<ADJUST_MAX_TIME)&& ((_bGainOffset_channel&0x07) != 0x07))
	             {
                     ...
                     else  
                     {
                            ...
                            if(bDrvEFuseOffsetREADY()==SV_FALSE)  // if never do offset then set the flag and store the offset
                            {
                                 ...
                                 {
                                 	  #ifdef AUTOCOLOR_NEW_METHOD
                                     _bAutoColorOffset[_bType][_bOrder] = bDrvGetHDADCOffset_Digital(_bOrder);
                                 	  #else
                                 ...
                                 //vDrvEnableBlankLevelAdjust();
                                 DBG_Printf(VGA_Debug," Offset_calibration spends %u times\n",(_bAutoColorTimeOutCNT-1));
                                 vDrvHDTVMeasureSetting(UPDATE_PER_1_FIELD);
                                 vDrvHDTV_HW_AUTO_ONOFF(CALI_ENABLE,_bType);
 			 		_fgblankfinish=1;
                                 _bAdjBlkState0= BLK_NOTHING;    //no need to jump to _bAdjBlkState0= BLK_CLAMP_DELAY;
        ...
        case BLK_OFFSET_CHECK:
           ...
           {   
               vDrvCalculateMaxBLKAvg();
               //DBG_Printf(VGA_Debug," Before check the _bGainOffset_channel=%u\n",_bGainOffset_channel);
               for(_bOrder=0; _bOrder<3; _bOrder++)
               {
                   #if COMPENSATE_GAIN
            ...
            if (_bBlankIteration==0)
               {
                  DBG_Printf(VGA_Debug,"     \n");
                  DBG_Printf(VGA_Debug,"before Blank adjust, Blank level: CH1=%3u,  CH2=%3u, CH3=%3u\n",_dwBlankAvg[0],_dwBlankAvg[1],_dwBlankAvg[2]);
                  DBG_Printf(VGA_Debug,"before Blank adjust, Maximum level: CH1=%3u,  CH2=%3u, CH3=%3u\n",_dwMaxLevelAvg[0],_dwMaxLevelAvg[1],_dwMaxLevelAvg[2]);
                  #if COMPENSATE_GAIN
               ...
               {
               #if  AUTO_BLANK_DEBUG
                  DBG_Printf(VGA_Debug,"After Blank adjust, Blank leve is : CH1=%u,  CH2=%u, CH3=%u\n",_dwBlankAvg[0],_dwBlankAvg[1],_dwBlankAvg[2]);
                  DBG_Printf(VGA_Debug,"_bBlankIteration=%x,Blank adjust tolerance is %x, 0x08 means 1 code(10bit)\n",_bBlankIteration,_bDiffOffset);
               #endif
               }
              _bBlankIteration++;
              ...
              {
                   _bAdjBlkState0 = BLK_OFFSET_ADJ;
                   DBG_Printf(VGA_Debug,"--- NEED_TO_DO_ANALOG_CALIBRATION ---\n");                  
              }
              else
              {
              #ifdef FULLY_HW_AUTO_CALIBRATION
                   DBG_Printf(VGA_Debug,"--- NO_NEED_TO_DO_ANALOG_CALIBRATION ---\n");
                   vDrvHDTVMeasureSetting(UPDATE_PER_1_FIELD);
                   vDrvHDTV_HW_AUTO_ONOFF(CALI_ENABLE,_bType);
		     _fgblankfinish=1;
	             _bAdjBlkState0= BLK_NOTHING;    //no need to jump to _bAdjBlkState0= BLK_CLAMP_DELAY;
        ...
        case BLK_MEASURE:
                  ...
                  for(_bOrder=0; _bOrder<3; _bOrder++)
                    {
                       ...
                       _dwMaxLevelAvg[_bOrder] = (_dwMaxLevelAvg[_bOrder]+(_bAutoColorTimeOutCNT/2) )/_bAutoColorTimeOutCNT;
                       //#if AUTO_BLANK_DEBUG
                       //DBG_Printf(VGA_Debug,"CH=%u,Offset=%x\n",_bOrder,bDrvGetHDADCOffset(_bOrder));
                       //#endif
                       if (_bType<=INT_VGA_COMPOENT) //for klocwork check  
                       {
                           if(_bDoubleCheck > 0)
               ...
               #if AUTO_BLANK_DEBUG
               DBG_Printf(VGA_Debug,"After digital gain ajust Blank level (10 bit) :CH1=%3u, CH2=%3u, CH3=%3u\n",_dwBlankAvg[0],_dwBlankAvg[1],_dwBlankAvg[2]);
               DBG_Printf(VGA_Debug,"After digital gain adjust, Maximum level : Max1=%3u, Max2=%3u, Max3=%3u\n",_dwMaxLevelAvg[0],_dwMaxLevelAvg[1],_dwMaxLevelAvg[2]);
               #endif
                   if(((_bGainOffset_channel & 0x07) != 0x07)  && (_bDoubleCheck>0))
                   {
                      _bAdjBlkState0= BLK_OFFSET_ADJ;
                      #if AUTO_BLANK_DEBUG
                      DBG_Printf(VGA_Debug,"go back to OFFSET adjust\n");
                      #endif
                      vDrvBlankAdjParaReset();
                      _bBlankIteration = 1;
                   ...
                   {
                     DBG_Printf(VGA_Debug,"Blank level adjust %u times\n",(ADJUST_BLANK_RETRY_COUNT-_bDoubleCheck+1));
                     DBG_Printf(VGA_Debug,"After vDrvAdjustBlankLevel, Blank level (10 bit) :CH1=%3u, CH2=%3u, CH3=%3u\n",_dwBlankAvg[0],_dwBlankAvg[1],_dwBlankAvg[2]);
                     DBG_Printf(VGA_Debug,"After vDrvAdjustBlankLevel, Maximum level (8 bit) :CH1=%u, CH2=%u, CH3=%u\n",_dwMaxLevelAvg[0],_dwMaxLevelAvg[1],_dwMaxLevelAvg[2]);
                    #if  AUTO_BLANK_DEBUG
                          _bType = bDrvOnChipAutoColorCheckSignalReady();
                          if(_bType<=INT_VGA_COMPOENT)
                          {
                               DBG_Printf(VGA_Debug,"Target blank level (10 bit):CH1=%3u, CH2=%3u, CH3=%3u\n",(wColorBlankValueNew[_bType][0]>>3),(wColorBlankValueNew[_bType][1]>>3),(wColorBlankValueNew[_bType][2]>>3));
                               DBG_Printf(VGA_Debug,"Target Maximum (8 bit):CH1=%4u, CH2=%4u, CH3=%4u\n",bDrvGetColorTargetValue(_bType,0),bDrvGetColorTargetValue(_bType,1),bDrvGetColorTargetValue(_bType,2));
                          }
                          for (_bOrder=0; _bOrder<3; _bOrder++)
                          {                    
                               vDrvShowDigGainOffset(_bOrder);
...
UINT8 vCust_Current_Cal_Status(void)
{
...
UINT16 wAdcBlankLevel[3];
  ...
  if((_bType <=INT_VGA_COMPOENT) && (bMode<=DIGITAL_OFFSET_MODE))   //for klocwork check
  {
       ...
       for(_bOrder=0;  _bOrder<3; _bOrder++)
       {
        ...
        DBG_Printf(VGA_Debug,"--- Check Gain/ Offset --\n");
     #endif
     for (_bOrder=0;  _bOrder<3; _bOrder++)
    {
	 //Printf("ch1=%x, ch2=%x, ch3=%x\n",_wData_v1v2[0] ,_wData_v1v2[1], _wData_v1v2[2] );
       ...
       if(bMode==GAIN_MODE)
       {
                 if((wAdcBlankLevel[_bOrder]<=_wBlank[_bOrder]+2)&&(wAdcBlankLevel[_bOrder]>=_wBlank[_bOrder]-2))
                   	{
                  	wAdcBlankLevel[_bOrder]=_wBlank[_bOrder];
                     }
      		_wData_v1v2[_bOrder]=((_wData_v1v2[_bOrder]<<5)-(wAdcBlankLevel[_bOrder]));   //gain-blank  13bit
      }
         #endif

           if(bMode==GAIN_MODE)
           {
        		if ((_wData_v1v2[_bOrder] >=( (_wADCTarget[_bOrder]<<5)- (bOnChipCheckTolerance[bMode][1]+bTolenence)))&&(_wData_v1v2[_bOrder] <= ((_wADCTarget[_bOrder]<<5) + (bOnChipCheckTolerance[bMode][0]+bTolenence)))) 
        		{
          			_bGainOffset_channel |= (0x01 << _bOrder);
			       ...
			       #if (AUTO_COLOR_DEBUG|| ON_CHIP_AUTO_COLOR_DEBUG)
		                 if(bMode==OFFSET_MODE)  {  DBG_Printf(VGA_Debug,"channel %u is OK, Blank=%u\n",(_bOrder),(_wData_v1v2[_bOrder])>>5);    }
		                 else {    DBG_Printf(VGA_Debug,"channel %u is OK, MaxLevel=%u\n",(_bOrder),(_wData_v1v2[_bOrder]));    }
			      #endif
	    		}
			else
			{
				  _bGainOffset_channel &= (~(0x01 << _bOrder));
			       #if (AUTO_COLOR_DEBUG|| ON_CHIP_AUTO_COLOR_DEBUG)
				    if(bMode==OFFSET_MODE)   {  DBG_Printf(VGA_Debug,"channel %u need cali, Blank=%u\n",(_bOrder),(_wData_v1v2[_bOrder])>>5); }
		                    else                     {  DBG_Printf(VGA_Debug,"channel %u need cali, MaxLevel=%u\n",(_bOrder),(_wData_v1v2[_bOrder]));  }
		              #endif
            		}
...
UINT16 wMeasure_End_point;
        ...
        if(location==0)
        {
          DBG_Printf(VGA_Debug,"H total=%u,  BP=%u,  ACT=%u\n",wHor_total,wBack_porch,wActive_area);  
        }
        #endif
        ...
        #if PGA_LINEARITY_DEBUG
        DBG_Printf(VGA_Debug,"pre-margin=%u,  post-margin=%u\n",wMeasure_Start_point,wMeasure_End_point);
        #endif
}
#endif
...
UINT8 i,j;
    ...
    if(_bType <=INT_VGA_COMPOENT)    //for klocwork check
    {
			...
			wGain_LOW_limit = GAIN_LOW_LIMIT_100_DIGITAL[_bType][_bOrder];
                    ...
                    if((wTmpGain<wGain_High_limit) && (wTmpGain > wGain_LOW_limit))
                    {
        			  ...
        			  //Printf("CH %x, Final GAIN value=0x%x, Hi_lim=0x%x, Low_lim=0x%x\n",_bOrder,wTmpGain,wGain_High_limit,wGain_LOW_limit);
                            ...
                            if(_bType==INT_SCART)
                            	{
                 			UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_SCART0_LOW_BYTE + _bOrder) , bTmpLowByte));
                 			UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_SCART0_HIGH_BYTE +_bOrder) , bTmpHighByte));
                ...
                //Printf("CH%x, Offset value=%x, Gain value=%x\n",_bOrder,_bAutoColorOffset[_bType][_bOrder],_bAutoColorGain[_bType][_bOrder]);
	             ...
	             #if  AUTO_COLOR_DEBUG
	                  DBG_Printf(VGA_Debug,"Gain HIGH limit=0x%x ,LOW limit=0x%x\n", bGain_High_limit,bGain_LOW_limit);
	             #endif
		...
		#if  AUTO_COLOR_DEBUG 
			 DBG_Printf(VGA_Debug,"The auto color result=%x\n",bCaliResult);
		#endif
		      DBG_Printf(VGA_Debug,"==== Iteration count=%u  ===\n",_bAutoColorTimeOutCNT);
              ...
              if((bCaliResult==0x07)&&(_bAutoColorTimeOutCNT<AUTO_COLOR_MAX_TIME))
              {
          		//_bInitDigSignBit = vReadSignBit(_bOrder); 
					...
					for(_bOrder=0;_bOrder<3;_bOrder++)
					{
                            		bTmpHighByte = (_bAutoColorGain[1][_bOrder]>>8)&0x03;
                            		bTmpLowByte = _bAutoColorGain[1][_bOrder] & 0xff;
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_DIG_GAIN_HDTV0_LOW_BYTE + (1*3)+(2*_bOrder), bTmpLowByte));
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_DIG_GAIN_HDTV0_HIGH_BYTE + (1*3)+(2*_bOrder), bTmpHighByte));
                            		bTmpHighByte = ( bVGADefaultOffsetTABLE_100[1][_bOrder]>>8)&0x03;
                            		bTmpLowByte =  bVGADefaultOffsetTABLE_100[1][_bOrder] & 0xff;
					 UNUSED(fgApiEepromWriteByte(EEP_VIDEO_AUTO_COLOR_START + (1*3)+_bOrder, bTmpHighByte)); 
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_OFFSET_START + (1*3)+_bOrder, bTmpLowByte)); 
					}
					...
					for(_bOrder=0;_bOrder<3;_bOrder++)
					{
                            		bTmpHighByte = (_bAutoColorGain[2][_bOrder]>>8)&0x03;
                            		bTmpLowByte = _bAutoColorGain[2][_bOrder] & 0xff;
                 			UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_SCART0_LOW_BYTE + _bOrder) , bTmpLowByte));
                 			UNUSED(fgApiEepromWriteByte((EEP_ADC_DIG_GAIN_SCART0_HIGH_BYTE +_bOrder) , bTmpHighByte));
                           		bTmpHighByte = ( bVGADefaultOffsetTABLE_100[2][_bOrder]>>8)&0x03;
                            		bTmpLowByte = bVGADefaultOffsetTABLE_100[2][_bOrder] & 0xff;
					 UNUSED(fgApiEepromWriteByte(EEP_VIDEO_AUTO_COLOR_START + (2*3)+_bOrder, bTmpHighByte)); 
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_OFFSET_START + (2*3)+_bOrder, bTmpLowByte)); 
					}
					...
					for(_bOrder=0;_bOrder<3;_bOrder++)
					{
                            		bTmpHighByte = (_bAutoColorGain[3][_bOrder]>>8)&0x03;
                            		bTmpLowByte = _bAutoColorGain[3][_bOrder] & 0xff;
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_DIG_GAIN_HDTV0_LOW_BYTE + (3*3)+(2*_bOrder), bTmpLowByte));
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_DIG_GAIN_HDTV0_HIGH_BYTE + (3*3)+(2*_bOrder), bTmpHighByte));
                            		bTmpHighByte = (bVGADefaultOffsetTABLE_100[3][_bOrder]>>8)&0x03;
                            		bTmpLowByte = bVGADefaultOffsetTABLE_100[3][_bOrder] & 0xff;
					 UNUSED(fgApiEepromWriteByte(EEP_VIDEO_AUTO_COLOR_START + (3*3)+_bOrder, bTmpHighByte)); 
					 UNUSED(fgApiEepromWriteByte(EEP_ADC_OFFSET_START + (3*3)+_bOrder, bTmpLowByte)); 
					}
					UNUSED(fgApiEepromWriteByte(EEP_VIDEO_AUTO_COLOR_HDTV_DONE+3,SV_TRUE));
                       }         	
                       DBG_Printf(VGA_Debug,"==== Auto color mapping finished  VGA Gain1=%2x, Gain2=%2x, Gain3=%x ====\n", _bAutoColorGain[1][0],_bAutoColorGain[1][1],_bAutoColorGain[1][2]);
                    }//
                    
                    DBG_Printf(VGA_Debug,"==== Auto color success  ===\n");
               }
               else
               {
                    if (_bEEPROM_ready==1)
                    ...
                    DBG_Printf(VGA_Debug,"==== Auto color fail !!!!  ===\n");
                }
          //DBG_Printf(VGA_Debug,"auto color history: HDTV=%x, VGA=%x, SCART=%x, VGACOMP=%x\n",_bAutoColorHistory[0],_bAutoColorHistory[1],_bAutoColorHistory[2],_bAutoColorHistory[3]);
          #if EEPROM_READY             
                ...
                bType = _bType;
                          for (bType=0; bType<4; bType++)
                          {
                            	...
                            	if(bType==INT_SCART)
                            	{
                 				bTmpLow[bCh] =  bApiEepromReadByte(EEP_ADC_DIG_GAIN_SCART0_LOW_BYTE + bCh) ;
                 				bTmpHigh[bCh] =  bApiEepromReadByte(EEP_ADC_DIG_GAIN_SCART0_HIGH_BYTE +bCh);
                                ...
                                DBG_Printf(VGA_Debug,"after Type=%u  ,Gain1  =0x%x%x ,Gain2  =0x%x%x ,Gain3  =0x%x%x \n",bType,bTmpHigh[0],bTmpLow[0],bTmpHigh[1],bTmpLow[1],bTmpHigh[2],bTmpLow[2]);
                        #else                          
                                for(bCh=0; bCh<3; bCh++)
                                {
...
PRIVATE void vDrvGetMaxValue16Times(void)
{
    ...
    //DBG_Printf(VGA_Debug,"_bAutoColorTimeOutCNT=%u, _wData_v1v2=0x%x, _dwMaxLevelAvg=0x%x\n",_bAutoColorTimeOutCNT  ,_wData_v1v2[0], _dwMaxLevelAvg[0] );
}
#endif
...
void vDrvPGALinearityVerify(void)
{
          ...
          case PGA_VERIFY_4: 
                  ...
                  {
                       vDrvCalculateMaxBLKAvg();
                      DBG_Printf(VGA_Debug,"Maximum level @ Gain= %3u :PGA_Max1= %3u ,PGA_Max2= %3u ,PGA_Max3= %3u\n",_bGain,_dwMaxLevelAvg[0],_dwMaxLevelAvg[1],_dwMaxLevelAvg[2]);
                      DBG_Printf(VGA_Debug,"Blank level   @ Gain= %3u :CH1_Blank= %3u ,CH2_Blank= %3u ,CH3_Blank= %3u\n",_bGain,_dwBlankAvg[0],_dwBlankAvg[1],_dwBlankAvg[2]);
                      if ( _bGain<0xff)
                      {
                          _bGain ++;
                          _bPGA_LINEARITY_state = PGA_VERIFY_2;
                      ...
                      {
                            DBG_Printf(VGA_Debug,"---To restore the gain and offset ---\n");
                            vDrvSetPGAGainOffsetFromEEPROM();
                            for(_bOrder=0; _bOrder<3; _bOrder++)
                            {
                            #ifdef AUTOCOLOR_NEW_METHOD
                                  DBG_Printf(VGA_Debug,"_bOrder=%x, Initial GAIN=0x%x, Initial OFFSET=0x%x\n",_bOrder,bDrvGetHDADCGain_Digital(_bOrder),bDrvGetHDADCOffset_Digital(_bOrder));
                            #else
...
void vDrvEnableVGARefVoltageVerify(UINT8 bType)
{
		...
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG2, 1, VCLAMP_MONEN);
       	vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(0xE,FLD_RG_VREFMON)|P_Fld(0x0,FLD_RG_VGA_TESTBUF_PWD));
		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		...
		//Clamp Ch2
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG2, 1, VCLAMP_MONEN);
       	vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(0xB,FLD_RG_VREFMON)|P_Fld(0x0,FLD_RG_VGA_TESTBUF_PWD));
		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		//Clamp Ch3
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG2, 1, VCLAMP_MONEN);
       	vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(0x8,FLD_RG_VREFMON)|P_Fld(0x0,FLD_RG_VGA_TESTBUF_PWD));
		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG0, 1, VBG_TEST_EN);
       	vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(0x3,FLD_RG_VREFMON)|P_Fld(0x0,FLD_RG_VGA_TESTBUF_PWD));
		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 2, FLD_RG_SRVADC_TESTIN);
       	break;
             case 4:
		//ADC_Ch1
		vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 2, RG_VGAADC1_MON_SEL);
       	vIO32WriteFldAlign(PDWNC_VGACFG2, 0x06,FLD_RG_VREFMON);
 		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		//ADC_Ch2
		vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 2, RG_VGAADC1_MON_SEL);
       	vIO32WriteFldAlign(PDWNC_VGACFG2, 0x05,FLD_RG_VREFMON);
 		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		//ADC_Ch3
		vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 2, RG_VGAADC1_MON_SEL);
       	vIO32WriteFldAlign(PDWNC_VGACFG2, 0x04,FLD_RG_VREFMON);
 		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 2, FLD_RG_SRVADC_TESTIN);
       	break;
             case 7:
		//VCM_Ch1
		vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, RG_VGAADC1_MON_SEL);
       	vIO32WriteFldAlign(PDWNC_VGACFG2, 0x06,FLD_RG_VREFMON);
 		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		...
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG2, 1, AGMONEN_1);
       	...
       	vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(0xD,FLD_RG_VREFMON)|P_Fld(0x0,FLD_RG_VGA_TESTBUF_PWD));
		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
		...
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG6, 1, AGMONEN_2);
       	vIO32WriteFldMulti(PDWNC_VGACFG2, P_Fld(0xA,FLD_RG_VREFMON)|P_Fld(0x0,FLD_RG_VGA_TESTBUF_PWD));
		vIO32WriteFldAlign(PDWNC_VGACFG1,0x0,FLD_RG_DIG_TST_EN);
		//vIO32WriteFldMulti(PDWNC_VGACFG3,P_Fld(0x1,FLD_RG_SYNC_REV_0)|P_Fld(0x0,FLD_RG_SYNC_REV_1));
		//vIO32WriteFldAlign(REG_VGA_Normal_CFG4, 1, VGA_ABIST_EN);
		vIO32WriteFldAlign(PDWNC_SADCCFG0, 0, FLD_RG_SRV_RES);
...
void vDrvEnableSyncSlicerRefVoltageVerify(UINT8 bType)
{
	...
	vIO32WriteFldAlign(PDWNC_VGACFG2,0,FLD_RG_VGA_TESTBUF_PWD);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,0,FLD_RG_DIG_TST_EN);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,1,FLD_RG_SYNC_TESTO_EN );	
	//vIO32WriteFldAlign(PDWNC_VGACFG5,0,FLD_RG_VREFMON11_8);	
        switch (bType)
        {
             case 0:
	//VBG	
	vIO32WriteFldAlign(PDWNC_VGACFG1,0,FLD_RG_SYNC_TSTSEL);       	
	break;
       case 1:
	//VTH_SOGY	
	//vIO32WriteFldAlign(PDWNC_VGACFG2,1,FLD_RG_TREF_EN);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,5,FLD_RG_SYNC_TSTSEL);	
	break;
       case 2:
	//VTL_SOGY	
	//vIO32WriteFldAlign(PDWNC_VGACFG2,1,FLD_RG_TREF_EN);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,4,FLD_RG_SYNC_TSTSEL);	
	break;
       case 3:
	//VTH_SYNC	
	//vIO32WriteFldAlign(PDWNC_VGACFG2,1,FLD_RG_TREF_EN);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,7,FLD_RG_SYNC_TSTSEL);	
	break;
       case 4:
	//VTL_SYNC	
	//vIO32WriteFldAlign(PDWNC_VGACFG2,1,FLD_RG_TREF_EN);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,6,FLD_RG_SYNC_TSTSEL);	
	break;
       case 5:
	//FB_VREFP	
	//vIO32WriteFldAlign(PDWNC_VGACFG2,1,FLD_RG_TREF_EN);
	vIO32WriteFldAlign(PDWNC_VGACFG1,2,FLD_RG_SYNC_TSTSEL);	
	//vIO32WriteFldAlign(PDWNC_VGACFG1,0,FLD_RG_FB_PWD);	
	break;
       case 6:
	//FB_VREFN	
	//vIO32WriteFldAlign(PDWNC_VGACFG2,1,FLD_RG_TREF_EN);	
	vIO32WriteFldAlign(PDWNC_VGACFG1,1,FLD_RG_SYNC_TSTSEL);	
...
UINT16 wTmp;
UINT16 wTmp2;
UINT16 UnitGainADCMaxLevel,UnitGainAdcBlankLevel;
#else
   ...
   switch (_bAutoColorState0)
   {
     ...
     case VDO_AUTO_COLOR_1P0_START:  //to set gain and offset for the corresponding signal type
          ...
          _bAutoColorState0 = VDO_AUTO_COLOR_1_START;
               ...
               {
    			bDrvSetHDADCGain_Digital(_bOrder,0x8000);
				vDrvSetHDADCDigitalOffset(_bOrder,0xFF);
                    wTmp = bDrvGetHDADCOffset_Digital(_bOrder);
                    wTmp2 = bDrvGetHDADCGain_Digital(_bOrder);
                    DBG_Printf(VGA_Debug,"== _bOrder=%x, Initial GAIN=0x%x, OFFSET=0x%x ====\n",_bOrder,wTmp2,wTmp);
               }         
          ...
          vIO32WriteFldAlign(HDFE_01, 0x8000, AD2_GAIN);	
          vIO32WriteFldAlign(HDFE_02, 0x8000, AD3_GAIN);
          vIO32WriteFldAlign(HDFE_00, 0, AD1_OFFSET);          
          vIO32WriteFldAlign(HDFE_00, 0, AD2_OFFSET);          
          vIO32WriteFldAlign(HDFE_00, 0, AD3_OFFSET);          
        ...
        if (fgIsSP0FlgSet(SP0_AUTOCOLOR_FLG))
	{
     ...
     vIO32WriteFldAlign(HDFE_03, 512 , AD3_GAIN_BIAS);   //5881 fix 512
          _wADCTarget[_bOrder]  = bDrvGetColorTargetValue(_bType, _bOrder); //bColorMaxTypeNew[bType][_bOrder];
          _wBlank[_bOrder] = wColorBlankValueNew[_bType][_bOrder];
			//bDrvSetHDADCGain_Digital(_bOrder,0x8000);
			//	vDrvSetHDADCDigitalOffset(_bOrder,0);
				vUtDelay1ms(50);
         			vDrvOnChipAutoColorGetADCData(GAIN_MODE);
         			//UnitGainADCMaxLevel= bDrvOnChipGetADCMaxMinValue(_bOrder,AS_PHASE_MAX_SEL);//_wData_v1v2 [_bOrder];
        			...
        			_bInitGain=((((_wADCTarget[_bOrder] <<2) -(_wBlank[_bOrder]>>3))*16*2048)/((UnitGainADCMaxLevel<<2)-(UnitGainAdcBlankLevel>>3)) );
          			bDrvSetHDADCGain_Digital(_bOrder,_bInitGain);
                 //Printf("initgain=0x%x, UnitGainADCMaxLevel=0x%x,UnitGainAdcBlankLevel=0x%x,_wADCTarget[_bOrder]=0x%x,_wBlank[_bOrder]=0x%x,gain=0x%x,offset=0x%x\n",_bInitGain,UnitGainADCMaxLevel,(UnitGainAdcBlankLevel>>3),(_wADCTarget[_bOrder] <<2),(_wBlank[_bOrder]>>3),wTmp,wTmp2);    
          	}
	...
	{
	   //DBG_Printf(VGA_Debug,"--- _bOffsetGainDone=%x, _bAutoColorTimeOutCNT=%x\n",_bOffsetGainDone,_bAutoColorTimeOutCNT);
     ...
     case VDO_AUTO_COLOR_4_START:
        if (fgIsSP0FlgSet(SP0_AUTOCOLOR_FLG))
        {
           ...
           if(_bType==INT_HDTV)
           {
               DBG_Printf(VGA_Debug,"HDTV: Offset_1=0x%x, Offset_2=0x%x, Offset_3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_VGA)
           {
               DBG_Printf(VGA_Debug,"VGA: Offset_1=0x%x, Offset_2=0x%x, Offset_3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_SCART)
           {
               DBG_Printf(VGA_Debug,"SCART: Offset_1=0x%x, Offset_2=0x%x, Offset_3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           
           else if(_bType==INT_VGA_COMPOENT)
           {
               DBG_Printf(VGA_Debug,"VGA_COMPOENT: Offset_1=0x%x, Offset_2=0x%x, Offset_3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           ...
           if(_bType==INT_HDTV)
           {
               DBG_Printf(VGA_Debug,"HDTV: Gain1=0x%x, Gain2=0x%x, Gain3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_VGA)
           {
               DBG_Printf(VGA_Debug,"VGA: Gain1=0x%x, Gain2=0x%x, Gain3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_SCART)
           {
               DBG_Printf(VGA_Debug,"SCART: Gain1=0x%x, Gain2=0x%x, Gain3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_VGA_COMPOENT)
           {
               DBG_Printf(VGA_Debug,"VGA_COMPOENT: Gain1=0x%x, Gain2=0x%x, Gain3=0x%x\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
          ...
          vDrvPreSetToReadBlank(AS_BLANK_ALWAYS);
           ...
           if(_bType==INT_HDTV)
           {
               DBG_Printf(VGA_Debug,"HDTV: 12 bits base: Blank1=0x%3x, Blank2=0x%3x, Blank3=0x%3x\n",(_wData_v1v2[0]>>1),(_wData_v1v2[1]>>1),(_wData_v1v2[2]>>1));
           }
           else if(_bType==INT_VGA)
           {
               DBG_Printf(VGA_Debug,"VGA: 12 bits base: Blank1=0x%3x, Blank2=0x%3x, Blank3=0x%3x\n",(_wData_v1v2[0]>>1),(_wData_v1v2[1]>>1),(_wData_v1v2[2]>>1));
           }
           else if(_bType==INT_SCART)
           {
               DBG_Printf(VGA_Debug,"SCART: 12 bits base: Blank1=0x%3x, Blank2=0x%3x, Blank3=0x%3x\n",(_wData_v1v2[0]>>1),(_wData_v1v2[1]>>1),(_wData_v1v2[2]>>1));
           }
           else if(_bType==INT_VGA_COMPOENT)
           {
               DBG_Printf(VGA_Debug,"VGA_COMPOENT: 12 bits base: Blank1=0x%3x, Blank2=0x%3x, Blank3=0x%3x\n",(_wData_v1v2[0]>>1),(_wData_v1v2[1]>>1),(_wData_v1v2[2]>>1));
           }
           //--------- to show the maximum level --------------
           vDrvOnChipGetADCMaxValue();
           if(_bType==INT_HDTV)
           {
               DBG_Printf(VGA_Debug,"HDTV: Max1=%3u, Max2=%3u, Max3=%3u\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_VGA)
	       {
               DBG_Printf(VGA_Debug,"VGA: Max1=%3u, Max2=%3u, Max3=%3u\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_SCART)
           {
               DBG_Printf(VGA_Debug,"SCART: Max1=%3u, Max2=%3u, Max3=%3u\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           else if(_bType==INT_VGA_COMPOENT)
           {
               DBG_Printf(VGA_Debug,"VGA_COMPOENT: Max1=%3u, Max2=%3u, Max3=%3u\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
           }
           vClrSP0Flg(SP0_AUTOCOLOR_FLG);
...
if((_bType<=INT_VGA_COMPOENT) &&(bMode<=DIGITAL_OFFSET_MODE))//for klocwork check
{
 switch (_OnChipAutoColorState) 
 {
    ...
    //DBG_Printf(VGA_Debug,"--------- ON_CHIP_AUTO_COLOR_INITIAL_STATE, mode=%x----\n",bMode);
       if((bMode== ON_CHIP_GAIN_MODE)||(bMode== ON_CHIP_OFFSET_MODE))
       {
         _bType= INT_VGA;
       }
          	...
          	/*		if(bMode==GAIN_MODE)
         			...
         			UnitGainADCMaxLevel= bDrvOnChipGetADCMaxMinValue(_bOrder,AS_PHASE_MAX_SEL);//_wData_v1v2 [_bOrder];
          			UnitGainADCMaxLevel=_wData_v1v2 [_bOrder];
           			//vDrvOnChipAutoColorGetADCData(OFFSET_MODE);	
        			UnitGainAdcBlankLevel=bDrvGetBlankVal(_bOrder);//_wData_v1v2 [_bOrder];
			bDrvSetHDADCGain_Digital(_bOrder,0x4000);
				vDrvSetHDADCDigitalOffset(_bOrder,0);
				vUtDelay1ms(50);
         			vDrvOnChipAutoColorGetADCData(GAIN_MODE);
         			UnitGainADCMaxLevel= bDrvOnChipGetADCMaxMinValue(_bOrder,AS_PHASE_MAX_SEL);//_wData_v1v2 [_bOrder];
          			UnitGainADCMaxLevel=_wData_v1v2 [_bOrder];
           			//vDrvOnChipAutoColorGetADCData(OFFSET_MODE);	
        			UnitGainAdcBlankLevel=bDrvGetBlankVal(_bOrder);//_wData_v1v2 [_bOrder];
        			_bInitOffset=((((_wADCTarget[_bOrder] <<2) -(_wBlank[_bOrder]>>3))*16*1024)/((UnitGainADCMaxLevel<<2)-UnitGainAdcBlankLevel)) ;
          			bDrvSetHDADCGain_Digital(_bOrder,_bInitOffset);
          			}*/
         		#else
                     	bDrvSetHDADCOffset(_bOrder,_bInitOffset);                  //Set offset=0 mv
                     #endif
	...
	break;
     ...
     case ON_CHIP_AUTO_COLOR_SEARCH_TARGET:  //get ADC raw data 
      // DBG_Printf(VGA_Debug,"--------- ON_CHIP_AUTO_COLOR_SEARCH_TARGET ----\n");
       _bTotalCnt ++;
       ...
       for(bIndexCnt=0; bIndexCnt<3; bIndexCnt++)
       {
	      ...
	      else
	      {
                ...
                #if (AUTO_COLOR_DEBUG || ON_CHIP_AUTO_COLOR_DEBUG)
                DBG_Printf(VGA_Debug,"order=%2u, Gain=%x, step=%x, toggle=%u, Data=0x%x\n",_bOrder,_bGain,_bCaliStep[_bOrder],_bChgDirToggle[_bOrder],_wData_v1v2[_bOrder]);    
                #endif
                vDrvOnChipAutoColorSearchTarget_V2(bMode);
  	          bDrvSetHDADCGain_Digital(_bOrder, _bGain);
            #else
       ...
       for(_bOrder=0; _bOrder<3; _bOrder++)
       {
            ...
            if((bMode==OFFSET_MODE)||(bMode==ON_CHIP_OFFSET_MODE))
             ...
             /*   if(bMode==OFFSET_MODE)
              ...
              _bAutoColorOffset[_bType][_bOrder] = _bGain;
                ...
                #if (AUTO_COLOR_DEBUG||ON_CHIP_AUTO_COLOR_DEBUG)
                DBG_Printf(VGA_Debug,"Channel %u Offset success. offset=%x, ADC=%u, target=%u\n", _bOrder,_bGain,_wData_v1v2[_bOrder],_wADCTarget[_bOrder]);
                #endif
             }
             else
             {
                ...
                #if (AUTO_COLOR_DEBUG ||ON_CHIP_AUTO_COLOR_DEBUG )
                DBG_Printf(VGA_Debug,"Channel %u Auto Gain success. Gain=%x, ADC=%u, target=%u\n", _bOrder,_bGain,_wData_v1v2[_bOrder],_wADCTarget[_bOrder]);  
                #endif
             }
          }
          else
          {
           bCaliNG =1;
             ...
             DBG_Printf(VGA_Debug,"ON_CHIP_AUTO_COLOR fail at mode=%u\n",bMode);
             #endif
          }
       }
       if(bCaliNG==0)
    ...
    case ON_CHIP_AUTO_COLOR_FINISH: 
          ...
          if(_bReadMAXLevel==1)
	  {
             _bReadMAXLevel = 0;
             vDrvOnChipGetADCMaxValue();
	     DBG_Printf(VGA_Debug,"Before auto color Max1=%3u, Max2=%3u, Max3=%3u\n",_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
	  }
        }
        else
      #endif
        ...
        switch (_bADC_LINEARITY_state)
        {
             ...
             case ADC_VERIFY_1:
               ...
               if(_bAutoColorTimeOutCNT<0x10)
               {
                     ...
                     //DBG_Printf(VGA_Debug,"TimeOutCNT=%u, _wData1=0x%x, _wData2=0x%x, _wData3=0x%x\n",_bAutoColorTimeOutCNT  ,_wData_v1v2[0],_wData_v1v2[1],_wData_v1v2[2]);
                     _bAutoColorTimeOutCNT ++;
               }
               else
               {
                     ...
                     _bADCLinearity =1;
                     DBG_Printf(VGA_Debug,"---- The top level of 3 CH after auto color is: %d %d %d\n",_dwAutoColorMax[0] ,_dwAutoColorMax[1] ,_dwAutoColorMax[2] );
               }
             break;
             ...
             case ADC_VERIFY_4:
                 ...
                 {
                     DBG_Printf(VGA_Debug,"The level of 3 CH at location %2d are CH1: %4d ,CH2: %4d ,CH3: %4d\n",_bLocation,_dwMaxLevelAvg[0] ,_dwMaxLevelAvg[1] ,_dwMaxLevelAvg[2] );
                     _dwMaxDiffTemp =0;
                     for (_bOrder=0; _bOrder<3; _bOrder++)   
                     {
                     ...
                     else
                     {
                          ...
                          _dwMaxDiffLocal = 0;
                          DBG_Printf(VGA_Debug,"The channel to channel different at position1,2,3,4... are \n");
                     if(_bLocation<128)//for klocwork check
                     {
                          for (_bOrder=0; _bOrder<TOTAL_POINT; _bOrder++)
                          {
                           ...
                           DBG_Printf(VGA_Debug,"Position %2d = %3d\n",_bOrder,_dwMaxDiff[_bOrder]);
                           }
                     }
                           DBG_Printf(VGA_Debug,"The maximum code difference (12 bit) between channel to channel is %d\n", _dwMaxDiffLocal);
                           _bLocation = 0;
                           _bADCLinearity =2;
                           _bADC_LINEARITY_state = ADC_VERIFY_2;
                      }
             ...
             case ADC_VERIFY_5:
                 ...
                 else
                 {
                     if(_bLocation==0)
                     {
                          ...
                          #if PGA_LINEARITY_DEBUG
                          DBG_Printf(VGA_Debug,"Start Point= %u , %u , %u\n",_dwStPoint[0],_dwStPoint[1],_dwStPoint[2]);
                          #endif
                          if(TOTAL_POINT<=16)
                          {
             ...
             case ADC_VERIFY_6:
                 ...
                 else
                 {
                     for (_bOrder=0; _bOrder<3; _bOrder++)   
                     {
                           ...
                           #if PGA_LINEARITY_DEBUG
                           DBG_Printf(VGA_Debug,"Channel= %u ,point %u  target level= %u ,Current level= %u\n",_bOrder,_bLocation,_dwMaxDiffTemp,_dwMaxLevelAvg[_bOrder]);
                           #endif
                           if(_dwMaxDiffTemp> _dwMaxLevelAvg[_bOrder])
                           {
                      ...
                      DBG_Printf(VGA_Debug,"The channel different at position %2u is %4u , %4u , %4u\n",_bLocation, _dwMaxDiff[0],_dwMaxDiff[1],_dwMaxDiff[2]);
