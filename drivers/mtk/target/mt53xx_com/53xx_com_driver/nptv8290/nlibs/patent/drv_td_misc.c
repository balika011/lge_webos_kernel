...
UINT8 u1DrvTTTSetMode(UINT8 u1VdpId, UINT8 u1TD22DEn, UINT8 u1ModeSel, UINT8 u1MaskSel)
{
	...
	if (u1VdpId == SV_VP_MAIN)
	{
		...
		if(u2Width > 4095)
		{
			vIO32WriteFldAlign(PRE_3D22D_MAIN_00, u2Width>>12, R_UIIMGWIDTH_M_MSB);
		}
		else
		{
			vIO32WriteFldAlign(PRE_3D22D_MAIN_00, 0, R_UIIMGWIDTH_M_MSB);
		}
	...
	else
	{
		...
		if(u2Width > 4095)
		{
			vIO32WriteFldAlign(PRE_3D22D_PIP_00, u2Width>>12, R_UIIMGWIDTH_P_MSB);
		}
		else
		{
			vIO32WriteFldAlign(PRE_3D22D_PIP_00, 0, R_UIIMGWIDTH_P_MSB);
		}
