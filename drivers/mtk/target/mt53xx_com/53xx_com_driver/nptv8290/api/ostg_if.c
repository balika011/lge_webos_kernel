...
extern UINT32 _u4ScanPWMTestPer;
 ...
 * Set Panel output frame rate
 */
void vApiSetGfxTimingGenode(UINT8  wGfxTgMode)
{
    vDrvSwitchGFXTiming(wGfxTgMode);
}
#endif
...
#endif
void BACKLT_SetScanDimming(UINT32 u4Delay, UINT32 u4Frequency, UINT32 u4Dimming)
{
    UINT32 u4Devider=1;
    UINT32 u4High=0;
    UINT32 u4Low=0;
    ...
    if(u4Devider ==0)
    {
        printf("BACKLT_SetScanDimming:u4Devider=0, force to 1:%d, u4Frequency:%d,vDrvGetLCDFreq(%d)  ", u4Devider,u4Frequency, vDrvGetLCDFreq());
        u4Devider = 1;
    }
	if ((u4Dimming>0)&&(u4Dimming<_u4ScanPWMTestPer))
    {
        u4High = u4Dimming/u4Devider;
        u4Low = (_u4ScanPWMTestPer-u4Dimming)/u4Devider;
    }
    else if (u4Dimming>=_u4ScanPWMTestPer)
    {
        u4High = _u4ScanPWMTestPer;
    }
    LOG(3, "[scanPWM]range:0~%d,freq:%dHz,dev:%d,start:%d%,high:%d%,low:%d%\n", _u4ScanPWMTestPer, u4Frequency, u4Devider, u4Delay, u4High, u4Low);
	LOG(0, "[BACKLT_SetScanDimming] bDelay = %d, bDuty = %d, bFrequency = %d(%d)\n", u4Delay, (UINT16)u4Dimming, u4Frequency,vDrvGetLCDFreq()); 		
	...
	vDrvSetScanPWMSW(DRVCUST_PanelGet(ePanelBacklightPwmPort),u4Delay,u4High,u4Low);
	vDrvSetScanPWMStructData(DRVCUST_PanelGet(ePanelBacklightPwmPort), u4Delay, u4High, u4Low);
}
void vApiSetScanDimming(UINT8  bDelay, UINT16 u2Freq, UINT16 u2Duty)
{
    //static UINT8 bCnt = 0;  
    UINT32 u4Delay =  ((UINT32)bDelay*(UINT32)_u4ScanPWMTestPer)/(UINT32)100; // No need to +1.
	
    _fgIsSync = (BOOL)SV_OFF;
    _wFreq = u2Freq;
    _wDuty = u2Duty; 
    
#ifndef CC_MTK_LOADER
    if(E_TDTV_UI_3D_MODE_OFF!=u4DrvTDTV3DModeQuery() &&(_bForceFrameRate!=0))
    {
        if(_wFreq == 100)
        {
            _wFreq = 120;
        ...
        //if(bCnt==0)
    	{
            //LOG(1, "[vApiSetDimming]bDelay=%d,  bDuty = %d, bFrequency = %d(%d) \n", bDelay, (UINT16)u2Duty, u2Freq,_wFreq); 
    	}
    }
    #endif
	
	LOG(3, "[vApiSetScanDimming] bDelay = %d, bDuty = %d, bFrequency = %d(%d)\n", bDelay, (UINT16)u2Duty, u2Freq,vDrvGetLCDFreq()); 		
    BACKLT_SetScanDimming((UINT32)u4Delay, (UINT32)_wFreq, (UINT32)_wDuty);
}
...
void vApiTCONPower(VIDEO_TCON_POWER_SEQ_T eTconPowSeq, BOOL fgEnable)
{
    if(PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_EPI)
    {
        ...
        printf("vApiTCONEPIPanelPower: %d \n",fgEnable);
...
void vApiTCONMINIPower(VIDEO_TCON_MINI_POWER_SEQ_T eTconPowSeq, BOOL fgEnable)
{
    if(PANEL_GetDisplayType()== PANEL_DISPLAY_TYPE_MLVDS)
    {
        ...
        printf("vApiTCONMINIPanelPower: %d \n",fgEnable);
...
void LDR_PmxDisplayHWInit(void)
{
    ...
    // Move MJC related initialization to MJC driver
    #ifdef CC_BYPASS_MJC
