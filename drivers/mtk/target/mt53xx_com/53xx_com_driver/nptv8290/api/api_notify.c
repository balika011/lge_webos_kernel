...
extern void vApiNotifyResolutionChg(UINT8 u1Path,UINT8 u1SigState)
{
    ...
    {//dual hybrid mode, main and sub stable --> stable , main or sub unstable --> unstable
        static UINT8 u1SigStateRec[VDP_NS];
        
        u1SigStateRec[u1Path] = u1SigState;
        
        if (SV_VDO_STABLE != u1SigState)
        {
            u1Path = VDP_1;
            //continuign callback
        }
        else if (SV_VDO_STABLE == u1SigState && u1SigStateRec[VDP_1] == u1SigStateRec[VDP_2])
        {
            u1Path = VDP_1;
            //continue
        }
