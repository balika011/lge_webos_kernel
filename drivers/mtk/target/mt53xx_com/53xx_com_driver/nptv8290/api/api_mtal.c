...
typedef enum
{
    ...
    VDO_PQ_SET_FB_TABLE,
    VDO_PQ_SET_COLOR_FILTER,
    VDO_PQ_SET_PRE_PATTERN,
    VDO_PQ_EXP_COLOR_CURVE,
...
static INT32 _PE_SetVdoExpColorCurve(UCHAR ucVdpId, const UINT8* prData, UINT32 u4Size)
{
	UINT8 i,j;
    if (u4Size != BASE_COLOR_MAX*sizeof(VDO_PQ_EXP_COLOR_T))
        return PQ_PARAMETER_ERROR;
	...
	for(j=0;j<SCE_MAX;j++)
	{
		...
		{	
		    vHalSetSCEAngular(j, i,prData[i+j*BASE_COLOR_MAX]);
		}
	}
...
INT32 _PE_GetVdoPqGammaCurveEX(UCHAR ucVdpId, UINT8* pGammaCorrectionLut, UINT32 u4Size)
{
    if (u4Size != (GAMMA_INDEX*sizeof(UINT8)*3))
        return PQ_PARAMETER_ERROR;
	...
	if(pGammaCorrectionLut==NULL)
	{
		if((pGammaCorrectionLut = (UINT8*)x_mem_alloc(3*GAMMA_INDEX*sizeof(UINT8))) == 0)
		{
			return PQ_NOT_OK;
		}
	}
	
	x_memcpy(pGammaCorrectionLut,GAMMA_256,3*GAMMA_INDEX*sizeof(UINT8));
	return PQ_OK;

}
INT32 _PE_GetVdoPqExpColorCurve(UCHAR ucVdpId, UINT8* pkvPqGetInfo, UINT32 u4Size)
{	
	if(pkvPqGetInfo==NULL)
	{
		if((pkvPqGetInfo = (UINT8*)x_mem_alloc(3*BASE_COLOR_MAX*sizeof(UINT8))) == 0)
		{
			return PQ_NOT_OK;
		}
	}
    pkvPqGetInfo[0] = IO32ReadFldAlign(SIXCOLOR_REG_00, SIXCOLOR_Y_M);
    pkvPqGetInfo[1] = IO32ReadFldAlign(SIXCOLOR_REG_00, SIXCOLOR_Y_R);
    pkvPqGetInfo[2] = IO32ReadFldAlign(SIXCOLOR_REG_00, SIXCOLOR_Y_Y);
    pkvPqGetInfo[3] = IO32ReadFldAlign(SIXCOLOR_REG_00, SIXCOLOR_Y_G);
    pkvPqGetInfo[4] = IO32ReadFldAlign(SIXCOLOR_REG_01, SIXCOLOR_Y_C);
    pkvPqGetInfo[5] = IO32ReadFldAlign(SIXCOLOR_REG_01, SIXCOLOR_Y_B);
    pkvPqGetInfo[6] = IO32ReadFldAlign(SIXCOLOR_REG_01, SIXCOLOR_S_M);
    pkvPqGetInfo[7] = IO32ReadFldAlign(SIXCOLOR_REG_01, SIXCOLOR_S_R);
    pkvPqGetInfo[8] = IO32ReadFldAlign(SIXCOLOR_REG_02, SIXCOLOR_S_Y);
    pkvPqGetInfo[9] = IO32ReadFldAlign(SIXCOLOR_REG_02, SIXCOLOR_S_G);
    pkvPqGetInfo[10] = IO32ReadFldAlign(SIXCOLOR_REG_02, SIXCOLOR_S_C);
    pkvPqGetInfo[11] = IO32ReadFldAlign(SIXCOLOR_REG_02, SIXCOLOR_S_B);
    pkvPqGetInfo[12] = IO32ReadFldAlign(SIXCOLOR_REG_03, SIXCOLOR_H_M);
    pkvPqGetInfo[13] = IO32ReadFldAlign(SIXCOLOR_REG_03, SIXCOLOR_H_R);
    pkvPqGetInfo[14] = IO32ReadFldAlign(SIXCOLOR_REG_03, SIXCOLOR_H_Y);
    pkvPqGetInfo[15] = IO32ReadFldAlign(SIXCOLOR_REG_03, SIXCOLOR_H_G);
    pkvPqGetInfo[16] = IO32ReadFldAlign(SIXCOLOR_REG_04, SIXCOLOR_H_C);
    pkvPqGetInfo[17] = IO32ReadFldAlign(SIXCOLOR_REG_04, SIXCOLOR_H_B);
...
static INT32 _PE_SetColorFilter(UCHAR ucVdpId, VDO_PQ_SET_COLOR_FILTER_T* prData, UINT32 u4Size)
{
    if (u4Size != sizeof(VDO_PQ_SET_COLOR_FILTER_T))
    {
        return PQ_PARAMETER_ERROR;
    }

    vHalSetColorFilter(ucVdpId, SV_RGB_R, prData->u1ROnOff);
    vHalSetColorFilter(ucVdpId, SV_RGB_G, prData->u1GOnOff);
    vHalSetColorFilter(ucVdpId, SV_RGB_B, prData->u1BOnOff);
    return PQ_OK;
}

static INT32 _PE_SetPrePattern(UCHAR ucVdpId, UINT8* prData, UINT32 u4Size)
{
    if (u4Size != sizeof(UINT8))
    {
        return PQ_PARAMETER_ERROR;
    ...
    vDrvPreprocEnablePattern(ucVdpId, *prData);
    return PQ_OK;
}
...
INT32 PE_SetIMTFlag(UINT32 ucVdpId,UINT32 prData)
{
    if (ucVdpId != VDP_1)
        return PQ_NOT_SUPPORTED;
	
#ifndef DEMO_BOARD
	u1ITMFlag_status=(UINT8)prData;
	printf("****PE_SetIMTFlag= %d ****\n",(UINT8)prData);
#endif
	return PQ_OK;
}
INT32 PE_SetPqInfo(UCHAR ucVdpId, UINT32 u4PqType, const void* pkvPqSetInfo, UINT32 u4Size)
{
    ...
    switch ((VDO_PQ_TYPE_T) u4PqType)
    {
        ...
        case VDO_PQ_EXP_COLOR:
            ...
            break;  
        case VDO_PQ_EXP_COLOR_CURVE:
            i4Ret = _PE_SetVdoExpColorCurve(ucVdpId, (UINT8*) pkvPqSetInfo, u4Size);
            break;                
        case VDO_PQ_XVYCC:
        ...
        case VDO_PQ_LCDIM_GAIN_TABLE:
            ...
            break;
        case VDO_PQ_SET_COLOR_FILTER:
            i4Ret = _PE_SetColorFilter(ucVdpId, (VDO_PQ_SET_COLOR_FILTER_T*) pkvPqSetInfo, u4Size);
            break;
        case VDO_PQ_SET_PRE_PATTERN:
            i4Ret = _PE_SetPrePattern(ucVdpId, (UINT8*) pkvPqSetInfo, u4Size);
            break;
        default:
...
INT32 PE_GetPqInfo(UCHAR ucVdpId, UINT32 u4PqType, void* pkvPqGetInfo, UINT32 u4Size)
{
    ...
    switch ((VDO_PQ_TYPE_T) u4PqType)
    {
        ...
        case VDO_PQ_TD_DISPARITY:
            ...
            break;        
		case VDO_PQ_GAMMA_CURVE_EX:
            i4Ret = _PE_GetVdoPqGammaCurveEX(ucVdpId, (UINT8*) pkvPqGetInfo, u4Size); 
			break;
		case VDO_PQ_EXP_COLOR_CURVE:
			i4Ret = _PE_GetVdoPqExpColorCurve(ucVdpId, (UINT8*) pkvPqGetInfo, u4Size);
			break;
        default:
            i4Ret = PQ_NOT_SUPPORTED;
            break;
