...
static PE_UI_RANGE_T aUiQtyItemMinMax[PE_ARG_NS] =
{
	...
	{QUALITY_SCE_BEGIN, 	QUALITY_SCE_END,     	R_QTYMAP | R_MAIN |R_NOSIG},				        // PE_ARG_SCE
    {QUALITY_SKIN_BEGIN,    QUALITY_SKIN_END,       R_QTYMAP | R_MAIN |R_NOSIG},                     // PE_ARG_SKIN_COLOR
	...
	{QUALITY_MODULE_ONOFF_BEGIN, QUALITY_MODULE_ONOFF_END, R_QTYMAP | R_NOSIG | R_OFF},   // PE_ARG_MODULE_ONOFF
};
...
static void _vMappingSrcTiming(void)
{
	...
	do
	{
		...
		bBegin = READ_Sce_SRC_GROUP(bCustomSrcTypTmg, QTY_IDX_RANGE_BEGIN);
		bEnd = READ_Sce_SRC_GROUP(bCustomSrcTypTmg, QTY_IDX_RANGE_END);
		
		if(bBegin <= SOURCE_TYPE_TIMING_MAX && bEnd <= SOURCE_TYPE_TIMING_MAX)
		{
			WRITE_Sce_SRC_GROUP(bCustomSrcTypTmg, QTY_IDX_RANGE_BEGIN,
			   bSrcTimingInverseTbl[bBegin]);
    ...
    for (i = 2; i != 0; --i)
    {
                 ...
                 SOURCE_TYPE_TIMING_MAX); bCustomSrcTypTmg++)
        {
                    ...
                    READ_CUSTOM_SRC_GROUP(bCustomSrcTypTmg,QTY_IDX_RANGE_END)))
            {
				...
				if(IO32ReadFldAlign(VDP_SCPQ_01, SCPQ_CUST_SOURCETIMINGMAP))
				{
					return DRVCUST_GetCustomSourceTypeTiming(bCustomSrcTypTmg);
				}
				else
#endif
				{
                     ...
                     INT16 const i2Dft, INT16 const i2Cur)
{
    PE_UI_RANGE_T UiRange;
    UINT16 ReturnVal = 0;
	UINT8 bCustUIMapping = IO32ReadFldAlign(VDP_SCPQ_01, SCPQ_CUST_UIMAPPING);
	...
	{
		ReturnVal = DRVCUST_QtyUIMapping(u1SrcTypTmg, u2QtyItem, UiRange.i4Min, UiRange.i4Dft, UiRange.i4Max ,i2Min, i2Max, i2Dft, i2Cur);
	}
    if (bApiGetPeUiDebugMode() == SV_ON)
    {
        LOG(2, "Qty Mapping: u2QtyItem %d  Min %d  Max %d  Dft %d  Cur %d\n", 
...
UINT32 PE_SetArg(UCHAR ucVdpId, PE_ARG_TYPE ucArg, INT16 i2Value)
{
	...
	i2LastValue = ai2Arg[ucVdpId][ucArg]; 
...
#endif
	{
		/* Gamma EEPROM have not ready, use flash pq gamma temporary
		if(DRVCUST_OptGet(eNVMGamma))
	    {
	        DRVCUST_SetGammaRemapForceLoad(SV_ON);
	    }
		else
		*/
	    {
...
UINT8 bApiVideoProc(UINT8 u1VidPath, PE_ARG_TYPE u1UiItem)
{
	...
	if (QtyRange.i4Dft & R_AUTO)
	{
		...
		i2Max = 3;
		bAuto = (i2Cur > i2Max) ? (i2Cur - i2Max) : SV_FALSE;
        i2Cur = (i2Cur > i2Max) ? i2Dft : i2Cur;
