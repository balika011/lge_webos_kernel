...
#ifdef CC_Improve_Geekbench
static UINT8 _u1VSyncCountForLockSem =0;  //120Hz Mloop -> 60Hz MLoop
#define DISABLE_UNLOCK_COUNTER   10
...
extern  UINT32 u4PreGFXType;
#endif 
#endif

#if defined(CC_SUPPORT_4K2K) && defined(CC_MT5890)
extern BOOL _fgBypassToPQ;
BOOL fgVB1PathChg= SV_FALSE;
E_TIMEGEN_TRIGGER_STATE E_TimeGen_Trig_Sta;
#endif

void vVdoOutISRCheckSetDataTrigger(void)
{  
	if((fgApplyScalerSetData) || (fgApplyScanPWMSetData)) 	
	{						
		if (fgApplyScalerSetData)
		{
			...
			fgApplyScalerSetData = 0;
			
			vDrvApplyPanelTiming(); 							        //set FSC or PSC panel timing				
			vDrvFireImportPortection(); 								//trigger set data
            #if defined(CC_MT5890)  
            #if defined(SUPPORT_PANEL_GRAPH_MODE)
			if (IS_SUPPORT_VB1_GRAPH_MODE)
			{
	            if(GetPANEL4K2K_TYPE() == PANEL_TYPE_4K1K_60 && u4PreGFXType == PANEL_TYPE_4K2K_30)
	            {
	                u4PreGFXType = 0xff;
	                vDrvSwitchGFXTiming(PANEL_TYPE_4K2K_30);//change back gfx output 4k2k
	            }
			}
            #endif
            #endif
			...
			if(GetPANEL4K2K_TYPE() == PANEL_TYPE_4K2K_60)
			{
				...
				{
					VDP_SetBypassHwEnable(SV_ON);
				}
			    vDrvTconTopVdoSel(E_VB1_PATH_BYPASS);
			}
			else if(_fgBypassToPQ)
			{
				fgVB1PathChg = SV_TRUE;
				_fgBypassToPQ = SV_FALSE;
				E_TimeGen_Trig_Sta = E_TIMEGEN_TRIGGER_STATE_WAITING;
			}
	...
	if(fgVB1PathChg)
	{
	    switch (E_TimeGen_Trig_Sta)
	    {
	        case E_TIMEGEN_TRIGGER_STATE_WAITING:
				E_TimeGen_Trig_Sta = E_TIMEGEN_TRIGGER_VB1_PATH_WAITING;
	            break;
			case E_TIMEGEN_TRIGGER_VB1_PATH_WAITING:
				E_TimeGen_Trig_Sta = E_TIMEGEN_TRIGGER_VB1_PATH;
	            break;
	        case E_TIMEGEN_TRIGGER_VB1_PATH:
				vDrvTconTopVdoSel(E_VB1_PATH_PQ);	
	            E_TimeGen_Trig_Sta = E_TIMEGEN_TRIGGER_DECODE_TIMEGEN_WAINTING;
				break;
			case E_TIMEGEN_TRIGGER_DECODE_TIMEGEN_WAINTING:
				E_TimeGen_Trig_Sta = E_TIMEGEN_TRIGGER_DECODE_TIMEGEN;
	            break;
	        case E_TIMEGEN_TRIGGER_DECODE_TIMEGEN:
	        default:
				fgVB1PathChg = SV_FALSE;
			#ifdef CC_SUPPORT_W2DR2R
			    vR2rEnable(SV_OFF);
			#endif
			    VDP_SetBypassHwEnable(SV_OFF);
	            break;
	    }
...
PRIVATE void vOutVSyncISR(void)
{
...
#if 0 //mt5890 and mt5861 no need this program
    vDrvMeterISR();    
    vDrvADLProc();      
    vDrvTTDSetQualityISR();    
    DRVCUST_OutVSyncISR();
#endif
	if (u1DrvMJCIsOnOff() == SV_FALSE) // if MJC bypass
	{
	    vDrvMeterISR();    
	    vDrvADLProc();      
	    vDrvTTDSetQualityISR();    
	...
	vVdoOutISRCheckApplyPanelTiming();
	vDrvVB1VideoMuteTiming();
    ...
    {
	#ifdef CC_Improve_Geekbench	
       if((_rMChannel.bIsChannelOn == SV_OFF )&&(_rMChannel.bDecType == SV_VD_NA))
       {
	   	   if(vDrvMJCGetDramInterrupt() == 1)
	   	   {
		     _u1VSyncCountForLockSem++;
			 if(_u1VSyncCountForLockSem >254)
			 {
			 	_u1VSyncCountForLockSem = DISABLE_UNLOCK_COUNTER ;
				LOG(5,"in reset unlock \n");
			 }
	   	   }
		   else
		   	 _u1VSyncCountForLockSem =0;
       }
	   else
	   {
	   	   _u1VSyncCountForLockSem =0;
	   }
       if(_u1VSyncCountForLockSem < DISABLE_UNLOCK_COUNTER)
       {
	   	LOG(5,"in unlock \n");
