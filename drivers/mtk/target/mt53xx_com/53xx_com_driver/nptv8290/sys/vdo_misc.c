...
BOOL fgIs4K2KInBypassPath(void)
{
    BOOL bRtn = SV_FALSE;
	#ifdef CC_MT5890  
	...
	{ 
		...
		if(IS_DISP_4K2K_TYPE4 && SRM_IsEpgMode())
		{
		    bRtn = SV_FALSE;
		}
		LOG(1,"4K2K in bypass path (%d)\n",bRtn);
	}
	#endif
	return bRtn;
}
...
BOOL fgIs4k2kNativeTimingOut(UINT8 bPath)
{
	...
	if (((((wDrvVideoInputWidth(bPath) > 1920) || (wDrvVideoInputHeight(bPath) > 1088)) && (bDrvVideoSignalStatus(bPath) == SV_VDO_STABLE))
	    || (_u1Force4K2KTiming == 1)) && (IS_SUPPORT_4K2K_DISPLAY || GAZELLE_2K2K_HYBRID || PANEL_IsSupport4K2KDD())) 
	{
		bRtn = SV_TRUE;
	}
	#endif
...
BOOL fgNeedRecalPanelFrameRate(void)
{
    BOOL bRtn = SV_FALSE;
	...
	#if defined(CC_MT5890) && defined(CC_SUPPORT_4K2K)  
...
#if defined(CC_SUPPORT_4K2K) && !defined(ANDROID) && !defined(CC_DRIVER_BUILD)
//from mt53_fb.c
...
BOOL fgIs4k2kPhoto3DSupport(UINT8 bPath)
{
    ...
    #ifdef CC_SUPPORT_4K2K
	#if !defined(ANDROID) && !defined(CC_DRIVER_BUILD)
...
void vSetVB1SrcType(BOOL fgPath)
{
     LOG(1,"vSetVB1SrcType =%s\n",fgPath?"NB2R":"HDMI");
     IO_B2R_WRITE(B2R_SRCSEL,VB1_PATH_SEL,fgPath);
}
void vSetVB1Color_Transf(BOOL fgEnable,UINT16 u2ColorMode)
{
     LOG(1,"vSetVB1Color_Transf =%x,fgEnable%d\n",u2ColorMode,fgEnable);
     if(fgEnable)
     {
         IO_B2R_WRITE(B2R_Color_Transf,B2R_CT_EN,TRUE);
         IO_B2R_WRITE(B2R_COLOR_TRANSFORM,B2R_COLOR_INT_TABLE,u2ColorMode);
     }
     else
     {
         IO_B2R_WRITE(B2R_Color_Transf,B2R_CT_EN,FALSE);
     }
     
}
#ifdef CC_HDMI_2_0_SUPPORT
extern UINT8 bGetSupportHdmi2ModeVideoTiming(UINT8 bPath);
#endif
UINT8 bIsScalerInput444(UINT8 bPath) reentrant
{
...
#ifdef CC_HDMI_2_0_SUPPORT
                     if((bDrvVideoIsSrcInterlace(bPath))||((bGetSupportHdmi2ModeVideoTiming(bPath)>= MODE_3D_720p_50_FP)&&(bGetSupportHdmi2ModeVideoTiming(bPath)<= MODE_4096_2160P_60_420))) 
#else
					 if((bDrvVideoIsSrcInterlace(bPath))||((bGetSupportHdmiModeVideoTiming()>= MODE_3D_720p_50_FP)&&(bGetSupportHdmiModeVideoTiming()<= MODE_4096_2160P_24)))
 #endif
                     {
                         bIs444 = SV_FALSE;
                     }
	...
	||(TD_MIB_IN(u4DrvTDTVDIModeQuery()) ==E_TD_IN_FS_I||TD_MIB_IN(u4DrvTDTVDIModeQuery())==E_TD_IN_FS_P)
	)
	{
		bIs444 = SV_FALSE;
	}
...
UINT8 bDrvVideoSignalStatus(UINT8 bPath)
{
    ...
    switch(bType)
    {
        ...
        case SV_VD_DVI:
			...
			{
			#ifdef CC_HDMI_2_0_SUPPORT
				bRet = bDvi2SigStatus(bPath);
			//Printf("main bret=%d___________________________\n",bRet);
			#else
				bRet = bDviSigStatus();
			#endif
			}
			else
			{
				...
				else
				{
					#ifdef CC_HDMI_2_0_SUPPORT
						bRet = bDvi2SigStatus(bPath);
						//Printf("sub bret=%d___________________________\n",bRet);
					#else
						bRet = bDviSigStatus();
					#endif
				}
...
UINT16 wDrvVideoGetHTotal(UINT8 bPath)
{
    ...
    switch(bType)
    {
        ...
        case SV_VD_DVI:
			...
			{
			#ifdef CC_HDMI_2_0_SUPPORT
				wHTotal = wDvi2InputHTotal(bPath);
			#else
...
UINT16 wDrvVideoGetVTotal(UINT8 bPath)
{
    ...
    switch(bType)
    {
        ...
        case SV_VD_DVI:
			...
			{
			#ifdef CC_HDMI_2_0_SUPPORT
				wVTotal=wDvi2InputVTotal(bPath);
			#else
    ...
    switch(bType)
    {
        ...
        case SV_VD_DVI:
            ...
            if(eActiveHdmiPort >= u1GetMinHDMI20Port())
			{
				...
				if(!fgHDMI2GetDuleMode((HdmiRxPort)eActiveHdmiPort))
				{
					return _bDviTiming;
				}
				else
			#endif
				{
					return DviSta[bPath]._bDviTiming;
				}
			}
			else
			{
...
UINT8 bDrvVideoGetSourceTypeTiming(UINT8 bPath) reentrant
{
    ...
    switch(bVideoTiming)
    {
		...
		case MODE_4096_2160P_50:
		case MODE_4096_2160P_60:
		case MODE_3840_2160P_50:
		case MODE_3840_2160P_60:
		case MODE_4096_2160P_50_420:
		case MODE_4096_2160P_60_420:
...
UINT16 wDrvVideoSetPorch(UINT8 bPath, UINT8 bPorchType, UINT16 wValue)
{
	...
	#endif
					{
					min=wDvi2GetPorch(bPath,bPorchType+2);
	                max=wDvi2GetPorch(bPath,bPorchType+3);

	                if(tmp<min) tmp=min;

	                if(tmp>max) tmp=max;

	                vDvi2SetPorch(bPath,bPorchType,tmp);
	                wDecPorch=wDvi2GetPorch(bPath,bPorchType);
					}
				}
				else
#endif
...
{  
    ...
    else if (IS_IC_5861() && SV_VD_DVI == bGetVideoDecType(VDP_1))
    {
#ifdef CC_HDMI_2_0_SUPPORT
        activePort = eActiveHdmiPort; //main path for condition VDP_1
        if(activePort >= u1GetMinHDMI20Port())
        {
        	if(fgHDMI2GetDuleMode(activePort))//active sourse
        	{
        		return TRUE;/// HDMI query function should be here
        	}
            else
            {
                return FALSE;
