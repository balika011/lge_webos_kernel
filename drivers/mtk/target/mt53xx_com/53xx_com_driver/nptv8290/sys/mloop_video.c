...
void _vVdoSetDualMode(UINT8 u1SigSta)
{
    static UINT8 u1PreDualMode = 0;
    static UINT8 u1PreSigSta = 0;
    static UINT8 u1DualState = 0; //  1  dual setting  0  normal setting
    UINT8 u1DualMode = bDrvVideoIsDecDualMode();

    if (u1PreDualMode != u1DualMode || u1PreSigSta != u1SigSta)
    {
        if (u1DualMode && u1SigSta == (UINT8)SV_VDO_STABLE)
        {
            //VDP_SetEnable(VDP_2, TRUE);
            bApiVideoSetVideoSrc(VDP_2, bApiVideoGetSrcType(VDP_1));
            u1DualState = 1;
        }
        else if (u1DualState == 1 && 
                    (FALSE == u1DualMode))
        {
            //VDP_SetEnable(VDP_2, FALSE);
            bApiVideoSetVideoSrc(VDP_2, SV_VS_MAX);
            u1DualState = 0;
        }
        u1PreDualMode = u1DualMode;
        u1PreSigSta = u1SigSta;
    }
}
...
LOG(1, "***** MAIN_FLG_MODE_CHG*****\n");
...
#ifndef CC_MTK_LOADER				
#ifdef CC_SUPPORT_4K2K
				else if(IS_SUPPORT_4K2K_DISPLAY)
				{
                ...
                _vVdoSetDualMode(bDrvVideoSignalStatus(SV_VP_MAIN));
            }
...
/**
 * Check DRAM Ring Counter Error Status
 */
void vCheckDRAM_FOR_ERROR(void)
{   
    if (u4IO32Read4B(0xf0007320) != 0 || u4IO32Read4B(0xf0007324) != 0 ||
        u4IO32Read4B(0xf000f320) != 0 || u4IO32Read4B(0xf000f324) != 0 ||
        u4IO32Read4B(0xf0015320) != 0 || u4IO32Read4B(0xf0015324) != 0)
    {
        Printf("Check DRAM ERROR - DRAM register NG\n"); // keyword
        Printf("Check DRAM ERROR - reboot\n"); // keyword
        vIO32Write4B(0xf0028100, 0x0000000f);
        vIO32Write4B(0xf0028104, 0x7fffffff);
    }
}
...
void vApiVideoMainloop(void)
{
    ...
    if(fgIsBypassFlgClr(BYPASS_SRC_DETECT))
    {
        ...
        if(!_IsDviDetectDone)
        {
			activePot = fgIsMainDVI() ? eActiveHdmiPort : eActiveHdmiPort1;
            ...
            else
            {
				...
				if(fgHDMI2GetDuleMode(activePot))
				{
					...
					if(fgIsMainDVI())
					{
						vDvi2ModeDetect(SV_VP_MAIN);
					}
					if( fgIsPipDVI())
					{
						vDvi2ModeDetect(SV_VP_PIP);
					}
#ifdef CC_HDMI_OLD_NEW_SUPPORT
				}
				else
        ...
        else
        {
            ...
            else
            {
			   ...
			   if(fgHDMI2GetDuleMode(activePot))
			   {
				   ...
				   if(fgIsMainDVI())
		           {
		           	 vDvi2ChkModeChange(SV_VP_MAIN);
		           }
				   
				   if( fgIsPipDVI())
				   {
				   	  vDvi2ChkModeChange(SV_VP_PIP);
				   }
#ifdef CC_HDMI_OLD_NEW_SUPPORT
			   }
			   else
