...
UINT32 u4DrvGfxOutputMute(void)
{
    ...
    {
        OSD_PLA_GetFastMute(u4Plane, &u4Mute);
        if (u4Mute == 0)
        {
            return 0;
        }
...
VOID vDrvVideoHandleUnMute(VOID)
{
    ...
    for(u4Path = SV_VP_MAIN; u4Path <= SV_VP_PIP; u4Path++)
    {
        ...
        if(_au4LastMuteState[u4Path] != u4MuteOnOFF)
        {
			...
			DRVCUST_SendEvent(E_CUST_PLANE_MUTE_CHANGE, u4Path);
            ...
            if ((_au4LastMuteState[SV_VP_MAIN] == SV_ON) && (u4MuteOnOFF == SV_OFF))
            {
                ...
                if (_rVideoMuteInfo[SV_VP_MAIN][MUTE_MODULE_MJC].u4Delay < u4Delay &&
                    _eApplicationMode != SRM_TV_MODE_TYPE_PIP &&
                    _eApplicationMode != SRM_TV_MODE_TYPE_POP)
                {
