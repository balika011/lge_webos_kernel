...
void vVRMUpdateRegenResolution(UINT8 bPath)
{
    ...
    if (SV_VP_PIP == bPath && bDrvVideoIsDecDualMode())
    {
        VRM_INFO_T MainVrmInfo;
        preVrmInfo = VRMGetVRMInfo(SV_VP_PIP, VRM_MODULE_REGEN);
        MainVrmInfo = *VRMGetVRMInfo(SV_VP_MAIN, VRM_MODULE_REGEN);
        x_memcpy((void*)preVrmInfo, (void*)(&MainVrmInfo), sizeof(VRM_INFO_T));
        return;
    }
...
void vVRMUpdateMIBResolutionWithoutOvsn(UINT8 bPath)
{
  ...
  if(bPath ==  SV_VP_MAIN &&
	  ...
	  )//for mheg5 test
    {
        ...
        if((getMixedHPorch(bPath) >0) && (SV_FALSE==u4DrvTDTVIs3DPhoto(NULL)))
        {
...
void vVRMUpdateMIBResolutionWithOvsn(UINT8 bPath)
{
    ...
    if (SV_VP_PIP == bPath && bDrvVideoIsDecDualMode())
    {
        VRM_INFO_T MainVrmInfo;
        preVrmInfo = VRMGetVRMInfo(SV_VP_PIP, VRM_MODULE_MIB);
        MainVrmInfo = *VRMGetVRMInfo(SV_VP_MAIN, VRM_MODULE_MIB);
        MainVrmInfo.u2X = 0;
        x_memcpy((void*)preVrmInfo, (void*)(&MainVrmInfo), sizeof(VRM_INFO_T));
        return;
    }
    ...
    u4HPDSFactor = (mibIn->u2Width<pdsIn->u2Width) ?  mibIn->u2Width *DFT_PDS_FACTOR / pdsIn->u2Width:
        DFT_PDS_FACTOR;
    u4VPDSFactor =(mibIn->u2Height<pdsIn->u2Height) ? (mibIn->u2Height *DFT_PDS_FACTOR+ (pdsIn->u2Height-1))/pdsIn->u2Height:
        DFT_PDS_FACTOR;
    ...
    if(curVrmInfo.u2Width > 1920 || bDrvVideoIsDecDualMode()) // 4k2k SC 3D mode => SBS
    {//for 4k, scaler needs 4 alignment
        u2CalAlign = (u2CalAlign << 1) + 0x1;
    }
	if (bDrvVideoIsDecDualMode() && VDP_1 == bPath)
	{
		curVrmInfo.u2X += ( curVrmInfo.u2Width & u2CalAlign ); // cannot cuat righ side for dual mode
	} 
	
    curVrmInfo.u2Width &= (~u2CalAlign);  //alignment
    ...
    //Final protection
    if(curVrmInfo.u2X > (mibIn->u2Width - curVrmInfo.u2Width))
    {
        curVrmInfo.u2X = mibIn->u2Width - curVrmInfo.u2Width;
    }
    if(curVrmInfo.u2Y > (mibIn->u2Height - curVrmInfo.u2Height))
    {
...
void vVRMUpdateFSCResolutionWithOvsn(UINT8 bPath)
{
    ...
    if(curVrmInfo.u2Width > 1920) //422 case
    {//for 4k, scaler needs 4 alignment
        u2CalAlign = (u2CalAlign << 1) + 0x1;
    }
    curVrmInfo.u2Width &= (~u2CalAlign);  //alignment
    ...
    //Final protection
    if(curVrmInfo.u2X > (fscIn->u2Width - curVrmInfo.u2Width))
    {
        curVrmInfo.u2X = fscIn->u2Width - curVrmInfo.u2Width;
    }
    if(curVrmInfo.u2Y > (fscIn->u2Height - curVrmInfo.u2Height))
    {
        curVrmInfo.u2Y = fscIn->u2Height - curVrmInfo.u2Height;
    }
...
inline void vVRMUpdateMixerResolutionWithMJC(UINT8 bPath)
{
    ...
    if(SV_VP_MAIN==bPath)
    {
        ...
        if(E_TD_OUT_3D_LI==eMJCOUT || (IS_DISP_3D_LBRB && E_TD_OUT_3D_FS==eMJCOUT))    // mjc do 1920*1080 + 1920*1080 = 1920 * 2160 in all output format when panel is L12R12 PDP
        {
            curVrmInfo.u2Height /=2;
            curVrmInfo.u2Y /= 2;
            curVrmInfo.u2VTotal /=2;
...
inline void vVRMUpdateMixerResolutionWithoutMJC(UINT8 bPath)
{
  ...
  //  E_TD_OUT eMIBOUT = TD_MIB_OUT(u4MIB);
    ...
    if((SV_VP_MAIN==bPath)&&u4DrvTDTVIs3DPhoto(&ePhotoCrop))
    {
        if(IS_DISP_4K2K_TYPE5)//for oryx 4KDD module, CC_SUPPORT_UHD  // pre_dn.MIB is not bypass,PQ_path output 2k2k@60
        {
        	curVrmInfo.u2Width = dispInfo->wWidth;
        	curVrmInfo.u2Height = dispInfo->wHeight;

			curVrmInfo.u2X = (u4GetPlaneWidth(VDP_PQPATH)- curVrmInfo.u2Width)/2;
			curVrmInfo.u2Y = (u4GetPlaneHeight(VDP_PQPATH) - curVrmInfo.u2Height)/2;		
        }	
		else  //pre_down/MIB by_pass case , PQ path output 4k2k@30
		{
    ...
    #ifdef CC_MT5890
    #ifdef CC_SUPPORT_4K2K
    else if(SV_VP_PIP == bPath && IS_SUPPORT_4K2K_DISPLAY && (GetPANEL4K2K_TYPE() == PANEL_TYPE_4K1K_60))
    {
        curVrmInfo.u2X += 1920; // sub at right half
    }
    #endif
    #endif
...
void vVRMUpdateVDPResolution(UINT8 bPath)
{
    VRM_INFO_T* vdpIn;
    VRM_INFO_T* preVrmInfo;
    VRM_INFO_T curVrmInfo;
    UINT8 bBypassPath = vDrvGetTconTopVdoSel();

    vdpIn = VRMGetVRMInfo(bPath, VRM_MODULE_MIXER);
    preVrmInfo = VRMGetVRMInfo(bPath, VRM_MODULE_VDP);
    x_memcpy((void*)&curVrmInfo, (void*)vdpIn, sizeof(VRM_INFO_T));
    ...
    if (bBypassPath)
    {
        ...
        else if (bPath == SV_VP_PIP)
        {
            curVrmInfo.u2X = 0;
            curVrmInfo.u2Y = 0;
            curVrmInfo.u2Width  = u4ScpipGetPanelWidth(E_GFX_TG);
            curVrmInfo.u2Height = u4ScpipGetPanelHeight(E_GFX_TG);
            curVrmInfo.u2HTotal = u4ScpipGetPanelHtotal(E_GFX_TG);
            curVrmInfo.u2VTotal = u4ScpipGetPanelVtotal(E_GFX_TG);
        }
    }
    else
    {
        curVrmInfo.u2X = 0;
        curVrmInfo.u2Y = 0;
        curVrmInfo.u2Width  = u4GetPlaneWidth(VDP_PQPATH);
        curVrmInfo.u2Height = u4GetPlaneHeight(VDP_PQPATH);
        curVrmInfo.u2HTotal = wDrvGetOutputHTotal();
        curVrmInfo.u2VTotal = wDrvGetOutputVTotal();

        if (bPath == SV_VP_PIP)
        {
            curVrmInfo.u2X = u4GetPlaneWidth(VDP_PQPATH);
        }
    }
    ...
    {
        x_memcpy((void*)preVrmInfo, (void*)(&curVrmInfo), sizeof(VRM_INFO_T));
        LOG(3, "vVRMUpdateVDPResolution: (%d, %d, %d).\n",
...
void vVRMUpdatePSCMixerResolution(UINT8 bPath)
{
    ...
    if(eMJCOUT != E_TD_OUT_NATIVE)
    {
        ...
        else
        {
            ...
            curVrmInfo.u2Y = dispInfo->wYOffset;
            curVrmInfo.u2Width = dispInfo->wWidth;
            if (eMJCOUT == E_TD_OUT_3D_FS && IS_DISP_3D_LBRB)
            {
                curVrmInfo.u2Y /= 2;
                curVrmInfo.u2Height = dispInfo->wHeight >> 1;
                curVrmInfo.u2FrameRate <<= 1;
            }
        }
    }
...
UINT32 u4VRMSetTriggerMode(UINT8 bPath)
{
    ...
    else if(bDrvVideoIsDecDualMode())
    {
        LOG(1, "path(%d) Dual Hybrid Mode,  set import module to NONE\n",bPath);
        vDrvSetImportTriggerModule(bPath, IMPORT_MODULE_NONE);
    }
#if defined(VRM_TURNOFF_IMPORT_AT_3D)
    else if(bPath == VDP_1 &&
            (TD_TTD_IN(u4DrvTDTVTTDModeQuery())  != E_TD_IN_2D_P||
            TD_TTD_OUT(u4DrvTDTVTTDModeQuery())  != E_TD_OUT_NATIVE)
           )
    {
...
UINT8 vVRMGetImportDelay(UINT8 bPath)
{
    ...
    if(u1Delay == 0)
    {
        ...
        u1Delay = (bDrvVideoIsSrcInterlace(bPath)) ? 4 : 2; 

        #ifdef CC_FLIP_MIRROR_SUPPORT
        {
            UINT32 u4FlipMirrorModule;
            
            u4FlipMirrorModule = u4GetFlipMirrorModule(bPath);
            if (FLIP_BY_PSCAN == u4FlipMirrorModule || FLIP_BY_SCALER == u4FlipMirrorModule)
            {
                u1Delay++;
            }
        }
        ...
        if(VRM_IMPORT_WAIT_FLAG_TRIG_END & vDrvGetImportWaitFlag(bPath))
        {
            ...
            vDrvClrImportWaitFlag(bPath, VRM_IMPORT_WAIT_FLAG_TRIG_END);
				...
				if(bPath == SV_VP_MAIN)
				{
					VRM_INFO_T* vrmInfo1;
					VRM_INFO_T* vrmInfo2;
					vrmInfo1 = VRMGetVRMInfo(bPath, VRM_MODULE_MIXER);
					vrmInfo2 = VRMGetVRMInfo(bPath, VRM_MODULE_OSTG);

					if((vrmInfo1->u2Width > 1920) || (vrmInfo1->u2Height > 1080) || (vrmInfo2->u2HTotal- vrmInfo2->u2Width > 1023))
					{
						DRVCUST_SharpnessEnhanceEn(SV_OFF);
					}
					else
					{
						DRVCUST_SharpnessEnhanceEn(SV_ON);
					}
...
void vDrvVrmSetAppStage(void)
{
    ...
    if (u4DrvVrmGetAppFlag() & VRM_APP_GRAPHIC_DUMP)
    {
        ...
        case GFX_MON_OUT:
            ...
            break;
		 case GFX_HYBRID_4K:
			 vrmInfo  =  VRMGetVRMInfo(SV_VP_MAIN,  VRM_MODULE_MIXER);
			 u2Width  =  vrmInfo->u2Width;
			 u2Height =  vrmInfo->u2Height;
			 u2Scan   =  vrmInfo->u2ScanMode;
            break;
...
BOOL bDrvVrmIsAppScalerFifoDramLimit(UINT8 bPath)
{
    if (bPath == VDP_2)
    {
         if (bDrvVrmIsAppFlagSet(VRM_APP_POP_PIP) ||
             bDrvVrmIsAppFlagSet(VRM_APP_SCART_OUT)
             )
        {
            return TRUE;
...
void vVrmUpdateOvsnModule(UINT8 bPath)
{
    ...
    if(bPath <= SV_VP_PIP)
    {
        ...
        {
            if(getScalerMode(bPath) == VDP_SCPOS_DISPMODE_OFF && !bDrvVideoIsDecDualMode())
            {
