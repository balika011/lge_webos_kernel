...
void vVRMUpdateVdpInforForMonOut( UINT8 u1MonOutMuxIn )
{
	...
	if(pMonOutInfo->u1MonOutInSel == E_MON_OUT_PANEL  ||
		(pMonOutInfo->u1MonOutInSel == E_MON_OUT_MIXER && !(fgIsMJCToOSTG()))
		)
	{
...
void vVRMUpdateMonOutRegenResolution( void )
{
	...
	pMonOutInfo->wXOffset = 0;
	pMonOutInfo->wYOffset = 0;	
	pMonOutInfo->wTargetW = pMonOutInfo->wSrcW;
	pMonOutInfo->wTargetH = pMonOutInfo->wSrcH;
	...
	if ( pMonOutInfo->u1MonOutInSel == E_MON_OUT_PANEL )
	{
	    ...
	    {
	    	pMonOutInfo->wXOffset = u4ScpipGetHsyncBp(E_FSC_TG);
	    	pMonOutInfo->wYOffset = u4ScpipGetVsyncBp(E_FSC_TG);
	    	pMonOutInfo->wTargetW = u4ScpipGetPanelWidth(E_FSC_TG);
	    	pMonOutInfo->wTargetH = u4ScpipGetPanelHeight(E_FSC_TG);
	    }

	}

	if( pMonOutInfo->u1MonOutInSel == E_MON_OUT_MIXER && !(fgIsMJCToOSTG()))
	{
    	pMonOutInfo->wXOffset = u4ScpipGetHsyncBp(E_FSC_TG);
    	pMonOutInfo->wYOffset = u4ScpipGetVsyncBp(E_FSC_TG);
    	pMonOutInfo->wTargetW = u4ScpipGetPanelWidth(E_FSC_TG);
	...
	pMonOutInfo->wXOffset =pMonOutInfo->wXOffset +  DIVIDE_WITH_ROUND(pMonOutInfo->wTargetW * pMonOutInfo->rRegenScan.u4X,VDP_MAX_REGION_WIDTH);
	pMonOutInfo->wYOffset =pMonOutInfo->wYOffset +  DIVIDE_WITH_ROUND(pMonOutInfo->wTargetH * pMonOutInfo->rRegenScan.u4Y,VDP_MAX_REGION_HEIGHT);
	pMonOutInfo->wTargetW = DIVIDE_WITH_ROUND(pMonOutInfo->wTargetW * pMonOutInfo->rRegenScan.u4Width,VDP_MAX_REGION_WIDTH);
	pMonOutInfo->wTargetH = DIVIDE_WITH_ROUND(pMonOutInfo->wTargetH * pMonOutInfo->rRegenScan.u4Height,VDP_MAX_REGION_HEIGHT);  
...
void vVRMMonOutUpdatePredownResolution(void)
{
	...
	//1280 fifo limit
 	if (IS_IC_5861())
 	{
		if((pMonOutInfo->wPdsOutWidth > 1920 && (pMonOutInfo->wPdsInWidth > pMonOutInfo->wPdsOutWidth)) ||
        (pMonOutInfo->wPdsOutWidth > 1920 && (pMonOutInfo->wPdsInHeight > pMonOutInfo->wPdsOutHeight))
        )
		{
			LOG(1, "Monitor out pre-down have 1920 line buffer limitation\n");
			pMonOutInfo->wPdsOutWidth = 1920;
		}
 	}
	else
	{
...
{    
    ...
    if( (_vrm_mon_out_info.wXOffset  != _rMonitorOutInfo.wXOffset ) || 
	    (_vrm_mon_out_info.wYOffset != _rMonitorOutInfo.wYOffset) ||
	    (_vrm_mon_out_info.wTargetW != _rMonitorOutInfo.wTargetW ) ||
	    (_vrm_mon_out_info.wTargetH!= _rMonitorOutInfo.wTargetH)
    )       
    {
        vVRMFireMonOutModeChange( VRM_MON_OUT_REGEN_CHG );
    }
