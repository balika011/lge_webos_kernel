...
#endif
static UINT16 _bPlaneEnable[OUTPUT_PLANE_NS];
static UINT16 _u2PlaneWidth[OUTPUT_PLANE_NS];
static UINT16 _u2PlaneHeight[OUTPUT_PLANE_NS];
//-----------------------------------------------------------------------------
// Static variables
//-----------------------------------------------------------------------------
...
/**
 * VDP Set Out Region Control BY backend IC.
 *
 * @ TRUE     control by backend IC
 * @ False    control by mtk IC
 */
 

#ifdef VDP_OutPutWindSrcRegenCtlByBE
BOOL VDP_GetOutRegionControlByBE(UINT8 bPath)
{
    BOOL bIsOutRegionControlByBE = FALSE; 
    if(IS_DISP_4K2K_TYPE3)
	{
		if((bPath == VDP_1 && vDrvGetTconTopVdoSel() == E_VB1_PATH_PQ && 
		    (SRM_GetTvMode() == SRM_TV_MODE_TYPE_NORMAL || SRM_GetTvMode() == SRM_TV_MODE_TYPE_PIP))
		    || (bPath == VDP_2 && vDrvGetTconTopVdoSel() == E_VB1_PATH_BYPASS)
		   )
		    
		{
		    bIsOutRegionControlByBE = FALSE;  // control by mtk IC
		}
		else
		{
		    bIsOutRegionControlByBE = TRUE;  //control by BE IC
		}
	}
    LOG(3, "OutPut Region Control By %s\n",bIsOutRegionControlByBE == TRUE ? "Backend IC":"MTK IC");
        
    return bIsOutRegionControlByBE;

}

BOOL VDP_GetSrcRegionControlByBE(UINT8 bPath)
{
    BOOL bIsSrcRegionControlByBE = FALSE;

    if(bPath == VDP_1 && vDrvGetTconTopVdoSel() == E_VB1_PATH_BYPASS)
    {
        bIsSrcRegionControlByBE = TRUE; //control by BE IC
    }
    else
    {
        bIsSrcRegionControlByBE = FALSE; // control by mtk IC
    }

    LOG(3, "Source Region Control By %s\n",bIsSrcRegionControlByBE == TRUE ? "Backend IC":"MTK IC");
    
    return bIsSrcRegionControlByBE;
}
#endif
...
static UINT8 CalculateOutputRegion(UINT8 bPath, const VDP_DISP_REGION_T *prIn, VDP_DISP_REGION_T *prOut)
{
    ...
    VDP_DISP_REGION_T dispRegion; // 1/10000 based
    ...
    u1Plane = u1GetOutputPlane(bPath);
    if (bGetPlaneEnable(u1Plane) == 0)
    {
        return (SV_FAIL);
    }
    u4Width  = u4GetPlaneWidth(u1Plane);
    u4Height = u4GetPlaneHeight(u1Plane);
    if (u4Width == 0 || u4Height == 0)
    {
        return (SV_FAIL);
    }

    dispRegion.u4X = prIn->u4X;
    dispRegion.u4Y = prIn->u4Y;
    dispRegion.u4Width  = prIn->u4Width;
    dispRegion.u4Height = prIn->u4Height;

    //transfer (0~10000) to pixel-based output window
    prOut->u4X      = DIVIDE_WITH_ROUND(u4Width  * dispRegion.u4X,      VDP_MAX_REGION_WIDTH);
    prOut->u4Width  = DIVIDE_WITH_ROUND(u4Width  * dispRegion.u4Width,  VDP_MAX_REGION_WIDTH);
    prOut->u4Y      = DIVIDE_WITH_ROUND(u4Height * dispRegion.u4Y,      VDP_MAX_REGION_HEIGHT);
    prOut->u4Height = DIVIDE_WITH_ROUND(u4Height * dispRegion.u4Height, VDP_MAX_REGION_HEIGHT);
...
static UINT8 CalculateInputRegion(UINT8 u1Path, const VDP_PIC_OVERSCAN_REGION_T *prOverScan,  const VDP_DISP_REGION_T *prIn, VDP_DISP_REGION_T *prOut)
{
    ...
    prOut->u4Y = DIVIDE_WITH_ROUND(rOverScaned.u4Height * prIn->u4Y,VDP_MAX_REGION_HEIGHT);
    prOut->u4Width = DIVIDE_WITH_ROUND(rOverScaned.u4Width * prIn->u4Width,VDP_MAX_REGION_WIDTH);
    prOut->u4Height = DIVIDE_WITH_ROUND(rOverScaned.u4Height * prIn->u4Height,VDP_MAX_REGION_HEIGHT);
...
static void _vVideoUpdate121DispRegion(UINT8 bPath,VDP_DISP_REGION_T *rOutRegion)
{
    UINT8 u1Plane;   
    UINT32 u4Width, u4Height;
    UINT32 u4VdpId;
    ...
    if(_arScalePrm[u4VdpId].u4121MapEn == SV_ON) //121 is enabled, calculate and overwrite output region setting.
    {
        ...
        {
            rOutRegion->u4X  = (((u4Width -(UINT32)picInfo->wTargetW) * VDP_MAX_REGION_WIDTH) / u4Width);
        }
        else // middle
        {
            rOutRegion->u4X  = ((((u4Width -(UINT32)picInfo->wTargetW) * VDP_MAX_REGION_WIDTH) / u4Width) /2);
        }

        rOutRegion->u4Y = ((((u4Height -(UINT32)picInfo->wTargetH) * VDP_MAX_REGION_HEIGHT) / u4Height) /2);
        rOutRegion->u4Width = (((UINT32)picInfo->wTargetW * VDP_MAX_REGION_WIDTH) / u4Width) ;
        rOutRegion->u4Height = (((UINT32)picInfo->wTargetH * VDP_MAX_REGION_HEIGHT)/ u4Height) ;

        //error handling
        rOutRegion->u4X      = (rOutRegion->u4X      >VDP_MAX_REGION_WIDTH)  ?VDP_MAX_REGION_WIDTH:  rOutRegion->u4X;
        rOutRegion->u4Y      = (rOutRegion->u4Y      >VDP_MAX_REGION_HEIGHT) ?VDP_MAX_REGION_HEIGHT: rOutRegion->u4Y;
        rOutRegion->u4Width  = (rOutRegion->u4Width  >VDP_MAX_REGION_WIDTH)  ?VDP_MAX_REGION_WIDTH:  rOutRegion->u4Width;
        rOutRegion->u4Height = (rOutRegion->u4Height >VDP_MAX_REGION_HEIGHT) ?VDP_MAX_REGION_HEIGHT: rOutRegion->u4Height;
...
static UINT8 bVideoUpdateDispRegion(UINT8 bPath, UINT16 wXOff, UINT16 wYOff, UINT16 wWidth, UINT16 wHeight)
{
    ...
    #ifdef VDP_OutPutWindSrcRegenCtlByBE
    if (VDP_GetOutRegionControlByBE(bPath)) // don't care VDP out region, overwrite with full screen
    {
        if(wWidth != 0 && wHeight != 0)
        {
            wXOff   = (UINT16)0;
            wYOff   = (UINT16)0;
            wWidth  = (UINT16)u4GetPlaneWidth(u1GetOutputPlane(bPath));
            wHeight = (UINT16)u4GetPlaneHeight(u1GetOutputPlane(bPath));  
        }   
	    LOG(0,"send evnet E_CUST_SCALING_REQ_CHANGE by bPath(%d)-------\n",bPath);
		DRVCUST_SendEvent(E_CUST_SCALING_REQ_CHANGE, bPath);
		DRVCUST_SendEvent(E_CUST_SRC_AREA_CHANGE, bPath);
    }
    #endif
...
UINT8 bVideoUpdateSrcRegion(UINT8 bPath)
{
    ...
    UINT8 u1Ret = SV_SUCCESS;

    prPicInfo = getPicInfo(bPath);
    rSrcRegion = prPicInfo->rSrcRegion;
    rOverScan = prPicInfo->rOverScan;
    ...
    LOG(3, "bVideoUpdateSrcRegion(%d): SRC(%d, %d, %d, %d), OVERSCAN(%d, %d, %d, %d)\n",
        bPath, rSrcRegion.u4X, rSrcRegion.u4Y, rSrcRegion.u4Width, rSrcRegion.u4Height,
        rOverScan.u4Top, rOverScan.u4Bottom, rOverScan.u4Left, rOverScan.u4Right);


    if(VDP_GetScalerPixelBased() == 0)
    {
        /////////////////////// Dual Mode convertion ////////////////
        if (bDrvVideoIsDecDualMode())
        {
            if (VDP_1 == bPath)
            {
                rOverScan.u4Left = (rOverScan.u4Left > rOverScan.u4Right) ? rOverScan.u4Left : rOverScan.u4Right;
                rOverScan.u4Left <<= 1;
                rOverScan.u4Right = 0;
                
                rSrcRegion.u4X <<= 1;
                if (rSrcRegion.u4Width != 0)
                {
                    rSrcRegion.u4Width = VDP_MAX_REGION_WIDTH - rSrcRegion.u4X;
                }
                
            }
            else
            {
                rOverScan  = getPicInfo(VDP_1)->rOverScan;
                rOverScan.u4Right = (rOverScan.u4Left << 1);
                rOverScan.u4Left   = 0;

                rSrcRegion  = getPicInfo(VDP_1)->rSrcRegion;
                rSrcRegion.u4X = 0;
            }
        }
        /////////////////////////////////////////////////////////
        ...
        //Check data range
        if((rSrcRegion.u4Width == 0) || (rSrcRegion.u4Height == 0))
        {
            return (SV_FAIL);
        }

        //Check data range
        if(((rSrcRegion.u4X + rSrcRegion.u4Width) > VDP_MAX_REGION_WIDTH) ||
           ((rSrcRegion.u4Y + rSrcRegion.u4Height) > VDP_MAX_REGION_HEIGHT))
        {
            return (SV_FAIL);
        }

        //Convert width, height into pixel based after overscan & source region. (Output stored in rOutRegion)
        if(CalculateInputRegion(bPath, &(rOverScan), &(rSrcRegion), &rOutRegion) != SV_SUCCESS)
        {
            return (SV_FAIL);
        }
        ...
        SRM_SendEvent(SRM_DRV_SCPOS, (SRM_SCPOS_EVENT_SRC_RATIO + (UINT32)bPath)
                      , rSrcRegion.u4Width -rOverScan.u4Left-rOverScan.u4Right
                      , rSrcRegion.u4Height -rOverScan.u4Top-rOverScan.u4Bottom);
#endif
    ...
    else    //pixel-based
    {
        ...
        if (bDrvVideoIsDecDualMode())
        {
            if (VDP_1 == bPath)
            {
                if (rSrcRegion.u4Width != 0)
                {
                    rSrcRegion.u4Width = prPicInfo->wSrcW - rSrcRegion.u4X;
                }
        ...
        u4XOff = rSrcRegion.u4X;
        u4YOff = rSrcRegion.u4Y;
        u4Width = rSrcRegion.u4Width;
        u4Height = rSrcRegion.u4Height;
    }
    ...
    if (bDrvVideoIsDecDualMode() && bPath == VDP_1)
    {
        u1Ret = bVideoUpdateSrcRegion(VDP_2);
    }
    return (u1Ret);
}
...
/**
 * Set / Get VDP width/height
 * Pixel based
 */
void vSetPlaneWidth(UINT32 u4Plane, UINT32 u4Width)
{
    if (u4Plane >= OUTPUT_PLANE_NS)
    {
        return;
    }
    _u2PlaneWidth[u4Plane] = u4Width;
}

UINT32 u4GetPlaneWidth(UINT32 u4Plane)
{
    if (u4Plane >= OUTPUT_PLANE_NS)
    {
        return 0;
    }
    return _u2PlaneWidth[u4Plane];
}

void vSetPlaneHeight(UINT32 u4Plane, UINT32 u4Height)
{
    if (u4Plane >= OUTPUT_PLANE_NS)
    {
        return;
    }
    _u2PlaneHeight[u4Plane] = u4Height;
}

UINT32 u4GetPlaneHeight(UINT32 u4Plane)
{
    if (u4Plane >= OUTPUT_PLANE_NS)
    {
        return 0;
    }
    return _u2PlaneHeight[u4Plane];
}

void vSetPlaneEnable(UINT32 u4Plane, UINT16 bEnable)
{
    if (u4Plane >= OUTPUT_PLANE_NS)
    {
        return;
    }
    _bPlaneEnable[u4Plane] = bEnable;
}

UINT8 bGetPlaneEnable(UINT32 u4Plane)
{
    if (u4Plane >= OUTPUT_PLANE_NS)
    {
        return 0;
    }
    return _bPlaneEnable[u4Plane];
}

void vSetOutputPlane(UINT32 u4VdpId, UINT32 u4Plane)
{
    UINT8 bPath;
    ...
    scalerInfo->u4OutputPlane = u4Plane;
}

UINT8 u1GetOutputPlane(UINT32 u4VdpId)
{
    UINT8 bPath;
    VDP_SCALER_PRM_T* scalerInfo;
    ...
    bPath = getScalerPath(u4VdpId);
    scalerInfo = getScalerInfo(bPath);
    return scalerInfo->u4OutputPlane;
}

void VDP_SetPlaneSize(UINT8 u1Plane, UINT32 u4Width, UINT32 u4Height)
{
    vSetPlaneWidth(u1Plane, u4Width);
    vSetPlaneHeight(u1Plane, u4Height);
    fireVRMModeChange(SV_VP_MAIN, VRM_EVENT_BY_VDP);
    fireVRMModeChange(SV_VP_PIP,  VRM_EVENT_BY_VDP);
}
