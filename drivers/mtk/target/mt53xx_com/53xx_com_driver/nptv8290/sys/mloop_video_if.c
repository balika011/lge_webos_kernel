...
EXTERN UINT16 u2AmbiLitArray[1920];
EXTERN UINT8 u1AmbiLitBLKArray[640];
#endif
...
static void _OutputVSyncIsr(UINT16 u2Vector)
{
    ...
    if(u2Vector == VECTOR_POST_PROC)
    {
		...
		if(u4TCONIrqStatus & INTR_AMBILIGHT_ISR)
		{
#if SUPPORT_AMBILIGHTV2
			vDrvAmbiLitUpdateLine(u2AmbiLitArray, u1AmbiLitBLKArray, u1DrvAmbiLitGetStrapID()); 	
			//LOG(0,"vDrvAmbiLitUpdateLine\n");
...
void vAllocateSWReg(void)
{
    ...
    SW_DRAM_BASE = (SW_DRAM_BASE_PHY | DRAM_A_BASE);
#else
    Printf("===========  Linux  ==============\n");
...
BOOL VdoMLInit(void)
{
    ...
    // to do
#else
	vDrvCvbsVfePD();    // for source detect
#endif
