...
UINT32 u4ScalerCoefMask[204] = 
{
0x00000000, 0x00000FFF, 0xFFFFFFFF, 0x03FFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
0x00000000, 0x00000FFF, 0xFFFFFFFF, 0x03FFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
0x00000000, 0x00000FFF, 0xFFFFFFFF, 0x03FFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
0x00000000, 0x00000FFF, 0xFFFFFFFF, 0x03FFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
0x00000000, 0x00000FFF, 0xFFFFFFFF, 0x03FFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
...
BOOL bGammaStressTest(UINT8 u1TablePattern)
{
    ...
    for(i = 0; i < 257; i++)
    {
        u2GammaDecodedTable[0][i] = MIN(4095, u2GammaDecodedTable[0][i]);
        u2GammaDecodedTable[1][i] = MIN(4095, u2GammaDecodedTable[1][i]);
        u2GammaDecodedTable[2][i] = MIN(4095, u2GammaDecodedTable[2][i]);   
    }
...
BOOL bSCEStressTest(UINT8 u1TablePattern)
{
    ...
    // 1. write test data into default table
    vDrvGenSRAMTestTable((UINT8*)u2GammaDecodedTable, 624, u1TablePattern);
...
BOOL bScalerStressTest(UINT8 u1TablePattern)
{
    ...
    // 1. write test data into default table
    vDrvGenSRAMTestTable((UINT8*)u4FilterCoeff, 816, u1TablePattern);
    ...
    // 3. read data back and compare
    vDrvCheckUpScalerFilter(SV_VP_MAIN, UPSCALER_HORI, 0);
    ...
    // 3. read data back and compare
    vDrvCheckUpScalerFilter(SV_VP_MAIN, UPSCALER_VERT, 0);
    if(vDrvCheckUpScalerFilter(SV_VP_MAIN, UPSCALER_VERT, 0) == SV_FALSE)
    {
        bRet = SV_FALSE;
    }
...
BOOL bLcDimStressTest(UINT8 u1TablePattern)
{
    ...
    // 3. read data back and compare
    vDrvVerifyLcDimMappingCurve();
    if(vDrvVerifyLcDimMappingCurve() == SV_FALSE)
    {
        bRet = SV_FALSE;
    }
    ...
    {    
        ...
        {        
            ...
            case 2:        
                ...
                break;
            case 3:        
                bRet = bLCSharpnessStressTest(u1TablePattern);
                break;
            case 4:        
...
BOOL bHalVideoVerifyGammaTable(UINT8 bMode)
{
    ...
    return (bDrvGammaReadTable(SV_RGB_R) && bDrvGammaReadTable(SV_RGB_R) && bDrvGammaReadTable(SV_RGB_R));
}
...
void vHalVideoGammaStressTest(void)
{
    ...
    {
    	vApiFlashPqSetGammaId(u4Mode);
        bApiFlashPqUpdateGamma();      
        vHalVideoGammaLoadTable(SV_ON);
        
        vUtDelay1ms(16);
        
        if (bHalVideoVerifyGammaTable(0))
        {
