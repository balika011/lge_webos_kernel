...
#endif
    LOG(1, "VDP(%d) Enable(%d) -> (%d)\n", ucVdpId, _arVdpConf[ucVdpId].ucVdpEnable,  ucEnable);
...
BOOL VDP_IsSupportOverscan(UCHAR ucVdpId)
{
	...
	if(fgIs4k2kNativeTimingOut(ucVdpId))
	{
		fgSupported = SV_FALSE;
	}
#endif	
...
UINT32 VDP_SetBg(UCHAR ucVdpId, UINT32 u4BgColor)
{
    ...
    vDrvVideoSetMuteColor(ucVdpId, u4BgColor);
      //vDrvVideoMuteColor(ucVdpId, u4BgColor);
#endif
    return VDP_SET_OK;
}
...
#if defined(CC_MT5398) || defined(CC_MT5368) || defined(CC_MT5389)
...
void VDP_GetDramInf(UINT8 bPath,DRAM_INFO_T *inf)
{
    ...
    UINT8 FlipMirrorCfg     = u1GetFlipMirrorConfig();

    //change flip&mirror flag index. high is flip, low is mirror.
     FlipMirrorCfg = ((FlipMirrorCfg & 0x1) << 1) + ((FlipMirrorCfg) >> 1);
#endif
    ...
    if(VDP_SCPOS_DISPMODE_OFF == u1ScpipGetDispMode(bPath))
    {
        ...
        {       
            inf->scaler_inf.u1CfgFlipMirror = FlipMirrorCfg;        
            inf->scaler_inf.u1DramFlipMirror = 1;    
        }
             ...
             bGetSignalType(bPath) != SV_ST_VGA  && !SRM_IsGameMode(bPath)))
    {
        ...
        if(FlipMirrorModule == FLIP_BY_B2R)    //b2r enable flip & mirror at the same time
        {
            //inf->scaler_inf.dram_from |= 0x10;
            //inf->scaler_inf.dram_from |= 0x20;
            inf->DI_inf.u1CfgFlipMirror = FlipMirrorCfg;         
            inf->DI_inf.u1DramFlipMirror = 1;
        }
        else
        {
