...
void vSet3DRightPosedge(UCHAR ucPosedge)
{
    vIO32WriteFldAlign(IRTX_IRTX1CFG,((TRUE == ucPosedge)?1:0),FLD_IRTX1_RIGHTPOSEDGE);
}

void vSet3DSonyMode(UCHAR ucSonyMode)
{
    vIO32WriteFldAlign(IRTX_IRTX1CFG,((TRUE == ucSonyMode)?1:0),FLD_IRTX1_SONY_MODE);
}

void vSet3DRealMode(UCHAR ucRealMode)
{
    vIO32WriteFldAlign(IRTX_IRTX1CFG,((TRUE == ucRealMode)?1:0),FLD_IRTX1_REAL_MODE);
}
...
void IRTX_SetRealDMode(UINT32 uA1Dealy,UINT32 uB1Dealy,UINT8 uMode,UINT8 uFreq)
{
	UINT32 uA1Cmd,uB1Cmd;
	//UINT32 u4A1DealayVal,u4B1DealayVal;
	if(uFreq != 120 && uFreq != 100) //only support 120Hz and 100Hz.
		return;

	//u4A1DealayVal = 391*uA1Dealy;//Y=(1000000/(120*2)/255)*X*24.
	//u4B1DealayVal = 784*uB1Dealy;
	
	if(uMode == 2) //mode2
	{
		uA1Cmd = 0xF3000000;
		uB1Cmd = 0xCF000000;
		vSet3DLogic0Pulse(0x001,0x400);
		vSet3DLogic1Pulse(0x200,0x200);
		vSet3DBitNumber(8);
		vSet3DHWDelay(0x00,0x00,uA1Dealy,uB1Dealy);
		vIO32WriteFldAlign(IRTX_IRTX1CFG,1,FLD_IRTX1_REAL_MODE);//Real 3D mode.
		vIO32WriteFldAlign(IRTX_IRTX1CFG,1,FLD_IRTX1_IGNORE_LEFT);//set bit 5
		vSet3DHWMode(uA1Cmd,uB1Cmd,0xF3000000,0xCF000000);
	}
	else if(uMode == 5) //mode5
	{
		uA1Cmd = 0xF3000000;
		uB1Cmd = 0xCF000000;
		vSet3DLogic0Pulse(0x001,0x400);
