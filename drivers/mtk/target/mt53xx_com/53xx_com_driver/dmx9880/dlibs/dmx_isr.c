...
#endif  // ENABLE_MULTIMEDIA
        {
			...
			if(prPidStruct->fgTS130 && !fgAC3PesHeader)
			{
				...
				&rPesHeader))
				{
					...
					rPes.fgContainAD = FALSE;
#endif  // CC_DMX_PES_AUDDESC
                   	//if pes has header ,save it.
                    prPidStruct->u4AudioPrePts = rPes.u4Pts; 
				}
				else
				{
...
static void _DmxProcessScrambleChange(UINT8 u1Pidx, BOOL fgScrambled)
{
    ...
    if (prPidStruct->pfnScramble != NULL)
    {
        ...
        fgRet = prPidStruct->pfnScramble(u1Pidx, DMX_NOTIFY_CODE_SCRAMBLE_STATE, 
			(UINT32)prPidStruct->eScrambleStatus, prPidStruct->pvScrambleTag);
...
static void _DmxPvrErrorInt(void)
{
    ...
    LOG(2, "PVR error! INT: 0x%08x, PID = 0x%x\n", u4Status, u2Pid);
...
static void _DmxDispatchFTuPInterrupt(void)
{
    ...
    while (1)
    {
        ...
        DMX_PANIC((u1Pidx < DMX_NUM_PID_INDEX) || (u1Pidx < FVR_NUM_PID_INDEX));
        DMX_PANIC_RET_VOID();
