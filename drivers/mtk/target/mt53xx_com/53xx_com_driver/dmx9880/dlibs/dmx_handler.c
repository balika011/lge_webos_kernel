...
static UINT8 _au1H264SliceNum[DMX_NUM_PID_INDEX];

/// VC1
static BOOL _afgInterlacedSource[DMX_NUM_PID_INDEX];
static INT32 _ai4CurrentTemporalField[DMX_NUM_PID_INDEX]; // 0:1st field or frame picture, 1: 2nd field
static UINT8 _au1SecondFieldType[DMX_NUM_PID_INDEX];
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectMPEGPictureType(UINT8 u1Pidx, UINT32 u4Addr, UINT32 u4BufStart,
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectH264PictureType(UINT8 u1Pidx, UINT32 u4Addr, UINT32 u4BufStart,
    UINT32 u4BufEnd, UINT8 *pu1PicInfo)
{
    UINT32 u4PicType;
    UINT8* p = NULL;
    UINT8 u1Type, h[PIC_HEADER_LEN], i, u1SliceNum;
    ...
    case H264_PIC_START_NONIDR:
        ...
        {
        	u1SliceNum = _au1H264SliceNum[u1Pidx];
        	u1SliceNum++;
        	_au1H264SliceNum[u1Pidx] = u1SliceNum;
        }
        break;
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectMPEG4PictureType(UINT8 u1Pidx, UINT32 u4Addr, UINT32 u4BufStart,
    UINT32 u4BufEnd, UINT8 *pu1PicInfo)
{
    UINT32 u4PicType;
    UINT8* p = NULL;
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectVC1PictureType(UINT8 u1Pidx, UINT32 u4Addr, UINT32 u4BufStart,
    UINT32 u4BufEnd, UINT8 *pu1PicInfo)
{
    UINT32 u4PicType;
    UINT8* p = NULL;
    ...
    case VC1_PIC_START_CODE_SEQ:
        _afgInterlacedSource[u1Pidx] = ((p[9] & 0x40) == 0x40) ? 1 : 0;
    ...
    case VC1_PIC_START_CODE_FRAME:
        if( _ai4CurrentTemporalField[u1Pidx] == 1 )
        {
            u1FrameCodingMode = (UINT8)INTERLACEFIELD;
        }
        else
        {
            if (_afgInterlacedSource[u1Pidx])
            {
        ...
        {
            if(_ai4CurrentTemporalField[u1Pidx] == 0)
            {
                ...
                _au1SecondFieldType[u1Pidx] = _u1vopSecondFieldType[u4Code];
            }
            else
            {
                u4PicType = _au1SecondFieldType[u1Pidx];
            }
            _ai4CurrentTemporalField[u1Pidx] ^= 1;
        }
        break;
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectWMVPictureType(UINT8 u1Pidx, UINT32 u4Addr, UINT32 u4BufStart,
    UINT32 u4BufEnd, UINT8 *pu1PicInfo)
{
    UNUSED(u4Addr);
    UNUSED(u4BufStart);
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectAVSPictureType(UINT8 u1Pidx, UINT32 u4Addr, UINT32 u4BufStart,
    UINT32 u4BufEnd, UINT8 *pu1PicInfo)
{
    UINT32 u4PicType;
    UINT8* p = NULL;
 ...
 */
//-----------------------------------------------------------------------------
static UINT32 _DmxDetectPictureType(UINT8 u1Pidx, DMX_VIDEO_TYPE_T eVideoType, UINT32 u4Addr, UINT32 u4BufStart, UINT32 u4BufEnd, UINT8 *pu1PicInfo)
{
    ...
    {
    case DMX_VIDEO_MPEG:
        u4Type = _DmxDetectMPEGPictureType(u1Pidx, u4Addr, u4BufStart, u4BufEnd, pu1PicInfo);
        break;

    case DMX_VIDEO_H264:
        u4Type = _DmxDetectH264PictureType(u1Pidx, u4Addr, u4BufStart, u4BufEnd, pu1PicInfo);
        break;

#ifdef ENABLE_MULTIMEDIA
    case DMX_VIDEO_MPEG4:
        u4Type = _DmxDetectMPEG4PictureType(u1Pidx, u4Addr, u4BufStart, u4BufEnd, pu1PicInfo);
        break;

    case DMX_VIDEO_VC1:
        u4Type = _DmxDetectVC1PictureType(u1Pidx, u4Addr, u4BufStart, u4BufEnd, pu1PicInfo);
        break;

    case DMX_VIDEO_WMV7:
    case DMX_VIDEO_WMV8:
    case DMX_VIDEO_WMV9:
        u4Type = _DmxDetectWMVPictureType(u1Pidx, u4Addr, u4BufStart, u4BufEnd, pu1PicInfo);
        break;
#endif  // ENABLE_MULTIMEDIA

    case DMX_VIDEO_AVS:
        u4Type = _DmxDetectAVSPictureType(u1Pidx, u4Addr, u4BufStart, u4BufEnd, pu1PicInfo);
        break;
...
static BOOL _DmxProcessPictureHeader(DMX_PES_MSG_T* prPes)
{
    ...
    if(eVideoType == DMX_VIDEO_H264)
    {
        ...
        {
            prPes->u1SliceCount = _au1H264SliceNum[u1Pidx];
            _au1H264SliceNum[u1Pidx] = 0;
        }
    }
    ...
    while (1)
    {
        ...
        case DMX_PES_MSG_TYPE_PES:
            if (!rPes.fgEOS && !rPes.fgMoveComplete && (prPidStruct->ePidType == DMX_PID_TYPE_ES_VIDEO))
            {
                ...
                {
                    #if defined(CC_TRUSTZONE_SUPPORT) && defined(CC_SVP_SUPPORT)
                    rPes.u4FrameType = _DmxDetectPictureType(rPes.u1Pidx, _DMX_GetPidxVideoType(rPes.u1Pidx),
...
BOOL _DMX_InitHandler(void)
{
    ...
    for (i = 0; i < DMX_NUM_PID_INDEX; i++)
    {
        ...
        _afgGop[i] = FALSE;
        _afgSeqEnd[i] = FALSE;
        _au1H264SliceNum[i] = 0;

        _afgInterlacedSource[i] = FALSE;
        _ai4CurrentTemporalField[i] = 0;
        _au1SecondFieldType[i] = 0;
...
BOOL _DMX_ClearPicFlag(UINT8 u1Pidx)
{
    ...
    _afgInterlacedSource[u1Pidx] = FALSE;
    _ai4CurrentTemporalField[u1Pidx] = 0;
    _au1SecondFieldType[u1Pidx] = 0;
