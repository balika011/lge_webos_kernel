...
BOOL DMX_EMU_PreByte_Test(UINT8 u1TsIndex, BOOL fgExtSync, UINT32 u4Size, UINT16 u2PktSize)
{
	...
	//diable framer befor reset framer
	u4Ctrl = IO_READ32(DEMUX0_BASE, 0x20) & 0xFFFFF000;
	IO_WRITE32(DEMUX0_BASE, 0x20, u4Ctrl);

    // Set framer, must after setpktsize
    u4Ctrl = IO_READ32(DEMUX0_BASE, 0x20) & 0xFFF0F000;
    u4Ctrl |= 0x30000;
    if(fgExtSync)
    {
        u4Ctrl |= 0xFFF;
