...
static BOOL _PVR_ResetTransfer(UINT32 u4Addr, UINT32 u4Size, BOOL fgIntr)
{
    ...
    while(1)
    {
        ...
        if (u4Rp == u4Wp)
        {
			...
			//In polling mode , when wp == rp ,we needn't reset PVR play control logic.
			//we need some time wait pvr play back transmit complete !
			//we will do PVR play control logic in _DMX_PVRPLAY_STOP()
