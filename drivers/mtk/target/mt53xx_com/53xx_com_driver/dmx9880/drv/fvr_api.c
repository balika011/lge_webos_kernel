...
static BOOL _SetFVRBuffer(UINT8 u1Idx, const FVR_GBL_T* prGBL)
{
    ...
    FVR_GBL_ARY_W(u1Idx, 8) = u4PicBuf;
                        ...
                        FVR_PID_STRUCT_T *prPid)
{
    ...
    else
    {
        ...
        FVR_PER_PID_S_W(u1Pidx, 2) = FVR_PER_PID_S_W(u1Pidx, 2) | 0x0000FF00;
...
BOOL _FVR_DropScrambledPacket(UINT8 u1Idx, BOOL fgEnalbe)
{
    ...
    if(u1Idx == 0)
    {
		u4Reg = FVR_GBL_ARY_W(0, 7) & (~0x01000000);
		if(fgEnalbe)
		{
            u4Reg = u4Reg | (0x01000000);
		}
		FVR_GBL_ARY_W(0, 7) = u4Reg;
    }
    else if(u1Idx == 1)
    {
		u4Reg = FVR_GBL_ARY_W(1, 7) & (~0x01000000);
		if(fgEnalbe)
		{
            u4Reg = u4Reg | (0x01000000);
		}
		FVR_GBL_ARY_W(1, 7) = u4Reg;
    }
    else if(u1Idx == 3)
    {
		u4Reg = FVR_GBL_ARY_W(3, 13) & (~0x01000000);
		if(fgEnalbe)
		{
            u4Reg = u4Reg | (0x01000000);
		}
        FVR_GBL_ARY_W(3, 13) = u4Reg;
    }
    else
    {
        // unexpected index
