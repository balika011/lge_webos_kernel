...
BOOL _DMX_AudHandler_SetActivePidx(UINT8 u1Idx)
{
    ...
    //LOG(1, "%s, Line: %d, Pidx:%d !\n", __FUNCTION__, __LINE__, u1Idx);
...
BOOL _DMX_AudHandler_Set(UINT8 u1Idx, BOOL fgToDec)
{
    ...
    //LOG(1, "%s, Line: %d, ToDec:%s !\n", __FUNCTION__, __LINE__, fgToDec ? "TRUE" : "FALSE");
...
BOOL _DMX_AudHandler_SeekAndUpdate(UINT8 u1Idx, UINT32 u4Pts, UINT32 *pu4Pts)
{
    ...
    *pu4Pts = 0;
    ...
    for(i=0; i<u2QSize; i++)
    {
        ...
        LOG(5, "%s, pts:0x%x, RP:0x%X, WP:0x%X\n", __FUNCTION__, 
            rElement.u4Pts, rElement.u4PesRp, rElement.u4Wp);
...
BOOL _DMX_AudHandler_UpdateRp(UINT8 u1Idx, UINT32 u4Size)
{
    ...
    while(1)
    {
        ...
        {
             LOG(3,"Flush Pidx(%u) Buffer When record size=0.\r\n",u1Idx);
