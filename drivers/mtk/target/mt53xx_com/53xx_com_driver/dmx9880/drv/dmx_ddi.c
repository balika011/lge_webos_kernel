...
#ifdef CC_DMX_PS_CPU
    u4Reg |= 0x8;                               // set Output Delay Byte to 3
#else
...
void _DMX_DDI_SetPortEx(DMX_DDI_PORT_T ePort, UINT8 u1Pidx, BOOL fgForce,  BOOL fgTs)
{
    ...
    else if(ePort == DMX_DDI_PORT_FRAMER3)
    {
        ...
        if(fgTs)
        {
            ...
            /* set up error handling table, Cobra has the individual table for framer3 */
            DMXCMD_WRITE32(DMX_REG_DBM_BUF_CTRL,
                DMXCMD_READ32(DMX_REG_DBM_BUF_CTRL) | (0xf0c0U << 16));
        }
        else
        {
            UINT32 u4Ctrl;
            ...
            /* set up error handling table, Cobra has the individual table for framer3 */
            DMXCMD_WRITE32(DMX_REG_DBM_BUF_CTRL,
                DMXCMD_READ32(DMX_REG_DBM_BUF_CTRL) & ~(0xffffU << 16));
        }
    }
    else if(ePort == DMX_DDI_PORT_NO_FRAMER)
    {
