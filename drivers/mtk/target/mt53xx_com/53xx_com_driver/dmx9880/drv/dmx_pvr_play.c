 ...
 */
//-----------------------------------------------------------------------------
static BOOL _DMX_PVRSetupProlog(UINT32 *pu4BufStart, UINT32 *pu4BufEnd)
{
    UINT32 u4SyncWord, i;

    ASSERT(pu4BufStart != NULL);
    ASSERT(pu4BufEnd   != NULL);
    ASSERT(_rPVRPlay.u1SyncOffset < MAX_SYNC_BYTE_OFFSET);

    // Setup two TS null packets for Internal Sync Lock at the stream head.
    // Null Packet (payload only): 0x471FFF10 + 0xFF...0xFF
    *pu4BufStart = (UINT32)_rPVRPlay.pu1Prolog;
    *pu4BufEnd   = (UINT32)_rPVRPlay.pu1Prolog + PVR_PROLOG_BUF_SIZE;

    if (_rPVRPlay.u1PrevSyncOffset != _rPVRPlay.u1SyncOffset)
    {
        x_memset((void*)_rPVRPlay.pu1Prolog, 0xFF, PVR_PROLOG_BUF_SIZE);

        u4SyncWord = 0x10FF1F47;
        for (i = 0; i < PVR_PROLOG_PACKETS; i++)
        {
            x_memcpy((void*)((*pu4BufStart) +
                (i * _rPVRPlay.u1PacketSize) + _rPVRPlay.u1SyncOffset),
                &u4SyncWord, sizeof(u4SyncWord));
        }

        HalFlushInvalidateDCache();
    }
    _rPVRPlay.u1PrevSyncOffset = _rPVRPlay.u1SyncOffset;
 ...
 */
//-----------------------------------------------------------------------------
BOOL _DMX_PVRPlay_SendNullPackets(void)
{
    BOOL fgRet = TRUE;
    DMX_MM_DATA_T rData;
    ...
    {
        LOG(3, "Please stop PVR before sending NULL packets!\n");
        fgRet = FALSE;
        goto EXITA;
    }
   
    /* save original mode and timestamp setting */ 
    if (!_DMX_PVRPlay_Get((UINT32)(PVRPLAY_FLAGS_MODE | PVRPLAY_FLAGS_TIMESTAMP), &rOrigPlay))
    {
        fgRet = FALSE;
        goto EXITA;
    }
    ...
    VERIFY(_DMX_PVRSetupProlog(&u4PrologBufStart, &u4PrologBufEnd));
    ...
    rData.u4FrameSize = _rPVRPlay.u4PrologSize;
...
EXITB:
    ...
    if (!_DMX_PVRPlay_Set((UINT32)(PVRPLAY_FLAGS_MODE | PVRPLAY_FLAGS_TIMESTAMP), &rOrigPlay))
    {
        fgRet = FALSE;
    }
...
BOOL _DMX_PVRPlay_Init()
{
    ...
    if ((_DMX_GetIsPanicReset()) || (!_fgInit))
    {
    _DMX_Lock();
    _rPVRPlay.u1PrevSyncOffset = PVR_MAX_PACKET_SIZE;
    _fgTSOut = FALSE;
        _DMX_Unlock();
    }
    ...
    if(!_DMX_SetFramerPktSize(DMX_FMR2, 188))
    {
        return FALSE;
    }

#ifndef CC_DMX_EMULATION
    _DMX_SetFramerMode( DMX_FMR2, 
                        DMX_FRAMER_PARALLEL, /*unused*/ 
                        FALSE, /*internal sync*/
                        TRUE); /*clock edge, unused for FMR2*/
#endif
    ...
    if(!_fgInit)
    {
        ...
        pu1Prolog = (UINT8*)BSP_AllocAlignedDmaMemory(PVR_PROLOG_BUF_SIZE,
...
BOOL _DMX_PVRPlay_Set(UINT32 u4Flags, const DMX_PVR_PLAY_T* prPVRPlay)
{
    ...
    if(u4Flags & PVRPLAY_FLAGS_TIMESTAMP)
    {
        ...
        if (_rPVRPlay.fgContainTimeStamp)
        {
            _rPVRPlay.u1PacketSize = PVR_TS_PACKET_SIZE;
            _rPVRPlay.u1SyncOffset = 4;
            _rPVRPlay.u4PrologSize = PVR_TS_PACKET_SIZE * PVR_PROLOG_PACKETS;
        }
        else
        {
            _rPVRPlay.u1PacketSize = PVR_PACKET_SIZE;
            _rPVRPlay.u1SyncOffset = 0;
            _rPVRPlay.u4PrologSize = PVR_PACKET_SIZE * PVR_PROLOG_PACKETS;
        }
    }
