...
{
    UINT32 u4VirtBufStart, u4PhyBufStart, u4W0, u4BufSize;
    ...
    UINT32* pu4PidWord0, PID_STRUCT_T* prPidStruct, BOOL fgKeepScramble)
{
    ...
    if (fgEnable)
    {
        ...
        if (prPidStruct->ePidType == DMX_PID_TYPE_ES_AUDIO)
        {
	  	...
	  	//if enable audio pid ,init u4AudioPrePts again.
        if(prPidStruct->ePidType == DMX_PID_TYPE_ES_AUDIO)
        {
          prPidStruct->u4AudioPrePts = 0 ; 
        }
        ...
        if ((prPidStruct->ePidType == DMX_PID_TYPE_ES_AUDIO) || (prPidStruct->ePidType == DMX_PID_TYPE_PES_AUDIO))
        {
            ...
            for(i = 0; i < DMX_NUM_PID_INDEX; i++)
            {
                ...
                if((i != u1Pidx) && (u4PidIndexTable & 0xFFFF0000U) == ((1U << 31) | (*pu4PidEntry & 0x7FFF0000)))
                {
        ...
        if (prPidStruct->ePidType == DMX_PID_TYPE_PES_AUDIO)
        {
            ...
            for(i = 0; i < DMX_NUM_PID_INDEX; i++)
            {
                ...
                if((i != u1Pidx) && (u4PidIndexTable & 0xFFFF0000U) == ((1U << 31) | (*pu4PidEntry & 0x7FFF0000)))
                {
                    if(_arPidStruct[i].ePidType == DMX_PID_TYPE_ES_AUDIO)
                    {
                        // already ES PID, set bit[9] =1 and second PID index
                        PID_S_W(i, 0) &= ~(1 << 9);
                        PID_S_W(i, 22) &= ~(0xFFU << 24);
                    }
                }
            }
        }
                              ...
                              const DMX_FILTER_GENERIC_T* prFilter)
{
    ...
    if ((u4Flags & DMX_FILTER_FLAG_VALID) != 0)
    {
        u4FilterWord &= ~0x80000000;
        u4FilterWord |= ((prFilter->fgEnable ? 1 : 0) << 31);
        DMXSECLOG(DMX_LOG_PSI 6, u1FilterIndex, "[PSI PARSER]the Section Filter %d,Pid index is %d\n",u1FilterIndex,_arFilterStruct[u1FilterIndex].rGeneric.u1Pidx);
    ...
    if ((u4Flags & DMX_FILTER_FLAG_VALID) != 0)
    {
        DMXSECLOG(DMX_LOG_PSI 6, u1FilterIndex, "%s filter %u\n", prFilter->fgEnable ? "Enable" : "Disable",
            u1FilterIndex);
    }
...
BOOL _DMX_SetSeamlessPID(UINT8 u1NewPidx, UINT8 u1OldPidx, UINT16 u2Pid)
{
    ...
    PID_S_W(u1NewPidx, 0) = PID_S_W(u1NewPidx, 0) | 0x100;
...
BOOL _DMX_SetIgnoreCCCheck(UINT8 u1Pidx, BOOL fgEnable)
{
    ...
    if(fgEnable)
    {
        PID_S_W(u1Pidx, 0) = PID_S_W(u1Pidx, 0) | (0x1 << 10);
    }
    else
    {
        PID_S_W(u1Pidx, 0) = PID_S_W(u1Pidx, 0) & ~(0x1 << 10);
        ...
        case EIT_SET_SECTION:
            {
                ...
                if((u4FltIndex >= CHANNEL_MAX_NUM) || (u1TableId > 0x6f) || (u1TableId < 0x50))
                {
                    ...
                    {
                        if((u1TableId == 0x4E) || (u1TableId == 0x4F))
                        {                                                      
                            return TRUE;   
                        }
                        else			
                        {
                            LOG(7, "CHANNEL_MAX_NUM u4FltIndex:%d, tableid: 0x%x error \n",u4FltIndex, u1TableId);
                            return FALSE;                        
                        }
                    }
                }
