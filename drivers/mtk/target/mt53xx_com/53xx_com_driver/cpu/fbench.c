...
{
    double d = 1.4e-45;
    /* Cast double variable whose value is 1.4e-45 to float variable. */
    float f = (float)d;
    float expected = FLT_MIN;

    printf("\nac_test_platform_02 double to float casting test\n");

    printf("Converted Value is %f.\n", f);
    printf("FLT_MIN is %f.\n", FLT_MIN); 

    /* Verify the casted variable is same to the expected float variable
     * whose value is 1.401298e-45. */
    if (f > 0.0f && f <= expected) {
        printf("TEST PASSED\n"); 
    } else {
        if (f == 0.0f) {
            printf("Converted Value should not be %f.\n", f);
        } else {
            printf("Converted Value should be smaller than %f.\n",
                    expected);
        }
        printf("TEST FAILED\n"); 
    }
