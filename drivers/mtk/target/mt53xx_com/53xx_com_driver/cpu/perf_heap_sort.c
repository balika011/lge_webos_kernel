...
void run_HeapSort(void)
{
  ...
  num = 30000;  //^^*((volatile int*)NUM_ITEM_ADDR);
  printf("--------< Heap Sort %0d items Begin >--------\n", num);
  ...
  printf("Heap Sort %0d items is done.\n", num);
#endif
}
...
// Heap sort: Uses non sequential memory access.
void Heap_Sort(int* array, int N)
{
  int end;
  int tmp;

  end = N - 1;  // N is array size, end is last *valid* element
