 ...
 *---------------------------------------------------------------------------*/
// ------------------------------------------------------------
// Example PMU test program for Cortex-A/R family core
//
...
int pmu_test(void)
{
    ...
    pmn_config(1, 0x04);       // Configure counter 0 to count event code 0x03
    pmn_config(2, 0x05);       // Configure counter 0 to count event code 0x03
    pmn_config(3, 0x06);       // Configure counter 0 to count event code 0x03
    pmn_config(4, 0x07);       // Configure counter 0 to count event code 0x03
    ...
    for (j=0; j<1024; j++)
    {
        for (i=0; i<64; i++)
        {
            k += *(UINT8*)(0x100000+i*16);
        }
        for (i=0; i<64; i++)
        {
            *(UINT8*)(0x100000+i*16) = k;
        }
    }
    ...
    printf("C0=%d, C1=%d, C2=%d, C3=%d, C4=%d, cycle=%d, k=%d\n",
        read_pmn(0), read_pmn(1), read_pmn(2), read_pmn(3), read_pmn(4), read_ccnt(), k);
