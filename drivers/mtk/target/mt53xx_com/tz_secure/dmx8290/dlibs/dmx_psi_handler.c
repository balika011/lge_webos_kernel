...
static BOOL _DmxPSIParser(BOOL fgFVR, UINT8 u1Pidx, UINT8 u1SecCount, UINT32 u4EndAddr, UINT8 u1SN)
{
    ...
    if ((u4SecRp + (DATASIZE(u4SecRp, u4EndAddr, u4BufLen))) > u4BufEnd)
    {
        HalInvalidateDCacheMultipleLine(VIRTUAL(u4SecRp), u4BufEnd - u4SecRp);
        HalInvalidateDCacheMultipleLine(VIRTUAL(u4BufStart), DATASIZE(u4SecRp, u4EndAddr, u4BufLen) + u4SecRp - u4BufEnd);
    }
    else
    {
        HalInvalidateDCacheMultipleLine(VIRTUAL(u4SecRp), DATASIZE(u4SecRp, u4EndAddr, u4BufLen));
    }
#endif
