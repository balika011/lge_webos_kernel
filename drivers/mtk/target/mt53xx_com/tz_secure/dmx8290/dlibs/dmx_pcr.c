...
static BOOL fgNotUpdateAStc[STC_SRC_NS]; 
#endif
...
void STC_SetSyncMode2(UINT8 ucStcId, UINT8 ucAdecId, UINT8 ucAdec2Id, UINT8 ucVdecId, UINT32 u4SyncMode)
{
    //STC_VERIFY_NULL(ucStcId);
    // Klocwork
    if(ucStcId >= STC_SRC_NS)
    ...
    case AV_SYNC_MODE_AUDIO_MASTER:
        if(ucStcId == STC_SRC_A1)
        {
            AUD_SetAvSynMode(ucAdec2Id, AV_SYNC_AUDIO_MASTER);
            AUD_SetAvSynMode(ucAdecId, AV_SYNC_AUDIO_MASTER);
    ...
    case AV_SYNC_MODE_VIDEO_MASTER:
        if(ucStcId == STC_SRC_A1)
        {
            AUD_SetAvSynMode(ucAdec2Id, AV_SYNC_SLAVE);
            AUD_SetAvSynMode(ucAdecId, AV_SYNC_SLAVE);
            AUD_SetAvSynMode(0, AV_SYNC_AUDIO_MASTER);
        }
        VDEC_SyncStc(ucVdecId, VID_SYNC_MODE_MASTER, ucStcId);
    ...
    default:
        ...
        if(ucStcId == STC_SRC_A1)
        {
            AUD_SetAvSynMode(ucAdec2Id, AV_SYNC_FREE_RUN);
            AUD_SetAvSynMode(ucAdecId, AV_SYNC_FREE_RUN);
            AUD_SetAvSynMode(0, AV_SYNC_AUDIO_MASTER);
        }
        VDEC_SyncStc(ucVdecId, VID_SYNC_MODE_NONE, ucStcId);
        if (_afgStcStart[ucStcId])
        {
            B2R_SetB2rUpdateStc(ucStcId,FALSE);
        }
        break;
    }
}
...
#ifdef CC_ENABLE_AOMX 
INT32 STC_SetCtrl(UINT8 u1StcId, UINT32 u4Ctrl, UINT32 u4Parm)
{
    if(u1StcId >= STC_SRC_NS)
	{
		return STC_SET_FAIL;
	...
	switch(u4Ctrl)
	{
	case STC_CTRL_NOT_UPDATE_ASTC:
		fgNotUpdateAStc[u1StcId] = (u4Parm > 0)? TRUE : FALSE;	
		break;
	}
