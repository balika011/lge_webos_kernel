...
void _DMX_PanicReset(void)
{
    ...
    DMX_VIDEO_TYPE_T au1TsIdxVcodeBase[DMX_FRAMER_COUNT];
    ...
    UNUSED(_DMX_PVRPlay_Stop(0, TRUE));
#endif  // TIME_SHIFT_SUPPORT
    //x_thread_delay(5);
    ...
    {
        prFilter = (DMX_FILTER_T*)x_mem_alloc((DMX_NUM_FILTER_INDEX * 2 * DMX_FRAMER_COUNT) * sizeof(DMX_FILTER_T));
        if(prFilter == NULL)
        {
            _u4DmxRecoveryErr = 5;
            goto PanicExit;
        }
        for (k = 0; k < DMX_FRAMER_COUNT; k++)
        {
            for(i=0; i<DMX_NUM_FILTER_INDEX; i++)
            {
                ...
                {
                    //init ts index.
                    prFilter[((i*2) + j) * k].u1Tsidx = k;
                    if(!_DMX_GetFilter(i, j, DMX_FILTER_FLAG_ALL, &prFilter[((i*2) + j) * k]))
                    {
                        _u4DmxRecoveryErr = 6;
                        goto PanicExit;
                    }
    ...
    {
        prGenFilter = (DMX_FILTER_GENERIC_T*)x_mem_alloc(DMX_NUM_FILTER_INDEX * DMX_FRAMER_COUNT * sizeof(DMX_FILTER_GENERIC_T));
        if(prGenFilter == NULL)
        {
            _u4DmxRecoveryErr = 7;
            goto PanicExit;
        ...
        for (k = 0; k < DMX_FRAMER_COUNT; k++)
        {
            ...
            {
                //init ts index.
                prGenFilter[i * k].u1Tsidx = k;
                if(!_DMX_GetGenFilter(i, DMX_FILTER_FLAG_ALL, &prGenFilter[i * k]))
                {
                    _u4DmxRecoveryErr = 8;
                    goto PanicExit;
                }
        ...
        for (k = 0; k < DMX_FRAMER_COUNT; k++)
        {
            for(i=0; i<DMX_NUM_FILTER_INDEX; i++)
            {
                for(j=0; j<2; j++)
                {
                    //init ts index.
                    prFilter[((i*2) + j) * k].u1Tsidx = k;
                    if(!_DMX_SetFilter(i, j, DMX_FILTER_FLAG_ALL, &prFilter[((i*2) + j) * k]))
                    {
    ...
    else
    {
        for (k = 0; k < DMX_FRAMER_COUNT; k++)
        {
            for(i=0; i<DMX_NUM_FILTER_INDEX; i++)
            {
                ...
                if(!_DMX_SetGenFilter(i, DMX_FILTER_FLAG_ALL, &prGenFilter[i * k]))
                {
