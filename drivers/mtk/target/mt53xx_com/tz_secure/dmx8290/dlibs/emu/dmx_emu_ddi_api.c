...
static BOOL _DDI_Transfer4(UINT32 u4Addr, UINT32 u4Size)
{
    ...
    while (u4Size > 0)
    {
        ...
        //For full push and half push/pull mode, it is not suitable to move data quickly.
        //Micro-code can't handle data in time if moving data quickly.
        //Reg 170C4 and 170CC contral the process rate of Micro-code.
        u4Wp += u4StepSize;
        if (u4Wp >= u4BufEnd)
        {
            u4Wp -= u4BufSize;
...
// Inter-file functions
//-----------------------------------------------------------------------------
static BOOL _DDI_Transfer6(UINT32 u4Addr, UINT32 u4Size)
{
    UINT32 u4BufStart, u4BufEnd, u4Wp;
...
#if defined(CC_MT5882)
    case 6:
		fgRet = _DDI_Transfer6(u4Addr, u4Size);  // do not wait for complete
		break;
#endif
    default:
        fgRet = FALSE;
...
BOOL _DMX_EMU_DDI_Init(UINT8 u1TsIndex)
{
    ...
    */
     //set delay from ddi->dmb
    //#if defined(CC_MT5890)
    u4Reg = DDI_READ32(DDI_REG_DMX_RX_CTRL) & (~0xF);
    u4Reg |= 0x6;    
    DDI_WRITE32(DDI_REG_DMX_RX_CTRL, u4Reg);
    //#endif
	...
	{
	   _DMX_DDI_SetPort(DMX_DDI_PORT_FRAMER4_BYPASS,TRUE);
	}
#endif
    else
    {
    ...
    LOG(7,"Set break to check the 0x17930.\r\n");
...
BOOL _DDI_RECPULL_Transfer(UINT8 u1TsIndex, UINT32 u4Addr, UINT32 u4Size)
{
    ...
    // Flash and invalidate cache
    HalFlushInvalidateDCache();
    //x_memset((void*)NONCACHE(0x3400000), 0xFF, 32);
//    x_thread_delay(10);
