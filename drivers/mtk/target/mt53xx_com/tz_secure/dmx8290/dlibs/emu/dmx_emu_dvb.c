...
static BOOL _Dmx_Emu_DvbInit(UINT8 u1TsIndex)
{
    ...
    _rDvbData[0].u1Pidx = 0 + random(16);
    _rDvbData[0].u2Pid = DMX_DVB_PID1;
    // Buffer for _rDvbData[0] is allocated somewhere else.
    Printf("PID index: %2d, TsIdx : %d, KeyIdx: %d.\n", _rDvbData[0].u1Pidx,u1TsIndex, _rDvbData[0].u1KeyIndex );
    ...
    _rDvbData[1].u1Pidx = 16 + random(16);
    _rDvbData[1].u2Pid = DMX_DVB_PID2;
    // Buffer for _rDvbData[1] is allocated somewhere else.
   Printf("PID index: %2d, TsIdx : %d, KeyIdx: %d.\n", _rDvbData[1].u1Pidx,u1TsIndex, _rDvbData[1].u1KeyIndex );
    ...
    _rDvbData[2].u1Pidx = 32 + random(16);
    _rDvbData[2].u2Pid = DMX_DVB_PID3;
    // Buffer for _rDvbData[2] is allocated somewhere else.
    Printf("PID index: %2d, TsIdx : %d, KeyIdx: %d.\n", _rDvbData[2].u1Pidx,u1TsIndex, _rDvbData[2].u1KeyIndex );
    ...
    _rDvbData[3].u1Pidx = 48 + random(16);
    _rDvbData[3].u2Pid = DMX_DVB_PID4;
    // Buffer for _rDvbData[3] is allocated somewhere else.
   Printf("PID index: %2d, TsIdx : %d, KeyIdx: %d.\n", _rDvbData[3].u1Pidx,u1TsIndex, _rDvbData[3].u1KeyIndex );
...
******************************************************************************/
static void _Dmx_Emu_DvbSetKeys()
{
    UINT32 i, j;
...
static BOOL _DmxEmu_DvbMain(UINT8 u1TsIndex, BOOL fgBypassFramer)
{
...
static BOOL _DmxEmu_RecDvbMain(UINT8 u1TsIndex ,BOOL fgBypassFramer)
{
    ...
    if (!FVR_SetGBL(u1TsIndex, FVR_GBL_FLAG_ALL, &rGblPid))
    {
        fgRet = FALSE;
        goto _exit;
    }
...
static BOOL _DmxEmu_RecDvbRecLevel2Test(UINT8 u1TsIndex, BOOL fgBypassFramer )
{
...
BOOL DMX_EMU_DvbMain(UINT8 u1TsIndex, UINT8 u1Type, BOOL fgBypassFramer)
{
    if (u1Type == 0)
    {
        return _DmxEmu_DvbMain(u1TsIndex, fgBypassFramer);
    }
    else if (u1Type == 1)
    {
        return _DmxEmu_RecDvbMain(u1TsIndex, fgBypassFramer);
    }
    else if (u1Type == 2)
    {
        return _DmxEmu_RecDvbRecLevel2Test(u1TsIndex, fgBypassFramer);
    }
