...
#endif
EXTERN DMX_NOTIFY_INFO_PTS_T _arDMXNotifyInfoPts;
...
static BOOL _DmxProcessVideoPesHeader(UINT8 u1Pidx, BOOL fgComplete)
{
    ...
    UINT32 u4PesRp, u4Wp, u4BufStart, u4BufEnd, u4BufLen, u4HeaderSize, u4PicInfoAddr, i;
...
_HeaderHandling:
    ...
    if ((u4PesRp + (DATASIZE(u4PesRp, u4Wp, u4BufLen))) > u4BufEnd)
    {
        HalInvalidateDCacheMultipleLine(u4PesRp, u4BufEnd - u4PesRp);
        HalInvalidateDCacheMultipleLine(u4BufStart, DATASIZE(u4PesRp, u4Wp, u4BufLen) + u4PesRp - u4BufEnd);
    }
    else
    {
        HalInvalidateDCacheMultipleLine(u4PesRp, DATASIZE(u4PesRp, u4Wp, u4BufLen));
    }
#endif
    ...
    DMXPIDLOG(DMX_LOG_PID DMX_LOG_AV 8, u1Pidx, "Found Picture count: %d\n", u4HwPicNum);
...
{
    UINT32 u4Wp, u4BufStart, u4BufEnd, u4BufLen, u4PesRp;
    UINT32 u4HeaderRp, u4HeaderWp, u4HeaderBufStart, u4HeaderBufEnd;
    ...
    if (fgIsToDecoder)
    {
...
#endif  // ENABLE_MULTIMEDIA
        {
                                   ...
                                   &rPesHeader))
            {
    			...
    			//audio pts to mw, the carrier is pes int nofify func,AUDIO no AD,not be disable output
    			if ((prPidStruct->ePidType == DMX_PID_TYPE_ES_AUDIO)&&(prPidStruct->fgDisableFifoOutput == FALSE)&&
    			(prPidStruct->eInputType == DMX_IN_PLAYBACK_TS)&&(prPidStruct->u1ContainAD == 0))
    			{
    			//confirm if this audio dev is audio master sync mode
        			if(STC_GetSyncMode(prPidStruct->u1DeviceId)==AV_SYNC_MODE_AUDIO_MASTER)
        			{
            			_arDMXNotifyInfoPts.u4Pts = rPes.u4Pts;
            			_arDMXNotifyInfoPts.u1TsIndex = prPidStruct->u1TsIndex;
            			LOG(3,"collect audio pts for pes,0x%x \n",rPes.u4Pts);
        			}                         
    			}
...
static BOOL _DmxProcessPes(UINT8 u1Pidx)
{
    ...
    if (prPidStruct->pfnNotify != NULL)
    {
        ...
        //in some scene,pes need audio's pts,TsIndex should same
        if(prPidStruct->u1TsIndex == _arDMXNotifyInfoPts.u1TsIndex)
        {
            rPes.u4Pts = _arDMXNotifyInfoPts.u4Pts;
        }
...
static void _DmxProcessScrambleChange(UINT8 u1Pidx, BOOL fgScrambled)
{
    ...
    DMXPIDLOG(DMX_LOG_SCRAMBLE 6, u1Pidx, "Pidx %u scrambling state: %s\n", u1Pidx, fgScrambled ? "SCRAMBLED" : "CLEAR");
...
static void _DmxPvrInt3(void)
{//DMX_REG_PVR_NONERR_STATUS_REG3
  
#ifdef ENABLE_MULTIMEDIA
	  UINT32 u4Status;
  
	  u4Status = DMXCMD_READ32(DMX_REG_PVR_NONERR_STATUS_REG3);
  
	  if (u4Status & (1 << 1))			  // Input buffer interrupt
	  {
		  _DMX_PVRPlay_Notify(2);
	  }
#endif  // ENABLE_MULTIMEDIA
	  // Clear interrupt
	  DMXCMD_WRITE32(DMX_REG_PVR_NONERR_STATUS_REG3, 1);
...
static void _DmxDbmErrorInt(void)
{
    ...
    {
	// ignore these errors
	LOG(5, "Ignore following dbm error.\n");
    }
	else
    {
#ifdef CC_DMX_TRACK_ERROR_INTS
...
//handle the error status for mini-pvr3
static void _DmxPvrErrorInt3(void)
{
    UINT32 u4Status;
    UINT16 u2Pid;

    u4Status = DMXCMD_READ32(DMX_REG_PVR_ERROR_STATUS_REG3);
    u2Pid = (UINT16)((u4Status >> 16) & 0x1fff);

    // Clear interrupt
    DMXCMD_WRITE32(DMX_REG_PVR_ERROR_STATUS_REG3, 1);

    LOG(2, "PVR error! Tsout 2,INT: 0x%08x, PID = 0x%x\n", u4Status, u2Pid);
...
//handle the ts2 playback error.
static void _DmxPlaybackSteerErrorInt2(void)
{
    return ;
}


// handle the ts3 playback error.
static void _DmxPlaybackSteerErrorInt3(void)
{
  return;
}
...
static void _DmxDispatchFTuPInterrupt(void)
{
    ...
    while (1)
    {
        ...
        DMX_PANIC(((u1Type < 64)&&(u1Pidx < DMX_NUM_PID_INDEX)) ||
                   ((u1Type >= 64)&&(u1Pidx < FVR_NUM_PID_INDEX))||
                  ((u1Type < 64)&&(u1Pidx >= DMX_FVR_START_PID) && (u1Pidx < (DMX_FVR_START_PID + FVR_NUM_PID_INDEX))));
        DMX_PANIC_RET_VOID();
...
//              ASSERT(0);
...
#endif  // ENABLE_MULTIMEDIA
        {
            ...
            else
            {
                ...
                case 2:                 // PSI,just for playback do section filter.
                    if (fgIsFVR)
                    {
...
static void _DmxDispatchInterrupt(UINT32 u4Status)
{
    ...
    if (u4Status & DMX_INT_ERR_MASK)
    {
        ...
        if(u4Status& DMX_INT_ERR_PLAYBACK_STERRING2)
        {
          _DmxPlaybackSteerErrorInt2();
        }
		if(u4Status& DMX_INT_ERR_PLAYBACK_STERRING3)
		{
		  _DmxPlaybackSteerErrorInt3();
		}
		if(u4Status & DMX_INT_ERR_PVR3)
		{
		  _DmxPvrErrorInt3();
		}
    ...
    if (u4Status & DMX_INT_STATUS_MASK)
    {
		...
		if (u4Status & DMX_INT_STATUS_PVR3)
		{
		    _DmxPvrInt3();
		}
    ...
    if (!fgRecuPErrorHandled)
    {
        ...
        if ((u1Pidx >= DMX_FVR_START_PID) && (u1Pidx < (DMX_FVR_START_PID + FVR_NUM_PID_INDEX)))
        {
            ...
            u1Idx = _FVR_GetRecordIdx(u1Pidx); //for ci+1.4 compatibility.
        }
        else
        {
            // Should not run this
