...
static unsigned long get_string(void *a, char* b, int c)
{
    LTC_ARGCHK(a != NULL);
    LTC_ARGCHK(b != NULL);
    LTC_ARGCHK(c != 0);
    return mp_get_string(a, b, c);
}
...
static void draw_hex(void *A)
{
  ...
  //dprintf("sizeof(mp_digit) %d\n", sizeof(mp_digit));
  
  dprintf("%d, %d, ", a->used, a->sign);
  for (x = a->used - 1; x >= 0; x--) {
     dprintf("%016lx ", a->dp[x]);
