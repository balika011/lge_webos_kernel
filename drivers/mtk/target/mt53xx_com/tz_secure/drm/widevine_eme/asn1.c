...
int asn1_get_next(const u8 *buf, size_t len, struct asn1_hdr *hdr)
{
	...
	if ((hdr->identifier & 0x1f) == 0x1f) {
		...
		do {
			if (pos >= end) {
				DMSG( "ASN.1: Identifier "
					   "underflow");
				return -1;
			}
			tmp = *pos++;
			DMSG( "ASN.1: Extended tag data: "
				   "0x%02x", tmp);
	...
	if (tmp & 0x80) {
		...
		hdr->length = 0;
		if (tmp > 4) {
			DMSG( "ASN.1: Too long length field");
			return -1;
		}
		while (tmp--) {
			if (pos >= end) {
		 ...
		 const u8 **next)
{
	...
	if (hdr.tclass != ASN1_CLASS_UNIVERSAL || hdr.tag != ASN1_TAG_OID) {
		DMSG( "ASN.1: Expected OID - found class %d "
			   "tag 0x%x", hdr.tclass, hdr.tag);
		return -1;
	}
