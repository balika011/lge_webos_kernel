...
crypto_rsa_import_public_key(const u8 *buf, size_t len)
{
	...
	if (pos != end) {
		//hexdump("RSA: Extra data in public key SEQUENCE",
			   // pos, end - pos);
		DMSG("RSA: pos != end"
			   "(public key) - found class %d tag 0x%x \n",
			   hdr.tclass, hdr.tag);
	...
	DMSG("crypto_rsa_import_public_key  return success \n");
	return key;
...
crypto_rsa_import_private_key(const u8 *buf, size_t len)
{
	...
	if (pos == NULL || bignum_cmp_d(zero, 0) != 0) {
		DMSG( "RSA: Expected zero INTEGER in the "
...
//			    pos, end - pos);
	...
	DMSG("crypto_rsa_import_private_key	return success \n");
	return key;
