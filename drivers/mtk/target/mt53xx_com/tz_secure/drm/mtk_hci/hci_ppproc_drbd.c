...
#define L_HASH_AND_PAD_SIZE     30

static uint32_t
l_DecryptPpprocMessage( uint32_t      in_enc_ppproc_data_len,
                        uint8_t      *in_enc_ppproc_data,
                        uint32_t     *out_dec_ppproc_data_len,
                        uint8_t     **out_dec_ppproc_data )
{
    uint32_t   ret                     = ERR_HCI_OK;
    uint8_t   *dec_ppproc_data         = NULL;
    uint32_t   dec_ppproc_data_len     = 0;
    np_size_t  ppproc_len              = 0;
    uint32_t   digest_len              = HCI_L_SHA1_DIGESTSIZE;
    uint8_t    iv[]                    = L_PPPROC_DIVERSITY_IV;
    uint8_t    digest[HCI_L_SHA1_DIGESTSIZE];

    MSGENT(("l_DecryptPpprocMessage"));

    /* check whether 2 bytes (i.e len field) are preset before we read them */
    if ( in_enc_ppproc_data_len < 2 ) {
        RET( ERR_HCI_ILLEGALPPPROC );
    }
    ppproc_len = byte_BE2R(in_enc_ppproc_data); /* get ppproc size in first 2 bytes */
    if ( (ppproc_len != (in_enc_ppproc_data_len - 2)) || (ppproc_len <= L_HASH_AND_PAD_SIZE) ) {
        RET(ERR_HCI_ILLEGALPPPROC);
    }
    in_enc_ppproc_data += 2;  /* skip length bytes */
    ppproc_len -= L_HASH_AND_PAD_SIZE;

    /* check hash */
    RET(hci_l_cryptoMakeDigest(HCI_DIG_ALGO_SHA1, ppproc_len, in_enc_ppproc_data, &digest_len, digest));
    if ( 0 != mem_Cmp(in_enc_ppproc_data + ppproc_len, digest, HCI_L_SHA1_DIGESTSIZE) ) {
        RET( ERR_HCI_ILLEGALPPPROC );
    }

    RET(hci_l_DecryptPpprocData(ppproc_len,
                                in_enc_ppproc_data,
    ...
    *out_dec_ppproc_data_len = dec_ppproc_data_len;
    *out_dec_ppproc_data     = dec_ppproc_data;
    dec_ppproc_data          = NULL;

EXIT:
    hci_l_DataScrub(dec_ppproc_data, dec_ppproc_data_len);
    hci_l_mem_Free(dec_ppproc_data);
    MSGEXT(("l_DecryptPpprocMessage"));
    return ret;
}
                     ...
                     uint8_t            **out_public_ppproc)
{
    ...
    if ( (HCI_ENCRYPTED_PPPROC_FORMAT == in_format)) {
        ...
        RET(l_DecryptPpprocMessage(in_ppproc_data_len ,
                                   in_ppproc_data,
                                   &dec_ppproc_data_len,
                                   &dec_ppproc_data));
...
#endif /* #if (PPPROC_FORMAT_VAL == HCI_L_PPPROC_FORMAT_MOA_DRBD) */
