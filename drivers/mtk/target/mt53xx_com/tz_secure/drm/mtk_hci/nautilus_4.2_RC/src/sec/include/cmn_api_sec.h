...
#define binstr_Alloc(in_size, in_buf) \
    npi_binstr_Alloc(NPI_CMN_CONTEXT_SECURE, in_size, in_buf)
...
#define binstr_Free(io_str) \
    npi_binstr_Free(NPI_CMN_CONTEXT_SECURE, io_str)
...
#define binstr_CmpBuf(in_len1, in_buf1, in_len2, in_buf2) \
    npi_binstr_CmpBuf(NPI_CMN_CONTEXT_SECURE, in_len1, in_buf1, in_len2, in_buf2)
...
#define binstr_Cmp(in_str1, in_str2) \
    npi_binstr_Cmp(NPI_CMN_CONTEXT_SECURE, in_str1, in_str2)
...
#define mem_ALLOC(in_size, out_ptr)           \
    do {                                                                \
        if ( NULL == ((out_ptr) = x_mem_alloc(in_size)) ) \
            RET( ERR_NOMEM );                                            \
        x_memset((out_ptr), 0, (in_size));                           \
    } while (0)
...
#define mem_Copy(out_dst, in_src, in_size) \
    x_memcpy(out_dst, in_src, in_size)

#define mem_Cmp(in_buf1, in_buf2, in_size) \
    x_memcmp(in_buf1, in_buf2, in_size)

#define mem_Set(in_buf, in_c, in_size) \
    x_memset(in_buf, in_c, in_size)
