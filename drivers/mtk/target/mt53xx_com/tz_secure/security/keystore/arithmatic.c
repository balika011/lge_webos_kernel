...
/* Useful macros */
#define LOHALF(x) ((UINT32)((x) & MAX_HALF_DIGIT))
#define HIHALF(x) ((UINT32)((x) >> BITS_PER_HALF_DIGIT & MAX_HALF_DIGIT))
#define TOHIGH(x) ((UINT32)((x) << BITS_PER_HALF_DIGIT))

int spMultiply(UINT32 p[2], UINT32 x, UINT32 y)
{	/*	Computes p = x * y */
...
int mpMultiply(UINT32 w[], const UINT32 u[], const UINT32 v[],
					UINT32 ndigits)
{
...
void mpSetZero(UINT32 a[], UINT32 ndigits)
{	/* Sets a = 0 */

	/* Prevent optimiser ignoring this */
	volatile UINT32 optdummy;
	UINT32 *p = a;
...
UINT32 mpShiftRight(UINT32 a[], const UINT32 b[], UINT32 shift, UINT32 ndigits)
{	/* Computes a = b >> shift */
...
UINT32 mpSizeof(const UINT32 a[], UINT32 ndigits)
...
void mpSetEqual(UINT32 a[], const UINT32 b[], UINT32 ndigits)
{	/* Sets a = b */
	UINT32 i;
...
UINT32 mpShiftLeft(UINT32 a[], const UINT32 *b,
	UINT32 shift, UINT32 ndigits)
{	/* Computes a = b << shift */
	/* [v2.1] Modified to cope with shift > BITS_PERDIGIT */
	UINT32 i, y, nw, bits;
	UINT32 mask, carry, nextcarry;
...
static void spMultSub(UINT32 uu[2], UINT32 qhat, UINT32 v1, UINT32 v0)
{
...
UINT32 spDivide(UINT32 *q, UINT32 *r, const UINT32 u[2], UINT32 v)
{	/*	Computes quotient q = u / v, remainder r = u mod v
	...
	*/
	UINT32 qhat, rhat, t, v0, v1, u0, u1, u2, u3;
	UINT32 uu[2], q2;
...
UINT32 mpShortDiv(UINT32 q[], const UINT32 u[], UINT32 v,
				   UINT32 ndigits)
{
...
int mpCompare(const UINT32 a[], const UINT32 b[], UINT32 ndigits)
{
	/*	Returns sign of (a - b)
	*/
...
void mpSetDigit(UINT32 a[], UINT32 d, UINT32 ndigits)
...
static int QhatTooBig(UINT32 qhat, UINT32 rhat,
					  UINT32 vn2, UINT32 ujn2)
...
static UINT32 mpMultSub(UINT32 wn, UINT32 w[], const UINT32 v[],
					   UINT32 q, UINT32 n)
{	/*	Compute w = w - qv
		where w = (WnW[n-1]...W[0])
		return modified Wn.
...
UINT32 mpAdd(UINT32 w[], const UINT32 u[], const UINT32 v[],
			   UINT32 ndigits)
{
	/*	Calculates w = u + v
...
int mpDivide(UINT32 q[], UINT32 r[], const UINT32 u[],
	UINT32 udigits, UINT32 v[], UINT32 vdigits)
{	/*	Computes quotient q = u / v and remainder r = u mod v
	...
	*/
	UINT32 shift;
	int n, m, j;
	UINT32 bitmask, overflow;
	UINT32 qhat, rhat, t[2];
	UINT32 *uu, *ww;
	int qhatOK, cmp;
	...
	*/
	...
	for (j = m; j >= 0; j--)
	{
		...
		ww = &uu[j];
		overflow = mpMultSub(t[1], ww, v, qhat, (UINT32)n);
		...
		{	/* Step D6. Add back if D4 was negative */
			q[j]--;
			overflow = mpAdd(ww, ww, v, (UINT32)n);
		}
...
static int moduloTemp(UINT32 r[], const UINT32 u[], UINT32 udigits,
			 UINT32 v[], UINT32 vdigits, UINT32 tqq[], UINT32 trr[])
{
...
int modMultTemp(UINT32 a[], const UINT32 x[], const UINT32 y[],
			  UINT32 m[], UINT32 ndigits,
			  UINT32 temp[], UINT32 tqq[], UINT32 trr[])
