...
static const GFX_PLANE_CAPS_T arMwPlaCaps[OSD_PLANE_MAX_NUM] =
{
    {
        /* ui4_cm_caps */
...
static INT32 _OsdReconfigBitmap(GFX_BITMAP_INIT_T* prBmpInit)
{
	...
	if (prBmp->rLockInfo.e_colormode == GFX_COLORMODE_YUV_422_RS)
	{
		prBmp->rLockInfo.at_subplane[0].ui4_pitch = (((u4bpp / 2 * prBmpInit->ui4_bmp_w) + (15 * 8)) >> 3) & (~0xf);;
	}
    return (INT32)OSD_RET_OK;
...
static INT32 _OsdGetScalerInfo(UINT32 u4Plane, GFX_ADV_SCALER_PARAM_T* prParam)
{
  ...
  //  x_memcpy(prParam->ai2_filter, ai2LpfParam, sizeof(ai2LpfParam)); //hw has delete ScLpf function
                            ...
                            VOID* pvGetInfo, SIZE_T* pzGetInfoLen)
{
	...
	case OSD_GET_VDPPHOTO_CREATE_BITMAP:
		SET_COMMAND(_VdpPhotoSetCommand(prCompId,rGetType,pvGetInfo, 0));
		break;
#endif
                            ...
                            const VOID* pvSetInfo, SIZE_T zSetInfoLen)
{
	...
	case OSD_SET_VDPPHOTO_DELETE_BITMAP:
		SET_COMMAND(_VdpPhotoSetCommand(prCompId,rSetType,pvSetInfo, zSetInfoLen));
#endif
