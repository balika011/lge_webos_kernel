...
static INT32 _NptvVideoRealDelayEx(INT32 i4Argc, const CHAR **szArgv);
#endif
static INT32 _NptvBypassWFDEnhance(INT32 i4Argc, const CHAR **szArgv);
...
///====================================
//// ----DRRVCUST_TEST_API----
#define DRRVCUST_TEST_API 1

#if DRRVCUST_TEST_API
#ifdef CC_MAPLE_CUST_DRV
static INT32 _NptvDRVCUST_GetSignalFormat(INT32 i4Argc, const CHAR **szArgv);
static INT32 _NptvDRVCUST_HDMIColorInfo(INT32 i4Argc, const CHAR **szArgv);
static INT32 _NptvDRVCUST_B2RColorInfo(INT32 i4Argc, const CHAR **szArgv);

EXTERN void CLI_DRVCUST_GetSignalFormat(UINT8 bPath);
EXTERN BOOL DRVCUST_HDMIIsPhotoMode(void);
EXTERN UINT8 DRVCUST_HDMIGetVideoColorSpace(void);
EXTERN UINT8 DRVCUST_HDMIGetPhotoColorSpace(void);
EXTERN UINT8 DRVCUST_HDMIGetColorimetry(void);
EXTERN UINT8 bHDMIRgbRange(void);
EXTERN BOOL DRVCUST_HDMIWithValidAviInfo(void);

CLI_EXEC_T arTestApiCmdTbl[] = {
#ifdef CC_CLI
    {"GetSignalFormat",  "sf",    _NptvDRVCUST_GetSignalFormat,  NULL, "DRVCUST_GetSignalFormat", CLI_GUEST},
    {"HDMIColorInfo",    "hci",   _NptvDRVCUST_HDMIColorInfo,    NULL, "HDMIColorInfo",           CLI_GUEST},
    {"B2RColorInfo",     "b2rci", _NptvDRVCUST_B2RColorInfo,     NULL, "B2RColorInfo",            CLI_GUEST},
...
CLI_EXEC_T arVdoCmdTbl[] = {
    ...
    #endif
    #ifdef TIME_MEASUREMENT
    {"RealDelayEx",      "rdlyex",  _NptvVideoRealDelayEx,NULL, "Real time video delay ex", CLI_GUEST},
    #endif
    {"BypassEnhance",  "bypassenh", _NptvBypassWFDEnhance,NULL, "Bypass video WFD enhance", CLI_SUPERVISOR},
...
CLI_EXEC_T arFrameTrackCmdTbl[] = {  
  #ifdef CC_CLI
  {"query", "q", _NptvFrameTrackQuery, NULL, "Query frame track status", CLI_SUPERVISOR}, 
  {"FrameTrackMode", "fftm", _NptvForceFrameTrackmode, NULL, "force frame track mode", CLI_SUPERVISOR},
  {"FrameTrackMode", "ffoo", _NptvForceFrameTrackOnOff, NULL, "force frame track on/off", CLI_SUPERVISOR},
  #endif
...
#ifdef SUPPORT_DGO
    {"DGO",             "dgo",   NULL, arDgoCmdTbl,         "DGO Command",                 CLI_GUEST},
#endif
...
//MT5399 MONITOR OUT
    ...
    #if DRRVCUST_TEST_API
    #ifdef CC_MAPLE_CUST_DRV
    {"testapi", 		  "testapi",	 NULL, arTestApiCmdTbl,  "Drvcust Api test Command",			   CLI_GUEST},
    #endif
    #endif
    {NULL, NULL, NULL, NULL, NULL, CLI_SUPERVISOR}
};
...
#endif
#ifdef TIME_MEASUREMENT
extern void vDrvVideoDelayCreateThreadEx(void);
static INT32 _NptvVideoRealDelayEx(INT32 i4Argc, const CHAR **szArgv)
{
    vApiVideoAllPQItemOnOff(SV_OFF);
    vDrvVideoDelayCreateThreadEx();
    return 0;
}
#endif
...
static INT32 _NptvBypassWFDEnhance(INT32 i4Argc, const CHAR **szArgv)
{
    if (i4Argc != 2)
    {
        ...
        Printf("<OnOff>\n");
        Printf("0: Enable WFD enhance.\n");
        Printf("1: Bypass WFD enhance.\n");
        return 1;
    }
    u1BypassWFDEnhance = StrToInt(szArgv[1]) ? SV_TRUE : SV_FALSE;
...
static INT32 _NptvDRVCUST_GetSignalFormat(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    bPath = StrToInt(szArgv[1]) ? SV_VP_PIP : SV_VP_MAIN;
    Printf("\n_NptvDRVCUST_GetSignalFormat:\n");
    CLI_DRVCUST_GetSignalFormat(bPath);
    //DRVCUST_GetSignalFormat(bPath, pSignalFormat);
    return 0;
}
static INT32 _NptvDRVCUST_HDMIColorInfo(INT32 i4Argc, const CHAR **szArgv)
{
    UINT8 u1HDMIIsPhotoMode = DRVCUST_HDMIIsPhotoMode();
    UINT8 u1HDMIVideoColorSpace = DRVCUST_HDMIGetVideoColorSpace();
    UINT8 u1HDMIPhotoColorSpace = DRVCUST_HDMIGetPhotoColorSpace();
    UINT8 u1HDMIColorimetry = DRVCUST_HDMIGetColorimetry();
    UINT8 u1HDMIRgbRange = bHDMIRgbRange();
    UINT8 u1HDMIWithValidAviInfo = DRVCUST_HDMIWithValidAviInfo();
    
    Printf("\n_NptvDRVCUST_HDMIColorInfo:\n");
    Printf("IsPhotoMode[%d],VideoColorSpace[%d],PhotoColorSpace[%d].\n",
        u1HDMIIsPhotoMode,u1HDMIVideoColorSpace,u1HDMIPhotoColorSpace);
    Printf("Colorimetry[%d],Range[%d],u1HDMIWithValidAviInfo[%d].\n",
        u1HDMIColorimetry,u1HDMIRgbRange,u1HDMIWithValidAviInfo);
    return 0;
}

static INT32 _NptvDRVCUST_B2RColorInfo(INT32 i4Argc, const CHAR **szArgv)
{
    UINT8 u1B2RColorinfo = 0xFF;
    UINT8 bPath;
	B2R_CUST_INFO_T 		t_cust_info;
    ...
    bPath = StrToInt(szArgv[1]) ? SV_VP_PIP : SV_VP_MAIN;
	B2R_Get_Cust_info(bPath,B2R_GET_TYPE_CUST_INFO,&t_cust_info,sizeof(B2R_CUST_INFO_T));
    u1B2RColorinfo = (UINT8)t_cust_info.eColorID;
    Printf("\n_NptvDRVCUST_B2RColorInfo: VDPGetColorInfo(%d)=[%d].\n",
        bPath,u1B2RColorinfo);
    return 0;
}
#endif
#endif
