...
static INT32 _ScposSetFrameTrackForceOnLR(INT32 i4Argc, const CHAR **szArgv);
...
static INT32 _ScpipDebugDramAddr(INT32 i4Argc, const CHAR **szArgv);
#endif /*#ifdef CC_CLI*/
...
CLI_EXEC_T arScposCmdTbl[] = {
  ...
  {"frametrackOnLR",	"ftlr",	_ScposSetFrameTrackForceOnLR,	NULL,  "Set Frame Track Base On L or R",	CLI_SUPERVISOR},
  ...
  {"scaler dram addr", "addr", _ScpipDebugDramAddr, NULL, "Force scaler dram address", CLI_SUPERVISOR},      
  #endif
...
{
   UINT8 uMaskInputLR;
   UINT8 uOutputLockAtLR;
    
    if( i4Argc !=3 )
    {
        Printf("Usage  : <MaskInputLR> <OutputLockAtLR>\n");
        Printf("MaskInputLR  :   \n");
        Printf("0  :  set to default  mask R \n");
        Printf("1  :  FrameTrack Mask Input Left \n");
        Printf("2  :  FrameTrack Mask Input Right\n");
        Printf("3  :  Turn off mask input \n");
		
        Printf("OutputLockAtLR  :   \n");
        Printf("0  :  set to default  lock at L\n");
        Printf("1  :  Output Lock At L  \n");
        Printf("2  :  Output Lock At R  \n");
        Printf("3  :  Turn off lock ouput \n");
		
        Printf("Current 3D status is : \n");
		vPrintfMIBSCStatus();
        return 0;
    }
	
	uMaskInputLR = (UINT8)StrToInt(szArgv[1]);
	uOutputLockAtLR = (UINT8)StrToInt(szArgv[2]);
	
	_fgFrametrackBaseOnLRByCLI = SV_OFF;
	
	if(uMaskInputLR  || uOutputLockAtLR)
	{
	    if(uMaskInputLR == 1)
    	{
    	    _fgFrametrackBaseOnLRByCLI |= (FRM_TRK_MASK_IN_EN | FRM_TRK_MASK_IN_LEFT);
    	}
		else if(uMaskInputLR == 2)
		{
		    _fgFrametrackBaseOnLRByCLI |= (FRM_TRK_MASK_IN_EN | FRM_TRK_MASK_IN_RIGHT);
		}
		else if(uMaskInputLR == 3)
		{
		    _fgFrametrackBaseOnLRByCLI = ( FRM_TRK_MASK_IN_RIGHT); //set enable flag to 0
		}
					
		if(uOutputLockAtLR == 1)
		{
		    _fgFrametrackBaseOnLRByCLI |= ( FRM_TRK_3D_EN | FRM_TRK_3D_LOCK_AT_L );
		}
		else if(uOutputLockAtLR == 2)
		{
		    _fgFrametrackBaseOnLRByCLI |= ( FRM_TRK_3D_EN | FRM_TRK_3D_LOCK_AT_R );
		}
		else if(uOutputLockAtLR == 3)
		{
		    _fgFrametrackBaseOnLRByCLI =  FRM_TRK_3D_LOCK_AT_R ; //set enable flag to 0
		}
...
INT32 _ScposSetInputPtnCmd(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    else if (i4Argc < 5)
    {
        ...
        if ((u4Mode != SCPIP_PTGEN_OFF) && (u4Mode !=  SCPIP_PTGEN_AUTO) && (u4Mode !=  SCPIP_PTGEN_STILL))
        {
            u4Invalid = 2;
        }
    }
...
#else
        vScpipSetOutputPattern(u4Path, SCPIP_PTGEN_MANUAL, StrToInt(szArgv[3]), StrToInt(szArgv[4])); 
#endif
    }
    else
    {
...
static INT32 _ScpipDebugDramAddr(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    {
        u4VdpId = (UINT32)StrToInt(szArgv[1]);
        u4Start = (UINT32)StrToInt(szArgv[2]);

        vScpipSetBufferAddress(u4VdpId, u4Start, 0, 0, 0);    
    }
    else if(i4Argc == 6)
    {
        u4VdpId  = (UINT32)StrToInt(szArgv[1]);
        ...
        u4Frame  = (UINT32)StrToInt(szArgv[5]);

        vScpipSetBufferAddress(u4VdpId, u4Start, u4Width, u4Height, u4Frame);    
    }
    else
    {
        Printf("Usage: <VdpId> <Starting addr>\n");
        Printf("Usage: <VdpId> <Starting addr> <Dram width> <Dram Height> <Frame numbers>\n");
    }
