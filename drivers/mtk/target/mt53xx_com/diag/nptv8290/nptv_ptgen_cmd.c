...
static INT32 _i4VencPat(INT32 i4Argc, const CHAR **szArgv);
#endif
#ifdef CC_SUPPORT_UHD
static INT32 _i4UhdPscPatt(INT32 i4Argc, const CHAR **szArgv);
#endif
#endif /*#ifdef CC_CLI*/
/******************************************************************************
* CLI Command Table
...
CLI_EXEC_T arPatgenCmdTbl[] = {
    ...
    #ifdef CC_SUPPORT_VENC
    {"Pat Enc Input"   , "enc"     , _i4VencPat               , NULL , "Venc Pat On Off        "      , CLI_GUEST}           ,
    #endif
	#ifdef CC_SUPPORT_UHD
	{"Pat UHD PSC"   ,  "uhd"     , _i4UhdPscPatt           , NULL , "UHD PSC patten         "      , CLI_GUEST}           ,
	#endif
#endif
...
static void vPtgenDisableAll(UINT8 u1Path)
{
    ...
    vScpipSetInputPattern(u1Path, SCPIP_PTGEN_OFF, 0, 0);    
    vScpipSetOutputPattern(u1Path, SCPIP_PTGEN_OFF, 0, 0);  
	...
	#ifdef CC_MT5890
    vScpipSetPostScalerPattern(0, SCPIP_PTGEN_OFF, 0, 0);
    vScpipSetPostScalerPattern(1, SCPIP_PTGEN_OFF, 0, 0);
    #endif
...
static INT32 _i4PatAuto(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    while (1)
    {
        ...
        case PTGEN_SC_IN:
            {   
...
#endif 
                {
                    if(u1OldPtGenNo != u1PtGenNo)
                    {
                        u1PtType = SCPIP_PTGEN_AUTO; // default patgen type
                    }
                    else
                    {
                        u1PtType = (u1PtType + 1) % 3; // change pattern type
                    }

                    vScpipSetInputPattern(u1Path, u1PtType, 0, 0);    
        ...
        case PTGEN_SC_OUT:
            ...
            vScpipSetOutputPattern(u1Path, u1PtType, 0, 0);  
        ...
        case PTGEN_POST_SCALER_IN:
            ...
            Printf("| )      | )   ( |/\\____) |/\\____) |\n");
            Printf("|/       |/     \\|\\_______)\\_______)\n");
            Printf("This Chip don't have this pattern, please ignore this pattern gen\n");
            break;
#endif
        case PTGEN_POST_SCALER_OUT:
            vScpipSetPostScalerPattern(1, SCPIP_PTGEN_AUTO, 0, 0);
...
static INT32 _i4VencPat(INT32 i4Argc, const CHAR **szArgv)
{
	...
	{
        Printf("Usage: %s  OnOff PatternType \n", szArgv[0]);
        Printf("<OnOff> 0: OFF 1:ON  <PatternType> 0: moving square \n");
        return 1;
	}

    if (i4Argc == 3)
    {
        u1DrvVencPatSetPatType((E_VENC_PAT_TYPE)StrToInt(szArgv[2]));
    }
    u1DrvVencPatOnOff(StrToInt(szArgv[1]));       
    return 0;
}
#endif
...
static INT32 _i4UhdPscPatt(INT32 i4Argc, const CHAR **szArgv)
{
	UINT32 u4InOut, u4OnOff;
		
	if (i4Argc != 3)
	{
        Printf("Usage: in/out on/off \n");
        return 1;
	}

	u4InOut = i4StrToInt(szArgv[1]);
	u4OnOff = i4StrToInt(szArgv[2]);

	vUhdSetPostScalerPattern(u4InOut,u4OnOff);
	return 0;
}
#endif
#endif //#ifdef CC_CLI
