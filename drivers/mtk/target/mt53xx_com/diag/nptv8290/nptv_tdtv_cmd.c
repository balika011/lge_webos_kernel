...
static INT32 _TdtvSetPanelSupport(INT32 i4Argc, const CHAR **szArgv);
static INT32 _TdtvSet3DScalerPattern(INT32 i4Argc, const CHAR **szArgv);
...
EXTERN void vDrvTDTVPanelSupport3Ddbg(UINT8 u1support3d);
EXTERN void vDrvTDTVScaler3DPattern(UINT8 u1InOut,UINT8 u1OnOff);
//TDNAVI CTRL
...
EXTERN UINT8 g_u1TdNaviAdjustDbgOnoff;
/******************************************************************************
* CLI Command Table
******************************************************************************/
...
CLI_EXEC_T arTDTVCmdTbl[] = {
    ...
    {"Set Panel 3d"   ,"sp3d"     , _TdtvSetPanelSupport  ,NULL, "Panel 3d Setting"          , CLI_SUPERVISOR},
    {"Set 3d pattern"  ,"s3dptn"  , _TdtvSet3DScalerPattern,NULL, "set 3d pattern"            , CLI_GUEST},
...
static INT32 _TdtvSet3DNaviInFwPara(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    u4Para[4] = g_u1TdNaviAdjustDbgOnoff;
	if (i4Argc != 6)
	{
        ...
        Printf("Para[4] = 0:off,1:on : 3D Navi debug mode\n");
		return 1;
	}
    ...
    g_u1TdNaviAdjustDbgOnoff = (UINT8)(UINT_CLIP(u4Para[4], 0, 1));
...
static INT32 _TdtvSetPanelSupport(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if (i4Argc < 2)
    {
        Printf("Set Panel support 3D Off:0, On:1 \n");
        return 0;
    }

    u1support3d = (StrToInt(szArgv[1]) == 1) ? SV_ON : SV_OFF;
    
    vDrvTDTVPanelSupport3Ddbg(u1support3d);
...
static INT32 _TdtvSet3DScalerPattern(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if (i4Argc < 3)
    {
        Printf("Set Scaler 3D Pattern u1support3d u1scalerInOut\n");
		Printf("u1support3d (0: Off, 1: On)\n");
		Printf("u1scalerInOut (0: In, 1: Out)\n");
        return 0;
    }
    
    u1OnOff  = (UINT8)(UINT_CLIP(StrToInt(szArgv[1]), 0, 1));
    u1InOut  = (UINT8)(UINT_CLIP(StrToInt(szArgv[2]), 0, 1));
    
    vDrvTDTVScaler3DPattern(u1InOut,u1OnOff);
    return 0;
}
