...
static INT32 _i4PrintAmbilightBlkIdx(INT32 i4Argc, const CHAR **szArgv);
...
CLI_EXEC_T arAmbilight[] = {
    ...
    {"Print Ambilight Info", "pblk", _i4PrintAmbilightBlkIdx, NULL, "Print", CLI_GUEST},
#endif
    {NULL, NULL, NULL, NULL, NULL, CLI_SUPERVISOR}
};
...
CLI_EXEC_T arDump[] = {
#ifdef CC_CLI
    {"Start Dump",          "start", _i4StartDump, NULL, "Start Dump", CLI_GUEST},
...
static INT32 _i4NRDNROnOff(INT32 i4Argc, const CHAR **szArgv)
{
	if (i4Argc != 3)
	{
		Printf("Usage: %s <2DOnOff> <3DOnOff> \n", szArgv[0]);

		return 0;
	}

	vDrvNRSet2DDotCrawlNROnOff((UINT32)StrToInt(szArgv[1]));
	vDrvNRSet3DDotCrawlNROnOff((UINT32)StrToInt(szArgv[2]));
...
static INT32 _i4LcDimContentInfo(INT32 i4Argc, const CHAR **szArgv)
{
    Printf("Not support in NPTV8290\n");
	return 0;
}
...
{	
	if(i4Argc< 3)	
	{		
		Printf("Usage: %s <Color> <1:StrapID/2:BlknumArray/3:lineBlknum>\n", szArgv[0]);		
		return 1;	
	}	
	if(i4Argc==3)	
	{	    
		vDrvAmbiLPrintBlkInfo(StrToInt(szArgv[1]),StrToInt(szArgv[2]));	
	}	
	if(i4Argc==4)	
	{		
		Printf("Set startBlkNumber:n.ycproc.amb.pblk <color> 0 <startblknum>,StartBlkNum=%d \n",StrToInt(szArgv[3]));
		//u1StartBlkNum=StrToInt(szArgv[3]);
	}   	
	return 0;
}
