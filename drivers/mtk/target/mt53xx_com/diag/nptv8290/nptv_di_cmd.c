...
static INT32 _i4DiHVPDTest(INT32 i4Argc, const CHAR ** szArgv);
...
CLI_EXEC_T arYcprocDiTbl[] = {
    ...
    {"HPSTest", "pdt", _i4DiHVPDTest, NULL, "Auto predown test", CLI_SUPERVISOR},
...
static HANDLE_T hThread;
extern UINT8 SerPollGetChar(void);
UINT8 u1MibPredown = 0;
static VOID _DiHVPSTestTimerDo(VOID *pv_tag)
{
    ...
    {//Transfer to 0x8000 base
        u2SourceWH = wDrvVideoInputWidth(prePDInfo->u1Path);
        prePDInfo->u2Step = prePDInfo->u2Step * 0x8000 /u2SourceWH; 
    }
    else
    {
        u2SourceWH = wDrvVideoInputHeight(prePDInfo->u1Path);
        prePDInfo->u2Step = prePDInfo->u2Step * 0x8000 /u2SourceWH; 
    }
    
    while((prePDInfo->u2Cnt >= prePDInfo->u2Low) && (u1MibPredown == 0))
    {   
        if(prePDInfo->u1HVPredown == 1)
        {
            u2WH = DIVIDE_WITH_ROUND(((UINT32)u2SourceWH * prePDInfo->u2Cnt), 0x8000);
            Printf("\nTest Width = %d ,u2EvenHCnt=0x%x\n", u2WH,prePDInfo->u2Cnt);
            vScpipSetForcedHpsFactor(prePDInfo->u1Path,prePDInfo->u2Cnt);
        }
        else
        {
            u2WH = DIVIDE_WITH_ROUND(((UINT32)u2SourceWH * prePDInfo->u2Cnt), 0x8000);
            Printf("\nTest Height = %d ,u2EvenHCnt=0x%x\n", u2WH,prePDInfo->u2Cnt);
            vScpipSetForcedVpsFactor(prePDInfo->u1Path,prePDInfo->u2Cnt);
        }
        prePDInfo->u2Cnt = prePDInfo->u2Cnt - prePDInfo->u2Step;
        x_thread_delay(prePDInfo->u1Delay*1000);
    } 
    Printf("Auto predown thread exit. \n");
}
...
static INT32 _i4DiHVPDTest(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    if (i4Argc < 7)
    {
    	Printf("Usage: %s <m/p> <h/v> high low delay step \n", szArgv[0]);
    	return 1;
    }
    ...
    curPDInfo.u1HVPredown = ((x_strcmp(szArgv[2], "h") == 0) ? 1 : 0);
    curPDInfo.u2Cnt = StrToInt(szArgv[3]);
    curPDInfo.u2Low = StrToInt(szArgv[4]);
    curPDInfo.u1Delay = StrToInt(szArgv[5]);
    curPDInfo.u2Step = StrToInt(szArgv[6]);
    u1MibPredown = 0;
    
    VERIFY(x_thread_create(&hThread, "AutoPreDown", 2048, 60, _DiHVPSTestTimerDo, sizeof(DI_TIMER_T), (VOID*)&curPDInfo) == OSR_OK);
    ...
    while(1)
    {
        if(SerPollGetChar() !=0)
        {
            ...
            Printf("Auto predown interrupted!\n");
            break;
        }
    }
