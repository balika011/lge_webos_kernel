...
CLI_EXEC_T arVbiCmdTbl[] = {
    ...
    #if SUPPORT_TYPE_B_VBI
    {"WssBLine", "wssbl", _i4VBI_TypeBWssLine, NULL, "Get Set Type B WSS line command", CLI_GUEST},
    #endif
    ...
    #if SUPPORT_TYPE_B_VBI
    {"VerTypeBWSS", "vbwss", _i4VBI_VerTypeBWSS, NULL, "Verify Type B WSS command", CLI_GUEST},
    #endif
...
void _vVBI_TypeBWSSNotify(UINT8 u1Path,UINT8 au1Data[])
{
    ...
    if (u1Path == SV_VP_MAIN)
    {
        ...
        {
            Printf("MAIN: Type B WSS 0x%x ", au1Data[u1Idx]);
        }
        Printf("\n");
    }
	else
	{
        for (u1Idx=0; u1Idx<17; u1Idx++)
        {
            Printf("PIP: Type B WSS 0x%x ", au1Data[u1Idx]);
        }
        Printf("\n");
    }
}
