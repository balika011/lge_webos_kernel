...
static INT32 PcmciaCmdSpiDTest(INT32 i4Argc, const CHAR** szArgv)
{
...
#ifdef __KERNEL__
    pu4TestBuf = (UINT8*)VIRTUAL(BSP_AllocAlignedDmaMemory(4096, 32));
    pu4GoldBuf = (UINT8*)VIRTUAL(BSP_AllocAlignedDmaMemory(4096, 32));
    pu4TempBuf = (UINT8*)VIRTUAL(BSP_AllocAlignedDmaMemory(4096, 32));
#else
    ...
    if ( pu4TestBuf == NULL || pu4GoldBuf == NULL || pu4TempBuf == NULL)
    {
        LOG(0, "DMA Alloc Memory Error!\n");
        return -1;
    }
    ...
    for ( i = 1 ; i <= u4TestLoop ; i++ )
    {
        ...
        for( j = 0 ; j < 4096 ; j++ )
        {
            pu4GoldBuf[j] = (UINT8)rand();
            Printf("DMA Write pu4GoldBuf[%d] = 0x%X\n",j,pu4GoldBuf[j]);
            pu4TestBuf[j] = 0;
        }
        ...
        for(j=0; j<(u4DataLen*4); j++)
        {
            ...
            Printf("DMA Read pu4TestBuf[%d] = 0x%X\n",j,pu4TestBuf[j]);
        }
