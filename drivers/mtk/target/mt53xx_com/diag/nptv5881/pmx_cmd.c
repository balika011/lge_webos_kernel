...
static INT32 _PmxSetCmdFg(INT32 i4Argc, const CHAR **szArgv);
...
#if defined(CC_MT5398) || defined(CC_MT5398)
...
static INT32 _PmxSetOutputFrameRate(INT32 i4Argc, const CHAR **szArgv);
...
{
    {"Vdoflag", NULL, _PmxSetCmdFg, NULL, "Vdoflag", CLI_SUPERVISOR},
...
static CLI_EXEC_T _arPmxCmdTbl[] =
{
    ...
    {"Set Out Frame Rate","sfr",   _PmxSetOutputFrameRate, NULL, "Set Out Frame Rate",     CLI_GUEST},
...
static INT32 _PmxSetOutputFrameRate(INT32 i4Argc, const CHAR **szArgv)
{
    UINT32 u4OutputFrRateType = 0;

    if (i4Argc != 2)
    {
		Printf("Usage: %s Set SetOutputFrameRate type\n", szArgv[0]);
		Printf("can support change more items , exp: FORCE_DCLK_FORCE_2D_50HZ| FORCE_DCLK_FORCE_3D_50HZ <=> 0x6\n");
		Printf("0x0:   SetOutputFrameRate NORMAL_RULE \n");
		Printf("0x1:   SetOutputFrameRate FORCE_48HZ \n");
		Printf("0x2:   SetOutputFrameRate FORCE_2D_50HZ \n");
		Printf("0x4:   SetOutputFrameRate FORCE_3D_50HZ \n");
		Printf("0x8:   SetOutputFrameRate FORCE_2D_60HZ \n");
		Printf("0x10:  SetOutputFrameRate FORCE_3D_60HZ \n");

        return 1;
    }

    u4OutputFrRateType = StrToInt(szArgv[1]);
    Printf("vDrvMJCSetUnicadenceType: %d\n", StrToInt(szArgv[1]));
    u1DrvSetForceOutputFrameRateByUser(u4OutputFrRateType);
...
static INT32 _PmxSetCmdBacklt(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    else if((vDrvGetLCDFreq() == 24) || (vDrvGetLCDFreq() == 48))
    {
        u4Devider = u4Freq/48;
    }
#endif
    ...
    if ((u4Bright>0)&&(u4Bright<100))
    {
		u4High = u4Bright/u4Devider;
		u4Low = (100-u4Bright)/u4Devider;
    }
    else if (u4Bright>=100)
    {
		u4High = 100;
    }
...
#if defined(CC_MT5368) || defined(CC_MT5398) || defined(CC_MT5389) || defined(CC_MT5398) || defined(CC_MT5880) || defined(CC_MT5881)
static INT32 _PmxSetBorder(INT32 i4Argc, const CHAR **szArgv)
{
    CHK_NUM_MIN_PARAM(2, "VdpId(0:main, 1:pip) fgEnable(0:off, 1:on)");
...
#if defined(CC_MT5398) || defined(CC_MT5398) // 95 support VB1
", [1]:VBI, [3]:EPI [4]:iDP"
#endif
...
static INT32 _PmxTXIFPWM_Init(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    { 
    	...
    	if ((u1PwmSrc>=1)&&(u1PwmSrc<=6))
    	{
			vDrvInitSCANPWM(u1PwmSrc); 
		}
		else
		{
			Printf("scanPWM_src (1: SCAN1, 2: SCAN2, 3: SCAN3, 4: SCAN4, 5: SCAN5, 6:SCAN6)\n");
		}
    }
    else
    {
...
static INT32 _PmxTXIFPWM_Set(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    else
    {
	 ...
	 Printf("high_period (0~100, ex: 40 --> 40%)\n");	
	 Printf("low_period (0~100, ex: 60 --> 60%)\n");	
    }	
	return 0;
}
		...
		if ((u1Source>=1)&&(u1Source<=6))
    	{
			...
			if (!u1mode) // 100%-->0%
			{
				for (u2Cnt=100; u2Cnt>0; u2Cnt--)
				{
					...
					u4Low = u4Low/u4Int;
					Printf("%d", u2Cnt);
					vDrvSetScanPWM(u1Source,u4Start,u4High,u4Low);
					vDrvScanPWMDataFire();
					vUtDelay10ms(5);
			    #if 0
					u1Char = SerPollGetChar();
