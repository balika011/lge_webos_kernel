 ...
 */
//-------------------------------------------------------------------------
DRV_ERR_CODE_T DRV_GFX_StretchBlt(DRV_COLOR_MODE_T rColorMode,
    DRV_GFX_STRETCHBLIT_T* prBlit)
{
    UINT32 u4SrcX, u4SrcY, u4DstX, u4DstY, u4SrcWidth, u4SrcHeight, u4DstWidth, u4DstHeight;
    UINT32 u4SrcPitch, u4DstPitch;

    // check parameters
    NULL_PT_CHECK(prBlit);
    XYWH_CHECK(prBlit->u2SrcX, prBlit->u2SrcY, 
               prBlit->u2SrcWidth, prBlit->u2SrcHeight);
    XYWH_CHECK(prBlit->u2DstX, prBlit->u2DstY, 
               prBlit->u2DstWidth, prBlit->u2DstHeight);
    ...
    u4DstX     = (UINT32)prBlit->u2DstX;
    u4DstY     = (UINT32)prBlit->u2DstY;
    u4SrcWidth    = (UINT32)prBlit->u2SrcWidth;
    u4SrcHeight   = (UINT32)prBlit->u2SrcHeight;
    u4DstWidth    = (UINT32)prBlit->u2DstWidth;
    u4DstHeight   = (UINT32)prBlit->u2DstHeight;    
    u4SrcPitch = (UINT32)prBlit->u2SrcPitch;
    u4DstPitch = (UINT32)prBlit->u2DstPitch;
    ...
    GFX_SetDst((UINT8 *)prBlit->pu1DstBaseAddr, 
                   _GetGfxDrvColorMode(rColorMode), u4DstPitch);
    GFX_SetBltOpt((UINT32)D_GFXFLAG_NONE, 0x00000000, 0xffffffff);   
    GFX_StretchBlt(u4SrcX, u4SrcY, u4SrcWidth, u4SrcHeight, u4DstX, u4DstY, u4DstWidth, u4DstHeight);
#endif
