...
static UINT32 _u4List[OSD_PLANE_MAX_NUM];
...
DRV_ERR_CODE_T DRV_OSD_Init(void)
{
    ...
    RET_ON_FAIL(OSD_RGN_LIST_Create(&_u4List[DRV_OSD_PLANE_3]));
...
DRV_ERR_CODE_T DRV_OSD_RGN_Create(DRV_OSD_RGN_T* prOsd, UINT32* pu4RgnID)
{
    ...
    if(prOsd->u4ScaleWidth != 0 && prOsd->u4ScaleHeight != 0 && prOsd->u1PlaID > 0)
    {
#if 0    
...
#else
        RET_ON_FAIL(OSD_SC_Scale(prOsd->u1PlaID - 1, 1, prOsd->u4Width, prOsd->u4Height, prOsd->u4ScaleWidth, prOsd->u4ScaleHeight));
#endif
    }
...
DRV_ERR_CODE_T DRV_OSD_RGN_SetAlphaMode(UINT32 u4RgnID, DRV_OSD_BLEND_MODE_T eMode)
{
...
DRV_ERR_CODE_T DRV_OSD_SetPlaneSwitchOrder(UINT32 u4TopPlane, UINT32 u4MidPlane, UINT32 u4BotPlane)
{
	RET_ON_FAIL(OSD_BASE_SetPlaneSwitchOrder(u4TopPlane,u4MidPlane, u4BotPlane));
    return DRV_E_OK;
}
#endif
