...
extern void vApiTVECtrlEnable(UINT8 bEnable);
//-----------------------------------------------------------------------------
// Static functions
//-----------------------------------------------------------------------------
...
static void _VgaAutoNfy(UINT16 u2Idx, UINT8 u1Progress)
{
    DRV_VDO_PARAM_T* prParam;
    PFN_DRV_CALLBACK pfnCallback;
    ...
    if((pfnCallback != NULL) && u1Progress == 100)
    {
        pfnCallback(u2Idx);
    }
}
...
#else
    UNUSED(_VbiTtxBufSysInit);
    UNUSED(_VbiTtxBufInit);
    UNUSED(_VbiTtxBufFlush);
    UNUSED(_VbiTtxNfy);
    UNUSED(_VbiCcNfy);
    UNUSED(_VbiVpsNfy);
    UNUSED(_VbiWssNfy);
...
static void _InitScart(void)
{
    ...
    _rScartStatus.rFs     = DRV_VDO_SCART_FS_NO_SIGNAL;
    _rScartStatus.rFb    = DRV_VDO_SCART_FB_UNKNOWN;   
...
DRV_ERR_CODE_T _DRV_VDO_SetVga(UINT32 u4Mask, DRV_VDO_VGA_PARAM_T* prParam)
{
    ...
    {
        fgApiVideoVgaSetClock(prParam->u1Clock);        
    }
    if(u4Mask & DRV_VDO_VGAMSK_PHASE)
    {
        fgApiVideoVgaSetPhase(prParam->u1Phase);    
    }
	if(u4Mask & DRV_VDO_VGAMSK_HPOS)
    {
        fgApiVideoGeoHPosition(SV_VP_MAIN,prParam->u1HPos);        
    }
    if(u4Mask & DRV_VDO_VGAMSK_VPOS)
    {
        fgApiVideoGeoVPosition(SV_VP_MAIN,prParam->u1VPos);    
    }    
    return rRetCode;
}
...
DRV_ERR_CODE_T _DRV_VDO_ANA_Start(UINT8 u1VideoIndex)
{
    ...
    fgApiVideoColorSystem((UINT8)SV_CS_AUTO);    
    VERIFY(_DRV_VDO_GetParameter(u1VideoIndex, &prParam) == DRV_E_OK);
...
DRV_ERR_CODE_T _DRV_TVE_CTRL_Enable(UINT8 bEnable) 
{
#if defined(CC_SUPPORT_TVE_CTRL_BY_DRIVER) 
    vApiTVECtrlEnable(bEnable);
#endif
    return DRV_E_OK;
}
