	...
	DRV_VDO_DISP_FMT_T rFmt)
{
    ...
    // initialize rSrcRegion & rOutRegion
    ...
    switch(rFmt)
    {
        ...
        case DRV_VDO_DISP_FMT_LETTERBOX:
            {
				...
				// VDP_MAX_REGION_WIDTH/4 to prevent UINT32 overflow
                u4W = ((_arVideoInternal[u1VideoIndex].rInfo.u2Width * PANEL_GetPanelHeight() * ((VDP_MAX_REGION_WIDTH)>>2)) 
                	/ (_arVideoInternal[u1VideoIndex].rInfo.u2Height * PANEL_GetPanelWidth())) << 2; 
                rOutRegion.u4X = (VDP_MAX_REGION_WIDTH - u4W) >> 1;
                rOutRegion.u4Y = 0;                
                rOutRegion.u4Width = u4W;
                rOutRegion.u4Height = VDP_MAX_REGION_HEIGHT;
                VERIFY(VDP_SetSrcRegion(u1VideoIndex, 1, rSrcRegion) == VDP_SET_OK);        
        ...
        case DRV_VDO_DISP_FMT_ZOOM:        
            	...
            	/ (PANEL_GetPanelHeight() * _arVideoInternal[u1VideoIndex].rInfo.u2Width) >= 1)
            {
				...
				// VDP_MAX_REGION_HEIGHT/4 to prevent UINT32 overflow
                u4H = ((_arVideoInternal[u1VideoIndex].rInfo.u2Width * PANEL_GetPanelHeight() * (VDP_MAX_REGION_HEIGHT >> 2)) 
            	/ (_arVideoInternal[u1VideoIndex].rInfo.u2Height * PANEL_GetPanelWidth())) << 2;                

                rSrcRegion.u4X = 0;
                rSrcRegion.u4Y = (VDP_MAX_REGION_HEIGHT - u4H) >> 1;
            ...
            else
            {
				...
				// VDP_MAX_REGION_WIDTH/4 to prevent UINT32 overflow
                u4W = ((_arVideoInternal[u1VideoIndex].rInfo.u2Height * PANEL_GetPanelWidth() * (VDP_MAX_REGION_WIDTH >> 2)) 
            	/ (_arVideoInternal[u1VideoIndex].rInfo.u2Width * PANEL_GetPanelHeight()) << 2);                
                rSrcRegion.u4X = (VDP_MAX_REGION_WIDTH - u4W) >> 1;
                rSrcRegion.u4Y = 0;
...
DRV_ERR_CODE_T _DRV_VDO_DriverUnmute(UINT8 u1VideoIndex)
{
    ...
    rRetCode = _DRV_VDO_SetOverScanRatio(u1VideoIndex, 
        _arVideoInternal[u1VideoIndex].u1TOverscan, _arVideoInternal[u1VideoIndex].u1BOverscan, 
        _arVideoInternal[u1VideoIndex].u1LOverscan, _arVideoInternal[u1VideoIndex].u1ROverscan);
    ...
    if(u1VideoIndex == 1)
    {
        UINT32 au4PlaneOrder[4] = {PMX_OSD1, PMX_OSD2, PMX_PIP, PMX_MAIN};
        PMX_SetPlaneOrderArray(au4PlaneOrder);
    }
	...
	DRV_VDO_INFO_T* prInfo)
{
    ...
    prInfo->rInputSrc           = _arVideoParam[u1VideoIndex].rInputSrc; //_arVideoInternal[u1VideoIndex].rInfo.rInputSrc;        
    ...
    prInfo->u1RefreshRate = bDrvVideoGetRefreshRate(u1VideoIndex);
    prInfo->u2HTotal = wDrvVideoGetHTotal(u1VideoIndex);
    prInfo->u2VTotal = wDrvVideoGetVTotal(u1VideoIndex);

#ifdef DRV_IF_3DTV
    {
        TV3D_STATUS_T r3DStatus;
        vTV3DGetStatus(u1VideoIndex, &r3DStatus);
        if(r3DStatus.u13DEnable) 
        {
            if(r3DStatus.u1Force2D) 
            {
                prInfo->r3DStatus = (r3DStatus.u1DisplayLR == 0)
                    ?DRV_VDO_3D_FORCE_2D_L:DRV_VDO_3D_FORCE_2D_L;
            }
            else 
            {
                prInfo->r3DStatus = DRV_VDO_3D_WO_FORCE;
            }
        }
        else 
        {
            prInfo->r3DStatus = DRV_VDO_3D_2D_ONLY;
        }
    }
#endif
...
DRV_ERR_CODE_T _DRV_VDO_Set3DTV(UINT8 u1VideoIndex, DRV_VDO_3DTV_STATUS_T r3DStatus) 
{
    VERIFY(u1VideoIndex < 2);
    UNUSED(r3DStatus); 
    
#ifdef DRV_IF_3DTV    
    if(r3DStatus == DRV_VDO_3D_WO_FORCE) 
    {
        PE_SetArg(u1VideoIndex, PE_ARG_TDTV_3D_TO_2D, 0);        
    }
    else if(r3DStatus == DRV_VDO_3D_FORCE_2D_L) 
    {
        PE_SetArg(u1VideoIndex, PE_ARG_TDTV_3D_TO_2D, 1);        
    }
    else if(r3DStatus == DRV_VDO_3D_FORCE_2D_R) 
    {
        PE_SetArg(u1VideoIndex, PE_ARG_TDTV_3D_TO_2D, 2);        
    }    
#endif
...
DRV_ERR_CODE_T _DRV_VDO_Start(UINT8 u1VideoIndex)
{
    ...
    {
        rRet = _DRV_VDO_DIG_Start(u1VideoIndex);
    }
    else
    { 
        rRet = _DRV_VDO_ANA_Start(u1VideoIndex);
    }
    
    if(_arVideoInternal[1].u1State == DRV_VDO_STATE_STOP)
    {
        UINT32 au4PlaneOrder[4] = {PMX_OSD1, PMX_OSD2, PMX_MAIN, PMX_PIP};    
		PMX_SetPlaneOrderArray(au4PlaneOrder);	  		
    }    
    else
    {
    ...
    UINT32 u4T, UINT32 u4B, UINT32 u4L, UINT32 u4R)
{
    ...
    _arVideoInternal[u1VideoIndex].u1TOverscan = u4T;
    _arVideoInternal[u1VideoIndex].u1BOverscan = u4B;    
    _arVideoInternal[u1VideoIndex].u1LOverscan = u4L;
    _arVideoInternal[u1VideoIndex].u1ROverscan = u4R;    

    rOverRegion.u4Left      = u4L;
    rOverRegion.u4Right     = u4R;    
    rOverRegion.u4Top       = u4T;
    rOverRegion.u4Bottom    = u4B;    
	...
	UINT32 u4Mask, DRV_VDO_QUALITY_T* prQuality)
{
    ...
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_BRIGHTNESS, prQuality->u1Brightness);
    }
    if(u4Mask & DRV_VDO_QTYMSK_CONTRAST)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_CONTRAST, prQuality->u1Contrast);
    }	
    if(u4Mask & DRV_VDO_QTYMSK_HUE)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_HUE, prQuality->i1Hue + 50);
    }
    if(u4Mask & DRV_VDO_QTYMSK_SATURATION)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_SATURATION, prQuality->u1Saturation);
    }
/*    
    if(u4Mask & DRV_VDO_QTYMSK_BLE)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_BLE, prQuality->u1Ble);
    }    
*/    
    if(u4Mask & DRV_VDO_QTYMSK_CTI)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_CTI, prQuality->u1Cti);
    }        
/*    
    if(u4Mask & DRV_VDO_QTYMSK_ETI)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_ETI, prQuality->u1Eti);
    }      
*/    
/*    
    if(u4Mask & DRV_VDO_QTYMSK_SHARPNESS_ON_OFF)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_SHARPNESS_ON_OFF, prQuality->u1SharpnessOnOff);
    }        
*/    
    if(u4Mask & DRV_VDO_QTYMSK_SHARPNESS)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_SHARPNESS_ON_OFF, 1);
        i4Ret |= PE_SetArg(u1VideoIndex, PE_ARG_SHARPNESS, prQuality->u1Sharpness);
    }        
    if(u4Mask & DRV_VDO_QTYMSK_NR)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_NR, prQuality->u1Nr);
    }        
/*    
    if(u4Mask & DRV_VDO_QTYMSK_DEINT_FILM)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_DEINT_FILM, prQuality->u1DeintFilm);
    }        
    if(u4Mask & DRV_VDO_QTYMSK_DEINT_DEINT)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_DEINT_DEINT, prQuality->u1DeintDeint);
    }          
    if(u4Mask & DRV_VDO_QTYMSK_DEINT_EDGE)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_DEINT_EDGE, prQuality->u1DeintEdge);
    }            
*/    
    if(u4Mask & DRV_VDO_QTYMSK_ADAPTIVE_LUMA)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_ADAPTIVE_LUMA, prQuality->u1AdaptiveLuma);
    }            
    if(u4Mask & DRV_VDO_QTYMSK_SCE)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_SCE, prQuality->u1Sce);
    }
    if(u4Mask & DRV_VDO_QTYMSK_R_GAIN)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_R_GAIN, prQuality->u1RGain);
    }                
    if(u4Mask & DRV_VDO_QTYMSK_G_GAIN)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_G_GAIN, prQuality->u1GGain);
    }                
    if(u4Mask & DRV_VDO_QTYMSK_B_GAIN)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_B_GAIN, prQuality->u1BGain);
    }                    
/*    
    if(u4Mask & DRV_VDO_QTYMSK_R_OFFSET)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_R_OFFSET, prQuality->u1ROffset);
    }                
    if(u4Mask & DRV_VDO_QTYMSK_G_OFFSET)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_G_OFFSET, prQuality->u1GOffset);
    }                
    if(u4Mask & DRV_VDO_QTYMSK_B_OFFSET)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_B_OFFSET, prQuality->u1BOffset);
    }                           
*/    
/*
...
*/    
    ...
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_WHITE_PEAK_LMT, prQuality->u1WhitePeakLmt);
    }                               
    if(u4Mask & DRV_VDO_QTYMSK_BACKLIGHT_LVL)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_BACK_LIGHT_LVL, prQuality->u1BackLightLvl);
    }                           
    if(u4Mask & DRV_VDO_QTYMSK_ADAPTIVE_BACKLIGHT)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_ADAPTIVE_BACK_LIGHT, prQuality->u1AdaptiveBackLight);
    }      
    if(u4Mask & DRV_VDO_QTYMSK_3D_NR)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_3D_NR, prQuality->u13dNr);
    }   
/*
    if(u4Mask & DRV_VDO_QTYMSK_NS)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_NS, prQuality->u1Ns);
    }             
    if(u4Mask & DRV_VDO_QTYMSK_SOFT_BWS)
    {
        i4Ret = PE_SetArg(u1VideoIndex, PE_ARG_SOFT_BWS, prQuality->u1SoftBws);
    }   
*/    
    if(i4Ret != VDP_SET_OK)
    {
...
{
    VERIFY(PE_SetArg(0, PE_ARG_ADAPTIVE_BACK_LIGHT, 0)    == VDP_SET_OK);
    
    VERIFY(PE_SetArg(0, PE_ARG_R_GAIN, 0x80)              == VDP_SET_OK);
    VERIFY(PE_SetArg(0, PE_ARG_G_GAIN, 0x80)              == VDP_SET_OK);
    VERIFY(PE_SetArg(0, PE_ARG_B_GAIN, 0x80)              == VDP_SET_OK);
//    VERIFY(PE_SetArg(1, PE_ARG_R_GAIN, 0x80)              == VDP_SET_OK);
//    VERIFY(PE_SetArg(1, PE_ARG_G_GAIN, 0x80)              == VDP_SET_OK);
//    VERIFY(PE_SetArg(1, PE_ARG_B_GAIN, 0x80)              == VDP_SET_OK);

    VERIFY(PE_SetArg(0, PE_ARG_R_OFFSET, 0x80)            == VDP_SET_OK);
    VERIFY(PE_SetArg(0, PE_ARG_G_OFFSET, 0x80)            == VDP_SET_OK);
    VERIFY(PE_SetArg(0, PE_ARG_B_OFFSET, 0x80)            == VDP_SET_OK);
//    VERIFY(PE_SetArg(1, PE_ARG_R_OFFSET, 0x0)            == VDP_SET_OK);
//    VERIFY(PE_SetArg(1, PE_ARG_G_OFFSET, 0x0)            == VDP_SET_OK);
//    VERIFY(PE_SetArg(1, PE_ARG_B_OFFSET, 0x0)            == VDP_SET_OK);
    return DRV_E_OK;
}
...
{    
    ...
    //VERIFY(PMX_SetPlaneOrderArray(au4PlaneOrder) == PMX_SET_OK);
    ...
    _arVideoParam[0].rVideoWindow.u4Height = VDP_MAX_REGION_HEIGHT;
    _arVideoParam[0].rVideoWindow.u4Width = VDP_MAX_REGION_WIDTH;
    ...
    VERIFY(VDP_SetEnable(0, 0) == VDP_SET_OK);
    VERIFY(VDP_SetEnable(1, 0) == VDP_SET_OK);    
