...
extern void vSetUploadReadPtr(UINT8 u1DspId, UINT32 u4ReadPtr);
...
extern UINT32 u4ReadInputUnderrun(UINT8 u1DspId, UINT8 u1DecId);
extern void AUD_DspMutePcmOut(BOOL fgEnable);
extern void AUD_DspFastDecode(BOOL fgEnable);
extern BOOL fgDecoderRealPlay(UINT8 u1DspId, UINT8 u1DecId);
extern void vResetUploadFlag(UINT8 u1DspId);
...
static UINT32 u4AudOmxFillFIFO(ADECOMX_HANDLE_T *prAdecOmx, UINT32 u4From, UINT32 u4Size)
{
	...
	u4FifoRp = VIRTUAL(u4GetABufPnt(ADECOMX_DSP_ID, AUD_DEC_MAIN));
...
{    
    ...
    vSetUploadReadPtr(ADECOMX_DSP_ID, PHYSICAL(u4OutputRp));
...
int adecomx_create(ADECOMX_HANDLE_T *prAdecOmx, unsigned long arg)
{
    ...
    //Driver related operation
		...
		vResetUploadFlag(ADECOMX_DSP_ID); 	   

        //Stop all decoder first
        AUD_DSPCmdStop(ADECOMX_DSP_ID, AUD_DEC_MAIN);
        AUD_DSPCmdStop(ADECOMX_DSP_ID, AUD_DEC_AUX);
#ifdef CC_MT5391_AUD_3_DECODER
        AUD_DSPCmdStop(ADECOMX_DSP_ID, AUD_DEC_THIRD);
#endif
#ifdef CC_AUD_4_DECODER_SUPPORT
        AUD_DSPCmdStop(ADECOMX_DSP_ID, AUD_DEC_4TH);
#endif

        //Load decode code
        AUD_SetDecType(ADECOMX_DSP_ID, AUD_DEC_MAIN, AUD_STREAM_FROM_MEMORY, eFmt);
    ...
    prAdecOmx->u4InputBufSA  = (UINT32)(VIRTUAL(u4GetAFIFOStart(ADECOMX_DSP_ID, AUD_DEC_MAIN)));
    prAdecOmx->u4InputBufSZ = (UINT32)(VIRTUAL(u4GetAFIFOEnd(ADECOMX_DSP_ID, AUD_DEC_MAIN))) - prAdecOmx->u4InputBufSA;
...
int adecomx_DoEmptyBuffer(ADECOMX_HANDLE_T *prAdecOmx, unsigned long arg)
{
    ...
    if (!prAdecOmx->fgPlay)
    {
        ...
        AUD_DSPCmdPlay(ADECOMX_DSP_ID, AUD_DEC_MAIN);
...
int adecomx_DoFillBuffer(ADECOMX_HANDLE_T *prAdecOmx, unsigned long arg)
{
    ...
    while (rOutputBuf.u4Offset != rOutputBuf.u4Size)
	{
		...
		if (i4Filled == 0) {
			...
			if (prAdecOmx->fgEOS) {
				prAdecOmx->u4InputBufRP = VIRTUAL(u4GetABufPnt(ADECOMX_DSP_ID, AUD_DEC_MAIN));
