...
INT32 flac_minfo_is_handler(VOID* pv_obj, MEDIA_FORMAT_T* pt_format)
{
    /************************************************************************/
    /* Check parameter                                                      */
    /************************************************************************/
 ...
 *                                the operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 flac_minfo_fopen(CHAR*              ps_path_name, 
                       VOID*              pv_obj)
{
    ...
    DBG_INFO(("API: flac_minfo_fopen begin \r\n"));
    /************************************************************************/
    /* Check parameter                                                      */
    /************************************************************************/
    if ((pv_obj == NULL)
        || (ps_path_name == NULL))
    {
        DBG_ERROR(("ERROR: flac_minfo_fopen Empty point variable\r\n"));
        return MINFOR_INV_ARG;
    }
    /************************************************************************/
    /* do                                                                   */
    ...
    /* Big Endian input for FLAC file */
    ...
    pt_flac_info_obj = (FLAC_STREAM_INFO_T*)_mm_util_mem_alloc(sizeof(FLAC_STREAM_INFO_T));
    
    if (pt_flac_info_obj == NULL)
    {
        DBG_ERROR(("ERROR: flac_minfo_fopen alloc memory01 fail \r\n"));
        return MINFOR_OUT_OF_MEMORY;
    }
    x_memset(pt_flac_info_obj, 0, sizeof(FLAC_STREAM_INFO_T));
    pt_minfo_obj->pv_handler_obj = (VOID*)pt_flac_info_obj;
    
    DBG_INFO(("API: flac_minfo_fopen OK \r\n"));    
    return MINFOR_OK;
 ...
 *                                the operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 flac_minfo_mopen(VOID*              pv_mem_data, 
                       SIZE_T             z_mem_leng, 
                       VOID*              pv_obj)
{    
    MINFO_OBJ_T*            pt_minfo_obj;
    ...
    DBG_INFO(("API: flac_minfo_mopen begin \r\n"));
    /************************************************************************/
    /* Check parameter                                                      */
    /************************************************************************/
    if ((pv_obj == NULL)
        || (pv_mem_data == NULL))
    {
        DBG_ERROR(("ERROR: flac_minfo_mopen Empty point variable\r\n"));
        return MINFOR_INV_ARG;
    }
    /************************************************************************/
    /* do                                                                   */
    ...
    if (pt_flac_info_obj == NULL)
    {
        DBG_ERROR(("ERROR: flac_minfo_mopen alloc memory01 fail \r\n"));
        return MINFOR_OUT_OF_MEMORY;
    }
    x_memset(pt_flac_info_obj, 0, sizeof(FLAC_STREAM_INFO_T));
    pt_minfo_obj->pv_handler_obj = (VOID*)pt_flac_info_obj;

    DBG_INFO(("API: flac_minfo_mopen OK \r\n"));
    return MINFOR_OK;
}    

/*-----------------------------------------------------------------------------
 * Name:  flac_minfo_close
 *
 ...
 *                                the operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 flac_minfo_close( 
                        VOID*              pv_obj)
{
    MINFO_OBJ_T*            pt_minfo_obj;
    FLAC_STREAM_INFO_T*     pt_flac_info_obj;

    DBG_INFO(("API: flac_minfo_close begin \r\n"));
    ...
    if (pt_flac_info_obj->pt_md5_signature != NULL)
    {
        _mm_util_mem_free(pt_flac_info_obj->pt_md5_signature);
        pt_flac_info_obj->pt_md5_signature = NULL;
    }
 ...
 *                                the operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 flac_minfo_parse(VOID*              pv_obj)
{
    INT32   i4_ret = MINFOR_OK;
    
    DBG_INFO(("API: flac_minfo_parse begin \r\n"));
    ...
    /* do                                                                   */
    /************************************************************************/
    /* parse flac file boxes */
    i4_ret = flac_parse_elements((MINFO_OBJ_T*)pv_obj);
    return i4_ret;

} 

/*-----------------------------------------------------------------------------
 * Name:  flac_minfo_get_num_recs
 *
 ...
 *                                the operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 flac_minfo_get_num_recs(VOID*             pv_obj, 
                              ...
                              UINT16*            pui2_num_recs)
{
    ...
    {
        DBG_ERROR(("ERROR: flac_minfo_get_num_recs - invalid parameter \r\n"));
        return MINFOR_INITIAL_ERR;
    }
 ...
 *                                the operation has been completed.
 ----------------------------------------------------------------------------*/
INT32 flac_minfo_get_info(VOID*             pv_obj, 
                         MINFO_INFO_TYPE_T  e_info_type, 
                         ...
                         SIZE_T*            pz_buf_leng)
{
    ...
    MINFO_FLAC_AUDIO_STM_ATRBT_T*   pt_flac_audio_atrbt;
    ...
    {
        DBG_ERROR(("ERROR: flac_minfo_get_info - invalid parameter \r\n"));
        return MINFOR_INITIAL_ERR;
    }
    /************************************************************************/
    /* do                                                                   */
    /************************************************************************/
    pt_minfo_obj = (MINFO_OBJ_T*) pv_obj;
    pt_flac_info_obj = pt_minfo_obj->pv_handler_obj;
    if (pv_buf == NULL)
    {   
        /*get length only*/
    ...
    else
    {
        ...
        {
            DBG_ERROR(("ERROR: flac_minfo_get_info - insufficient buffer \r\n"));  
    ...
    if (pt_flac_info_obj)
    {
        if (pt_minfo_obj->b_parsing_successful == FALSE)
        {
            DBG_ERROR(("ERROR: flac_minfo_get_info - not ready \r\n"));
            return MINFOR_INFO_NOT_READY;    
        }
        ...
        switch(e_info_type)
        {
            case MINFO_INFO_TYPE_GENERIC:
            {
                ...
                if (pt_general_info)
                {
                    pt_general_info->ui8_pb_duration = 
                        (UINT64)_mm_div64(pt_flac_info_obj->ui8_total_sample, pt_flac_info_obj->ui4_sample_rate, NULL) * 90000;
                    pt_general_info->ui8_media_sz = (UINT64)(pt_flac_info_obj->ui8_file_size);
                    pt_general_info->ui1_strm_ns = 1;
                }
                else
                {
                    #ifndef __KERNEL__
                        DBG_ERROR(("[FLAC_MINFO]pt_general_info IS NULL\n"));
                    #endif
                }
            }
            break;
            case MINFO_INFO_TYPE_AUDIO_ATTR:
            {
                ...
                pt_flac_audio_atrbt = (MINFO_FLAC_AUDIO_STM_ATRBT_T*)(&(pt_minfo_rec->u.t_stm_attribute.u.t_flac_audio_stm_attr));
                if (pt_flac_audio_atrbt != NULL)
                {
                    pt_flac_audio_atrbt->e_enc = MINFO_INFO_AUD_ENC_FLAC;
                    pt_flac_audio_atrbt->ui2_min_block_size = pt_flac_info_obj->ui2_min_block_size;
                    pt_flac_audio_atrbt->ui2_max_block_size = pt_flac_info_obj->ui2_max_block_size;
                    pt_flac_audio_atrbt->ui4_min_frame_size = pt_flac_info_obj->ui4_min_frame_size;
                    pt_flac_audio_atrbt->ui4_max_frame_size = pt_flac_info_obj->ui4_max_frame_size;
                    pt_flac_audio_atrbt->ui4_sample_rate = pt_flac_info_obj->ui4_sample_rate;
                    pt_flac_audio_atrbt->ui1_num_channels = pt_flac_info_obj->ui1_num_channels;
                    pt_flac_audio_atrbt->ui1_bits_per_sample = pt_flac_info_obj->ui1_bits_per_sample;
                    pt_flac_audio_atrbt->ui8_total_sample = pt_flac_info_obj->ui8_total_sample;
                    pt_flac_audio_atrbt->pt_md5_signature = pt_flac_info_obj->pt_md5_signature;
                    pt_flac_audio_atrbt->ui4_bitrate = pt_flac_info_obj->ui4_bitrate;
                }
            }
            break;
            case MINFO_INFO_TYPE_IDX_INFO:
            {
                ...
                pt_minfo_rec->u.t_idx_info.ui8_data_offset = pt_flac_info_obj->ui8_cluster_position;
                pt_minfo_rec->u.t_idx_info.ui8_data_sz = (UINT64)pt_flac_info_obj->ui8_cluster_leng;
            }    
            break;
            case MINFO_INFO_TYPE_META_DATA:
            {
                ...
                {
                    pt_meta_data_info->ui8_pb_duration = 
                        (UINT64)_mm_div64(pt_flac_info_obj->ui8_total_sample, pt_flac_info_obj->ui4_sample_rate, NULL) * 90000;
                }
                else
                {
                    #ifndef __KERNEL__
                        DBG_ERROR(("[FLAC_MINFO]pt_meta_data_info IS NULL\n"));
                    #endif
                }
            }
            break;
