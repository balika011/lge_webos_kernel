...
typedef struct
{
	...
	unsigned char *iv;//[MTDRM_HDCP2_KEYLEN_LC128];
	unsigned int  data_len;
}MTDRM_HDCP2_DataDecrypt_T;



typedef struct
{
	unsigned char protected_decryption_key[PROTECTED_KEY_MAX];
	unsigned int  len;
}MTDRM_HDCP2_ProtectedDecryptionKey;
 ...
 *  @retval MTR_NOT_OK      Fail.
 */
int MTDRM_HDCP2_DataDecrypt (unsigned char *pOutputbuf, const unsigned char *pInputbuf, int length, unsigned char *pCounter);
 ...
 *  @retval MTR_NOT_OK      Fail.
 */
int MTDRM_SIGN_SNAPSHOT_IMAGE (const char* blkdev, unsigned int imgSize, unsigned int snapOffset,unsigned int frag_unit_size,unsigned int num_of_frag);
...
int MTDRM_HDCP2_SW_DataDecrypt (MTDRM_HDCP2_DataDecrypt_T* pdataDecrypt);
...
typedef struct SecureDataMoveArg{
    SECURE_BUF_HANDLE *Des; 
    SECURE_BUF_HANDLE Src; 
    size_t  *Size;
    unsigned int RDP;
}SecureDataMoveArg;

typedef struct SecureGetDataArg{
    SECURE_BUF_HANDLE u4SecureSrc;
    UINT8 *pu4NormalDes;
    UINT32 len; 
}SecureGetDataArg;

typedef struct SecureDataMoveDoneArg{
    UINT32 vdecHandle;
    SECURE_BUF_HANDLE rdPoint;
    UINT32 rdSize;
}SecureDataMoveDoneArg;


typedef struct SecureDataMoveInfoArg{
    UINT32 vdecHandle;
    SECURE_BUF_HANDLE *rdPoint;
    UINT32 *rdSize;
}SecureDataMoveInfoArg;


int MTDRM_SECURE_BUF_DATAMOVE(SECURE_BUF_HANDLE *Des,SECURE_BUF_HANDLE Src, size_t *size, unsigned int RDP);
int MTDRM_SECURE_BUF_GETDATA(SECURE_BUF_HANDLE u4SecureSrc,UINT8 *pu4NormalDes,UINT32 len);

int MTDRM_SECURE_CPB_DATAMOVE_DONE(UINT32 vdecHandle, SECURE_BUF_HANDLE handle ,UINT32 size);
int MTDRM_SECURE_CPB_DATAMOVE_INFO(UINT32 vdecHandle,SECURE_BUF_HANDLE *rdPoint , UINT32 *rdSize);
