...
#define MTDMX_KEY_MAX_NUM                 32
#elif defined(CC_MT5882)/***L5L***/
#define MTDMX_NUM_PID_INDEX				  64
#define MTDMX_NUM_FILTER_INDEX            128
#define MTDMX_KEY_MAX_NUM                 16
#else
...
#define MTDMX_EXTEND_PID_START            MTDMX_NUM_PID_INDEX
#define MTDMX_NUM_EXTEND_PID_INDEX        (MTDMX_EXTEND_PID_START + MTDMX_NUM_PID_INDEX)/***L5O, MTDMX_NUM_EXTEND_PID_INDEX means MTDMX_MAX_EXTEND_PID_INDEX***/
#define MTDMX_EXTEND_FILTER_START         48/***L5O, built-in***/
#define MTDMX_NUM_EXTEND_FILTER_INDEX     (MTDMX_EXTEND_FILTER_START + MTDMX_NUM_FILTER_INDEX)/***L5O, MTDMX_NUM_EXTEND_FILTER_INDEX means MTDMX_MAX_EXTEND_FILTER_INDEX***/

#define MTDMX_NUM_TS_INDEX                4

#define MTDMX_MAX_NUM_FILTER_INDEX        192/***L5L?***/

#if (MTDMX_MAX_NUM_FILTER_INDEX < MTDMX_NUM_FILTER_INDEX)/***L5L***/
#error "MTDMX_MAX_NUM_FILTER_INDEX must be bigger than MTDMX_NUM_FILTER_INDEX"
#endif
...
#define MTSEC_CNT ((MTDMX_NUM_EXTEND_FILTER_INDEX + 31)/ 32)/***L5O***/
/***L5L, avoid unreasonably large filteridx***/
#define MTSEC_NUM(filteridx)         \
        (((filteridx) >= ((MTSEC_CNT-1)*32))? (MTSEC_CNT-1) :((filteridx)/32))
#define MTFILTERINDEX(filteridx)    ((filteridx)&0x1F)/***L5L***/
...
typedef enum
{
    MTDMX_CRYPTO_KEY_SECURE_PVR_AES_0 = 0x0,
    MTDMX_CRYPTO_KEY_SECURE_PVR_AES_IV_0,
    MTDMX_CRYPTO_KEY_TZ_MARLIN_VOD, 
    MTDMX_CRYPTO_KEY_TZ_DTV_CIPLUS,
    MTDMX_CRYPTO_KEY_MAX
} MTDMX_CRYPTO_KEY_TYPE_T;

typedef enum
{
    MTDMX_KEY_IDX_MARLIN_EVEN = 0,
    MTDMX_KEY_IDX_MARLIN_ODD,
    MTDMX_KEY_IDX_SECURE_PVR
} MTDMX_KEY_IDX_T;
...
typedef enum
{
    ...
    MTDMX_DESC_MODE_CA_SECURE             ///<CA key in TZ
} MTDMX_DESC_MODE_T;
...
/** MTDMX_ConnectFilter
 *
 *  @param  u1VirtualPidx         virtural Filterx,
 *  @param  eConType                 eConType,
 */
//-----------------------------------------------------------------------------
EXTERN MT_RESULT_T MTDMX_ConnectFilter(UINT8 u1VirFilterx, MTDMX_CONN_TYPE_T eConType);

//-----------------------------------------------------------------------------
/** MTDMX_ConnectAvailableFilterx
 * 
 *  @param  eConType                 eConType,
 *  @param  pu1VirtualFilterx         virtural filterx,
 */
//-----------------------------------------------------------------------------
EXTERN MT_RESULT_T MTDMX_ConnectAvailableFilterx(MTDMX_CONN_TYPE_T eConType, UINT8 *pu1VirtualFilterx);

//-----------------------------------------------------------------------------
/** MTDMX_DisconnectFilter
 *
 *  @param  u1VirtualFilterx         virtural Filterx,
 */
//-----------------------------------------------------------------------------
EXTERN MT_RESULT_T MTDMX_DisconnectFilter(UINT8 u1VirtualFilterx);
...
//-----------------------------------------------------------------------------
/** MTDMX_ConnectAvailablePidx
 *
 *  @param  rPidInfo           rPidInfo
  
 *  @param  pu1VirtualPidx         virtural pidx,
 */
//-----------------------------------------------------------------------------
EXTERN MT_RESULT_T MTDMX_ConnectAvailablePidx(const MTDMX_PID_INFO_T  *prPidInfo, UINT8 *pu1VirtualPidx);
...
//-----------------------------------------------------------------------------
/** MTDMX_GetDMXPIDInfo
 * Get Fbm DMXPID information
 *
 *  @param  prHwInfo         prHwInfo
 *  @retval MTR_OK          Success.
 *  @retval MTR_NOT_OK      Fail. 
 */
//-----------------------------------------------------------------------------
EXTERN MT_RESULT_T MTDMX_GetDMXPIDInfo(MTDMX_DMXPID_INFO_T *prDMXPIDInfo);
/** MTDMX_SetEITSection
...
//-----------------------------------------------------------------------------
/** Set the CA key index which in Secure mode
 *
 *  @param  u1DmxKeyIdx           Demux  key index value
 *  @param  u1SecureKeyIdx       Secure mode Key index 
 *  @param  eKeyType                Key type
 */
//-----------------------------------------------------------------------------
EXTERN MT_RESULT_T MTDMX_ReloadSecureKey(UINT8 u1DmxKeyIdx, UINT32 u4SecureKeyIdx, MTDMX_CRYPTO_KEY_TYPE_T eKeyType);

//-----------------------------------------------------------------------------
/** MTDMX_GetStcId
 * This function will find stc_id for vdec on the corresponding ts index.
 * If PCR PID doesn't exist, the stc_id will be zero value.
 * If u1TsIdx != 0xFF, it will find stc_id by ts_id
 * If u1TsIdx == 0xFF, it will find stc_id by es_id
 *
 *  @param  u1TsIdx         TS index
 *  @param  ucEsId          vdec id
 *  @param  pu1StcId [out]  pointer to stc_id
 *
 *  @retval MTR_OK          Success.
 *  @retval MTR_NOT_OK      Fail.
 */
//-----------------------------------------------------------------------------
MT_RESULT_T MTDMX_GetStcId(UINT8 u1TsIdx, UCHAR ucEsId, UINT8 *pu1StcId);
