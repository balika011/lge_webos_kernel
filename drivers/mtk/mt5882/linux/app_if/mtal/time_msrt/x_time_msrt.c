...
#define CLI_SET_TMS_PATH_HELP      "Set TMS main path (n = none,b = boot,c = channel change,s = auto scan,w = wfd latency,o = power off)"
#define CLI_GET_TMS_PATH_HELP      "Get TMS main path (n = none,b = boot,c = channel change,s = auto scan,w = wfd latency,o = power off)"
         ...
         ((ps_tms_sel = pps_argv[1]) != NULL))
    {
        ...
        if (x_strlen (ps_tms_sel) == 1)
        {
            switch (*ps_tms_sel)
            {
                ...
                case 'W':
                case 'w':
                    ui4_flag = TMS_FLAG_WFD_LATENCY;
                    break;

                case 'O':
                case 'o':
                    ui4_flag = TMS_FLAG_POWER_OFF;
                    break;  
                default:
    ...
    const CHAR*           s_comment )
{
    if (b_tms_init)
    {
        ...
        if (s_comment)
        {
            x_strncpy(pt_tms->s_comment,
                      s_comment,
                      TMS_COMMENT_MAX_LENGTH);
        }
        else
        {
            pt_tms->s_comment[0] = 0;
        }
    ...
    const CHAR*           s_comment )
{
    ...
    if ( ui4_no != 0 )
    {
        x_dbg_stmt("[TIME_MSRT] #%.2u, %-32s, %-32s, SPOT at  %8llu ms\r\n",
                   ui4_no, s_name, (s_comment) ? s_comment : "", ui8_time_stamp);
    }
    else
    {
        x_dbg_stmt("[TIME_MSRT] ###, %-32s, %-32s, SPOT at  %8llu ms\r\n",
    ...
    const CHAR*           s_comment )
{
    ...
    if ( ui4_no != 0 )
    {
        x_dbg_stmt("[TIME_MSRT] #%.2u, %-32s, %-32s, DIFF is         %8llu ms\r\n",
                   ui4_no, s_name, (s_comment) ? s_comment : "", ui8_time_stamp);
    }
    else
    {
        x_dbg_stmt("[TIME_MSRT] ###, %-32s, %-32s, DIFF spot at    %8llu ms\r\n",
                   s_name, (s_comment) ? s_comment : "", ui8_time_stamp);
    }
}
    ...
    const CHAR*           s_comment)
{
    ...
    if ( ui4_no != 0 )
    {
        x_dbg_stmt("[TIME_MSRT] #%.2u, %-32s, %-32s BEGIN-END costs %8llu ms\r\n",
                   ui4_no, s_name, s_comment, ui8_time_stamp );
    }
    else
    {
        x_dbg_stmt("[TIME_MSRT] ###, %-32s, %-32s, %-16s BEGIN-END costs %8llu ms\r\n",
    ...
    const CHAR*           s_comment )
{
    ...
    x_dbg_stmt("[TIME_MSRT] ###, %-32s, %-32s, BEGIN at time   %8llu ms\r\n",
               s_name, (s_comment) ? s_comment : "", ui8_time_stamp );
}
#if 0
/*-----------------------------------------------------------------------------
    ...
    const CHAR*           s_comment )
{
    ...
    x_dbg_stmt("[TIME_MSRT] ###, %-32s, %-32s, END at time     %8llu ms\r\n",
               s_name, (s_comment) ? s_comment : "", ui8_time_stamp );
