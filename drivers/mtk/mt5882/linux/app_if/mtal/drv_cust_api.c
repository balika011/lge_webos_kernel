...
extern UINT8 u1HDMI_Get5VStatus(E_HDMI_SWITCH_NUM ePort);
    ...
    BOOL                      b_store )
{
	...
	//#endif
       ...
       case DRV_CUSTOM_SET_CRNT_INP_SRC_INFO:
	    ...
	    case DRV_CUSTOM_MISC_TYPE_SRC_AUTO_SETTING:
       {        
#ifdef SYS_MHL_SUPPORT
         vMhlSetSourceStatus(0,*(UINT8*)pv_set_info);
#endif
            i4Ret = RMR_OK;
            break;
        }
        ...
        case DRV_CUSTOM_MISC_TYPE_CEC_ENABLE_SET:
        {
            ...
            {
#ifdef SYS_MHL_SUPPORT
        		vMhlSetRapOnOff(0,((UINT32)pv_set_info ? 1 : 0));
#endif
        ...
        case DRV_CUSTOM_MISC_TYPE_MHL_RCP_RECEIVE_SET:
        {
            ...
            i4Ret = -1;
            if ((z_size == 1) && (u4Addr = DRVCUST_InitGet(eEepromNHLRCPOffset)) != 0)
            {
                i4Ret = EEPROM_Write(u4Addr, (UINT32)pv_set_info, 1);
                if (i4Ret == 0)
                {
...
extern void CustomGetPanelMiscInfoFromFlash(MISC_TYPE_PANEL_INFO_T *pData, UINT32 u4Num);
extern UINT32 FlashPanel_GetPanelNumber(void);
#endif
    ...
    SIZE_T*                   pz_size )
{
    ...
    switch (e_misc_type)
    {
        ...
        case DRV_CUSTOM_GET_AUTOTEST_BOOT_FLAG_EEPROM_OFFSET:
        {
            if (pz_size == NULL)
            {
                return RMR_DRV_INV_GET_INFO;
            }
            else if ((*pz_size < sizeof(UINT32)) || (pv_get_info==NULL))
            {
...
#ifdef LOAD_PANEL_FROM_FLASH_CUSTOM	
		case DRV_CUSTOM_MISC_TOTAL_PANEL_NUM:
		{ 
		    *(UINT32*)pv_get_info=FlashPanel_GetPanelNumber();
			Printf("[%s]Total Panel Num=%d\n",__FUNCTION__,*(UINT32*)pv_get_info);
			i4Ret = RMR_OK;
            break;
		}
		case DRV_CUSTOM_MISC_TYPE_PANEL_INFO:
		{
		    CustomGetPanelMiscInfoFromFlash((MISC_TYPE_PANEL_INFO_T *)pv_get_info,FlashPanel_GetPanelNumber());
			Printf("[_%s]_DRV_CUSTOM_MISC_TYPE_PANEL_INFO\n",__FUNCTION__);
			i4Ret = RMR_OK;
            break;
		}
#endif		
...
//             ASSERT( (((MISC_TYPE_TUNER_SIF_T *)pv_get_info)->ui1_offset_len) ==0);
	 ...
	 case DRV_CUSTOM_MISC_TYPE_HDMI_KSV_VALUE:
         ...
         case DRV_CUSTOM_MISC_TYPE_HDMI_HPD_LOW:
        {
		 ...
		 case DRV_CUSTOM_MISC_TYPE_HDMI_STATUS:
		 {
			 UINT8 u1hdmi_portid=0;
			 u1hdmi_portid = (UINT8)(((MISC_TYPE_HDMI_STATUS_T *)pv_get_info)->ui1_hdmi_comp_id);
			 ((MISC_TYPE_HDMI_STATUS_T *)pv_get_info)->aui1_5VActive = u1HDMI_Get5VStatus((E_HDMI_SWITCH_NUM)u1hdmi_portid);
			 i4Ret = RMR_OK;	 
			 break;
		 }
        ...
        {
            UINT8 i=0;
             static UINT32 u4_max_counter;
             if (0 != DRVCUST_OptQuery(eDrvLogoEarlyReturnCounter, &u4_max_counter))
             {
                u4_max_counter = 0xFFFF;
            }
        ...
        case DRV_CUSTOM_MISC_TYPE_MHL_RCP_RECEIVE_GET:
        {
            ...
            i4Ret = -1;
            if ((u4Addr = DRVCUST_InitGet(eEepromNHLRCPOffset)) != 0)
            {
...
*/
         ...
         case DRV_CUSTOM_MISC_TYPE_CURRENT_NAND_NAME:
		{
			#ifdef CC_NAND_BOOT
			SIZE_T z_len = 0;
			const CHAR* z_getname=NAND_GetName();
			if(z_getname == NULL)
				return RMR_DRV_INV_GET_INFO;
			z_len = x_strlen(z_getname);
			if( z_len >= *pz_size)
				z_len = *pz_size;

			x_memset( (CHAR *) pv_get_info, 0x00, *pz_size);
			x_strncpy((CHAR *)pv_get_info,(const CHAR*) NAND_GetName(), z_len);
			i4Ret = RMR_OK;
			#endif 
        ...
        case DRV_CUSTOM_GET_AUTOTEST_BOOT_FLAG_EEPROM_OFFSET:
        {
            i4Ret = DRVCUST_InitQuery(eUsbEepromAutoTestOffset, pv_get_info);
            break;
        }
#endif
