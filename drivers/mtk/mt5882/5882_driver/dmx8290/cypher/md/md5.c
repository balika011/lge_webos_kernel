...
void MD5_Input(MD5_CONTEXT_T *context, const UINT8 *input, UINT32 inputLen)
{
    ...
    if (inputLen >= partLen) 
    {
        x_memcpy((UINT8 *)&context->au1Buffer[index], (UINT8 *)input, partLen);
        MD5Transform (context->au4State, context->au1Buffer);

        for (i = partLen; i + 63 < inputLen; i += 64)
        {
            MD5Transform (context->au4State, (UINT8 *)&input[i]);
        }
    ...
    x_memcpy((UINT8 *)&context->au1Buffer[index], (UINT8 *)&input[i], inputLen-i);
}
...
void MD5_Result(MD5_CONTEXT_T *context, UINT8 digest[MD5_HASH_SIZE]) 
{
    ...
    x_memset((UINT8 *)context, 0, sizeof (*context));
}
...
UINT32 MD_PadMessage(UINT8 *u1Data, UINT32 u4Size)
{
    ...
    padLen = (index < 56) ? (56 - index) : (120 - index);
    x_memcpy((UINT8 *)(u1Data+u4Size), PADDING, padLen);
