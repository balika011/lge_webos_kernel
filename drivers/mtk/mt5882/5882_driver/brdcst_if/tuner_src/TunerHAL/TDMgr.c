...
#ifdef CC_SUPPORT_STR
static BOOL  u1SuspendMode = FALSE;
...
STATIC UINT8 u1TunerType = 0;
void TunerSelectType(UINT8 TunerType)
{
    u1TunerType = TunerType;
}
STATIC UINT8 TunerGetType(void)
{
   return u1TunerType;
}
#endif
...
#ifdef CC_SET_TUNER_TYPE_BY_MODEL_INDEX   
    #if (fcTUNER_TYPE >= cTUNER_ALL_START) || (fcTUNER_TYPE==cCUST_TUNER)
        _u1TunerType = TunerGetType();
         if(_u1TunerType==0) 
         {     
...
S32 TunerClose(void)
{
    ...
    x_sema_delete(uniTDCtxLock);//lei140207
            ...
            x_break_fct _BreakFct, void* pvArg)
{
    ...
    BOOL                    fgRetSts = FALSE;
#ifdef CC_DUAL_TUNER_SUPPORT
    static TUNER_CONN_TYPE_COMP Comp_conn_type[MAX_COMP_NUM];
#else
...
#endif
#ifdef CC_DUAL_TUNER_SUPPORT
    //1.this is for one demod can only support one spec connection at the same time
    //so,we should disconnect the previous connection
    if((e_conn_type != Comp_conn_type[TunerID].pre_conn_type)
		&&(Comp_conn_type[TunerID].pre_conn_type != cCONN_INVALID))
	{		
		mcSHOW_DRVAPI_MSG(("[Func TunerAcq] Connect type changed, disconnect previous connect type\r\n"));
		TunerDisc(TunerID, Comp_conn_type[TunerID].pre_conn_type); 
        #ifdef CC_DEMOD_NON_DVBS_LNBC_OUTPUT_0V
        if(Comp_conn_type[TunerID].pre_conn_type == TUNER_CONN_TYPE_SAT_DIG){
            BOOL s_State = 0;
            if(DRVAPI_TUNER_OK != TunerSetAttribute(TunerID,TUNER_SET_TYPE_LNBC_SET_SLEEP,(void *)&s_State, sizeof(BOOL))){
                return fgRetSts;
            }
        }
        #endif        
	...
	//2.this is for our internal demod or ConnExclusive demod,that at the same
	//time only one of them can in connection,so we need discnnect the Exclusive connection
	if((ptTDCtx->tTunerCfg.ConnExclusiveID != MTK_INVALID_ID)
		&&
	  (Comp_conn_type[ptTDCtx->tTunerCfg.ConnExclusiveID].pre_conn_type != cCONN_INVALID))
	{	
		mcSHOW_DRVAPI_MSG(("[Func TunerAcq] Exclusive connect, disconnect Exclusive connect\r\n"));
		TunerDisc(ptTDCtx->tTunerCfg.ConnExclusiveID, Comp_conn_type[ptTDCtx->tTunerCfg.ConnExclusiveID].pre_conn_type);
        #ifdef CC_DEMOD_NON_DVBS_LNBC_OUTPUT_0V
        if(Comp_conn_type[ptTDCtx->tTunerCfg.ConnExclusiveID].pre_conn_type == TUNER_CONN_TYPE_SAT_DIG){
            BOOL s_State = 0;
            if(DRVAPI_TUNER_OK != TunerSetAttribute(ptTDCtx->tTunerCfg.ConnExclusiveID,TUNER_SET_TYPE_LNBC_SET_SLEEP,(void *)&s_State, sizeof(BOOL))){
                return fgRetSts;
            }
        }
        #endif        
        ...
        (e_conn_type != prev_e_conn_type))
    {
        ...
        #ifdef CC_DEMOD_NON_DVBS_LNBC_OUTPUT_0V
        if(prev_e_conn_type == TUNER_CONN_TYPE_SAT_DIG){
            BOOL s_State = 0;
            if(DRVAPI_TUNER_OK != TunerSetAttribute(prev_TunerID,TUNER_SET_TYPE_LNBC_SET_SLEEP,(void *)&s_State, sizeof(BOOL))){
                return fgRetSts;
            }
        }
        #endif        
...
#ifdef CC_DEMOD_SUPPORT_SHUTDOWN_MODE
/* Shut_Down External Demod @ATV Mode if shut_function is ready in Ext_Demod driver part.//Xiaohong_D20140801*/
    //if((TunerID==MTK_ANALOG_0)&&((prev_TunerID==ptTDCtx->tTunerCfg.ConnExclusiveID)||(prev_TunerID==0xffff)))
    if(TunerID==MTK_ANALOG_0)
    {

        BOOL fgShutdown = 1;// This value can be 0 or 1 but can't be NULL to aviod NULL pointer hung up issue.
        if(DRVAPI_TUNER_OK != TunerSetAttribute(ptTDCtx->tTunerCfg.ConnExclusiveID,
        TUNER_SET_TYPE_DEMOD_SHUTDOWN,(void *)&fgShutdown, sizeof(BOOL)))
        {
            return fgRetSts;
        }
        mcSHOW_DRVAPI_MSG(("Tuner Shut_Down Mode CONFIG\n"));
    }
/*End Shut_Down Mode*/
#endif
...
#ifdef CC_DUAL_TUNER_SUPPORT
    Comp_conn_type[TunerID].pre_conn_type = e_conn_type;
#else
