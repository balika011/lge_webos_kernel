...
#define MPV_FRAMERATE_UNIT ((UINT64)1000)
...
UINT32 MPV_GetDecMode(UCHAR ucEsId)
{
    ...
    {
        // tmp workaround for swdmx check vdec status
        u4DecMode = (UINT32)VDEC_ST_STOP;
    }
    else
    {
...
INT32 MPV_SetIFrameBuf(UCHAR ucEsId, const MPV_DEC_I_FRAME_BUFF_T* prIFrameBuff)
{
    ...
    VDEC_SetWptr(ucEsId, _arMpvPesInfo[ucEsId].u4FifoEnd);
...
//    VDEC_ES_INFO_T *prVdecEsInfo;
    ...
    //prEsInfo = (MPV_ES_INFO_T*)_MpvGetHdlr(ES 0);
    //prVdecEsInfo = _VDEC_GetEsInfo(ucEsId);
    //ASSERT(prEsInfo);
                          ...
                          UINT32 u4QpAvgThrsdP, UINT32 u4DbkRatioThrsdP, UINT32 u4MvThrsd)
{
    ...
    if (!fgDisable)
    {
        ...
        prEsInfo->u4QpAvgPerIThrsd = u4QpAvgThrsd;
        prEsInfo->u4DbkCurRatioThrsd = u4DbkRatioThrsdP;
        prEsInfo->u4DbkPerIRatioThrsd = u4DbkRatioThrsd;
#else
...
void MPV_QueryStatus(void)
{
    ...
    for (ucEsId = 0; ucEsId < MPV_MAX_ES; ucEsId++)
    {
        ...
        if (prVdecEsInfoKeep->eCurState != VDEC_ST_IDLE)
         {
            ...
            {
                Printf("Receive(%d) OK(%d) Skip(%d) Drop(%d) Timeout(%d) Q(%d) ",
                        prMpvCounter->u4RetrieveNs, prMpvCounter->u4DecOkNs,
                ...
                if (prMpvCounter->u4DecOkNs != 0)
                {
                    ...
                    {
                        Printf("(%d.%06d/", prMpvCounter->rIDeltaTime.u4Seconds, prMpvCounter->rIDeltaTime.u4Micros);
                    }

                    Printf("%dx%d", prMpvCounter->u2HSize, prMpvCounter->u2VSize);
        ...
        {
            LOG(1, "MPV(%d) Idle Receive(%d) OK(%d) Skip(%d) Drop(%d) Timeout(%d) Q(%d)\n",
...
BOOL MPV_GetUserData(UCHAR ucEsId, UCHAR* pucUserDataTmpBuf, UINT32 u4Rptr, UINT32 u4DataLen)
{
    ...
    if (u4DataLen > 0)
    {
        ...
        {
            x_memcpy(pucUserDataTmpBuf, &(prExtInfo->aucUserDataBuf[ucEsId][u4Rptr]), u4DataLen);
        }
        else
        {
            ASSERT((MAX_USERDATA_SIZE - u4Rptr) > 0);
            x_memcpy(pucUserDataTmpBuf, &(prExtInfo->aucUserDataBuf[ucEsId][u4Rptr]), (MAX_USERDATA_SIZE - u4Rptr));

            x_memcpy(pucUserDataTmpBuf + (MAX_USERDATA_SIZE - u4Rptr) ,
                prExtInfo->aucUserDataBuf[ucEsId], u4DataLen - (MAX_USERDATA_SIZE -u4Rptr));
        }
    }
    else
    {
...
void _MPV_ClearEsInfo(UCHAR ucEsId)
{
    ...
    //if(prVdecInfo->eCurFMT == VDEC_FMT_MPV)
    {
        ...
        //DBK threshold
#if defined(VDEC_IS_POST_MT5398)
        prMpvEsInfo->u4QpAvgPerIThrsd = MPV_DBK_PER_I_QP_THRSD;
        prMpvEsInfo->u4DbkCurRatioThrsd = MPV_DBK_CUR_FRM_RATIO_THRSD;
        prMpvEsInfo->u4DbkPerIRatioThrsd = MPV_DBK_PER_I_RATIO_THRSD;
        prMpvEsInfo->u4QpAvgPerI = 0;
        prMpvEsInfo->u4DbkRatioPerI = 0;
#else
...
void _MPV_SkipPic(UCHAR ucMpvId, UCHAR ucEsId)
{
        ...
        prVdecEsInfo->u4VldReadPtr))
    {
        ...
        LOG(10, "ES(%d) VDEC_SetRptr F1 0x%x\n", ucEsId,
            prVdecEsInfo->u4DecReadPtr);
    }
#endif
}

BOOL _MPV_PutFb2DispQ(UCHAR ucEsId, UCHAR ucMpvId, UCHAR ucFbId)
{   
    BOOL fgRet = FALSE;
    BOOL fgNotDisplay=FALSE, fgWaitI=FALSE;
    UINT32 u4PTS;
    INT32 i4TempRef;
    UINT64 u8Offset;
    VDEC_ES_INFO_T* prVdecEsInfo;
    ...
    {
        u4PTS = _MPV_CalcPts(ucEsId, ucFbId);
        ...
        if (prFbmPicHdr)
        {
            ...
            fgWaitI = ((prFbmPicHdr->ucPicCdTp == I_TYPE) ||
                       (prFbmPicHdr->ucPicCdTp == P_TYPE)) ? IS_DECFLAG_SET(prVdecEsInfo, DEC_FLG_WAIT_NEXT_I) : FALSE;
        }
        else
        {
            i4TempRef = 0;
        ...
        else
        {
            ...
            if ((FBM_FB_STATUS_READY == ucFbStatus) ||
                (FBM_FB_STATUS_FIELD_READY == ucFbStatus) ||
                (FBM_FB_STATUS_DECODE == ucFbStatus))
            {
               ...
               if (VDEC_PRINT_PQ_INFO & prVdecEsInfoKeep->u4LOGCtrl)
               {
                ...
                VDecPrintf(ucEsId, VDEC_LOG_VDEC2VDP, "[VDEC-VDP(%d)] PicType = %d , PTS = %d, STC = %d, FbgID = %d, FbId = %d\n",ucEsId,
                        prFbmPicHdr?prFbmPicHdr->ucPicCdTp:0,
                        prFbmPicHdr?prFbmPicHdr->u4PTS:0,
                        STC_GetStcValue(prVdecEsInfo->ucStcId),
                        prVdecEsInfo->ucFbgId, ucFbId);
#endif
            ...
            else
            {
                if (VDEC_PRINT_PQ_INFO & prVdecEsInfoKeep->u4LOGCtrl)
                {
                    LOG(0, "PQ(%d) (%d %3d) (%d %d) 0x%X/0x%llX 0x%X Fail(%d)\n", ucEsId,
                           prFbmPicHdr?prFbmPicHdr->ucPicCdTp:0,
                           prFbmPicHdr?prFbmPicHdr->i4TemporalRef:0xFF,
                           prVdecEsInfo->ucFbgId, ucFbId,
                           prFbmPicHdr?prFbmPicHdr->u4PTS:0x0,
...
BOOL MPV_EsFlwCtrl(UCHAR ucEsId, UCHAR* pucMpvId, INT8 i1EsBufStatus, UCHAR ucSkipMode)
{
    ...
    {        
        ...
        if((prVdecEsInfo->u4DropDecFlag & (DEC_FLG_2ND_FLD_PIC)) > 0)
        {
            prVdecEsInfo->u4DropDecFlag ^= (DEC_FLG_2ND_FLD_PIC);
        }
        if (IS_DECFLAG_SET(prVdecEsInfo, DEC_FLG_2ND_FLD_PIC))
                        ...
                        else
                        {
                            ...
                            if (IS_DECFLAG_SET(prVdecEsInfo, DEC_FLAG_DBK) && (!prVdecEsInfo->fgRefNoDbk))
                            {
                                if (prVdecEsInfo->ucDbkRefId == FBM_FB_ID_UNKNOWN) //change normal mode to DBK mode
                                {
                                    ...
                                    UNUSED(_MPV_PutFb2DispQ(ucEsId, *pucMpvId, ucBRefFbId));  
                                }
                                else
                                {
                                    UNUSED(_MPV_PutFb2DispQ(ucEsId, *pucMpvId, prVdecEsInfo->ucDbkRefId));   
                            ...
                            #endif
                            {
                                fgRet = _MPV_PutFb2DispQ(ucEsId, *pucMpvId, ucBRefFbId); 
                                if(fgRet)
                                {
                                    *pucMpvId = VDEC_MAX_VLD;
                                    return TRUE;
               ...
               (!prVdecEsInfo->fgMMPlayback))))
        {
            ...
            if (!prVdecEsInfo->ucSequenceHdrFound)
            {
                if (!prPesInfo->fgSeqHdr)
                {
                    ...
                    if (!VDEC_SetRptr(prPesInfo->ucEsId,
                        prPesInfo->u4VldReadPtr, prPesInfo->u4VldReadPtr))
                    {
                        LOG(10, "ES(%d) VDEC_SetRptr F0\n", prPesInfo->ucEsId);
                    }
...
UINT32 _MPV_CalcPtsBase(UCHAR ucEsId, VOID* pvrFbmPicHdr)
{
    UINT64 u8PTSOffset;
    UINT32 u4PTSOffset;
    VDEC_ES_INFO_T* prVdecEsInfo = 0;
    VDEC_ES_INFO_KEEP_T* prVdecEsInfoKeep = 0;
    FBM_SEQ_HDR_T *prFbmSeqHdr = 0;
    FBM_PIC_HDR_T* prFbmPicHdr = pvrFbmPicHdr;
    ...
    {
        LOG(3, "_MPV_CalcPts prVdecEsInfoKeep null\n");
        return 0;
    }
    ...
    u8PTSOffset = _VDEC_Div6464(VDEC_U8_SECOND_CLOCK*MPV_FRAMERATE_UNIT, (UINT64)prVdecEsInfo->u4FrameRate);
    ...
    if (prFbmSeqHdr->fgProgressiveSeq)        // Progressive Sequence
    {
        ...
        if ((prFbmPicHdr->fgRepFirstFld) && (prFbmPicHdr->fgTopFldFirst))
        {
            ...
            if (!prFrcPrm->ucProgressiveOut)
            {
                ...
                u4PTSOffset /= 2;
                u8PTSOffset = _VDEC_Div6464(u8PTSOffset, (UINT64)2);
            }
            */
        }
        else
    ...
    {
        if(prVdecEsInfo->u8PTSLast != 0)
        {
            prFbmPicHdr->u8PTS = prVdecEsInfo->u8PTSLast;
            prVdecEsInfo->u8PTSLast = prVdecEsInfo->u8PTSLast + u8PTSOffset;
        }
        else
        {        
            if(prVdecEsInfoKeep->fgVPush)
            {
                prVdecEsInfo->u8PTSLast = 1;
            }
        }
    }
    else
    {
        prVdecEsInfo->u8PTSLast = prFbmPicHdr->u8PTS + u8PTSOffset;
    }
...
UINT32 _MPV_CalcPts(UCHAR ucEsId, UCHAR ucFbId)
{
    ...
    return _MPV_CalcPtsBase(ucEsId, prFbmPicHdr);
}


VOID _MPV_SetIsr(UCHAR ucEsId, UCHAR ucMpvId)
{
    _MPV_IsrInit(ucMpvId);
    UNUSED(ucEsId);
}
...
UINT32 MPV_OsdDisplay(UINT32 u4OsdPlane, UINT32 u4Scaler, UINT32 u4ColorMode, UINT32 u4BmpAddr, UINT32 u4Width, UINT32 u4Height)
{
    ...
    // protect OSD avoid  Scale and FlipTO
    ...
    // unlock OSD, let other module use it
...
BOOL MPV_ShowLogo(VOID)
{
        ...
        (DRVCUST_OptGet(eDrvLogoVdecDefault)))
    {
		...
		prVdecFbOp.u4Size = u4DataSize;
		prVdecFbOp.u4SrcAddr = u4DataAddr;
		prVdecFbOp.u4DesAddr = prFbmPool->u4Addr;
		LOG(3,"----000-----MPV_ShowLogo-----------MEMCPY\n");
...
#if 1
        STORG_SyncRead(DRVCUST_OptGet(eDrvLogoVdecFlashDevId), u4DataAddr, (VOID*)VIRTUAL(prFbmPool->u4Addr), u4DataSize);
#else
#ifdef CC_NAND_ENABLE
	...
	#if defined(CC_TRUSTZONE_SUPPORT) && defined(CC_TVP_SUPPORT)
	x_memset(&prVdecFbOp,0x0,sizeof(VDEC_TO_TZ_FB_OPERATION_T));
	prVdecFbOp.u4Size = rMpvBuf.u4Size;
	prVdecFbOp.u4SrcAddr = (UINT32)(rMpvBuf.pucIFrameBufAddr);

	LOG(3,"----000-----TZ_VDEC_FB_Operation------_MPV_ShowLogo-----Flush\n");
