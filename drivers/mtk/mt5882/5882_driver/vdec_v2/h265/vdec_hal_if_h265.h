...
/// Initialize video decoder hardware
/// \return If return value < 0, it's failed. Please reference hal_vdec_errcode.h.
INT32 i4VDEC_HAL_H265_LAE_InitVDecHW(
    UINT32 u4VDecID                                    ///< [IN] Video decoder hardware ID
);
...
/// Get next start code
/// \return Current input window of vld while finding start code 
UINT32 u4VDEC_HAL_H265_LAE_GetStartCode_PicStart(
    UINT32 u4BSID,                                     ///< [IN] Barrel shifter hardware ID of one video decoder
    UINT32 u4VDecID                                    ///< [IN] Video decoder hardware ID 
); 
...
/// Initialize barrel shifter with byte alignment
/// \return If return value < 0, it's failed. Please reference hal_vdec_errcode.h.
INT32 i4VDEC_HAL_H265_LAE_InitBarrelShifter(
    UINT32 u4BSID,                                      ///< [IN] Barrel shifter hardware ID of one video decoder
    UINT32 u4VDecID,                                     ///< [IN] Video decoder hardware ID
    VDEC_INFO_H265_BS_INIT_PRM_T *prH265BSInitPrm
);
...
void vVDEC_HAL_H265_SetRefPicListReg(
    ...
    VDEC_INFO_H265_FBUF_INFO_T *prH265FBBufInfo
);
...
void vVDecWriteLAEHEVCVLD(
    UINT32 u4VDecID, 
    UINT32 u4Addr, 
    UINT32 u4Val
);

UINT32 u4VDecReadLAEHEVCVLD(
    UINT32 u4VDecID, 
    UINT32 u4Addr
);

void vVDEC_HAL_H265_SetLAEDecAddr(
    UINT32 u4VDecID,                                   ///< [IN] Video decoder hardware ID
    UINT32 u4CABACAddr,          ///< [IN] Pointer to struct of sequence parameter set
    UINT32 u4ECAddr
);

void vVDEC_HAL_H265_SetLAEDecCore(
    UINT32 u4VDecID,                ///< [IN] Video decoder hardware ID
    UINT32 u4LAECore             ///< [IN] LAE Core Number
);
...
INT32 i4VDEC_HAL_H265_DecStart(
    UINT32 u4VDecID,
    VDEC_INFO_H265_DEC_PRM_T *prDecPrm              ///< [IN] Pointer to H265 decode Information
);
...
/// Set h265 ufo info
void u4VDEC_HAL_H265_SetUFO(
    UINT32 u4VDecID,                                    ///< [IN] Video decoder hardware ID
    VDEC_INFO_H265_FBUF_INFO_T *prCurrentFBuf
);

/// Set h265 error concealment info
void u4VDEC_HAL_H265_SetECInfo(
    UINT32 u4VDecID,                                    ///< [IN] Video decoder hardware ID
    VDEC_INFO_H265_FBUF_INFO_T *prECRefFBuf,
    VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm
);

/// Set still image param
/// \return None
void vVDEC_HAL_H265_SetStillImg(
    UINT32 u4VDecID,                                              ///< [IN] Video decoder hardware ID
    VDEC_INFO_H265_DEC_PRM_T *prH265DecPrm
);
...
UINT32 u4VDEC_HAL_LAE_VDec_ReadFinishFlag(
    UINT32 u4VDecID
);

UINT32 u4VDEC_HAL_MCORE_VDec_ReadFinishFlag(
    UINT32 u4VDecID
);
...
/// Write core0 & core1 common setting when do PIP playback
/// \return None
void vVDEC_HAL_H265_VDec_SetPipComReg(
    UINT32 u4VDecID,
    H265_SPS_Data *prSPS
);

/// Write core1 common setting when do PIP playback
/// \return None
void vVDEC_HAL_H265_VDec_SetPipCore1Reg(
    UINT32 u4VDecID
);

/// Write core1 common setting when do PIP playback
/// \return None
void vVDEC_HAL_H265_VDec_DisableMCore(
    UINT32 u4VDecID
);

void vVDEC_HAL_H265_VDec_DumpReg(
    UINT32 u4VDecID,
    UINT32 u4VdecBaseAddr,
    UINT32 u4Start,
    UINT32 u4End
);

//Set err-concealment register
//Lae must be set also, or, will make mcore decode timeout if current bitstream is 
//error bitstream
