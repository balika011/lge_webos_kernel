...
/*----------------------------------------------------------------------------
 * Function: ThreadDelete
 *
 * Description:
 *      Terminate a given thread, by sending a message to the exit reaper,
 *      then suspending the thread waiting for reaping.
 *
 * Inputs:
 *      pv_th_id: The thread id
 *
 * Outputs: -
 *
 * Returns:
 ...
 *      OSR_DRV_FAIL: Fail
 *---------------------------------------------------------------------------*/
static INT32 ThreadDelete(VOID* pv_th_id)
{
    NU_TASK*          pTask;
    OS_DRV_THREAD_T*  pThread;
    UNSIGNED au4Message[4];
    ...
    // Suspend the thread to wait termination
    VERIFY(NU_Suspend_Task(pTask) == NU_SUCCESS);
                       ...
                       x_os_drv_thread_main_fct pf_main)
{
    ...
    ASSERT(NU_Current_Task_Pointer() != NU_NULL);
...
/*----------------------------------------------------------------------------
 * Function: x_os_drv_thread_delete
 *
 * Description:
 *      This API exits a given thread
 *
 * Inputs: -
 *      pv_th_id: The thread id
 *
 ...
 *      OSR_DRV_FAIL: Fail
 *---------------------------------------------------------------------------*/
INT32 x_os_drv_thread_delete(VOID *pv_th_id)
{
    return ThreadDelete(pv_th_id);
}
...
x_os_drv_thread_delay(UINT32 u4_delay_in_ms)
{
    ...
    u4_delay_in_ms -= (rTimeResult.u4Seconds * 1000);
    if ((int) u4_delay_in_ms < 0)
    {
        return;
    }
    if ((u4_delay_in_ms * 1000) < rTimeResult.u4Micros)
    {
        return;
    }
