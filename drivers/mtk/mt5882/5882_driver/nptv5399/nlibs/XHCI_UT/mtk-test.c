...
#include <linux/dmapool.h>
 ...
 volatile char g_port_connect;
 volatile char g_port_reset;
...
static int dbg_sif_init(int argc, char** argv);
/********************************************************
 *	slt.disable
...
static int dbg_phy_eyescan_tx_off(int argc, char** argv);
static int dbg_phy_eyescan_init(int argc, char** argv);
...
CMD_TBL_T _arPCmdTbl[] =
{
	...
	{"dbg.u3TxOff", &dbg_phy_eyescan_tx_off},
    {"dbg.u3eyescaninit", &dbg_phy_eyescan_init},
...
static int t_u3auto_ctrl_loopback(int argc, char** argv){
	int ret,loop,length,num=0,max_length;	
...
static int t_u3auto_loopback(int argc, char** argv){
	...
	if(argc > 11){
		count = (int)simple_strtol(argv[6], &argv[6], 10);
		printk(KERN_ERR "start add offset set to %d\n", count);
	}
#if 0
	if(u3auto_hcd_reset() != RET_SUCCESS)
...
#endif
extern int f_add_rdn_len_str_threads(int dev_num, int ep_num, int maxp, char isCompare, struct usb_device *usbdev, char isEP0);
static int t_u3auto_stress(int argc, char** argv){
	int ret, num_ep, i, speed=0;
	int transfer_type[5];
...
extern int f_add_random_access_reg_thread(struct xhci_hcd *xhci, int port_id, int port_rev, int power_required);
static int t_power_random_access_regs(int argc, char** argv){
...
static int t_power_remotewakeup(int argc, char** argv){
    if(argc > 1){
		g_port_id = (int)simple_strtol(argv[1], &argv[1], 10);
		xhci_dbg(xhci, "g_port_id set to %d\n", g_port_id);
	}
...
static int t_power_u2_lpm(int argc, char** argv){
//	int ret;
	int hle=0, rwe=0, hirdm=0, besl=0, besld=0, pdn=0, int_nak_active=0, bulk_nyet_active=0;
	int L1_timeout=0;
	if(argc > 1){
...
static int t_power_u2_swlpm(int argc, char** argv){
	...
	if(is_wakeup){
		wait_event_on_timeout((volatile int *)&g_port_plc, 1, 5000);
		if(g_port_plc == 0){
...
//	int speed;
	int sg_len=0;
	int hub_num, port_num, dev_num=0, round, i;
	int dram_offset, extension;
...
static int t_hub_configuresubhub(int argc, char** argv){
	int parent_hub_num, hub_num, port_num=0;
	struct xhci_hcd *xhci;
	xhci = hcd_to_xhci(my_hcd);
	parent_hub_num = 1;
...
static int t_hub_configure_eth_device(int argc, char** argv){
	int hub_num=0, port_num=0, dev_num=0;
	int ret;
...
//	int transfer_type, bInterval, maxp;
	struct usb_device *udev;
	USB_DEV_SPEED speed=DEV_SPEED_INACTIVE;
	int ret;
...
//	USB_DEV_SPEED speed;
	...
	udev = dev_list[dev_num-1];
	g_slot_id = udev->slot_id;
	return dev_hub_remotewakeup(0, udev);
}
...
//	type_value = type+1;
	...
	if(g_dev_not_value != value){
		printk(KERN_ERR "device notification value error, expected %d, recived %d!!\n", value, (int)g_dev_not_value);
		return RET_FAIL;
	}
	return ret;
}
...
//        short gpd_buf_size,bd_buf_size;
        ...
        int maxp, count;
//        uint rand_num;
        uint dev_delay_us, host_delay_ms=0;
...
//    uint rand_num;
    ...
    u32 __iomem *addr;
//    int temp; 
    int dev_delay_time=0, host_delay_time=0, do_ux=0;
	int port_id;
...
static int u2ct_disconnect_detect(void){
	...
	for(i=0; i<num_u2_port; i++){
		...
		temp = temp & (~SSUSB_U2_PORT_CKBG_EN) & (~SSUSB_U2_PORT_PDN);
...
static int dbg_sch_algorithm(int argc, char** argv){
	...
	int ep_dir, transfer_type=0, maxp=0, bInterval=0, mult, burst;
	int ret;
	int ep_index;
...
static int dbg_u3_calibration(int argc, char** argv){
	char u1TimeDelay; 
	char u1TimingValue=0;
	char u1PhyDrvValue=0;
	char u1TxSel;
	char u1RxSel;
	char u1PhyDrv;
	int u4LatchValue;
...
//	__u32 __iomem *ip_reset_addr;
	...
	u3_cmd_addr = (__u32 __iomem *)U3_XHCI_CMD_ADDR;
	u3_sts_addr = (__u32 __iomem *)U3_XHCI_STS_ADDR;
	u3_p1_sc_addr = (__u32 __iomem *)U3_P1_SC;
	u3_p1_ltssm_addr = (__u32 __iomem *)U3_P1_LTSSM;
	u3_link_err_count_addr = (__u32 __iomem *)U3_LINK_ERR_COUNT;
	u3_recovery_count_addr = (__u32 __iomem *)U3_RECOVERY_COUNT;
	u3phy_init();
	u1TxSel = 0;
	for(u1RxSel = 0; u1RxSel<2; u1RxSel++){
		u1PhyDrv = 2;
		for(u1TimeDelay=0; u1TimeDelay<32; u1TimeDelay++){
			printk(KERN_ERR "Test TxSel[0x%02x] RxSel[0x%02x] TimingDelay[0x%02x] Reg[0x%02x]...\n"
...
//			msleep(10);
			...
			if((u4USBStsVal&U3_CNR) == U3_CNR){
				printk(KERN_ERR "CNR not 0 after HCRST \n");
				continue;
			}
...
//				msleep(10);
			...
			if(u1IsPass == 0){
				printk(KERN_ERR "[PASS] PLS=0, Error=%d, Recovery=%d, TxSel[0x%02x] RxSel[0x%02x] PhyDrv[0x%02x] Reg[0x%02x] TimingDelay[0x%02x] Reg[0x%02x]\n"
...
static int dbg_u3w(int argc, char**argv){
	...
	U3PhyWriteReg32(u4TimingAddress, u1TimingValue);
...
static int dbg_u3r(int argc, char**argv){
	...
	mdelay(500);
	u4TimingAddress = (int)simple_strtol(argv[1], &argv[1], 16);
	u1ReadTimingValue = U3PhyReadReg32(u4TimingAddress);
...
{	
    PHY_UINT32 value = 0;
    value = U3PhyReadReg32(0xF0070904);
    printk(KERN_ERR "reg value = 0x%08X\n", value);

    value = value & (0xFFFF7FFF);
    value = value | (0x00004000);
    U3PhyWriteReg32(0xF0070904, value);
...
static struct cdev xchi_mtk_test_cdev;

static int __init mtk_test_init(void)
{
	int retval = 0;
    dev_t dev = MKDEV(XHCI_MTK_TEST_MAJOR, 0);

	printk(KERN_DEBUG "xchi_mtk_test Init\n");


retval = register_chrdev_region(dev, 1, "xhci_mtk_test_reg");
if (retval != 0)
{
    printk(KERN_ERR "[xchi_mtk_test] Unable to allocate region!\n");
    return retval;
}
cdev_init(&xchi_mtk_test_cdev, &xhci_mtk_test_fops);
xchi_mtk_test_cdev.owner = THIS_MODULE;
retval = cdev_add(&xchi_mtk_test_cdev, dev, 1);
if (retval != 0)
{
    printk(KERN_ERR "[xchi_mtk_test] Unable to add device!\n");
    return retval;
}
