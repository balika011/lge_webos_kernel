...
extern void disableAllClockPower(void);
...
{
	printk("xhci_hcd_release 0x%08X.\n", (uint32_t)dev);
	dev->parent = NULL;
...
void setInitialReg(void){
    ...
    addr = (int*) (SSUSB_U3_MAC_BASE+U3_LFPS_TIMING_PARAMETER_1);//u3_ext_t13_t11
    temp = readl(addr);
    temp &= ~(0x3fff << 16);
    temp |= (0x1388 << 16);
    writel(temp, addr);
/*
	...
	u3phy_init();
*/
/*********** for U1 test workaround brfore ECO
	addr = (int*) (0xf0070904);//rg_ssusb_pdnctl 
	temp = readl(addr);
	temp &= ~(0x1);
//	temp |= PM_LC_TIMEOUT_VALUE;
	writel(temp, addr);
