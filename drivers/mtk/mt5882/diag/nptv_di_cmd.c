...
#define MIB_PRE_DOWN_FOR_END (0x1)
#define MIB_PRE_DOWN_FOR_EXIT (0x2)
...
CLI_EXEC_T arYcprocDiTbl[] = {
    ...
    {"OnOff", "oo", _i4DiOnOff, NULL, "Enable/disable MDDi/DRAM controller", CLI_GUEST},
...
static HANDLE_T hThreadtimer;
UINT8 u1MibPredown=0;
static VOID _DiHVPSTestTimerDo(HANDLE_T  pt_tm_handle, VOID *pv_tag)
{
    DI_TIMER_T *prePDInfo=(DI_TIMER_T*)pv_tag;
    UINT16 u2WH = 0,u2WidthPre=0;
    static DI_TIMER_T curPDInfo;

    if((u1MibPredown & MIB_PRE_DOWN_FOR_EXIT)!=0)
    {
        Printf("Exit Predown loop!\n");
        u1MibPredown = 0;
        return;
    }
    
    if(prePDInfo->u1HVPredown==1)
    {
        u2WidthPre = DIVIDE_WITH_ROUND(((UINT32)wDrvVideoInputWidth(prePDInfo->u1Path) * prePDInfo->u2Cnt), 0x8000);
    }
    else
    {
        u2WidthPre = DIVIDE_WITH_ROUND(((UINT32)wDrvVideoInputHeight(prePDInfo->u1Path) * prePDInfo->u2Cnt), 0x8000);
    }
    
    while(prePDInfo->u2Cnt >= prePDInfo->u2Low)
    {   
        if(prePDInfo->u1HVPredown==1)
        {
            u2WH = DIVIDE_WITH_ROUND(((UINT32)wDrvVideoInputWidth(prePDInfo->u1Path) * prePDInfo->u2Cnt), 0x8000);
        }
        else
        {
            u2WH = DIVIDE_WITH_ROUND(((UINT32)wDrvVideoInputHeight(prePDInfo->u1Path) * prePDInfo->u2Cnt), 0x8000);
        }
        ...
        if (u2WidthPre != u2WH)
        {
            if (u2WH%2 == 0)
            {
                prePDInfo->u2EvenWH = u2WH;
                prePDInfo->u2EvenWHCnt = prePDInfo->u2Cnt;
            }
            if ((u2WH%prePDInfo->u2Step) == 0)
            {
                x_memcpy((void*)&curPDInfo, (void*)(prePDInfo), sizeof(DI_TIMER_T));
                
                if(curPDInfo.u1HVPredown==1)
                {
                    Printf("\nTest Width = %d ,u2EvenHCnt=0x%x\n", curPDInfo.u2EvenWH,curPDInfo.u2EvenWHCnt );
                    CLI_Parser_Arg("vdp.s.hps %d %d", curPDInfo.u1Path, curPDInfo.u2EvenWHCnt); 
                }
                else
                {
                    Printf("\nTest Height = %d ,u2EvenVCnt=0x%x\n", curPDInfo.u2EvenWH,curPDInfo.u2EvenWHCnt);
                    CLI_Parser_Arg("vdp.s.vps %d %d", curPDInfo.u1Path, curPDInfo.u2EvenWHCnt); 
                }

                VERIFY(x_timer_start(hThreadtimer,curPDInfo.u1Delay*1000, X_TIMER_FLAG_ONCE, _DiHVPSTestTimerDo, (VOID*)&curPDInfo) == OSR_OK);
                break;
            }
        }
        prePDInfo->u2Cnt--;
    ...
    if( prePDInfo->u2Cnt < prePDInfo->u2Low)
    {
        u1MibPredown |=MIB_PRE_DOWN_FOR_END; 
        Printf("Predown Success!\n");
        return;
    }
}
...
static INT32 _i4DiHPSTest(INT32 i4Argc, const CHAR ** szArgv)
{
    ...
    if((x_strcmp(szArgv[1], "m") != 0)&&(x_strcmp(szArgv[1], "p") != 0))
    {
        Printf("Usage: %s <m/p> high low delay (option:step) \n", szArgv[0]);
    	return 1;
    }
    curPDInfo.u1Path = ((x_strcmp(szArgv[1], "m") == 0) ? SV_VP_MAIN : SV_VP_PIP);
    curPDInfo.u2Cnt = StrToInt(szArgv[2]);
    curPDInfo.u2Low = StrToInt(szArgv[3]);
    curPDInfo.u1Delay = StrToInt(szArgv[4]);
    
    if (i4Argc == 6)
    {
        curPDInfo.u2Step = StrToInt(szArgv[5]);
        
        if(curPDInfo.u2Step==0)
        {
            Printf("Predown command invalid!!\n"); 
            return 0;
        }
    }
    else
    ...
    curPDInfo.u2EvenWHCnt=0x8000;
    curPDInfo.u2EvenWH =0;
    curPDInfo.u1HVPredown= 1;
    
    if((u1MibPredown & MIB_PRE_DOWN_FOR_EXIT)!=0)
    {
        Printf("Enter predown command too fast!!\n"); 
        return 0;
    }   
    
    VERIFY(x_timer_create(&hThreadtimer) == OSR_OK);
    VERIFY(x_timer_start(hThreadtimer, 300, X_TIMER_FLAG_ONCE, _DiHVPSTestTimerDo, (VOID*)&curPDInfo) == OSR_OK);
    while(1)
    {
        if(SerPollGetChar() !=0)//exit the predown loop
        {
            if((u1MibPredown&MIB_PRE_DOWN_FOR_END)==0)
            {
                u1MibPredown |= MIB_PRE_DOWN_FOR_EXIT;
            }
            else
            {
                 u1MibPredown = 0;
