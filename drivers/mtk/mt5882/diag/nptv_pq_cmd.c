...
INT32 _i4YcprocDemo(INT32 i4Argc, const CHAR **szArgv);
INT32 _i4YcprocPQ(INT32 i4Argc, const CHAR **szArgv);
INT32 _i4YcprocSPBypass(INT32 i4Argc, const CHAR **szArgv);
...
static INT32 _i4YcprocGenTiming(INT32 i4Argc, const CHAR **szArgv);
static INT32 _i4YcprocGenTimingByCopy(INT32 i4Argc, const CHAR **szArgv);
#endif
...
static INT32 _i4StartDump(INT32 i4Argc, const CHAR **szArgv);
static INT32 _i4ContinuousDump(INT32 i4Argc, const CHAR **szArgv);
static INT32 _i4SetDumpDRAMAddr(INT32 i4Argc, const CHAR **szArgv);
...
CLI_EXEC_T arDump[] = {
#ifdef CC_CLI
    {"Start Dump",          "start", _i4StartDump, NULL, "Start Dump", CLI_SUPERVISOR},
    {"Set Mode", "mode",  _i4ContinuousDump, NULL, "Set Continuous Mode", CLI_GUEST},
    {"Force Addr",  "waddr",  _i4SetDumpDRAMAddr, NULL, "Force DRAM Address", CLI_GUEST},        
#endif
    {NULL, NULL, NULL, NULL, NULL, CLI_SUPERVISOR}
};
...
CLI_EXEC_T arAdaptiveLuma[] = {
    ...
    {"Set Adaptive Luma", "sal", _i4SetALPara, NULL, "Set Adaptive Luma Param", CLI_SUPERVISOR},
    ...
    {"Get Info", "g", _i4GetALInfo, NULL, "Get Info", CLI_SUPERVISOR},
#endif
    {NULL, NULL, NULL, NULL, NULL, CLI_SUPERVISOR}
};
...
CLI_EXEC_T arYcprocCmdTbl[] = {
    ...
    {"SCE Idx", "sceidx", _i4YcprocSCEIdx, NULL, "Update SCE Table by Using Function and Index", CLI_SUPERVISOR},
    ...
    {"Demo function", "demo", _i4YcprocDemo, NULL, "Enable/Disable Demo function", CLI_SUPERVISOR},
#ifndef NDEBUG
	...
	{"Dump", "dump",  NULL, arDump, "Dump Command",  CLI_GUEST},        	    
	#ifdef CC_SUPPORT_PREPROC_TIMING
	{"Timing", "tim",  _i4YcprocGenTiming, NULL,"Timing Set Command",  CLI_SUPERVISOR}, 
	{"Timingcopy", "timcpy",  _i4YcprocGenTimingByCopy, NULL,"Timing copy from omux Set Command",  CLI_SUPERVISOR},
	#endif
#endif
    {NULL, NULL, NULL, NULL, NULL, CLI_SUPERVISOR}
};
...
CLI_EXEC_T arNRCmdTbl[] = {
    ...
    {"BNR Blk LS", "bls", _i4NRBNRLineSum, NULL, "BNR Block LineSum", CLI_SUPERVISOR},
...
extern UINT8 DRVCUST_Get_BLBlockData(UINT8 * pu1BLBlockDataTbl);
#endif
static INT32 _i4LcDimContentInfo(INT32 i4Argc, const CHAR **szArgv)
{
	UINT8 u4H,u4V;
    ...
    vDrvLcDimLog((UINT8)StrToInt(szArgv[1]));
		...
		if (i4Argc != 3)
		{
			Printf("Usage: [0]:HBlock [1]:VBlock \n");
			return 0;
		}
		u1BlkNum=StrToInt(szArgv[1])*StrToInt(szArgv[2]);
		UINT8 *bLcDimBlkAvg = (UINT8*)x_mem_alloc(sizeof(u1BlkNum));
	
		if(!DRVCUST_Get_BLBlockData((UINT8 *)bLcDimBlkAvg))
		{
			Printf("%s %d} Error !!!\n",__FUNCTION__, __LINE__);
	        return (-1);
		}
		Printf("Driver Backlight Block Data: \n\n");
		for (u4V=0 ; u4V<StrToInt(szArgv[2]); u4V++)
		{
			for (u4H=0 ; u4H<StrToInt(szArgv[1]); u4H++)
			{
				Printf("%3d\t",bLcDimBlkAvg[u4H+u4V*StrToInt(szArgv[1])]);
			}
			Printf("\n");
		}
		Printf("\n\n");
...
static INT32 _i4LcDimSPISetCmd(INT32 i4Argc, const CHAR **szArgv)
{
	if ((i4Argc != 2) && (i4Argc != 3))
	{
		...
		Printf("[0] write [1] read [2] light up [3] logoff [4] repair [5] set scanning\n");
		return 0;
	}
...
static INT32 _i4StartDump(INT32 i4Argc, const CHAR **szArgv)
{
	if ((i4Argc != 6) && (i4Argc != 4))
	{
		Printf("Usage (%s): start <module> <width> <height> <shrink width (optional)> <shrink width (height)>\n", szArgv[0]);
        Printf("  <module od>     = 0x1\n");
        Printf("  <module pscan>  = 0x2\n");
        Printf("  <module scaler> = 0x8\n");
	...
	if (i4Argc == 4)
	{
        vDrvDumpSetSize(StrToInt(szArgv[2]), StrToInt(szArgv[3]));
	}
	else // (i4Argc == 6)
	{
        vDrvDumpSetSize(StrToInt(szArgv[2]), StrToInt(szArgv[3]));
        vDrvDumpSetShrinkSize(StrToInt(szArgv[4]), StrToInt(szArgv[5]));        
	}
    vDrvDumpStart(StrToInt(szArgv[1]));
   	return 0;
}
...
static INT32 _i4ContinuousDump(INT32 i4Argc, const CHAR **szArgv)
{
	if (i4Argc != 2)
	{
		Printf("Usage (%s): set continuous dump mode <ON(1)/OFF(0)>\n", szArgv[0]);

		return 0;
	}

    vDrvDumpSetContinuousMode(StrToInt(szArgv[1]));
...
static INT32 _i4SetDumpDRAMAddr(INT32 i4Argc, const CHAR **szArgv)
{
	if (i4Argc != 2)
	{
		Printf("Usage ()%s : force set dump DRAM address <address (hex)>\n", szArgv[0]);

		return 0;
	}

	vDrvDumpForceDRAMAddress(StrToInt(szArgv[1]));
...
static INT32 _i4YcprocGenTiming(INT32 i4Argc, const CHAR **szArgv)
{
	 ...
	 if(i4Argc != 3)
    {
        ...
        Printf("path: 0 Main.\n");
        Printf("path: 1 Sub.\n");
        Printf("Timing: 0 MODE_480P.\n");
        Printf("Timing: 1 MODE_525I.\n");  
        Printf("Timing: 2 MODE_576P.\n");
        Printf("Timing: 3 MODE_625I.\n");
        Printf("Timing: 4 MODE_720p_60.\n");
        Printf("Timing: 5 MODE_1080p_60.\n");
        Printf("Timing: 6 MODE_1080I.\n");
		Printf("Timing: 7 1024*768@60.\n");
		Printf("Timing: 8 MODE_4096_2160P_24.\n");
        return 0;
    }
    
	bPath = ((UINT8)StrToInt(szArgv[1])==0)? SV_VP_MAIN : SV_VP_PIP;
    bTiming = (UINT8)StrToInt(szArgv[2]);
	
    vDrvSetPreProcTiming(bPath,(PRE_PROC_TIMING)bTiming);
    return 0;
}
static INT32 _i4YcprocGenTimingByCopy(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    vDrvSetOmuxTimToPreProc(bPath,bOnOff);
    return 0;
}
...
{
    Region rDummyRegion;
    UINT8 bDEMOModules=0;
    if ((i4Argc != 2)&&(i4Argc != 3))
    {
        Printf("Usage: DEMO <Right(1)/Left(2)/Still_Rec(3)/Moving_Rec(4)/OFF(other)> \n");
        Printf("            <ECTI(1<<7)|COLOR|TDSHARP|3DNR|2DNR|HSHARP|MDDI|TDC(1<<0)> \n");
        return 0;
    }

    bDEMOModules = (i4Argc == 3) ? (UINT8)StrToInt(szArgv[2]) : 0xF0;
        
    //support only Split demo.
    rDummyRegion.u1OnOff = SV_ON;
    rDummyRegion.wHEnd = 500;
    rDummyRegion.wHStart=0;
    rDummyRegion.wVEnd = 500;
    rDummyRegion.wVStart = 0;
    vApiVideoDemoMode((UINT8)StrToInt(szArgv[1]), rDummyRegion, bDEMOModules);
...
INT32 _i4YcprocSPBypass(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if (i4Argc != 2)
    {
        Printf("Usage: Sharpness <OFF(0)/ON(1)> \n");
        return 0;
    }

    u1OnOff = ((UINT8)StrToInt(szArgv[1]) == 0) ? SV_OFF : SV_ON;

    vDrvSharpnessBypass(u1OnOff);
...
static INT32 _i4YcprocPqVerify(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if (i4Argc < 2)
    {
        Printf("pqv <(0)PartID/(1)Header/(2)Gamma/(3)SCE/(4)Qty/(5)System Info> <id_0> <id_1>\n");
        return 0;
    }
    
    if (i4Argc > 2)     id_0 = StrToInt(szArgv[2]);
    if (i4Argc > 3)     id_1 = StrToInt(szArgv[3]);

	switch (StrToInt(szArgv[1]))
	{
	default:
        Printf("pqv <(0)PartID/(1)Header/(2)Gamma/(3)SCE/(4)Qty/(5)System Info/(6)Panel>\n");
	case 0:
	...
	case 1:
		...
		break;
	case 2:	
        if (i4Argc > 2) vApiFlashPqSetGammaId((UINT8)id_0);
		bApiFlashPqUpdateGamma();
		break;
	case 3:	
        if (i4Argc > 2) vApiFlashPqSetSceId((UINT8)id_0);
    ...
    case 6:
        ...
        break;    
    case 7:        
        if (i4Argc > 2)     vApiFlashPqSetRawDataTblId((UINT16)id_0); 
        if (i4Argc > 3)     vApiFlashPqSetRawDataCaseId((UINT16)id_1);
