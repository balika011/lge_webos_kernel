...
static INT32 _i4VgaDebug_LOG(INT32 i4Argc, const CHAR **szArgv);
...
static INT32 _i4VgaSetAutoSP0SwitchMux(INT32 i4Argc, const CHAR **szArgv);
...
CLI_EXEC_T arVgaCmdTbl[] = {
#ifdef CC_CLI
  {"reset", "r", _i4VgaReset, NULL, "reset(reset VGA FSM)", CLI_GUEST},
  {"status", "s", _i4VgaStatus, NULL, "status(report VGA status)", CLI_GUEST},
  {"auto", "a", _i4VgaAuto, NULL, "auto(now),auto [0,1](set default)", CLI_GUEST},
  ...
  {"clock", "clk", _i4VgaClock, NULL, "clk ,clk [xx]", CLI_GUEST},
  {"usermode", "um", _i4VgaUserMode, NULL, "um ", CLI_SUPERVISOR},
  {"position", "pos", _i4VgaPos, NULL, "pos,pos [xx]", CLI_GUEST},
  {"phasesel", "ps", _i4VgaPhaseSel, NULL, "ps [0~31] (set input phase+fixup)", CLI_GUEST},
  {"phase_plus", "ps+", _i4VgaPhaseSel, NULL, "phase +=1  (max 31)", CLI_GUEST},
  {"phase_minus", "ps-", _i4VgaPhaseSel, NULL, "phase -=1 (min 0) ", CLI_GUEST},
  ...
  {"debug", "dbg", _i4VgaDebug, NULL, "Debug [1:MDET, 2:MDCHG 4:AUTO]", CLI_GUEST},
  {"debug_LOG", "dbg_log", _i4VgaDebug_LOG, NULL, "Debug [0:print all]", CLI_SUPERVISOR},
  {"setpll", "setpll", _i4VgaSetAdcPll, NULL, "pixelclk(/10 MHz)  htotal ", CLI_SUPERVISOR},
  {"adslicer", "adslicer", _i4VgaSetAdSlicer, NULL, "0/1 ", CLI_GUEST},
  {"automux", "automux", _i4VgaSetAutoSP0SwitchMux, NULL, "0/1 ", CLI_SUPERVISOR},
...
static INT32 _i4VgaAutoCLR(INT32 i4Argc, const CHAR **szArgv)
{
	LOG(0,"doing auto color...must use full Black+White,Full Color pattern\n");
	_bAutoColorState0=1;
    return 0;
}
...
static INT32 _i4VgaSDDSLoopFilterID(INT32 i4Argc, const CHAR **szArgv)   
{
   ...
   if(i4Argc==1)
   {
      if (x_strcmp(szArgv[0], "sddsid") == 0)
      {
         ...
         KI0_start = 2;
         KI0_end = 0x0a;
        LOG(0,"KP0_start =%d, KP0_end =%d,KI0_start =%d,KI0_end =%d\n",KP0_start,KP0_end,KI0_start,KI0_end);
   ...
   else
   {
      ...
      {
         LOG(0,"Input parameter not completely\n");
      }
   }
    return 0;
}                       
...
static INT32 _i4VgaDDDSLoopFilterID(INT32 i4Argc, const CHAR **szArgv)   
{
   ...
   if(i4Argc==1)
   {
      if (x_strcmp(szArgv[0], "sddsid") == 0)
      {
         ...
         KI0_end = 0x0a;
         ERROR_LIM =0xff;
        LOG(0,"DKP0_start =%d, DKP0_end =%d,DKI0_start =%d,DKI0_end =%d\n",KP0_start,KP0_end,KI0_start,KI0_end);
      #if defined(CC_MT8223)
      #else
   ...
   else
   {
      if(i4Argc==7)
      {
        ...
        LOG(0,"DKP0_start =%d, DKP0_end =%d,DKI0_start =%d,DKI0_end =%d, ERROR_LIM=%d, CW=%x\n",KP0_start,KP0_end,KI0_start,KI0_end,ERROR_LIM,CW);
      #if defined(CC_MT8223)
      #else
	  vDrvEnable_DDDS_Calibration(KP0_start,KP0_end,KI0_start,KI0_end,ERROR_LIM,CW);
      #endif 
...
static INT32 _i4VgaSyncSlicerRefVoltageVerify(INT32 i4Argc, const CHAR **szArgv)   
{
	LOG(0,"SyncSlicer Ref Voltage Verify\n");
...
//			vDrvVGASetPhase_Simple(StrToInt(szArgv[1])&0x1f);
	...
	else
	{
		if (x_strcmp(szArgv[0], "ps+") == 0)
			...
			LOG(0,"current phase [%d], may set [0~31]\n",
				bVgaCurPhase());
	}
    return 0;
}
...
static INT32 _i4VgaSetAdSlicer(INT32 i4Argc, const CHAR **szArgv)
{
	if(i4Argc==2)
        ...
        {
            LOG(0,"adslicer  0/1 \n");
            LOG(0,"Current Slicer setting :%d\n", bHdtvOpt05_AdaptiveSlicer);
            vQuerySlicer();
        }
        return 0;
}
	...
	if(i4Argc==2)
        {
                if(StrToInt(szArgv[1]) == 1)
                {
                	bVgaOpt08_AutoSP0SwitchMux = 1 ;
                }
                else if(StrToInt(szArgv[1]) == 0)
                {
                	bVgaOpt08_AutoSP0SwitchMux = 0;
                }
                else
                {
                	LOG(0,"automux  0/1 \n");
					LOG(0,"Current AutoSP0SwitchMux setting :%d\n", bVgaOpt08_AutoSP0SwitchMux);
                }
        }
        else
        {
...
static INT32 _i4VgaBoundSel(INT32 i4Argc, const CHAR **szArgv)
{
	...
	{
		LOG(0,"current bdthrsh [%d], may set [0~255]\n",
			_bBdThrsh);
	}
    return 0;
}
...
//extern UINT8   VGA_Debug;
static INT32 _i4VgaDebug_LOG(INT32 i4Argc, const CHAR **szArgv)
{

	if(i4Argc==2)
	{
//        VGA_Debug=StrToInt(szArgv[1]);
	}
	else
	{
		//LOG(0,"VGA_Debug  %d\n",VGA_Debug);
...
static INT32 _i4VgaAmbiguousQuery(INT32 i4Argc, const CHAR **szArgv)
{
    ...
    if(bCandNum <= 1)
    {
        LOG(0,"Input is not ambiguous timing\n");
    }
    else
    {
...
static INT32 _i4EmbeddedGain(INT32 i4Argc, const CHAR **szArgv)
{
	...
	LOG(0,"The Embedded Gain1=0x%x, Gain2=0x%x, Gain3=0x%x\n",_bEmbeddedGain[0],_bEmbeddedGain[1],_bEmbeddedGain[2]);
...
static INT32 _i4EmbeddedOverRule(INT32 i4Argc, const CHAR **szArgv)
{
	...
	if(i4Argc==2)
	{
      if(x_strcmp(szArgv[1],"on")==0)//on->overrule
      {
	   ...
	   LOG(0,"Use Traditional auto color gain\n");
      }
      else if(x_strcmp(szArgv[1],"off")==0)
      {
	   UNUSED(fgApiEepromWriteByte(EEP_ADC_AUTOCOLOR_OVER_RULE_EFUSE, 0xFF));
	   LOG(0,"Use Embedded E Fuse gain\n");
      }
      else
      {
	    LOG(0,"Please add parameter on/off\n");
      }
    }
    else
    {
	...
	if(i4Argc==2)
        {
            if(StrToInt(szArgv[1]) == 0)
            {
               ...
               {
                 bTmpLowByte= bApiEepromReadByte((EEP_ADC_DIG_GAIN_HDTV0_LOW_BYTE + (2*bOrder)));
                bTmpHighByte= bApiEepromReadByte((EEP_ADC_DIG_GAIN_HDTV0_HIGH_BYTE + (2*bOrder)));
                bGain[bOrder] =  (((UINT16)bTmpHighByte<<8)&0xFF00)|(UINT16) bTmpLowByte;
                bTmpLowByte= bApiEepromReadByte(EEP_ADC_OFFSET_HDTV_CLAMP0+bOrder);
                bTmpHighByte= bApiEepromReadByte(EEP_ADC_OFFSET_HDTV_CLAMP0_HIGH_BYTE+bOrder);
                bOffset[bOrder]=  (((UINT16)bTmpHighByte<<8)&0xFF00)|(UINT16) bTmpLowByte;
                
               }
               if(bStatus==1)
               	{
               	   LOG(0,"YPbPr AC done! Gain1=%2x, Gain2=%2x, Gain3=%2x, bOffset1=%2x, bOffset2=%2x, bOffset3=%2x\n",bGain[0],bGain[1],bGain[2],bOffset[0],bOffset[1],bOffset[2]);
               	}
               else
               	{  
               	   LOG(0,"Not doing YPbPr autocolor yet\n");
               	}
            ...
            else if(StrToInt(szArgv[1]) == 1)
            {
               ...
               for(bOrder=0;bOrder<3;bOrder++)
               {
                 bTmpLowByte= bApiEepromReadByte((EEP_ADC_DIG_GAIN_VGA0_LOW_BYTE + (2*bOrder)));
                bTmpHighByte= bApiEepromReadByte((EEP_ADC_DIG_GAIN_VGA0_HIGH_BYTE + (2*bOrder)));
                bGain[bOrder] =  (((UINT16)bTmpHighByte<<8)&0xFF00)|(UINT16) bTmpLowByte;
                bTmpLowByte= bApiEepromReadByte(EEP_ADC_OFFSET_VGA_CLAMP0+bOrder);
                bTmpHighByte= bApiEepromReadByte(EEP_ADC_OFFSET_VGA_CLAMP0_HIGH_BYTE+bOrder);
                bOffset[bOrder]=  (((UINT16)bTmpHighByte<<8)&0xFF00)|(UINT16) bTmpLowByte;
               }
               if(bStatus==1)
               	{
               	   LOG(0,"VGA AC done! Gain1=%2x, Gain2=%2x, Gain3=%2x, bOffset1=%2x, bOffset2=%2x, bOffset3=%2x\n",bGain[0],bGain[1],bGain[2],bOffset[0],bOffset[1],bOffset[2]);
               	}
               else
               	{  
               	   LOG(0,"Not doing VGA autocolor yet\n");
               	}
            }
            else
            {
               ...
               if(bStatus==1)
               	{
               	   LOG(0,"SCART AC done! Gain1=%2x, Gain2=%2x, Gain3=%2x, bOffset1=%2x, bOffset2=%2x, bOffset3=%2x\n",bGain[0],bGain[1],bGain[2],bOffset[0],bOffset[1],bOffset[2]);
               	}
               else
               	{  
               	   LOG(0,"Not doing SCART autocolor yet\n");
               	}
            }
        }
        else
