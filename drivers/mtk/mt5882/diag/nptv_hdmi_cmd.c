...
static INT32 _u1HDMIEDIDCTRL(INT32 i4Argc, const CHAR **szArgv);
...
CLI_EXEC_T arHDMICmdTbl[] = {
...
#ifdef __MODEL_slt__
  {"CRC1", "cc", _u1vHDMICRCMulti, NULL, "HDMI CRC1 check", CLI_GUEST},
#endif
  ...
  {"SPD Info Check","spd",_u1HDMISPDINFOCHECK,NULL,"SPD Info Check",CLI_SUPERVISOR},
  {"SPD Info Set","spds",_u1HDMISPDINFOSET,NULL,"SPD Info Set",CLI_SUPERVISOR},
  #ifdef CC_LGE_PROTO_PCBA
  {"Internal EDID on/off","edidctrl",_u1HDMIEDIDCTRL,NULL,"Internal EDID on/off",CLI_SUPERVISOR},
  #endif
  //#endif
...
static INT32 _u1HDMIEDIDCTRL(INT32 i4Argc, const CHAR **szArgv)
{
	if (i4Argc != 3)
	{
			Printf("Usage: %s port_number on_off\n", szArgv[0]);
			return 0;
	}
	vHDMI_EdidOnOffCtrl((UINT8)StrToInt(szArgv[1]), (UINT8)StrToInt(szArgv[2]));
	Printf("Set port %d EDID control is %d\n", (UINT8)StrToInt(szArgv[1]),(UINT8)StrToInt(szArgv[2]));
    return 0;
