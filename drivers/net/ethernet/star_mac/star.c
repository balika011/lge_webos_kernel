...
static u8 macAddress[ETH_ADDR_LEN] = DEFAULT_MAC_ADDRESS;

static int __init macadr_setup(char *adr)
{
	char macstr[MAC_CHAR_LEN];
	int i;
	printk("enter in  macadr_setup , data is  %s \n",adr);
	strncpy(macstr,adr,MAC_CHAR_LEN);

	macAddress[0] = _toDigit(&macstr[0]);
	macAddress[1] = _toDigit(&macstr[3]);
	macAddress[2] = _toDigit(&macstr[6]);
	macAddress[3] = _toDigit(&macstr[9]);
	macAddress[4] = _toDigit(&macstr[12]);
	macAddress[5] = _toDigit(&macstr[15]);

	for(i=0;i<ETH_ADDR_LEN;i++)
		printk("macAddress[%d] = 0x%x \n",i,macAddress[i]);

    return 1;
}
__setup("macadr=", macadr_setup);
#endif
...
#if defined(CONFIG_LG_KERNEL_EXPAND)
	memcpy(macAddr,macAddress,ETH_ADDR_LEN);
#else
